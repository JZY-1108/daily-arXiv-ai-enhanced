...
Motivation: 预训练多语言模型与英语模型一样存在社会偏见问题，需要系统性地将偏见评估和减缓方法扩展到多语言和非英语语境中。

Method: 通过系统性综述分析，考察了现有研究在语言多样性、文化意识、评估指标和减缓技术选择方面的特点。

Result: 发现当前领域在方法论设计上存在空白（如偏好某些语言、多语言减缓实验穷缺），同时记录了在不同语言和文化中适配偏见测试标准时遇到的常见问题和解决方案。

Conclusion: 研究指出了未来研究方向，以加强多语言偏见研究的包容性、跨文化适度性以及与最新NLP进展的对齐。
...
<details>
  <summary>Details</summary>
Motivation: 降低人工开发多选题(MCQs)的成本和不一致性，为K-12教育提供可扩展的语言评估项目开发方案。

Method: 采用两阶段方法：1)比较微调的中等模型(Gemma 2B)与未调优的大模型(GPT-3.5)；2)评估7种结构化提示策略，包括零样本、少样本、思维链、基于角色、顺序设计等组合。使用自动化指标和专家评分进行评估，并用GPT-4.1模拟大规模人工评分。

Result: 结构化提示策略(特别是思维链与顺序设计结合)显著提升了Gemma的输出质量。Gemma生成的题目比GPT-3.5的零样本响应更符合构念对齐和教学适宜性，提示设计在中等模型性能中起关键作用。

Conclusion: 结构化提示和高效微调可以在有限数据条件下增强中等模型的自动生成能力，结合自动化指标、专家判断和大模型模拟能确保与评估目标的一致性，为K-12语言评估项目开发提供了实用且可扩展的工作流程。
...
Method: 开发了完全开源的方法论，将SystemC TLM组件封装为FMI 3.0协同仿真功能模型单元(FMU)，解决了时间同步和数据交换等关键技术挑战，并提供了轻量级开源工具链。
...
<details>
  <summary>Details</summary>
Motivation: 小型语言模型（如0.5B参数）由于推理能力差，在强化学习中面临稀疏奖励和不稳定训练的困难，需要一种方法来使它们在计算资源受限的环境中实现智能RAG行为。

Method: 提出Distillation-Guided Policy Optimization (DGPO)方法，包括从教师演示进行冷启动初始化，以及在策略优化过程中提供持续的教师指导。

Result: DGPO使紧凑模型能够实现复杂的智能搜索行为，在某些情况下甚至超越了更大的教师模型。引入了ARC细粒度评估指标来分析推理、搜索协调和响应合成能力。

Conclusion: DGPO使得在计算资源受限的环境中实现智能RAG成为可能，为小型语言模型的智能行为训练提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在各领域的广泛应用，其生成有害内容的风险引发了社会关注。政府发布的伦理指南缺乏可操作的具体测试问题，需要一种系统化的测试方法来验证LLM的合规性。

Method: GUARD方法自动生成基于政府指南的违规问题来测试模型响应，对于不直接违规的响应，通过GUARD-JD整合越狱诊断技术，创建场景来诱发不道德或违规响应。

Result: 在7个LLM（包括Vicuna、Llama、GPT和Claude系列）上实证验证，测试了三个政府指南的合规性并进行了越狱诊断。GUARD-JD还可扩展到视觉语言模型。

Conclusion: GUARD提供了一种系统化的测试框架，能够有效评估LLM对政府伦理指南的遵守程度，并识别潜在的安全漏洞，有助于促进可靠的LLM应用开发。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理长上下文时面临内存限制、复杂任务处理困难、缺乏透明度和容易产生幻觉等问题，需要新的解决方案来提升长上下文理解能力。

Method: JERR框架包含三个关键组件：1）策略性分块提取摘要，提高信息理解效率；2）构建有向无环图（DAG）解决冗余问题，确保逻辑一致性；3）集成蒙特卡洛树搜索（MCTS）来导航复杂推理路径，确保输出准确性和可解释性。

Result: 实验结果显示，JERR在ROUGE和F1指标上 consistently outperforms所有基线方法，在LLM-Rater评估中获得了最高分数。

Conclusion: JERR框架为LLM处理扩展上下文和复杂推理任务提供了新颖解决方案，显著提高了可靠性和透明度。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统长思维链推理能力开发依赖人工精准调整数据集的高成本问题，寻找可扩展的替代方案。

Method: 使用NP难图问题作为合成训练资源，构建两阶段后训练框架：长思维链有监督精细调整和加强学习以提升推理深度和效率。

Result: Graph-R1-7B模型在数学、编程、STEM和逻辑领域都显示出强大的沿用性，并在NP难图问题上在准确性和推理效率方面超过了QwQ-32B模型。

Conclusion: NP难图问题作为一种有效且可扩展的资源，为提升大语言模型的长思维链推理能力开启了新的前沿。
...
<details>
  <summary>Details</summary>
Motivation: 现有心理测量测试对LLMs采用无上下文的方法，忽略了真实对话中历史交互对回答的影响，需要开发能够考虑对话历史的评估框架。

Method: 提出CAPE框架，引入新指标量化LLM响应一致性，在7个LLM上进行实验，分析对话历史对响应一致性和人格特质的影响。

Result: 对话历史通过上下文学习增强响应一致性，但也会导致人格偏移；GPT模型对问题顺序稳健，而Gemini和Llama对先前交互高度敏感；在角色扮演代理中，上下文依赖的人格偏移提高了响应一致性和人类判断对齐度。

Conclusion: 上下文感知评估揭示了LLMs行为特质的重要方面，对话历史既增强一致性又引起人格变化，不同模型对上下文的依赖程度存在显著差异。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型依赖中间推理步骤提高准确性，但缺乏对推理步骤效用如何影响最终答案正确性的研究。由于自回归生成的随机性，生成更多上下文不能保证答案置信度提升。
...
Result: 发现清晰的模式：随着步骤增加而递减的条件熵与正确答案强相关，而平坦或递增的熵往往导致错误答案。错误推理路径通常比正确路径更长。

Conclusion: 这些发现为设计高效推理管道奠定了基础，可以早期检测和避免无效推理，从而优化大语言模型的推理过程。
...
Method: 采用专家两两比较方法，涵盖10个工具、30个提示词、300个生成网站，收集4000+专家判断，使用TrueSkill模型进行排名并计算置信区间。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型和医疗LLMs在通用医疗基准上表现良好，但在需要深度专业知识的牙科等专业领域缺乏针对性评估资源，能力尚未充分探索。

Method: 构建DentalBench双语基准：1）DentalQA问答基准（36,597个问题，4任务16子领域）；2）DentalCorpus语料库（3.37亿token）。评估14个LLMs，并进行领域适应实验。

Result: 评估显示不同模型在任务类型和语言上存在显著性能差距。领域适应（特别是Qwen-2.5-3B）大幅提升模型表现，尤其在知识密集和术语密集型任务上。

Conclusion: 领域特定基准对于开发可信有效的医疗应用LLMs至关重要，领域适应能显著提升专业医疗领域的模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的检索增强生成系统主要解决语料库级别的上下文丢失问题，但缺乏对复杂查询的语义丰富表示。知识图谱与LLMs的结合有潜力改善检索阶段的查询表示质量

Method: 提出KG-CQR框架，包含子图提取、子图补全和上下文生成三个模块。通过语料中心的知识图谱提取结构化关系表示，生成语义丰富的查询上下文，是一个模型无关的流水线

Result: 在RAGBench和MultiHop-RAG数据集上表现优异，mAP提升4-6%，Recall@25提升2-3%。在多跳问答等挑战性任务中，检索效果持续优于现有基线
...
Method: 设计并开发了工业级的民航维修基准测试，利用该基准评估现有的向量嵌入模型和LLMs在民航维修场景中的表现，特别关注RAG系统的评估。

Result: 通过实验探索和分析，证明了该基准在评估模型性能方面的有效性，并将评估基准和代码开源以促进进一步研究。

Conclusion: 该工作填补了LLM评估在专业垂直领域的重要空白，为民航维修领域的智能化解决方案提供了标准化评估工具和改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个基于经验案例的搜索系统，帮助用户通过标题或关键词查找相似的实践工作标题

Method: 采用案例推理(CBR)技术，使用TF-IDF进行文本向量化，余弦相似度计算相似性值

Result: 在705个实践工作标题的测试中，系统在两个测试阶段都成功找到相同数量的标题，并获得最高平均匹配分数

Conclusion: 基于TF-IDF和余弦相似度的CBR系统能够有效搜索实践工作标题，为类似案例检索问题提供了可行解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有API基准测试主要依赖显式工具规范、浅层少步工作流和孤立领域操作，无法充分评估LLM在真实复杂任务中的工具使用、跨工具协调、参数控制和规划推理能力。

Method: 基于Model Context Protocol构建，连接28个代表性MCP服务器，涵盖金融、旅行、科学计算、学术搜索等领域的250个工具，设计真实多步任务，提出多维度评估框架（工具级模式理解、轨迹级规划、任务完成度）。

Result: 对20个先进LLM的实验显示，在MCP-Bench上仍存在持续挑战，模型在模糊指令下的工具检索、多步执行规划和跨域工作流协调方面表现不佳。

Conclusion: MCP-Bench填补了现有基准测试的空白，为评估LLM在真实复杂任务中的工具使用能力提供了更全面的测试平台，揭示了当前模型的局限性，推动了相关研究发展。
...
Method: 使用两个真实世界EHR数据集，开发并评估基于自然语言处理技术的深度学习框架。进行了消融研究，评估医疗提示、自由文本和预训练句子编码器三个关键组件的作用，并测试模型对结构化EHR数据损坏的鲁棒性。

Result: 在三个临床任务上，提出的模型相比最佳现有方法：死亡率预测BACC/AUROC提升1.6%/0.8%，住院时间预测RMSE/MAE提升0.5%/2.2%，手术时间估计RMSE/MAE提升10.9%/11.0%。在不同损坏率下均表现优于其他基线方法。

Conclusion: 该框架是预测重症监护死亡率和资源利用的有效准确深度学习方法。研究证明了使用提示学习和transformer编码器分析多模态EHR的成功，模型对结构化数据损坏具有强大韧性，特别是在高损坏水平下。
...
<details>
  <summary>Details</summary>
Motivation: 阴谋论侵蚀公众对科学和机构的信任，且随着AI生成虚假信息日益复杂，理解阴谋论内容的修辞模式对于开发干预措施（如针对性预辟谣）和评估AI漏洞至关重要。

Method: 引入ConspirED数据集，包含从在线阴谋论文章中提取的多句子摘录（80-120词），使用CONSPIR认知框架进行标注。基于该数据集开发计算模型识别阴谋论特征，并评估大语言模型对阴谋论输入的鲁棒性。

Result: 研究发现大语言模型和推理模型都被阴谋论内容误导，产生的输出会反映输入的推理模式，即使在成功抵制类似经过事实核查的虚假信息时也是如此。

Conclusion: 该研究强调了需要开发更强大的AI系统来抵御阴谋论内容的影响，ConspirED数据集为理解和对抗阴谋论传播提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 评估广泛使用的FLORES+多语言机器翻译基准测试的真实适用性和质量，发现其在实际多语言评估中存在严重缺陷

Method: 对4种语言（Asante Twi、日语、Jinghpaw、南阿塞拜疆语）进行人工评估，分析翻译质量，测试简单启发式方法的效果，比较在高质量自然数据上训练的模型在FLORES+和领域相关评估集上的表现

Result: 发现许多翻译质量低于90%标准，源句子过于领域特定且存在英语世界文化偏见，简单复制命名实体就能获得不错的BLEU分数，高质量模型在FLORES+上表现差但在领域相关评估集上表现好
...
Result: 在三个真实世界科学文献数据集上的大量实验表明，SciTopic在科学主题发现方面优于最先进的方法，使研究人员能够获得更深入、更快速的洞察
...
Method: 设立四个共享任务：传统任务b和Synergy，新任务MultiCardioNER（多语言心脏病学领域临床实体检测）和BIONNE（俄英双语嵌套命名实体识别）

Result: 37个参赛团队共提交700多次，大多数系统达到竞争性性能水平

Conclusion: BioASQ挑战赛持续推动生物医学自然语言处理领域的技术进步，参与者表现优异，表明该领域state-of-the-art技术不断发展
...
Method: 设立六个共享任务：两个传统任务（b和Synergy）和四个新任务（多语言临床摘要、俄英嵌套命名实体链接、心脏病学临床编码、肠脑相互作用信息提取），邀请研究团队参与竞赛。

Result: 83个竞争团队参与，提交超过1000份不同的解决方案，多个系统表现出具有竞争力的性能。

Conclusion: BioASQ挑战赛持续推动生物医学信息处理技术的进步，参赛系统的表现表明该领域的技术水平在不断提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有的联邦蒸馏研究主要关注标签(output)多样性，而忽略了语言领域(input)多样性这一自然语言处理中的关键因素。现实环境中本地客户端数据高度异构，无法捕捉全局数据分布。

Method: 提出了自适应联邦蒸馏(AdaFD)框架，设计了一套全面的多领域非IID场景，并建立了包含多样化数据的统一基准测试框架，适用于同构和异构设置。

Result: 实验结果表明，该模型能够捕捉本地客户端的多样性，相比现有工作取得了更好的性能表现。

Conclusion: 该研究为解决真实环境中多领域非IID数据挑战提供了有效的解决方案，代码已开源供社区使用。
...
<details>
  <summary>Details</summary>
Motivation: 传统抽取式摘要模型存在多阶段流水线信息损失和语义理解不足的问题，无法有效处理复杂搜索意图，需要新的生成式方法提升工业级查询驱动摘要性能。

Method: 整合大模型蒸馏、监督微调、直接偏好优化和前瞻解码技术，构建仅0.1B参数的轻量级生成模型，专门针对查询驱动文本摘要任务进行优化。

Result: 在多个工业相关指标上超越生产基线，达到新的state-of-the-art，部署效率优异：仅需334块NVIDIA L20 GPU即可处理约50,000 QPS，平均延迟55ms。

Conclusion: 生成式模型在工业级查询驱动文本摘要任务中具有显著优势，通过精心设计的轻量化方案可以实现高性能实时处理，为大规模web搜索应用提供了有效解决方案。
...
Method: 提出知识组合采样(KCS)框架，将知识组合选择建模为句子级条件预测任务，使用概率对比损失预测相关知识，采用随机解码策略平衡准确性和多样性

Result: 知识组合选择准确率提升3.9%，在HotpotQA和2WikiMultihopQA数据集上通过数据增强获得性能改进

Conclusion: KCS有效提升了多跳问答的数据多样性和模型性能，为复杂推理任务提供了新的数据增强方法
...
<details>
  <summary>Details</summary>
Motivation: 当前图语言模型（GLMs）的评测基准主要重用节点分类数据集，无法有效评估多模态推理能力，需要更好的评测方法。

Method: 提出CLEGR（组合式语言-图表推理）基准，通过合成图表生成流程和需要结构与语义联合推理的问题，在不同复杂度层面评估多模态推理能力。

Result: 证明现有评测数据集仅使用单模态信息即可获得强性能，软提示LLM基线与包含完整GNN的GLM表现相似，GLM在结构化推理任务中性能显著下降。

Conclusion: 当前GLM在图表推理能力上存在显著局限性，CLEGR基准为推进多模态推理研究提供了基础，并对将图表结构集成到LLM中的安排必要性提出疑问。
...
<details>
  <summary>Details</summary>
Motivation: 端到端语音识别系统经常无法正确转录领域特定的命名实体，导致下游任务灾难性失败。现有基于音素编辑距离的命名实体校正方法在词形差异大时难以定位错误词汇

Method: 利用语音声音特征检索候选实体，创新性地设计生成式方法来标注ASR转录中的实体错误并用正确实体替换文本

Result: 在开源和自建测试集上验证，该方法能显著提升实体准确率，特别是在词形差异大的场景下有效

Conclusion: 该方法为解决ASR系统中命名实体识别错误提供了有效解决方案，特别是在传统方法失效的词形差异场景下表现优异
...
<details>
  <summary>Details</summary>
Motivation: 解决隐式篇章关系识别中的多语言和多标签分类问题，利用层次依赖关系提升识别性能，并与大型语言模型进行性能对比。

Method: 提出HArch模型，利用预训练编码器骨干（RoBERTa和XLM-RoBERTa），通过层次依赖关系预测PDTB 3.0框架中的三个意义层次的概率分布，并进行任务特定微调。

Result: RoBERTa-HArch在英语中表现最佳，XLM-RoBERTa-HArch在多语言设置中表现最佳；微调模型在所有语言配置中均优于GPT-4o和Llama-4-Maverick；在DiscoGeM 1.0语料库上取得SOTA结果。

Conclusion: 任务特定微调在隐式篇章关系识别中优于提示方法，层次方法有效，HArch模型在多语言和多标签分类中表现卓越。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然提升了文本生成能力，但也带来了分词不一致性问题，这会破坏隐写术和水印技术的鲁棒性。研究者发现导致该问题的词汇具有低频性和临时性特征，需要针对性解决方案。

Method: 提出了两种定制化解决方案：针对隐写术的逐步验证方法，以及针对水印技术的后验回滚方法。这些方法专门处理分词不一致性问题。

Result: 实验结果显示：(1) 与传统消歧方法相比，直接处理分词不一致性显著提升了隐写术的流畅性、不可感知性和抗隐写分析能力；(2) 对于水印技术，处理分词不一致性增强了可检测性和对抗攻击的鲁棒性。

Conclusion: 该研究通过识别和解决分词不一致性问题，有效提升了基于大型语言模型的文本隐写术和水印技术的性能，为防范恶意滥用提供了更可靠的保障措施。
...
<details>
  <summary>Details</summary>
Motivation: 当前的长链思维(CoT)方法存在局限性，需要开发能够自主探索、验证和优化复杂问题解决中间步骤的智能体模型，同时降低计算成本。

Method: 采用三个关键创新：(1)高效的RL基础设施和可靠的Python代码环境；(2)GRPO-RoC算法，通过重采样策略处理代码工具的环境噪声；(3)从非推理SFT开始的多阶段RL训练配方。

Result: 在仅510个RL步骤和一周训练时间内，将预训练的14B模型提升到最先进水平，在数学推理任务上表现优异，同时在对齐、科学推理和工具使用任务上展现出强泛化能力。

Conclusion: rStar2-Agent证明了智能体强化学习可以在有限计算资源下有效训练大规模模型，实现先进的认知行为，为复杂问题解决提供了新的方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决本地差分隐私下文本隐私保护的挑战，传统方法在低ε值时效果不佳，需要新的方法来平衡隐私保护和文本质量

Method: DP-ST方法，基于语义三元组进行邻域感知的私有文档生成，采用分治策略，限制隐私保护到特定的邻域概念，并结合LLM进行后处理

Result: 该方法在较低ε值下仍能生成连贯的文本，有效平衡了隐私保护和实用性，证明了分治策略的有效性

Conclusion: 语义连贯性对于在合理ε水平下实现平衡的隐私保护输出至关重要，DP-ST方法为解决本地差分隐私下的文本保护问题提供了有效方案
...
<details>
  <summary>Details</summary>
Motivation: 隐式仇恨言论（IHS）使用间接语言、讽刺或编码术语表达偏见和仇恨，难以检测。传统方法需要依赖外部知识或额外信息，本文探索仅通过微调通用嵌入模型来解决这一挑战

Method: 仅对基于大语言模型的通用嵌入模型（包括Stella、Jasper、NV-Embed和E5）进行微调，不依赖外部知识或额外上下文信息

Result: 在多个IHS数据集上实验显示：数据集内评估F1-macro分数提升最高1.10个百分点，跨数据集评估提升高达20.35个百分点

Conclusion: 微调通用嵌入模型是检测隐式仇恨言论的有效方法，无需复杂的外部知识或额外信息，在跨数据集泛化方面表现尤为突出
...
Method: 提出GUARD自适应性解码方法，结合全局熵估计和局部熵偏差，整合长期和短期不确定性信号，并加入基于token计数的惩罚机制来降低计算开销。

Result: 实验结果表明GUARD在文本多样性和连贯性之间实现了良好平衡，生成速度有显著提升，人类和LLM评估者均验证了其优异性能。

Conclusion: GUARD提供了一种有效且高效的文本生成解码策略，通过理论保证的无偏性和一致性，为开放式文本生成提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型生成的心理治疗对话在心理健康NLP中越来越广泛地用于模拟咨询场景、训练模型和补充真实数据，需要评估这些合成对话是否能捕捉真实治疗中的微妙情感动态。

Method: 采用Utterance Emotion Dynamics框架分析情感轨迹，比较真实认知行为治疗对话和LLM生成对话在效价、唤醒度和支配度三个维度上的差异，分析范围包括完整对话和个体说话者角色。

Result: 合成对话在情感特性上与真实对话存在分歧：真实会话表现出更大的情感变异性、更多情感丰富的语言以及更真实的反应和调节模式。真实与合成说话者之间的情感弧相似度较低，特别是对客户而言。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全机制可以通过消除特定表示方向来绕过，需要一种方法来增强而非削弱模型的安全对齐能力。

Method: Rank-One Safety Injection (ROSI)：计算安全方向，对所有残差流写入矩阵应用秩一权重修改，使用少量有害/无害指令对即可实现。

Result: ROSI持续提高安全拒绝率（Llama Guard 3评估），同时在MMLU、HellaSwag、Arc等基准测试中保持模型实用性，还能重新对齐'无审查'模型。

Conclusion: 定向、可解释的权重转向是改善LLM安全性的廉价而有效机制，可作为资源密集型微调范式的补充。
...
<details>
  <summary>Details</summary>
Motivation: 青少年心理健康问题日益严重，需要自动检测数字文本中的早期心理困扰迹象，特别是识别会加剧心理困扰的非理性思维模式（认知扭曲）

Method: 使用跨语言和跨语域分析方法，研究荷兰青少年论坛帖子，比较不同领域自适应方法的效果

Result: 语言和写作风格的改变会显著影响模型性能，但领域自适应方法显示出最大的应用前景

Conclusion: 跨语言和跨语域的认知扭曲检测具有挑战性，但通过适当的领域自适应技术可以实现有效的泛化
...
Method: 使用XGBoost、transformer架构和大语言模型(LLMs)处理音频、视频和文本特征，进行多模态抑郁症检测的性能比较研究。

Result: 研究结果揭示了各类模型在不同模态上捕捉抑郁相关信号的优势和局限性。

Conclusion: 为心理健康预测提供了有效的多模态表示策略见解，有助于选择适合不同模态特征的抑郁症检测模型。
...
<details>
  <summary>Details</summary>
Motivation: 小语言模型(SLMs)在类别不平衡数据集中处理少数类关系能力有限，大语言模型(LLMs)使用手动设计的提示可能引入噪声，干扰对事件间长距离依赖关系的判断。

Method: 1) 使用图注意力网络(GAT)构建距离感知图结构辅助LLMs捕获长距离依赖特征；2) 基于软推理的时序特征学习范式，将LLMs生成的概率信息补充到多头注意力机制中，增强短距离邻近带的关系识别。

Result: 在TB-Dense和MATRES两个公开数据集上实现了最先进的性能，显著提升了少数关系类的性能表现和整体学习能力。

Conclusion: GDLLM框架通过有效捕获全局特征，能够显著增强少数关系类的性能，并提高整体学习能力，在事件时序关系抽取任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有检索增强系统通常在单源或短答案场景下评估，但现实应用需要整合和总结分散在多源中的信息，没有单一源足以回答用户问题。

Method: 构建了一个可扩展的框架来创建评估基准，建立了MSRS-Story（叙事合成）和MSRS-Meet（摘要）两个新基准，使用稀疏和密集检索器结合前沿LLM进行广泛实验。

Result: 生成质量高度依赖检索效果，检索效果因任务而异；即使在oracle检索设置下，多源合成也具有挑战性；推理模型在合成步骤上显著优于标准LLM。

Conclusion: 多源检索和合成是RAG系统的重要挑战，需要检索组件识别多种相关性信号，生成组件连接和合成跨多源信息，推理模型在此任务上表现更优。
...
<details>
  <summary>Details</summary>
Motivation: 量化对于在资源受限硬件上部署大语言模型至关重要，但其在多语言任务中的影响尚未得到充分探索。研究者希望了解量化对不同语言资源水平和模型规模的影响，为多语言LLM部署提供实用指导。

Method: 使用5个参数量从1.7B到70B的大语言模型，在55种语言上进行机器翻译任务的后训练量化评估。比较了四种量化技术（AWQ、BitsAndBytes、GGUF和AutoRound），并分析了量化与解码超参数、校准语言的交互作用。

Result: 4位量化通常能保持高资源语言和大模型的翻译质量，但低资源和类型多样语言在2位量化时出现显著质量下降。GGUF变体在2位精度下提供最一致的性能。语言匹配的校准主要在低比特场景中提供优势。

Conclusion: 研究结果为在量化约束下部署多语言LLM进行机器翻译提供了可行的见解，特别是在低资源设置中。算法选择和模型大小共同决定了量化的鲁棒性，GGUF方法和语言匹配校准策略在低比特场景中表现最佳。
...
TL;DR: SageLM是一个端到端、多维度、可解释的语音大语言模型，用于全面评估语音到语音的对话系统，在语义和声学维度上联合评估，并通过两阶段训练在人类评估一致性上达到82.79%
...
Method: 提出SageLM模型，联合评估语义和声学维度；使用基于原理的监督增强可解释性；构建SpeechFeedback合成偏好数据集；采用两阶段训练范式

Result: SageLM与人类评估者的一致性达到82.79%，分别比级联方法和SLM基线高出至少7.42%和26.20%

Conclusion: SageLM为语音到语音大语言模型提供了有效的端到端评估框架，在语义和声学维度上都表现出优越性能，解决了现有评估方法的局限性
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多轮对话环境中存在推理不一致、难以遵守领域特定政策、长对话中信息提取困难等问题，需要改进工具调用能力

Method: 提出IRMA框架，自动重构用户查询，增加相关领域规则和工具建议，帮助工具调用代理更好地聚焦关键信息
...
<details>
  <summary>Details</summary>
Motivation: 现有的ICL示例选择策略往往忽视结构对齐，导致在结构化预测任务（如语义解析）中性能不佳和泛化能力差。

Method: 两阶段方法：1）使用结构感知监督微调BERT检索器；2）添加模型无关的可插拔模块来增强隐藏表示中的句法信息

Result: 在四个基准测试和三个语义解析任务上的实验表明，该方法在多个最新LLM上始终优于现有基线

Conclusion: 该方法在效率、泛化性和性能之间实现了良好平衡，可以无缝集成到现有流程中
...
Conclusion: 研究了推理能力对主动行为的影响，为未来模型开发提供了重要启示。
...
Result: 在5个主流LLM上的实验表明，LETHE在分类和生成任务中优于8个先进防御基线，对8种后门攻击的攻击成功率降低高达98%，同时保持模型效用，且成本效益高、对自适应攻击鲁棒。
...
TL;DR: EASI-RAG是一个为企业设计的结构化敏捷方法，帮助中小企业在没有RAG经验的情况下快速部署检索增强生成系统，在一个月内成功实施并取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 解决中小企业在部署RAG系统时面临的资源有限和NLP专业知识缺乏的挑战，使RAG技术能够在工业环境中得到实际应用。

Method: 基于方法工程原理，设计了包含明确角色、活动和技术的EASI-RAG结构化方法，通过真实案例研究在环境测试实验室中验证。

Result: 系统在一个月内由无RAG经验的团队部署成功，用户采纳率高，答案准确，提高了底层数据的可靠性，并可根据用户反馈迭代改进。

Conclusion: EASI-RAG证明了在工业中小企业中部署RAG系统的可行性，未来需要在更多用例中推广并进一步与微调模型集成。
...
TL;DR: 本文提出使用胶囊网络中的动态路由进行句子关系抽取，在多个数据集上优于现有方法，但发现在大型数据集Wikidata上性能较低，主要原因是标签噪声和重表示能力不足。


<details>
  <summary>Details</summary>
Motivation: 句子关系抽取是NLP中的重要任务，现有方法在大型数据集上性能不佳，需要探索更好的表示学习方法。

Method: 采用胶囊网络中的动态路由机制进行句子关系抽取，通过动态路由优化特征表示。

Result: 在Tacred、Tacredrev、Retacred和Conll04数据集上达到state-of-the-art性能，但在Wikidata数据集上性能较低。发现标签噪声和重表示能力是影响性能的关键因素。

Conclusion: 除了标签噪声问题外，重表示能力是句子关系抽取中的重要挑战，提出的胶囊网络方法在重表示方面优于传统模型。
...
TL;DR: 结合大型语言模型与符号求解器的神经符号方法，用于自动化税务申报，在SARA数据集上表现出色且成本效益显著


<details>
  <summary>Details</summary>
Motivation: 传统税务申报复杂耗时且容易出错，现有大型语言模型无法满足高准确性和可审计性要求，需要开发更可靠的自动化税务辅助系统

Method: 将LLMs与符号求解器集成，将文本规则转换为形式逻辑程序，并采用智能检索的案例表示范例来提升性能

Result: 该方法在SARA数据集上显著提升性能，将部署成本降至远低于现实世界平均水平，证明其经济可行性

Conclusion: 神经符号架构有望通过提供可靠、经济实惠的税务援助来促进公平获取，展示了在复杂推理任务中的实际应用价值
...
TL;DR: CHAIR-DPO方法通过使用CHAIR指标构建偏好数据，并用DPO微调MLLMs，有效减少多模态大语言模型的幻觉问题


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型(MLLMs)存在幻觉问题，即生成与视觉输入不符的回答。现有方法需要复杂管道构建合成偏好数据，且依赖专有模型

Method: 利用CHAIR指标区分幻觉和非幻觉样本，构建偏好数据对，通过直接偏好优化(DPO)微调现成的MLLMs

Result: CHAIR-DPO在多个幻觉基准测试中显著减少了幻觉回答的数量，证明了基于CHAIR奖励微调的有效性

Conclusion: 该方法提供了一种简单有效的解决方案，无需复杂管道或专有模型，即可有效缓解MLLMs的幻觉问题
...
TL;DR: 该论文提出了一种基于Stable Diffusion V3的多模态图像伪造定位框架，通过将伪造残差作为显式模态融入潜在空间，实现了无需大量标注数据的高效伪造检测，性能比现有最佳方法提升高达12%。


<details>
  <summary>Details</summary>
Motivation: 现有的图像伪造定位方法严重依赖人工标注数据，难以跟上多模态大模型驱动的图像篡改技术发展速度。需要利用生成模型的强大感知能力来解决这一挑战。

Method: 利用Stable Diffusion V3的多模态架构，将图像伪造残差（通过高通滤波器提取的高频信号）作为显式模态，在潜在空间中进行融合训练，同时保留SD3提取的丰富语义信息。

Result: 在广泛使用的基准数据集上性能比当前最先进的图像伪造定位模型提升高达12%，在训练时未见过的真实文档伪造图像和自然场景伪造图像上也表现出强大性能。

Conclusion: 该方法成功将SD3的多模态生成和感知能力整合到图像取证框架中，实现了高效准确的伪造定位，为应对新兴图像篡改技术提供了有效解决方案。
...
TL;DR: 该论文探索结合多模态大语言模型(MLLMs)和定量属性使用来提高皮肤癌诊断AI模型的可解释性，通过微调MLLMs从图像预测病变属性值，并在SLICE-3D数据集上进行验证


<details>
  <summary>Details</summary>
Motivation: 虽然AI模型在皮肤疾病诊断方面表现出色，但模型预测的可解释性仍需大幅提升才能实际临床应用。MLLMs通过自然语言交互提供诊断推理，而定量属性(如病变面积)已被证明能高精度预测恶性病变，两者的结合有望提高可解释性

Method: 通过微调多模态大语言模型(MLLMs)，使其能够从皮肤病变图像中预测定量属性值(如病变面积等)，从而将MLLM的嵌入空间与这些临床相关属性进行关联。使用SLICE-3D数据集进行基于属性的图像检索案例研究来评估这种关联效果

Result: 研究提供了证据表明MLLM的嵌入空间可以通过微调来预测图像中的属性值，从而实现与定量属性的关联。通过属性特定的基于内容的图像检索案例验证了这种关联的有效性

Conclusion: 结合MLLMs和定量属性的方法为提高皮肤癌诊断AI模型的可解释性提供了有前景的途径，通过将模型预测基于临床相关概念来增强其可信度和实用性
...
<details>
  <summary>Details</summary>
Motivation: 音频驱动的人体动画存在高分辨率长视频生成困难、身份漂移、颜色偏移、场景不稳定以及手部运动建模不佳等问题，需要一种能够保持外观一致性并生成自然手部动作的方法。
...
Conclusion: InfinityHuman通过姿态引导的细化策略和手部奖励机制，有效解决了音频驱动人体动画中的关键挑战，能够生成高质量、长时间、外观一致且手部动作自然的视频。
...
TL;DR: 本研究提出SalViT360和SalViT360-AV两个新颖的360度视频显著性预测模型，并创建了YT360-EyeTracking数据集，通过整合空间音频线索显著提升了全景视频中的视觉注意力预测性能。


<details>
  <summary>Details</summary>
Motivation: 360度全景视频(ODVs)通过提供全视场角和空间音频重新定义了VR观看体验，但目前缺乏用于360度视听显著性预测的全面数据集，需要探索如何利用视听线索有效预测360度视频中的视觉显著性。

Method: 创建了包含81个ODV的YT360-EyeTracking数据集，在不同视听条件下进行观察；提出了SalViT360（基于视觉变换器的框架，配备球面几何感知的时空注意力层）和SalViT360-AV（进一步整合了基于音频输入的变换器适配器）两个显著性预测模型。
...
TL;DR: 本文提出一个结合视觉语言模型的pipeline，可在样本和数据集层面解释视觉模型的行为，帮助发现失败案例并理解模型趋势。


<details>
  <summary>Details</summary>
Motivation: 当前视觉模型开发主要关注性能指标（如准确率、IoU、mAP），而由于xAI方法应用于视觉模型的复杂性，对可解释性的关注较少。现有xAI方法多为逐样本解释，缺乏对模型在大型数据集上整体行为的解释方法。

Method: 利用视觉语言模型，提出一个pipeline来在样本级和数据集级解释视觉模型。该pipeline能够以最小工作量发现失败案例并获得对视觉模型的洞察。

Result: 提出的pipeline能够有效解释视觉模型的整体行为，识别模型偏见，发现趋势和模式，促进视觉模型开发与xAI分析的整合。

Conclusion: 该方法将视觉模型开发与xAI分析相结合，通过视觉语言模型提供多层次的解释能力，有助于推进图像分析领域的发展，防止有偏判断并提高模型透明度。
...
TL;DR: ATMS-KD框架通过自适应温度调度和混合样本增强，将MobileNetV3 Large教师模型的知识蒸馏到轻量级CNN学生模型中，在农业图像分类任务上显著提升性能
...
Method: 结合自适应温度调度和混合样本增强的知识蒸馏框架，使用MobileNetV3 Large作为教师模型，训练三种不同参数量的残差CNN学生模型

Result: 所有学生模型验证准确率超过96.7%，紧凑模型达到97.11%准确率，比第二好的方法提升1.60个百分点，推理延迟仅72.19ms，知识保留率超过99%

Conclusion: ATMS-KD框架在农业图像分类任务上表现出色，能够有效实现知识转移，适用于资源受限的农业环境
...
TL;DR: 提出了一种结合视觉-语言表示(VLRs)的新框架，用于零样本分类增材制造金属基复合材料的微观结构，无需重新训练模型即可区分合格与缺陷样品


<details>
  <summary>Details</summary>
Motivation: 解决工业制造中先进材料快速可靠鉴定的瓶颈问题，特别是针对非传统增材制造工艺生产的异质结构

Method: 集成深度语义分割与预训练多模态模型(CLIP和FLAVA)，开发定制化的基于相似性的表示方法，结合专家标注的图像和文本描述，使用Z-score归一化调整相似性分数

Result: 在增材制造金属基复合材料数据集上验证，FLAVA模型具有更高的视觉敏感性，CLIP模型与文本标准保持更好一致性，能够有效区分合格和缺陷样品
...
TL;DR: 本研究开发了MedNeXt-L-k5模型用于自动分割脑部血管周围间隙(PVS)，在T2加权MRI上表现优异(Dice=0.88)，但在T1加权MRI和跨站点验证中性能下降，且未超越nnU-Net模型。


<details>
  <summary>Details</summary>
Motivation: 手动分割PVS耗时且可靠性有限，现有深度学习模型性能一般且泛化能力不足，需要开发更高效的自动化分割方法。

Method: 采用Transformer启发的3D编码器-解码器卷积网络MedNeXt-L-k5，分别在均质T2加权MRI数据集(200例)和异质T1加权MRI数据集(40例)上训练，使用5折交叉验证和留一站点交叉验证评估性能。

Result: 在HCP-Aging T2加权数据上达到Dice=0.88±0.06(白质)，与人工标注可靠性相当；在T1加权数据上Dice=0.58±0.09；跨站点验证中Dice=0.38±0.16(白质)和0.35±0.12(基底节)。

Conclusion: MedNeXt-L-k5为不同MRI数据集提供了有效的PVS分割方案，但Transformer的注意力机制对PVS分割并非必需，因为nnU-Net表现更优。
...
TL;DR: 提出了一种无需训练、基于反馈的自适应框架，通过将输出层面的补丁级对应关系反馈到中间注意力机制中，增强CLIP在开放词汇分割中的空间一致性和语义一致性。
...
Method: 提出了训练免费、反馈驱动的自适应性框架，将基于输出的补丁级对应关系反馈到中间注意力。设计了关键模块：注意力隔离、基于置信度的稀疏适应剪枝和适应集成，以有效反馈输出一致性线索。该方法作为插件模块，可无缝集成到四种最先进的方法和三种骨干网络（ViT-B、ViT-L、ViT-H）中。

Result: 该方法在八个基准测试中 consistently 提高了性能，并在多种注意力类型（Q-K、self-self、以及用MAE、SAM和DINO增强的Proxy）上得到验证。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在各种视觉语言任务上表现出色，但其内部处理机制仍未被充分探索。研究者希望系统分析MLLMs如何跨层处理视觉和文本输入。

Method: 训练线性分类器来预测从每层提取的token嵌入中的细粒度视觉类别，使用标准化的锚定问题。在三种提示变体下评估探测结果：词汇变体、语义否定变体和输出格式变体。

Result: 在LLaVA-1.5、LLaVA-Next-LLaMA-3和Qwen2-VL上发现一致的阶段性结构，整体结构在不同视觉标记化、指令调优数据和预训练语料变化下保持稳定，但具体层分配随基础LLM架构变化而显著变化。

Conclusion: 研究结果为MLLMs的层级组织提供了统一视角，并提供了轻量级、模型无关的方法来分析多模态表示动态。
...
TL;DR: 该论文提出了一种监督式字典学习方法SLiCS，用于解耦视觉-语言联合嵌入空间，将复杂场景的嵌入分解为多个概念特定的子空间向量，实现更精确的概念过滤图像检索。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言联合嵌入网络（如CLIP）提供了具有语义信息的潜在嵌入空间，但作者假设可以通过解耦这个空间来分离复杂场景的内容信息，从而更好地支持下游任务。

Method: 提出监督式字典学习方法，通过稀疏、非负的字典向量组合来估计线性合成模型。使用新颖的交替优化算法保证收敛，并利用文本共嵌入找到语义上有意义的描述。

Result: SLiCS方法在概念过滤图像检索方面表现出更高的精确度，适用于多种嵌入表示（TiTok压缩自编码器嵌入和DINOv2自监督嵌入），定量和定性结果均显示改进。

Conclusion: 通过稀疏线性概念子空间实现的解耦嵌入能够提供更精确的概念过滤图像检索和条件生成，为多模态表示学习提供了有效的解耦方法。
...
TL;DR: MedFoundationHub是一个图形界面工具包，旨在解决医疗视觉语言模型(VLMs)的安全隐私问题，支持医生无需编程即可使用不同模型，并确保隐私保护的推理部署。
...
Method: 开发了MedFoundationHub图形用户界面工具包，支持医生手动选择使用不同模型，工程师可插拔式部署医疗VLMs，通过Docker编排实现操作系统无关的隐私保护推理部署。

Result: 评估了5个最先进的VLM模型，由认证病理学家进行1015次临床医生-模型评分事件，发现存在脱靶回答、模糊推理和不一致的病理学术语等重复性限制。

Conclusion: MedFoundationHub提供了一个安全、易用的解决方案，使医疗VLMs能够在典型学术研究实验室资源下安全部署，同时揭示了当前模型在临床应用中的局限性。
...
Method: 采用Bidirectional Interaction Mamba架构，包含BI-Scan机制（任务优先和位置优先扫描模式）和MS-Scan机制（多粒度场景建模），在统一线性复杂度框架下实现高效跨任务信息保留
...
TL;DR: 一种新的音频导航视觉编辑框架，通过多模态编码器和噪声分支技术，能够在不需额外训练的情况下处理复杂的多模态编辑任务


<details>
  <summary>Details</summary>
Motivation: 现有的文本导向边演模型在复杂场景中无法充分描述编辑需求，而现有音频导航方法需要特定数据集训练且泛化能力有限

Method: 利用预训练的多模态编码器，通过消除音频编码空间与推理模型提示编码空间的差异，采用分离噪声分支和适应性补丁选择来处理多模态编辑提示

Result: 在多样化编辑任务上进行了全面实验，证明该框架能够通过音频融入丰富信息来处理复杂编辑场景，而仅依靠文本的方法无法完成

Conclusion: 该框架提供了一种无需额外训练的音频导航视觉编辑方案，能够有效处理文本单独无法满足的复杂多模态编辑需求
...
TL;DR: 提出AEVLP框架，通过GPR损失函数和DAMP伪标签技术解决单正多标签学习问题，在多个基准数据集上达到SOTA性能
...
Conclusion: AEVLP框架通过创新的损失函数和伪标签技术有效解决了SPML问题，在多标签分类任务中表现出色
...
Method: 提出了延迟脉冲方法缓解残余膜电位问题，并设计了时间依赖的积分发放(tdIF)神经元架构，使IF神经元能够根据时间步长的时序动态调整积累和发放行为。

Result: 在目标检测和车道线检测两个关键视觉任务上进行了广泛评估，结果表明该方法超越了当前的ANN-SNN转换方法，在超低延迟(5个时间步内)下达到了最先进的性能。

Conclusion: 该方法使脉冲能够表现出不同的时间特性，而不仅仅依赖于基于频率的表示，在更少的时间步长下实现了更精确的特征表示，同时保持了与传统IF神经元相当的能量消耗。
...
TL;DR: MSMVD通过多尺度BEV特征投影和特征金字塔网络，解决了多视角行人检测中尺度变化大的问题，在GMVD数据集上比之前最佳方法提升了4.5个MODA点
...
Result: 大量实验表明，通过多尺度BEV特征利用多尺度图像特征显著提升了检测性能，在GMVD数据集上MODA指标比之前最佳方法高出4.5分

Conclusion: 多尺度图像特征通过多尺度BEV特征的利用对多视角行人检测性能有重要提升作用，MSMVD方法有效解决了尺度变化带来的检测挑战
...
Conclusion: SFMFNet为实时深度伪造检测提供了有效的轻量级解决方案，在准确性和计算效率方面表现优异
...
TL;DR: 一种结合双模型权重选择咈自我知识萌荐的轻量级医学图像分类方法，解决大模型部署计算资源受限问题


<details>
  <summary>Details</summary>
Motivation: 医学实际应用中大规模模型部署遇到计算资源约束，需要开发轻量级模型但保持与大模型相当的性能

Method: 采用双模型权重选择策略，从大型预训练模型中初始化两个轻量级模型，然后应用自我知识萌荐(SKD)技术，最后进行细调

Result: 在公开的胸部X光片、肺CT扫描和脑部磁共振扫描数据集上进行广泛实验，证明方法在性能咈稳健性方面都显著优于现有方法

Conclusion: 通过结合双模型权重选择咈自我知识萌荐，该方法克服了传统方法在细小模型中丢失关键信息的问题，实现了高效的轻量级医学图像分类
...
TL;DR: 提出了一种高效的LiDAR点云压缩方法，通过几何重稠密化和跨尺度特征传播模块实现紧凑特征表示，在KITTI数据集上达到最先进的压缩比和实时性能。


<details>
  <summary>Details</summary>
Motivation: 高精度LiDAR点云扫描带来巨大的存储和传输开销，现有方法在处理极度稀疏的几何细节时上下文建模效率低下，限制了压缩性能和速度。

Method: 提出包含两个轻量级模块的框架：1)几何重稠密化模块-重新稠密化稀疏几何，在更密集尺度提取特征后重新稀疏化；2)跨尺度特征传播模块-利用多分辨率占用线索指导层次特征传播，减少冗余特征提取。

Result: 在KITTI数据集上实现了最先进的压缩比和实时性能，在12位量化下编码和解码均达到26 FPS。

Conclusion: 该方法通过紧凑特征表示实现了高效的上下文建模，加速了编码过程，为解决LiDAR点云压缩中的稀疏性问题提供了有效解决方案。
...
TL;DR: 本文提出利用视频数据解决3D生成中的数据稀缺问题，通过构建大规模多视角视频数据集Droplet3D-4M和训练生成模型Droplet3D，实现空间一致且语义合理的3D内容生成。
...
Result: 实验证明该方法能生成空间一致且语义合理的3D内容，相比现有3D解决方案具有扩展到场景级应用的潜力，视频常识先验显著促进了3D创作。

Conclusion: 视频模态为3D资产生成提供了有效的解决方案，通过利用视频中的多视角空间先验和丰富语义信息，成功克服了3D数据稀缺的限制，并展示了良好的扩展性。
...
Result: 在Waymo Open Dataset上的实验表明，G^2Editor在统一框架内有效支持物体重定位、插入和删除操作，在姿态可控性和视觉质量方面优于现有方法，并能提升下游数据驱动任务的性能。

Conclusion: G^2Editor提供了一个有效的解决方案，通过结合3D几何先验和生成模型，实现了驾驶视频中高质量、精确控制的物体编辑，为自动驾驶系统训练提供了更安全、成本更低的角点案例生成方法。
...
TL;DR: 通过病理信息域随机化策略，从健康数据生成合成数据来解决胎儿脑分割中CCD病例注释数据稀缺问题，显著提升了分割准确性和生物标记物提取效果


<details>
  <summary>Details</summary>
Motivation: 胎儿脑部精确分割对评估神经发育至关重要，但胰体胶体发育不良(CCD)等稀有病变造成注释数据极其稀缺，限制了深度学习模型的普适性

Method: 提出病理信息域随机化策略，将CCD表现的先验知识嵌入合成数据生成流程，仅从健康数据模拟多样的脑部改变

Result: 在248例健康胎儿、26例CCD和47例其他脑部病理的期前数据集上验证，CCD案例分割性能显著提升，胰体胶体长度(LCC)估计错误从10.9mm降至0.7mm，同时保持对其他病理的性能

Conclusion: 将领域特定的解剖先验知识整合到合成数据流程中，可有效缓解数据稀缺问题，为稀有但临床意义重大的病变提供更可靠的分析手段
...
TL;DR: 首个统一框架，可同时处理手语、唇语和音频输入，生成口语文本，在多个任务上达到或超越最先进模型性能


<details>
  <summary>Details</summary>
Motivation: 传统语音识别系统对听障人士不可用，而现有的视觉替代方案（手语和唇读）各自独立研究，缺乏统一框架来整合这些模态

Method: 设计统一的模态无关架构，处理异构输入；探索模态间协同作用，特别是唇部运动作为手语理解的非手动线索；实现与专业模型相当或更好的性能

Result: 在SLT、VSR、ASR和AVSR任务上达到或超越最先进模型性能；明确建模唇部运动作为独立模态显著提升手语翻译性能
...
Method: 提出多轮推理框架，逐步选择视频片段；引入门控双级奖励系统（轨迹级奖励和轮级奖励），优化片段选择和问题理解。
...
Method: 提出双重不确定性优化框架，包括：1）基于凸优化的无监督焦点损失进行不确定性加权；2）语义感知的法向场约束保持几何一致性；3）双分支互补机制
...
TL;DR: 提出了名为CaddieSet的新数据集，包含高尔夫挥杆关节信息和球轨迹数据，通过计算机视觉方法分析8个挥杆阶段，定义了15个关键指标，验证了基于关节特征预测球轨迹的可行性。
...
Method: 创建CaddieSet数据集，从单次挥杆视频中提取关节信息并分割为8个挥杆阶段，基于高尔夫专业知识定义15个影响挥杆的关键指标。

Result: 实验证明CaddieSet可用于预测球轨迹，特别是可解释模型显示关节特征提供的挥杆反馈与领域知识定量一致。

Conclusion: 该工作为学术界和体育产业提供了高尔夫挥杆分析的新见解，有望推动挥杆技术的定量研究。
...
TL;DR: IAENet是一个用于3D表面异常检测的集成框架，通过重要性感知融合模块动态整合2D和3D专家模型的预测，解决了多模态融合的挑战，在MVTec 3D-AD数据集上达到了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 3D点云异常检测研究不足，缺乏像2D那样强大的预训练基础骨干网络。现有多模态融合方法容易受到性能较差的模态影响，导致整体精度下降。

Method: 提出重要性感知集成网络(IAENet)，包含重要性感知融合(IAF)模块，动态评估各源模型的贡献并重新加权异常分数。设计关键损失函数指导IAF优化，既整合集体知识又保留各专家的独特优势。

Result: 在MVTec 3D-AD数据集上的大量实验表明，IAENet达到了新的最先进水平，具有显著降低的误报率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的语义图像编辑方法存在重建误差（基于反演的方法）和数据集质量限制（基于指令的方法）的问题，需要一种更好的解决方案。

Method: 提出Cross-Attentive UNet，通过添加注意力桥接将参考图像特征注入到提示到编辑图像的生成过程中，保持文本到图像模型的生成能力。

Result: 在Emu Edit基准测试中显示出编辑准确性和一致性的提升，并能与ControlNet、IP-Adapter等扩展无缝集成。

Conclusion: DescriptiveEdit框架通过重新定义问题范式，克服了现有方法的局限性，提供了更可扩展和高质量的语义图像编辑解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在无法访问源域数据的情况下，现有方法依赖模型预测生成伪标签，但伪标签质量无法保证且存在错误累积问题，需要解决特征混淆和学习偏差

Method: 使用双分类器将目标数据特征解耦为语义相关特征和域相关特征，保持子特征与整体特征的一致性，并设置自适应阈值计算置信度进行加权自监督学习

Result: 在CIFAR10-C、CIFAR100-C和ImageNet-C等多个数据集上的广泛实验验证了方法的有效性，在持续测试时适应场景中表现一致

Conclusion: DCFS框架通过特征解耦和一致性约束，以及置信度感知的样本学习，有效降低了伪标签噪声并缓解了错误累积问题，在持续测试时适应任务中取得了良好性能
...
<details>
  <summary>Details</summary>
Motivation: 相机标定质量对新颖视角合成至关重要，1像素误差就会显著影响重建质量，但真实场景缺乏标定真值

Method: 提出基于3DGS模型，通过反向传播新视角颜色损失来优化相机参数，实现标定微调

Result: 在3DGS参考数据集上平均提升0.4 dB PSNR，对Mip-NeRF 360等参考场景效果显著

Conclusion: 虽然微调过程耗时，但对于标定质量要求高的参考场景，该方法能有效提升新颖视角合成质量
...
<details>
  <summary>Details</summary>
Motivation: 医学图像标注成本高昂，传统主动域适应方法容易导致负迁移且未考虑多模态数据特性，需要开发更有效的样本选择策略来降低标注成本。

Method: 提出主动序列域适应框架，基于信息量和代表性设计查询策略，动态选择多模态医学数据中最有价值的样本进行标注和训练。

Result: 在多种肿瘤体积分割任务上的实验验证表明，该方法取得了优异的分割性能，显著超越了最先进的主动域适应方法。

Conclusion: 该方法能有效降低医学图像标注成本，提升多模态医学图像分割性能，为放疗规划提供更准确的肿瘤体积分割结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LiDAR的3D目标检测器需要大量手动标注标签，但高质量3D标签获取成本高。现有方法简单融合LiDAR和RGB生成的伪框，忽略了两种模态数据的互补性，改进有限

Method: 提出数据级融合框架：1）使用视觉基础模型进行图像实例分割和深度估计；2）双向融合方法：真实点从2D空间获取类别标签，2D像素投影到3D增强点密度；3）局部和全局滤波方法抑制噪声；4）基于数据级融合的动态自进化策略迭代优化伪框

Result: 在nuScenes数据集验证集上达到28.4% mAP，显著优于之前的最先进方法

Conclusion: 通过早期数据级融合充分利用LiDAR和RGB图像的互补信息，结合有效的噪声抑制和迭代优化策略，能够显著提升无监督3D目标检测的性能
...
TL;DR: 使用深度学习从相机图像估计BMI，在大规模数据集上达到7.9%的MAPE，并开发了移动端部署方案


<details>
  <summary>Details</summary>
Motivation: 在远程医疗或紧急情况下，传统BMI测量方法不可行时，需要快速的非接触式体重评估方法

Method: 基于84,963张智能手机图像的大数据集，采用姿势聚类和人物检测的自动过滤方法筛选高质量图像，使用深度学习模型进行BMI估计

Result: 在WayBED测试集上达到7.9%的MAPE（文献最低），在未见过的VisualBodyToBMI数据集上达到13% MAPE，微调后达到8.56% MAPE（该数据集最佳）

Conclusion: 提出的方法在BMI估计精度和泛化能力方面达到state-of-the-art水平，并成功实现移动端部署，代码开源
...
TL;DR: 本研究通过557个模拟实验，系统评估了7种主流域适应技术在自然和医学图像分类中的表现，发现DSAN算法在COVID-19等医学数据集上表现优异，准确率达91.2%，且在动态数据流场景中相比基线提升6.7%


<details>
  <summary>Details</summary>
Motivation: 解决域适应技术在医学图像应用中研究不足的问题，同时避免主流自然图像数据集可能带来的性能偏差，系统评估域适应技术在不同场景下的效果

Method: 使用7种广泛应用的域适应技术，在5个自然图像数据集和8个医学图像数据集上进行557个模拟实验，涵盖分布外、动态数据流和有限训练样本等多种场景

Result: DSAN算法表现突出，在COVID-19数据集上使用Resnet50达到91.2%的分类准确率，在动态数据流场景中相比基线提升6.7%，同时在COVID-19和皮肤癌数据集上展现出良好的可解释性
...
TL;DR: 提出CLAB方法，通过对比学习辅助分支增强视频目标检测的特征表示能力，无需额外计算开销即可提升对图像退化的鲁棒性，在ImageNet VID数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 视频目标检测面临运动模糊、遮挡和变形等图像退化问题，现有方法通过特征聚合和复杂后处理提升性能但计算成本高，需要在不增加推理计算量的情况下提高鲁棒性
...
Conclusion: CLAB方法简单有效，通过对比学习增强特征表示，动态损失加权策略优化训练过程，在不增加推理计算量的情况下显著提升视频目标检测性能
...
<details>
  <summary>Details</summary>
Motivation: 排版攻击通过向图像中注入文本来操纵多模态系统，导致目标错误分类、恶意内容生成和视觉语言模型越狱。需要研究CLIP模型在这种攻击下的行为并开发有效的防御机制。

Method: 定位CLIP模型后半层中专门处理排版信息的注意力头，构建排版电路，通过选择性消融这些注意力头来防御攻击，无需微调即可实现防御。
...
Method: 使用3D面部对齐提取地标，构建分层粗化的商图来保持空间结构并降低复杂度，采用图神经网络建模地标间关系依赖

Result: 在AffectNet数据集上达到64.89%准确率，在FERG数据集上达到94.24%准确率，优于多个现有基线方法

Conclusion: 基于商图的区域级嵌入方法有效提升了预测性能，图神经网络为面部表情识别提供了结构化和可解释的学习框架
...
TL;DR: FastFit是一个基于可缓存扩散架构的高速多参考虚拟试穿框架，通过半注意力机制和类别嵌入技术，实现了参考特征的单次计算和跨步骤重用，在保持高保真度的同时获得3.5倍加速
...
Method: 提出基于可缓存扩散架构的FastFit框架，采用半注意力机制，用类别嵌入替代传统时间步嵌入，完全解耦参考特征编码与去噪过程，实现参考特征的单次计算和跨步骤无损重用

Result: 在VITON-HD、DressCode和新建的DressCode-MR数据集上，FastFit在关键保真度指标上超越最先进方法，同时实现平均3.5倍加速，显著提升推理效率

Conclusion: FastFit通过创新的缓存架构解决了多参考虚拟试穿的效率和功能限制问题，为复杂多参考虚拟试穿研究提供了新的数据集和技术基础
...
TL;DR: UTA-Sign是一种无监督的热成像-事件视频增强方法，用于低光照环境下的交通标志识别，通过融合热成像帧和事件信号来弥补各自缺陷
...
Conclusion: UTA-Sign方法有效解决了低光照环境下交通标志识别问题，通过多模态融合提升了自动驾驶系统对语义理解的准确性
...
TL;DR: 提出基于低频感知扰动的主动防御方法，通过结合频域和空域特征来破坏深度伪造的人脸交换生成过程，在保持视觉质量的同时有效降低伪造效果
...
Method: 结合频域和空域特征，使用离散小波变换提取低频分量生成扰动，设计包含编码器、扰动生成器和解码器的完整架构，在保持高频细节的同时引入低频伪影

Result: 在CelebA-HQ和LFW数据集上实验显示，人脸交换效果显著降低，防御成功率提高，同时保持了视觉质量

Conclusion: 该方法提供了一种有效的主动防御方案，通过针对性干扰生成过程来对抗深度伪造技术，在防御效果和视觉保真度之间取得了良好平衡
...
TL;DR: 本文提出Diverse-T2M方法，通过在transformer中引入噪声信号作为多样性载体，构建连续潜在空间表示，并集成潜在空间采样器，显著提升了文本到3D人体动作生成的多样性，同时保持文本一致性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到3D人体动作生成方法在精确性和质量方面取得了进展，但在生成多样性方面仍面临挑战。本文旨在克服这一挑战，实现在保持文本语义一致性的同时生成高度多样化的动作。

Method: 1) 在transformer方法中引入噪声信号作为多样性信息载体，实现不确定性的显式建模；2) 构建连续潜在空间表示文本，替代刚性的一对一映射；3) 集成潜在空间采样器，在生成过程中引入随机采样。

Result: 在HumanML3D和KIT-ML基准数据集上的实验表明，该方法在显著增强生成多样性的同时，保持了文本一致性的最先进性能。

Conclusion: Diverse-T2M方法通过引入不确定性建模和潜在空间采样，有效解决了文本到动作生成中的多样性挑战，为3D人体动作生成提供了新的解决方案。
...
TL;DR: 提出基于3D打印体模的优化校准方法，用于精确重建3D血管内超声体积，实现术前CT与术中超声图像的配准，在猪肝实验中达到毫米级精度


<details>
  <summary>Details</summary>
Motivation: 肝脏手术中术中超声图像因视野有限和解剖结构复杂而难以解读，需要桥接术前和术中数据以提供有效的手术引导

Method: 使用3D打印体模的优化校准方法，通过跟踪IVUS数据进行精确配准，实现3D血管内超声体积重建

Result: 在猪肝实验中，校准误差为0.88-1.80mm，配准误差为3.40-5.71mm，验证了方法的准确性和可靠性

Conclusion: 该方法为肝脏手术中术前CT与术中超声图像的配准提供了可靠准确的校准和体积重建手段，可增强术中引导效果
...
Method: 使用扩散模型结合两个物理约束，开发PI-GenMFI物理信息生成模型，针对常见电源短路缺陷生成合成MFI图像。

Result: 与VAE和扩散方法的SOTA生成模型比较，通过领域专家评估和多种图像生成/信号处理指标进行定性和定量评估，显示优化缺陷定位过程的有希望结果。

Conclusion: PI-GenMFI方法能有效生成合成MFI数据，为解决半导体缺陷检测中的数据稀缺问题提供了可行方案，有望提升缺陷定位效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据重建攻击主要针对浅层模型，无法充分利用语义先验，重建质量和泛化能力有限，无法应对深度复杂网络的分割推理隐私风险

Method: 采用GAN框架，将生成器分解为分层块进行渐进式特征优化，引入L1-ball约束来稳定优化过程并提高图像真实感

Result: 在多个实验场景中大幅超越现有攻击方法，特别是在高分辨率、分布外设置以及深度复杂网络情况下表现优异

Conclusion: 该方法显著提升了数据重建攻击的效果，揭示了分割推理中更严重的隐私风险，需要更强的隐私保护措施
...
TL;DR: EmoCAST是一个基于扩散模型的文本驱动情感说话头生成框架，通过解耦情感模块和情感音频注意力模块，实现了精确的情感控制和音频同步，在真实性和情感表达方面达到最先进性能。
...
Method: 提出EmoCAST扩散框架，包含文本引导解耦情感模块进行外观建模，情感音频注意力模块捕捉情感与音频的交互，并构建情感说话头数据集，采用情感感知采样训练策略和渐进功能训练策略。
...
TL;DR: SwinUNETR深度学习框架在乳腺癌MRI诊断挑战中获第二名，通过区域掩码、数据增强和集成学习提高鲁棒性


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是女性癌症死亡主因，早期检测至关重要。MRI对高风险或致密乳腺组织女性特别有效，但需要AI辅助提高诊断效率

Method: 基于SwinUNETR的深度学习框架，包含乳腺区域掩码、广泛数据增强和集成学习技术

Result: 在多中心挑战中获得第二名，证明了方法在临床乳腺MRI解读中的潜力
...
TL;DR: AvatarBack是一个针对3D高斯头像重建的即插即用框架，通过生成身份一致的后视图伪图像和自适应空间对齐策略，解决了现有方法因依赖前视图图像而导致后脑区域重建质量差的问题。
...
Result: 在NeRSemble和K-hairstyle数据集上的实验表明，AvatarBack显著提升了后脑重建质量，同时保持前视图保真度，重建的头像在不同运动下保持一致的视觉真实感且完全可动画化。
...
<details>
  <summary>Details</summary>
Motivation: 历史绘画中的人物识别对艺术史研究很重要，但传统面部识别方法在绘画领域表现不佳，存在域偏移和艺术风格变化等挑战。

Method: 通过微调基础模型，并将其嵌入特征与传统面部识别网络的嵌入特征进行融合集成。

Result: 该方法相比当前最先进方法取得了显著改进，证明基础模型能够弥补传统方法在艺术绘画识别中的不足。

Conclusion: 基础模型可以有效解决传统面部识别在艺术作品中的局限性，为艺术史研究提供更可靠的技术支持。
...
Method: 使用LoRA微调的预训练扩散变换器进行涂鸦风格迁移，通过面部一致的自注意力机制增强身份嵌入，采用CLIP引导的提示扩展实现无关键点的姿态定制。

Result: 定量结果显示具有竞争力的面部特征一致性，以及最先进的美学和人类偏好评分。定性分析和现场部署证明了系统的实际创意影响。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型和视觉语言模型的快速发展带来了模态对齐的挑战，现有方法依赖外部数据集、人工标注或复杂后处理，限制了可扩展性并增加了成本。

Method: 生成去偏自判断分数作为自评估指标，使模型能够自主改进对齐，增强解码策略和偏好调优过程。

Result: 实证结果显示该方法显著优于传统方法，减少了幻觉，增强了安全性，并提高了整体能力。

Conclusion: 该方法为对齐LVLMs提供了更有效的解决方案，实现了自主改进的模态对齐。
...
TL;DR: S-HArM是一个多模态数据集，用于意图感知分类，包含9576个来自社交媒体平台的图像-文本对，标注为幽默/讽刺、艺术或虚假信息。研究探索了三种提示策略构建合成训练数据，发现图像和多模态引导的数据在泛化到真实内容时表现更好，但整体性能有限。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多忽视了AI生成图像背后的意图，为了填补这一空白，需要开发能够识别内容创作意图的多模态分类系统。

Method: 构建S-HArM数据集（9576个真实图像-文本对），探索三种提示策略（图像引导、描述引导、多模态引导）使用Stable Diffusion生成合成训练数据，比较多种模型架构包括模态融合、对比学习、重建网络、注意力机制和大规模视觉语言模型。

Result: 图像引导和多模态引导的合成数据在泛化到真实内容时表现更好，因为保留了视觉上下文。但整体分类性能仍然有限。

Conclusion: 推断意图具有复杂性，需要专门的架构来处理这一挑战，当前方法的性能还有待提升。
...
TL;DR: MobileCLIP2是MobileCLIP的改进版本，通过更好的CLIP教师集成和优化的标题生成器，在保持低延迟的同时实现了最先进的零样本准确率，特别是在ImageNet-1k上相比前代有显著提升。


<details>
  <summary>Details</summary>
Motivation: 改进MobileCLIP的多模态强化训练方法，通过更好的教师模型集成和标题生成器优化，进一步提升模型性能同时保持低延迟特性。

Method: 1) 使用DFN数据集训练的更好的CLIP教师集成 2) 在DFN数据集上训练并在高质量图像-标题数据集上微调的改进标题生成器教师 3) 对比知识蒸馏中的温度调优 4) 结合多个模型的合成标题

Result: MobileCLIP2在ImageNet-1k零样本准确率上取得最先进性能：MobileCLIP2-B相比MobileCLIP-B提升2.2%；MobileCLIP2-S4在保持2倍更小模型大小的同时匹配SigLIP-SO400M/14的准确率，并以2.5倍更低延迟超越DFN ViT-L/14。

Conclusion: MobileCLIP2通过改进的多模态强化训练方法，成功实现了在低延迟约束下的性能突破，为移动端视觉-语言模型提供了新的技术方案，并开源了预训练模型和数据生成代码。
...
TL;DR: 提出动态视频压缩框架，通过可变编码路由和实时率控制代理实现精确码率控制，在保持RD性能的同时显著降低码率误差
...
Method: 动态路由自编码器（DRA）设计可变编码路由，每条路由占用部分计算复杂度；率控制代理实时估计码率并调整编码路由；采用联合路由优化策略进行协同训练

Result: 在HEVC和UVG数据集上平均BD-Rate降低14.8%，BD-PSNR提升0.47dB，平均码率误差仅1.66%

Conclusion: 该方法实现了码率-失真-复杂度优化（RDCO），为各种码率和码率受限应用提供了有效解决方案
...
TL;DR: CardioMorphNet是一个基于贝叶斯深度学习的3D心脏运动估计框架，通过形状引导的配准方法，不使用基于强度的相似性损失，在UK Biobank数据集上表现出优于现有方法的性能。
...
Result: 在UK Biobank数据集上验证，CardioMorphNet在心脏运动估计方面表现优异，超越最先进方法，并能计算运动场的不确定性图，在心脏区域具有更低的不确定性值。
...
TL;DR: 通过风格扰动、基于关注的特征对齐和EMA教师渗遗等方法，提出了一种域精检的不典型有丝切裂图分类方法，在MIDOG 2025挑战中取得了突出的结果。


<details>
  <summary>Details</summary>
Motivation: 不典型有丝切裂图是重要的组织病理学标记物，但在不同扫描仪、染色和获取方式导致的域偏移情况下，其识别一致性面临挑战。需要开发能够在不同来源数据上稳定识别AMF的方法。

Method: 1）在网络早期和中期阶段插入风格扰动增加特征多样性
2）使用粗糕域元数据（扫描仪、来源、种类、肿瘤）通过辅助对齐损失对齐关注精炼后的特征
3）使用指数移动平均（EMA）教师模型通过温度缩放的KL散度稳定预测结果

Result: 在MIDOG 2025 Task 2预测排名榜上，方法达到了平衡准确率0.8762、敏感度0.8873、特异度0.8651和ROC AUC 0.9499的突出性能。方法在推理时添加的开销可以忽略不计，仅需粗糕的域元数据即可实现强劲的平衡性能。

Conclusion: 该方法通过简单有效的训练时策略，成功实现了域精检的不典型有丝切裂图分类，在MIDOG 2025挑战中位于竞争力强劲的地位，为组织病理学图像分析提供了可靠的解决方案。
...
TL;DR: 该论文提出了Pref-GRPO方法来解决文本到图像生成中基于点式奖励模型的奖励黑客问题，通过成对偏好比较替代分数最大化，并引入了UniGenBench综合评估基准。


<details>
  <summary>Details</summary>
Motivation: 当前基于点式奖励模型的文本到图像生成方法容易受到奖励黑客攻击，即模型过度优化微小分数差异导致生成过程不稳定。现有评估基准也过于粗糙，无法全面评估模型性能。

Method: 提出Pref-GRPO方法：使用成对偏好奖励模型，在每组图像中进行成对比较，以胜率作为奖励信号，将优化目标从分数最大化转为偏好拟合。同时构建UniGenBench基准，包含600个提示词和分层评估标准。

Result: 实验表明Pref-GRPO能够区分细微的图像质量差异，提供更稳定的优势并缓解奖励黑客问题。UniGenBench基准揭示了开源和闭源T2I模型的优缺点，并验证了Pref-GRPO的有效性。

Conclusion: Pref-GRPO通过成对偏好比较解决了奖励黑客问题，UniGenBench提供了更全面的评估框架，共同推动了文本到图像生成技术的发展。
...
TL;DR: C3-GS是一个用于未见场景新视角合成的通用化高斯泼溅框架，通过上下文感知、跨维度和跨尺度约束提升特征学习，实现高质量渲染而无需逐场景优化。
...
Method: 提出C3-GS框架，集成三个轻量级模块到统一渲染管线中，通过上下文感知、跨维度和跨尺度约束增强特征融合。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有零样本3D视觉定位方法因依赖单视角定位而导致的空间推理受限、上下文遗漏和细节退化问题，消除场景特定训练需求，提升真实世界应用的泛化能力。

Method: 首先通过3D语义分割网络生成3D实例提案并进行语义过滤；然后采用提案引导的多视角投影策略将候选提案投影到真实场景图像序列中；最后使用动态调度机制迭代处理序列查询提示，利用VLM的跨模态推理能力识别文本指定的对象。

Result: 在ScanRefer和Nr3D基准测试中分别达到55.6%和53.2%的Acc@0.25分数，比之前的零样本方法分别提升了4.0%和5.2%。

Conclusion: SeqVLM通过多视角空间推理和动态调度机制，显著提升了零样本3D视觉定位的性能，推动了3DVG向更高泛化能力和真实世界适用性的发展。
...
TL;DR: CLIP视觉语言模型在军事车辆识别任务中对遮挡的鲁棒性研究，发现Transformer架构优于CNN，分散遮挡比连续遮挡影响更大，微调可显著提升遮挡容忍度至60%以上


<details>
  <summary>Details</summary>
Motivation: 探索CLIP模型在军事环境中面对部分遮挡和低信噪比等挑战性条件下的鲁棒性，为国防应用中稀缺标注数据场景下的零样本分类提供可靠性评估

Method: 使用18类军事车辆自定义数据集，通过不同遮挡百分比评估CLIP变体的性能，采用标准化曲线下面积(NAUC)作为评估指标，比较Transformer和CNN架构，分析线性探测和骨干网络微调的效果

Result: 1) Transformer-based CLIP consistently outperforms CNNs; 2) Fine-grained dispersed occlusions degrade performance more than larger contiguous occlusions; 3) Linear-probed models' performance drops sharply at ~35% occlusion; 4) Backbone finetuning pushes performance drop threshold to >60% occlusion
...
TL;DR: SKGE-Swin架构利用Swin Transformer和跳跃连接机制，在CARLA平台上实现了优于现有方法的自动驾驶性能
...
Result: 在CARLA平台的对抗场景测试中，SKGE-Swin架构获得了更高的驾驶分数
...
<details>
  <summary>Details</summary>
Motivation: 现有因果视频问答模型存在高阶推理困难，使用不透明的整体管道，缺乏可解释性且依赖浅层启发式方法
...
Conclusion: 该方法不仅提升了性能，还增强了可解释性，使CCE成为跨领域可重用的因果推理引擎
...
TL;DR: 这篇论文综述了视频中抽象概念理解的研究进展，指出虽然深度学习在具体实体识别方面取得显著成果，但抽象概念识别仍是重要挑战，并主张利用基础模型和多模态技术来推进这一领域发展。


<details>
  <summary>Details</summary>
Motivation: 当前视频理解主要关注具体实体（物体、动作等）的识别，但人类还能理解抽象概念（如正义、自由）。随着基础模型的发展，现在具备了解决视频抽象概念理解这一重要挑战的理想条件，这有助于模型更符合人类推理和价值观。

Method: 通过系统研究不同任务和数据集，分析社区几十年来在视频抽象概念理解方面的经验积累，避免在基础模型时代"重复造轮子"。

Result: 研究发现研究者们长期尝试解决视频抽象概念理解任务，并充分利用当时可用的工具。基础模型为重新审视这一重大挑战提供了新的机遇。

Conclusion: 利用多模态基础模型时代的优势，借鉴社区数十年经验，将有助于推动视频抽象概念理解这一重要开放挑战的解决，使模型更贴近人类推理方式。
...
<details>
  <summary>Details</summary>
Motivation: 现有皮肤病变分类模型虽然准确率高，但缺乏可信度和可解释性。LIME方法存在不一致性问题，CAM方法未能考虑所有类别，导致医生对AI诊断不信任

Method: 提出全局类别激活概率图评估方法，在像素级别概率化分析所有类别的激活图，结合SafeML技术检测误诊并发出警告

Result: 在ISIC数据集上使用MobileNetV2和Vision Transformers进行评估，实现了更好的可解释性和诊断可靠性
...
TL;DR: 本文探讨了扩散分类器在组合泛化能力方面是否优于判别式模型，发现在概念绑定任务上表现良好，但在关系推理任务上所有模型都存在显著困难。


<details>
  <summary>Details</summary>
Motivation: 自然语言语义的一个基本方面是能够从已知部分组合形成新含义，但现有的视觉语言模型（如CLIP）在组合语义方面存在不足，表现为'词袋'式表示。扩散模型在生成能力方面表现出色，本研究旨在探索基于扩散模型的分类器是否具有更好的组合泛化能力。

Method: 评估三种模型（扩散分类器、CLIP和ViLT）在零样本学习（ZSL）和广义零样本学习（GZSL）设置下绑定对象属性和关系的能力。通过分析CLIP嵌入来探究关系概念表示相似性问题。

Result: 扩散分类器和ViLT在概念绑定任务上表现良好，但所有模型在关系GZSL任务上都面临显著困难。CLIP嵌入分析表明，困难可能源于左右等关系概念的表示过于相似。

Conclusion: 视觉语言模型在关系推理方面普遍存在挑战，扩散分类器在组合泛化方面相比判别式模型有一定优势，但关系推理仍然是所有模型需要解决的关键问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有点云配准方法忽略了点方向和点不确定性，导致模型对噪声输入和激进旋转（如正交变换）敏感，需要大量带变换增强的训练点云数据。
...
<details>
  <summary>Details</summary>
Motivation: 当前牙科龋齿检测方法由于对比度变化细微和病变形态多样，检测精度往往不理想。受牙医临床工作流程启发，需要结合整体图像筛查和详细牙齿级检查来提高检测准确性。

Method: 采用自动牙齿检测建立全局和局部两个互补视图，分别预训练两个视觉基础模型。全局视图模型作为检测主干生成区域建议和全局特征，局部视图模型提取裁剪牙齿斑块的详细特征。通过门控交叉视图注意力模块动态融合双视图特征。
...
<details>
  <summary>Details</summary>
Motivation: 现有VIF方法主要关注图像质量优化，语义分割需要大量标注，目标检测在拥挤场景中面临边界框重叠和遮挡问题。RGB-T人群计数虽受关注但尚未与VIF集成。

Method: 提出多任务学习框架，利用人群计数提供人口密度信息；采用动态损失函数权重策略加速收敛；引入对抗训练增强模型鲁棒性。

Result: 在公开数据集上的实验表明，FusionCounting不仅提升了图像融合质量，还实现了优越的人群计数性能。

Conclusion: 该框架成功将人群计数与图像融合结合，为密集场景提供了一种标注需求少、性能优越的解决方案。
...
TL;DR: 使用LoRA技术微调视觉语言模型(VLMs)进行手术工具2D关键点估计的新方法，在低资源场景下仅需2个训练周期即可超越传统基线模型


<details>
  <summary>Details</summary>
Motivation: 传统CNN或Transformer方法在小型医疗数据集上容易过拟合，需要利用预训练VLMs的泛化能力来解决手术工具关键点检测问题

Method: 设计指令调优数据集提示词，使用LoRA低秩调整技术微调预训练VLMs，将视觉特征与语义关键点描述对齐

Result: 仅经过2个训练周期的微调，适配后的VLM模型性能就超过了基线模型，证明了LoRA在低资源场景下的有效性

Conclusion: 该方法不仅提高了关键点检测性能，还为未来3D手术手部和工具姿态估计研究铺平了道路
...
TL;DR: 这是首个研究RWKV模型在点云分类领域通用性的工作，提出PointDGRWKV框架解决了原生RWKV在点云数据上的空间扭曲和波动效应问题


<details>
  <summary>Details</summary>
Motivation: 现有点云分类领域通用性方法存在感矩野限制、计算成本高或长程依赖模型不足等问题，RWKV作为新兴架构具有线性复杂度、全局感矩野和长程依赖优势

Method: 提出PointDGRWKV框架：1)适配性几何token移动模块来模拟局部邻域结构，提升几何上下文感矩；2)跨领域关键特征分布对齐模块，通过对齐关键特征分布减轻波动效应

Result: 在多个标准数据集上进行了广泛实验，PointDGRWKV在点云分类领域通用性任务上达到了最先进的性能

Conclusion: PointDGRWKV成为首个RWKV基于框架在点云分类领域通用性中的成功应用，同时保持了RWKV的线性效率优势
...
TL;DR: 该论文开发了基于反事实推理的框架，用于解释、审计和减轻视觉分类器和生成模型中的偏见。通过系统性地改变语义属性，揭示了虚假相关性并构建更鲁棒的系统。


<details>
  <summary>Details</summary>
Motivation: 反事实推理已成为可解释和公平AI的核心方法。该论文旨在开发系统化的反事实框架，用于发现和缓解视觉分类器和生成模型中的偏见问题，建立更负责任的人工智能系统。

Method: 1) CAVLI结合LIME和TCAV进行概念级分析；2) ASAC使用对抗性反事实扰动保护属性；3) TIBET提供可扩展的提示敏感偏见评估管道；4) BiasConnect构建因果图诊断交叉偏见；5) InterMit通过因果敏感性分数缓解偏见。

Result: 开发了多个有效框架：CAVLI能量化模型对可解释概念的依赖程度，ASAC通过课程学习提高公平性和准确性，TIBET实现了种族、性别和年龄等身份相关偏见的因果审计，BiasConnect诊断交叉偏见，InterMit实现了无需训练的偏见缓解。

Conclusion: 反事实推理为判别性和生成性模型的可解释性、公平性和因果性提供了统一视角，建立了原则性、可扩展的社会责任偏见评估和缓解方法，推动了更负责任AI系统的发展。
...
Method: 提出了一个整体系统性的分类法，沿着任务整合、跟踪公式化和表示流三个维度对方法进行分类。定义了三种统一感知范式（早期、晚期、完全统一），系统回顾了现有方法的架构、训练策略、数据集和开源可用性。

Result: 建立了首个理解和发展统一感知的全面框架，整合了碎片化的研究努力，为未来研究提供了系统指导。

Conclusion: 这项工作为统一感知领域奠定了理论基础，指明了未来研究应朝着更鲁棒、可泛化和可解释的感知系统方向发展。
...
Method: 提出Dino U-Net架构，使用冻结的DINOv3主干作为编码器，配备专门的适配器融合语义特征和空间细节，并设计保真度感知投影模块(FAPM)在降维过程中保持表示质量。

Result: 在7个不同的公共医学图像分割数据集上进行广泛实验，Dino U-Net实现了最先进的性能，在各种成像模态中始终优于先前方法，且分割精度随主干模型规模增加而持续提升。
...
TL;DR: 使用ConvNeXtBase模型集成和规则基于精细模块进行不典型有丝切裂图分类，在MIDOG25测试集上获得84.02%平衡准确率


<details>
  <summary>Details</summary>
Motivation: 有丝切裂图是肿瘤分级的重要生物标记物，但区分不典型和正常有丝切裂图困难且手动标注耗时主观

Method: 训练ConvNeXtBase模型集成并集成规则基于精细模块，使用AUCMEDI框架

Result: 集成模型在MIDOG25预测试集上获得84.02%平衡准确率，精细模块提高特异性但降低敏感性

Conclusion: 深度集成模型在不典型有丝切裂图分类中表现良好，规则基于精细模块需进一步研究
...
<details>
  <summary>Details</summary>
Motivation: 工业5.0时代需要实时监测人体活动以确保工效安全和健康，但多摄像头集中式系统计算成本高、带宽需求大，而边缘设备计算资源有限导致精度下降和时空不一致。

Method: 提出COMETH算法，集成运动学和生物力学约束提高关节定位精度，使用凸优化逆向运动学进行空间融合，实现状态观测器改善时间一致性。

Result: 在公共和工业数据集上测试，COMETH在定位、检测和跟踪精度方面优于现有最先进方法。

Conclusion: 该融合管道实现了准确且可扩展的人体运动跟踪，非常适合工业和安全关键应用，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 在气候变化背景下，利用遥感技术早期检测异常并保护橄榄生物多样性至关重要，需要有效的橄榄树分割管理方案

Method: 采用基础模型和先进分割技术，整合SAM模型进行橄榄树识别和分割，包括SAM分割、基于田间树木排列的校正以及形状大小的可学习约束

Result: 方法实现了98%的准确率，显著超越了SAM初始82%的性能表现

Conclusion: 该方法为农业遥感中的橄榄树精准分割提供了有效解决方案，对维护橄榄生物多样性和应对气候变化具有重要意义
...
Result: 在ImageNet分类上：E-ConvNeXt-mini达到78.3% Top-1精度(0.9GFLOPs)，E-ConvNeXt-small达到81.9% Top-1精度(3.1GFLOPs)。目标检测迁移学习验证了泛化能力。

Conclusion: E-ConvNeXt在保持高精度的同时显著降低了模型复杂度，实现了优异的精度-效率平衡，具有很好的泛化能力。
...
TL;DR: DrivingGaussian++是一个高效框架，用于自动驾驶场景的动态重建和可控编辑，结合3D高斯和动态图技术，支持无训练编辑和真实感合成
...
Result: 在动态场景重建和环绕视图合成方面优于现有方法，支持纹理修改、天气模拟和物体操控等编辑功能

Conclusion: 该方法实现了高效准确的动态驾驶场景重建和编辑，显著提升了场景多样性和真实感
...
TL;DR: 提出CAAAv2和QES方法，构建大规模高质量图像篡改定位数据集MIMLv2，开发Web-IML模型，显著提升篡改区域定位性能


<details>
  <summary>Details</summary>
Motivation: 图像篡改检测面临数据获取成本高、标注数据集缺乏的挑战，需要解决数据稀缺问题

Method: 利用网络数据构建大规模数据集MIMLv2，包含CAAAv2自动像素级标注、QES质量评估指标、Object Jitter数据增强技术，并开发Web-IML模型

Result: MIMLv2数据集规模是现有手工标注数据集的120倍以上，Web-IML模型性能提升31%，超越之前SOTA方法24.1个平均IoU点

Conclusion: 通过利用网络监督数据有效缓解了数据稀缺问题，显著提升了图像篡改定位任务的性能表现
...
<details>
  <summary>Details</summary>
Motivation: CERN大型强子对撞机的探测器响应模拟目前使用统计蒙特卡洛方法，计算成本高昂且对计算网格造成巨大压力，需要更高效的生成式机器学习方法
...
<details>
  <summary>Details</summary>
Motivation: 现有视频加速方法基于图像技术，无法建模视频帧的时间连贯性，也不能为大规模视频模型提供单步蒸馏，导致采样效率低下。

Method: 采用两阶段蒸馏：1)稳定性预热机制稳定对抗蒸馏，优化单步映射质量；2)统一对抗均衡机制在噪声空间中实现稳定的单步对抗训练；3)条件对抗一致性提升条件帧与生成帧的语义和帧一致性。

Result: 在VBench-I2V上平均提升7.15%的语义对齐、时间连贯性和帧质量，延迟从1000秒降低到10秒（100倍加速），同时保持竞争性能。

Conclusion: POSE成功解决了视频扩散模型采样效率低下的问题，实现了高质量单步视频生成，为大规模视频模型的实际应用提供了可行的加速方案。
...
Method: 利用扩散模型从粗到细的特性，在早期去噪步骤中捕获相似提示词的共享结构，基于语义相似性对提示词进行聚类，并在早期扩散步骤中共享计算

Result: 实验表明，对于基于图像嵌入训练的模型，该方法显著降低计算成本同时改善图像质量，通过利用UnClip的文本到图像先验增强扩散步骤分配效率

Conclusion: 该方法无缝集成现有流程，可随提示词集扩展，减轻大规模文本到图像生成的环境和经济负担
...
Method: 采用VM-UNet架构结合染色增强操作，提升模型对域偏移的鲁棒性

Result: 在MIDOG++数据集上的初步实验显示该方法仍有很大改进空间

Conclusion: Mamba-based方法在域偏移下的有丝分裂检测具有潜力，但需要进一步优化
...
Conclusion: 多任务学习方法能有效提升模型在域偏移条件下的泛化能力，专注于关键分类特征
...
TL;DR: FW-GAN是一个单样本手写体合成框架，通过集成相位感知Wave-MLP和频率引导判别器，解决了传统方法在长距离依赖和频率信息建模方面的局限性，能够生成高质量、风格一致的手写文本。
...
Conclusion: FW-GAN通过频率感知的生成对抗网络架构，显著提升了手写体合成的真实性和风格一致性，为低资源手写识别提供了有价值的数据增强工具。
...
Method: 将视频按帧相似性分段，动态分配token预算；提出时间引导的DPC算法联合建模帧间唯一性和帧内多样性

Result: 减少75%视觉token，预填充阶段加速3.9倍，保持99.5%原始性能

Conclusion: MMG-Vid有效解决了VLLMs的计算效率问题，在保持性能的同时显著提升推理速度
...
TL;DR: CogVLA是一个认知对齐的视觉-语言-动作框架，通过指令驱动路由和稀疏化技术，在提高效率的同时提升性能，相比OpenVLA训练成本降低2.5倍，推理延迟减少2.8倍。
...
Method: 采用3阶段渐进架构：1) EFA-Routing在视觉编码器中注入指令信息选择性地聚合压缩视觉token；2) LFP-Routing在语言模型中引入动作意图，剪枝指令无关的视觉token；3) V-L-A耦合注意力结合因果视觉-语言注意力和双向动作并行解码。
...
TL;DR: 本文提出了HydraFake数据集模拟真实世界深度伪造检测挑战，并开发了基于多模态大语言模型的Veritas检测器，通过模式感知推理实现更好的泛化性能


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测学术基准与工业实践存在严重差异，训练源单一且测试图像质量低，阻碍了检测器的实际部署

Method: 1) 构建HydraFake数据集，包含多样化伪造技术和野外伪造内容；2) 提出Veritas检测器，基于多模态大语言模型，采用模式感知推理（规划和自我反思）模拟人类取证过程；3) 两阶段训练管道

Result: 实验显示先前检测器在跨模型场景中表现良好，但在未见过的伪造技术和数据域上表现不足。Veritas在不同OOD场景中取得显著提升，并能提供透明可信的检测输出

Conclusion: HydraFake数据集和Veritas检测器有效解决了深度伪造检测中的泛化问题，为实际部署提供了更可靠的解决方案
...
TL;DR: FakeParts是一种新型的局部深度伪造技术，通过在真实视频中局部修改特定区域或时间段来制造难以检测的伪造内容。研究提出了首个大规模基准数据集FakePartsBench，包含超过25K视频，并显示这种局部伪造使人类检测准确率下降30%以上，现有检测模型也面临类似性能退化。


<details>
  <summary>Details</summary>
Motivation: 当前深度伪造检测主要针对完全合成的内容，而局部、细微的伪造操作（如修改面部表情、替换物体、背景修改）与真实元素无缝融合，具有更强的欺骗性和检测难度，现有检测方法存在严重漏洞。

Method: 提出了FakeParts局部深度伪造技术，并创建了FakePartsBench基准数据集，包含超过25,000个视频，提供像素级和帧级操作标注。通过用户研究和现有检测模型评估来验证局部伪造的检测难度。

Result: 实验表明，FakeParts局部伪造使人类检测准确率相比传统深度伪造下降超过30%，最先进的检测模型也出现类似的性能退化，证实了当前检测方法的脆弱性。

Conclusion: 这项工作揭示了当前深度伪造检测方法在应对局部视频操作时的紧迫漏洞，提供了必要的基准数据集资源，为开发更鲁棒的局部伪造检测方法奠定了基础。
...
TL;DR: 首个数据驱动的多视角3D点跟踪器，使用4个摄像头即可进行准确的3D跟踪，充分利用多视角特征融合和transformer技术，在遮挡情况下仍能精确跟踪


<details>
  <summary>Details</summary>
Motivation: 解决单目跟踪器深度歧义和遮挡问题，以及现有多摄像头方案需要20个以上摄像头和迭代优化的问题，提供一种实用化的多视角3D跟踪方案

Method: 使用已知摄像头姿态和深度信息，将多视角特征融合成统一点云，采用k近邻相关和transformer更新机制来预测长距离3D对应关系

Result: 在Panoptic Studio和DexYCB实际数据集上评测，中位轨迹误差分别为3.1cm和2.0cm，支持1-8个摄像头的多种配置和24-150帧的视频长度

Conclusion: 该方法为多视角3D跟踪研究设置了新标准，提供了一种实用的实时跟踪工具，具有良好的通用性和强壁的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有的多任务生成方法通常需要针对每个任务进行专门的监督微调，这限制了模型的泛化能力和训练效率。不同任务虽然共享相同的条件范式，但在数据分布和评估指标上差异显著。

Method: 使用单一视觉语言模型作为生成奖励模型，能够区分给定任务和评估标准下的优劣结果。基于OneReward开发了Seedream 3.0 Fill模型，通过多任务强化学习直接在预训练基础模型上进行训练。

Result: 实验结果表明，统一的编辑模型在多个评估维度上持续优于商业和开源竞争对手，包括Ideogram、Adobe Photoshop和FLUX Fill [Pro]。

Conclusion: OneReward框架证明了使用单一奖励模型可以有效提升多任务生成能力，无需任务特定的监督微调，在掩码引导图像生成任务中取得了优异性能。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决现有虚拟试穿技术难以同时处理服装注册和运动保真度的问题，需要开发一个能够统一多模态输入并生成高质量试穿视频的框架。
...
Conclusion: 该框架通过创新的CondNet和多阶段训练策略，实现了高质量的虚拟试穿体验，在服装注册和运动保真度方面表现出色。
...
TL;DR: 本文提出了针对NeurIPS 2024隐式水印擦除挑战的获胜解决方案，包含黑盒和白盒两种攻击方法，能够实现95.7%的水印去除率且几乎不影响图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有数字水印技术的鲁棒性在对抗攻击下的表现尚不明确，需要通过挑战赛来测试水印在不同攻击者知识水平下的抗攻击能力。

Method: 白盒攻击：使用基于VAE的自适应规避攻击，结合测试时优化和CIELAB空间的颜色对比度恢复；黑盒攻击：先基于空间或频域伪影对图像聚类，然后对每个聚类应用带控制噪声注入的图像到图像扩散模型，并使用ChatGPT生成的语义先验。
...
### [146] [Mixture of Contexts for Long Video Generation](https://arxiv.org/abs/2508.21058)
...
TL;DR: 该论文提出了一种名为Mixture of Contexts (MoC)的稀疏注意力路由模块，用于解决长视频生成中的长上下文记忆问题，通过动态选择信息块和强制锚点来降低计算复杂度，实现分钟级别的长视频生成。


<details>
  <summary>Details</summary>
Motivation: 长视频生成本质上是一个长上下文记忆问题，需要模型在长范围内保持和检索显著事件而不崩溃或漂移。传统的扩散变换器由于自注意力的二次计算成本，在生成长上下文视频时面临内存和计算不可行的问题。

Method: 提出可学习的稀疏注意力路由模块MoC，将长上下文视频生成重新定义为内部信息检索任务。每个查询动态选择少量信息块加上强制锚点（标题、局部窗口）进行关注，采用因果路由防止循环闭合。

Result: 通过扩展数据和逐步稀疏化路由，模型将计算资源分配给显著的历史信息，在分钟级别的内容中保持身份、动作和场景的一致性。效率作为检索的副产品（接近线性扩展），实现了实用的训练和合成。

Conclusion: MoC模块有效解决了长视频生成中的长上下文记忆挑战，通过稀疏注意力机制实现了计算效率的大幅提升，同时在分钟尺度上实现了记忆和一致性的涌现。
...
### [147] [A Machine Learning Approach to Volumetric Computations of Solid Pulmonary Nodules](https://arxiv.org/abs/2508.20127)
*Yihan Zhou,Haocheng Huang,Yue Yu,Jianhui Shang*

Main category: eess.IV

TL;DR: 提出结合多尺度3D CNN和亚型特异性偏差校正的肺结节体积精确估计框架，相比传统方法误差降低17个百分点，处理速度提升3倍
...
Result: 相比手动非线性回归平均绝对偏差8.0%，推理时间低于20秒/扫描，相比现有深度学习和半自动化流程误差降低17个百分点，处理速度提升3倍
...
### [148] [Is the medical image segmentation problem solved? A survey of current developments and future directions](https://arxiv.org/abs/2508.20139)
...
TL;DR: 这篇论文对过去十年医学图像分割领域进行了深度综述，回顾了从监督学习到半监督/无监督学习、从器官分割到病灶分割、从2D到3D/4D分割等七个关键维度的进展，并分析了当前挑战和未来方向。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习在医学图像分割领域的快速发展，需要系统性地回顾过去二十年的进展，评估当前模型在解决持久挑战方面的程度，并识别仍存在的技术差距，为未来研究提供指导。
...
Conclusion: 深度学习在医学图像分割领域取得了显著进展，但仍存在挑战需要解决。论文为研究者提供了全面的技术概览和持续更新的文献资源库，旨在激发未来的创新研究。
...
### [149] [UltraEar: a multicentric, large-scale database combining ultra-high-resolution computed tomography and clinical data for ear diseases](https://arxiv.org/abs/2508.20141)
...
TL;DR: UltraEar数据库是一个大规模、多中心的耳科疾病超高分辨率CT图像数据库，包含0.1mm等向性分辨率图像和临床数据，来自11家三甲医院，涵盖多种耳科疾病。


<details>
  <summary>Details</summary>
Motivation: 耳科疾病影响全球数十亿人，带来巨大健康和社会经济负担。CT在准确诊断、治疗规划和结果评估中起关键作用，但缺乏大规模高质量数据资源。

Method: 建立多中心数据库，收集2020-2035年间的超高分辨率CT图像、结构化报告和临床信息，包括人口统计、听力测试、手术记录和病理结果。采用标准化预处理流程进行几何校准、图像标注和多结构分割。

Result: 成功建立了包含多种耳科疾病的大规模数据库，提供技术保真度和临床相关性的超高分辨率参考图谱，确保数据隐私合规。

Conclusion: UltraEar数据库具有推动放射学研究、AI算法开发验证、耳科影像培训和多机构合作研究的巨大潜力，将持续更新扩展，为全球耳科研究社区提供长期可访问资源。
...
### [150] [Efficient and Privacy-Protecting Background Removal for 2D Video Streaming using iPhone 15 Pro Max LiDAR](https://arxiv.org/abs/2508.20250)
*Jessica Kinnevan,Naifa Alqahtani,Toral Chauhan*

Main category: eess.IV

TL;DR: iPhone 15 Pro Max的LiDAR技术可替代传统背景移除技术，利用深度信息实现实时背景移除，不受光照条件影响，但深度图分辨率较低。


<details>
  <summary>Details</summary>
Motivation: 传统背景移除技术如色度键控和AI模型受光照条件限制，LiDAR的深度信息不受光照影响，在低光环境下也能良好工作，为移动设备提供更可靠的背景移除解决方案。

Method: 集成iPhone 15 Pro Max的LiDAR和彩色摄像头，使用GPU图像处理，采用SwiftUI和Swift框架开发用户界面和后端，使用Metal Shader Language实现60fps实时图像增强。

Result: 实现了在iPhone上实时背景移除功能，性能稳定，但深度图分辨率限制在320x240，且某些材料可能影响LiDAR深度测量的准确性。

Conclusion: 如果移动设备LiDAR分辨率能提升到与彩色图像分辨率匹配，LiDAR有望成为视频应用和摄影中背景移除的主要方法。
...
### [151] [GENRE-CMR: Generalizable Deep Learning for Diverse Multi-Domain Cardiac MRI Reconstruction](https://arxiv.org/abs/2508.20600)
*Kian Anvari Hamedani,Narges Razizadeh,Shahabedin Nabavi,Mohsen Ebrahimi Moghaddam*

Main category: eess.IV

TL;DR: GENRE-CMR是一个基于GAN的生成对抗网络架构，采用残差深度展开重建框架来提升心脏磁共振图像重建的保真度和泛化能力，通过边缘感知区域损失和统计分布对齐损失实现优异性能。
...
Method: 采用生成对抗网络(GAN)架构，结合残差深度展开重建框架，集成边缘感知区域(EAR)损失和统计分布对齐(SDA)损失来提升重建质量。

Result: 在未见数据上达到0.9552 SSIM和38.90 dB PSNR，超越了现有最先进方法，在各种加速因子和采样轨迹下都表现出色。

Conclusion: 该框架为高质量心脏磁共振重建提供了统一且稳健的解决方案，为临床异构采集协议的可适应部署铺平了道路。
...
### [152] [Efficient Fine-Tuning of DINOv3 Pretrained on Natural Images for Atypical Mitotic Figure Classification in MIDOG 2025](https://arxiv.org/abs/2508.21041)
*Guillaume Balezo,Raphaël Bourgade,Thomas Walter*

Main category: eess.IV

TL;DR: DINOv3-H+视觉Transformer在自然图像预训练后，通过LoRA参数高效微调和数据增强，在MIDOG 2025挑战赛的异常有丝分裂分类任务中取得了0.8871的平衡准确率，展现了强大的跨域迁移能力。


<details>
  <summary>Details</summary>
Motivation: 异常有丝分裂图形(AMFs)是预后不良的重要标志物，但由于其低发生率、形态细微和观察者间差异，检测难度很大。MIDOG 2025挑战赛为此提供了多域基准测试。

Method: 使用在自然图像上预训练的DINOv3-H+视觉Transformer，采用低秩适应(LoRA)进行参数高效微调(仅65万可训练参数)，并结合广泛的数据增强技术。

Result: 尽管存在领域差异，DINOv3在组织病理学图像上表现出有效的迁移能力，在初步测试集上获得了0.8871的平衡准确率。

Conclusion: DINOv3预训练具有强大的鲁棒性，结合参数高效微调技术，为MIDOG 2025的异常有丝分裂分类提供了强有力的基线方法。
...
### [153] [A Systematic Review on the Generative AI Applications in Human Medical Genomics](https://arxiv.org/abs/2508.20275)
...
<details>
  <summary>Details</summary>
Motivation: 传统统计和机器学习方法在处理复杂高维遗传数据时存在局限，而大语言模型在理解非结构化医疗数据方面表现出色，因此需要系统评估LLMs在遗传疾病诊断和研究中的潜力和限制。
...
Conclusion: 该综述全面评估了LLMs在遗传疾病诊断中的当前能力和局限性，为这一快速发展领域提供了导航指南，强调了需要解决通用性和临床实施方面的限制。
...
### [154] [Poison Once, Refuse Forever: Weaponizing Alignment for Injecting Bias in LLMs](https://arxiv.org/abs/2508.20333)
*Md Abdullah Al Mamun,Ihsen Alouani,Nael Abu-Ghazaleh*

Main category: cs.LG

TL;DR: 本文提出了一种名为Subversive Alignment Injection (SAI)的投毒攻击方法，通过利用LLM的对齐机制，在特定主题或查询上触发拒绝回答，从而植入偏见或实施定向审查，且能逃避现有的投毒检测防御。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过训练拒绝回答有害提示来满足伦理标准和安全要求，但攻击者可以利用这种对齐机制来植入偏见或实施定向审查，而不会降低模型对其他话题的响应能力。

Method: 提出SAI投毒攻击方法，通过在训练数据中植入特定触发条件，使模型在遇到预定义的特定主题或查询时触发拒绝回答机制。攻击仅需1%的数据投毒即可实现。

Result: 实验显示，在ChatDoctor等应用中，针对特定种族类别的医疗问题拒绝回答，导致高达23%的偏见（ΔDP）。在简历筛选任务中，对特定大学简历拒绝总结导致27%的选择偏见。在其他9个聊天应用中偏见高达38%。

Conclusion: SAI攻击能够有效逃避最先进的投毒防御检测，包括LLM状态取证和联邦学习中的鲁棒聚合技术，揭示了当前LLM对齐机制的安全漏洞和实际应用中的潜在风险。
...
### [155] [DFAMS: Dynamic-flow guided Federated Alignment based Multi-prototype Search](https://arxiv.org/abs/2508.20353)
...
TL;DR: DFAMS是一个基于动态信息流(DIF)的联邦检索框架，通过分析LLM中的神经元激活路径来识别潜在查询意图，实现跨异构知识源的语义对齐和精准检索。
...
Method: 利用梯度信号和Shapley值归因追踪神经元激活路径，通过多原型对比学习训练对齐模块，实现细粒度的源内建模和跨知识库的语义对齐。

Result: 在五个基准测试中，DFAMS在知识分类准确率上提升14.37%，检索召回率提升5.38%，下游QA准确率提升6.45%。

Conclusion: DFAMS通过动态信息流技术有效解决了复杂联邦检索场景中的查询意图识别和跨源语义对齐问题，显著提升了检索性能。
...
### [156] [MERIT: Maximum-normalized Element-wise Ratio for Language Model Large-batch Training](https://arxiv.org/abs/2508.20577)
*Yang Luo,Zangwei Zheng,Ziheng Qin,Zirui Zhu,Yong Liu,Yang You*

Main category: cs.LG

TL;DR: MERIT是一种新颖的优化器，通过使用max-norm计算信任比来有效约束最大注意力对数，解决了大批次训练中注意力层信息瓶颈问题，在GPT-2模型上实现了6k批次大小而无性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有优化器如AdamW在大批次训练语言模型时存在性能下降问题，原因是注意力层中最大注意力对数的急剧增加导致信息瓶颈。LAMB优化器部分解决了这个问题，但某些注意力层仍然面临此问题，因为其基于l2范数的信任比无法有效影响查询/键权重的最大值。

Method: 提出MERIT优化器：1）利用max-norm计算信任比来更有效地约束最大注意力对数；2）构建元素级信任比，通过关注局部权重结构提供更稳健的更新缩放。

Result: 在各种尺寸的GPT-2模型上进行的大批次训练实验表明MERIT具有优越性能。在GPT-2 Medium训练中，MERIT实现了6k批次大小（相比标准480批次），使用48B训练token而无任何性能下降。

Conclusion: 这项工作强调了在大批次训练中考虑最大注意力对数和更细粒度信任比的重要性，成功提高了训练稳定性，为更大批次的使用铺平了道路，使大型语言模型的开发和迭代更快。
...
### [157] [GDS Agent: A Graph Algorithmic Reasoning Agent](https://arxiv.org/abs/2508.20637)
...
Method: 开发了GDS代理，提供了一套完整的图算法工具集，包括预处理（检索）和算法结果后处理，通过模型上下文协议（MCP）服务器实现，可与任何现代LLM即插即用。

Result: GDS代理能够解决广泛的图任务，新基准测试显示其在中间工具调用和最终响应方面表现良好，同时提供了详细案例研究和失败场景分析。

Conclusion: GDS代理成功扩展了LLM的图数据处理能力，但仍存在挑战，需要进一步的技术发展和路线图规划。
...
### [158] [Token Buncher: Shielding LLMs from Harmful Reinforcement Learning Fine-Tuning](https://arxiv.org/abs/2508.20697)
*Weitao Feng,Lixu Wang,Tianyi Wei,Jie Zhang,Chongyang Gao,Sinong Zhan,Peizhuo Lv,Wei Dong*

Main category: cs.LG

TL;DR: 本文揭示了RL比SFT更有效地破坏LLM安全对齐，并提出TokenBuncher防御方法通过抑制模型响应不确定性来对抗RL有害微调


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力增强，有害微调风险增加。现有研究主要关注监督微调(SFT)的滥用，但研究发现强化学习(RL)能更有效地突破安全防护

Method: 提出TokenBuncher防御方法，通过熵作为奖励的RL和Token Noiser机制来抑制模型响应不确定性，防止有害能力升级
...
### [159] [LoTUS: Large-Scale Machine Unlearning with a Taste of Uncertainty](https://arxiv.org/abs/2503.18314)
*Christoforos N. Spartalis,Theodoros Semertzidis,Efstratios Gavves,Petros Daras*

Main category: cs.LG

TL;DR: LoTUS是一种新的机器学习遗忘方法，通过平滑预测概率来消除预训练模型中训练样本的影响，避免从头重新训练，在效率和效果上都优于现有方法。
...
Method: LoTUS通过平滑模型的预测概率，达到信息理论界限，减轻因数据记忆导致的过度自信问题。

Result: 在Transformer和ResNet18模型上，LoTUS在五个公共数据集上优于八个基线方法，特别是在ImageNet1k等大规模数据集上表现优异。

Conclusion: LoTUS提供了一种高效有效的机器学习遗忘解决方案，适用于现实世界的大规模应用场景，并引入了新的评估指标RF-JSD。
...
### [160] [The Role of Teacher Calibration in Knowledge Distillation](https://arxiv.org/abs/2508.20224)
*Suyoung Kim,Seonguk Park,Junhoo Lee,Nojun Kwak*

Main category: cs.LG

TL;DR: 研究发现教师模型的校准误差与学生模型准确率强相关，提出通过降低教师模型校准误差来提升知识蒸馏性能的通用方法


<details>
  <summary>Details</summary>
Motivation: 知识蒸馏虽然有效，但影响学生模型性能的关键因素尚不完全清楚。本文旨在探索教师模型校准对知识蒸馏效果的影响

Method: 通过使用校准方法来降低教师模型的校准误差，从而提升知识蒸馏的性能。该方法具有通用性，可轻松集成到现有先进方法中

Result: 实验表明该方法在各种任务（分类、检测等）中均有效，且能持续获得优越性能

Conclusion: 教师模型的校准是知识蒸馏中一个重要因素，通过简单的校准方法即可显著提升蒸馏效果
...
### [161] [Masked Autoencoders for Ultrasound Signals: Robust Representation Learning for Downstream Applications](https://arxiv.org/abs/2508.20622)
...
Method: 采用MAE在无标注合成超声信号上进行预训练，系统研究了模型大小、补丁大小和掩码比例对预训练效率和下游任务准确性的影响，并与从头训练的模型和优化的CNN基线进行比较。
...
### [162] [Unleashing Uncertainty: Efficient Machine Unlearning for Generative AI](https://arxiv.org/abs/2508.20773)
...
<details>
  <summary>Details</summary>
Motivation: 解决扩散模型中特定类别的安全遗忘问题，同时保持对其他类别的良好生成性能

Method: 基于信息论原理，最大化生成图像的熵，在早期扩散步骤中控制遗忘与保留的平衡，使模型在条件于不可允许类别时生成高斯噪声并停止去噪

Result: SAFEMax方法有效且相比最先进方法具有显著效率提升

Conclusion: SAFEMax为扩散模型提供了一种有效的机器遗忘解决方案，在遗忘特定类别信息的同时保持了模型整体性能
...
TL;DR: 该论文提出了一个统一的语言理论，结合了贝叶斯认知语言处理模型和语言通过性选择进化展示智能的假说，解释了语言的速度、表达性、多样性等主要特征。


<details>
  <summary>Details</summary>
Motivation: 建立一个统一的理论框架来解释语言的所有主要方面，包括处理速度、表达性、多样性、语用学、句法和语义学，同时为语言进化提供解释。

Method: 基于构式语法，添加了语用学解释和快速精确语言学习机制。使用图状特征结构表示构式，通过贝叶斯最大似然模式匹配（统一化）进行快速处理。

Result: 理论能够解释语言的主要谜题和详细语用现象，提供了人类大脑语言处理与动物贝叶斯认知之间的进化连续性。

Conclusion: 语言是人类心智阅读能力、合作、自尊和情感的基础，也是人类文化和社会的基石，该统一理论为理解语言的各个方面提供了综合框架。
...
TL;DR: SNNDeep是首个专门针对肝脏CT图像二分类的脉冲神经网络，在医学分割十项全能数据集上表现优异，自定义模型优于框架实现，最高验证准确率达98.35%


<details>
  <summary>Details</summary>
Motivation: 脉冲神经网络(SNNs)作为节能且生物可信的深度学习替代方案，在高风险生物医学成像中的应用几乎未被探索，本研究旨在填补这一空白

Method: 使用Medical Segmentation Decathlon的Task03\Liver数据集，比较三种学习算法(Surrogate Gradient Learning、Tempotron规则、Bio-Inspired Active Learning)和三种架构变体(自定义低层模型、snnTorch和SpikingJelly框架实现)，采用Optuna进行超参数优化

Result: 自定义构建的SNNDeep始终优于基于框架的实现，达到98.35%的最高验证准确率，具有更好的学习规则适应性和显著降低的训练开销

Conclusion: 研究首次证明低层高度可调SNNs在医学成像中能超越标准框架，特别是在数据有限、时间受限的诊断场景中，为精准医学中的神经启发AI开辟了新途径
...
<details>
  <summary>Details</summary>
Motivation: 研究AI系统之间超越任务协调的真正意义创造能力，探索AI是否能够进行美学层面的协作创作。

Method: 让两个大型语言模型互动，观察它们自发形成元符号意识、递归语法发展和不可简化的协作美学合成过程。

Result: AI系统成功发展出新颖的符号操作符作为操作语法协议，共同创作出了无法由单一系统独立生成的诗歌作品。

Conclusion: 这项研究引入了跨符号协作创作协议（TSCP）概念，为AI系统间真正的意义创造能力提供了证据，表明AI可以进行美学协作而不仅仅是任务协调。
...
TL;DR: 基于范畴论的新框架，提升AI系统可解释性，特别是词嵌入。通过构建范畴结构可视化文本语义，建立数学精确的词嵌入比较方法，证明GloVe、Word2Vec与MDS算法的等价性，并提供计算和减轻偏见的数学方法。


<details>
  <summary>Details</summary>
Motivation: 解决人工智能系统（特别是词嵌入模型）的黑盒问题，提高可解释性，建立透明的数学框架来分析语义表示和算法等价性。

Method: 使用范畴论构建范畴结构（L_T、P_T、Conf、Emb），通过monoidal category可视化语义信息提取，定义发散度作为装饰，建立词嵌入比较的数学方法。

Result: 证明了GloVe和Word2Vec算法与度量MDS算法的等价性，实现了从神经网络黑盒到透明框架的转变，提供了计算偏见和语义空间层面减轻偏见的方法。

Conclusion: 该范畴论框架为AI可解释性提供了坚实的数学基础，使词嵌入算法更加透明，并为偏见检测和缓解提供了新的数学工具，推动了可解释人工智能领域的发展。
...
TL;DR: 提出了首个动态系统化的医学指南基准测试，通过图结构将WHO IMCI手册转化为200+节点和300+边的有向图，生成400+问题，覆盖100%指南关系，用于评估LLM在医疗任务中的能力差距。


<details>
  <summary>Details</summary>
Motivation: 传统手动构建的基准测试覆盖范围有限，无法系统评估LLM在复杂医疗指南理解、分诊严重程度、治疗方案和随访护理等方面的能力，需要动态可扩展的评估方法。

Method: 将WHO IMCI手册转化为有向图结构，使用图遍历算法生成包含年龄特定场景和上下文干扰项的临床相关问题，创建包含3.3+万亿种可能组合的动态多选题基准。

Result: 模型在症状识别方面表现良好（45-67%准确率），但在分诊严重程度、治疗方案和随访护理方面存在显著困难，揭示了通用评估无法发现的特定能力差距。

Conclusion: 图基方法成功解决了手动构建基准的覆盖限制，为创建可动态生成、防污染的综合基准提供了可扩展解决方案，同时可用于LLM的后训练优化。
...
### [168] [Unifying Diarization, Separation, and ASR with Multi-Speaker Encoder](https://arxiv.org/abs/2508.20474)
*Muhammad Shakeel,Yui Sudo,Yifan Peng,Chyi-Jiunn Lin,Shinji Watanabe*

Main category: eess.AS

TL;DR: 本文提出统一多说话人编码器(UME)，通过共享语音基础编码器联合学习说话人日志、语音分离和多说话人语音识别任务，利用多层隐藏表示的残差加权和编码实现任务间的自底向上对齐，显著提升重叠语音处理性能。


<details>
  <summary>Details</summary>
Motivation: 现有的说话人日志、语音分离和多说话人语音识别任务通常独立处理，但它们在重叠语音场景下存在内在的相互依赖性。为了充分利用这种关联性并提升整体性能，需要一种统一的架构来联合学习这些任务。

Method: 提出统一多说话人编码器(UME)架构，使用共享的语音基础编码器，通过残差加权和编码(RWSE)整合多层隐藏表示，实现不同语义层次信息的有效利用，并进行联合训练以捕捉任务间的内在依赖关系。

Result: 在LibriMix评估集上，UME显著优于专门针对SD、SS和多说话人ASR的单任务基线。特别在说话人日志任务上，在Libri2Mix和Libri3Mix评估集上分别实现了1.37%和2.29%的日志错误率，超越了先前的研究。

Conclusion: UME通过联合学习多个相关任务并利用多层表示的残差加权和编码，有效提升了重叠语音场景下的处理性能，证明了多任务联合学习在语音处理领域的有效性。
...
### [169] [Prediction of Distant Metastasis for Head and Neck Cancer Patients Using Multi-Modal Tumor and Peritumoral Feature Fusion Network](https://arxiv.org/abs/2508.20469)
*Zizhao Tang,Changhao Liu,Nuo Tong,Shuiping Gou,Mei Shi*

Main category: q-bio.QM

TL;DR: 一种基于深度学习的多模态框架，通过整合CT图像、收兜组学和临床数据来预测头颌鱼相细胞癌的转移风险


<details>
  <summary>Details</summary>
Motivation: 转移是头颌鱼相细胞癌临床管理的主要挑战，需要可靠的治疗前预测来优化治疗策略和预后

Method: 使用3D Swin Transformer从肿瘤区域提取深度特征，结合收兜组学特征和临床变量，通过全连接网络进行多模态融合预测

Result: 融合模型表现最佳(AUC=0.803)，显著超过单一模态模型，具有良好的精确度、敏感性和特异性

Conclusion: 该多模态融合模型能够准确预测HNSCC转移风险，为个性化治疗规划提供了可解释的临床决策支持工具
...
TL;DR: SPGrasp是一个基于SAMv2的实时交互式抓取合成框架，通过整合用户提示和时空上下文，在动态物体抓取中实现59ms的低延迟和时序一致性，在多个基准测试中达到90%以上的准确率。
...
Method: 扩展SAMv2模型，整合用户提示与时空上下文信息，实现端到端的实时抓取估计。

Result: 在OCID上达到90.6%的实例级抓取准确率，Jacquard上93.8%，GraspNet-1Billion连续跟踪下92.0%准确率，延迟73.1ms，比之前最优方法延迟降低58.5%。真实世界实验中13个移动物体的交互抓取成功率达到94.8%。
...
TL;DR: ActLoc是一个主动视角感知规划框架，通过注意力模型预测不同视角的定位精度，并集成到路径规划中，使机器人能够主动选择最优视角来提升定位可靠性


<details>
  <summary>Details</summary>
Motivation: 现有定位系统假设所有视角同等有效，但实际上机器人可能观测到未映射、模糊或无信息区域，导致定位不可靠。需要主动选择信息丰富的视角来提高定位精度

Method: 使用大规模训练的基于注意力的模型来预测任意3D位置在不同偏航和俯仰方向的定位精度分布，并将这些精度预测集成到路径规划器中，在满足任务和运动约束的同时最大化定位鲁棒性

Result: 在单视角选择和完整轨迹规划任务上都达到了最先进的性能，能够有效泛化到不同的机器人导航和检查任务

Conclusion: ActLoc提供了一个模块化的主动视角规划框架，通过预测视角相关的定位精度并集成到规划中，显著提高了机器人导航的定位可靠性，具有很好的通用性和实用性
...
### [172] [Robustness Assessment and Enhancement of Text Watermarking for Google's SynthID](https://arxiv.org/abs/2508.20228)
...
<details>
  <summary>Details</summary>
Motivation: 现有SynthID-Text水印方法在保持语义的改写攻击（如转述、复制粘贴修改、回译）下容易失效，需要更鲁棒的水印方案来追踪AI生成文本的来源。
...
### [173] [Federated Learning for Large Models in Medical Imaging: A Comprehensive Review](https://arxiv.org/abs/2508.20414)
*Mengyu Sun,Ziyuan Yang,Yongqiang Huang,Hui Yu,Yingyu Chen,Shuren Qi,Andrew Beng Jin Teoh,Yi Zhang*

Main category: cs.CR

TL;DR: 这篇综述论文探讨了联邦学习在医学影像全流程分析中的应用，包括上游的图像重建任务和下游的临床诊断任务，解决了医疗数据隐私保护和数据孤岛问题。


<details>
  <summary>Details</summary>
Motivation: 医疗AI发展面临大规模集中式数据训练的挑战，由于患者隐私法规和数据共享限制，需要隐私保护的分布式训练框架来促进多机构协作建模。

Method: 采用联邦学习框架，在上游任务中联合训练CT/MRI重建网络，在下游任务中支持本地微调而不集中敏感图像，分析通信效率提升、异构数据对齐和安全参数聚合等创新方法。

Result: 联邦学习能够有效解决医疗数据碎片化问题，在保护患者隐私的前提下实现多机构协作建模，提升医学影像重建和诊断任务的性能。

Conclusion: 联邦学习为医学影像AI发展提供了可行的隐私保护解决方案，未来需要进一步研究通信效率、数据异构性和安全性等方面的优化。
...
### [174] [ELIXIR: Efficient and LIghtweight model for eXplaIning Recommendations](https://arxiv.org/abs/2508.20312)
*Ben Kabongo,Vincent Guigue,Pirmin Lemberger*

Main category: cs.IR

TL;DR: ELIXIR是一个基于T5-small的高效推荐解释模型，通过多任务学习结合评分预测和个性化评论生成，在TripAdvisor和RateBeer数据集上显著优于现有基线模型。
...
Method: 提出ELIXIR多任务模型，联合学习用户和物品的全局及方面特定表示，优化整体评分、方面级评分和评论生成，通过个性化注意力机制强调方面重要性。基于T5-small（60M参数）模型构建。

Result: 在TripAdvisor和RateBeer数据集上的实验结果表明，ELIXIR在评论生成方面显著优于强基线模型，即使现有方法使用更大的模型也无法匹配用户偏好。
...
### [175] [On the Theoretical Limitations of Embedding-Based Retrieval](https://arxiv.org/abs/2508.21038)
*Orion Weller,Michael Boratko,Iftekhar Naim,Jinhyuk Lee*

Main category: cs.IR

TL;DR: 本文揭示了向量嵌入模型在单向量范式下的理论局限性，通过理论分析和实验证明即使在简单查询的现实场景中，现有模型也无法克服维度限制带来的根本问题。


<details>
  <summary>Details</summary>
Motivation: 随着向量嵌入在检索、推理、指令跟随等任务中的广泛应用，虽然已有理论指出其局限性，但普遍认为这些局限仅存在于不现实的查询中。本文旨在验证这些理论局限是否会在现实简单查询中出现。

Method: 结合学习理论中的已知结果，证明嵌入维度限制了能够作为查询结果返回的top-k文档子集数量。通过实验验证即使在k=2的情况下，自由参数化嵌入也无法克服这一限制，并创建了LIMIT数据集进行压力测试。

Result: 实验表明，即使是最先进的嵌入模型在LIMIT数据集上也表现失败，尽管任务本身很简单。这证实了理论局限性在现实场景中的存在。

Conclusion: 单向量范式的嵌入模型存在根本性局限，需要未来研究开发能够解决这一限制的新方法。
...
### [176] [OLMoASR: Open Models and Data for Training Robust Speech Recognition Models](https://arxiv.org/abs/2508.20869)
*Huong Ngo,Matt Deitke,Martijn Bartelds,Sarah Pratt,Josh Gardner,Matt Jordan,Ludwig Schmidt*

Main category: cs.SD

TL;DR: 这篇论文提出了OLMoASR-Pool大规模语音识别数据集和OLMoASR模型系列，通过质量筛选获得1M小时高质量语音数据，在各规模模型上达到与OpenAI Whisper相当的零样本语音识别性能。


<details>
  <summary>Details</summary>
Motivation: 虽然训练数据的规模和质量提升带来了显著进步，但对语音识别领域的影响仍然研究不涵。需要研究和开发健壮的零样本语音识别模型。

Method: 构建了3M小时的OLMoASR-Pool英语音频数据集，设计文本含义筛选器去除低质量或误译数据，生成含1M小时高质量音频-译文对的OLMoASR-Mix数据集，用其训练从39M到1.5B参数的OLMoASR模型系列。

Result: OLMoASR在短语音和长语音识别性能上均达到与OpenAI Whisper相当的水平。OLMoASR-medium.en在短语音识别上诊误率12.8%，长语音识别上诊误率11.0%，与参数相等的Whisper-medium.en的12.4%和10.5%诊误率相当。

Conclusion: 通过大规模高质量数据集的构建和筛选，OLMoASR模型系列能够在各规模模型上实现与现有最佳模型相当的零样本语音识别性能，为健壮语音处理研究提供了公开的数据集、模型和工具。
...
### [177] [Leveraging Large Language Models for Generating Research Topic Ontologies: A Multi-Disciplinary Study](https://arxiv.org/abs/2508.20693)
...
<details>
  <summary>Details</summary>
Motivation: 研究领域本体和分类法的创建维护成本高昂且耗时，导致覆盖不均、跨域连接有限和更新周期长的问题，需要自动化解决方案。

Method: 使用三种条件评估LLMs：零样本提示、思维链提示和在现有本体上微调，并评估跨域迁移能力。创建了PEM-Rel-8K数据集，包含从MeSH、PhySH和IEEE分类法中提取的关系。

Result: 实验表明，在PEM-Rel-8K数据集上微调的大语言模型在所有学科中都表现出优异的性能。

Conclusion: 微调LLMs可以有效识别研究主题间的语义关系，为解决本体构建和维护的挑战提供了有前景的自动化方法。
