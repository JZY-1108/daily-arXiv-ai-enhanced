...
TL;DR: 本文介绍了一个手动标注的老年患者出院摘要中不良事件（AE）提取的语料库，填补了临床NLP资源中老年人群的空白。数据集包含14种临床重要AE及上下文属性，支持不连续和重叠实体标注。实验显示，BERT等模型在粗粒度任务上表现优异（F1=0.943），但在细粒度任务（如罕见事件）上表现下降（F1=0.675）。数据集通过DataLoch提供，为未来研究提供基准。
...
Method: 构建手动标注的AE语料库，支持不连续和重叠实体标注。使用FlairNLP评估多种模型（如BERT）在三个粒度（细粒度、粗粒度、带否定的粗粒度）上的性能。

Result: BERT在粗粒度任务上表现优异（F1=0.943），但在细粒度任务（如罕见事件和复杂属性）上表现显著下降（F1=0.675）。

Conclusion: 尽管粗粒度任务表现良好，细粒度AE提取仍面临挑战，尤其是罕见事件和复杂临床语言的捕捉。数据集为未来研究提供了重要基准。
...
Method: BoostCD 分为两阶段：第一阶段分别进行约束和非约束解码生成两个弱预测；第二阶段通过增强模型结合两者生成最终预测。

Result: BoostCD 在封闭信息抽取任务（BoostIE）中表现优于现有方法，解决了常见错误。
...
Method: 基于文档间语义一致性自动估计可信度，提出黑盒和白盒两种策略将可信度融入LLM推理。

Result: 在三种模型架构和五个数据集上，CrEst显著优于基线，准确率提升26.86%，F1分数提高3.49%，且在高噪声下表现稳健。

Conclusion: CrEst通过自动化可信度评估有效提升LLM性能，为处理不可靠上下文提供了可行方案。
...
TL;DR: MDBench是一个用于评估大语言模型在多文档推理任务上的新数据集，通过合成生成方法高效创建，挑战现有模型能力。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在多文档推理能力上的快速发展，缺乏相关基准测试，且传统标注成本高昂。

Method: 采用基于结构化种子知识的合成生成技术，通过LLM辅助编辑生成文档集和问答示例。

Result: MDBench对现有模型和提示技术均构成显著挑战，且生成方法支持针对性分析和快速适应新需求。

Conclusion: MDBench为多文档推理评估提供了高效可控的解决方案，并展示了未来扩展潜力。
...
TL;DR: 研究探讨了LLMs在糖尿病预测中的效果，比较了开源与专有模型，发现专有模型表现更优，但需进一步优化提示策略和领域微调。
...
Method: 使用零样本、单样本和三样本提示方法，在PIDD数据集上测试六种LLMs（四种开源、两种专有），并与三种传统ML模型对比。

Result: 专有LLMs（如GPT-4o和Gemma-2-27B）在少样本设置中表现最佳，部分开源模型甚至优于传统ML模型。
...
<details>
  <summary>Details</summary>
Motivation: 探索LLM是否能够通过嵌入技术实现精确文本重建，以应用于记忆检索、压缩和可控文本生成。

Method: 引入特殊记忆令牌，通过训练优化其嵌入，使模型能根据嵌入重建固定序列。

Result: 在英语和西班牙语数据集上，模型（包括Llama 3.1 8B）成功重建了长达240个令牌的序列。

Conclusion: LLM具备通过嵌入实现精确文本重建的能力，为记忆检索和文本压缩等应用提供了新思路。
...
TL;DR: 利用自然语言处理技术分析执法和法医叙述，识别社会孤立和孤独感，开发高质量分类器，并发现特定人群（如男性、同性恋者、离婚者）更容易被分类为慢性社会孤立。


<details>
  <summary>Details</summary>
Motivation: 社会孤立和孤独感近年增加，与自杀率密切相关，但未被美国国家暴力死亡报告系统（NVDRS）记录，需通过技术手段识别。

Method: 使用主题建模生成词典开发，并结合监督学习分类器，分析2002年至2020年30万例自杀数据。

Result: 开发出高质量分类器（平均F1：0.86，准确率：0.82），识别出1,198例慢性社会孤立，发现男性、同性恋者、离婚者等群体风险更高。
...
Method: 提出PrefBERT，基于两个多样化的长文本风格和质量评估数据集训练，为GRPO提供语义奖励反馈。

Result: PrefBERT在多句子和段落长度响应中表现可靠，与传统指标相比，能更好地与人类偏好对齐。

Conclusion: PrefBERT作为奖励信号训练策略模型，生成的响应更符合人类偏好，优于传统指标训练的结果。
...
TL;DR: 研究探讨了学习时知识编码对语言模型后验遗忘（unlearning）效果的影响，发现使用转述描述学习能提升遗忘性能，但遗忘文本块中的单个知识仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）的广泛应用，后验遗忘特定知识的需求日益重要，涉及隐私法规和修正有害内容等问题。

Method: 通过实验研究学习时知识编码（如转述描述）对遗忘效果的影响。
...
Result: 在STAC和Molweni数据集上的实验表明，该方法显著优于现有基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在医学知识表示和临床部署效率上存在不足。
...
TL;DR: 论文提出了一种两阶段框架（MRG和PS）来建模开放域对话的一对多特性，通过新数据集o2mDial和改进的训练策略，提升了小规模LLM的多样性和质量。


<details>
  <summary>Details</summary>
Motivation: 开放域对话存在一对多特性，现有LLM未显式建模，导致多样性不足。

Method: 将对话生成分解为多响应生成（MRG）和偏好选择（PS），并引入o2mDial数据集及新训练策略。

Result: 实验表明，该框架显著提升小规模LLM的多样性和质量（最高提升90%），接近大规模模型性能。

Conclusion: 显式建模一对多特性可有效提升开放域对话的多样性和质量，为小模型提供优化路径。
...
TL;DR: Thunder-Tok是一种新的韩语分词器，旨在降低分词数量（token fertility）而不影响模型性能。


<details>
  <summary>Details</summary>
Motivation: 设计一种更高效的韩语分词器，减少分词数量以提升推理速度，同时保持语言信息。

Method: 采用基于规则的分词方法，结合韩语语言结构，并使用分支熵算法选择种子词汇。

Result: 实验显示，Thunder-Tok比BPE减少约10%的分词数量，推理速度提升10%，且不影响下游任务性能。

Conclusion: 该研究表明，基于语言学的分词方法能有效设计高效的分词器。
...
TL;DR: 论文研究了状态空间语言模型中的记忆机制，通过首因效应和近因效应揭示了信息如何随时间保留和遗忘。研究发现Mamba架构在输入序列的开头和结尾表现最佳，并提出了三种机制解释这一现象。


<details>
  <summary>Details</summary>
Motivation: 探索状态空间语言模型中记忆的形成和遗忘机制，以理解信息在序列中的保留方式。

Method: 应用结构化回忆任务于Mamba架构，分析其记忆模式，并通过目标消融和输入扰动验证发现。

Result: 发现长期记忆由稀疏通道支持，短期记忆受指数衰减调节，记忆分配受语义规律动态影响。

Conclusion: 研究揭示了Mamba架构中记忆的三重机制，为语言模型的记忆行为提供了新见解。
...
<details>
  <summary>Details</summary>
Motivation: 由于可持续发展目标（SDGs）数据规模庞大且复杂，追踪进展困难，需要高效的文本分类工具。大型语言模型（LLMs）因其强大的语言理解能力成为潜在解决方案。

Method: 研究比较了不同专有和开源LLMs在单标签多类文本分类任务中的表现，并评估了零样本学习、少样本学习和微调等任务适应技术的效果。

Result: 优化后的小型模型通过提示工程可以达到与大型模型（如OpenAI的GPT）相当的性能。

Conclusion: 在SDGs文本分类任务中，小型模型经过优化后可以替代大型模型，提示工程是关键。
...
TL;DR: ProtoReasoning框架通过抽象推理原型提升LLMs的推理能力，实验表明其在逻辑推理、规划任务等方面优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 探索大型推理模型跨领域泛化能力的机制，假设共享抽象推理原型是关键。

Method: 提出ProtoReasoning框架，包括自动化原型构建、验证系统和原型空间问题合成。

Result: 在逻辑推理、规划等任务上显著优于基线模型，验证了原型学习的泛化优势。

Conclusion: 推理原型是大型语言模型可泛化推理的基础，ProtoReasoning验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统评估指标（如ROUGE和BERTScore）在捕捉语义相似性方面表现不佳，而现有的基于LLM的评估方法缺乏直观解释性，且未区分问题类型。因此，需要一种更精细、适应性更强的评估方法。

Method: MinosEval首先区分事实性和非事实性问题，对事实性问题采用自适应关键点评分策略，对非事实性问题采用实例感知的列表排序策略。

Result: 实验表明，MinosEval在多个开放QA数据集上优于传统方法，更符合人工标注结果，并提供更可解释的评估。

Conclusion: MinosEval通过问题分类和针对性评估策略，显著提升了开放QA任务的评估效果，为LLM能力评估提供了新工具。
...
TL;DR: 提出Graph RAG框架，结合链式思维提示、自指令生成和过程监督，构建历史人物关系数据集，降低人工标注成本，提升通用模型在历史知识提取中的性能。
...
Method: 结合知识图谱与检索增强生成，提出Graph RAG框架，通过链式思维提示和自指令生成优化历史知识提取。

Result: Xunzi-Qwen1.5-14B模型在关系提取任务中F1达0.68；GraphRAG集成DeepSeek模型在C-CLUE数据集上F1提升11%，缓解幻觉现象。
...
TL;DR: TopClustRAG是一种结合稀疏和密集索引的混合检索策略，通过K-Means聚类生成语义相似的段落组，并利用聚类特定提示生成中间答案，最终合成全面回答。该系统在LiveRAG挑战中表现优异。
...
Method: 采用混合检索策略（稀疏和密集索引），结合K-Means聚类生成聚类特定提示，通过LLM生成中间答案并最终合成回答。

Result: 在FineWeb Sample-10BT数据集上，TopClustRAG在忠实性排名第2，正确性排名第7。

Conclusion: 基于聚类的上下文过滤和提示聚合在大规模RAG系统中表现有效。
...
TL;DR: 论文提出了一种名为Thunder-DeID的去标识化框架，以解决韩国司法系统中公开判决时个人数据保护不足的问题。
...
Method: 构建首个韩国法律数据集，系统化分类个人身份信息（PII），并开发基于深度神经网络（DNN）的端到端去标识化流程。

Result: 实验结果表明，该模型在判决去标识化任务中达到最优性能。
...
TL;DR: 综述分析了LLM在临床试验招募中匹配患者与试验的潜力，批判现有方法依赖专有模型和弱评估基准，并探讨未来方向。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在通用NLP任务中表现优异，但在临床试验招募等关键领域应用有限，需探索其知识聚合与推理能力。

Method: 分析现有LLM辅助方法，批判性评估基准、方法及评估框架，探讨临床研究中LLM技术的挑战与未来方向。

Result: LLM在临床试验匹配中潜力巨大，但现有方法依赖专有模型且评估不足。

Conclusion: 需改进LLM在临床研究中的应用，开发更通用的解决方案和强评估基准。
...
Result: 零样本模型反事实推理更连贯，微调模型更稳定；人口统计学因素对输出有持续影响。
...
Result: 测试显示许多模型在处理文化细微差别和区域特定内容时表现不佳。
...
TL;DR: 论文介绍了COSMMIC数据集，填补了印度语言多模态多语言摘要研究的空白，整合了文章、图片和用户评论，并评估了不同配置对摘要生成的影响。
...
Method: 构建包含9种印度语言的COSMMIC数据集，结合文章、图片和用户评论，采用先进语言模型（如LLama3、GPT-4）评估不同配置的摘要生成效果。

Result: 数据集包含4,959篇文章-图片对和24,484条评论，通过实验确定了结合文本、评论和图片的最有效配置。

Conclusion: COSMMIC填补了印度语言资源空白，推动了NLP研究的包容性发展。
...
TL;DR: 论文提出了一种名为Targeted Lexical Injection (TLI)的新方法，通过针对低资源语言（如斯瓦希里语）的早期内部层进行微调，显著提升了跨语言词汇对齐能力。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在低资源语言中因数据稀缺和预训练不足导致的性能不佳问题，特别是跨语言词汇对齐的挑战。

Method: 采用Low-Rank Adaptation (LoRA)和对比学习目标，针对模型早期层（如Layer 2）进行微调。

Result: TLI显著提升了623个斯瓦希里语-英语词对的词汇对齐（平均余弦相似度从0.3211提升至0.4113），并在未见过的词对上表现出良好的泛化能力。
...
TL;DR: 提出了一种细粒度评估框架和Peak Sharpness Score（PSS）指标，结合Context-Aware Cropping技术，提升GUI代理的可靠性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在GUI代理中存在定位错误的幻觉问题，影响可靠性。

Method: 提出细粒度评估框架分类预测类型，引入PSS指标量化不确定性，并提出训练无关的Context-Aware Cropping技术。

Result: 实验表明框架和方法能提供可操作见解，增强GUI代理行为的可解释性和鲁棒性。

Conclusion: 通过新框架和PSS指标，结合Context-Aware Cropping，显著改善了GUI代理的可靠性。
...
TL;DR: AgentGroupChat-V2是一种新型多智能体框架，通过并行架构、自适应协作引擎和优化策略，显著提升了复杂任务处理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统在架构设计、跨领域泛化和性能保障方面存在挑战，尤其是任务复杂度和智能体数量增加时。
...
TL;DR: 论文提出RE-IMAGINE框架，通过生成不同层次的问题变体，评估大语言模型（LLMs）的真实推理能力，而非仅依赖统计记忆。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在推理任务中表现优异，但无法确定其是否真正具备推理能力，还是仅依赖训练数据的统计记忆。

Method: 基于因果阶梯理论（Pearl, 2009），提出RE-IMAGINE框架，生成不同层次（关联、干预、反事实）的问题变体，并通过符号表示生成无法仅靠记忆解决的问题。

Result: 实验表明，LLMs在面对问题变体时性能下降，表明其部分依赖统计记忆。

Conclusion: RE-IMAGINE为评估LLMs的真实推理能力提供了新方法，并揭示了其对统计记忆的依赖，为未来研究提供了方向。
...
TL;DR: 论文提出了一种名为CINGS的后训练监督方法，通过将相关上下文附加到响应前并仅计算响应标记的损失，以增强语言模型在外部知识基础上的生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在推理时简单附加上下文并不能确保生成的响应基于外部知识，因此需要一种更有效的方法来增强模型的接地性。

Method: 提出CINGS方法，在训练时将相关上下文附加到响应前，仅计算响应标记的损失并屏蔽上下文。

Result: 实验表明，CINGS训练的模型在文本和视觉领域均表现出更强的接地性，且在多个数据集上优于其他方法，同时不影响下游任务性能。

Conclusion: CINGS通过改变模型的先验知识和行为，隐式鼓励其更依赖外部上下文，从而在不降低性能的情况下显著提升接地性。
...
TL;DR: SPARE是一种新颖的自动化过程标注框架，通过参考解决方案对齐步骤并显式评估推理，显著提升LLM的多步推理能力，同时提高效率。


<details>
  <summary>Details</summary>
Motivation: 解决高效、高质量自动化过程标注的挑战，以提升LLM在复杂推理任务中的表现。

Method: 提出SPARE框架，通过单次标注和参考引导的评估，对齐解决方案步骤并显式评估推理。

Result: 在数学推理、多跳问答和空间推理任务中表现优异，效率提升2.6倍，运行时间减少62%。

Conclusion: SPARE在提升推理性能和效率方面具有显著优势，为未来研究提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 夸张和隐喻在自然语言处理中具有重要意义，但由于其语义模糊和表达多样性，现有方法难以准确识别。

Method: 通过情感分析模块挖掘情感内涵，基于情感的领域映射模块理解隐含意义，双向动态交互模块促进夸张和隐喻的相互提升，并设计验证机制确保准确性。

Result: 在四个数据集上表现优于基线方法，F1分数在TroFi和HYPO-L数据集上分别提升了28.1%和23.1%。

Conclusion: EmoBi框架有效提升了夸张和隐喻检测的性能，具有潜在的应用价值。
...
TL;DR: 论文探讨了如何通过强化学习和内部推理提升大语言模型（LLM）的可靠性和可验证性，提出GRPO方法优化答案正确性、引用充分性和拒绝质量。
...
Method: 使用GRPO（Group Relative Policy Optimization）方法，通过可验证的结果奖励训练模型，结合两阶段训练和GPT-4蒸馏。

Result: 实验表明，结合推理的模型在多个数据集上表现优于仅指令调优的模型，尤其在处理不可回答问题和生成引用充分的回答时。

Conclusion: 强化学习、分阶段优化和结果驱动的RL对构建更可靠和可验证的LLM具有重要价值。
...
Conclusion: RATTENTION在性能和效率之间实现了更优的平衡，且不牺牲训练速度。
...
<details>
  <summary>Details</summary>
Motivation: 现代语言模型通常开放权重但封闭训练数据，研究旨在解决从模型权重中近似训练数据的问题。

Method: 提出梯度基方法，从大型公共文本库中选择匹配度最高的数据，用于恢复原始和微调模型的权重。

Result: 在AG News分类任务中，性能从65%提升至80%；在MSMARCO文档的SFT模型中，困惑度从3.3降至2.3。

Conclusion: 该方法能有效从公共数据中恢复近似训练数据，显著提升模型性能，接近专家基准。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)在实时语音聊天中因生成首句延迟导致用户体验不佳，尤其是在计算资源有限的设备上。

Method: 提出PredGen框架，利用用户输入时的预测性解码生成候选响应，提前启动TTS处理。

Result: 在Lmsys和MT-Bench数据集上的模拟实验显示，延迟降低约2倍，计算成本增加极小。

Conclusion: PredGen有效减少LLMs在语音聊天中的延迟，提升实时性，适用于资源受限场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注二元性别区分，缺乏对非二元性别包容性的评估，因此需要一种更全面的指标。

Method: 开发了GIFI指标，通过多层次的评估（如代词测试、生成行为和认知行为测试）量化LLMs的性别包容性。
...
TL;DR: SciVer是首个用于评估基础模型在多模态科学背景下验证声明能力的基准，包含3000个专家标注的示例，覆盖4种常见推理类型。评估了21种先进多模态模型，发现其与人类专家存在显著差距，并分析了开源模型的局限性。
...
Method: 构建了包含3000个专家标注示例的SciVer基准，覆盖4种推理类型，并评估了21种多模态模型的性能。

Result: 实验显示现有模型与人类专家存在显著差距，开源模型在检索增强生成（RAG）等方面存在关键局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本场景图解析器针对单句设计，无法有效处理多句描述中的跨句共指现象，导致图碎片化和下游任务性能下降。

Method: 提出DiscoSG任务和数据集DiscoSG-DS，并开发DiscoSG-Refiner模型，通过两个小型PLM协作生成和优化场景图。

Result: DiscoSG-Refiner在SPICE指标上比基线提升约30%，推理速度比GPT-4快86倍，并提升了下游任务性能。

Conclusion: DiscoSG-Refiner是一种高效且性能优越的解决方案，适用于多句描述的文本场景图解析任务。
...
TL;DR: 论文介绍了WikiMixQA基准，用于评估跨模态推理能力，发现现有模型在长上下文多模态推理中表现不佳，尤其是开源模型。


<details>
  <summary>Details</summary>
Motivation: 文档理解面临复杂布局和多模态信息的挑战，现有VLLMs在长上下文视觉输入处理上的效果尚不明确。

Method: 构建WikiMixQA基准，包含1000个多选题，基于4000个Wikipedia页面的表格和图表，评估12种先进视觉语言模型。
...
TL;DR: 论文探讨了生成式AI在葡萄牙语阅读多选题（MCQ）生成中的应用，评估其质量与可靠性，发现模型生成题目质量接近人工，但仍存在语义清晰度和干扰项设计问题。


<details>
  <summary>Details</summary>
Motivation: 手动创建不同难度和目标的MCQ耗时且昂贵，生成式AI提供了自动化可能，但对其质量评估不足，尤其是非英语语言。

Method: 研究生成葡萄牙语阅读MCQ，结合课程相关叙事元素和不同难度，通过专家评审和学生回答的心理测量学分析评估。

Result: 模型生成的MCQ质量接近人工，但存在语义清晰度和干扰项设计问题。

Conclusion: 生成式AI在葡萄牙语MCQ生成中潜力显著，但需进一步优化语义和干扰项设计。
...
TL;DR: 论文研究了大型语言模型中的后悔机制，提出了构建后悔数据集的方法和两种新指标（S-CDI和RDS/GIC），用于识别和分析后悔神经元及其激活模式。
...
Method: 通过设计提示场景构建后悔数据集，提出S-CDI指标识别最优后悔表示层，以及RDS/GIC指标分析后悔神经元。

Result: 实验成功识别了最优后悔表示层，并发现神经元可分为三类：后悔神经元、非后悔神经元和双功能神经元。

Conclusion: 提出的方法和指标为理解后悔机制提供了有效工具，揭示了信息处理的耦合与解耦模式。
...
TL;DR: 论文提出Ordered CommonGen基准，用于评估大语言模型（LLM）的组合泛化和指令遵循能力，发现LLM在指定概念顺序时表现不足。


<details>
  <summary>Details</summary>
Motivation: 研究LLM在生成符合指定概念顺序的句子时的能力，揭示其在指令遵循和组合泛化方面的局限性。

Method: 提出Ordered CommonGen基准，通过有序覆盖率评估LLM的表现，并分析了36种LLM的性能。

Result: LLM虽能理解指令意图，但对特定顺序模式有偏见，导致输出多样性低；最佳模型的有序覆盖率仅约75%。

Conclusion: LLM在指令遵循和组合泛化方面仍需改进，Ordered CommonGen为未来研究提供了有效工具。
...
TL;DR: 研究使用六种机器学习方法（SVM、LR、k-NN、DT、RF、ANN）对罗马尼亚语文本进行作者归属分析，其中ANN模型表现最佳，尤其在5-gram特征下达到最高准确率。


<details>
  <summary>Details</summary>
Motivation: 解决罗马尼亚语文本的作者归属问题，评估轻量级字符n-gram方法的有效性。

Method: 使用ROST语料库，比较六种机器学习技术（SVM、LR、k-NN、DT、RF、ANN），采用字符n-gram特征分类。

Result: ANN模型表现最佳，5-gram特征下四次完美分类。

Conclusion: 轻量级字符n-gram方法在罗马尼亚语作者归属中表现优异，适用于资源有限或研究较少的语言环境。
...
Method: 采用基于同类问题的强化学习框架（CC-Learn），定义复合目标（包括同类准确率、检索奖励和拒绝惩罚）进行优化。
...
TL;DR: 研究发现大型推理模型的推理轨迹中存在隐私泄露风险，敏感数据可能通过提示注入或意外泄露。增加推理步骤会放大泄露，揭示了推理效用与隐私攻击面之间的核心矛盾。
...
Result: 增加推理步骤会提高模型答案的谨慎性，但也导致更详细的推理和更多隐私泄露。
...
TL;DR: 研究评估了21种机器翻译系统在性别模糊翻译中的表现，发现大多数系统缺乏性别中立翻译，但少数系统能根据目标语言采用特定策略。


<details>
  <summary>Details</summary>
Motivation: 性别包容的机器翻译需保留源语言的性别模糊性，避免性别错误和代表性伤害，但在语法性别语言中实现此目标具有挑战性。

Method: 评估21种机器翻译系统在三种不同难度的翻译方向中对性别模糊的敏感性，并分类讨论实际观察到的性别中立策略。

Result: 大多数系统未能提供性别中立翻译，但少数系统能根据目标语言采用特定策略实现性别中立。

Conclusion: 性别中立翻译在机器翻译中仍不普遍，但部分系统的策略表明改进的可能性。
...
TL;DR: 提出了一种名为GenRecal的新框架，用于解决异构视觉语言模型（VLM）之间的知识蒸馏问题，通过特征对齐实现高效知识迁移。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在资源受限设备上部署困难，且不同架构的VLM之间知识迁移存在挑战。

Method: GenRecal框架引入Recalibrator模块，对齐异构VLM的特征表示，实现跨模型知识蒸馏。

Result: 实验表明，GenRecal显著提升基线性能，甚至超越大规模开源和闭源VLM。

Conclusion: GenRecal为异构VLM的知识蒸馏提供了一种通用解决方案，具有实际应用潜力。
...
TL;DR: 论文提出PhantomHunter，一种专门检测私有调优LLM生成文本的方法，通过家族级特征学习框架，显著提升检测性能。
...
Method: 提出PhantomHunter，采用家族级特征学习框架，捕捉基础模型及其衍生模型的共享特征，而非个体特征。
...
TL;DR: 论文提出Argus Inspection多模态基准和Eye of Panoptes框架，用于评估MLLMs的视觉细粒度感知和常识因果推理能力，发现现有模型性能仍有提升空间。


<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs的认知和推理能力显著提升，但在视觉细粒度感知和常识因果推理方面仍存在挑战，需开发更全面的评估方法。

Method: 引入Argus Inspection多模态基准（含两个难度级别）和Eye of Panoptes框架（结合Sigmoid度量与指示函数），对26种主流MLLMs进行实验评估。

Result: 实验显示，视觉细粒度推理的最高性能仅为0.46，表明模型仍有较大改进空间。
...
<details>
  <summary>Details</summary>
Motivation: 猎鹰训练和狩猎中，健康监测至关重要，传统方法效率不足，需更精准的疾病检测手段。

Method: 采用ConvNeXt和EfficientNet混合模型，针对三种疾病（正常、肝病、曲霉病）进行分类，利用大数据集训练和验证。

Result: 混合模型在准确率、精确率、召回率和F1分数上优于传统方法和单一模型。

Conclusion: 该混合AI模型为猎鹰疾病检测提供了高效解决方案，并为未来AI驱动的鸟类医疗发展奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 解决自然环境中动物监测的需求，提供可解释的系统以理解视觉数据和人类语言查询。

Method: 集成视觉检测模块、语言解析器和符号推理层，通过逻辑推断回答问题。

Result: 系统在动物图像任务中表现良好，能准确回答数量、存在和位置问题。

Conclusion: ViLLa通过模块化和透明性，成功连接视觉内容与结构化查询。
...
TL;DR: 提出GraphGSOcc模型，通过双高斯图注意力解决3D高斯溅射中的语义相关性和边界模糊问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射方法在特征聚合中忽视语义相关性，且边界模糊问题严重。

Method: 结合语义和几何图Transformer，提出双高斯图注意力（几何图和语义图）和多尺度图注意力框架。
...
TL;DR: 论文提出了DAVID-X数据集和DAVID-XR1模型，用于透明化地检测AI生成视频，并提供详细的缺陷分类、时空定位和自然语言解释。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成视频在媒体平台上的普及，可靠区分合成内容与真实视频的需求变得迫切。现有方法仅提供二分类结果，缺乏解释性。

Method: 通过DAVID-X数据集（含缺陷级注释和解释）训练DAVID-XR1模型，结合链式思维蒸馏，实现可解释的检测。

Result: 模型在多种生成器和模式下表现出强泛化能力，验证了可解释检测方法的有效性。

Conclusion: 透明化的检测方法为AI生成视频的可信识别提供了新方向。
...
TL;DR: 综述了2020-2024年基于深度学习的多智能体轨迹预测研究，重点分析了ETH/UCY基准测试中的模型架构、输入表示和预测策略，并探讨了未来研究方向。
...
Method: 对近年研究进行分类，基于架构设计、输入表示和预测策略，重点关注ETH/UCY基准测试中的模型。

Result: 总结了多智能体轨迹预测的最新进展，并识别了关键挑战。

Conclusion: 提出了未来研究方向，强调了多智能体HTP领域的潜在改进空间。
...
Method: 构建ArchForms-4000数据集，开发ArchShapeNet 3D卷积神经网络，并引入显著性模块突出关键空间特征。

Result: 模型在区分形式来源上表现优于人类专家，准确率94.29%，精确率96.2%，召回率98.51%。

Conclusion: 研究揭示了人类设计在空间组织、比例和谐与细节优化上的优势，为未来生成设计工具的改进提供了方向。
...
TL;DR: 提出了一种高性能、低延迟的视频监控系统，适用于资源受限环境，结合熵自适应帧缓冲算法与MobileNetV2，实现高吞吐量与低延迟。


<details>
  <summary>Details</summary>
Motivation: 针对资源受限设备（如Raspberry Pi、NVIDIA Jetson Nano）设计高效视频监控系统，满足低延迟、高准确率和隐私合规需求。

Method: 采用熵自适应帧缓冲算法，结合MobileNetV2，优化视频流处理。

Result: 在标准数据集上保持92%以上检测准确率，端到端推理延迟低于50ms，适应不同光照、背景和速度。

Conclusion: 系统具有可扩展性、低成本及隐私合规性，适用于智能城市或嵌入式安全架构。
...
TL;DR: MonoVQD是一种基于DETR的单目3D检测新框架，通过改进自注意力机制、引入变分查询去噪技术和自蒸馏策略，显著提升了性能。
...
Method: 1. 提出掩码分离自注意力机制；2. 引入变分查询去噪技术；3. 设计自蒸馏策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在图表到代码生成任务中表现不佳，需要同时具备精确的视觉理解和准确的代码翻译能力。直接提示MLLMs完成此任务效果不理想。
...
Result: PictSure在域外基准测试中优于现有ICL模型，同时在域内任务中保持可比性能。
...
TL;DR: 本文提出了一种名为BKSEF的框架，用于在CNN中优化逐层核大小选择，通过信息增益、计算效率和准确性平衡，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 卷积神经网络中核大小的选择对模型性能至关重要，但常被忽视。本文旨在提供一种数学和实证支持的框架，优化核大小选择。

Method: 提出BKSEF框架，结合信息理论、信号处理和学习理论，通过实验验证其在多数据集上的有效性。

Result: 实验显示，BKSEF引导的模型在准确率上提升3.1%，计算量减少42.8%，并在实际应用中验证了其优势。

Conclusion: BKSEF为CNN设计提供了新的优化视角，适用于架构搜索和实时系统，核大小可作为可优化参数而非固定启发式。
...
TL;DR: 提出了一种基于深度学习的零售视频自动标注方法，显著降低人工标注成本并保持高质量。


<details>
  <summary>Details</summary>
Motivation: 传统零售视频标注依赖耗时的人工标注，成本高且效率低，亟需自动化解决方案。

Method: 利用深度神经网络学习视频帧特征，结合零售环境定制的目标检测技术，实现关键帧自动识别和标注。

Result: 实验表明，该方法在准确性上与人工标注相当，同时节省50%成本，仅需人工验证5%的帧。

Conclusion: 该方法高效且经济，适用于多种零售应用，如顾客行为分析和产品交互检测。
...
TL;DR: 论文提出了一种基于轻量级神经网络UPNet的主动视角选择方法，用于高效3D重建，显著减少计算开销并保持高精度。
...
Method: 使用UPNet预测不确定性地图，通过启发式方法选择最具信息的视角，训练3D神经渲染模型。

Result: 仅用一半视角即可达到与上限相当的精度，计算开销降低400倍，资源使用减少50%以上。

Conclusion: 该方法在无需额外训练的情况下，能泛化到新物体类别，显著提升了3D重建的效率与准确性。
...
TL;DR: 论文提出DPO-Kernels方法，通过混合损失、核化表示和散度选择增强文本到图像模型的校准能力，并引入DETONATE基准和AQI指标。


<details>
  <summary>Details</summary>
Motivation: 提升文本到图像模型在用户意图捕捉、安全性和公平性方面的校准能力。

Method: 采用混合损失、核化表示（RBF、多项式和小波核）和多种散度选择（Wasserstein和R'enyi），提出DETONATE基准和AQI指标。

Result: DPO-Kernels在DETONATE基准上表现出色，通过HT-SR保持强泛化能力。

Conclusion: DPO-Kernels有效提升模型校准能力，DETONATE和AQI为未来研究提供工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态强化学习方法主要局限于单图像空间推理，难以推广到涉及多图像位置推理的复杂场景。
...
TL;DR: 提出了一种针对医学图像扩散模型的频率校准重建误差（FCRE）方法，用于成员推断攻击（MIA），通过聚焦中频区域的重建误差，解决了现有方法因图像固有难度和高频细节重建困难导致的局限性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在医学图像生成中的隐私风险日益凸显，现有MIA方法依赖重建误差，但受图像固有难度和高频重建困难影响，效果不佳。
...
TL;DR: 本文评估了Vision Transformer (ViT)及其混合模型在区分夸克和胶子喷注中的性能，发现其优于传统CNN方法。
...
<details>
  <summary>Details</summary>
Motivation: 历史地图中的文本信息具有重要价值，但现有方法难以处理多样化的地图样式和复杂背景。

Method: 提出PALETTE，结合超局部采样模块和位置嵌入，优化文本边界点和字符的检测与识别。

Result: PALETTE在历史地图数据集上优于现有方法，并成功应用于大规模地图处理。

Conclusion: PALETTE为历史地图文本识别提供了高效解决方案，并通过开源项目推广使用。
...
Method: 1. 基于BLIP生成与风格图像语义对齐的文本描述，利用大语言模型移除风格相关描述，创建语义间隙以微调模型。2. 基于人类反馈的数据增强策略，逐步学习以减少过拟合。3. 设计无需训练的三重扩散过程，通过自注意力层特征操作实现风格注入。

Result: 实现了高质量的图像驱动风格迁移和文本驱动风格化，同时保留原始图像内容，并首次在风格迁移中实现图像颜色编辑。

Conclusion: StyleWallfacer通过创新的语义注入和扩散过程，显著提升了风格迁移的质量和灵活性，为领域带来突破。
...
Method: 采用Wasserstein距离对齐特征与码向量分布，提升码本利用率并减少量化误差。

Result: 实验和理论分析验证了方法的有效性，实现了近100%的码本利用率。

Conclusion: 通过分布对齐，显著改善了向量量化的性能，为自回归模型提供了更稳定的训练基础。
...
TL;DR: SynPo是一种基于大视觉模型（如SAM）的无训练少样本医学图像分割方法，通过改进负提示质量提升低对比度医学图像的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于大视觉模型的无训练方法未能有效利用负提示，导致在低对比度医学图像上表现不佳。
...
TL;DR: 提出了一种名为PDSA的模块，通过高维空间相关性校正特征分布，解决了点云分析中局部坐标的干扰和特征层次差距问题，提高了计算效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在聚合邻域点时存在无关点干扰和特征层次差距问题，且基于直接几何结构编码的方法计算开销大且对噪声敏感。

Method: 提出PDSA模块，利用高维空间相关性校正特征分布，通过轻量级跨阶段结构描述符区分点相关性，并通过减少邻域特征矩阵方差和长距离建模增强结构同质性。引入关键点机制优化计算开销。

Result: 在语义分割和分类任务中验证了方法的泛化性，性能显著提升且参数成本更低。消融实验和可视化结果证明了方法的有效性和合理性。

Conclusion: PDSA模块通过高维空间相关性校正特征分布，有效解决了点云分析中的问题，具有高效性和鲁棒性。
...
TL;DR: 本文提出了一种新型的双噪声扩散模型Echo-DND，用于超声图像中左心室的高精度分割，解决了噪声和低对比度问题，并在两个数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 超声图像噪声大、对比度低，左心室边界模糊，传统分割方法效果不佳，需要更精确的解决方案。

Method: Echo-DND结合高斯和伯努利噪声，采用多尺度融合条件模块和空间一致性校准，提升分割精度。
...
TL;DR: ReSeDis任务统一了图像检索和像素级定位，提出了新的基准和评估指标，并展示了零样本基线的潜力。
...
Result: 展示了零样本基线的性能，表明未来研究有较大改进空间。
...
TL;DR: 本文探讨了如何通过视觉上下文学习（VICL）训练通用模型，用于视网膜光学相干断层扫描（OCT）领域，并提出了针对VICL的评估协议。


<details>
  <summary>Details</summary>
Motivation: 尽管专业模型在特定临床任务中表现优异，但其适用性有限且开发成本高。通用模型则允许医疗从业者灵活定义任务，无需针对特定任务开发模型。

Method: 采用视觉上下文学习（VICL）方法，训练模型在推理时基于少量示例泛化到不同任务。

Result: 在多个视网膜OCT数据集上评估了先进的医学VICL方法，建立了首个基准，展示了VICL在OCT中的潜力和当前限制。

Conclusion: 研究为OCT领域的通用模型提供了初步成果，并公开代码以促进进一步研究和实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言预训练模型语义理解能力的提升，公开图像的隐私保护需求增加。

Method: 提出PSIC方法，结合CLTG模块和UAEO优化函数，支持多解码选项和自适应优化。

Result: 实验证明该方法在保护隐私和保持图像质量方面有效。

Conclusion: PSIC是一种即插即用的解决方案，可集成到现有图像压缩模型中。
...
Method: 采用两阶段扩散模型：第一阶段训练UNet进行图像重建，第二阶段将噪声图像输入融合网络，结合三个关键融合模块和混合损失函数。

Result: 实验表明，该方法在多种医学图像类型上表现优异，融合图像亮度、放射性示踪剂分布、纹理和边缘清晰度均得到提升。

Conclusion: DM-FNet通过两阶段设计和混合损失函数，显著提高了多模态医学图像融合的质量和信息密度。
...
Conclusion: DBACS方法能有效提升领域适应技术的性能，为半导体领域提供了实用解决方案。
...
TL;DR: MSNeRV是一种基于多尺度特征融合的隐式神经表示（INR）视频压缩方法，通过时间窗口和GoP级网格增强时间一致性，并利用多尺度空间解码器和自适应损失函数提升细节表现。实验表明其压缩效率优于VTM-23.7。
...
<details>
  <summary>Details</summary>
Motivation: 腹腔镜手术中准确识别解剖结构具有挑战性，AR系统结合MRI/CT与腹腔镜图像可提升导航精度，但需依赖精确的曲线标志检测。

Method: 提出BCRNet框架，包括多模态特征提取模块（MFE）、自适应曲线提案初始化（ACPI）和分层曲线细化机制（HCR），通过多阶段迭代优化Bezier曲线。
...
TL;DR: ViMAT是一种无需标记或固定工作环境的AI系统，用于实时监控工业装配任务，结合感知和推理模块，有效处理部分和不确定的视觉观察。


<details>
  <summary>Details</summary>
Motivation: 工业装配任务的视觉监控对防止设备损坏和确保工人安全至关重要，现有商业方案通常需要固定工作环境或标记，限制了灵活性。

Method: ViMAT结合多视角视频流的感知模块和基于任务知识的推理模块，实时推断装配动作。

Result: 在LEGO组件更换和液压模具重组任务中，ViMAT通过定量和定性分析验证了其有效性。

Conclusion: ViMAT在复杂真实场景中表现出色，为工业装配监控提供了灵活高效的解决方案。
...
TL;DR: 论文探讨了面部微表情（MEs）的识别、定位和生成，提出结合定位与识别的统一任务（ME-STR）和基于视觉问答的ME理解（ME-VQA），利用多模态大语言模型提升分析能力。


<details>
  <summary>Details</summary>
Motivation: 传统方法将微表情的定位与识别分开处理，在长视频分析中效果不佳，需探索更高效的多模态方法。

Method: 提出ME-STR任务（统一序列管道）和ME-VQA任务（视觉问答），利用MLLMs/LVLMs的多模态能力。

Result: MEGC 2025挑战赛引入这两项任务，要求算法在测试集上运行并提交结果。

Conclusion: 多模态大语言模型为微表情分析提供了新方向，ME-STR和ME-VQA任务有望推动领域发展。
...
TL;DR: 本文提出了一种名为MapFM的端到端模型，用于在线生成矢量化的高清地图，通过结合强大的基础模型和辅助预测头，显著提升了地图生成的质量和准确性。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶中，高清地图和鸟瞰视角的语义地图对精确定位、规划和决策至关重要，但现有方法在特征表示和预测质量上仍有提升空间。

Method: MapFM模型结合了基础模型对相机图像进行编码，并引入辅助预测头进行鸟瞰视角的语义分割，采用多任务学习方法提升场景理解。

Result: 实验表明，该方法显著提升了特征表示质量，生成的矢量高清地图具有更高的准确性和质量。

Conclusion: MapFM通过多任务学习和丰富的上下文监督，为自动驾驶提供了更全面的场景表示和更高质量的矢量地图。
...
Method: 结合任务特定提示选择信息样本，后续查询中采用多样信息ID采样（DIS）确保样本纯度和信息量。

Result: 在两个公开病理图像数据集上，OpenPath显著优于现有开放集主动学习方法。

Conclusion: OpenPath通过高效样本选择和避免OOD数据，显著提升了病理图像分类的性能。
...
TL;DR: 论文提出了一种新任务：开放世界视频中的物体计数，并介绍了CountVid模型，结合图像计数和视频分割跟踪技术，显著优于基线方法。
...
Method: DDIS利用文本到图像扩散模型作为图像先验，通过领域对齐指导（DAG）和类对齐标记（CAT）优化生成过程。
...
<details>
  <summary>Details</summary>
Motivation: 在医学影像领域，确保深度学习模型的可靠性至关重要，而OOD检测是评估模型可靠性的关键指标。现有方法可能无法完全捕捉OOD多样性，因此需要更有效的解决方案。
...
Conclusion: NERO通过神经元级相关性和距离度量显著提升了OOD检测能力，同时支持可解释性，为医学影像中的模型可靠性提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 3D生成领域技术复杂，难以普及，本文旨在通过教程形式简化流程，使其更易于开发者使用。

Method: 使用Hunyuan3D 2.1系统，包括形状生成（Hunyuan3D-DiT）和纹理合成（Hunyuan3D-Paint）两大核心组件，涵盖数据准备、模型架构、训练策略和评估指标。

Result: 通过教程，读者能够掌握开发或优化3D生成模型的技能，适用于游戏、虚拟现实和工业设计等领域。

Conclusion: Hunyuan3D 2.1教程为3D生成模型的开发提供了实用指南，推动了该技术在更广泛领域的应用。
...
Method: 结合冻结LLM与可学习视觉编码器，引入动态提示定制机制，通过视觉特征生成实例特定提示。
...
Method: 第一阶段使用Object-AnchorNet从3D HOI数据集中学习稀疏3D HOI关键帧重建；第二阶段通过Contact-Aware Diffusion Model（ContactDM）插值生成密集4D HOI序列，并引入Contact-Aware Encoder和HOI Attention。

Result: 在OMOMO和3D-FUTURE数据集上取得了最先进的成果，展示了强大的泛化能力和高保真生成能力。
...
Result: 306名参与者中17支团队成功提交解决方案，挑战赛展示了多样化的阴影去除方法。
...
Method: 提出CLAIM框架，结合SMILE模块（基于临床知识的疤痕生成）和联合训练策略，优化疤痕生成与分割。

Result: CLAIM生成的疤痕解剖学一致且多样化，分割性能优于基线模型。

Conclusion: CLAIM实现了可控且真实的疤痕合成，对下游医学影像任务有实用价值。
...
Conclusion: RaCalNet证明了稀疏监督的有效性，能生成结构一致且细节丰富的深度图。
...
TL;DR: WinWinLay提出了一种无需训练的方法，通过非局部注意力能量函数和自适应更新策略，提升布局到图像生成的精确性和真实感。
...
Method: 采用非局部注意力能量函数重新分配注意力分数，以及基于Langevin动力学的自适应更新规则，避免偏离预训练域。
...
<details>
  <summary>Details</summary>
Motivation: 旨在通过统一的多模态表示和高效的训练方法，提升跨模态任务的性能。

Method: 基于3D因果变分自编码器空间，采用双路径时空融合构建统一视觉表示，结合语言模型的自回归建模和流匹配技术。

Result: Show-o2在多模态理解和生成任务中表现出色，支持文本、图像和视频的多样化处理。

Conclusion: Show-o2展示了强大的跨模态能力，并通过开源代码和模型推动相关研究。
...
TL;DR: 提出了一种参数高效的半监督分割框架，用于0.3米空间分辨率影像，结合SAM2知识和FreqWeaver Adapter，显著提升细粒度建模能力。
...
Method: 采用半监督学习框架，结合SAM2知识，引入FreqWeaver Adapter增强细粒度建模，模型参数仅占总参数的5.96%。

Result: 在未标注数据的有效利用下，实现了结构一致性更强的分割结果，性能提升1.78%（参数高效策略）和3.44%（高分辨率遥感分割方法）。

Conclusion: 该方法在超高分辨率遥感影像分割中表现出色，为弱监督条件下的精细土地覆盖分析提供了有效解决方案。
...
TL;DR: 提出了一种基于图的新型统一框架，用于大规模点云的端到端处理，显著提升了森林地上生物量（AGB）估计的可行性和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前定量结构模型（QSM）方法依赖高质量点云数据且预处理步骤复杂，限制了其可扩展性和实际应用。本研究旨在解决这些局限性。

Method: 采用图操作（如路径和抽象）无缝集成树木分割、叶木分离和3D骨架重建，实现端到端处理。

Result: 在多种条件下（如叶覆盖和低密度点云）表现优异，相对误差低（叶覆盖约20%，低密度点云约30%）。

Conclusion: 该框架为大规模AGB估计提供了稳健且可扩展的解决方案，降低了预处理依赖，并验证了无人机激光扫描（ULS）的可行性。
...
Method: 提出双LoRA学习范式（DLoRAL），包括跨帧检索（CFR）模块和一致性-LoRA（C-LoRA）模块，用于提取时间一致性先验；随后训练细节-LoRA（D-LoRA）增强空间细节。

Result: 实验表明，DLoRAL在精度和速度上均表现优异。

Conclusion: DLoRAL通过双LoRA学习，成功实现了细节丰富且时间一致的视频超分辨率。
...
<details>
  <summary>Details</summary>
Motivation: 临床中多模态图像配准（如PET与MRI）因模态差异大，传统无监督方法难以准确配准，易导致图像失真。

Method: 提出M2M-Reg框架，利用单模态相似性训练多模态配准模型，并引入GradCyCon正则化器；支持半监督学习。
...
TL;DR: 提出了一种无需重建的实时3D物体检测框架，结合预训练视觉基础模型和CLIP，实现高效内存使用和开放词汇语义捕捉。
...
Result: 在ScanNetV2和CA-1M数据集上达到在线方法的最优性能，支持超过1000平方米环境的实时感知。

Conclusion: 该框架无需重建，具有高效性和泛化能力，适用于实时3D检测任务。
...
Result: 在GRAB数据集上，HOIDiNi在接触精度、物理有效性和整体质量上优于现有方法，并能生成复杂的可控交互。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在处理长期记忆和大量图像时表现不足，限制了其在机器人控制中的应用。需要一种新的基准测试来评估和改进这些模型。

Method: 在Habitat模拟器中设计了一个包含60个任务的基准测试，这些任务需要长期记忆和上下文感知。同时提出了结合导航策略的基线模型。

Result: 基准测试展示了模型在长期记忆任务中的表现，并指出了改进方向。

Conclusion: 该工作为长期记忆和推理任务提供了新的评估工具，并揭示了当前模型的局限性。
...
TL;DR: 研究发现，视觉质量对多模态大语言模型（MLLMs）的影响存在悖论：图像质量偏离人类感知时，模型表现反而更好。作者提出VQ-TTT方法，通过动态调整输入图像提升任务性能。


<details>
  <summary>Details</summary>
Motivation: 探讨视觉输入质量如何影响MLLMs的表现，并解决现有模型对图像质量的特殊偏好问题。

Method: 提出VQ-TTT方法，通过插入可学习低秩核和浅层微调，动态调整输入图像以适应模型偏好。

Result: VQ-TTT显著提升了MLLMs在多种任务中的平均准确率，无需外部模型或额外数据。
...
Method: 第一阶段筛选高价值候选标题，第二阶段选择性优化弱视觉基础部分，并引入边缘惩罚机制。

Result: ViMaR显著提升标题的可靠性、准确性和细节，速度提升4倍，并能跨模型泛化。

Conclusion: ViMaR是一种可扩展、可迁移的解码策略，还能通过自训练提升模型性能。
...
Method: 通过视频扩散模型联合估计反照率并合成重光照输出，结合合成多光照数据和自动标注的真实视频进行训练。

Result: 模型在多样场景中表现出强泛化能力，视觉保真度和时间一致性优于先前方法。

Conclusion: 该方法通过单阶段联合优化，显著提升了重光照任务的真实性和适应性。
...
<details>
  <summary>Details</summary>
Motivation: 视障人士通常依赖盲文书籍和音频记录，但这些方式有限，无法满足个性化需求。语音作为一种更有效的沟通方式，更适合视障人士使用。

Method: 使用LabVIEW开发了一种基于OCR的语音合成系统，该系统准确、可靠、经济且用户友好。

Result: 系统成功实现，能够将文本转换为语音，帮助视障人士获取所需书籍内容。

Conclusion: 该OCR语音合成系统为视障人士提供了一种高效、便捷的知识获取方式。
...
TL;DR: SonicVerse是一个多任务音乐描述模型，结合了描述生成和辅助音乐特征检测任务，通过投影架构将音频输入转换为语言标记，提升描述的丰富性和准确性。
...
Method: 采用投影架构，将音频输入转换为语言标记，同时通过辅助头检测音乐特征，并将特征投影为语言标记以增强描述输入。

Result: 实验表明，结合音乐特征提升了生成描述的细节和质量，支持对长音乐片段的时间感知描述。

Conclusion: SonicVerse通过多任务学习和特征整合，显著提升了音乐描述的丰富性和准确性。
...
### [120] [Hypothesis Testing for Quantifying LLM-Human Misalignment in Multiple Choice Settings](https://arxiv.org/abs/2506.14997)
*Harbin Hong,Sebastian Caldas,Liu Leqi*

Main category: cs.CY

TL;DR: 提出一个定量框架，评估大语言模型（LLMs）在多选调查中模拟人类行为的偏差，发现模型在争议性问题上的模拟效果不佳。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在社会科学研究中模拟人类行为的准确性，避免盲目使用。

Method: 使用假设检验构建定量框架，分析LLMs在多选调查中与人类行为的偏差。

Result: 发现模型在模拟不同种族、年龄和收入的子群体时表现不佳，尤其在争议性问题上。

Conclusion: LLMs在社会科学研究中需谨慎使用，需开发新方法以更好模拟人类行为。
...
TL;DR: HAC是一种分层神经语音编解码器，通过知识蒸馏目标将瓶颈分解为声学、音素和词汇三个层次，实现了语音的离散表示。


<details>
  <summary>Details</summary>
Motivation: 为了在单一模型中统一语音的声学细节和词汇意义，提出HAC以解决传统单层次编解码器的局限性。

Method: 利用HuBERT和LaBSE的知识蒸馏目标，分别提取音素和词汇信息，构建分层瓶颈。

Result: 实验表明HAC的令牌集能解耦音素和词汇信息，且在自然度和重建质量上优于基线。

Conclusion: HAC作为一种统一的离散语音表示，有望应用于语音生成和理解任务。
...
Result: Matterport Pro3生成的点云密度更高（1,877,324点），对齐精度更高（RMSE为0.0118米），优于iPhone（506,961点）。
...
TL;DR: 该研究提出了一种基于Edge Impulse的边缘设备实时糖尿病视网膜病变（DR）检测方法，通过优化深度学习模型在边缘设备上的部署，实现了高效且低成本的早期诊断。
...
Method: 研究使用Kaggle EyePACS数据集中的3,662张视网膜图像，通过预处理和增强技术优化数据。设计了多种CNN模型（如MobileNet、ShuffleNet等），并转换为TensorFlowLite格式，量化至8位整数以减少模型大小并提升推理速度。

Result: MobileNet准确率达96.45%，SqueezeNet在GPU上延迟仅17ms且模型大小为176KB，ShuffleNet和自定义DNN在资源效率上表现优异。

Conclusion: 边缘AI技术为DR早期检测提供了可扩展且经济高效的解决方案，尤其适用于资源有限的医疗环境。
...
<details>
  <summary>Details</summary>
Motivation: 解决前列腺MRI分割中因读者差异和跨站点数据偏移导致的自动分割精度问题。

Method: 比较UNETR、SwinUNETR和3D UNet模型，采用单队列、5折交叉验证混合队列和腺体大小分组的训练策略，使用Optuna调参。
...
TL;DR: FAHR-Face是一个基于面部图像的基础模型，通过微调用于生物年龄估计和生存风险预测，表现出色且具有临床实用性。


<details>
  <summary>Details</summary>
Motivation: 利用面部外观作为非侵入性健康指标，开发能够估计生物年龄和预测生存风险的模型。

Method: FAHR-FaceAge通过两阶段微调在公共数据集上训练；FAHR-FaceSurvival在癌症患者照片上微调。模型测试了鲁棒性和独立性，并在临床数据集中验证。

Result: FAHR-FaceAge在年龄估计上误差最低（5.1年），FAHR-FaceSurvival能稳健预测死亡率。两者结合提高了预后准确性。
...
TL;DR: 提出了一种递归变分神经网络（RvNN）来生成复杂且多样化的血管结构，解决了现有方法无法捕捉真实解剖数据多样性的问题。
...
Conclusion: RvNN首次用于血管合成，生成的模型准确且多样化，具有广泛的应用潜力。
...
TL;DR: 提出了一种基于Transformer的混合专家（MoE）框架，用于整合多模态MRI和临床数据，显著提升神经退行性疾病的诊断准确性。
...
Conclusion: 该框架在多模态学习应用于临床数据方面具有潜力，能显著提升神经退行性疾病的诊断准确性。
...
<details>
  <summary>Details</summary>
Motivation: 由于DICOM头部信息常因协议多样性和技术员错误而不准确，影响了MRI数据的读取效率，因此需要一种自动分类方法。

Method: 使用ResNet、EfficientNet和DenseNet等深度学习模型对8种MRI系列进行分类，并比较其性能。研究了不同训练数据量对模型性能的影响，并在外部数据集上进行了评估。

Result: DenseNet-121模型表现最佳，F1分数和准确率分别为0.966和0.972。在训练数据量超过729项时，模型准确率超过0.95。在外部数据集DLDS和CPTAC-UCEC上，准确率分别为0.872和0.810。

Conclusion: DenseNet-121模型在内部和外部数据集上均表现出高准确率，适用于MRI系列分类任务。
...
<details>
  <summary>Details</summary>
Motivation: 医疗图像包含敏感信息，需隐私保护，但同态加密计算成本高，尤其是大图像（如胸片）。

Method: 使用VQGAN压缩图像为潜在表示，多项式近似激活函数，结合压缩因子为8的降采样和squeeze and excitation模块优化HE框架。

Result: 在胸片多标签分类任务中，性能与未加密推理接近，计算负担显著降低。

Conclusion: 该方法在医疗图像隐私保护中具有实用潜力，尽管HE推理仍较慢且性能略有差异。
...
<details>
  <summary>Details</summary>
Motivation: 在医学图像分析中，联邦学习（FL）面临异构计算资源和隐私保护的挑战，尤其是在全切片图像（WSI）分类任务中。

Method: 提出FedWSIDD，利用数据集蒸馏生成合成切片，并在服务器端聚合和分发这些切片。客户端采用定制化的DD算法，结合染色标准化生成高信息密度的合成切片。

Result: 在CAMELYON16和CAMELYON17等WSI分类任务中，FedWSIDD提升了分类性能，支持异构本地模型，并保护了患者隐私。
...
<details>
  <summary>Details</summary>
Motivation: 超小型内窥镜传感器因光敏区域有限和模拟信号传输引入噪声，导致图像质量下降，需要有效的去噪方法。

Method: 开发了全面的噪声模型，并提出结合传统图像处理与学习技术的混合去噪系统。

Result: 实验显示，该方法在FPGA平台上实时运行，PSNR从21.16提升至33.05，且不损失细节或色彩。

Conclusion: 混合去噪系统有效解决了内窥镜图像噪声问题，具有实用性和高效性。
...
TL;DR: 研究探讨了图像预处理技术对CNN在宫颈癌分类中的性能影响，提出了一种混合PMD-CLAHE方法，显著提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 宫颈癌早期检测至关重要，但CNN性能受图像质量影响，研究旨在通过预处理技术提升分类效果。

Method: 评估了PMD滤波、CLAHE增强及混合PMD-CLAHE方法，测试了多种预训练模型。

Result: 混合PMD-CLAHE方法显著提升了图像质量和CNN性能，准确率提升13.62%。

Conclusion: 混合PMD-CLAHE技术为宫颈癌分类提供了新思路，显著提升了CNN性能。
...
TL;DR: 本研究提出了一种基于混合UNet-Transformer模型的肿瘤自动分割方法，针对本地医院MRI数据集，通过高效的数据预处理和增强，结合多种注意力模块，实现了较高的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI分割模型多基于公开数据集，缺乏对本地患者群体的适应性，本研究旨在开发适用于本地医院的高效肿瘤分割模型，以优化放疗治疗计划。

Method: 采用混合UNet-Transformer架构，结合多种注意力模块（如SE、CBAM等），并通过数据增强和预训练权重初始化，使用双NVIDIA T4 GPU进行训练。

Result: 在本地MRI数据集上，Dice系数为0.764，IoU为0.736，表现出色，验证了模型在有限数据下的有效性。
...
### [134] [Towards Perception-based Collision Avoidance for UAVs when Guiding the Visually Impaired](https://arxiv.org/abs/2506.14857)
*Suman Raj,Swapnil Padhi,Ruchi Bhoot,Prince Modi,Yogesh Simmhan*

Main category: cs.RO

TL;DR: 无人机结合机器学习和计算机视觉算法，为视障人士提供户外导航辅助。


<details>
  <summary>Details</summary>
Motivation: 利用无人机帮助视障人士在户外城市环境中导航，提升其独立性和安全性。

Method: 提出基于感知的局部路径规划系统，结合GPS和地图的全局规划，采用几何问题建模和多DNN框架实现无人机和视障人士的避障。

Result: 在大学校园环境中验证了算法在三种场景（人行道、停车区、拥挤街道）中的可行性。

Conclusion: 系统能够有效辅助视障人士在复杂户外环境中导航。
...
### [135] [Robust Instant Policy: Leveraging Student's t-Regression Model for Robust In-context Imitation Learning of Robot Manipulation](https://arxiv.org/abs/2506.15157)
...
Conclusion: RIP通过处理幻觉问题，提高了模仿学习的可靠性和性能。
...
### [136] [MCOO-SLAM: A Multi-Camera Omnidirectional Object SLAM System](https://arxiv.org/abs/2506.15402)
...
Method: 集成点特征和物体级地标，结合语义-几何-时间融合策略，设计全景闭环模块，构建分层3D场景图。

Result: 实验证明MCOO-SLAM在真实场景中实现精确定位和可扩展的物体级建图，对遮挡、姿态变化和环境复杂性具有更强鲁棒性。
...
### [137] [Particle-Grid Neural Dynamics for Learning Deformable Object Models from RGB-D Videos](https://arxiv.org/abs/2506.15680)
*Kaifeng Zhang,Baoyu Li,Kris Hauser,Yunzhu Li*

Main category: cs.RO

TL;DR: 提出了一种结合粒子与空间网格的神经动力学框架，用于模拟可变形物体的动态行为，并通过实验验证其优于现有方法。
...
Result: 模型能够从稀疏RGB-D记录中学习多种物体的动态，并在类别级别泛化到未见实例。

Conclusion: 该方法在有限视角下优于现有方法，并能用于模型规划任务。
...
Conclusion: 为松散IMU传感器姿态估计提供了新方向，结合服装参数可提升性能。
...
Method: 通过优化扩散模型的文本指令，利用梯度优化生成素描，并引入OS-Sketch数据集进行评测。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D生成模型在遵循指令、对齐人类偏好及生成真实纹理和几何方面表现不足，需要改进。

Method: 基于Nabla-GFlowNet方法，利用2D奖励信号对3D扩散模型进行强化学习对齐。

Result: 实验表明，Nabla-R2D3在少量微调步骤内实现更高奖励并减少先验遗忘。
...
TL;DR: 本文提出了一种针对360度视频的超分辨率方法S3PO，并创建了新的数据集360VDS，解决了传统VSR模型在360度视频中的失真问题。
...
Method: 提出了S3PO模型，采用循环建模和注意力机制，设计了专用特征提取器和损失函数以应对球形失真。
...
### [142] [Embodied Web Agents: Bridging Physical-Digital Realms for Integrated Agent Intelligence](https://arxiv.org/abs/2506.15677)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI代理在物理世界交互和网络推理方面是割裂的，限制了其在需要跨领域智能的任务中的应用。
...
TL;DR: 该研究提出了一种基于持续同调的拓扑框架，用于检测全球事件对公共话语结构的根本性改变。通过分析重大事件期间的新闻文章，构建名词短语共现图，并计算Wasserstein距离和持续熵，捕捉语义中断和叙事波动。结果显示，重大事件与H0和H1维度的突变相关，表明叙事结构的突然重组。
...
Method: 使用持续同调分析新闻文章中的名词短语共现图，通过Vietoris-Rips过滤生成持续图，计算Wasserstein距离和持续熵以量化语义变化。

Result: 研究发现，重大事件与H0（连通分量）和H1（环路）的突变相关，表明叙事结构的突然重组。俄罗斯入侵乌克兰事件中，H1熵领先于H0，可能反映自上而下的叙事框架。

Conclusion: 持续同调提供了一种数学严谨的无监督方法，能够实时检测公共话语的语义重构，为计算社会科学提供了新工具。
...
### [144] [Cost-Efficient Serving of LLM Agents via Test-Time Plan Caching](https://arxiv.org/abs/2506.14852)
*Qizheng Zhang,Michael Wornow,Kunle Olukotun*

Main category: cs.DC

TL;DR: 提出了一种基于LLM的代理应用计划缓存方法，通过提取、存储和重用结构化计划模板，显著降低成本。
...
Conclusion: 该方法为LLM代理服务提供了高效且经济的解决方案，与现有基础设施互补。
...
### [145] [Identifying economic narratives in large text corpora -- An integrated approach using Large Language Models](https://arxiv.org/abs/2506.15041)
*Tobias Schmidt,Kai-Robin Lange,Matthias Reccius,Henrik Müller,Michael Roos,Carsten Jentsch*

Main category: econ.GN

TL;DR: 论文探讨了使用大型语言模型（如GPT-4）提取经济叙事的有效性，并与专家标注的黄金标准进行比较。


<details>
  <summary>Details</summary>
Motivation: 近年来，经济叙事的研究兴趣增加，但现有方法缺乏深层语义理解，无法区分经济叙事提取与传统任务。

Method: 通过分析《华尔街日报》和《纽约时报》关于通胀的文章，比较GPT-4输出与专家标注的叙事。
...
TL;DR: 本文首次全面分析了数据可视化库中的错误，研究了564个来自五个常用库的bug，系统分析了其症状和根本原因，并提出了分类法。研究发现错误/不准确的图表普遍存在，主要原因是图形计算错误，需进一步自动化测试方法。此外，研究还探索了视觉语言模型（VLMs）在检测错误图表中的可行性。


<details>
  <summary>Details</summary>
Motivation: 数据可视化库的准确性对用户体验和信息传达至关重要，错误可能导致误导性决策。目前缺乏对这类错误的系统研究，因此需要深入分析其特点和解决方案。

Method: 研究收集了五个常用数据可视化库中的564个bug，系统分析其症状和根本原因，并提出分类法。同时探索了视觉语言模型（VLMs）在检测错误图表中的应用。

Result: 研究发现错误/不准确的图表普遍存在，图形计算错误是主要原因。VLMs在检测错误图表中的效果因提示不同而异，效果在29%至57%之间。

Conclusion: 研究为数据可视化库的错误检测和修复提供了系统分析，并提出了未来自动化测试技术的研究方向，同时探索了VLMs的潜在应用。
...
### [147] [ETS: Open Vocabulary Electroencephalography-To-Text Decoding and Sentiment Classification](https://arxiv.org/abs/2506.14783)
*Mohamed Masry,Mohamed Amen,Mohamed Elzyat,Mohamed Hamed,Norhan Magdy,Maram Khaled*

Main category: cs.LG

TL;DR: 提出了一种结合EEG和眼动追踪数据的框架ETS，用于开放词汇文本生成和情感分类，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决非侵入性EEG在开放词汇场景下解码自然语言的挑战，传统方法因噪声和变异性难以应对。

Method: 整合EEG与同步眼动追踪数据，开发ETS框架，用于开放词汇文本生成和情感分类任务。

Result: 在BLEU和Rouge分数上表现优异，情感分类F1分数提升10%，且能处理多源数据。

Conclusion: ETS框架在开放词汇EEG解码中具有高性能潜力，适用于多被试和多数据源场景。
...
### [148] [Assembly of Experts: Linear-time construction of the Chimera LLM variants with emergent and adaptable behaviors](https://arxiv.org/abs/2506.14794)
...
<details>
  <summary>Details</summary>
Motivation: 预训练大型语言模型（LLM）计算成本极高，需要优化以更好地利用现有模型资源。

Method: 通过线性时间内的权重张量插值，生成父模型的子模型，增强或抑制语义特征。

Result: 生成的子模型功能强大且稳定，DeepSeek R1T 'Chimera'模型在减少40%输出令牌的同时保持了高性能。

Conclusion: AoE方法为高效生成高质量子模型提供了可行方案，无需微调或蒸馏。
...
### [149] [Revisiting Reinforcement Learning for LLM Reasoning from A Cross-Domain Perspective](https://arxiv.org/abs/2506.14965)
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习（RL）在大型语言模型（LLM）推理中的应用主要集中在数学和代码领域，缺乏对其他推理领域的广泛研究。

Method: 通过构建Guru语料库，涵盖六个推理领域，并设计领域特定的奖励信号，进行去重和过滤以确保可靠性。

Result: Guru-7B和Guru-32B模型在公开数据训练的RL模型中表现最佳，优于基线模型7.9%和6.7%。

Conclusion: RL在不同领域的表现差异显著，某些领域需要领域内训练才能取得显著性能提升，表明RL可能促进真正的技能获取。
...
### [150] [Optimal Embedding Learning Rate in LLMs: The Effect of Vocabulary Size](https://arxiv.org/abs/2506.15025)
...
<details>
  <summary>Details</summary>
Motivation: μP方法在实践中表现出色，但其理论假设词汇量固定，而实际中词汇量远大于宽度，导致理论与实践的冲突。本文旨在填补这一理论空白。

Method: 通过理论分析词汇量对训练动态的影响，提出LV机制，并验证其缩放规则。实验包括训练1B参数的模型。

Result: 发现词汇量增加时训练动态从μP过渡到LV机制，嵌入学习率的最优缩放比例为Θ(√width)，与μP的Θ(width)不同。

Conclusion: 词汇量对训练动态有显著影响，提出的LV机制和缩放规则在实践中表现优于μP方法。
...
### [151] [When and How Unlabeled Data Provably Improve In-Context Learning](https://arxiv.org/abs/2506.15329)
*Yingcong Li,Xiangyu Chang,Muti Kara,Xiaofeng Liu,Amit Roy-Chowdhury,Samet Oymak*

Main category: cs.LG

TL;DR: 论文研究了上下文学习（ICL）在标签缺失或错误时的有效性，通过理论分析展示了不同模型结构对未标记数据的利用能力。
...
Result: 单层模型无法利用未标记数据，而多层/循环模型能有效利用，且性能随深度指数提升。
...
### [152] [Capturing Polysemanticity with PRISM: A Multi-Concept Feature Description Framework](https://arxiv.org/abs/2506.15538)
...
Result: PRISM在描述质量和捕捉多义性概念方面优于现有方法，提升了描述的准确性和忠实度。
...
### [153] [LoX: Low-Rank Extrapolation Robustifies LLM Safety Against Fine-tuning](https://arxiv.org/abs/2506.15606)
*Gabrel J. Perin,Runjin Chen,Xuxi Chen,Nina S. T. Hirata,Zhangyang Wang,Junyuan Hong*

Main category: cs.LG

TL;DR: 论文提出了一种无需训练的方法LoX，通过外推安全子空间增强LLM的安全鲁棒性，有效抵御微调攻击。


<details>
  <summary>Details</summary>
Motivation: 尽管对齐技术提升了LLM的安全性，但微调仍可能破坏其安全保护，需要一种更鲁棒的解决方案。

Method: 基于安全关键低秩子空间的敏感性，提出LoX方法，外推对齐LLM的安全子空间。

Result: 实验显示，LoX显著降低了攻击成功率（11%-54%），同时保持模型对新任务的适应性。

Conclusion: LoX通过将参数移至平坦区域，减少对扰动的敏感性，成功提升了LLM的安全鲁棒性。
...
### [154] [AutoRule: Reasoning Chain-of-thought Extracted Rule-based Rewards Improve Preference Learning](https://arxiv.org/abs/2506.15651)
...
Result: 在Llama-3-8B模型上，AutoRule使AlpacaEval2.0的胜率相对提升28.6%，MT-Bench子集第二轮表现提升6.1%。
...
### [155] [PIPE: Physics-Informed Position Encoding for Alignment of Satellite Images and Time Series](https://arxiv.org/abs/2506.14786)
...
Method: PIPE包含两个关键创新：1）物理信息位置索引方案；2）变频率位置编码机制，用于编码物理变量的频率信息和令牌的顺序信息。

Result: 在最大开源卫星图像数据集上，PIPE在深度学习和气候领域方法中均达到最先进性能，台风强度预测比先前工作提升了12%。

Conclusion: PIPE通过嵌入物理信息，显著提升了多模态时间序列预测的准确性和对齐能力，具有广泛的应用潜力。
...
### [156] [Dense SAE Latents Are Features, Not Bugs](https://arxiv.org/abs/2506.15679)
...
<details>
  <summary>Details</summary>
Motivation: 研究密集潜在特征的几何、功能和起源，验证其是否为训练过程中的不良产物。

Method: 系统分析密集潜在特征的几何结构、功能分类及其在模型各层的演变。

Result: 密集潜在特征形成对偶对，重构残差流中的特定方向，且具有功能分类（如位置跟踪、语义绑定等）。
...
TL;DR: 小规模视觉语言模型通过GRPO优化和外部工具（如缩放）提升视觉推理能力，在有限计算资源下表现优于基线模型。
...
Result: 在部分视觉问答任务中表现优于同类基线模型，得益于外部工具提供的详细视觉信息。
...
<details>
  <summary>Details</summary>
Motivation: 医疗数据通常不完整且有限，机器学习模型的透明度和可信度问题影响了其在医疗领域的应用，尤其是在AMD的早期诊断中。

Method: 开发了CACTUS工具，结合遗传、饮食、临床和人口统计学因素，提供可解释性和灵活性，优于标准机器学习模型。

Result: CACTUS在AMD分期分类中表现优异，能识别关键因素并提高决策信心，同时减少偏见数据的影响。

Conclusion: CACTUS为AMD诊断提供了更可靠和透明的解决方案，有助于临床决策和减少数据偏见。
...
### [159] [Pixel-level Certified Explanations via Randomized Smoothing](https://arxiv.org/abs/2506.15499)
*Alaa Anani,Tobias Lorenz,Mario Fritz,Bernt Schiele*

Main category: cs.LG

TL;DR: 该论文提出了首个针对后验归因方法的认证框架，通过随机平滑技术保证像素级鲁棒性，并提出了三种评估指标。


<details>
  <summary>Details</summary>
Motivation: 后验归因方法在解释深度学习预测时存在非鲁棒性问题，微小扰动可能导致归因图显著变化，影响可信度。
