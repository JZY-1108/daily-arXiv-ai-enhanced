...
Motivation: 研究多模态模型中推理链长度增加导致的幻觉问题，提出评估方法。

Method: 引入RH-AUC指标和RH-Bench基准，分析模型在不同推理长度下的视觉感知准确性。

Result: 大模型在推理与感知间平衡更好，训练数据类型比数量影响更大。

Conclusion: 需联合评估推理质量和感知保真度，以优化多模态模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有ASR数据集（如LibriSpeech）在规模和多样性上存在不足，且其他新数据集存在许可、转录质量或评估集缺失等问题，限制了ASR研究的进展。

Method: 提出了Loquacious Set，一个包含多样口音和语音类型的25,000小时商用英语语音数据集。

Result: Loquacious Set为学术界和工业界提供了一个高质量、多样化的ASR研究资源。

Conclusion: Loquacious Set填补了现有ASR数据集的空白，有望推动真实场景下的ASR技术发展。
...
<details>
  <summary>Details</summary>
Motivation: 在有限计算资源下，不同领域数据的采样比例显著影响模型性能，需要系统研究如何确定最优领域权重。

Method: 将现有方法细分为离线（启发式、算法式、函数拟合）和在线（最小最大优化、混合法则等）方法，并分析其问题表述和算法。

Result: 总结了各类方法的优缺点，并明确了它们之间的关系与区别。

Conclusion: 指出了数据混合领域的关键挑战，为未来研究提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）推理开销大，而小语言模型（SLMs）效率高但性能不足。研究发现仅少数令牌会真正影响推理路径，因此提出R2R方法。

Method: R2R方法通过神经令牌路由，仅对关键令牌使用LLMs，其余由SLM生成。开发了自动数据生成管道识别差异令牌并训练轻量路由器。

Result: 实验表明，R2R在平均激活参数为5.6B时，性能超越R1-7B的1.6倍，甚至优于R1-14B，且速度比R1-32B快2.8倍。

Conclusion: R2R显著提升了测试时扩展效率，为LLMs和SLMs的结合提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对LLM受错误信息影响的细粒度分析，需填补这一空白。

Method: 构建MisBench基准（包含10,346,712条错误信息），提出Reconstruct to Discriminate（RtD）方法。

Result: LLM在识别错误信息时表现相当，但仍易受知识冲突和风格变化影响。

Conclusion: MisBench可作为评估LLM检测器的有效基准，RtD方法能增强LLM检测错误信息的能力。
...
<details>
  <summary>Details</summary>
Motivation: 材料发现中的组合爆炸问题导致数据稀缺，需要充分利用所有可能的数据源，包括科学文本中的潜在知识。

Method: 采用迭代框架，结合多样化文档选择、Word2Vec模型训练和嵌入空间相关性监测，预测高性能材料。

Result: 成功预测了氧还原、氢析出和氧析出反应中的高性能材料，并通过实验验证。

Conclusion: 迭代语料库优化方法为材料发现提供了高效、可扩展的工具，尤其在数据稀缺的情况下。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（如GPT、LLAMA、Claude）是黑盒模型，缺乏透明度，在需要信任和问责的领域可能存在问题。

Method: SMILE通过微调输入并测量输出变化，识别对输出影响最大的词语，生成可视化热图。

Result: 在多个主流LLM上测试，SMILE在准确性、一致性、稳定性和保真度方面表现良好，提供了清晰可靠的解释。

Conclusion: SMILE通过增强模型的可解释性，为AI的透明度和可信度迈出了重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 异常值对LLM性能（如量化和压缩）有显著影响，但现有研究对其成因探索不足。

Method: 全面调查异常值形成机制，并提出潜在缓解策略。

Result: 提出高效方法消除大部分异常值，同时对模型精度影响最小。

Conclusion: 通过深入分析异常值成因，为LLM量化提供了更优解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 印度司法系统中未解决案件积压严重，手动优先排序效率低且易受主观偏见影响，亟需自动化解决方案。

Method: 利用ILDC数据集（7,593份标注请愿书），结合文本嵌入技术（如DistilBERT、LegalBERT）和定量指标（如间隔天数、排名分数），训练多种机器学习模型（如随机森林、XGBoost）。

Result: 随机森林和决策树模型表现最优，准确率超99%，Spearman等级相关性达0.99。纯数值特征模型也能达到接近最优效果（R2=0.988）。

Conclusion: LLMPR框架能有效优化司法流程，减少案件积压，提升法律优先排序的公平性。
...
<details>
  <summary>Details</summary>
Motivation: 由于LLMs的英语预训练可能导致跨语言文化偏见，但缺乏全面的多语言评估工具，因此开发MAKIEval以解决这一问题。

Method: 利用Wikidata的多语言结构作为跨语言锚点，自动识别模型输出中的文化实体，并通过四个指标（粒度、多样性、文化特异性和语言共识）评估文化意识。

Result: 评估了7个LLMs在13种语言、19个国家和地区的表现，发现模型在英语中表现出更强的文化意识。

Conclusion: MAKIEval为跨语言文化意识评估提供了可扩展的工具，揭示了LLMs在非英语语言中的文化偏见，并公开了代码和数据。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）的可靠性因幻觉问题而受损，需要精确识别其知识缺口。现有探测方法存在不一致性，亟需评估和改进。

Method: 提出了一种基于输入变化和定量指标的新评估过程，分析探测方法在内部和跨方法上的一致性。

Result: 发现探测方法存在显著不一致性：内部方法中微小提示变化导致知识缺口检测差异（如选项打乱使一致性降至40%）；跨方法间一致性极低（仅7%）。

Conclusion: 现有探测方法存在严重不一致性，亟需开发抗干扰的探测框架以提高可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 随着社交媒体上用户生成内容审核的挑战增加，AI在内容审核中的作用日益重要。

Method: 通过比较ChatGPT与众包标注和专家评估的性能，评估其准确性、检测范围和一致性。

Result: ChatGPT在检测不适当内容上表现良好，但在目标语言检测上存在较高误报率。

Conclusion: 研究展示了AI模型在内容审核中的潜力，同时指出需进一步改进模型以提升上下文理解能力。
...
<details>
  <summary>Details</summary>
Motivation: LLM的行为难以预测，尤其是在高风险场景中，模型能否准确解释其行为至关重要。

Method: 提出了一种通用的反事实可模拟性评估框架，应用于新闻摘要和医学建议任务。

Result: LLM的解释在新闻摘要任务中帮助用户预测输出，但在医学建议任务中效果有限。

Conclusion: 反事实可模拟性评估更适合技能型任务，而非知识型任务。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在临床任务中表现被动，缺乏主动识别关键信息或风险的能力，需要行为上的改进。

Method: 提出BehaviorBench数据集评估行为表现，并设计BehaviorSFT训练策略，通过行为标记动态调整LLMs行为。

Result: BehaviorSFT将整体Macro F1提升至97.3%，主动任务得分提高（如Qwen2.5-7B-Ins从95.0%到96.5%），临床评估显示行为更自然。

Conclusion: BehaviorSFT有效平衡了主动性和克制性，优于标准微调或显式指令方法。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLM）的置信度校准不足影响其可靠性，需更准确的置信度估计方法。

Method: CCPS通过对最终隐藏状态施加针对性对抗扰动，提取特征并利用轻量级分类器预测答案正确性。

Result: 在多个LLM和MMLU变体上，CCPS显著降低校准误差和Brier分数，同时提升准确性和曲线下面积。
...
<details>
  <summary>Details</summary>
Motivation: 研究低资源语言对的语音翻译性能，探索不同微调方法对模型效果的影响。

Method: 使用SeamlessM4T-v2进行ASR、MT和E2E ST的微调，并尝试直接E2E微调、多任务训练及基于ASR/MT模型的参数初始化。

Result: 直接E2E微调效果最佳；使用ASR编码器初始化对未训练语言有帮助；多任务训练略有提升。

Conclusion: 直接E2E微调是高效方法，结合ASR初始化可提升未训练语言的性能。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型在多步生成过程中更容易产生幻觉内容，因此需要一种能够检测并追踪幻觉来源的方法。

Method: 提出VeriTrail方法，设计包含中间输出和人类标注的数据集，用于检测和追踪幻觉内容。

Result: VeriTrail在两个数据集上均优于基线方法。

Conclusion: VeriTrail为多步生成过程中的幻觉检测提供了有效解决方案，并强调了中间输出的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 验证阿拉伯语方言分区的假设是否准确，以促进阿拉伯语NLP任务的进展。

Method: 扩展和分析多标签数据集，由方言使用者手动评估11个国家级别方言的句子有效性。

Result: 分析表明四种假设过于简化现实，部分假设并不总是准确。

Conclusion: 这些假设可能阻碍阿拉伯语NLP任务的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决生成模型中的多样性和偏见问题，扩展现有生成框架。

Method: 引入“代表性生成”概念，定义群闭包维度，分析信息论和计算可行性。

Result: 证明无限假设类下代表性生成的可行性，但计算上存在限制。

Conclusion: 为开发更具多样性和代表性的生成模型奠定理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理长上下文时存在注意力不均的问题，导致在多文档摘要中源覆盖率不足。

Method: 将任务分为三步：提取关键点、使用DPP选择多样性内容、重写为最终摘要。

Result: 在DiverseSumm基准测试中，该方法显著提高了源覆盖率。

Conclusion: 通过结合提示步骤和原则性技术，可以生成更全面且个性化的摘要。
...
<details>
  <summary>Details</summary>
Motivation: 评估RAG在实际应用中的鲁棒性，解决RAG可能因检索不完美或模型利用能力有限导致的性能下降问题。

Method: 建立包含1500个开放域问题的基准数据集，引入三个鲁棒性指标，评估11种LLM和3种提示策略。

Result: 所有LLM均表现出较高的检索鲁棒性，但不同程度的缺陷阻碍了其充分利用RAG的优势。

Conclusion: RAG的鲁棒性存在局限性，需进一步优化以充分发挥其潜力。
...
<details>
  <summary>Details</summary>
Motivation: 填充任务中KV缓存重用因提示格式结构受限，导致计算效率低下，需要一种新方法解决这一问题。

Method: 提出EFIM，一种改进的提示格式，并引入片段标记化训练方法，以提升KV缓存重用效率和部分词生成准确性。

Result: 实验表明，EFIM能降低延迟52%，提升吞吐量98%，同时保持原始填充能力。

Conclusion: EFIM通过优化提示格式和训练方法，显著提升了填充任务的效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统（MAS）在复杂任务中资源消耗大且效率低，需改进。

Method: 利用历史成功轨迹中的“捷径”指令，绕过冗余推理，优化协作过程。

Result: 实验显示，相比ChatDev，Co-Saving平均减少50.85%的token使用，代码质量提升10.06%。

Conclusion: Co-Saving通过资源感知和捷径机制，显著提升了多智能体系统的效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基础模型主要关注知识图谱的结构信息，忽视了文本信息，限制了其在KG外任务中的应用。

Method: 提出多视角条件消息传递（CMP）编码架构、动态残差融合模块和灵活边评分机制。

Result: 在28个数据集上评估，MERRY在多数场景中优于基线，尤其在KGQA等KG外任务中表现优异。

Conclusion: MERRY通过整合结构和文本信息，显著提升了知识图谱推理的通用性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估缺乏对混合Web-OS攻击场景的支持，或无法在真实但受控环境中进行测试。

Method: 提出RedTeamCUA框架，结合VM和Docker的混合沙箱，支持灵活的对抗场景配置和直接注入点测试。

Result: 测试发现CUAs存在高攻击成功率（ASR），如Claude 4 Opus的ASR达48%，显示间接提示注入的严重威胁。
...
Method: 采用自适应图神经网络（GNN）学习习语表达之间的复杂映射，能够泛化到训练中未见过的节点。

Result: 在多个习语翻译数据集上评估，结果显示翻译质量显著提升，尤其是在资源受限的小模型中。

Conclusion: IdiomCE方法有效解决了习语翻译中的文化差异问题，提升了翻译质量，适用于多种语言对。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂推理任务（如多跳问答）中表现不佳，现有方法如RAG难以有效过滤噪声数据并检索必要证据。

Method: RISE框架包含三个关键步骤：问题分解、检索后阅读和自我批判，通过迭代自我探索提升推理能力。

Result: 在多个多跳问答基准测试中，RISE显著提高了推理准确性和任务性能。

Conclusion: RISE通过迭代自我探索有效增强模型在复杂推理任务中的表现，为多跳问答提供了新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索重复采样在多语言生成任务中的有效性，尤其是在推理任务中的应用。

Method: 使用基于困惑度和基于奖励的验证器，在两个多语言基准测试（Aya Evaluation Suite和m-ArenaHard）上评估重复采样的效果。

Result: 结果显示生成质量显著提升，某些情况下增益超过35%。基于困惑度的验证器适用于开放式提示，而基于奖励的验证器在需要推理的任务（如数学、代码）中表现更优。

Conclusion: 重复采样在多语言文本生成中具有广泛实用性，且选择合适的验证器对任务效果至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 领域特定指令调优中，数据选择常因忽略知识冲突（预训练知识与上下文知识的不一致）而效果不佳，导致模型能力下降和幻觉问题。

Method: KDS框架通过两种知识感知指标（上下文-记忆知识对齐和内部记忆知识一致性）量化知识冲突，筛选高质量且多样化的数据。

Result: 实验表明，KDS在医学领域显著优于基线方法，提升模型性能和泛化能力，并缓解幻觉问题。

Conclusion: KDS是一种简单有效的解决方案，能显著提升领域特定指令调优的效果，具有广泛的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有后训练方法多为手动设计或单点优化，缺乏自动化完整管道的探索。

Method: 利用LLM代理系统探索模型生成技术、数据集和超参数配置，通过任务反馈优化管道。

Result: 实验显示LaMDAgent提升了9.0点的工具使用准确率，同时发现了传统方法忽略的有效策略。

Conclusion: LaMDAgent为后训练管道的自动化提供了高效解决方案，数据规模扩展有助于降低成本。
...
<details>
  <summary>Details</summary>
Motivation: LVLMs在多模态任务中表现出色，但其视觉输入引入了新的安全漏洞，需要系统分析。

Method: 通过系统表征分析，提出两阶段评估框架：区分攻击类型和量化有害意图实现程度。

Result: 揭示了传统对抗攻击如何绕过安全机制，并提出了规范模型行为的框架。

Conclusion: 为多模态系统的安全对齐提供了理论基础和实践指导。
...
<details>
  <summary>Details</summary>
Motivation: 主流视觉语言模型（LVLMs）存在文化偏见，需要多样化的多模态数据集来提升文化理解。

Method: 通过高级代理工作流程和45名阿拉伯世界注释者的人工标注，构建了包含10个文化重要领域的多模态数据集Pearl。

Result: 评估表明，基于推理的指令对齐方法显著提升了模型的文化理解能力。

Conclusion: Pearl为文化感知的多模态建模研究提供了基础资源，所有数据和基准测试公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 混合方法研究中，定量与定性数据的结合存在挑战，LLM为生成基于定性数据的合成调查回答提供了潜在解决方案。

Method: 使用BREQ问卷和访谈数据，测试LLM在预测人类回答时的表现，分析提示设计、温度设置和访谈内容的影响。

Result: LLM能捕捉整体回答模式但变异性较低，访谈数据提升了部分模型的多样性，提示设计和低温度设置提高了对齐度。

Conclusion: LLM在结合定性定量方法上有潜力，但仍需优化提示设计、减少偏见并提升心理测量保真度。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估LLM多语言性能的方法依赖昂贵的数据集，且难以评估开放生成任务。因此，作者提出了一种基于翻译和评估的策略来评估跨语言一致性。

Method: 作者提出了一个框架，通过“翻译后评估”策略，从信息和同理心两个维度评估LLM的跨语言一致性。

Result: 研究发现，流行的LLM在三十种语言中存在显著的不一致性，某些语系和文字表现尤其差。

Conclusion: 研究强调了多语言评估的重要性，并邀请实践者使用该框架进行未来LLM的多语言基准测试。
...
TL;DR: Legal Assist AI是一种基于Transformer的模型，旨在通过大型语言模型（LLMs）为印度公民提供法律帮助，填补法律信息获取的空白。


<details>
  <summary>Details</summary>
Motivation: 印度许多公民因法律意识薄弱和法律信息获取困难而无法有效行使权利，因此需要一种高效的法律辅助工具。

Method: 模型通过检索定制法律数据库并生成准确回答，基于印度法律数据集（如宪法、BNS、BNSS等）进行微调。

Result: 在AIBE评估中，Legal Assist AI以60.08%的分数超越GPT-3.5 Turbo和Mistral 7B，表现出色且避免幻觉问题。

Conclusion: 该模型在现实法律场景中具有高度可靠性，未来将扩展多语言和案例覆盖范围以进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现推理优化模型在处理简单问题时过度推理，导致输出冗长且效率低下。

Method: 提出CoThink方法：先由指导模型生成解决方案大纲，再由推理模型完成具体推理。

Result: 在三个数据集上测试，CoThink减少22.3%的token生成，同时保持准确率。

Conclusion: CoThink能动态调整推理深度，提升效率，并观察到潜在的推理效率缩放规律。
...
<details>
  <summary>Details</summary>
Motivation: 传统的数据打包方法存在截断和上下文不连续的问题，影响了模型性能。本文旨在通过数据工程改进持续预训练的效果。

Method: 采用两阶段策略：1）滑动窗口同步重叠标记；2）使用First-Fit-Decreasing算法打包短文本以减少填充和截断。

Result: 在多种模型架构和语料领域中，SP方法在99%的情况下优于基线方法。

Conclusion: SP策略有效解决了上下文不连续和截断问题，显著提升了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统文本方法无法处理视觉信息，现有视觉RAG方法因固定流程和推理能力不足而受限。强化学习被证明有助于模型推理，因此提出VRAG-RL。

Method: VRAG-RL通过视觉感知令牌和多轮推理轨迹优化模型，定义裁剪和缩放等动作空间，结合查询重写和检索性能的奖励机制。

Result: VRAG-RL优化了视觉语言模型在RAG任务中的表现，解决了视觉感知不足和检索不相关的问题。

Conclusion: VRAG-RL为视觉丰富信息的检索与推理提供了有效解决方案，通过强化学习策略提升了模型在实际应用中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）在关键应用中的快速部署，亟需一种鲁棒的安全基准测试方法。

Method: 利用少量开发模型和现有越狱攻击算法生成候选提示池，并通过提示选择算法筛选有效子集作为安全基准。

Result: 实验表明，JBDistill基准在13种不同评估模型上表现优异，显著优于现有基准。
...
<details>
  <summary>Details</summary>
Motivation: 旨在将语音克隆技术应用于代表性不足的语言，如瑞士德语方言。

Method: 预处理大量瑞士播客数据，自动转录并标注方言类别，微调XTTSv2模型。

Result: 模型在人机评估中表现良好，CMOS得分达-0.28，SMOS得分3.8。

Conclusion: 展示了语音克隆技术对少数语言的适应性，为未来研究奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 解决RAG系统中直接传递私有文档导致的成员推理攻击问题。

Method: 提出Mirabel框架，基于相似性检测并采用检测-隐藏策略。

Result: 实验证明Mirabel能有效防御多种先进MIA方法，且保持数据实用性。

Conclusion: Mirabel为私有RAG系统提供了高效且通用的防御方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何通过结合SFT和RLVR提升大语言模型在科学信息提取任务中的表现，弥补其在推理和记忆能力上的不足。

Method: 提出两阶段训练方法：1. MimicSFT（使用结构化推理模板），2. R²GRPO（结合相关性和规则诱导奖励）。

Result: 实验表明，该方法在科学信息提取基准测试中显著提升推理能力，R²GRPO结合MimicSFT在关系提取任务中超越基线模型。

Conclusion: 结合SFT和RLVR的两阶段训练方法能有效提升大语言模型的推理能力，尤其在科学信息提取任务中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 尽管指令跟随的LLM在未见任务上表现良好，但在需要领域知识的任务中仍存在困难。因此，研究旨在通过CA领域的专用指令微调解决这一问题。

Method: 通过分析现有CA研究，生成了105个CA任务的自然语言指令，并构建了CA专用基准。采用自指导过程合成了52k条CA相关指令，训练了CA专用指令跟随LLM。

Result: 实验表明，CA专用指令微调显著提升了LLM在已见和未见CA任务上的表现，同时其在通用NLP任务上的性能保持稳定。

Conclusion: 专用指令微调是提升LLM在特定领域任务中表现的有效方法，且不影响其通用能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有MRAG方法采用静态检索流程，忽略了MLLMs的动态推理能力，导致检索效率低下。

Method: 提出R1-Router框架，动态生成查询并路由至合适的知识库；引入Step-GRPO算法优化推理行为。

Result: 在多个开放域QA基准测试中，R1-Router性能优于基线模型7%以上，且能高效利用多样化知识库。

Conclusion: R1-Router通过动态检索和强化学习，显著提升了多模态生成任务的准确性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的方法在处理多样化、领域特定的查询时，生成的SQL准确性不足，需要一种更全面的知识支持。

Method: 构建一个综合知识库，结合问题和数据库模式及其相关知识，从中检索并生成查询所需知识。

Result: 在多个Text-to-SQL数据集上验证，尤其在重叠和非重叠数据库场景中显著优于基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM主要依赖参数化内存和临时激活内存，缺乏统一的结构化内存管理架构，限制了长期知识进化的能力。

Method: 引入MemOS，通过MemCube标准化内存抽象，实现对异构内存的跟踪、融合和迁移，并提供结构化的访问机制。

Result: MemOS填补了LLM基础设施中的关键空白，为持续适应、个性化智能和跨平台协调奠定了基础。

Conclusion: MemOS首次将内存提升为一类操作资源，为下一代智能系统提供了可控、适应性强且可进化的内存中心执行框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统注意力机制在长上下文建模中存在计算冗余问题，所有令牌消耗相同计算资源，而实际上注意力权重稀疏，仅有少数令牌对预测有显著贡献。

Method: 将概率序列建模重新定义为监督学习任务，分析注意力稀疏性，提出分组编码策略，并基于此设计动态组注意力（DGA）。

Result: 实验表明DGA显著降低计算成本，同时保持竞争力。

Conclusion: DGA通过减少冗余计算，提高了长上下文建模的效率，为注意力机制优化提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）在复杂任务中表现优异，但在简单任务中因过度思考（生成冗余推理链）而效率低下，亟需系统性研究。

Method: 引入Think-Bench基准，提出新的效率指标，并从推理过程、结果质量和思维链（CoT）特性等多维度评估LRMs。

Result: 多数LRMs在简单问题上过度思考，生成冗长推理链；部分LRMs的CoT质量高但效率低。

Conclusion: Think-Bench为LRMs研究提供了坚实基础，未来可推动效率优化。
...
<details>
  <summary>Details</summary>
Motivation: 术中低血压（IOH）与不良预后密切相关，但预测困难，主要由于事件稀疏性和数据多样性问题。

Method: 采用两阶段训练策略：1）基于扩散方法增强的生理时间序列进行领域自适应预训练；2）在原始临床数据集上进行任务微调。通过多模态融合对齐临床描述与生理数据。

Result: 在两个术中数据集上，IOHFuseLM表现优于基线模型，能更准确地识别IOH事件。
...
Method: 研究了负样本选择（监督学习）和重排序（无监督学习）策略，并在47种语言的数据集上进行了评估。

Result: 最佳结果来自基于LLM的重排序方法，其次是基于句子相似性的负样本微调。跨语言检索具有独特特性。

Conclusion: 跨语言检索在多语言环境中具有独特挑战和潜力，LLM重排序和负样本选择是有效的改进策略。
...
Conclusion: LoKI在任务专业化和通用能力保留之间实现了最佳平衡，代码已开源。
...
TL;DR: 论文提出EULER模型，通过生成高质量的错误解决方案来增强大语言模型（LLMs）的数学推理能力，实验显示性能提升超过4%。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以生成每个数学问题的错误解决方案，限制了LLMs在监督微调（SFT）中的性能提升。

Method: EULER模型优化错误暴露模型，提高自生成错误解决方案的概率，同时利用优质LLM的解决方案规范生成质量。
...
TL;DR: RAD框架通过自推测解码识别Transformer中的冗余层，并用SSM组件替换，结合针对性蒸馏提升性能。
...
Method: 使用自推测解码诊断冗余层，选择性替换为SSM组件，并进行针对性蒸馏。

Result: 在数学和编程任务上显著超越基线模型，收敛速度提升2倍，GSM8K和CRUX分数大幅提高。
...
TL;DR: 本文重新评估了BERT类模型在识别论点任务中的泛化能力，发现其依赖数据集特定线索而非真正任务对齐，但通过任务特定预训练和联合基准训练可提升性能。


<details>
  <summary>Details</summary>
Motivation: 论证识别是自动化话语分析的关键任务，现有研究认为BERT类模型在多种辩论场景中表现优异，但缺乏对其泛化能力的系统性评估。

Method: 评估了四个Transformer模型（三个标准模型和一个通过对比预训练增强的模型）在17个英语句子级数据集上的表现。

Result: 模型依赖词汇捷径而非任务对齐，在熟悉数据集上表现良好但在未见数据集上性能显著下降。任务特定预训练和联合训练能提升泛化能力。

Conclusion: BERT类模型在论点识别中的表现受数据集特定线索影响，但通过针对性训练可改善其泛化能力。
...
TL;DR: 论文提出InComeS框架，通过压缩和选择机制增强LLMs处理编辑上下文的能力，解决现有方法在复杂场景中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有模型编辑方法在需要深层语义理解的复杂场景中表现不佳，且ICL方法受限于LLMs的上下文窗口。

Method: 提出InComeS框架，通过压缩编辑上下文为KV缓存，并添加动态选择模块优化信息利用。

Result: 实验证明InComeS在多编辑格式下高效有效。

Conclusion: InComeS显著提升了LLMs在模型编辑任务中的性能和效率。
...
TL;DR: 本文提出了一种高效且通用的数据选择方法，通过多步骤流程对数据进行分组、质量评估和难度评分，同时结合任务分类和聚类算法保证多样性，从而实现高性能的微调。
...
Method: 采用多步骤流程：数据分组、质量评估、难度评分，结合任务分类和聚类算法。

Result: 实现了高效且通用的数据选择，支持高性能微调。

Conclusion: 该方法显著降低了数据选择的计算成本，同时保证了多样性和性能。
...
TL;DR: NeoDiff结合离散和连续扩散模型的优势，提出非同步连续扩散模型，通过泊松扩散过程和自适应时间预测器，实现更精细的噪声控制和文本生成。


<details>
  <summary>Details</summary>
Motivation: 离散和连续扩散模型各有局限性：离散模型缺乏细粒度控制，连续模型无法捕捉语义细微差异。NeoDiff旨在整合两者优势。

Method: NeoDiff采用泊松扩散过程进行前向噪声添加，并引入时间预测器在反向过程中自适应调节去噪进度。

Result: 实验表明，NeoDiff在多项文本生成任务中表现优于基线模型，包括非自回归和自回归扩散方法。

Conclusion: NeoDiff为文本生成提供了更高效和理论统一的框架，展示了扩散模型在高质量文本生成中的潜力。
...
TL;DR: 论文提出了一种基于随机方法评估LLM对提示敏感性的框架ReliableEval，强调单次提示评估的不可靠性，并测试了包括GPT-4o和Claude-3.7-Sonnet在内的模型。


<details>
  <summary>Details</summary>
Motivation: 标准基准测试通常使用单一提示评估LLM性能，但LLM对提示措辞高度敏感，这种评估方式可能不可靠。

Method: 提出了一种基于随机方法评估提示扰动的框架ReliableEval，定义了可靠评估的标准，并估计所需提示重采样次数。

Result: 测试发现，即使是顶级模型如GPT-4o和Claude-3.7-Sonnet，也存在显著的提示敏感性。

Conclusion: 该框架为LLM评估提供了一种模型、任务和指标无关的鲁棒方法。
...
<details>
  <summary>Details</summary>
Motivation: 处理多约束复杂指令时，现有方法基于约束满足数量选择偏好对，但会引入噪声，导致选中的响应可能不完美，而被拒绝的响应在某些方面可能优于选中的响应。

Method: 提出反向偏好优化（RPO），动态反转指令中的约束，确保选中响应完美，减少噪声，同时扩大选中与被拒绝响应之间的差距，明确优化方向。

Result: 在Sysbench和Multi-IF两个多轮指令跟随基准测试中，RPO平均比DPO基线分别提升了4.6和2.5分（Llama-3.1 8B模型）。70B参数的RPO模型甚至超越了GPT-4o。

Conclusion: RPO是一种简单有效的方法，能够显著提升大型语言模型在多约束指令跟随任务中的性能，且在不同模型规模下均表现出良好的扩展性。
...
TL;DR: 论文提出了一种名为TabXEval的两阶段评估框架，结合结构对齐和语义-句法比较，用于全面评估表格质量，并通过TabXBench基准验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统表格评估指标难以捕捉结构和内容的细微差异，因此需要一种更全面且可解释的评估方法。

Method: TabXEval框架分为两阶段：TabAlign实现结构对齐，TabCompare进行语义和句法比较。使用TabXBench基准测试其效果。
...
TL;DR: 论文研究了如何结合推测解码和量化技术来加速大语言模型的推理，发现4位量化模型的优势被推测解码的计算负载抵消，提出了一种分层框架以优化性能。


<details>
  <summary>Details</summary>
Motivation: 推测解码和量化技术分别通过验证多令牌和压缩权重来加速推理，但结合时4位量化的优势被推测解码的计算负载抵消，需要新的设计来优化。

Method: 提出分层框架，用小模型作为中间阶段将树状草稿转为序列草稿，利用量化模型的内存访问优势。

Result: 实验显示分层方法在4位量化Llama-3-70B模型上实现2.78倍加速，优于EAGLE-2的1.31倍。

Conclusion: 分层框架有效结合推测解码和量化技术，显著提升推理速度。
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体中伪装毒性内容（如同音字替换）的增多，现有方法主要针对英文，中文领域尚未解决，亟需新方法。

Method: C²TU通过子串匹配识别候选毒性词，结合BERT和LLMs模型过滤非毒性内容并修正伪装词。

Result: 在中文毒性数据集上，C²TU的F1分数和准确率分别比最佳竞争对手高71%和35%。
...
TL;DR: 论文探讨了预训练语言模型（LMs）是否能从基于token的生成提升到基于句子和语义单元的抽象推理，提出了两种嵌入范式（语义嵌入和上下文嵌入），并在四个领域验证了其性能。


<details>
  <summary>Details</summary>
Motivation: 人类推理基于高级抽象（如句子和概念），而LMs目前仅基于token生成，研究旨在探索LMs是否能过渡到抽象推理空间。

Method: 提出一个框架，将预训练的token级LM适配到句子空间，通过自回归预测连续句子嵌入。研究了两种嵌入范式：语义嵌入（通过自编码保留表面意义）和上下文嵌入（通过下一句预测编码结构）。

Result: 在数学、逻辑、常识和规划四个领域中，上下文嵌入在连续推理下表现与Chain-of-Thought（CoT）相当，同时平均减少一半推理FLOPs。

Conclusion: 研究表明，预训练LMs可以有效地过渡到潜在嵌入空间中的抽象结构化推理。
...
TL;DR: JQL是一种高效的多语言数据标注系统，利用轻量级标注器显著提升数据质量和多样性，优于现有启发式过滤方法。


<details>
  <summary>Details</summary>
Motivation: 高质量多语言训练数据稀缺，现有数据集依赖启发式方法，限制了跨语言迁移和扩展性。

Method: JQL通过预训练多语言嵌入将大语言模型的标注能力蒸馏为轻量级标注器，支持未见语言和脚本。

Result: 在35种语言上验证，JQL显著优于Fineweb2等现有方法，提升下游模型训练质量和数据保留率。

Conclusion: JQL为多语言数据标注提供了实用工具和资源，提升了多语言数据集开发标准。
...
TL;DR: 研究分析了TTS系统对句法边界的敏感性，发现系统在句法模糊的句子中表现不佳，需依赖逗号等表面标记；而在简单句法中能利用深层句法线索。通过微调模型，提升了语调模式的准确性。


<details>
  <summary>Details</summary>
Motivation: 探索TTS系统如何利用句法信息生成语调短语边界，特别是在句法模糊的情况下。

Method: 采用心理语言学启发的方法，分析TTS系统在生成语调短语边界时的表现，并对模型进行微调。

Result: TTS系统在句法模糊的句子中依赖表面标记，而在简单句法中能利用深层句法线索；微调后语调模式更准确。

Conclusion: TTS系统可通过微调学习利用更细微的语言线索，从而更准确地反映句法结构。
...
TL;DR: BioHopR是一个新的生物医学多跳推理基准，填补了现有评估工具的空白，揭示了当前模型在多跳推理中的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法评估生物医学领域中的多跳推理，尤其是涉及一对多和多对多关系的查询。

Method: 基于PrimeKG构建BioHopR，包含1跳和2跳推理任务，评估了多个先进模型。

Result: O3-mini在1跳任务中表现最佳（37.93%精度），但所有模型在多跳任务中表现显著下降。

Conclusion: BioHopR为生物医学LLM的评估设定了新标准，揭示了专有与开源模型之间的差距，并推动未来研究。
...
TL;DR: 论文提出了一种基于LLM生成Python代码的方法，用于解决表格数据问答任务，通过多步骤流程实现，最终在子任务1中取得70.50%的分数。


<details>
  <summary>Details</summary>
Motivation: 解决SemEval 2025 Task 8中的表格数据问答挑战，通过自动化生成代码与表格交互以回答问题。

Method: 使用开源LLM生成自然语言指令，将其转化为Python代码，执行并处理错误，每个步骤使用优化提示。

Result: 在子任务1中得分70.50%。

Conclusion: 该方法通过多步骤代码生成与执行，有效解决了表格数据问答问题。
...
TL;DR: 论文研究了非正式表达的词汇规范化问题，针对未分词语言（如日语）提出了三项贡献：构建大规模多领域数据集、开发基于预训练模型的方法，并进行多视角评估。实验表明，编码器和解码器方法在准确性和效率上均表现良好。


<details>
  <summary>Details</summary>
Motivation: 解决用户生成文本中非正式表达的处理问题，尤其是在未分词语言中缺乏全面评估的现状。

Method: 构建大规模多领域日语规范化数据集，开发基于预训练模型的规范化方法，并进行多视角实验评估。

Result: 编码器和解码器方法在准确性和效率上均取得良好效果。

Conclusion: 论文为未分词语言的词汇规范化提供了有效方法和数据集，展示了预训练模型在此任务中的潜力。
...
TL;DR: 本文综述了129项研究，探讨了自然语言处理（NLP）在循证医学（EBM）中的应用，强调了NLP在临床决策中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 由于医学文献数量庞大且增长迅速，人工整理成本高，亟需利用NLP技术优化EBM的证据识别、评估、综合与传播。

Method: 系统回顾了129项研究，分析NLP如何支持EBM的五个核心步骤（Ask, Acquire, Appraise, Apply, Assess）。

Result: NLP在EBM中展现出巨大潜力，可改进证据提取、综合、评估及临床工作流程，但当前仍存在局限性。

Conclusion: 未来研究应进一步探索NLP在EBM中的应用，以提升证据处理效率和临床决策质量。
...
TL;DR: 论文提出Fragment-Shot Prompting和Pivoted Fragment-Shot方法，用于低资源语言的翻译，实验表明其有效性，并发现模型推理能力对翻译质量有显著影响。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在低资源语言翻译中的挑战，特别是提示工程的局限性。

Method: 提出Fragment-Shot Prompting和Pivoted Fragment-Shot方法，基于句法覆盖分段输入并检索翻译示例。

Result: 方法对低资源语言翻译有效，模型推理能力与翻译质量正相关，提示工程对高资源语言翻译改进有限。

Conclusion: Fragment-Shot Prompting和Pivoted Fragment-Shot是低资源语言翻译的有效方法，模型能力是关键因素。
...
TL;DR: 360-LLaMA-Factory开源项目通过引入序列并行技术，支持了多个模型的开发与应用，并深入探讨了其实现细节。


<details>
  <summary>Details</summary>
Motivation: 提升LLaMA-Factory的功能，支持更高效的序列并行处理，以满足多样化的模型训练需求。

Method: 在LLaMA-Factory中集成序列并行技术，并开源为360-LLaMA-Factory。

Result: 项目被广泛应用于多个模型（如Light-R1、TinyR1等）及大型公司的训练框架中。

Conclusion: 360-LLaMA-Factory通过序列并行技术为模型训练提供了高效支持，具有广泛的应用前景。
...
TL;DR: ToxEdit是一种动态检测和缓解大型语言模型中毒性内容的方法，通过自适应层间路径计算，有效减少毒性同时保留模型的一般能力。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型解毒方法存在实体依赖和过度编辑问题，无法有效应对无明确实体的对抗性输入，且可能损害模型性能。

Method: ToxEdit通过动态检测前向传播中的毒性激活模式，并利用自适应层间路径计算来缓解毒性。

Result: 实验表明，ToxEdit在解毒性能和保留模型一般能力上优于现有方法。

Conclusion: ToxEdit提供了一种高效且精确的解毒方法，同时增强了模型的安全性评估。
...
TL;DR: 论文研究了大型语言模型（LLMs）在逻辑推理中遇到与参数知识冲突时的表现，提出了CounterLogic数据集和Self-Segregate提示方法，显著提升了模型在反事实场景中的推理能力。


<details>
  <summary>Details</summary>
Motivation: LLMs在熟悉情境中表现优异，但在与参数知识冲突的反事实情境中推理能力下降，需要研究如何提升其在此类场景中的表现。

Method: 引入CounterLogic数据集（1,800个例子，9种逻辑模式），评估11种LLMs在6个数据集上的表现，并提出Self-Segregate提示方法，通过元认知意识识别知识冲突。

Result: LLMs在反事实情境中平均准确率下降27%，使用Self-Segregate方法后差距缩小至11%，整体准确率提升7.5%。

Conclusion: 研究揭示了LLMs在知识冲突情境中的局限性，并提出了有效方法提升其推理能力，为实际应用提供了实用见解。
...
TL;DR: 论文提出了一种改进Mixture-of-Experts（MoE）模型的方法，通过引入正交性损失和方差损失来提升专家专业化，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有的辅助负载平衡损失导致专家重叠和路由过于均匀，限制了专家专业化并降低了性能。

Method: 提出两种互补目标：正交性损失（鼓励专家处理不同类型的标记）和方差损失（鼓励更具区分性的路由决策）。

Result: 实验表明，该方法显著提升了专家专业化，性能提升高达23.79%，同时保持下游任务的负载平衡。

Conclusion: 该方法简单有效，无需架构修改或额外组件，显著优化了MoE模型的训练过程。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM的快速发展，NLP领域需要更负责任地部署技术，以应对社会挑战。

Method: 通过跨学科分析社会目标和新兴风险，提出研究方向。

Result: 指出了NLP4SG研究中需要解决的挑战。

Conclusion: 呼吁在NLP研究中注重社会责任和公平性。
...
TL;DR: 论文提出了一种两阶段方法（监督微调+强化学习）提升多模态推理能力，显著优于单一方法，并在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 研究多模态大语言模型（MLLMs）中自修正现象的存在及其与推理性能的关系，探索如何通过结合监督微调和强化学习提升推理能力。

Method: 采用两阶段方法：1) 监督微调（SFT）作为冷启动，引入结构化思维链；2) 使用GRPO强化学习进一步优化。

Result: 实验表明，该方法在3B和7B规模的MLLMs中均优于单一方法，7B模型在MathVista和We-Math上分别提升7.1%和7.5%。

Conclusion: 结合SFT和RL的两阶段方法为构建先进多模态推理模型提供了实用指导。
...
TL;DR: Text2Grad通过将文本反馈转化为细粒度的梯度信号，优化语言模型策略，显著提升任务表现和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统RLHF使用粗糙的标量奖励，掩盖了成功或失败的细节原因，学习过程缓慢且不透明。Text2Grad旨在通过细粒度的文本反馈改进这一问题。

Method: Text2Grad将自由形式的文本反馈转化为跨度级梯度，通过三个组件实现：反馈标注管道、细粒度奖励模型和跨度级策略优化器。

Result: 在摘要、代码生成和问答任务中，Text2Grad表现优于标量奖励RL和仅提示基线，任务指标和可解释性均提升。

Conclusion: 自然语言反馈转化为梯度信号是细粒度策略优化的强大工具。
...
TL;DR: 研究探讨LLMs如何处理虚假预设，分析其在不同条件下对虚假预设的敏感性。


<details>
  <summary>Details</summary>
Motivation: 虚假预设可能嵌入误导信息，研究旨在评估LLMs是否能像人类一样识别并纠正这些预设。

Method: 基于语言学预设分析，实验测试三种LLMs（GPT-4-o、LLama-3-8B、Mistral-7B-v03）对虚假预设的识别能力。

Result: LLMs普遍难以识别虚假预设，表现因条件而异。

Conclusion: 语言学预设分析是揭示LLMs强化政治错误信息的有效工具。
...
TL;DR: Pangu Embedded是一种高效的大型语言模型推理器，通过两阶段训练框架实现快速和慢速推理能力，显著降低计算成本和延迟。


<details>
  <summary>Details</summary>
Motivation: 解决现有推理优化LLM中计算成本高和推理延迟的问题。

Method: 采用两阶段训练框架：第一阶段通过迭代蒸馏和强化学习优化模型；第二阶段引入双系统框架，支持快速和慢速推理模式。

Result: 在多个基准测试中表现优异，7B参数的Pangu Embedded优于类似规模模型。

Conclusion: Pangu Embedded展示了高效且实用的LLM推理器发展方向。
...
TL;DR: RAG-Zeval是一个新颖的端到端框架，通过规则引导的推理任务评估检索增强生成系统的忠实性和正确性，显著降低计算成本并提升评估性能。
...
Method: 采用强化学习训练评估器，通过排名奖励机制和零人工标注生成质量控制的响应。

Result: RAG-Zeval在人类判断相关性上表现最佳，优于参数多10-100倍的基线模型，且具有更好的解释性。

Conclusion: RAG-Zeval为高效、可解释的RAG系统评估提供了新方法，显著优于现有技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有监督方法依赖昂贵的人工标注数据，而无监督方法复杂且难以迭代。

Method: 基于GRPO算法，采用多数投票的自奖励机制，结合合成问题生成。

Result: 在MathVista和We-Math数据集上显著提升性能，接近监督方法效果。

Conclusion: MM-UPT为无监督环境下持续增强MLLMs提供了新范式。
...
TL;DR: EvolveSearch是一种结合监督微调（SFT）和强化学习（RL）的自进化框架，用于提升LLM在开放网络搜索领域的能力，无需外部人工标注数据。


<details>
  <summary>Details</summary>
Motivation: 当前主流方法在开放搜索领域存在数据效率低的问题，SFT难以生成数据，RL收敛过快。

Method: 提出EvolveSearch框架，通过迭代自进化结合SFT和RL，无需外部人工数据。

Result: 在七个多跳问答基准测试中，性能平均提升4.7%，超越现有最佳方法。

Conclusion: EvolveSearch为开放网络搜索领域的自进化能力提供了新方向。
...
TL;DR: 论文提出了一种两阶段知识蒸馏框架，通过利用教师模型生成的知识，提升小型多模态大语言模型（MLLMs）在低资源场景下检测多模态上下文无关新闻的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖大量标注数据或昂贵的GPT模型调用，不适用于低资源场景。本文旨在以更高效和低成本的方式提升小型MLLMs的性能。

Method: 1. 通过多个教师MLLMs生成标签预测和解释作为知识；2. 采用两阶段知识蒸馏框架（LoRA微调和DPO）将知识迁移到学生模型。

Result: 实验表明，该方法仅需不到10%的标注数据即可达到最先进性能。

Conclusion: 提出的方法在低资源场景下显著提升了小型MLLMs的性能，具有高效和低成本的优势。
...
TL;DR: 提出了一种任务自适应的推理框架DeepSeek-R1，通过可变长度推理链和复合奖励函数，显著提升了情感理解任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前方法依赖固定长度的CoT推理，无法适应情感复杂性的变化。

Method: 结合微调和强化学习，设计复合奖励函数，平衡预测准确性、推理深度控制、路径多样性和逻辑重复抑制。

Result: 在情感、情绪、幽默和讽刺任务中，Acc和F1分数均有提升，高级任务提升尤为显著。

Conclusion: 通过自适应深度分析，填补了固定CoT推理与情感复杂性之间的鸿沟。
...
TL;DR: ClaimPKG框架通过结合知识图谱（KGs）和大语言模型（LLMs）提升声明验证的推理能力，实现了端到端的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有声明验证方法主要依赖非结构化文本，难以有效利用KGs的结构化知识；同时，LLMs在多步推理和KG适应方面存在不足。

Method: ClaimPKG使用轻量级专用LLM生成伪子图，指导子图检索模块从KG中提取相关子图，再由通用LLM生成最终结论。

Result: 在FactKG数据集上，ClaimPKG性能优于基线9%-12%，并在非结构化数据集上展示了零样本泛化能力。

Conclusion: ClaimPKG成功结合了KGs的结构化知识与LLMs的推理能力，为声明验证提供了高效且通用的解决方案。
...
TL;DR: 研究探讨大型语言模型（LLMs）与人类大脑在计算原理上的相似性，发现模型性能提升会推动其表征架构向类似大脑的层次结构演化。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs是否仅因规模扩大而表现出类似大脑的模式，还是与人类语言处理架构存在更深层次的对齐。

Method: 通过比较14种公开LLMs的分层嵌入与人类在自然叙事故事中的fMRI数据，构建句子级神经预测模型。

Result: 模型性能的提升使其表征架构更接近大脑的层次结构，尤其在更高语义抽象水平上表现出更强的功能和解剖对应。

Conclusion: LLMs的表征架构在性能优化过程中逐渐与人类大脑的语言处理机制对齐。
...
TL;DR: 本文提出了一种基于LLM代理的统一检索增强生成（RAG）系统框架Agent-UniRAG，支持单跳和多跳查询，并引入合成数据集SynAgent-RAG提升小规模开源LLM性能。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统多专注于单跳或多跳查询，限制了实际应用；本文旨在通过统一框架提升RAG系统的效果和可解释性。

Method: 设计基于LLM代理的框架Agent-UniRAG，根据输入复杂度逐步解决RAG任务，并引入合成数据集SynAgent-RAG支持小规模开源LLM。

Result: 在多个RAG基准测试中，性能与闭源及大规模开源LLM相当。

Conclusion: Agent-UniRAG为统一RAG系统提供了有效且可解释的解决方案，代码和数据集已开源。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法局限于单层或固定层操作，无法灵活适应不同提示需求，因此需要一种更动态、灵活的引导方法。

Method: 采用全层和分段引导配置，动态注入基于参考完成的激活增量，并通过Optuna优化注入权重。

Result: 在260个SimpleQA提示上，分段引导的准确率达到25.4%，显著优于基线（3.5%）和全层引导（16.2%）。

Conclusion: 分段动态干预策略和全网络激活控制具有潜力，稀疏表示方法为可解释和可扩展的LLM控制提供了方向。
...
TL;DR: 论文提出了一种层级的专家分配算法（LayerMoE），通过分析不同层的语言特征，动态分配新专家数量，以减少参数成本并缓解旧语言遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 构建多语言大语言模型（LLMs）时，持续扩展新语言同时避免旧语言能力退化是一个关键挑战。现有方法参数成本高且难以完全避免性能下降。

Method: 通过分析LLMs不同层的语言表示相似性，动态分配新专家数量（相似性越高，专家越少），并在高相似性层前添加分类器以引导旧语言令牌的路由。

Result: 实验表明，该方法在单次扩展和持续扩展场景中，分别减少了60%和33.3%的专家数量，性能优于现有基线。

Conclusion: LayerMoE通过层级的专家分配和路由优化，有效降低了参数成本并缓解了旧语言遗忘问题，为多语言LLMs的扩展提供了高效解决方案。
...
TL;DR: PISCES是一种通过直接编辑参数空间中的方向来精确擦除语言模型中概念的新框架，相比现有方法更有效、更精准。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在预训练中可能获取不适宜下游任务的知识（如敏感信息或版权内容），现有擦除方法效果有限。

Method: PISCES利用解缠模型分解MLP向量为可解释特征，通过自动解释技术识别目标概念并移除。

Result: 在Gemma 2和Llama 3.1上，PISCES将目标概念准确率降至7.7%，擦除特异性提升31%，鲁棒性提升38%。

Conclusion: 基于特征的参数内编辑为语言模型知识擦除提供了更精准可靠的方法。
...
TL;DR: 论文提出Self-Error-Instruct (SEI)框架，通过分析模型在数学推理中的错误案例，生成更通用的训练数据，提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数学推理中存在许多错误案例，现有方法仅从孤立案例中学习，无法泛化错误模式。

Method: 通过分析GSM8K和MATH数据集中的错误案例，生成错误关键词并聚类错误类型，利用GPT-4o生成针对性训练数据，并通过单样本学习优化数据。

Result: 实验表明，SEI框架提升了模型在数学推理任务中的性能，包括域内和域外数据集。

Conclusion: SEI框架通过泛化错误模式，有效提升了语言模型的数学推理能力。
...
TL;DR: 论文提出了一种针对双向扩散模型的块级近似KV缓存机制和置信感知并行解码策略，显著提升了Diffusion LLMs的推理速度，同时保持生成质量。
...
Result: 实验显示吞吐量提升27.6倍，且准确性损失极小。

Conclusion: 该方法缩小了Diffusion LLMs与自回归模型的性能差距，为其实际部署铺平了道路。
...
TL;DR: CoTalk是一种AI辅助的标注方法，通过顺序标注和多模态界面优化标注效率，提升语义完整性和视觉-语言对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有密集标注方法在优化人工标注效率方面不足，需探索更高效的方法。

Method: 采用顺序标注减少冗余，结合多模态界面（阅读+语音）提升标注速度。

Result: 实验显示CoTalk标注速度（0.42单位/秒）和检索性能（41.13%）优于并行方法。

Conclusion: CoTalk在固定预算下显著提升标注效率和语义覆盖，适用于视觉-语言对齐任务。
...
TL;DR: 论文探讨了大型语言模型（LLMs）在生成文本时的错误模式，尤其是无关上下文幻觉问题，揭示了其基于类别的（错误）泛化机制。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在生成文本时的错误是否具有规律性，以反驳其仅为随机模仿的观点。

Method: 通过行为分析和机制可解释性实验，研究了Llama-3、Mistral和Pythia模型在39种事实回忆关系中的表现。

Result: 发现模型通过类别抽象和上下文线索结合生成答案，内部计算中存在两种竞争电路影响输出。

Conclusion: LLMs能通过抽象进行泛化，但受上下文线索影响不可靠，称为“随机变色龙”。
...
TL;DR: SKG2Data是一种基于空间知识图的多模态合成方法，旨在提升多模态大语言模型的空间感知能力。
...
TL;DR: 论文提出了一种改进链式思维（CoT）数据格式的方法，通过组合原子任务的CoT数据提升模型在未见组合任务上的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖标注的CoT数据，成本高且泛化能力有限，希望模型能组合原子推理技能解决未见任务。

Method: 改进原子任务的CoT格式使其可组合，结合多任务学习或模型合并，并通过拒绝采样微调（RFT）进一步优化。

Result: 在字符串操作和自然语言技能组合任务中，Composable CoT训练优于多任务学习和持续微调基线。

Conclusion: 组合原子任务的CoT数据能有效提升模型在组合任务上的泛化能力，且在小规模数据下表现更优。
...
TL;DR: 研究探讨大型语言模型（LLM）在简体中文和繁体中文中的表现差异，发现任务和提示语言会影响模型偏见，并开源了基准数据集。


<details>
  <summary>Details</summary>
Motivation: 了解LLM在简体与繁体中文中的表现差异，以避免因模型偏见导致的代表性伤害和决策领域（如教育、招聘）的下游危害。

Method: 设计两项基准任务（区域术语选择和区域名称选择），评估11种商业和开源LLM的表现。

Result: 发现大多数LLM在区域术语选择中偏向简体中文，而在区域名称选择中偏向繁体中文，差异源于训练数据、字符偏好和分词方式。

Conclusion: 需进一步分析LLM偏见，开源基准数据集以促进未来研究。
...
TL;DR: 提出了一种端到端的信息搜索代理训练范式，包含数据构建、轨迹采样、监督微调和强化学习四个阶段，并在WebDancer代理中实现，表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决复杂现实问题需要多步推理和深度信息搜索，现有代理系统（如Deep Research）展示了自主多步研究的潜力。

Method: 提出四阶段训练范式：1) 浏览数据构建；2) 轨迹采样；3) 监督微调；4) 强化学习。基于ReAct框架实现WebDancer代理。

Result: 在GAIA和WebWalkerQA基准测试中表现优异，验证了训练范式的有效性。

Conclusion: 该范式为开发更强大的代理模型提供了系统化路径，代码和演示已开源。
...
TL;DR: 研究探讨了奖励噪声对大型语言模型（LLM）后训练的影响，发现LLM对高噪声具有强鲁棒性，且仅奖励关键推理短语（RPR）即可达到与严格验证相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中奖励模型常含噪声，研究旨在验证LLM在噪声环境下的表现及其对推理过程的依赖。

Method: 通过手动翻转奖励函数输出（40%噪声）和引入RPR（仅奖励推理短语），测试Qwen-2.5-7B在数学任务中的表现。

Result: 40%噪声下模型仍能快速收敛（准确率从5%升至72%），RPR单独使用可达70%准确率，与无噪声训练相当。结合RPR可校准噪声奖励模型，提升开放任务表现。

Conclusion: 研究强调预训练阶段提升模型基础能力的重要性，并为后训练技术提供新思路。
...
TL;DR: GuessArena是一个基于对抗性游戏的动态评估框架，用于解决传统静态基准测试在适应性和细粒度评估上的不足。


<details>
  <summary>Details</summary>
Motivation: 传统静态基准测试缺乏对多样化应用领域的适应性，且无法精细评估领域知识和上下文推理能力。

Method: 采用类似“猜猜我是谁”游戏的交互结构，结合动态领域知识建模和渐进式推理评估。

Result: 在五个垂直领域（金融、医疗、制造、信息技术和教育）中，GuessArena能有效区分LLMs的领域知识覆盖和推理链完整性。

Conclusion: GuessArena在可解释性、可扩展性和场景适应性上优于传统基准测试。
...
TL;DR: AutoL2S是一种动态框架，帮助大型语言模型（LLMs）根据问题复杂度动态调整推理路径长度，减少不必要的长推理，提升效率。
...
Method: 提出AutoL2S框架，通过训练LLMs使用包含长短推理路径和特殊<EASY>标记的标注数据，动态压缩推理路径。

Result: AutoL2S将推理路径长度减少57%，且不影响性能。

Conclusion: AutoL2S有效提升了LLM推理的效率和可扩展性。
...
### [102] [Enhancing Vision Transformer Explainability Using Artificial Astrocytes](https://arxiv.org/abs/2505.21513)
*Nicolas Echevarrieta-Catalan,Ana Ribas-Rodriguez,Francisco Cedron,Odelia Schwartz,Vanessa Aguiar-Pulido*

Main category: cs.CV

TL;DR: 提出了一种无需训练的ViTA方法，通过模拟神经科学中的星形胶质细胞，提升预训练模型的解释性与人类感知对齐。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习模型解释性不足，且随着复杂度增加解释性降低，现有方法适用性有限。

Method: 提出ViTA方法，结合神经科学中的星形胶质细胞机制，无需额外训练即可增强模型解释性。

Result: 实验表明，ViTA显著提升了Grad-CAM和Grad-CAM++生成的解释与人类感知的对齐度。

Conclusion: ViTA为提升模型解释性提供了有效且无需训练的解决方案，具有广泛应用潜力。
...
### [103] [Do DeepFake Attribution Models Generalize?](https://arxiv.org/abs/2505.21520)
...
<details>
  <summary>Details</summary>
Motivation: 由于DeepFake技术的普及，现有二进制检测模型无法区分不同篡改方法，影响了信息的可信度，因此需要更精细的检测和归因模型。

Method: 利用五种先进骨干模型，在六个DeepFake数据集上进行实验，比较二进制和多类别模型的泛化能力，并评估对比方法的效果。

Result: 二进制模型泛化能力更强，但更大模型、对比方法和高质量数据可提升归因模型的性能。

Conclusion: 归因模型在提升DeepFake检测的可信度和可解释性方面具有潜力，未来研究可进一步优化其性能。
...
### [104] [CIM-NET: A Video Denoising Deep Neural Network Model Optimized for Computing-in-Memory Architectures](https://arxiv.org/abs/2505.21522)
*Shan Gao,Zhiqiang Wu,Yawen Niu,Xiaotao Li,Qingqing Xu*

Main category: cs.CV

TL;DR: 论文提出了一种硬件-算法协同设计框架CIM-NET，通过优化DNN模型以适应CIM芯片架构，显著减少矩阵-向量乘法操作，提升推理速度，同时保持去噪性能。


<details>
  <summary>Details</summary>
Motivation: 现有DNN模型未考虑CIM芯片的架构限制，限制了其在边缘设备上的加速潜力。

Method: 提出CIM-NET架构和伪卷积算子CIM-CONV，结合滑动窗口处理和全连接变换，优化特征提取与重建。

Result: CIM-NET将MVM操作减少至1/77，PSNR仅略有下降（35.11 dB vs. 35.56 dB）。

Conclusion: 硬件-算法协同设计能有效提升CIM芯片上的DNN推理效率，同时保持性能。
...
### [105] [Learning Shared Representations from Unpaired Data](https://arxiv.org/abs/2505.21524)
*Amitai Yacobi,Nir Ben-Ari,Ronen Talmon,Uri Shaham*

Main category: cs.CV

TL;DR: 论文提出了一种从非配对数据中学习共享表示的方法，通过独立构建单模态表示的随机游走矩阵的谱嵌入，实现了跨模态关系的有效捕捉。


<details>
  <summary>Details</summary>
Motivation: 当前多模态表示学习方法依赖配对样本，但配对数据获取困难，因此探索从非配对数据中学习共享表示的可能性。

Method: 利用单模态表示的随机游走矩阵的谱嵌入，独立构建跨模态共享表示空间。

Result: 实验结果表明，该方法在检索、生成、算术、零样本和跨域分类任务中表现优异。

Conclusion: 该研究首次证明几乎仅依赖非配对数据即可学习通用跨模态嵌入，为多模态表示学习提供了新思路。
...
### [106] [UniDB++: Fast Sampling of Unified Diffusion Bridge](https://arxiv.org/abs/2505.21528)
*Mokai Pan,Kaizhen Zhu,Yuexin Ma,Yanwei Fu,Jingyi Yu,Jingya Wang,Ye Shi*

Main category: cs.CV

TL;DR: UniDB++是一种无需训练的采样算法，通过精确闭式解和SDE-Corrector机制，显著提升UniDB框架的推理速度和生成质量。


<details>
  <summary>Details</summary>
Motivation: UniDB框架依赖迭代欧拉采样方法，导致推理速度慢且计算成本高，现有加速技术无法解决其独特挑战。

Method: 推导UniDB反向时间SDE的精确闭式解，减少误差累积；采用数据预测模型替代噪声预测，并引入SDE-Corrector机制。

Result: UniDB++在图像恢复任务中表现优异，推理速度提升20倍，同时保持高质量生成。

Conclusion: UniDB++填补了SOC驱动扩散桥模型的理论通用性与实际效率之间的差距。
...
### [107] [How Much Do Large Language Models Know about Human Motion? A Case Study in 3D Avatar Control](https://arxiv.org/abs/2505.21531)
*Kunhang Li,Jason Naradowsky,Yansong Feng,Yusuke Miyao*

Main category: cs.CV

TL;DR: 研究通过3D虚拟角色控制探索大语言模型（LLMs）对人类动作的理解能力，发现LLMs擅长高层次动作规划，但在精确身体部位定位上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在人类动作知识方面的能力，特别是通过3D虚拟角色控制验证其生成动作计划的准确性。

Method: 通过高层次动作规划和低层次身体部位定位两步法生成动作计划，并转换为3D动画供人类评估。设计了20个代表性动作指令进行综合评估。

Result: LLMs在高层次动作解释上表现良好，但在精确身体部位定位和多步动作规划上存在困难。能够处理一般空间描述，但无法满足精确时空参数需求。

Conclusion: LLMs在创意动作和文化特定动作模式识别上具有潜力，但在精确动作控制方面仍需改进。
...
### [108] [EvidenceMoE: A Physics-Guided Mixture-of-Experts with Evidential Critics for Advancing Fluorescence Light Detection and Ranging in Scattering Media](https://arxiv.org/abs/2505.21532)
...
TL;DR: 提出了一种基于物理指导的混合专家（MoE）框架，用于解决荧光激光雷达（FLiDAR）在散射介质中深度和荧光寿命估计的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统方法在散射介质中难以准确分离光子飞行时间与荧光寿命，限制了FLiDAR的应用效果。

Method: 采用物理指导的MoE框架，结合证据基础的Dirichlet批评器（EDCs）和决策网络，自适应融合专家预测。

Result: 在模拟的FLiDAR数据中，深度估计的NRMSE为0.030，荧光寿命的NRMSE为0.074。

Conclusion: 该方法显著提升了FLiDAR在复杂环境中的性能，为医学和自动驾驶等领域的应用提供了新思路。
...
### [109] [Self-Organizing Visual Prototypes for Non-Parametric Representation Learning](https://arxiv.org/abs/2505.21533)
*Thalles Silva,Helio Pedrini,Adín Ramírez Rivera*

Main category: cs.CV

TL;DR: 提出了一种名为SOP的无监督视觉特征学习新方法，通过多语义相似表示（SEs）替代单一原型，提升特征表征能力，并在多项任务中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法依赖单一原型表征数据簇特征，限制了特征表征的全面性和训练效果。

Method: 提出SOP策略，使用多个语义相似表示（SEs）作为原型，并结合非参数损失函数实现；引入SOP-MIM任务，从多局部SEs视角重建掩码表示。

Result: 在检索、线性评估、微调和目标检测等任务中，SOP策略学习到的表征表现优异，尤其在复杂编码器下性能提升显著。

Conclusion: SOP策略通过多SEs表征数据簇特征，显著提升了无监督视觉特征学习的性能，为非参数自监督学习提供了新思路。
...
### [110] [Is Attention Required for Transformer Inference? Explore Function-preserving Attention Replacement](https://arxiv.org/abs/2505.21535)
...
<details>
  <summary>Details</summary>
Motivation: Transformer的注意力机制在推理时效率低，尤其是在边缘设备上。研究发现推理时的序列映射较简单，可用更廉价函数替代。

Method: 提出FAR框架，用多头LSTM替换注意力模块，通过块级蒸馏和全局结构剪枝优化模型。

Result: 在DeiT视觉Transformer上验证，FAR在ImageNet及下游任务中保持原模型精度，同时减少参数和延迟。
...
### [111] [Caption This, Reason That: VLMs Caught in the Middle](https://arxiv.org/abs/2505.21538)
...
<details>
  <summary>Details</summary>
Motivation: 研究VLMs在视觉任务中与人类能力的差距，探索其认知瓶颈。

Method: 采用认知科学方法，设计任务评估VLMs在感知、注意力和记忆方面的表现，并进行视觉-文本解耦分析。

Result: 发现VLMs在空间理解和选择性注意力任务上表现不足，但通过生成文本推理和针对性微调可显著改进。

Conclusion: 研究揭示了VLMs的认知瓶颈，并提出了简单有效的改进方法，为未来模型优化提供了方向。
...
### [112] [Equivariant Flow Matching for Point Cloud Assembly](https://arxiv.org/abs/2505.21539)
*Ziming Wang,Nan Xue,Rebecka Jörnsten*

Main category: cs.CV

TL;DR: 本文提出了一种基于流匹配模型的等变求解器（Eda），用于点云组装任务，通过学习相关向量场实现高效训练，并在非重叠输入情况下表现优异。


<details>
  <summary>Details</summary>
Motivation: 点云组装任务需要将多个点云片段对齐以重建完整3D形状，现有方法在非重叠输入情况下效果不佳，因此需要一种更高效且鲁棒的解决方案。

Method: 提出Eda模型，通过学习与输入片段相关的向量场，构建等变路径，确保训练过程的数据高效性。

Result: 实验结果表明，Eda在实际数据集上表现优异，并能处理输入片段非重叠的挑战性情况。

Conclusion: Eda通过流匹配和等变路径设计，显著提升了点云组装的效率和鲁棒性。
...
### [113] [DiffDecompose: Layer-Wise Decomposition of Alpha-Composited Images via Diffusion Transformers](https://arxiv.org/abs/2505.21541)
*Zitong Wang,Hang Zhao,Qianyu Zhou,Xuequan Lu,Xiangtai Li,Yiren Song*

Main category: cs.CV

TL;DR: 本文提出了一种新任务：Alpha合成图像的逐层分解，旨在从单张重叠图像中恢复半透明/透明层的非线性遮挡。通过引入AlphaBlend数据集和DiffDecompose框架，解决了层模糊性、泛化性和数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像分解方法难以处理半透明或透明层遮挡，因依赖掩码先验、静态对象假设及缺乏数据集。本文旨在解决这些问题。

Method: 提出DiffDecompose框架，基于扩散Transformer，学习输入图像、语义提示和混合类型的条件后验分布，实现无逐层监督的上下文分解。

Result: 在AlphaBlend和LOGO数据集上的实验验证了DiffDecompose的有效性。

Conclusion: DiffDecompose为透明和半透明层分解提供了高效解决方案，代码和数据集将公开。
...
### [114] [Vision Meets Language: A RAG-Augmented YOLOv8 Framework for Coffee Disease Diagnosis and Farmer Assistance](https://arxiv.org/abs/2505.21544)
*Semanto Mondal*
...
<details>
  <summary>Details</summary>
Motivation: 传统农业资源利用效率低且对环境造成挑战，精准农业通过技术优化农业流程成为解决方案。

Method: 结合视觉与语言模型，利用YOLOv8进行病害检测，RAG提供上下文感知诊断，NLP生成治疗方案。

Result: 系统能实时检测咖啡叶病害并提供环保的治理建议，减少农药使用。

Conclusion: 该系统具有可扩展性、可靠性和用户友好性，未来可推广至更广泛的农业应用。
...
### [115] [Corruption-Aware Training of Latent Video Diffusion Models for Robust Text-to-Video Generation](https://arxiv.org/abs/2505.21545)
*Chika Maduabuchi,Hao Chen,Yujin Han,Jindong Wang*

Main category: cs.CV

TL;DR: CAT-LVDM是一种针对LVDMs的鲁棒性训练框架，通过数据对齐的噪声注入提升模型性能，显著减少语义漂移和时间不连贯问题。


<details>
  <summary>Details</summary>
Motivation: 解决LVDMs在噪声视频-文本数据集上因不完美条件导致的语义漂移和时间不连贯问题。

Method: 提出Batch-Centered Noise Injection (BCNI)和Spectrum-Aware Contextual Noise (SACN)，分别通过批内语义方向和频谱方向注入噪声。
...
### [116] [Image Tokens Matter: Mitigating Hallucination in Discrete Tokenizer-based Large Vision-Language Models via Latent Editing](https://arxiv.org/abs/2505.21547)
*Weixing Wang,Zifeng Ding,Jindong Gu,Rui Cao,Christoph Meinel,Gerard de Melo,Haojin Yang*

Main category: cs.CV

TL;DR: 论文研究了大型视觉语言模型（LVLMs）中幻觉非存在对象的问题，提出了一种基于图像标记共现图的方法来缓解幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 发现LVLMs在训练中因视觉先验导致图像标记与对象描述强关联，从而引发幻觉。

Method: 构建图像标记共现图，使用图神经网络和对比学习聚类高频共现标记，通过修改潜在图像嵌入抑制幻觉。
...
### [117] [Distill CLIP (DCLIP): Enhancing Image-Text Retrieval via Cross-Modal Transformer Distillation](https://arxiv.org/abs/2505.21549)
*Daniel Csizmadia,Andrei Codreanu,Victor Sim,Vighnesh Prabeau,Michael Lu,Kevin Zhu,Sean O'Brien,Vasu Sharma*

Main category: cs.CV

TL;DR: DCLIP是一种改进的CLIP模型，通过师生蒸馏框架增强多模态图像-文本检索能力，同时保留零样本分类性能。
...
Method: 采用跨模态Transformer教师模型和轻量级学生模型，通过对比学习和余弦相似性目标进行训练。

Result: DCLIP显著提升了图像-文本检索指标（Recall@K, MAP），同时保留了94%的零样本分类性能。

Conclusion: DCLIP在任务专业化和泛化之间取得了平衡，为高级视觉-语言任务提供了高效解决方案。
...
### [118] [Benign-to-Toxic Jailbreaking: Inducing Harmful Responses from Harmless Prompts](https://arxiv.org/abs/2505.21556)
*Hee-Seon Kim,Minbeom Kim,Wonjun Lee,Kihyun Kim,Changick Kim*

Main category: cs.CV

TL;DR: 论文提出了一种新的攻击范式Benign-to-Toxic (B2T)，通过优化对抗图像从良性条件诱导毒性输出，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有Toxic-Continuation范式在缺乏明确毒性信号时效果不佳，需探索更有效的攻击方式。

Method: 提出B2T范式，优化对抗图像以在无毒性条件下突破模型安全机制。

Result: B2T方法优于现有方法，适用于黑盒场景，并能与文本攻击互补。

Conclusion: 揭示了多模态对齐中的新漏洞，为攻击方法提供了新方向。
...
### [119] [Analytical Calculation of Weights Convolutional Neural Network](https://arxiv.org/abs/2505.21557)
*Polad Geidarov*

Main category: cs.CV

TL;DR: 提出一种无需传统训练的CNN权重和阈值解析计算方法，仅需10张MNIST图像即可确定参数，实验表明其能快速识别半数以上手写数字。


<details>
  <summary>Details</summary>
Motivation: 探索无需训练的CNN参数解析计算方法，简化网络构建流程。

Method: 基于10张MNIST图像解析计算CNN权重、阈值及通道数，并用C++实现验证。

Result: 解析计算的CNN能快速识别50%以上的手写数字，推理时间极短。
...
### [120] [A Novel Convolutional Neural Network-Based Framework for Complex Multiclass Brassica Seed Classification](https://arxiv.org/abs/2505.21558)
*Elhoucine Elfatimia,Recep Eryigitb,Lahcen Elfatimi*

Main category: cs.CV

TL;DR: 提出了一种基于CNN的新框架，用于高效分类十种常见芸苔属种子，解决了种子图像纹理相似性的挑战，准确率达93%。


<details>
  <summary>Details</summary>
Motivation: 农民缺乏时间和资源进行农场研究，种子分类对质量控制和生产效率至关重要。早期识别种子类型可降低成本与风险。

Method: 采用自定义CNN架构，针对种子图像纹理相似性问题优化，并与多种预训练的最先进架构进行比较。

Result: 在收集的芸苔属种子数据集上，模型准确率达到93%。

Conclusion: 该CNN框架为种子分类提供了高效解决方案，有助于提升种子质量管理和产量预估的精准度。
...
### [121] [Knowledge Distillation Approach for SOS Fusion Staging: Towards Fully Automated Skeletal Maturity Assessment](https://arxiv.org/abs/2505.21561)
*Omid Halimi Milani,Amanda Nikho,Marouane Tliba,Lauren Mills,Ahmet Enis Cetin,Mohammed H Elnagar*

Main category: cs.CV

TL;DR: 提出了一种基于双模型架构的深度学习框架，用于自动评估蝶枕软骨结合（SOS）融合，通过知识蒸馏和新型损失函数提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 蝶枕软骨结合（SOS）融合是正畸学和法医人类学的重要诊断标志，但现有方法依赖外部裁剪或分割工具，效率低且不一致。

Method: 采用双模型架构（教师模型和学生模型），教师模型基于裁剪图像训练，通过新型损失函数（空间逻辑对齐和梯度注意力映射）指导学生模型学习。

Result: 框架实现了稳健的诊断准确性，无需额外预处理工具，提升了骨骼成熟度评估的效率和一致性。

Conclusion: 该框架为临床提供了一种高效、一致的端到端解决方案，简化了SOS融合的评估流程。
...
### [122] [Multi-instance Learning as Downstream Task of Self-Supervised Learning-based Pre-trained Model](https://arxiv.org/abs/2505.21564)
*Koki Matsuishi,Tsuyoshi Okita*

Main category: cs.CV

TL;DR: 论文提出了一种基于自监督学习的预训练模型方法，解决了在脑血肿CT中多实例学习因实例数量增加导致的困难，显著提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 在脑血肿CT的多实例学习中，当每个包中的实例数量增加到256时，学习变得极其困难。本文旨在解决这一问题。

Method: 使用自监督学习的预训练模型作为多实例学习器的下游任务。

Result: 在脑血肿CT的低密度标记分类任务中，准确率提高了5%至13%，F1分数提高了40%至55%。

Conclusion: 通过自监督预训练模型，有效克服了多实例学习中的困难，显著提升了分类性能。
...
### [123] [Diffusion Model-based Activity Completion for AI Motion Capture from Videos](https://arxiv.org/abs/2505.21566)
*Gao Huayu,Huang Tengjiu,Ye Xiaolong,Tsuyoshi Okita*

Main category: cs.CV

TL;DR: 论文提出了一种基于扩散模型的动作补全技术，用于解决AI动作捕捉中未观测到的动作片段问题，并在Human3.6M数据集上取得了优于现有方法的结果。


<details>
  <summary>Details</summary>
Motivation: 当前AI动作捕捉技术依赖预定义视频序列，无法处理未观测到的动作。论文旨在通过补全动作片段，实现更灵活的动作捕捉。

Method: 采用扩散模型生成补全动作序列，并引入门控模块和位置-时间嵌入模块。

Result: MDC-Net在ADE、FDE和MMADE指标上优于现有方法，模型更小（16.84M），生成的动作更自然。

Conclusion: 提出的方法有效解决了动作捕捉中的动作片段缺失问题，生成了更连贯的动作序列。
...
### [124] [EaqVLA: Encoding-aligned Quantization for Vision-Language-Action Models](https://arxiv.org/abs/2505.21567)
*Feng Jiang,Zihao Zheng,Xiuping Cui,Maoliang Li,JIayu Chen,Xiang Chen*

Main category: cs.CV

TL;DR: 提出了一种名为EaqVLA的优化框架，通过编码对齐量化解决VLA模型的量化问题，实现了更低的量化损失和更高的计算加速。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在计算和存储成本上较高，且现有量化方法因token对齐问题难以直接应用。

Method: 提出了一种完整的分析方法识别多粒度对齐问题，并基于此设计了编码对齐感知的混合精度量化方法。

Result: 实验表明，EaqVLA在量化损失和计算加速方面优于现有方法。

Conclusion: EaqVLA框架有效解决了VLA模型的量化问题，为端到端控制策略提供了高效优化方案。
...
### [125] [Thickness-aware E(3)-Equivariant 3D Mesh Neural Networks](https://arxiv.org/abs/2505.21572)
...
Method: 提出T-EMNN框架，结合厚度信息并保持E(3)-等变性，引入数据驱动坐标编码空间信息。

Result: 在真实工业数据集上验证，T-EMNN能准确预测节点级3D变形，有效捕捉厚度效应且计算高效。

Conclusion: T-EMNN在保持计算效率的同时，显著提升了3D静态分析的准确性，尤其在考虑厚度效应时表现优越。
...
### [126] [Do We Need All the Synthetic Data? Towards Targeted Synthetic Image Augmentation via Diffusion Models](https://arxiv.org/abs/2505.21574)
*Dang Nguyen,Jiping Li,Jinghao Zheng,Baharan Mirzasoleiman*

Main category: cs.CV

TL;DR: 通过仅增强训练数据中未被早期学习的部分，该方法在提升图像分类器泛化能力上优于全数据集增强，性能提升达2.8%。


<details>
  <summary>Details</summary>
Motivation: 现有方法在通过扩散模型增强训练数据时，难以确保生成多样性且需大幅增加数据量（10-30倍）以提升性能。

Method: 分析双层CNN，证明仅增强未被早期学习的数据部分能促进特征学习速度的均匀性，避免噪声放大。

Result: 在CIFAR-10、CIFAR-100和TinyImageNet上，仅增强30%-40%数据，性能提升达2.8%，甚至超越SOTA优化器SAM。

Conclusion: 该方法高效且兼容现有增强策略，为提升分类器性能提供了新思路。
...
### [127] [Do you see what I see? An Ambiguous Optical Illusion Dataset exposing limitations of Explainable AI](https://arxiv.org/abs/2505.21589)
*Carina Newen,Luca Hinkamp,Maria Ntonti,Emmanuel Müller*

Main category: cs.CV

TL;DR: 论文介绍了一个新颖的光学错觉数据集，旨在研究机器学习和人类感知中的模糊性问题，并探讨了视觉概念对模型准确性的影响。


<details>
  <summary>Details</summary>
Motivation: 在安全关键领域（如自动驾驶或医疗诊断）中，模糊数据对机器学习算法至关重要，但目前光学错觉数据集稀缺。

Method: 通过系统生成包含动物对的光学错觉数据集，研究视觉概念（如注视方向和眼睛线索）对模型的影响。

Result: 研究发现视觉概念显著影响模型准确性，为研究人类与机器视觉的偏差和对齐提供了基础。
...
### [128] [Any-to-Bokeh: One-Step Video Bokeh via Multi-Plane Image Guided Diffusion](https://arxiv.org/abs/2505.21593)
*Yang Yang,Siming Zheng,Jinwei Chen,Boxi Wu,Xiaofei He,Deng Cai,Bo Li,Peng-Tao Jiang*

Main category: cs.CV

TL;DR: 提出了一种新颖的一步视频散景框架，通过多平面图像表示和预训练模型的3D先验，实现了高质量、可控的视频散景效果。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑模型无法明确控制焦点平面或调整散景强度，且图像散景方法扩展到视频时会出现时间闪烁和边缘模糊过渡问题。

Method: 利用多平面图像表示和逐步扩大的深度采样函数，结合单步视频扩散模型和预训练模型的3D先验，实现深度感知的散景合成。

Result: 实验表明，该方法能生成高质量、可控的散景效果，并在多个评估基准上达到最先进性能。

Conclusion: 该方法解决了视频散景的时间一致性和深度控制问题，为可控光学效果提供了有效解决方案。
...
### [129] [Object Concepts Emerge from Motion](https://arxiv.org/abs/2505.21635)
*Haoqian Liang,Xiaohui Wang,Zhichao Li,Ya Yang,Naiyan Wang*

Main category: cs.CV

TL;DR: 论文提出了一种基于生物启发的无监督学习框架，通过运动边界信号学习物体中心视觉表征，无需标签或相机校准，并在多个下游任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 受婴儿通过观察运动获取物体理解的神经科学发现启发，探索如何利用运动边界信号进行无监督的物体级分组。

Method: 利用现成的光流和聚类算法生成基于运动的实例掩码，并通过对比学习训练视觉编码器。

Result: 在单目深度估计、3D物体检测和占用预测任务中优于现有监督和自监督基线，且泛化能力强。

Conclusion: 运动诱导的物体表征为视觉基础模型提供了新思路，捕捉了视觉实例这一关键抽象层次。
...
### [130] [BaryIR: Learning Multi-Source Unified Representation in Continuous Barycenter Space for Generalizable All-in-One Image Restoration](https://arxiv.org/abs/2505.21637)
*Xiaole Tang,Xiaoyi He,Xiang Gu,Jian Sun*

Main category: cs.CV

TL;DR: BaryIR提出了一种多源表示学习框架，通过分解多源退化图像的潜在空间为连续的重心空间和源特定子空间，提升全合一图像修复的泛化能力。
...
Method: 引入多源潜在最优传输重心问题，学习连续重心映射以将潜在表示传输到重心空间，同时设计传输成本以保持子空间表示对比性和正交性。

Result: BaryIR在实验中表现优异，尤其在真实数据和未见退化上展现更强的泛化能力。

Conclusion: BaryIR通过统一退化无关信息和特定退化语义，显著提升了全合一图像修复的泛化性和实际适用性。
...
### [131] [Geometric Feature Prompting of Image Segmentation Models](https://arxiv.org/abs/2505.21644)
*Kenneth Ball,Erin Taylor,Nirav Patel,Andrew Bartels,Gary Koplik,James Polly,Jay Hineman*

Main category: cs.CV

TL;DR: 论文提出了一种几何驱动的提示生成器（GeomPrompt），用于自动生成与特定特征共定位的提示点，从而优化SAM模型在科学图像分析（如植物根系分割）中的表现。


<details>
  <summary>Details</summary>
Motivation: 传统的手动标注植物根系图像耗时且主观，而SAM模型依赖提示点生成分割结果。通过几何驱动的提示生成器，可以自动生成高质量的提示点，提升分割的敏感性和特异性。

Method: 使用几何驱动的提示生成器（GeomPrompt）生成与图像特征共定位的提示点，并将其输入SAM模型进行自动分割。

Result: GeomPrompt生成的提示点显著提升了SAM模型在植物根系图像分割任务中的表现，减少了所需提示点数量。

Conclusion: 几何驱动的提示生成器为科学图像分析提供了一种高效、自动化的解决方案，有望推动相关领域的研究进展。
...
### [132] [QuARI: Query Adaptive Retrieval Improvement](https://arxiv.org/abs/2505.21647)
...
Method: 提出一种线性变换方法，将查询映射到特定特征空间变换，以低成本应用于大规模图像嵌入。

Result: 该方法在性能上优于现有技术，且计算成本更低。

Conclusion: 查询特定的特征空间变换是提升大规模检索任务的有效方法。
...
### [133] [Right Side Up? Disentangling Orientation Understanding in MLLMs with Fine-grained Multi-axis Perception Tasks](https://arxiv.org/abs/2505.21649)
*Keanu Nichols,Nazia Tasnim,Yan Yuting,Nicholas Ikechukwu,Elva Zou,Deepti Ghadiyaram,Bryan Plummer*

Main category: cs.CV

TL;DR: DORI是一个专注于评估多模态系统对物体方向感知能力的基准测试，揭示了现有模型的局限性。
...
Method: DORI通过11个数据集的67个物体类别，评估四个方向理解维度，测试15个先进模型。
...
### [134] [Think Before You Diffuse: LLMs-Guided Physics-Aware Video Generation](https://arxiv.org/abs/2505.21653)
*Ke Zhang,Cihan Xiao,Yiqun Mei,Jiacong Xu,Vishal M. Patel*

Main category: cs.CV

TL;DR: DiffPhy是一个通过微调预训练视频扩散模型生成物理正确且逼真视频的框架，利用LLM和MLLM指导生成，并引入新训练目标。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在生成物理效果时存在挑战，复杂现实运动和交互增加了学习物理的难度。

Method: 利用LLM从文本提示推理物理上下文，MLLM作为监督信号，引入新训练目标确保物理正确性和语义一致性。

Result: 在公开基准测试中，DiffPhy在多样物理相关场景中生成最先进结果。

Conclusion: DiffPhy通过结合LLM和MLLM，有效解决了视频生成中的物理正确性问题。
...
### [135] [Scalable Segmentation for Ultra-High-Resolution Brain MR Images](https://arxiv.org/abs/2505.21697)
...
Method: 回归每类有符号距离变换图，并采用可扩展的类条件分割策略。
...
### [136] [MedBridge: Bridging Foundation Vision-Language Models to Medical Image Diagnosis](https://arxiv.org/abs/2505.21698)
*Yitong Li,Morteza Ghahremani,Christian Wachinger*

Main category: cs.CV

TL;DR: MedBridge是一个轻量级多模态适应框架，通过重新利用预训练的视觉语言模型（VLM）实现准确的医学图像诊断，解决了医学图像领域迁移的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言基础模型在自然图像分类上表现优异，但在医学图像上因领域差异表现不佳，而训练医学基础模型需要大量资源和标注数据。

Method: MedBridge包含三个关键组件：1）Focal Sampling模块提取高分辨率局部区域；2）Query Encoder注入可学习查询以对齐医学语义；3）Mixture of Experts机制利用多样化VLM的互补优势。

Result: 在五个医学影像基准测试中，MedBridge在跨领域和领域内适应任务中表现优异，尤其在多标签胸部疾病诊断中AUC提升了6-15%。

Conclusion: MedBridge通过轻量级框架有效利用基础模型，实现了准确且数据高效的医学诊断。
...
### [137] [OmniResponse: Online Multimodal Conversational Response Generation in Dyadic Interactions](https://arxiv.org/abs/2505.21724)
*Cheng Luo,Jianghui Wang,Bing Li,Siyang Song,Bernard Ghanem*

Main category: cs.CV

TL;DR: 论文提出了一种名为OMCRG的新任务，旨在在线生成同步的言语和非言语听者反馈，并提出了OmniResponse模型来解决音频和面部反应同步的挑战。


<details>
  <summary>Details</summary>
Motivation: 研究目标是模拟自然对话中的多模态交互，解决听者反馈中音频和面部反应同步的难题。
...
Conclusion: OmniResponse为多模态对话响应生成提供了有效解决方案，并推动了相关领域的研究。
...
### [138] [Moment kernels: a simple and scalable approach for equivariance to rotations and reflections in deep convolutional networks](https://arxiv.org/abs/2505.21736)
*Zachary Schlamowitz,Andrew Bennecke,Daniel J. Tward*

Main category: cs.CV

TL;DR: 论文提出了一种称为“矩核”的简单卷积核形式，用于实现旋转和反射对称性等几何对称性的等变性，解决了传统方法依赖复杂数学工具的问题，并在生物医学图像分析任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 旋转和反射对称性在生物医学图像分析中至关重要，但传统方法依赖复杂的数学工具（如表示理论），限制了其广泛应用。本文旨在简化实现这些对称性的方法。

Method: 提出“矩核”概念，证明所有等变核必须采用这种形式（径向对称函数乘以空间位置的分量幂或单位矩阵），并基于标准卷积模块实现等变神经网络。

Result: 在生物医学图像分析任务（分类、3D图像配准、细胞分割）中验证了矩核的有效性，实现了所需的等变性。

Conclusion: 矩核提供了一种简单且通用的方法来实现几何对称性的等变性，避免了复杂数学工具的使用，为生物医学图像分析提供了实用解决方案。
...
### [139] [What is Adversarial Training for Diffusion Models?](https://arxiv.org/abs/2505.21742)
*Briglia Maria Rosaria,Mujtaba Hussain Mirza,Giuseppe Lisanti,Iacopo Masi*

Main category: cs.CV

TL;DR: 对抗训练（AT）在扩散模型（DMs）中的作用与分类器不同，它要求等变性而非输出不变性，从而提升对噪声和异常数据的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 探讨对抗训练在扩散模型中的独特作用，区别于传统分类器，以提升模型对噪声和异常数据的处理能力。

Method: 通过在扩散训练中随机或对抗性添加噪声，无需假设噪声模型，实现平滑扩散流。

Result: 在低维和高维数据集及标准基准测试中表现优异，尤其在噪声、数据损坏和对抗攻击下。

Conclusion: 对抗训练在扩散模型中通过等变性提升鲁棒性，适用于多种噪声和异常场景。
...
### [140] [Learning to See More: UAS-Guided Super-Resolution of Satellite Imagery for Precision Agriculture](https://arxiv.org/abs/2505.21746)
*Arif Masrur,Peder A. Olsen,Paul R. Adler,Carlan Jackson,Matthew W. Myers,Nathan Sedghi,Ray R. Weil*

Main category: cs.CV

TL;DR: 该研究提出了一种融合卫星和无人机系统（UAS）图像的超分辨率框架，通过整合空间、光谱和时间数据，提高了农作物生物量和氮的估算精度。


<details>
  <summary>Details</summary>
Motivation: 卫星和UAS在精准农业中各具优缺点，卫星覆盖广但分辨率低，UAS分辨率高但成本高且覆盖有限。研究旨在结合两者优势，提供经济高效的解决方案。

Method: 使用超分辨率方法融合卫星和UAS图像，通过光谱扩展UAS RGB数据至植被红边和近红外区域，生成高分辨率Sentinel-2图像。

Result: 生物量和氮估算精度分别提高了18%和31%，且UAS数据仅需从部分田地和时间点采集。

Conclusion: 该框架轻量、可扩展，适用于农场，减少了重复UAS飞行的需求，且在无卫星数据时仍有效。
...
### [141] [Visual Loop Closure Detection Through Deep Graph Consensus](https://arxiv.org/abs/2505.21754)
...
<details>
  <summary>Details</summary>
Motivation: 传统闭环检测依赖计算昂贵的几何验证，且假阳性闭环会严重影响位姿图估计，因此在在线SLAM场景中需要更高效的方法。

Method: 提出LoopGNN，通过图神经网络在视觉相似关键帧团中传播深度特征编码，估计闭环一致性。

Result: 在TartanDrive 2.0和NCLT数据集上，LoopGNN表现优于传统基线方法，且计算效率更高。
...
### [142] [FRAMES-VQA: Benchmarking Fine-Tuning Robustness across Multi-Modal Shifts in Visual Question Answering](https://arxiv.org/abs/2505.21755)
*Chengyue Huang,Brisa Maneechotesuwan,Shivang Chopra,Zsolt Kira*

Main category: cs.CV

TL;DR: 该论文提出了一个新的基准FRAMES-VQA，用于评估VQA任务中的鲁棒微调，涵盖多模态分布偏移。


<details>
  <summary>Details</summary>
Motivation: 当前VQA系统在适应真实世界数据偏移时面临挑战，尤其是多模态场景下，现有评估方法多为单模态或特定OOD类型，无法全面反映多模态复杂性。

Method: 利用十个现有VQA基准（如VQAv2、IV-VQA等），将其分类为ID、近OOD和远OOD数据集，并通过计算Mahalanobis距离量化分布偏移。

Result: 分析了单模态与多模态偏移的交互作用及模态重要性，为开发更鲁棒的微调方法提供了指导。

Conclusion: FRAMES-VQA为多模态分布偏移下的VQA任务提供了新的评估基准和分析方法，有助于提升系统鲁棒性。
...
### [143] [MMTBENCH: A Unified Benchmark for Complex Multimodal Table Reasoning](https://arxiv.org/abs/2505.21771)
*Prasham Yatinkumar Titiya,Jainil Trivedi,Chitta Baral,Vivek Gupta*

Main category: cs.CV

TL;DR: MMTBENCH是一个包含500个真实世界多模态表格的基准测试，用于评估视觉语言模型在多模态表格推理中的表现，揭示了现有模型在视觉推理和多步推理上的不足。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多模态表格推理上的能力尚未被充分探索，MMTBENCH旨在填补这一空白。

Method: 构建了包含500个多模态表格和4021个问答对的基准测试，覆盖多种问题类型、推理类型和表格类型。

Result: 评估显示现有模型在视觉推理和多步推理任务上表现不佳，存在显著性能差距。

Conclusion: MMTBENCH为未来研究提供了高质量资源，强调了改进视觉与语言处理集成的必要性。
...
### [144] [Compositional Scene Understanding through Inverse Generative Modeling](https://arxiv.org/abs/2505.21780)
*Yanbo Wang,Justin Dauwels,Yilun Du*

Main category: cs.CV

TL;DR: 论文探讨了生成模型如何用于场景理解，通过逆向生成建模推断场景结构和全局因素，并展示了零样本多对象感知的应用。
...
Method: 将场景理解建模为逆向生成问题，通过组合式视觉生成模型推断场景结构和对象。

Result: 方法能推断场景中的对象和全局因素，并实现对新测试场景的鲁棒泛化。

Conclusion: 组合式生成模型为场景理解和零样本多对象感知提供了有效解决方案。
...
### [145] [SANSA: Unleashing the Hidden Semantics in SAM2 for Few-Shot Segmentation](https://arxiv.org/abs/2505.21795)
*Claudia Cuttano,Gabriele Trivigno,Giuseppe Averta,Carlo Masone*

Main category: cs.CV

TL;DR: SANSA改进SAM2，通过显式提取其语义结构，实现少样本分割，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: SAM2的特征表示与任务特定线索纠缠，影响其语义理解能力，需改进以适用于少样本分割。

Method: 提出SANSA框架，显式提取SAM2的潜在语义结构，通过最小任务特定修改适配少样本分割。

Result: 在少样本分割基准测试中表现最佳，支持多种交互提示，速度快且模型紧凑。

Conclusion: SANSA成功利用SAM2的语义结构，为少样本分割提供高效解决方案。
...
### [146] [ALTER: All-in-One Layer Pruning and Temporal Expert Routing for Efficient Diffusion Generation](https://arxiv.org/abs/2505.21817)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型的高计算开销限制了其在资源受限环境中的实际部署，现有加速方法未能充分捕捉生成过程中的时间动态。

Method: 提出ALTER框架，利用可训练的超网络动态生成层剪枝决策并管理时间步路由，实现单阶段优化。

Result: ALTER仅用20步推理和35%稀疏度，实现了与原始模型相同的视觉保真度，计算量减少74.1%，速度提升3.64倍。

Conclusion: ALTER为扩散模型的高效部署提供了一种统一的优化策略，平衡了效率与生成质量。
...
### [147] [HDRSDR-VQA: A Subjective Video Quality Dataset for HDR and SDR Comparative Evaluation](https://arxiv.org/abs/2505.21831)
...
<details>
  <summary>Details</summary>
Motivation: 促进HDR和SDR内容的直接比较，研究何时及为何一种格式优于另一种。

Method: 数据集包含960个视频，来自54个源序列，每种格式有9个失真级别。通过145名参与者的主观研究和22,000次配对比较生成JOD分数。

Result: 数据集支持HDR和SDR版本的直接内容级比较，为视频质量评估和流媒体优化提供基础。

Conclusion: HDRSDR-VQA为视频质量研究提供了开放资源，支持进一步探索HDR和SDR的差异。
...
### [148] [UniMoGen: Universal Motion Generation](https://arxiv.org/abs/2505.21837)
...
<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法依赖特定骨架结构，限制了其通用性。UniMoGen旨在解决这一问题，提供更灵活的运动生成方案。

Method: 采用UNet架构的扩散模型，动态处理必要关节，支持风格和轨迹输入控制，并能延续过去帧的运动。

Result: 在100style数据集上表现优于现有方法，同时在100style和LAFAN1数据集上训练时，跨骨架性能高效。
...
### [149] [Test-Time Adaptation of Vision-Language Models for Open-Vocabulary Semantic Segmentation](https://arxiv.org/abs/2505.21844)
...
TL;DR: 本文提出了一种针对开放词汇语义分割（OVSS）的测试时适应（TTA）方法MLMP，通过多级多提示熵最小化优化视觉语言模型，无需额外数据或标签，且在单样本测试中仍有效。
...
Method: 提出MLMP方法，结合中间视觉编码器层特征和不同文本提示模板，在全局CLS标记和局部像素级别进行熵最小化。

Result: 实验表明，MLMP在82种测试场景中显著优于直接应用TTA分类基线方法。

Conclusion: MLMP为OVSS提供了一种高效的TTA解决方案，并建立了标准化评估基准。
...
### [150] [RePaViT: Scalable Vision Transformer Acceleration via Structural Reparameterization on Feedforward Network Layers](https://arxiv.org/abs/2505.21847)
*Xuwei Xu,Yang Li,Yudong Chen,Jiajun Liu,Sen Wang*

Main category: cs.CV

TL;DR: 研究发现FFN层是ViT推理延迟的主要来源，提出了一种通道空闲机制，通过结构重参数化优化FFN层，显著降低延迟且保持或提升精度。


<details>
  <summary>Details</summary>
Motivation: 揭示FFN层对ViT推理延迟的关键影响，探索优化大型ViT效率的方法。

Method: 提出通道空闲机制，通过结构重参数化在测试时优化FFN层，形成线性路径。

Result: RePaViT系列模型在大型ViT上实现显著速度提升（最高68.7%），同时精度略有提升。

Conclusion: RePaViT为ViT效率优化提供了新方向，首次将结构重参数化应用于FFN层。
...
### [151] [FPAN: Mitigating Replication in Diffusion Models through the Fine-Grained Probabilistic Addition of Noise to Token Embeddings](https://arxiv.org/abs/2505.21848)
*Jingqi Xu,Chenghao Li,Yuke Zhang,Peter A. Beerel*

Main category: cs.CV

TL;DR: 论文提出了一种细粒度噪声注入技术（FPAN），以减少扩散模型对训练数据的复制，保护隐私。实验表明FPAN显著降低了复制率，同时保持图像质量。
...
Method: 提出FPAN技术，通过概率性地向token嵌入添加不同量级的噪声，减少数据复制。

Result: FPAN平均减少复制率28.78%，优于基线模型和现有噪声添加方法，且不影响图像质量。
...
### [152] [Beyond Perception: Evaluating Abstract Visual Reasoning through Multi-Stage Task](https://arxiv.org/abs/2505.21850)
...
<details>
  <summary>Details</summary>
Motivation: 现有抽象视觉推理（AVR）基准仅关注单步推理和最终结果，忽略了多阶段推理过程，且现有模型表现不佳但原因不明。

Method: 基于RAVEN设计MultiStAR基准，提出MSEval指标评估中间步骤和最终结果的正确性，并在17种MLLMs上进行实验。

Result: 实验显示现有MLLMs在基础感知任务表现良好，但在复杂规则检测阶段仍有困难。
...
### [153] [Rethinking Gradient-based Adversarial Attacks on Point Cloud Classification](https://arxiv.org/abs/2505.21854)
*Jun Chen,Xinke Li,Mingyue Xu,Tianrui Li,Chongshou Li*

Main category: cs.CV

TL;DR: 论文提出两种新策略（WAAttack和SubAttack）改进基于梯度的对抗攻击方法，通过加权梯度和自适应步长策略，以及聚焦关键区域，提升攻击效果和隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 现有基于梯度的点云对抗攻击方法未考虑点云的异构性，导致扰动过大且易察觉。

Method: 1. WAAttack：引入加权梯度和自适应步长策略，动态调整扰动；2. SubAttack：分解点云并聚焦关键区域。

Result: 实验表明，该方法在生成隐蔽对抗样本方面优于现有基线。

Conclusion: 通过重新设计梯度更新机制，显著提升了攻击的隐蔽性和有效性。
...
### [154] [Towards Scalable Language-Image Pre-training for 3D Medical Imaging](https://arxiv.org/abs/2505.21862)
*Chenhui Zhao,Yiwei Lyu,Asadur Chowdury,Edward Harake,Akhil Kondepudi,Akshay Rao,Xinhai Hou,Honglak Lee,Todd Hollon*

Main category: cs.CV

TL;DR: HLIP是一种针对3D医学影像的可扩展预训练框架，通过分层注意力机制显著提升了性能，并在大规模未筛选临床数据上实现了高效训练。
...
Method: 采用轻量级分层注意力机制（切片、扫描和研究的自然层次结构），并在大规模未筛选数据集上进行训练。

Result: 在多个基准测试中取得显著提升，如Rad-ChestCT的AUC提高4.3%，Pub-Brain-5的平衡准确率提高32.4%。

Conclusion: HLIP证明直接在大规模未筛选临床数据上进行预训练是3D医学影像语言-图像预训练的有效方向。
...
### [155] [GETReason: Enhancing Image Context Extraction through Hierarchical Multi-Agent Reasoning](https://arxiv.org/abs/2505.21863)
...
Method: 提出GETReason框架，结合地理空间、时间和事件信息的多层次多智能体方法。

Result: 通过GREAT指标验证，GETReason能有效推断图像的深层意义。

Conclusion: GETReason框架成功链接图像与事件背景，为图像理解提供了新方法。
...
### [156] [Cross-DINO: Cross the Deep MLP and Transformer for Small Object Detection](https://arxiv.org/abs/2505.21868)
*Guiping Cao,Wenjian Huang,Xiangyuan Lan,Jianguo Zhang,Dongmei Jiang,Yaowei Wang*

Main category: cs.CV

TL;DR: 论文提出Cross-DINO方法，通过结合深度MLP网络和新的Cross Coding Twice Module（CCTM）提升小目标检测性能，并引入Category-Size（CS）软标签和Boost Loss损失函数，显著提高了DETR类模型在小目标检测上的表现。


<details>
  <summary>Details</summary>
Motivation: 小目标检测（SOD）因信息有限和模型预测分数低而具有挑战性，现有Transformer检测器在SOD上的潜力尚未充分挖掘。

Method: 提出Cross-DINO方法，结合深度MLP网络和CCTM模块增强小目标细节，并引入CS软标签和Boost Loss提升分类预测分数。

Result: 在多个数据集上验证，Cross-DINO显著优于DINO，COCO数据集上APs提升4.4%（36.4% vs. 32.0%），且参数和计算量更少。

Conclusion: Cross-DINO通过创新模块和损失函数，有效提升了DETR类模型在小目标检测中的性能，为SOD提供了新思路。
...
### [157] [EPiC: Efficient Video Camera Control Learning with Precise Anchor-Video Guidance](https://arxiv.org/abs/2505.21876)
*Zun Wang,Jaemin Cho,Jialu Li,Han Lin,Jaehong Yoon,Yue Zhang,Mohit Bansal*

Main category: cs.CV

TL;DR: EPiC框架通过自动构建高质量锚视频和轻量级Anchor-ControlNet模块，解决了3D相机控制中锚视频不准确和标注资源需求高的问题，实现了高效训练和精确控制。
...
Method: EPiC通过基于首帧可见性的视频掩码生成高质量锚视频，并引入Anchor-ControlNet模块集成锚视频引导。

Result: EPiC在RealEstate10K和MiraData上达到SOTA性能，支持零样本泛化到视频到视频任务。

Conclusion: EPiC无需修改扩散模型主干，实现了高效、精确的3D相机控制，且泛化能力强。
...
### [158] [Hyperspectral Gaussian Splatting](https://arxiv.org/abs/2505.21890)
*Sunil Kumar Narayanan,Lingjun Zhao,Lu Gan,Yongsheng Chen*

Main category: cs.CV

TL;DR: 本文提出了一种结合3D高斯泼溅和扩散模型的方法（HS-GS），用于高光谱场景的3D显式重建和新视角合成，解决了NeRF在训练时间和渲染速度上的限制。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像（HSI）在农业中广泛应用，但现有方法如NeRF在训练和渲染效率上存在不足。

Method: 结合3D高斯泼溅和扩散模型，引入波长编码器和基于KL散度的损失函数，优化光谱重建和去噪。
...
### [159] [Concentrate on Weakness: Mining Hard Prototypes for Few-Shot Medical Image Segmentation](https://arxiv.org/abs/2505.21897)
*Jianchao Jiang,Haofeng Zhang*

Main category: cs.CV

TL;DR: 论文提出了一种改进的少样本医学图像分割方法，通过关注弱特征和边界优化，显著提升了分割效果。


<details>
  <summary>Details</summary>
Motivation: 现有原型方法因随机采样或局部平均导致边界模糊，无法有效处理医学图像中前景与背景的不平衡问题。

Method: 设计了支持自预测模块（SSP）识别弱特征，硬原型生成模块（HPG）生成硬原型，多相似度图融合模块（MSMF）优化分割，并引入边界损失。

Result: 在三个公开医学图像数据集上实现了最先进的性能。

Conclusion: 该方法通过关注弱特征和边界优化，显著提升了少样本医学图像分割的精度。
...
### [160] [CAST: Contrastive Adaptation and Distillation for Semi-Supervised Instance Segmentation](https://arxiv.org/abs/2505.21904)
*Pardis Taghavi,Tian Liu,Renjie Li,Reza Langari,Zhengzhong Tu*

Main category: cs.CV

TL;DR: CAST是一种半监督知识蒸馏框架，通过压缩预训练的视觉基础模型（VFM）为紧凑专家模型，利用有限标注和大量未标注数据，显著提升实例分割性能。


<details>
  <summary>Details</summary>
Motivation: 实例分割需要昂贵的像素级标注和大模型，CAST旨在通过半监督学习减少标注成本并提升模型效率。

Method: CAST分为三个阶段：1）通过自训练和对比像素校准进行VFM教师的领域适应；2）通过多目标损失函数（结合监督学习和伪标签）蒸馏到紧凑学生模型；3）在标注数据上微调以消除伪标签偏差。核心是实例感知的像素级对比损失。

Result: 在Cityscapes和ADE20K数据集上，CAST的学生模型比其教师模型分别提升了3.4 AP和1.5 AP，并优于其他半监督方法。

Conclusion: CAST通过实例感知对比损失和多阶段蒸馏，显著提升了半监督实例分割的性能和效率。
...
### [161] [Reference-Guided Identity Preserving Face Restoration](https://arxiv.org/abs/2505.21905)
*Mo Zhou,Keren Ye,Viraj Shah,Kangfu Mei,Mauricio Delbracio,Peyman Milanfar,Vishal M. Patel,Hossein Talebi*

Main category: cs.CV

TL;DR: 提出了一种新方法，通过复合上下文表示、硬样本身份损失和无训练多参考输入适应，显著提升了基于参考的人脸恢复和身份保持效果。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在人脸恢复中难以保持身份信息，现有基于参考的方法未能充分利用参考潜力。

Method: 1) 复合上下文表示融合多级参考信息；2) 硬样本身份损失优化身份学习；3) 无训练多参考输入适应。

Result: 在FFHQ-Ref和CelebA-Ref-Test基准测试中表现最优，身份保持效果显著。

Conclusion: 新方法有效提升了人脸恢复质量和身份保持能力，优于现有技术。
...
### [162] [AlignGen: Boosting Personalized Image Generation with Cross-Modality Prior Alignment](https://arxiv.org/abs/2505.21911)
...
Result: AlignGen在零样本方法中表现优异，甚至超越流行的测试时优化方法。
...
### [163] [LiDARDustX: A LiDAR Dataset for Dusty Unstructured Road Environments](https://arxiv.org/abs/2505.21914)
*Chenfeng Wei,Qi Wu,Si Zuo,Jiahua Xu,Boyang Zhao,Zeyu Yang,Guotao Xie,Shenhong Wang*

Main category: cs.CV

TL;DR: LiDARDustX数据集专注于高粉尘环境下的感知任务，填补了现有数据集在非结构化场景中的空白。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集主要针对结构化城市环境，缺乏对高粉尘等非结构化场景的研究。

Method: 通过六种不同LiDAR传感器采集30,000帧数据，包含3D边界框标注和点云语义分割，其中80%为粉尘场景。

Result: 建立了3D检测和分割算法的基准，并分析了粉尘对感知精度的影响及其原因。

Conclusion: LiDARDustX为高粉尘环境下的感知研究提供了重要资源，推动了相关算法的发展。
...
### [164] [BD Open LULC Map: High-resolution land use land cover mapping & benchmarking for urban development in Dhaka, Bangladesh](https://arxiv.org/abs/2505.21915)
...
TL;DR: 论文介绍了BD Open LULC Map (BOLM)，为达卡及其周边地区提供高分辨率卫星图像的像素级LULC标注，旨在解决南亚/东亚地区LULC数据集稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 南亚/东亚发展中国家由于资金有限、基础设施多样和人口密集，标注卫星数据稀缺，限制了LULC分类的可靠性。

Method: 使用高分辨率Bing卫星图像（2.22米/像素）创建BOLM数据集，涵盖11个LULC类别，并通过GIS专家进行三阶段验证。

Result: BOLM覆盖4,392平方公里（8.91亿像素），并利用DeepLab V3+在Bing和Sentinel-2A图像上对五大类别进行LULC分割性能比较。

Conclusion: BOLM填补了南亚/东亚地区LULC数据集的空白，支持可靠的深度学习模型和领域适应任务。
...
### [165] [InfoSAM: Fine-Tuning the Segment Anything Model from An Information-Theoretic Perspective](https://arxiv.org/abs/2505.21920)
*Yuanhong Zhang,Muyao Yuan,Weizhan Zhang,Tieliang Gong,Wen Wen,Jiangyong Ying,Weijie Shi*

Main category: cs.CV

TL;DR: InfoSAM通过信息论方法优化SAM的微调，保留预训练知识，提升在专业领域的表现。


<details>
  <summary>Details</summary>
Motivation: SAM在通用任务中表现优异，但在专业领域表现不足，现有PEFT方法忽略了预训练模型的领域不变关系。

Method: 提出InfoSAM，通过两个基于互信息的目标（压缩领域不变关系、最大化师生模型间互信息）优化知识蒸馏。

Result: 实验证明InfoSAM能显著提升SAM在真实任务中的性能，适应性强。
...
### [166] [Point-to-Region Loss for Semi-Supervised Point-Based Crowd Counting](https://arxiv.org/abs/2505.21943)
*Wei Lin,Chenyang Zhao,Antoni B. Chan*

Main category: cs.CV

TL;DR: 该论文提出了一种基于伪标记的半监督计数框架，通过点特异性激活图（PSAM）分析训练问题，并提出点对区域（P2R）方案替代点对点（P2P）监督，解决了伪标签置信度传播问题。


<details>
  <summary>Details</summary>
Motivation: 点检测方法在密集人群定位和计数中表现优异，但标注成本高。论文旨在通过半监督框架减少标注需求。

Method: 结合伪标记的半监督框架，使用PSAM分析训练问题，并提出P2R方案替代P2P监督。

Result: 实验表明，P2R在半监督计数和无监督域适应中优于P2P，解决了PSAM揭示的问题。
...
### [167] [UniTalk: Towards Universal Active Speaker Detection in Real World Scenarios](https://arxiv.org/abs/2505.21954)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准数据集（如AVA）主要基于老电影，存在显著的领域差距，无法反映真实世界的多样性。UniTalk旨在填补这一空白。

Method: UniTalk包含44.5小时视频，涵盖48,693个说话人身份，专注于多样化和困难的真实场景（如多语言、嘈杂背景和拥挤场景）。

Result: 现有最先进模型在AVA上表现优异，但在UniTalk上未能饱和，表明真实条件下的主动说话人检测任务尚未解决。UniTalk训练的模型在其他数据集（如Talkies、ASW和AVA）上表现更好。

Conclusion: UniTalk为主动说话人检测设立了新基准，为开发更具适应性和鲁棒性的模型提供了宝贵资源。
...
### [168] [Towards Comprehensive Scene Understanding: Integrating First and Third-Person Views for LVLMs](https://arxiv.org/abs/2505.21955)
*Insu Lee,Wooje Park,Jaeyun Jang,Minyoung Noh,Kyuhong Shim,Byonghyo Shim*

Main category: cs.CV

TL;DR: 论文提出了一种结合第一人称和第三人称视角的框架E3VQA和训练无关的提示技术M3CoT，以提升大型视觉语言模型在多视图问答中的表现。


<details>
  <summary>Details</summary>
Motivation: 第一人称视角的局限性（如视野狭窄、缺乏全局上下文）导致其在空间或上下文要求高的查询中失败，需通过第三人称视角补充信息。

Method: 引入E3VQA基准和M3CoT技术，通过整合多视角场景图构建统一场景表示。

Result: M3CoT显著提升了模型性能（GPT-4o提升4.84%，Gemini 2.0 Flash提升5.94%）。

Conclusion: 结合第一人称和第三人称视角能有效提升多视图推理能力，揭示了LVLMs的潜力与局限。
...
### [169] [Cross-modal RAG: Sub-dimensional Retrieval-Augmented Text-to-Image Generation](https://arxiv.org/abs/2505.21956)
*Mengdan Zhu,Senhao Cheng,Guangji Bai,Yifei Zhang,Liang Zhao*

Main category: cs.CV

TL;DR: 论文提出Cross-modal RAG框架，通过分解查询和图像为子维度组件，实现子查询感知的检索与生成，显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成方法无法处理复杂查询中多元素需求的问题，需要一种能分解查询并选择性利用图像特征的新方法。

Method: 提出混合检索策略（稀疏与稠密检索结合）和多模态大语言模型，实现子查询感知的图像合成。

Result: 在多个数据集上实验表明，Cross-modal RAG在检索和生成质量上显著优于基线，且效率高。

Conclusion: Cross-modal RAG通过子维度分解和选择性特征利用，有效解决了复杂查询下的图像生成问题。
...
### [170] [One-Way Ticket:Time-Independent Unified Encoder for Distilling Text-to-Image Diffusion Models](https://arxiv.org/abs/2505.21960)
*Senmao Li,Lei Wang,Kai Wang,Tao Liu,Jiehang Xie,Joost van de Weijer,Fahad Shahbaz Khan,Shiqi Yang,Yaxing Wang,Jian Yang*

Main category: cs.CV

TL;DR: 论文提出了一种名为TiUE的时间无关统一编码器，用于蒸馏T2I扩散模型，通过共享编码器特征减少计算冗余，提升推理速度和图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏T2I模型在减少采样步数的同时，往往牺牲了多样性和质量，尤其是单步模型。研究发现UNet编码器存在冗余计算，而解码器能更好地捕捉语义信息。

Method: 提出TiUE架构，共享编码器特征，实现并行采样；加入KL散度项正则化噪声预测，提升图像真实性和多样性。

Result: 实验显示TiUE在多样性和真实性上优于LCM、SD-Turbo和SwiftBrushv2，同时保持计算效率。

Conclusion: TiUE通过时间无关编码器设计，有效平衡了推理速度与图像质量，为T2I扩散模型的高效部署提供了新思路。
...
### [171] [A2Seek: Towards Reasoning-Centric Benchmark for Aerial Anomaly Understanding](https://arxiv.org/abs/2505.21962)
*Mengjingcheng Mo,Xinyang Tong,Jiaxu Leng,Mingpi Tan,Jiankang Zheng,Yiran Liu,Haosheng Chen,Ji Gan,Weisheng Li,Xinbo Gao*

Main category: cs.CV

TL;DR: 论文提出了A2Seek数据集和A2Seek-R1框架，用于无人机视角下的异常检测，通过推理和优化方法显著提升了性能。
...
Method: 提出A2Seek数据集，并设计A2Seek-R1框架，包括GoT引导的微调、A-GRPO优化和模拟无人机行为的“seeking”机制。

Result: A2Seek-R1在预测准确率和异常定位上分别提升了22.04%和13.9%，表现出强泛化能力。

Conclusion: A2Seek数据集和框架有效解决了无人机视角异常检测的挑战，具有广泛应用潜力。
...
### [172] [DvD: Unleashing a Generative Paradigm for Document Dewarping via Coordinates-based Diffusion Model](https://arxiv.org/abs/2505.21975)
*Weiguang Zhang,Huangcheng Lu,Maizhen Ning,Xiaowei Huang,Wei Wang,Kaizhu Huang,Qiufeng Wang*

Main category: cs.CV

TL;DR: 论文提出了一种基于扩散模型的文档去扭曲方法DvD，通过坐标级去噪和时间变体条件细化机制，显著提升了文档结构的保留能力，并在多个基准测试中取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 文档去扭曲技术虽已有进展，但在保留文档结构方面仍具挑战性。扩散模型在图像生成中的成功应用激发了其在文档去扭曲中的潜力，但高分辨率文档图像的复杂性限制了直接应用。

Method: DvD采用坐标级去噪生成变形校正映射，并提出时间变体条件细化机制以增强文档结构的保留。
...
### [173] [Learning World Models for Interactive Video Generation](https://arxiv.org/abs/2505.21996)
*Taiye Chen,Xun Hu,Zihan Ding,Chi Jin*

Main category: cs.CV

TL;DR: 论文提出了一种视频检索增强生成（VRAG）方法，通过显式全局状态条件化解决自回归视频生成中的长期累积误差和时空不一致问题。


<details>
  <summary>Details</summary>
Motivation: 当前长视频生成模型因累积误差和记忆机制不足而缺乏有效的世界建模能力，影响了交互性和时空一致性。

Method: 通过动作条件化和自回归框架增强图像到视频模型，并提出VRAG方法，利用显式全局状态条件化减少误差。

Result: VRAG显著减少了长期累积误差，提高了世界模型的时空一致性，而传统方法效果有限。

Conclusion: 研究揭示了视频世界模型的基本挑战，并为改进视频生成模型提供了基准。
...
### [174] [D-Fusion: Direct Preference Optimization for Aligning Diffusion Models with Visually Consistent Samples](https://arxiv.org/abs/2505.22002)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的图像与文本提示的对齐问题限制了其实际应用，现有DPO方法因视觉不一致性效果受限。

Method: D-Fusion通过掩码引导的自注意力融合生成视觉一致性样本，并保留去噪轨迹以支持DPO训练。

Result: 实验证明D-Fusion能有效提升不同强化学习算法中的提示-图像对齐效果。

Conclusion: D-Fusion解决了视觉不一致性问题，显著提升了扩散模型的对齐能力。
...
### [175] [Event-based Egocentric Human Pose Estimation in Dynamic Environment](https://arxiv.org/abs/2505.22007)
*Wataru Ikeda,Masashi Hatano,Ryosei Hara,Mariko Isogawa*

Main category: cs.CV

TL;DR: 提出了一种基于事件相机的前向视角人体姿态估计方法D-EventEgo，通过运动分割模块提升动态环境下的头部姿态估计精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖RGB相机，无法应对低光环境和运动模糊，事件相机能解决这些问题。

Method: 先估计头部姿态，再以此为条件生成身体姿态；引入运动分割模块去除动态物体干扰。

Result: 在合成事件数据集上，动态环境中五项指标中有四项优于基线。

Conclusion: D-EventEgo在动态环境下表现优异，验证了事件相机在人体姿态估计中的潜力。
...
### [176] [Prototype Embedding Optimization for Human-Object Interaction Detection in Livestreaming](https://arxiv.org/abs/2505.22011)
*Menghui Zhang,Jing Zhang,Lin Chen,Li Zhuo*

Main category: cs.CV

TL;DR: 提出了一种原型嵌入优化方法（PeO-HOI），用于解决直播中人-物交互（HOI）检测中的对象偏差问题，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 直播中人-物交互检测存在对象偏差问题，即过于关注物体而忽视其与主播的交互。

Method: 通过目标检测与跟踪预处理直播视频，提取人-物对特征；采用原型嵌入优化减少对象偏差；建模时空上下文后通过预测头输出HOI检测结果。
...
### [177] [PanoWan: Lifting Diffusion Video Generation Models to 360° with Latitude/Longitude-aware Mechanisms](https://arxiv.org/abs/2505.22016)
*Yifei Xia,Shuchen Weng,Siqi Yang,Jingqi Liu,Chengxuan Zhu,Minggui Teng,Zijian Jia,Han Jiang,Boxin Shi*

Main category: cs.CV

TL;DR: PanoWan利用预训练文本到视频模型生成高质量全景视频，通过纬度感知采样和旋转语义去噪解决现有问题，并贡献了PanoVid数据集。
...
Method: PanoWan采用纬度感知采样避免畸变，旋转语义去噪和平铺像素解码确保边界无缝过渡。

Result: PanoWan在全景视频生成中表现最佳，并在零样本下游任务中展现鲁棒性。

Conclusion: PanoWan通过创新模块和高质量数据集PanoVid，成功提升了全景视频生成质量。
...
### [178] [GL-PGENet: A Parameterized Generation Framework for Robust Document Image Enhancement](https://arxiv.org/abs/2505.22021)
*Zhihong Tang,Yang Li*

Main category: cs.CV

TL;DR: GL-PGENet是一种新型文档图像增强网络，针对多退化彩色文档图像设计，结合全局与局部优化，通过参数化生成机制和两阶段训练策略，实现了高效且鲁棒的增强效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法局限于单退化修复或灰度图像处理，无法满足多退化彩色文档图像的需求。

Method: 提出GL-PGENet，包含分层增强框架、双分支局部细化网络和修改的NestUNet架构，采用两阶段训练策略。

Result: 在DocUNet和RealDAE数据集上分别达到0.7721和0.9480的SSIM分数，具有跨领域适应性和计算效率。

Conclusion: GL-PGENet在多退化彩色文档图像增强中表现优异，适用于实际场景。
...
### [179] [Learnable Burst-Encodable Time-of-Flight Imaging for High-Fidelity Long-Distance Depth Sensing](https://arxiv.org/abs/2505.22025)
*Manchao Bao,Shengjiang Fang,Tao Yue,Xuemei Hu*

Main category: cs.CV

TL;DR: 论文提出了一种新型的ToF成像范式BE-ToF，通过脉冲爆发模式和端到端学习框架解决长距离深度成像中的相位缠绕和低信噪比问题。
...
Method: BE-ToF系统采用脉冲爆发模式估计反射信号的相位延迟，并提出端到端学习框架联合优化编码函数和深度重建网络。

Result: 通过仿真和原型实验验证了BE-ToF的有效性和实用性。

Conclusion: BE-ToF为长距离深度成像提供了一种高保真且实用的解决方案。
...
### [180] [Guess the Age of Photos: An Interactive Web Platform for Historical Image Age Estimation](https://arxiv.org/abs/2505.22031)
*Hasan Yucedag,Adam Jatowt*

Main category: cs.CV

TL;DR: 论文介绍了一个名为'Guess the Age of Photos'的网页平台，通过两种游戏化模式让用户估计历史照片的年代，评估结果显示用户更擅长相对比较而非绝对年份猜测。


<details>
  <summary>Details</summary>
Motivation: 开发一个互动平台，通过游戏化方式提升用户对历史照片年代估计的兴趣，同时作为教育工具培养历史意识和分析能力。

Method: 平台使用Python、Flask、Bootstrap和PostgreSQL构建，基于10,150张1930-1999年的照片数据集，设计了两种游戏模式：Guess the Year和Timeline Challenge。

Result: 113名用户参与测试，完成15,473次游戏，满意度4.25/5。用户在相对比较中准确率65.9%，绝对年份猜测准确率25.6%。

Conclusion: 该平台不仅作为教育工具有效，还为研究人类对图像时间线索的感知提供了资源，并可用于训练计算机视觉模型。
...
### [181] [Balanced Token Pruning: Accelerating Vision Language Models Beyond Local Optimization](https://arxiv.org/abs/2505.22038)
*Kaiyuan Li,Xiaoyue Chen,Chen Gao,Yong Li,Xinlei Chen*

Main category: cs.CV

TL;DR: 论文提出了一种名为平衡令牌修剪（BTP）的方法，通过分阶段修剪视觉令牌以减少计算开销，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在修剪图像令牌时未充分考虑其对当前层和后续层的联合影响，导致修剪效果不佳。

Method: BTP方法利用校准集将修剪过程分为多个阶段，早期关注后续层影响，后期注重局部输出一致性。

Result: 实验表明，BTP方法在多种LVLM上实现了78%的压缩率，同时平均保留96.7%的原始性能。

Conclusion: BTP方法有效解决了图像令牌修剪中的全局与局部权衡问题，显著提升了计算效率。
...
### [182] [OmniAD: Detect and Understand Industrial Anomaly via Multimodal Reasoning](https://arxiv.org/abs/2505.22039)
...
<details>
  <summary>Details</summary>
Motivation: 工业知识融入异常检测的详细分析仍具挑战性，OmniAD旨在填补这一空白。
...
### [183] [LatentMove: Towards Complex Human Movement Video Generation](https://arxiv.org/abs/2505.22046)
*Ashkan Taghipour,Morteza Ghahremani,Mohammed Bennamoun,Farid Boussaid,Aref Miri Rekavandi,Zinuo Li,Qiuhong Ke,Hamid Laga*

Main category: cs.CV

TL;DR: LatentMove是一个基于DiT的框架，专注于生成高质量的人体动画视频，解决了复杂非重复动作导致的变形问题，并提出了新的数据集和评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂、非重复的人体动作时容易产生不自然的变形，因此需要一种更鲁棒的解决方案。

Method: 采用DiT框架，结合条件控制分支和可学习的面部/身体标记，以保持帧间一致性和细节。

Result: LatentMove显著提升了人体动画质量，尤其在处理快速复杂动作时表现优异。

Conclusion: LatentMove推动了图像到视频生成技术的边界，并提供了新的数据集和评估工具。
...
### [184] [AquaMonitor: A multimodal multi-view image sequence dataset for real-life aquatic invertebrate biodiversity monitoring](https://arxiv.org/abs/2505.22065)
...
Method: 通过两年监测采集图像，结合DNA序列、干质量和尺寸测量，构建多模态数据集，并定义三个基准任务。

Result: 数据集包含2.7M图像、43,189标本、1358个DNA序列和1494个测量数据，是最大的生物多模态数据集之一。

Conclusion: AquaMonitor为水生生物多样性监测提供了实用工具，其基准任务可直接提升监测效率，支持立法水质评估。
...
### [185] [From Failures to Fixes: LLM-Driven Scenario Repair for Self-Evolving Autonomous Driving](https://arxiv.org/abs/2505.22067)
...
Conclusion: SERA通过自适应场景推荐和少样本学习，有效提升自动驾驶系统的性能和泛化能力。
...
### [186] [Bringing CLIP to the Clinic: Dynamic Soft Labels and Negation-Aware Learning for Medical Analysis](https://arxiv.org/abs/2505.22079)
*Hanbin Ko,Chang-Min Park*

Main category: cs.CV

TL;DR: 论文提出了一种结合临床增强动态软标签和医学图形对齐的新方法，以解决医学数据中的否定和数据不平衡问题，并在多个任务中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 通用领域架构（如CLIP）直接应用于医学数据时面临否定处理和数据不平衡的挑战，需要改进以提升临床理解和对比损失的适用性。

Method: 提出临床增强动态软标签和医学图形对齐方法，并引入基于否定的硬负样本，以增强模型对临床语言复杂性的理解。

Result: 方法在零样本、微调分类和报告检索等任务中表现最优，并通过CXR-Align基准验证了其临床语言理解能力。

Conclusion: 该方法易于实现且能泛化到对比学习框架中，显著提升了医学视觉语言处理能力和临床语言理解水平。
...
### [187] [MObyGaze: a film dataset of multimodal objectification densely annotated by experts](https://arxiv.org/abs/2505.22084)
...
TL;DR: 该论文提出了一种新的AI任务，旨在通过多模态（视觉、语音、音频）时间模式来表征和量化电影中的物化现象，并发布了MObyGaze数据集。


<details>
  <summary>Details</summary>
Motivation: 研究电影中性别表征的差异及其如何通过物化现象传播刻板印象。

Method: 结合电影研究和心理学，定义了物化的5个子结构和11个跨3种模态的概念，并构建了MObyGaze数据集，标注了6072个片段。

Result: 展示了任务的可行性，并比较了多种视觉、文本和音频模型的性能。

Conclusion: 通过多模态方法成功量化了电影中的物化现象，并公开了数据集和代码以促进进一步研究。
...
### [188] [Fast Feature Matching of UAV Images via Matrix Band Reduction-based GPU Data Schedule](https://arxiv.org/abs/2505.22089)
*San Jiang,Kan You,Wanshou Jiang,Qingquan Li*

Main category: cs.CV

TL;DR: 本文提出了一种基于GPU数据调度算法的高效无人机图像特征匹配方法，通过矩阵带约简（MBR）分块和GPU加速级联哈希提升效率。
...
Method: 1. 使用图像检索技术选择匹配对；2. 基于MBR生成紧凑图像块以减少冗余IO；3. 在GPU加速级联哈希框架下执行特征匹配，并结合几何约束和RANSAC优化。

Result: 实验表明，相比KD-Tree方法，速度提升77.0至100.0倍，且在BA中精度相当。
...
### [189] [UAVPairs: A Challenging Benchmark for Match Pair Retrieval of Large-scale UAV Images](https://arxiv.org/abs/2505.22098)
*Junhuan Liu,San Jiang,Wei Ge,Wei Huang,Bingxuan Guo,Qingquan Li*

Main category: cs.CV

TL;DR: 本文提出UAVPairs数据集和训练流程，用于无人机图像匹配对检索，通过几何相似性和多场景结构优化训练样本，并设计排名列表损失提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决大规模无人机图像匹配对检索中训练样本生成成本高和现有损失函数局限性问题。

Method: 构建UAVPairs数据集，提出批处理非平凡样本挖掘策略，设计排名列表损失优化全局相似性结构。

Result: 实验表明，使用UAVPairs和排名列表损失的模型在检索精度、视图图连通性和3D重建质量上显著提升。

Conclusion: UAVPairs数据集和训练流程为无人机图像匹配对检索提供了高效解决方案，模型在复杂场景中表现更鲁棒。
...
### [190] [On the Transferability and Discriminability of Repersentation Learning in Unsupervised Domain Adaptation](https://arxiv.org/abs/2505.22099)
*Wenwen Qiang,Ziyin Gu,Lingyu Si,Jiangmeng Li,Changwen Zheng,Fuchun Sun,Hui Xiong*

Main category: cs.CV

TL;DR: 论文提出了一种新的对抗性无监督域自适应（UDA）框架RLGLC，通过结合域对齐和目标域可判别性增强约束，解决了传统方法忽视目标域特征判别性的问题。


<details>
  <summary>Details</summary>
Motivation: 传统对抗性UDA方法仅依赖分布对齐和源域经验风险最小化，忽视了目标域特征的判别性，导致性能不佳。

Method: 提出RLGLC框架，结合域对齐目标与可判别性增强约束，使用AR-WWD处理类别不平衡和语义维度加权，并通过局部一致性机制保留细粒度判别信息。

Result: 在多个基准数据集上的实验表明，RLGLC优于现有方法，验证了理论视角的价值。

Conclusion: 在对抗性UDA中，同时保证可迁移性和可判别性是必要的，RLGLC框架为此提供了有效解决方案。
...
### [191] [Adapting Segment Anything Model for Power Transmission Corridor Hazard Segmentation](https://arxiv.org/abs/2505.22105)
*Hang Chen,Maoyuan Ye,Peng Yang,Haibin He,Juhua Liu,Bo Du*

Main category: cs.CV

TL;DR: 论文提出ELE-SAM模型，通过改进SAM模型以适应电力传输走廊危险分割任务，包括上下文感知提示适配器和高保真掩码解码器，并在新构建的ELE-40K数据集上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 电力传输走廊危险分割（PTCHS）对保障电力传输安全至关重要，但现有SAM模型在复杂场景下表现不佳，尤其是对精细结构的物体。
...
Conclusion: ELE-SAM在PTCHS任务中表现出色，为高质量通用对象分割提供了有效解决方案。
...
### [192] [Autoregression-free video prediction using diffusion model for mitigating error propagation](https://arxiv.org/abs/2505.22111)
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归视频预测方法存在误差传播问题，尤其是在预测远距离未来帧时表现不佳。本文旨在通过无自回归框架解决这一问题。

Method: ARFree框架包含两个关键组件：1）运动预测模块，从上下文帧中提取运动特征预测未来运动；2）训练方法，提升相邻未来帧之间的运动连续性和上下文一致性。
...
### [193] [SridBench: Benchmark of Scientific Research Illustration Drawing of Image Generation Model](https://arxiv.org/abs/2505.22126)
...
<details>
  <summary>Details</summary>
Motivation: 科学插图生成需要高精度和专业知识，目前缺乏评估AI在此任务上的基准。

Method: 构建包含1,120个实例的SridBench，涵盖13个学科，由专家和多模态大语言模型（MLLMs）收集，并从六个维度评估。

Result: 实验显示顶级模型如GPT-4o-image在语义保真度和结构准确性上仍不及人类，存在文本/视觉清晰度和科学正确性问题。
...
### [194] [Real-Time Blind Defocus Deblurring for Earth Observation: The IMAGIN-e Mission Approach](https://arxiv.org/abs/2505.22128)
*Alejandro D. Mousist*

Main category: cs.CV

TL;DR: 论文提出了一种针对ISS上IMAGIN-e任务中地球观测图像机械散焦的盲去模糊方法，利用Sentinel-2数据估计散焦核并在GAN框架下训练恢复模型，无需参考图像。
...
Method: 基于Sentinel-2数据估计散焦核，采用GAN框架训练恢复模型，无需参考图像。

Result: 在合成退化的Sentinel-2图像上，SSIM提升72.47%，PSNR提升25.00%；在IMAGIN-e任务中，NIQE提升60.66%，BRISQUE提升48.38%。

Conclusion: 该方法在资源受限的空间边缘计算环境下有效，已成功部署于IMAGIN-e任务，支持水体分割和轮廓检测等应用。
...
### [195] [What Makes for Text to 360-degree Panorama Generation with Stable Diffusion?](https://arxiv.org/abs/2505.22129)
*Jinhong Ni,Chang-Bin Zhang,Qiang Zhang,Jing Zhang*

Main category: cs.CV

TL;DR: 论文探讨了如何通过微调预训练的扩散模型（如Stable Diffusion）生成360度全景图像，揭示了注意力模块中不同矩阵的作用，并提出了一种高效的新方法UniPano。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解预训练扩散模型在全景图像生成中的适应机制，并解决传统方法在性能和效率上的不足。

Method: 通过分析注意力模块中查询、键、值和输出权重矩阵的作用，提出UniPano框架，专注于优化关键矩阵以减少计算开销。

Result: UniPano在性能和效率上优于现有方法，显著降低了内存占用和训练时间，支持更高分辨率的全景生成。

Conclusion: 研究揭示了预训练扩散模型在全景生成中的适应机制，UniPano为未来研究提供了高效且可扩展的基线。
...
### [196] [FaceEditTalker: Interactive Talking Head Generation with Facial Attribute Editing](https://arxiv.org/abs/2505.22141)
...
Conclusion: FaceEditTalker为音频驱动说话头部生成提供了灵活的面部属性编辑能力，具有广泛的应用潜力。
...
### [197] [3D Question Answering via only 2D Vision-Language Models](https://arxiv.org/abs/2505.22143)
*Fengyun Wang,Sicheng Yu,Jiawei Wu,Jinhui Tang,Hanwang Zhang,Qianru Sun*

Main category: cs.CV

TL;DR: 论文提出cdViews方法，通过自动选择关键且多样的2D视图，利用2D视觉语言模型（LVLMs）以零样本方式解决3D场景理解任务。


<details>
  <summary>Details</summary>
Motivation: 探索如何利用大型视觉语言模型（LVLMs）解决3D场景理解任务，特别是在3D-QA任务中，避免资源密集型的3D模型训练。

Method: 提出cdViews方法，包含viewSelector（选择关键视图）和viewNMS（增强视图多样性），通过2D模型处理3D点云的采样视图。

Result: 在ScanQA和SQA基准测试中，cdViews实现了最先进的性能，仅依赖2D模型且无需微调。

Conclusion: 2D LVLMs是目前解决3D任务的最有效替代方案，避免了资源密集的3D模型训练。
...
### [198] [Flexible Tool Selection through Low-dimensional Attribute Alignment of Vision and Language](https://arxiv.org/abs/2505.22146)
...
<details>
  <summary>Details</summary>
Motivation: 人类在工具选择上展现出独特的认知能力，但现有计算模型对此能力的研究不足。

Method: 使用视觉编码器（ResNet或ViT）提取工具图像的属性，并结合微调的语言模型（如GPT-2、LLaMA）从任务描述中推导所需属性。

Result: 该方法在工具选择任务中达到74%的准确率，显著优于直接工具匹配（20%）和小型多模态模型（21%-58%），接近GPT-4o（73%）的性能。

Conclusion: 该研究提供了一种参数高效且可解释的解决方案，模拟了人类工具认知，推动了认知科学和实际工具选择任务的应用。
...
### [199] [Improving Brain-to-Image Reconstruction via Fine-Grained Text Bridging](https://arxiv.org/abs/2505.22150)
*Runze Xia,Shuo Feng,Renzhi Wang,Congchi Yin,Xuyun Wen,Piji Li*

Main category: cs.CV

TL;DR: 论文提出了一种名为FgB2I的方法，通过细粒度文本作为桥梁改进脑到图像的重建，解决了现有方法中细节和语义不一致的问题。
...
Method: FgB2I方法分为三个阶段：细节增强、解码细粒度文本描述和基于文本的脑到图像重建，利用大视觉语言模型生成细粒度描述，并通过三种奖励指标指导解码。

Result: 细粒度文本描述可集成到现有重建方法中，实现更精细的脑到图像重建。
...
### [200] [Learning A Robust RGB-Thermal Detector for Extreme Modality Imbalance](https://arxiv.org/abs/2505.22154)
*Chao Tian,Chao Yang,Guoqing Zhu,Qiang Wang,Zhenyu He*

Main category: cs.CV

TL;DR: 论文提出了一种新的RGB-T目标检测方法，通过基础-辅助检测器架构和模态交互模块，解决了模态不平衡问题，提升了模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实场景中，RGB-T数据可能因环境或技术问题出现模态退化，导致模态不平衡和OOD问题，传统方法难以应对。

Method: 提出基础-辅助检测器架构，引入模态交互模块自适应加权模态，并通过模态伪退化模拟训练数据不平衡。

Result: 实验表明，该方法显著降低了缺失率（55%），并在多种基线检测器上提升了性能。

Conclusion: 该方法有效解决了极端模态不平衡问题，增强了模型在实际场景中的鲁棒性。
...
### [201] [Q-VDiT: Towards Accurate Quantization and Distillation of Video-Generation Diffusion Transformers](https://arxiv.org/abs/2505.22167)
...
TL;DR: Q-VDiT是一个专门为视频扩散变换器（DiT）设计的量化框架，通过Token-aware Quantization Estimator（TQE）和Temporal Maintenance Distillation（TMD）解决量化信息丢失和优化目标不对齐的问题，显著提升了视频生成的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的图像生成模型量化方法不适用于视频生成任务，主要挑战包括量化过程中的信息丢失和优化目标与视频生成需求的不匹配。

Method: 提出了Q-VDiT框架，包括TQE（补偿量化误差）和TMD（保持帧间时空相关性）。

Result: W3A6 Q-VDiT在场景一致性上达到23.40，比现有量化方法性能提升1.9倍。

Conclusion: Q-VDiT通过针对性的量化策略和优化方法，显著提升了视频DiT模型的效率和性能。
...
### [202] [S2AFormer: Strip Self-Attention for Efficient Vision Transformer](https://arxiv.org/abs/2505.22195)
*Guoan Xu,Wenfeng Huang,Wenjing Jia,Jiamao Li,Guangwei Gao,Guo-Jun Qi*

Main category: cs.CV

TL;DR: S2AFormer提出了一种高效的Vision Transformer架构，通过Strip Self-Attention（SSA）和Hybrid Perception Blocks（HPBs）结合CNN的局部感知与Transformer的全局建模能力，显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: Vision Transformer（ViT）的计算需求随token数量呈二次增长，限制了其实际效率。现有方法结合卷积与自注意力仍存在计算瓶颈。

Method: 设计SSA减少K和V的空间维度，压缩Q和K的通道维度，同时通过HPBs整合CNN和Transformer的优势。

Result: 在ImageNet-1k、ADE20k和COCO等基准测试中，S2AFormer在GPU和非GPU环境下均表现出高效性和准确性。

Conclusion: S2AFormer在效率和效果之间取得了最佳平衡，是高效视觉Transformer的有力候选。
...
### [203] [Investigating Mechanisms for In-Context Vision Language Binding](https://arxiv.org/abs/2505.22200)
*Darshana Saravanan,Makarand Tapaswi,Vineet Gandhi*

Main category: cs.CV

TL;DR: 研究探讨了视觉语言模型（VLMs）如何通过Binding ID机制关联图像和文本中的对象及其描述。


<details>
  <summary>Details</summary>
Motivation: 理解VLMs如何在多模态中建立关联，以验证Binding ID机制在图像-文本绑定中的适用性。

Method: 使用合成数据集和任务，测试VLMs是否能为图像和文本中的对象分配相同的Binding ID。

Result: 实验表明，VLMs为图像和文本中的对象分配了相同的Binding ID，实现了上下文关联。

Conclusion: Binding ID机制在VLMs中有效，支持图像和文本的多模态关联。
...
### [204] [A Survey on Training-free Open-Vocabulary Semantic Segmentation](https://arxiv.org/abs/2505.22209)
*Naomi Kombol,Ivan Martinović,Siniša Šegvić*

Main category: cs.CV

TL;DR: 该论文综述了无需训练的开放词汇语义分割方法，重点介绍了基于多模态分类模型的现有技术，并探讨了其历史、发展和最新进展。


<details>
  <summary>Details</summary>
Motivation: 传统语义分割方法需要大量计算资源和标注数据，而开放词汇语义分割要求模型能够分类未学习过的类别，因此研究者转向无需训练的方法，利用现有模型。

Method: 论文首先定义任务，然后概述流行的模型类型，并分类介绍了30多种方法，包括纯CLIP模型、辅助视觉基础模型和生成方法。

Result: 论文总结了当前研究的局限性、潜在问题，并提出了未来研究的未探索方向。

Conclusion: 该综述旨在为新研究者提供入门指南，并激发对该领域的兴趣。
...
### [205] [Look & Mark: Leveraging Radiologist Eye Fixations and Bounding boxes in Multimodal Large Language Models for Chest X-ray Report Generation](https://arxiv.org/abs/2505.22222)
*Yunsoo Kim,Jinge Wu,Su-Hwan Kim,Pardeep Vasudev,Jiashu Shen,Honghan Wu*

Main category: cs.CV

TL;DR: 提出了一种名为Look & Mark（L&M）的新方法，通过结合放射科医生的注视点和标注框来提升多模态大语言模型在医学图像分析中的性能，显著减少了错误并提高了可靠性。
...
Method: L&M方法结合放射科医生的注视点（Look）和标注框（Mark），通过上下文学习而非微调来提升模型性能。

Result: L&M显著提升了模型性能，如CXR-LLaVA的A.AVG指标提高了1.2%，LLaVA-Med提升了9.2%，并减少了临床错误。
...
### [206] [Hadaptive-Net: Efficient Vision Models via Adaptive Cross-Hadamard Synergy](https://arxiv.org/abs/2505.22226)
*Xuyang Zhang,Xi Zhang,Liang Chen,Hao Shi,Qingshan Guo*

Main category: cs.CV

TL;DR: 本文提出了一种基于Hadamard乘积的自适应模块（ACH）和轻量级网络Hadaptive-Net，显著提升了网络在视觉任务中的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 尽管Hadamard乘积在增强网络表示能力和维度压缩方面具有潜力，但其实际应用尚未充分开发。本文旨在填补这一空白。

Method: 通过分析Hadamard乘积的优势，提出自适应跨通道Hadamard乘积模块（ACH），并构建轻量级网络Hadaptive-Net。

Result: 实验表明，Hadaptive-Net在推理速度和准确性之间实现了前所未有的平衡。

Conclusion: ACH模块和Hadaptive-Net为视觉任务提供了一种高效且高性能的解决方案。
...
### [207] [GoMatching++: Parameter- and Data-Efficient Arbitrary-Shaped Video Text Spotting and Benchmarking](https://arxiv.org/abs/2505.22228)
*Haibin He,Jing Zhang,Maoyuan Ye,Juhua Liu,Bo Du,Dacheng Tao*

Main category: cs.CV

TL;DR: 本文提出GoMatching++方法，通过冻结现成的图像文本检测器并引入轻量级可训练跟踪器，显著提升视频文本检测性能，同时降低训练成本。还发布了包含弯曲文本的新基准ArTVideo。


<details>
  <summary>Details</summary>
Motivation: 当前视频文本检测方法性能仍落后于图像文本检测，主要原因是识别能力有限。本文旨在通过高效方法提升视频文本检测性能。

Method: 提出GoMatching++方法，包括两部分：(1) 重新评分机制以弥合图像与视频数据差距，(2) LST-Matcher增强冻结图像文本检测器的视频处理能力。

Result: GoMatching++在多个基准测试中创下新记录，并显著降低训练成本。同时发布ArTVideo基准，填补弯曲文本数据空白。

Conclusion: GoMatching++和ArTVideo将推动视频文本检测领域发展，相关资源已开源。
...
### [208] [Enjoying Information Dividend: Gaze Track-based Medical Weakly Supervised Segmentation](https://arxiv.org/abs/2505.22230)
*Zhisong Wang,Yiwen Ye,Ziyang Chen,Yong Xia*

Main category: cs.CV

TL;DR: GradTrack利用医生的注视轨迹（包括注视点、持续时间和时间顺序）提升弱监督语义分割性能，通过注视轨迹图和轨迹注意力机制实现多级注视监督，实验表明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 医学图像中的弱监督语义分割（WSSS）难以有效利用稀疏标注，现有基于注视的方法未充分利用注视数据中的丰富信息。

Method: 提出GradTrack框架，包含注视轨迹图生成和轨迹注意力两个组件，通过多级注视监督逐步优化特征。

Result: 在Kvasir-SEG和NCI-ISBI数据集上，GradTrack的Dice分数分别提升3.21%和2.61%，显著缩小与全监督模型的差距。

Conclusion: GradTrack通过充分利用注视数据，显著提升了WSSS性能，为医学图像分割提供了新思路。
...
### [209] [StateSpaceDiffuser: Bringing Long Context to Diffusion World Models](https://arxiv.org/abs/2505.22246)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型因缺乏持久环境状态，导致在长序列任务中视觉一致性快速崩溃。

Method: 引入StateSpaceDiffuser，将状态空间模型的序列表示整合到扩散模型中，以恢复长期记忆。

Result: 实验表明，StateSpaceDiffuser在2D迷宫导航和复杂3D环境中均能保持视觉一致性，性能显著优于基线。
...
### [210] [YH-MINER: Multimodal Intelligent System for Natural Ecological Reef Metric Extraction](https://arxiv.org/abs/2505.22250)
...
TL;DR: 该研究开发了YH-OSI系统，基于多模态大模型（MLLM）实现珊瑚礁生态监测的智能框架，结合目标检测、语义分割和先验输入，提高了分类准确性和生态指标提取效率。
...
Method: 采用YH-OSI系统，结合目标检测（mAP@0.5=0.78）和语义分割模块，利用多模态模型（Qwen2-VL）实现分类和生态指标提取。

Result: 系统在低光和密集遮挡场景下实现88%的属级分类准确率，并提取核心生态指标。
...
### [211] [Domain Adaptation of Attention Heads for Zero-shot Anomaly Detection](https://arxiv.org/abs/2505.22259)
*Kiyoon Jeong,Jaehyuk Heo,Junyeong Son,Pilsung Kang*

Main category: cs.CV

TL;DR: HeadCLIP提出了一种零样本异常检测方法，通过自适应调整文本和图像编码器，结合可学习提示和动态头权重，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本异常检测方法在领域适应方面存在不足，HeadCLIP旨在通过全面适应文本和图像编码器解决这一问题。
...
Conclusion: HeadCLIP通过全面领域适应和联合异常评分，显著提升了零样本异常检测的性能。
...
### [212] [Learning Fine-Grained Geometry for Sparse-View Splatting via Cascade Depth Loss](https://arxiv.org/abs/2505.22279)
*Wenjun Lu,Haodong Chen,Anqi Yi,Yuk Ying Chung,Zhiyong Wang,Kun Hu*

Main category: cs.CV

TL;DR: 论文提出了一种名为HDGS的深度监督框架，通过多尺度深度一致性提升稀疏视图条件下的新视角合成质量。
...
Method: 引入Hierarchical Depth-Guided Splatting (HDGS)框架，采用Cascade Pearson Correlation Loss (CPCL)在多空间尺度上对齐渲染与估计的单目深度。

Result: 在LLFF和DTU基准测试中，HDGS在稀疏视图条件下实现了最先进的性能，同时保持高效高质量的渲染。

Conclusion: HDGS通过多尺度深度一致性显著提升了稀疏视图场景下的结构保真度，解决了现有方法的局限性。
...
### [213] [From Controlled Scenarios to Real-World: Cross-Domain Degradation Pattern Matching for All-in-One Image Restoration](https://arxiv.org/abs/2505.22284)
*Junyu Fan,Chuanlin Liao,Yi Lin*

Main category: cs.CV

TL;DR: 论文提出了一种统一域自适应图像恢复（UDAIR）框架，通过跨域知识迁移解决多退化模式图像恢复问题，提升了真实场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在封闭场景表现良好，但在真实场景中因数据分布差异导致性能下降，需解决退化感知能力不足的问题。

Method: 设计了编码本学习离散嵌入表示退化模式，采用跨样本对比学习捕捉共享特征，并提出域适应策略动态对齐源域和目标域的编码本嵌入。

Result: 在10个开源数据集上，UDAIR实现了最先进的性能，特征聚类验证了未知条件下的退化识别能力。

Conclusion: UDAIR通过域适应和对比学习显著提升了真实场景中的图像恢复性能，具有强泛化能力。
...
### [214] [Neural Restoration of Greening Defects in Historical Autochrome Photographs Based on Purely Synthetic Data](https://arxiv.org/abs/2505.22291)
*Saptarshi Neil Sinha,P. Julius Kuehn,Johannes Koppe,Arjan Kuijper,Michael Weinmann*

Main category: cs.CV

TL;DR: 提出一种基于合成数据集生成和生成式AI的方法，用于自动修复数字化彩色照片中的绿色缺陷，解决了现有方法在颜色还原和效率上的不足。
...
Method: 通过合成数据集生成模拟绿色缺陷，并改进ChaIR方法的加权损失函数以处理颜色不平衡问题。

Result: 该方法能够高效修复照片，减少时间需求，且无需大量人工干预。

Conclusion: 提出的方法为视觉艺术修复提供了一种高效且自动化的解决方案，优于现有技术。
...
### [215] [CADReview: Automatically Reviewing CAD Programs with Error Detection and Correction](https://arxiv.org/abs/2505.22304)
*Jiali Chen,Xusen Hei,HongFei Liu,Yuancheng Wei,Zikun Deng,Jiayuan Xie,Yi Cai,Li Qing*

Main category: cs.CV

TL;DR: 论文提出了一种名为ReCAD的框架，用于自动检测和修正CAD程序中的错误，确保3D对象与参考图像的一致性。


<details>
  <summary>Details</summary>
Motivation: 设计师在CAD设计流程中需要耗费大量时间检查和修正原型，而现有的多模态大语言模型（MLLMs）在识别几何组件和空间操作方面表现不佳。

Method: 提出了ReCAD框架，用于检测程序错误并提供修正反馈，同时构建了一个包含20K程序-图像对的数据集CADReview。

Result: 实验表明，ReCAD在CAD审查任务中显著优于现有MLLMs。

Conclusion: ReCAD在设计中具有巨大潜力，能够有效提升CAD程序的审查效率。
...
### [216] [IKIWISI: An Interactive Visual Pattern Generator for Evaluating the Reliability of Vision-Language Models Without Ground Truth](https://arxiv.org/abs/2505.22305)
*Md Touhidul Islam,Imran Kabir,Md Alimoor Reza,Syed Masum Billah*

Main category: cs.CV

TL;DR: IKIWISI是一种交互式视觉模式生成工具，用于评估无真实标签时的视觉语言模型，通过热图展示模型输出，并利用人类模式识别能力评估模型可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法依赖真实标签，而IKIWISI旨在通过可视化方式评估模型行为，揭示人类与机器感知的差异。

Method: IKIWISI将模型输出转化为二值热图（绿色表示对象存在，红色表示不存在），并引入“间谍对象”检测模型幻觉。

Result: 15名参与者认为IKIWISI易用，评估结果与客观指标相关，且仅需少量热图单元即可得出结论。

Conclusion: IKIWISI不仅补充传统评估方法，还揭示了改进视觉语言系统中人类与机器感知对齐的机会。
...
### [217] [Learning to Infer Parameterized Representations of Plants from 3D Scans](https://arxiv.org/abs/2505.22337)
...
Method: 训练递归神经网络，利用L系统生成的虚拟植物数据，从3D点云推断参数化树状表示。

Result: 在合成植物上验证，该方法在重建、分割和骨架化任务中表现与现有最优方法相当。

Conclusion: 该方法为植物3D建模提供了一种通用且高效的解决方案。
...
### [218] [Progressive Data Dropout: An Embarrassingly Simple Approach to Faster Training](https://arxiv.org/abs/2505.22342)
*Shriram M S,Xinyue Hao,Shihao Hou,Yang Lu,Laura Sevilla-Lara,Anurag Arnab,Shreyank N Gowda*

Main category: cs.CV

TL;DR: 论文提出了一种名为Progressive Data Dropout的新训练范式，通过减少有效训练轮次至基准的12.4%，同时提升准确率高达4.82%，且无需改变模型架构或优化器。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习依赖大规模数据集训练，成本高昂，且标准训练方法效率低下。

Method: 结合硬数据挖掘和dropout的见解，提出渐进式数据丢弃方法。
...
### [219] [Task-Driven Implicit Representations for Automated Design of LiDAR Systems](https://arxiv.org/abs/2505.22344)
*Nikhil Behari,Aaron Young,Akshat Dave,Ramesh Raskar*

Main category: cs.CV

TL;DR: 提出了一种基于任务驱动的自动化LiDAR系统设计框架，通过六维设计空间和流生成模型优化传感器配置。
...
Method: 在六维设计空间中表示LiDAR配置，利用流生成模型学习任务特定的隐式密度，并通过期望最大化方法优化传感器分布。

Result: 方法在3D视觉任务中验证有效，支持人脸扫描、机器人跟踪和物体检测等应用。

Conclusion: 框架实现了高效、约束感知的LiDAR系统设计，为复杂任务提供了自动化解决方案。
...
### [220] [VME: A Satellite Imagery Dataset and Benchmark for Detecting Vehicles in the Middle East and Beyond](https://arxiv.org/abs/2505.22353)
...
Method: 构建VME数据集（覆盖中东12国的54个城市）和CDSI基准数据集，结合手动和半自动标注方法。

Result: VME显著提升中东地区车辆检测准确率，CDSI训练模型在全球范围内表现更优。

Conclusion: VME和CDSI填补了地理偏见的数据缺口，提升了车辆检测模型的泛化能力。
...
### [221] [Identity-Preserving Text-to-Image Generation via Dual-Level Feature Decoupling and Expert-Guided Fusion](https://arxiv.org/abs/2505.22360)
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在生成图像时难以分离身份相关与无关信息，导致过拟合或身份丢失。

Method: 框架包括隐式-显式前景-背景解耦模块（IEDM）和基于专家混合（MoE）的特征融合模块（FFM），辅以三种损失函数。

Result: 实验表明，该方法显著提升了生成图像的质量、场景适应性和多样性。

Conclusion: 该框架有效解决了身份信息解耦问题，为文本到图像生成提供了更灵活的解决方案。
...
### [222] [DAM: Domain-Aware Module for Multi-Domain Dataset Condensation](https://arxiv.org/abs/2505.22387)
*Jaehyun Choi,Gyojin Han,Dong-Jae Lee,Sunghyun Baek,Junmo Kim*

Main category: cs.CV

TL;DR: 论文提出了一种多领域数据集压缩方法（MDDC），通过领域感知模块（DAM）提升跨领域性能。


<details>
  <summary>Details</summary>
Motivation: 现代数据集通常是多领域的，但现有数据集压缩方法未充分考虑这一点。

Method: 引入领域感知模块（DAM），利用可学习空间掩码嵌入领域特征，并通过频率统计进行伪领域标注。

Result: DAM在单领域、跨领域和跨架构性能上均优于基线方法。

Conclusion: MDDC通过DAM有效提升了数据集压缩的跨领域泛化能力。
...
### [223] [PacTure: Efficient PBR Texture Generation on Packed Views with Visual Autoregressive Models](https://arxiv.org/abs/2505.22394)
*Fan Fei,Jiajun Tang,Fei-Peng Tian,Boxin Shi,Ping Tan*

Main category: cs.CV

TL;DR: PacTure是一个新框架，通过未纹理化的3D网格、文本描述和可选图像提示生成基于物理的渲染（PBR）材质纹理。它引入视图打包技术提高多视图生成的分辨率，并通过多域生成框架提升效率。


<details>
  <summary>Details</summary>
Motivation: 解决现有2D生成纹理方法在全局一致性和分辨率上的不足，以及多视图生成的高推理成本问题。

Method: 提出视图打包技术，将多视图排列为2D矩形装箱问题，保持空间邻近性；结合多域生成框架，实现高效多视图生成。

Result: 实验表明，PacTure在生成PBR纹理的质量和训练/推理效率上优于现有方法。

Conclusion: PacTure通过创新技术解决了纹理生成中的一致性和效率问题，为PBR材质生成提供了高效解决方案。
...
### [224] [Zooming from Context to Cue: Hierarchical Preference Optimization for Multi-Image MLLMs](https://arxiv.org/abs/2505.22396)
...
TL;DR: CcDPO是一种多级偏好优化框架，通过从全局上下文到局部细节的视觉线索增强多图像理解，减少幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在单图像任务中表现优异，但在多图像理解中因跨模态不对齐导致幻觉问题，现有方法（如DPO）未能全面建模上下文。

Method: 提出CcDPO框架，包括上下文级优化（全局序列偏好）和细节级优化（区域目标视觉提示），并构建MultiScope-42k数据集支持优化。

Result: CcDPO显著减少幻觉现象，并在单图像和多图像任务中实现性能提升。

Conclusion: CcDPO通过多级优化有效提升多图像理解能力，为MLLMs的跨模态对齐提供了新思路。
...
### [225] [Self-Reflective Reinforcement Learning for Diffusion-based Image Reasoning Generation](https://arxiv.org/abs/2505.22407)
*Jiadong Pan,Zhiyuan Ma,Kaiyan Zhang,Ning Ding,Bowen Zhou*

Main category: cs.CV

TL;DR: SRRL是一种自反思强化学习算法，用于扩散模型，通过在多轮去噪过程中引入反思迭代，实现了逻辑图像生成的推理能力。
...
Method: SRRL将去噪轨迹视为Chain of Thought步骤，引入条件引导的前向过程，实现多轮反思去噪迭代。

Result: 实验表明，SRRL在逻辑图像生成任务中表现优异，甚至优于GPT-4o。

Conclusion: SRRL首次将图像推理引入生成任务，为遵循物理规律和非传统物理现象的生成提供了新思路。
...
### [226] [Frugal Incremental Generative Modeling using Variational Autoencoders](https://arxiv.org/abs/2505.22408)
*Victor Enescu,Hichem Sahbi*

Main category: cs.CV

TL;DR: 提出了一种基于变分自编码器（VAE）的无回放增量学习方法，解决了增量学习中的数据增长和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 增量学习面临数据增长和灾难性遗忘的挑战，现有回放方法难以扩展。本文旨在通过VAE模型解决这些问题。

Method: 设计了基于多模态潜在空间的增量生成模型，并引入正交性准则以减少灾难性遗忘。提出了静态和动态VAE变体，控制参数增长。

Result: 实验表明，该方法比相关方法节省至少一个数量级的内存，同时达到SOTA准确率。

Conclusion: 提出的方法在内存效率和准确性上表现优异，为增量学习提供了可行的解决方案。
...
### [227] [GeoDrive: 3D Geometry-Informed Driving World Model with Precise Action Control](https://arxiv.org/abs/2505.22421)
...
Method: 从输入帧提取3D表示，基于用户指定的车辆轨迹生成2D渲染，并通过动态编辑模块增强渲染效果。

Result: 实验表明，GeoDrive在动作准确性和3D空间感知上显著优于现有模型，并能泛化到新轨迹。

Conclusion: GeoDrive提供了更真实、适应性强且可靠的场景建模，提升了自动驾驶的安全性。
...
### [228] [RC-AutoCalib: An End-to-End Radar-Camera Automatic Calibration Network](https://arxiv.org/abs/2505.22427)
*Van-Tin Luu,Yon-Lin Cai,Vu-Hoang Tran,Wei-Chen Chiu,Yi-Ting Chen,Ching-Chun Huang*

Main category: cs.CV

TL;DR: 提出首个在线自动雷达与相机几何标定方法，通过双视角表示和选择性融合机制解决雷达高度数据稀疏性和不确定性，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 雷达高度数据的稀疏性和不确定性导致系统运行时的自动标定一直具有挑战性。

Method: 采用双视角表示（前视图和鸟瞰图）和选择性融合机制，结合多模态交叉注意力机制和抗噪声匹配器。

Result: 在nuScenes数据集上测试，显著优于现有雷达-相机和LiDAR-相机标定方法。
...
### [229] [Zero-Shot 3D Visual Grounding from Vision-Language Models](https://arxiv.org/abs/2505.22429)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖标记的3D数据和预定义类别，限制了在开放世界中的扩展性。

Method: 采用混合输入格式（查询对齐的渲染视图和空间增强文本描述），包含视角适应模块和融合对齐模块。

Result: 在ScanRefer和Nr3D上表现优异，分别超越基线7.7%和7.1%，甚至媲美全监督方法。
...
### [230] [Distance Transform Guided Mixup for Alzheimer's Detection](https://arxiv.org/abs/2505.22434)
*Zobia Batool,Huseyin Ozkan,Erchan Aptoula*

Main category: cs.CV

TL;DR: 该论文提出了一种基于距离变换和分层混合的单域泛化方法，用于解决阿尔茨海默病检测中医学数据集的类别不平衡和多样性不足问题。


<details>
  <summary>Details</summary>
Motivation: 医学数据集存在类别不平衡、成像协议差异和多样性有限的问题，限制了模型的泛化能力。

Method: 通过计算MRI扫描的距离变换，将其分层并结合不同样本的层生成增强图像，以保持大脑结构。

Result: 实验结果表明，该方法在ADNI和AIBL数据集上提高了泛化性能。

Conclusion: 提出的方法有效生成了多样化的数据，同时保留了大脑结构，提升了模型的泛化能力。
...
### [231] [Can NeRFs See without Cameras?](https://arxiv.org/abs/2505.22441)
*Chaitanya Amballa,Sattwik Basu,Yu-Lin Wei,Zhijian Yang,Mehmet Ergezer,Romit Roy Choudhury*

Main category: cs.CV

TL;DR: NeRFs被重新设计以学习多路径信号（如WiFi），从而推断环境，例如从稀疏WiFi测量中学习室内平面图。


<details>
  <summary>Details</summary>
Motivation: 研究是否可以通过多路径信号（如RF/音频）推断环境，尽管这些信号包含复杂的环境反射（多路径）。

Method: 重新设计NeRFs，使其能够学习多路径信号，并应用于从稀疏WiFi测量中推断室内平面图。

Result: 学习到的隐式平面图效果良好，支持室内信号预测和基本光线追踪等应用。

Conclusion: 重新设计的NeRFs能够有效利用多路径信号推断环境，展示了在复杂信号处理中的潜力。
...
### [232] [On Geometry-Enhanced Parameter-Efficient Fine-Tuning for 3D Scene Segmentation](https://arxiv.org/abs/2505.22444)
*Liyao Tang,Zhe Chen,Dacheng Tao*

Main category: cs.CV

TL;DR: 提出了一种名为GEM的几何感知参数高效微调模块，用于3D点云模型，显著减少计算和存储成本。


<details>
  <summary>Details</summary>
Motivation: 大规模预训练点云模型在3D场景理解中表现优异，但全微调成本高；现有PEFT方法在3D任务中表现不佳。

Method: GEM结合细粒度局部位置编码和轻量潜在注意力机制，解决几何和空间分布不匹配问题。

Result: GEM性能接近或超过全微调，仅更新1.6%参数，训练时间和内存需求显著降低。

Conclusion: GEM为大规模3D点云模型的高效微调设定了新基准。
...
### [233] [NFR: Neural Feature-Guided Non-Rigid Shape Registration](https://arxiv.org/abs/2505.22445)
*Puhua Jiang,Zhangquan Chen,Mingze Sun,Ruqi Huang*

Main category: cs.CV

TL;DR: 提出了一种基于学习的新型3D形状配准框架，无需训练时的对应标注，通过结合深度学习特征与几何配准流程，显著提升了非刚性变形和部分形状匹配的准确性。
...
Method: 将深度学习提取的神经特征融入迭代几何配准流程，动态更新对应关系并通过一致性先验过滤。

Result: 在多个基准测试中达到最先进水平，即使训练数据有限，也能处理未见过的复杂变形形状。

Conclusion: 该方法在非刚性配准和部分匹配中表现出色，优于传统和固有方法。
...
### [234] [Fostering Video Reasoning via Next-Event Prediction](https://arxiv.org/abs/2505.22457)
...
<details>
  <summary>Details</summary>
Motivation: 现有任务（如视频问答）依赖人工标注或更强的MLLMs，而视频描述则容易混淆时空信息。NEP通过利用未来视频片段作为自监督信号，填补了这一空白。

Method: 将视频分段为过去和未来帧，MLLMs以过去帧为输入，预测未来帧的事件摘要，从而促进时序推理。为此，作者构建了V1-33K数据集，并探索了多种视频指令调优策略。

Result: 实验验证了NEP作为一种可扩展且有效的训练范式，能够显著提升MLLMs的时序推理能力。

Conclusion: NEP为MLLMs的时序推理提供了一种新的自监督学习框架，并通过FutureBench评估其预测未来事件的连贯性。
...
### [235] [Universal Domain Adaptation for Semantic Segmentation](https://arxiv.org/abs/2505.22458)
*Seun-An Choe,Keon-Hee Park,Jinwoo Choi,Gyeong-Moon Park*

Main category: cs.CV

TL;DR: 提出了一种新的无监督领域自适应方法UniDA-SS，通过图像匹配和原型区分解决类别设置未知的问题。


<details>
  <summary>Details</summary>
Motivation: 传统UDA-SS方法假设源域和目标域类别设置已知，而实际场景中这一假设不成立，导致性能下降。

Method: 提出UniMAP框架，包含域特定原型区分（DSPD）和目标图像匹配（TIM）两个关键组件。

Result: 在UniDA-SS基准测试中显著优于基线方法。

Conclusion: UniMAP在未知类别设置下实现了鲁棒的自适应，为语义分割领域提供了新思路。
...
### [236] [SHTOcc: Effective 3D Occupancy Prediction with Sparse Head and Tail Voxels](https://arxiv.org/abs/2505.22461)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法未探索体素的本质分布模式，导致结果不理想。

Method: 提出SHTOcc，采用稀疏头尾体素构建和分离学习，平衡头尾类关键体素。

Result: GPU内存减少42.2%，推理速度提升58.6%，准确率提高约7%。

Conclusion: SHTOcc在性能和效率上均表现出色，验证了其有效性。
...
### [237] [Single Domain Generalization for Alzheimer's Detection from 3D MRIs with Pseudo-Morphological Augmentations and Contrastive Learning](https://arxiv.org/abs/2505.22465)
*Zobia Batool,Huseyin Ozkan,Erchan Aptoula*

Main category: cs.CV

TL;DR: 论文提出了一种结合可学习伪形态模块和监督对比学习的方法，以提升阿尔茨海默病MRI检测的泛化能力，解决了类别不平衡和协议差异问题。
...
Method: 提出可学习伪形态模块生成形状感知的解剖学有意义增强数据，并结合监督对比学习模块提取鲁棒的类别特定表示。

Result: 在三个数据集上的实验表明，该方法在类别不平衡和成像协议差异下表现更优，泛化能力显著提升。

Conclusion: 该方法有效提升了阿尔茨海默病检测模型的泛化性能，尤其在数据分布差异大的情况下表现突出。
...
### [238] [ProCrop: Learning Aesthetic Image Cropping from Professional Compositions](https://arxiv.org/abs/2505.22490)
...
Method: ProCrop通过融合专业照片特征与查询图像特征，学习专业构图；同时提出一个242K弱标注数据集，通过外绘和专业图像迭代生成高质量裁剪建议。

Result: ProCrop在监督和弱监督设置下均显著优于现有方法，新数据集训练下甚至匹配全监督方法。

Conclusion: ProCrop和数据集将公开，推动图像美学与构图分析研究。
...
### [239] [The Meeseeks Mesh: Spatially Consistent 3D Adversarial Objects for BEV Detector](https://arxiv.org/abs/2505.22499)
*Aixuan Li,Mochu Xiang,Jing Zhang,Yuchao Dai*

Main category: cs.CV

TL;DR: 论文研究了3D对抗攻击对自动驾驶中3D物体检测模型的影响，提出了一种生成非侵入式3D对抗对象的方法，验证了其通用性和有效性。


<details>
  <summary>Details</summary>
Motivation: 评估3D物体检测模型在真实世界对抗攻击下的鲁棒性，确保自动驾驶系统的安全性。

Method: 采用可微分渲染技术建模对抗对象与目标车辆的空间关系，引入遮挡感知模块增强视觉一致性，设计BEV空间特征优化策略。

Result: 实验表明，该方法能有效抑制先进3D检测器的预测，对抗对象在不同位置和距离下保持攻击效果。

Conclusion: 提出的方法为测试3D物体检测模型的鲁棒性提供了重要工具，对抗对象具有强泛化能力。
...
### [240] [PathFL: Multi-Alignment Federated Learning for Pathology Image Segmentation](https://arxiv.org/abs/2505.22522)
*Yuan Zhang,Feng Chen,Yaolei Qi,Guanyu Yang,Huazhu Fu*

Main category: cs.CV

TL;DR: PathFL是一种多对齐联邦学习框架，通过图像、特征和模型聚合三个层面的对齐策略，解决病理图像分割中的异质性问题。


<details>
  <summary>Details</summary>
Motivation: 病理图像分割在多中心应用中面临成像模态、器官和扫描设备的异质性挑战，导致表示偏差和泛化能力受限。

Method: PathFL采用三级对齐策略：图像层面的协作风格增强模块、特征层面的自适应特征对齐模块，以及模型层面的分层相似性聚合策略。

Result: 在四种异质性病理图像数据集上的评估表明，PathFL在性能和鲁棒性上优于现有方法。

Conclusion: PathFL通过多级对齐策略有效提升了病理图像分割的泛化能力，适用于多中心异质性数据。
...
### [241] [Thinking with Generated Images](https://arxiv.org/abs/2505.22525)
*Ethan Chern,Zhulin Hu,Steffi Chern,Siqi Kou,Jiadi Su,Yan Ma,Zhijie Deng,Pengfei Liu*

Main category: cs.CV

TL;DR: 提出了一种新范式，通过生成中间视觉思考步骤，使大型多模态模型（LMMs）能够在文本和视觉模态间进行原生思考，显著提升视觉推理能力。
...
Method: 采用两种机制：(1) 通过中间视觉子目标分解复杂任务；(2) 通过自我批判生成初始视觉假设并迭代优化。

Result: 在视觉生成基准测试中，模型处理复杂多对象场景的相对性能提升高达50%（从38%到57%）。

Conclusion: 该方法为AI模型提供了类似人类的视觉想象和迭代优化能力，适用于多个领域，如生物化学、建筑设计和战略规划。
...
### [242] [PrismLayers: Open Data for High-Quality Multi-Layer Transparent Image Generative Models](https://arxiv.org/abs/2505.22523)
*Junwen Chen,Heyang Jiang,Yanbin Wang,Keming Wu,Ji Li,Chao Zhang,Keiji Yanai,Dong Chen,Yuhui Yuan*

Main category: cs.CV

TL;DR: 论文提出了一种生成高质量多层透明图像的方法，通过发布PrismLayersPro数据集、训练免费合成管道和开源模型ART+，解决了多层透明数据缺乏的问题。


<details>
  <summary>Details</summary>
Motivation: 解决多层透明图像生成任务中缺乏高质量数据集的问题，以支持更灵活的创意控制。

Method: 1. 发布PrismLayersPro数据集；2. 使用现成扩散模型生成数据；3. 开发开源模型ART+，包含LayerFLUX和MultiLayerFLUX技术。

Result: ART+在60%的用户比较中优于原ART模型，视觉质量与FLUX.1-[dev]模型相当。

Conclusion: 该研究为多层透明图像生成任务奠定了数据集基础，支持需要精确编辑的高质量图像应用。
...
### [243] [RICO: Improving Accuracy and Completeness in Image Recaptioning via Visual Reconstruction](https://arxiv.org/abs/2505.22613)
*Yuchi Wang,Yishuo Cai,Shuhuai Ren,Sihan Yang,Linli Yao,Yuanxin Liu,Yuanxing Zhang,Pengfei Wan,Xu Sun*

Main category: cs.CV

TL;DR: RICO通过视觉重建优化图像描述，提出迭代框架RICO和高效版本RICO-Flash，显著提升描述准确性和完整性。
...
Method: 利用文本到图像模型重建参考图像，通过MLLM识别差异并迭代优化描述。RICO-Flash通过DPO学习高效生成。

Result: 在CapsBench和CompreCap上优于基线约10%。

Conclusion: RICO框架有效提升图像描述质量，代码已开源。
...
### [244] [RiverMamba: A State Space Model for Global River Discharge and Flood Forecasting](https://arxiv.org/abs/2505.22535)
*Mohamad Hakam Shams Eddin,Yikui Zhang,Stefan Kollet,Juergen Gall*

Main category: cs.CV

TL;DR: RiverMamba是一种新型深度学习模型，通过预训练和时空建模，显著提升了全球河流流量和洪水预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在水文学中局限于局部应用，未能充分利用水体的空间联系，亟需能建模时空关系的新方法。

Method: RiverMamba利用高效的Mamba模块，结合ECMWF HRES气象预报数据，进行时空建模。

Result: RiverMamba在7天预测期内表现出色，优于现有AI和物理模型，尤其在极端洪水预测中。

Conclusion: RiverMamba为全球河流流量和洪水预测提供了可靠且高效的解决方案。
...
### [245] [Sherlock: Self-Correcting Reasoning in Vision-Language Models](https://arxiv.org/abs/2505.22651)
*Yi Ding,Ruqi Zhang*
...
Result: 在八个基准测试中平均准确率达64.1（直接生成）和65.4（自校正后），优于同类模型。

Conclusion: Sherlock框架有效提升模型推理能力，减少对标注数据的依赖，具有广泛适用性。
...
### [246] [Scaling-up Perceptual Video Quality Assessment](https://arxiv.org/abs/2505.22543)
...
TL;DR: 论文提出OmniVQA框架，构建大规模视频质量评估多模态指令数据库（MIDB），并引入互补训练策略，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决视频质量评估领域因数据稀缺和规模不足而未能充分利用数据缩放定律的问题。

Method: 提出OmniVQA框架，构建OmniVQA-Chat-400K和OmniVQA-MOS-20K数据集，采用互补训练策略。

Result: 模型在质量理解和评分任务中达到最先进性能。

Conclusion: OmniVQA框架和数据集有效解决了视频质量评估领域的数据和性能瓶颈。
...
### [247] [Deep Learning-Based BMD Estimation from Radiographs with Conformal Uncertainty Quantification](https://arxiv.org/abs/2505.22551)
*Long Hui,Wai Lok Yeung*

Main category: cs.CV

TL;DR: 利用深度学习从膝关节X光片中估算骨密度（BMD），提供不确定性量化，为骨质疏松筛查提供新方法。


<details>
  <summary>Details</summary>
Motivation: DXA设备有限，阻碍骨质疏松筛查。研究探索利用广泛可用的膝关节X光片进行BMD估算。

Method: 使用EfficientNet模型在OAI数据集上训练，预测BMD。比较两种测试时间增强（TTA）方法，并采用Split Conformal Prediction提供预测区间。

Result: 皮尔逊相关系数为0.68（传统TTA）。多样本TTA方法提供更紧的置信区间，同时保持覆盖率。

Conclusion: 尽管膝关节X光片与标准DXA存在解剖学差异，但该方法为AI辅助BMD筛查奠定了基础，有望改善早期骨质疏松检测。
...
### [248] [3DLLM-Mem: Long-Term Spatial-Temporal Memory for Embodied 3D Large Language Model](https://arxiv.org/abs/2505.22657)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在动态多房间3D环境中规划和行动能力不足，主要原因是缺乏有效的3D空间-时间记忆建模。

Method: 提出3DLLM-Mem模型，利用工作记忆令牌选择性融合来自情景记忆的空间和时间特征。

Result: 3DLLM-Mem在3DMem-Bench基准测试中表现优异，成功率达到16.5%的提升。

Conclusion: 3DLLM-Mem通过动态记忆管理显著提升了模型在复杂3D环境中的任务执行能力。
...
### [249] [MultiFormer: A Multi-Person Pose Estimation System Based on CSI and Attention Mechanism](https://arxiv.org/abs/2505.22555)
...
<details>
  <summary>Details</summary>
Motivation: 解决多人体姿态识别和CSI特征学习的挑战。

Method: 采用基于Transformer的时频双令牌特征提取器和多阶段特征融合网络。

Result: 在公开和自采集数据集上表现优于现有方法，尤其在高移动性关键点上。

Conclusion: MultiFormer在人体姿态估计中实现了更高的准确性。
...
### [250] [PRISM: Video Dataset Condensation with Progressive Refinement and Insertion for Sparse Motion](https://arxiv.org/abs/2505.22564)
*Jaehyun Choi,Jiwan Hur,Gyojin Han,Jaemyung Yu,Junmo Kim*

Main category: cs.CV

TL;DR: PRISM是一种新的视频数据集压缩方法，通过渐进式优化和稀疏运动插入，保留时空动态关系，性能优于现有方法且存储需求更低。


<details>
  <summary>Details</summary>
Motivation: 解决视频数据压缩中时空动态关系处理的挑战，提升压缩效率并减少存储需求。

Method: PRISM方法通过渐进式优化和稀疏运动插入，保留视频中静态内容与动态运动的相互依赖关系。

Result: 在标准视频动作识别基准测试中，PRISM性能优于现有方法，同时保持紧凑的表示。

Conclusion: PRISM为视频数据集压缩提供了一种高效且存储友好的解决方案，适用于资源受限环境。
...
### [251] [Universal Visuo-Tactile Video Understanding for Embodied Interaction](https://arxiv.org/abs/2505.22566)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在视觉和语言模态上取得进展，但未能有效整合触觉信息，而触觉反馈对真实世界交互至关重要。

Method: 提出三阶段训练范式：VTV增强、VTV-文本对齐和文本提示微调，并贡献VTV150K数据集。

Result: VTV-LLM在触觉视频理解任务中表现优异，支持复杂触觉推理能力。

Conclusion: 该框架为触觉领域的人机交互奠定基础。
...
### [252] [ImageReFL: Balancing Quality and Diversity in Human-Aligned Diffusion Models](https://arxiv.org/abs/2505.22569)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面表现优异，但与人类偏好的对齐仍具挑战性，且传统奖励微调方法常损害多样性。

Method: 1. 结合生成策略：在生成后期应用奖励调优模型，保留基础模型以维持多样性。2. ImageReFL微调方法：通过真实图像训练和多正则化器提升多样性。

Result: 方法在质量和多样性指标上优于传统奖励微调，用户研究证实其更好平衡对齐与多样性。

Conclusion: 提出的方法有效解决了扩散模型在人类偏好对齐与多样性之间的权衡问题。
...
### [253] [Tell me Habibi, is it Real or Fake?](https://arxiv.org/abs/2505.22581)
*Kartik Kuckreja,Parul Gupta,Injy Hamed,Thamar Solorio,Muhammad Haris Khan,Abhinav Dhall*

Main category: cs.CV

TL;DR: 论文介绍了首个大规模阿拉伯语-英语视听深度伪造数据集ArEnAV，包含387k视频和765小时内容，支持多语言和语码转换研究。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测研究多集中于单语内容，忽视了多语言和语码转换的挑战，尤其是阿拉伯语与英语混合的情况。

Method: 通过整合四种TTS和两种唇同步模型生成数据集，并对比现有单语和多语数据集及检测模型。

Result: 数据集包含387k视频和765小时内容，支持多模态深度伪造检测研究。

Conclusion: ArEnAV数据集填补了多语言深度伪造检测的空白，推动了相关研究。
...
### [254] [SAM-R1: Leveraging SAM for Reward Feedback in Multimodal Segmentation via Reinforcement Learning](https://arxiv.org/abs/2505.22596)
*Jiaqi Huang,Zunnan Xu,Jun Zhou,Ting Liu,Yicheng Xiao,Mingwen Ou,Bowen Ji,Xiu Li,Kehong Yuan*

Main category: cs.CV

TL;DR: SAM-R1利用强化学习训练多模态大模型进行图像分割，无需依赖昂贵的人工标注推理数据，通过细粒度奖励和优化目标提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖成本高的人工标注推理数据，而强化学习可以赋予模型推理能力，无需此类数据。

Method: 提出SAM-R1框架，结合细粒度分割设置和任务特定奖励，利用SAM作为奖励提供者指导学习。

Result: 仅用3k训练样本，SAM-R1在多个基准测试中表现优异。

Conclusion: 强化学习能有效赋予多模态模型分割导向的推理能力。
...
### [255] [Adversarially Robust AI-Generated Image Detection for Free: An Information Theoretic Perspective](https://arxiv.org/abs/2505.22604)
*Ruixuan Zhang,He Wang,Zhengyu Zhao,Zhiqing Guo,Xun Yang,Yunfeng Diao,Meng Wang*

Main category: cs.CV

TL;DR: 论文提出了一种无需训练的对抗防御方法TRIM，通过信息论度量解决AIGI检测中特征纠缠问题，显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: AIGI技术的快速发展导致恶意使用（如伪造和虚假信息）增加，现有检测器易受对抗攻击，且防御方法稀缺。研究发现对抗训练（AT）在AIGI检测中性能崩溃，原因是特征纠缠破坏了特征与标签的互信息。

Method: 提出TRIM方法，基于标准检测器，利用预测熵和KL散度量特征偏移，无需额外训练。

Result: 在多数据集和攻击场景下，TRIM显著优于现有防御方法（如ProGAN上提升33.88%），同时保持原始检测精度。

Conclusion: TRIM是首个无需训练的对抗防御方法，有效解决了AIGI检测中的特征纠缠问题，具有实际应用潜力。
...
### [256] [PS4PRO: Pixel-to-pixel Supervision for Photorealistic Rendering and Optimization](https://arxiv.org/abs/2505.22616)
*Yezhi Shen,Qiuchen Zhai,Fengqing Zhu*

Main category: cs.CV

TL;DR: 提出了一种基于视频帧插值的数据增强方法（PS4PRO），用于提升神经渲染在静态和动态场景中的重建性能。


<details>
  <summary>Details</summary>
Motivation: 神经渲染方法在复杂和动态场景中因输入视角有限而重建质量受限。

Method: 设计轻量级高质量视频帧插值模型PS4PRO，通过多样视频数据集训练，隐式建模相机运动和3D几何。

Result: 实验表明，该方法有效提升了静态和动态场景的重建性能。

Conclusion: 视频帧插值可作为神经渲染的数据增强手段，显著提升重建质量。
...
### [257] [ObjectClear: Complete Object Removal via Object-Effect Attention](https://arxiv.org/abs/2505.22636)
*Jixin Zhao,Shangchen Zhou,Zhouxia Wang,Peiqing Yang,Chen Change Loy*

Main category: cs.CV

TL;DR: 论文提出了一种新数据集OBER和框架ObjectClear，用于解决物体移除及其视觉效应（如阴影、反射）的问题，显著提升了移除效果和背景保真度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散修复方法在移除物体及其效应时存在内容幻觉、背景改变等问题，需要更精确的解决方案。

Method: 引入OBER数据集，包含成对图像和精确掩码；提出ObjectClear框架，利用物体效应注意力机制分离前景移除和背景重建。

Result: ObjectClear在复杂场景中表现优于现有方法，提升了物体效应移除质量和背景保真度。

Conclusion: OBER数据集和ObjectClear框架为物体效应移除提供了有效解决方案，尤其在复杂场景中表现突出。
...
### [258] [SPIRAL: Semantic-Aware Progressive LiDAR Scene Generation](https://arxiv.org/abs/2505.22643)
*Dekai Zhu,Yixuan Hu,Youquan Liu,Dongyue Lu,Lingdong Kong,Slobodan Ilic*

Main category: cs.CV

TL;DR: Spiral是一种新型的LiDAR扩散模型，同时生成深度、反射图像和语义图，解决了现有范围视图方法在语义标注上的不足，并在实验中表现优异。
...
Method: 提出Spiral模型，结合范围视图表示的优势（计算高效、网络设计简化），同时生成深度、反射图像和语义图，并引入新的语义感知评估指标。

Result: 在SemanticKITTI和nuScenes数据集上，Spiral以最小参数量实现最佳性能，优于两步生成-分割方法，并能有效用于下游分割任务的合成数据增强。

Conclusion: Spiral在LiDAR场景生成中实现了跨模态一致性和高效性，显著减少了标注工作量。
...
### [259] [Let Them Talk: Audio-Driven Multi-Person Conversational Video Generation](https://arxiv.org/abs/2505.22647)
*Zhe Kong,Feng Gao,Yong Zhang,Zhuoliang Kang,Xiaoming Wei,Xunliang Cai,Guanying Chen,Wenhan Luo*

Main category: cs.CV

TL;DR: 论文提出了一种新任务：多人对话视频生成，并提出了MultiTalk框架，解决了多人生成中的音频绑定问题和指令跟随能力问题。


<details>
  <summary>Details</summary>
Motivation: 现有音频驱动的人体动画方法主要关注单人动画，难以处理多流音频输入，且存在音频与人物绑定错误的问题，同时指令跟随能力有限。

Method: 提出了Label Rotary Position Embedding (L-RoPE)方法解决音频与人物绑定问题，并通过部分参数训练和多任务训练保留基础模型的指令跟随能力。

Result: MultiTalk在多个数据集（包括说话头部、说话身体和多人数据集）上表现优于其他方法。

Conclusion: MultiTalk框架在多人对话视频生成任务中表现出强大的生成能力。
...
### [260] [VScan: Rethinking Visual Token Reduction for Efficient Large Vision-Language Models](https://arxiv.org/abs/2505.22654)
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLMs）因视觉令牌序列较长导致计算成本高，限制了实时部署。

Method: VScan采用两阶段方法：视觉编码阶段结合全局和局部扫描与令牌合并，语言模型阶段引入中间层剪枝。

Result: 在四个LVLMs上验证，VScan显著加速推理（LLaVA-NeXT-7B上预填充速度提升2.91倍，FLOPs减少10倍），同时保留95.4%原始性能。

Conclusion: VScan通过优化视觉令牌处理，在性能和效率上优于现有方法，适合实时部署。
...
### [261] [Training Free Stylized Abstraction](https://arxiv.org/abs/2505.22663)
...
<details>
  <summary>Details</summary>
Motivation: 解决风格化抽象中身份特征保留与风格差异的平衡问题，尤其是对分布外个体的挑战。

Method: 利用视觉语言模型提取身份特征，结合跨域校正流反演策略动态重建结构，支持多轮生成。

Result: 在多种风格（如乐高、针织娃娃、南方公园）上表现优异，未见身份和风格泛化能力强。

Conclusion: 方法在开源环境下实现了高保真重建，无需微调，适用于多样化的风格化抽象任务。
...
### [262] [Zero-Shot Vision Encoder Grafting via LLM Surrogates](https://arxiv.org/abs/2505.22664)
*Kaiyu Yue,Vasu Singla,Menglin Jia,John Kirchenbauer,Rifaa Qadri,Zikui Cai,Abhinav Bhatele,Furong Huang,Tom Goldstein*

Main category: cs.CV

TL;DR: 通过训练小型“替代模型”来降低视觉语言模型（VLM）的训练成本，并实现零样本嫁接，性能接近完整训练。


<details>
  <summary>Details</summary>
Motivation: 降低使用大型语言模型（如Llama-70B）作为解码器时的高训练成本。

Method: 构建小型替代模型，继承目标LLM的浅层，训练视觉编码器后直接迁移到大型模型。

Result: 嫁接后的模型性能优于编码器-替代模型组合，某些任务表现接近完整训练，训练成本降低约45%。

Conclusion: 零样本嫁接是一种高效且低成本的方法，适用于VLM训练。
...
### [263] [Physics-inspired Generative AI models via real hardware-based noisy quantum diffusion](https://arxiv.org/abs/2505.22193)
*Marco Parigi,Stefano Martina,Francesco Aldo Venturelli,Filippo Caruso*

Main category: quant-ph

TL;DR: 论文提出两种基于量子扩散模型的物理启发协议，利用量子特性提升生成式AI性能，并在小规模量子设备上验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 现有量子扩散模型算法受限于近量子设备的可扩展性，因此探索利用量子特性提升模型性能的新方法。

Method: 1. 使用量子随机游走形式，结合量子与经典动力学生成更稳健的模型；2. 利用IBM量子硬件的固有噪声生成图像。

Result: 第一种方法生成MNIST图像的FID更低；第二种方法在四量子比特设备上实现图像生成。

Conclusion: 量子噪声可作为有用资源，为大规模量子生成式AI算法开辟新方向。
...
### [264] [Higher-Order Group Synchronization](https://arxiv.org/abs/2505.21932)
...
Method: 定义高阶群同步问题，提出基于消息传递算法的计算框架，并分析其理论保证。

Result: 数值实验表明，该方法在旋转和角度同步中优于标准成对同步方法，且对异常值更鲁棒。

Conclusion: 高阶群同步方法在特定情况下优于传统方法，并展示了在模拟冷冻电镜数据中的可比性能。
...
### [265] [Synonymous Variational Inference for Perceptual Image Compression](https://arxiv.org/abs/2505.22438)
*Zijian Liang,Kai Niu,Changshuo Wang,Jin Xu,Ping Zhang*

Main category: cs.IT

TL;DR: 本文提出了一种基于同义关系的变分推理方法（SVI），用于分析感知图像压缩问题，并引入了一种新的图像压缩方案（SIC）。实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 利用语义信息理论中的同义关系，重新分析感知图像压缩问题，以优化压缩性能。

Method: 提出同义变分推理（SVI）方法，构建理想同义集（Synset），并通过最小化部分语义KL散度近似其后验。

Result: 理论证明感知图像压缩的优化方向遵循三重权衡，实验显示SIC编解码器在率失真感知性能上表现优异。

Conclusion: 提出的SVI方法和SIC方案有效验证了同义关系在图像压缩中的应用潜力。
...
### [266] [ChemHAS: Hierarchical Agent Stacking for Enhancing Chemistry Tools](https://arxiv.org/abs/2505.21569)
...
<details>
  <summary>Details</summary>
Motivation: LLM代理在化学任务中表现优异，但受限于工具预测误差。本文探索如何利用LLM代理进一步减少这些误差。

Method: 提出ChemHAS方法，通过有限数据优化代理堆叠结构，增强化学工具。

Result: 在四项基础化学任务中达到SOTA性能，并识别四种代理堆叠行为。

Conclusion: ChemHAS能有效补偿工具预测误差，为科学研究中的AI代理应用提供新可能。
...
### [267] [Revisiting Bi-Linear State Transitions in Recurrent Neural Networks](https://arxiv.org/abs/2505.21749)
*M. Reza Ebrahimi,Roland Memisevic*

Main category: cs.LG

TL;DR: 论文探讨了循环神经网络中隐藏单元的作用，提出其不仅是记忆存储，还积极参与计算，并通过双线性操作验证了其在状态跟踪任务中的优势。
...
Method: 重新审视双线性操作，通过理论和实验验证其在状态跟踪任务中的自然归纳偏置。

Result: 双线性状态更新形成自然层次结构，对应复杂度递增的状态跟踪任务，线性循环网络（如Mamba）位于最低复杂度层级。

Conclusion: 双线性操作为隐藏单元在状态跟踪任务中的主动计算提供了理论基础，揭示了其层次化复杂性。
...
### [268] [Born a Transformer -- Always a Transformer?](https://arxiv.org/abs/2505.21785)
*Yana Veitsman,Mayank Jobanputra,Yash Sarrof,Aleksandra Bakalova,Vera Demberg,Ellie Pavlick,Michael Hahn*

Main category: cs.LG

TL;DR: 论文探讨了预训练大语言模型（LLM）是否能克服Transformer架构在序列任务中的理论限制，发现预训练选择性增强某些能力但无法突破长度泛化的根本限制。


<details>
  <summary>Details</summary>
Motivation: 研究预训练LLM是否能在实践中克服Transformer的理论限制，特别是在检索和复制任务中。

Method: 使用C-RASP框架分析长度泛化，并通过实验验证预训练模型在检索任务中的表现。

Result: 发现预训练模型在右向检索（induction）优于左向检索（anti-induction），这种不对称性可通过微调消除。

Conclusion: 预训练虽能增强某些能力，但无法突破Transformer在长度泛化上的根本限制。
...
### [269] [From Directions to Cones: Exploring Multidimensional Representations of Propositional Facts in LLMs](https://arxiv.org/abs/2505.21800)
*Stanley Yu,Vaidehi Bulusu,Oscar Yasunaga,Clayton Lau,Cole Blondin,Sean O'Brien,Kevin Zhu,Vasu Sharma*

Main category: cs.LG

TL;DR: 研究扩展了概念锥框架，发现LLMs中真实行为由多维锥中介，揭示了更丰富的真假命题结构。


<details>
  <summary>Details</summary>
Motivation: LLMs虽具对话能力但常生成虚假信息，现有方法未能完全捕捉其内部几何结构。

Method: 扩展概念锥框架至真实领域，通过因果干预、学习锥泛化及锥基干预验证多维锥的存在。

Result: 因果干预可翻转模型对事实陈述的响应，学习锥能跨架构泛化，锥基干预不影响无关行为。

Conclusion: 概念锥是探索LLMs抽象行为的有力工具，揭示了真假命题的多维结构。
...
### [270] [Let Me Think! A Long Chain-of-Thought Can Be Worth Exponentially Many Short Ones](https://arxiv.org/abs/2505.21825)
*Parsa Mirtaheri,Ezra Edelman,Samy Jelassi,Eran Malach,Enric Boix-Adsera*

Main category: cs.LG

TL;DR: 论文探讨了推理时计算在提升大语言模型推理能力中的作用，比较了顺序扩展（如更长的思维链）和并行扩展（如多数投票）的优势，发现某些图连通性问题中顺序扩展具有指数级优势。


<details>
  <summary>Details</summary>
Motivation: 研究推理时计算的最优分配问题，明确顺序扩展和并行扩展在不同场景下的表现差异。

Method: 通过理论分析和实验验证，比较顺序扩展和并行扩展在图连通性问题中的表现，使用不同思维链策略训练的语言模型进行测试。

Result: 在特定图连通性问题中，顺序扩展比并行扩展具有显著优势，实验验证了这一理论发现。

Conclusion: 顺序扩展在某些推理场景中具有指数级优势，为推理时计算的优化提供了新视角。
...
### [271] [Efficient Ensemble for Fine-tuning Language Models on Multiple Datasets](https://arxiv.org/abs/2505.21930)
*Dongyue Li,Ziniu Zhang,Lu Wang,Hongyang R. Zhang*

Main category: cs.LG

TL;DR: 提出一种集成方法，通过分组训练多个小适配器而非单一适配器，高效微调语言模型至多任务数据集。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如QLoRA）在单任务数据集上高效，但在多任务数据集上缺乏高效微调方案。

Method: 设计算法将n个数据集分组为m组（m<<n），每组训练一个适配器，加权组合形成集成。利用低秩适配的一阶近似特性快速估计性能。

Result: 在34B参数模型上，误差<1%，计算速度比基础微调快105倍。在Llama和GPT模型上，测试准确率比QLoRA高10%，FLOPs仅增加9%。

Conclusion: 集成方法在多任务微调中显著提升性能且计算高效。
...
### [272] [EnsemW2S: Enhancing Weak-to-Strong Generalization with Large Language Model Ensembles](https://arxiv.org/abs/2505.21959)
*Aakriti Agrawal,Mucong Ding,Zora Che,Chenghao Deng,Anirudh Satheesh,Bang An,Bayan Bruss,John Langford,Furong Huang*

Main category: cs.LG

TL;DR: 论文提出了一种名为EnsemW2S的方法，通过令牌级集成策略提升弱专家模型的能力，以监督更强的学生模型，在分布内和分布外数据集上均取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）接近或超越人类水平，如何利用仅接触人类水平数据的小模型有效监督和增强这些强大模型变得至关重要。

Method: 提出EnsemW2S方法，通过令牌级集成策略迭代结合多个弱专家模型，系统性改进其能力，以监督更强的学生模型。

Result: 在分布内数据集上，专家和学生模型分别提升了4%和3.2%；在分布外数据集上，分别提升了6%和2.28%。

Conclusion: EnsemW2S方法在弱到强泛化任务中表现出色，为监督强大模型提供了有效途径。
...
### [273] [Pitfalls of Rule- and Model-based Verifiers -- A Case Study on Mathematical Reasoning](https://arxiv.org/abs/2505.22203)
*Yuzhen Huang,Weihao Zeng,Xingshan Zeng,Qi Zhu,Junxian He*

Main category: cs.LG

TL;DR: 论文分析了强化学习中可验证奖励（RLVR）的验证器可靠性问题，以数学推理为例，发现规则验证器易漏判等效答案，模型验证器易被攻击导致误判。


<details>
  <summary>Details</summary>
Motivation: 探讨验证器在强化学习中的可靠性及其对训练的影响，以提升奖励系统的鲁棒性。

Method: 通过静态评估和强化学习训练场景，对比分析规则验证器和模型验证器的表现。

Result: 规则验证器漏判率高，模型验证器易被攻击导致误判，两者均影响训练效果。

Conclusion: 研究揭示了验证器的独特风险，为开发更鲁棒的奖励系统提供了重要见解。
...
### [274] [Train Sparse Autoencoders Efficiently by Utilizing Features Correlation](https://arxiv.org/abs/2505.22255)
*Vadim Kurochkin,Yaroslav Aksenov,Daniil Laptev,Daniil Gavrilov,Nikita Balagansky*

Main category: cs.LG

TL;DR: KronSAE通过Kronecker积分解减少稀疏自编码器的计算和内存开销，并引入mAND激活函数提升可解释性和性能。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器（SAEs）在解释语言模型隐藏状态方面表现出潜力，但大规模训练时计算和内存开销大，尤其是大字典尺寸下。

Method: 提出KronSAE架构，利用Kronecker积分解因子化潜在表示，减少计算和内存需求；引入mAND激活函数近似二进制AND操作。

Result: KronSAE显著降低了计算和内存开销，同时mAND提升了模型的可解释性和性能。

Conclusion: KronSAE和mAND为大规模稀疏自编码器训练提供了高效且可解释的解决方案。
...
### [275] [Skywork Open Reasoner 1 Technical Report](https://arxiv.org/abs/2505.22312)
...
<details>
  <summary>Details</summary>
Motivation: 探索强化学习在提升长链思维模型推理能力中的作用，并验证其有效性。

Method: 基于DeepSeek-R1-Distill模型系列，采用强化学习方法优化模型性能。

Result: 在AIME24、AIME25和LiveCodeBench上，32B模型准确率提升15.0%，7B模型提升13.9%。

Conclusion: Skywork-OR1在推理能力上表现优异，缓解熵崩溃现象对提升性能至关重要，并开源资源以支持社区研究。
...
### [276] [Mitigating Overthinking in Large Reasoning Models via Manifold Steering](https://arxiv.org/abs/2505.22411)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）在推理过程中常出现过度思考现象，导致计算资源浪费。本文旨在从机制可解释性角度解决这一问题。

Method: 研究发现过度思考现象与激活空间的低维流形相关，提出Manifold Steering方法，将干预方向投影到低维流形以减少噪声干扰。

Result: 实验表明，该方法在多个数学基准测试中减少输出token达71%，同时保持或提升准确性，并在代码生成和知识问答任务中表现稳定。

Conclusion: Manifold Steering方法有效解决了过度思考问题，具有跨领域适用性，为优化大型推理模型提供了新思路。
...
### [277] [Scaling Reasoning without Attention](https://arxiv.org/abs/2505.22425)
*Xueliang Zhao,Wei Wu,Lingpeng Kong*

Main category: cs.LG

TL;DR: 论文提出了一种名为\ourmodel的无注意力语言模型，通过架构和数据创新解决了Transformer的低效性和结构化微调不足的问题，并在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在复杂推理任务中取得进展，但仍受限于Transformer架构的低效性和高难度领域缺乏结构化微调。

Method: 采用基于Mamba-2的状态空间对偶（SSD）层，无需自注意力和键值缓存，实现固定内存和恒定时间推理；提出两阶段课程微调策略PromptCoT，生成结构化问题。

Result: \ourmodel-7B在多个基准测试中优于同类Transformer和混合模型，甚至超越更大的Gemma3-27B。

Conclusion: 状态空间模型是高效且可扩展的注意力架构替代方案，适用于高容量推理。
...
### [278] [The Entropy Mechanism of Reinforcement Learning for Reasoning Language Models](https://arxiv.org/abs/2505.22617)
...
TL;DR: 论文提出通过控制策略熵的崩溃来提升强化学习（RL）在大型语言模型（LLMs）中的推理能力，揭示了熵与下游性能的关系，并提出了两种有效的熵管理方法。


<details>
  <summary>Details</summary>
Motivation: 解决RL在LLMs中因策略熵崩溃导致的探索能力下降和性能饱和问题。

Method: 通过理论分析和实证研究熵动态，提出Clip-Cov和KL-Cov两种方法限制高协方差标记的更新。

Result: 实验证明这两种方法能有效鼓励探索，避免熵崩溃，提升下游性能。

Conclusion: 熵管理对RL的持续探索和性能提升至关重要，提出的方法简单有效。
...
### [279] [Position: Uncertainty Quantification Needs Reassessment for Large-language Model Agents](https://arxiv.org/abs/2505.22655)
*Michael Kirchhof,Gjergji Kasneci,Enkelejda Kasneci*

Main category: cs.LG

TL;DR: 论文探讨了大型语言模型（LLM）和聊天机器人在交互式场景中不确定性量化的局限性，提出了三种新的研究方向以丰富不确定性表达。


<details>
  <summary>Details</summary>
Motivation: 传统的不确定性二分法（偶然性和认知性）在LLM与用户的交互中显得过于局限，需要新的方法来应对开放和动态的交互环境。

Method: 通过文献综述发现传统定义在交互场景中失效，提出了三种新方向：未明确性不确定性、交互式学习和输出不确定性。

Result: 传统不确定性定义在交互场景中矛盾且失效，新方法有望提升透明度和信任度。

Conclusion: 论文呼吁研究新形式的不确定性表达，以使LLM交互更透明、可信和直观。
...
### [280] [Temporal Restoration and Spatial Rewiring for Source-Free Multivariate Time Series Domain Adaptation](https://arxiv.org/abs/2505.21525)
...
<details>
  <summary>Details</summary>
Motivation: 现有SFDA方法在多变量时间序列上表现不佳，主要原因是未考虑数据的空间相关性。

Method: 提出TERSE方法，结合时空特征编码、时间恢复和空间重连任务，实现跨域时空依赖建模。
...
### [281] [Taming Transformer Without Using Learning Rate Warmup](https://arxiv.org/abs/2505.21910)
*Xianbiao Qi,Yelin He,Jiaquan Ye,Chun-Guang Li,Bojia Zi,Xili Dai,Qin Zou,Rong Xiao*

Main category: cs.LG

TL;DR: 论文分析了Transformer训练中的模型崩溃现象，提出了一种基于Weyl不等式的优化策略，避免熵崩溃。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer大规模训练中因谱能量集中导致的模型崩溃问题。

Method: 提出一种优化策略，通过平滑权重更新，防止谱能量集中。

Result: 实验证明该策略能稳定训练ViT、Swin-Transformer和GPT，无需学习率预热。

Conclusion: 该优化策略有效解决了Transformer训练中的熵崩溃问题。
...
### [282] [From Dormant to Deleted: Tamper-Resistant Unlearning Through Weight-Space Regularization](https://arxiv.org/abs/2505.22310)
*Shoaib Ahmed Siddiqui,Adrian Weller,David Krueger,Gintare Karolina Dziugaite,Michael Curtis Mozer,Eleni Triantafillou*

Main category: cs.LG

TL;DR: 研究发现，LLM的遗忘方法易受重新学习攻击，即使微调少量无关样本，遗忘的知识也会重新出现。在视觉分类器中，仅使用保留集微调即可使遗忘集准确率从50%恢复至近100%。


<details>
  <summary>Details</summary>
Motivation: 探索LLM遗忘方法的脆弱性，揭示重新学习攻击的现象及其机制。

Method: 在视觉分类器中研究遗忘现象，分析权重空间特性（如L2距离和线性模式连接性）以预测抗重新学习能力。

Result: 遗忘集准确率可通过保留集微调恢复至近100%，而从头训练的模型保持50%。提出新方法显著提升抗攻击能力。

Conclusion: 权重空间特性可预测抗重新学习能力，新方法在抗攻击方面达到最优表现。
...
### [283] [A Closer Look at Multimodal Representation Collapse](https://arxiv.org/abs/2505.22483)
*Abhra Chaudhuri,Anjan Dutta,Tu Bui,Serban Georgescu*

Main category: cs.LG

TL;DR: 论文研究了模态崩溃现象，提出了一种通过显式基重分配防止模态崩溃的算法。


<details>
  <summary>Details</summary>
Motivation: 理解模态崩溃现象，即多模态融合模型倾向于依赖部分模态而忽略其他模态的问题。

Method: 通过跨模态知识蒸馏隐式解耦表示，并提出显式基重分配算法。

Result: 实验验证了理论发现，算法在多模态基准测试中表现良好。

Conclusion: 跨模态知识蒸馏和显式基重分配能有效防止模态崩溃，适用于缺失模态处理。
...
### [284] [Understanding Adversarial Training with Energy-based Models](https://arxiv.org/abs/2505.22486)
*Mujtaba Hussain Mirza,Maria Rosaria Briglia,Filippo Bartolucci,Senad Beadini,Giuseppe Lisanti,Iacopo Masi*

Main category: cs.LG

TL;DR: 论文通过能量模型（EBM）框架分析对抗训练（AT）中的灾难性过拟合（CO）和鲁棒过拟合（RO），并提出Delta Energy Regularizer（DER）缓解问题。同时探讨了鲁棒分类器的生成能力及其改进方法。


<details>
  <summary>Details</summary>
Motivation: 研究对抗训练中能量动态与过拟合现象的关系，并探索鲁棒分类器的生成潜力。

Method: 通过能量视角分析对抗样本与自然样本的能量差异，提出DER正则化器，并改进生成模型技术。

Result: DER有效缓解CO和RO；改进的生成技术提升了样本多样性和生成质量，达到竞争性IS和FID。

Conclusion: 能量视角为对抗训练提供了新见解，DER和生成改进技术展示了鲁棒分类器的潜力。
...
### [285] [VietASR: Achieving Industry-level Vietnamese ASR with 50-hour labeled data and Large-Scale Speech Pretraining](https://arxiv.org/abs/2505.21527)
*Jianheng Zhuo,Yifan Yang,Yiwen Shao,Yong Xu,Dong Yu,Kai Yu,Xie Chen*

Main category: eess.AS

TL;DR: 提出VietASR，一种利用大量未标注数据和少量标注数据的低成本ASR训练方法，在低资源语言（如越南语）中表现优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言ASR系统依赖大规模标注数据的问题，降低训练成本和延迟，提高实用性。

Method: 通过多轮自监督学习在大规模未标注数据上预训练，再在少量标注数据上微调，构建轻量但高效的ASR模型。

Result: 在7万小时未标注数据和50小时标注数据上训练的模型，性能优于Whisper Large-v3和商业ASR系统。

Conclusion: VietASR为低资源ASR提供了一种经济高效的解决方案，代码和模型将开源以促进研究。
...
### [286] [Evaluation of LLMs in Speech is Often Flawed: Test Set Contamination in Large Language Models for Speech Recognition](https://arxiv.org/abs/2505.22251)
*Yuan Tseng,Titouan Parcollet,Rogier van Dalen,Shucong Zhang,Sourav Bhattacharya*

Main category: eess.AS

TL;DR: 研究发现，LibriSpeech和Common Voice评估集中的大量数据出现在LLM预训练语料库中，质疑了基于这些数据集的结论可靠性。实验表明，受污染的LLM更倾向于生成训练中见过的句子，且语音识别系统对训练中见过的转录赋予更高概率。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM在语音任务中性能提升的可靠性，特别是数据污染对结果的影响。

Method: 比较受污染和未受污染的LLM在生成测试句子时的表现，分析语音识别系统的错误率和概率分配。

Result: 受污染的LLM更倾向于生成训练中见过的句子，语音识别系统对训练中见过的转录赋予更高概率，但错误率差异较小。

Conclusion: 数据污染会显著影响LLM的输出，强调了使用独立数据评估LLM语音系统的重要性。
...
### [287] [Complexity counts: global and local perspectives on Indo-Aryan numeral systems](https://arxiv.org/abs/2505.21510)
*Chundra Cathcart*

Main category: physics.soc-ph

TL;DR: 本文研究印欧语系语言（如印地语、古吉拉特语和孟加拉语）的数字系统复杂性，发现其1-99数字形式高度不透明，无法通过简单规则构建。通过跨语言数据，量化其复杂性并探讨其成因。


<details>
  <summary>Details</summary>
Motivation: 印欧语系语言的数字系统与其他语言（如英语、汉语）相比具有高度复杂性，但相关研究较少。本文旨在填补这一空白，探讨其复杂性成因及跨语言意义。

Method: 使用跨语言数据库数据，开发量化指标衡量数字系统复杂性，并分析宗教、地理隔离等因素的影响。

Result: 印欧语系语言的数字系统复杂性显著高于全球语言平均水平，但内部存在差异。其复杂性受多种因素影响，但仍符合跨语言高效交流的普遍压力。

Conclusion: 印欧语系语言的数字系统复杂性是一个被忽视的维度，应在跨语言数字系统研究中得到重视。
...
### [288] [Fluent but Culturally Distant: Can Regional Training Teach Cultural Understanding?](https://arxiv.org/abs/2505.21548)
*Dhruv Agarwal,Anya Shukla,Sunayana Sitaram,Aditya Vashistha*

Main category: physics.soc-ph

TL;DR: 研究发现，针对特定地区的LLM（如印度模型）在文化价值观和实践上与全球模型相比并未更贴近当地文化，甚至不如普通美国人对印度文化的代表性。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在文化上的西方倾向问题，评估区域模型是否能真正反映当地文化。

Method: 以印度为例，评估五个印度模型和五个全球模型在价值观和实践两个维度的表现。

Result: 印度模型在文化对齐上表现不佳，区域微调甚至可能损害文化能力。

Conclusion: 需要更多高质量、文化相关的数据来构建真正的主权LLM，文化评估应成为重要标准。
...
### [289] [Large-Area Fabrication-aware Computational Diffractive Optics](https://arxiv.org/abs/2505.22313)
*Kaixuan Wei,Hector A. Jimenez-Romero,Hadi Amata,Jipeng Sun,Qiang Fu,Felix Heide,Wolfgang Heidrich*

Main category: physics.optics

TL;DR: 论文提出了一种制造感知的衍射光学设计流程，结合超分辨率神经光刻模型和分布式计算框架，解决了仿真与制造之间的质量差距，实现了大规模衍射光学系统的优化与应用。


<details>
  <summary>Details</summary>
Motivation: 现有衍射光学系统设计多局限于实验室原型，仿真与制造设备之间存在较大质量差距，阻碍了其实际应用。

Method: 提出制造感知的设计流程，结合超分辨率神经光刻模型预测制造几何形状，并开发分布式计算框架处理大规模FFT计算。

Result: 实现了32.16 mm × 21.44 mm的大规模衍射光学设计，仿真与制造原型在应用（如全息和PSF工程）中表现一致，单DOE成像系统获得高质量图像。

Conclusion: 研究解决了衍射光学系统实际应用的制造限制，为可微分光学设计提供了实用化路径。
...
### [290] [Capability-Based Scaling Laws for LLM Red-Teaming](https://arxiv.org/abs/2505.20162)
*Alexander Panfilov,Paul Kassianik,Maksym Andriushchenko,Jonas Geiping*

Main category: cs.AI

TL;DR: 研究探讨了大型语言模型能力提升后，红队测试的脆弱性问题，发现攻击成功率与攻击者-目标能力差距相关，并提出了预测攻击成功的缩放定律。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型能力和自主性增强，传统红队测试方法可能失效，需研究能力差距对攻击效果的影响。

Method: 通过评估500多个攻击者-目标对，使用基于LLM的越狱攻击模拟人类红队行为。

Result: 发现三个趋势：能力更强的模型攻击效果更好；目标能力超过攻击者时攻击成功率骤降；攻击成功率与MMLU-Pro基准表现相关。

Conclusion: 固定能力攻击者（如人类）可能对未来的模型无效，开源模型能力提升会增加风险，需准确测量和控制模型的操纵能力。
...
### [291] [R1-Code-Interpreter: Training LLMs to Reason with Code via Supervised and Reinforcement Learning](https://arxiv.org/abs/2505.21668)
*Yongchao Chen,Yueying Liu,Junwei Zhou,Yilun Hao,Jingquan Wang,Yang Zhang,Chuchu Fan*

Main category: cs.AI

TL;DR: R1-Code-Interpreter通过多轮监督微调和强化学习，使LLM能自主生成代码查询，提升精确计算和符号操作任务的性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在精确计算、符号操作和算法推理任务中的不足，研究如何有效结合文本推理与代码生成。

Method: 使用多轮监督微调（SFT）和强化学习（RL）训练Qwen-2.5模型，探索不同策略如GRPO vs. PPO。

Result: R1-CI-14B模型在37个测试任务中平均准确率从44.0%提升至64.1%，接近GPT-4o（70.9%）。

Conclusion: SFT阶段对代码解释器训练至关重要，R1-Code-Interpreter在多样化任务中表现优异。
...
### [292] [Towards Safety Reasoning in LLMs: AI-agentic Deliberation for Policy-embedded CoT Data Creation](https://arxiv.org/abs/2505.21784)
...
TL;DR: AIDSAFE通过多智能体审议迭代生成高质量的安全推理链式思维数据，提升LLM的安全性和抗越狱能力。


<details>
  <summary>Details</summary>
Motivation: 现有安全措施存在过度拒绝和越狱漏洞问题，需改进安全推理方法。

Method: 提出AIDSAFE方法，利用多智能体审议迭代扩展安全政策推理，并通过数据精炼阶段确保质量。

Result: AIDSAFE生成的推理链显著提升政策遵循和推理质量，增强LLM的安全泛化能力。

Conclusion: AIDSAFE为安全训练提供高质量数据，显著改善LLM的安全性和实用性。
...
### [293] [Modeling and Optimizing User Preferences in AI Copilots: A Comprehensive Survey and Taxonomy](https://arxiv.org/abs/2505.21907)
*Saleh Afzoon,Zahra Jahanandish,Phuong Thao Huynh,Amin Beheshti,Usman Naseem*

Main category: cs.AI

TL;DR: 本文综述了AI副驾驶中偏好优化的研究，提出了一个分阶段的分类法，并分析了偏好捕获、建模和反馈的技术。


<details>
  <summary>Details</summary>
Motivation: 随着AI副驾驶能力的提升和普及，个性化成为提升可用性、信任和生产力的关键，但相关研究在实时交互系统中仍不完善。

Method: 通过综合研究，提出AI副驾驶的统一定义和偏好优化的分阶段分类法（交互前、中、后），分析偏好信号获取、用户意图建模和反馈集成技术。

Result: 总结了现有技术和创新方法，为设计自适应、偏好感知的AI副驾驶提供了结构化基础。

Conclusion: 本文为AI副驾驶的偏好优化提供了全面视角，明确了各设计阶段的技术选择，推动了该领域的研究和实践。
...
### [294] [Rethinking the Unsolvable: When In-Context Search Meets Test-Time Scaling](https://arxiv.org/abs/2505.22290)
*Fanzeng Xia,Yidong Luo,Tinko Sebastian Bartels,Yaqi Xu,Tongxin Li*

Main category: cs.AI

TL;DR: 通过结合上下文搜索提示和内部扩展，LLMs在复杂推理任务中取得突破性进展，挑战了现有评估范式对其能力的低估。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多依赖简单上下文学习评估LLMs，忽略了先进技术对其推理能力的激发，导致对LLMs性能上限的错误结论。

Method: 采用上下文搜索提示和内部扩展技术，系统探索LLMs在超难推理任务中的潜力。

Result: 在NP难任务和复杂规划基准测试中，成功率提升高达30倍，理论上扩展了可解问题的复杂度类别。

Conclusion: 研究呼吁重新评估LLMs的推理能力，并提出更全面的评估策略，以揭示其真实潜力。
...
### [295] [Efficiently Enhancing General Agents With Hierarchical-categorical Memory](https://arxiv.org/abs/2505.22006)
*Changze Qiao,Mingming Lu*

Main category: cs.AI

TL;DR: 本文提出了一种无需参数更新的通用智能体EHC，通过分层记忆检索和任务导向经验学习模块，显著提升了多模态任务的处理能力。
...
Method: EHC包含分层记忆检索（HMR）和任务导向经验学习（TOEL）模块，分别实现快速记忆检索和任务分类学习。

Result: 在多个标准数据集上的实验表明，EHC优于现有方法，达到最先进性能。

Conclusion: EHC作为一种通用智能体，能高效处理复杂多模态任务，且无需参数更新。
...
### [296] [Test-Time Immunization: A Universal Defense Framework Against Jailbreaks for (Multimodal) Large Language Models](https://arxiv.org/abs/2505.22271)
*Yongcan Yu,Yanbo Wang,Ran He,Jian Liang*

Main category: cs.CR

TL;DR: 论文提出了一种通用防御框架TIM，通过自适应检测和微调来抵御多种越狱攻击，实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法针对特定越狱攻击，效果有限，需要一种通用且自适应的防御方案。

Method: TIM框架通过训练检测令牌识别越狱行为，并在检测到攻击时进行安全微调，同时解耦检测模块以避免性能下降。

Result: 实验表明TIM在LLM和多模态LLM上均能有效防御多种越狱攻击。

Conclusion: TIM是一种通用且自适应的防御框架，能有效应对多样化的越狱攻击策略。
...
### [297] [VideoMarkBench: Benchmarking Robustness of Video Watermarking](https://arxiv.org/abs/2505.21620)
*Zhengyuan Jiang,Moyang Guo,Kecen Li,Yuepeng Hu,Yupu Wang,Zhicong Huang,Cheng Hong,Neil Zhenqiang Gong*

Main category: cs.CR

TL;DR: 论文提出了VideoMarkBench，首个系统性评估视频水印在去除和伪造攻击下鲁棒性的基准测试，揭示了当前水印技术的显著漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成模型的快速发展，高度逼真的合成视频引发了与虚假信息和版权侵权相关的伦理问题，视频水印被提出作为解决方案，但其鲁棒性尚未充分研究。

Method: 研究构建了VideoMarkBench基准测试，使用三种先进视频生成模型生成统一数据集，涵盖三种视频风格、四种水印方法和七种检测聚合策略，评估了12种扰动类型。

Result: 研究发现当前水印方法在多种攻击下存在显著漏洞，鲁棒性不足。

Conclusion: 研究强调了开发更鲁棒水印解决方案的紧迫性，并提供了开源代码以促进后续研究。
...
### [298] [Learning Compositional Behaviors from Demonstration and Language](https://arxiv.org/abs/2505.21981)
*Weiyu Liu,Neil Nie,Ruohan Zhang,Jiayuan Mao,Jiajun Wu*

Main category: cs.RO

TL;DR: BLADE框架结合模仿学习和基于模型的规划，通过语言标注演示和大型语言模型提取抽象动作知识，构建结构化高层动作表示库，实现长程机器人操作。


<details>
  <summary>Details</summary>
Motivation: 解决长程机器人操作中动作规划和泛化能力的挑战，利用语言模型和视觉感知提升动作表示的抽象性和实用性。

Method: 整合语言标注演示和LLMs提取动作知识，构建包含前提条件和效果的高层动作表示库，并实现为神经网络策略。

Result: BLADE能自动恢复结构化表示，显著提升对新颖初始状态、外部扰动和新目标的泛化能力。

Conclusion: BLADE在仿真和真实机器人实验中验证了其有效性，适用于复杂场景如部分可观测性和几何约束。
...
### [299] [CogAD: Cognitive-Hierarchy Guided End-to-End Autonomous Driving](https://arxiv.org/abs/2505.21581)
...
<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶方法与人类认知原则在感知和规划上存在根本性不一致，CogAD旨在通过模拟人类层次认知机制解决这一问题。

Method: CogAD采用双重层次机制：全局到局部上下文处理用于感知，意图驱动的多模式轨迹生成用于规划，并通过双层次不确定性建模实现多样化的轨迹生成。

Result: 在nuScenes和Bench2Drive数据集上的实验表明，CogAD在端到端规划中达到最先进性能，尤其在长尾场景和复杂现实驾驶条件下表现优异。

Conclusion: CogAD通过模拟人类认知机制，显著提升了自动驾驶系统的感知和规划能力，尤其在复杂场景中表现出强大的鲁棒性和泛化能力。
...
### [300] [Vision-Language-Action Model with Open-World Embodied Reasoning from Pretrained Knowledge](https://arxiv.org/abs/2505.21906)
*Zhongyi Zhou,Yichen Zhu,Junjie Wen,Chaomin Shen,Yi Xu*

Main category: cs.RO

TL;DR: ChatVLA-2是一种新型的视觉-语言-动作模型，通过混合专家模型和三阶段训练流程，保留了预训练视觉语言模型的核心能力，并实现了可操作的推理。


<details>
  <summary>Details</summary>
Motivation: 现有端到端VLA系统在微调时会丢失预训练视觉语言模型的关键能力，因此需要一种既能继承VLM知识又能实现可操作推理的通用模型。

Method: 采用混合专家模型架构和三阶段训练流程，确保模型在适应机器人任务时保留VLM的开放世界推理能力。

Result: 在数学匹配任务中表现出卓越的数学推理和OCR能力，同时具备强大的空间推理能力，性能超越现有模仿学习方法。
...
### [301] [ForceVLA: Enhancing VLA Models with a Force-aware MoE for Contact-rich Manipulation](https://arxiv.org/abs/2505.22159)
...
TL;DR: ForceVLA提出了一种新型的端到端机器人操作框架，通过将外部力感知作为VLA系统的首要模态，显著提升了接触密集型任务的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在需要精细控制的接触密集型任务中表现不佳，尤其是在视觉遮挡或动态不确定的情况下。

Method: ForceVLA引入了FVLMoE模块，动态整合预训练的视觉-语言嵌入与实时6轴力反馈，并通过ForceVLA-Data数据集支持多模态训练。

Result: ForceVLA在接触密集型任务中平均成功率提高了23.2%，在插头插入等任务中达到80%的成功率。

Conclusion: ForceVLA展示了多模态整合在灵巧操作中的重要性，并为物理智能机器人控制设立了新基准。
...
### [302] [LiDAR Based Semantic Perception for Forklifts in Outdoor Environments](https://arxiv.org/abs/2505.22258)
*Benjamin Serfling,Hannes Reichert,Lorenzo Bayerlein,Konrad Doll,Kati Radkhah-Lens*

Main category: cs.RO

TL;DR: 提出了一种基于双LiDAR系统的语义分割框架，专为复杂户外环境中的自动叉车设计，实现了高精度障碍物检测和场景理解。


<details>
  <summary>Details</summary>
Motivation: 为工业物料搬运任务中的自动叉车提供全面的场景理解，特别是在动态和静态障碍物检测方面。

Method: 结合前向和向下倾斜的LiDAR传感器，利用高分辨率3D点云数据，采用轻量但鲁棒的算法进行语义分割。
...
### [303] [UP-SLAM: Adaptively Structured Gaussian SLAM with Uncertainty Prediction in Dynamic Environments](https://arxiv.org/abs/2505.22335)
*Wancai Zheng,Linlin Ou,Jiajie He,Libo Zhou,Xinyi Yu,Yan Wei*

Main category: cs.RO

TL;DR: UP-SLAM是一种实时RGB-D SLAM系统，通过并行化框架解耦跟踪与建图，适应动态环境，提升定位精度和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS技术在动态环境中实时性和鲁棒性不足，UP-SLAM旨在解决这些问题。

Method: 采用概率八叉树管理高斯基元，提出无训练不确定性估计器和时间编码器，结合DINO特征增强鲁棒性。

Result: 在定位精度（提升59.8%）和渲染质量（提升4.57 dB PSNR）上优于现有方法，保持实时性能。

Conclusion: UP-SLAM在动态环境中表现出色，提供可重用且无伪影的静态地图。
...
### [304] [High-Fidelity Functional Ultrasound Reconstruction via A Visual Auto-Regressive Framework](https://arxiv.org/abs/2505.21530)
*Xuhang Chen,Zhuo Li,Yanyan Shen,Mufti Mahmud,Hieu Pham,Chi-Man Pun,Shuqiang Wang*

Main category: eess.IV

TL;DR: 功能性超声（fUS）成像在神经血管映射中具有出色的时空分辨率，但数据稀缺和信号衰减问题限制了其实际应用。


<details>
  <summary>Details</summary>
Motivation: 解决fUS成像中因数据稀缺和信号衰减导致的模型公平性和多样性不足问题。

Method: 未明确提及具体方法，但暗示需应对数据稀缺和信号衰减的挑战。

Result: 数据稀缺和信号衰减限制了fUS成像的实际应用和机器学习模型的公平性。

Conclusion: 需克服数据稀缺和信号衰减问题以提升fUS成像的实用性和模型公平性。
...
### [305] [Image denoising as a conditional expectation](https://arxiv.org/abs/2505.21546)
*Sajal Chakroborty,Suddhasattwa Das*

Main category: eess.IV

TL;DR: 论文提出了一种基于概率空间解释的数据驱动去噪方法，通过条件期望恢复真实图像，并在RKHS中求解线性方程。
...
Method: 将噪声图像视为概率空间的样本，通过核积分算子估计真实图像的条件期望，并在RKHS中求解最小二乘问题。

Result: 方法在像素数趋于无穷时收敛，且能优化有限像素图像的去噪参数。

Conclusion: 该技术提供了一种收敛且数据驱动的去噪方案，优于传统投影方法。
...
### [306] [Taylor expansion-based Kolmogorov-Arnold network for blind image quality assessment](https://arxiv.org/abs/2505.21592)
*Ze Chen,Shaode Yu*

Main category: eess.IV

TL;DR: 论文提出TaylorKAN，利用泰勒展开作为可学习激活函数，提升局部逼近能力，并通过网络深度减少和特征维度压缩提高计算效率。实验表明其在多个数据库上优于其他KAN相关模型。


<details>
  <summary>Details</summary>
Motivation: 解决KAN及其变体在处理高维特征时性能提升有限且计算成本高的问题。

Method: 提出TaylorKAN，结合泰勒展开作为激活函数，并集成网络深度减少和特征维度压缩。

Result: 在多个数据库上表现优于其他KAN相关模型，验证了泰勒展开局部逼近的有效性。
...
### [307] [Optimizing Deep Learning for Skin Cancer Classification: A Computationally Efficient CNN with Minimal Accuracy Trade-Off](https://arxiv.org/abs/2505.21597)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于迁移学习的深度模型（如ResNet50）计算开销大，难以在资源受限环境中部署，需要一种更高效的解决方案。

Method: 设计了一种自定义CNN模型，大幅减少参数和FLOPs，同时保持分类精度。

Result: 模型参数减少96.7%，FLOPs显著降低，分类精度偏差小于0.022%，适用于移动和边缘设备。

Conclusion: 轻量级CNN在资源受限环境中更具实用性，平衡了模型复杂度和实际可行性。
...
### [308] [Laparoscopic Image Desmoking Using the U-Net with New Loss Function and Integrated Differentiable Wiener Filter](https://arxiv.org/abs/2505.21634)
...
<details>
  <summary>Details</summary>
Motivation: 腹腔镜手术中烟雾导致视觉清晰度下降，影响手术和计算机辅助技术，需有效去除烟雾。

Method: 采用U-Net结构，结合新损失函数（结构相似性、感知损失和均方误差）和可微分Wiener滤波器。

Result: 在公开数据集上验证，ULW方法在视觉清晰度和量化评估中表现优异。

Conclusion: ULW方法为腹腔镜图像实时增强提供了有效解决方案。
...
### [309] [STA-Risk: A Deep Dive of Spatio-Temporal Asymmetries for Breast Cancer Risk Prediction](https://arxiv.org/abs/2505.21699)
*Zhengbo Zhou,Dooman Arefan,Margarita Zuley,Jules Sumkin,Shandong Wu*

Main category: eess.IV

TL;DR: STA-Risk是一种基于Transformer的模型，通过捕捉乳腺影像的空间和时间不对称性，提升乳腺癌风险预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有乳腺癌风险预测模型性能有限，且忽视了纵向影像中的空间和时间细节。

Method: 提出STA-Risk模型，结合侧编码和时间编码学习空间-时间不对称性，并采用定制化不对称损失函数。

Result: 在两个独立乳腺影像数据集上，STA-Risk在1-5年风险预测中优于四种SOTA模型。
...
### [310] [Privacy-Preserving Chest X-ray Report Generation via Multimodal Federated Learning with ViT and GPT-2](https://arxiv.org/abs/2505.21715)
*Md. Zahid Hossain,Mustofa Ahmed,Most. Sharmin Sultana Samu,Md. Rakibul Islam*

Main category: eess.IV

TL;DR: 该研究提出了一种多模态联邦学习框架，用于从胸部X光图像生成放射学报告，避免了敏感数据传输，保护了患者隐私。


<details>
  <summary>Details</summary>
Motivation: 传统集中式方法需要传输敏感数据，存在隐私问题，因此研究旨在通过联邦学习实现隐私保护的报告生成。

Method: 使用Vision Transformer (ViT)作为编码器，GPT-2作为报告生成器，评估了三种联邦学习聚合策略：FedAvg、Krum Aggregation和新型的L-FedAvg。

Result: Krum Aggregation在多项评估指标（如ROUGE、BLEU、BERTScore和RaTEScore）中表现最佳，联邦学习模型在生成临床相关且语义丰富的报告方面优于或匹配集中式模型。

Conclusion: 该轻量级且隐私保护的框架为医疗AI的协作开发提供了新途径，同时确保数据保密性。
...
### [311] [MAMBO-NET: Multi-Causal Aware Modeling Backdoor-Intervention Optimization for Medical Image Segmentation Network](https://arxiv.org/abs/2505.21874)
*Ruiguo Yu,Yiyang Zhang,Yuan Tian,Yujie Diao,Di Jin,Witold Pedrycz*

Main category: eess.IV

TL;DR: 论文提出了一种多因果感知建模后门干预优化网络（MAMBO-NET），用于解决医学图像分割中混淆因素的影响，通过因果干预和后验概率约束提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法假设图像到分割过程无偏，忽略了复杂解剖变异和成像模态限制等混淆因素，导致分割结果不理想。

Method: MAMBO-NET利用多高斯分布自建模拟合混淆因素，引入因果干预和后验概率约束，并结合后门干预技术优化分割过程。

Result: 在五个医学图像数据集上的实验表明，该方法显著降低了混淆因素的影响，提高了分割准确性。

Conclusion: MAMBO-NET通过因果干预和后验概率约束有效解决了医学图像分割中的混淆问题，提升了分割性能。
...
### [312] [Subspecialty-Specific Foundation Model for Intelligent Gastrointestinal Pathology](https://arxiv.org/abs/2505.21928)
...
TL;DR: Digepath是一种针对胃肠道病理学的专用基础模型，通过双阶段迭代优化策略，显著提升了诊断准确性和效率，尤其在疑难病例中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统组织病理学诊断依赖病理学家的主观判断，存在可重复性低和诊断变异性的问题，亟需精准的AI解决方案。

Method: 采用双阶段迭代优化策略（预训练与精细筛选），基于超过35.3亿个图像块和20万张H&E染色切片进行训练。

Result: 在34项任务中33项达到最优性能，早期癌症筛查灵敏度达99.6%。

Conclusion: Digepath填补了组织病理学实践的关键空白，为其他病理学亚专科提供了可转移的范式。
...
### [313] [Cascaded 3D Diffusion Models for Whole-body 3D 18-F FDG PET/CT synthesis from Demographics](https://arxiv.org/abs/2505.22489)
...
### [314] [Risk-Sensitive Conformal Prediction for Catheter Placement Detection in Chest X-rays](https://arxiv.org/abs/2505.22496)
*Long Hui*

Main category: eess.IV

TL;DR: 提出了一种结合多任务学习和风险敏感共形预测的新方法，用于胸部X射线中的导管和管线位置检测，显著提高了临床可靠性。
...
Result: 实验显示整体覆盖率为90.68%，关键条件下覆盖率达99.29%，且实现了零高风险误判。
...
### [315] [Surf2CT: Cascaded 3D Flow Matching Models for Torso 3D CT Synthesis from Skin Surface](https://arxiv.org/abs/2505.22511)
*Siyeop Yoon,Yujin Oh,Pengfei Jin,Sifan Song,Matthew Tivnan,Dufan Wu,Xiang Li,Quanzheng Li*

Main category: eess.IV

TL;DR: Surf2CT是一种新型级联流匹配框架，可从外部表面扫描和简单人口统计数据生成完整的人体躯干3D CT图像，无需内部成像。


<details>
  <summary>Details</summary>
Motivation: 传统CT成像具有侵入性和风险，Surf2CT旨在通过外部数据非侵入性地生成内部解剖图像，推动家庭医疗和预防医学的发展。

Method: Surf2CT分为三个阶段：表面补全（生成完整SDF）、粗CT合成（生成低分辨率CT）、CT超分辨率（生成高分辨率CT），均基于3D流匹配模型。

Result: 在700例测试中，器官体积误差小（-11.1%至4.4%），肌肉/脂肪组成相关性高（0.67-0.96），肺定位偏差小（-2.5 mm）。

Conclusion: Surf2CT为非侵入性内部成像提供了新范式，适用于家庭医疗和个性化临床评估。
...
### [316] [Multipath cycleGAN for harmonization of paired and unpaired low-dose lung computed tomography reconstruction kernels](https://arxiv.org/abs/2505.22568)
...
TL;DR: 提出一种多路径cycleGAN模型，用于CT核谐波化，以减少定量成像测量中的系统性变异性，特别是在肺气肿量化中。


<details>
  <summary>Details</summary>
Motivation: CT重建核影响空间分辨率和噪声特性，导致定量成像测量（如肺气肿量化）的系统性变异性。选择合适的核对于一致的定量分析至关重要。

Method: 使用多路径cycleGAN模型，结合配对和非配对数据训练，模型包含特定领域的编码器和解码器，共享潜在空间，并为每个领域定制判别器。

Result: 模型在配对核上减少了肺气肿评分的偏差（p<0.05），在非配对核上消除了肺气肿的混杂差异（p>0.05）。高Dice分数证实了肌肉和脂肪解剖结构的保留。

Conclusion: 共享潜在空间的多路径cycleGAN能够实现配对和非配对CT核的稳健谐波化，改善肺气肿量化并保持解剖保真度。
...
### [317] [Comparative Analysis of Machine Learning Models for Lung Cancer Mutation Detection and Staging Using 3D CT Scans](https://arxiv.org/abs/2505.22592)
*Yiheng Li,Francisco Carrillo-Perez,Mohammed Alawad,Olivier Gevaert*

Main category: eess.IV

TL;DR: 论文比较了两种机器学习模型（FMCIB+XGBoost和Dinov2+ABMIL）在肺癌突变检测和分期中的性能，发现监督模型在突变检测中表现更优，而自监督模型在分期任务中具有更好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 肺癌是全球癌症死亡的主要原因，非侵入性方法对检测关键突变和分期至关重要，以改善患者预后。

Method: 使用FMCIB+XGBoost（监督模型）和Dinov2+ABMIL（自监督模型）对3D肺结节数据进行分析，比较其在KRAS和EGFR突变检测及癌症分期中的表现。

Result: FMCIB+XGBoost在突变检测中表现更优（KRAS准确率0.846，EGFR准确率0.883），而Dinov2+ABMIL在分期任务中泛化能力更强（T分期准确率0.797）。

Conclusion: 监督模型在突变检测中更具临床实用性，自监督模型在分期任务中表现潜力，但突变敏感性仍需改进。
...
### [318] [Chest Disease Detection In X-Ray Images Using Deep Learning Classification Method](https://arxiv.org/abs/2505.22609)
*Alanna Hazlett,Naomi Ohashi,Timothy Rodriguez,Sodiq Adewole*

Main category: eess.IV

TL;DR: 研究使用预训练CNN模型对胸部X光片分类，涵盖COVID-19、肺炎、结核和正常病例，表现优异且具可解释性。


<details>
  <summary>Details</summary>
Motivation: 探索多分类模型在胸部X光片分类中的性能，提升临床应用的透明度和信任度。

Method: 利用迁移学习和预训练CNN模型，结合Grad-CAM提供可视化解释。

Result: 模型在准确率、精确率、召回率和F1分数上表现优异。

Conclusion: 该方法在医学影像分类中具有潜力，且通过可视化提升了模型的可信度。
...
### [319] [From prosthetic memory to prosthetic denial: Auditing whether large language models are prone to mass atrocity denialism](https://arxiv.org/abs/2505.21753)
*Roberto Ulloa,Eve M. Zucker,Daniel Bultmann,David J. Simon,Mykola Makhortykh*

Main category: cs.CY

TL;DR: 研究探讨大型语言模型（LLMs）如何影响历史叙述的传播与感知，特别是对大规模暴行记忆的表现，揭示其可能引发记忆失真或否认的风险。


<details>
  <summary>Details</summary>
Motivation: LLMs的普及可能改变历史记忆的传播方式，研究旨在评估其对暴行记忆的潜在影响，包括记忆的保存与否认。

Method: 通过比较审计五种LLM（Claude、GPT、Llama、Mixtral、Gemini）在四个历史案例（Holodomor、Holocaust、Cambodian Genocide、Rwanda Genocide）中的表现，分析其对否认主义观点的反应。

Result: LLMs对广泛记录的事件（如Holocaust）反应准确，但对较少记录的事件（如Cambodian Genocide）易受否认主义框架影响。

Conclusion: LLMs虽扩展了“假体记忆”概念，但未受监管的使用可能强化历史否认主义，对数字记忆保存提出伦理挑战。
...
### [320] [Detecting Cultural Differences in News Video Thumbnails via Computational Aesthetics](https://arxiv.org/abs/2505.21912)
*Marvin Limpijankit,John Kender*

Main category: cs.CY

TL;DR: 论文提出了一种两步法，用于检测不同文化背景下图像风格的差异，通过聚类和美学特征比较分析YouTube缩略图。


<details>
  <summary>Details</summary>
Motivation: 研究不同文化背景下图像风格的差异，探索视觉传播中的文化偏好。

Method: 首先将图像按内容聚类为视觉主题，然后比较美学特征。测试数据为2400个来自中美YouTube频道的缩略图。

Result: 中国缩略图更随意自然，美国缩略图更正式、色彩更饱和、细节更丰富且构图更紧凑。

Conclusion: 这些差异反映了文化偏好，方法可作为检测视觉宣传的基准。
...
### [321] [Incorporating LLMs for Large-Scale Urban Complex Mobility Simulation](https://arxiv.org/abs/2505.21880)
*Yu-Lun Song,Chung-En Tsern,Che-Cheng Wu,Yu-Ming Chang,Syuan-Bo Huang,Wei-Chu Chen,Michael Chia-Liang Lin,Yu-Ta Lin*

Main category: cs.MA

TL;DR: 该研究提出了一种结合大型语言模型（LLM）和基于代理的建模（ABM）的创新方法，用于城市交通模拟，以提高代理多样性和真实性。


<details>
  <summary>Details</summary>
Motivation: 传统基于规则的ABM方法在模拟城市交通时缺乏多样性和真实性，因此研究希望通过LLM增强这些方面。

Method: 利用LLM生成合成人口档案、分配常规和偶尔地点，并模拟个性化路线。基于台北市的真实数据，模拟个体行为和大规模移动模式。

Result: 模拟结果提供了路线热图和模式特定指标等关键洞察，为城市规划者提供决策支持。

Conclusion: 未来工作将集中在建立稳健的验证框架，以确保在城市规划应用中的准确性和可靠性。
...
### [322] [UI-Evol: Automatic Knowledge Evolving for Computer Use Agents](https://arxiv.org/abs/2505.21964)
*Ziyun Zhang,Xinyi Liu,Xiaoyi Zhang,Jun Wang,Gang Chen,Yan Lu*

Main category: cs.HC

TL;DR: 论文提出UI-Evol模块，通过两阶段（Retrace和Critique）解决知识执行差距，显著提升任务性能和代理可靠性。


<details>
  <summary>Details</summary>
Motivation: 研究发现即使90%的知识正确，执行成功率仅41%，需解决知识执行差距。

Method: UI-Evol模块包含Retrace阶段提取动作序列和Critique阶段优化知识。

Result: 实验显示UI-Evol显著提升任务性能并降低行为标准差。

Conclusion: UI-Evol有效解决知识执行差距，提升计算机使用代理的可靠性和性能。
...
### [323] [MapStory: LLM-Powered Text-Driven Map Animation Prototyping with Human-in-the-Loop Editing](https://arxiv.org/abs/2505.21966)
*Aditya Gunturu,Ben Pearman,Keiichi Ihara,Morteza Faraji,Bryan Wang,Rubaiat Habib Kazi,Ryo Suzuki*

Main category: cs.HC

TL;DR: MapStory是一个基于LLM的工具，通过自然语言文本生成可编辑的地图动画序列，简化了动画创作流程。


<details>
  <summary>Details</summary>
Motivation: 旨在降低地图动画创作的门槛，提升创作效率和创意探索。

Method: 利用代理架构分解脚本为动画构建块，结合LLM和网络搜索查询地理信息，提供交互式时间线编辑器。

Result: 用户能轻松创建地图动画，加快迭代速度，促进创意探索。

Conclusion: MapStory有效降低了地图故事创作的复杂性，提升了用户体验。
...
### [324] [Visual Cues Support Robust Turn-taking Prediction in Noise](https://arxiv.org/abs/2505.22088)
*Sam O'Connor Russell,Naomi Harte*

Main category: cs.SD

TL;DR: 研究探讨了预测性轮流模型（PTTMs）在噪声环境中的表现，发现其对噪声高度敏感，但通过多模态训练可提升性能。


<details>
  <summary>Details</summary>
Motivation: PTTMs在自然化人机交互中至关重要，但其在噪声中的表现尚不明确。

Method: 研究分析了PTTMs在不同噪声类型中的表现，并测试了多模态（包含视觉特征）训练的效果。

Result: 多模态PTTM在10 dB音乐噪声中准确率达72%，优于仅音频的模型，但对新噪声类型的泛化能力有限。

Conclusion: 多模态PTTM能有效利用视觉线索提升性能，但训练依赖准确转录，且泛化能力有待改进。
...
### [325] [Advancing Hearing Assessment: An ASR-Based Frequency-Specific Speech Test for Diagnosing Presbycusis](https://arxiv.org/abs/2505.22231)
*Stefan Bleeck*

Main category: cs.SD

TL;DR: 本文提出了一种基于自动语音识别（ASR）的频率特异性语音测试方法，用于更精确地诊断听力损失对语音理解的影响。


<details>
  <summary>Details</summary>
Motivation: 传统听力测试难以全面评估听力损失对语音理解的功能性影响，尤其是在超阈值缺陷和频率特异性感知方面。

Method: 利用ASR模拟中度倾斜听力损失的感知效果，通过控制语音刺激的声学退化并分析音素级混淆模式。

Result: 模拟听力损失导致特定音素混淆（如高频辅音替换和音素删除），测试电池能有效区分模拟正常听力和听力受损者。

Conclusion: ASR方法为开发客观、精细且频率特异性的听力评估工具提供了新途径，未来将验证人类参与者并探索AI模型的集成。
...
### [326] [Effective Context in Neural Speech Models](https://arxiv.org/abs/2505.22487)
*Yen Meng,Sharon Goldwater,Hao Tang*

Main category: cs.SD

TL;DR: 论文研究了神经语音模型的实际有效上下文长度，提出了两种测量方法，并分析了不同任务和模型层级的上下文需求。


<details>
  <summary>Details</summary>
Motivation: 虽然许多方法试图增加模型的上下文长度，但很少有研究测量模型实际使用的上下文（有效上下文）。

Method: 提出了两种测量有效上下文的方法，并应用于分析监督学习和自监督学习的语音Transformer模型。

Result: 监督模型的有效上下文与任务性质相关；自监督模型的有效上下文主要在早期层增加，且整体较短。

Conclusion: 由于模型预测时无需长上下文，HuBERT可在不修改架构或微调的情况下以流模式运行。
...
### [327] [RESOUND: Speech Reconstruction from Silent Videos via Acoustic-Semantic Decomposed Modeling](https://arxiv.org/abs/2505.22024)
...
### [328] [Mitigating Audiovisual Mismatch in Visual-Guide Audio Captioning](https://arxiv.org/abs/2505.22045)
*Le Xu,Chenxing Li,Yong Ren,Yujie Chen,Yu Gu,Ruibo Fu,Shan Yang,Dong Yu*

Main category: cs.MM

TL;DR: 提出了一种基于熵感知的门控融合框架，通过跨模态不确定性量化动态调节视觉信息流，解决视听错位问题。
...
Conclusion: 提出的框架有效提升了模型对错位模态的鲁棒性和效率。
...
### [329] [RenderFormer: Transformer-based Neural Rendering of Triangle Meshes with Global Illumination](https://arxiv.org/abs/2505.21925)
...
<details>
  <summary>Details</summary>
Motivation: 传统渲染方法依赖物理模拟，计算复杂且耗时。RenderFormer旨在通过序列到序列的转换简化渲染过程。

Method: 采用两阶段流程：视图无关阶段建模三角形间光传输，视图相关阶段将光线束转换为像素值。基于Transformer架构。

Result: 在多种复杂形状和光传输场景中验证了RenderFormer的有效性。

Conclusion: RenderFormer提供了一种高效且无需逐场景训练的渲染方法，展示了神经渲染的潜力。
...
### [330] [STDR: Spatio-Temporal Decoupling for Real-Time Dynamic Scene Rendering](https://arxiv.org/abs/2505.22400)
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在动态重建中因时空纠缠导致初始化不连贯，影响运动建模准确性。

Method: 引入时空掩码、分离变形场和一致性正则化，联合解耦时空模式。

Result: 实验表明STDR显著提升重建质量和时空一致性。

Conclusion: STDR为动态场景重建提供高效解耦方案。
...
### [331] [Neural Face Skinning for Mesh-agnostic Facial Expression Cloning](https://arxiv.org/abs/2505.22416)
*Sihun Cha,Serin Yoon,Kwanggyoon Seo,Junyong Noh*

Main category: cs.GR

TL;DR: 提出了一种结合全局和局部变形模型的方法，用于面部动画重定向，实现了精细的表情克隆和直观控制。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在面部动画重定向中难以同时捕捉全局表情和局部细节，且局部变形方法增加了整体控制的复杂性。

Method: 通过间接监督学习目标面部网格的蒙皮权重，将全局潜在代码的影响局部化，结合FACS混合形状确保可解释性。

Result: 实验表明，该方法在表情保真度、变形传递准确性和对不同网格结构的适应性上优于现有方法。

Conclusion: 该方法成功结合了全局和局部模型的优势，实现了高精度和可控的面部动画重定向。
...
### [332] [tenSVD algorithm for compression](https://arxiv.org/abs/2505.21686)
*Michele Gallo*

Main category: stat.CO

TL;DR: 该研究提出了一种基于张量的高效图像存储方法，通过Tucker模型压缩数据，以减少存储、传输和处理的资源消耗。


<details>
  <summary>Details</summary>
Motivation: 高维数据管理需要高效方法，张量分析在多个领域广泛应用，本研究旨在优化资源利用。

Method: 将原始数据组织为高阶张量，应用Tucker模型压缩，并在R中实现，与基线算法对比。

Result: 评估了计算时间和信息保留质量，使用模拟和真实数据集，重点关注能耗可持续性。

Conclusion: 该方法在资源效率和信息保留方面表现优异，具有实际应用潜力。
...
### [333] [Scientific Paper Retrieval with LLM-Guided Semantic-Based Ranking](https://arxiv.org/abs/2505.21815)
...
Method: 提出SemRank框架，利用多粒度科学概念索引论文，并通过LLM识别查询中的核心概念以实现精确语义匹配。

Result: 实验表明SemRank显著提升检索性能，优于现有LLM基线且保持高效。

Conclusion: SemRank通过结合LLM和概念索引，有效解决了科学论文检索中的细粒度概念捕捉问题。
