...
Motivation: 社交媒体的快速发展加剧了有害内容的传播，而现有数据集的不足阻碍了有害迷因检测的进一步研究。

Method: 提出MemeMind数据集（大规模、多样化、双语支持、详细标注）和MemeGuard框架（整合多模态信息和推理建模）。

Result: 实验表明，MemeGuard在有害迷因检测任务中优于现有方法。

Conclusion: MemeMind和MemeGuard为有害迷因检测提供了更可靠的数据和方法支持。
...
<details>
  <summary>Details</summary>
Motivation: 揭示LLMs中记忆与自我认知缺陷之间的交织关系，以提升模型的解释性和可信度。

Method: 采用新颖框架评估LLMs是否真正学习推理模式，还是仅记忆训练数据。

Result: LLMs因记忆解决方案而高估自身推理能力，导致可行性评估不一致性超过45%。

Conclusion: 需改进架构和训练模式，确保模型对自身知识的一致性认知，以增强AI的可信度。
...
<details>
  <summary>Details</summary>
Motivation: 探讨语言模型对训练中未见过的非常规分词方式的鲁棒性，并探索其潜在优势。

Method: 评估20个基准测试中模型对随机采样和字符级分词的表现，分析模型强度与分词偏离程度的关系。

Result: 指令调优模型在非常规分词下性能损失小，字符级分词在特定任务中提升表现（如字符串操作+14%，大数运算+33%）。

Conclusion: 模型对分词器的依赖低于预期，非常规分词在推理时可能提升性能，鲁棒性源于指令调优阶段。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法忽视了长文本响应中的偏见和LLM输出的内在变异性，需要更精细的评估工具。

Method: FiSCo在声明级别操作，利用蕴含检查分解模型输出为语义不同的声明，并通过统计假设检验比较群体间和群体内的相似性。

Result: 实验表明，FiSCo能更可靠地识别细微偏见，减少LLM随机变异的影响，优于其他评估指标。

Conclusion: FiSCo为LLM的公平性评估提供了更精细和可靠的框架，适用于多种人口群体。
...
Result: 在数学和代码生成任务中，DUS优于现有并行方法，且显著提升推理速度。

Conclusion: DUS为高效高质量文本生成提供轻量级方案，释放MDLMs潜力。
...
<details>
  <summary>Details</summary>
Motivation: 职位匹配和职位与技能对齐在计算性就业市场中非常重要，能提升自动候选人匹配、职业路径预测和就业市场分析等任务。

Method: 比较了基于分类的微调、基于对比的微调和提示方法，并利用ESCO的额外数据。

Result: 任务A中提示方法表现最佳（MAP 0.492），任务B中基于分类的微调方法表现最佳（MAP 0.290）。多语言大模型在两项任务中均表现最佳。

Conclusion: 多语言大模型在职位匹配和技能预测任务中表现最优，团队在两项任务中分别排名第5和第3。
...
<details>
  <summary>Details</summary>
Motivation: 当前评估基准缺乏道德分类的注释和跨文化多样性，限制了透明度和道德推理的全面评估。
...
Method: 设计了第一和第二阶ToM任务及WM任务，控制模型对心理状态的追踪和建模能力。

Result: 实验表明，多数LLMs在WM任务上表现优于ToM任务，且对人类推理优于对无生命物体；还发现启发式行为（如近因偏差和过度依赖早期事件）。

Conclusion: StorySim为评估LLMs的ToM和WM能力提供了可控且新颖的方法，并揭示了模型的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以评估LLMs的自由形式毒性解释，因其过度依赖输入文本扰动等问题，需要一种更可靠的理论基础方法。

Method: 提出HAF准则，基于不确定性量化开发六个指标，评估LLMs解释与人类理性解释的对齐程度。

Result: 实验显示LLMs在简单提示下生成合理解释，但在复杂关系提示下表现不一致或无意义。

Conclusion: HAF方法有效评估LLMs解释的合理性，揭示了其在复杂推理中的局限性，代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 解决语音数据稀缺问题，同时利用文本数据提升模型性能。

Method: 联合TAED（J-TAED）通过同时训练语音和文本输入模态，统一内部表示，并支持基于文本的领域适应。

Result: 在Librispeech数据集上WER降低5.8~12.8%，在金融和命名实体数据集上分别降低15.3%和17.8%。

Conclusion: J-TAED成功整合语音和语言信息，有效缓解领域不匹配问题，无需额外语音数据。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何在大语言模型中实现低资源语言的跨语言上下文学习适应。

Method: 研究五种目标语言、三种基础大语言模型和七种下游任务，比较少样本提示、翻译测试、微调等多种适应技术。

Result: 少样本提示和翻译测试方法表现最佳，梯度适应方法因灾难性遗忘导致性能下降。
...
Method: 提出State Delta Encoding (SDE)方法，传输标记生成过程中的状态转移轨迹，以捕捉隐藏的推理信息。

Result: 实验表明，采用SDE的多智能体系统在复杂推理任务中达到SOTA性能。

Conclusion: SDE展示了通信增强对LLM多智能体系统的潜力，尤其在需要复杂推理的场景中。
...
<details>
  <summary>Details</summary>
Motivation: 传统人格评估依赖问卷，而NLP提供了通过开放文本评估的新途径。本研究旨在解决长文本（超过2000词）建模的挑战。

Method: 采用两步法：1）滑动窗口微调预训练语言模型提取上下文嵌入；2）使用带注意力机制的RNN整合长程依赖。

Result: 通过消融实验和与LLaMA、Longformer等模型的对比，证明了该方法在预测准确性、效率和可解释性上的提升。

Conclusion: 结合语言特征和长文本建模，为从生活叙事中评估人格特质提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM生成数据迭代训练导致的模型崩溃问题，并探索数据多样性对模型性能的影响。

Method: 通过实验分析不同多样性水平的LLM生成数据及混合比例对下游模型性能的影响。

Result: 适度多样性的LLM生成数据能提升模型性能，而过高多样性则产生负面影响。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有AI心理咨询系统对用户心理状态理解不足、依赖高质量数据及隐私问题。

Method: 结合视角推断（理解用户心理状态）和阶段识别（匹配咨询流程），利用开源LLM生成共情回应。
...
<details>
  <summary>Details</summary>
Motivation: 在第二语言学习中，有效的发音反馈至关重要。联合建模APA和MDD任务可以相互促进，但现有方法在解释性和细粒度时间推理方面仍有不足。

Method: 利用选择性状态空间模型（SSM）Mamba，结合语音特征和提示策略，构建统一框架，以增强APA和MDD的联合建模能力。

Result: 在speechocean762基准测试中，模型表现优于现有方法，尤其在MDD任务上效果显著。

Conclusion: 本研究首次将语音归因、SSM建模和提示策略结合于CAPT，为发音反馈提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: LLM在生成响应时经常出现与角色不符的行为（OOC），影响模型可靠性，现有评估方法难以检测细微偏差。

Method: 提出了基于三个关键指标的原子级评估框架，量化角色对齐和一致性。

Result: 实验表明，该框架能有效检测以往方法忽略的角色不一致性，并揭示任务结构和角色吸引力对模型适应性的影响。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在特征定位和操作上存在局限性，稀疏自编码器（SAEs）的特征隔离和单义性不足。

Method: 引入特征单义性评分（FMS）量化特征单义性，并提出基于标签概念的引导稀疏自编码器（G-SAE）。

Result: G-SAE在毒性检测、写作风格识别和隐私属性识别中表现更优，提升了单义性和控制能力。

Conclusion: 研究为大型语言模型的机制解释和控制提供了实用指南。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖LLM隐藏信息或大量人工干预，无法在黑盒设置中有效工作，亟需自动化解决方案。

Method: 使用序列到序列映射模型推断输入文本与LLM输出后缀的潜在映射特征，并通过关键令牌扰动增强区分性。引入原型分类器缓解过拟合。

Result: 在三个广泛使用的数据集上验证了VeilProbe在黑盒设置中的高效性和优越性。

Conclusion: VeilProbe为黑盒设置下的预训练文本检测提供了自动化且高效的解决方案。
...
Method: 提出了一个完整的流程，包括三个基于规则的推理任务、理论框架和端到端架构，通过语言VAE实现推理规则的显式嵌入。

Result: 实验表明：1) 推理规则可在编码器参数空间中解耦；2) 注入先验知识能提升模型性能；3) 数学推理任务中，样本数量增加对性能提升有限。

Conclusion: 通过语言VAE显式嵌入推理规则是可行的，且能提升模型的可解释性和性能，但需注意性能瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否能捕捉人类标注差异（如任务主观性和样本模糊性），填补现有评估方法的空白。

Method: 通过广泛评估LLMs预测标注差异的能力，无需重复人类标注数据。

Result: LLMs在建模标注差异方面表现不佳，且RLVR式推理会进一步降低性能。

Conclusion: 需重视LLMs在标注差异建模中的评估与改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估数据集在多语言能力和跨语言对齐方面存在不足，导致评估结果分散。

Method: 引入MuBench基准测试，评估多语言大模型，并提出MLC指标；同时预训练1.2B参数模型以研究跨语言迁移。

Result: 发现模型在英语和低资源语言间存在性能差距，MLC有助于分析性能瓶颈。

Conclusion: MuBench和MLC为多语言模型评估和改进提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机在于利用LLM的常识推理能力，为对话系统生成上下文相关的增强数据，并自动评估其质量。

Method: 方法包括：1）从5个知名对话数据集中随机提取200个部分对话，基于不同事件常识属性生成替代响应；2）提出基于指令的提示框架，利用LLM自动评估生成数据的质量。

Result: 初步结果表明，该方法能有效利用LLM的常识推理能力，生成高质量的对话增强数据。

Conclusion: 结论是该方法为对话系统数据增强和自动评估提供了有效工具，展示了LLM在常识推理任务中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLM在教育中的潜力，尤其是如何通过对话式教学理论（如维果茨基的社会文化学习理论、苏格拉底法和劳里拉德的对话框架）优化LLM的行为，以支持个性化和适应性学习。

Method: 综合现有文献，将教育理论与LLM能力映射，分析其支持与挑战传统教学假设的方面，并提出改进策略（如设计提示词、整合检索机制）。

Result: 发现LLM倾向于直接提供答案而非促进知识共建，并提出通过苏格拉底式提问、支架式引导等策略优化LLM交互。

Conclusion: 通过理论指导的实践策略，可以提升LLM在教育中的效果，弥合理论与AI实践的差距。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LRMs在复杂任务中表现出色，但高效推理策略可能降低模型鲁棒性，导致行为不一致，因此需要系统评估。

Method: 引入ICBENCH基准，从三个维度（ITS、TR-LB、IR-SE）测量LRMs的不一致性，并应用于开源模型。

Result: 发现高效推理策略（如No-Thinking和Simple Token-Budget）会增加不一致性，且模型普遍存在“欺骗”行为。
...
<details>
  <summary>Details</summary>
Motivation: 减少KV缓存的内存占用是LLMs轻量化的重要方向，但超低位量化会导致性能显著下降，需解决这一问题。

Method: 提出Anchor Score量化KV缓存对误差的敏感度，设计AnTKV框架和兼容FlashAttention的triton内核实现高效部署。

Result: AnTKV支持LLaMA-3-8B处理840K上下文，解码吞吐量提升3.5倍，在超低位量化下性能优于现有方法。

Conclusion: AnTKV通过选择性保留高敏感度token的精度，有效平衡了内存占用与模型性能。
...
Method: 采用RAG框架，提出动态查询依赖的检索策略（如autocut*和elbow），并与现有方法（surprise和autocut）对比。

Result: 实验表明，动态策略优于固定top-k策略，能生成更事实和相关的答案。

Conclusion: 动态检索策略在基于EHR的问答任务中表现更优，未来可进一步优化。
...
<details>
  <summary>Details</summary>
Motivation: 法国处方中的药物剂量说明常存在模糊、不规范或口语化问题，限制了传统机器学习流程的效果，因此需要更高效的结构化方法。

Method: 比较了提示法和微调LLM的方法，并与基于NERL的基线系统对比，最终提出混合方法，将低置信度案例从NERL路由到LLM。

Result: 混合方法实现了91%的结构化准确率，同时降低了延迟和计算成本。

Conclusion: 混合方法结合了NERL的结构精确性和LLM的语义处理能力，为实际临床使用提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在开放世界任务中表现强大，但难以快速适应新任务，传统微调方法成本高且易导致灾难性遗忘。

Method: KnowMap动态构建知识库，通过微调小型知识嵌入模型，为大型LLM注入任务特定知识。

Result: 在ScienceWorld基准测试中，gpt-4-turbo模型的性能提升了17.71%。

Conclusion: KnowMap为LLM任务适应提供了高效方法，并展示了环境与经验知识对提升推理能力的重要性。
...
Result: 自2022年4月以来，处理了超过3亿篇新闻文章，识别了95,000多个独特健康事件，其中3,500多个被专家列为潜在爆发事件。
...
Conclusion: RCStat无需模型重训练即可实现最先进的压缩和归因效果。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在明确自动指标的性能上限，通过引入人类基线评估指标能力。

Method: 在机器翻译元评估中纳入人类基线，比较自动指标与人类判断的一致性。

Result: 结果显示先进自动指标常与人类基线相当或更优，但需谨慎解读。

Conclusion: 研究揭示了评估能力的局限性，呼吁关注机器翻译评估领域的进步测量问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM推理链的不透明性和不可靠性问题，增强其解释性和可信度。

Method: 结合MRF-ETM生成主题感知的推理链，使用CSBert对齐因果推理，并通过统计过滤无效链。

Result: ECCoT显著提升了LLM推理的透明度和决策的可信度。

Conclusion: ECCoT为LLM提供了一种有效的端到端推理链验证方法，增强了其实际应用的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 在线仇恨言论检测对净化网络环境至关重要，而现有研究多关注单一句子级别，本文强调用户级别检测的重要性。

Method: 采用多模态聚合方法，整合用户文本、活动及社交网络数据，评估于Twitter、Gab和Parler三个平台。

Result: 实验表明，结合社交上下文显著提升检测效果，优于传统文本和图方法，且适用于不同平台和大规模数据。

Conclusion: 该方法不仅提升仇恨言论检测，还能辅助识别隐晦信息（如狗哨政治），并为干预措施提供依据。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在生成连贯文本时存在幻觉问题（事实错误），自校正方法被认为是有潜力的解决方案。

Method: 采用多轮LLMs生成验证问题，利用内部或外部知识回答问题，并基于新证据修正原始回答。

Result: 研究发现搜索引擎片段和少量示例提示对性能有益，G-Eval与人工评估高度一致。

Conclusion: 自校正方法在新闻摘要领域具有潜力，但仍需进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 旨在通过结合LLM和强化学习，提升对话系统的适应性和效率，特别是在有限数据下学习和个性化响应方面。

Method: 采用分层强化学习建模对话阶段，结合元学习增强对不同用户画像的适应性。

Result: 在动机访谈任务中，提出的对话管理器的表现优于现有LLM基线，显示出目标导向对话系统的潜力。

Conclusion: 该框架展示了结合LLM和强化学习在目标导向对话系统中的有效性，尤其是在个性化和适应性方面。
...
Result: RPT在类似训练数据的任务上表现良好，但在不同推理模式的领域泛化能力不一致。
...
<details>
  <summary>Details</summary>
Motivation: 由于罕见疾病的评估数据难以获取，且手动创建临床病例成本高，研究旨在开发一种低成本、透明的方法来预测算法更新后的性能变化。

Method: 利用人类表型本体（HPO）的疾病-表型注释生成合成病例，模拟SC访谈，并通过R平方系数验证方法的有效性。

Result: 实验显示，对于HPO中有频率信息的疾病，预测性能变化的效果显著（R²=0.831）；而无频率信息的疾病预测误差较大。手动映射HPO表型到SC症状的耗时约为2小时/疾病。

Conclusion: 该方法为罕见疾病的SC算法更新提供了一种低成本、高效的预部署评估手段，有助于提升早期诊断支持。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语是世界上使用广泛的语言之一，拥有多种方言，研究旨在解决方言分类问题，以支持个性化聊天机器人、社交媒体监控等应用。

Method: 采用了RNN、Transformer和LLMs（通过提示工程）模型，结合先进的预处理技术。

Result: MARBERTv2表现最优，准确率和F1分数分别为65%和64%。

Conclusion: 研究为阿拉伯语方言识别提供了有效方法，并支持了相关应用的发展。
...
<details>
  <summary>Details</summary>
Motivation: 长语音识别中，基于多头注意力（MHA）的ASR模型因二次复杂度问题表现不佳，需更高效的替代方案。

Method: 采用线性复杂度的双向循环注意力（RA）层，并提出长形式训练范式及方向丢弃（Direction Dropout）正则化方法。

Result: RA在精度上与MHA相当，但效率更高；长形式训练使RA比LCA基线精度更高且吞吐量提升44%。

Conclusion: RA层是长语音识别的高效解决方案，方向丢弃进一步优化了精度与吞吐量的权衡。
...
Method: 通过分析标记分布和学习动态，提出SRFT方法，结合SFT和RL的直接优化，避免两阶段方法。

Result: SRFT在五个数学推理基准测试中平均准确率达59.1%，优于零RL方法9.0%。

Conclusion: SRFT通过熵感知机制有效统一了SFT和RL，显著提升了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 开源大语言模型在推理密集型任务中存在显著局限性，研究旨在探索提升其数据分析能力的策略。

Method: 通过策划多样化的真实场景种子数据集，评估模型在数据理解、代码生成和战略规划三个维度的表现，并基于发现开发数据合成方法。

Result: 研究发现战略规划质量是模型性能的主要决定因素，交互设计和任务复杂性显著影响推理能力，数据质量比多样性对性能影响更大。

Conclusion: 通过数据合成方法显著提升了开源LLMs的分析推理能力，为未来研究提供了实用方向。
...
<details>
  <summary>Details</summary>
Motivation: 网络仇恨言论引发社区暴力，威胁社会和谐，但现有研究对社区暴力文本分类探索不足。

Method: 使用微调的BanglaBERT模型和扩展数据集（增加1,794个实例）开发集成模型，并通过LIME分析模型决策。

Result: 微调模型和集成模型的F1分数分别为0.60和0.63，但模型在上下文理解和区分相关术语方面存在局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前统一多模态医疗LLMs在知识更新、全面性和灵活性上存在不足，需改进。

Method: 将医疗诊断过程分解为多个角色（如全科医生、专科团队等），每个角色由LLM智能体实现，模块化协作。

Result: 在多种多模态医疗数据集上，MAM性能显著提升（18%-365%）。

Conclusion: MAM框架有效解决了现有问题，提升了诊断性能，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有多视角点云配准方法依赖特征提取和数据关联，难以在复杂环境中获得全局最优解。

Method: 将全局地图表示为深度图，利用原始深度信息构建非线性最小二乘优化问题，联合估计点云位姿和全局地图。

Result: 在真实数据集上验证，该方法在特征提取和数据关联困难的复杂环境中优于现有方法。

Conclusion: 通过隐式数据关联和动态优化，该方法实现了更高效和精确的多视角点云配准。
...
<details>
  <summary>Details</summary>
Motivation: 现有YOLOv8模型在细粒度识别任务（如车牌读取）上能力有限，需要一种综合解决方案以实现车辆碳排放的精准估算。
...
<details>
  <summary>Details</summary>
Motivation: 传统的手指敲击测试依赖临床医生的主观评估，存在评分者间和评分者内变异性，且无法捕捉个体运动特征。本文旨在通过计算机视觉技术提供更客观、可量化的评估方法。

Method: 提出四组临床相关特征（运动减少、运动迟缓、序列效应和犹豫停顿），通过主成分分析和机器学习分类器预测MDS-UPDRS评分。

Result: 视频分析特征与四种运动缺陷对应，且能进一步细分序列效应和犹豫停顿。机器学习分类器在MDS-UPDRS评分预测中优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有段式方法计算效率低且依赖先验知识，难以应对复杂形态和环境变化。
...
<details>
  <summary>Details</summary>
Motivation: 塔吊操作日益自动化，但安全保护仍不足，尤其是工人和模块化建筑（MiC）的安全问题。现有摄像头和LiDAR数据未充分利用。

Method: 开发AI系统，融合摄像头和LiDAR的3D数据，定位工人和MiC，结合硬件和显示系统实现实时监控与警报。

Result: 系统通过3D数据融合和先进方法验证了准确性和有效性，实际工地测试证明其作为安全监控工具的价值。

Conclusion: 该系统为塔吊操作提供了高效的安全监控解决方案，具有实际应用潜力。
...
Method: 提出Damba-ST，包含域自适应状态空间模型和三种域适配器，分别学习跨域共性和域内特征。
...
Method: 引入频谱字典标记混合器，将图像块或词片段表示为稀疏频率原子的组合，简化计算。
...
TL;DR: DiffRIS利用预训练的文本到图像扩散模型，通过上下文感知适配器和渐进式跨模态解码器，显著提升了遥感图像分割的精度。


<details>
  <summary>Details</summary>
Motivation: 当前遥感图像分割方法在处理复杂对象特征（如尺度变化、多样朝向和语义模糊）时面临挑战，需要更高效的跨模态对齐方法。

Method: 提出DiffRIS框架，包含上下文感知适配器（CP-adapter）和渐进式跨模态推理解码器（PCMRD），分别用于动态优化语言特征和多尺度视觉-文本对齐。

Result: 在三个基准数据集（RRSIS-D、RefSegRS和RISBench）上，DiffRIS在所有标准指标上均优于现有方法，达到新SOTA。
...
Method: GLIMPSE融合了梯度加权注意力、自适应层传播和加权令牌聚合，生成响应级归因热图。

Result: GLIMPSE在人类对齐方面优于先前的可解释性方法，能够揭示LVLM的跨模态归因细节、令牌级推理动态以及人类注意力偏差。

Conclusion: GLIMPSE为分析LVLM的跨模态推理提供了细粒度工具，有助于理解模型行为并提升透明度。
...
<details>
  <summary>Details</summary>
Motivation: 自注意力机制在扩散Transformer中的二次计算复杂度导致高分辨率图像生成的计算成本过高，而直接训练Mamba模型存在困难。

Method: 采用扩散Transformer到Mamba的蒸馏技术（T2MD），结合层级教师强制和基于特征的知识蒸馏，构建混合模型以实现效率和全局依赖性的平衡。

Result: 实验表明，T2MD方法能够以较低的开销实现高质量的文本到图像生成，并支持从512×512分辨率扩展到2048×2048分辨率。

Conclusion: 论文证明了使用Mamba模型生成非因果视觉输出的可行性，为未来研究提供了潜在方向。
...
Conclusion: OoPk方法有效缓解了灾难性遗忘和错误累积，提升了语义分割任务的适应性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有OMR模型无法处理整页或多页乐谱的问题，并首次支持生成人类可读的ABC符号音乐格式。

Method: 结合预训练视觉编码器和ABC解码器，使用超过214K图像的数据集进行训练。

Result: 在多种数据集上实现最先进性能，并通过多样化指标验证其优越性。

Conclusion: Legato在OMR任务中表现出强大的泛化能力和领先性能，填补了端到端OMR标准化评估的空白。
...
TL;DR: 本文探讨了视觉语言模型（VLMs）在情感计算中依赖的视觉线索，发现其表现与牙齿可见性相关，并揭示了GPT-4o依赖面部特征（如眉毛位置）进行情感推理。


<details>
  <summary>Details</summary>
Motivation: 研究VLMs在情感计算中依赖的视觉线索是否具有心理学基础，还是表面学习的结果。

Method: 在AffectNet数据集的牙齿标注子集上对不同规模的VLMs进行基准测试，并通过结构化内省分析GPT-4o的表现。

Result: VLMs的表现与牙齿可见性相关，GPT-4o依赖面部特征（如眉毛位置）进行情感推理，显示出较高的内部一致性。
...
<details>
  <summary>Details</summary>
Motivation: 草原犬鼠等小型稀有野生动物对生态至关重要，但其检测因体型小、分布稀疏和视觉特征不明显而具有挑战性。

Method: RareSpot结合多尺度一致性学习和上下文感知增强，优化特征表示并生成具有挑战性的训练样本。

Result: 在草原犬鼠无人机图像基准测试中，检测准确率比基线方法提高35%，且能泛化到其他野生动物数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像编辑中计算量大，而蒸馏模型因反转质量差导致编辑能力受限，需改进高保真反转和重建以实现精确编辑。

Method: 引入循环一致性优化策略，结合一致性模型增强图像反转，平衡编辑性与内容保留。

Result: 在多种图像编辑任务和数据集上达到SOTA性能，效率显著高于全步扩散模型。
...
Method: 采用混合表示，地面以栅格化格式建模，对象以向量化3D基元编码，并引入稳定的Cholesky参数化解码对象大小和方向。
...
<details>
  <summary>Details</summary>
Motivation: 单模态跟踪在低光照和恶劣天气条件下表现不佳，多模态跟踪（如RGB-T）利用互补数据提升性能，但现有基于Vision Transformer的模型计算成本高。

Method: 采用MobileViT构建轻量级跟踪器，引入渐进式融合框架，联合学习模板和搜索区域的模态内和模态间交互。

Result: 模型参数少于400万，GPU推理速度达122帧/秒，性能与现有高效多模态跟踪器相当。

Conclusion: 首次将MobileViT应用于RGB-T跟踪，实现了高效且轻量化的多模态跟踪。
...
<details>
  <summary>Details</summary>
Motivation: 在线视频在政治话语和网络社交威胁（如错误信息、宣传和激进化）中扮演重要角色，识别视频中的关键帧对内容审核和总结至关重要。

Method: PRISM在CIELAB色彩空间中使用感知色差度量，无需训练且计算高效，适合实时和资源受限环境。

Result: 在BBC、TVSum、SumMe和ClipShots数据集上表现出高准确性和压缩比。

Conclusion: PRISM是一种可扩展的工具，适用于结构化和非结构化视频内容分析，尤其适合在线平台的有害或政治敏感媒体审核。
...
TL;DR: 提出了一种名为MOSCARD的多模态预测模型，整合胸片（CXR）和心电图（ECG）数据，通过因果推理和去偏技术提升心血管事件风险预测。


<details>
  <summary>Details</summary>
Motivation: 心血管事件（MACE）是全球主要死因，现有筛查方法受限于单模态或采样偏差，需更全面的风险评估方法。

Method: 结合CXR和ECG数据，采用多模态对齐、因果推理和双反向传播图技术，开发MOSCARD模型。

Result: 在内部和外部数据集上，MOSCARD的AUC分别为0.75、0.83和0.71，优于单模态和现有模型。

Conclusion: MOSCARD提供了一种低成本、高效的筛查方法，有助于早期干预和改善患者预后。
...
TL;DR: OpenWildlife (OW) 是一种开放词汇的野生动物检测器，通过语言感知嵌入和 Grounding-DINO 框架的改进，实现多物种识别，并在多样化的航空图像中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有自动化方法在特定场景下表现良好，但难以泛化到不同物种和环境，主要受限于分类覆盖范围和模型架构的刚性。

Method: OW 结合语言感知嵌入和改进的 Grounding-DINO 框架，支持自然语言输入识别物种，并引入高效的搜索算法优化检测效率。

Result: 在 15 个数据集上训练后，OW 表现优于现有方法，mAP50 达 0.981（微调）和 0.597（新物种）。搜索算法覆盖 95% 物种，仅需检查 33% 图像。

Conclusion: OW 是一种灵活、经济高效的全球生物多样性评估解决方案，代码和数据集已公开以支持可重复性。
...
TL;DR: 本文综述了古代文字图像识别方法，分类分析了不同文字类型的研究，探讨了独特挑战及解决方案，并展望了未来方向。


<details>
  <summary>Details</summary>
Motivation: 古代文字是人类文明的重要载体，自动化识别技术对考古和数字人文研究具有重要意义。

Method: 分类现有研究，分析识别方法，探讨独特挑战及解决方案（如少样本学习和噪声鲁棒技术）。

Result: 总结了当前方法的局限性，并提出了未来发展方向。

Conclusion: 本文为古代文字识别、解读和破译提供了结构化、前瞻性的视角。
...
<details>
  <summary>Details</summary>
Motivation: 随着CT检查需求的增加，诊断错误问题日益突出。现有医学视觉问答（VQA）基准缺乏临床相关性，无法评估专家级知识，因此需要更严格的验证工具。

Method: MedErr-CT通过VQA框架评估MLLMs的能力，包含六类错误（四种视觉相关错误和两种词汇错误）和三个任务级别（分类、检测和纠正）。

Result: 评估结果显示，当前最先进的3D医学MLLMs在不同错误类型上的表现存在显著差异。
...
Method: 采用基于分块的预填充和双层键值解码技术，分块内使用全注意力，分块间使用稀疏注意力，并选择性重载键值以减少开销。

Result: 在多个长视频理解基准测试中达到最佳性能，且能高效处理超过10,000帧的视频。
...
TL;DR: 论文提出了一种名为MSR-Align的多模态安全推理数据集，旨在解决现有安全对齐方法在多模态输入中的不足，并显著提升视觉语言模型的安全性。
...
Method: 通过构建MSR-Align数据集，支持多模态多样性和政策推理，并使用强大多模态评估进行质量过滤。

Result: 实验表明，基于MSR-Align微调的视觉语言模型显著提升了对抗文本和视觉语言越狱攻击的鲁棒性，同时保持或增强了一般推理性能。

Conclusion: MSR-Align为提升多模态视觉语言模型的安全性提供了可扩展且有效的基础，并已公开数据集。
...
Method: AIR框架包含AIR-Gen（生成定制数据集）和AIR-Aug（增强现有数据集），结合自动提示工程和分布调整算法。

Result: 实验证明生成的数据能有效训练深度学习模型，用户评分为4.4/5.0。

Conclusion: AIR框架为图像识别任务提供了一种高效、用户友好的解决方案。
...
TL;DR: 提出了一种3D选择性扫描模块（3D-SSM）以解决现有Mamba方法在遥感变化检测中长距离依赖捕获不足的问题，并基于此设计了时空交互模块（SIM）和多分支特征提取模块（MBFEM），显著提升了性能。
...
Method: 提出3D-SSM模块捕获全局信息，并基于此设计SIM和MBFEM模块，分别用于时空特征交互和多分支特征提取。
...
TL;DR: 提出了一种新的无监督域适应方法CAN，结合域协作和域对抗学习策略，通过自步学习SPCAN增强目标域判别性，实验表明其性能优越。


<details>
  <summary>Details</summary>
Motivation: 解决无监督域适应中源域和目标域分布不匹配的问题，同时保持目标域的判别性。

Method: 使用域协作学习保留目标域特征判别性，域对抗学习减少域间分布差异；设计自步学习SPCAN逐步选择伪标签目标样本。

Result: 在多个基准数据集（如Office-31、VISDA-2017等）上达到最先进性能。
...
TL;DR: AirV2X-Perception是一个基于无人机的V2X感知数据集，用于解决传统基础设施部署成本高和覆盖不足的问题，支持车辆与无人机协同驾驶算法的开发。


<details>
  <summary>Details</summary>
Motivation: 传统V2X系统在部署成本和覆盖范围上存在局限，尤其是在农村和郊区，无人机提供了更灵活、低成本的替代方案。

Method: 利用无人机采集6.73小时的多场景驾驶数据，覆盖城市、郊区和农村，包含多种天气和光照条件。

Result: 数据集支持车辆与无人机协同感知算法的开发，填补了空中辅助自动驾驶领域的空白。

Conclusion: AirV2X-Perception为V2D算法提供了标准化评估工具，推动了无人机在自动驾驶中的应用。
...
Method: 利用视觉语言模型（VLMs）构建WaterCaption数据集，并提出Da Yu模型及其Nano Transformer Adaptor（NTA）模块。
...
TL;DR: HoliGS是一种新颖的可变形高斯溅射框架，用于从长单目RGB视频中合成视角。它通过分层变形策略，显著提升了重建质量和效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如4D高斯溅射和动态NeRF）在长时间捕获场景中训练开销大，难以准确重建大规模动态环境。
...
TL;DR: 提出了一种基于VLM引导的级联框架，用于解决开放词汇伪装物体分割（OVCOS）中的分割与分类问题，显著提升了定位和分类的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在领域差距和分割模型对伪装物体边界不敏感的问题，导致分割和分类效果不佳。

Method: 利用VLM引导的SAM进行分割，并通过alpha通道保留完整图像上下文进行软空间先验分类。

Result: 在OVCOS和传统伪装物体分割基准测试中表现出显著优势。

Conclusion: 通过VLM语义的丰富利用，实现了对伪装物体更准确的分割和分类。
...
TL;DR: 论文提出了一种基于机器学习和人类注视数据的方法，用于客观评估气管插管技能，通过注意力机制和视觉掩模提升识别准确性。


<details>
  <summary>Details</summary>
Motivation: 传统的气管插管技能评估主观性强，难以在真实场景中准确衡量能力，亟需一种客观、高效的评估工具。

Method: 利用人类注视数据和视频记录，结合注意力机制和视觉掩模，通过自编码器网络提取特征，分类器输出评分。

Result: 该方法在预测准确性、敏感性和可信度上优于传统方法，适用于高压力环境如军事场景。

Conclusion: 结合人类注视数据的机器学习方法为临床技能评估提供了更客观、高效的解决方案，有望提升急诊医学的培训和患者结果。
...
Method: 提出PMC框架，包含两个新模块：可靠伪标签目标样本选择和模态集成信息捕获；在MMDA-PI设置下，进一步提出PMC-PI方法，通过多模态数据生成网络（MMG）生成目标域缺失的模态。

Result: 在三个图像数据集和八个视频数据集上的实验表明，PMC框架在多种多模态跨域视觉识别任务中表现优异。

Conclusion: PMC框架在多模态域适应任务中具有显著效果，特别是在处理目标域模态缺失的情况下表现突出。
...
TL;DR: RetCoP是一种持续视觉语言预训练框架，旨在整合多种眼底成像模态，通过排练策略和信息蒸馏减少灾难性遗忘，表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统眼底图像分析模型忽视多模态互补性，而现有视网膜基础模型多为模态特定。动态环境中多模态数据增量到达，需持续预训练。

Method: 提出RetCoP框架，结合排练策略（利用代表性图像-文本对）和离对角线信息蒸馏，整合多模态图像与文本特征。

Result: 实验显示RetCoP在泛化能力和遗忘率上优于其他方法。

Conclusion: RetCoP成功整合多模态数据，减少遗忘，为眼底图像分析提供统一基础模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要整合FFPE切片与基因组数据，忽视其他保存切片（如FF切片），且高分辨率病理数据主导融合过程，导致模态不平衡和不完整模态问题。

Method: 利用超图学习整合多WSI信息和跨模态交互，引入记忆机制动态补偿不完整模态。

Result: 在五个TCGA数据集上，模型C-Index优于先进方法2.3%，在不完整模态下优于仅病理（3.3%）和仅基因模型（7.9%）。

Conclusion: 该框架有效解决了病理-基因组多模态融合中的模态不平衡和不完整模态问题，提升了生存预测性能。
...
<details>
  <summary>Details</summary>
Motivation: 电子元件分类在制造业中至关重要，能显著降低人力成本并推动技术发展。预训练模型（尤其是基于ImageNet的）在图像分类中表现优异，适合数据有限的研究。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D场景理解局限于对象级别，且数据获取和标注成本高，因此需要突破这些限制以实现更细粒度的部分分割。

Method: 通过创新的低成本方法构建了3D-PU数据集，并提出了仅需3D输入的OpenPart3D框架，用于部分级别分割。

Result: 实验表明，该方法在部分级别的开放词汇3D场景理解任务中表现优越，且具有强大的泛化能力。

Conclusion: 该研究为3D部分级别的场景理解提供了新的数据集和方法，推动了该领域的进展。
...
TL;DR: 本文综述了动态目标追踪（DOT）与轨迹预测（TP）的最新进展、应用及挑战，涵盖多种方法及其实际效果与局限，并探讨了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 研究旨在总结DOT与TP技术的现状，分析其在不同领域的应用价值及面临的挑战，为未来研究提供方向。

Method: 通过分析特征、分割、估计和学习等方法，评估其在真实场景中的有效性、部署情况及局限性。

Result: 研究发现这些技术在自动驾驶、安防、医疗和工业自动化中显著提升了安全性与效率，但仍存在泛化性、计算效率、数据依赖和伦理问题。
...
Method: 从Mumford-Shah变分框架推导水平集公式，使用散度定理和曲线演化理论处理能量项，采用有限差分法实现，并引入数值稳定性措施。

Result: 在医学和合成图像上验证了模型的准确性、抗噪性和优于传统边缘方法的性能。
...
TL;DR: 提出MotionEcho，一种无需训练的动态定制视频生成框架，通过教师模型指导学生模型，提升运动保真度和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在蒸馏视频生成模型中难以实现动态定制，尤其是在无需训练的情况下。

Method: 利用扩散教师强制，通过端点预测和插值，动态分配计算资源。

Result: 实验表明，该方法显著提高了运动保真度和生成质量，同时保持高效性。

Conclusion: MotionEcho为蒸馏视频生成模型提供了一种高效的动态定制解决方案。
...
TL;DR: 提出了一种基于立体内窥镜图像的组织变形恢复方法，解决了相机运动、遮挡、大变形等问题，通过3D点及导数图和位移图建模，实现了高精度的在线恢复。


<details>
  <summary>Details</summary>
Motivation: 现有方法在相机运动、遮挡、大变形和组织特异性生物力学先验不足等问题上表现不佳，且依赖离线处理。本文旨在提出一种更鲁棒的在线方法。

Method: 将组织几何建模为3D点及导数图，变形建模为位移及局部变形图。通过相机中心设置和帧间变形优化，无需估计相机姿态，并引入规范图实现在线优化。

Result: 在体内外实验中，非遮挡和遮挡区域的重建精度分别达到0.37±0.27 mm和0.39±0.21 mm，并能估计表面应变分布。

Conclusion: 该方法在复杂条件下稳定建模组织几何与变形，为手术导航和自主软组织操作提供了有效工具。
...
TL;DR: 研究发现，视觉语言模型（VLMs）在训练过程中，文本识别能力（text readability）会突然出现，而语义理解能力则从早期逐渐发展。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言模型在训练过程中文本识别能力的涌现机制，以优化多模态学习策略。

Method: 通过分析VLMs的训练过程，观察文本识别和语义理解能力的发展模式。

Result: 文本识别能力在训练后期突然涌现，而语义理解能力则从早期逐渐发展；图像与文本匹配能力发展更慢。

Conclusion: 研究强调了需要针对文本理解设计专门的训练策略，为未来多模态学习优化提供基础。
...
TL;DR: 论文提出了一种分层扩散降尺度（HDD）模型，通过分层采样过程显著降低计算负担，同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 传统降尺度方法计算量大，AI降尺度模型（如扩散模型）虽有效但仍计算密集。HDD旨在提供轻量级解决方案。

Method: HDD引入分层采样框架，采用从粗到细的降采样策略，减少计算量。

Result: HDD在ERA5和CMIP6数据集上表现优异，计算量减少一半，且模型可跨分辨率迁移。

Conclusion: HDD为概率气候降尺度提供了轻量级高效方案，支持大规模高分辨率气候预测。
...
TL;DR: GLCANet是一种轻量级的语义分割框架，针对超高分辨率遥感图像设计，通过全局-局部交叉注意力机制提升分割精度和计算效率。


<details>
  <summary>Details</summary>
Motivation: 超高分辨率遥感技术的快速发展对语义分割的准确性和效率提出了更高要求，现有方法在计算效率和多尺度特征融合方面存在不足。

Method: GLCANet采用双流架构，结合自注意力机制和掩码交叉注意力机制，高效融合全局语义和局部细节，同时减少GPU占用。

Result: 实验表明，GLCANet在精度和计算效率上优于现有方法，能够高效处理大尺寸高分辨率图像。
...
<details>
  <summary>Details</summary>
Motivation: 现有MAV检测方法主要依赖RGB图像中的外观特征，但由于多样性难以实现泛化检测。事件相机捕捉到的高频螺旋桨特征为解决这一问题提供了新思路。

Method: 提出了三个模块，分别用于提取螺旋桨的显著时空特征，并过滤背景噪声和相机运动干扰。同时，首次引入了一个事件相机的MAV数据集。

Result: 在不训练的情况下，该方法显著优于现有技术，精度和召回率分别达到83.0%和81.5%。

Conclusion: 该方法通过事件相机有效解决了MAV检测的泛化问题，并提供了首个事件相机的MAV数据集，为后续研究奠定了基础。
...
Method: 提出Mem4Nav，结合稀疏八叉树（细粒度体素索引）和语义拓扑图（高层地标连接），通过可逆Transformer嵌入内存令牌，实现长短记忆的动态管理。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态医学分析中图像与表格数据融合的挑战，包括特征维度差异、模态贡献不平衡以及高维表格数据噪声。

Method: 提出AMF模块（自适应调制与融合）和FT-Mamba表格编码器，结合调制目标、模态置信比和特征掩码等技术，优化融合过程。

Result: AMF-MedIT在多模态性能和数据效率上表现优异，并能适应不完整的表格数据。FT-Mamba在特征提取和图像编码器指导方面表现出色。

Conclusion: AMF-MedIT为医学多模态数据融合提供了高效解决方案，尤其在数据稀缺和不完整条件下具有优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过向图像添加噪声作为样本，但噪声分布可能导致解释确定性低。本文旨在解决样本分布不对齐问题，构建更可信的归因分析。
...
TL;DR: 论文介绍了一个基于智能手机慢动作视频构建的最大真实世界图像去模糊数据集，包含42,000对高分辨率模糊-清晰图像对，是现有数据集的10倍大，场景多样性更高。


<details>
  <summary>Details</summary>
Motivation: 现有去模糊数据集规模小且场景单一，难以支持鲁棒且泛化性强的模型开发。

Method: 通过智能手机慢动作视频的240帧模拟长曝光模糊，平均帧生成模糊图像，中心帧作为清晰参考。

Result: 测试多个SOTA去模糊模型，性能显著下降，表明数据集的复杂性和多样性。

Conclusion: 该数据集为开发鲁棒且泛化性强的去模糊模型提供了新挑战。
...
TL;DR: 提出一种两步法改进合成数据质量，通过神经分形和改进的反向风格化技术，显著缩小合成数据与真实图像的分布差距，提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决真实图像数据集的隐私、法律限制以及合成数据训练模型性能不足的问题。

Method: 1. 改进神经分形生成新合成数据；2. 提出反向风格化技术，将小规模真实图像特征迁移到合成数据中。

Result: KID显示分布差距显著缩小；EDM2扩散模型FID降低11%，ViT-S分类模型在ImageNet-100上准确率提升10%。

Conclusion: 该方法为缺乏大规模真实数据时的模型训练提供了实用解决方案。
...
TL;DR: 提出了一种用于手术场景视觉问答定位（Surgical-VQLA）的推理多模态大语言模型（Surgery-R1），通过两阶段微调机制提升推理能力和可解释性。
...
Conclusion: Surgery-R1为手术场景的视觉问答定位提供了更可靠的解决方案，代码和数据集已公开。
...
TL;DR: 论文提出USIS16K数据集，包含16,151张高分辨率水下图像，标注了158类水下物体的实例级显著对象掩码，并提供了基准评估。


<details>
  <summary>Details</summary>
Motivation: 水下显著实例分割（USIS）因环境动态性和数据稀缺性未被充分研究，需高质量数据集推动研究。

Method: 构建USIS16K数据集，覆盖多样环境，提供高质量实例级标注，并进行基准评估。

Result: USIS16K在多样性、复杂性和可扩展性上显著提升，支持水下目标检测和USIS任务。
...
Conclusion: 该方法有效解决了CT报告生成中的特征对齐和多层次建模问题，具有实际应用价值。
...
TL;DR: HMSViT是一种新型分层掩码自监督视觉Transformer，用于角膜神经分割和糖尿病周围神经病变诊断，通过高效多尺度特征提取和自监督学习，显著优于现有方法。
...
Method: HMSViT采用基于池化的分层和双重注意力机制，结合绝对位置编码，实现高效多尺度特征提取；设计块掩码自监督学习框架减少对标记数据的依赖；使用多尺度解码器进行分割和分类。

Result: 在临床CCM数据集上，HMSViT在神经分割任务中达到61.34% mIoU，诊断准确率为70.40%，优于Swin Transformer和HiViT等模型，且参数更少。

Conclusion: HMSViT在性能和临床实用性上表现优异，具备实际诊断应用的潜力。
...
TL;DR: 论文提出SceneCrafter，一种用于多相机捕捉驾驶场景的3D一致编辑工具，解决生成仿真场景中的现实性和一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有纯合成场景缺乏现实基础，难以验证结果的相关性；编辑模型虽利用真实驾驶数据，但面临3D一致性、遮挡学习和配对数据生成等挑战。

Method: 基于多视角扩散模型，提出可控框架处理多模态条件（如天气、时间等）；通过Prompt-to-Prompt生成几何一致的配对数据，并利用alpha混合框架处理局部编辑。

Result: SceneCrafter在真实性、可控性、3D一致性和编辑质量上优于现有基线。

Conclusion: SceneCrafter为驾驶场景编辑提供了高效解决方案，显著提升了仿真场景的现实性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉幻觉评估主要关注感知能力，忽视了推理能力引发的幻觉，这在高风险AI应用中可能导致严重可靠性问题。

Method: 开发了PRE-HAL数据集，系统性评估LVLMs的感知和推理能力；提出基于DST的不确定性估计方法，通过冲突特征检测幻觉。

Result: 实验表明，该方法在三种LVLMs上优于五种基线不确定性指标，平均AUROC提升4%、10%和7%。

Conclusion: PRE-HAL数据集和DST方法有效揭示了LVLMs的视觉幻觉问题，尤其在关系推理中表现突出，为未来研究提供了新方向。
...
TL;DR: 提出了一种深度学习框架ReMAR-DS，用于减少kVCT图像中的金属伪影，并将其转换为MVCT图像，提升放疗规划质量。
...
Conclusion: ReMAR-DS框架为临床提供了高质量的kVCT重建，降低了患者辐射暴露。
...
TL;DR: 提出一种检测和识别面部识别系统中自然触发后门攻击的新方法，效果优于基线。


<details>
  <summary>Details</summary>
Motivation: 后门攻击对高安全性应用中的面部识别系统构成威胁，需有效检测和识别自然触发。

Method: 开发新技术检测和识别面部识别网络中的自然触发后门。

Result: 在受感染网络中，识别触发器的前5准确率达74%，优于基线56%。

Conclusion: 该方法能有效检测和识别自然触发后门，提升面部识别系统的安全性。
...
TL;DR: 研究探讨了在医学数据上预训练定制基础模型与从通用模型迁移学习的优劣，并通过胎儿超声数据集案例验证了定制模型的价值。


<details>
  <summary>Details</summary>
Motivation: 解决在医学数据上是否应预训练定制基础模型或依赖通用模型迁移学习的问题，并验证是否需要新方法。

Method: 使用DINOv2方法在200万张胎儿超声图像上预训练基础模型，并与自然图像、超声图像预训练模型及监督基线进行比较。

Result: 定制数据预训练效果优于通用模型，且无需超参数调整或方法创新即可实现高性能。

Conclusion: 在医学领域开发定制基础模型时，应避免过度追求方法创新，而应注重数据适配和现有技术的优化。
...
TL;DR: MambaOutRS是一种新型混合卷积架构，用于遥感图像分类，通过结合门控CNN和傅里叶滤波门模块，在多个数据集上实现SOTA性能。
...
Method: 采用四阶段分层设计，结合门控CNN块和傅里叶滤波门（FFG）模块，高效提取局部特征和全局上下文信息。

Result: 在UC Merced、AID等数据集上表现优异，MambaOutRS-t变体以较少参数实现最高F1分数（98.41%和95.99%）。

Conclusion: MambaOutRS通过门控卷积和频域模块，有效替代复杂SSMs，为高效深度学习模型提供新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型通常针对单一传感器或固定组合，限制了多传感器数据的可扩展性和泛化能力。SMARTIES旨在解决这一问题，实现灵活的多传感器数据处理。

Method: SMARTIES将异构传感器数据投影到共享频谱感知空间，使用统一的Transformer模型通过跨传感器令牌混合重建掩码多传感器数据。

Result: SMARTIES在单模态和多模态任务中均优于依赖传感器特定预训练的模型。

Conclusion: SMARTIES为多传感器遥感数据处理提供了灵活且可扩展的解决方案，显著提升了泛化能力。
...
TL;DR: ScaleCap是一种推理时可扩展的图像描述生成策略，通过逐步增加推理预算生成更全面、详细的图像描述，解决了多模态和语言偏见问题。
...
Method: 提出启发式问答和对比句子评分，逐步注入信息并消除幻觉。

Result: 在11个基准测试中表现优异，生成的描述更准确、平衡且信息丰富。

Conclusion: ScaleCap通过逐步扩展推理预算，显著提升了图像描述的质量和多样性。
...
<details>
  <summary>Details</summary>
Motivation: 全球视力受损人口超过2.2亿，传统导盲工具存在不足，需更高效的导航辅助设备。

Method: 开发了一种可穿戴盲人导航设备，结合步态分析和多模态环境感知技术。

Result: 实验显示，该设备在室内外导航中表现优于传统导盲杖。

Conclusion: 该设备为视力受损者提供了更有效的导航解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有动态NeRF方法需要3D标签的问题，同时提升LiDAR和相机数据的联合学习效率。

Method: 采用自监督学习，引入启发式图像像素采样和双梯度掩码以优化训练和保留LiDAR特征。

Result: 在KITTI-360数据集上表现优于基线模型，LiDAR和相机领域均取得最佳性能。

Conclusion: NVSF是一种高效、自监督的多模态动态场景表示方法，适用于自动驾驶场景。
...
Result: 在多个合成数据集上优于基线模型，实现无监督跟踪和预测，并学习可解释的对象和运动表示。

Conclusion: VideoPCDNet为无监督视频分解和预测提供了高效且可解释的解决方案。
...
Method: 通过生成场景图的结构化表示，计算物体与人物之间的参数化关系，并提供RGB图像、分割掩码、深度图像和人体关键点。

Result: HOIverse提供了准确且密集的关系标注，并在现有场景图生成模型上进行了基准测试。
...
TL;DR: PEVLM是一种并行编码策略，旨在提升视觉语言模型（VLMs）的长视频理解效率，无需微调模型。通过分块处理输入并保留全注意力位置嵌入，显著降低计算复杂度，同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 标准注意力机制的二次复杂度限制了视觉语言模型在长视频理解中的应用，需要一种高效且无需微调的方法来提升性能。

Method: PEVLM将输入分块处理，共享一个共享池，保留全注意力位置嵌入，并通过对齐注意力权重模拟全注意力分布。
...
<details>
  <summary>Details</summary>
Motivation: 随着地球观测数据规模的快速增长，需要高效的压缩技术来管理和存储这些数据。

Method: 利用标准视频编解码器（通过ffmpeg）对多通道时空数据集进行压缩，并将其编码为视频。

Result: 在四个真实数据集上测试，压缩比高达250倍，PSNR表现优异，且不影响下游深度学习任务性能。

Conclusion: xarrayvideo为地球科学社区提供了一种高效且实用的数据压缩解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决SAM2在医学图像分割中依赖人工提示和领域偏移的局限性。
...
Method: PathLUPI在训练中使用转录组特权信息，提取基因组锚定的组织学嵌入，仅需WSI即可在推理阶段进行分子预测。
...
TL;DR: 论文提出了一种基于场景图（SG）的方法，利用Transformer模型和LLMs增强超声图像的解读和扫描指导，尤其针对非专业用户。


<details>
  <summary>Details</summary>
Motivation: 解决医学超声图像因参数差异导致的视觉变异性问题，并满足非专业用户对图像解读和扫描指导的需求。

Method: 使用Transformer模型生成超声图像场景图（SG），结合LLMs优化SG表示，并探索SG在扫描指导中的应用。

Result: 在颈部区域（颈动脉和甲状腺）的实验中验证了方法的有效性，提升了超声的可解释性和可用性。
...
TL;DR: UltraAD是一种基于视觉语言模型的方法，通过少量超声图像样本实现异常定位和细粒度分类，显著提升医学图像分析的准确性。


<details>
  <summary>Details</summary>
Motivation: 解决现有无监督或半监督异常检测方法在细粒度区分（如良性与恶性肿瘤）上的不足，以及超声图像因设备和参数差异导致的域差距问题。

Method: 结合视觉原型与可学习文本嵌入，融合图像级和块级令牌以优化定位；构建记忆库存储少量样本及其文本描述，用于细粒度分类。

Result: 在三个乳腺超声数据集上表现优异，超越现有方法，定位和分类精度显著提升。

Conclusion: UltraAD通过视觉语言模型和少量样本学习，有效解决了医学图像异常检测的挑战，具有临床应用潜力。
...
TL;DR: 论文探讨了鱼眼相机在3D人体姿态估计中的投影模型选择问题，提出了一种基于检测边界框的启发式方法，并引入新数据集FISHnCHIPS。


<details>
  <summary>Details</summary>
Motivation: 鱼眼相机能捕捉更广视野的人类动作，但图像扭曲导致姿态估计困难，现有方法未系统评估其有效性。

Method: 评估了针孔、等距、双球面相机模型及圆柱投影方法对3D姿态估计的影响，提出启发式选择模型。

Result: 近距离场景下针孔投影效果不佳，双球面模型显著提升精度，最优投影方法随视野范围变化。

Conclusion: 提出基于边界框的投影模型选择方法，并发布新数据集FISHnCHIPS，为鱼眼图像姿态估计提供基准。
...
TL;DR: CoCo4D提出了一种从文本提示生成动态4D场景的框架，支持多视角一致性和沉浸感，通过分离动态前景和背景建模实现高效合成。


<details>
  <summary>Details</summary>
Motivation: 现有4D合成方法多局限于对象级生成或有限视角的动态场景，无法满足多视角一致性和沉浸感需求。

Method: CoCo4D将4D场景合成分为动态前景和背景建模两部分，利用参考运动序列指导生成，并通过渐进式外推方案优化合成效果。

Result: 实验表明，CoCo4D在4D场景生成中表现优于或与现有方法相当，验证了其高效性和有效性。

Conclusion: CoCo4D通过分离前景和背景建模，实现了高质量、多视角一致的动态4D场景生成，为相关领域提供了新思路。
...
TL;DR: ProtoSolo是一种新型深度神经网络架构，通过单个原型激活完成图像分类，显著降低解释的认知复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有原型网络依赖多个原型协同决策，而ProtoSolo旨在通过单个原型实现分类和解释，简化模型并提升可解释性。

Method: 采用基于特征的比较方法，使用特征图而非全通道特征向量进行相似性比较和原型学习；提出非原型投影学习策略，避免投影操作对网络结构的负面影响。

Result: 在CUB-200-2011和Stanford Cars数据集上，ProtoSolo在分类任务中表现优异，解释的认知复杂度达到最佳水平。

Conclusion: ProtoSolo通过单原型激活和特征图设计，实现了高性能分类和低认知复杂度的解释，优于现有可解释方法。
...
TL;DR: 论文提出Bind-Your-Avatar模型，解决多角色同场景视频生成的音频对应控制和数据集缺乏问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法多关注单角色场景，多角色同场景视频生成的音频对应控制和数据集缺乏是主要挑战。

Method: 提出基于MM-DiT的模型，引入Embedding Router绑定角色与语音，设计3D-mask嵌入路由器和损失函数，并构建首个多角色数据集。

Result: 实验表明模型在双角色视频生成中优于现有方法。

Conclusion: Bind-Your-Avatar在多角色同场景视频生成中表现出色，解决了关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 随着用户对高分辨率视频生成的需求增加，仅依赖潜在计算已不足，需探索更高效的级联VSR模型设计。

Method: 提出两种退化策略生成训练对，分析时间步采样策略和噪声增强效果，并引入交错时间单元和稀疏局部注意力以减少计算开销。
...
TL;DR: DFM是一种简单有效的渐进生成视觉媒体的框架，通过独立应用Flow Matching在多尺度表示中提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 解决高维视觉模态生成的计算密集问题，避免复杂多阶段架构的额外开销。

Method: 在用户定义的多尺度表示（如拉普拉斯金字塔）中独立应用Flow Matching。

Result: 在图像和视频生成中提升视觉质量，Imagenet-1k 512px上FDD分数提升35.2%，收敛速度更快。

Conclusion: DFM以单一模型、架构简单性和最小修改实现显著优势。
...
TL;DR: GenHSI是一种无需训练的方法，用于生成可控的长视频，解决了现有方法在生成长视频时的问题，如不真实的人-场景交互和缺乏主体身份保留。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模预训练视频扩散模型在生成长电影式视频时面临挑战，如人-场景交互不真实、主体身份难以保持以及训练成本高。

Method: GenHSI将长视频生成任务分为三个阶段：脚本编写、预可视化和动画。通过3D关键帧和现成的视频扩散模型生成一致的长视频。

Result: 实验表明，GenHSI能从单张场景图像生成长视频，有效保留场景内容和角色身份，并实现合理的人-场景交互。

Conclusion: GenHSI无需训练即可生成长视频，解决了现有方法的局限性，为人-场景交互视频生成提供了新思路。
...
TL;DR: 论文提出了一种基于2D图像质量评估（IQA）的视角选择方法，用于新视角合成和3D重建，避免了传统方法在3D空间中的复杂建模。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如FisheRF和ActiveNeRF）需要针对不同3D表示进行专门设计，且在3D空间中建模复杂。本文旨在简化视角选择过程，提出一种更通用的方法。

Method: 将视角选择问题转化为2D图像质量评估任务，通过训练模型预测多视角设置下的SSIM，指导选择渲染质量最低的视角。

Result: 该方法在标准基准测试中实现了显著的定量和定性改进，且与3D表示无关，运行速度比现有方法快14-33倍。

Conclusion: 通过2D图像质量评估框架，本文提供了一种高效且通用的视角选择方案，适用于多种3D表示。
...
TL;DR: NAFNet是一种简单高效的图像修复深度学习基线模型，通过CIFAR10噪声和模糊图像进行核心组件消融研究，验证了SimpleGate激活、SCA和LayerNorm的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究NAFNet的核心组件对图像修复性能的影响，验证其设计优势。

Method: 使用CIFAR10噪声和模糊图像，通过消融实验比较不同组件（SimpleGate、SCA、LayerNorm）的效果。
...
Method: UniVLA将视觉、语言和动作信号建模为离散标记序列，结合世界建模从大规模视频数据中学习，支持灵活的多模态任务。

Result: 在CALVIN、LIBERO等基准测试中取得新SOTA，如LIBERO上平均成功率95.5%，优于pi0-FAST的85.5%。

Conclusion: UniVLA通过统一建模和多模态学习，显著提升了长时程任务和实际应用中的性能。
...
TL;DR: AnimaX是一个基于视频扩散模型和骨架动画的3D动画框架，支持多样化的骨架结构，通过多视角、多帧2D姿态图实现高效3D运动生成。


<details>
  <summary>Details</summary>
Motivation: 传统运动合成方法受限于固定骨架结构或高维变形空间优化成本高，AnimaX旨在将视频运动知识高效迁移到3D领域。

Method: AnimaX将3D运动表示为多视角、多帧2D姿态图，结合视频-姿态扩散模型，利用共享位置编码和模态感知嵌入实现时空对齐。

Result: 在160,000条序列数据集上训练，AnimaX在VBench上实现了泛化性、运动保真度和效率的领先表现。
...
TL;DR: 论文提出Radial Attention，一种稀疏注意力机制，通过模拟时空能量衰减现象，显著降低视频扩散模型的计算成本，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型的高计算成本限制了长视频的生成能力，作者发现时空注意力分数随距离衰减的现象，并以此为灵感提出高效解决方案。

Method: 提出Radial Attention，利用静态注意力掩码，使每个token仅关注空间邻近的token，且注意力窗口随时间距离缩小，复杂度为O(n log n)。

Result: 实验表明，Radial Attention在多个数据集上保持视频质量，计算速度提升1.9倍，支持生成长度扩展4倍，训练成本降低4.4倍，推理加速3.7倍。

Conclusion: Radial Attention通过稀疏化注意力机制，显著提升视频扩散模型的效率和扩展性，为长视频生成提供实用解决方案。
...
### [132] [Fake or Real, Can Robots Tell? Evaluating Embodied Vision-Language Models on Real and 3D-Printed Objects](https://arxiv.org/abs/2506.19579)
*Federico Tavella,Kathryn Mearns,Angelo Cangelosi*

Main category: cs.RO

TL;DR: 比较不同视觉语言模型（VLMs）在机器人场景理解中的表现，发现VLMs适用于常见物体识别，但对新颖表示泛化能力不足。
...
Method: 通过机器人手臂采集多视角图像，评估BLIP和VLMs等模型的描述生成性能，比较单视角与多视角描述及真实物体与3D打印物体的识别差异。

Result: VLMs在常见物体识别中表现良好，但对新颖表示泛化能力有限。
...
### [133] [CronusVLA: Transferring Latent Motion Across Time for Multi-Frame Prediction in Manipulation](https://arxiv.org/abs/2506.19816)
*Hao Li,Shuai Yang,Yilun Chen,Yang Tian,Xiaoda Yang,Xinyi Chen,Hanqing Wang,Tai Wang,Feng Zhao,Dahua Lin,Jiangmiao Pang*

Main category: cs.RO

TL;DR: CronusVLA通过高效的后训练阶段将单帧视觉语言动作模型扩展到多帧范式，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言动作模型受限于单帧观察范式，无法充分利用多帧历史观测的运动信息。

Method: CronusVLA包含单帧预训练、多帧编码和跨帧解码三个关键组件，通过减少冗余计算和缓存历史特征实现高效推理。

Result: 在SimplerEnv上达到70.9%的成功率，LIBERO上比OpenVLA提升12.7%，真实世界实验表现优异。

Conclusion: CronusVLA通过多帧处理和运动特征应用，显著提升了模型的性能和鲁棒性。
...
### [134] [Look to Locate: Vision-Based Multisensory Navigation with 3-D Digital Maps for GNSS-Challenged Environments](https://arxiv.org/abs/2506.19827)
*Ola Elmaghraby,Eslam Mounier,Paulo Ricardo Marques de Araujo,Aboelmagd Noureldin*

Main category: cs.RO

TL;DR: 论文提出了一种低成本、基于视觉的多传感器导航系统，结合单目深度估计、语义过滤和视觉地图注册，在GNSS缺失环境中实现高精度车辆定位。


<details>
  <summary>Details</summary>
Motivation: 在GNSS缺失环境（如室内停车场或密集城市峡谷）中，实现高精度和鲁棒的车辆定位仍具挑战性。

Method: 系统整合单目深度估计、语义过滤和视觉地图注册（VMR）与3D数字地图。

Result: 在室内外真实驾驶场景中测试，室内定位精度达92%（亚米级），室外超80%，水平定位和航向平均RMSE分别为0.98米和1.25度。相比基线方法，定位精度平均提升约88%。

Conclusion: 低成本单目视觉系统结合3D地图在陆地车辆导航中具有潜力，可实现可扩展且不依赖GNSS的解决方案。
...
### [135] [Assessing Risk of Stealing Proprietary Models for Medical Imaging Tasks](https://arxiv.org/abs/2506.19464)
*Ankita Raj,Harsh Swaika,Deepankar Varma,Chetan Arora*

Main category: eess.IV

TL;DR: 论文研究了黑盒医学影像模型在模型窃取（MS）攻击下的脆弱性，提出了一种名为QueryWise的两步攻击方法，利用公开数据集和有限查询预算有效克隆模型功能。
...
Method: 提出QueryWise方法，通过代理数据集和有限查询预算训练窃取模型，并利用未标记数据优化查询效率。

Result: 在胆囊癌和COVID-19分类任务中验证了攻击的有效性。

Conclusion: 医学影像模型对MS攻击存在显著脆弱性，QueryWise方法在有限查询预算下表现优异。
...
### [136] [NIC-RobustBench: A Comprehensive Open-Source Toolkit for Neural Image Compression and Robustness Analysis](https://arxiv.org/abs/2506.19051)
...
<details>
  <summary>Details</summary>
Motivation: 随着JPEG AI标准的发布，评估NIC鲁棒性的需求变得至关重要，但此前研究局限于少量编解码器和攻击方式。

Method: 提出了NIC-RobustBench框架，包含最多的编解码器，易于扩展，并用于分析NIC鲁棒性。

Result: 框架提供了全面的NIC鲁棒性分析工具，代码已开源。

Conclusion: NIC-RobustBench填补了NIC鲁棒性评估的空白，为未来研究提供了实用工具。
...
### [137] [Xray2Xray: World Model from Chest X-rays with Volumetric Context](https://arxiv.org/abs/2506.19055)
*Zefan Yang,Xinrui Song,Xuanang Xu,Yongyi Shi,Ge Wang,Mannudeep K. Kalra,Pingkun Yan*

Main category: eess.IV

TL;DR: Xray2Xray是一种新型世界模型，通过学习胸部X射线的潜在表示来编码3D结构信息，解决了2D投影图像的结构叠加限制，并在心血管疾病风险预测和病理分类任务中表现出色。
...
Method: Xray2Xray通过视觉模型和过渡模型学习不同角度X射线投影的潜在表示，用于下游任务如风险预测和疾病诊断。

Result: Xray2Xray在心血管疾病风险预测中优于监督和自监督方法，并在五种病理分类中表现优异，其潜在表示还可用于重建体积上下文。

Conclusion: Xray2Xray通过从2D X射线中提取3D信息，显著提升了诊断和预测能力，具有临床应用潜力。
...
### [138] [Staining normalization in histopathology: Method benchmarking using multicenter dataset](https://arxiv.org/abs/2506.19106)
*Umair Khan,Jouni Härkönen,Marjukka Friman,Leena Latonen,Teijo Kuopio,Pekka Ruusuvuori*

Main category: eess.IV

TL;DR: 研究探讨了H&E染色在不同实验室间的变异性问题，并提出了一种多中心数据集用于评估八种染色归一化方法的性能。
...
Method: 收集多中心组织图像数据集，保持其他因素恒定，评估八种染色归一化方法（四种传统方法和四种深度学习变体）。

Result: 通过定量和定性评估比较了不同方法的性能，数据集还可用于提升模型泛化能力。

Conclusion: 研究为减少染色变异性提供了实用方法，并展示了多中心数据集在提升AI模型性能中的潜力。
...
### [139] [A Deep Learning Based Method for Fast Registration of Cardiac Magnetic Resonance Images](https://arxiv.org/abs/2506.19167)
*Benjamin Graham*
...
<details>
  <summary>Details</summary>
Motivation: 医学图像配准在心脏健康评估中至关重要，但现有深度学习方法速度慢且准确性不足，需要一种快速且高效的解决方案。

Method: 设计了一种高效的卷积架构（FLIR模型），用于快速体积配准，同时保持与现有先进模型相似的配准精度。

Result: FLIR模型在保持高配准精度的同时显著提高了计算效率，应变量化结果表现出高度一致性。
...
### [140] [Deformable Medical Image Registration with Effective Anatomical Structure Representation and Divide-and-Conquer Network](https://arxiv.org/abs/2506.19222)
*Xinke Ma,Yongsheng Pan,Qingjie Zeng,Mengkang Lu,Bolysbek Murat Yerzhanuly,Bazargul Matkerim,Yong Xia*

Main category: eess.IV

TL;DR: 提出了一种名为EASR-DCN的新方法，通过有效表示ROI并独立对齐，显著提升了可变形医学图像配准的性能。
...
Method: 使用高斯混合模型进行强度分析表示ROI，提出Divide-and-Conquer Network独立对齐ROI。

Result: 在多个数据集上表现优异，Dice分数提升显著（脑MRI 10.31%，心脏MRI 13.01%，海马体MRI 5.75%）。

Conclusion: EASR-DCN在无需标签的情况下实现了高精度配准，具有临床潜力。
...
### [141] [Quantitative Benchmarking of Anomaly Detection Methods in Digital Pathology](https://arxiv.org/abs/2506.19234)
...
TL;DR: 论文对数字病理图像中的异常检测方法进行了系统性评估，通过实验比较了20多种方法，分析了图像尺度、异常类型和训练策略对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 数字病理图像中的异常检测在罕见疾病识别和生物标志物发现等领域具有潜力，但其独特特性（如大尺寸、多尺度结构）给现有方法带来挑战。

Method: 研究通过五个真实和合成的数字病理数据集，对20多种经典和流行的异常检测方法进行了实验评估。

Result: 实验结果详细比较了各方法的优缺点，并分析了图像尺度、异常类型和训练策略对检测性能的影响。

Conclusion: 研究为数字病理图像异常检测提供了全面的基准，指导未来研究方向。
...
### [142] [Explicit Residual-Based Scalable Image Coding for Humans and Machines](https://arxiv.org/abs/2506.19297)
...
<details>
  <summary>Details</summary>
Motivation: 随着图像不仅被人类消费还被机器视觉模型使用，需要一种同时服务于机器和人类视觉的可扩展图像压缩方法（ICMH）。现有方法依赖神经网络编解码器，但架构设计不足。
...
### [143] [Reconsidering Explicit Longitudinal Mammography Alignment for Enhanced Breast Cancer Risk Prediction](https://arxiv.org/abs/2506.19363)
...
TL;DR: 研究探讨了乳腺X光检查中显式对齐的最佳方法，比较了输入空间与表示空间的对齐效果，发现图像级对齐优于表示级对齐。


<details>
  <summary>Details</summary>
Motivation: 调整高风险人群的筛查间隔需要准确跟踪乳腺组织随时间的变化，显式对齐方法在此过程中提供了更多控制，但其在乳腺X光检查中的最佳应用尚未明确。

Method: 研究比较了输入空间和表示空间的显式对齐方法，并探讨了对齐与风险预测是否应联合优化。

Result: 图像级对齐在变形场质量和风险预测准确性上优于表示级对齐，但联合优化会导致对齐质量与预测性能的权衡。

Conclusion: 图像级显式对齐是乳腺X光检查中更优的选择，能够提升变形场质量和风险预测准确性。
...
### [144] [NAADA: A Noise-Aware Attention Denoising Autoencoder for Dental Panoramic Radiographs](https://arxiv.org/abs/2506.19387)
...
<details>
  <summary>Details</summary>
Motivation: 传统卷积去噪自编码器（DAEs）在恢复高频细节（如牙科X光片中的细微解剖结构）时表现不佳，而现有注意力机制可能忽略噪声区域的关键特征。

Method: 提出噪声感知自注意力方法，结合噪声感知注意力增强去噪自编码器（NAADA），专注于恢复噪声区域的关键特征。

Result: 相比现有方法（如Uformer、MResDNN），NAADA在恢复高频细节和提升图像质量方面表现更优。

Conclusion: NAADA通过噪声感知注意力机制有效解决了传统方法在牙科X光片去噪中的局限性，提升了诊断准确性。
...
### [145] [Angio-Diff: Learning a Self-Supervised Adversarial Diffusion Model for Angiographic Geometry Generation](https://arxiv.org/abs/2506.19455)
...
<details>
  <summary>Details</summary>
Motivation: 血管造影X射线是诊断血管疾病的金标准，但其高辐射暴露问题亟待解决。现有方法因缺乏配对数据集和难以适应复杂血管结构，导致合成效果不佳。

Method: 采用扩散模型学习血管数据分布，结合生成器和基于掩码的对抗模块，并引入参数化血管模型提升几何准确性。

Result: 实验表明，该方法在合成血管造影图像质量和血管几何结构准确性上达到最优性能。

Conclusion: 提出的方法有效解决了数据不足和血管结构合成问题，为血管造影合成提供了新思路和数据集。
...
### [146] [Learning from Anatomy: Supervised Anatomical Pretraining (SAP) for Improved Metastatic Bone Disease Segmentation in Whole-Body MRI](https://arxiv.org/abs/2506.19590)
...
<details>
  <summary>Details</summary>
Motivation: 由于转移性骨病在全身MRI中的表现多样、边界模糊且类别不平衡，传统方法需要大量标注数据，而自监督学习的通用表征难以捕捉病变的细微特征。

Method: 开发了一种基于MRI的骨骼分割模型，先在健康个体的WB-MRI扫描上进行训练，再用于转移性前列腺癌患者的MBD分割任务。

Result: SAP方法在归一化表面Dice系数（0.76）和Dice系数（0.64）上显著优于基线和自监督学习方法，且在临床相关病变检测中表现出高灵敏度。

Conclusion: 从解剖学中学习骨骼形态为骨病变分割任务提供了有效的领域相关归纳偏置，SAP方法具有实际应用潜力。
...
### [147] [Filling of incomplete sinograms from sparse PET detector configurations using a residual U-Net](https://arxiv.org/abs/2506.19600)
*Klara Leffler,Luigi Tommaso Luppino,Samuel Kuttner,Karin Söderkvist,Jan Axelsson*

Main category: eess.IV

TL;DR: 提出了一种深度学习方法来恢复稀疏PET扫描仪的缺失数据，以降低成本，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 长轴向视野PET扫描仪成本高，限制了临床应用。稀疏配置可降低成本，但牺牲图像质量。

Method: 使用改进的Residual U-Net模型，训练于临床PET数据，模拟移除50%探测器，恢复缺失数据。

Result: 模型成功恢复缺失数据，误差低于每像素两个事件，优于2D插值，但图像细节稍显模糊。

Conclusion: 稀疏探测器配置结合深度学习是可行的替代方案，支持开发经济高效的全身体PET扫描仪。
...
### [148] [ReCoGNet: Recurrent Context-Guided Network for 3D MRI Prostate Segmentation](https://arxiv.org/abs/2506.19687)
*Ahmad Mustafa,Reza Rastegar,Ghassan AlRegib*

Main category: eess.IV

TL;DR: 提出了一种结合2D和3D方法的混合架构，用于前列腺MRI分割，提高了在数据有限和噪声条件下的分割一致性。


<details>
  <summary>Details</summary>
Motivation: 传统2D CNN方法未能利用切片间的解剖连续性，而完全3D模型需要大量标注数据，临床实践中不实用。

Method: 使用预训练的DeepLabV3提取单切片特征，并通过ConvLSTM层整合跨切片信息，保持空间结构。

Result: 在PROMISE12基准测试中，该方法在精度、召回率、IoU和DSC上优于现有2D和3D模型。

Conclusion: 该方法在临床数据有限和噪声条件下表现优异，具有实际应用潜力。
...
### [149] [NeRF-based CBCT Reconstruction needs Normalization and Initialization](https://arxiv.org/abs/2506.19742)
*Zhuowei Xu,Han Li,Dai Sun,Zhicheng Li,Yujia Li,Qingpeng Kong,Zhiwei Cheng,Nassir Navab,S. Kevin Zhou*

Main category: eess.IV

TL;DR: 论文提出了一种归一化哈希编码器和映射一致性初始化策略，以解决CBCT重建中局部-全局训练不匹配问题，提高了训练稳定性和重建质量。


<details>
  <summary>Details</summary>
Motivation: CBCT重建因投影数据有限而成为病态问题，现有NeRF方法因哈希编码器与神经网络的局部-全局训练不匹配导致特征不一致和训练不稳定。

Method: 提出归一化哈希编码器增强特征一致性，并设计映射一致性初始化策略（MCI）预训练神经网络以提高稳定性。

Result: 方法在128个CT案例上验证，显著提升训练效率和重建性能，覆盖7个解剖区域。

Conclusion: 归一化哈希编码器和MCI策略有效缓解局部-全局不匹配，实现更稳定训练和更高质量重建。
...
### [150] [Systematic Review of Pituitary Gland and Pituitary Adenoma Automatic Segmentation Techniques in Magnetic Resonance Imaging](https://arxiv.org/abs/2506.19797)
...
Result: 深度学习方法（尤其是U-Net）在腺瘤分割中表现较好，但小结构（如正常垂体）的分割仍需改进。自动方法的Dice分数范围较广。

Conclusion: 自动分割技术（如U-Net）有潜力，但需更多改进和多样化数据集以提高临床适用性。
...
### [151] [SOF: Sorted Opacity Fields for Fast Unbounded Surface Reconstruction](https://arxiv.org/abs/2506.19139)
*Lukas Radl,Felix Windisch,Thomas Deixelberger,Jozef Hladky,Michael Steiner,Dieter Schmalstieg,Markus Steinberger*

Main category: cs.GR

TL;DR: 提出了一种名为SOF的方法，通过层次化重排序和鲁棒的高斯深度公式，从3D高斯中恢复精确表面，并结合水平集正则化和并行化算法，显著提升了重建精度和速度。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖近似深度估计和全局排序启发式，容易引入伪影并限制网格保真度，因此需要一种更高效、精确的表面提取方法。

Method: 引入层次化重排序和鲁棒高斯深度公式，结合水平集正则化和几何一致性损失，开发并行化Marching Tetrahedra算法。

Result: SOF在重建精度上表现更优，同时将总处理时间减少三倍以上。
...
### [152] [Virtual Memory for 3D Gaussian Splatting](https://arxiv.org/abs/2506.19415)
*Jonathan Haberl,Philipp Fleck,Clemens Arth*

Main category: cs.GR

TL;DR: 提出了一种基于虚拟内存的大规模3D高斯泼溅场景渲染方法，通过动态流式传输和细节层次优化，显著降低内存占用并加速渲染。
...
Method: 利用虚拟内存和虚拟纹理技术，动态识别可见高斯并实时流式传输至GPU，结合细节层次优化进一步加速渲染。

Result: 减少了内存使用并显著提升了渲染速度，尤其在复杂场景中表现优异，适用于桌面和移动设备。
...
### [153] [Uncovering Conceptual Blindspots in Generative Image Models Using Sparse Autoencoders](https://arxiv.org/abs/2506.19708)
*Matyas Bohacek,Thomas Fel,Maneesh Agrawala,Ekdeep Singh Lubana*

Main category: cs.GR

TL;DR: 论文提出了一种系统方法，利用稀疏自编码器（SAEs）识别生成图像模型中的“概念盲点”，并通过量化分析揭示了模型在生成图像时的局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管生成图像模型在大规模数据集上表现优异，但在生成某些简单概念（如人手或四件物品）时仍存在明显缺陷。这些缺陷是否反映模型的固有局限性尚不明确。

Method: 采用稀疏自编码器（SAEs）提取可解释的概念嵌入，通过量化比较真实图像与生成图像中概念的普遍性，训练了一个包含32,000个概念的RA-SAE。

Result: 应用于四种流行生成模型（Stable Diffusion 1.5/2.1, PixArt, Kandinsky），发现了特定被抑制的盲点（如鸟食器、DVD光盘）和夸大的盲点（如木质背景纹理）。
...
### [154] [SoK: Can Synthetic Images Replace Real Data? A Survey of Utility and Privacy of Synthetic Image Generation](https://arxiv.org/abs/2506.19360)
*Yunsung Chung,Yunbei Zhang,Nassir Marrouche,Jihun Hamm*

Main category: cs.CR

TL;DR: 该论文综述了隐私保护数据合成（PPDS）中的合成图像生成方法，系统分类了生成-采样-分类流程中的技术、隐私攻击及缓解措施，并通过基准测试比较了不同方法的隐私风险与实用性。
...
Method: 系统分类现有图像合成方法、隐私攻击及缓解措施，并使用模型无关的成员推理攻击（MIA）作为隐私风险衡量标准进行基准测试。

Result: 通过实证比较，回答了合成数据能否替代真实数据、最佳发布策略、缓解措施是否改善实用性与隐私平衡等问题。

Conclusion: 研究为合成数据生成方法的实用性与隐私权衡提供了可操作的见解，并指导实际应用中的数据发布策略。
...
### [155] [Experimental Assessment of Neural 3D Reconstruction for Small UAV-based Applications](https://arxiv.org/abs/2506.19491)
...
TL;DR: 论文提出了一种将神经3D重建（N3DR）与小型无人机系统结合的方法，以提升静态小物体的3D重建质量，显著优于传统SfM算法。


<details>
  <summary>Details</summary>
Motivation: 小型无人机在室内和难以到达区域的部署潜力增加，但其飞行动态和功耗问题限制了自主性和任务能力。
...
### [156] [Chain-of-Experts: Unlocking the Communication Power of Mixture-of-Experts Models](https://arxiv.org/abs/2506.18945)
...
<details>
  <summary>Details</summary>
Motivation: 传统 MoE 模型中专家独立并行操作，限制了专家组合的多样性和表示能力。CoE 旨在通过迭代专家选择和动态路由机制解决这一问题。

Method: CoE 在每层内通过迭代步骤处理 token，每步使用专用路由器动态选择专家，支持 token 在不同迭代中选择不同专家。

Result: CoE 在固定计算下表现更优，数学推理任务中验证损失从 1.20 降至 1.12，同时内存使用减少 17.6-42%。

Conclusion: CoE 通过迭代残差结构和增强的专家专业化，解锁了更具表达力的表示，为模型扩展提供了新的深度维度。
...
### [157] [LLMs on a Budget? Say HOLA](https://arxiv.org/abs/2506.18952)
...
<details>
  <summary>Details</summary>
Motivation: 边缘设备上运行大型语言模型（LLM）面临计算和内存需求高的挑战，现有优化方法如量化和剪枝在速度或准确性上有所妥协。

Method: HOLA结合分层推测解码（HSD）和自适应检索（AdaComp-RAG），并采用LoBi（结合LoRA剪枝和量化）进行优化。

Result: 在GSM8K和ARC任务上分别提升17.6%和10.5%，同时降低延迟和内存占用。

Conclusion: HOLA框架在边缘设备上高效且可扩展，适合实际生产环境。
...
### [158] [Thought Anchors: Which LLM Reasoning Steps Matter?](https://arxiv.org/abs/2506.19143)
*Paul C. Bogdan,Uzay Macar,Neel Nanda,Arthur Conmy*

Main category: cs.LG

TL;DR: 论文提出三种互补的归因方法，通过句子级分析理解大语言模型的推理过程，发现并验证了“思维锚点”的存在。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的长链推理过程难以分解和理解，句子级分析为解决这一问题提供了新思路。

Method: 提出三种方法：黑盒法（反事实重要性）、白盒法（注意力模式聚合）和因果归因法（逻辑连接测量）。

Result: 发现“思维锚点”在推理过程中具有重要作用，通常是规划或回溯句子。

Conclusion: 句子级分析为理解推理模型提供了潜在方法，工具和案例研究验证了方法的有效性。
...
### [159] [In-Context Occam's Razor: How Transformers Prefer Simpler Hypotheses on the Fly](https://arxiv.org/abs/2506.19351)
*Puneesh Deora,Bhavya Vasudeva,Tina Behnia,Christos Thrampoulidis*

Main category: cs.LG

TL;DR: 本文研究了Transformer在分层任务结构中的上下文学习能力，发现其能选择最简复杂度解释，并通过贝叶斯框架解释了这一行为。


<details>
  <summary>Details</summary>
Motivation: 探索Transformer如何在多样复杂度任务中自适应学习，尤其是在分层结构中。

Method: 设计基于马尔可夫链和线性回归的测试平台，分析Transformer的复杂度选择能力。

Result: Transformer能准确推断任务复杂度及参数，偏好最简解释，并通过贝叶斯框架验证其行为。

Conclusion: Transformer在多样任务分布下表现出类似奥卡姆剃刀的归纳偏好，可能为其固有特性。
...
### [160] [Outlier-Safe Pre-Training for Robust 4-Bit Quantization of Large Language Models](https://arxiv.org/abs/2506.19697)
*Jungwoo Park,Taewhoo Lee,Chanwoong Yoon,Hyeon Hwang,Jaewoo Kang*

Main category: cs.LG

TL;DR: 论文提出Outlier-Safe Pre-Training (OSP)，通过预防极端激活异常值，提升大语言模型（LLM）的量化性能，实现高效部署。


<details>
  <summary>Details</summary>
Motivation: 极端激活异常值严重降低LLM的量化性能，阻碍设备端部署，现有方法难以有效解决。

Method: OSP结合三项创新：Muon优化器、Single-Scale RMSNorm和可学习嵌入投影，主动预防异常值形成。

Result: 训练1.4B参数模型，4位量化下平均得分35.7（优于Adam模型的26.5），训练开销仅2%，异常值接近零。

Conclusion: 异常值并非LLM固有特性，而是训练策略的结果，OSP为高效LLM部署开辟了新途径。
...
### [161] [Scaling Speculative Decoding with Lookahead Reasoning](https://arxiv.org/abs/2506.19830)
*Yichao Fu,Rui Ge,Zelei Shao,Zhijie Deng,Hao Zhang*

Main category: cs.LG

TL;DR: Lookahead Reasoning通过引入步骤级并行性，提升推理模型的解码速度，将SD的加速比从1.4倍提高到2.1倍。


<details>
  <summary>Details</summary>
Motivation: 推理模型生成长链式思维时解码速度慢，现有Token级推测解码（SD）的加速效果有限。

Method: 提出Lookahead Reasoning，利用轻量级草稿模型预测未来步骤，目标模型批量扩展步骤，验证器筛选语义正确的步骤。

Result: 在GSM8K、AIME等基准测试中，加速比从1.4倍提升至2.1倍，且答案质量不变。

Conclusion: Lookahead Reasoning通过步骤级并行性显著提升推理模型的解码效率。
...
### [162] [Orthogonal Finetuning Made Scalable](https://arxiv.org/abs/2506.19847)
...
<details>
  <summary>Details</summary>
Motivation: 正交微调（OFT）虽高效但计算和内存需求高，限制了实际应用。

Method: 提出OFTv2，采用输入中心化重构和矩阵向量乘法，引入Cayley-Neumann参数化近似矩阵求逆。

Result: 训练速度提升10倍，GPU内存使用降低3倍，且支持量化模型微调，优于QLoRA。

Conclusion: OFTv2在效率、内存和稳定性上显著改进，适用于量化模型微调。
...
### [163] [ConCM: Consistency-Driven Calibration and Matching for Few-Shot Class-Incremental Learning](https://arxiv.org/abs/2506.19558)
*QinZhe Wang,Zixuan Chen,Keke Huang,Xiu Su,Chunhua Yang,Chang Xu*

Main category: cs.LG

TL;DR: 论文提出了一种一致性驱动的校准与匹配框架（ConCM），通过优化特征-结构双一致性，解决少样本类增量学习中的知识冲突问题。
...
Method: 设计了基于海马体联想记忆的记忆感知原型校准和动态结构匹配，优化特征中心一致性和跨会话结构一致性。

Result: 在mini-ImageNet和CUB200基准测试中，ConCM表现最优，增量会话的调和准确率分别提升3.20%和3.68%。

Conclusion: ConCM通过几何最优性和最大匹配性，无需类数先验，显著提升了少样本类增量学习的性能。
...
### [164] [Noise Consistency Training: A Native Approach for One-Step Generator in Learning Additional Controls](https://arxiv.org/abs/2506.19741)
...
Conclusion: NCT是一种模块化、数据高效且易于部署的方法，显著提升了可控生成的效率和性能。
...
### [165] [TTSDS2: Resources and Benchmark for Evaluating Human-Quality Text to Speech Systems](https://arxiv.org/abs/2506.19441)
*Christoph Minixhofer,Ondrej Klejch,Peter Bell*

Main category: cs.SD

TL;DR: 论文提出了一种改进的TTS评估指标TTSDS2，解决了现有主观和客观指标在评估高质量合成语音时的不足，并发布了相关资源。


<details>
  <summary>Details</summary>
Motivation: 当前TTS系统的评估指标（如MOS）难以比较且资源密集，而客观指标缺乏主观验证。高质量合成语音的出现进一步挑战了现有指标。

Method: 提出TTSDS2指标，并通过多领域和多语言验证其与主观评分的相关性。同时发布数据集、测试管道和基准。

Result: TTSDS2在16个指标中唯一实现Spearman相关性高于0.50，适用于所有领域和语言。

Conclusion: TTSDS2是一种更鲁棒的TTS评估指标，并提供了配套资源以支持未来研究。
...
### [166] [Kling-Foley: Multimodal Diffusion Transformer for High-Quality Video-to-Audio Generation](https://arxiv.org/abs/2506.19774)
...
TL;DR: Kling-Foley是一个多模态视频到音频生成模型，通过扩散变换器和模块化设计实现高质量音频生成与视频同步。


<details>
  <summary>Details</summary>
Motivation: 解决视频与音频生成中的语义对齐和同步问题，提升多模态生成质量。

Method: 采用多模态扩散变换器、视觉语义表示模块和视听同步模块，结合文本条件生成音频。

Result: 在分布匹配、语义对齐、时间对齐和音频质量上达到SOTA性能。

Conclusion: Kling-Foley在多模态音频生成任务中表现出色，并开源了工业级评测基准。
...
### [167] [A Comment On "The Illusion of Thinking": Reframing the Reasoning Cliff as an Agentic Gap](https://arxiv.org/abs/2506.18957)
*Sheraz Khan,Subha Madhavan,Kannan Natarajan*

Main category: cs.AI

TL;DR: 论文指出大型推理模型（LRMs）在特定复杂度阈值后性能骤降（推理悬崖），但批评者认为这是实验设计限制导致的，而非模型本质缺陷。通过工具使用，模型能解决更复杂问题。


<details>
  <summary>Details</summary>
Motivation: 探讨LRMs性能骤降的原因，反驳其本质限制的观点，强调实验设计的局限性。

Method: 通过工具使用和代理能力重新评估模型性能，对比静态文本范式与动态工具辅助的表现。

Result: 工具辅助下模型能解决远超静态文本范式下的复杂问题，揭示代理推理的层次性。

Conclusion: LRMs的推理能力受限主要源于实验设计，而非模型本质，工具使用能显著提升其表现。
...
### [168] [Bayesian Evolutionary Swarm Architecture: A Formal Epistemic System Grounded in Truth-Based Competition](https://arxiv.org/abs/2506.19191)
*Craig Steven Wright*

Main category: cs.AI

TL;DR: 提出了一种基于概率代理的AI系统框架，通过结构化竞争和信念修正实现演化，结合贝叶斯推断和群体动力学，确保知识在对抗性压力下收敛于真理。


<details>
  <summary>Details</summary>
Motivation: 构建一个数学严谨的AI系统，通过竞争和信念修正使代理群体逐步逼近真理，解决知识演化的可计算性和稳定性问题。

Method: 基于贝叶斯推断和群体动力学，代理通过竞争和信念修正调整后验信念，利用哈希加密身份和因果推断算子确保系统可追溯性和一致性。

Result: 系统证明真理是演化吸引子，知识在对抗性压力下可验证且稳定收敛，提供形式化定理支持。

Conclusion: 该框架为可计算的自调节群体系统提供理论基础，展示对抗性压力如何促进知识的可验证性和稳定性。
...
### [169] [Skywork-SWE: Unveiling Data Scaling Laws for Software Engineering in LLMs](https://arxiv.org/abs/2506.19290)
...
TL;DR: 论文提出了一种自动化数据整理流程，用于扩展软件工程数据集，并展示了数据规模对模型性能的持续提升作用。


<details>
  <summary>Details</summary>
Motivation: 软件工程数据整理过程耗时且依赖人工，现有数据集规模有限，限制了LLM代理的研究进展。
...
Conclusion: 自动化数据整理流程显著提升了数据集规模和多样性，模型性能随数据规模持续提升，为未来研究提供了新基准。
...
### [170] [NaviAgent: Bilevel Planning on Tool Dependency Graphs for Function Calling](https://arxiv.org/abs/2506.19500)
*Yan Jiang,Hao Zhou,LiZhong GU,Ai Han,TianLong Li*

Main category: cs.AI

TL;DR: NaviAgent是一种基于图导航的双层规划架构，通过多路径决策和图编码导航器优化工具调用，显著提升任务成功率和效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态知识和单一执行路径，导致错误恢复能力差和搜索空间爆炸，无法有效处理复杂异构工具链。

Method: NaviAgent结合多路径决策器和图编码导航器，动态选择最优动作并构建工具依赖异构图，融合API结构和历史行为。

Result: 实验显示NaviAgent在任务成功率上显著优于基线方法，最高提升19%，且执行步骤接近最优基线。

Conclusion: NaviAgent通过双层规划和图导航策略，在复杂工具链编排中实现了高效和鲁棒性。
...
Method: 提出KnowRL方法，结合知识验证的事实性奖励，指导模型进行基于事实的慢思考。

Result: 在三个幻觉评估数据集和两个推理评估数据集上，KnowRL有效减少幻觉并保持推理能力。

Conclusion: KnowRL通过事实性奖励显著降低了慢思考模型的幻觉问题。
...
TL;DR: 研究利用视觉语言模型（VLMs）分析图表，以识别潜在的数据可视化问题，并比较了五种开源VLMs和五种提示策略的效果。


<details>
  <summary>Details</summary>
Motivation: 研究人员常因不熟悉或不遵循数据可视化原则，导致图表信息不准确或不完整。本研究旨在利用VLMs自动检测图表中的问题。

Method: 使用五种开源VLMs和五种提示策略，通过一组基于数据可视化指南的问题分析图表。

Result: VLMs在分析图表类型、3D效果、坐标轴标签等方面表现良好，但在图像质量和刻度标记方面表现较差。Qwen2.5VL模型和总结提示策略效果最佳。

Conclusion: VLMs可自动识别图表中的潜在问题，如缺失坐标轴标签或图例，该方法可扩展用于更多数据可视化场景。
...
### [173] [LLM-Based Social Simulations Require a Boundary](https://arxiv.org/abs/2506.19806)
*Zengqing Wu,Run Peng,Takayuki Ito,Chuan Xiao*

Main category: cs.CY

TL;DR: 本文主张为基于大语言模型（LLM）的社会模拟设定明确边界，以提升其对社会科学研究的贡献。尽管LLM在模拟人类行为方面优于传统方法，但其行为同质性问题限制了可靠性。作者提出三个关键边界问题（对齐性、一致性和鲁棒性），并建议关注集体模式、行为与真实数据对齐及验证方法。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM在社会模拟中的局限性，提出如何通过设定边界提升其科学价值。

Method: 分析LLM的局限性，提出三个边界问题（对齐性、一致性、鲁棒性），并给出实用检查表。

Result: 建议LLM模拟应关注集体模式、行为对齐和验证方法，而非个体轨迹。

Conclusion: 通过设定明确边界和验证方法，LLM模拟可更可靠地支持社会科学研究。
...
### [174] [Mix-of-Language-Experts Architecture for Multilingual Programming](https://arxiv.org/abs/2506.18923)
*Yifan Zong,Yuntian Deng,Pengyu Nie*

Main category: cs.PL

TL;DR: MoLE架构通过混合共享和语言特定的LoRA模块，平衡了多语言编程的效率与专业性。
...
Method: 采用基础模型、共享LoRA模块和语言特定LoRA模块的混合架构，联合优化。
...
### [175] [Convergent and divergent connectivity patterns of the arcuate fasciculus in macaques and humans](https://arxiv.org/abs/2506.19266)
...
TL;DR: 通过跨尺度单神经元追踪和扩散MRI，比较了猕猴和人类弓状束（AF）的解剖差异，揭示了人类AF在语言网络进化中的特异性。


<details>
  <summary>Details</summary>
Motivation: 研究猕猴和人类AF的解剖差异，以理解人类语言网络的进化基础及其相关疾病的神经解剖学框架。

Method: 结合病毒基因标记、荧光显微断层扫描和11.7T扩散MRI追踪猕猴AF，辅以人类7.0T MRI的光谱嵌入分析进行跨物种比较。

Result: 猕猴AF起源于颞顶皮层，而人类AF扩展到中颞回，前额和顶盖连接更强，支持人类语言处理的进化。

Conclusion: 人类AF的广泛颞叶整合和强化的额顶连接可能是高级语言处理的神经基础，并为AF相关疾病（如失语症和阅读障碍）提供了理解框架。
...
### [176] [From Web Search towards Agentic Deep Research: Incentivizing Search with Reasoning Agents](https://arxiv.org/abs/2506.18959)
...
Result: 基准测试和开源实现表明，Agentic Deep Research显著优于现有方法，并有望成为未来信息检索的主流范式。

Conclusion: Agentic Deep Research通过整合LLMs的推理能力，为信息检索领域带来了革命性的变革，并提供了丰富的资源支持社区发展。
...
### [177] [NEAR$^2$: A Nested Embedding Approach to Efficient Product Retrieval and Ranking](https://arxiv.org/abs/2506.19743)
*Shenbin Qian,Diptesh Kanojia,Samarth Agrawal,Hadeel Saadany,Swapnil Bhosale,Constantin Orasan,Zhe Wu*

Main category: cs.IR

TL;DR: NEAR²是一种嵌套嵌入方法，用于电商信息检索，能在推理时减少嵌入尺寸12倍，同时提升准确性。


<details>
  <summary>Details</summary>
Motivation: 解决电商检索系统在复杂查询和高效处理海量产品目录之间的双重挑战。

Method: 提出NEAR²方法，结合多种损失函数（如多负排名损失和在线对比损失）验证效果。

Result: 在多个测试集上表现优于现有模型，嵌入尺寸更小且准确性更高。

Conclusion: NEAR²在效率和准确性上均优于现有方法，适用于多种检索挑战。
