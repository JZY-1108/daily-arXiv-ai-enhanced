...
Result: LLM引导规划实现接近完美的准确率（micro-F1 > 0.90），嵌入引导神经搜索实现100倍以上的加速且保持竞争力准确率。通过知识蒸馏将规划能力压缩到4B参数模型中。

Conclusion: 可验证的多跳推理不需要大规模模型，而是需要结合符号结构与学习表示的适当架构归纳偏置，结构化规划比直接答案生成更具可迁移性。
...
TL;DR: 评估LLM在波斯语情感分类中生成解释的忠实性，发现模型解释与人类判断存在显著差异，提示策略对忠实性影响有限。
...
Method: 通过比较LLM识别的影响词与人类标注结果，使用基于token级对数概率的置信度评估忠实性，测试两种提示策略（先预测后解释 vs 先解释后预测）。

Result: LLM分类性能强但解释忠实性差，模型解释之间的一致性高于与人类判断的一致性，提示策略对忠实性影响有限。
...
TL;DR: 该论文提出语言理解需要将信息从核心语言系统导出到其他大脑区域，以构建丰富的心理模型，因为语言系统本身处理能力有限。


<details>
  <summary>Details</summary>
Motivation: 探索语言理解的深层认知和神经机制，超越表层意义提取，关注如何构建情境的心理模型。

Method: 回顾现有证据并提出假设，认为认知神经科学的进展为直接测试该假设提供了概念基础和方法。

Result: 提出了语言理解需要多脑区协作的新框架，核心语言系统负责初步处理，其他区域负责构建情境模型。

Conclusion: 该研究为揭示语言理解的认知和神经本质开辟了新策略，强调多脑区协同工作的重要性。
...
TL;DR: 该研究评估大语言模型在情绪心理理论中的性别偏见，并测试了多种去偏策略，发现基于训练的方法比提示工程更有效。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在日常生活中的广泛应用，评估和确保其公平性变得至关重要，特别是在情绪心理理论领域是否存在性别偏见。

Method: 通过向模型描述人物及其环境，询问"这个人感觉如何？"来评估性别偏见，并测试了多种去偏策略，包括提示工程和基于训练的方法。

Result: 研究发现大语言模型在情绪心理理论中存在性别偏见，基于训练的去偏方法比仅使用提示工程能更有效地减少偏见。
...
TL;DR: 本文提出了NRC VAD词典v2版本，新增了1万个英语多词表达及其组成词的效价、唤醒度和支配度人工评分，并将单词语料扩展到2.5万个词，增强了2018年v1版本的覆盖范围。


<details>
  <summary>Details</summary>
Motivation: 现有词典如2018年发布的NRC VAD词典主要包含单词的VAD关联评分，但缺乏对多词表达的情感分析，且需要更新近年更常用的词汇。

Method: 通过人工评分收集了10k个多词表达及其组成词的效价、唤醒度和支配度数据，并扩展了单词语料库，特别是2018年后更常见的词汇。

Result: 新词典v2版本包含10k个多词表达和25k个单词的条目，评分具有高可靠性。研究发现多词表达具有强烈的情感特征，并分析了其情感组合性。

Conclusion: 该词典为NLP、心理学、公共卫生、数字人文和社会科学等多个领域的研究提供了重要资源，可支持广泛的情感分析研究。
...
Result: 在SAfriSenti多语言情感语料库上，英语推文准确率66%，Sepedi推文69%，Setswana推文63%，平均只需修正34%的自动标注。

Conclusion: 该方法能有效减少人工标注工作量，为低资源语言情感分析提供可行解决方案。
...
TL;DR: 提出了PersonaPulse框架，通过迭代优化提示词来最大化LLM中个性表达，使用情境响应基准作为评分工具，实现更真实和情境化的个性建模。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽然探索了使用提示词来激发LLM的特定个性特征，但未能优化这些提示词以最大化个性表达，需要更有效的个性建模方法。

Method: PersonaPulse框架利用LLM对个性特征的固有知识，迭代增强角色扮演提示词，同时整合情境响应基准作为评分工具，指导优化过程。

Result: 定量评估显示PersonaPulse生成的提示词优于基于心理学研究设计的先前工作；探索了模型大小与个性建模的关系；发现某些个性特征的激发程度可以通过暂停优化过程来部分控制。

Conclusion: 提示词优化在塑造LLM个性表达中至关重要，为未来自适应AI交互研究提供了有价值的见解。
...
TL;DR: 评估大语言模型在医疗错误处理中的不同提示策略，发现检索增强动态提示(RDP)在错误检测和修正方面优于零样本提示和静态提示。


<details>
  <summary>Details</summary>
Motivation: 临床文档中存在可能危及患者安全的事实性、诊断性和管理性错误，需要有效的方法来检测和修正这些错误。

Method: 使用MEDEC数据集评估9个指令调优的大语言模型，比较零样本提示、静态随机示例提示和检索增强动态提示在错误标记检测、错误句子检测和错误修正三个子任务中的表现。

Result: 检索增强动态提示将假阳性率降低约15%，在错误句子检测中召回率提高5-10%，并生成更符合上下文的修正结果。

Conclusion: 检索增强动态提示在多样化的大语言模型中表现最佳，能提高检测准确性、减少假阳性并增强医疗错误修正的可靠性。
...
TL;DR: 提出了AppSelectBench基准测试，用于评估计算机使用代理的应用选择能力，包含10万+真实用户任务和100个桌面应用，发现现有模型在跨应用推理方面仍有困难。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估细粒度API选择，缺乏对模型在不同应用间进行推理和选择能力的评估，应用选择是计算机使用代理有效运行的基本能力。

Method: 开发了用户任务生成流水线，生成真实、多样且语义基础的用户意图，提供统一的评估协议，包括随机、启发式、零样本、少样本和检索增强设置。
...
TL;DR: R2R是一个领域感知的重排序框架，通过动态专家路由和两阶段训练策略解决领域专业化问题，在金融、法律等高风险领域超越通用模型和单领域微调方法。


<details>
  <summary>Details</summary>
Motivation: 通用重排序模型在高风险领域（如金融、法律）缺乏领域特定知识，而直接微调会导致表面形式过拟合和灾难性遗忘。

Method: 采用动态专家路由和两阶段训练策略（EAG），通过掩码最具预测性的表面线索来学习领域不变的相关性模式，并使用轻量级潜在语义路由器选择最优LoRA专家。

Result: 在多个重排序骨干模型和不同领域（法律、医疗、金融）的实验中，R2R始终优于通用模型和单领域微调基线。
...
TL;DR: 本文通过合成基准测试发现，即使在没有语言统计偏见的受控环境中，因果Transformer模型在方向性学习上存在固有优化差距，反转任务比正向任务更难学习。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer模型在方向性学习上的根本问题：是语言统计特性还是架构本身导致了方向性失败？通过完全合成的基准测试消除语言先验和语料库时间不对称性的影响。

Method: 使用随机字符串映射构建合成基准，通过可调分支因子K创建零条件熵的正向任务和具有分析确定熵下限的反向任务，训练GPT-2模型并测量超出熵下限的损失。

Result: 从头训练的GPT-2模型表现出强烈且可重现的方向性优化差距（如K=5时1.16 nats），远大于在相同数据上训练的MLP。预训练初始化改变了优化行为但未消除差距，LoRA在高熵反向映射上遇到容量墙。

Conclusion: 研究分离出了一个最小化的、无语义的方向性摩擦特征，这是因果Transformer训练固有的，即使在移除语言先验、词频和语料库级时间不对称性后仍然存在。
...
TL;DR: 本文提出了一个统一的多类别分类框架，用于从社交媒体数据中检测10种不同的心理健康和网络欺凌类别。通过端到端微调的MentalBERT模型在准确率和Macro F1得分上表现最佳，并开发了混合SHAP-LLM可解释性框架和原型仪表板。


<details>
  <summary>Details</summary>
Motivation: 数字空间中日益增长的心理健康挑战和网络欺凌问题需要可扩展且可解释的检测系统，以支持在线安全和计算心理健康领域的发展。

Method: 从Twitter和Reddit收集数据集，采用"分割后平衡"流程，在平衡数据上训练但在现实的不平衡测试集上评估。比较了传统词汇模型、混合方法和多种端到端微调transformer模型。

Result: 端到端微调对性能至关重要，领域适应的MentalBERT成为最佳模型，准确率达到0.92，Macro F1得分为0.76，超越了通用对应模型和零样本LLM基线。

Conclusion: 该系统被设计为人在环路的筛查辅助工具而非诊断工具。工作为在线安全和计算心理健康交叉领域提供了稳健基准，强调了未来对多标签、临床验证数据集的需求。
...
Method: 利用双曲表示开发在线概念学习框架，采用免训练的方式在测试时生成概念嵌入，使个性化视觉语言模型更具可扩展性和效率。

Result: 在包含1,292个概念和超过30K高质量实例的OP-Eval基准测试中表现出最先进的性能。

Conclusion: Online-PVLM框架有效解决了个性化视觉语言模型在线概念学习的可扩展性和效率问题，为实际应用提供了可行方案。
...
TL;DR: 提出了MTA框架解决PLLMs的个性化问题，通过合并-适应方法实现可扩展的个性化，避免为每个用户存储独立模块。


<details>
  <summary>Details</summary>
Motivation: 现有方法为每个用户微调独立模块存在存储成本线性增长和稀疏数据用户性能不佳的问题。

Method: 三阶段框架：构建共享元LoRA库、自适应LoRA融合、少样本个性化LoRA堆叠。

Result: 在LaMP基准测试中优于现有SOTA方法。

Conclusion: MTA框架有效解决了PLLMs的可扩展性和少样本个性化问题。
...
TL;DR: BiasPrompting是一种新颖的推理框架，通过引导LLMs生成并批判性评估所有可能答案选项的推理过程，来提升多项选择题任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多项选择题任务中存在局限性：答案选项通常缺乏上下文背景或解释，导致无法充分探索所有可能答案，从而降低了模型的推理能力。

Method: BiasPrompting包含两个组件：1) 推理生成阶段，模型为每个答案选项生成支持性推理；2) 推理引导的一致性阶段，综合生成的推理来选择最合理的答案。

Result: 在五个广泛使用的多项选择题回答基准测试中，BiasPrompting显示出显著改进，特别是在现有方法表现不佳的复杂和具有挑战性的问题设置中。
...
TL;DR: SSA是一种统一的稀疏注意力训练框架，通过双向对齐稀疏和全注意力，解决了现有稀疏注意力方法中梯度更新不足的问题，实现了在稀疏和全注意力推理下的最优性能，并改善了长上下文外推能力。


<details>
  <summary>Details</summary>
Motivation: 解决全注意力的二次复杂度问题，同时克服现有稀疏注意力方法训练时梯度更新不足导致的性能下降问题。

Method: 提出SSA训练框架，同时考虑稀疏和全注意力，在每一层强制双向对齐，保持所有token的梯度流，并鼓励稀疏注意力输出与全注意力输出对齐。

Result: 在多个常识基准测试中，SSA在稀疏和全注意力推理下均达到最优性能，能够平滑适应不同的稀疏预算，并展现出最强的长上下文外推能力。

Conclusion: SSA通过统一的训练框架有效解决了稀疏注意力中的梯度更新问题，实现了性能与计算效率的良好平衡，并显著改善了长上下文处理能力。
...
Method: 收集来自在线平台的自发情感表达，在32个类别上进行细粒度情感分布标注，使用自监督学习模型进行实验基线评估。

Result: HuBERT-large-EN模型在说话人独立、性别、年龄和个性评估中表现最优，展示了稳健性能。

Conclusion: EM2LDL通过整合语言多样性和生态效度，为多语言环境中复杂情感动态研究提供了多功能测试平台，支持情感计算应用开发。
...
TL;DR: 提出了一种无需训练的发音错误检测与诊断框架，利用预训练ASR模型和检索技术，避免传统方法需要训练音素级模型或评分模型的复杂性。
...
Method: 使用预训练自动语音识别模型结合检索技术，无需额外训练或音素特定建模，实现发音错误的检测和诊断。

Result: 在L2-ARCTIC数据集上达到69.60%的F1分数，优于传统方法且避免了模型训练的复杂性。

Conclusion: 该方法提供了一种高效、无需训练的发音错误检测解决方案，在保持准确性的同时显著降低了实现复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 印地语系语言的语法错误校正面临资源有限、语言多样性和复杂形态学的挑战，而基于Transformer的模型和大型标注数据集主要针对英语等高资源语言。
...
Conclusion: 精心设计的提示和轻量级适配显著提高了多种印地语系语言的校正质量，证明了大规模LLMs在多语言GEC中弥合资源差距的潜力。
...
TL;DR: 该论文通过将图像数据增强技术集成到基于网格的模型中，解决了不连续命名实体的分割和遗漏问题，显著提升了识别性能。


<details>
  <summary>Details</summary>
Motivation: 传统命名实体识别方法在处理跨句不连续实体时存在分割错误和遗漏问题，严重影响识别准确率。

Method: 将图像数据增强技术（裁剪、缩放、填充）集成到基于网格的标记模型中，增强对不连续实体的识别能力。

Result: 在CADEC、ShARe13和ShARe14数据集上，整体F1分数提升1-2.5%，不连续实体识别提升3.7-8.4%。

Conclusion: 基于网格的模型结合数据增强技术能有效解决不连续实体的分割挑战，显著提升命名实体识别性能。
...
TL;DR: 开发了首个公开的吉尔吉斯语单语BERT模型KyrgyzBERT，包含35.9M参数和定制分词器，在情感分析任务上表现与更大模型相当


<details>
  <summary>Details</summary>
Motivation: 吉尔吉斯语作为低资源语言缺乏基础NLP工具，需要开发专门的预训练模型

Method: 构建KyrgyzBERT模型，使用定制分词器适应语言形态结构，创建kyrgyz-sst2情感分析基准数据集

Result: KyrgyzBERT在情感分析任务上F1得分为0.8280，与参数量5倍大的mBERT模型表现相当

Conclusion: 成功开发了首个吉尔吉斯语BERT模型，为吉尔吉斯语NLP研究提供了重要工具
...
TL;DR: REFLEX是一个用于自动化事实核查的即插即用范式，通过利用骨干模型的内部知识，在无需外部知识源的情况下提高裁决准确性和解释质量。
...
Method: 将事实核查重构为角色扮演对话，联合训练裁决预测和解释生成，通过提取骨干模型与其微调变体之间的对比激活对来构建引导向量，自然地将真相分解为风格和实质。

Result: 在真实世界数据集上的实验表明，REFLEX优于先前方法，仅用465个自精炼训练样本就达到最先进性能，解释性目标训练的模型可有效指导无此目标的模型，提升达7.57%。

Conclusion: REFLEX证明了内部解释信号在事实推理中既具有解释作用又能增强推理能力的双重作用，为实时事实核查提供了高效可靠的解决方案。
...
TL;DR: SpecFormer是一种新颖的架构，结合了单向和双向注意力机制，通过并行生成草稿序列来加速LLM推理，无需依赖大型前缀树，在低验证资源和调度成本下实现持续加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法假设有大量计算资源，但在批处理等主流推理系统中，可用空闲计算能力被压缩，因此需要能在低验证资源和低调度成本下进行推测解码的方法。

Method: 提出SpecFormer架构，集成单向和双向注意力机制，结合自回归模型从完整输入序列提取信息的能力和非自回归模型的并行生成优势，消除对大型前缀树的依赖。

Result: 在各种规模模型的实验中，SpecFormer为扩展LLM推理设定了新标准，具有更低的训练需求和计算成本。

Conclusion: SpecFormer通过创新的架构设计，在低资源环境下实现了持续加速，为LLM推理优化提供了有效解决方案。
...
TL;DR: LLMs能够编码类比实体间的关系，但在应用关系信息到新实体时存在困难，通过隐藏表示修补可以部分改善，成功类比需要强结构对齐。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs是否能够编码高层次关系概念并将其应用到新情境中，通过结构化比较来理解类比推理能力。

Method: 使用比例类比和故事类比任务，分析LLMs隐藏层表示，通过修补关键token位置的隐藏表示来测试信息传递。

Result: LLMs能够有效编码类比关系，但在应用关系信息到新实体时表现不佳，成功案例显示强结构对齐，失败案例显示对齐退化。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在广泛的多语言基准测试中表现出色，但在低资源语境下的比喻和文化推理评估仍然不足，特别是在孟加拉语这样的低资源语言中。

Method: 构建包含435个独特谜语的BengaliFig数据集，每个项目沿五个正交维度进行标注，并通过约束感知的AI辅助流程自动转换为多项选择格式。评估了八个前沿LLM在零样本和少样本思维链提示下的表现。

Result: 揭示了LLM在隐喻和文化特定推理方面的一致弱点，表明现有模型在低资源文化语境下的推理能力有限。

Conclusion: BengaliFig既为评估LLM在低资源文化背景下的鲁棒性提供了诊断工具，也朝着包容性和文化遗产意识的NLP评估迈出了一步。
...
TL;DR: 本文提出了一种新颖的任务导向词干提取评估框架，包含词干提取有效性得分(SES)、模型性能差异(MPD)和平均归一化编辑距离(ANLD)三个维度，能够区分效率提升和语义保持。
...
Method: 提出包含三个方面的综合评估框架：SES评估词干提取效用，MPD评估对下游任务的影响，ANLD评估词干化词与原词的语义相似度。

Result: 孟加拉语词干提取器SES最高(1.67)但ANLD显示存在有害的过度词干提取(0.26)，而英语词干提取器SES适中(1.31)且语义距离安全(ANLD=0.14)，对下游任务有积极贡献。

Conclusion: 该框架能有效区分潜在效率增益和语义保持，英语词干提取器更可靠，为词干提取方法评估提供了有价值的工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决在没有配对数据时训练生成模型以及不依赖人类判断评估风格文本的挑战，探索大语言模型在风格学研究中的应用。

Method: 使用最小化单token提示微调大语言模型生成特定作者风格的文本，训练基于transformer的检测器进行分类和风格解释，结合句法比较和可解释AI方法分析语言线索。

Result: 生成的文本成功反映了作者的独特写作模式，AI驱动的评估为人类评估提供了可靠替代方案。

Conclusion: 该框架有效实现了19世纪小说家风格的文本生成和评估，展示了AI在风格学研究中的实用价值。
...
TL;DR: 提出了一种针对多模态大语言模型的新型对抗攻击——对抗性混淆攻击，旨在诱导模型产生不连贯或自信错误的输出，从而破坏MLLM驱动代理的可靠运行。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击主要关注越狱或目标误分类，而该研究旨在开发能系统性地破坏MLLM可靠性的攻击方法，防止MLLM代理在嵌入对抗图像的网站上正常运行。

Method: 使用开源MLLM的小型集成，通过最大化下一个标记的熵来生成对抗图像。在自盒设置下，采用基本对抗技术（PGD）生成可迁移到未见模型的扰动。

Result: 单个对抗图像能够破坏集成中的所有模型，包括完整图像和对抗CAPTCHA设置。生成的扰动可迁移到未见开源模型（如Qwen3-VL）和专有模型（如GPT-5.1）。

Conclusion: 对抗性混淆攻击是一种有效的威胁，能够系统性地破坏MLLM的可靠性，且具有很好的迁移性，突显了MLLM安全性的脆弱性。
...
TL;DR: 本文介绍了文本失语症电池(TAB)，这是一个基于临床失语症评估改编的文本基准，用于评估大型语言模型中的失语症样语言缺陷。


<details>
  <summary>Details</summary>
Motivation: 传统临床评估不适合评估LLMs，因为它们预设了人类语用压力并探测人工架构不具备的认知过程。需要专门工具来研究LLMs中的语言障碍。

Method: 从快速失语症电池(QAB)改编开发TAB，包含四个子测试：连贯文本、单词理解、句子理解和重复。使用Gemini 2.5 Flash验证自动化评估协议。

Result: 自动化评估协议达到与专家人类评分者相当的可靠性（模型-共识一致性Cohen's kappa = 0.255 vs 人类-人类一致性0.286）。

Conclusion: TAB为分析人工系统中的语言缺陷提供了一个基于临床的、可扩展的框架。
...
TL;DR: 提出一种新的语音数据增强技术，旨在缩小低资源语言与英语等资源丰富语言在语音识别性能上的差距。
...
Result: 实验表明该方法显著提高了低资源语言自动语音识别系统的性能，且优于现有的增强策略。

Conclusion: 该方法为提升代表性不足语言社区的语音技术提供了实用解决方案。
...
TL;DR: 该论文构建了一个教育对话数据集，用于自动识别学生对话中的知识建构和任务完成话语特征，并测试了GPT-3.5和Llama-3.1等预训练模型在此任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 教育研究者需要识别学生对话中的话语特征来分析教学效果，但人工分析耗时耗力。现有NLP研究很少关注教育对话数据，因此需要开发自动检测方法。

Method: 构建了标注的教育对话数据集，包含知识建构和任务完成话语特征，并使用GPT-3.5和Llama-3.1等预训练大语言模型建立基线模型来预测对话中的话语属性。

Result: 实验结果表明，这些最先进的模型在此任务上表现不佳，显示该任务具有挑战性。

Conclusion: 当前模型在教育对话话语特征识别任务上仍有改进空间，为未来研究提供了方向。
...
TL;DR: 本文提出AlignEval基准，通过评估LLM作为评判者的能力来间接衡量其与人类偏好的一致性，避免了直接评估生成输出的需求。研究发现生成与评估能力存在强相关性，新基准在LLM排名方面表现优于现有自动评估方法。


<details>
  <summary>Details</summary>
Motivation: 评估LLM与人类偏好的一致性通常需要直接评估其开放域生成结果，这需要大量人工标注或强LLM评判者。本文旨在探索LLM生成能力与评估能力之间的关系，并开发更高效的评估方法。

Method: 首先分析各种LLM的生成-评估一致性，发现两者在强LLM偏好预言机评估下存在强相关性。基于此提出AlignEval基准，通过评估LLM作为评判者的能力来间接衡量其对齐程度。

Result: AlignEval基准在捕捉人类偏好和排名LLM方面，匹配或超越了AlpacaEval和Arena-Hard等广泛使用的自动评估基准。生成与评估能力之间存在强相关性。

Conclusion: 研究揭示了LLM生成与评估能力之间的重要联系，提出的AlignEval基准提供了一种无需直接评估模型输出的对齐评估方法，为LLM评估提供了新视角。
...
TL;DR: LatentMAS是一个无需训练的端到端框架，通过潜在空间协作实现多智能体系统，相比基于文本的MAS具有更高表达能力和无损信息交换，在推理质量和效率上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体依赖基于文本的中介进行推理和通信，这限制了系统级智能的发展。作者希望让模型能够在连续潜在空间中直接协作，实现更高效的多智能体系统。

Method: LatentMAS框架包含两个核心组件：1）每个智能体通过最后一层隐藏嵌入进行自回归潜在思维生成；2）共享潜在工作内存保存和传输每个智能体的内部表示，确保无损信息交换。

Result: 在9个基准测试中，LatentMAS在数学和科学推理、常识理解和代码生成方面表现优于单模型和基于文本的MAS基线，准确率提高14.6%，输出token使用减少70.8%-83.7%，端到端推理速度提升4-4.3倍。

Conclusion: LatentMAS通过潜在协作框架显著提升了系统级推理质量，同时在不需额外训练的情况下实现了显著的效率增益，为多智能体系统的发展提供了新方向。
...
TL;DR: 提出PuzzlePole圆柱形标记物，基于PuzzleBoard校准模式，支持360度视角的可靠识别和姿态估计，具有高精度和遮挡鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自主系统需要可靠的环境感知，其中校准和定位任务通常依赖鲁棒的视觉标记。

Method: 利用PuzzleBoard模式的独特组合结构，设计圆柱形标记物PuzzlePole，实现360度视角的识别和姿态估计。

Result: PuzzlePole在定位和定向方面提供高精度，同时对遮挡具有鲁棒性。

Conclusion: 该设计为自主系统（从机器人导航、SLAM到有形界面）提供了灵活的部署方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型的评估方法（通用奖励函数或基于相似性的指标）无法捕捉个人视觉喜好的多样性和复杂性，需要开发能够评估与个人用户偏好对齐的方法。

Method: 采用自引导策略，通过有限参考数据进行推理构建丰富用户上下文，使用思维链推理动态生成用户条件评估维度，无需用户特定训练即可实现个性化评估。

Result: PIGReward在准确性和可解释性方面超越现有方法，建立了可扩展的基于推理的个性化T2I评估和优化基础。

Conclusion: PIGReward是朝着个体对齐的文本到图像生成迈出的坚实一步，提供了强大的个性化评估和优化能力。
...
Method: 提出SG-OIF框架：使用随机Richardson和预处理Neumann维护轻量级锚点IHVPs；通过稳定性引导的残差阈值、异常门控和置信度模块化曲率后端来调节每个样本的影响力得分。

Result: 在CIFAR-10（20%不对称噪声）上，前1%预测样本达到91.1%准确率；在MNIST上获得99.8%的AUPR分数，在多个数据集的各种损坏情况下均表现优异。

Conclusion: SG-OIF是首个将算法稳定性作为实时控制器的在线影响力估计框架，为在线影响力估计提供了实用的控制器解决方案。
...
Result: Pistachio基准在规模、多样性和复杂性方面表现出色，揭示了现有方法面临的新挑战。

Conclusion: 该基准为动态和多事件异常理解研究提供了新的动力，展示了生成式方法在构建复杂视频基准方面的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理跟踪和分割任务时使用专门的架构或模态特定参数，限制了泛化性和可扩展性。现有统一方法忽视了跨模态分布差异和跨任务特征表示差距这两个关键挑战。
...
TL;DR: 本文提出了一种基于行列式比值矩阵(DRaM)的新方法，用于解决3D-3D(EnP)和3D-2D正交投影(OnP)的姿态估计问题，并将现有方法统一到DRaM框架中。


<details>
  <summary>Details</summary>
Motivation: 姿态估计是计算机视觉中的重要问题，现有方法如SVD和四元数特征系统能精确解决带噪声的3D-3D对齐问题，但对于3D-2D正交投影问题缺乏类似的闭式解。

Method: 使用行列式比值矩阵(DRaM)方法求解无误差EnP和OnP问题的最小二乘系统，对于带噪声数据采用直接的旋转校正方案。

Result: 提出了EnP和OnP旋转估计问题的DRaM家族解决方案，并与现有方法进行了比较分析。

Conclusion: 这项工作不仅为3D和2D正交投影姿态估计提供了新解决方案，还对这些问题的类别提供了有价值的见解，DRaM方法可推广到所有类似的N维欧几里得姿态估计问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动3D生成方法难以同时实现快速、详细和高保真的生成效果，需要一种能平衡这些要求的解决方案。

Method: 使用预训练变分自编码器将3D几何映射到潜在特征，然后通过流水线顺序生成粗略几何、精细几何和真实纹理。

Result: 生成的3D物体对输入图像具有高保真度，整个生成过程可在70秒内完成，在少量训练下展现出与当代方法竞争的性能。

Conclusion: LatentDreamer通过潜在特征表示有效降低了3D生成的难度，实现了快速且高质量的3D物体生成。
...
<details>
  <summary>Details</summary>
Motivation: 受最小描述长度原理启发，研究模型容量与视觉语义空间所需最小token数量的基本关系，旨在理解图像语义复杂性的本质。

Method: 将图像token重新解释为视觉语义空间中的向量，定义图像内在语义复杂性为跨越该空间所需的最小基向量集。提出正交过滤方法，自适应地将冗余token聚类为紧凑的正交基集。

Result: 在多种ViT模型上的实验揭示了token与模型规模的缩放规律：更大的模型需要显著更少的token来跨越视觉语义空间。同时贡献了一个视觉长上下文数据集。

Conclusion: 模型容量与所需视觉token数量存在明确的缩放关系，提出的正交过滤方法能有效压缩冗余token，为理解视觉语义复杂性提供了新视角。
...
TL;DR: 提出了GroundingAgent，一种无需任务特定微调的代理视觉定位框架，通过集成预训练组件实现零样本视觉定位，在多个基准测试中达到65.1%的平均准确率。
...
Conclusion: GroundingAgent证明了无需微调即可实现有效视觉定位的可行性，同时提供强可解释性，展示了LLM推理能力的关键作用。
...
TL;DR: 提出InfoPrune信息理论框架，用于视觉语言模型的自适应结构压缩，基于信息瓶颈原理平衡任务相关语义保留和冗余依赖去除。


<details>
  <summary>Details</summary>
Motivation: 现有压缩方法依赖启发式重要性度量或经验剪枝规则，缺乏信息保留的理论保证，而视觉语言模型规模不断增长带来部署和效率挑战。

Method: 基于信息瓶颈原理，引入基于熵的有效秩(eRank)和KS距离量化注意力头贡献，提出训练引导的注意力头剪枝和训练自由的FFN自适应低秩近似两种压缩方案。

Result: 在VQAv2、TextVQA和GQA数据集上实现最高3.2倍FLOP减少和1.8倍加速，性能损失可忽略。
...
TL;DR: 提出了一种新的眼睑角度(ELA)指标，用于可靠检测驾驶员疲劳状态，该指标基于3D面部特征点，比传统2D方法更稳定，并可用于生成合成数据集。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶员疲劳检测方法存在可靠性不足的问题，传统2D指标如EAR对摄像头角度变化敏感，且自然疲劳数据收集困难且危险。

Method: 使用3D面部特征点计算眼睑角度(ELA)，设计眨眼检测框架提取时间特征，并利用ELA信号在Blender 3D中生成逼真的合成数据集。

Result: 在公开驾驶员监控数据集上的实验表明，ELA在视角变化下比EAR具有更低的方差，实现了准确的眨眼检测，合成数据增强扩展了训练数据的多样性。
...
TL;DR: VideoChat-M1提出了一种多智能体协作策略规划框架，通过动态策略生成、执行和通信机制，解决了视频理解中静态工具调用限制的问题，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体视频理解系统大多采用静态、不可学习的工具调用机制，限制了在时空复杂视频中发现多样化线索的能力，影响了感知和推理的鲁棒性。

Method: 采用协作策略规划(CPP)范式，包含三个关键过程：策略生成（每个智能体生成独特的工具调用策略）、策略执行（顺序调用工具探索视频内容）、策略通信（执行过程中智能体交互更新策略），并配备多智能体强化学习进行联合优化。

Result: 在8个基准测试的4个任务中达到SOTA性能，在LongVideoBench上比Gemini 2.5 pro高出3.6%，比GPT-4o高出15.6%。

Conclusion: VideoChat-M1通过动态协作策略规划机制，显著提升了多智能体视频理解系统的性能，证明了协作策略规划和强化学习在复杂视频理解任务中的有效性。
...
TL;DR: 提出了Perceptual Taxonomy基准，用于评估物理基础的视觉推理能力，包含3173个对象的84个精细属性标注和28033个模板问题，发现现有视觉语言模型在属性驱动问题上表现下降10-20%


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言基准缺乏对结构化场景理解的全面评估，主要关注表面识别或图像文本对齐，而人类认知需要进行对象识别、空间配置和任务相关属性推理的目标导向推理

Method: 构建Perceptual Taxonomy基准，标注3173个对象的4个属性家族共84个精细属性，创建5802张图像的多选题基准，包含28033个模板问题和50个专家设计问题

Result: 领先的视觉语言模型在识别任务上表现良好，但在属性驱动问题上性能下降10-20%，特别是在需要结构化属性多步推理的问题上

Conclusion: 当前模型在结构化视觉理解方面存在持续差距，主要依赖模式匹配，而基于模拟场景的上下文推理示例能提高真实世界和专家策划问题的性能
...
TL;DR: MapRF是一个弱监督框架，仅使用2D图像标签学习构建3D地图，通过条件NeRF模块生成高质量伪标签，在自训练过程中逐步改进地图网络，无需额外监督。
...
Method: 提出条件NeRF模块生成视图一致的3D几何和语义伪标签，采用自训练方式迭代优化地图网络，并使用Map-to-Ray匹配策略减轻误差累积。

Result: 在Argoverse 2和nuScenes数据集上的实验表明，MapRF达到与全监督方法相当的性能，约为基线的75%，超越了仅使用2D标签的多种方法。

Conclusion: MapRF有潜力实现可扩展且经济高效的在线高精地图构建，为自动驾驶提供支持。
...
TL;DR: Vidi2是一个先进的视频理解模型，具备细粒度时空定位能力，在时空检索和视频问答任务中表现出色，超越了Gemini 3 Pro和GPT-5等领先专有系统。


<details>
  <summary>Details</summary>
Motivation: 视频已成为互联网通信和创作的主要媒介，需要可扩展、高质量的视频生产解决方案。Vidi2旨在推进视频理解能力，实现端到端的时空定位。

Method: Vidi2模型扩展了视频理解能力，包括细粒度时空定位和视频问答。通过文本查询，能够识别对应时间戳和边界框，支持复杂编辑场景。

Result: Vidi2在VUE-TR-V2和VUE-STG基准测试中显著优于Gemini 3 Pro和GPT-5等专有系统，在视频问答基准上与类似规模的开源模型竞争激烈。

Conclusion: Vidi2代表了视频理解技术的重大进步，其端到端时空定位能力为复杂视频编辑应用开辟了新可能性。
...
TL;DR: 本研究探讨了多模态大语言模型在分布式光伏系统全球评估中的跨域泛化能力，通过结构化提示和微调，在未见区域实现了优于传统计算机视觉方法的性能。
...
Result: 跨区域评估显示，所提模型在未见区域表现退化最小，优于传统计算机视觉和transformer基线方法。
...
TL;DR: 该论文提出了大规模分析制图遗产的方法和数据集，从文化视角研究地图作为语义符号系统和反映政治认知期望的文化对象。


<details>
  <summary>Details</summary>
Motivation: 现有自动化地图分析技术很少与制图史结合，也未能将地图视为反映政治和认知期望的文化对象。该研究旨在从文化视角大规模分析制图遗产。

Method: 整合来自38个数字目录的771,561条地图记录和99,715张数字化图像，使用语义分割和目标检测模型进行土地类别和制图符号的通用识别，并分析合作与传播模式。

Result: 揭示了制图的空间焦点与政治动态的关联，记录了国家化趋势和军事冲突对出版量的影响，发现地图是经过设计的图像，其构图通过中心和语义对称强调特征。

Conclusion: 该研究为大规模制图遗产分析提供了方法和数据集，揭示了制图作为文化实践的本质，强调了合法性、大型参与者和主要城市在传播图形规范和符号文化中的作用。
...
<details>
  <summary>Details</summary>
Motivation: 现有高斯溅射变形方法依赖代理（如笼子或网格），存在代理质量依赖和额外计算开销问题；而直接应用拉普拉斯变形技术会因缺乏显式结构而无法正确捕捉表面信息
...
Conclusion: SpLap方法通过表面感知的溅射图和拉普拉斯算子，实现了更真实的高斯溅射变形，能有效保持细节和拓扑结构
...
TL;DR: ThiFAN-VQA是一个用于灾害场景视觉问答的两阶段推理框架，通过思维链提示和上下文学习生成结构化推理轨迹，结合答案选择模块提升模型性能，在无人机灾害评估数据集上表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI灾害评估方法存在数据标注成本高、数据集有限、传统分类框架答案空间固定、生成模型易产生幻觉或通用回答等问题，需要开发更灵活且准确的解决方案。

Method: 提出两阶段推理框架：第一阶段使用思维链提示和上下文学习生成结构化推理轨迹；第二阶段通过答案选择模块评估和选择最一致、上下文准确的答案；集成自定义信息检索系统、领域特定提示和推理引导答案选择。

Result: 在FloodNet和RescueNet-VQA两个无人机灾害数据集上的实验表明，ThiFAN-VQA在准确性、可解释性和适应性方面均优于现有方法。

Conclusion: ThiFAN-VQA通过结合零样本和监督方法的优势，在有限监督下实现了灵活且一致的灾害评估性能，为现实世界灾后损害评估任务提供了有效解决方案。
...
TL;DR: HunyuanOCR是一个商业级开源轻量级视觉语言模型，专门用于OCR任务，在1B参数下实现了超越商业API和更大模型的性能表现。
...
Method: 采用原生视觉Transformer和轻量级LLM架构，通过MLP适配器连接，使用端到端范式消除预处理依赖，并应用数据驱动和强化学习策略。

Result: 在ICDAR 2025 DIMT挑战赛小型模型赛道获得第一名，在OCRBench上达到SOTA水平，性能超越商业API和Qwen3-VL-4B等更大模型。

Conclusion: HunyuanOCR为前沿研究和工业应用提供了坚实基础，实现了多功能、高效率的OCR解决方案，并已在HuggingFace开源。
...
<details>
  <summary>Details</summary>
Motivation: 缺血性卒中是时间紧迫的医疗紧急情况，非对比增强CT作为一线成像工具在早期阶段往往无法显示细微的缺血变化，这会延迟关键干预措施。

Method: 使用生成对抗网络框架，结合少量标注数据和大量未标注数据进行半监督分割。采用Dice损失、交叉熵损失、特征匹配损失和自训练损失来训练模型识别和描绘早期梗死区域。

Result: 在公开的急性缺血性卒中数据集（AISD）上的实验表明，该方法能够增强诊断能力，减少手动标注负担，并支持更高效的卒中护理临床决策。

Conclusion: 该半监督分割方法有潜力改善早期缺血性卒中的诊断，特别是在梗死区域微弱或体积较小的情况下，为临床决策提供更有效的支持。
...
TL;DR: 提出了一种基于VAE的多尺度向量量化变分自编码器(MSVQ-VAE)方法，用于生成无线胶囊内窥镜(WCE)图像，特别是能够无缝地将异常引入正常WCE图像中，支持条件生成多种异常类型。


<details>
  <summary>Details</summary>
Motivation: 胃肠道影像通过WCE产生大量需要人工筛查的图像，但医疗数据稀缺（由于隐私限制和标注成本）阻碍了临床决策支持系统的发展，需要生成式机器学习来解决这一限制。

Method: 开发了多尺度向量量化变分自编码器(MSVQ-VAE)，能够无缝地将异常引入正常WCE图像，支持条件生成多种异常类型（息肉、血管和炎症等）。

Result: 通过图像分类评估生成图像的效用，实验表明使用该方法生成的异常图像训练的CDS分类器与仅使用真实数据训练的分类器结果相当。

Conclusion: 该方法在WCE图像生成方面表现出色，能够有效解决医疗数据稀缺问题，其通用性有望应用于各种医学多媒体领域。
...
TL;DR: SkillSight是一个用于第一人称视角技能评估的系统，通过结合注视数据和视频来预测技能水平，然后蒸馏出仅需注视数据的轻量模型，大幅降低功耗。


<details>
  <summary>Details</summary>
Motivation: 解决智能眼镜上自动技能评估的技术挑战，利用注视数据来理解技能水平，同时降低功耗以实现实时应用。

Method: 采用两阶段框架：首先联合建模注视和第一人称视频来预测技能水平，然后蒸馏出仅需注视输入的学生模型。

Result: 在烹饪、音乐和体育三个数据集上验证了注视在技能理解中的价值。教师模型达到最先进性能，仅注视的学生模型保持高准确率且功耗降低73倍。

Conclusion: 注视数据在跨领域技能理解中具有重要价值，SkillSight为实现野外AI辅助技能学习铺平了道路。
...
TL;DR: 本研究提出了一种基于视觉语言基础模型的语义分布引导重建框架，通过对比学习目标将重建图像与目标语义分布对齐，显著提升了欠采样MRI重建的感知质量和解剖结构保留能力。
...
Method: 提出语义分布引导重建框架，使用预训练的视觉语言基础模型将重建图像和辅助信息编码为高级语义特征，通过对比目标确保重建表示与目标语义分布一致。

Result: 在膝关节和脑部数据集上的实验表明，基于图像的语义先验能保留精细解剖结构，获得更优的感知质量（更低的LPIPS值、更高的Tenengrad分数和读者研究评分），图像-语言信息进一步扩展了语义分布并实现了对重建属性的高级控制。
...
TL;DR: GIANT框架让大模型能够像病理学家一样迭代导航全玻片图像，在病理学图像解读任务中显著优于传统方法，接近或超越专门训练的专业模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有通用大模型在医学图像解读（特别是病理学中的千兆像素图像）上表现不佳，先前研究使用低分辨率缩略图或随机图像块可能低估了模型性能。

Method: 提出GIANT框架，让大模型能够迭代导航全玻片图像；同时发布MultiPathQA基准，包含934个WSI级问题和128个病理学家编写的问题。

Result: GIANT系统显著优于传统的图像块和缩略图基线方法，在病理学家编写的问题上达到62.5%准确率，优于TITAN(43.8%)和SlideChat(37.5%)等专业模型。

Conclusion: 研究揭示了当前基础模型的优势和局限性，为未来开发用于病理学专家推理的大模型奠定了基础。
...
TL;DR: 本文提出CodeV视觉代理模型和TAPO训练框架，通过代码化视觉工具和过程级强化学习，显著提升了视觉推理的忠实性，在保持高准确率的同时解决了工具使用不忠实的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在调用图像操作时存在不忠实的视觉推理问题——可能在无关区域调用工具或忽略工具输出但仍能猜对答案，这影响了模型的可信度。

Method: 提出CodeV代码化视觉代理，使用Tool-Aware Policy Optimization (TAPO)训练框架，将视觉工具表示为可执行Python代码，通过基于工具输入输出的密集奖励来监督中间工具行为。

Result: CodeV在视觉搜索基准测试中实现了竞争性或更优的准确率，同时显著提高了忠实工具使用率，在多种多模态推理和数学基准测试中也表现强劲。
...
TL;DR: OncoVision是一个多模态AI系统，结合乳腺X光图像和临床数据，通过注意力编码器-解码器架构分割四个关键区域，并预测十项临床特征，采用后期融合策略提高诊断精度。


<details>
  <summary>Details</summary>
Motivation: 通过整合多模态数据减少观察者间差异，提高乳腺癌诊断准确性，为医疗资源匮乏地区提供可扩展的筛查解决方案。

Method: 使用基于注意力的编码器-解码器架构分割肿块、钙化、腋窝发现和乳腺组织四个ROI，预测临床特征，开发两种后期融合策略整合影像和临床数据。

Result: 实现了最先进的分割精度，能够稳健预测十项结构化临床特征，通过后期融合策略显著提高诊断精度。

Conclusion: OncoVision为AI乳腺X光诊断设立了新标准，提供可扩展的公平解决方案，通过早期检测和及时干预改善乳腺癌治疗结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有的层剪枝方法在应用于视觉语言模型时会导致显著的性能下降，需要一种能够有效剪枝同时保持性能的方法。

Method: 分析连续三层组来识别局部冗余，移除前两层中最冗余的层，微调剩余层以补偿损失的能力，并冻结第三层作为微调期间的稳定锚点。

Result: 在FineVision数据集的1%数据上仅微调一个epoch，剪除25%网络层后实现了88.9%的平均性能保留，达到最先进性能。

Conclusion: 交错的微调-冻结设计能够在剪枝后使用最少数据实现快速收敛，有效解决了VLM剪枝的性能下降问题。
...
TL;DR: IndEgo是一个多模态工业任务数据集，包含3460个第一人称视角记录和1092个第三人称视角记录，专注于协作工作场景，提供丰富的多模态数据和详细标注。


<details>
  <summary>Details</summary>
Motivation: 解决工业任务中缺乏高质量多模态数据集的问题，特别是针对协作工作和认知密集型任务的场景。

Method: 收集了工业任务的多模态记录，包括装配/拆卸、物流组织、检查维修、木工等，提供眼动追踪、语音叙述、声音、动作等上下文数据。

Result: 数据集包含约197小时第一人称视角记录和97小时第三人称视角记录，提供了详细的标注和基准测试，包括错误检测、问答和协作任务理解。

Conclusion: IndEgo数据集对现有最先进的多模态模型提出了挑战，为工业任务理解研究提供了有价值的资源。
...
TL;DR: 提出基于原型的可解释细胞计数方法，通过密度图估计实现透明化细胞计数，在保持计数准确性的同时提供模型决策的可视化解释。
...
Result: RADSeg在基础ViT类别中mIoU提升6-30%，速度快3.95倍，参数少2.5倍。RADSeg-base仅105M参数却优于之前850-1350M参数的大型视觉模型组合。
...
TL;DR: 提出一种基于分支的结构重参数化技术，通过训练阶段的并行分支设计，在推理时合并为精简的单路径模型，成功将ViT-Tiny从12层压缩到3-6层，在保持ImageNet-1K分类精度的同时实现移动CPU上37%的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有ViT加速策略主要关注算法级优化（如token剪枝和注意力加速），而忽略了深度架构本身的计算开销。本研究探索能否减少Transformer层数同时保持相当的表示能力。

Method: 采用基于分支的结构重参数化技术，在训练阶段设计并行分支，通过逐步合并非线性组件入口处的分支，实现FFN和MHSA模块的精确数学重参数化，无推理时近似误差。

Result: 在ViT-Tiny上成功将12层架构压缩到6、4甚至3层，在ImageNet-1K上保持分类精度，移动CPU平台推理速度提升最高达37%。

Conclusion: 传统偏好极深Transformer堆叠的智慧可能过于保守，本研究为构建高效视觉Transformer开辟了新途径。
...
TL;DR: 提出了一种基于高度感知的动态分块方法，通过根据无人机高度动态调整图像分块大小，显著提升小物体检测性能并加快推理速度。


<details>
  <summary>Details</summary>
Motivation: 无人机在搜救任务中面临小物体检测困难的问题，特别是在高海拔下物体与背景像素比低，导致检测精度下降。

Method: 结合高度依赖的缩放和自适应分块因子，动态调整图像分块大小，减少不必要的计算量。

Result: 在SeaDronesSee数据集上测试，小物体的平均精度提升38%，推理速度比静态分块快两倍以上。
...
TL;DR: 本文研究了切片最优传输(min-STP)框架中优化切片的可迁移性，证明在数据分布轻微扰动下优化切片保持稳定，并提出小批量min-STP以提升可扩展性。


<details>
  <summary>Details</summary>
Motivation: 切片传输方法虽能降低最优传输的计算成本，但优化切片能否迁移到新分布对的问题尚未解决，这对处理演化数据或相关分布重复计算至关重要。

Method: 提出min-Sliced Transport Plan框架，研究优化切片的可迁移性，引入小批量min-STP公式并提供统计精度保证。

Result: 理论证明优化切片在数据分布轻微扰动下保持稳定，实验显示可迁移min-STP在点云对齐和流生成建模中实现强效的单次匹配性能。

Conclusion: 优化切片具有可迁移性，min-STP框架为相关分布间的重复最优传输计算提供了高效解决方案。
...
Method: 开发PathFMTools工具包，用于与CONCH和MUSK两个先进的视觉-语言基础模型交互，并在440例cSCC H&E全切片图像上评估多种适应策略。
...
TL;DR: 该论文提出了一种多模态原型网络，用于物种检测任务，解决了传统多模态神经网络的两个主要问题：缺乏可解释性和基因数据收集成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 物种检测对生态系统监测和入侵物种识别很重要，但现有方法存在两个问题：黑盒模型缺乏可解释性，基因数据收集昂贵且需要侵入性程序。

Method: 扩展原型网络到多模态、成本感知设置，通过集成每个模态的原型并使用相关权重确定预测对每个模态的依赖程度，引入方法识别不需要昂贵基因信息即可做出自信预测的情况。

Result: 该方法能智能分配昂贵的基因数据用于细粒度区分，同时使用丰富的图像数据进行清晰视觉分类，达到与始终使用两种模态模型相当的准确率。

Conclusion: 提出的多模态原型网络方法既保持了可解释性，又能有效降低基因数据收集成本，在物种检测任务中表现出色。
...
TL;DR: TPSO是一种无需训练、模型无关的模块，通过优化token嵌入空间来提升文本到图像扩散模型的生成多样性，避免模型重复生成强模态样本，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型存在生成多样性不足的问题，容易产生重复输出，限制了创意探索和下游应用。现有方法如噪声重采样、提示重写或引导方法仍会坍缩到主导模式或引入失真。

Method: 提出Token-Prompt嵌入空间优化(TPSO)，引入可学习参数探索token嵌入空间中的欠表示区域，减少模型重复生成强模态样本的倾向。提示级空间提供全局语义约束，调节分布偏移。

Result: 在MS-COCO和三个扩散骨干网络上的实验表明，TPSO显著提升了生成多样性，将基线性能从1.10提高到4.18分，且不牺牲图像质量。

Conclusion: TPSO有效解决了扩散模型生成多样性不足的问题，通过优化token嵌入空间实现了多样性和质量的平衡，是一种实用且通用的解决方案。
...
Conclusion: VESSA成功将基础级视觉语义理解集成到SSL框架中，为医学图像分割提供了有效的解决方案，特别是在标注数据稀缺的情况下。
...
TL;DR: CropVLM是一种低成本外部方法，通过强化学习训练模型动态放大图像相关区域，提升视觉语言模型在细粒度图像理解任务中的性能，无需修改或微调目标VLM。
...
Method: 使用强化学习训练CropVLM模型，无需人工标注边界框监督信号或昂贵的合成评估，模型训练一次后可与开源和专有VLM配对使用。

Result: 在需要高分辨率图像理解的任务上，特别是目标VLM域外基准测试中，该方法带来了显著性能提升。

Conclusion: CropVLM提供了一种有效的外部增强方法，能够提升VLM在细粒度视觉任务中的表现，同时避免灾难性遗忘问题。
...
TL;DR: 提出相对角度特征用于点云损伤重建，该特征在保持与法向量相似性能的同时，显著减少存储需求和输入通道数。
...
Conclusion: 相对角度特征可在不修改模型架构的情况下，在资源受限硬件上实现更大批量的执行。
...
TL;DR: MAPS提出了一种模块化邻近调度方法，用于稳健微调视觉-语言-动作模型，通过系统性地放松约束来平衡稳定性和灵活性，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型微调方法要么过度约束适应（冻结模块），要么忽略不同组件的作用（统一正则化），破坏了预训练VLM的强先验知识，损害泛化能力。

Method: MAPS通过系统性分析发现模块约束放松的实证顺序，并线性调度这种放松过程，使视觉编码器保持接近预训练先验，而动作导向的语言层可以更自由地适应。

Result: 在多个基准测试（MiniVLA-VQ、MiniVLA-OFT、OpenVLA-OFT、SimplerEnv、CALVIN、LIBERO）和真实世界评估中，MAPS一致提升了分布内和分布外性能（最高+30%）。

Conclusion: 研究表明，基于实证指导的与预训练VLM的邻近性是保持VLM到VLA迁移中广泛泛化能力的简单而强大的原则。
...
TL;DR: CrispFormer通过三个小改动显著提升弱监督语义分割效果：边界分支监督物体轮廓、不确定性引导的细化器、动态多尺度融合层，在保持SegFormer主干的同时改善边界精度和小物体召回率。


<details>
  <summary>Details</summary>
Motivation: 弱监督语义分割需要从噪声和不足的监督信号中学习密集掩码，现有方法在处理边界精度和小物体方面存在不足。

Method: 1) 边界分支使用轻量级边缘头和边界感知损失监督物体轮廓；2) 不确定性引导细化器预测像素级不确定性并用于加权损失和校正分割logits；3) 动态多尺度融合层用空间softmax门控替换静态特征拼接。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视频理解方面取得显著进展，但其反事实推理能力（推断假设条件下的替代结果）尚未得到充分探索，这对鲁棒的视频理解至关重要。

Method: 引入CounterVQA视频基准测试，包含三个渐进难度级别；开发CFGPT后训练方法，通过从语言模态蒸馏反事实推理能力来增强模型的视觉反事实推理能力。

Result: 评估发现最先进的开源和闭源模型在简单反事实问题上表现合理，但在复杂多跳因果链上性能显著下降；CFGPT方法在所有CounterVQA难度级别上带来一致改进。

Conclusion: 视觉语言模型的反事实推理能力存在显著差距，CFGPT方法能有效提升这种能力，为更鲁棒的视频理解提供了解决方案。
...
TL;DR: Prune-Then-Plan框架通过步骤级校准解决大视觉语言模型在具身问答中的前沿振荡问题，使用Holm-Bonferroni启发式剪枝和覆盖规划器来稳定探索。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在具身问答中直接用于步骤级探索时会出现前沿振荡问题，导致导航效率低下和答案质量下降。

Method: 提出Prune-Then-Plan框架：首先使用Holm-Bonferroni启发式剪枝过程剪除不可信的前沿选择，然后将最终决策委托给基于覆盖的规划器。

Result: 在3D-Mem EQA框架中集成该方法，在视觉基础SPL和LLM-Match指标上分别比基线相对提升49%和33%，在OpenEQA和EXPRESS-Bench数据集上实现了更好的场景覆盖。

Conclusion: 该方法通过将过度自信的预测转化为保守、可解释的动作，有效校准了VLM的步骤级行为，在同等探索预算下实现了更好的性能。
...
TL;DR: 本文提出Cross-Resolution Phase-Aligned Attention (CRPA)方法，解决了扩散变换器中旋转位置编码在混合分辨率去噪时的相位混叠问题，实现训练即用的高保真混合分辨率生成。


<details>
  <summary>Details</summary>
Motivation: 当在扩散变换器中使用线性插值进行混合分辨率去噪时，不同空间网格的令牌混合会导致注意力机制崩溃。这是由于线性坐标重映射迫使单个注意力头比较以不兼容速率采样的RoPE相位，产生相位混叠，破坏分数景观。

Method: 提出Cross-Resolution Phase-Aligned Attention (CRPA)，这是一种无需训练的直接修复方法。CRPA仅修改每个注意力调用的RoPE索引映射：所有Q/K位置都基于查询的步幅表示，确保相等的物理距离始终产生相同的相位增量。

Result: CRPA恢复了扩散变换器依赖的精确相位模式，稳定所有头和层，在图像和视频生成任务上优于现有最先进方法，实现高保真和高效的混合分辨率生成。

Conclusion: CRPA通过相位对齐的注意力机制，从根本上解决了扩散变换器中混合分辨率生成的相位混叠问题，为预训练模型提供了稳定可靠的混合分辨率生成能力。
...
<details>
  <summary>Details</summary>
Motivation: VLM在安全关键应用中需要具备在不确定时弃权的能力，特别是STVQA任务中OCR错误可能导致严重后果。现有弃权方法效果不佳，但不确定性信号可能隐藏在VLM内部表示中。

Method: 提出LRP方法：在隐藏状态或注意力模式上训练轻量级探针。探索三种探针设计：跨层连接表示、聚合视觉token注意力、通过多数投票集成单层探针。

Result: 在四个基准测试中，LRP比最佳基线方法提升7.6%的弃权准确率。探针能泛化到各种不确定性来源和数据集，最优信号出现在中间层而非最终层。

Conclusion: 建立了一个从内部状态而非不可靠输出中检测置信度信号的原则性框架，为构建可部署AI系统提供基础。
...
TL;DR: ReDirector是一种新颖的相机控制视频重拍生成方法，通过改进RoPE在时空位置对齐和引入Rotary Camera Encoding，实现了对动态捕捉变长视频的精确控制。
...
Method: 提出Rotary Camera Encoding (RoCE)，一种相机条件的RoPE相位偏移，捕捉并整合输入和目标视频内外的多视角关系。

Result: 方法能够泛化到分布外的相机轨迹和视频长度，显著改善动态对象定位和静态背景保持，在相机可控性、几何一致性和视频质量方面均有显著提升。

Conclusion: ReDirector通过将相机条件整合到RoPE中，实现了对视频重拍生成的精确控制，在多种轨迹和长度下都表现出优越性能。
...
TL;DR: 提出BHD-RAG框架，通过多模态检索增强生成技术整合DCLD专业知识，提升Birt-Hogg-Dube综合征的CT诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习在BHD诊断中面临的临床样本有限和疾病间差异小的问题，同时减少MLLMs在罕见疾病诊断中的幻觉风险。

Method: 使用专门代理生成CT图像描述构建多模态语料库，基于余弦相似度的检索器匹配相关图像-描述对，MLLM综合检索证据进行诊断。

Result: 在四种DCLD数据集上验证，获得优越的准确率，生成基于证据的描述与专家见解高度一致。

Conclusion: BHD-RAG框架有效整合领域知识和临床先例，显著提高了BHD诊断的准确性和可靠性。
...
TL;DR: DesignPref是一个包含12k对UI设计比较的数据集，由20位专业设计师标注多级偏好评分，研究发现设计师之间存在显著分歧，传统多数投票方法无法准确反映个体偏好，个性化模型在预测个体设计师偏好方面表现更好。


<details>
  <summary>Details</summary>
Motivation: 由于视觉设计具有主观性和高度个性化特征，个体偏好差异很大，现有基于人类标注偏好的生成模型微调和基准测试方法无法准确捕捉这种个性化差异。

Method: 引入DesignPref数据集，包含专业设计师的多级偏好评分和自然语言理由；研究传统多数投票方法的局限性；探索多种个性化策略，特别是微调或将设计师特定标注整合到RAG流程中。

Result: 设计师之间存在显著分歧（Krippendorff's alpha = 0.25）；个性化模型在预测个体设计师偏好方面始终优于聚合基线模型，即使使用少20倍的示例；分歧源于对设计方面重要性的不同认知和个人偏好。

Conclusion: 该研究为个性化视觉设计评估提供了首个数据集，支持未来对个体设计品味建模的研究，证明个性化方法在捕捉设计偏好方面比聚合方法更有效。
...
TL;DR: 本文提出Rectified SpaAttn方法，通过修正注意力分配偏差来解决现有稀疏注意力方法导致的性能下降问题，在保持生成质量的同时显著提升计算速度。
...
Method: 提出Rectified SpaAttn方法：1) 对关键token使用孤立池化注意力重分配，计算准确的修正因子；2) 对非关键token使用增益感知池化修正，确保修正后的增益始终超过引入的误差。
...
Method: 引入UniSandbox解耦评估框架，配合受控合成数据集，分析理解-生成差距，探索思维链和自我训练方法的作用。

Result: 发现理解-生成差距主要体现在推理生成和知识迁移两个维度，显式思维链能有效弥合差距，自训练可内化推理能力，查询式架构具有隐式思维链特性。

Conclusion: 为设计真正弥合理解与生成差距的未来统一架构和训练策略提供了初步见解。
...
<details>
  <summary>Details</summary>
Motivation: 传统基准测试缺乏对世界真实感能力的统一评估，而世界生成模型需要构建物理一致的多维世界，因此需要新的评估体系。

Method: 通过将多模态条件映射到统一文本空间，结合LLM-as-judge、MLLM-as-judge和传统网络方法进行自适应评估，涵盖图像/视频/文本到3D/4D的转换任务。
...
TL;DR: 提出IMIC方法，在多任务学习中同时处理物体识别、高低质量人脸识别和全身人物识别，避免灾难性遗忘，在统一嵌入空间中实现四个任务的线性可分表示。


<details>
  <summary>Details</summary>
Motivation: 解决基础模型在多任务微调时的灾难性遗忘问题，创建能同时处理多个视觉任务的统一模型。

Method: 引入IMIC（交错多域身份课程）方法，采用梯度耦合的交错训练策略，在DINOv3、CLIP和EVA-02三个基础模型上同时微调四个任务。

Result: EVA-02和CLIP模型在所有四个任务上表现与领域专家相当，在跨人脸、身体和物体数据集的多任务处理上比人类更准确，且不影响分布外泛化能力。

Conclusion: IMIC方法成功创建了统一嵌入空间，四个任务线性可分但特征高度共享，仅需少量主成分即可完成跨任务识别，保持了基础模型的关键特性。
...
TL;DR: 提出了一种基于贝塞尔曲线的道路网络提取方法DOGE，无需曲线标注数据，通过全局优化直接从分割掩码生成参数化贝塞尔图。


<details>
  <summary>Details</summary>
Motivation: 现有道路提取方法依赖折线表示，难以准确建模曲线几何形状，且需要难以构建的矢量标注数据。

Method: DOGE框架包含两个交替模块：DiffAlign通过可微分渲染持续优化几何形状，TopoAdapt使用离散算子优化拓扑结构。

Result: 在SpaceNet和CityScale基准测试中达到新的最先进水平，能够生成高保真度的矢量道路地图。

Conclusion: 提出了一种新的道路网络矢量地图生成范式，基于贝塞尔曲线的参数化表示优于传统折线方法。
...
TL;DR: STAvatar提出了一种从单目视频重建高保真可动画3D头部头像的方法，通过UV自适应软绑定框架和时间自适应密度控制策略，解决了现有方法在运动刚性和遮挡区域处理方面的限制。
...
Method: 1. UV自适应软绑定框架：利用图像和几何先验学习UV空间中每个高斯特征偏移，支持动态重采样，与自适应密度控制完全兼容。2. 时间ADC策略：首先聚类结构相似帧以进行更有针对性的密度计算，引入融合感知误差作为克隆标准，联合捕捉几何和纹理差异。

Result: 在四个基准数据集上的广泛实验表明，STAvatar实现了最先进的重建性能，特别是在捕捉细粒度细节和重建频繁遮挡区域方面表现优异。

Conclusion: STAvatar通过创新的UV自适应绑定和时间密度控制策略，显著提升了3D头部头像的重建质量和动画能力，代码将公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将时间序列转换为"伪图像"进行时间预测，但未能建立语义级对齐；非视觉连续序列作为图像生成条件信号的潜力尚未充分探索。
...
<details>
  <summary>Details</summary>
Motivation: 解决可扩展、数据高效的具身AI需求，为VLA学习提供可控、多样且物理真实的合成数据引擎，避免真实世界交互训练的成本和限制。

Method: 集成两个组件：GigaWorld-0-Video（大规模视频生成控制外观、视角和动作语义）和GigaWorld-0-3D（3D生成建模、高斯泼溅重建、物理系统识别和运动规划），通过GigaTrain框架的FP8精度和稀疏注意力实现高效训练。

Result: 生成高质量、多样且可控的具身交互数据，VLA模型（如GigaBrain-0）在物理机器人上实现强泛化性能和任务成功率，无需真实世界交互训练。

Conclusion: GigaWorld-0作为数据引擎，能够有效支持VLA模型的训练，在零真实世界交互的情况下实现物理机器人的高性能表现，为具身AI提供了可扩展的解决方案。
...
TL;DR: ChessMamba是一个用于多时相遥感图像变化检测的结构感知框架，通过棋盘交错和状态空间建模解决时空错位问题，在多个变化检测任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉Transformer或状态空间模型的方法在时间序列化过程中会破坏局部结构一致性，在错位情况下模糊判别性线索，阻碍可靠的变化定位

Method: 整合SpatialMamba编码器和轻量级跨源交互模块，采用棋盘交错扫描顺序和结构感知融合的多扩张卷积，在单次前向传播中序列化多时相特征

Result: 在二元变化检测、语义变化检测和多模态建筑损伤评估三个任务上的全面评估表明，ChessMamba有效融合异构特征，相比最先进方法实现显著精度提升

Conclusion: ChessMamba通过结构感知的状态空间建模，为多时相遥感图像变化检测提供了鲁棒的解决方案，能够准确进行变化定位
...
Method: 利用频域特征解耦知识迁移：低频特征强制强对齐，高频特征采用宽松对齐；引入尺度一致性损失处理分布偏移；使用共享分类器统一特征空间

Result: 在多个基准数据集上的实验表明，该方法显著优于传统知识蒸馏和最先进的跨模态知识蒸馏方法

Conclusion: 频率解耦的跨模态知识蒸馏方法有效解决了跨模态知识迁移的挑战，通过频域特征分离实现了更好的知识传递效果
...
<details>
  <summary>Details</summary>
Motivation: 现有计算机辅助诊断(CAD)数据集通常只支持单一任务，限制了CAD技术的发展。需要构建多任务数据集来探索任务间的相关性并避免数据集异质性问题。

Method: 基于动脉期增强CT构建了包含150个病例的多任务肝脏数据集，涵盖四种肝脏疾病和正常病例，每个病例都经过经验丰富的临床医生仔细标注和校准。

Result: 提供了LiMT数据集和相关基线实验结果，该数据集可能成为医学影像研究社区的宝贵资源。

Conclusion: LiMT数据集为肝脏CAD技术提供了一个探索性解决方案，能够研究任务间的相关性，同时避免训练过程中任务特定数据集间的异质性问题。
...
Method: 采用生成-验证框架：首先生成带文本上下文的问答对，然后通过跨模态一致性检查和辅助过滤器消除错误对，构建高质量的VeriSciQA数据集。

Result: VeriSciQA成为具有挑战性的基准，开源模型准确率仅64%，而专有模型达82%。在该数据集上微调的模型在多个SVQA基准上表现一致提升，性能随数据规模扩大而增强。

Conclusion: 验证中心的生成-验证框架能够有效扩展高质量科学视觉问答数据，持续的数据扩展可以进一步推动开源社区在SVQA能力上的进步。
...
TL;DR: 提出了Agent0-VL，一个自我进化的视觉语言智能体，通过工具集成推理实现持续改进，无需外部奖励或人工标注。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言智能体依赖人工标注监督的限制，以及纯文本自我评估在复杂视觉推理中的幻觉问题。

Method: 将工具使用集成到推理、自我评估和自我修复中，包含Solver和Verifier两个协同角色，通过自我进化推理循环实现稳定改进。

Result: 在几何问题解决和视觉科学分析任务上，相比基础模型实现了12.5%的性能提升。

Conclusion: Agent0-VL通过工具基础的自我评估和强化学习，实现了无需外部奖励的持续自我改进，有效对齐了推理和评估行为。
...
<details>
  <summary>Details</summary>
Motivation: 解决连续手语识别中准确检测手语开始和结束边界的问题，提高识别鲁棒性。

Method: 使用3D骨骼特征捕捉手语动态特性，预训练87种手形分类器，通过多模态融合模块结合视频分割和手形信息进行边界检测。

Result: 在ASLLRP语料库上的评估显示，相比之前工作有显著改进。

Conclusion: 多模态方法结合骨骼特征和手形信息能有效提升连续手语识别的边界检测和识别性能。
...
TL;DR: Motion Marionette是一个零样本框架，用于从单目源视频到单视图目标图像的刚性运动迁移。该方法通过内部先验而非外部约束来指导迁移过程，确保时间一致性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用几何、生成或模拟先验来指导运动迁移，但这些外部先验会引入辅助约束，导致在泛化能力和时间一致性之间进行权衡。

Method: 首先将源视频和目标图像提升到统一的3D表示空间，从源视频提取运动轨迹构建时空先验，该先验与对象几何和语义无关。然后将其与目标对象集成以合成可控速度场，并使用基于位置的动力学进行细化。

Result: 实证结果表明，Motion Marionette能够泛化到不同对象，生成与源运动对齐且时间一致的视频，并支持可控视频生成。

Conclusion: 该方法通过内部时空先验有效解决了运动迁移中的泛化与时间一致性权衡问题，为高效视频制作提供了灵活解决方案。
...
TL;DR: 提出一个预测3D打印物体中光化学转换的框架，通过C-PGA架构从3D视觉数据预测非视觉体积物理性质
...
Conclusion: C-PGA框架为从3D视觉数据预测复杂物理性质提供了有效解决方案，在材料科学和制造领域具有重要应用价值
...
TL;DR: LoTTS提出了一种局部测试时缩放方法，通过识别和修复图像中的缺陷区域来提升扩散模型质量，同时大幅降低计算成本。
...
Conclusion: 局部测试时缩放是扩散模型在推理时扩展的一个有前景的新方向。
...
TL;DR: HybriDLA是一个新颖的文档布局分析框架，结合扩散和自回归解码，通过多尺度特征融合编码器处理现代复杂文档布局，在DocLayNet和M$^6$Doc基准测试中达到83.5% mAP的最先进性能。
...
Result: 在DocLayNet和M$^6$Doc基准测试中达到83.5%的平均精度(mAP)，超越了先前的方法，实现了最先进的性能。
...
TL;DR: DetVLM提出了一种融合目标检测和视觉大模型的智能图像检索框架，通过两阶段流水线实现细粒度图像检索，支持状态搜索和零样本搜索，在车辆部件数据集上达到94.82%的检索准确率。
...
Method: 采用两阶段流水线：首先使用YOLO检测器进行高效的组件级筛选，然后使用VLM作为召回增强单元，对检测器漏检的组件进行二次验证。

Result: 在车辆部件数据集上达到94.82%的整体检索准确率，零样本搜索中驾驶员戴口罩的准确率达到94.95%，状态搜索任务平均准确率超过90%。

Conclusion: DetVLM成功融合了目标检测和VLM的优势，实现了高效的细粒度图像检索，支持状态搜索和零样本搜索，显著优于仅使用检测器的方法。
...
TL;DR: CPDATrack是一个基于Transformer的跟踪框架，通过目标概率估计和选择性注意力机制来抑制背景和干扰物干扰，同时提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统单流Transformer跟踪器中，过多背景搜索token关注目标模板token会削弱跟踪器的判别能力，现有token剪枝方法容易丢失目标附近的关键上下文信息，且干扰物会降低目标识别准确性。

Method: 1) 在编码器层间集成可学习模块估计搜索token与目标的关联概率；2) 基于概率剪枝非信息性背景token但保留目标上下文；3) 早期层使用判别性选择性注意力完全阻断搜索到模板的注意力；4) 后续层从局部区域选择性提取高概率目标token关注模板token。
...
<details>
  <summary>Details</summary>
Motivation: 图像扩散模型在生成过程中隐式捕获了丰富的语义结构，可用于超越合成的识别和定位任务，特别是利用其自注意力图作为语义标签传播核。
...
Conclusion: 扩散模型的自注意力机制可作为强大的语义标签传播工具，为视频对象跟踪提供有效的零样本解决方案，展示了扩散模型在生成任务之外的广泛应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像编辑方法仅限于低分辨率设置（通常只支持1K分辨率），无法满足高分辨率内容创作的需求，需要开发有效可控的高分辨率图像编辑机制。

Method: 采用分块优化方法处理高分辨率源图像，结合精细细节传输模块和新型同步策略来保持各分块间的一致性。

Result: 大量实验表明该方法能生成高质量的编辑结果，为高分辨率内容创作开辟了道路。

Conclusion: 该工作成功解决了高分辨率图像编辑的挑战，提出的测试时优化框架在保持图像质量的同时实现了高分辨率编辑。
...
TL;DR: SPROUT是一个完全无需训练和标注的核实例分割提示框架，利用组织学先验构建特定切片的参考原型，通过部分最优传输方案逐步引导特征对齐，将特征转换为正负点提示，使SAM模型无需参数更新即可生成精确的核分割结果。


<details>
  <summary>Details</summary>
Motivation: 当前大多数核实例分割方法仍依赖密集监督和计算昂贵的微调，而无需训练的方法是一个有前景的研究方向但尚未充分探索。
...
TL;DR: GFT-GCN是一个保护隐私的3D人脸识别框架，结合了谱图学习和基于扩散的模板保护技术，在保持高识别准确率的同时提供强大的隐私保护。


<details>
  <summary>Details</summary>
Motivation: 3D人脸识别虽然具有抗欺骗性，但存储的生物特征模板需要保护。需要开发既能保持高识别性能又能保护隐私的安全解决方案。

Method: 集成图傅里叶变换(GFT)和图卷积网络(GCN)从3D人脸网格提取紧凑谱特征，引入谱扩散机制生成不可逆、可更新且不可链接的模板，采用轻量级客户端-服务器架构。

Result: 在BU-3DFE和FaceScape数据集上表现出高识别准确率和强抗重建攻击能力。
...
TL;DR: MambaEye是一种新颖的因果序列视觉编码器，基于Mamba2架构，采用严格单向处理，通过相对移动嵌入实现平移不变性，能够在任意图像分辨率下保持线性复杂度。
...
Method: 使用纯Mamba2因果序列编码器，采用相对移动嵌入编码空间位移，引入扩散启发的损失函数进行密集监督。

Result: 在ImageNet-1K分类任务中，尤其在1536^2等高分辨率下表现稳健，同时保持相对于补丁数量的线性时间和内存复杂度。

Conclusion: MambaEye实现了真正输入尺寸无关的视觉编码，为构建更接近人类视觉能力的模型提供了新途径。
...
Method: 基于链式合成范式，先用LLM分解复杂提示为最小语义单元，然后迭代合成这些单元；引入强化学习框架，采用衰减随机性调度增强探索，并使用分层奖励机制

Result: 实验表明该方法在概念覆盖率和组合准确性上显著优于现有方法
...
<details>
  <summary>Details</summary>
Motivation: 现有4D方法依赖外部先验、后优化或需要在4D数据集上微调，而VGGT在动态物体主导时性能下降，需要一种无需训练的方法来提升动态场景重建能力。
...
Conclusion: VGGT4D成功扩展了3D基础模型到4D场景重建，无需额外训练即可有效处理动态场景，在多个任务上表现出色。
...
TL;DR: 本文提出了一种名为Activation Replay的训练免费方法，通过重放基础LMM的低熵激活来增强RLVR后训练模型的多模态推理能力，无需昂贵的策略优化。


<details>
  <summary>Details</summary>
Motivation: RLVR（可验证奖励强化学习）已成为激励大型多模态模型推理能力的有效方法，但其背后的机制尚不明确。本文旨在理解RLVR如何影响输入激活，并探索提升推理能力的新途径。

Method: 1）通过logit lens视角分析RLVR对输入激活的影响；2）提出Activation Replay方法，在测试时重放基础LMM的低熵激活来调节RLVR模型；3）在数学推理、视觉代理和视频推理等场景验证效果。

Result: Activation Replay显著提升了多模态推理能力，提高了Pass@K指标，缓解了RLVR推理覆盖范围较窄的问题。相比重放高熵激活或直接跨模型干预，该方法表现更优。

Conclusion: RLVR主要影响低熵激活，而调节低熵激活对推理有益。Activation Replay是一种简单有效的训练免费方法，能显著增强后训练LMM的推理能力。
...
TL;DR: 提出EmoFeedback2方法，通过生成-理解-反馈的强化学习范式，利用大型视觉语言模型为连续情感图像生成提供奖励和文本反馈，解决现有方法缺乏情感反馈和情感保真度不足的问题。
...
Method: EmoFeedback2方法：1）情感感知奖励反馈策略，使用LVLM评估生成图像的情感值并计算奖励；2）自我提升文本反馈框架，LVLM迭代分析图像情感内容并自适应生成提示优化建议。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于引导的方法理论上可用于图像修复等逆问题，但实际效果有限，导致需要专门的修复模型。本文认为无需训练修复的关键缺失要素是初始种子噪声的优化。

Method: 优化初始种子噪声以近似匹配未掩码数据区域（仅需几十步优化），采用线性近似避免昂贵展开计算，在谱域进行优化以提升稳定性。

Result: 在各种图像修复任务中表现出色，超越了现有最先进方法。

Conclusion: 通过优化初始种子噪声，实现了无需训练的高效图像修复，为通用文本到图像模型在逆问题中的应用提供了新思路。
...
TL;DR: 提出一种多模态VR注视预测方法，结合时间注视模式、头部运动和视觉场景信息，通过门控融合机制和跨模态注意力自适应加权不同数据源，在22个VR场景数据集上验证了预测准确性的提升。


<details>
  <summary>Details</summary>
Motivation: VR环境中的注视行为预测对于渲染优化和界面设计具有重要意义，但现有方法面临挑战。该研究旨在通过多模态融合提高预测准确性，为构建更高效的VR系统提供支持。

Method: 采用多模态方法，结合时间注视模式、头部运动数据和视觉场景信息，使用门控融合机制和跨模态注意力来自适应加权不同数据源的贡献。

Result: 在包含22个VR场景、530万注视样本的数据集上，多模态融合相比单一数据流显著提高了预测准确性，验证准确率达到93.1%，在1-3个未来帧的预测中表现出良好的时间一致性。

Conclusion: 该方法通过整合过去注视轨迹、头部朝向和场景内容，提升了VR注视预测性能，为渲染优化、交互设计和用户体验评估提供了潜在应用价值，是向无需昂贵眼动硬件的高效VR系统迈出的重要一步。
...
TL;DR: 提出了一个两阶段的参考引导图像生成框架，通过全局一致性细化和局部编辑强化来改善细节保持和参考对齐


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在基于参考图像细化生成图像时难以保持细粒度视觉细节，VAE潜在压缩会丢失纹理信息，现有方法在放大局部细节时会产生光照、纹理或形状不一致的问题

Method: 采用两阶段参考驱动修正：1）微调单图像扩散编辑器同时输入草稿图像和参考图像进行全局一致性细化；2）应用强化学习增强局部编辑能力，优化细节准确性和语义一致性

Result: 在挑战性参考引导恢复基准测试中显著改善了参考对齐和细粒度细节保持，超越了开源和商业模型

Conclusion: 该方法能够生成忠实且视觉一致的编辑结果，有效解决了参考引导图像生成中的细节保持问题
...
TL;DR: 本文提出了CREward模型，这是首个类型特定的创造力奖励模型，涵盖几何、材质和纹理三个创造力维度，用于评估和生成创意图像。
...
Conclusion: CREward模型在创造力评估、可解释创造力和创意样本获取三个应用场景中表现出色，可用于人类设计灵感和引导创意生成。
...
TL;DR: 本文揭示了一种新型威胁：单个扰动可以劫持整个决策链，使模型在多个预定义目标上同时产生错误输出。


<details>
  <summary>Details</summary>
Motivation: 传统对抗攻击只关注操纵单个决策，但现实世界模型往往在决策序列中运行，级联错误可能导致严重风险。

Method: 引入语义感知通用扰动(SAUPs)，通过归一化空间搜索和语义分离策略开发有效算法。

Result: 在三个多模态大语言模型上的实验显示，仅使用一个对抗帧就能在控制五个不同目标时达到70%的攻击成功率。

Conclusion: SAUPs暴露了模型在决策链中的严重脆弱性，对现实世界应用构成实际威胁。
...
Method: 使用基于Transformer的特征提取模块从原始输入中提取运动和视觉特征，设计深度引导注意力模块利用深度信息引导注意力，并采用模态注意力和时间注意力机制选择性地强调重要模态和捕捉时间依赖关系。
...
TL;DR: 提出多上下文融合Transformer(MFT)，通过融合行人行为、环境、行人定位和车辆运动四个维度的上下文信息，实现准确的行人过街意图预测。
...
Method: 采用渐进式融合策略：首先进行上下文内互注意力实现特征序列融合，然后通过跨上下文互注意力整合特征，最后通过引导式注意力机制细化上下文表示。

Result: 在JAADbeh、JAADall和PIE数据集上分别达到73%、93%和90%的准确率，优于现有最先进方法。

Conclusion: MFT通过有效的多上下文融合机制，显著提升了行人过街意图预测的准确性。
...
TL;DR: 提出注意力引导的跨模态交互Transformer（ACIT）用于行人过街意图预测，通过六种视觉和运动模态的分组交互，结合注意力机制和Transformer时序建模，在JAAD数据集上达到SOTA性能。
...
Method: 将六种模态分为三组交互对：全局语义图与光流、局部RGB图像与光流、自车速度与行人边界框。使用双路径注意力机制增强主模态显著区域，并通过光流引导注意力与辅助模态深度交互。采用跨模态注意力建模运动特征，多模态融合模块促进跨模态交互，Transformer模块捕获时序依赖。

Result: 在JAADbeh和JAADall数据集上分别达到70%和89%的准确率，优于现有最优方法。消融研究验证了各模块的贡献。

Conclusion: ACIT通过精心设计的跨模态交互机制有效整合多模态信息，显著提升了行人过街意图预测性能。
...
TL;DR: 提出了安全关键推理任务，通过多视角输入解决自动驾驶中避免一个风险可能引发另一个风险的问题，并发布了包含35,000个标注问答对的WaymoQA数据集来支持该任务。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在自动驾驶场景理解方面表现良好，但在安全关键场景中的高级推理能力仍然不足，特别是当避免一个交通风险可能引发另一个风险时。

Method: 将安全关键推理分解为两个阶段：首先解决即时风险，然后缓解决策引发的下游风险。通过多视角输入获得环境全面视图，并使用WaymoQA数据集进行微调。
...
TL;DR: SAM-MI是一个新颖的掩码注入框架，通过文本引导稀疏点提示、浅层掩码聚合和解耦掩码注入，有效解决SAM在开放词汇语义分割中的过分割和硬组合问题，在多个基准测试中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 虽然SAM在通用分割方面表现出色，但在开放词汇语义分割中存在两个主要挑战：(1) SAM倾向于过分割，(2) 固定掩码与标签之间的硬组合问题。

Method: 1. 文本引导稀疏点提示器替代密集网格提示，加速掩码生成；2. 浅层掩码聚合合并部分掩码缓解过分割；3. 解耦掩码注入分别在低频和高频注入SAM生成的掩码进行指导。

Result: 在MESS基准测试中，相对于Grounded-SAM实现了16.7%的相对mIoU提升，同时速度提升了1.6倍。

Conclusion: SAM-MI可以作为有效将SAM与OVSS模型结合的一种替代方法学。
...
<details>
  <summary>Details</summary>
Motivation: MLLMs虽然能准确提取视觉语义，但在后续推理中未能充分利用这一优势，导致有限的位置能力引发幻觉问题。

Method: VGA是一种无需训练的方法，首先构建精确的视觉基础，然后利用该基础引导模型关注相关视觉区域。在图像描述任务中，VGA还会动态抑制已描述区域。
...
TL;DR: COVec是一个基于明暗对比原理的图像矢量化框架，首次在矢量域引入本征图像分解，将图像分离为反照率、阴影和光照层，实现高保真度和可编辑性的矢量表示。


<details>
  <summary>Details</summary>
Motivation: 现有矢量化方法难以表示复杂的真实世界图像，通常以牺牲语义简洁性为代价产生碎片化形状。

Method: 引入本征图像分解到矢量域，将图像分离为反照率、阴影和光照层；采用语义引导初始化和两阶段优化，结合可微分渲染。

Result: 在多个数据集上的实验表明，COVec相比现有方法实现了更高的视觉保真度和显著改进的可编辑性。

Conclusion: COVec通过明暗感知的矢量表示框架，成功解决了复杂图像矢量化中的保真度和可编辑性问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于点的方法直接生成点云，具有训练成本低和算法简单的优点，但性能往往不如基于表示的方法。本文旨在提升基于点方法的性能和可扩展性。

Method: 采用多尺度生成算法，从粗到细生成点云；引入结构化下采样和上采样策略，在保持几何结构的同时确保不同分辨率间分布的平滑过渡。

Result: MFM-Point在基于点的方法中实现了最佳性能，并挑战了最好的基于表示的方法，特别是在多类别和高分辨率生成任务中表现出色。

Conclusion: MFM-Point框架显著提升了基于点方法的点云生成能力，在保持简单高效的同时达到了与基于表示方法相竞争的性能水平。
...
TL;DR: HACBSR是一种无监督盲超分辨率框架，无需真实图像和外部核先验，通过对比核采样和历史增强对比学习解决行星遥感图像中的未知退化问题。
...
Method: 包含两个组件：(1) 对比核采样机制，通过核相似性控制减轻高斯采样的分布偏差；(2) 历史增强对比学习，利用历史模型生成负样本实现非贪婪优化和强凸性。

Result: 在多个上采样因子下，HACBSR与最先进的无监督方法相比具有竞争力，并发布了Ceres-50数据集用于行星应用评估。

Conclusion: HACBSR为行星遥感图像的无监督盲超分辨率提供了有效解决方案，无需真实图像或外部核先验即可处理未知退化问题。
...
TL;DR: DeLight-Mono是一个新颖的自监督单目深度估计框架，通过光照解耦解决内窥镜图像中不均匀光照导致的深度估计性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 内窥镜导航系统中，自监督单目深度估计是关键任务，但由于内窥镜图像固有的不均匀光照（特别是在低强度区域），性能会下降。现有的低光增强技术无法有效指导深度网络，而其他领域（如自动驾驶）的解决方案需要良好光照的图像，不适合内窥镜应用且增加数据收集负担。

Method: 提出了DeLight-Mono框架，采用设计的照明-反射-深度模型表示内窥镜图像，并通过辅助网络进行分解。提出了一个自监督联合优化框架，利用解耦组件的新损失函数来减轻不均匀光照对深度估计的影响。

Result: 在两个公共数据集上进行了广泛的比较和消融研究，严格验证了所提方法的有效性。
...
TL;DR: FLaTEC是一种频率感知的点云压缩方法，通过解耦低频结构和高频纹理，使用混合潜在三平面作为紧凑代理，实现高压缩比下的高质量重建。


<details>
  <summary>Details</summary>
Motivation: 点云压缩方法需要在比特率和重建失真之间平衡，但低频和高频分量在同一分辨率下贡献不同，使得平衡难以实现。

Method: 将体素化嵌入转换为三平面表示以减少稀疏性和计算成本；设计频率解耦技术提取低频内容并收集高频细节；使用二进制格式存储解耦分量；通过调制块逐步恢复全频谱信号；引入基于频率的高效注意力机制补偿3D相关性损失。

Result: 在SemanticKITTI和Ford数据集上，BD-rate分别比标准编解码器提升78%和94%，实现了最先进的率失真性能。

Conclusion: FLaTEC通过频率感知机制有效解决了点云压缩中低频和高频分量的平衡问题，显著提升了压缩效率和重建质量。
...
TL;DR: PRADA提出了一种基于概率比率的检测方法，能够可靠地检测自回归生成的图像并归因到具体模型


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成技术发展迅速，产生了逼真的图像，但缺乏专门针对此类生成图像的检测方法

Method: 通过检查模型对图像标记序列的条件概率与无条件概率之比，利用该比率的独特特征进行阈值检测和归因

Result: PRADA对8个类别到图像模型和4个文本到图像模型都表现出高效检测能力

Conclusion: 该方法简单可解释，能有效检测自回归生成的图像并进行模型归因
...
TL;DR: 提出了Task-Step-State (TSS)框架，通过引入状态层作为视觉基础语义，将抽象程序与可观察细节连接起来，采用渐进式预训练策略，在多个下游任务上优于基线模型。
...
Conclusion: 渐进式预训练策略比标准联合训练更有效，能更好地强制执行预期的层次结构，状态监督在所有任务中都是性能提升的关键驱动因素。
...
TL;DR: 提出了一种新的盲自适应局部去噪（BALD）方法，用于解决CEST MRI中的空间变化噪声问题，无需先验噪声知识即可稳定噪声分布并提高定量对比度映射的准确性。
...
TL;DR: 提出CONVAD方法，将概念瓶颈模型(CBMs)应用于视觉异常检测(VAD)，通过概念学习提供人类可解释的异常描述，同时保持与传统VAD方法相当的性能。
...
Method: 扩展概念瓶颈模型到VAD场景，开发概念数据集，改进CBM架构以同时生成概念和视觉解释，并引入人工异常合成管道。

Result: CONVAD方法在性能上与经典VAD方法相当，但提供了更丰富、基于概念的解释，增强了系统的可解释性和可信度。

Conclusion: 概念瓶颈模型能有效提升VAD系统的解释能力，通过语义概念为异常检测提供更深入的洞察，是VAD解释性研究的重要进展。
...
TL;DR: WPT提出了一种世界模型到策略的迁移训练范式，通过在线蒸馏在端到端世界模型指导下训练轻量级策略，实现高性能和实时部署的平衡。
...
TL;DR: 本文提出了一种基于深度学习的森林火灾早期预警系统，通过游戏模拟器合成包含烟雾和初期火灾图像的数据集，并比较了YOLOv7和检测变换器模型在火灾检测任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有森林火灾检测方法因缺乏大规模数据集和专门优化的模型而存在漏检问题，需要开发专门针对早期火灾识别的数据集和检测系统。

Method: 利用Red Dead Redemption 2等游戏模拟器合成包含烟雾羽流和初期火灾的图像数据集，结合已发布的图像构建更全面的数据集，然后使用YOLOv7和不同版本的检测变换器模型进行图像分类和定位比较。

Result: 成功构建了专门针对早期森林火灾识别的合成数据集，该数据集包含多个烟雾羽流和初期火灾实例，与现有仅包含大规模火灾图像的数据集形成对比。

Conclusion: 通过游戏模拟器合成数据集的方法为森林火灾早期检测提供了有效的数据支持，YOLOv7和检测变换器模型在该任务上表现出良好的应用潜力。
...
TL;DR: 本文提出了一种基于深度学习CNN（Inception V3）和多头注意力机制的集成方法，用于从X射线图像中自动检测心脏肥大症，实现了高精度的诊断性能。


<details>
  <summary>Details</summary>
Motivation: 医疗影像技术革命性地改变了心血管疾病诊断方式，特别是对心脏肥大等结构异常的检测需求日益增长，需要开发自动化的精准诊断工具。

Method: 采用CNN架构结合Inception V3模型作为核心模块，并集成多层注意力机制，通过多头注意力机制自动学习特征，选择性聚焦输入图像的特定区域来敏感识别心脏肥大。

Result: 模型在准确率、精确率、召回率、灵敏度、特异性和AUC等方面均表现优异，具体指标为：准确率95.6%、精确率95.2%、召回率96.2%、灵敏度95.7%、特异性96.1%、AUC 96.0%。

Conclusion: 该方法通过严格的评估验证了其在心脏肥大检测方面的临床有效性，展示了深度学习与注意力机制在医疗影像诊断中的巨大潜力。
...
TL;DR: LungEvaty是一个基于transformer的框架，用于从单次低剂量CT扫描预测1-6年肺癌风险，无需像素级标注，在9万多个CT扫描上训练，性能达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 随着基于低剂量CT的肺癌筛查项目普及，需要可扩展的方法来处理整个肺部体积，现有方法要么过度依赖像素级标注，要么将肺部分析为片段，限制了性能。

Method: 使用完全基于transformer的框架处理整个肺部输入，从大规模筛查数据中学习解剖和病理特征，可选使用解剖感知注意力引导损失来增强解剖聚焦注意力。

Result: 仅使用影像数据且无区域监督的情况下，LungEvaty达到最先进性能，在超过9万个CT扫描上训练，包括2.8万个用于微调和6千个用于评估。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散变换器难以泛化到训练长度之外，存在周期性内容重复和普遍质量下降两种失败模式。

Method: 从注意力图角度分析问题，发现两种失败模式源于注意力分散，提出训练即插即用的UltraViCo方法，通过常数衰减因子抑制训练窗口外token的注意力。

Result: 在4倍外推时，动态程度和图像质量分别比之前最佳方法提升233%和40.5%，外推限制从2倍提升到4倍。

Conclusion: UltraViCo能有效解决视频长度外推问题，且可无缝泛化到可控视频合成和编辑等下游任务。
...
TL;DR: 提出PETRG-3D框架，用于自动生成PET/CT报告，解决专科医生短缺问题。该框架采用3D双分支结构，包含风格自适应提示机制，并在多中心淋巴瘤数据集上验证效果显著。


<details>
  <summary>Details</summary>
Motivation: PET/CT扫描仪快速增长，但专业医生数量不足，需要自动化报告生成技术来减轻临床工作负担。相比结构成像，功能PET面临代谢模式变化和全身3D上下文信息需求的独特挑战。

Method: 提出PETRG-3D端到端3D双分支框架，分别编码PET和CT体积数据，引入风格自适应提示机制来缓解医院间报告实践差异。构建了多中心淋巴瘤数据集PETRG-Lym和公开基准AutoPET-RG-Lym。

Result: PETRG-3D在自然语言指标（如ROUGE-L提升31.49%）和临床效能指标（如PET-All提升8.18%）上显著优于现有方法，验证了体积双模态建模和风格感知提示的益处。

Conclusion: 这项工作为未来强调疾病感知推理和临床可靠评估的PET/CT专用模型奠定了基础。代码、模型和AutoPET-RG-Lym基准将公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Transformer和状态空间模型虽然具有强大的长程建模能力，但在图像压缩中可能导致结构信息丢失或忽略重要的频率特性，而CNN擅长捕捉局部高频细节但长程建模能力有限。

Method: 提出HCFSSNet混合架构：使用CNN提取局部高频结构；引入视觉频率状态空间(VFSS)块建模长程低频信息，包含全向邻域状态空间(VONSS)模块和自适应频率调制模块(AFMM)；集成AFMM与Swin Transformer形成频率Swin Transformer注意力模块(FSTAM)。

Result: 在Kodak、Tecnick和CLIC数据集上的实验表明，HCFSSNet相比VTM锚点分别减少18.06%、24.56%和22.44%的BD率，与最近的MambaIC等SSM编解码器相比具有竞争性的率失真性能，同时使用显著更少的参数。

Conclusion: HCFSSNet为未来学习图像压缩系统提供了一种高效且可解释的混合架构，成功结合了CNN的局部建模能力和状态空间模型的长程建模优势。
...
TL;DR: Restora-Flow是一种无需训练的流匹配方法，通过退化掩码指导采样并加入轨迹校正机制，在图像修复任务中实现高质量和快速处理。


<details>
  <summary>Details</summary>
Motivation: 解决现有流模型在图像修复中处理时间长或产生过度平滑结果的问题，利用流匹配的灵活轨迹设计优势。

Method: 使用退化掩码指导流匹配采样过程，并引入轨迹校正机制确保与退化输入的一致性。

Result: 在自然和医学数据集上的多种图像修复任务（修复、超分辨率、去噪）中，相比扩散模型和流匹配基准方法，在感知质量和处理时间方面表现更优。

Conclusion: Restora-Flow提供了一种高效且高质量的图像修复解决方案，无需额外训练即可实现快速采样和良好的感知质量。
...
Method: 首先将高维sMRI特征投影到流形空间保持疾病进展的内在几何结构，然后使用时感知神经常微分方程建模观测间潜在状态的连续演化，注意力机制黎曼门控循环单元自适应整合历史信息处理不规则间隔。

Result: 在疾病状态预测和认知评分回归任务中均优于现有最优模型，消融研究验证各模块的互补作用，在不同序列长度和缺失率下表现稳定，跨数据集验证显示强鲁棒性。

Conclusion: R-TNAG框架通过联合设计提高了时间一致性，在不规则采样下实现了稳健的AD轨迹预测，具有良好的临床适用性。
...
TL;DR: MAP-World是一个无先验的多模态规划框架，结合掩码动作规划和路径加权世界模型，无需手工锚点或强化学习即可生成多样化的未来轨迹。
...
Method: 使用掩码动作规划将未来自车运动视为序列补全问题，通过注入噪声生成多样化轨迹查询，结合轻量级世界模型预测BEV语义。

Result: 在NAVSIM上匹配基于锚点的方法，在世界模型方法中达到最先进性能，同时保持实时推理延迟。

Conclusion: 该方法避免了强化学习，从完整可信未来分布中学习，无需锚点库或教师策略即可实现多模态规划。
...
TL;DR: SKEL-CF是一个从粗到细的框架，用于估计SKEL参数，通过转换器编码器-解码器架构预测并逐步优化相机和SKEL参数，在MOYO数据集上显著优于之前的SKEL-based方法。


<details>
  <summary>Details</summary>
Motivation: 现有的参数化3D人体模型如SMPL在人体姿态和形状估计方面取得了显著进展，但其简化的运动学限制了生物力学的真实性。SKEL模型通过重新绑定具有解剖学准确骨骼的SMPL解决了这一限制，但直接估计SKEL参数仍然具有挑战性。

Method: 提出SKEL-CF框架，采用基于转换器的编码器-解码器架构，编码器预测粗略的相机和SKEL参数，解码器在连续层中逐步优化它们。将现有的SMPL数据集4DHuman转换为SKEL对齐版本4DHuman-SKEL，提供高质量训练数据。
...
TL;DR: 本文提出HPA框架来解决安全对齐多模态大语言模型在持续视觉指令调优中的安全退化问题，通过参数划分、平衡选择和正交约束来协调安全性和任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有持续视觉指令调优研究主要关注未安全对齐的模型，忽视了真实世界MLLMs需要安全机制来降低风险。安全对齐模型在持续适应中不仅会出现任务遗忘，还会出现安全性能退化。

Method: 提出和谐参数适应(HPA)框架，包括基于关注度的参数划分、和谐平衡的参数选择和正交参数调整。将参数按关注安全或任务性能划分为两类，从平衡角度选择要保留的参数，并施加正交约束来缓解灾难性遗忘。

Result: 在CVIT基准测试和安全评估数据集上的大量实验表明，HPA比现有基线更好地保持了高安全性并减轻了遗忘问题。

Conclusion: HPA框架能够有效协调安全对齐MLLMs在持续视觉指令调优中的安全性和任务性能，为解决这一关键挑战提供了有效方案。
...
TL;DR: 研究探索大型多模态模型在视频交互事件中的感知基础能力，发现模型虽然能描述对象和动作，但无法准确定位交互开始/结束的时间和位置。


<details>
  <summary>Details</summary>
Motivation: 验证大型多模态模型是否真正基于视觉输入建立语义理解，特别是在物理交互事件的时空定位能力。

Method: 构建包含2万多个标注交互事件的数据集，使用250名人工标注者标记接触和释放事件，测试两个LMM模型在短视频中定位这些事件的能力。

Result: 模型能可靠地命名目标对象、识别动作并提供连贯推理，但无法准确识别交互开始/结束的帧，也无法在场景中定位事件位置。
...
TL;DR: 提出了ADNet，一个包含380个类别的大规模多领域异常检测基准数据集，涵盖电子、工业、农业食品、基础设施和医疗等领域。实验显示现有方法在扩展到多类别时性能显著下降，为此提出了Dinomaly-m方法来解决可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的异常检测基准（如MVTec-AD仅15个类别）覆盖范围有限，无法充分评估跨上下文泛化能力和可扩展性，需要更全面的基准来推动异常检测研究。

Method: 收集了49个公开数据集，构建包含380个类别、196,294张RGB图像的大规模基准ADNet，所有图像都标准化为MVTec风格的像素级标注和结构化文本描述。提出了Dinomaly-m方法，这是Dinomaly的上下文引导的专家混合扩展，在不增加推理成本的情况下扩展解码器容量。

Result: 现有最先进方法在单类别设置下达到90.6% I-AUROC，但在扩展到380个类别的多类别设置时下降到78.5%。提出的Dinomaly-m方法达到83.2% I-AUROC和93.1% P-AUROC，优于现有方法。

Conclusion: ADNet为异常检测研究提供了标准化和可扩展的基准，支持跨领域数据集扩展，并为未来异常检测基础模型提供了可扩展的基础。
...
TL;DR: 首个电池供电的可穿戴瞳孔中心追踪系统，结合事件视觉传感器和神经形态处理，在Speck2f芯片上实现100Hz双目瞳孔追踪，每眼功耗低于5mW。


<details>
  <summary>Details</summary>
Motivation: 解决可穿戴平台实现鲁棒、高频追踪与超低功耗的挑战，事件视觉传感器虽有微秒级分辨率和稀疏数据流，但缺乏完全集成的低功耗实时推理解决方案。

Method: 使用事件视觉传感器和神经形态处理，在Speck2f系统级芯片上集成不确定性量化脉冲神经网络，配合低功耗微控制器进行轻量级坐标解码，采用系统化部署机制弥合现实差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有基础视频生成模型如WAN 2.2仅限于同视角生成，无法实现跨视角视频合成，特别是从第三人称到第一人称视角的转换。

Method: 包含三个关键模块：EgoExo-Align实现视角对齐，MultiExoCon聚合多视角视频条件，PoseInj注入相机姿态信息指导几何感知合成。
...
<details>
  <summary>Details</summary>
Motivation: 视频文本VQA任务面临重大挑战，需要模型准确感知和理解不同尺度、方向和清晰度的场景文本，同时有效整合时间和语义上下文。模型必须识别问题相关文本线索并过滤冗余信息。

Method: 提出SFA训练免费框架，模拟人类答题过程：自适应扫描视频帧、选择性关注关键区域、直接放大这些区域，引导Video-LLM注意力到关键线索。

Result: SFA在多个公开Video TextVQA数据集上取得新的最先进结果，大幅超越先前方法，证明了其有效性和泛化能力。

Conclusion: SFA作为首个基于Video-LLM的Video TextVQA方法，通过引导模型注意力机制，在视频文本问答任务中表现出色，具有很好的应用前景。
...
TL;DR: GHR-VQA是一个基于场景图的人为中心视频问答框架，通过图神经网络处理视频级图结构，实现跨帧推理和层次关系推理，在AGQA数据集上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统基于像素的方法难以捕捉视频中复杂的人-物交互关系，需要更结构化、可解释的方法来理解视频中的时空动态和人类行为。

Method: 将视频帧表示为场景图，通过全局根节点连接跨帧的人节点形成视频级图，使用图神经网络处理图结构，并与问题特征在层次网络中集成。

Result: 在AGQA数据集上验证，相比现有最佳方法，在对象关系推理任务上提升了7.3%的性能。

Conclusion: 基于场景图的显式人中心结构能够有效分解动作为人-物交互，增强视频内容理解的深度和可解释性。
...
TL;DR: 本文提出了一种用于3D脑部MRI图像健康组织合成的深度学习框架，在BraTS-Inpainting 2025挑战赛中取得第一名，性能超越前两届冠军方案。


<details>
  <summary>Details</summary>
Motivation: 解决脑肿瘤MRI定量分析中深度学习模型受数据集偏差限制的问题，通过合成健康组织来减轻数据偏见影响。

Method: 采用U-Net架构训练合成受损区域的修复能力，结合随机掩码增强策略提升模型泛化性能。

Result: 验证集上SSIM为0.873±0.004，PSNR为24.996±4.694，MSE为0.005±0.087；最终测试集上SSIM为0.919±0.088，PSNR为26.932±5.057，RMSE为0.052±0.026。

Conclusion: 该方法在BraTS-Inpainting 2025挑战赛中表现优异，获得第一名，证明了其在脑部MRI图像修复任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 解决RGBA操作领域的分裂现状：专业单任务模型缺乏通用性，而统一多任务框架局限于RGB领域。

Method: 提出MSRoPE-BiL（新颖的RoPE方法，具有双向可扩展层轴）用于DiT骨干网络，支持并发处理多个RGBA层；构建AlphaLayers数据集（1000个高质量多层三元组）。

Result: 在AIM-500上实现无掩码抠图的SAD相对降低84.8%；在层条件补全中赢得90%以上人类偏好；在所有21个任务上一致优于专业基线模型。

Conclusion: 证明统一多任务模型能够学习到更优的RGBA共享表示，为更强大的层感知生成系统铺平道路。
...
TL;DR: CT-CIG是一种可控文本引导的伪装图像生成方法，通过大型视觉语言模型生成高质量文本提示，结合轻量级控制器和频率交互细化模块，生成逼真且逻辑合理的伪装图像。


<details>
  <summary>Details</summary>
Motivation: 现有伪装图像生成方法往往忽视伪装物体与背景环境之间的逻辑关系，导致生成结果不自然。本文旨在解决这一问题，生成既逼真又逻辑合理的伪装图像。

Method: 1. 设计伪装揭示对话机制(CRDM)为现有伪装数据集标注高质量文本提示；2. 利用构建的图像-提示对微调Stable Diffusion；3. 引入轻量级控制器指导伪装物体的位置和形状；4. 设计频率交互细化模块(FIRM)捕捉高频纹理特征。

Result: 通过CLIPScore评估和伪装效果评估，实验证明生成的文本提示具有良好的语义对齐性，CT-CIG能够生成逼真的伪装图像。

Conclusion: CT-CIG方法能够有效生成语义对齐且视觉逼真的伪装图像，解决了现有方法忽视逻辑关系的问题，在伪装图像生成领域具有重要价值。
...
TL;DR: 该论文针对胶质母细胞瘤的异质性诊断挑战，开发了基于Vision Transformer的深度学习模型，在BraTS-Path 2025挑战赛中获得了第二名。
...
Conclusion: 建立了基于ViT的组织病理学分析的坚实基础，未来工作将致力于缩小未见验证数据上的性能差距。
...
TL;DR: 提出V-Attack方法，通过操纵transformer注意力块中的value特征(V)来实现对大型视觉语言模型中特定概念的精确语义攻击，相比现有方法攻击成功率平均提升36%。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法在大型视觉语言模型上缺乏可控性，无法精确操纵图像中特定概念的语义，主要原因是patch-token表示中的语义纠缠问题。

Method: V-Attack方法包括两个核心组件：1）Self-Value Enhancement模块增强V特征的语义丰富性；2）Text-Guided Value Manipulation模块利用文本提示定位源概念并优化到目标概念。

Result: 在LLaVA、InternVL、DeepseekVL和GPT-4o等多种LVLM上的实验表明，V-Attack相比最先进方法平均提升攻击成功率36%。

Conclusion: V特征作为更精确的操纵手柄，能够实现高度有效的语义控制，揭示了现代视觉语言理解中的关键漏洞。
...
TL;DR: 提出HistoSpeckle-Net深度学习架构，用于从多模光纤散斑重建医学图像，通过分布感知学习策略和直方图互信息损失，在有限数据下实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂真实世界成像任务中适用性有限，且需要大量数据。多模光纤成像尚未充分考虑散斑和重建图像的统计特性。

Method: 开发光学实验装置，构建临床相关数据集；提出包含直方图计算单元和三级特征细化模块的深度学习架构；使用直方图互信息损失和多尺度SSIM损失。

Result: 在复杂OrganAMNIST数据集上，比U-Net和Pix2Pix等基线模型获得更高保真度，在有限训练样本和光纤弯曲条件下表现优异。

Conclusion: HistoSpeckle-Net通过减少数据需求和在光纤扰动下有效重建复杂解剖特征，使多模光纤成像更接近实际临床部署。
...
TL;DR: 提出一种两阶段管道，将乒乓球3D运动估计分为前端感知和后端2D转3D提升任务，解决了合成数据训练方法在真实世界视频中泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于合成数据训练的方法难以泛化到真实世界视频中噪声和不完美的球与球台检测结果，主要原因是缺乏真实世界视频的3D轨迹和旋转标注。

Method: 两阶段管道：前端使用新创建的TTHQ数据集进行2D监督训练，后端使用物理正确的合成数据训练提升网络，并重新设计提升模型以应对真实世界常见问题。

Result: 通过整合球检测器和球台关键点检测器，将概念验证的提升方法转变为实用、鲁棒且高性能的端到端3D乒乓球轨迹和旋转分析应用。

Conclusion: 该方法成功解决了从单目视频中精确估计乒乓球3D运动的挑战，实现了在真实世界场景中的鲁棒性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在长提示词下虽然保真度高但多样性不足，导致输出重复且缺乏创意，需要解决这一困境。

Method: 提出PromptMoG方法，在嵌入空间中使用混合高斯分布采样提示词嵌入，增加采样熵以增强多样性，同时保持语义一致性。

Result: 在SD3.5-Large、Flux.1-Krea-Dev、CogView4和Qwen-Image四个先进模型上的实验表明，PromptMoG能一致提升长提示词生成的多样性且无语义漂移。

Conclusion: PromptMoG是一种无需训练的有效方法，能显著改善长提示词生成的多样性问题，为文本到图像生成提供了新的解决方案。
...
Conclusion: 训练即用的方法在真实世界3D理解中展现出强大的能力和适应性。
...
TL;DR: 开发了一个基于ResNet18的轻量级管道，用于自动检测DaVinci Xi手术系统中摄像头图块的位置和激活状态，为手术数据科学任务提供有价值的元数据。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助微创手术仅依赖内窥镜视频作为术中视觉反馈。检测摄像头激活状态可以提供包括摄像头移动信息在内的有价值元数据，支持下游手术数据科学任务如工具跟踪、技能评估或摄像头控制自动化。

Method: 基于ResNet18卷积神经网络开发轻量级管道，在SurgToolLoc数据集的手动标注数据上进行微调，并在三个公共数据集上评估，涵盖超过70,000帧。

Result: 模型在二分类检测活动摄像头方面F1分数达到0.993到1.000之间，在所有情况下正确定位摄像头图块，没有出现错误的多摄像头检测。

Conclusion: 提出的管道能够可靠、实时地从手术视频中提取摄像头激活元数据，为各种下游应用促进自动化预处理和分析。所有代码、训练模型和标注都已公开。
...
TL;DR: Adv-GRPO是一个对抗性强化学习框架，通过迭代更新奖励模型和生成器来解决传统预训练偏好模型的奖励黑客问题，使用参考图像和视觉基础模型提供密集视觉奖励，显著提升图像质量和美学效果。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习图像生成方法依赖预训练偏好模型提供标量奖励，但这些奖励往往无法准确捕捉人类感知，容易受到奖励黑客攻击，即高分不代表更好的图像质量。
...
Conclusion: Adv-GRPO通过对抗性奖励设计和密集视觉信号有效解决了奖励黑客问题，实现了更高质量的图像生成，同时支持灵活的分布迁移和风格定制。
...
TL;DR: MBCD是一个针对多模态领域泛化的协作蒸馏框架，通过自适应模态丢弃、梯度一致性约束和基于权重平均的跨模态蒸馏，解决传统权重平均在多模态场景中因优化速度差异导致的模态偏差问题。


<details>
  <summary>Details</summary>
Motivation: 传统权重平均在多模态领域泛化中面临挑战：不同模态优化速度差异导致早期偏向快速收敛模态，抑制了慢速但互补模态的贡献，阻碍有效模态融合并导致损失曲面偏向尖锐、泛化性差的极小值。

Method: 1. 学生模型中使用自适应模态丢弃来抑制早期对主导模态的偏向；2. 梯度一致性约束对齐单模态分支与融合表示的学习信号；3. 基于权重平均的教师模型进行跨模态蒸馏，将融合知识传递到各单模态分支。

Result: 在MMDG基准测试上的广泛实验表明，MBCD持续优于现有方法，在不同未见域上实现了更高的准确性和鲁棒性。

Conclusion: MBCD框架成功保留了权重平均的平坦化优势，同时克服了其在多模态环境中的缺点，实现了更好的模态融合和泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统分类方法在图像损坏或训练数据有限等高不确定性条件下性能不佳，需要更鲁棒的分类建模方法

Method: 使用离散扩散过程建模类别标签的条件后验分布，支持在类别概率或离散标签上进行扩散预测

Result: 在ImageNet数据集上，少量扩散迭代即可获得比基线更高的分类准确率，任务越具挑战性准确率提升越明显

Conclusion: DiDiCM框架在不确定性条件下优于标准分类器，为图像分类提供了更灵活和鲁棒的解决方案
...
TL;DR: 提出了一种半监督深度强化学习框架，通过自适应采样、自编码器和预测器的协同工作，在工业视觉检测中有效检测细微缺陷，仅需少量标注数据。
...
Method: 基于强化学习的采样器自适应选择信息丰富的图像块，自编码器生成损失分布图突出异常区域，预测器在损失分布空间进行分割。

Result: 在MVTec AD数据集上，相比现有最优方法，F1_max平均提升0.15，AUC平均提升0.06，最佳情况下F1_max提升达0.37。
...
TL;DR: VKnowU是一个评估多模态大语言模型视觉知识理解能力的基准，包含1680个问题和1249个视频，涵盖8种核心视觉知识类型。研究发现当前SOTA模型在视觉知识理解上仍落后于人类表现，特别是世界中心型知识。为此提出了VKnowQA数据集和VideoKnow+基线模型，采用See-Think-Answer范式和强化学习，在多个基准上取得显著提升。
...
Method: 提出了VKnowU基准用于系统评估视觉知识能力；开发了VKnowQA数据集和VideoKnow+基线模型，采用结构化的See-Think-Answer范式，并结合强化学习与视觉知识奖励机制。

Result: 评估23个SOTA MLLMs发现领先模型在视觉知识理解上仍落后于人类，特别是在世界中心型知识方面存在显著差距。VideoKnow+在VKnowU上取得+3.7%的提升，并在MVBench、Video-MME和MMVU等其他基准上表现一致提升。

Conclusion: 视觉知识是开发更通用MLLMs的关键基石，使模型不仅能看，还能真正理解物理和社会世界。这项工作强调了将视觉知识显式融入MLLMs的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有CLIP模型主要处理单对象分类或短描述检索，但真实场景图像具有丰富的组合结构。现有方法缺乏对关系和组合结构的显式建模。

Method: 提出ScenarioCLIP模型，接受输入文本、基础关系、输入图像和突出关系的聚焦区域。在策划的场景数据上预训练，然后针对下游任务微调。

Result: ScenarioCLIP在各种领域特定任务中展现出强大的零样本和微调性能，在场景基准测试中优于多个基线方法。

Conclusion: ScenarioCLIP通过显式建模关系和组合结构，有效提升了场景分析能力，为复杂场景理解提供了新的解决方案。
...
TL;DR: 提出了DAPointMamba框架，这是首个将状态空间模型(SSMs)应用于领域自适应点云补全(DA PCC)的工作，通过三个创新模块解决直接应用SSMs时的挑战，在计算复杂度和推理延迟更低的情况下超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用CNNs或视觉Transformer存在感受野有限或二次复杂度的问题，而直接将SSMs应用于DA PCC会遇到破坏空间拓扑和局部几何特征、忽视领域无关表示设计等挑战。

Method: 提出DAPointMamba框架，包含三个新模块：跨域补丁级扫描引入补丁级几何对应关系实现局部对齐；跨域空间SSM对齐通过基于跨域相似性调制补丁特征来增强空间一致性；跨域通道SSM对齐通过交错和对齐特征通道来主动解决全局语义差距。

Result: 在合成和真实世界基准测试上的广泛实验表明，DAPointMamba在计算复杂度和推理延迟更低的情况下超越了最先进的方法。

Conclusion: DAPointMamba在领域自适应点云补全中表现出强大的跨域适应性，同时具备全局感受野和高效线性复杂度的优势。
...
Method: 基于MOTR-like模型实际上是强大检测模型的洞察，提出SelfMOTR，依赖自生成的检测先验，并开发了一套实用工具来有效利用这些隐藏的检测能力。

Result: 在DanceTrack数据集上，SelfMOTR实现了强大的性能，与最近最先进的端到端跟踪方法竞争。

Conclusion: 通过广泛分析和消融研究，揭示并证明了MOTR-like模型的隐藏检测能力，提出了一种实用的自生成检测先验方法。
...
TL;DR: 提出基于大语言模型和视觉语言模型的迭代自优化框架，通过多模态思维链过程检测和修正物理不一致性，提升视频生成的物理合理性。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在视觉质量上取得显著进展，但在遵循现实世界物理原理方面仍存在不足，需要改进物理一致性。

Method: 采用训练免费、即插即用的迭代自优化框架，利用多模态思维链过程基于物理不一致反馈逐步优化提示词。
...
TL;DR: BTTF是一个优化框架，用于为视频分类器生成反事实解释，通过条件化初始潜在噪声和两阶段优化策略，确保生成的时间连贯、物理合理的反事实视频。


<details>
  <summary>Details</summary>
Motivation: 现有反事实解释方法主要针对图像分类器设计，无法生成时间连贯、运动平滑且物理合理的视频反事实解释，视频分类器的解释方法研究不足。

Method: 提出BTTF优化框架，包含：1）基于输入视频第一帧的条件化初始潜在噪声检索；2）在输入视频附近搜索反事实视频的两阶段优化策略；3）渐进式优化策略加速收敛。

Result: 在Shape-Moving、MEAD和NTU RGB+D等视频数据集上的实验表明，BTTF能有效生成有效、视觉相似且真实的反事实视频，为分类器决策机制提供具体见解。

Conclusion: BTTF成功解决了视频反事实解释的生成问题，生成的视频具有时间连贯性和物理合理性，为理解视频分类器决策提供了可靠工具。
...
TL;DR: 提出PromptCT框架，通过Lipschitz约束网络LipNet和显式提示模块解决稀疏视图CT重建中的收敛性证明和存储成本问题，实现单一模型处理多种稀疏视图配置。


<details>
  <summary>Details</summary>
Motivation: 现有深度展开算法存在两个主要问题：(i) 先验网络难以明确证明满足Lipschitz约束；(ii) 多视图设置下需要训练多个模型，存储成本高，阻碍临床应用。
...
TL;DR: CrossEarth-Gate是一个用于遥感图像语义分割的参数高效微调方法，通过构建包含空间、语义和频率模块的工具箱，并结合Fisher信息引导的自适应选择机制，动态激活关键模块来处理多方面的域间差异。


<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调方法在处理大规模地球观测任务时往往失败，因为它们无法充分处理遥感数据中多方面的不可预测的域间差异（如空间、语义和频率偏移）。

Method: 1) 建立全面的遥感模块工具箱，包含空间、语义和频率模块来处理多方面域间差异；2) 开发基于Fisher信息引导的自适应选择机制，量化每个模块的重要性并动态激活最关键的模块。

Result: 在16个跨域遥感语义分割基准测试中取得了最先进的性能，验证了方法的有效性和泛化能力。

Conclusion: CrossEarth-Gate通过模块化工具箱和自适应选择机制，有效解决了遥感数据中的多方面域间差异问题，实现了参数高效且性能优越的跨域语义分割。
...
TL;DR: TaCo是一个时空语义一致性网络，通过引入文本引导的过渡生成器和时空语义联合约束，解决了遥感变化检测中语义不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法仅依赖掩码监督，虽然能有效指导空间定位，但对时间语义转换的约束有限，导致预测结果存在语义不一致问题。

Method: 提出文本引导的过渡生成器，将文本语义与双时相视觉特征结合构建跨时相过渡特征；设计时空语义联合约束，包括双时相重构约束和过渡约束。

Result: 在六个公开数据集上的实验表明，TaCo在二值和语义变化检测任务中均达到最先进的性能，且推理时不引入额外计算开销。

Conclusion: TaCo通过时空语义联合约束有效提升了变化检测性能，证明了将变化概念化为语义转换的有效性。
...
TL;DR: TReFT是一种新方法，通过直接使用预训练Rectified Flow模型预测的速度作为输出来实现单步图像翻译，解决了对抗训练中的收敛问题，实现实时推理。


<details>
  <summary>Details</summary>
Motivation: Rectified Flow模型在图像翻译中仍依赖多步去噪，阻碍实时应用。直接应用CycleGAN-Turbo到RF模型会导致严重收敛问题。

Method: 直接使用预训练DiT或UNet预测的速度作为输出，结合内存高效的潜在循环一致性和身份损失，以及轻量级架构简化。

Result: 在多个图像翻译数据集上达到与最先进方法相当的性能，同时支持实时推理。

Conclusion: TReFT成功驯服了Rectified Flow模型，实现了高效的单步图像翻译，为实时应用提供了可行方案。
...
TL;DR: 提出IrisNet，一种元学习框架，通过图像到解码器的变换器动态适应红外小目标检测策略，解决静态模式学习在多样化场景中的模式漂移问题。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测面临低信噪比、复杂背景和目标特征缺失等挑战，现有深度学习方法存在跨场景模式漂移问题，限制了鲁棒性。

Method: 建立红外图像特征与整个解码器参数之间的动态映射，使用图像到解码器变换器，将参数化解码器表示为结构化2D张量，通过自注意力建模层间依赖关系，交叉注意力生成自适应解码模式，并集成高频组件增强感知能力。

Result: 在NUDT-SIRST、NUAA-SIRST和IRSTD-1K数据集上的实验表明，IrisNet实现了最先进的性能。

Conclusion: IrisNet通过动态适应检测策略，有效解决了红外小目标检测中的模式漂移问题，提升了检测鲁棒性。
...
TL;DR: 本文提出了一种基于公正世界模型的端到端自动驾驶策略优化框架，通过反事实合成方法教导模型诚实预测危险，显著提升了安全性能。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶模型在安全性和处理长尾事件方面面临挑战，强化学习虽然有望解决这些问题，但受限于世界模型的乐观偏见。

Method: 开发了反事实合成数据管道，系统生成可信的碰撞和偏离道路事件，构建公正世界模型作为内部评判器，在策略优化中预测候选动作的后果。

Result: 在风险预见基准测试中显著优于基线模型，在挑战性模拟环境中大幅减少了安全违规行为。

Conclusion: 教导模型预见危险是构建真正安全智能自动驾驶系统的关键步骤。
...
TL;DR: 本文训练了一个用于感知双目视觉目标三维运动信息的网络，能够提供实时三维坐标、速度和加速度，具有基本的时空感知能力。从PID角度理解神经网络拟合非线性问题的能力，设计了17层41.3万参数的PID卷积神经网络，在模拟随机运动球数据集上取得了接近输入图像分辨率上限的预测精度。


<details>
  <summary>Details</summary>
Motivation: 从PID控制理论的角度理解神经网络处理非线性问题的能力，探索神经网络设计的基本原理，开发具有实时三维运动感知能力的视觉系统。

Method: 将单层神经网络视为使用二阶差分方程和非线性描述局部问题，多层网络通过多次组合逐步转换原始表示。设计了17层PID卷积神经网络，采用拼接和池化的特征复用方法，使用模拟随机运动球数据集进行训练和测试。

Result: 实验结果显示预测精度接近输入图像分辨率所能表示的上限，验证了网络的有效性。分析了实验结果、误差以及存在的不足和改进方向。

Conclusion: 高维卷积在提高计算效率和特征空间利用率方面具有优势，使用PID信息实现记忆和注意力机制具有潜在优势，为神经网络设计提供了新的视角和方法。
...
TL;DR: 提出基于深度学习的单图像单应性估计框架，使用ConvNeXt骨干网络和归一化坐标回归，通过合成单应性增强数据，在货架图像校正任务中达到1.298像素的平均角点误差。


<details>
  <summary>Details</summary>
Motivation: 解决零售领域单视角货架监控和产品对齐的挑战，传统方法在单图像单应性估计方面存在困难。

Method: 使用ConvNeXt骨干网络提取特征，采用归一化坐标回归提高稳定性，提出合成单应性增强策略解决数据稀缺问题。

Result: 在测试集上达到1.298像素的平均角点误差，在准确性和推理速度方面均优于传统计算机视觉和深度学习方法。

Conclusion: 该方法为实际单视角图像校正提供了鲁棒高效的解决方案，并公开数据集和代码以促进该领域研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有3D重建方法在度量尺度重建、任务泛化性和计算效率方面的局限性，提供一种紧凑且通用的3D场景表示方法。

Method: 使用稀疏但紧凑的体素场景表示作为后端，支持多视图重建，并可直接扩展到未标定视觉里程计和大规模运动恢复结构任务。

Result: 在相机姿态、深度和度量尺度估计、3D重建方面达到最先进性能，甚至超越基于优化的SLAM和SfM方法。

Conclusion: AMB3R展示了紧凑体素表示在多任务3D视觉中的有效性，为通用3D感知系统提供了有前景的方向。
...
TL;DR: 提出仅使用相机的3D重建管道，通过3D高斯泼溅从多视角图像重建场景，结合语义材料掩码和物理材质属性，实现与LiDAR-相机融合相当的传感器仿真精度，同时消除硬件复杂性和校准需求。


<details>
  <summary>Details</summary>
Motivation: 传统LiDAR-相机融合方法需要复杂校准，且对玻璃等材料处理不佳。相机能自然捕捉语义和纹理信息，但缺乏准确的物理材质属性。

Method: 使用多视角图像通过3D高斯泼溅重建场景，通过视觉模型提取语义材料掩码，将高斯表示转换为带材料标签的网格表面，并分配物理材质属性用于传感器仿真。

Result: 在内部数据集上验证，使用LiDAR作为反射率真值，结合图像相似度指标，证明该方法能达到与LiDAR-相机融合相当的传感器仿真精度。

Conclusion: 相机仅方法结合了照片级真实重建与物理材质分配，在保持传感器仿真精度的同时，显著简化了硬件设置和校准流程。
...
TL;DR: 提出人形视觉搜索任务，在360°全景图像中模拟人类头部旋转搜索物体或路径，构建H* Bench基准测试，发现现有模型表现不佳，通过后训练技术显著提升性能，但路径搜索仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉搜索方法局限于静态图像，忽略了物理体现与3D世界的交互，需要开发能像人类一样高效搜索的具身视觉搜索代理。

Method: 提出人形视觉搜索任务，构建H* Bench基准测试，包含交通枢纽、大型零售空间等复杂场景，使用后训练技术增强开源模型Qwen2.5-VL。

Result: 顶级专有模型成功率仅约30%，通过后训练将Qwen2.5-VL的物体搜索成功率从14.83%提升至47.38%，路径搜索从6.44%提升至24.94%。

Conclusion: 路径搜索的难度揭示了构建能够无缝融入人类日常生活的MLLM代理仍面临巨大挑战，但展示了有前景的发展方向。
...
TL;DR: 提出GS-Checker方法用于定位3D高斯泼溅模型中的篡改区域，通过引入3D篡改属性和对比机制，无需昂贵3D标签即可实现准确篡改定位。


<details>
  <summary>Details</summary>
Motivation: 随着3D高斯泼溅编辑技术的发展，恶意篡改3D内容的风险增加，需要有效方法来定位篡改区域以防止恶意应用。

Method: 在3D高斯参数中集成篡改属性；设计3D对比机制比较高斯间关键属性相似性；引入循环优化策略精炼篡改属性。

Result: 大量实验结果表明该方法能有效定位篡改的3DGS区域。

Conclusion: GS-Checker是一种无需3D监督标签即可精确定位3D高斯泼溅模型篡改区域的有效方法。
...
TL;DR: BoxPromptIML是一个弱监督图像篡改定位框架，通过粗粒度区域标注策略降低标注成本，使用知识蒸馏训练轻量学生模型，结合双引导特征融合模块提升定位精度。
...
Method: 采用粗粒度区域标注策略生成相对准确的篡改掩码；设计轻量学生模型通过知识蒸馏从SAM教师模型学习；基于人类潜意识记忆机制的双引导特征融合模块。
...
TL;DR: VGGTFace：基于3D基础模型VGGT的自动面部几何重建方法，从日常用户拍摄的多视角图像中重建拓扑一致的面部几何，在10秒内完成16视图的高质量重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要大量人工操作、对野外数据泛化能力不足，或受限于3D形变模型的表达能力有限。需要一种能自动重建拓扑一致面部几何的方法。

Method: 利用VGGT 3D基础模型获得泛化能力和表达能力，通过Pixel3DMM注入拓扑信息，提出拓扑感知的束调整策略，构建拉普拉斯能量进行优化。

Result: 在单张RTX 4090上10秒内完成16视图的高质量重建，在基准测试中达到最先进水平，对野外数据具有出色的泛化能力。

Conclusion: VGGTFace成功解决了拓扑一致面部几何重建的挑战，结合了VGGT的泛化能力和拓扑信息注入，实现了高效高质量的重建。
...
TL;DR: FREE框架通过特征级自回归和并行验证，在扩散变换器（DiTs）上实现无损加速，并通过不确定性引导的松弛策略进一步提升了加速效果。


<details>
  <summary>Details</summary>
Motivation: DiTs虽然生成质量优秀，但需要长序列去噪轨迹导致推理延迟高。现有的推测推理方法在DiTs上加速有限，因为验证过程中的草稿准确性不足。

Method: 提出FREE框架，利用DiTs顶层特征的时间一致性和语义抽象特性，通过轻量级草稿器进行特征级自回归和并行验证。并引入不确定性引导的松弛策略（FREE (relax)）动态调整接受概率。

Result: 在ImageNet-512²上，FREE实现了1.86倍加速，FREE (relax)进一步达到2.25倍加速，同时保持了高质量生成。

Conclusion: FREE框架有效解决了DiTs推理延迟问题，通过特征级推测推理和不确定性感知机制实现了显著加速，且保持生成质量无损。
...
TL;DR: MultiID是一个无需训练的多ID定制方法，通过ID解耦交叉注意力机制将多个个体身份无缝整合到生成图像中，同时解决了复制粘贴问题和文本控制性问题。


<details>
  <summary>Details</summary>
Motivation: 多ID定制比单ID定制更困难，面临两个主要挑战：1）训练时基于裁剪人物区域重建图像导致推理时出现复制粘贴问题；2）模型文本控制性较差，生成结果简单组合多个人物而不考虑与输入文本的对齐。

Method: 提出ID解耦交叉注意力机制，将不同ID嵌入注入对应图像区域；引入局部提示、深度引导空间控制和扩展自注意力三种策略增强生成可控性；构建IDBench基准进行评估。

Result: 广泛的定性和定量结果表明MultiID在解决上述两个挑战方面有效，其性能与基于训练的多ID定制方法相当甚至更好。

Conclusion: MultiID成功实现了无需训练的多ID定制，通过创新的注意力机制和控制策略有效解决了复制粘贴和文本控制性问题，在多ID定制任务上表现出色。
...
TL;DR: 提出轨迹反向一致性模型(TBCM)，通过直接从教师模型的生成轨迹中提取潜在表示，消除对外部训练数据的依赖，显著提升扩散模型生成效率。


<details>
  <summary>Details</summary>
Motivation: 解决当前连续时间一致性蒸馏方法对训练数据和计算资源依赖严重的问题，使其能够在资源受限场景中部署并扩展到多样化领域。

Method: 采用自包含蒸馏范式，从教师模型生成轨迹中提取潜在表示，无需VAE编码和大规模数据集，通过轨迹提取样本弥合训练与推理分布差距。

Result: 在MJHQ-30k数据集上，单步生成达到6.52 FID和28.08 CLIP分数，训练时间比Sana-Sprint减少约40%，节省大量GPU内存。

Conclusion: TBCM在保持质量的同时实现了优越的效率，揭示了连续时间一致性蒸馏中的扩散-生成空间差异，为未来蒸馏研究提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以平衡基于文本生成的创意灵活性和显式结构表示的对象级可编辑性，需要一种能同时满足风格多样性、细粒度和可控性的3D城市生成方案。

Method: 将城市表示为可控布局、资源和材料的组合，采用四阶段流水线；集成MajutsuAgent支持五种对象级编辑操作；构建包含2D语义布局、高度图、3D建筑资源和PBR材料的多模态数据集。

Result: 布局FID比CityDreamer降低83.7%，比CityCraft降低20.1%；在所有AQS和RDR评分中排名第一，明显优于现有方法。

Conclusion: MajutsuCity在几何保真度、风格适应性和语义可控性方面达到新的最先进水平，为3D城市生成研究开辟了新途径。
...
Conclusion: StableTrack方法在低频检测条件下显著提升了多目标跟踪性能，为资源受限环境下的实时跟踪提供了有效解决方案。
...
TL;DR: Block Cascading通过训练无关的并行化技术，显著缓解了块因果视频生成中的速度-质量权衡问题，实现了2倍加速且不损失生成质量。


<details>
  <summary>Details</summary>
Motivation: 块因果视频生成面临严重速度-质量权衡：小模型(1.3B)仅16FPS，大模型(14B)仅4.5FPS，用户必须在响应性和质量间做选择。

Method: 利用关键洞察：未来视频块不需要完全去噪的当前块即可开始生成。通过使用部分去噪的上下文启动块生成，将顺序流水线转换为并行级联，多个块同时去噪。

Result: 使用5个GPU实现约2倍加速：1.3B模型从16FPS加速到30FPS，14B模型从4.5FPS加速到12.5FPS，同时消除了KV重缓存的开销(~200ms)。

Conclusion: Block Cascading在保持生成质量的同时显著提升了推理速度，为交互式视频生成提供了高效解决方案。
...
Method: BRIC在测试时动态适应物理控制器以适应噪声运动计划，同时通过损失函数保留预训练技能；引入轻量级测试时引导机制在信号空间引导扩散模型。

Result: 在多种长期任务（运动组合、避障、人-场景交互）上实现最先进性能，确保一致且物理合理的长期执行。

Conclusion: BRIC通过结合两种适应策略，以高效方式确保跨多样环境的一致性和物理合理性。
...
TL;DR: OC-VTP是一种直接且保证性的视觉token剪枝方法，通过轻量级预训练的目标中心视觉token剪枝器，选择最具代表性的视觉token来提升VLM推理效率，同时保持精度。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中视觉token数量多但信息分散，消耗过多不必要的计算资源，现有方法都是间接且无法保证效果的方式。

Method: 通过轻量级预训练一个小的目标中心视觉token剪枝器，最小化从选定token重建原始未剪枝token的误差，保证保留最具代表性的视觉token。

Result: 在各种视觉剪枝比例下，OC-VTP始终帮助主流VLM保持最高的推理精度，并展现出有趣的解释性。

Conclusion: OC-VTP提供了一种直接且保证性的视觉token剪枝方案，无需在任何数据集上微调任何模型，即可实现高效率且精度保持的VLM推理。
...
TL;DR: 提出人类-人类-对象交互(HHOIs)新问题，开发数据集和生成方法，通过扩散模型合成多人类交互动作


<details>
  <summary>Details</summary>
Motivation: 人类交互行为具有情境依赖性，需要建模多人与场景的关系，现有研究主要关注单人交互，缺乏多人对象交互的专门研究

Method: 构建HHOI数据集，使用图像生成模型合成数据，分别训练文本到HOI和HHI的扩散模型，然后整合成统一生成框架

Result: 能够根据文本描述生成真实的HHOIs，在多人交互生成方面优于仅关注单人HOI的方法，支持多于两人的交互

Conclusion: 提出的HHOI框架成功扩展了多人类交互生成，为涉及对象的多人类运动生成提供了有效解决方案
...
TL;DR: ZoomSearch是一个无需训练、即插即用的超高分遥感视觉问答流水线，通过自适应多分支缩放搜索和布局感知补丁重组来解决超高分辨率遥感图像处理中的效率和精度问题。


<details>
  <summary>Details</summary>
Motivation: 当前遥感基础模型无法有效处理超高分辨率图像：全图编码会耗尽token和内存预算，而基于缩放的预处理会丢失细粒度和关键细节。因此需要引导模型在预测前关注重要区域。

Method: ZoomSearch结合了自适应多分支缩放搜索（对图像块进行分层搜索以定位查询相关区域）和布局感知补丁重组（将选定块重新组织成紧凑、布局忠实的画布）。

Result: 在LLaVA-ov上集成ZoomSearch后，在LRS-VQA上比基线提升26.3%，在MME-RealWorld-RS上提升114.8%，同时推理效率比之前基于搜索的方法快20%~44%。

Conclusion: ZoomSearch在超高分辨率遥感视觉问答任务中实现了最先进的精度和更高的推理效率，证明了其方法的有效性。
...
TL;DR: STARFlow-V是基于归一化流的视频生成模型，通过全局-局部架构和流-分数匹配技术，在保持因果依赖的同时实现高质量视频生成，支持文本到视频、图像到视频等多种任务。
...
Method: 采用时空潜在空间的全局-局部架构，限制因果依赖到全局潜在空间，同时保留帧内局部交互；提出流-分数匹配技术增强生成一致性；使用视频感知的Jacobi迭代提高采样效率。

Result: STARFlow-V在视觉保真度和时间一致性方面表现强劲，采样吞吐量优于扩散基线模型，证明了归一化流能够实现高质量自回归视频生成。

Conclusion: 这是首个证明归一化流能够实现高质量自回归视频生成的工作，为构建世界模型提供了一个有前景的研究方向。
...
TL;DR: 提出了一种基于Laban运动分析的轻量级舞蹈风格分类框架，使用姿态估计提取时空描述符和傅里叶变换特征来识别不同舞蹈风格。


<details>
  <summary>Details</summary>
Motivation: 舞蹈是人类文化的重要组成部分，但基于运动数据识别舞蹈风格很复杂，因为许多风格具有相似的姿势、手势和时间运动模式。

Method: 从视频中提取姿态估计，设计受Laban运动分析启发的时空描述符，捕捉关节动态特征，并集成快速傅里叶变换特征来编码运动的节奏和周期性方面。

Result: 该方法能够以较低计算成本对不同舞蹈风格进行稳健分类，无需复杂模型架构。

Conclusion: 可解释的运动表示能够有效捕捉风格细微差别，证明了轻量级框架在舞蹈风格分类中的有效性。
...
TL;DR: 本文提出了一种模块化架构，包含三个独立感知模块：CNN用于眼睑状态检测，深度CNN用于面部表情识别，LSTM用于语音识别，在各自数据集上分别达到93.0%、97.8%和96.89%的准确率。
...
Method: 提出三个独立感知模块：CNN用于眼睑状态检测（困倦/注意力），深度CNN用于面部表情识别，LSTM用于基于语音的说话人识别。

Result: 在Eyes Image、FER2013和定制音频数据集上，模型准确率分别达到93.0%、97.8%和96.89%。

Conclusion: 研究表明轻量级、领域特定模型可以在离散任务上实现高保真度，为未来资源受限辅助设备中的实时多模态集成建立了验证基础。
...
TL;DR: 提出了一种基于物理学的损失函数（PIL），用于改进脑血管DSA序列的分割精度，通过模拟弹性边界交互来增强血管几何结构的捕捉能力。
...
Result: 在DIAS和DSCA两个公开基准测试中，PIL在U-Net、U-Net++、SegFormer和MedFormer等架构上均优于交叉熵、Dice、主动轮廓和表面损失等传统损失函数，实现了更高的灵敏度、F1分数和边界一致性。
...
TL;DR: AlignBench是一个新的图像-文本对齐评估基准，通过评估多样化模型生成的详细图像-描述对来提供对齐指标，揭示了CLIP模型在细粒度对齐评估中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要依赖规则扰动或简短描述，无法有效衡量细粒度的图像-文本对齐，需要更全面的评估方法。

Method: 构建AlignBench基准，包含由多样化图像到文本和文本到图像模型生成的详细图像-描述对，每个句子都进行正确性标注，直接评估视觉语言模型作为对齐评估器的能力。

Result: 评估发现：(i) CLIP模型在细粒度对齐上表现接近盲视；(ii) 检测器系统性地高估早期句子；(iii) 模型表现出强烈的自偏好，偏向自身输出并损害检测性能。

Conclusion: AlignBench为图像-文本对齐提供了更全面的评估框架，揭示了现有模型的局限性，特别是CLIP模型在细粒度对齐评估中的不足。
...
TL;DR: HBridge提出了一种非对称的H形架构，用于统一多模态模型，通过选择性桥接中间层来优化异构专家的预训练先验利用，减少40%以上的注意力共享，提高效率和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型采用对称的Transformer混合设计，但由于模态差异导致性能次优。需要一种能够更好利用异构专家各自模态领域预训练先验的架构。

Method: 提出HBridge非对称H形架构，选择性桥接中间层，解耦浅层和深层模态特定表示，中层桥接促进语义对齐，并引入语义重建令牌来增强跨模态连贯性。

Result: 在多个基准测试上的广泛实验证明了HBridge的有效性和优越性能。

Conclusion: HBridge为统一多模态生成建立了新的范式，通过非对称设计和选择性桥接实现了更好的性能和效率。
...
TL;DR: 提出了Mistake Attribution (MATT)任务，用于细粒度理解第一人称视频中的人类错误，通过MisEngine数据引擎自动构建错误样本，并开发了统一的MisFormer模型进行错误归因。


<details>
  <summary>Details</summary>
Motivation: 现有错误理解工作缺乏细粒度输出，无法具体将错误归因到指令文本或尝试视频中。

Method: 开发MisEngine数据引擎自动构建错误样本，创建EPIC-KITCHENS-M和Ego4D-M数据集；提出MisFormer统一注意力模型，在语义、时间和空间维度进行错误归因。

Result: 在新建数据集和现有基准测试中，MisFormer优于强基线方法，包括视频语言、时序定位、手物交互和错误检测模型。
...
TL;DR: 该论文比较了不同CNN编码器的U-Net架构在文化遗产裂缝检测中的语义分割性能，在OmniCrack30k数据集上进行定量评估，并在真实世界雕像和纪念碑上进行定性评估。
...
Method: 使用不同CNN编码器的U-Net架构进行像素级裂缝识别，在OmniCrack30k测试集上进行定量评估（mIoU、Dice系数、Jaccard指数），并在未标记的真实世界雕像数据集上进行定性评估。

Result: 模型展现出对未见过的文化遗产环境的良好泛化能力，即使从未在雕像或纪念碑图像上明确训练过。

Conclusion: 研究为不同CNN编码器在细粒度裂缝分割中的能力提供了有价值的见解，证明了模型在文化遗产背景下的泛化潜力。
...
<details>
  <summary>Details</summary>
Motivation: 动物嗅觉感知丰富，但机器嗅觉发展受限，主要瓶颈是缺乏在自然环境中收集的多样化多模态嗅觉训练数据。

Method: 构建了纽约气味数据集，包含7000个气味-图像对，覆盖3500个不同物体，涵盖室内外环境，比现有嗅觉数据集物体数量多70倍。

Result: 实验表明视觉数据能够实现跨模态嗅觉表征学习，学习到的嗅觉表征优于广泛使用的手工特征。

Conclusion: 该数据集为机器嗅觉研究提供了重要资源，证明了视觉数据在跨模态嗅觉学习中的有效性。
...
TL;DR: Flash-DMD是一个新颖的扩散模型加速框架，通过高效的时间步蒸馏和联合RL微调，显著降低训练成本并提升生成质量，在少步采样中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的迭代采样过程计算昂贵，现有时间步蒸馏方法需要大量训练且导致图像质量下降，而基于RL的微调过程不稳定且容易陷入奖励攻击。

Method: 提出高效的时间步感知蒸馏策略，大幅降低训练成本；引入联合训练方案，在继续时间步蒸馏的同时进行RL目标微调，利用蒸馏损失作为正则化器稳定RL训练。

Result: 仅需DMD2 2.1%的训练成本就能超越其性能；在基于分数和流匹配的模型上，Flash-DMD收敛更快，在少步采样中在视觉质量、人类偏好和文本图像对齐指标上优于现有方法。

Conclusion: Flash-DMD为训练高效、高保真且稳定的生成模型提供了一个有效范例，解决了扩散模型加速中的训练成本和稳定性问题。
...
Method: 两阶段方法：1) 通过部分感知的物理属性重建估计图像中所有物体的静态初始物理属性；2) 通过时间指导和物理可编辑的模拟合成高质量视频
...
TL;DR: ReaDe是一个通用的、模型无关的指令解释器，通过"先推理后描述"的范式将用户指令转换为精确的视频生成规范，显著提升可控视频生成的意图对齐效果。


<details>
  <summary>Details</summary>
Motivation: 当前扩散变换器虽然提升了视频保真度和时序一致性，但实际可控性有限。用户输入的简洁性、模糊性与训练时使用的详细提示词之间存在不匹配问题。

Method: 采用两阶段优化：1) 推理增强监督，通过逐步推理轨迹和密集标注进行解析训练；2) 多维奖励分配器，通过反馈驱动优化自然风格描述。

Result: 在单条件和多条件场景下均取得一致提升，在指令忠实度、标注准确性和下游视频质量方面表现优异，对推理密集型和新输入具有强泛化能力。

Conclusion: ReaDe为可控视频生成与用户意图的准确对齐提供了一条实用路径。
...
TL;DR: DINO-Tok是一个基于DINO的视觉分词器，通过整合浅层细节特征和深层语义特征，在信息完整的潜在空间中统一层次表示，显著提升了重建性能。
...
Method: 整合DINO的浅层和深层特征，提出全局PCA重加权机制来稳定向量量化，防止信息丢失和码本坍塌。

Result: 在ImageNet 256×256上达到28.54 PSNR（自编码）和23.98 PSNR（VQ建模），显著优于先前方法，与亿级数据训练模型相当。
...
TL;DR: 本文提出VQ-VA World框架，通过大规模数据构建和训练，使开源模型具备视觉问答-视觉回答能力，显著提升在IntelligentBench基准上的表现。


<details>
  <summary>Details</summary>
Motivation: 让开源模型也具备像NanoBanana和GPT-Image等专有系统那样的视觉问答-视觉回答能力，即根据视觉问题生成图像而非文本回答。

Method: 构建VQ-VA World数据中心框架，采用智能代理管道进行大规模目标数据构建，爬取约180万高质量交错图像-文本样本用于模型训练。

Result: 使用VQ-VA World数据训练后，LightFusion在IntelligentBench上达到53.06分，大幅超越之前最佳开源基线（普通LightFusion为7.78，UniWorld-V1为1.94），显著缩小与领先专有系统的差距（NanoBanana为81.67，GPT-Image为82.64）。
...
TL;DR: 本文提出ImageCritic，通过参考图像引导的后编辑方法解决生成图像中的细节不一致问题，包含数据集构建、注意力对齐损失和细节编码器设计，可集成到智能体框架中进行多轮局部编辑。


<details>
  <summary>Details</summary>
Motivation: 现有定制化生成方法在生成一致细粒度细节方面仍存在限制，本文旨在解决生成图像中的不一致性问题。

Method: 构建参考-退化-目标三元组数据集，基于注意力机制和内在表示分析设计注意力对齐损失和细节编码器，集成到智能体框架进行多轮局部编辑。

Result: 广泛实验表明ImageCritic能有效解决各种定制化生成场景中的细节相关问题，相比现有方法有显著改进。

Conclusion: ImageCritic通过参考引导的后编辑方法成功解决了生成图像中的细节不一致问题，在多种场景下表现出色。
...
TL;DR: 该研究探索了使用深度神经网络预测动态负重活动中的全身人体姿态，提出了基于BLSTM和Transformer的时序模型，并通过优化成本函数来保持身体段长度恒定，显著提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 研究旨在开发能够准确预测动态负重活动中人体姿态的神经网络模型，为理解手动物料搬运活动中的运动动力学提供新方法。

Method: 使用BLSTM和Transformer两种时序模型，输入包括手部负荷位置3D坐标、举重技术、身体参数以及任务前25%的姿势数据，预测剩余75%时间的身体坐标。提出新成本函数来强制保持身体段长度恒定。

Result: 新成本函数使手臂和腿部模型的预测误差分别降低约8%和21%。Transformer模型表现更优，均方根误差为47.0mm，比BLSTM模型准确度高约58%。

Conclusion: 研究表明能够捕捉时间序列依赖性的神经网络在3D运动帧预测中具有优势，为理解手动物料搬运活动中的运动动力学提供了独特方法。
...
TL;DR: Wanderland是一个用于具身AI评估的真实到仿真框架，通过多传感器捕获、可靠重建和准确几何，解决了现有方法在视觉导航中的可复现性问题。


<details>
  <summary>Details</summary>
Motivation: 解决具身AI中可复现闭环评估的瓶颈，现有视频-3DGS方法存在较大的视觉和几何仿真到真实差距，不适合基准测试。

Method: 开发了包含多传感器捕获、可靠重建、准确几何和鲁棒视图合成的真实到仿真框架，构建了室内外城市场景的多样化数据集。

Result: 系统证明了仅图像方法扩展性差，几何质量影响新视图合成，这些因素都影响导航策略学习和评估可靠性。

Conclusion: Wanderland为开放世界具身AI的可复现研究建立了新基础，同时可作为3D重建和新视图合成模型的基准测试平台。
...
TL;DR: ShapeGen通过改进3D表示和监督、提升分辨率以及利用线性transformer优势，实现了高质量的图像到3D形状生成，解决了现有方法缺乏细节、表面过度平滑和薄壳结构碎片化的问题。
...
Conclusion: 这些改进的协同效应使ShapeGen在图像到3D生成方面取得了重大突破，为各种应用提供了高质量的3D资产。
...
<details>
  <summary>Details</summary>
Motivation: 解决RLHF中多奖励联合优化时的对齐税问题，即优化一个维度时其他维度性能下降的问题。
...
<details>
  <summary>Details</summary>
Motivation: 预训练视频模型具有强大的时间一致性先验，但其动态范围受限于连续训练数据。通过将图像数据的丰富多样性注入这个时间框架，可以生成既保持自然过渡又具有更广动态范围的图像集。

Method: 提出优雅且最小侵入性的适配策略，配合定制数据整理过程和训练范式，使模型获得广泛图像操作能力而不破坏原有的运动先验。

Result: iMontage在多个主流多输入多输出任务中表现出色，不仅保持强大的跨图像上下文一致性，还生成了超越传统范围的非凡动态场景。

Conclusion: 该框架成功将视频模型重新用于图像生成，实现了时间一致性与图像多样性的有效结合，为图像生成和编辑任务提供了统一解决方案。
...
TL;DR: 提出了一种通过编辑稀疏轨迹来修改视频运动的方法，称为"运动编辑"，结合生成式骨干网络实现强大的视频编辑功能


<details>
  <summary>Details</summary>
Motivation: 虽然生成视频模型在保真度和一致性方面取得了显著进展，但将其应用于视频编辑仍然是一个复杂挑战。精确的运动控制作为编辑现有视频的有前景但尚未充分探索的范式

Method: 通过直接编辑从输入视频中提取的稀疏轨迹来修改视频运动，引入生成"运动反事实"视频对的流程，并在该数据集上微调运动条件视频扩散架构

Result: 在四向头对头用户研究中，该模型相对于先前工作获得了超过65%的偏好率

Conclusion: 该方法允许从任何时间戳开始编辑并自然传播，实现了强大的视频编辑能力
...
TL;DR: CAPNET是一个针对长尾多标签视觉识别的新框架，通过CLIP文本编码器显式建模标签相关性，使用图卷积网络进行标签感知传播，并通过可学习的软提示优化嵌入表示。


<details>
  <summary>Details</summary>
Motivation: 解决长尾多标签视觉识别中存在的两个主要问题：1）现有方法从不平衡数据集中直接推导语义类间关系，导致尾部类别的相关性不可靠；2）CLIP的零样本范式针对单标签图像-文本匹配优化，不适用于多标签任务。

Method: 提出相关性自适应提示网络（CAPNET），包含：图卷积网络用于标签感知传播、可学习软提示用于精炼嵌入、分布平衡的Focal损失用于类别感知重加权、测试时集成提升泛化能力、参数高效微调实现视觉-文本模态对齐。

Result: 在VOC-LT、COCO-LT和NUS-WIDE等基准测试上的广泛实验表明，CAPNET相比现有最先进方法取得了显著改进。

Conclusion: CAPNET验证了其在真实世界长尾多标签视觉识别任务中的有效性，能够在不损害头部类别性能的情况下避免尾部类别的过拟合。
...
TL;DR: 提出DataConcept数据集和概念感知批量采样框架CABS，实现基于特定概念分布的在线数据筛选，显著提升视觉语言模型性能
...
Method: 构建DataConcept数据集，提出CABS框架（包括多样性最大化和频率最大化两种变体），基于目标概念分布动态构建训练批次

Result: 在28个基准测试中，CABS方法显著提升了CLIP/SigLIP模型的性能，成为专有在线数据筛选算法的开源替代方案

Conclusion: CABS框架提供了灵活的概念感知批量采样方法，使研究人员能够根据特定下游任务定制概念分布，优化模型性能
...
TL;DR: VLM²是一个具有持久记忆的视觉语言模型，通过双记忆模块解决视频空间推理中的语义-几何错位和缺乏持久记忆问题，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在基于视频的空间推理中仍达不到人类水平，主要存在语义-几何错位和缺乏持久记忆两个挑战。

Method: 提出VLM²模型，采用双记忆模块：工作记忆作为滑动窗口关注即时上下文，情景记忆整合存储关键长期信息，实现固定计算成本下的高效长时程空间推理。

Result: 在多个基准测试中，VLM²在仅使用视频输入的模型中达到了最先进的性能。

Conclusion: VLM²显著推进了视觉空间智能的前沿，通过持久记忆和3D感知表示解决了视频空间推理的关键挑战。
...
TL;DR: 提出了PixelDiT，一种直接在像素空间进行扩散建模的单阶段端到端模型，消除了对预训练自编码器的依赖，通过双层级Transformer架构实现高效的像素级扩散训练。


<details>
  <summary>Details</summary>
Motivation: 解决传统潜在空间扩散模型的两阶段流程问题：预训练自编码器引入有损重建，导致误差累积并阻碍联合优化。

Method: 采用完全基于Transformer的双层级架构：补丁级DiT捕获全局语义，像素级DiT细化纹理细节，实现像素空间的端到端扩散建模。

Result: 在ImageNet 256×256上达到1.61 FID，大幅超越现有像素生成模型；在1024×1024分辨率文本到图像生成中，GenEval达到0.74，DPG-bench达到83.5，接近最佳潜在扩散模型性能。

Conclusion: 有效的像素级token建模是像素扩散成功的关键，PixelDiT证明了在像素空间直接进行端到端扩散建模的可行性，性能接近甚至超越传统潜在空间方法。
...
TL;DR: 提出一种通过跨视图相关性注入几何一致性的多任务学习方法，在单网络中同时处理分割和深度估计等密集预测任务
...
Method: 引入轻量级跨视图模块(CvM)，通过跨视图信息交换和相关性捕捉来整合几何一致性，该模块与MTL编码器特征结合进行多任务预测

Result: 在NYUv2和PASCAL-Context数据集上的广泛实验表明，该方法能有效提升现有MTL方法的性能

Conclusion: 提出的跨视图模块能够为多任务学习注入几何一致性，改善密集预测任务的性能表现
...
TL;DR: DPP-GRPO是一个用于多样化视频生成的框架，通过结合行列式点过程和组相对策略优化理论，在保持提示保真度和感知质量的同时，显式地奖励多样化生成。
...
Method: 将多样化视频生成建模为集合级策略优化问题，使用DPP对冗余样本施加递减回报，通过GRPO对候选集提供组级反馈。

Result: 在WAN和CogVideoX上实现，在VBench、VideoScore和人类偏好研究中持续提升视频多样性。

Conclusion: 该框架是即插即用且模型无关的，能够在不牺牲提示保真度或感知质量的情况下，促进视觉外观、相机运动和场景结构的多样化生成。
...
Method: 使用Chain-of-Sight序列：先在2D中检测物体，然后预测距离、尺寸和姿态；采用从近到远的课程学习策略和中心-尺寸-旋转的分解方法。

Result: 在Omni3D基准上达到49.89 AP_3D，比之前最佳方法提升15.51，即使基线使用真实2D框；对未见类别具有强零样本泛化能力。
...
TL;DR: ∞-RoPE是一个推理时框架，解决了自回归视频扩散模型的三个核心瓶颈：有限的时间范围、缓慢的提示响应和无法实现不连续的电影过渡。


<details>
  <summary>Details</summary>
Motivation: 当前自回归视频扩散模型受限于3D-RoPE的有限时间范围、长序列生成时提示响应缓慢，以及无法在单个生成流中实现不连续的电影过渡。

Method: 提出三个组件：Block-Relativistic RoPE（将时间编码重新表述为移动局部参考系）、KV Flush（通过保留全局接收器和最后生成的潜在帧来更新KV缓存）、RoPE Cut（在时间RoPE坐标中引入受控的不连续性）。

Result: ∞-RoPE在VBench评分中持续优于先前的自回归模型，实现了无限范围、可控和电影化的视频扩散。

Conclusion: ∞-RoPE作为一个无需训练的框架，为无限范围、可控和电影化的视频扩散奠定了基础。
...
Result: MedROV在医学图像检测上比现有最佳基础模型平均提升40 mAP50，比闭集检测器提升超过3 mAP50，同时运行速度达到70 FPS。

Conclusion: MedROV为医学影像开放词汇检测设立了新的基准，有效解决了闭集检测的局限性，在检测性能和速度上都表现出色。
...
TL;DR: RubricRL是一个基于评分标准的强化学习框架，用于文本到图像生成模型的对齐，通过可分解的细粒度视觉标准提供可解释和可组合的奖励设计。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定权重的复合指标或单一标量奖励，限制了可解释性和灵活性，需要更透明和可控的奖励设计方法。

Method: 为每个提示动态构建结构化评分标准，包含对象正确性、属性准确性、OCR保真度和真实感等细粒度标准，由多模态评估器独立评估，并使用提示自适应加权机制。

Result: 实验表明RubricRL提高了提示忠实度、视觉细节和泛化能力，同时为可解释的RL对齐提供了灵活可扩展的基础。

Conclusion: RubricRL提供了一个简单通用的框架，通过结构化评分标准实现更好的可解释性、可组合性和用户控制，适用于各种文本到图像架构。
...
### [227] [PhysDNet: Physics-Guided Decomposition Network of Side-Scan Sonar Imagery](https://arxiv.org/abs/2511.19539)
*Can Lei,Hayat Rajani,Nuno Gracias,Rafael Garcia,Huigang Wang*

Main category: physics.ao-ph

TL;DR: PhysDNet是一个物理引导的多分支网络，通过将侧扫声纳图像分解为三个可解释的物理场（海底反射率、地形高程和传播损耗），解决了声纳图像视角依赖性强的问题。


<details>
  <summary>Details</summary>
Motivation: 侧扫声纳图像的强度受到海底反射率、地形高程和声学路径损耗的复杂影响，导致图像具有强烈的视角依赖性，降低了后续分析的鲁棒性。

Method: 嵌入Lambertian反射模型，使用多分支网络自监督地分解声纳图像为三个物理场，无需地面真值标注即可训练。

Result: 分解后的表示保持了稳定的地质结构，捕捉了物理一致的照明和衰减，并产生了可靠的阴影图。

Conclusion: 物理引导的分解为侧扫声纳分析提供了一个稳定且可解释的领域，提高了物理一致性和下游任务（如配准和阴影解释）的性能。
...
### [228] [Redefining Radar Segmentation: Simultaneous Static-Moving Segmentation and Ego-Motion Estimation using Radar Point Clouds](https://arxiv.org/abs/2511.20003)
*Simin Zhu,Satish Ravindran,Alexander Yarovoy,Francesco Fioranelli*

Main category: eess.SP

TL;DR: 提出了一种基于神经网络的雷达点云分割方法，能同时分割静态和动态物体，并估计车辆自身运动速度，无需复杂的中间信号处理步骤。


<details>
  <summary>Details</summary>
Motivation: 传统雷达分割研究主要关注物体类别标签，但雷达与光学传感器的差异使得类别预测可靠性不同。在汽车雷达感知任务中，判断物体是否运动是大多数任务的前提条件。

Method: 使用多层感知机(MLPs)和循环神经网络(RNNs)等简单有效的构建块进行特征提取，直接从原始点云中提取信息，无需点云聚合、多普勒补偿、运动补偿等中间处理步骤。

Result: 在RadarScenes数据集上测试，该方法在双重任务上表现良好，并展示了在其他雷达感知任务中的广泛应用潜力。

Conclusion: 该方法首次实现了从雷达点云中同时分割静态/动态物体和估计自身运动速度，证明了直接从原始点云提取信息的可行性，为雷达感知任务提供了新思路。
...
TL;DR: FaraGen是一个用于多步骤网页任务的合成数据生成系统，能够生成多样化的任务和解决方案，并通过验证器筛选成功轨迹。基于此数据训练的Fara-7B模型在多个基准测试中表现出色，甚至能与更大的前沿模型竞争。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理（CUAs）的发展受到缺乏大规模高质量人类与计算机交互数据的限制，而LLMs得益于丰富的文本数据，CUAs领域缺乏可比的数据集。

Method: 开发FaraGen系统生成多步骤网页任务的合成数据，包括提出多样化任务、生成多个解决方案尝试，并使用多个验证器筛选成功轨迹。基于这些数据训练Fara-7B模型，该模型仅通过截图感知计算机，通过预测坐标执行动作，且足够小可在设备上运行。

Result: FaraGen以约1美元的成本生成每个已验证轨迹，实现高吞吐量、产量和多样性。Fara-7B在WebVoyager、Online-Mind2Web和WebTailBench等基准测试中优于同类规模的CUA模型，且与更大的前沿模型具有竞争力。
...
### [230] [Scaling Agentic Reinforcement Learning for Tool-Integrated Reasoning in VLMs](https://arxiv.org/abs/2511.19773)
...
TL;DR: VISTA-Gym是一个可扩展的训练环境，旨在提升视觉语言模型的多步骤视觉推理能力，通过工具集成和强化学习实现视觉代理推理。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在图像理解方面表现良好，但在多步骤视觉交互推理方面仍有局限，需要提升工具选择、调用和协调能力。

Method: 开发VISTA-Gym统一环境，整合7个任务13个数据集，提供标准化视觉工具接口、可执行交互循环和反馈信号，通过多轮轨迹采样和端到端强化学习训练VISTA-R1模型。
...
### [231] [CostNav: A Navigation Benchmark for Cost-Aware Evaluation of Embodied Agents](https://arxiv.org/abs/2511.20216)
...
TL;DR: CostNav是首个将导航性能与商业可行性结合的微导航经济测试平台，通过成本-收益分析揭示导航算法在真实商业部署中的经济表现。
...
Method: 建立完整的成本-收益模型，包括硬件、训练、能源、维护成本和配送收入，使用行业参数进行经济生命周期分析。

Result: 基准算法达到43.0%的服务水平协议合规率，但商业上不可行：每次运行亏损30.009美元，无盈亏平衡点，维护成本占运行成本的99.7%。

Conclusion: 优化任务成功率与优化经济部署存在根本差异，碰撞避免是关键的优化目标，CostNav为导航研究提供了经济可行性评估基础。
...
### [232] [VibraVerse: A Large-Scale Geometry-Acoustics Alignment Dataset for Physically-Consistent Multimodal Learning](https://arxiv.org/abs/2511.20422)
*Bo Pang,Chenxi Xu,Jierui Ren,Guoping Wang,Sheng Li*

Main category: cs.AI

TL;DR: VibraVerse是一个大规模几何-声学对齐数据集，通过物理一致的因果链连接3D几何、物理属性、模态参数和声学信号，支持跨模态学习。
...
Method: 引入CLASP对比学习框架进行跨模态对齐，保持物体物理结构与声学响应之间的因果对应关系。
...
### [233] [Beyond Generation: Multi-Hop Reasoning for Factual Accuracy in Vision-Language Models](https://arxiv.org/abs/2511.20531)
*Shamima Hossain*

Main category: cs.AI

TL;DR: 提出一个基于知识图谱的视觉语言模型推理框架，通过多跳验证提升事实准确性


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型缺乏稳健推理能力，常产生事实错误输出，需要整合外部知识进行多模态推理

Method: 利用结构化知识图谱进行多跳验证，包括视觉实体识别、知识图谱遍历和基于事实的标题优化

Result: 在混合数据集上的初步实验显示事实准确性提升约31%，揭示了推理模式和失败案例

Conclusion: 整合外部知识可显著提升视觉语言模型的推理能力，为更可靠的多模态系统铺平道路
...
### [234] [Dual-Path Knowledge-Augmented Contrastive Alignment Network for Spatially Resolved Transcriptomics](https://arxiv.org/abs/2511.17685)
*Wei Zhang,Jiajun Chu,Xinci Liu,Chen Tong,Xinyue Li*

Main category: q-bio.QM

TL;DR: DKAN是一种新型双路径知识增强对比对齐网络，通过整合组织病理图像和基因表达谱来预测空间基因表达，解决了现有方法在生物上下文利用、范例依赖和异质模态对齐方面的局限性。
...
Method: 提出DKAN网络，包含基因语义表示模块（利用外部基因数据库）、统一单阶段对比学习范式（结合对比学习和监督学习）、双路径对比对齐模块（使用基因语义特征作为跨模态协调器）。

Result: 在三个公共ST数据集上的广泛实验表明，DKAN在空间基因表达预测方面优于最先进模型，建立了新的性能基准。

Conclusion: DKAN为空间基因表达预测提供了强大的工具，有望推动生物学和临床研究的进展。
...
### [235] [SPQR: A Standardized Benchmark for Modern Safety Alignment Methods in Text-to-Image Diffusion Models](https://arxiv.org/abs/2511.19558)
*Mohammed Talha Alam,Nada Saadi,Fahad Shamshad,Nils Lukas,Karthik Nandakumar,Fahkri Karray,Samuele Poppi*

Main category: cs.CR

TL;DR: 该论文研究了文本到图像扩散模型在良性微调后安全对齐失效的问题，提出了SPQR基准来评估安全对齐技术在微调后的安全性、效用和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前的安全对齐方法在部署后的良性微调（如LoRA个性化、风格/领域适配器）中经常失效，而真正的安全对齐必须能够承受部署后的各种适应性调整。

Method: 引入SPQR基准（安全性-提示遵循-质量-鲁棒性），这是一个单一评分指标，通过报告单一排行榜分数来标准化和可重复地评估安全对齐扩散模型在良性微调下的表现。

Result: 通过多语言、领域特定和分布外分析以及类别细分，识别了良性微调后安全对齐失败的情况。

Conclusion: SPQR作为一个简洁而全面的基准，为T2I模型的安全对齐技术提供了有效的评估框架。
...
### [236] [Frequency Bias Matters: Diving into Robust and Generalized Deep Image Forgery Detection](https://arxiv.org/abs/2511.19886)
*Chi Liu,Tianqing Zhu,Wanlei Zhou,Wei Zhao*

Main category: cs.CR

TL;DR: 本文从频率角度分析深度伪造检测器的泛化性和鲁棒性问题，提出频率对齐方法，既能作为反取证攻击手段，也能作为取证防御手段。


<details>
  <summary>Details</summary>
Motivation: AI生成的深度伪造图像对数字安全构成挑战，现有检测器在未知GAN模型和噪声样本下的泛化性和鲁棒性不足，且这两个问题的根本原因尚未明确。

Method: 提出两阶段频率对齐方法，消除真实图像和伪造图像之间的频率差异，该方法可双向应用：作为反取证的黑盒攻击或作为取证的通用防御手段。

Result: 在涉及12个检测器、8个伪造模型和5个指标的多种实验设置中验证了方法的有效性，展示了频率对齐方法的效果。

Conclusion: 频率偏差是深度伪造检测器泛化和鲁棒性问题的可能原因，频率对齐方法为解决这些问题提供了有效的双向解决方案。
...
### [237] [Optimization of Sums of Bivariate Functions: An Introduction to Relaxation-Based Methods for the Case of Finite Domains](https://arxiv.org/abs/2511.20607)
*Nils Müller*

Main category: math.OC

TL;DR: 本文研究了在有限域上优化双变量和函数的复杂性，证明了该问题是NP等价的，并提出了基于松弛、ℓ²-近似和熵正则化的可处理求解方法。


<details>
  <summary>Details</summary>
Motivation: 研究具有n>2个变量的函数优化问题，这些函数可以表示为多个仅包含两个变量的函数之和，即双变量和函数。这类问题在多种应用中普遍存在，但优化复杂度未知。

Method: 使用测度值扩展（松弛）、ℓ²-近似和熵正则化方法，将问题转化为可通过线性规划、坐标上升和闭式解求解的可处理形式。

Result: 证明了双变量和函数优化是NP等价的，存在"免费午餐"现象。通过实验验证了算法在随机函数、顶点着色和信号重建问题中的有效性。

Conclusion: 双变量和函数优化虽然复杂，但通过适当的松弛和正则化方法可以获得可处理的求解方案，为建模不同函数类别提供了有效工具。
...
### [238] [Splatblox: Traversability-Aware Gaussian Splatting for Outdoor Robot Navigation](https://arxiv.org/abs/2511.18525)
*Samarth Chopra,Jing Liang,Gershom Seneviratne,Yonghan Lee,Jaehoon Choi,Jianyu An,Stephen Cheng,Dinesh Manocha*

Main category: cs.RO

TL;DR: Splatblox是一个用于户外密集植被环境的实时自主导航系统，通过融合分割RGB图像和LiDAR点云构建可通行性感知的ESDF地图，在植被丰富场景中比现有方法成功率提高50%以上。
...
Method: 使用高斯泼溅融合分割RGB图像和LiDAR点云，构建可通行性感知的欧几里得符号距离场(ESDF)，在线更新并支持语义推理。

Result: 在植被丰富场景中，成功率提高50%以上，冻结事件减少40%，路径缩短5%，目标到达时间加快13%，支持长达100米的远程任务。

Conclusion: Splatblox系统在复杂户外环境中表现出优越的导航性能，成功区分可穿越植被和障碍物，并验证了在四足和轮式平台上的可转移性。
...
### [239] [ArtiBench and ArtiBrain: Benchmarking Generalizable Vision-Language Articulated Object Manipulation](https://arxiv.org/abs/2511.20330)
*Yuhan Wu,Tiantian Wei,Shuo Wang,ZhiChao Wang,Yanyong Zhang,Daniel Cremers,Yan Xia*

Main category: cs.RO

TL;DR: 提出了ArtiBench基准测试和ArtiBrain框架，用于解决关节物体操作中的泛化挑战，通过结合高层推理和自适应底层控制，显著提升了多模态方法的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言和基于扩散的策略在跨部件、实例和类别的关节操作任务中泛化能力不足，需要解决长时程、多步骤交互中的物理一致性问题。

Method: 提出ArtiBrain模块化框架，包含基于VLM的任务推理器进行子目标分解和验证，以及混合控制器结合几何感知关键帧执行和可供性引导扩散，通过可供性记忆库积累成功经验。
...
### [240] [BlockCert: Certified Blockwise Extraction of Transformer Mechanisms](https://arxiv.org/abs/2511.17645)
*Sandro Andric*

Main category: cs.LG

TL;DR: BlockCert是一个用于认证式块级提取transformer机制并支持认证局部编辑的框架，提供机器可检查的证书来约束近似误差。


<details>
  <summary>Details</summary>
Motivation: 机制可解释性和模型编辑领域缺乏形式化保证，需要明确边界来确保提取或编辑模型在相关输入上与原始模型的偏差范围。

Method: 使用基于Lipschitz的组合定理，提取transformer残差块的结构化替代实现，并提供机器可检查的证书来记录覆盖指标和哈希底层工件。

Result: 在GPT-2 small、TinyLlama-1.1B-Chat和Llama-3.2-3B上获得高块级覆盖率和小的残差误差，TinyLlama设置中完全拼接模型在压力提示下与基线困惑度匹配在约6e-5范围内。
...
TL;DR: 提出了基于部分信息分解的框架，量化多模态模型中各模态的贡献，区分固有信息价值和交互价值


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖准确率指标，无法区分模态本身的信息价值与通过交互产生的价值，特别是在跨注意力架构中

Method: 使用部分信息分解将内部嵌入的预测信息分解为独特、冗余和协同成分，基于迭代比例拟合程序开发无需重新训练的算法

Result: 提供了原则性的表示层面分析，比基于结果的指标更清晰和可解释

Conclusion: 该框架能够量化多模态贡献，提供比传统方法更深入的见解
...
### [242] [EfficientXpert: Efficient Domain Adaptation for Large Language Models via Propagation-Aware Pruning](https://arxiv.org/abs/2511.19935)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在资源受限环境中部署困难，现有压缩方法要么跨领域泛化能力差，要么开销过高，需要开发高效的领域专用模型压缩方法。

Method: 结合传播感知剪枝准则（Foresight Mask）和高效适配器更新算法（Partial Brain Surgeon），集成到LoRA微调过程中，实现一步式领域剪枝。

Result: 在健康和法务任务中，在40%稀疏度下保持密集模型98%的性能，优于现有最先进方法。

Conclusion: 领域依赖的结构变化显著影响通用剪枝掩码的有效性，需要为每个领域定制自适应、领域感知的剪枝策略。
...
### [243] [The Devil in the Details: Emergent Misalignment, Format and Coherence in Open-Weights LLMs](https://arxiv.org/abs/2511.20104)
*Craig Dickson*
...
### [244] [Beyond Components: Singular Vector-Based Interpretability of Transformer Circuits](https://arxiv.org/abs/2511.20273)
*Areeb Ahmad,Abhinav Joshi,Ashutosh Modi*

Main category: cs.LG

TL;DR: 本文提出了一种细粒度视角，将transformer的注意力头和MLP层分解为正交奇异方向，揭示了单个组件内叠加的独立计算。
...
### [245] [Geometry of Decision Making in Language Models](https://arxiv.org/abs/2511.20315)
*Abhinav Joshi,Divyanshu Bhatt,Ashutosh Modi*

Main category: cs.LG

TL;DR: 该论文通过内在维度分析LLM在多项选择题任务中的决策几何结构，发现模型在早期层使用低维流形，中间层扩展维度，后期层压缩维度并收敛到决策相关表示。


<details>
  <summary>Details</summary>
Motivation: 研究LLM内部决策过程的几何结构，理解其预测背后的隐藏表示空间如何支持泛化和推理能力。

Method: 使用28个开源transformer模型，通过多个内在维度估计器分析各层表示，并量化每层在MCQA任务上的性能。
...
### [246] [Soft Adaptive Policy Optimization](https://arxiv.org/abs/2511.20347)
*Chang Gao,Chujie Zheng,Xiong-Hui Chen,Kai Dang,Shixuan Liu,Bowen Yu,An Yang,Shuai Bai,Jingren Zhou,Junyang Lin*

Main category: cs.LG

TL;DR: 提出了Soft Adaptive Policy Optimization (SAPO)方法，用平滑的温度控制门替代硬截断，自适应地衰减离策略更新同时保留有用的学习信号，解决了强化学习中策略优化的稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于分组的策略优化方法（如GSPO和GRPO）通过硬截断缓解重要性比率高方差问题，但难以同时保持稳定性和有效学习。特别是在混合专家模型中，token级重要性比率的高方差现象更加严重。

Method: SAPO使用平滑的温度控制门替代硬截断，形成连续的信任区域。当序列包含少数高度离策略token时，SAPO选择性地仅对问题token进行降权，同时保留接近策略token的学习信号。

Result: 在数学推理基准测试中，SAPO在可比较的训练预算下表现出改进的训练稳定性和更高的Pass@1性能。使用SAPO训练Qwen3-VL模型系列，在不同任务和模型大小上都获得了持续的性能提升。
...
### [247] [Beyond Binary Classification: A Semi-supervised Approach to Generalized AI-generated Image Detection](https://arxiv.org/abs/2511.19499)
*Hong-Hanh Nguyen-Le,Van-Tuan Tran,Dinh-Thuc Nguyen,Nhien-An Le-Khac*

Main category: cs.LG

TL;DR: 本文提出TriDetect检测器，通过半监督方法发现"假"类图像中的潜在架构模式，解决GAN和扩散模型检测器跨架构泛化问题
...
Method: 提出TriDetect半监督方法，使用Sinkhorn-Knopp算法进行平衡聚类分配和跨视图一致性机制，学习基础架构差异

Result: 在两个标准基准和三个真实数据集上评估，与13个基线方法相比，证明了向未见生成器的泛化能力
...
### [248] [Shortcut Invariance: Targeted Jacobian Regularization in Disentangled Latent Space](https://arxiv.org/abs/2511.19525)
*Shivam Pal,Sakshi Varshney,Piyush Rai*

Main category: cs.LG

TL;DR: 提出一种通过注入各向异性潜在噪声来使分类器对捷径特征不敏感的训练方法，在捷径学习基准上实现了最先进的OOD性能


<details>
  <summary>Details</summary>
Motivation: 深度神经网络容易学习训练数据中的捷径相关性，导致OOD泛化失败。现有方法学习鲁棒表示通常复杂且难以扩展

Method: 在解耦的潜在空间中识别与标签强相关的捷径特征，通过在训练中注入有针对性的各向异性潜在噪声，使分类器对捷径特征不敏感

Result: 在捷径学习基准上实现了最先进的OOD性能

Conclusion: 通过使分类器对捷径特征功能不变，而非学习鲁棒表示，可以更简单有效地实现OOD鲁棒性
...
### [249] [Merging without Forgetting: Continual Fusion of Task-Specific Models via Optimal Transport](https://arxiv.org/abs/2511.19561)
...
TL;DR: OTMF是一种基于最优传输理论的模型融合框架，通过发现任务向量的共同掩码来解决参数插值带来的分布偏移问题，支持持续融合范式。
...
Method: OTMF通过最优传输计划发现任务向量的共同掩码，选择性地提取可转移和任务无关的组件，同时保持每个任务的独特结构身份。支持持续融合范式，增量集成新任务向量。
...
### [250] [Learning Massively Multitask World Models for Continuous Control](https://arxiv.org/abs/2511.19584)
*Nicklas Hansen,Hao Su,Xiaolong Wang*

Main category: cs.LG

TL;DR: 提出了Newt——一个基于语言条件的世界模型，通过在200个多样化任务上进行大规模在线强化学习训练，实现了通用控制能力。


<details>
  <summary>Details</summary>
Motivation: 解决连续控制领域强化学习主要局限于单任务或离线学习的问题，探索单一智能体能否通过在线交互在数百个任务上训练。

Method: 首先在演示数据上进行预训练获取任务感知表示和动作先验，然后通过在线交互在所有任务上联合优化语言条件多任务世界模型。

Result: Newt在多任务性能和数据效率上优于强基线方法，展现出强大的开环控制能力，并能快速适应未见任务。
...
### [251] [Terminal Velocity Matching](https://arxiv.org/abs/2511.19797)
*Linqi Zhou,Mathias Parger,Ayaan Haque,Jiaming Song*

Main category: cs.LG

TL;DR: TVM是一种流匹配的泛化方法，支持高保真的一步和少步生成建模，通过终端时间正则化和最小架构修改实现稳定训练，在ImageNet上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决流匹配方法在终端时间行为不稳定问题，提升一步和少步生成模型的性能，同时克服扩散变换器缺乏Lipschitz连续性的限制。

Method: 提出终端速度匹配方法，建模任意两个扩散时间步之间的转换；引入最小架构修改实现稳定单阶段训练；开发支持Jacobian-向量乘积反向传播的融合注意力核。

Result: 在ImageNet-256x256上：1步生成FID 3.29，4步生成FID 1.99；在ImageNet-512x512上：1步生成FID 4.32，4步生成FID 2.94，达到少步生成模型的SOTA性能。

Conclusion: TVM通过终端时间正则化和高效注意力实现，为一步和少步生成建模提供了有效解决方案，在多个数据集上实现了最先进的性能。
...
### [252] [On-Demand Multi-Task Sparsity for Efficient Large-Model Deployment on Edge Devices](https://arxiv.org/abs/2511.19986)
*Lianming Huang,Haibo Hu,Qiao Li,Nan Guan,Chun Jason Xue*

Main category: cs.LG

TL;DR: 提出了一种按需多任务稀疏框架，通过最大化参数重用来最小化任务切换开销，在边缘设备上实现高效的多任务部署。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏优化方法针对单个任务独立优化，忽略了任务频繁切换时显著的I/O开销问题。

Method: 将权重分解为可重用的块粒度单元，跨任务对齐稀疏结构以最大化重叠，动态加载下一个任务所需的少量差异块。

Result: 在真实自动驾驶平台上，相比现有稀疏方法平均加速任务切换超过6.6倍。

Conclusion: 该框架通过参数重用显著降低了任务切换延迟，为资源受限边缘平台上的多任务部署提供了高效解决方案。
...
### [253] [Zero-Shot Transfer Capabilities of the Sundial Foundation Model for Leaf Area Index Forecasting](https://arxiv.org/abs/2511.20004)
*Peining Zhang,Hongchen Qin,Haochen Zhang,Ziqi Guo,Guiling Wang,Jinbo Bi*

Main category: cs.LG

TL;DR: 时间序列基础模型在零样本设置下，当输入上下文窗口足够长时（覆盖超过1-2个完整季节周期），可以超越完全训练的LSTM模型进行叶面积指数预测。
...
Conclusion: 预训练的时间序列基础模型在农业和环境应用中具有作为即插即用预测器的强大潜力，通用基础模型首次在无需任务特定调优的情况下超越了专门的监督模型。
...
### [254] [Latent Diffusion Inversion Requires Understanding the Latent Space](https://arxiv.org/abs/2511.20592)
*Mingxing Rao,Bowen Qu,Daniel Moyer*

Main category: cs.LG

TL;DR: 本文发现潜在扩散模型(LDMs)在潜在空间中的记忆化现象不均匀，主要集中在解码器回拉度量的高失真区域。通过提出一种基于维度贡献的排序方法，识别出对记忆化最关键的潜在维度，显著提升了成员推理攻击的性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型反转技术主要关注数据域的扩散模型，而忽略了潜在空间生成模型中的编码器/解码器对和潜在代码在记忆化中的作用。

Method: 提出一种原则性方法，通过计算解码器回拉度量的每维度贡献来对潜在维度进行排序，识别出对记忆化最关键的维度。在计算基于分数的成员推理攻击统计量时，移除较少记忆化的维度。

Result: 在CIFAR-10、CelebA、ImageNet-1K、Pokémon、MS-COCO和Flickr等多样化数据集上，平均AUROC提升2.7%，TPR@1%FPR显著增加6.42%，表明在极低误报率下识别成员的能力更强。
...
### [255] [Not Quite Anything: Overcoming SAMs Limitations for 3D Medical Imaging](https://arxiv.org/abs/2511.19471)
*Keith Moore*

Main category: eess.IV

TL;DR: 提出了一种组合式方法，将SAM-2等基础分割模型的输出作为额外输入通道与MRI图像一起处理，用于解决脑部MRI分割问题，避免了微调基础模型。


<details>
  <summary>Details</summary>
Motivation: 基础分割模型在自然图像上表现良好，但在脑部MRI中表现不佳，因为脑部结构（如尾状核和丘脑）缺乏清晰边界且对比度低。

Method: 使用轻量级3D U-Net生成SAM-2提示，将基础模型输出作为额外输入通道与MRI图像组合，并平滑边缘以改善对齐。还测试了使用DINO注意力图的无提示分割。
...
### [256] [A Multi-Stage Deep Learning Framework with PKCP-MixUp Augmentation for Pediatric Liver Tumor Diagnosis Using Multi-Phase Contrast-Enhanced CT](https://arxiv.org/abs/2511.19478)
*Wanqi Wang,Chun Yang,Jianbo Shao,Yaokai Zhang,Xuehua Peng,Jin Sun,Chao Xiong,Long Lu,Lianting Hu*

Main category: eess.IV

TL;DR: 开发了一个多阶段深度学习框架，使用多期相增强CT实现儿童肝肿瘤的自动诊断，解决了数据稀缺和类别不平衡问题，在良恶性分类和亚型分类中均表现出色。


<details>
  <summary>Details</summary>
Motivation: 儿童肝肿瘤是儿科常见实体肿瘤，传统病理活检具有侵入性风险，且儿童配合度差需要麻醉。现有AI研究大多忽视儿童肝肿瘤领域，需要建立无创诊断方法。

Method: 采用多阶段深度学习框架：1) 使用PKCP-MixUp数据增强解决数据问题；2) 训练肿瘤检测模型提取ROI；3) 建立两阶段诊断流程，使用ROI掩膜图像进行良恶性分类和亚型分类。

Result: 肿瘤检测模型mAP=0.871，良恶性分类AUC=0.989，良性亚型分类AUC=0.915，恶性亚型分类AUC=0.979。进行了消融研究和可解释性分析。

Conclusion: 该框架填补了儿童特异性DL诊断空白，为CT期相选择和模型设计提供实用见解，推动了精确、可及的儿童肝肿瘤诊断发展。
...
### [257] [The Selective Disk Bispectrum and Its Inversion, with Application to Multi-Reference Alignment](https://arxiv.org/abs/2511.19706)
*Adele Myers,Nina Miolane*

Main category: eess.IV

TL;DR: 提出选择性圆盘双谱：一种快速、旋转不变的图像形状分析表示方法，解决了传统圆盘双谱计算复杂度高和不可逆的问题。


<details>
  <summary>Details</summary>
Motivation: 在计算机视觉和形状分析任务中，需要学习物体的形状而忽略其方向，因此需要一种旋转不变的图像表示方法，该方法需要计算高效且可逆以便可视化。

Method: 推导出圆盘双谱的显式逆变换，定义"选择性"圆盘双谱，仅使用最小数量的系数来实现准确的形状恢复。

Result: 该表示方法能够实现旋转图像的多参考对齐——这是传统圆盘双谱方法以前无法处理的任务。

Conclusion: 圆盘双谱成为学习旋转不变形状数据的实用且理论基础扎实的工具。
...
### [258] [DLADiff: A Dual-Layer Defense Framework against Fine-Tuning and Zero-Shot Customization of Diffusion Models](https://arxiv.org/abs/2511.19910)
...
TL;DR: 本文提出DLADiff双层次防御机制，同时保护人脸隐私免受扩散模型微调和零样本生成方法的攻击。


<details>
  <summary>Details</summary>
Motivation: 随着扩散模型技术的发展，恶意攻击者仅需少量甚至单张人脸图像就能创建高度相似的合成身份，现有防御方法主要针对微调方法而忽视了零样本生成防御。

Method: DLADiff包含双层次保护机制：第一层使用双代理模型(DSUR)和交替动态微调(ADFT)防御微调攻击；第二层采用简单但有效的方法防御零样本生成。

Result: 实验结果表明，该方法在防御扩散模型微调方面显著优于现有方法，在防御零样本生成方面实现了前所未有的性能。

Conclusion: DLADiff为扩散模型定制化攻击提供了全面的防御解决方案，有效保护人脸隐私安全。
...
### [259] [Development of a fully deep learning model to improve the reproducibility of sector classification systems for predicting unerupted maxillary canine likelihood of impaction](https://arxiv.org/abs/2511.20493)
*Marzio Galdi,Davide Cannatà,Flavia Celentano,Luigia Rizzo,Domenico Rossi,Tecla Bocchino,Stefano Martina*

Main category: eess.IV

TL;DR: 本研究开发了深度学习模型来减少上颌未萌尖牙撞击预测中扇形分类系统的操作者间和操作者内变异性，发现3扇形系统重现性最高，DenseNet121模型表现最佳。


<details>
  <summary>Details</summary>
Motivation: 减少上颌未萌尖牙撞击预测中扇形分类系统的操作者间和操作者内变异性，提高诊断一致性。

Method: 3名正畸医生和3名全科牙医使用3种扇形分类系统(5、4、3扇形)对306张X光片进行分类，评估重现性；同时使用在1,222张X光片上预训练的AI模型进行测试。

Result: 3扇形系统重现性最高(Cohen's K: 0.80-0.92)；DenseNet121模型表现最佳，总体准确率76.8%；教育背景不影响重现性。

Conclusion: AI模型可以自动分类上颌未萌尖牙位置，减少操作者间差异。
...
### [260] [It Hears, It Sees too: Multi-Modal LLM for Depression Detection By Integrating Visual Understanding into Audio Language Models](https://arxiv.org/abs/2511.19877)
...
TL;DR: 提出了一种新颖的多模态大语言模型框架用于抑郁症检测，通过音频语言模型增强视觉理解能力，并在时间戳级别对齐视听特征，在DAIC-WoZ数据集上表现优于单模态和先前多模态方法。


<details>
  <summary>Details</summary>
Motivation: 抑郁症是全球最普遍的心理健康障碍之一。传统大语言模型主要处理文本，无法处理音频和视觉模态中的丰富非语言线索，而这些线索在心理健康评估中至关重要。虽然多模态LLM提供了有前景的方向，但很少有专门针对心理学应用的研究。

Method: 提出多模态LLM框架，通过音频语言模型增强视觉理解能力，在时间戳级别对齐视听特征。这种细粒度对齐改进了跨模态的时间动态建模，同时减少了对大量训练数据和计算资源的需求。
...
### [261] [QiMeng-Kernel: Macro-Thinking Micro-Coding Paradigm for LLM-Based High-Performance GPU Kernel Generation](https://arxiv.org/abs/2511.20100)
...
TL;DR: MTMC是一个分层框架，通过将优化策略与实现细节解耦来解决GPU内核生成问题，在保持正确性的同时实现高效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的方法直接生成整个优化后的低级程序，需要在优化策略和实现代码的极大空间中进行探索，导致正确性和效率的冲突。

Method: Macro Thinking使用强化学习指导轻量级LLM学习语义优化策略，Micro Coding利用通用LLM逐步实现优化提案，避免全内核生成错误。

Result: 在KernelBench上达到近100%和70%的准确率，比现有最佳方法提升50%以上，速度提升最高达7.3倍；在TritonBench上达到59.64%准确率和34倍加速。

Conclusion: MTMC通过分层方法有效导航优化空间和实现细节，使LLM能够生成高性能GPU内核。
