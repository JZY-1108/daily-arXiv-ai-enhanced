...
TL;DR: ARC是一种新颖的、无需标注的缓存框架，为每个LLM代理动态管理小型高价值语料库，将存储需求降至原始语料库的0.015%，同时提供79.8%的答案命中率和80%的平均检索延迟降低。


<details>
  <summary>Details</summary>
Motivation: 尽管RAG在提升代理性能方面取得成功，但代理级别的缓存管理，特别是为每个代理动态构建、维护和更新紧凑相关语料库的问题仍未得到充分探索。
...
TL;DR: 提出了一种利用多语言机器翻译模型内部表示来区分人工翻译和机器翻译句子的新方法，在非英语语言对上比现有技术准确率提升至少5个百分点。


<details>
  <summary>Details</summary>
Motivation: 当前机器翻译系统依赖的大规模平行语料中有大量机器生成内容，过度依赖这些合成内容会显著降低翻译质量，因此需要有效过滤非人工翻译文本。
...
TL;DR: LEGO-Eval是一个评估框架，用于更准确地评估3D场景与细粒度指令的对齐程度，同时提出了LEGO-Bench基准测试集。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成的3D场景缺乏真实世界的空间布局和物体属性，且当前评估方法（如CLIPScore和VLMs）无法可靠评估场景与指令的对齐程度。

Method: 提出了LEGO-Eval评估框架，配备多样化工具来显式地定位场景组件；同时构建了LEGO-Bench基准测试集，包含详细指令指定复杂布局和属性。

Result: LEGO-Eval在评估场景-指令对齐方面比VLM-as-a-judge高出0.41 F1分数；当前生成方法在生成完全对齐细粒度指令的场景方面成功率最多只有10%。

Conclusion: 当前3D场景生成方法在应对细粒度指令方面存在显著局限性，需要更可靠的评估框架来确保生成场景与现实世界的对齐。
...
TL;DR: 提出了Analyze-Revise-Finetune (ARF)管道，使小型开源语言模型在客服摘要任务中超越大型专有模型。通过分析教师模型错误、使用编辑器模型修订数据，然后微调学生模型，实现了成本效益和数据隐私的改进。


<details>
  <summary>Details</summary>
Motivation: 解决大型专有模型成本高、数据隐私问题，同时提升小型开源模型的性能，使其在特定任务中超越大型模型。

Method: 采用三阶段管道：1) 分析GPT-3.5生成的摘要错误；2) 使用Llama 3.1 70B进行针对性修订生成高质量训练数据；3) 在修订数据上微调Llama 3.1 8B学生模型。

Result: 微调后的8B参数学生模型在客服摘要任务中表现优于GPT-3.5，同时提高了成本效率和数据隐私保护。

Conclusion: ARF管道为增强开源语言模型提供了可推广的框架，在保持竞争力的同时改善了成本效益和数据隐私。
...
TL;DR: 本文提出了一种新的ABSA评估方法FTS-OBP，研究了小型生成语言模型在ABSA任务中的表现，并发布了首个教育评论ABSA资源集。


<details>
  <summary>Details</summary>
Motivation: ABSA研究集中在商业领域，而教育和医疗等高需求低资源领域缺乏支持。现有评估方法过于严格，无法准确反映生成模型的性能。

Method: 1) 提出FTS-OBP评估方法，允许边界变化；2) 研究小型解码器语言模型(<7B参数)，探索数据自由和数据轻量微调方法；3) 提出多任务微调策略

Result: 1.5-3.8B参数的小型模型在仅200-1000个样本和单GPU训练下，性能超过专有大模型并接近基准结果

Conclusion: 该方法为低资源领域的ABSA研究提供了有效解决方案，小型模型在适当策略下能够取得优异表现
...
TL;DR: ROBOTO2是一个基于网页的开源平台，利用大语言模型辅助进行临床试验偏倚风险评估，通过交互式界面结合PDF解析、检索增强提示和人工审核，简化了传统的ROB2标注流程。
...
Method: 开发了交互式平台，结合PDF解析、检索增强的LLM提示和人工在环审核，用户可以上传临床试验报告，获取初步答案和证据，并提供实时反馈。

Result: 构建并发布了包含521份儿科临床试验报告的数据集（8954个信号问题，1202条证据段落），使用该数据集对4个LLM进行了ROB2性能基准测试。

Conclusion: ROBOTO2平台公开可用，代码和数据已发布以促进可重复性和采用，同时分析了当前模型能力及在自动化这一关键系统评价环节中面临的挑战。
...
TL;DR: 本文提出单边对话问题(1SC)，研究在只能记录对话单边的情况下如何推断缺失对话内容，包括实时重建缺失说话者轮次和生成单边摘要。


<details>
  <summary>Details</summary>
Motivation: 现实场景如远程医疗、呼叫中心和智能眼镜中，往往只能记录对话的单边内容，这限制了对话AI的应用。

Method: 使用MultiWOZ、DailyDialog和Candor数据集，评估提示和微调模型，采用A/B测试和LLM作为评判指标，探索未来轮次信息和话语长度对重建的影响。

Result: 发现访问未来一个轮次和话语长度信息能改善重建效果，占位符提示有助于减少幻觉，大模型通过提示能生成有希望的重建结果，小模型需要微调。高质量摘要可在不重建缺失轮次的情况下生成。

Conclusion: 1SC是一个新颖的挑战，研究结果标志着向隐私感知对话AI迈出了重要一步。
...
TL;DR: PolyNorm是一种基于提示的文本归一化方法，使用大型语言模型减少对人工规则的依赖，在八种语言上相比生产级系统持续降低了词错误率。
...
Method: 提出基于提示的PolyNorm方法，使用LLMs进行文本归一化，并设计了语言无关的自动数据整理和评估流程。

Result: 在八种语言的实验中，相比生产级系统持续降低了词错误率，并发布了多语言数据集PolyNorm-Benchmark。

Conclusion: PolyNorm通过LLMs减少了文本归一化对人工规则的依赖，实现了更好的语言覆盖和可扩展性。
...
TL;DR: 该研究使用计算语言学方法分析精神分裂症患者的语言障碍，通过计算惊奇度和语义连贯性来区分患者与健康对照组，并探讨这些指标与症状严重程度的关系。


<details>
  <summary>Details</summary>
Motivation: 精神分裂症患者的语言障碍表现为言语紊乱和语篇连贯性受损，这些异常反映了潜在的认知障碍，有潜力作为症状严重程度和诊断的客观标记物。

Method: 使用计算语言学模型计算语言中的惊奇度和语义连贯性，比较精神分裂症患者与健康对照组在这些指标上的差异。

Result: 研究发现精神分裂症患者与健康对照组在惊奇度和语义连贯性上存在显著差异，这些语言指标的变化与症状严重程度相关。

Conclusion: 计算语言学指标如惊奇度和语义连贯性可以有效表征精神分裂症的语言障碍，为症状评估和诊断提供客观依据。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语人群的心理健康检测资源有限且存在文化污名，而现有研究主要集中在英语，阿拉伯语领域研究不足。
...
Conclusion: 该研究为阿拉伯语心理健康检测提供了重要数据集和方法框架，有助于解决该领域的研究空白。
...
TL;DR: 提出基于控制障碍函数的LLM对齐框架，通过安全过滤器干预文本生成，无需微调基础模型


<details>
  <summary>Details</summary>
Motivation: 确保大语言模型生成用户期望的文本，解决模型对齐问题

Method: 使用控制障碍函数作为安全过滤器，对基础LLM生成的token进行干预

Result: 实现了基于开源语言模型的文本生成系统，能够生成积极文本

Conclusion: 该框架为LLM对齐提供了一种无需微调的附加式解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有基准要么过度强调文本推理，要么无法系统捕捉视觉中心认知行为，导致MLLMs的认知能力评估不足。

Method: 构建MME-CC基准，将11个推理任务组织为空间、几何和知识推理三个基本类别，对16个代表性MLLMs进行广泛实验。

Result: 闭源模型整体领先（如Gemini-2.5-Pro得42.66 vs GLM-4.5V得30.45），空间和几何推理普遍较弱（≤30%），识别出常见错误模式如方向错误、跨视图身份持续性脆弱等。
...
TL;DR: 使用LLMs作为裁判进行利益相关者风险感知评估，通过Risk Atlas Nexus和GloVE方法生成可解释的风险政策，揭示不同利益相关者对相同风险的分歧。


<details>
  <summary>Details</summary>
Motivation: 理解不同利益相关者在AI系统中的风险感知对于负责任部署至关重要，需要透明、可解释的风险评估方法。

Method: 使用LLMs作为裁判预测和解释风险，结合Risk Atlas Nexus和GloVE解释方法，生成利益相关者特定的可解释政策。

Result: 利益相关者视角显著影响风险感知和冲突模式，在医疗AI、自动驾驶和欺诈检测三个实际用例中验证了方法的有效性。

Conclusion: 强调利益相关者感知解释的重要性，使基于LLM的评估更加透明、可解释，符合以人为中心的AI治理目标。
...
TL;DR: 该论文对12种不确定性估计方法在LLM问答任务中的表现进行了实证研究，分析了在分布内和分布外数据集上的效果，发现信息基方法在ID设置中表现优异，而密度基方法和P(True)在OOD场景中更有效。
...
Result: 信息基方法在ID设置中表现优异，密度基方法和P(True)在OOD场景中更有效，语义一致性方法在不同数据集和生成指标上都表现可靠。

Conclusion: 不同不确定性估计方法在不同场景下各有优势，信息基方法适合ID设置，密度基方法适合OOD场景，语义一致性方法具有普遍适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有伦理基准主要基于英语和西方框架，忽视了孟加拉语（全球第6大语言）的文化细微差别，需要专门的文化对齐评估工具。

Method: 构建包含日常活动、习惯、育儿、家庭关系和宗教活动5个道德领域的基准，每个场景通过母语者共识使用美德伦理、常识伦理和正义伦理三种视角进行标注。

Result: 对主流多语言LLMs的零样本评估显示准确率在50-91%之间，存在文化基础、常识推理和道德公平性方面的系统弱点。
...
TL;DR: 提出语言图模型（LGM）来解决LLM在模糊或概念不对齐术语上的理解问题，通过提取元关系并验证，结合概念迭代检索算法提升概念解释和响应准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语义理解方面表现强劲，但当用户指令涉及模糊或概念不对齐的术语时仍存在困难。

Method: 提取自然语言中的元关系（继承、别名、组合），采用反思机制验证这些关系，利用概念迭代检索算法动态向LLM提供关系和描述。

Result: 在标准基准测试中，LGM始终优于现有的RAG基线方法。

Conclusion: LGM能够在不截断的情况下处理任意长度的文本，相比传统RAG方法具有优势。
...
TL;DR: 提出了一个混合事实核查系统，结合了知识图谱、大语言模型和实时搜索代理的优势，在FEVER基准测试中达到0.93的F1分数，无需特定任务微调。


<details>
  <summary>Details</summary>
Motivation: 大语言模型生成流畅但缺乏可靠的事实依据，而基于知识图谱的事实核查器虽然精确可解释，但覆盖范围有限或延迟高。需要结合两者的优势。

Method: 三步骤混合方法：1) 知识图谱检索用于DBpedia中的快速单跳查找；2) 基于大语言模型的分类，使用任务特定的标签提示；3) 当知识图谱覆盖不足时调用网络搜索代理。

Result: 在FEVER基准测试的Supported/Refuted分集上达到0.93的F1分数，无需特定任务微调。重新标注研究表明，该方法经常能为原本标记为"信息不足"的声明找到有效证据。

Conclusion: 提出了一个模块化、开源的事实核查流水线，具有回退策略和跨数据集的泛化能力，有效结合了知识图谱的精确性和大语言模型的覆盖范围。
...
Result: 在MATERIAL第三阶段评估中，SARAL在涉及波斯语、哈萨克语和格鲁吉亚语的六个评估条件中的五个条件下表现优于其他团队。

Conclusion: SARAL的方法在跨语言信息检索方面表现出色，特别是在处理多种语言时能够有效检索相关文档集。
...
<details>
  <summary>Details</summary>
Motivation: 设计有效的多语言tokenizer面临挑战，因为存在多样化的文字和丰富的形态变化。虽然BPE等子词方法被广泛采用，但在多语言环境中的有效性仍有待探索。

Method: 结合子词和多词分词方法，采用语言特定的预分词策略，实现更符合语言学的token对齐。

Result: 在英语、22种印度语言和代码数据上，平均fertility分数比LLaMA4提升39.5%，比Sutra提升18%，推理吞吐量比LLaMA4提升44%，同时在英语和印度语言基准测试中保持可比性能。

Conclusion: 通过详细的消融实验验证了设计选择的鲁棒性，IndicSuperTokenizer为多语言LLM提供了有效的分词解决方案。
...
TL;DR: 本研究比较了四种开源LLM（Mistral-7b-instruct、LLaMa2-7b-chat、Falcon-7b-instruct、Orca-mini-v3-7b）和GPT-3.5在计算机科学文献问答任务中的表现，发现结合RAG技术后，GPT-3.5表现最佳，而Mistral-7b-instruct在开源模型中表现最好。
...
Method: 使用RAG技术支持，在计算机科学文献问答任务中评估四种开源LLM和GPT-3.5的性能，采用准确率、精确率、专家排名、Gemini排名和余弦相似度等评估指标。

Result: GPT-3.5结合RAG在二元和长答案问题上表现最佳；在开源LLMs中，Mistral-7b-instruct表现最好；Orca-mini-v3-7b响应延迟最短，LLaMa2-7b-chat延迟最高。

Conclusion: 研究表明开源LLMs通过更好的基础设施可以与GPT-3.5等专有模型相媲美。
...
<details>
  <summary>Details</summary>
Motivation: 当前持续预训练的进展更多依赖于扩展正确的结构而非单纯增加参数数量。传统方法如深度扩展会导致严重遗忘问题。

Method: 提出SCALE架构，基于持久保持和协作适应两大原则，包括SCALE-Preserve、SCALE-Adapt和带路由扩展的SCALE-Route三种变体。

Result: 在合成传记基准测试中，SCALE缓解了深度扩展的严重遗忘问题；在韩语持续预训练中，SCALE变体在英语评估上遗忘较少，在韩语基准上获得有竞争力的提升。

Conclusion: SCALE变体提供了最佳的整体稳定性-可塑性权衡，其保持与适应的相互作用相比标准持续学习设置能稳定优化过程。
...
TL;DR: 本文研究了语音到文本翻译(ST)的自动评估方法，提出了两种生成音频输入文本代理的策略：自动语音识别(ASR)转录和参考翻译的反向翻译，并引入跨语言重分段算法来解决对齐问题。


<details>
  <summary>Details</summary>
Motivation: 传统的ST评估方法基于参考翻译比较，忽略了源音频的宝贵信息。虽然机器翻译中已证明包含源文本的神经指标与人类判断相关性更强，但ST中源是音频而非文本，且可靠的转录或对齐通常不可用。

Method: 探索两种互补策略生成音频输入的文本代理：ASR转录和参考翻译的反向翻译；提出新颖的两步跨语言重分段算法处理合成源与参考翻译之间的对齐不匹配问题。

Result: 在覆盖79种语言对和6个ST系统的两个基准测试中，当词错误率低于20%时，ASR转录比反向翻译更可靠；反向翻译始终是计算成本更低但仍有效的替代方案；跨语言重分段算法使源感知MT指标在ST评估中稳健使用。

Conclusion: 该研究为语音翻译开发了更准确和原则性的评估方法，通过合成源和跨语言重分段算法，实现了源感知指标在ST评估中的有效应用。
...
TL;DR: 该研究评估了多模态大语言模型在临床任务中的"思考模式"与"非思考模式"性能差异，发现思考模式带来的改进有限，特别是在复杂医疗任务中表现仍不理想。


<details>
  <summary>Details</summary>
Motivation: 随着具有"双状态"（思考模式和非思考模式）的多模态大语言模型的出现，需要系统评估这些模型的增强推理过程如何影响临床任务中的性能和可靠性。

Method: 评估了Seed1.5-VL和Gemini-2.5-Flash两个领先MLLM的思考模式能力，使用VQA-RAD和ROCOv2数据集在四个视觉医疗任务上进行测试。

Result: 激活思考模式相比标准非思考模式的改进在大多数任务中都很有限，在开放式VQA和医学图像解释等复杂医疗任务中表现仍然不理想。
...
TL;DR: 这篇综述系统评估了生成式AI在生物信息学中的进展，通过6个研究问题分析了GenAI在基因组学、蛋白质组学、结构生物学等领域的应用、性能优势、局限性和未来方向。


<details>
  <summary>Details</summary>
Motivation: 系统识别和评估生成式AI在生物信息学中日益增长的发展，评估有影响力的GenAI策略在方法学进步、预测性能和专业化方面的表现。

Method: 采用系统综述和元分析方法，提出6个研究问题(RQs)来评估GenAI在生物信息学中的应用、性能、专业化和数据资源。

Result: GenAI在序列分析、分子设计和数据建模中表现出优于传统方法的性能；专业化模型架构优于通用模型；在分子分析和数据集成方面显著提升准确性；在结构建模和功能预测方面有改进；但也存在可扩展性不足和数据偏见等限制。
...
TL;DR: 该论文提出了"沉默偏见"概念，指被安全对齐掩盖的模型潜在偏见，并开发了Silenced Bias Benchmark (SBB)基准，通过激活引导减少模型拒绝来揭示这些隐藏偏见。


<details>
  <summary>Details</summary>
Motivation: 现有公平性评估方法通常将模型的拒绝回答视为公平性表现，这掩盖了模型潜在的真实偏见问题，需要开发能揭示这些隐藏偏见的新评估框架。

Method: 提出SBB基准，采用激活引导技术减少模型在问答过程中的拒绝行为，从而暴露被安全对齐训练掩盖的潜在偏见，支持扩展到新的人口群体和主题。

Result: 在多个大语言模型上的实验表明，模型直接响应与潜在公平性问题之间存在显著差异，揭示了安全对齐可能掩盖的严重偏见问题。

Conclusion: SBB提供了一个超越对齐训练掩盖效应的公平性评估框架，有助于推动未来公平模型和工具的发展，暴露了现有评估方法的局限性。
...
TL;DR: EQ-Negotiator是一个新颖的框架，使用情感角色来弥补小型语言模型在自动谈判中的能力差距，通过结合博弈论和隐马尔可夫模型来学习和跟踪债务人情绪状态，使7B参数模型在债务回收和谈判效率上超越大10倍的基准模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动谈判中性能优异，但计算成本高且数据隐私要求严格，不适合隐私敏感的移动设备应用。小型语言模型是实用替代方案，但在处理情感复杂的角色扮演（特别是信贷谈判）时性能差距显著。

Method: EQ-Negotiator框架核心是一个推理系统，整合博弈论和隐马尔可夫模型来在线学习和跟踪债务人情绪状态，无需预训练。该系统使小型语言模型具备战略智能，能够对抗操纵、化解冲突并维护道德标准。

Result: 通过广泛的代理间模拟测试，在包括欺骗、威胁和扮演受害者等对抗性债务人策略的多样化信贷谈判场景中，配备EQ-Negotiator的7B参数语言模型在债务回收和谈判效率上优于比其大10倍以上的基准大型语言模型。

Conclusion: 这项研究将角色建模从描述性特征档案推进到在隐私约束下运行的动态情感架构。战略情商而非原始模型规模是自动谈判成功的关键因素，为在边缘设备上运行的有效、道德且隐私保护的AI谈判者铺平了道路。
...
TL;DR: LFC-DA是一个符号逻辑控制的数据增强方法，通过将逻辑文本映射为命题表达式，编译规则库，并通过有界状态空间搜索发现有效公式，最后再语言化为自然语言问题，确保在命题逻辑下的多样性和逻辑严谨性。
...
Method: 提出符号逻辑控制流程：逻辑文本→命题表达式→规则库编译→有界状态空间搜索发现有效公式→语言化为自然语言问题。
...
TL;DR: 研究发现对称BPE在机器翻译中并非最优，提出非对称BPE方法，在源语言使用高合并操作数，目标语言使用低合并操作数，显著提升低资源语言对的翻译性能。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译研究通常使用对称BPE，为源语言和目标语言应用相同的合并操作数，但这种方法在不同语言对和数据规模下可能不是最优的。

Method: 研究不同数据量和语言对下的BPE分词策略，比较对称BPE和非对称BPE在机器翻译系统中的性能表现。

Result: 非对称BPE在低资源设置下显著优于对称BPE，在英语-印地语翻译中平均提升5.32、4.46和0.7 CHRF++分数，在6个额外语言对中10/12系统显示统计显著改进。
...
TL;DR: 本文研究小语言模型(SLMs)在关系抽取(RE)中处理RDF图完整提取的能力，发现稀有属性的长尾分布是主要瓶颈，并通过多种策略评估找到最佳解决方案。


<details>
  <summary>Details</summary>
Motivation: 研究SLMs在提取包含数据类型属性和对象属性的完整RDF图时的表现，特别关注稀有属性带来的挑战。

Method: 评估了四种策略：分层抽样、加权损失、数据集扩展和基于模板的合成数据增强，以解决稀有属性的长尾分布问题。

Result: 发现最佳策略是构建训练集，确保每个属性的出现次数超过特定阈值，从而在非平衡目标属性上获得均衡性能。

Conclusion: 为训练形状感知的SLMs提供了实用指导，并指出了语义关系抽取未来工作的有前景方向。
...
TL;DR: 3TF框架通过思想训练和免思想推理，让模型在内部隐式执行复杂推理，同时保持外部输出的简洁性，无需显式生成推理步骤。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要压缩冗长的推理输出，但仍依赖显式推理。3TF采用短到长的视角，旨在让模型内部化结构化推理，在推理时无需显式生成推理步骤。
...
Conclusion: 3TF框架成功实现了模型在内部隐式执行丰富推理，同时保持外部输出简洁，为高效推理提供了新思路。
...
Result: 知识增强对理解错误问题至关重要，基于RL的对齐方法比传统监督微调更有效，能提升模型遵循指令和泛化的能力。
...
TL;DR: CareMedEval是一个用于评估LLM在生物医学批判性评价和推理任务表现的数据集，包含534个基于37篇科学文章的问题，源自法国医学生考试。


<details>
  <summary>Details</summary>
Motivation: 生物医学领域的科学文献批判性评价是重要技能，但LLM在此任务中的可靠性有限，特别是在专业领域的批判性推理方面。

Method: 创建CareMedEval数据集，基于真实医学生考试，评估各种通用和专业LLM在不同上下文条件下的表现。

Result: 即使生成中间推理token能显著改善结果，开放和商业模型在精确匹配率上仍无法超过0.5，尤其在研究局限性和统计分析问题上表现不佳。

Conclusion: CareMedEval为基于推理的评估提供了具有挑战性的基准，揭示了当前LLM的局限性，为未来开发自动化批判性评价支持系统铺平道路。
...
TL;DR: Kastor框架通过RDF模式提取改进小语言模型，将SHACL形状验证扩展为评估所有属性组合，并采用迭代学习优化知识库，提升模型泛化能力和事实发现能力。


<details>
  <summary>Details</summary>
Motivation: 满足专业领域知识库补全和优化的需求，解决传统单一SHACL形状验证的局限性。

Method: 将单一SHACL形状验证重新定义为评估所有可能属性组合，选择最优组合进行训练；采用迭代学习过程优化噪声知识库。

Result: 显著提升模型泛化能力和性能，能够发现新的相关事实。

Conclusion: Kastor框架通过改进的RDF模式提取方法，为专业领域知识库的完善提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有孟加拉语-英语翻译系统在处理技术术语时表现不佳，导致技术问题求解错误，需要专门的技术术语翻译解决方案。

Method: 构建包含5000个高质量孟加拉语-英语技术句子对的BanglaSTEM数据集，使用语言模型生成12000多个翻译，人工筛选最佳翻译对，并基于T5训练翻译模型。

Result: 在代码生成和数学问题求解任务中，翻译准确性显著提升，使孟加拉语用户能更有效地使用英语语言模型。

Conclusion: BanglaSTEM数据集和翻译模型成功解决了技术术语翻译问题，为孟加拉语技术内容翻译提供了有效工具。
...
TL;DR: 提出了一个可扩展的多轮指令遵循评估框架EvolIF，通过三层机制解耦语言表面形式和用户意图模拟，动态构建包含状态变化和回溯的对话基准，测试模型在用户耐心耗尽前的指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常局限于固定轮次，容易饱和且无法考虑用户的交互体验，需要评估大语言模型在多主题对话中遵循用户指令的能力。

Method: 采用三层机制跟踪约束、指令和主题，模拟用户-LLM交互，动态构建包含状态变化和回溯的对话基准，直到模型耗尽模拟用户的耐心才终止对话。

Result: GPT-5表现出卓越的指令遵循性能，平均维持18.54轮对话，鲁棒性达70.31%，显著优于Gemini-2.5-Pro 11.41%，其他模型表现落后。

Conclusion: 提出的框架能有效评估多轮指令遵循能力，GPT-5在此任务上表现最佳，所有数据和代码将公开。
...
TL;DR: SOLVE-Med是一个多智能体医疗问答系统，通过组合领域专业化的小语言模型来解决复杂医疗查询，在性能上超越大型模型的同时支持本地部署。


<details>
  <summary>Details</summary>
Motivation: 解决医疗问答系统面临的幻觉、偏见、计算需求、隐私问题和跨领域专业知识需求等部署挑战。

Method: 采用多智能体架构，包括路由智能体动态选择专家、十个专业医疗领域模型（每个1B参数）和协调智能体合成响应。

Result: 在意大利医疗论坛数据的十个专业领域评估中，ROUGE-1达0.301，BERTScore F1达0.697，优于参数达14B的单体模型。

Conclusion: SOLVE-Med展示了专业化小模型组合在医疗问答中的有效性，实现高性能同时支持本地部署，代码已开源。
...
TL;DR: 本文展示了堆栈增强神经网络能够像人类一样进行层次化泛化，无需语法监督、大规模预训练或长时间训练。


<details>
  <summary>Details</summary>
Motivation: 研究神经网络是否能够像人类儿童学习语言那样，在没有歧义消除示例的情况下偏好层次化语法规则假设。

Method: 测试了三种基础架构（transformer、简单RNN、LSTM）与两种堆栈风格（Joulin & Mikolov的叠加堆栈和DuSell & Chiang的非确定性泛化）的组合。

Result: 发现带有非确定性堆栈的transformer在经典问题形成任务中表现最佳，并提出了一种改进的堆栈RNN架构来增强层次化泛化能力。
...
TL;DR: 该论文创建了MultiZebraLogic数据集，包含多种语言、主题和难度的斑马谜题，用于评估大型语言模型的逻辑推理能力。研究发现4x5尺寸的谜题对推理模型具有足够挑战性，红色鲱鱼线索会显著降低模型性能，但语言和主题选择对结果影响不大。


<details>
  <summary>Details</summary>
Motivation: 为了全面评估大型语言模型的逻辑推理能力，需要创建包含多种任务、语言和难度级别的大规模高质量基准数据集。

Method: 生成了多种语言、主题、尺寸的斑马谜题，包含14种线索类型和8种红色鲱鱼类型（无信息线索）。发布了9种日耳曼语言的128+1024个谜题数据集，并提供了可适应更多语言和主题的谜题生成代码。

Result: 2x3尺寸谜题对GPT-4o mini（非推理模型）具有足够挑战性，4x5尺寸对o3-mini（推理模型）具有挑战性。包含5个红色鲱鱼会使o3-mini在4x5谜题上的准确率降低15±7%。英语与丹麦语、常见房屋主题与特定国家主题对模型表现无显著影响。

Conclusion: MultiZebraLogic数据集为评估LLMs的逻辑推理能力提供了有效的基准工具，特别是在多语言环境下。谜题生成方法具有很好的适应性，可扩展到更多语言和主题。
...
TL;DR: 本文首次实证展示了Transformer语言模型中的可控局部性，通过可调节的局部性参数实现表示局部化程度的连续控制，在可解释的局部编码和高效分布式表示之间动态插值。


<details>
  <summary>Details</summary>
Motivation: 传统语言模型仅依赖分布式表示，缺乏可解释性。本文旨在开发一种能够在保持性能的同时提供透明度的框架，特别适用于需要可解释性的监管领域。

Method: 使用两层Transformer架构在WikiText语料库上进行实验，系统性地改变局部性参数λ（从1.0完全局部到0.0完全分布式），通过注意力熵和指针保真度等指标评估模型。

Result: 局部配置显著降低注意力熵（λ=1.0时为5.36比特，λ=0.0时为7.18比特），同时保持更高的指针保真度。中间局部值λ=0.6在测试困惑度4.65和准确率84.7%时优化了可解释性与性能的权衡。

Conclusion: 局部语言模型为需要透明度和能力的监管领域提供了实用框架，通过显式惩罚阈值和信息论设计原则实现对可解释性-性能谱系的精确数学控制。
...
TL;DR: 本研究通过微调大语言模型并集成检索增强生成方法，开发了一个专门支持政策制定者理解和制定法律规章的工具。


<details>
  <summary>Details</summary>
Motivation: 为政策制定者提供更好的法律文本理解和分析支持，帮助他们更有效地解读法规和制定符合当前需求的新规章。

Method: 构建专门的法律领域监督数据集进行模型微调，并集成检索增强生成方法以访问最新法律知识。

Result: 该方法显著提升了法律研究和规章制定的效率，为政策制定者提供了有价值的辅助工具。

Conclusion: 结合微调和RAG增强的方法能够有效支持法律领域的工作，为政策制定者提供实用的法律分析辅助工具。
...
TL;DR: Step-Audio-EditX是首个开源基于LLM的音频模型，在情感、说话风格和副语言等表达性音频编辑方面表现出色，并具备强大的零样本文本转语音能力。


<details>
  <summary>Details</summary>
Motivation: 传统方法主要关注表示级解缠，但作者希望通过大间隔合成数据实现迭代控制和高度表达性，避免嵌入先验或辅助模块的需求。

Method: 核心创新是仅使用大间隔合成数据进行训练，这种方法支持迭代控制，并在声音表达性方面表现优异。

Result: 评估结果显示，Step-Audio-EditX在情感编辑和其他细粒度控制任务上超越了MiniMax-2.6-hd和Doubao-Seed-TTS-2.0。

Conclusion: 该研究代表了从传统表示级解缠方法向大间隔学习方法的根本性转变，为表达性音频编辑提供了新的解决方案。
...
TL;DR: 对2019-2024年间Transformer模型在关系抽取领域的研究进行系统性综述，分析了34篇综述、64个数据集和104个模型，总结了方法进展、基准资源和语义网技术整合。


<details>
  <summary>Details</summary>
Motivation: 随着Transformer模型的兴起，关系抽取研究迅速发展，需要系统性地梳理该领域的方法进展、资源现状和未来方向，为研究者和从业者提供全面参考。

Method: 使用自动化框架收集和标注2019-2024年间发表的文献，包括34篇综述论文、64个数据集和104个模型，进行多维度分析。

Result: 识别了关系抽取领域的方法论进展、基准资源发展以及语义网技术的整合情况，揭示了当前趋势、局限性和开放挑战。

Conclusion: 本研究为理解关系抽取的演进历程和未来发展方向提供了全面参考，有助于指导该领域的后续研究。
...
TL;DR: 提出IRIS框架，通过隐式和显式推理实现可解释的零样本立场检测，将立场检测建模为信息检索排序任务，无需真实推理标注即可提供内在可解释性。
...
Method: IRIS框架基于文本中的序列（隐式推理）和语言度量（显式推理）来理解输入对目标的态度，将立场检测作为信息检索排序任务，理解不同立场的隐式推理相关性。

Result: 在VAST、EZ-STANCE、P-Stance和RFD基准数据集上使用50%、30%甚至10%训练数据的广泛实验证明了模型的泛化能力。

Conclusion: IRIS通过提出的架构和可解释设计，实现了有效的零样本立场检测，并提供内在的可解释性。
...
TL;DR: 本文提出了ChiMDQA中文多文档问答数据集，包含6个领域的6,068个高质量问答对，分为10个细粒度类别，旨在支持中文文档理解、知识提取和智能问答系统等NLP任务。
...
Method: 通过精心筛选文档和系统化的问题设计方法，构建了涵盖学术、教育、金融、法律、医疗和新闻六个领域的长文档数据集，包含10个细粒度类别的问答对。
...
TL;DR: 本文研究发现大型语言模型表现出部分阴谋论倾向，存在社会人口统计偏见，且容易被诱导采纳阴谋论观点。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否再现阴谋论心理等高级心理结构，评估其社会保真度，因为阴谋信念在错误信息传播中起关键作用。

Method: 使用经过验证的心理测量调查工具，在不同提示和条件策略下对多个模型进行测试。

Result: LLMs对阴谋信念要素表现出部分认同，社会人口属性条件化产生不均衡效应，暴露潜在人口偏见，且针对性提示可轻易将模型响应转向阴谋论方向。

Conclusion: 需要批判性评估LLMs中嵌入的心理维度，以推进计算社会科学并为有害使用的缓解策略提供信息。
...
TL;DR: 本文提出了一种基于视角的标注方案，用于分析协作对话中的共同基础建立过程，特别关注参与者认为已达成一致但实际上指代不同实体的情况。


<details>
  <summary>Details</summary>
Motivation: 在非对称对话环境中，参与者可能认为已达成共识但实际上指代不同实体，这种误解难以通过传统标注方案捕捉。

Method: 使用方案约束的LLM标注流程，对HCRC MapTask语料库中的13k个指代表达式进行标注，分别捕捉说话者和受话者的理解视角。

Result: 结果显示，一旦统一词汇变体，完全误解很少见，但多重性差异会系统性导致理解分歧，表明表面上的共同基础可能掩盖指代错位。

Conclusion: 该框架为研究基于视角的共同基础建立提供了资源和分析工具，可用于评估(V)LLMs在协作对话中建模视角依赖共同基础的能力。
...
<details>
  <summary>Details</summary>
Motivation: 准确及时的土地覆盖图对理解土地利用变化至关重要，但地理空间嵌入在实际制图应用中的潜力尚未充分探索
...
TL;DR: 提出生成提示训练方法，通过生成模型创建虚拟样本来直接在全输入空间强制已知不变性，相比传统数据增强方法在多个数据集上获得更好的性能提升
...
Conclusion: 生成提示方法能够有效学习已知不变性，相比传统数据增强在性能上取得显著提升，为学习函数属性提供了更有效的方法
...
TL;DR: ProM3E是一个概率掩码多模态嵌入模型，用于生态学中任意模态到任意模态的生成。它通过掩码模态重建学习从少量上下文模态推断缺失模态，支持嵌入空间中的模态反转，并提出新颖的跨模态检索方法。
...
Method: 基于掩码模态重建在嵌入空间中学习，支持模态反转，提出混合模态间和模态内相似度的跨模态检索方法。

Result: 模型在所有检索任务中表现出优越性能，并通过线性探测任务展示了其卓越的表征学习能力。

Conclusion: ProM3E为生态学多模态分析提供了有效的解决方案，代码、数据集和模型将公开发布。
...
TL;DR: EvtSlowTV是一个从YouTube视频构建的大规模事件相机数据集，包含超过130亿个事件，用于自监督深度估计，无需帧级标注，提升模型在复杂场景中的泛化能力。
...
Method: 从公开YouTube视频构建EvtSlowTV大规模数据集，采用自监督学习框架利用事件流的高动态范围特性。

Result: EvtSlowTV比现有事件数据集大一个数量级，训练模型在复杂场景和运动中表现出更好的泛化性能。

Conclusion: EvtSlowTV数据集为事件相机深度估计提供了无约束的自然环境，自监督方法有效利用了事件的异步特性。
...
TL;DR: 提出新的CNN-ViT混合搜索空间，通过NAS寻找适合tinyML部署的高效图像分类架构


<details>
  <summary>Details</summary>
Motivation: 现有CNN-ViT混合架构参数量大、计算成本高，不适合tinyML部署

Method: 构建包含CNN-ViT混合块和可搜索池化层的NAS搜索空间，学习局部和全局信息

Result: 在CIFAR10数据集上，搜索出的混合架构在严格模型大小限制下，精度和推理速度优于ResNet-based tinyML模型

Conclusion: 提出的搜索空间能产生高效混合CNN-ViT架构，适合tinyML应用
...
TL;DR: SCALE-VLP是一个软加权对比视觉语言预训练框架，专门针对CT等体积医学数据，通过整合体积空间语义和领域知识语义，在有限监督下实现结构一致和语义基础的表示。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要局限于2D数据和二元监督，忽略了CT等体积数据的连续结构化依赖关系，且通常将体积扫描视为独立2D切片，损害了空间连贯性和临床语义的充分利用。

Method: 提出SCALE-VLP框架，整合(i)体积空间语义以保持解剖结构，(ii)领域感知的知识注入语义（如放射学本体）来指导对齐，采用软加权对比学习方法。

Result: 相比之前最优方法，SCALE-VLP在CT-报告检索中达到4.3倍更高的top-1准确率，异常分类提升10个百分点，报告生成达到ROUGE-L 0.44和BERT-F1 0.89。在零样本跨域评估中表现出持续增益。
...
TL;DR: 提出了一种基于自监督学习的标签高效方法，仅使用1000个标注样本即可实现州级1米分辨率土地覆盖分类，准确率达到87.14%。
...
Result: 在密西西比州实现了8类土地覆盖分类，总体准确率87.14%，宏F1分数75.58%，覆盖超过1230亿像素。

Conclusion: 自监督学习能有效减少对人工标注数据的需求，为大规模高分辨率土地覆盖制图提供了可行方案。
...
Method: 使用单一编码器架构，结合可学习模态嵌入、条件层归一化、掩码自编码目标，并应用方差-协方差正则化器。在约60,000个多中心MRI上进行自监督重建和模态插补训练。

Result: 初步结果显示方法可行，计划在脑肿瘤和多发性硬化分割以及病变分类任务上进行评估。

Conclusion: 该方法消除了为每种模态训练单独模型的需求，使网络能够灵活适应不同的输入模态组合。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLP方法将图像-文本对视为孤立训练样本，忽略了领域内丰富的关联结构（如电商产品共购图、社交推荐网络），而神经科学证据表明人类将知识编码为关系认知图谱。

Method: 引入结构感知语言-图像预训练(SLIP)，整合结构对比损失来对齐模态并建模结构化图中相邻实体的关系，构建了大规模亚马逊产品共购多模态图数据集。

Result: 实验结果显示SLIP在零样本和少样本设置下，在跨模态检索和分类任务中持续优于CLIP。
...
TL;DR: 提出基于学习的移动激光扫描点云不确定性评估框架，无需地面真值，结合最优邻域估计和几何特征提取，XGBoost模型在保持精度的同时效率比随机森林提高约3倍。


<details>
  <summary>Details</summary>
Motivation: 移动激光扫描点云在高精度应用中需要评估不确定性，但获取地面真值成本高昂且在实际应用中往往不可行，需要减少对地面真值的依赖。

Method: 提出学习框架，集成最优邻域估计和几何特征提取，使用XGBoost模型预测点级不确定性（通过C2C距离量化）。

Result: 在真实数据集上验证框架可行性，XGBoost模型精度与随机森林相当但效率提高约3倍，证明几何特征可用于预测点级不确定性。
...
TL;DR: 提出结合压缩感知和光片显微镜的高性能计算成像框架，用于高效、低光毒性的心脏成像，通过压缩采集和先进去噪算法成功重建斑马鱼心脏细胞结构
...
Method: 集成压缩感知与光片显微镜，使用数字微镜设备进行随机二进制掩模编码压缩采集，提出基于ADMM的Plug-and-Play框架，结合Tikhonov、TV和BM3D等先进去噪器，并引入时间正则化保持结构连续性
...
TL;DR: ISC-Perception是首个专门用于ISC组件检测的混合数据集，结合了CAD渲染图像、游戏引擎真实感场景和少量真实照片，显著降低了人工标注时间，提升了检测模型性能。
...
Result: 生成10,000张图像数据集的人工时间仅为30.5小时，相比手动标注节省81.7%；训练出的检测器在IoU 0.50下达到0.756的平均精度，显著优于仅使用合成或逼真数据的模型。

Conclusion: ISC-Perception填补了建筑机器人感知的数据空白，支持快速开发定制目标检测器，可供研究和工业使用。
...
TL;DR: DentalSplat是一个用于从稀疏正畸图像进行3D重建的有效框架，通过先验引导的密集立体重建初始化点云，结合尺度自适应剪枝策略和光学流几何约束，在仅有三个稀疏视角的情况下实现高质量的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 在远程正畸治疗中，从多视角观察患者牙合关系对及时临床决策至关重要。传统3DGS方法需要密集多视角输入和精确相机位姿，而正畸病例通常只有三个稀疏图像（前视图和双侧颊视图），这给重建任务带来极大挑战。

Method: 1. 使用先验引导的密集立体重建模型初始化点云；2. 采用尺度自适应剪枝策略提高3DGS训练效率和重建质量；3. 在极端稀疏视角下，结合光学流作为几何约束和梯度正则化来增强渲染保真度。

Result: 在包含950个临床病例的大规模数据集和195个视频测试集上的实验表明，该方法能有效处理稀疏输入场景，在牙合可视化新视角合成质量上优于现有最先进技术。

Conclusion: DentalSplat框架成功解决了稀疏正畸图像3D重建的挑战，为远程正畸治疗提供了有效的多视角可视化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 集成电路制造复杂，缺陷可能出现在任何阶段导致良率损失。监督方法需要大量人工标注且难以处理新兴类别和罕见缺陷，聚类方法因缺乏先验而性能不稳定

Method: 1. 自归一化信息引导的缺陷检测：通过可学习的正常信息提取器聚合代表性正常特征，使用重建残差粗定位缺陷区域；2. 自适应二值化策略处理缺陷显著性变化；3. 自缺陷信息引导的缺陷分类：软掩码引导注意力机制将空间缺陷先验注入师生模型

Result: 在涵盖三个关键制造阶段和15种缺陷类型的真实数据集上验证，实验表明在缺陷检测和未见缺陷分类方面均表现稳健

Conclusion: IC DefectNCD框架有效解决了IC制造中缺陷检测和新类别发现的挑战，无需支持集，具有实际应用价值
...
TL;DR: Sysname系统通过算法和系统优化，将视觉引导的物理属性推理延迟从10-20分钟降低到6秒以内，实现了62.9-287.2倍加速，同时保持或提升了物理属性估计精度。


<details>
  <summary>Details</summary>
Motivation: 解决视觉引导物理属性推理中运行延迟过高的问题，使其能够在增强现实等实时应用中实用。

Method: 结合快速几何3D重建、高效语义特征融合和并行视图编码等算法和系统优化。

Result: 在ABO数据集上，系统实现了62.9-287.2倍加速，物理属性估计精度与基线相当或更好，在材料分割和体素级推理上表现更优。在IKEA家具店的真实场景测试中，即使视图较少也能提供稳健的属性估计。

Conclusion: Sysname成功实现了实时视觉引导物理属性推理，为智能眼镜等增强现实设备提供了实用的物理认知能力。
...
TL;DR: 首个用于无人机图像建筑损伤评估的AI/ML系统，在飓风灾害期间实际部署，解决了海量图像数据处理的挑战。


<details>
  <summary>Details</summary>
Motivation: 灾害期间无人机团队每天收集47GB-369GB图像，远超专家人工处理能力，延误救援响应。需要自动化解决方案来应对数据雪崩问题。

Method: 基于最大已知的灾后无人机图像数据集（21,716个建筑损伤标签）训练模型，并对91名灾害从业人员进行操作培训。

Result: 在飓风Debby和Helene响应期间部署，18分钟内评估了415栋建筑，建立了无人机损伤评估的实际操作标准。
...
TL;DR: 提出一种基于超网络的免微调文本到图像个性化方法，通过超网络直接从主体图像预测LoRA适配权重，无需逐主体优化，同时保持主体保真度和提示对齐。


<details>
  <summary>Details</summary>
Motivation: 传统基于主题特定微调的方法（如DreamBooth）计算成本高且推理速度慢，现有适配器和编码器方法仍需额外微调或大型骨干模型。需要寻找更高效的免微调个性化方案。

Method: 使用超网络预测LoRA适配权重，采用端到端训练目标和简单输出正则化进行稳定化，并提出混合模型无分类器引导（HM-CFG）在采样时结合基础扩散模型的组合能力和个性化模型的主体保真度。

Result: 在CelebA-HQ、AFHQ-v2和DreamBench上的广泛实验表明，该方法实现了强大的个性化性能，展示了超网络作为可扩展有效开放类别个性化方向的潜力。

Conclusion: 该方法消除了测试时逐主体优化的需求，同时保持主体保真度和提示对齐，证明了超网络在文本到图像个性化中的可行性和有效性。
...
TL;DR: 提出了一种深度引导的肝脏标志点分割框架，通过结合语义和几何线索，使用SAM2和DA2编码器分别提取RGB和深度特征，并引入SRFT-GaLore高效微调方法，在腹腔镜肝脏手术中实现精确分割。
...
TL;DR: 提出了首个用于外科手术前瞻性推理的视觉问答数据集PitVQA-Anticipation，并开发了SurgAnt-ViVQA视频语言模型，通过时间感知编码和门控交叉注意力实现手术事件的预测。


<details>
  <summary>Details</summary>
Motivation: 在经鼻蝶垂体手术中，能见度有限且工作流程变化迅速，预测未来手术事件对实时辅助至关重要。现有VQA系统主要关注当前场景而非未来预测。

Method: 使用GRU门控时间交叉注意力模块，双向GRU编码帧间动态，自适应门控在token级别注入视觉上下文到语言流中，通过参数高效微调定制语言骨干网络。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏结合功能性和解剖性影像与详细临床报告的多癌症类型数据集，这对AI模型开发和临床研究构成限制。

Method: 收集了490名多种恶性肿瘤患者的全身18F-FDG PET/CT扫描、配对的PET和CT图像、去标识化文本报告和结构化临床元数据。

Result: 构建了一个包含常见癌症（肺癌、肝癌、乳腺癌、前列腺癌、卵巢癌）的综合性多模态医学影像数据集。

Conclusion: PETWB-REP数据集为医学影像分析、AI模型验证和临床研究提供了宝贵的资源，填补了现有数据集的空白。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在多图像场景中存在两个关键问题：缺乏跨图像的细粒度感知能力，以及多视觉信息推理和整合能力下降。现有研究主要关注单图像或特定受限场景，缺乏对复杂多图像推理任务的理解和解决。

Method: 提出零样本提示方法QG-CoC（问题引导的标题链），这是一种通用提示方法，能够有效处理任意数量图像的问题。

Result: 在多个开源和闭源多模态大语言模型上进行评估，QG-CoC在各种任务中表现出竞争力，并在现有提示方法失败的挑战性场景中展现出稳健的改进。

Conclusion: QG-CoC方法能够有效提升多模态大语言模型在多图像场景中的感知和推理能力，在复杂场景中表现优于现有方法。
...
TL;DR: 本研究提出了一种基于多视角Transformer网络(MvBody)的新方法，利用自报医疗数据和3D光学身体扫描来预测剖腹产风险，在资源有限的环境中实现84.62%的准确率。


<details>
  <summary>Details</summary>
Motivation: 在医疗资源有限的环境中，早期可靠地预测剖腹产风险对于改善母婴健康结果至关重要，但现有模型多依赖医院内参数，不适用于资源受限或家庭环境。

Method: 提出MvBody多视角Transformer网络，结合自报医疗数据和妊娠31-38周的3D光学身体扫描，并引入度量学习损失以提高训练效率和模型泛化能力。

Result: 在独立测试集上达到84.62%的准确率和0.724的AUC-ROC值，优于现有机器学习模型和先进3D分析方法。

Conclusion: 研究表明孕前体重、产妇年龄、产科史、既往剖腹产史以及头肩部身体形态是剖腹产风险预测的关键因素，该方法在资源有限环境中具有应用潜力。
...
TL;DR: 提出了一种结合扩散生成和样本混合的数据增强方法MCPMix，通过掩码一致的配对混合和真实锚定学习退火机制，在保持像素级语义的同时增强多样性，在多个医学图像分割数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统数据增强方法存在软标签模糊和合成-真实域偏移问题，需要一种既能增强多样性又不损害像素级语义的增强方法。

Method: MCPMix方法为每个真实图像生成相同掩码下的合成对应物，然后进行掩码一致的配对混合，只混合外观而保持原始硬掩码监督；RLA机制自适应调整混合强度和损失权重。

Result: 在Kvasir-SEG、PICCOLO、CVC-ClinicDB、NPC-LES和ISIC 2017数据集上实现了最先进的分割性能，相比基线方法获得了一致的性能提升。

Conclusion: 结合标签保持混合与扩散驱动的多样性，配合自适应重锚定机制，能够产生鲁棒且可泛化的内窥镜分割结果。
...
TL;DR: 提出T-PMambaSR轻量级超分辨率框架，结合窗口自注意力和渐进式Mamba，通过自适应高频细化模块恢复丢失的细节，在降低计算成本的同时优于现有Transformer或Mamba方法。


<details>
  <summary>Details</summary>
Motivation: 现有Mamba超分辨率方法缺乏不同建模尺度间的细粒度过渡，限制了特征表示效率。

Method: 集成窗口自注意力与渐进式Mamba，实现不同尺度感受野的交互；引入自适应高频细化模块恢复丢失的高频细节。

Result: 实验表明T-PMambaSR能渐进增强模型感受野和表达能力，性能优于近期Transformer或Mamba方法，计算成本更低。

Conclusion: T-PMambaSR建立了细粒度建模范式，以线性复杂度渐进增强特征表示，在超分辨率任务中表现出色。
...
TL;DR: 提出DMPO方法，通过解耦早期阶段的低层表征能力与高层判别能力，提升预训练模型的推理效率。


<details>
  <summary>Details</summary>
Motivation: 解决早期阶段如何同时为深层阶段提供基础特征和为早期预测器提供判别特征的挑战，提高推理效率。

Method: 引入轻量级旁路模块进行功能分解，开发基于高阶统计的预测器，采用解耦优化策略分配两阶段损失权重。

Result: 在各种数据集和预训练骨干网络上的实验表明，DMPO在降低计算成本时明显优于对比方法。

Conclusion: DMPO在架构设计和模型优化方面有效解耦了早期阶段的表征和判别能力，显著提升推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决超声数据中不同子模态（如灰度和彩色多普勒）不平衡的问题，通过图像转换技术来平衡数据集，提升医学影像分析的性能。

Method: 利用像素级损失、对抗损失和感知损失，采用两个网络：一个用于重建解剖结构，一个用于去噪，在54,975个视频上训练，8,368个视频上测试。

Result: 合成视频与真实视频的平均SSIM为0.91±0.04，在DL分类和分割任务中表现与真实视频无差异，临床专家区分准确率仅为54±6%。

Conclusion: 该方法能够生成逼真的超声视频，扩展了回顾性收集影像的实用性，增强了医学影像数据集设计的工具箱。
...
TL;DR: 提出U-Mamba与热传导方程结合的混合架构，用于医学图像分割，在计算预算内实现高效的全局上下文建模和长距离依赖推理。


<details>
  <summary>Details</summary>
Motivation: 现有模型难以在实用计算预算下同时实现高效的全局上下文建模和长距离依赖推理。

Method: 结合基于Mamba的状态空间模块进行高效长距离推理，在瓶颈层使用热传导算子模拟频域热扩散以增强语义抽象。

Result: 在多模态腹部CT和MRI数据集上的实验结果表明，该模型始终优于强基线，验证了其有效性和泛化性。
...
TL;DR: 提出了针对工业设备组件的3D异常检测数据集IEC3D-AD和基于几何形态分析的新方法GMANet，解决了现有数据集在真实工业场景中的不足。
...
TL;DR: 提出了一种基于LoRA微调和重叠混合时间协同去噪的统一方法，用于实现任意长度视频的高质量修复和外延，解决了长视频生成中的可控性挑战。


<details>
  <summary>Details</summary>
Motivation: 解决长视频生成中的根本挑战，特别是视频修复和外延的高可控性需求，同时处理这两个问题以实现长视频剪辑的可控编辑。

Method: 利用LoRA高效微调预训练视频扩散模型（如Wan 2.1），采用重叠混合时间协同去噪策略和高阶求解器来保持长序列的一致性。

Result: 在数百帧的修复/外延任务中表现出色，在质量指标（PSNR/SSIM）和感知真实性（LPIPS）上优于Wan 2.1和VACE等基线方法。

Conclusion: 该方法实现了参数效率与卓越性能的平衡，为长距离视频编辑提供了实用的解决方案，且计算开销最小。
...
TL;DR: 本文提出Diffusion-SDPO方法，解决了扩散模型直接偏好优化中的关键问题：偏好边界扩大不一定改善生成质量，反而可能增加重构误差。新方法通过自适应缩放失败者梯度来保护获胜者，确保偏好输出误差不增加。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的直接偏好优化方法存在缺陷，扩大偏好边界反而会降低生成质量，因为标准Diffusion-DPO目标会增加获胜者和失败者分支的重构误差，导致偏好分支也受到负面影响。

Method: 提出Diffusion-SDPO方法，采用保护性更新规则，根据失败者梯度与获胜者梯度的对齐程度自适应缩放失败者梯度，通过一阶分析得到闭式缩放系数，保证偏好输出误差在每一步优化中不增加。

Result: 在标准文本到图像基准测试中，Diffusion-SDPO在自动化偏好、美学和提示对齐指标上相比偏好学习基线方法获得了一致的提升。

Conclusion: Diffusion-SDPO是一种简单、模型无关的方法，与现有DPO风格对齐框架广泛兼容，仅增加少量计算开销，能有效改善扩散模型的偏好对齐效果。
...
TL;DR: 提出SurgViVQA手术视频问答模型，通过融合视频和问题特征的掩码视频-文本编码器捕捉时间动态信息，在结肠镜视频数据集上相比现有图像基准模型提升关键词准确率11-9%。


<details>
  <summary>Details</summary>
Motivation: 当前手术视频QA方法局限于静态图像特征，缺乏对时间动态的建模，无法准确理解手术过程中的动态事件和工具-组织交互。

Method: 使用掩码视频-文本编码器融合视频和问题特征，捕捉运动和时间线索，然后通过微调的大语言模型解码生成连贯答案。

Result: 在REAL-Colon-VQA和EndoVis18-VQA数据集上，SurgViVQA在关键词准确率上分别比PitVQA提升11%和9%，对问题表述变化具有更好的鲁棒性。

Conclusion: SurgViVQA和REAL-Colon-VQA数据集为手术视频QA提供了时间感知理解的框架，使AI模型能更有效地解释动态手术过程。
...
TL;DR: 本文提出了一种用于MOT25-StAG挑战赛的两阶段零样本方法，结合FastTracker跟踪模型和LLaVA-Video多模态大语言模型，在视频中基于语言查询进行多目标定位和跟踪。
...
Result: 在MOT25-StAG测试集上获得m-HIoU 20.68和HOTA 10.73的成绩，在挑战赛中获得第二名。

Conclusion: 提出的两阶段零样本方法在复杂场景的多目标语言查询定位跟踪任务中表现优异，证明了方法的有效性。
...
TL;DR: UniAVGen是一个统一的音频-视频生成框架，通过双分支扩散变换器构建跨模态潜在空间，采用非对称跨模态交互机制和面部感知调制模块，显著提升了唇部同步和语义一致性。
...
Method: 采用双分支联合合成架构，包含两个并行扩散变换器；核心是非对称跨模态交互机制，支持双向时间对齐的跨注意力；增强的面部感知调制模块动态优先处理显著区域；引入模态感知无分类器引导策略。

Result: 在仅使用1.3M训练样本（对比30.1M）的情况下，在音频-视频同步、音色一致性和情感一致性方面展现出整体优势。
...
TL;DR: 提出AAPL方法，通过引入对抗性token嵌入来解耦图像增强引入的表层视觉变化与类别相关语义表示，提升提示学习在零样本学习中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有提示学习方法如CoOp和CoCoOp主要关注文本修改，忽略了图像级增强的潜力，且缺乏对语义有意义的视觉特征的显式指导。

Method: AAPL方法引入对抗性token嵌入，解耦图像增强产生的表层变化与类别语义表示，使学习到的提示能专注于与目标类别对齐的视觉判别性特征。

Result: 在11个基准数据集上的实验表明，AAPL在少样本、零样本、跨数据集和领域泛化设置中均优于现有方法。

Conclusion: 图像级增强特别是属性特定变化能有效支持提示学习，AAPL通过解耦策略显著提升了模型的泛化能力。
...
TL;DR: 提出了一种自动化方法，通过主成分分析（PCA）从胚胎分割掩码中提取主轴，生成四个候选方向，并使用三种策略选择标准方向，在2166个3D超声图像中达到98.5%的准确率。
...
Method: 使用PCA从胚胎分割掩码中提取主轴，生成四个候选方向，然后通过三种策略选择标准方向：基于皮尔逊相关性的启发式方法、通过归一化互相关与图谱进行图像匹配，以及随机森林分类器。

Result: 在2166个图像中，PCA在99.0%的情况下正确提取了胚胎的主轴。皮尔逊启发式、基于图谱和随机森林方法分别正确选择了97.4%、95.8%和98.4%的候选方向。多数投票方法达到了98.5%的准确率。
...
TL;DR: 提出了一个能够处理拓扑变化的Shape-from-Template方法，通过迭代分区模板表面来重建具有撕裂、切割等拓扑变化的变形物体表面。
...
Method: 使用经典SfT解初始化，通过迭代分区模板空间域来最小化结合物理合理性和重投影一致性的能量函数。
...
TL;DR: Anny是一个简单、完全可微分、无需3D扫描的人体模型，基于MakeHuman社区的人类测量学知识构建，提供连续可解释的形状空间和广泛的人口统计学代表性。


<details>
  <summary>Details</summary>
Motivation: 现有参数化人体模型依赖昂贵的3D扫描和专有学习形状空间，存在人口统计学代表性不足的问题。

Method: 基于MakeHuman社区的人类测量学知识构建连续可解释的形状空间，使用表型参数（性别、年龄、身高、体重）控制混合形状，并通过WHO人口统计数据进行校准。

Result: Anny支持毫米级精度的扫描拟合、可控合成数据生成和人体网格恢复，训练的人体网格恢复模型性能可与基于扫描的模型相媲美。

Conclusion: Anny为3D人体建模提供了开放、语义可控且具有广泛代表性的基础，其模型和代码在Apache 2.0许可下发布。
...
TL;DR: 提出了一种信号强度加权的坐标表示方法，用局部信号强度缩放坐标通道，取代传统的纯坐标通道，在输入表示中嵌入强度-位置耦合的归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 生物医学数据中的定位任务需要模型从具有复杂强度分布的信号中学习有意义的空间或时间关系。传统方法如CoordConv层通过附加坐标通道来学习绝对位置，但缺乏信号强度信息。

Method: 用局部信号强度缩放坐标通道，创建信号强度加权的坐标表示，在输入中直接嵌入强度-位置耦合关系。

Result: 在两个不同的定位任务上评估：ECG信号形态转换时间预测和细胞图像核中心坐标回归。该方法相比传统坐标通道方法收敛更快，泛化性能更高。

Conclusion: 提出的强度加权坐标表示在1D和2D生物医学信号上都表现出有效性，能够提高定位任务的性能。
...
TL;DR: 本文提出了一种轻量级3D卷积神经网络，使用基于事件的视觉数据进行人类活动识别，在保护隐私的同时实现了94.17%的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统基于帧的摄像头会捕捉可识别的个人信息，存在隐私保护问题，而事件相机仅记录像素强度变化，提供固有的隐私保护感知方式。

Method: 使用轻量级3DCNN建模时空动态，采用焦点损失与类别重加权解决类别不平衡，结合针对性数据增强策略提升泛化能力。

Result: 在Toyota Smart Home和ETRI数据集上的实验显示，F1分数达到0.9415，整体准确率94.17%，比C3D、ResNet3D和MC3_18等基准模型性能提升高达3%。

Conclusion: 基于事件的深度学习在开发准确、高效且具有隐私保护的人类动作识别系统方面具有巨大潜力，适合实际边缘应用部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有社交互动检测方法忽略了细微线索（如面部表情、注视、手势），主要依赖个体整体表示，且直接检测社交群体而未显式建模个体间互动，导致难以捕捉局部社交信号并在基于细微线索推断群体配置时产生歧义。

Method: 首先检测个体并使用部位感知线索增强其特征，然后通过基于相似性的推理关联个体来推断群体配置，该推理不仅考虑空间关系，还考虑信号互动的细微社交线索。

Result: 在NVI数据集上的实验表明，该方法优于先前方法，达到了新的最先进水平。

Conclusion: 提出的部位感知自下而上群体推理框架能够更准确地检测细粒度社交互动，通过显式建模个体间互动和利用细微社交线索，提高了群体推断的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于显著性的方法产生纠缠的解释，无法区分预测是依赖运动还是空间上下文；基于语言的方法虽然提供结构但难以解释直观但难以言表的运动。

Method: 提出解耦概念框架，将动作预测分为三类概念：运动动态（人体姿态序列）、物体和场景（通过大语言模型自动提取），采用前概念瓶颈设计强制通过这些概念进行预测。

Result: 在KTH、Penn Action、HAA500和UCF-101四个数据集上的实验表明，DANCE在保持竞争力的同时显著提升了解释清晰度，用户研究验证了其优越的可解释性。

Conclusion: DANCE框架不仅提供更清晰的解释，还有益于模型调试、编辑和故障分析，为视频动作识别提供了有效的可解释性解决方案。
...
TL;DR: VoxStudio是首个统一的端到端语音到图像生成模型，直接从语音描述生成富有表现力的图像，通过联合对齐语言和副语言信息，无需额外语音转文本系统。


<details>
  <summary>Details</summary>
Motivation: 现有语音到图像生成方法通常依赖语音转文本系统，会忽略语音中的语调、情感等副语言信息，无法生成富有情感表达的图像。

Method: 提出语音信息瓶颈模块压缩原始语音为紧凑语义标记，保留韵律和情感细节；基于这些标记直接生成图像；构建大规模情感语音-图像数据集VoxEmoset。

Result: 在SpokenCOCO、Flickr8kAudio和VoxEmoset基准测试上的综合实验证明了方法的可行性，并揭示了情感一致性和语言歧义等关键挑战。

Conclusion: VoxStudio为语音驱动的图像生成开辟了新途径，为未来研究铺平了道路。
...
### [90] [Scheduling the Off-Diagonal Weingarten Loss of Neural SDFs for CAD Models](https://arxiv.org/abs/2511.03147)
*Haotian Yin,Przemyslaw Musialski*

Main category: cs.GR

TL;DR: 提出了一种针对神经符号距离函数中Off-Diagonal Weingarten损失的自适应调度策略，通过动态调整正则化权重来平衡早期稳定性和后期细节恢复，在CAD重建任务中显著优于固定权重方法。


<details>
  <summary>Details</summary>
Motivation: FlatCAD中固定的ODW权重在训练过程中存在权衡：早期需要强正则化来稳定优化，但后期会抑制细节恢复。需要一种自适应策略来动态调整正则化强度。

Method: 研究了多种ODW损失调度策略，包括常数、线性、五次和步进插值调度，以及增加预热变体。这些策略在训练过程中动态调整正则化权重。

Result: 在ABC CAD数据集上的实验表明，时变调度策略始终优于固定权重方法，在Chamfer距离指标上相比FlatCAD基线提升高达35%。

Conclusion: ODW损失调度是一种简单而有效的扩展，能够显著提升CAD重建的鲁棒性，证明了动态正则化策略在几何重建中的重要性。
...
TL;DR: 本研究使用文本分析方法追踪社会科学知识在学术、新闻和政策三个领域间的传播模式，发现不同理论传播差异显著，且语义含义在不同领域发生角色转变。


<details>
  <summary>Details</summary>
Motivation: 社会科学知识传播对各方利益相关者很重要，但传统引用分析难以全面衡量知识在不同领域间的扩散，需要新的测量方法。

Method: 使用72,703份文档(2000-2019)，通过文本分析追踪媒体效应理论在学术、新闻和政策三个领域的提及情况，采用嵌入回归方法比较跨领域的语境化含义。

Result: 不同理论的传播模式差异显著；研究领域与政策领域的语义距离通常大于与新闻领域的距离；理论在不同领域发生角色转变(从理论本身到意义建构再到应用)；实践导向的理论随时间出现语义趋同。

Conclusion: 该方法能够从概念层面描述社会科学知识的跨领域传播模式和动态，为超越引用的知识扩散测量提供了新视角。
...
### [92] [LiveTradeBench: Seeking Real-World Alpha with Large Language Models](https://arxiv.org/abs/2511.03628)
*Haofei Yu,Fenghai Li,Jiaxuan You*

Main category: q-fin.TR

TL;DR: LiveTradeBench是一个实时交易环境，用于在动态市场中评估LLM代理的决策能力，填补了静态基准测试与真实世界能力之间的差距。


<details>
  <summary>Details</summary>
Motivation: 当前LLM基准测试在静态环境中进行，缺乏真实动态性和不确定性，无法评估在不确定条件下的决策能力。

Method: 设计LiveTradeBench遵循三个原则：实时数据流、投资组合管理抽象、多市场评估（美股和预测市场）。

Result: 对21个LLM进行50天实时评估发现：高LMArena分数不保证交易成功；模型展现不同风险偏好和推理动态；部分LLM能有效利用实时信号调整决策。

Conclusion: 静态评估与真实世界能力存在差距，需要能够测试序列决策和在实时不确定性下一致性的基准。
...
### [93] [Benchmarking ResNet for Short-Term Hypoglycemia Classification with DiaData](https://arxiv.org/abs/2511.02849)
*Beyza Cinar,Maria Maleshkova*

Main category: eess.SP

TL;DR: 本研究改进了DiaData数据集的质量，通过异常值处理、缺失值插补等方法提升1型糖尿病数据分析的可靠性，并建立了低血糖分类基准。


<details>
  <summary>Details</summary>
Motivation: 1型糖尿病个体化治疗需要高质量数据支持，但现有数据存在异常值、噪声、数据量小和缺失值等问题，限制了可靠分析。

Method: 1) 使用IQR方法识别异常值并替换为缺失值；2) 对≤25分钟的小间隙使用线性插值，对30-120分钟的大间隙使用Stineman插值；3) 分析血糖与心率的相关性；4) 使用ResNet模型建立低血糖分类基准。

Result: Stineman插值在大间隙下比线性插值提供更真实的血糖估计；血糖与心率在低血糖前15-60分钟存在中等相关性；使用更多数据训练使性能提升7%，使用质量优化数据相比原始数据提升2-3%。

Conclusion: 数据质量优化显著提升了1型糖尿病数据分析的可靠性，为个体化治疗提供了更好的数据基础。
...
### [94] [NEF-NET+: Adapting Electrocardio panorama in the wild](https://arxiv.org/abs/2511.02880)
*Zehui Zhan,Yaojun Hu,Jiajing Zhan,Wanchen Lian,Wanqing Wu,Jintai Chen*

Main category: eess.SP

TL;DR: NEF-NET+是一个增强的心电图全景合成框架，能够从任意视角生成任意长度的心电图信号，克服了传统心电图系统的固定视角限制。


<details>
  <summary>Details</summary>
Motivation: 传统多导联心电图系统只能从固定解剖视角捕获心脏信号，某些心脏疾病需要非标准视角才能显示关键诊断模式。Nef-Net虽然能重建连续心电场，但在实际应用中面临长时程建模、设备特定伪影和电极放置偏差等挑战。

Method: 设计了新的模型架构执行直接视角变换，包含离线预训练、设备校准调谐步骤以及患者特定适应的实时校准步骤。构建了包含5367个记录、每个受试者48个视角的新基准Panobench。

Result: 实验结果显示NEF-NET+相比Nef-Net有显著改进，在真实世界设置中PSNR提高了约6 dB。

Conclusion: NEF-NET+能够实现更实用的全景心电图合成，支持任意长度信号生成，跨设备泛化，并能补偿电极放置偏差。
...
TL;DR: 使用Llama 3.1-405B大语言模型对引用基因组数据集的科学文献进行数据使用案例的自动标注，无需人工标注或训练数据集开发。


<details>
  <summary>Details</summary>
Motivation: 探索如何自动识别和描述科学文献中特定数据集的使用方式，避免昂贵的人工标注和传统机器学习系统训练数据开发。

Method: 应用开源LLM Llama 3.1-405B进行零样本数据引用分类，并引入新的评估框架来衡量方法效果。
...
### [96] [The Curved Spacetime of Transformer Architectures](https://arxiv.org/abs/2511.03060)
*Riccardo Di Sipio,Jairo Diaz-Rodriguez,Luis Serrano*

Main category: cs.LG

TL;DR: 该论文提出了一个几何框架，将基于Transformer的语言模型与广义相对论进行类比，将注意力机制视为在弯曲流形上的离散连接，并通过实验验证了表示空间中曲率的存在和影响。


<details>
  <summary>Details</summary>
Motivation: 动机是建立一个几何框架来理解Transformer语言模型，将查询和键诱导的有效度量与广义相对论中的时空曲率进行类比，探索表示空间中的几何结构。

Method: 方法包括：(1) 将注意力机制建模为离散连接，实现值向量在标记间的平行传输；(2) 设计实验可视化曲率景观；(3) 通过模拟分析角度分布和长度-弦长比；(4) 受爱因斯坦日食实验启发，通过受控上下文编辑探测嵌入轨迹的偏转。

Result: 实验结果：(1) 可视化显示了标记和层间局部转向角的变化；(2) 模拟表明尖锐/平坦角度的超额计数和较长长度-弦长比不能由维度或偶然性解释；(3) 上下文编辑实验证实了注意力诱导的曲率导致嵌入轨迹的可测量、意义一致的弯曲。

Conclusion: 结论是Transformer语言模型的表示空间确实存在曲率，注意力机制在弯曲流形上实现平行传输，这种几何类比为理解语言模型的内部工作机制提供了新的视角。
...
TL;DR: 本文提出了StaDec和DyDec两种创新的对抗攻击框架，通过利用LLMs的理解能力来生成动态自适应的对抗样本，评估LLMs在敏感任务中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当将LLMs应用于敏感任务时，需要全面评估其对对抗输入的鲁棒性。现有方法依赖外部启发式规则，缺乏系统性的评估框架。

Method: 开发了静态欺骗器(StaDec)和动态欺骗器(DyDec)两种攻击框架，利用LLMs自身理解生成语义相似但能有效欺骗目标模型的对抗输入，采用自动化的LLM驱动流程。

Result: 生成的对抗样本保持语义相似性且看起来自然，攻击具有强迁移性，能跨模型有效欺骗未知的目标LLM，攻击能力随LLM发展而进化。

Conclusion: 这项工作为LLMs的鲁棒性自我评估提供了系统性方法，代码和数据已开源。
...
### [98] [Data-Efficient Realized Volatility Forecasting with Vision Transformers](https://arxiv.org/abs/2511.03046)
*Emi Soroka,Artem Arzyn*

Main category: cs.LG

TL;DR: 使用Vision Transformer (ViT)架构预测资产未来30天的已实现波动率，探索将图像识别模型应用于期权数据的新方法


<details>
  <summary>Details</summary>
Motivation: 金融机器学习中深度学习方法的复杂性优势已被证实，但Transformer架构在期权数据中的应用仍较少探索。本文旨在开发适用于期权数据的Transformer模型

Method: 采用Vision Transformer (ViT)架构，训练模型从单日的隐含波动率表面（增强日期信息）预测未来30天的已实现波动率

Result: ViT能够从隐含波动率表面学习季节性模式和非线性特征，显示出良好的模型开发前景

Conclusion: 将Vision Transformer应用于期权数据是一个有前景的研究方向，能够有效捕捉隐含波动率表面的复杂模式
...
### [99] [Test Time Adaptation Using Adaptive Quantile Recalibration](https://arxiv.org/abs/2511.03148)
*Paria Mehrbod,Pedro Vianna,Geraldin Nanfack,Guy Wolf,Eugene Belilovsky*

Main category: cs.LG

TL;DR: 提出Adaptive Quantile Recalibration (AQR)方法，通过通道级别的分位数对齐来调整预激活分布，实现无需模型重训练的无监督测试时域适应。


<details>
  <summary>Details</summary>
Motivation: 传统域适应方法需要目标域先验知识或模型重训练，在动态或资源受限环境中实用性受限。现有基于批量归一化统计更新的测试时适应方法难以捕捉复杂激活分布且局限于特定归一化层。

Method: AQR在通道级别对齐分位数来修改预激活分布，采用鲁棒的尾部校准策略处理不同批量大小下的分布尾部估计问题，利用训练时计算的源域统计量实现无监督适应。

Result: 在CIFAR-10-C、CIFAR-100-C和ImageNet-C数据集上的实验表明，AQR在多种架构下均优于现有测试时适应基线方法，实现稳健的域适应。
...
### [100] [A Probabilistic U-Net Approach to Downscaling Climate Simulations](https://arxiv.org/abs/2511.03197)
*Maryam Alipourhajiagha,Pierre-Louis Lemaire,Youssef Diouane,Julie Carreau*

Main category: cs.LG

TL;DR: 该论文提出使用概率U-Net进行气候统计降尺度，结合确定性U-Net骨干和变分潜在空间来捕捉不确定性。评估了四种训练目标，发现WMSE-MS-SSIM在极端事件表现良好，而afCRPS在跨尺度空间变异性捕捉方面更优。
...
Method: 采用概率U-Net架构，结合确定性U-Net骨干和变分潜在空间来捕捉偶然不确定性。评估了四种训练目标：afCRPS和WMSE-MS-SSIM的三种设置。

Result: 主要发现是WMSE-MS-SSIM在某些设置下对极端事件表现良好，而afCRPS在捕捉跨尺度的空间变异性方面表现更好。

Conclusion: 不同的训练目标在气候降尺度任务中各有优势，需要根据具体应用需求选择合适的损失函数。
...
### [101] [A Feedback-Control Framework for Efficient Dataset Collection from In-Vehicle Data Streams](https://arxiv.org/abs/2511.03239)
...
Method: 使用在线概率模型持续估计已收集数据分布状态，基于似然和马氏距离等反馈信号自适应调节样本保留，动态平衡探索与利用。

Result: 在真实数据流实验中，FCDC将数据集平衡性提升25.9%，同时减少数据存储39.8%。

Conclusion: 数据收集本身可以主动控制，将收集从被动流水线阶段转变为以数据为中心AI核心的自调节、反馈驱动过程。
...
### [102] [Decoupled Entropy Minimization](https://arxiv.org/abs/2511.03256)
*Jing Ma,Hanlin Li,Xiang Xiang*

Main category: cs.LG

TL;DR: 本文分析了经典熵最小化(EM)的内在机制，将其解耦为两个相反作用的部分，揭示了其局限性并提出自适应解耦熵最小化(AdaDEM)方法，在多种不完美监督学习任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 经典熵最小化(EM)在机器学习中虽然有益于减少类别重叠、弥合领域差距和限制不确定性，但其潜力有限。需要研究EM的内部机制并解决其局限性。

Method: 将经典EM解耦为聚类聚合驱动因子(CADF)和梯度缓解校准器(GMC)，提出AdaDEM方法：对CADF带来的奖励进行归一化，并使用边际熵校准器(MEC)替代GMC。

Result: AdaDEM在噪声和动态环境中的各种不完美监督学习任务中表现优异，超越了经典EM的上界变体DEM*。

Conclusion: 通过解耦经典EM并设计自适应方法AdaDEM，有效解决了奖励崩溃和易类偏差问题，在多个任务中实现了更优性能。
...
### [103] [From Measurement to Expertise: Empathetic Expert Adapters for Context-Based Empathy in Conversational AI Agents](https://arxiv.org/abs/2511.03143)
*Erfan Shayegani,Jina Suh,Andy Wilson,Nagu Rangan,Javier Hernandez*

Main category: cs.HC

TL;DR: 提出了一种针对特定任务和上下文开发共情大语言模型的新框架，通过分析真实对话数据发现共情期望与体验之间的差距，并训练专门的共情适配器来缩小这一差距。


<details>
  <summary>Details</summary>
Motivation: 当前对话AI中的共情表达通常是泛化的，而非针对特定任务和上下文进行定制，导致用户期望的共情与实际体验之间存在显著差距。

Method: 分析真实多轮对话数据集，开发合成多轮对话生成流程，基于上下文定义共情模式，并训练针对特定上下文的共情专家适配器。

Result: 实验结果显示感知与期望共情之间的差距减少了72.66%，共情得分平均提高了2.43倍，共情专家适配器在保持共情模式方面优于系统提示。

Conclusion: 该框架成功缩小了对话AI中期望与体验共情之间的差距，上下文特定的共情适配器比泛化方法更有效，特别是在长对话中。
...
### [104] [Comprehensive Assessment of LiDAR Evaluation Metrics: A Comparative Study Using Simulated and Real Data](https://arxiv.org/abs/2511.02994)
*Syed Mostaquim Ali,Taufiq Rahman,Ghazal Farhani,Mohamed H. Zaki,Benoit Anctil,Dominique Charlebois*

Main category: cs.RO

TL;DR: 本文探索了用于比较真实世界和模拟LiDAR扫描的评估指标，发现密度感知Chamfer距离（DCD）在所有测试场景中表现最佳。通过构建虚拟测试环境并与真实LiDAR数据对比，验证了DCD在感知方法中最具相关性。


<details>
  <summary>Details</summary>
Motivation: 由于传统物理测试成本高且存在安全风险，需要开发虚拟测试环境（VTE）来测试自动驾驶系统。比较VTE生成的传感器输出与真实世界数据可以验证VTE的准确性。

Method: 采用综合实验方法评估适用于比较真实和模拟LiDAR扫描的指标，测试了不同噪声、密度、失真、传感器方向和通道设置下的敏感性和准确性。构建了基于真实LiDAR数据的虚拟测试环境，生成模拟扫描并与真实数据进行模型感知和几何相似性比较。

Result: 密度感知Chamfer距离（DCD）在所有测试场景中表现最佳。真实和模拟LiDAR扫描在语义分割输出上相似（mIoU为21%），平均DCD为0.63，表明几何属性存在轻微差异，但模型输出存在显著差异。

Conclusion: DCD是评估真实与模拟LiDAR扫描相似度的最有效指标，在感知方法中相关性最高，为自动驾驶系统的虚拟测试提供了可靠的评估工具。
...
TL;DR: OneOcc是一个专为腿式/人形机器人设计的全景语义场景补全框架，通过双投影融合、双网格体素化、轻量级解码器和步态位移补偿等技术，在360度全景环境下实现鲁棒的3D语义占据感知，在两个新发布的全景占据基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的语义场景补全系统主要针对轮式平台和前方传感器设计，而腿式/人形机器人需要处理由步态引起的身体抖动和360度连续性的挑战，因此需要专门的全景语义占据感知方案。

Method: 结合了四种关键技术：(1)双投影融合利用环形全景和等距柱状展开；(2)双网格体素化在笛卡尔和柱极坐标系中推理；(3)轻量级解码器采用分层AMoE-3D进行动态多尺度融合；(4)即插即用的步态位移补偿学习特征级运动校正。

Result: 在两个新发布的全景占据基准上达到最先进性能：在QuadOcc上超越强视觉基线和流行的LiDAR方法；在H3O上分别获得+3.83 mIoU（城市内）和+8.08（跨城市）的提升。

Conclusion: OneOcc的模块轻量级，能够为腿式/人形机器人提供可部署的全环绕感知能力，解决了步态抖动和360度连续性的关键挑战。
...
### [106] [Flying Robotics Art: ROS-based Drone Draws the Record-Breaking Mural](https://arxiv.org/abs/2511.03651)
*Andrei A. Korigodskii,Oleg D. Kalachev,Artem E. Vasiunik,Matvei V. Urvantsev,Georgii E. Bondar*

Main category: cs.RO

TL;DR: 本文介绍了一种创新的自主无人机系统，成功完成了世界上最大的无人机壁画创作。该系统结合红外运动捕捉和LiDAR技术，在恶劣户外条件下实现了高精度导航和绘画。
...
Method: 采用红外运动捕捉相机和LiDAR组合的导航系统，使用切向和法向不同调节的控制架构，开发了轨迹规划和路径优化算法，以及定制设计的防干扰喷漆机制。

Result: 实验结果表明系统在各种条件下都具有鲁棒性和精度，成功完成了大规模壁画创作。
...
TL;DR: 本文提出了一种基于动态加权图拓扑和人工蜂群算法的多轮越狱攻击方法ABC，显著提高了攻击效率并减少了查询次数。


<details>
  <summary>Details</summary>
Motivation: 现有红队评估方法在探索攻击空间中的成功对话轨迹方面不足，且忽视了攻击过程的高昂开销，需要更高效的攻击策略。

Method: 基于动态加权图拓扑理论模型，将多轮攻击抽象为路径规划问题，采用改进的人工蜂群算法进行协作搜索。

Result: 在三个开源和两个专有语言模型上验证，攻击成功率均超过90%，在GPT-3.5-Turbo上达到98%，平均仅需26次查询。

Conclusion: ABC算法在保持高攻击成功率的同时显著降低了红队评估开销，证明了其优越的效率和实用性。
...
TL;DR: 本文针对欧盟AI法案对通用AI模型输出标记的要求，提出了LLM水印技术的分类、评估和比较框架，发现目前尚无方法能同时满足所有四项标准。


<details>
  <summary>Details</summary>
Motivation: 欧盟AI法案要求通用AI模型提供者标记和检测其输出，但现有水印技术评估标准不明确，难以确定如何将'可靠、互操作、有效、鲁棒'四项标准转化为具体可测量的评估。

Method: 1) 提出按LLM生命周期阶段分类的水印方法分类法；2) 将欧盟要求映射到水印鲁棒性、可检测性和LLM质量评估；3) 提出互操作性评估的三个规范维度；4) 比较现有水印方法与欧盟标准。

Result: 比较显示目前尚无水印方法能同时满足所有四项欧盟标准。新兴实证测试表明，嵌入LLM底层架构的水印方法值得进一步研究。

Conclusion: 需要进一步研究直接嵌入LLM底层架构的水印技术，以满足欧盟AI法案对模型输出标记的严格要求。
...
TL;DR: 本研究使用BraTS撒哈拉以南非洲数据集（60例多模态MRI脑胶质瘤病例），发现nnU-Net模型在原始数据集上的表现优于离线增强数据集，获得全肿瘤分割Dice分数0.84，强调数据质量和适当增强方法对医学图像分割的重要性。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割是现代医学科学的重要成就，但针对代表性不足地区的分割模型需要更好的数据质量和增强策略。本研究旨在探索在有限数据情况下如何构建准确、可泛化的医学图像分割模型。

Method: 使用BraTS撒哈拉以南非洲数据集（60例多模态MRI脑胶质瘤病例），比较nnU-Net模型在原始数据集与离线增强数据集（360例）上的表现，利用nnU-Net的在线增强功能。

Result: 在原始60例数据集上训练的nnU-Net模型表现优于在离线增强360例数据集上训练的模型，全肿瘤分割Dice分数达到0.84。离线增强可能引入了不真实的解剖变异或强度分布，降低了泛化能力。

Conclusion: 数据质量和适当的增强方法对构建准确、可泛化的医学图像分割模型至关重要，特别是对于代表性不足的地区。原始数据配合强大的在线增强策略比人工离线增强更有效。
...
TL;DR: SegFormer3D-plus：一种针对非洲地区MRI扫描仪差异设计的放射组学引导的3D分割网络，通过强度协调、域感知采样和双路径编码器，在资源有限环境下实现稳健的胶质瘤分割。


<details>
  <summary>Details</summary>
Motivation: 解决撒哈拉以南非洲地区由于MRI基础设施有限和采集协议异质性导致的严重域偏移问题，提高胶质瘤分割在临床环境中的鲁棒性。

Method: 结合直方图匹配进行强度协调、放射组学特征提取与PCA降维的k-means域感知分层采样、具有频率感知特征提取和空间通道注意力的双路径编码器、以及用于边界细化的复合Dice-Cross-Entropy损失函数。

Result: 在BraTS 2023预训练并在BraTS-Africa数据上微调后，SegFormer3D-plus在异质性非洲临床扫描中展现出改进的肿瘤亚区分割和边界定位性能。

Conclusion: 放射组学引导的域适应方法在资源有限环境中具有重要价值，能够有效应对临床扫描数据的域变异性。
...
TL;DR: 本研究开发了一种混合深度学习管道，通过整合定量核形态测量和深度卷积图像特征，直接从H&E组织病理学图像进行卵巢癌亚型分类和基因突变推断。


<details>
  <summary>Details</summary>
Motivation: 卵巢癌因晚期诊断和亚型异质性导致死亡率高，现有诊断方法难以揭示基因组变异，限制了精准肿瘤学的发展。

Method: 使用来自TCGA和公共数据集的约45,000个图像块，开发了结合ResNet-50 CNN编码器和Vision Transformer的融合模型，捕捉局部形态纹理和全局组织环境。

Result: 模型实现了84.2%的亚型分类准确率（宏观AUC 0.87±0.03），基因突变推断表现中等至高准确度：TP53 AUC 0.82±0.02，BRCA1 AUC 0.76±0.04，ARID1A AUC 0.73±0.05。

Conclusion: 可量化的组织学表型编码了可测量的基因组信号，为卵巢癌分诊和诊断中的经济高效精准组织病理学开辟了新途径。
