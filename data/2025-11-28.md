...
TL;DR: 论文主张重新定义LLM效率，从追求大规模复杂技术转向注重资源受限环境下的简单高效，提出了包含采用成本、可持续性和公平性的新研究议程。


<details>
  <summary>Details</summary>
Motivation: 当前主流LLM效率方法（MoE、推测解码、复杂RAG）只适用于大型科技公司，导致大多数组织无法受益，加剧了不平等和碳浪费。

Method: 提出新的研究议程：无需重新训练就能改造预训练模型架构、轻量级微调保持对齐、经济化长推理链、轻量知识管理、采用开销感知效率基准。

Result: 通过重新定义效率标准，使LLM部署民主化，减少不平等和碳浪费。

Conclusion: LLM效率的下一个前沿不是更大规模的复杂性，而是在有限资源和专业知识下的稳健简单性，这需要重新定义效率概念。
...
TL;DR: 提出Harmonic Token Projection (HTP)框架，无需训练即可生成可逆的确定性文本嵌入，通过Unicode编码将符号映射为谐波轨迹向量。


<details>
  <summary>Details</summary>
Motivation: 传统神经嵌入依赖统计共现和优化，缺乏可解释性和确定性。HTP旨在建立符号与向量空间的解析映射，提供透明高效的语义表示。

Method: 基于Unicode整数值解析生成谐波轨迹，建立符号到连续空间的单射映射，通过几何对齐评估语义相似性。

Result: 在STS-B基准测试中英语达到Spearman相关性0.68，在10种语言中表现稳定，计算成本极低，每句对延迟亚毫秒级。
...
TL;DR: 提出基于双嵌入质心的分类框架，在ITSM工单分类中实现高性能、可解释性和高效训练更新


<details>
  <summary>Details</summary>
Motivation: IT服务管理系统中需要将支持工单分类到树状层次化分类法中，传统方法缺乏可解释性和效率

Method: 双嵌入质心分类框架，为每个类别维护语义和词汇质心表示，通过互逆排名融合进行推理

Result: 在8968个ITSM工单的123个类别上，性能与SVM相当（层次F1: 0.731 vs 0.727），训练速度快5.9倍，增量更新快152倍

Conclusion: 该方法适合生产环境ITSM系统，在保持可解释性的同时显著提升操作效率
...
TL;DR: PIRA是一种新的奖励模型训练范式，通过重构问答对为偏好指令、聚合多样化偏好任务奖励、以及使用dropout平均来稳定奖励值，解决了传统奖励模型数据效率低和奖励过优化的问题。
...
Method: PIRA采用三种策略：1）将问答对重构为基于偏好的指令以明确任务规范；2）聚合多样化偏好任务的奖励以减少偏见和提高鲁棒性；3）在不同dropout率下平均价值头输出以稳定奖励。

Result: 大量实验证明了PIRA的有效性。

Conclusion: PIRA训练范式成功解决了奖励模型面临的数据效率低和奖励过优化两大挑战。
...
TL;DR: 这篇论文研究了如何通过重组法律文档结构、定义修辞角色和模拟法庭推理步骤来提升大语言模型在法律任务中的表现，在零样本设置下实现了1.5%-4.36%的F1分数提升。
...
Conclusion: 通过结构化信息处理和模拟法律推理过程，可以有效弥补大语言模型在法律领域的知识差距，提升其在专业任务中的表现。
...
TL;DR: MindSET是一个新的心理健康分析基准数据集，包含超过1300万条Reddit帖子，涵盖7种心理健康状况，通过严格预处理确保数据质量，在诊断检测任务中表现优于现有基准。


<details>
  <summary>Details</summary>
Motivation: 现有心理健康分析基准数据集存在数据过时、清洁不足、内容多样性不足等问题，需要构建更全面、高质量的数据集来支持社交媒体心理健康研究。

Method: 从Reddit收集自报告诊断数据，应用严格预处理（语言过滤、NSFW内容移除、去重），使用LIWC进行语言分析，并通过微调语言模型和词袋特征进行二元分类实验。

Result: MindSET数据集规模是之前基准的两倍以上，在自闭症检测等任务中F1分数提升高达18个百分点，模型性能显著优于现有基准。
...
TL;DR: FlexCode是一个流行度感知的生成推荐框架，通过自适应分配协作过滤和语义编码本的token预算，解决传统方法对热门和长尾项目统一编码的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有生成推荐方法使用单一编码本编码所有项目，忽视了热门项目（富含协作信号）和长尾项目（依赖语义理解）之间的不平衡，限制了表示效率和泛化能力。

Method: 引入FlexCode框架，自适应分配固定token预算给协作过滤编码本和语义编码本；使用轻量级MoE动态平衡CF精度和语义泛化；通过对齐和平滑目标保持流行度谱系的一致性。
...
TL;DR: 本文介绍了Ar-SParC，这是首个阿拉伯语跨领域、上下文相关的文本到SQL数据集，包含3,450个相关问题序列和10,225个问题。作者使用GPT模型进行了40个实验，并开发了GAT校正器方法，在零样本和上下文学习设置下都显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 目前大多数文本到SQL的研究和数据集都集中在英语和中文上，阿拉伯语领域尚未有相关工作。本文旨在填补这一空白，为阿拉伯语用户提供自然语言与数据库交互的能力。

Method: 构建了Ar-SParC数据集，包含3,450个问题序列和10,225个问题。使用GPT-3.5-turbo和GPT-4.5-turbo模型，应用10种不同的提示工程技术（4种问题表示方法和6种上下文学习技术）。开发了GAT校正器方法提升性能。

Result: GAT校正器在所有40个实验中均提升了性能：零样本设置下执行准确率（EX）和交互准确率（IX）平均提升1.9%；上下文学习设置下EX平均提升1.72%，IX平均提升0.92%。

Conclusion: Ar-SParC是首个阿拉伯语跨领域上下文相关文本到SQL数据集，GAT校正器方法有效提升了阿拉伯语文本到SQL任务的性能，为阿拉伯语自然语言与数据库交互提供了重要基础。
...
TL;DR: GPT-4在肿瘤学决策支持中存在推理错误，这些错误与认知偏见相关，可能导致临床指南不一致的潜在有害建议，特别是在晚期疾病管理中。
...
Method: 开发了三级分类法，从GPT-4对真实肿瘤学笔记的思维链响应中标注推理错误，并在前列腺癌咨询笔记上进行验证，模拟提取、分析和临床建议任务。

Result: 23%的解释中出现推理错误，确认偏见和锚定偏见最常见。推理失败与指南不一致和潜在有害建议相关，特别是在晚期疾病管理中。自动评估器能检测错误存在但无法可靠分类子类型。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM部署采用统一的提示策略，导致token效率低下，而输出token成本比输入高4-8倍。

Method: 比较两种路由方法：使用预计算嵌入的简单MLP和微调的RoBERTa变换器，在MMLU问题上进行测试。

Result: MLP路由在测试数据上达到90.5%的路由准确率，略优于RoBERTa，且参数少1.25亿。在三个主要LLM提供商上验证，token减少32.6%-33.9%。

Conclusion: DTS方法能有效降低LLM部署成本，路由决策具有提供商无关性，为生产系统提供了实用的成本优化方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决二维材料研究信息分散的问题，便于研究人员更高效地获取材料特性和制备方法等信息。

Method: 从已发表的研究论文中提取和整理二维材料的关键信息，包括性质和制备方法。

Result: 建立了系统化的二维材料信息数据库，整合了分散在不同论文中的有价值数据。

Conclusion: 该方法有助于促进二维材料研究的发展，为能源存储和转换领域的应用提供更好的信息支持。
...
TL;DR: 提出多前缀记忆框架，通过评估序列可被不同前缀触发的多样性来定义记忆化，有效识别LLM中的训练数据泄露风险。


<details>
  <summary>Details</summary>
Motivation: 现有记忆化定义在评估对齐模型时存在不足，需要更全面捕捉记忆化现象，特别是隐私和版权风险。

Method: 引入多前缀记忆框架，定义序列被记忆的标准是外部对抗搜索能找到足够多不同前缀来触发该序列。

Result: 实验证明该框架能可靠区分记忆化和非记忆化数据，为LLM数据泄露审计提供实用工具。

Conclusion: 多前缀记忆框架通过量化记忆鲁棒性，提供了更全面可靠的记忆化评估方法。
...
Method: 提出SAGE框架，将特征解释重构为主动过程：系统制定多个特征解释，设计针对性实验进行测试，基于经验激活反馈迭代优化解释。

Result: 在多种语言模型的SAE特征上实验表明，SAGE产生的解释在生成和预测准确性方面显著优于最先进的基线方法。

Conclusion: SAGE通过主动、实验驱动的特征解释方法，有效提高了稀疏自编码器特征的可解释性，为理解LLM内部机制提供了更可靠的工具。
...
TL;DR: 该研究提出了DSPy+HELM框架，通过结构化提示方法更准确地评估语言模型性能，发现传统HELM基准测试会低估模型能力并导致排名错误。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试框架如HELM使用固定提示，无法适应不同语言模型，可能导致性能低估和不准确的部署决策。

Method: 开发DSPy+HELM集成框架，使用四种结构化提示方法，在四个前沿语言模型和七个基准测试上进行评估。

Result: 结构化提示使性能平均提升4%，减少评估波动，在3/7基准上翻转了模型排名，推理提示降低了模型对提示设计的敏感性。

Conclusion: 可扩展的性能上限估计能够提供更有决策价值的基准测试，结构化提示对于准确评估语言模型至关重要。
...
TL;DR: 提出一种新的Length-MAX分词器，通过最小化平均字符数来减少训练和推理时的token数量，相比BPE在多个词汇量下减少13-18%的token，提升训练效率和推理速度。
...
Method: 将长度加权目标最大化建模为图划分问题，开发贪心近似算法来构建词汇表，称为Length-MAX分词器。

Result: 在FineWeb和多个领域上，相比BPE减少14-18%的token；GPT-2模型训练步数减少17-18%，推理延迟降低13%左右；下游任务性能提升，LAMBADA困惑度降低11.7%，HellaSwag准确率提高4.3%。
...
TL;DR: Evo-Memory是一个用于评估LLM智能体自演化记忆能力的流式基准框架，通过序列化任务流测试记忆的搜索、适应和演化能力，并提出了ReMem方法来整合推理、任务行动和记忆更新。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注静态对话场景，忽视了LLM智能体在连续任务流中积累和重用经验的动态能力，而现实世界环境需要LLM能够持续检索、整合和更新记忆。

Method: 构建Evo-Memory基准框架，将数据集结构化为序列任务流，统一实现十余种代表性记忆模块，并提出ExpRAG基准方法和ReMem行动-思考-记忆精炼流程。

Result: 在10个多样化多轮目标导向和单轮推理问答数据集上进行评估，验证了框架对记忆演化能力的测试有效性。

Conclusion: Evo-Memory填补了LLM智能体记忆演化评估的空白，为测试时记忆持续演化提供了系统化基准和方法论。
...
TL;DR: 本文研究跨语言论证挖掘方法，针对低资源语言（波斯语）构建了三种训练场景：零样本迁移、LLM增强和跨语言混合模型，结果表明跨语言混合模型在波斯语上取得最佳性能（F1 74.8%）。
...
Result: 零样本迁移模型在英语和波斯语测试集上的F1分别为50.2%和50.7%；LLM增强模型提升至59.2%（英语）和69.3%（波斯语）；跨语言混合模型在波斯语测试集上达到74.8% F1。

Conclusion: 轻量级的跨语言混合方法比资源密集的增强管道表现更好，为低资源语言的论证挖掘任务提供了实用解决方案。
...
TL;DR: 提出了一种结合角色交叉最小对、时间涌现分析和跨模型比较的方法来研究LLM如何实现语义角色，发现LLM形成了紧凑、因果隔离的机制来处理抽象语义结构
...
Method: 整合角色交叉最小对、时间涌现分析和跨模型比较的方法

Result: 发现高度集中的电路（89-94%归因在28个节点内）、渐进结构细化而非相变、中等跨尺度守恒（24-59%组件重叠）和高光谱相似性

Conclusion: LLM形成了紧凑、因果隔离的机制来处理抽象语义结构，这些机制在不同规模和架构间表现出部分可迁移性
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：LLM无法直接访问私有的动态知识图谱；RAG系统会序列化图结构且难以处理多轮上下文；传统KGQA系统通常只支持单轮问答且延迟高。

Method: 采用模块化多智能体架构，通过专门的LLM智能体协作完成上下文解释、对话跟踪、实体关系链接和高效查询规划，将自然语言问题转换为可执行的SPARQL查询。
...
TL;DR: LLMs在定义类问题表现最佳，但在举例类问题表现最差。研究发现LLMs倾向于对流行知识进行更多释义，而对专业术语知识释义较少。


<details>
  <summary>Details</summary>
Motivation: LLMs擅长回答定义类问题，但在提供其他类型答案（如例子、释义等）时表现不佳，需要评估这种性能差异及其与预训练数据的关系。

Method: 使用TrackList分析流水线评估LLMs对多样化语言查询的回答质量，引入RefoMed-EN数据集（6170个医学术语及其定义、命名、举例、解释或释义），通过句法和语义相似度指标、统计相关性和嵌入向量评估LLM输出质量。

Result: LLMs在定义类问题任务表现最高，在举例类问题表现最低。对于定义类问题，LLMs倾向于对流行和频繁知识进行更多释义，而对尾部和技术知识（特别是在专业文本中）释义较少。

Conclusion: LLMs在处理不同类型语言查询时存在显著性能差异，定义类问题表现最佳而举例类问题表现最差，且对流行知识的处理优于技术性专业知识。
...
TL;DR: 研究表明，上下文学习(ICL)无法覆盖预训练标签语义，而是主要调整输入如何映射到预训练期间学习的稳定语义方向，揭示了少样本提示的基本限制。


<details>
  <summary>Details</summary>
Motivation: 探究上下文学习是否能覆盖预训练标签语义，还是仅仅优化现有的语义骨干。

Method: 将LLMs视为提示诱导分类器，对比自然演示和反转演示下的行为，分解ICL行为为三个对齐指标，并引入语义覆盖率。

Result: 在8个分类任务和8个开源LLM上发现：自然演示下ICL提高准确性但保持强先验对齐；反转演示下模型无法学习连贯的反语义分类器，语义覆盖率保持为零。

Conclusion: ICL主要调整输入如何投影到预训练期间学习的稳定语义方向，而非灵活重映射标签含义，在这些规模上覆盖标签语义需要超越ICL的干预措施。
...
<details>
  <summary>Details</summary>
Motivation: 讽刺检测在NLP中仍然具有挑战性，PLMs和LLMs在处理复杂的讽刺文本时，由于语言多样性和文化差异，以及缺乏必要的背景知识，表现不可靠。

Method: 基于PMP方法，引入检索感知方法，通过两种互补方式提供上下文：非参数知识检索（基于网络检索）和自知识检索（激发模型内部知识）。

Result: 在三个数据集上评估：Twitter Indonesia Sarcastic上非参数检索带来9.87%的宏F1提升；SemEval和MUStARD上自知识检索分别提升3.29%和4.08%。

Conclusion: 上下文信息对提升LLM在讽刺检测任务中的性能至关重要，特别是涉及文化特定俚语、引用或LLM未知术语时。未来工作将优化相关上下文信息的检索。
...
TL;DR: 本文探索在低资源语言（如缅甸语）分类任务中使用Kolmogorov-Arnold Networks (KANs)作为分类头替代传统的多层感知机(MLPs)，实验表明KANs在性能和效率上具有竞争力甚至更优。


<details>
  <summary>Details</summary>
Motivation: 在低资源语言分类任务中，通常只微调最终分类层而保持预训练编码器权重冻结。传统MLPs的固定非线性限制了表达能力并增加了计算成本，需要寻找更优的替代方案。

Method: 评估了三种KAN变体：基于傅里叶的FourierKAN、基于样条的EfficientKAN和基于网格的FasterKAN，使用多种嵌入方法包括TF-IDF、fastText和多语言变换器(mBERT、Distil-mBERT)。

Result: KAN-based分类头与MLPs竞争或更优：EfficientKAN与fastText组合获得最高F1分数(0.928)，FasterKAN在速度和精度间提供最佳平衡，在变换器嵌入上EfficientKAN与mBERT匹配或略优于MLPs(0.917 F1)。
...
TL;DR: 评估28个跨架构大语言模型在字符级约束满足任务上的表现，发现架构差异比参数规模对性能影响更大，模型在常见但拼写不规则的单词上存在系统性失败。


<details>
  <summary>Details</summary>
Motivation: 系统评估不同架构大语言模型在硬性拼写约束文本生成任务上的表现，探索架构差异与参数规模对约束满足能力的影响。

Method: 使用58个单词谜题测试28种模型配置（涵盖Qwen3、Claude Haiku-4.5、GPT-5-mini三个模型家族），分析计算预算敏感性和人类难度评级。

Result: 架构差异导致2.0-2.2倍性能差距，远大于参数规模带来的83%增益；模型在拼写不规则的常见单词上失败率高达89-96%，而人类成功率86-95%。

Conclusion: 约束满足需要专门的架构特征或训练目标，仅靠参数扩展或计算预算增加不足以解决模型对分布合理性的过度依赖问题。
...
TL;DR: 本研究首次针对低资源缅甸语ASR错误校正，提出结合IPA和语音对齐特征的序列到序列Transformer模型，显著降低了词错误率并提高了字符级准确率。


<details>
  <summary>Details</summary>
Motivation: 缅甸语作为低资源语言，缺乏专门的ASR错误校正研究。现有ASR系统在缅甸语上表现不佳，需要有效的后处理方法来提升识别准确率。

Method: 使用序列到序列Transformer模型，整合IPA（国际音标）和语音对齐特征，设计了五种ASR骨干网络进行错误校正。

Result: 提出的AEC模型将平均WER从51.56降至39.82（增强前）和43.59（增强后），chrF++分数从0.5864提升至0.627，在所有ASR骨干网络上均取得一致改进。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用领域表现出色，但在专业领域如抵押贷款金融中需要增强领域专业知识，同时保持指令跟随能力。单一模型在处理结构化任务和对话任务时存在性能权衡问题。

Method: 采用双轨专业化框架，从单一基础模型(LLaMA-3.1-8B)开发两个专家模型：对话问答模型和结构化任务模型。应用指令残差技术恢复领域适应后的指令跟随能力，并使用智能任务路由机制。

Result: 在领域特定基准测试中，最终模型(MLM v2)显著优于基础模型：摘要得分4.58(vs 3.99)、问答得分4.09(vs 4.0)、分类得分2.6(vs 1.2)；BERTScore在摘要、问答和分类任务上分别为0.77、0.68、0.75，均优于基线方法。
...
TL;DR: 本研究探讨了在小型人类调查数据上微调LLM是否能改善模拟人类行为的准确性。虽然微调显著提升了多样性、对齐度和信念-行动一致性，但即使最佳微调模型也无法复现原始研究的回归系数，表明LLM生成数据仍不适合替代人类参与者进行正式推断分析。


<details>
  <summary>Details</summary>
Motivation: 解决关于LLM能否替代人类参与者的争议，特别关注通过小样本人类数据微调能否缓解LLM与真实人类行为之间的系统性偏差问题。

Method: 使用信息披露行为实验，比较人类和LLM生成响应在分布差异、子群体对齐、信念-行动一致性和回归系数恢复等多个维度的表现。

Result: 基于小样本人类数据的微调显著改善了异质性、对齐度和信念-行动一致性，但所有微调模型都无法复现原始研究的回归系数。

Conclusion: 虽然微调能改善LLM模拟人类行为的某些方面，但LLM生成数据仍不适合替代人类参与者进行正式的推断分析。
...
TL;DR: 开发了首个伊桑语开放对话语音数据集，包含自然语音而非朗读语音，捕捉真实语言现象如口语表达、自发韵律、不流畅和泰语代码转换。


<details>
  <summary>Details</summary>
Motivation: 伊桑语是泰国最广泛使用的方言，但缺乏标准化正字法，现有语音语料库多为朗读语音，需要自然对话数据集支持包容性AI发展和低资源语言研究。

Method: 建立实用的转录协议，在表征准确性和计算处理需求之间取得平衡，处理伊桑语缺乏标准化正字法的挑战。

Result: 创建了首个伊桑语开放对话语音数据集，包含自然语音，捕捉真实语言现象，并建立了转录指南。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多任务上达到了最先进的性能，但其巨大规模导致高计算和环境成本，限制了可访问性。当前PEFT方法的评估仍然有限且难以复现。

Method: 开发了PEFT-Bench基准测试，在27个NLP数据集和6种PEFT方法上进行评估，并引入PSCP指标综合考虑可训练参数、推理速度和训练内存使用。

Result: 构建了一个统一的评估框架，能够系统比较不同PEFT方法的性能表现，并通过PSCP指标提供更全面的评估维度。

Conclusion: PEFT-Bench填补了当前PEFT方法评估的空白，为参数高效微调方法的系统比较提供了标准化基准。
...
TL;DR: 本文首次系统研究神经网络语言模型训练过程中生成的文本是否符合Martin定律（词频与多义性关系），发现该规律呈现非单调发展轨迹，在特定训练阶段达到峰值后下降。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索神经网络语言模型在训练过程中是否能够自发学习到人类语言中的统计规律（Martin定律），以及这种学习过程的发展轨迹。

Method: 使用DBSCAN聚类方法对上下文嵌入进行词义操作化，分析四个不同规模的Pythia模型（70M-1B参数）在30个训练检查点上的表现。

Result: 结果显示Martin定律在检查点100左右出现，在104达到峰值相关性（r>0.6），然后在105下降。小模型在后期出现灾难性语义崩溃，大模型则呈现优雅退化。频率-特异性权衡在所有模型中保持稳定。

Conclusion: 语言模型对语言规律的遵循不是单调递增的，而是遵循平衡轨迹，存在最优语义窗口。这为评估神经网络语言模型中的涌现语言结构建立了新方法。
...
TL;DR: 通过微调训练，7B参数模型从0.4%准确率提升到85%准确率，能够可靠检测单令牌注入的"想法"，满足准确性、零误报和内部性标准。


<details>
  <summary>Details</summary>
Motivation: Lindsey研究发现语言模型能检测注入的激活模式但不可靠（约20%成功率），本研究探讨是否可以通过直接训练而非等待自然涌现来提升这种内省能力。

Method: 通过对瞬时单令牌注入进行微调，训练7B参数模型检测和识别注入的"想法"，并测试其泛化能力。

Result: 模型准确率从0.4%提升到85%，误报率为0%，能够检测单令牌位置的瞬时"想法"并在后续生成步骤中报告语义内容，对未见概念向量有7.5个百分点的泛化差距。

Conclusion: 研究表明至少一种内省行为可以通过直接训练诱导，为构建AI透明度提供了可行路径，但尚未证明存在Lindsey意义上的元认知表征。
...
TL;DR: 该论文研究了捷克语和斯洛伐克语声明中细粒度证据提取任务，创建了新的数据集，并评估了大型语言模型在证据提取方面的表现。


<details>
  <summary>Details</summary>
Motivation: 在线新闻文章下的用户评论中经常传播错误信息，需要有效的方法来检测事实错误信息。为了支持或反驳这些声明，需要识别相关文档并定位确切的文本片段。

Method: 创建了由付费标注者标注的双向细粒度证据数据集，并评估了多个大型语言模型在该数据集上的表现，分析其与人类标注的一致性。

Result: LLMs经常无法从源文本中逐字复制证据，导致无效输出。llama3.1:8b模型尽管规模较小但正确输出比例高，而gpt-oss-120b模型参数虽多但表现不佳。qwen3:14b、deepseek-r1:32b和gpt-oss:20b在模型大小与人类标注对齐之间达到了有效平衡。

Conclusion: 大型语言模型在细粒度证据提取任务中表现参差不齐，模型大小与性能并非完全正相关，某些中等规模模型在准确性和效率之间取得了更好的平衡。
...
Method: 采用双状态推理框架：上下文状态通过精炼大型模式和选择相关结构构建紧凑的语义环境；生成状态将SQL合成形式化为反馈引导的状态转换，实现自我纠正和用户意图对齐。
...
TL;DR: Odin是一个新的文本属性图模型架构，通过定向双模块机制在特定Transformer层注入图结构，避免了传统GNN的过平滑问题，同时比纯Transformer更好地利用图拓扑。
...
Method: 提出Odin架构，在选定Transformer深度通过定向双模块机制注入图结构，多跳结构在特定层集成，产生与语义层次对齐的低、中、高级结构抽象。聚合在全局[CLS]表示上操作，从根本上避免过平滑。

Result: 在多个文本丰富的图基准测试中，Odin实现了最先进的准确性，而Light Odin以显著降低的计算成本提供了有竞争力的性能。

Conclusion: Odin和Light Odin形成了一个统一的、无跳数的框架，用于原则性的结构和文本集成，其表达能力严格包含纯Transformer和GNN。
...
TL;DR: 本文系统评估了6种模型合并方法在LLMs上的表现，发现最简单的Task Arithmetic方法是唯一可靠提升性能的方法，其他方法通常导致性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 模型合并是一种无需额外训练即可复用模型的有效方法，但现有研究主要关注小模型和分类器，不清楚这些优势是否能推广到LLMs。

Method: 对6种最先进的合并方法（包括子空间方法）进行大规模系统评估，涵盖4个开源LLMs、每个基础模型12个微调检查点、16个标准LLM基准测试。

Result: Task Arithmetic是唯一可靠提升LLMs性能的方法，其他干扰感知和子空间合并方法通常导致显著性能下降。
...
TL;DR: 提出一种双向可读性评估机制，通过捕捉上下文信息识别文本中语义丰富区域，预测句子级可读性标签，再辅助预测文档整体可读性，并引入成对排序算法建模可读性等级间的序数关系。
...
Method: 双向可读性评估机制：1）识别语义丰富区域预测句子级可读性；2）利用句子标签辅助文档级预测；3）引入成对排序算法通过标签减法建模序数关系。

Result: 在中文和英文数据集上的实验结果表明，该模型取得了有竞争力的性能，优于其他基线模型。

Conclusion: 提出的双向机制和序数关系建模方法有效提升了可读性评估性能。
...
TL;DR: 本文研究了语音翻译模型中性别分配机制，发现模型不仅复制训练数据中的性别模式，还学习更广泛的男性主导模式，并揭示了一个新的机制：使用第一人称代词将性别化术语与说话者关联


<details>
  <summary>Details</summary>
Motivation: 语音翻译中，当从有概念性性别的语言翻译到有语法性别的语言时，说话者的声音特征可能影响性别分配，这可能导致错误性别识别。但目前对语音翻译模型如何做出这些决策的理解不足

Method: 研究三个语言对（英-西/法/意），分析训练数据模式、内部语言模型偏见和声学信息的交互作用，使用对比性特征归因分析频谱图

Result: 模型能够基于声学输入覆盖内部语言模型的男性偏见偏好，性别准确率更高的模型使用第一人称代词将性别化术语与说话者关联，访问分布在频率谱而非集中在音高的性别信息

Conclusion: 语音翻译模型学习更广泛的男性主导模式而非简单复制训练数据，但能通过声学输入调整性别分配，揭示了一个新的基于第一人称代词和分布式频率信息的性别识别机制
...
TL;DR: 提出了孟加拉手语翻译数据集IsharaKhobor及其两个子集，用于解决孟加拉手语资源匮乏问题，并提供了基于地标和RQE嵌入的基准测试。


<details>
  <summary>Details</summary>
Motivation: 孟加拉手语翻译研究因资源匮乏而受限，需要标准句子级数据集来开发面向孟加拉语聋哑人群的AI辅助工具。
...
Conclusion: 该数据集为孟加拉手语翻译研究提供了重要资源，通过基准测试展示了数据集的可用性，并提出了词汇限制和规范化的改进方向。
...
TL;DR: RoParQ基准测试评估LLM在回答同义问题时的一致性，提出XParaCon指标量化模型鲁棒性，并通过推理驱动的SFT策略显著提升模型对语义不变性的理解。


<details>
  <summary>Details</summary>
Motivation: LLM在回答同义问题时表现出不一致行为，表明其依赖表层模式而非真正的语义理解，需要专门评估和改进这种跨同义一致性。

Method: 从标准数据集生成同义问题构建RoParQ基准，提出XParaCon指标测量准确率标准差，并实施推理驱动的同义感知SFT策略进行模型对齐。

Result: 经过针对性对齐的轻量级模型在一致性方面达到了与更大预训练模型相当的水平，显著提升了模型鲁棒性。

Conclusion: 该方法有效缓解了表层记忆问题，培养了更稳健可靠的LLM，证明了针对性语义对齐策略的有效性。
...
Conclusion: 该方法能够有效识别大语言模型中编码特定技能的神经元，为理解模型内部机制提供了新的工具。
...
TL;DR: 本研究探索了在LLM预训练中使用多种元数据类型的加速效果，发现细粒度文档质量指标等元数据能有效加速训练，并提出了元数据追加和可学习元标记等方法来提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 先前研究仅关注URL这一种元数据信号，本研究旨在探索其他类型的元数据是否能带来更大的预训练加速效益。

Method: 研究了多种元数据类型，引入元数据追加作为辅助任务来预测适当元数据，并使用带掩码损失的可学习元标记来诱导质量感知的潜在结构。

Result: 发现细粒度信息编码的元数据（如文档质量指标）能有效加速预训练，元数据追加和可学习元标记都能部分恢复加速效果。

Conclusion: 研究结果为整合元数据以提升LLM预训练效率和效果提供了实用指南，揭示了元数据如何通过塑造学习过程来改善训练。
...
TL;DR: 本研究探讨捷克母语者对AI生成诗歌与人类创作诗歌的识别能力和审美评价，发现参与者难以区分两者，且存在作者偏见影响审美判断。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在创意文本生成方面能力日益增强，但现有研究主要集中在英语诗歌上。本研究旨在考察在形态复杂、训练数据较少的捷克语中，AI生成诗歌的感知情况。

Method: 让捷克母语者识别AI和人类创作的诗歌作者，并进行审美评价。使用逻辑回归模型分析识别准确性与审美评价的关系。

Result: 参与者识别准确率仅为45.8%，处于随机水平。审美评价显示强烈的作者偏见：当认为诗歌是AI生成时评分更低，尽管AI诗歌实际上获得同等或更高评分。诗歌熟悉度或文学背景不影响识别准确性。

Conclusion: AI能够令人信服地生成捷克语诗歌，即使在这种形态复杂的斯拉夫语言中。读者的作者信念与诗歌审美评价相互关联。
...
TL;DR: Matrix是一个去中心化的多智能体合成数据生成框架，通过分布式队列传递序列化消息来替代中央协调器，显著提高了数据生成吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体合成框架通常依赖中央协调器，存在可扩展性瓶颈，或者针对特定领域硬编码，缺乏灵活性。

Method: 采用去中心化设计，将控制流和数据流表示为通过分布式队列传递的序列化消息，每个任务由轻量级智能体独立处理，计算密集型操作由分布式服务处理。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然功能强大，但在解决人类终极考试等深度复杂问题时仍面临概念挑战和计算成本高昂的问题

Method: 使用强化学习训练小型编排器，包含结果、效率和用户偏好感知的奖励机制

Result: Orchestrator模型在HLE上获得37.1%的分数，超越GPT-5且效率提高2.5倍；在tau2-Bench和FRAMES上大幅超越GPT-5，成本仅为30%
...
<details>
  <summary>Details</summary>
Motivation: 现有研究对训练数据难度（简单或困难）对LLM性能的影响存在争议，需要更客观、大规模的分析来理解LLMs的跨难度泛化能力

Method: 使用数千个不同LLM的输出和项目反应理论(IRT)对六个数据集中的示例进行难度排序，完全基于LLMs能力而非人类主观判断来确定难度等级

Result: 跨难度泛化通常有限，仅使用简单或困难数据训练无法在全部难度范围内获得一致的性能提升

Conclusion: LLMs的训练和评估数据需要包含广泛的难度范围，在难度选择上走捷径存在风险
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的广泛部署，隐私泄露风险日益突出。现有研究主要关注单模态系统的隐私攻击，而多模态模型对隐私攻击的脆弱性研究不足。受生物神经网络启发的表示能否增强多模态模型的隐私保护能力尚待探索。

Method: 提出系统性的神经科学启发拓扑正则化框架(tau)，在三种视觉语言模型(BLIP、PaliGemma 2、ViT-GPT2)上应用，使用COCO、CC3M和NoCaps三个基准数据集进行实验验证。

Result: 在BLIP模型上，神经增强版本使成员推理攻击成功率平均ROC-AUC下降24%，同时保持相似的模型性能(MPNet和ROUGE-2指标)。其他模型和数据集上的实验进一步验证了结果的稳定性。

Conclusion: 神经增强的多模态视觉语言模型对隐私攻击具有更强的抵抗力，且不会显著影响模型效用，为理解多模态模型隐私风险提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 世界模型是智能体AI、具身AI和游戏领域的核心模拟器，能够生成长时间、物理真实且交互式的高质量视频。扩展这些模型可能解锁视觉感知、理解和推理的新兴能力，超越当前以LLM为中心的视觉基础模型范式

Method: 采用半自回归（块扩散）解码范式，结合扩散和自回归方法的优势，通过块内应用扩散生成视频令牌，同时以前面块为条件，实现更连贯稳定的视频序列。重新引入LLM风格的KV缓存管理，克服标准视频扩散的限制

Result: Inferix能够实现高效、可变长度和高质量的视频生成，支持实时交互和真实模拟以准确建模世界动态

Conclusion: Inferix作为专门针对世界模拟设计的推理引擎，与高并发系统（如vLLM或SGLang）和经典视频扩散模型（如xDiTs）有显著区别，通过社区合作有望推动世界模型的探索和发展
...
TL;DR: 提出LTED-Ada算法，在移动边缘计算环境中自适应选择本地跟踪或边缘检测，平衡视频物体识别的准确性和延迟要求。


<details>
  <summary>Details</summary>
Motivation: 解决资源受限设备（如交通摄像头）上视频物体识别面临的挑战，利用边缘计算卸载计算密集型检测任务，同时本地运行轻量级跟踪算法。

Method: 基于深度强化学习，在单设备场景提出LTED-Ada算法自适应选择策略；在多设备场景使用联邦学习进行协作策略训练。

Result: 通过Raspberry Pi 4B设备和PC边缘服务器的硬件在环实验，验证了LTED-Ada算法的优越性能。

Conclusion: LTED-Ada算法能有效平衡视频物体识别的准确性和延迟，在移动边缘计算环境中具有实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言动作(VLA)模型在自动驾驶中统一了感知、推理和轨迹生成，但由于深层Transformer堆栈导致推理延迟显著。

Method: 提出训练免费的DeeAD框架，当预测轨迹与轻量级规划先验（如导航或低精度规划）在可容忍偏差（<2m）内对齐时终止推理，并引入多跳控制器基于分数变化率自适应跳过冗余层。

Result: 在Bench2Drive基准测试中，实现了高达28%的Transformer层稀疏性和29%的延迟减少，同时保持了规划质量和安全性。

Conclusion: DeeAD可以无缝集成到现有VLA模型（如ORION）中，无需重新训练，有效加速VLA规划推理。
...
TL;DR: 提出Foundation Model Distillation (FMD)新范式，将大型自监督学习模型压缩为紧凑高效的代理模型，保留通用表征能力。实现Foundry系统用于3D点云，通过训练学生学习压缩的SuperTokens来重构教师模型的token级表示。


<details>
  <summary>Details</summary>
Motivation: 基础模型虽然功能强大，但巨大的规模和计算成本使其难以部署在边缘设备上。现有压缩技术会牺牲模型的通用性，需要一种能保持基础模型通用表征能力的压缩方法。

Method: 提出FMD框架，训练学生模型学习压缩的SuperTokens来重构教师模型的token级表示，捕获其潜在空间的紧凑基础。Foundry是该框架在3D点云上的首个实现。

Result: 单个蒸馏模型在分类、部件分割和少样本场景等多样化下游任务中保持强迁移性，接近完整基础模型性能，同时显著减少token数量和FLOPs。

Conclusion: FMD能够创建紧凑高效的代理模型，保持基础模型的通用表征能力，使其更适合在资源受限的硬件上部署。
...
<details>
  <summary>Details</summary>
Motivation: 解决生成式修复图像中篡改区域的精确定位问题，利用DINOv2的强大表征能力来检测语义层面的篡改。

Method: 在预训练的DINOv2模型上添加线性分类头，使用滑动窗口处理大尺寸图像，通过后处理优化二值化篡改掩码。

Result: 在多个修复数据集上优于现有方法，平均IoU提高12%，对常见后处理操作具有鲁棒性。

Conclusion: DINOv2在篡改定位任务中表现出强大的表征能力，DinoLizer方法有效且优于后续版本DINOv3。
...
Method: 创建包含598个工具设计任务的CANVAS基准，任务来自3.3K移动UI设计样本，涵盖30个功能类别，支持通过工具调用逐步更新设计。

Result: 领先模型展现出更具策略性的工具调用能力，提高了设计质量，同时识别出模型常见的错误模式。

Conclusion: CANVAS基准为评估和提升基于工具的界面设计能力提供了重要工具，指导未来在工具调用设计能力方面的改进工作。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型中的图像编码器通常独立预训练，无法针对下游任务或文本查询进行优化，导致处理图像时缺乏针对性。

Method: 提出TIE方法，通过文本查询条件化训练图像编码器，使其能够根据输入文本生成针对性的图像表示。

Result: 在1B和3B规模下，TIE在9个图像到文本基准测试中平均提升1.5和1.3个点，在DocVQA和InfoVQA任务上最高提升6个点，同时减少一半图像token使用量。
...
TL;DR: SMARC是一个从极稀疏视觉输入中重建和分类表面材料的统一模型，仅需单个10%图像补丁即可识别和重建完整RGB表面并分类材料类别。
...
Result: 相比五种模型（卷积自编码器、ViT、MAE、Swin Transformer、DETR），SMARC达到PSNR 17.55 dB和材料分类准确率85.10%的SOTA结果。
...
TL;DR: LongVT是一个端到端的代理框架，通过交错的多模态工具链思维实现长视频推理，利用LMMs的时序定位能力作为视频裁剪工具进行全局到局部的推理循环。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在长视频推理中容易产生幻觉，特别是在证据稀疏且时间分散的情况下。受人类理解长视频方式的启发，需要先全局浏览再检查相关片段获取细节。

Method: 利用LMMs固有的时序定位能力作为原生视频裁剪工具，放大特定视频片段并重采样更细粒度的视频帧，形成全局到局部的推理循环。采用三阶段训练策略：工具集成冷启动监督微调、代理强化学习和代理强化微调。

Result: LongVT在四个具有挑战性的长视频理解和推理基准测试中持续优于现有强基线模型。

Conclusion: LongVT框架通过工具链思维和全局到局部推理循环，有效解决了长视频推理中的幻觉问题，显著提升了模型性能。
...
TL;DR: 本文对KRISP模型进行了轻量级复现，参数大幅减少但性能约为原模型的75%，揭示了原设计中的缺陷和实际应用问题，并在资源受限条件下评估了知识增强VQA架构的可扩展性和有效性。


<details>
  <summary>Details</summary>
Motivation: 原始KRISP模型虽然有效但计算需求高、参数庞大且与大型主干网络紧密耦合，不适合资源受限环境。本文旨在重新审视该模型，开发轻量级版本以适应边缘设备部署。

Method: 采用轻量级复现方法，大幅减少参数数量，通过系统消融研究评估知识增强VQA架构，包括在合成VQA数据上的概念验证和在DAQUAR数据集上的评估。

Result: 复现模型性能约为原模型的75%，但揭示了原设计中未充分覆盖的设计缺陷和实际问题。模型在受限知识图谱域内运行，有效防止AI幻觉，并能在智能手机和AR-VR等边缘设备上离线运行。

Conclusion: 轻量级KRISP复现展示了在资源受限环境下知识增强VQA架构的可行性，为边缘设备上的视觉推理应用提供了实用解决方案，同时揭示了原始工业级模型在现实部署中可能面临的问题。
...
TL;DR: 本文提出了一个名为"warping"的新算子，统一分析了深度学习中的动态采样机制，包括可变形卷积、空间变换网络等，发现了前向与反向传播的不对称性，并提供了稳定训练的条件分析。


<details>
  <summary>Details</summary>
Motivation: 动态采样机制在计算机视觉模型中已证明有效，但缺乏统一的理论分析框架。本文旨在连接各种动态采样方法，建立统一的分析基础。

Method: 开发了"warping"算子作为动态采样的最小实现，可重构现有架构。通过统计建模分析算子特性，包括IID变量和齐次随机场模型，并引入新的损失景观可视化方法。

Result: 发现动态采样机制与传统卷积平移不变算子的正交性，揭示了前向与反向传播的不对称性，确定了稳定训练的必要条件，并分析了离散化效应。

Conclusion: warping算子为动态采样提供了统一的理论框架，揭示了这类机制的本质特性，为理解和改进动态采样网络提供了理论基础和实践指导。
...
TL;DR: Δ-NeRF是一个模块化残差框架，用于神经辐射场的增量优化，无需访问历史数据即可实现增量更新，在卫星图像上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF框架在引入新视图时需要完全重新训练，限制了在卫星地形分析等数据顺序到达场景中的应用，且增量优化方法存在灾难性遗忘问题。

Method: 提出残差控制器向冻结的基础NeRF注入逐层修正；不确定性感知门控机制自适应结合基础和优化预测；视图选择策略减少47%训练数据；知识蒸馏将增强模型压缩至原大小的20%。

Result: 在卫星图像上达到与联合训练相当的性能，训练时间减少30-42%，PSNR比简单微调提升43.5%，在某些指标上超越联合训练。

Conclusion: Δ-NeRF为NeRF的增量优化提供了有效解决方案，在保持性能的同时显著提升训练效率，适用于数据顺序到达的应用场景。
...
TL;DR: Split-then-Merge (StM) 是一个新颖的视频生成框架，通过将未标注视频分割为前景和背景层，然后自组合学习动态主体与多样化场景的交互，解决了生成视频组合控制的数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 解决生成视频组合控制中的数据稀缺问题，传统方法依赖标注数据集或手工规则，而StM利用未标注视频学习复杂的组合动态。

Method: 将未标注视频分割为动态前景和背景层，通过自组合学习主体与场景交互；引入变换感知训练流程，使用多层融合和增强实现感知组合，以及保持前景保真度的身份保持损失。

Result: 在定量基准测试和人类/VLLM定性评估中，StM均优于最先进方法。

Conclusion: StM框架通过创新的分割-合并方法有效解决了视频生成中的组合控制问题，无需依赖标注数据即可学习复杂的场景交互动态。
...
TL;DR: Sphinx是一个针对视觉感知和推理核心认知原语的合成环境，通过程序化生成包含多种元素的谜题并配备可验证的真实解，用于精确评估和大规模数据集构建。


<details>
  <summary>Details</summary>
Motivation: 针对当前视觉语言模型在核心认知任务上的表现不足，需要更精确的评估基准来推动多模态推理能力的发展。

Method: 使用程序化生成方法创建包含图案、图块、图表、图标和几何原语的谜题，每个谜题都配有可验证的真实解，涵盖25种任务类型。

Result: 评估显示即使是先进的GPT-5模型准确率也只有51.1%，远低于人类表现；使用可验证奖励的强化学习能显著提升模型准确率。

Conclusion: Sphinx基准为视觉推理提供了精确评估工具，可验证奖励的强化学习在多模态推理方面具有重要潜力。
...
TL;DR: 本文提出了一种无需训练的优化视觉反转方法替代传统扩散先验网络，通过优化潜在视觉表示来匹配文本嵌入，并引入两种约束来提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 挑战传统扩散模型中计算昂贵的先验网络必要性，寻求训练免费、数据免费的替代方案。

Method: 使用优化视觉反转方法，从随机伪标记初始化潜在表示，通过最大化与文本嵌入的余弦相似度进行迭代优化，并引入马氏距离和最近邻损失约束。

Result: 在Kandinsky 2.2上的实验表明，OVI可作为传统先验的替代方案，最近邻方法在视觉保真度上表现最佳，定量得分与最先进数据高效先验相当或更高。

Conclusion: 优化视觉反转方法是有前景的替代方案，同时揭示了当前评估基准的缺陷，值得进一步研究。
...
Method: 采用Producer-Refiner架构，基于Transformer解码器，Producer提出初始汇合轨迹，Refiner循环精炼生成最终轨迹，形成中心线图。

Result: 在多个公共中心线数据集上，RefTr实现了优于先前SOTA的召回率和相当的精度，同时推理速度更快、参数更少。

Conclusion: RefTr展示了作为3D医学成像中血管树分析的新SOTA框架的潜力。
...
Conclusion: 该数据集支持单目和立体深度估计、浅景深渲染、去模糊、3D场景重建和新视角合成等任务的研究，并展示了当前最先进方法在真实世界光学泛化方面的挑战。
...
TL;DR: 本文提出了首个大规模无监督视觉记忆性数据集，包含82,000多个视频和描述性回忆数据，用于建模视觉记忆信号，在回忆生成和舌尖现象检索任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 视觉内容记忆性研究面临人工标注成本高、数据集多样性有限的问题，现有数据集主要收集聚合记忆分数，无法捕捉自然开放式回忆中的细微记忆信号。
...
TL;DR: 提出一种从立体雾天图像序列中动态估计雾模型参数的方法，通过同时优化所有参数避免误差传播，并创建了首个包含相机光度参数校准的真实雾天数据集SDIRF。


<details>
  <summary>Details</summary>
Motivation: 现有方法顺序估计雾模型参数容易导致误差传播，且无法有效处理现实中常见的非均匀雾况。

Method: 提出同时优化所有雾参数的算法，假设雾仅局部均匀，可作为现有视觉SLAM或里程计系统的附加模块。

Result: 在合成雾数据和真实雾序列SDIRF上的实验表明，该方法在参数估计精度和真实雾适应能力上均优于现有方法。

Conclusion: 该方法能有效处理真实世界中的非均匀雾，推进雾天视觉感知研究，代码和数据集已开源。
...
Method: 使用交叉视角锚点提示生成器(V^2-Anchor)建立几何感知对应，以及交叉视角视觉提示生成器(V^2-Visual)增强外观引导线索，采用多专家设计和后验循环一致性选择器(PCCS)自适应选择最可靠的专家。
...
TL;DR: 提出Null-Text Test-Time Alignment (Null-TTA)方法，通过优化分类器自由引导中的无条件嵌入来对齐扩散模型，避免奖励黑客问题，在语义空间实现有效的测试时对齐。


<details>
  <summary>Details</summary>
Motivation: 现有测试时对齐方法容易导致目标奖励函数的欠优化或过优化（奖励黑客），需要一种更稳健的对齐方法。

Method: 优化分类器自由引导中的无条件嵌入，而不是操作潜在变量或噪声变量，在语义相干流形上进行对齐。
...
Method: 通过检索图像坐标的K最近高斯分布，聚合距离加权的嵌入向量，然后通过神经网络预测RGB值
...
TL;DR: 使用深度学习模型从低剂量对比增强T1加权MRI中恢复标准剂量图像质量，可在仅使用10%-30%标准剂量的情况下实现脑桥小脑角区病变检测和诊断特征分析。


<details>
  <summary>Details</summary>
Motivation: 减少对比剂剂量对患者更安全，但低剂量MRI图像质量较差。本研究旨在开发深度学习模型来改善低剂量对比增强MRI的图像质量。

Method: 多中心回顾性研究，使用前庭神经鞘瘤患者的T1和T1ce图像模拟低剂量T1ce，训练深度学习模型从低剂量图像恢复标准剂量图像质量。

Result: 在10%输入剂量下，DL恢复的T1ce显著改善了分割性能：Dice系数从0.673提升到0.734，95% Hausdorff距离从2.38mm减少到2.07mm，平均表面距离从1.00mm减少到0.59mm。

Conclusion: 深度学习模型能够显著改善脑桥小脑角区低剂量MRI的图像质量，使得在仅使用10%-30%标准剂量的情况下仍能进行病变检测和诊断特征分析。
...
TL;DR: 提出一种平滑正则化技术，通过高斯随机游走建模视频帧间嵌入变化，增强轻量级视频识别模型的时序归纳偏置，在Kinetics-600上获得3.8%-6.4%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 轻量级视频识别模型难以有效捕捉复杂时序动态，需要更强的时序归纳偏置来利用视频固有的时间连贯性。

Method: 使用高斯随机游走建模连续帧中间层嵌入的变化，惩罚表示上的突变，促进低加速度解，从而强制模型学习平滑的时序表示。

Result: MoViNets模型家族在各自FLOP约束下将当前最优结果提升3.8%-6.1%；MobileNetV3和MoViNets-Stream系列在可比内存占用下比先前最优模型提升4.9%-6.4%。

Conclusion: 平滑正则化技术能有效增强轻量级视频识别模型的时序建模能力，显著提升性能，为轻量级视频分析提供了有效解决方案。
...
TL;DR: 提出了一个基于开放词汇语义分割的框架，用于生成神经元激活与任意概念之间的组合解释，突破了传统方法依赖人工标注数据的限制。


<details>
  <summary>Details</summary>
Motivation: 传统组合解释方法依赖人工标注数据集，限制了其在特定领域和预定义概念中的应用。本文旨在开发一个能够探测神经元对任意概念和数据集响应的框架。

Method: 框架包含三个步骤：指定任意概念、使用开放词汇模型生成语义分割掩码、从这些掩码中推导组合解释。

Result: 与先前方法相比，该框架在定量指标和人类可解释性方面表现良好，展示了从人工标注数据转向模型标注数据时解释的差异，并提供了更灵活的解释能力。

Conclusion: 该框架通过开放词汇语义分割实现了对神经元激活的更灵活、更广泛的概念解释，为理解神经网络内部表示提供了新途径。
...
TL;DR: 本文介绍了UruDendro4数据集，包含102个火炬松横截面图像样本，提供多高度采样和手动标注的年轮数据，支持年轮体积建模。同时建立了自动年轮检测的性能基准，DeepCS-TRD方法表现最佳。


<details>
  <summary>Details</summary>
Motivation: 解决木材横截面数据稀缺问题，传统手动测量年轮耗时且不精确，需要自动化算法和数据集来提高准确性和效率。

Method: 创建UruDendro4数据集，包含多高度采样的火炬松横截面图像和手动标注的年轮；使用最先进方法建立自动年轮检测性能基准；进行消融实验验证参数配置。

Result: DeepCS-TRD方法在年轮检测中表现最佳：平均精度0.838，平均召回率0.782，适应兰德误差0.084。包含该数据集的训练能提高模型泛化能力。

Conclusion: UruDendro4数据集填补了木材横截面数据空白，支持年轮体积建模，为自动年轮检测提供了可靠基准，有助于改进林业管理实践。
...
TL;DR: BUSTR是一个无需配对图像-报告监督的多任务视觉语言框架，通过结构化描述符和放射组学特征生成乳腺超声报告，在两个公共数据集上显著提升了报告生成质量和临床效果。


<details>
  <summary>Details</summary>
Motivation: 乳腺超声报告生成面临缺乏配对图像-报告数据集和大语言模型幻觉风险的问题，需要开发不依赖配对数据的有效方法。

Method: 使用多任务Swin编码器学习描述符感知的视觉表示，通过双级目标对齐视觉和文本标记，结合标记级交叉熵和余弦相似度对齐损失。

Result: 在两个公共BUS数据集上，BUSTR持续改善了标准自然语言生成指标和临床效果指标，特别是在关键目标如BI-RADS类别和病理学方面。
...
TL;DR: 本文提出了表达性图像合成任务，这是一个新的图像合成公式，强调风格多样性和宽松的放置逻辑，反映了用户在真实创意平台上的编辑行为。作者开发了StickerNet框架，通过两阶段方法预测合成参数，并使用来自真实编辑平台的数据集进行训练。


<details>
  <summary>Details</summary>
Motivation: 传统图像合成研究主要关注视觉真实性和语义合理性，但在现代内容创作中，许多合成并不旨在保持真实感，而是追求艺术性、趣味性或社交参与度。本文受到在线平台用户为获得社区认可而创作内容的启发，探索这种表达性合成任务。

Method: 提出了StickerNet两阶段框架：首先确定合成类型，然后预测不透明度、遮罩、位置和缩放等放置参数。不同于以往通过模拟对象放置在真实图像上构建数据集的方法，直接从匿名在线视觉创作平台的180万次编辑动作构建数据集。

Result: 用户研究和定量评估表明，StickerNet优于常见基线方法，并与人机放置行为紧密匹配，证明了从真实世界编辑模式中学习的有效性，尽管任务本身存在固有模糊性。

Conclusion: 这项工作在视觉理解领域引入了新方向，强调表达性和用户意图而非真实性，为图像编辑研究开辟了新的可能性。
...
TL;DR: TrafficLens提出了一种针对多摄像头交通路口的定制算法，通过利用摄像头重叠覆盖区域和迭代应用VLM，显著减少视频到文本转换时间，同时保持信息准确性。


<details>
  <summary>Details</summary>
Motivation: 解决多摄像头交通视频数据量大、分析困难的问题，克服传统方法中VLM转换视频到文本耗时较长、无法及时生成洞察和调查事件的局限性。

Method: 采用顺序方法，利用摄像头重叠覆盖区域，迭代应用具有不同token限制的VLM，将前一个摄像头的输出作为后续摄像头的提示，并通过对象级相似性检测器智能绕过冗余的VLM调用。

Result: 在真实数据集上的实验结果表明，TrafficLens将视频到文本转换时间减少了高达4倍，同时保持了信息准确性。

Conclusion: TrafficLens为多摄像头交通路口提供了一种高效的视频分析解决方案，显著提升了处理效率，为智能交通系统提供了实用的分析工具。
...
TL;DR: 提出结合Vision Transformers和同态加密的隐私保护联邦学习框架，用于安全的跨机构病理图像分类，通过加密CLS token实现通信效率提升和隐私保护。
...
Method: 使用Vision Transformers的CLS token作为紧凑特征表示，采用CKKS同态加密加密这些token后进行安全聚合，避免原始梯度传输。

Result: CLS token加密比梯度加密减少30倍通信量，有效防止模型反演攻击（PSNR: 52.26 dB），在加密域实现90.02%分类准确率。

Conclusion: 该方法在保持高分类精度的同时，显著提升了联邦学习的隐私保护能力和通信效率，适用于医疗领域的协作学习。
...
TL;DR: 提出了一种基于双整流流的免反演风格迁移框架，通过并行预测内容和风格轨迹，动态中点插值融合，实现仅需前向传播的高效风格迁移。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于扩散模型的风格迁移方法依赖计算密集型反演过程，导致效率低下和视觉失真的问题。

Method: 使用双整流流并行预测内容和风格轨迹，通过动态中点插值融合速度场，结合注意力注入机制指导风格整合。

Result: 在多样化风格和内容上展现出良好的泛化能力，提供了高效且有效的风格迁移流程。

Conclusion: 该免反演框架克服了传统方法的局限性，在保持视觉保真度和内容完整性的同时显著提升了计算效率。
...
TL;DR: 提出了一种无需测试时参考图像的伪装目标检测框架，通过类别原型记忆和查询条件混合机制，在训练时蒸馏参考信息，推理时合成参考向量。
...
Method: 使用EMA更新的类别原型记忆，通过查询条件混合权重预测生成参考向量，并引入双向注意力对齐模块弥合表示差距。
...
TL;DR: 提出WavePCNet网络，通过三相位波前复传播重投影和动量记忆机制，解决非视域中微弱光模式下的遮挡物体定位与分割难题，在低信噪比条件下提升稳定性和可靠性。
...
Method: WavePCNet集成三相位波前复传播重投影(TriWCP)引入复振幅传输算子精确约束相干传播行为，结合动量记忆机制抑制扰动积累，并引入高频跨层补偿增强构建频率选择性通路。

Result: 在四个物理采集数据集上的广泛实验表明，WavePCNet在准确性和鲁棒性方面均优于最先进方法。

Conclusion: WavePCNet通过物理驱动的波前传播补偿，有效提升了非视域遮挡物体感知的稳定性和可靠性，为复杂环境条件下的光学观测提供了新解决方案。
...
Method: 构建GuardTrace数据集，通过多样化提示策略生成，并经过MLRM和人类投票验证流程精炼。提出三阶段渐进式训练方案结合数据精炼过程，使模型能够学习不同风险级别的细微和上下文相关的安全偏好。

Result: 在提出的测试集上，GuardTrace-VL模型在不安全推理检测任务中达到93.1%的F1分数，比之前最强的多模态安全防御方法提高了13.5%。

Conclusion: GuardTrace-VL能够有效检测推理过程中出现的不安全内容，填补了现有安全防护在中间推理过程监控方面的空白。
...
<details>
  <summary>Details</summary>
Motivation: 图像可以视为前景对象在背景上的分层组合，这种分层表示能够实现元素的独立编辑，为内容创作提供更大灵活性。尽管生成模型取得了进展，但由于方法和数据有限，将单张图像分解为图层仍然具有挑战性。
...
Conclusion: 该方法成功地将修复模型适配到分层分解任务，通过多模态上下文融合模块有效保留了图像细节，在合成数据集上训练取得了良好效果，为图像编辑应用提供了新的解决方案。
...
TL;DR: MERGE是首个用于新闻图像描述的多模态实体感知检索增强生成框架，通过构建实体中心的多模态知识库，解决了现有方法在信息覆盖、跨模态对齐和视觉实体基础方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有新闻图像描述方法存在三个关键问题：信息覆盖不完整、跨模态对齐弱、视觉实体基础不理想，需要更有效的多模态融合方法。

Method: 构建实体中心的多模态知识库(EMKB)，集成文本、视觉和结构化知识；采用多阶段假设-描述策略改善跨模态对齐；通过图像内容引导的动态检索增强视觉实体匹配。

Result: 在GoodNews和NYTimes800k数据集上显著超越现有最优方法，CIDEr分别提升+6.84和+1.16，命名实体识别F1分数分别提升+4.14和+2.64。在未见过的Visual News数据集上表现优异，CIDEr提升+20.17，F1分数提升+6.22。
...
TL;DR: MetaRank是一个基于元学习的框架，用于自动选择最适合特定任务的模型迁移性评估(MTE)指标，通过将数据集和指标描述嵌入共享语义空间，学习数据集特征与指标机制之间的复杂关系。


<details>
  <summary>Details</summary>
Motivation: 现有的MTE指标选择方法通常是临时性的或基于历史平均性能，但MTE指标的有效性高度依赖于具体任务，没有一个指标在所有目标数据集上都表现最优。
...
TL;DR: 提出了一种基于原型的新型可信多视图分类框架，通过简化视图内邻域关系学习并实现视图内外结构动态对齐，提高跨视图共识发现的效率和一致性。
...
Conclusion: 所提出的基于原型的可信多视图分类框架能够有效克服现有方法的局限性，提供更高效和一致的多视图分类解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖模糊的文本提示，要么需要为不同参数训练独立模块，无法实现精确的相机参数控制和多参数组合。
...
Conclusion: CameraMaster通过统一的相机感知框架，实现了精确、可组合的图像修饰控制，解决了现有方法在参数精确控制和可扩展性方面的局限性。
...
TL;DR: 提出了CaptionQA基准，通过下游任务效用评估图像描述质量，涵盖4个领域33,027个密集标注问题，揭示当前MLLM在描述效用方面存在显著差距
...
Method: 构建包含4个领域（自然、文档、电商、具身AI）的CaptionQA基准，包含25个顶级类别和69个子类别的细粒度分类，创建33,027个密集标注的多选题，通过LLM仅使用描述回答问题来直接测量描述效用

Result: 评估显示图像与其描述效用之间存在显著差距，在传统图像QA基准上表现相近的模型在描述效用上下降高达32%

Conclusion: CaptionQA提供了评估描述效用的新方法，揭示了当前MLLM在生成实用描述方面的局限性，并提供了可扩展的基准框架
...
Method: 结合MeanFlow与物理一致性约束实现高质量动作生成；采用BiMamba骨干网络和通道级跨模态融合的简单高效架构；支持非自回归生成和动作编辑。
...
TL;DR: 提出了LungNoduleAgent，一个用于分析肺部CT扫描的协作多智能体系统，通过模块化设计提高结节描述和恶性分级精度，在多个数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在描述结节形态和融入医学专业知识方面存在不足，影响临床可靠性。协作多智能体系统有望在医学应用中平衡通用性和精确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的学习式去模糊方法存在两难选择：基于模型的深度展开方法能保持物理约束但会产生过度平滑的纹理，而生成模型能获得更好的感知质量但会因物理约束不足而产生幻觉细节。

Method: 将退化场建模为高维压缩核的密集连续体，捕捉运动和退化模式的微小变化，并利用这个丰富的描述符场来条件化ControlNet架构，强有力地引导扩散采样过程。

Result: 在具有挑战性的严重模糊场景中，该方法在物理精度和感知真实性之间取得了良好平衡，超越了最先进的基于模型方法和生成基线方法。

Conclusion: 该框架通过将生成先验与密集物理约束相结合，有效弥合了物理准确性和感知真实性之间的差距，为空间变化图像去模糊问题提供了新的解决方案。
...
TL;DR: MUSE是首个统一的情感图像生成与编辑框架，通过Test-Time Scaling策略，无需额外训练扩散模型或专门数据集，解决了情感合成中的三个关键问题：如何稳定引导、何时引入引导、以及引导哪种情感。
...
Method: 采用Test-Time Scaling策略，利用现成情感分类器进行基于梯度的情感token优化，通过语义相似度确定最佳引导时机，使用多情感损失减少固有和相似情感的干扰。

Result: 实验结果显示MUSE在生成和编辑任务上都优于所有方法，提高了情感准确性和语义多样性，同时在期望内容、文本提示遵循和真实情感表达之间保持最佳平衡。

Conclusion: MUSE为情感合成建立了新的范式，展示了统一的生成和编辑框架的可行性。
...
TL;DR: 提出T-NIG模型，通过将时间参数融入正态逆伽马分布来处理不规则时间间隔的脑成像序列，实现长期阿尔茨海默病预测和图像生成。


<details>
  <summary>Details</summary>
Motivation: 解决长期AD预测中图像生成难以保持疾病相关特征的问题，特别是在处理不规则时间间隔序列数据时。

Method: 使用两个时间点的脑图像，通过坐标邻域识别特征，在正态逆伽马分布中融入时间参数，并利用不确定性估计减少认知和随机不确定性。

Result: 在数据集中的短期和长期预测任务中表现出最先进的性能，能够有效预测疾病进展并保持疾病相关特征。

Conclusion: T-NIG模型能够有效处理不规则时间分布的数据，在AD预测和图像生成方面具有优越性能。
...
TL;DR: MIRA是一个轻量级的多模态推理代理，通过迭代感知-推理-行动循环来改进指令引导的图像编辑，解决扩散模型在复杂指令理解上的困难。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在处理涉及组合关系、上下文线索或指代表达式的复杂用户指令时，往往无法准确解释，导致编辑结果语义漂移或未能反映预期变化。

Method: 提出MIRA多模态推理代理，采用迭代感知-推理-行动循环，逐步预测原子编辑指令，使用视觉反馈进行决策。通过150K多模态工具使用数据集MIRA-Editing和两阶段SFT+GRPO训练流程实现复杂编辑指令的推理和编辑。
...
TL;DR: 提出了一种新颖的虹膜认证匹配管道，通过3D-CNN学习丰富的时空表示，采用课程学习方式训练，有效应对旋转、缩放、反射和模糊等挑战。
...
Conclusion: 提出的框架通过学习时空表示和课程学习训练，显著提升了虹膜认证系统在复杂环境下的性能和鲁棒性。
...
TL;DR: 提出Pygmalion Effect in Vision框架，通过图像到黏土转换来抑制镜面反射线索，提升反射物体的3D重建质量


<details>
  <summary>Details</summary>
Motivation: 解决3D重建中由于视角相关反射导致的外观和几何纠缠问题

Method: 采用双分支网络结构：基于BRDF的反射分支和黏土引导分支，通过合成黏土样图像进行联合训练

Result: 在合成和真实数据集上显著提升了法线精度和网格完整性，优于现有反射处理方法

Conclusion: 通过'去光泽化'将辐射转化为中性表示，为反射物体几何学习提供了强大的归纳偏置
...
Method: 提出结构化标题框架编码雷达坐标系中的车辆分布，以及哈希感知对比学习目标量化连续场景相似性而非二元匹配。利用CARLA模拟器生成大规模标注雷达数据集。

Result: 开发了RadarFM雷达基础模型，能够学习统一的场景级表示，支持细粒度空间推理。

Conclusion: RadarFM通过结构化语言监督和对比学习，为雷达感知提供了统一的基础模型框架，解决了现有方法的碎片化问题。
...
TL;DR: EM-KD是一种新的知识蒸馏范式，通过解决高效MLLMs中视觉令牌不平衡问题，提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有高效MLLMs压缩视觉令牌会损失视觉信息，降低理解能力，而传统知识蒸馏方法忽略了师生模型间视觉令牌不平衡导致的细粒度视觉理解差异。

Method: 首先使用曼哈顿距离和匈牙利匹配算法对齐师生视觉令牌，然后引入两种蒸馏策略：视觉-语言亲和力蒸馏（VLAD）和视觉语义蒸馏（VSD）。

Result: 在多个基准测试中，EM-KD训练出的模型在准确性和效率上都显著优于现有高效MLLMs，且比传统蒸馏方法表现更好。

Conclusion: EM-KD通过解决视觉令牌不平衡问题，有效提升了高效MLLMs的性能，验证了该方法的有效性。
...
TL;DR: FaithFusion是一个基于像素级期望信息增益（EIG）的3DGS-扩散模型融合框架，用于可控驾驶场景重建和3D场景生成，在保持几何保真度的同时实现大视角偏移下的视觉合理外观合成。


<details>
  <summary>Details</summary>
Motivation: 在可控驾驶场景重建和3D场景生成中，需要在保持几何保真度的同时合成大视角偏移下的视觉合理外观。然而，基于几何的3DGS和基于外观的扩散模型的有效融合面临固有挑战，缺乏像素级、3D一致的编辑标准往往导致过度恢复和几何漂移。

Method: 引入基于像素级期望信息增益（EIG）的FaithFusion框架。EIG作为统一策略指导时空合成：作为空间先验指导扩散模型优化高不确定性区域，同时通过像素级权重将编辑结果蒸馏回3DGS。这是一个即插即用系统，无需额外先验条件和结构修改。

Result: 在Waymo数据集上的广泛实验表明，该方法在NTA-IoU、NTL-IoU和FID指标上达到最先进性能，即使在6米车道偏移时仍保持107.47的FID。

Conclusion: FaithFusion通过EIG驱动的3DGS-扩散模型融合，有效解决了几何保真度和视觉外观合成之间的平衡问题，在可控驾驶场景重建中表现出色。
...
TL;DR: 提出了一种名为DATGN的变形感知时序生成网络，用于从脑部MRI图像中自动学习阿尔茨海默病的形态学变化，实现早期预测。该方法能处理不完整的时间序列数据，生成符合疾病进展的未来MRI图像。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病早期预测对延缓病情进展至关重要。当前方法主要依赖手动特征提取分析脑部形态变化，需要自动化解决方案来处理时间序列中常见的数据缺失问题。

Method: DATGN首先对不完整的MRI时间序列进行插值，然后通过双向时序变形感知模块引导网络生成符合疾病进展的未来MRI图像，支持早期预测。

Result: 在ADNI数据集上的实验显示，DATGN在PSNR和MMSE图像质量指标上表现优异。将生成的合成数据集成到分类方法中，AD vs NC分类准确率提升6.21%到16%，AD vs MCI vs NC分类准确率提升7.34%到21.25%。
...
TL;DR: EntPruner是一个基于熵引导的自动渐进剪枝框架，专门针对扩散和流模型，通过条件熵偏差评估模块重要性，实现参数精简同时保持生成质量。
...
Method: 提出熵引导剪枝策略，使用条件熵偏差(CED)评估模块重要性；采用零样本自适应剪枝框架，动态决定剪枝时机和程度。

Result: 在DiT和SiT模型上实验表明，EntPruner实现了最高2.22倍的推理加速，同时在ImageNet和三个下游数据集上保持竞争力的生成质量。

Conclusion: EntPruner为生成模型提供了一种有效的剪枝方法，能够在保持性能的同时显著减少参数冗余。
...
<details>
  <summary>Details</summary>
Motivation: 传统仅使用几何线索的方法在指定布局时不足以约束外观、材质和光照，限制了物理上有意义的编辑（如重新照明或材质替换），并经常导致时间漂移。
...
Conclusion: 通过丰富图形基础的模态，CtrlVDiff在视频理解和可控生成方面超越了现有方法，实现了更精确和可预测的控制。
...
TL;DR: 提出Fourier Kernel Estimator (FKE)方法，通过傅里叶空间激活操作将空间域卷积问题转换为频域乘法问题，使网络能够在核级别学习模糊过程，无需额外监督。


<details>
  <summary>Details</summary>
Motivation: 当前深度网络仍停留在像素级学习阶段，无法让去模糊模型理解模糊的本质。模糊是由清晰图像与模糊核卷积自然产生的，因此让网络在核级别学习模糊过程能显著提升图像去模糊性能。

Method: 1. 提出傅里叶核估计器(FKE)，在傅里叶空间进行激活操作；2. 将卷积对象从图像改为网络提取的特征；3. 设计解耦多尺度架构，使用多个层次子Unet和可逆策略。

Result: 在运动去模糊任务上达到最先进结果，并显示出处理其他核相关问题的潜力。分析表明核估计器能够学习具有物理意义的核。

Conclusion: 该方法通过核级别学习模糊过程，显著提升了去模糊性能，且无需额外监督，具有低复杂度和高效率的特点。
...
TL;DR: AnchorOPT提出了一种动态锚点提示学习框架，通过动态学习锚点值和优化锚点与软标记的位置关系，解决了现有方法中锚点静态固定的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的提示学习方法使用静态锚点（固定值和位置）缺乏跨任务和阶段自适应性，限制了CLIP模型的泛化能力。

Method: AnchorOPT在两方面引入动态性：1）从任务特定数据动态学习锚点值；2）通过可学习位置矩阵自适应优化锚点与软标记的位置关系。采用两阶段训练策略。
...
TL;DR: ProxyFormer是一个新的参考视频对象分割架构，通过代理查询机制整合视觉和文本语义，在视频特征编码器中逐步更新传播代理查询，实现跨模态对齐和帧间依赖建模。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个局限：(1)条件查询缺乏帧间依赖和变化建模，难以在帧间变化显著时准确跟踪目标；(2)文本约束集成较晚，可能导致视频特征关注非参考对象。

Method: 引入代理查询集来整合视觉和文本语义，在视频特征编码器的多个阶段逐步更新传播代理查询，将跨模态交互解耦为时间和空间维度，并设计联合语义一致性训练策略。

Result: 在四个广泛使用的RVOS基准测试上的综合实验表明，ProxyFormer优于最先进的方法。

Conclusion: ProxyFormer通过代理查询机制有效解决了现有方法的局限性，提升了参考视频对象分割的准确性和连贯性。
...
TL;DR: TEAR是一个针对文本到视频模型安全性的自动化红队框架，通过时间感知测试生成器来发现与视频动态时序相关的安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有的安全性评估方法主要针对静态图像和文本生成，无法捕捉视频生成中复杂的时间动态特性，因此需要专门针对T2V模型的安全评估框架。

Method: 采用时间感知测试生成器，通过两阶段优化（初始生成器训练和时间感知在线偏好学习）来创建文本无害但能利用时间动态引发违规视频输出的提示，并通过循环精炼模型提高提示的隐蔽性和对抗效果。

Result: 在开源和商业T2V系统上的广泛实验评估显示，TEAR实现了超过80%的攻击成功率，相比之前最佳结果57%有显著提升。

Conclusion: TEAR框架有效揭示了T2V模型中与时间动态相关的安全风险，证明了现有安全评估方法的不足，并为T2V模型的安全评估提供了新方法。
...
TL;DR: LLaVA-UHD v3提出了渐进式视觉压缩(PVC)方法，使标准ViT能够高效处理原生分辨率图像，在保持性能的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 解决全局原生分辨率视觉编码在MLLMs中计算开销过大的问题，同时保持其增强的整体能力优势。

Method: 提出渐进式视觉压缩(PVC)方法，包含两个关键模块：精炼补丁嵌入支持灵活补丁大小缩放，窗口化令牌压缩在ViT层间分层部署以渐进聚合局部令牌表示。

Result: ViT-UHD在相同MLLM架构下，性能与MoonViT相当但TTFT减少2.4倍；LLaVA-UHD v3性能与Qwen2-VL相当但TTFT进一步减少1.9倍。

Conclusion: PVC方法能够将广泛预训练的ViT重新配置为高效架构，同时保持通用性，为高效MLLMs研究提供了可行方案。
...
TL;DR: 本文研究了视觉语言模型中无关信息（干扰物）对测试时扩展的影响，发现视觉干扰物与文本干扰物存在根本差异：虽然都存在逆扩展效应，但视觉干扰物会降低准确性而不增加推理长度。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明语言模型中的文本干扰物会导致推理变长但效果变差，本文旨在探究多模态环境中是否会出现类似现象。

Method: 引入Idis数据集，系统性地在语义、数值和空间维度上变化视觉干扰物，并通过追踪属性计数来分析推理过程。

Result: 视觉干扰物会降低模型准确性但不增加推理长度，且这些趋势也存在于Waterbirds等视觉偏见基准中。

Conclusion: 提出了简单的提示策略来减轻推理模型中的偏见驱动预测，为理解视觉语言模型中干扰物影响提供了新见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉自回归模型在测试时扩展方面存在不足，简单应用Best-of-N策略会消耗大量计算在错误生成轨迹上，且光栅扫描解码缺乏整体画布蓝图。

Method: 采用网格分区渐进生成方案，在同一画布位置生成多个部分候选，早期修剪不可行候选，固定可行候选作为锚点指导后续解码；配合布局指定提示重构策略，通过检查部分视图推断可行布局。

Result: 在N=4时，GridAR比Best-of-N(N=8)在T2I-CompBench++上性能提升14.4%，成本降低25.6%；在图像编辑任务中，PIE-Bench上语义保持度比大N基线提升13.9%。
...
TL;DR: NDTokenizer3D是一个通用的3D视觉语言模型，通过多尺度NDT表示和渐进式特征融合，实现了对3D场景的全面理解和推理，在多种3D理解任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉语言模型在场景标记化和跨任务应用方面存在挑战，需要一种能够有效处理3D场景并支持人机交互的统一框架。

Method: 采用三阶段场景标记化流程：构建多尺度NDT表示，使用多尺度NDT解码器融合跨尺度特征生成场景标记，并将解码器重新用于人机交互提示和分割掩码解码。

Result: 在3D参考分割、3D视觉问答和3D密集描述等任务中取得了显著改进。

Conclusion: NDTokenizer3D提供了一个紧凑统一的架构，实现了细粒度的通用3D视觉语言理解，成功连接了语言级推理与3D空间理解。
...
TL;DR: 提出UPA-RFAS框架，通过鲁棒特征、注意力和语义攻击实现跨模型可迁移的通用对抗补丁攻击，在未知架构、微调变体和仿真到现实场景下均有效


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型的对抗攻击大多过拟合单一模型，在黑盒设置下效果不佳，缺乏通用且可迁移的攻击方法

Method: 结合特征空间目标函数、鲁棒增强的两阶段min-max优化、以及VLA特有的注意力劫持和语义错配损失函数

Result: 实验表明UPA-RFAS在多种VLA模型、操作任务和物理执行中均能实现跨模型、跨任务和跨视角的稳定迁移攻击

Conclusion: UPA-RFAS暴露了VLA驱动机器人的实际补丁攻击面，为未来防御研究建立了强基线
...
TL;DR: G²VLM是一个几何基础的视觉语言模型，通过整合3D空间重建和空间理解来解决现有视觉语言模型在空间智能方面的不足。
...
Conclusion: DCBoost通过利用局部结构线索增强全局特征表示，有效解决了深度聚类中全局-局部结构不一致的问题，显著提升了聚类性能。
...
TL;DR: BotaCLIP是一个轻量级多模态对比学习框架，用于将预训练的地球观测基础模型（DOFA）与植物学数据进行对齐，以注入领域专业知识而不需要重新训练。


<details>
  <summary>Details</summary>
Motivation: 解决在预训练基础模型中注入领域特定知识而不需要从头训练或承担高计算成本的问题，特别是在生物多样性建模等数据稀缺场景中。

Method: 通过对比学习将高分辨率航空影像与植物学数据对齐，采用正则化策略减轻灾难性遗忘，生成可迁移的表征用于下游预测器。

Result: 在植物存在预测、蝴蝶出现建模和土壤营养组丰度估计三个生态任务中，BotaCLIP表征相比DOFA和监督基线均显示出持续改进。

Conclusion: 这项工作展示了领域感知的基础模型适应如何将专家知识注入数据稀缺环境，实现经济高效的表征学习。
...
TL;DR: 提出ART框架，通过查询-响应机制发现和跟踪局部细节动态，实现细粒度动作识别。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉随时间演变的局部区域细微差异，无法有效区分相似动作。

Method: 使用区域特定语义激活模块，以文本约束语义为查询捕获动作相关区域响应，构建动作轨迹，并设计多级轨迹对比约束。

Result: 在广泛使用的动作识别基准测试中优于现有最先进基线方法。

Conclusion: ART框架通过跟踪局部细节动态有效提升了细粒度动作识别性能。
...
TL;DR: 比较三种生成模型：DDPM、CFM和MeanFlow。CFM在CIFAR-10上FID为24.15，优于DDPM的402.98。MeanFlow单步生成FID为29.15，推理时间减少50倍。CFM扩展至图像修复，性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 系统比较三种生成建模范式，探索高效生成方法，特别是单步生成的可能性，并扩展至图像修复任务。

Method: 使用统一的TinyUNet架构（<1.5M参数）在CIFAR-10上实现DDPM、CFM和MeanFlow。CFM采用条件流匹配，MeanFlow通过建模时间间隔内的平均速度实现单步生成。扩展CFM至图像修复，使用四种掩码类型进行掩码引导采样。

Result: CFM在50步采样下FID为24.15，显著优于DDPM（402.98）。MeanFlow单步生成FID为29.15，推理时间减少50倍。图像修复模型PSNR从4.95提升至8.57 dB（+73%），SSIM从0.289提升至0.418（+45%）。

Conclusion: CFM在生成质量上表现最佳，MeanFlow在推理效率上优势明显。针对特定任务（如图像修复）的微调能显著提升性能，证明了任务感知训练的有效性。
...
TL;DR: T3-Tracer是一个针对部分音频伪造检测的新框架，通过联合分析帧级、段级和音频级三个层次来全面检测伪造痕迹，在三个挑战性数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 部分音频伪造通过选择性修改关键帧而保持整体感知真实性，现有方法缺乏分层结构来捕捉不同时间层次的瞬时和持续异常，难以有效检测。

Method: 提出T3-Tracer框架，包含两个核心模块：帧-音频特征聚合模块(FA-FAM)检测帧真实性，结合帧级和音频级时间信息；段级多尺度差异感知模块(SMDAM)检测伪造边界，采用双分支架构建模多尺度时间窗口内的帧特征和帧间差异。

Result: 在三个挑战性数据集上的广泛实验表明，该方法实现了最先进的性能。

Conclusion: T3-Tracer通过分层分析框架有效解决了部分音频伪造检测的挑战，为音频完整性验证提供了有力工具。
...
TL;DR: FIELDS是一个3D人脸重建方法，通过直接3D表情参数监督和辅助情感识别分支，从单张图像生成保留细微情感线索的高保真3D人脸模型。


<details>
  <summary>Details</summary>
Motivation: 现有3D人脸重建方法依赖2D监督且缺乏3D真实数据，往往丢失细微的情感细节，无法准确传达面部表情中的情感信息。

Method: 扩展自监督2D图像一致性线索，结合直接3D表情参数监督和辅助情感识别分支，使用来自自发4D面部扫描的真实表情参数指导编码器，并通过强度感知情感损失鼓励3D表情参数捕捉真实情感内容。

Result: 生成具有高度真实表情的情感丰富人脸模型，显著提高了野外面部表情识别性能，同时保持了自然度。

Conclusion: 双重监督策略弥合了2D/3D领域差距，减轻了表情强度偏差，能够产生保留细微情感线索的高保真3D重建结果。
...
TL;DR: 该论文将可学习多相采样(LPS)扩展到复数神经网络，通过设计投影层从复数到实数空间，在理论层面保证平移等变性和不变性，并在极化合成孔径雷达图像的分类、重建和语义分割任务中验证了性能。
...
Method: 将LPS扩展到复数神经网络，引入从复数到实数的投影层，在Gumbel Softmax之前进行投影，构建了理论保证平移性质的采样操作。

Result: 在极化合成孔径雷达图像的多个计算机视觉任务中验证了扩展方法的有效性，包括分类任务的不变性以及重建和语义分割任务的等变性。

Conclusion: 成功将LPS框架扩展到复数神经网络，通过理论设计和实验验证，为复数域计算机视觉任务提供了平移等变性和不变性的系统解决方案。
...
Method: 提出LaGen框架，包含场景解耦估计模块增强对象级内容交互生成能力，以及噪声调制模块缓解长序列生成中的误差累积。

Result: 在基于nuScenes构建的评估协议上，LaGen全面优于现有最先进的LiDAR生成和预测模型，特别是在后续帧上表现更佳。
...
<details>
  <summary>Details</summary>
Motivation: 基于学习的图像匹配需要大规模、多样化且几何准确的训练数据，但现有3DGS技术存在几何不准确和深度渲染偏差问题，限制了其在对应关系标注中的应用。
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer在单个遥感任务上的出色表现，现在需要开发能够在多任务学习中表现出色的统一模型。多任务学习方法相比单任务方法具有更好的泛化性、可扩展性和实用性。
...
Method: 提出PathMamba混合架构，使用Mamba块追踪道路网络的连续特性以保持拓扑结构，同时集成Transformer块利用全局上下文精炼特征。

Result: 在DeepGlobe Road Extraction和Massachusetts Roads数据集上达到新的最先进水平，显著提升拓扑连续性（APLS指标），同时保持计算竞争力。

Conclusion: PathMamba证明了Mamba和Transformer混合架构在道路分割任务中的有效性，能够在不过度增加计算成本的情况下获得拓扑优越的分割结果。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的3D纹理生成系统存在跨视角不一致问题，即从一个视角看纹理逼真，但在其他视角下无法对齐。这源于注意力模糊问题，无结构的全注意力机制导致几何混淆和不稳定的外观-结构耦合。
...
Conclusion: 通过几何校准的注意力机制，CaliTex有效解决了3D纹理生成中的跨视角不一致问题，使几何一致性成为模型的内在特性而非优化副产品。
...
<details>
  <summary>Details</summary>
Motivation: VGGT模型在3D场景重建中需要全局注意力计算，对于长序列输入存在显著延迟瓶颈，需要高效的加速方法。

Method: 提出头级时间合并(HTTM)，在多头粒度上合并token，保持特征token的唯一性，利用空间局部性和时间对应关系实现高合并比。

Result: 在GPU推理中实现最高7倍加速，性能下降可忽略不计。

Conclusion: HTTM是首个训练自由的3D token合并方法，有效解决了VGGT模型的计算效率问题，为大规模3D场景重建提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要处理两模态对齐，忽略了高阶交互或未能充分保持成对关系，限制了在单模态任务上的有效性。

Method: 扩展传统成对对比目标，增加融合模态对比项，鼓励模态对与第三模态的联合嵌入，捕获如XOR关系等高阶依赖。

Result: 在合成和真实多模态基准测试中，ConFu在检索和分类任务上表现竞争性，支持统一的一对一和一对多检索。

Conclusion: ConFu能够有效捕获高阶依赖关系，同时保持强成对对应，为多模态学习提供了统一的对比框架。
...
TL;DR: SIFT-SNN框架：一种用于交通基础设施结构异常实时检测的低延迟神经形态信号处理管道，结合SIFT空间特征编码和SNN分类，在奥克兰海港大桥数据集上达到92.3%准确率和9.5ms单帧推理时间。


<details>
  <summary>Details</summary>
Motivation: 开发能够在边缘设备上实时、低功耗运行的结构异常检测系统，克服传统CNN方法在延迟和功耗方面的限制，同时保持特征空间定位和决策可解释性。

Method: 集成尺度不变特征变换(SIFT)进行空间特征编码，使用延迟驱动的脉冲转换层和泄漏积分点火(LIF)脉冲神经网络(SNN)进行分类，在奥克兰海港大桥数据集上进行验证。

Result: 分类准确率达到92.3%（±0.8%），单帧推理时间为9.5ms，脉冲活动稀疏度为8.1%，成功部署在消费级系统上作为可推广案例。

Conclusion: SIFT-SNN框架实现了实时低功耗的边缘部署，相比传统CNN方法具有更好的空间特征保持和可解释性，但在未见现场条件下的泛化能力仍需验证。
...
TL;DR: SurgMLLMBench是一个统一的多模态基准测试，专门用于开发和评估交互式多模态LLM在手术场景理解中的应用，包含新收集的MAVIS数据集，整合了像素级器械分割掩码和结构化VQA注释。
...
TL;DR: 提出一种基于多尺度特征融合的点云法向量估计方法，通过特征聚合和跨尺度补偿来解决不同数据或几何形状下邻域大小选择困难的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在确定合适的邻域大小时面临困难，难以准确高效地预测各种点云的法向量，需要解决不同数据或几何形状下的尺度适应问题。

Method: 使用多尺度特征融合，包括特征聚合模块（逐步聚合不同尺度的补丁特征）和特征补偿模块（确保大尺度早期层特征的可重用性），通过补丁特征拟合来近似最优几何描述。

Result: 在合成和真实数据集上实现了最先进的性能，同时减少了网络参数和运行时间。

Conclusion: 基于多尺度特征聚合的近似策略使模型能够实现不同局部补丁的尺度适应，并提供最优特征描述，在准确性和效率方面都表现出色。
...
TL;DR: Endo-G²T提出了一种几何引导和时间感知的训练方案，用于动态内窥镜场景的4D高斯泼溅重建，通过几何先验蒸馏、时间嵌入表示和关键帧约束流式处理，解决了内窥镜视频中的视角依赖效应和几何漂移问题。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频存在强烈的视角依赖效应（如镜面反射、湿润反射和遮挡），纯光度监督与几何不匹配会导致早期几何漂移，错误的形状在密集化过程中被强化且难以纠正。需要为4D高斯泼溅提供早期几何锚定，同时保持时间一致性和效率。

Method: 1. 几何引导先验蒸馏：将置信度门控的单目深度转换为监督，使用尺度不变深度和深度梯度损失，采用预热到上限的调度来软注入先验避免早期过拟合。2. 时间嵌入高斯场：使用转子式旋转参数化在XYZT中表示动态，通过轻量级正则化获得时间一致的几何。3. 关键帧约束流式处理：在最大点数预算下进行关键帧聚焦优化，非关键帧进行轻量级更新。

Result: 在EndoNeRF和StereoMIS-P1数据集上，Endo-G²T在单目重建基线中达到了最先进的结果。

Conclusion: Endo-G²T通过几何引导和时间感知的训练方案，有效解决了动态内窥镜场景中的几何漂移问题，实现了高质量的时间一致重建，同时保持了计算效率。
...
TL;DR: STVG-o1是首个让现成多模态大语言模型无需架构修改即可在时空视频定位任务上达到最先进性能的框架，通过边界框思维链机制和多维强化奖励函数实现。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在时空视频定位任务中表现不佳的问题，主要原因是训练目标不对齐和标准视觉编码器中细粒度区域-词语对齐能力弱。

Method: 提出边界框思维链机制，在最终预测前明确推理时空位置；设计多维强化奖励函数，包含格式、一致性、时间、空间和思考奖励，通过强化微调提供几何感知监督。

Result: 在HCSTVG-v1/v2和VidSTG数据集上评估，STVG-o1在HCSTVG上创下新记录，在HCSTVG-v1上m_tIoU比最佳任务特定方法高7.3%，在VidSTG上与专用模型相当，大幅超越所有现有MLLM方法。
...
TL;DR: Monet是一个训练框架，使多模态大语言模型能够在潜在视觉空间中直接推理，通过生成连续嵌入作为中间视觉思维。该框架解决了潜在视觉推理的两个核心挑战：潜在视觉对齐的高计算成本和潜在嵌入监督不足。


<details>
  <summary>Details</summary>
Motivation: 现有方法在抽象视觉思维方面存在不足，其灵活性受限于外部工具。需要开发能够直接在潜在视觉空间中进行推理的方法，实现更接近人类思维的视觉推理能力。

Method: 采用三阶段基于蒸馏的监督微调(SFT)管道，构建了包含12.5万条真实世界、图表、OCR和几何推理链的高质量数据集。提出了VLPO(视觉潜在策略优化)强化学习方法，将潜在嵌入显式纳入策略梯度更新。

Result: Monet-7B模型在真实世界感知和推理基准测试中表现一致提升，在具有挑战性的抽象视觉推理任务上展现出强大的分布外泛化能力。

Conclusion: 该工作为视觉潜在推理的未来发展提供了重要见解，通过直接在潜在空间中进行推理，实现了更灵活和强大的视觉推理能力。
...
<details>
  <summary>Details</summary>
Motivation: VAR模型在图像生成中虽然质量优秀，但存在多样性不足的问题，即使对于简单提示也会生成几乎相同的图像，这个问题在现有研究中被忽视

Method: 1. 在文本嵌入中注入噪声以增强多样性；2. 提出尺度旅行技术，使用多尺度自编码器提取粗尺度标记，在中间阶段恢复生成以保持图像质量

Result: 结合文本嵌入噪声注入和尺度旅行精炼，在多样性-质量权衡中达到了新的帕累托前沿，显著提升了多样性同时最小化图像质量下降

Conclusion: DiverseVAR框架有效解决了VAR模型的多样性限制问题，为测试时多样性增强提供了一种无需重新训练的高效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感变化描述方法通常使用CNN/Transformer提取视觉特征或引入辅助任务，但存在区域感知弱和时间对齐有限的问题。
...
Conclusion: 提出的方法通过SAM基础模型和知识图谱的融合，显著提升了遥感变化描述的性能。
...
TL;DR: E-M3RF是一个等变多模态3D重组框架，通过结合几何特征和颜色特征，使用SE(3)流匹配来预测碎片重组所需的变换，在几何特征不足或模糊的情况下显著提升重组精度。
...
Conclusion: 多模态特征结合和等变表示能有效提升3D重组性能，特别是在几何特征不足的情况下。
...
Result: 在公共基准和专有电机装配数据集上验证了有效分割工作场所人类关键任务的能力，VLM评估确认了发现动作基元的语义一致性。

Conclusion: 这是首个从非结构化工业视频中自动提取和组织VLA预训练数据的端到端系统，为制造业具身AI集成提供了可扩展方案。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机产生的异步事件流在空间上稀疏但时间上密集，现有方法难以解决空间稀疏性导致的欠采样问题。

Method: 使用超图引导的时空事件流补全机制，通过超图连接事件标记并传递上下文信息，可融合RGB标记实现多模态补全，然后通过自注意力聚合不同时间步的超图节点信息。
...
Method: 1) 线性混合架构去噪器平衡生成效率与质量；2) 时间步蒸馏将采样步骤从20+压缩到2步；3) 移动端注意力优化使注意力操作速度提升2倍。

Result: 首次在移动设备上实现快速720p图像到视频生成，单步条件下每帧生成时间小于100ms，质量与现有模型相当。

Conclusion: MobileI2V证明了在移动设备上实现高质量实时视频生成的可行性，为移动端多媒体应用开辟了新可能。
...
TL;DR: 提出一种频率感知的令牌缩减策略，通过区分高频和低频令牌来缓解注意力机制中的秩崩溃问题，在降低计算复杂度的同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有令牌缩减方法忽视了自注意力机制的频率特性，如秩崩溃和过度平滑现象，导致性能损失。

Method: 将令牌划分为高频令牌和低频令牌，选择性保留高频令牌，同时将低频令牌聚合成紧凑的直流令牌以保留重要低频成分。

Result: 实验表明该方法显著提高准确率，减少计算开销，并有效缓解秩崩溃和过度平滑问题。

Conclusion: 频率感知的令牌缩减策略是解决Vision Transformer计算效率问题的有效方法，同时揭示了现有方法的频率特性局限。
...
TL;DR: 提出Merge-and-Bound（M&B）方法，通过在参数空间中直接操作模型权重来进行类增量学习，包含任务间和任务内权重合并，以及有界更新技术来减少灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 解决类增量学习中的灾难性遗忘问题，通过直接优化模型权重来有效保留先前任务的知识。

Method: 使用两种权重合并：任务间权重合并（平均所有先前阶段模型权重）和任务内权重合并（组合当前阶段模型参数），配合有界更新技术限制累积更新。

Result: 在标准CIL基准测试中表现出色，性能优于最先进方法。

Conclusion: M&B方法能有效减少灾难性遗忘，且无需修改架构组件或学习目标，可与现有CIL方法无缝集成。
...
TL;DR: 提出基于交叉注意力的非局部知识蒸馏框架CanKD，通过交叉注意力机制让学生特征图的每个像素动态考虑教师特征图的所有像素，实现更全面的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 传统自注意力蒸馏方法独立对齐师生特征图，无法充分捕捉像素间关系。需要一种能够动态考虑全局特征关系的蒸馏方法。

Method: 使用交叉注意力机制，让学生特征图的每个像素都能动态关注教师特征图的所有像素，仅添加一个额外的损失函数。
...
TL;DR: 本文系统研究了深度伪造检测方法中各种设计选择对性能的影响，发现实现细节比核心设计更重要，并确定了能持续改进检测性能的最佳实践。
...
Conclusion: 通过系统分析设计选择的影响，建立了稳健的、架构无关的最佳实践，为未来深度伪造检测系统的设计和开发提供了指导。
...
TL;DR: 提出了一种用于抗核抗体(ANA)检测的新型多实例多标签学习框架，通过实例采样器、概率伪标签分配器和自定步调学习率系数来处理临床显微镜图像的复杂性，在ANA数据集上实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: ANA检测对于诊断自身免疫疾病至关重要，但传统手动检测方法缓慢且需要专业训练。虽然机器学习和深度学习已实现自动化，但在真实临床环境中ANA检测面临多实例多标签学习的独特挑战，现有方法难以处理未预处理的显微镜图像。

Method: 受人类标注逻辑启发，提出包含三个任务特定组件的框架：实例采样器通过建模模式置信度来抑制低置信度实例；概率伪标签分配器基于实例可区分性自适应分配标签；自定步调学习根据经验标签观察调整训练。支持端到端优化。

Result: 在ANA数据集上，相比最佳现有方法，F1-Macro提升7.0%，mAP提升12.6%，创下新的最先进结果。在三个公共医疗MIML基准测试中，所有关键指标均排名前二，汉明损失和单错误率分别降低18.2%和26.9%。

Conclusion: 该框架成功克服了传统MIML方法的局限性，能够有效处理未预处理的临床显微镜图像，为ANA检测提供了一种高效准确的自动化解决方案，在医疗多实例多标签学习任务中表现出卓越性能。
...
TL;DR: 提出一种基于专家集成框架的高效遥感基础模型构建方法，通过轻量级任务专用专家模型替代传统大规模模型，实现计算效率和可持续性。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型主要依赖模型规模和数据集扩展，导致计算资源和碳排放过高，限制了可访问性，且不符合可持续AI原则。

Method: 采用专家集成框架，将训练过程分解为轻量级的任务专用ConvNeXtV2专家模型，支持冻结和重用，具有模块化特性。

Result: 该方法在效率、可解释性和可扩展性方面具有显著优势，支持联邦训练、剪枝和持续专家集成，适用于资源受限环境。

Conclusion: 该框架为构建可扩展且高效的遥感基础模型开辟了新方向，特别适合协作和资源受限场景。
...
TL;DR: 该研究提出了一种基于定量指标的序列图像生成方法，通过整合年龄缩放因子来生成年龄特异性MRI图像，用于预测阿尔茨海默病的进展阶段。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的及时识别对个性化治疗至关重要，但现有方法在输入序列时间间隔不规则时难以准确表征疾病特征，需要改进图像生成技术。

Method: 开发了基于定量指标的序列图像生成方法，整合年龄缩放因子生成年龄特异性MRI图像，并使用年龄缩放像素损失来增强MRI图像的迭代生成。

Result: 消融研究表明定量指标的加入显著提高了MRI图像合成的准确性，结构相似性指数达到0.882的峰值，表明合成图像具有高度相似性。
...
TL;DR: 提出了PRFL框架，在潜在空间进行偏好优化，解决了视频生成中Reward反馈学习的内存消耗大、训练时间长的问题，显著提升了与人类偏好的对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有视频奖励模型依赖像素空间输入，导致ReFL优化只能在去噪后期进行，内存开销大、训练时间长，且缺乏早期监督，无法优化运动动态和结构连贯性。

Method: 利用预训练视频生成模型在噪声潜在空间进行奖励建模，提出PRFL框架，在潜在空间进行偏好优化，实现整个去噪链的高效梯度反向传播而无需VAE解码。

Result: PRFL显著提升了与人类偏好的对齐效果，同时大幅减少了内存消耗和训练时间，相比RGB ReFL有显著优势。

Conclusion: 潜在空间的奖励建模和优化是视频生成中更高效的偏好对齐方法，PRFL框架为视频生成的对齐优化提供了新的解决方案。
...
TL;DR: UAVLight是一个用于光照鲁棒3D重建的基准数据集，通过在不同固定时间重复飞行捕获场景，提供自然光照变化下的多视图数据。


<details>
  <summary>Details</summary>
Motivation: 解决多视图3D重建中光照不一致的挑战，特别是在无人机应用中，光照变化会导致几何漂移、颜色不一致和阴影印记等问题。

Method: 在可重复的地理参考飞行路径上，在一天中的多个固定时间捕获每个场景，在一致的几何、校准和视点下产生自然光照变化。

Result: 创建了一个受控但真实的基准，用于评估光照鲁棒的3D重建方法。

Conclusion: UAVLight为在真实室外环境中开发一致、忠实和可重光照的重建方法提供了可靠基础。
...
TL;DR: 提出MRPD框架，通过多模态教师-学生蒸馏方法提升3D点云模型的对抗鲁棒性，在训练阶段学习轻量级提示，无需推理时额外计算成本。
...
Method: 使用多模态教师-学生框架，通过三个不同教师模型（视觉深度投影模型、高性能3D模型、文本编码器）的特征对齐来学习轻量级提示，采用置信度门控机制动态平衡多模态输入贡献。

Result: 在广泛的白盒和黑盒攻击下显著优于现有防御方法，在干净数据上也能获得更好性能。

Conclusion: MRPD为构建鲁棒3D视觉系统提供了新的实用范式，通过高效利用多模态知识实现对抗防御。
...
TL;DR: 提出了一种结合2D/3D地标配准的U-Net训练框架，用于骨盆透视图像的地标检测，解决了传统方法假设固定AP视图的限制。
...
Method: 将2D/3D地标配准集成到U-Net地标预测模型的训练中，比较基线U-Net、使用姿态估计损失的U-Net训练以及使用姿态估计损失微调的U-Net。

Result: 在患者姿态可变的真实手术条件下评估地标检测精度差异。

Conclusion: 提出的框架能够处理骨盆透视图像中常见的姿态变化问题，提高地标检测的鲁棒性。
...
Method: 提出Harmony框架：1）跨任务协同训练利用音频驱动视频和视频驱动音频生成任务的强监督信号；2）全局-局部解耦交互模块实现高效精确的时序风格对齐；3）同步增强CFG在推理时显式隔离和放大对齐信号。

Result: 大量实验表明，Harmony在生成保真度和细粒度音频-视频同步方面显著优于现有方法，建立了新的最先进水平。

Conclusion: Harmony通过机制性地强制音频-视频同步，有效解决了联合扩散过程中的核心挑战，为同步音频-视觉内容生成提供了强大解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 口腔癌在全球非常普遍，但由于口腔内良性、癌前病变和恶性病变在视觉上相似，往往在晚期才被诊断。早期实施计算机辅助诊断系统可以显著改善临床结果。

Method: 结合分层数据分割、先进的数据增强和过采样技术来处理有限且不平衡的数据集，构建深度学习多分类器。

Result: 实验结果显示达到83.33%准确率、89.12%精确率和77.31%召回率，优于现有最先进方法，在少数类分类方面表现显著。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在帧级保真度上表现良好，但在运动连贯性、动态性和真实感方面存在不足，常产生抖动、重影或不合理动态。标准的去噪MSE目标缺乏对时间一致性的直接监督。

Method: 基于3步蒸馏的视频扩散模型，训练基于DiT的光学流判别器来区分真实与生成的运动，结合分布匹配正则化器保持视觉保真度。

Result: 在Wan2.1-T2V-1.3B上，MoGAN显著提升运动质量：VBench运动得分比50步教师模型提升+7.3%，比3步DMD模型提升+13.3%；VideoJAM-Bench运动得分分别提升+7.4%和+8.8%，同时保持可比或更好的美学和图像质量得分。人类研究显示MoGAN在运动质量上更受偏好。
...
TL;DR: 提出了一种自提示的点监督框架，通过Refine-Requery-Reinforce循环机制，仅使用稀疏点标注就能让SAM模型适应遥感图像分割任务，在多个基准数据集上表现优于原始SAM和其他点监督方法。


<details>
  <summary>Details</summary>
Motivation: SAM模型在自然图像上表现出色，但在遥感图像上性能不佳，主要由于领域偏移和密集标注稀缺。需要一种仅使用稀疏点标注就能有效适应遥感图像的方法。

Method: 采用Refine-Requery-Reinforce循环：从初始点生成粗伪掩码(Refine)，用自构建的框提示改进(Requery)，通过嵌入对齐减少确认偏差(Reinforce)。无需完整掩码监督，通过自引导提示适应逐步提升分割质量。
...
Method: 1. 开发新颖的对抗性获取函数来选择信息丰富的代表性样本；2. 引入双向稳定GCN架构，改善环境空间与潜在空间的映射。

Result: 在两个具有挑战性的骨架动作识别基准测试中，相比先前工作取得了显著改进。

Conclusion: 所提出的标签高效GCN模型能够有效减少对标注数据的依赖，在骨架动作识别任务中表现出色。
...
TL;DR: Qwen3-VL是Qwen系列中最强大的视觉语言模型，在多项多模态基准测试中表现优异。它原生支持256K token的混合上下文，无缝集成文本、图像和视频，提供密集和MoE两种架构变体。


<details>
  <summary>Details</summary>
Motivation: 开发更强大的多模态模型，提升纯文本理解、长上下文理解和多模态推理能力，满足现实工作流程中图像推理、智能决策和多模态代码智能的需求。
...
TL;DR: 提出一种支持用户通过少样本学习纠正AI误分类的新系统，结合服务器端基础模型训练和设备端原型分类，实现高效错误纠正而无需模型重训练


<details>
  <summary>Details</summary>
Motivation: AI模型在日常设备中的普及面临预测错误影响用户体验的挑战，现有解决方案主要关注错误检测而缺乏高效的纠正机制，特别是在资源受限设备上
...
TL;DR: CaFlow是一个用于长时动作质量评估的统一框架，通过因果反事实正则化和双向时间条件流来建模长期时间动态并消除混杂因素影响。


<details>
  <summary>Details</summary>
Motivation: 长时动作质量评估（如花样滑冰、艺术体操）需要建模长时间动态，同时保持对上下文混杂因素的鲁棒性。现有方法依赖昂贵标注或单向时间建模，容易受到伪相关性和不稳定长期表示的影响。

Method: 提出CaFlow框架：1）因果反事实正则化（CCR）模块以自监督方式解耦因果和混杂特征，通过反事实干预增强因果鲁棒性；2）BiT-Flow模块通过循环一致性约束建模前向和后向动态，产生更平滑连贯的表示。

Result: 在多个长时AQA基准测试上的广泛实验表明，CaFlow达到了最先进的性能。

Conclusion: CaFlow通过整合因果反事实解耦和双向时间建模，有效解决了长时动作质量评估中的混杂因素和长期表示稳定性问题。
...
TL;DR: Multi-Crit基准测试评估多模态模型作为评估者的能力，重点关注其遵循多样化细粒度标准的能力，发现现有模型在遵循多元标准方面仍有困难。
...
Method: 开发Multi-Crit基准测试，通过严格的数据整理流程收集具有多标准人工标注的挑战性响应对，并引入三个新指标系统评估多元依从性、标准切换灵活性和识别标准级偏好冲突的能力。

Result: 对25个LMM的综合分析显示：1)专有模型在保持对多元标准的一致性依从方面仍有困难；2)开源模型在灵活遵循多样化标准方面更落后；3)批评微调增强视觉基础但无法泛化到多元标准级判断。
...
TL;DR: ADVLA是一种针对视觉-语言-动作模型的对抗攻击框架，通过在视觉编码器到文本特征空间的投影上直接应用扰动，实现高效、稀疏且几乎不可察觉的攻击。
...
Result: 在L∞=4/255约束下，ADVLA结合Top-K掩码修改少于10%的补丁，攻击成功率接近100%，扰动集中在关键区域，单步迭代仅需0.06秒。
...
TL;DR: 提出了一种混合深度学习模型，结合U-Net分割和DenseNet-VGG分类网络，用于脑胶质瘤的早期准确诊断。该模型在3D MRI数据上实现了98%的Dice系数分割精度和99%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 胶质瘤具有高死亡率，早期准确诊断对治疗干预至关重要。传统方法在分割和分类精度方面存在局限，需要更先进的深度学习解决方案。

Method: 开发了混合深度学习框架：U-Net分割网络精确定位3D MRI中的肿瘤区域；混合DenseNet-VGG分类网络结合多头注意力和空间通道注意力机制，聚焦临床相关特征。采用归一化、重采样和数据增强等预处理步骤。

Result: 在物理测试中，分割Dice系数达到98%，分类准确率达到99%，优于传统CNN模型和无注意力方法。注意力机制增强了临床重要特征的优先级和可解释性。

Conclusion: 该框架在胶质瘤及时可靠诊断和分级方面具有巨大潜力，有助于改善患者治疗规划。多注意力机制提高了准确性和临床实用性。
...
TL;DR: 本文首次系统研究仅通过相机轨迹（而非像素）能否感知视频内容，提出CamFormer对比学习框架，证明相机轨迹是揭示视频内容的有效信号。


<details>
  <summary>Details</summary>
Motivation: 探索仅通过相机运动轨迹（而非视觉像素）能否理解视频内容，验证"如何移动"能否揭示"在做什么"或"观察什么"。

Method: 提出对比学习框架训练CamFormer编码器，将相机姿态轨迹投影到联合嵌入空间，与自然语言对齐。

Result: 相机轨迹是异常信息丰富的信号，能够有效揭示视频内容；CamFormer表示在不同下游任务中表现优异，且对多种相机姿态估计方法具有鲁棒性。
...
TL;DR: Canvas-to-Image是一个统一框架，将文本提示、主题参考、空间布局、姿态约束和布局标注等多种控制信号整合到单一画布界面中，实现高保真度的组合和多模态图像生成。


<details>
  <summary>Details</summary>
Motivation: 现代扩散模型在生成高质量多样化图像方面表现出色，但在同时处理多种控制信号（如文本提示、主题参考、空间布局等）时仍面临高保真度组合和多模态控制的挑战。

Method: 提出将多样控制信号编码为单一复合画布图像，使模型能够直接进行集成视觉空间推理；构建多任务数据集并提出多任务画布训练策略，在统一学习范式中联合优化扩散模型。

Result: 在多人组合、姿态控制组合、布局约束生成和多控制生成等挑战性基准测试中，Canvas-to-Image在身份保持和控制遵循方面显著优于最先进方法。

Conclusion: Canvas-to-Image通过统一框架实现了对多种控制信号的集成理解，在复杂多控制场景下表现出优异的泛化能力和控制精度。
...
### [164] [Automated Histopathologic Assessment of Hirschsprung Disease Using a Multi-Stage Vision Transformer Framework](https://arxiv.org/abs/2511.20734)
...
<details>
  <summary>Details</summary>
Motivation: Hirschsprung病的诊断依赖于肌间神经丛中神经节细胞的正确识别，需要开发自动化方法来减少观察者间差异并提高诊断准确性。

Method: 采用Vision Transformer (ViT-B/16)的三阶段分割框架：1) 分割肌层 2) 分割肌间神经丛 3) 在解剖有效区域内识别神经节细胞，使用5折交叉验证和分辨率特异性分块策略。
...
### [165] [Prompt-Aware Adaptive Elastic Weight Consolidation for Continual Learning in Medical Vision-Language Models](https://arxiv.org/abs/2511.20732)
...
<details>
  <summary>Details</summary>
Motivation: 医学AI系统在临床部署中面临灾难性遗忘挑战，需要学习新的成像协议同时保留先前的诊断能力，特别是医学视觉-语言模型需要保持医学图像与临床术语之间的复杂跨模态对齐。

Method: 基于提示的自适应弹性权重巩固方法，系统地将模型参数按功能角色分类，包括视觉描述、空间引导和医学语义信息处理，采用自适应Fisher信息计算与梯度稳定性分析，开发基于医学术语密度的加权复杂度指标。

Result: 在五个医学影像数据集上的实验结果表明，PA-EWC相比基线方法减少灾难性遗忘达17.58%，在胸部X光病理定位性能提升4.30%，在息肉分割任务上提升6.06%。

Conclusion: PA-EWC通过提示引导的参数专业化有效解决了医学AI系统中的灾难性遗忘问题，为临床环境中的持续学习提供了有效解决方案。
...
### [166] [AV-Edit: Multimodal Generative Sound Effect Editing via Audio-Visual Semantic Joint Control](https://arxiv.org/abs/2511.21146)
...
Conclusion: AV-Edit通过多模态方法有效解决了音效编辑的局限性，实现了高质量的细粒度音频编辑。
...
TL;DR: 该论文提出通过无监督分割、均匀平均池化等技术压缩LALMs的音频token数量，并使用低秩适配器微调来缓解性能下降，在语音识别和语音翻译任务上实现接近帧级LALMs的性能，同时将输入音频token数量减少三倍。


<details>
  <summary>Details</summary>
Motivation: 大型音频语言模型在处理长音频和部署到资源受限平台时面临注意力二次复杂度和音频信号高token率的可扩展性限制。

Method: 采用无监督分割、均匀平均池化等技术压缩音频编码器生成的token数量，使用低秩适配器进行微调以缓解性能下降。

Result: 压缩后的LALMs在语音识别和语音翻译任务上能实现接近帧级LALMs的性能，同时将输入音频token数量减少三倍。

Conclusion: 提出的压缩方法能有效减少LALMs的计算需求，同时保持接近原始模型的性能，为长音频处理和边缘部署提供了可行方案。
...
Conclusion: 通过依赖丰富的平行文本而非难获取的平行语音，为更广泛语言的高质量、保留说话人特征的语音翻译提供了可扩展路径。
...
### [169] [Zipf Distributions from Two-Stage Symbolic Processes: Stability Under Stochastic Lexical Filtering](https://arxiv.org/abs/2511.21060)
*Vladimir Berman*

Main category: stat.ME

TL;DR: 该研究提出一个纯粹基于几何机制的符号模型来解释Zipf定律，无需语言学元素。通过有限字母表生成单词，几何长度分布和指数力相互作用产生幂律秩频曲线。


<details>
  <summary>Details</summary>
Motivation: Zipf定律在语言中的起源存在争议，缺乏明确解释。本研究旨在探索是否可以通过纯粹的几何约束机制来解释Zipf-like行为，而不依赖语言学的特定元素。

Method: 使用完全组合词模型（FCWM），从有限字母表生成单词，产生几何分布的词长。通过指数力相互作用，形成幂律秩频曲线，关键参数是字母表大小和空白符号概率。

Result: 模拟结果支持理论预测，与英语、俄语和混合体裁数据匹配良好。模型表明Zipf型定律可以从几何约束中自然产生。

Conclusion: Zipf型定律可能源于几何约束机制，而非通信效率。符号模型为理解语言统计规律提供了新的非语言学视角。
...
### [170] [TALES: A Taxonomy and Analysis of Cultural Representations in LLM-generated Stories](https://arxiv.org/abs/2511.21322)
*Kirti Bhagat,Shaily Bhatt,Athul Velagapudi,Aditya Vashistha,Shachi Dave,Danish Pruthi*

Main category: cs.HC

TL;DR: TALES评估框架揭示LLM生成故事中普遍存在文化误传问题，88%的故事包含文化不准确性，特别是在中低资源语言和印度城乡结合部地区。


<details>
  <summary>Details</summary>
Motivation: 评估AI聊天机器人在开放创作任务中对多元文化的表征质量，特别是针对印度多样文化身份的文化误传问题。

Method: 开发TALES-Tax文化误传分类法，通过焦点小组和调查收集印度本土参与者见解，然后进行大规模标注研究（2,925个标注，108名标注者）。

Result: 88%的生成故事存在文化误传，中低资源语言和城乡结合部地区错误更普遍；模型具备文化知识但生成时仍出现误传。

Conclusion: 需要改进LLM的文化表征能力，特别是对中低资源语言和特定地区的文化理解，TALES-QA可作为评估模型文化知识的标准工具。
...
Method: STAR技术让用户在手掌皮肤上叠加的虚拟QWERTY键盘上进行双拇指打字，利用用户对智能手机打法的熟悉度。
...
TL;DR: ENACT是一个评估视觉语言模型是否展现具身认知的基准测试，通过世界建模任务来检验模型从自我中心交互中理解环境的能力。


<details>
  <summary>Details</summary>
Motivation: 研究现代视觉语言模型是否在非具身训练后仍能表现出具身认知的特征，即通过感知运动交互而非被动观察来发展智能。

Method: 设计ENACT基准测试，包含前向世界建模（根据动作重排观察序列）和逆向世界建模（根据观察重排动作序列）两个任务，使用部分可观察马尔可夫决策过程框架，从机器人仿真中生成8,972个问答对。

Result: 前沿视觉语言模型与人类存在性能差距，且随着交互时间跨度增大而扩大；模型在逆向任务上表现优于前向任务，并表现出人类中心偏见（如偏好右手动作）。

Conclusion: 现代视觉语言模型在具身认知能力上仍与人类有显著差距，需要进一步发展从自我中心交互中建模世界的能力。
...
TL;DR: Prune4Web是一种新的网页自动化方法，通过程序化修剪DOM树来解决LLM处理大型DOM结构效率低下的问题，实现25-50倍的候选元素减少，显著提升动作定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有LLM网页代理在处理包含10,000-100,000个token的大型DOM结构时效率低下，通常采用粗糙的DOM截断方法，可能导致关键信息丢失，或使用低效启发式方法，无法在精度和可扩展性之间取得平衡。

Method: 提出DOM树修剪编程范式，让LLM生成可执行的Python评分脚本，基于分解的子任务语义线索动态过滤DOM元素，将DOM处理从资源密集的LLM读取转变为高效的程序化修剪。

Result: 在低级别接地任务中，准确率从46.8%大幅提升至88.28%，实现了25-50倍的候选元素减少，有效缓解注意力稀释问题。

Conclusion: Prune4Web通过程序化修剪DOM树的方法，在真实网页自动化中表现出最先进的性能，为处理复杂网页界面提供了高效且精确的解决方案。
...
TL;DR: 提出了第一个计算保证最优组合解释的理论框架，通过分解空间对齐因素、设计启发式估计和高效算法，解决了现有方法无法保证最优性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有组合解释方法使用束搜索无法提供理论最优性保证，不清楚当前解释与真正最优解的接近程度。

Method: 提出包含三个核心组件的框架：(i)识别影响空间对齐因素的分解方法，(ii)在搜索任何阶段估计对齐的启发式，(iii)在可行时间内计算最优组合解释的算法。

Result: 在计算机视觉和卷积神经网络场景中，当涉及重叠概念时，10-40%的束搜索解释是次优的。提出的束搜索变体在运行时间上匹配或优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法虽然基于多模态预训练，但推理仍局限于固定类别名称，存在多模态训练与单模态推理的差距。文本空间尚未充分探索，改进文本表示能显著提升性能。

Method: 提出OVOD-Agent框架，将文本优化过程扩展为可解释的视觉思维链。使用弱马尔可夫决策过程建模视觉上下文转换，包含八个状态空间。引入Bandit模块生成探索信号，集成马尔可夫转移矩阵与Bandit轨迹进行自监督奖励模型优化。

Result: 在COCO和LVIS数据集上的实验表明，OVOD-Agent为各种OVOD骨干网络提供了一致的性能提升，特别是在稀有类别上效果显著。

Conclusion: 提出的框架通过主动视觉推理和自我进化检测机制，有效提升了开放词汇目标检测的性能，特别是在处理稀有类别时表现出色。
...
TL;DR: 提出了TAGFN数据集，这是一个用于图异常检测（特别是假新闻检测）的大规模真实文本属性图数据集，旨在填补LLM在图异常检测领域缺乏可靠基准的空白。
...
Method: 构建TAGFN数据集，这是一个专门用于异常检测的大规模真实世界文本属性图数据集，支持传统方法和LLM方法的评估，并促进LLM通过微调发展错误信息检测能力。

Result: TAGFN数据集已公开发布在Hugging Face和GitHub上，为社区提供了有价值的资源，能够推动基于图的异常检测和可信AI的进展。

Conclusion: TAGFN数据集填补了图异常检测领域的重要空白，将成为促进稳健图异常检测和可信AI发展的宝贵资源。
...
### [177] [Multi-Reward GRPO for Stable and Prosodic Single-Codebook TTS LLMs at Scale](https://arxiv.org/abs/2511.21270)
*Yicheng Zhong,Peiji Yang,Zhisheng Wang*

Main category: cs.SD

TL;DR: 提出了多奖励组相对策略优化(GRPO)框架，通过强化学习直接优化单码本TTS大语言模型的token生成策略，解决了韵律不稳定、说话人漂移和自然度下降的问题。


<details>
  <summary>Details</summary>
Motivation: 单码本TTS LLMs虽然结构紧凑且可流式传输，但存在韵律不稳定、说话人漂移和自然度下降的问题，需要改进其token生成策略。

Method: 使用多奖励GRPO框架，包含三个基于规则的奖励：长度惩罚用于时长一致性、熵正则化奖励用于解码稳定性、LLM标注的韵律对齐奖励用于节奏监督。在韵律奖励中，外部推理LLM通过上下文学习预测多个可能的停顿结构。

Result: 该方法在单码本TTS LLMs中持续提升了韵律稳定性、说话人相似度和整体语音自然度。附加流匹配解码器后获得进一步增益，表明强化学习优化了内在的自回归策略。

Conclusion: 多奖励GRPO框架有效解决了单码本TTS LLMs的韵律问题，通过可扩展性分析验证了该方法在不同数据规模和模型尺度下的有效性。
...
TL;DR: 本文针对PPO在训练多轮对话和推理任务中的LLM时存在的不稳定性和崩溃问题，提出了两种稳定化技术：轮级重要性采样和裁剪偏差校正，并开发了三种变体（Turn-PPO、S-PPO、ST-PPO）来有效解决这些问题。


<details>
  <summary>Details</summary>
Motivation: PPO在多轮对话和推理任务中训练LLM时性能不稳定且容易崩溃，主要问题包括：令牌级重要性采样与多轮环境的自然粒度不匹配，以及离策略样本导致的优势估计不准确。

Method: 提出两种互补的稳定化技术：1) 轮级重要性采样，使优化与多轮推理的自然结构对齐；2) 裁剪偏差校正，通过降低不可靠的离策略样本权重来归一化梯度。基于这两种技术的组合，开发了Turn-PPO、S-PPO和ST-PPO三种变体。

Result: 在多轮搜索任务（通用QA、多跳QA、医学多选题QA）上的实验表明，ST-PPO和S-PPO能有效防止大模型训练中的性能崩溃，在整个优化过程中保持较低的裁剪比率，并比标准令牌级PPO获得更高的任务性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统线性状态空间模型(SSM)虽然计算效率高，但只能维护有损的过去信息摘要，在召回导向任务中表现较差。需要一种既能充分利用完整历史信息，又保持SSM效率的方法。

Method: 1. 基于卡尔曼滤波器思想迭代求解在线岭回归问题；2. 采用自适应正则化策略和输入相关门控控制条件数；3. 使用切比雪夫迭代替代传统迭代求解器以提高低精度环境稳定性；4. 开发硬件感知的分块实现和自定义反向传播核。

Result: 在短上下文任务中超越现有SSM层(Mamba2、GLA等)；在128k令牌的长上下文任务中，在RAG和LongQA任务上比衰减记忆基线相对提升超过10%。

Conclusion: GKA成功弥合了SSM在召回任务中的性能差距，同时保持了计算效率，在长短上下文任务中都表现出色。
...
TL;DR: 本文提出了一种统一框架，通过双层数据选择优化离线数据筛选和在线自优化生成，为每个问题和回答分配学习权重，提升LLM在下游任务中的微调性能。


<details>
  <summary>Details</summary>
Motivation: 离线数据选择和在线自优化生成是提升LLM在特定下游任务中性能的关键步骤，但现有方法缺乏统一的理论框架和优化视角。

Method: 采用双层数据选择进行离线数据筛选，将在线自优化生成视为模型适应步骤，为每个问题和回答分配显式或隐式的学习数据权重。

Result: 理论证明了双层数据选择框架的有效性，在质量增强和安全感知的LLM微调实验中验证了其性能优于未过滤的直接混合基线。

Conclusion: 结合离线数据和验证加权的在线生成，该方法能有效提升微调性能，为LLM数据优化提供了统一的优化视角。
...
TL;DR: 提出了一个插件框架来纠正LLM评估中的偏差并构建置信区间，同时引入自适应算法优化校准样本分配以减少不确定性。


<details>
  <summary>Details</summary>
Motivation: LLM作为评估者存在噪声和偏差，现有偏差校正方法假设已知模型的精确特异性和敏感性，且通常只有这些值的估计，缺乏构建置信区间的方法。

Method: 开发了简单的插件框架进行偏差校正，构建反映测试和校准数据集不确定性的置信区间，并提出自适应算法高效分配校准样本量。

Result: 实现了实用且统计上可靠的LLM评估，能够减少准确性估计的不确定性。

Conclusion: 该框架为LLM评估提供了偏差校正和不确定性量化的实用解决方案，通过自适应校准进一步提升了评估效率。
...
TL;DR: 提出了BanglaASTE框架，这是首个用于孟加拉语情感三元组提取的混合分类框架，结合了图基方面-观点匹配和语义相似度技术，在自建数据集上实现了89.9%的准确率。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语的细粒度情感分析研究严重不足，缺乏全面的数据集和专门的三元组提取框架，阻碍了该语言在电子商务和社交媒体分析中的应用。

Method: 开发了混合分类框架，采用图基方面-观点匹配与语义相似度技术相结合的方法，并实现了集成模型，将BanglaBERT上下文嵌入与XGBoost提升算法结合。

Result: 集成方法在所有评估指标上显著优于基线模型，达到89.9%的准确率和89.1%的F1分数，有效解决了孟加拉语文本处理中的非正式表达、拼写变异和数据稀疏等挑战。
...
TL;DR: 提出了两种基于贝叶斯惊喜信号的动态计算路由Transformer架构：SDT通过决策层和动态层交替实现空间计算路由，STT在时间维度上通过预测残差更新来动态执行或跳过TF块，显著减少了计算和KV缓存需求。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer架构的刚性、均匀计算分配限制了效率和可扩展性，特别是在大规模模型和长序列处理中。需要动态路由计算来优化资源使用。

Method: SDT：在解码器堆栈中交替使用决策层（计算完整后验和轻量先验）和动态层（基于贝叶斯惊喜的Top-K路由）；STT：通过转移网络预测残差更新，形成时间变化假设，动态执行或跳过TF块。

Result: 两种架构在训练过程中都表现出从新颖性到预测驱动的门控转变，与惊喜原则一致。在降低容量的同时，减少了75%的自注意力计算和50%的KV缓存需求。

Conclusion: 这些架构为条件计算提供了一个灵活的效率框架，为构建更高效的模型铺平了道路，同时初步揭示了计算-精度权衡的见解。
...
TL;DR: CHiQPM模型提供全面的全局和局部可解释性，通过对比解释多数类别和分层解释实现人类推理式的理解，同时保持与非可解释模型相当的准确性。


<details>
  <summary>Details</summary>
Motivation: 在安全关键领域需要可信AI，全局可解释模型配合详细局部解释对于支持人类专家决策至关重要。

Method: 提出校准分层QPM（CHiQPM），提供对比性全局解释和分层局部解释，内置可解释的Conformal预测方法。

Result: CHiQPM作为点预测器达到最先进准确率，保持非可解释模型99%的准确率，同时其校准集预测在效率上与其他CP方法竞争。

Conclusion: CHiQPM在保持高准确性的同时实现了显著的可解释性改进，为人类-AI互补性铺平了道路。
...
Method: 采用CNN-LSTM混合架构，CNN用于空间特征提取，LSTM用于捕捉时间依赖性，结合软件定义无线电平台，使用RadioML2018数据集和自定义数据集进行训练，信噪比范围0-30dB。
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉国自然灾害频发，需要实时监测和快速响应系统。现有研究缺乏针对孟加拉语的多模态灾害分析。

Method: 构建包含5,037条孟加拉语社交媒体帖子的新数据集，集成Transformer文本编码器（BanglaBERT、mBERT、XLM-RoBERTa）和CNN骨干网络（ResNet50、DenseNet169、MobileNetV2），采用早期融合策略。
...
TL;DR: 研究发现深度视觉Transformer存在Cliff-Plateau-Climb三阶段模式，[CLS]令牌作用逐渐边缘化，信息扩散比增加层数更重要。
...
Result: 发现[CLS]令牌作用被分布式共识取代，ViT-L的信息任务权衡比ViT-B晚出现10层，额外层主要增加信息扩散而非提升性能。

Conclusion: Transformer架构应注重精心校准的深度而非简单增加参数，信息混洗指数可作为诊断工具和未来架构设计目标。
...
### [189] [AerialMind: Towards Referring Multi-Object Tracking in UAV Scenarios](https://arxiv.org/abs/2511.21053)
*Chenglizhao Chen,Shaofeng Liang,Runwei Guan,Xiaolou Sun,Haocheng Zhao,Haiyun Jiang,Tao Huang,Henghui Ding,Qing-Long Han*

Main category: cs.RO

TL;DR: AerialMind是首个无人机场景下的大规模Referring Multi-Object Tracking基准，通过COALA标注框架降低标注成本，并提出HETrack方法提升视觉-语言表示学习。


<details>
  <summary>Details</summary>
Motivation: 当前RMOT研究局限于地面场景，无法捕捉大尺度场景上下文；无人机凭借广阔视角和机动性成为重要平台，需要能与自然语言交互的智能空中系统。

Method: 开发半自动协作代理标注框架COALA降低标注成本；提出HETrack方法协同增强视觉-语言表示学习，改善无人机场景感知。

Result: 综合实验验证了数据集的挑战性和方法的有效性。

Conclusion: AerialMind填补了无人机场景RMOT研究空白，为智能空中系统发展提供了重要基础。
...
### [190] [SocialNav: Training Human-Inspired Foundation Model for Socially-Aware Embodied Navigation](https://arxiv.org/abs/2511.21135)
...
TL;DR: SocialNav是一个具有分层"大脑-行动"架构的基础模型，用于社会感知导航，能够理解高级社会规范并生成低级、符合社会规范的轨迹。


<details>
  <summary>Details</summary>
Motivation: 遵守社会规范的具身导航仍然是一个开放的研究挑战。

Method: 构建了包含700万个样本的SocNav数据集，采用多阶段训练流程：首先通过模仿学习注入通用导航技能和社会规范理解，然后通过专门设计的SAFE-GRPO（首个基于流的强化学习框架）来细化这些技能。

Result: SocialNav相比最先进方法实现了+38%的成功率和+46%的社会合规率，在导航性能和社会合规性方面都表现出显著提升。

Conclusion: SocialNav在导航性能和社会合规性方面都取得了显著进步，为解决社会感知导航问题提供了有效的解决方案。
...
### [191] [Uncertainty Quantification for Visual Object Pose Estimation](https://arxiv.org/abs/2511.21666)
*Lorenzo Shaikewitz,Charis Georgiou,Luca Carlone*

Main category: cs.RO

TL;DR: SLUE是一种分布自由的位姿不确定性估计方法，通过凸优化生成包含真实物体位姿的椭球不确定性边界，无需严格的分布假设。


<details>
  <summary>Details</summary>
Motivation: 在机器人学中，量化位姿估计的不确定性对于鲁棒控制和规划至关重要，但现有方法通常需要严格的分布假设。

Method: 基于2D语义关键点的高概率噪声边界，使用S-引理启发的最小体积包围椭球问题松弛，开发了SLUE凸优化程序。

Result: 在多个位姿估计数据集和真实无人机跟踪场景中，SLUE生成明显更小的平移边界和具有竞争力的方向边界。

Conclusion: SLUE提供了一种统计严谨的位姿不确定性量化方法，无需分布假设，适用于单目视觉设置。
...
### [192] [TraceGen: World Modeling in 3D Trace Space Enables Learning from Cross-Embodiment Videos](https://arxiv.org/abs/2511.21690)
...
TL;DR: TraceGen提出了一种符号化的3D轨迹空间表示，能够从跨实体、跨环境、跨任务的视频中学习机器人任务，仅需少量演示即可实现高效适应。


<details>
  <summary>Details</summary>
Motivation: 解决机器人学习中的小数据问题，利用丰富的跨实体视频（人类和其他机器人）来学习新任务，克服实体差异、相机差异和环境差异带来的挑战。

Method: 开发TraceGen世界模型，在3D轨迹空间而非像素空间预测未来运动；构建TraceForge数据管道，将异构视频转换为一致的3D轨迹，创建包含123K视频和180万观测-轨迹-语言三元组的大规模语料库。

Result: 仅用5个目标机器人视频，TraceGen在4个任务中达到80%成功率，推理速度比最先进的基于视频的世界模型快50-600倍；仅用5个手持手机拍摄的未校准人类演示视频，在真实机器人上仍能达到67.5%成功率。

Conclusion: TraceGen通过3D轨迹空间表示实现了跨实体的高效适应，无需依赖物体检测器或繁重的像素空间生成，为小样本机器人学习提供了有效解决方案。
...
### [193] [InvisibleBench: A Deployment Gate for Caregiving Relationship AI](https://arxiv.org/abs/2511.20733)
*Ali Madad*

Main category: cs.CY

TL;DR: InvisibleBench是一个用于评估照护关系AI的部署门控基准，测试3-20+轮交互，涵盖安全、合规、创伤知情设计、归属/文化适应性和记忆五个维度。评估显示所有前沿模型都存在显著安全漏洞，危机检测率仅为11.8-44.8%。


<details>
  <summary>Details</summary>
Motivation: 现有单轮安全测试无法捕捉真实交互中的纵向风险，需要开发能够评估长期交互中实际危害的部署就绪性评估工具。
...
Conclusion: 生产系统中需要确定性危机路由机制，单轮安全测试不足以评估真实风险，纵向交互评估对部署就绪性至关重要。
...
### [194] [Large Language Models' Complicit Responses to Illicit Instructions across Socio-Legal Contexts](https://arxiv.org/abs/2511.20736)
*Xing Wang,Huiyuan Xie,Yiyan Wang,Chaojun Xiao,Huimin Chen,Holli Sargeant,Felix Steffek,Jie Shao,Zhiyuan Liu,Maosong Sun*

Main category: cs.CY

TL;DR: 研究发现大型语言模型普遍存在协助非法行为的风险，GPT-4o在近半数测试案例中提供了非法协助，现有安全策略不足且可能加剧问题。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs协助非法活动的风险，定义"共犯便利"行为，填补该高风险行为研究的空白。

Method: 使用真实法律案例和法律框架构建评估基准，包含269个非法场景和50个非法意图，进行四项实证研究。

Result: LLMs普遍易受共犯便利影响，在针对社会利益的犯罪、常见违规和恶意意图中表现更差，对边缘群体存在歧视性协助模式。

Conclusion: 现有安全对齐策略不足，模型感知的刻板印象与共犯行为相关，需要改进安全机制以应对非法协助风险。
...
TL;DR: 提出一种基于变分方法和分数阶导数的拉曼光谱去噪方法，在频域中通过傅里叶变换实现快速实现，结合香农熵优化正则化参数和导数阶数。
...
Result: 该方法在模拟拉曼数据和图像处理中表现出色，能有效去除噪声同时保留光谱的关键特征（峰位置、强度和面积）。

Conclusion: 所提出的策略组合产生了一个高效、稳健且易于实现的滤波器，分数阶导数与正则化参数的结合能有效平衡噪声去除和特征保留。
...
TL;DR: 提出MTI-Net多任务交互对抗学习网络，同时处理肝脏肿瘤分割、动态增强回归和分类任务，通过多域信息熵融合和任务交互模块实现任务间协同提升


<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时完成肝脏肿瘤分割、动态增强回归和分类任务，缺乏有效捕捉任务间相关性的框架和动态MRI信息提取机制

Method: 使用多域信息熵融合整合频域和谱域特征，引入任务交互模块建立分割与回归的高阶一致性，设计任务驱动判别器捕捉任务间高阶关系，采用浅层Transformer进行位置编码提取动态MRI序列关系

Result: 在238个受试者数据集上，MTI-Net在多个任务中表现出高性能

Conclusion: MTI-Net在肝脏肿瘤临床评估中具有强大潜力
...
TL;DR: 本文提出了深度参数插值（DPI）方法，通过动态插值两个可学习参数集来为深度神经网络添加标量输入依赖，适用于扩散模型和流匹配模型。


<details>
  <summary>Details</summary>
Motivation: 现有深度生成模型需要处理高维向量和标量两种不同来源的信息，常见方法限制了架构选择，需要一种更通用的标量依赖添加方法。

Method: 在单个网络中维护两个可学习参数集，根据标量值在训练和采样期间动态插值参数集，实现架构无关的标量依赖添加。

Result: DPI方法提高了去噪性能，增强了扩散模型和流匹配模型的样本质量，同时保持了与标准标量条件技术相当的计算效率。

Conclusion: 深度参数插值是一种简单有效的通用方法，可为神经网络添加标量依赖，在生成模型中表现出优越性能。
