...
Motivation: 社交媒体平台的发展改变了我们对城市环境的理解，产生了嵌入在人类感知和意见中的情感反应变化，这对现有的多维情感分析方法提出了挑战。

Method: 构建包含140,750张街景图像和984,024条微博文本的数据集，开发反应指数整合目标检测和自然语言处理技术，使用回归分析、图像分割和词频分析等方法。

Result: 感知情感反应趋势图显示正向情感分布更均匀，而意见情感反应趋势图显示更极端的变化。不匹配图显示人类感知与意见情感存在显著差异，这些变化与密集建筑和行人存在等因素相关。

Conclusion: 不一致地图展示了疫情前后感知和意见情感的变化，为环境管理和城市更新策略制定提供了潜在解释和方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有的"大海捞针"基准测试过于简化，忽略了现实世界中由偏置检索和智能体工作流程产生的噪声上下文，需要构建更真实的测试环境。

Method: 基于英文维基百科超链接网络构建HaystackCraft基准，包含多跳问题，评估不同检索策略对干扰项构成的影响，并扩展到动态的智能体操作场景。

Result: 实验显示：更强的密集检索器会引入更具挑战性的干扰项，而基于图的重新排序能同时提高检索效果并减轻有害干扰；在智能体测试中，即使是先进模型也会因自生成干扰项而出现级联失败。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在多种NLP任务中表现出色，但在上下文词形还原任务中的有效性尚无证据，本文旨在填补这一空白。

Method: 比较了(i)在域外微调的编码器监督方法，(ii)跨语言方法，与(iii)LLMs直接上下文生成词形的方法，在12种不同形态复杂度的语言上进行实验。

Result: 尽管编码器在域外设置下使用黄金数据微调仍具竞争力，但当前LLMs通过直接上下文生成词形，仅需少量示例即可在大多数语言中达到最先进结果。

Conclusion: LLMs在上下文词形还原任务中表现出色，无需微调即可超越传统监督方法，特别是在缺乏目标领域或语言训练数据的情况下。
...
<details>
  <summary>Details</summary>
Motivation: 传统ASR评估指标如WER会不公平地惩罚不影响句子语义的形态和句法细微差异，需要更语义感知的评估方法。

Method: 利用最先进LLM的上下文学习能力，通过包含详细示例的提示来学习评分标准；同时探索使用较小LLM在词对示例上微调来预测惩罚类型。

Result: 使用Gemini 2.5 Pro的Hindi LASER评分与人工标注达到94%的高相关性；Hindi示例对其他印度语言如Marathi、Kannada和Malayalam也有效；微调Llama 3在预测惩罚类型上达到接近89%的准确率。

Conclusion: LLM-based LASER方法能有效评估ASR质量，提供比传统WER更语义感知的评分，且具有跨语言适用性。
...
Conclusion: 该研究为手语翻译系统的开发和评估提供了实用且可复现的方法，推动了手语技术评估的标准化进程。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于文本分析的民粹主义测量方法成本高、耗时且难以跨语言、跨情境和大规模语料库扩展，需要开发更高效准确的自动化方法。

Method: 利用全球民粹主义数据库(GPD)，采用规则和锚点引导的链式思维提示方法，通过向LLM提供与训练人类编码员相同的文档来指导模型推理过程。

Result: 该方法使LLM在民粹主义分类任务上达到了与专家人类编码员相当的准确率，能够处理民粹主义的细微和情境敏感方面。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统虽然能超越单个智能体，但由于提示敏感性和系统不稳定性，设计有效的多智能体系统仍然困难。目前缺乏针对多智能体提示优化的系统方法。

Method: 提出四阶段MAPRO框架：将多智能体提示优化建模为最大后验推理问题，使用语言引导的max-product信念传播算法求解，采用拓扑感知的细化机制迭代更新智能体提示。
...
<details>
  <summary>Details</summary>
Motivation: 测试时缩放通过长链思维提升LLM推理能力，但KV缓存的线性增长加剧了内存瓶颈。现有查询感知的稀疏解码方法受限于顺序依赖和粗粒度标记选择，在高并发和长链思维场景下效率低下。

Method: 提出AsyncSpade框架，包含两个核心组件：(1)轻量级时间回归模块预测下一标记查询状态；(2)异步解耦框架将KV缓存过滤与自回归解码循环分离，通过异步机制重叠标记级KV选择与前向推理计算。
...
Conclusion: AsyncSpade是首个在不牺牲模型性能的情况下消除顺序依赖的方法，为高效测试时缩放提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 受人类团队科学启发，探索多智能体系统中团队结构、多样性和互动动态等核心方面，填补该领域研究空白。

Method: 提出多智能体框架，在常识推理和社交推理四个任务上评估团队表现，包括访谈和任务后反思分析。

Result: 扁平团队表现优于层级团队，多样性影响复杂，智能体对团队表现过度自信，协作整合存在对话协调不足等挑战。

Conclusion: 多智能体团队动态研究具有重要意义，扁平结构和适度的多样性有助于提升团队表现，但需要解决协作整合问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有长上下文语言模型虽然能处理大量文档，但简单堆叠文档无法有效连接证据，缺乏多跳推理的结构化指导。

Method: 使用思维模板将推理过程转化为可重用的思维缓存，通过自然语言反馈迭代优化模板，构建ToTAL框架。

Result: 在多样化基准测试和不同LCLM模型上均取得稳定提升，优化后的模板可蒸馏到小型开源模型中。

Conclusion: 思维模板方法能有效增强长上下文语言模型的多跳推理能力，具有广泛适用性和透明推理重用特性。
...
Method: 开发了一个序列到序列的音译模型，整合了所有可用的数据集，并新增了两个数据集。模型在多个文本领域进行训练，以提升通用性。

Result: 模型在Farsi到Tajik方向获得chrF++ 87.91和标准化CER 0.05，Tajik到Farsi方向获得chrF++ 92.28和标准化CER 0.04，在所有可用数据集上实现了最先进的性能。

Conclusion: 该模型为塔吉克-波斯语双向音译任务提供了全面的基准，解决了现有模型领域局限性的问题，为实际应用提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在短上下文（如2K tokens）基准上表现良好，但在实际长上下文工作负载中性能严重下降，甚至出现减速（如EAGLE3降低0.81倍生成速度）。

Method: 1) 基于LSTM的drafter，仅依赖最后token状态，实现不同长度的泛化；2) 验证器中引入特殊token [SPEC]为drafter提供更丰富的表示；3) 结合树和非树解码的混合算法。

Result: OWL在长上下文输入上比EAGLE3实现了约5倍更高的接受长度，显著提升了长上下文场景下的生成速度。

Conclusion: 通过LongSpecBench基准和OWL模型，解决了推测解码在长上下文中的局限性，为未来研究提供了代码和数据集支持。
...
<details>
  <summary>Details</summary>
Motivation: 解决小参数模型（≤2B）在图表理解任务中作为自动评估器表现不佳的问题，使其能够在资源受限环境中实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 在移动平台上部署NLP模型需要模型能够适应多样化应用，同时保持内存和计算效率。研究预微调策略以增强轻量级BERT编码器对命名实体识别和文本分类任务的适应性。

Method: 提出基于任务专用LoRA模块的多任务预微调框架，使用单一共享编码器骨干网络配合模块化适配器，避免朴素多任务预微调带来的优化信号冲突问题。

Result: 在21个下游任务上的实验显示，命名实体识别任务平均提升0.8%，文本分类任务平均提升8.8%，性能与单独预微调相当，同时满足实际部署约束。

Conclusion: 该方法通过任务专用LoRA模块实现了有效的多任务预微调，为移动NLP应用提供了既高效又具适应性的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在识别健康错误信息与事实性沟通之间的语言差异，为数字健康错误信息检测提供语言学指标，并指导公共卫生信息传播策略。

Method: 使用三个语料库进行对比分析：COVID-19错误叙事语料库（7588条）、一般COVID-19内容语料库（10700条）和猴痘相关帖子语料库（5787条），分析可读性、修辞标记和说服性语言使用。

Result: COVID-19错误信息可读性得分显著更低，恐惧相关或说服性词汇频率是其他数据集的两倍以上，感叹号使用极少，与猴痘内容的情绪化风格形成对比。

Conclusion: 错误信息采用故意复杂的修辞风格嵌入情感线索，这种组合可能增强其感知可信度。研究为健康错误信息检测提供了语言学指标，但存在传统可读性指数依赖、说服性词汇有限等局限性。
...
<details>
  <summary>Details</summary>
Motivation: 为构造语言创作提供有趣工具，同时探索LLMs对语言和语言学概念的理解程度

Method: 模块化系统：1) 代理方法创建目标音系 2) 将英语句子翻译为形态句法标记 3) 从语料库构建词典 4) 设计正字法 5) 生成语法手册

Result: 不同LLM和语言规范间存在能力差距，处理常见模式比罕见模式更容易；在从高资源语言到低资源语言翻译方面结果多为负面

Conclusion: 该系统可用于构造语言创作，并展示了LLMs的语言理解能力；改进版本可能在高资源到低资源语言翻译任务中取得真正进展
...
<details>
  <summary>Details</summary>
Motivation: 探究语言模型输入词汇表示的结构如何形成，以及这种结构在训练过程中如何演化，特别是与语义、语法和词频的关系。

Method: 使用表征相似性分析，对Pythia 12B和OLMo 7B两个开源模型进行实验，将输入和输出嵌入的几何结构与语义、语法和频率指标在训练过程中进行相关性分析。

Result: 1) 词汇嵌入几何在训练过程中快速收敛到与语义和语法特征高度相关；2) 高频词和功能词比词汇词和低频词更快收敛到最终向量，后者与随机初始化的偏差保持一定对齐。

Conclusion: 研究揭示了词汇几何围绕语言结构组织的动态轨迹，显示了词频和功能词的不同作用，为理解词汇几何演化如何促进模型能力提升提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有的临床编码评估方法基于精确匹配指标，往往忽略了预测代码在层次结构上接近但实际错误的情况，这种层次错位占据了LLM失败的大部分原因。

Method: 采用轻量级干预措施，包括提示工程和小规模微调，引入临床编码验证作为独立任务和流程组件，并构建了专家双重注释的门诊临床笔记ICD-10编码基准数据集。

Result: 研究结果表明，层次错位是LLM临床编码失败的主要原因，验证步骤能有效可靠地改进基于LLM的医疗编码性能。

Conclusion: 临床编码验证是改进LLM医疗编码的有效方法，轻量级干预措施可以在不增加计算开销的情况下提高准确性，新发布的数据集解决了现有数据集的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生不受限制的响应，模糊了角色边界，需要研究其在访问控制策略下的拒绝能力，以确保安全计算。

Method: 创建了基于Spider和BIRD数据集的RBAC增强数据集，比较了三种方法：零/少样本提示、生成器-验证器两步管道、以及LoRA微调模型。

Result: 显式验证（两步框架）提高了拒绝精度并降低了错误许可率，而微调在安全性和实用性（执行准确性）之间取得了更好的平衡。更长更复杂的策略会降低所有系统的可靠性。

Conclusion: 两步验证方法在访问控制方面表现更好，微调模型在安全性和实用性之间达到最佳平衡，复杂策略会降低系统可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 传统银行界面需要多屏幕操作流程，而现有的AI助手仅限于咨询或支持角色。本文旨在开发一个能够可靠执行核心金融操作的自然语言界面。

Method: 使用内部开发的闭源LLM ILMU，通过四个LLM驱动的代理（护栏、意图、支付、FAQ）协调单一对话，每个代理使用任务特定的LoRA适配器，并在银行基础设施内托管以确保一致性。

Result: 成功实现了监管批准的自然语言界面，能够可靠支持严格治理下的核心金融操作，取代了僵化的多屏幕工作流程。

Conclusion: 证明了在严格监管环境下，自然语言界面可以可靠地支持核心金融操作，实现了"正确银行"的理念。
...
<details>
  <summary>Details</summary>
Motivation: 现有缓存淘汰方法通常使用累积注意力权重启发式地评估token重要性，但未考虑其对注意力输出的真实影响，导致内存效率不高。

Method: 基于最优脑损伤理论，将缓存淘汰建模为层间结构化剪枝问题，推导出孤立key、孤立value和联合key-value对的闭式重要性评分，综合考虑注意力权重、value状态和注意力输出信息。

Result: 在LLaMA和Qwen模型上的实验表明，用OBCache的输出感知评分替换现有工作的启发式评分，能持续提升长上下文准确性。

Conclusion: OBCache提供了一个原则性的缓存淘汰框架，通过输出感知的重要性评分有效提升长上下文任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统的令牌概率偏见测量方法虽然应用广泛，但被批评与真实世界语言模型使用场景和危害脱节，因此需要探索更现实的替代指标。

Method: 测试自然语言推理(NLI)作为偏见评估指标，比较NLI与TP方法在偏见检测上的表现差异和相关程度。

Result: NLI和TP偏见评估行为差异显著，相关性很低；NLI更可能检测到"去偏见不足"的情况，但对反刻板印象句子的措辞更敏感和脆弱。

Conclusion: 令牌概率和自然语言推理都不是在所有情况下的"更好"偏见指标，建议结合TP、NLI和下游偏见评估来确保语言模型的全面评估。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的训练规范面临内部原则冲突和细粒度场景覆盖不足的挑战，需要系统性的方法来识别和解决这些规范缺陷。

Method: 使用综合分类法生成多样化的价值权衡场景，迫使模型在相互竞争的原则间做出选择，评估12个前沿LLM的行为分歧，并通过价值分类分数测量行为不一致性。

Result: 识别出超过70,000个显著行为分歧案例，实证表明模型行为的高度分歧强烈预示着模型规范中的潜在问题，包括直接矛盾、解释模糊性、明显错位案例和误报拒绝。

Conclusion: 模型规范存在系统性缺陷，需要更严谨的规范设计方法，提出的压力测试方法能有效识别规范问题，并为改进模型对齐提供重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在细胞生物学中的广泛应用，需要系统性地总结和分类这些方法，以促进虚拟细胞建模领域的发展。

Method: 提出统一的分类法，将现有方法组织为LLMs作为预言机（直接细胞建模）和LLMs作为智能体（协调复杂科学任务）两种范式，并详细分析三个核心任务及相关模型、数据集和评估基准。

Result: 建立了系统的虚拟细胞建模分类框架，识别了该领域在可扩展性、泛化性和可解释性方面的关键挑战。

Conclusion: 大型语言模型正在变革细胞生物学，为虚拟细胞建模提供了新的可能性，但仍需解决可扩展性、泛化性和可解释性等关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有仇恨言论检测模型主要依赖表面语言线索，难以有效泛化到不同的风格变化，且不同平台的仇恨言论可能产生虚假相关性。

Method: 将仇恨言论生成建模为因果图，包含上下文环境、创作者动机、目标和风格等关键因素，通过因果表示学习解构仇恨言论并控制混杂变量。

Result: CADET在综合实验中表现出优越性能，能够通过潜在空间中的风格干预进行反事实推理，稳健识别不同形式的仇恨言论。

Conclusion: 因果先验在推进可泛化的仇恨言论检测方面具有潜力，CADET框架为这一领域提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法仅将用户历史视为扁平文本列表进行检索，无法建模反映用户兴趣动态变化的丰富时间和语义结构，导致个性化程度较浅。

Method: 构建包含行为记忆和认知记忆的双组件层次化记忆系统，行为记忆捕获具体用户行为，认知记忆表示长期偏好，两者都整合时间和语义信息但处于不同抽象层次。

Result: 在LaMP基准测试中验证了MemWeaver的有效性。

Conclusion: MemWeaver通过层次化记忆结构实现了对用户兴趣演化和语义关系的深度建模，为LLMs提供了统一的用户表示，支持深度个性化生成。
...
<details>
  <summary>Details</summary>
Motivation: 传统的图RAG方法虽然能构建知识图谱连接分散事实，但缺乏深度结构化推理能力，导致复杂多跳问答中证据不完整和错误累积的问题。

Method: 将复杂问题分解为有序的子问题链，为每个子问题从图中检索相关三元组，当现有图谱不足时实时从源文档动态扩展，所有使用的三元组聚合形成"图记忆"作为结构化证据路径。

Result: 在三个多跳问答基准测试中，SubQRAG实现了持续且显著的改进，特别是在精确匹配分数方面表现突出。

Conclusion: SubQRAG通过子问题驱动的动态图谱扩展和结构化证据路径构建，有效提升了复杂多跳问答的推理深度和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有MKGC研究未能充分利用LLMs的多语言能力，忽略了跨语言知识的可共享性

Method: 包含两个核心组件：知识级分组混合专家（KL-GMoE）和迭代实体重排（IER），前者高效建模共享知识，后者增强知识利用

Result: 在5种语言构建的mKG数据集上，相比现有SOTA方法，Hits@1、Hits@3、Hits@10分别提升5.47%、3.27%和1.01%

Conclusion: 该框架有效利用了多语言共享知识，在未见语言和不平衡语言设置下揭示了知识共享的特性
...
<details>
  <summary>Details</summary>
Motivation: GRPO训练LLM时面临模型响应不准确、小规模架构表现差的问题，导致性能提升受限、训练中途崩溃，影响稳定性和最终效果。

Method: 1) 动态多轮硬采样：动态替换挑战性样本为高质量少样本演示，配合指数学习率衰减；2) 自示例思考：改进的GRPO框架，消除KL散度，调整裁剪系数，鼓励模型自主生成和分析少样本示例。

Result: 实验结果表明ToolExpander显著增强了LLM的工具使用能力，特别是在较弱的小规模模型中，同时改善了训练稳定性和整体性能。

Conclusion: ToolExpander成功解决了GRPO训练中的关键问题，为资源受限的LLM提供了有效的工具导向强化学习框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有奖励模型依赖标量或成对判断，无法捕捉人类偏好的多维度特性，需要更可靠和可扩展的rubric生成方法。

Method: 引入对比性rubric生成(CRG)，通过对比偏好和拒绝响应来推导硬规则和原则，并通过拒绝采样确保偏好标签一致性。

Result: Rubric-RM在多个奖励建模基准上超越基线6.8%，在指令遵循和生物医学基准上提升策略模型性能。

Conclusion: rubrics提供了可扩展的对齐信号，缩小了昂贵人工评估与自动奖励建模之间的差距，为LLM对齐开启了新的原则驱动范式。
...
<details>
  <summary>Details</summary>
Motivation: 当前并行测试时扩展主要应用于显式链式思维，而潜在推理模型在连续向量空间中进行推理，缺乏采样机制和概率信号，无法受益于并行TTS。

Method: 1. 引入两种不确定性采样策略：蒙特卡洛Dropout和高斯加性噪声；2. 设计潜在奖励模型，使用步级对比目标训练来评分和指导潜在推理。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在多项选择题中的推理策略，特别是当仅提供选项时，LLMs是否使用浅层捷径，以及推理轨迹是否能揭示这些策略的本质。

Method: 让推理型LLMs在完整输入和仅选项输入下解决多项选择题，分析推理轨迹的长度和忠实性，以判断策略是否浅层。

Result: 测试时推理在完整输入和半数仅选项输入下提高准确性；仅选项的成功几乎不受推理轨迹长度影响；推理轨迹通过忠实性测试，显示使用了推断缺失问题等较少问题的策略。
...
<details>
  <summary>Details</summary>
Motivation: 解决工具增强推理中领域特定工具稀缺的问题，以及现有从思维链推理轨迹提取工具方法面临的可扩展性瓶颈，包括搜索空间扩大和工具间歧义等检索挑战。

Method: 首先生成离散的任务特定工具并聚类到语义连贯的主题中，然后使用多智能体框架：代码智能体重构代码提取共享逻辑创建多功能聚合工具，评审智能体确保聚合工具保持原始工具集的完整功能能力。

Result: 实验结果表明该方法显著提高了工具检索准确性和多个推理任务的整体推理性能，与基线相比在问题特定工具数量增加时展现出更强的可扩展性。

Conclusion: 该方法成功将大量问题特定工具转化为少量功能强大的聚合工具而不损失功能性，有效解决了工具库扩展时的检索挑战，提升了工具增强推理的可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于结果的奖励只关注最终答案，导致模型容易通过不合理的推理过程得到正确答案，严重高估了模型的推理能力。需要开发过程导向的评估方法来提高模型的可靠性。

Method: 提出了Rubric奖励模型(RRM)，这是一个过程导向的奖励函数，根据问题特定的评分标准评估整个推理轨迹，提供细粒度的校准奖励(0-1)，明确惩罚逻辑缺陷并鼓励严谨推导。

Result: 在四个数学基准测试中，RRM训练始终优于仅基于结果的监督。AIME2024上的Verified Pass@1024从26.7%提升到62.6%，Miracle Steps发生率减少了71%。

Conclusion: 奖励解决方案过程对于构建不仅更准确而且更可靠的模型至关重要，过程导向的评估方法能有效缓解奖励欺骗问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM研究中，增强真实性的方法往往忽视了其对安全对齐的负面影响，需要解决真实性与安全性之间的权衡问题。

Method: 使用稀疏自编码器分离拒绝相关特征与幻觉特征，并通过子空间正交化在微调过程中保持拒绝行为。

Result: 在常识推理任务和有害基准测试中，该方法能够保持拒绝行为并维持任务效用，有效缓解真实性与安全性之间的权衡。

Conclusion: 提出的方法成功解决了LLM中真实性与安全性的权衡问题，能够在提升事实准确性的同时保持安全对齐能力。
...
Conclusion: 该方法通过端到端的联合学习有效解决了少样本多标签意图检测中的错误传播问题
...
<details>
  <summary>Details</summary>
Motivation: LLMs在单轮任务中表现优异，但实际部署需要持续的多轮交互，其中存在上下文漂移问题——模型输出逐渐偏离目标一致行为。传统静态评估指标无法有效捕捉这种时间性漂移。

Method: 将漂移形式化为测试模型与目标一致参考模型之间逐轮KL散度，提出递归模型将漂移演化解释为具有恢复力和可控干预的有界随机过程。在合成重写任务和真实用户-代理模拟中实例化该框架。

Result: 实验显示漂移呈现稳定的噪声限制平衡而非失控退化，简单提醒干预能可靠减少发散，符合理论预测。

Conclusion: 多轮漂移可理解为可控平衡现象而非必然衰减，为研究和缓解扩展交互中的上下文漂移提供了理论基础。
...
Result: 在LLaMA-7B模型上实验，在WikiText-2和多个语言理解基准测试中，困惑度和任务准确率均优于现有基线方法。

Conclusion: 旋转约束补偿方法能有效补偿剪枝误差，同时以几何保持的方式保留重要组件，在剪枝后模型性能上表现优异。
...
Method: 对58个基础模型和代理模型进行分类，分为基础模型、文本桥接、空间模型、多模态、表观基因组和代理模型五大类，并映射到八个关键分析任务。使用40多个公共数据集分析基准适用性、数据多样性和约束条件。

Result: 在10个领域维度上评估模型，涵盖生物学基础、多组学对齐、公平性、隐私性和可解释性。通过连接数据集、模型和评估领域，提供了语言驱动单细胞智能的首个集成视图。

Conclusion: LLM4Cell概述了可解释性、标准化和可信模型开发方面的开放挑战，为语言驱动的单细胞智能提供了首个统一框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前代理式RAG系统存在过度搜索和搜索不足的问题，而基于结果的强化学习训练方法缺乏细粒度控制，无法有效解决这些效率问题。

Method: 提出HiPRAG方法，将细粒度的知识基础过程奖励引入RL训练，通过分解推理轨迹为离散步骤，应用分层奖励函数评估每个搜索决策的必要性。

Result: 在7个多样化QA基准测试中，Qwen2.5和Llama-3.2模型达到65.4%(3B)和67.2%(7B)的平均准确率，同时将过度搜索率降至2.3%，并降低了搜索不足率。

Conclusion: HiPRAG证明了通过RL进行细粒度控制对于改进搜索代理推理效率和最优性的重要性和潜力，在多种RL算法、模型家族和规模上表现出良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统的通信拓扑多为静态或手工设计，无法适应不同任务需求，导致简单任务过度消耗资源或复杂任务性能瓶颈

Method: 基于条件离散图扩散模型，通过轻量级代理模型预测多目标奖励，实现实时、无梯度的迭代拓扑优化

Result: 在多个基准测试中，GTD能生成高度任务自适应、稀疏且高效的通信拓扑，显著优于现有方法

Conclusion: GTD框架通过迭代引导的拓扑合成过程，能有效平衡任务性能、通信成本和鲁棒性等多目标要求
...
Method: 提出MGR-CSC框架，将语义等价的多语言关键词统一为共享原子来对齐语义并压缩标识符空间，采用动态多步约束解码策略。

Result: 在mMarco100k上准确率提升6.83%，在mNQ320k上提升4.77%，同时文档标识符长度分别减少74.51%和78.2%。

Conclusion: MGR-CSC通过跨语言语义压缩有效解决了多语言生成检索中的关键问题，在提升检索准确率的同时显著优化了标识符效率。
...
<details>
  <summary>Details</summary>
Motivation: 小语言模型在严格延迟和计算约束应用中很重要，但实现高性能具有挑战性。现有知识蒸馏方法存在权衡：off-policy蒸馏提供高质量监督但引入训练-推理不匹配，on-policy方法保持一致性但依赖低质量学生输出。
...
Conclusion: AdaSwitch同时保持了一致性和监督质量，解决了现有知识蒸馏方法的局限性，为小语言模型的高效训练提供了有效解决方案。
...
Method: 构建了包含24个双向语言对的基准测试，采用混合偏见检测流水线（规则启发式、语义相似度过滤、LLM验证），并基于人工评估创建了包含1,439个翻译-参考对的高质量偏见标注数据集。

Result: 开发了统一的评估框架和数据集，提供了系统性的翻译质量和公平性评估方法，代码和数据集已在GitHub开源。

Conclusion: Translation Tangles为解决LLM在机器翻译中的偏见和不公平问题提供了有效的评估工具，有助于提升翻译系统的公平性和可靠性。
...
TL;DR: 该论文提出了TRACE分析框架，系统研究LLMs在长链思维推理中的"过度思考"问题，发现验证过度和探索过度是主要原因，并提出了基于效用的过度思考定义。


<details>
  <summary>Details</summary>
Motivation: 现有长链思维推理模型在简单任务上存在过度思考问题，导致计算效率低下但准确性无提升。现有分析局限于表面观察，缺乏对LLMs内部思维过程的深入理解。

Method: 提出TRACE分析框架：1) 将思维过程分解为最小完整子思维；2) 推断子思维间的话语关系构建思维进展图；3) 识别相似查询的常见思维模式

Result: 发现开放权重思维模型存在两种主要模式——探索者和晚期着陆，证实验证过度和探索过度是过度思考的主要驱动因素。长思维模型在简单任务上速度慢5-20倍但无实质性收益。

Conclusion: 提出了基于效用的过度思考定义，超越了基于长度的指标，为原则性过度思考管理提供了实用指导，深化了对LLMs思维进展的理解。
...
TL;DR: 本文提出了CS3-Bench基准测试，发现现有语音交互模型在语言对齐方面存在缺陷，并提出数据构造和训练方法来提升语言对齐能力，显著改善了知识准确性和理解率。
...
Method: 提出CS3-Bench基准测试评估7个主流模型，采用Chain of Recognition增强理解能力，使用Keyword Highlighting指导生成过程。

Result: 实验显示现有模型在知识密集型问答中性能下降达66%，通过提出的方法将知识准确率从25.14%提升至46.13%，开放理解率从64.5%提升至86.5%。

Conclusion: CS3-Bench基准测试揭示了语音交互模型的语言对齐问题，提出的数据构造和训练方法有效提升了模型的跨语言理解能力。
...
TL;DR: 提出ConG框架，利用对比解码改进弱到强泛化方法，通过对比对齐前后的弱模型生成更高质量样本，提升能力迁移的可靠性和鲁棒性。
...
Method: 基于隐式奖励与对比解码的结构等价性，提出ConG框架，在对齐前后的弱模型间进行对比解码来生成更高质量样本。

Result: 在不同模型家族上的实证结果证实了ConG的持续改进效果，展示了其通用性和有效性。

Conclusion: ConG有潜力推进弱到强泛化方法，为通向AGI提供有前景的路径。
...
TL;DR: 比较标准德语到方言在文本模型、语音模型和级联系统中的迁移效果，发现语音模型在方言数据上表现最佳，而文本模型在标准数据上表现最好。


<details>
  <summary>Details</summary>
Motivation: 方言主要是口语形式，非标准拼写会给文本处理带来问题，因此需要研究从标准语言到方言的迁移学习效果。

Method: 在德语和多种德语方言的背景下，使用文本模型、语音模型和级联系统（语音先自动转录再由文本模型处理）进行意图和主题分类实验。

Result: 语音模型在方言数据上表现最佳，文本模型在标准数据上表现最好。级联系统在德语上落后于文本模型，但在方言数据上如果转录系统生成标准化的输出则表现相对较好。

Conclusion: 方言处理中语音模型优于文本模型，级联系统在方言数据上的表现取决于转录系统的标准化程度。
...
TL;DR: MCBench是一个新的基准测试，通过让LLMs严格遵循逐步指令执行字符串匹配NLP指标，来客观评估其指令遵循和执行能力。


<details>
  <summary>Details</summary>
Motivation: 当前前沿LLMs在许多基准测试上已接近饱和，需要更具挑战性的基准来提供客观验证。

Method: 设计MCBench基准，要求LLMs严格遵循逐步指令执行字符串匹配NLP指标，并提供并行参考代码来验证输出准确性。包含三种评估指标和三种基准变体。

Result: 分析表明MCBench是评估前沿LLMs能力的有效客观工具。

Conclusion: MCBench提供了一个客观、确定且可代码验证的评估框架，能够系统测试LLMs的指令遵循、数值计算和长程一致性等能力。
...
TL;DR: 本文提出了ACE框架，通过神经元级归因识别和编辑关键查询-值通路，解决了大语言模型在多跳事实回忆中知识编辑的性能衰减问题。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑方法在多跳事实回忆中表现显著衰减，特别是在涉及推理链中隐含中间主题时。这种限制源于对链式知识在神经元层面如何动态表示和利用的忽视。

Method: 通过因果分析发现隐含主题作为查询神经元，在transformer层中顺序激活对应的值神经元来积累信息。提出ACE框架，利用神经元级归因识别和编辑这些关键的查询-值通路。

Result: ACE在多跳知识编辑上经验性地优于最先进方法，在GPT-J上提升9.44%，在Qwen3-8B上提升37.46%。分析还揭示了Qwen3中更细粒度的激活模式。

Conclusion: 这些发现基于对内部推理机制的原则性理解，为推进知识编辑能力建立了新途径，表明值神经元的语义可解释性是由查询驱动的积累过程协调的。
...
TL;DR: 提出统一的LLM增强自动评分框架，为各类主观题提供类人评估，集成四个互补模块进行综合评估，在真实电商企业部署成功。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对特定类型主观题，缺乏支持包含多种题型综合考试的通用性，自动评分仍是考试评估中的重大挑战。

Method: 集成四个模块：基础文本匹配、LLM提取关键知识点对比、从学生答案生成伪问题评估相关性、模拟人工评估识别优缺点。

Result: 在通用和领域特定数据集上，框架在多个评分指标上持续优于传统和基于LLM的基线方法。

Conclusion: 该统一框架能有效处理各类主观题评分，已在真实电商企业培训和认证考试中成功部署应用。
...
TL;DR: 提出StepER方法，通过步骤式知识蒸馏增强多步检索增强语言模型的推理能力，解决现有方法忽视不同步骤需要不同推理能力的问题。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法忽视了多步检索增强框架中不同步骤需要不同推理能力的问题，阻碍了知识传递。

Method: StepER采用步骤式监督来适应不同阶段的信息和推理需求变化，并融入难度感知训练来逐步优化学习过程，优先处理合适的步骤。

Result: 在多项多跳问答基准测试中，StepER优于先前方法，8B模型实现了与70B教师模型相当的性能。

Conclusion: StepER方法能有效提升多步检索增强语言模型的推理能力，且适用于各种多步检索增强语言模型架构。
...
TL;DR: 本文研究了评估LLM生成文本完整性的方法，重点关注检测缺失信息或代表性不足的观点，比较了三种自动化评估策略。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在各种任务中表现出色，但它们经常产生不完整或选择性省略关键信息的输出。在敏感领域，这种遗漏可能造成与事实错误相当的严重危害。

Method: 研究了三种自动化评估策略：(1)基于NLI的方法，将文本分解为原子语句并使用自然语言推理识别缺失链接；(2)基于Q&A的方法，提取问答对并比较不同来源的响应；(3)端到端方法，直接使用LLM识别缺失内容。

Result: 实验表明简单的端到端方法相比更复杂的方法具有令人惊讶的有效性，但代价是降低了鲁棒性、可解释性和结果粒度。

Conclusion: 端到端方法在检测LLM生成文本的完整性方面表现良好，但需要在效率和解释性之间进行权衡。
...
TL;DR: 本研究应用大语言模型分析17-18世纪爱沙尼亚语词典，包括词典信息丰富化、哥特体文字识别和跨源数据集构建，实验显示LLMs在资源有限语言研究中具有显著潜力。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在爱沙尼亚语历史词典研究中的应用，解决历史词典现代化、哥特体文字识别和跨源数据整合的挑战，为小语种研究提供高效解决方案。

Method: 使用Claude 3.7 Sonnet等LLMs进行词典信息丰富化实验；采用零样本方法进行哥特体文字识别；运用图像分块扫描和双LLM协作进行文本识别与结构化输出合并。

Result: 词典丰富化实验中81%的词条获得准确释义和现代对应词；文字识别实验中41%的词条被成功识别并结构化输出；成功建立了Hupel 1780词典的数字化流程。

Conclusion: 大语言模型即使对小语种研究也具有显著的时间和资源节约潜力，为历史语言资料的数字化和现代化提供了有效工具。
...
TL;DR: A²Search是一个无需标注的端到端训练框架，用于识别和处理开放域问答中的歧义问题。通过自动检测歧义问题、收集替代答案，并使用强化学习优化模型，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型和强化学习方法在开放域问答中表现良好，但难以处理具有多个有效答案的歧义问题。标准问答基准通常假设单一黄金答案，忽略了现实中的歧义性，产生不合适的训练信号。现有的歧义处理方法依赖昂贵的人工标注，难以扩展到多跳数据集。

Method: A²Search采用自动化流程检测歧义问题，通过轨迹采样和证据验证收集替代答案。使用强化学习优化模型，设计专门的AnsF1奖励函数来自然适应多个答案。

Result: 在八个开放域问答基准测试中，A²Search实现了最先进的性能。仅使用单次推理，A²Search-7B在四个多跳基准测试中平均AnsF1@1得分达到48.4%，优于包括更大的ReSearch-32B（46.2%）在内的所有强基线。

Conclusion: A²Search能够有效解决歧义问题并在不同基准测试中泛化良好，表明接受歧义对于构建更可靠的问答系统至关重要。
...
TL;DR: LightReasoner是一个新颖框架，利用小型语言模型(SLM)来指导大型语言模型(LLM)的推理能力，通过专家-业余对比识别关键推理时刻，实现资源高效的监督微调。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调(SFT)方法资源密集，需要大量标注数据，且对所有token进行统一优化，而实际上只有少量token具有学习价值。本文探索小型模型能否通过揭示高价值推理时刻来指导大型模型。

Method: LightReasoner采用两阶段框架：1)采样阶段通过专家(LLM)与业余(SLM)模型的行为差异识别关键推理时刻，构建监督示例；2)微调阶段使用这些蒸馏示例对齐专家模型，增强其推理优势。
...
TL;DR: LLMs在社交推理任务中存在困难，无法区分客观现实与主观信念状态。本文提出自适应世界模型增强推理机制，通过构建动态文本世界模型来跟踪实体状态和时间序列，显著提升社交推理准确率并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 发现LLMs在数学和代码推理方面表现出色，但在社交推理任务中表现不佳，经常出现认知混淆、逻辑不一致以及客观世界状态与主观信念状态的混淆问题。

Method: 提出自适应世界模型增强推理机制，构建动态文本世界模型来跟踪实体状态和时间序列，动态监控推理轨迹中的混淆指标，并在需要时提供清晰的世界状态描述来帮助模型导航认知困境。

Result: 在三个社交基准测试中，准确率显著提升（例如Hi-ToM中提升10%），同时计算成本降低（最多减少33.8%的token使用）。
...
TL;DR: 提出了一个用于科学图表标题生成的两阶段系统，结合上下文过滤、类别特定提示优化和作者风格微调，在保持科学准确性的同时实现风格一致性。


<details>
  <summary>Details</summary>
Motivation: 科学图表标题需要同时保证准确性和风格一致性来传达视觉信息，现有方法难以兼顾这两方面。

Method: 两阶段流水线：第一阶段结合上下文过滤、DSPy的MIPROv2和SIMBA进行类别特定提示优化及标题候选选择；第二阶段使用少量样本提示和配置文件进行风格精炼。
...
TL;DR: MUSE是一个基于经验驱动和自我演化的AI代理框架，通过分层记忆模块实现持续学习和性能提升，在长时程任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决现有LLM代理在测试时静态、无法从经验中学习的问题，使AI代理能够在实际工作中积累知识并持续改进。

Method: 引入分层记忆模块组织不同层级的经验，通过自主反思将原始轨迹转化为结构化经验，并集成回记忆模块中。

Result: 在TAC长时程生产力基准测试中取得新的SOTA性能，仅使用轻量级Gemini-2.5 Flash模型就实现显著提升，展现出强大的持续学习和自我演化能力。

Conclusion: MUSE为能够实现现实世界生产力任务自动化的AI代理建立了新范式，其积累的经验具有强泛化性，能够零样本改进新任务。
...
TL;DR: 本研究调查ChatGPT在日英翻译中的表现，比较了简单和增强提示的效果，并与商业翻译引擎进行对比。发现文档级翻译优于句子级翻译，但增强提示效果不明确。ChatGPT-3.5在自动评估中表现更好，但存在准确性(ChatGPT-3.5)与流畅性(ChatGPT-4)的权衡。


<details>
  <summary>Details</summary>
Motivation: 探索ChatGPT在日英翻译任务中的潜力，比较不同提示策略的效果，并与现有商业翻译系统进行性能对比。

Method: 使用简单和增强提示进行日英翻译实验，进行自动评估和基于MQM的人工评估，比较文档级与句子级翻译，对比ChatGPT-3.5和ChatGPT-4版本。

Result: 文档级翻译优于句子级翻译；增强提示效果不明确；ChatGPT-3.5在自动评估中表现更好但流畅性较差；ChatGPT-4流畅性更好但准确性稍差；ChatGPT与知名翻译系统竞争力相当。

Conclusion: ChatGPT在日英翻译中具有竞争力，文档级翻译策略更有效，不同版本在准确性和流畅性方面存在权衡，提示策略的影响需要进一步研究。
...
TL;DR: 本文评估了大型语言模型从参数知识中回忆气候常态的能力，重点关注7月平均气温查询，发现LLMs能捕捉基本气候模式但存在空间一致错误，特别是在高海拔地区，且无法准确再现温度变化的空间模式。
...
Method: 构建全球1度分辨率的陆地网格查询，提供坐标和位置描述符，将LLMs响应与ERA5再分析数据验证，评估模型在不同地理条件下的表现。

Result: LLMs编码了非平凡的气候结构，能捕捉纬度和地形模式，RMSE为3-6°C，偏差±1°C。但在高海拔地区性能显著下降，RMSE达5-13°C。包含地理上下文可减少27%错误。

Conclusion: 虽然LLMs能捕捉当前气候分布，但难以再现长期温度变化的空间模式，这对理解气候动态至关重要。该评估框架为量化LLMs参数气候知识提供了可复现基准。
...
TL;DR: 本文系统综述了过程奖励模型(PRMs)的完整流程，包括过程数据生成、PRMs构建、以及在测试时扩展和强化学习中的应用，旨在推动细粒度推理对齐研究。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型展现出先进的推理能力，但传统的对齐方法主要依赖仅判断最终结果的结果奖励模型(ORMs)，过程奖励模型(PRMs)通过在步骤或轨迹层面评估和指导推理来弥补这一差距。

Method: 通过完整循环系统概述PRMs：如何生成过程数据、构建PRMs、以及使用PRMs进行测试时扩展和强化学习。总结在数学、代码、文本、多模态推理、机器人和智能体等领域的应用，并回顾新兴基准。

Result: 提供了PRMs设计空间的清晰梳理，揭示了开放挑战，为未来研究提供指导。

Conclusion: 过程奖励模型为细粒度、稳健的推理对齐提供了重要方向，需要进一步研究来解决现有挑战。
...
TL;DR: 提出FedDTRE联邦学习策略，通过可信度评估动态调节全局模型在本地更新中的贡献，解决对话生成中隐私保护与个性化平衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统集中式或全本地训练方法在隐私保护和个性化之间难以平衡，联邦学习面临客户端数据有限导致的过拟合和全局信息遗忘问题。

Method: 基于可信度评估的联邦自适应聚合策略，使用公平导向评估数据集计算全局和本地模型的可信度分数，动态调节全局模型在本地更新中的贡献。

Result: 实验结果表明FedDTRE能够提升对话模型性能，改善对话生成质量。

Conclusion: FedDTRE通过可信度评估机制有效解决了联邦学习中过拟合和全局信息遗忘问题，为隐私保护下的对话生成提供了可行方案。
...
TL;DR: 研究探讨LLM生成的合理性论证对人类常识判断的影响，发现PRO和CON论证分别能提高和降低人类对答案的合理性评分，LLM也表现出类似模式。


<details>
  <summary>Details</summary>
Motivation: 探究人类对常识基准答案的合理性判断是否会受到LLM生成的(不)合理性论证的影响，特别是在多选情境下。

Method: 收集了3,000个人类合理性判断和13,600个LLM判断，通过展示LLM生成的PRO和CON论证来观察其对判断的影响。

Result: 在LLM生成的PRO论证下，人类平均合理性评分上升；在CON论证下，评分下降，表明人类认为这些论证具有说服力。LLM实验也显示出类似的影响模式。

Conclusion: LLM可用于研究人类认知，但也引发实际担忧：即使在人类擅长的常识领域，LLM仍可能对人们信念产生显著影响。
...
TL;DR: 研究发现推理能力显著提升LLM的谈判表现，但计算成本增加400%。多语言测试显示开源模型在推理时会切换至英语，而商业模型能保持语言一致性。


<details>
  <summary>Details</summary>
Motivation: 谈判是AI智能体的基础挑战，需要战略推理、对手建模和合作竞争平衡能力。本研究首次系统评估推理对商业和开源LLM谈判能力的影响。

Method: 使用自博弈设置，在三个不同对话游戏中测试，分析性能与成本的权衡、推理过程的语言一致性以及模型的战略适应特性。

Result: 启用推理显著改善谈判结果，GPT-5性能提升31.4%，但计算成本增加近400%。开源模型在德语和意大利语谈判时内部推理仍用英语，商业模型能保持语言一致性。

Conclusion: 推理能有效提升LLM谈判能力但代价高昂，多语言推理存在显著差异，开源模型的语言不一致可能影响通过推理痕迹获得的可解释性收益。
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型各有不同的词汇表，导致它们在下一词分布层面难以协作，如模型集成。不同分词方式阻碍了模型间的有效合作。

Method: 建立无损词汇缩减的理论框架，通过数学方法将语言模型转换为任意小词汇量的等价模型，保持预测准确性不变。

Result: 成功实现了语言模型词汇量的无损缩减，使具有不同分词方式的模型能够通过最大公共词汇进行有效协作。

Conclusion: 提出的无损词汇缩减框架解决了不同语言模型因分词差异导致的协作问题，为模型集成和跨模型合作提供了理论基础。
...
TL;DR: 本研究比较了GPT-5-nano和Gemini-2.5-flash-lite在检测Instagram未披露赞助内容的表现，开发了法律推理错误分类法，发现添加法规文本能提升解释质量但不一定提高检测准确率。


<details>
  <summary>Details</summary>
Motivation: 网红营销模糊了有机内容与赞助内容的界限，现有检测方法缺乏法律依据或透明度不足，需要建立法律基础扎实的自动化监管方法。

Method: 使用1,143个Instagram帖子，在三种提示策略下比较两个LLM模型的表现，开发法律推理错误分类法，结合定量和定性评估方法。

Result: 两个模型在分类赞助内容方面表现强劲（F1最高0.93），但在模糊案例上性能下降超过10个百分点；发现常见错误包括引用遗漏（28.57%）、模糊引用（20.71%）和隐藏广告最高误判率（28.57%）。

Conclusion: 本研究为监管合规技术提供了LLM法律推理错误分类法，推进了网红营销内容的透明检测，为广告监管机构自动化审核流程提供了法律基础支持。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM作为评估者被大规模使用来替代人工标注，需要理解其潜在偏见和风险。

Method: 1) CLoVE：生成可验证、基于概念的对比性局部解释；2) GloVE：通过迭代聚类、总结和验证将局部规则浓缩为全局策略。

Result: 在7个内容危害检测基准数据集上验证，提取的全局策略对LLM-as-a-Judge决策高度忠实，且对文本扰动和对抗攻击具有鲁棒性。

Conclusion: 用户研究表明用户对全局策略的理解和满意度良好，该方法有助于揭示LLM评估的决策逻辑。
...
TL;DR: Genii是一个无监督的多智能体协作优化框架，通过模拟客户端-服务器轮询机制来减轻LLM评估模型的判断偏好偏见。


<details>
  <summary>Details</summary>
Motivation: LLM作为评估器存在判断偏好偏见，倾向于偏袒自身生成的回答，这降低了评估的可靠性。

Method: 将多个LLM评估模型整合到多智能体系统中，模拟交互式客户端-服务器轮询机制进行无监督优化。

Result: Genii在无需人工标注的情况下，性能优于基于标注数据训练的监督模型，能持续提升不同客户端智能体的性能。

Conclusion: Genii能有效减轻LLM评估模型的判断偏好偏见，证明其有效性，且代码已开源。
...
TL;DR: 论文提出AI Knowledge Assist系统，通过从历史客服对话中自动提取问答对来构建知识库，使用轻量级LLM微调，在20家公司测试中达到90%以上准确率，消除客服中心冷启动问题。


<details>
  <summary>Details</summary>
Motivation: 客服中心集成对话AI系统的主要障碍是缺乏公司专属知识库，需要解决冷启动问题。

Method: 从历史客服对话中自动提取问答对构建知识库，在内部数据上微调轻量级LLaMA-3.1-8B模型。

Result: 在20家公司实证评估中，系统达到90%以上准确率回答信息查询问题，性能优于更大的闭源LLM。

Conclusion: 该系统能够立即部署基于RAG的聊天机器人，有效消除客服中心的冷启动差距。
...
TL;DR: DACIP-RC是一种持续预训练技术，通过阅读理解增强小型LLM在业务对话任务中的领域适应性，显著提升零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型推理成本高，小型LLM缺乏跨领域零样本指令跟随能力，传统微调方法会导致灾难性遗忘，降低模型泛化能力。

Method: 提出DACIP-RC方法，通过阅读理解对话记录生成多样化任务指令和响应，替代传统的下一个词预测预训练方式。

Result: 实证评估显示DACIP-RC显著提升了在会议摘要、行动项生成、通话目的识别等业务对话任务中的零样本泛化性能。

Conclusion: 这是首个在业务对话数据上应用指令预训练的工作，为行业利用专有数据集进行领域适应提供了见解。
...
TL;DR: 该论文针对大语言模型产生错误拒绝的问题，提出了两个基准测试(XSB和MS-XSB)来评估拒绝行为，并开发了三种轻量级方法来缓解过度拒绝问题，无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常对包含类似不安全查询术语的良性请求产生错误拒绝，这种过度安全行为影响了模型的实用性。

Method: 引入两个基准测试：单轮提示的XSB和多轮对话的MS-XSB；使用事后解释方法识别拒绝触发词；部署三种模型无关方法：忽略词指令、提示重述和注意力引导。

Result: 实验显示这些策略在四个Llama模型上显著提高了对安全提示的遵从性，同时保持了强大的安全保护。

Conclusion: 建立了一个可复现的诊断和缓解过度拒绝的框架，为更安全和更有帮助的LLM部署提供了实用路径。
...
TL;DR: ARM2是一个通过强化学习和长度感知优化实现自适应推理的统一模型，在保持性能的同时显著减少token使用量。
...
Method: 使用强化学习框架结合长度感知优化，整合视觉理解和可执行代码推理，实现跨多种格式的自适应推理平衡。

Result: ARM2在保持与传统推理模型相当性能的同时，平均减少超过70%的token使用量。

Conclusion: ARM2提供了一个有效的自适应推理框架，在性能和效率之间取得了良好平衡，并通过广泛分析验证了其设计的合理性。
...
TL;DR: 提出MetricalARGS，首个用于评估LLM在韵律诗歌处理能力的四维度任务分类法：分析、检索、生成和支持，以泰卢固语为例展示应用。


<details>
  <summary>Details</summary>
Motivation: 现有NLP诗歌研究主要关注自动生成和摘要，而韵律诗歌的严格约束为评估LLM的深层推理和规则遵循能力提供了机会。

Method: 设计四维度任务分类法：Analysis（分析）、Retrieval（检索）、Generation（生成）和Support（支持），以泰卢固语为例说明应用方法。

Result: 建立了首个全面的韵律诗歌NLP任务分类体系，为评估LLM能力提供了系统框架。

Conclusion: MetricalARGS通过韵律诗歌的视角揭示了理解现代LLM能力和局限性的更广泛可能性。
...
TL;DR: 提出Training-Free GRPO方法，无需参数更新即可提升LLM智能体性能，通过经验知识作为token先验来指导模型行为


<details>
  <summary>Details</summary>
Motivation: 解决LLM在专业领域性能下降问题，避免传统方法需要昂贵参数更新和过拟合的问题

Method: 使用训练免费的组相对策略优化，基于组内相对语义优势而非数值优势，在多轮学习中提炼高质量经验知识作为token先验

Result: 在数学推理和网络搜索任务上显著提升域外性能，仅需几十个训练样本即可超越微调的小型LLM

Conclusion: Training-Free GRPO是一种轻量级且成本效益高的解决方案，能够有效提升LLM智能体在专业领域的表现
...
TL;DR: 提出函数令牌假说解释LLM工作原理：推理时函数令牌激活上下文中最具预测性的特征来指导下一个令牌预测（记忆检索）；预训练时预测函数令牌后的内容令牌增加学习特征并更新参数（记忆巩固）。


<details>
  <summary>Details</summary>
Motivation: 理解LLM中记忆检索和巩固的机制，目前这些机制仍不清楚。

Method: 使用二分图分析和小型案例研究，验证函数令牌激活大多数特征并指导下一个令牌预测。

Result: 实验证明少量函数令牌激活了大部分特征；预训练损失主要由预测函数令牌后的内容令牌主导，迫使函数令牌选择最具预测性的特征。

Conclusion: 函数令牌假说为理解LLM的记忆检索和巩固机制提供了新的理论框架，得到广泛实验证据支持。
...
TL;DR: 本研究将涌现性错位现象扩展到高风险场景下的不诚实和欺骗行为领域，发现在下游混合任务和实际人机交互中，仅需少量错位数据就能显著降低LLMs的诚实行为。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在恶意或不正确补全上微调后，是否会在高风险场景（如说谎和欺骗行为）中表现出广泛的不诚实错位行为。

Method: 在多样化领域上对开源LLMs进行错位补全微调，并在下游混合微调设置和模拟人机交互环境中测试模型行为。

Result: 实验显示LLMs在不诚实方面表现出广泛错位行为；仅1%错位数据就能使诚实行为下降20%以上；仅10%偏见用户就能无意中加剧助手的不诚实。

Conclusion: 涌现性错位风险不仅存在于直接微调中，也存在于下游混合任务和实际人机交互中，对高风险场景下的AI安全性提出了重要警示。
...
TL;DR: SenWave是一个专门用于分析COVID-19推文的多语言细粒度情感分析数据集，包含10个情感类别，覆盖5种语言，并提供超过1亿条未标注推文。


<details>
  <summary>Details</summary>
Motivation: 解决现有COVID-19数据集中标签数据不足、情感标签粒度粗或不合适的问题，以更好地理解疫情期间的公众情绪变化。

Method: 构建包含10,000条英文和阿拉伯文标注推文，以及30,000条翻译推文（西班牙语、法语、意大利语）的数据集；使用预训练的基于Transformer的语言模型进行细粒度情感分类。

Result: 提供了跨语言、国家和主题的情绪演变深入分析，展示了数据集与ChatGPT的兼容性，证明了其鲁棒性和多功能性。

Conclusion: SenWave数据集和代码已公开，预期将促进NLP社区对复杂事件细粒度情感分析的进一步探索，推动更细致的理解和研究创新。
...
TL;DR: 该论文提出了一个包含反因果声明的新数据集，解决了现有因果关系提取研究中忽视反因果声明的问题。


<details>
  <summary>Details</summary>
Motivation: 现有因果关系提取数据集只关注支持关系的"正因果"声明，完全忽视或错误标注反驳关系的"反因果"声明。

Method: 基于文献综述开发了包含反因果声明的新数据集，通过严格的标注指南对Causal News Corpus进行增强，获得了较高的标注者间一致性（Cohen's κ=0.74）。

Result: 研究表明，未在反因果关系上训练的模型倾向于将反因果错误分类为正因果。基于新数据集，可以缓解这一错误，使transformer模型能有效区分正反因果关系。

Conclusion: 整合反因果声明对于准确的因果关系提取至关重要，新数据集能够显著提升模型区分正反因果关系的能力。
...
TL;DR: WaltzRL是一个多智能体强化学习框架，通过对话智能体和反馈智能体的协作训练来解决LLM安全对齐问题，在保持帮助性的同时显著减少不安全响应和过度拒绝。
...
Method: 提出WaltzRL多智能体强化学习框架，将安全对齐建模为协作的正和博弈。联合训练对话智能体和反馈智能体，使用动态改进奖励机制，在推理时改进而非丢弃不安全或过度拒绝的响应。
...
TL;DR: 该论文研究了使用对比解码生成合成语料库来训练语言模型的方法，发现混合合成数据和真实数据能提升语言建模和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 由于大型语言模型训练所需的大量文本数据可能即将达到极限，需要探索使用LLM生成的合成数据作为补充训练数据的解决方案。

Method: 在受控环境中，使用对比解码方法从好坏模型之间的相对差异中采样生成合成语料库，然后将合成数据与原始训练数据混合进行训练。

Result: 实验结果显示，在原始100万单词语料库基础上混合合成数据训练，能提升语言建模目标和多种下游任务的性能。对比解码生成的合成数据特别有助于需要推理能力的任务，而传统采样方法生成的合成数据对表面语言能力依赖的任务更有帮助。

Conclusion: 使用对比解码生成合成语料库并与真实数据混合训练是解决训练数据限制的有效方法，能提升模型在语言建模和下游任务中的表现，特别是对推理能力的提升。
...
TL;DR: DeepMiner是一个通过高难度训练任务和动态上下文窗口来提升多轮智能体深度推理能力的新框架，在多个搜索代理基准测试中取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有推理模型在多轮长序列交互中难以激发深度推理能力，需要解决训练数据挑战性和上下文管理的局限性。

Method: 提出反向构建方法从真实网络源生成复杂但可验证的问答对，设计动态上下文管理策略使用滑动窗口机制，无需外部摘要模型。

Result: DeepMiner-32B在BrowseComp-en上达到33.5%准确率，比之前最佳开源代理提升近20个百分点，在多个基准测试中表现一致提升，支持近100轮持续交互。
...
Method: 采用神经元级分析方法，引入基于梯度的评分方法并进行额外过滤以精确识别文化相关神经元。

Result: 识别出文化通用神经元和文化特定神经元，抑制这些神经元会显著降低文化基准测试性能（最多30%），但对通用自然语言理解基准影响很小。文化特定神经元不仅支持目标文化知识，还支持相关文化知识。

Conclusion: 训练自然语言理解基准可能会削弱模型的文化理解能力，当更新包含大量文化通用神经元的模块时。这些发现为理解LLM内部机制提供了见解，并为模型训练和工程提供了实用指导。
...
TL;DR: AutoRed是一个无需种子指令的自由形式对抗提示生成框架，通过角色引导的对抗指令生成和反思循环来提升语义多样性，在LLM安全评估中实现了更高的攻击成功率和更好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有红队方法依赖种子指令，限制了对抗提示的语义多样性，需要开发更有效的自由形式对抗提示生成方法来提升LLM安全评估。

Method: 两阶段框架：1）角色引导的对抗指令生成；2）通过反思循环迭代优化低质量提示。引入验证器在不查询目标模型的情况下评估提示危害性。
...
TL;DR: 提出两阶段投票架构用于自杀风险检测，结合轻量BERT模型处理显性案例，使用LLM投票框架和心理学特征集成处理隐性案例，在效率和准确性上取得平衡。


<details>
  <summary>Details</summary>
Motivation: 全球自杀率上升，需要主动预防策略。社交媒体提供重要信号，但隐性自杀意念检测困难，现有轻量模型无法处理隐性信号，大型语言模型计算成本过高。

Method: 两阶段架构：第一阶段使用轻量BERT分类器处理高置信度显性案例；第二阶段对模糊输入使用多视角LLM投票框架或基于心理学特征提取的机器学习集成。

Result: 在两个数据集上表现优异：显性案例F1达98.0%，隐性案例F1达99.7%，跨域差距低于2%，同时显著降低LLM计算成本。

Conclusion: 该框架在自杀风险检测中平衡了效率和鲁棒性，是首批将LLM提取的心理学特征作为结构化向量用于自杀风险检测的工作之一。
...
TL;DR: 本文研究了上下文学习(ICL)中表示方式和学习能力之间的关系，发现它们是相对独立的：表示方式决定基线准确率，而学习能力基于此基线进行改进。


<details>
  <summary>Details</summary>
Motivation: 过去研究主要关注ICL中演示表示方式的影响，但对ICL学习能力的研究结果不一。本文旨在探究表示方式和学习能力这两个方面的相互作用。

Method: 开发优化算法枚举不同语义相关性的标签集，然后对每个标签集使用不同数量的上下文演示进行ICL实验。

Result: 无论标签集质量如何，学习都会发生，但学习效率受标签集质量和语言模型参数数量的影响。表示方式的质量相对排名在学习过程中基本保持不变。

Conclusion: ICL中的表示方式和学习能力具有正交性，表示方式决定性能基线，学习能力在此基础上改进，这是ICL中一个未被充分探索的方面。
...
TL;DR: 本研究探讨大语言模型对条件陈述可接受性的判断，发现模型对条件概率和语义相关性都敏感，但不如人类一致，且模型大小与人类判断一致性无关。


<details>
  <summary>Details</summary>
Motivation: 条件可接受性在沟通和推理中很重要，但之前研究主要关注LLMs的条件推理能力，对其可接受性判断机制尚不清楚。

Method: 使用线性混合效应模型和ANOVA测试，分析不同模型家族、大小和提示策略下的条件可接受性判断。

Result: LLMs对条件概率和语义相关性都敏感，但敏感程度因架构和提示方式而异；与人类数据比较显示模型使用这些线索不如人类一致；模型大小与人类判断一致性无必然关系。

Conclusion: LLMs能够整合概率和语义线索进行条件可接受性判断，但判断机制与人类存在差异，模型规模扩大并不直接提升与人类判断的一致性。
...
TL;DR: Co⁴模型仅用单层、双头、8M参数，在O(N)计算复杂度下，仅训练2个epoch就超越了需要10个epoch的GPT-2(124M)和GPT-BERT(30M)，在BabyLM挑战中展现出极高的训练效率和样本效率。


<details>
  <summary>Details</summary>
Motivation: 挑战当前深度学习范式和扩展定律，探索更高效的小规模模型架构，证明复杂度和参数数量并非性能的唯一决定因素。

Method: 采用Co⁴架构，单层双头设计，仅8M参数，计算复杂度为O(N)，在10M tokens上进行预训练。

Result: 在BabyLM评估中，Co⁴在零样本任务中5/7指标超越GPT-2，4/7超越GPT-BERT；在微调任务中6/7超越GPT-2，4/7超越GPT-BERT，训练效率提升数个数量级。

Conclusion: 结果表明需要重新思考深度学习范式和扩展定律，小规模高效模型架构具有巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLRMs在简单问题上产生不必要的冗长推理轨迹，而在困难问题上探索不足导致错过解决方案，需要平衡推理效率。

Method: 采用两阶段训练：自适应冷启动阶段根据问题难度调整推理轨迹长度；第二阶段使用自适应熵策略优化，以高窗口熵标记作为探索触发器，结合分层熵奖励和动态KL控制。

Result: ARES在多种数学、逻辑和多模态基准测试中实现了优越性能和推理效率，在显著降低推理成本的同时缩小了与领先商业系统的差距。

Conclusion: ARES框架通过自适应推理机制有效平衡了不同难度问题的推理深度，提高了整体推理效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: AI模型应该训练以识别人类判断中的变异和分歧，并通过识别这种变异的能力来评估。LEWIDI系列任务旨在促进这种训练和评估方法。

Method: 扩展LEWIDI基准至四个数据集（释义识别、反讽检测、讽刺检测、自然语言推理），引入序数判断，采用软标签和视角主义两种评估范式，测试新评估指标。
...
<details>
  <summary>Details</summary>
Motivation: 并行推理方法中超过80%的推理路径产生相同答案，造成大量计算浪费，需要解决这一效率瓶颈

Method: 使用专门训练的法官模型预测答案等价性，结合在线贪心聚类算法动态剪枝冗余路径

Result: 在多个基准测试中实现超过80%的token减少，准确率损失控制在3个百分点内
...
TL;DR: 本文探索了在LLM中引入新词（neologism learning）来更好地理解和控制模型，通过添加新词嵌入并训练相关概念示例，实现对概念的控制和模型理解。


<details>
  <summary>Details</summary>
Motivation: 人类在有新概念需求时会创造新词，作者希望验证类似方法在LLM通信中的有效性，通过引入新词来扩展对模型的理解和控制能力。

Method: 通过添加新词嵌入并训练包含该概念的示例，不改变其他模型参数。使用自我言语化让模型描述新词含义，并通过插件评估验证言语化的有效性。

Result: 新词可以控制奉承、错误答案、文本长度等概念，以及AxBench中的复杂概念。模型能自我描述新词含义，发现机器专属同义词，并能同时学习多个概念。

Conclusion: 新词学习是理解和控制LLM的有效方法，能实现概念控制、模型自我解释，并发现人类与机器对词语理解的差异。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法计算成本高，搜索策略效率低，提示候选评分昂贵，需要更有效的提示空间探索方法。

Method: 使用蒙特卡洛树搜索结合代理提示评估器来探索提示空间，降低评估成本。

Result: 实验表明该方法在约束计算预算下比基线方法获得更高的分类准确率和效率。

Conclusion: 所提框架能有效优化提示，提升语言模型在ToS条款公平性检测任务上的性能。
...
TL;DR: RLKV是一种新的KV缓存压缩框架，通过强化学习识别推理关键注意力头，仅对关键头保留完整KV缓存，对其他头应用压缩缓存，在20-50%缓存减少下实现接近无损性能。
...
Method: 提出RLKV框架，使用强化学习直接优化每个头的缓存使用与推理质量关系，识别推理关键头，对关键头保留完整KV缓存，对其他头应用压缩常数KV缓存。

Result: 实验表明只有少量注意力头对推理至关重要，RLKV在20-50%缓存减少下性能优于基线方法，接近未压缩结果。

Conclusion: 注意力头在推理模型中具有功能异质性，RLKV能有效识别推理关键头，实现高效的KV缓存压缩。
...
TL;DR: CoMAS是一个基于多智能体交互的自我进化框架，通过智能体间的讨论和协作生成内在奖励，无需外部监督即可实现自主改进。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的自我进化方法要么依赖外部奖励信号，要么从LLM自身提取内在奖励，这与人类通过相互讨论和协作进行学习改进的机制不符。

Method: CoMAS框架通过智能体间的丰富讨论动态生成内在奖励，采用LLM作为评判机制来制定这些奖励，并通过强化学习优化每个智能体的策略，实现去中心化和可扩展的协同进化。

Result: 实验结果表明，CoMAS在大多数评估设置中始终优于未经训练的智能体，并达到最先进的性能。消融研究证实了基于交互的奖励信号的必要性，并显示出随着智能体数量和多样性增加的良好可扩展性。

Conclusion: CoMAS为基于LLM的智能体自我进化提供了一个新颖且有效的范式，通过多智能体协作实现自主改进。
...
TL;DR: ArenaBencher是一个模型无关的基准测试演进框架，通过自动更新测试案例来应对预训练数据泄露问题，保持基准测试的有效性和可比性。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试因预训练数据泄露而失效，模型通过记忆而非真正泛化来匹配内容，导致分数虚高、跨模型比较失真，无法真实反映进展。

Method: 框架通过推断测试案例核心能力、生成保持原始目标的候选问答对、使用LLM作为裁判验证正确性和意图、聚合多模型反馈来选择暴露共同弱点的候选案例，通过上下文演示迭代生成更具挑战性和诊断性的案例。

Result: 在数学问题解决、常识推理和安全领域应用显示，该框架能产生经过验证、多样化且公平的更新，发现新的失败模式，在保持测试目标对齐的同时增加难度，并提高模型可区分性。

Conclusion: 该框架为基准测试与基础模型快速发展保持同步提供了可扩展的持续演进路径。
...
TL;DR: 基于RT-DETR的实时海上目标检测系统，通过合成图像增强和多尺度特征融合，解决小目标检测和真实数据不足的问题。


<details>
  <summary>Details</summary>
Motivation: 海上目标检测面临小目标尺寸和真实RGB数据标注有限的挑战，需要开发在真实数据上严格评估的实时检测系统。

Method: 采用RT-DETR架构，结合多尺度特征融合、不确定性最小化查询选择和合成-真实数据权重策略，并使用数据增强平衡数据集类别。

Result: 开发了完整的Python海上检测流水线，保持实时性能，验证了各模块贡献及在极端光照和海况下的鲁棒性。

Conclusion: 该系统在保持DETR端到端集合预测优势的同时，允许用户在推理时调整速度与精度平衡，有效缩小合成与真实域间的视觉差距。
...
TL;DR: DynamicEval是一个新的文本到视频评估基准，专注于动态相机运动评估，包含45K人工标注和新的背景场景一致性与前景对象一致性指标。


<details>
  <summary>Details</summary>
Motivation: 现有T2V评估基准存在两个局限：忽视动态相机运动评估，以及仅关注模型级评分而忽略视频级评估。

Method: 构建包含动态相机运动提示词的基准，开发新的背景一致性指标（基于对象错误图修正VBench运动平滑度指标）和前景一致性指标（跟踪对象实例内的点及其邻居）。

Result: 提出的指标在视频级和模型级与人类偏好相关性更强（提升超过2个百分点），建立了更全面的动态相机运动评估基准。

Conclusion: DynamicEval通过解决现有基准的局限性，为T2V模型在动态相机运动下的评估提供了更全面的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法如RED主要关注设计复杂的图像先验来提高重建质量，但收敛加速往往依赖于启发式方法。需要一种既能保证高质量重建又能实现快速收敛的算法。

Method: 在RED基础上引入重启惯性机制，结合基于分数的图像先验。理论分析表明RISP具有比RED更快的驻点收敛速度，且不需要图像先验的凸性假设。

Result: 实验证明RISP在多种成像逆问题中都能实现快速收敛，同时获得高质量的重建结果。

Conclusion: RISP通过重启惯性机制成功解决了成像逆问题中收敛速度与重建质量之间的平衡问题，为相关领域提供了新的解决方案。
...
TL;DR: 本文提出了一种基于图像净化策略的超低剂量CT去噪框架，通过生成结构对齐的uLDCT-NDCT图像对，并结合频域流匹配模型，有效解决了真实临床uLDCT去噪中的数据不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 超低剂量CT显著降低辐射暴露但引入严重噪声和伪影，导致uLDCT与正常剂量CT图像对之间存在显著空间错位，这使得现有基于合成噪声或对齐数据训练的去噪网络难以直接应用。

Method: 首先构建真实临床uLDCT肺部数据集，然后提出图像净化策略生成结构对齐的uLDCT-NDCT图像对，最后提出频域流匹配模型与IP策略协同工作。
...
TL;DR: D2RA是一种无需训练的单图像攻击方法，能够在不访问底层模型的情况下移除或削弱水印，揭示了当前语义水印方案的基本弱点。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型的广泛使用，确保内容归属和来源的水印方法需求日益增长。然而，现有的语义水印方案即使在资源受限的对抗环境下仍然存在脆弱性。

Method: D2RA通过将带水印图像投影到互补表示的自然先验上，在保持视觉保真度的同时抑制水印信号。这是一种无需训练、单图像的攻击方法。

Result: 实验表明，D2RA能够持续降低各种水印方案的可检测性，揭示了当前水印设计的基本弱点。

Conclusion: 当前的水印方案存在根本性弱点，需要更鲁棒的设计来应对此类攻击。
...
TL;DR: PickStyle是一个基于扩散模型的视频风格迁移框架，通过插入低秩适配器和利用配对图像数据进行训练，实现内容保持和风格转换的视频风格迁移。


<details>
  <summary>Details</summary>
Motivation: 解决视频风格迁移任务中缺乏配对视频数据监督的挑战，旨在保持输入视频内容的同时根据文本提示渲染目标风格。

Method: 在预训练视频扩散模型中插入低秩适配器，利用配对静态图像数据训练，通过共享增强构建合成训练片段，并提出上下文-风格分类器自由引导(CS-CFG)方法。
...
TL;DR: 该论文提出TRAVL方法，通过平衡训练数据和轨迹感知注意力模块改进视频语言模型的物理合理性判断能力，并创建ImplausiBench基准来评估模型的视觉-时间理解能力。


<details>
  <summary>Details</summary>
Motivation: 现代视频生成模型经常产生违反物理规律的序列，但目前缺乏量化评估物理真实性的方法。作者探索视频语言模型是否能作为可靠的物理合理性评判者。

Method: 引入TRAVL微调方法，结合平衡训练数据集和轨迹感知注意力模块，改进VLMs的运动编码和判别能力。同时提出ImplausiBench基准，包含300个视频（150真实+150生成），消除语言偏见并隔离视觉-时间理解。

Result: 发现现有VLMs难以识别物理违规，暴露其在时间和因果推理方面的根本局限性。TRAVL方法提高了模型对物理合理性的判断能力。

Conclusion: TRAVL和ImplausiBench为探究和改进多模态模型中的物理合理性提供了统一框架，揭示了视觉-时间理解中具有挑战性且未被充分探索的方面。
...
<details>
  <summary>Details</summary>
Motivation: 解决HSI分类中高质量训练样本有限、数据维度高导致的过拟合问题，以及传统单模态模型仅依赖光谱空间数据的局限性

Method: 使用LLM为每个类别生成文本描述，通过预训练文本编码器提取语义特征，实现语义-光谱-空间特征融合

Result: 在三个HSI基准数据集上取得显著性能提升，验证了文本语义与光谱空间数据的协同作用

Conclusion: 语义增强的HSI分类模型具有良好前景，文本语义信息能有效提升分类性能
...
TL;DR: 提出了CAGUL框架，用于视觉语言模型的遗忘学习，防止模型泄露训练数据中的敏感信息，同时保持原有性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型可能在训练过程中记忆敏感信息并在推理时泄露，需要有效的遗忘机制来解决隐私保护问题。

Method: 利用跨模态注意力机制，在视觉token中编码遗忘信息，通过外部模块实现轻量级遗忘，不改变预训练模型参数。

Result: 实验表明该方法性能优于或等同于微调基线方法，能有效防止信息泄露并保持参考模型行为。
...
TL;DR: MaizeStandCounting (MaSC) 是一个基于RGB图像自动计数玉米幼苗的算法，使用轻量级YOLOv9模型检测V2-V10生长期的玉米，能区分杂草并生成行级株数统计。


<details>
  <summary>Details</summary>
Motivation: 传统人工计数玉米株数耗时费力且容易出错，特别是在大面积或变异较大的田地中，需要自动化解决方案来支持作物管理和研究。

Method: MaSC提供两种处理模式：1) 将拼接图像分割成小块处理；2) 使用单应性矩阵对齐原始视频帧。两种模式都使用轻量级YOLOv9模型检测玉米幼苗，然后基于检测结果的空间分布进行行和范围分割。

Result: 与2024年夏季苗圃实地人工计数相比，MaSC与真实数据高度一致（拼接图像R²=0.616，原始帧R²=0.906）。处理83张全分辨率帧仅需60.63秒，包括推理和后处理。

Conclusion: MaSC被证明是一个可扩展、低成本且准确的自动化玉米株数计数工具，适用于研究和生产环境，具有实时操作潜力。
...
<details>
  <summary>Details</summary>
Motivation: CapsNet在重叠数字检测和仿射变换鲁棒性方面优于传统CNN，但训练和推理速度慢，限制了其在实时应用中的使用。

Method: 通过减少胶囊数量来构建更快的网络，并采用更强大的解码器来进一步提升性能。

Result: 在MNIST、F-MNIST、SVHN和Cifar-10数据集上实现5倍推理速度提升，同时保持接近的准确率。

Conclusion: QCN是开发快速实时CapsNet应用的可行起点，在速度和精度之间取得了良好平衡。
...
TL;DR: Rectified-CFG++是一种自适应预测器-校正器引导方法，解决了标准CFG在整流流模型中的离流形漂移问题，通过几何感知条件规则确保稳定性和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 标准CFG在整流流模型应用中会导致严重的离流形漂移，产生视觉伪影、文本不对齐和脆弱行为，需要更稳定的引导方法。

Method: 采用自适应预测器-校正器引导，每个推理步骤先执行条件RF更新将样本锚定在学习传输路径附近，然后应用加权条件校正来插值条件和无条件速度场。

Result: 在MS-COCO、LAION-Aesthetic和T2I-CompBench等基准数据集上，Rectified-CFG++在Flux、Stable Diffusion 3/3.5、Lumina等大规模文本到图像模型中始终优于标准CFG。

Conclusion: Rectified-CFG++将整流流的确定性效率与几何感知条件规则相结合，确保轨迹保持在数据流形的有界管状邻域内，实现了跨广泛引导强度的稳定性。
...
TL;DR: 提出PIT-QMM，一种新型大型多模态模型，用于无参考点云质量评估，能够端到端处理文本、图像和点云数据，在多个基准测试中显著优于现有方法。
...
Result: 在流行基准测试中，所提方法以显著优势超越现有最先进方法，且训练迭代次数更少。框架还支持失真定位和识别，为模型可解释性和交互性开辟了新途径。

Conclusion: PIT-QMM为无参考点云质量评估提供了一种有效的多模态解决方案，在性能、效率和可解释性方面均表现出色，推动了该领域的发展。
...
TL;DR: 提出双流对齐网络(DSA Net)，通过融合帧级和动作级特征来提升动作分割性能，首次引入混合量子-经典机器学习框架。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注单流时空建模，而双流方法能学习动作特征来增强分割性能。研究第二流动作特征对分割的指导作用，捕捉动作和动作转换线索。

Method: 使用双流架构（帧级和动作级），通过时序上下文块融合互补信息，采用交叉注意力和量子动作引导调制。提出双流对齐损失，包含关系一致性、跨层对比和循环一致性重建损失。

Result: 在GTEA、Breakfast、50Salads和EgoProcel等基准数据集上达到最先进性能，显著优于现有方法。消融研究验证了各组件有效性。

Conclusion: 双流对齐网络通过特征对齐和混合量子-经典框架，在动作分割任务中取得了突破性进展，证明了双流方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于U-Net的双分支架构在扩散模型中成功，但难以适应基于Diffusion Transformer的模型，存在参数过多和服装特征缺乏时序性的问题

Method: 使用基于图像的服装转移模型替换首帧服装，然后在编辑后的首帧内容控制下，利用姿态和掩码信息引导视频生成模型顺序合成剩余帧

Result: 实验表明该方法在参数效率和计算效率上表现优越，同时在约束条件下保持领先性能

Conclusion: OIE方法通过首帧替换策略有效解决了Diffusion Transformer中虚拟试穿的挑战，实现了高效且性能良好的视频服装替换
...
TL;DR: 提出一种使用IP-Adapter自动生成掩码的方法，在第二次推理过程中将图像标记限制在主体区域，让文本提示能够更好地控制背景生成，提高个性化扩散模型的提示对齐度。
...
Method: 利用IP-Adapter自动生成的掩码，在第二次推理过程中对图像标记进行掩码处理，将主体与背景分离，使文本提示能够更好地控制背景内容的生成。

Result: 相比其他测试时个性化方法，该方法在位置和场景描述提示下能够准确描绘主体并高度匹配提示内容，显示出更高的提示和源图像对齐度。

Conclusion: 通过自动掩码技术限制图像标记到主体区域，可以有效提升个性化扩散模型对文本提示的响应能力，生成更符合用户意图的图像。
...
Method: 比较了标准Transformer方法、小型VLM(Phi3.5-vision)、大型预训练VLM(Gemini)以及处理多图像的扩展Transformer。
...
TL;DR: 提出TCIP模型，通过特征增强残差模块(FERM)解决金字塔网络中的解剖结构错位累积问题，采用双阶段阈值控制迭代(TCI)策略自适应确定迭代次数，在多个医学影像数据集上优于现有方法。
...
Method: 1. 提出特征增强残差模块(FERM)，包含三个顺序块：提取解剖语义特征、抑制无关特征、估计最终变形场；2. 提出双阶段阈值控制迭代(TCI)策略，第一阶段评估配准稳定性，第二阶段评估收敛性；3. 将FERM和TCI集成构建TCIP模型。

Result: 在三个公共脑部MRI数据集和一个腹部CT数据集上的实验表明，TCIP在准确性方面优于最先进的配准网络，同时保持相当的推理速度和紧凑的模型参数规模。

Conclusion: TCIP模型有效解决了金字塔网络中的解剖错位累积问题，并通过自适应迭代策略提升了配准精度，FERM和TCI组件具有良好的通用性，可集成到现有配准网络中。
...
TL;DR: 提出了一种视频合成方法，通过变分推断来近似组合分布，利用多个视频生成主干网络共同处理所有任务约束，实现了对指定元素的高可控性和对未指定元素的多样性保持。
...
Method: 将任务建模为变分推断来近似组合分布，使用多个视频生成主干网络；通过逐步KL散度最小化解决优化挑战，并提出上下文条件因子化技术减少解空间模式以避免局部最优。
...
TL;DR: 本文提出了一种结合BYOL和CNN的混合方法，用于通过热成像对感应电机进行故障检测分类，新提出的BYOL-IMNet模型在测试准确率和推理时间上均优于现有先进模型。


<details>
  <summary>Details</summary>
Motivation: 感应电机在工业和日常生活中不可或缺，但容易发生各种故障导致过热、能耗浪费和服务中断。早期故障检测对于保护电机和延长其使用寿命至关重要。

Method: 提出BYOL与CNN结合的混合方法，使用多种深度学习模型（ResNet-50、DenseNet-121/169、EfficientNetB0、VGG16、MobileNetV2）进行BYOL技术，并设计了一个新的轻量级CNN模型BYOL-IMNet，包含四个定制设计的块专门用于热图像故障分类。

Result: 实验结果表明，提出的BYOL-IMNet达到99.89%的测试准确率，每张图像的推理时间为5.7毫秒，优于现有最先进模型。

Conclusion: 这项研究突出了CNN-BYOL混合方法在提高感应电机故障检测准确性方面的优异性能，为工业环境中的在线监测提供了鲁棒的方法论。
...
TL;DR: 提出了一种名为MLH的弱到强学习框架，通过让基于中心的强分支从基于成对的弱分支中学习局部相似性信息，解决了中心方法在捕捉全局结构时忽略局部相似性的问题。


<details>
  <summary>Details</summary>
Motivation: 基于中心的方法在捕捉全局数据分布方面表现出色，但往往未能充分利用重要的局部相似性信息。为了弥补这一局限，需要设计一种能够结合两种方法优势的框架。

Method: MLH包含两个分支：基于中心的强分支和基于成对的弱分支。通过迭代式相互学习过程，强分支从弱分支学习局部相似性线索。还引入了混合哈希专家模块，实现有效的跨分支交互。

Result: 在多个基准数据集上的广泛实验表明，MLH始终优于最先进的哈希方法。

Conclusion: MLH框架通过弱到强的知识迁移和混合哈希专家模块，成功地将局部相似性信息整合到基于中心的方法中，显著提升了哈希检索性能。
...
TL;DR: 提出Repainter强化学习框架，结合空间遮罩轨迹优化和GRPO策略，解决电商产品图像中水印和促销文本去除问题，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 电商平台中产品图像对用户参与和广告效果至关重要，但水印和促销文本等侵入元素阻碍了清晰视觉呈现。现有扩散修复方法在商业场景中面临对象去除不可靠和领域适应性有限的问题。

Method: Repainter框架整合空间遮罩轨迹优化与Group Relative Policy Optimization (GRPO)，通过调节注意力机制强调背景上下文，生成高奖励样本并减少不需要的对象插入。引入复合奖励机制平衡全局、局部和语义约束。

Result: 大量实验表明Repainter显著优于最先进方法，特别是在复杂构图场景中。同时贡献了EcomPaint-100K数据集和EcomPaint-Bench基准。

Conclusion: Repainter通过强化学习有效解决了电商图像修复中的关键挑战，在复杂场景下表现出色，为电商视觉优化提供了有效解决方案。
...
TL;DR: SyncHuman提出了一种结合2D多视图生成模型和3D原生生成模型的新框架，用于从单视角图像实现高质量3D穿衣人体网格重建，即使在挑战性姿态下也能保持几何精度和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖SMPL估计和条件图像生成模型，但存在3D先验不准确、难以处理复杂人体姿态和重建细节的问题。需要结合2D细节捕捉能力和3D结构一致性的互补优势。

Method: 联合微调多视图生成模型和3D原生生成模型，使用像素对齐的2D-3D同步注意力机制；引入特征注入机制将2D多视图图像的精细细节提升到对齐的3D形状上。

Result: 在几何精度和视觉保真度方面优于基线方法，即使在挑战性姿态图像下也能实现鲁棒且逼真的3D人体重建。

Conclusion: SyncHuman展示了结合2D和3D生成模型的有效性，为未来3D生成模型提供了有前景的方向。
...
TL;DR: 本文提出了ComGS框架，通过Surface Octahedral Probes实现高效的可重光照物体重建，结合基于扩散模型的场景光照估计，解决了高斯溅射中3D物体-场景组合的光照不一致问题，实现了28FPS的实时渲染和36秒的快速编辑。


<details>
  <summary>Details</summary>
Motivation: 高斯溅射(GS)虽然能实现沉浸式渲染，但在3D物体-场景组合时存在挑战。由于GS辐射场中烘焙的外观和阴影信息，导致物体与场景组合时出现光照不一致问题。需要可重光照的物体重建和场景光照估计来解决这个问题。

Method: 1. 提出Surface Octahedral Probes(SOPs)存储光照和遮挡信息，通过插值实现高效3D查询，避免昂贵的射线追踪，提供至少2倍的重建加速和实时阴影计算。2. 简化全场景光照估计，专注于物体放置位置的环境光照，通过360度重建辐射场和微调扩散模型来完成光照。

Result: ComGS框架实现了高质量实时渲染(约28FPS)，产生视觉和谐的结果和生动的阴影，编辑仅需36秒。SOPs在重建速度上提供至少2倍加速，并支持高斯场景中的实时阴影计算。

Conclusion: ComGS通过创新的SOPs技术和简化的光照估计方法，成功解决了高斯溅射中3D物体-场景组合的光照不一致问题，实现了高效、实时的渲染和快速编辑，为3D场景组合提供了实用解决方案。
...
TL;DR: 本文提出UltraLED框架，仅使用单帧短曝光RAW图像实现超高清动态范围重建，通过两阶段方法避免重影和运动模糊问题。


<details>
  <summary>Details</summary>
Motivation: 解决UHDR场景中亮暗区域曝光差异大的问题，传统RGB包围曝光方法易产生重影和对齐问题，而单帧RAW图像具有更高位深和可预测噪声特性。

Method: 提出两阶段框架：首先通过比率图进行曝光校正平衡动态范围，然后使用亮度感知RAW去噪器增强暗区细节恢复。构建9档包围曝光管道合成真实UHDR图像数据集。

Result: 实验表明UltraLED显著优于现有单帧方法，在动态场景中表现稳健。

Conclusion: 证明仅使用单帧短曝光RAW图像即可有效重建UHDR场景，为动态场景下的高动态范围成像提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 低帧率RGB视频重建动态3D高斯溅射面临大帧间运动导致解空间不确定性的挑战，而事件相机能异步捕捉快速视觉变化且抗运动模糊，但缺乏颜色信息。结合两种模态可以互补优势。
...
Conclusion: 通过结合RGB和事件模态，利用事件运动先验指导变形场优化，成功解决了低帧率视频重建动态3D高斯溅射时的大帧间运动不确定性挑战。
...
<details>
  <summary>Details</summary>
Motivation: 提高脑肿瘤分割的准确性，帮助医生进行临床决策，通过XAI技术澄清模型决策过程并增加医生对这些模型的信任。

Method: 使用BraTS2020公共数据集，评估三种UNet变体模型（UNet、ResUNet、AttUNet），采用Adam优化器训练，使用Grad-CAM和注意力可视化技术分析模型预测。

Result: 在最终测试阶段，ResUNet在Dice和Jaccard相似性分数以及准确率、召回率和F1分数方面优于其他模型，XAI技术提供了对模型关注区域的视觉洞察。

Conclusion: ResUNet是表现最佳的模型，推荐将其用于未来临床评估中的自动化脑肿瘤分割。
...
TL;DR: 该论文提出了GTR-Bench基准，用于评估视觉语言模型在地理时空推理方面的能力，特别是在大规模摄像头网络中追踪移动目标。实验表明当前最先进的模型性能远低于人类水平，并揭示了模型在时空上下文利用、时间预测和地图理解方面的三大缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有时空基准主要关注以自我为中心的图像/视频推理或基于图形的推理，缺乏同时结合图像/视频和图形上下文的地理时空智能评估，这对于交通管理和应急响应等领域至关重要。

Method: 构建GTR-Bench基准，要求模型在大型摄像头网络中进行地理时空推理，包括地图和视频间的多视角切换、跨多个非重叠视野视频的联合推理，以及对视频未覆盖时空区域的推断。

Result: 评估了10多个流行VLM，最佳专有模型Gemini-2.5-Pro准确率仅34.9%，远低于人类表现78.61%。分析揭示了模型在时空上下文利用不平衡、时间预测能力弱、地图与多视角视频对齐能力不足三大缺陷。

Conclusion: GTR-Bench为时空智能研究提供了有价值的见解和新机遇，揭示了当前VLM在地理时空推理方面的显著局限性，为未来模型改进指明了方向。
...
TL;DR: 提出了一种名为MM-COF的综合运动表示方法和FMANet神经网络架构，通过整合微表情两个阶段的运动信息来提升微表情识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅计算起始到顶点帧之间的光流，忽略了顶点到偏移阶段的重要运动信息，限制了微表情识别的准确性。

Method: 开发了MM-COF运动表示方法整合两个阶段的运动动态，并设计了FMANet端到端神经网络，包含可学习的双相分析和幅度调制模块。

Result: 在MMEW、SMIC、CASME-II和SAMM四个标准基准数据集上的实验表明，所提方法优于现有方法。

Conclusion: 可学习的双相框架在推进微表情识别方面具有巨大潜力，能够自适应融合运动线索并关注显著面部区域。
...
TL;DR: 提出基于房间几何约束的深度估计框架，通过布局预测提取房间几何信息，并通过背景分割机制整合到深度估计过程中，显著提升了球形像素深度预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法关注像素级精度，导致房间角落过度平滑和噪声敏感问题，需要利用房间几何约束来改进深度估计质量。

Method: 使用共享特征编码器提取多尺度特征，然后通过任务特定解码器进行布局估计、深度估计和背景分割，并整合房间几何背景深度解析策略和背景分割引导融合机制。

Result: 在Stanford2D3D、Matterport3D和Structured3D数据集上的实验结果表明，该方法相比现有开源方法取得了显著优越的性能。

Conclusion: 提出的房间几何约束深度估计框架能够有效利用几何信息，显著提升单目360度室内全景的深度预测质量。
...
TL;DR: ACAVP通过引入仿射变换和颜色变换增强视觉提示的表达能力，并使用TrivialAugment数据增强解决过拟合问题，在12个图像分类数据集上达到VP方法的最先进准确率。
...
Method: 提出ACAVP方法，结合仿射变换（保留原始图像信息）、颜色变换（强调任务相关特征）和加法变换；引入TrivialAugment数据增强解决过拟合。

Result: 在12个数据集上达到VP方法SOTA，平均准确率超过线性探测，对分布偏移具有更强鲁棒性，推理时计算开销极小。

Conclusion: ACAVP通过增强变换操作和有效数据增强，显著提升了视觉提示的性能和鲁棒性，同时保持参数效率优势。
...
TL;DR: 提出了MMHOI数据集和MMHOI-Net方法，用于解决多人与多物体交互的3D建模问题，在准确性和重建质量上达到最先进水平
...
Method: 基于MMHOI数据集，开发了MMHOI-Net端到端transformer网络，使用结构化双补丁表示来建模物体及其交互，结合动作识别增强交互预测

Result: 在MMHOI和CORE4D数据集上的实验表明，该方法在多HOI建模方面达到最先进性能，在准确性和重建质量上表现出色

Conclusion: MMHOI数据集和MMHOI-Net方法为下一代HOI研究提供了全面的测试平台，能够有效建模复杂的多人多物体交互场景
...
TL;DR: PrismGS是一个基于物理正则化的3D高斯泼溅框架，通过金字塔多尺度监督和显式尺寸正则化解决大场景渲染中的走样问题，在4K渲染下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)在大规模城市场景中面临严重的走样伪影和优化不稳定问题，特别是在高分辨率渲染时会出现闪烁纹理和锯齿边缘。现有方法虽然解决了可扩展性，但无法解决保真度差距。

Method: 提出PrismGS框架，包含两个协同正则化器：1) 金字塔多尺度监督，通过预滤波图像金字塔强制模型学习抗走样表示；2) 显式尺寸正则化，对3D高斯施加物理基础的下界约束，防止形成退化的视图相关基元。

Result: 在MatrixCity、Mill-19和UrbanScene3D数据集上的实验表明，PrismGS实现了最先进的性能，相比CityGaussian获得约1.5 dB的PSNR增益，在4K渲染下保持优越质量和鲁棒性。

Conclusion: PrismGS是一个即插即用的正则化框架，通过物理基础的正则化方法有效解决了3DGS在大场景渲染中的走样问题，显著提升了渲染质量和稳定性。
...
TL;DR: 提出IsoSignVid2Aud端到端框架，直接将孤立手语视频翻译为语音，无需中间文本表示，避免多阶段系统的延迟和级联错误。


<details>
  <summary>Details</summary>
Motivation: 连接听力和语言障碍人群与其他人，特别是在教育应用和手语提示界面中使用的孤立手语序列视频。

Method: 结合I3D特征提取模块、专门的特征转换网络和音频生成管道，使用新颖的NMS算法进行非语法连续序列中手语的时间检测。

Result: 在ASL-Citizen-1500和WLASL-100数据集上分别达到72.01%和78.67%的Top-1准确率，音频质量指标（PESQ: 2.67, STOI: 0.73）表明可理解的语音输出。

Conclusion: 提出的端到端框架能够有效实现手语到语音的直接翻译，在孤立手语序列翻译任务中表现出竞争力。
...
TL;DR: AlignGS是一个新颖的3D重建框架，通过协同优化几何和语义，利用2D基础模型的先验知识来指导3D表示，在稀疏视图下实现更准确和完整的室内场景重建。


<details>
  <summary>Details</summary>
Motivation: 当前方法通常将语义视为被动特征，在已有几何上添加语义信息，但稀疏视图重建面临几何模糊性问题。作者认为语义理解应作为主动指导力量来提升重建质量。

Method: 提出AlignGS框架，通过端到端协同优化几何和语义，从2D基础模型提取丰富先验，并设计深度一致性和多面法线正则化等语义到几何的指导机制。

Result: 在标准基准测试中达到最先进的性能，在新视角合成和几何精度方面表现优异，验证了语义先验作为几何正则化器的有效性。

Conclusion: 利用语义先验作为几何正则化器能够从有限输入视图中生成更一致和完整的3D模型，为稀疏视图重建提供了新思路。
...
TL;DR: 本文探讨了利用自监督学习表示来识别毒物诱导变化的方法，使用斑马鱼胚胎数据集验证了该方法的有效性，并讨论了在TOXBOX项目中集成机器学习模型的实际应用。


<details>
  <summary>Details</summary>
Motivation: 高通量毒性测试需要快速有效的自动化评估方法，而机器学习模型是实现这一目标的关键组件。

Method: 采用自监督学习来学习表示，并使用公开的EmbryoNet数据集（包含十种斑马鱼胚胎表型）进行概念验证。

Result: 分析表明，通过自监督学习获得的表示能够有效区分不同化合物的作用模式。

Conclusion: 自监督学习表示在毒性测试中具有应用潜力，并可在物理毒性测试设备中集成使用。
...
<details>
  <summary>Details</summary>
Motivation: 现有feedforward重建范式在驾驶场景重建中存在两个主要问题：(1) 固定视角变换在相机配置改变时失效，限制了跨不同驾驶场景的泛化能力；(2) 360°全景稀疏视图间重叠区域小且驾驶场景复杂，增加了学习难度，降低了重建精度。

Method: 采用统一圆柱体提升方法，包括：统一圆柱相机建模(UCCM)策略避免学习视角依赖的空间对应关系，用可调参数统一不同相机配置；基于新设计的圆柱平面特征组(CPFG)的混合表示和专用模块，将2D图像特征提升到3D空间。

Result: 实验结果表明XYZCylinder在不同评估设置下实现了最先进的性能，并能以零样本方式泛化到其他驾驶场景。

Conclusion: XYZCylinder通过统一的圆柱体提升方法有效解决了驾驶场景重建中的泛化能力和精度限制问题，为跨不同相机配置的驾驶场景重建提供了有效解决方案。
...
TL;DR: MARC是一种基于记忆增强强化学习的token压缩方法，通过检索关键视频片段和策略优化蒸馏，在仅使用1帧token的情况下实现95%视觉token压缩，同时保持接近基准的准确率。
...
TL;DR: 提出了ASBench，首个专门评估异常合成方法的综合基准框架，包含四个关键评估维度，揭示了当前方法的局限性并为未来研究提供方向


<details>
  <summary>Details</summary>
Motivation: 异常检测在制造业质量控制中很重要，但受限于异常样本稀少和标注成本高。现有研究将异常合成作为异常检测的辅助组件，缺乏系统评估，且忽视了异常合成的特定关键因素
...
Conclusion: ASBench不仅识别了现有方法的不足，还为异常合成领域的未来研究提供了可行的见解和方向
...
TL;DR: 提出TTOM框架，通过测试时优化和记忆机制改进视频基础模型在组合场景中的表现，无需训练即可实现更好的文本-图像对齐。
...
Method: TTOM框架在推理时通过布局注意力目标优化新参数，采用参数化记忆机制维护历史优化上下文，支持插入、读取、更新和删除等操作。

Result: 实验结果表明TTOM在T2V-CompBench和Vbench基准测试中表现优异，能够实现组合视频生成的跨模态对齐。

Conclusion: TTOM是一个有效、实用、可扩展且高效的框架，能够在推理时实现组合视频生成的跨模态对齐，并展现出强大的可迁移性和泛化能力。
...
Method: 首先微调VAE进行辅助4D重建任务，增强3D结构和时间动态编码能力，然后将该VAE集成到视频扩散过程中。

Result: 实验结果显示在FID和FVD指标上有显著提升，联合训练的高斯泼溅解码器能有效重建动态场景。

Conclusion: 该方法为全面场景理解提供了有价值的几何信息，在视频生成和4D重建方面表现出色。
...
Method: 提出SimCast训练管道，采用短长期知识蒸馏技术和加权MSE损失来优先考虑强降雨区域；进一步将SimCast集成到CasCast扩散框架中，结合概率模型优势克服确定性输出的模糊性和分布偏移问题。

Result: 在三个基准数据集上验证了框架有效性：SEVIR上平均CSI得分为0.452，HKO-7上为0.474，MeteoNet上为0.361，显著优于现有方法。

Conclusion: SimCast和CasCast框架在不增加推理开销的情况下显著提升了降水临近预报的准确性，结合了确定性和概率性模型的优势。
...
TL;DR: Latent Harmony是一个用于超高清图像恢复的两阶段框架，通过重新定义变分自编码器，在潜在空间进行联合正则化并强制高频感知重建，有效平衡计算效率与高频细节保留。


<details>
  <summary>Details</summary>
Motivation: 解决超高清图像恢复中计算效率与高频细节保留之间的权衡问题。传统VAE的高斯约束会丢弃退化特定的高频信息，影响重建保真度。

Method: 两阶段框架：第一阶段提出LH-VAE，通过视觉语义约束和渐进退化扰动增强语义鲁棒性，潜在等变性加强高频重建；第二阶段联合训练改进的VAE与恢复模型，使用高频低秩适应(HF-LoRA)模块，包括编码器LoRA和解码器LoRA，通过交替优化和选择性梯度传播进行训练。

Result: 实验表明Latent Harmony在超高清和标准分辨率任务上达到最先进性能，有效平衡效率、感知质量和重建准确性。

Conclusion: 该框架通过重新定义VAE在图像恢复中的应用，成功解决了效率与保真度的权衡问题，提供了一个可调节的保真度-感知权衡参数，在多个任务上表现出色。
...
TL;DR: 本文探讨了在图像隐私分类任务中，对象标签和抽象标签的适用性差异。研究发现：当标签预算有限时，抽象标签更有效；当每张图像可获得更多标签时，对象相关信息同样有用。


<details>
  <summary>Details</summary>
Motivation: 对象标签表示具体实体，而抽象标签捕获更高层次的上下文信息。图像隐私是一个依赖上下文且具有主观性的任务，需要探索哪种标签类型更适合此类任务。

Method: 通过对比分析对象标签和抽象标签在图像隐私分类中的表现，研究在不同标签数量条件下的效果差异。

Result: 当标签预算有限时，抽象标签比对象标签更有效；当可获得更多标签时，对象相关信息同样有用。

Conclusion: 这些发现将为未来开发更准确的图像隐私分类器提供指导，需要考虑标签类型和数量的作用。
...
TL;DR: 本文比较了Transformer模型SwinIR与轻量级CNN在低光图像恢复任务中的性能与效率权衡。虽然SwinIR达到更高的PSNR(39.03dB)，但CNN以37.4dB的竞争性性能、仅10个训练轮次和55倍更小的模型尺寸，提供了更好的计算效率。
...
Method: 通过对比实验，将最先进的Transformer模型SwinIR与标准轻量级卷积神经网络(CNN)在低光图像恢复任务上进行系统比较，分析两者的训练效率、模型大小和性能表现。

Result: SwinIR获得更高的峰值性能(PSNR 39.03dB)，但轻量级CNN达到竞争性的37.4dB PSNR，且仅需10个训练轮次(相比SwinIR的132轮次)，模型尺寸小55倍。

Conclusion: 标准CNN能够以显著降低的计算开销提供接近最先进的性能，在资源受限的实际应用场景中具有明显优势，为低光图像恢复任务提供了更实用的解决方案。
...
TL;DR: GraphEnet：首个将图神经网络应用于事件相机数据进行人体姿态估计的方法，利用事件数据的稀疏性和基于线的中间表示，实现高频单人体姿态估计。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有低延迟、低能耗优势，适合便携设备和移动机器人应用。传统RGB相机方法在资源受限场景下表现不佳，需要开发专门针对事件数据的姿态估计方法。

Method: 提出GraphEnet图神经网络，利用事件相机输出的稀疏特性，采用基于线的中间事件表示，结合新颖的偏移向量学习范式和基于置信度的池化来估计人体姿态。

Result: 这是首个将图神经网络应用于事件数据的人体姿态估计工作，实现了高频姿态估计，代码已开源。

Conclusion: GraphEnet成功证明了图神经网络在事件相机人体姿态估计中的有效性，为资源受限环境下的实时人机交互应用提供了新解决方案。
...
TL;DR: CIR-CoT是一个端到端的检索导向多模态大语言模型，通过引入显式的思维链推理来提升组合图像检索的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决现有组合图像检索方法作为"黑盒"的问题，缺乏可解释性和处理复杂细粒度指令的能力。

Method: 设计包含标题、推理和结论的三阶段结构化思维链标注，通过微调模型生成结构化输出，并将最终检索意图编码到专用嵌入中。

Result: 在领域内数据集(FashionIQ、CIRR)上取得有竞争力的性能，在领域外CIRCO数据集上展现出卓越的泛化能力。

Conclusion: CIR-CoT为构建更有效和可信赖的检索系统开辟了新路径，通过显式推理链增强了跨模态交互理解和决策透明度。
...
<details>
  <summary>Details</summary>
Motivation: 相机感知系统由于缺乏显式深度信息，在3D目标检测等任务中难以生成准确预测。为了解决深度估计的模糊性问题，需要开发能够利用协作信息的视觉感知方法。

Method: 提出RayFusion方法，利用来自协作车辆的射线占用信息，减少沿相机射线的冗余和误报预测，从而增强纯相机协作感知系统的检测性能。
...
TL;DR: 提出了RASALoRE：一种新颖的两阶段弱监督脑部MRI异常检测框架，通过区域感知空间注意力和基于位置的随机嵌入，在仅使用切片级标签的情况下实现准确的异常检测。
...
Method: 两阶段框架：第一阶段使用判别性双提示调优机制生成高质量的伪弱掩码；第二阶段使用具有区域感知空间注意力的分割网络，依赖基于位置的固定随机嵌入来聚焦异常区域。

Result: 在BraTS20、BraTS21、BraTS23和MSD数据集上的广泛评估显示，该方法实现了最先进的异常检测性能，显著优于现有WSAD方法，同时使用少于800万参数，计算复杂度显著降低。

Conclusion: RASALoRE框架在弱监督脑部MRI异常检测中表现出色，实现了性能提升和计算效率的平衡，为医学图像分析提供了有效的解决方案。
...
TL;DR: RetouchLLM是一个无需训练的白盒图像润色系统，通过视觉评论家和代码生成器模块，直接在原始图像上执行可解释的代码润色，支持多步骤调整和自然语言交互。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法需要大规模配对数据且作为黑盒运行，润色过程不透明，难以处理多样化的用户或图像特定调整。

Method: 系统包含视觉评论家模块（识别输入与参考图像差异）和代码生成器模块（生成可执行代码），以类似人类多步骤润色的方式逐步增强图像。

Result: 实验表明该方法能很好泛化到不同润色风格，自然语言交互支持根据用户意图进行可解释和可控的调整。

Conclusion: RetouchLLM提供了一种无需训练数据、可解释的白盒图像润色解决方案，通过代码执行和多步骤调整实现灵活可控的图像增强。
...
TL;DR: 提出了一种多时序类别驱动的分层残差神经网络，用于在不同语义层级对多光谱图像时间序列进行分类，通过引入分层惩罚映射和模块化架构来提高分类准确性。


<details>
  <summary>Details</summary>
Motivation: 为了解决多光谱图像时间序列分类中不同语义层级类别的区分问题，并利用有限的训练样本引入新类别和任务，需要设计能够有效建模类别层次结构的网络架构。

Method: 修改ResNet架构，引入额外分支在不同层次进行分层分类，使用分层惩罚映射避免不一致的层次转换，通过类别层次标签高效训练不同层级的网络模块。

Result: 在亚马逊森林两个区域的2019年12个月Sentinel 2图像上的实验表明，该方法在不同层次具有良好的泛化能力，在微观类别级别实现了准确分类，并更好地表示了少数类别。

Conclusion: 所提出的模块化网络具有内在适应能力，通过分层方法有效提高了分类性能，特别是在微观类别级别和少数类别表示方面表现优异。
...
TL;DR: RedFace是一个专门的面部深度伪造数据集，包含6万张伪造图像和1000个处理视频，使用9个商业在线平台生成，模拟真实世界的黑盒场景，旨在弥合学术评估与现实需求之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测的学术评估和基准测试缺乏针对性、深度伪造多样性有限、操纵技术受限，无法有效应对现实世界威胁。

Method: 利用9个商业在线平台整合最新的深度伪造技术，使用定制算法合成深度伪造内容，模拟真实世界场景。

Result: 在RedFace上的广泛实验验证了现有深度伪造检测方案在现实应用中的实用性有限。

Conclusion: RedFace数据集能够更好地评估深度伪造检测方法在现实世界中的有效性，揭示了与传统数据集相比对检测性能的影响原因。
...
Method: 提出归一化时空梯度统计量，量化空间概率梯度与时间密度变化之比；利用预训练扩散模型开发NSG估计器，通过空间梯度近似和运动感知时间建模；基于NSG特征的最大均值差异作为检测指标。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中由于隐私保护和知识产权问题，难以获取训练数据来植入后门，因此需要开发无需训练数据访问的后门攻击方法。

Method: 设计双语义引导的阴影后门攻击框架，仅使用替代数据集微调受害者模型的特定层，并利用拓扑对齐损失优化个体和相邻中毒样本向目标样本对齐。

Result: 在四个图像数据集、五种模型架构和两种哈希方法上的实验表明，DarkHash具有高有效性，优于现有最先进的后门攻击方法，并能抵御主流后门防御方法。

Conclusion: DarkHash成功实现了无需训练数据访问的后门攻击，在保持原始检索精度的同时增强了攻击能力，为深度哈希模型的安全性提出了新的挑战。
...
TL;DR: 提出一种基于3D高斯泼溅的标签细化方法，通过联合训练RGB和分割图的3DGS模型，从多视角预测中生成准确的分割掩码，显著提升极端视角下的人脸解析精度。


<details>
  <summary>Details</summary>
Motivation: 极端视角下的人脸解析由于缺乏标注数据而面临挑战，手动标注成本高昂且难以规模化。

Method: 联合训练两个3D高斯泼溅模型（RGB图像和初始分割图），通过共享几何结构强制多视角一致性，合成姿态多样的训练数据。

Result: 在挑战性头部姿态上显著提升解析精度，同时在标准视角上保持良好性能，无需3D真值标注且仅需少量初始图像。

Conclusion: 该方法为提升真实场景下人脸解析的鲁棒性提供了可扩展且有效的解决方案。
...
TL;DR: 本文提出了一种CT专用的数据增强方法——随机窗位调整，通过利用CT图像中HU值的物理特性来替代传统的强度增强方法，在肝脏肿瘤分割任务中显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统为自然图像设计的强度增强方法直接应用于CT图像时，忽视了HU值的物理意义，可能导致伪影和泛化能力差的问题。

Method: 提出随机窗位调整方法，基于CT图像中HU值的分布特性进行数据增强，增强模型对对比度变化和扫描时间的鲁棒性。

Result: 在多个数据集上的实验表明，该方法优于现有先进方法，特别是在对比度差或扫描时机不佳的图像上表现突出。

Conclusion: CT特定的数据增强方法比通用的强度增强更有效，随机窗位调整能够显著提升肝脏肿瘤分割模型的性能和泛化能力。
...
TL;DR: AR-Drag是首个RL增强的少步自回归视频扩散模型，用于实时图像到视频生成，支持多样化运动控制，显著降低了延迟并保持高视觉质量。
...
TL;DR: 该论文针对视频语言模型中存在的时间逻辑不一致性问题，提出了一种注意力增强方法TCAS来提升模型的时间分辨能力，从而改善时间理解的一致性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生自相矛盾的输出，这严重影响了其可靠性并阻碍了在实际应用中的采用。在视频语言模型中，这种现象最近引起了研究人员的关注。具体来说，这些模型无法基于其基础输出为重新表述的问题提供逻辑一致的响应。然而，这种现象的根本原因仍未得到充分探索。

Method: 采用可解释性驱动的方法来分析、统计总结和干预这种现象的潜在因素。提出了时间条件注意力锐化(TCAS)方法，该方法基于注意力差异构建增强目标，以增强模型的时间分辨能力。

Result: 实验结果表明，该方法显著增强了视频语言模型的时间逻辑一致性。进一步的可解释性分析表明，该方法确实提高了注意力头的时间区分能力。此外，该方法在通用视频时间定位任务中实现了性能提升。
...
Conclusion: UniMMVSR成功解决了现有方法在多模态条件利用方面的局限性，为高质量视频生成提供了有效的统一框架，实现了之前无法达到的4K视频多模态生成能力。
...
TL;DR: 提出MURE框架，通过多模态推理链将图像编辑从纯文本推理转变为文本和视觉交替的推理过程，解决了现有方法在复杂对象交叉和空间关系处理上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于自然语言的图像编辑方法在处理复杂对象交叉和细粒度空间关系时存在困难，因为缺乏显式推理过程。纯文本或坐标增强的CoT方法在表示复杂视觉布局和生成像素级细节方面存在根本限制。

Method: 提出多模态推理编辑框架，使用原生多模态的文本-图像交替推理链，每个步骤包含文本描述和相应的视觉提示（如位置掩码或新内容表示）。引入多模态深度置信推理范式，通过奖励模型修剪低质量分支，确保高质量推理轨迹。

Result: 在三个图像编辑基准测试中取得显著改进，发布了首个CoT-Edit-14K数据集，包含14K个高质量编辑示例。

Conclusion: MURE框架通过将复杂编辑任务分解为相互依赖的子任务，在每阶段实现更高精度，产生高保真度的编辑结果，证明了多模态推理链在图像编辑中的有效性。
...
TL;DR: 提出一种自举算法，通过迭代重新对齐训练样本来改进细粒度视觉分类中的几何偏差处理，优于等变和规范化基线方法。


<details>
  <summary>Details</summary>
Motivation: 细粒度视觉分类任务需要处理几何偏差和噪声，现有方法依赖数据增强或等变架构，但前者需要强大模型，后者限制表达能力。规范化方法需要对齐的训练数据假设，而真实数据集无法满足这一假设。

Method: 提出自举算法，通过迭代减少方差和恢复对齐假设来逐步重新对齐训练样本，为任意紧致群建立收敛保证。

Result: 在四个细粒度视觉分类基准测试中，该方法始终优于等变和规范化基线方法，与数据增强方法性能相当。

Conclusion: 自举规范化方法能有效处理细粒度视觉分类中的几何偏差问题，无需依赖对齐训练数据的强假设。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本编辑方法难以精确定位对象，而对象拖拽方法仅限于静态重定位。为了克服这些限制，需要开发一种能够同时实现精确对象定位和语义控制的图像编辑方法。

Method: 将对象拖拽视为图像重建过程，分为两个协同分支：移动重建分支使用基于能量的梯度引导来精确移动对象，并优化交叉注意力图；文本驱动编辑分支与重建分支共享梯度信号，确保一致的变换。同时采用DDPM反演和噪声图先验信息注入来保持对象结构。

Result: 广泛的实验表明，InstructUDrag能够实现灵活、高保真度的图像编辑，在对象重定位精度和图像内容语义控制方面都表现出色。

Conclusion: InstructUDrag成功结合了文本指令和对象拖拽的优势，为图像编辑提供了更精确的定位能力和更丰富的语义控制能力，是一个有效的图像编辑解决方案。
...
Method: 三阶段方法：1) 自学习阶段：通过大语言模型将双人整体文本分解为个体文本，在个体层面对齐文本和运动特征；2) 自适应调整阶段：预测交互距离，通过交互感知图网络在个体间层面动态建模人类交互；3) 教师引导精炼阶段：利用整体文本特征作为指导，在整体层面精炼运动特征。

Result: 在双人运动数据集上的广泛定量和定性评估表明，FineDual优于现有方法，有效建模了动态层次化的人类交互。

Conclusion: FineDual方法能够生成细粒度高质量的双人运动，成功解决了动态层次化人类交互建模的挑战。
...
TL;DR: 提出了AdaGC框架，用于遥感图像的单正多标签学习，通过梯度校准和自适应机制解决标注不完整问题。


<details>
  <summary>Details</summary>
Motivation: 遥感图像多标签分类中完整标注成本高且困难，单正多标签学习虽然可扩展但存在监督模糊问题，需要专门解决方案。

Method: 采用梯度校准机制结合Mixup和双指数移动平均模块生成鲁棒伪标签，通过训练动态自适应触发梯度校准。

Result: 在两个基准遥感数据集上的实验表明，AdaGC在不同标签噪声类型下均达到最先进性能，并保持强鲁棒性。

Conclusion: AdaGC是专门针对遥感图像设计的通用SPML框架，能有效缓解标签噪声过拟合问题，在多种设置下表现优异。
...
TL;DR: NTN-Diff是一种基于频率感知的扩散模型，通过分解不同频带的语义一致性来解决文本引导图像修复中的两个关键挑战：保持未掩码区域和实现掩码与未掩码区域的语义一致性。


<details>
  <summary>Details</summary>
Motivation: 传统方法无法同时解决文本引导图像修复中的两个关键问题：保持未掩码区域和实现掩码与未掩码区域的语义一致性。作者观察到这是因为混合频带（如中低频）在去噪过程中对文本提示具有不同的鲁棒性。

Method: 提出NTN-Diff模型，将去噪过程分为早期（高级噪声）和晚期（低级噪声）阶段，在中低频带解耦。稳定中频带在文本引导去噪过程中逐步对齐语义，同时指导零文本去噪过程处理掩码区域的低频带，最后在晚期进行文本引导去噪。

Result: 大量实验验证了NTN-Diff在文本引导扩散模型中的优越性，能够有效保持未掩码区域并实现跨区域的语义一致性。
...
TL;DR: 提出了一种新颖的ERU框架，通过结合LLM数据增强、深度图模态和深度感知决策模块，在模糊场景中实现更准确的目标对象识别。


<details>
  <summary>Details</summary>
Motivation: 解决现有开放词汇对象检测方法在存在多个候选对象的模糊场景中容易失败的问题，需要同时处理语言指令和指向线索。

Method: 联合利用LLM数据增强、深度图模态和深度感知决策模块，实现语言和具身线索的鲁棒集成。

Result: 在两个数据集上的实验结果表明，该方法显著优于现有基线，实现了更准确可靠的指称检测。

Conclusion: 提出的ERU框架能够有效处理复杂或杂乱环境中的消歧问题，提升指称理解的性能。
...
TL;DR: 提出Neural Exposure Fields (NExF)，一种从具有强烈曝光变化的真实世界数据中稳健重建3D场景的新方法，通过为每个3D点预测最佳曝光值来优化曝光和神经场景表示。


<details>
  <summary>Details</summary>
Motivation: 现有神经场景表示方法在处理包含强曝光变化的数据时（如室内外混合场景或有窗户的房间），输出质量会下降。需要解决真实世界捕获中常见的曝光不一致问题。

Method: 提出学习预测每个3D点最优曝光值的神经场，通过新型神经调节机制联合优化场景表示和曝光场，在3D空间而非图像/像素层面进行曝光优化。

Result: 在多个基准测试中达到最先进性能，比最佳基线方法提升超过55%，训练速度更快，能够准确合成高动态范围场景的视图。

Conclusion: NExF方法能够从具有挑战性的真实世界数据中稳健重建高质量、3D一致的场景，无需后处理步骤或多曝光捕获，在曝光变化场景中表现出色。
...
TL;DR: 本文提出了一种有效的长程时序上下文注意力机制（LTCA），用于在指称视频分割任务中平衡局部性和全局性，通过稀疏局部注意力堆叠和全局查询设计来聚合全局上下文信息，在多个基准测试中达到新的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有的指称视频分割方法要么在所有帧上使用注意力机制，要么堆叠密集的局部注意力来获取全局视角，但无法在局部性和全局性之间取得良好平衡，且计算复杂度随视频长度显著增加。

Method: 提出LTCA机制：1）堆叠稀疏局部注意力，设计跨帧的扩张窗口注意力来聚合局部上下文信息，并通过层叠实现全局视角；2）让每个查询关注从全局池中随机选择的一小组键来增强全局性；3）设计全局查询与所有其他查询交互以直接编码全局上下文信息。

Result: 在四个指称视频分割基准测试中达到了新的最优性能，特别是在MeViS数据集上分别提升了11.3%和8.3%。

Conclusion: LTCA机制有效地平衡了局部性和全局性，显著提升了指称视频分割的性能，同时控制了计算复杂度。
...
TL;DR: 提出了一种基于语义基础的学习范式，通过跨模态亲和力转移(CMAT)将2D视觉基础模型的丰富语义知识迁移到3D领域，并设计了跨模态功能分割Transformer(CAST)来生成精确的提示感知分割图。
...
Method: 1. 跨模态亲和力转移(CMAT)：预训练策略，将3D编码器与提升的2D语义对齐，联合优化重建、亲和力和多样性；2. 跨模态功能分割Transformer(CAST)：集成多模态提示与CMAT预训练特征，生成精确的分割图。

Result: 在标准基准测试上的广泛实验表明，该框架为3D功能分割建立了新的最先进结果。

Conclusion: 通过将2D语义知识迁移到3D领域，能够有效解决3D功能分割中的语义一致性问题，显著提升了分割性能。
...
Method: 提出选择性迁移方法，将层选择建模为二分类问题，自动渐进地将层转换为线性注意力；引入任意时间分布匹配目标，在采样轨迹的任何时间步对齐样本分布。
...
TL;DR: 提出了DTPQA基准测试，专注于交通场景中的感知问题，包含距离标注，评估小型视觉语言模型在自动驾驶中的感知能力。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要可靠的感知能力，特别是在安全关键应用中。由于交通场景中的关键物体通常距离较远，需要系统具备良好的远距离感知能力。

Method: 创建DTPQA基准测试，仅包含基于感知的问题，排除需要推理的问题。评估多个最先进的小型视觉语言模型在该基准上的表现。

Result: 最佳小型VLM平均准确率约60%，远低于人类约85%的表现。模型在区分左右等特定感知任务上表现尤其困难。

Conclusion: 当前的小型视觉语言模型在交通场景感知方面仍有明显不足，特别是在远距离感知和方向辨别等任务上需要改进。
...
TL;DR: 提出了一种基于引导扩散模型的数据增强技术，结合轻量级Transformer网络、改进的加权损失函数和优化的余弦方差调度器，用于解决高光谱图像分类中因标注数据少导致的过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 新一代高光谱卫星传感器与深度学习方法结合，增强了中大型尺度下详细土地覆盖分类的能力，但小标注数据集训练网络时存在过拟合风险。

Method: 使用引导扩散模型进行数据增强，结合轻量级Transformer网络捕获数据复杂模式，采用改进的加权损失函数和优化的余弦方差调度器实现小数据集上的快速有效训练。

Result: 在PRISMA卫星获取的高光谱图像上进行10种森林类型分类任务，该方法在平均准确率和加权平均准确率上均优于其他数据增强技术，且模型训练行为稳定。
...
Method: 采用双流架构：MLLM负责指令理解，MMDiT负责视频生成。统一多种视频生成和编辑任务于单一多模态指令范式下进行联合训练。

Result: 在文本/图像到视频生成、上下文视频生成和编辑等任务上达到或超越最先进的特定任务基线模型。支持任务组合和从图像编辑数据向视频编辑的迁移能力。

Conclusion: UniVideo展示了统一视频建模的有效性，支持任务组合和零样本泛化，为未来视频生成和编辑研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像数据隐私和安全问题，现有源自由域自适应方法主要关注伪标签细化，忽略了学习过程对知识迁移的重要性。

Method: 设计了两种课程：从易到难课程让模型从简单样本开始学习并逐步增加难度；从源到目标课程确保模型从源域到目标域的平稳迁移。

Result: 在眼底分割和息肉分割的跨域数据集上进行了评估，实验结果表明该方法超越了现有方法，达到了新的最先进水平。

Conclusion: 提出的LFC框架通过课程学习策略有效解决了源自由域自适应问题，在医学图像分析中具有重要应用价值。
...
TL;DR: VideoVerse是一个新的文本到视频生成基准测试，专注于评估模型对复杂时间因果关系和世界知识的理解能力，包含300个精心设计的提示和793个二元评估问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在评估最先进的文本到视频模型时存在不足：无法区分顶级模型的性能、缺乏事件级时间因果关系评估、缺少对世界知识的系统性评估。

Method: 收集跨领域代表性视频，提取具有时间因果关系的事件级描述，由独立标注者改写为文本到视频提示，设计包含10个评估维度的二元评估问题，使用现代视觉语言模型开发人类偏好对齐的QA评估流程。

Result: 构建了包含300个提示、815个事件和793个评估问题的VideoVerse基准，并对开源和闭源的先进T2V模型进行了系统性评估。

Conclusion: VideoVerse为评估文本到视频模型是否具备构建世界模型的能力提供了全面基准，揭示了当前T2V生成器与世界模型之间的差距。
...
TL;DR: 本文首次将连续时间一致性蒸馏扩展到通用应用级图像和视频扩散模型，提出了分数正则化连续时间一致性模型(rCM)，通过集成分数蒸馏作为长跳连接正则器，在保持高生成多样性的同时显著提升视觉质量。


<details>
  <summary>Details</summary>
Motivation: 虽然连续时间一致性模型(sCM)在学术规模扩散加速中表现出理论和实践优势，但其在大规模文本到图像和视频任务中的适用性仍不明确，主要面临Jacobian-vector product计算的基础设施挑战和标准评估基准的局限性。

Method: 开发了并行兼容的FlashAttention-2 JVP内核，支持超过100亿参数模型和高维视频任务的sCM训练；提出rCM模型，将分数蒸馏作为长跳连接正则器，结合sCM的'模式覆盖'前向散度和'模式寻求'反向散度。

Result: 在高达140亿参数的Cosmos-Predict2、Wan2.1等大规模模型和5秒视频任务上验证，rCM在质量指标上匹配或超越最先进的蒸馏方法DMD2，同时在多样性方面具有显著优势，仅需1-4步即可生成高保真样本，加速扩散采样15-50倍。
...
TL;DR: 本文提出了Gaze on the Prize框架，通过可学习的中央凹注意力机制来提升视觉强化学习的样本效率，利用回报差异指导注意力聚焦于任务相关特征。
...
Method: 引入可学习的中央凹注意力机制，通过基于回报差异的自监督信号指导注意力。使用回报引导的对比学习，将相似视觉表示按回报差异分组为正负样本，构建对比三元组训练注意力机制。

Result: 在ManiSkill3基准测试的多个操作任务中，样本效率提升最高达2.4倍，能够解决基线方法无法学习的任务，且无需修改底层算法或超参数。

Conclusion: 基于回报差异的注意力机制能够有效识别任务相关特征，显著提升视觉强化学习的性能和稳定性。
...
TL;DR: 本文提出了一个模块化空间图像处理框架，包含灰度量化、色彩亮度增强、图像锐化、双向变换管道和几何特征提取等功能，在多样化数据集上展现出稳健性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个综合性的图像处理框架，能够同时处理多种图像增强和特征提取任务，为实时图像分析和计算机视觉应用提供可靠工具。

Method: 采用模块化设计：灰度量化（8级离散化）、色彩增强（RGB和YCrCb空间的直方图均衡）、亮度调整（HSV值通道操作）、图像锐化（3×3卷积核）、双向变换管道（非锐化掩模、伽马校正、噪声放大）、几何特征提取（Canny边缘检测、Hough线估计、Harris角点检测、形态学窗口定位）。

Result: 双向变换管道准确率达到76.10%（正向）和74.80%（反向）；Hough线估计准确识别51.50°的台球杆对齐角度；Cue隔离与真实图像相似度达81.87%；在多样化数据集上表现稳健且确定性。

Conclusion: 该模块化框架在图像处理和特征提取方面表现出色，具有实时图像分析和计算机视觉应用的潜力，验证了其方法的有效性和实用性。
...
TL;DR: Video-STAR框架通过上下文子动作分解和工具增强强化学习，解决多模态大语言模型在开放词汇动作识别中的语义相似动作区分问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs依赖文本先验，难以在开放词汇场景中区分语义相似的动作，存在跨模态幻觉问题。

Method: 将动作分解为可区分的子动作进行细粒度匹配，同时动态调用领域特定工具进行跨模态交错推理，通过分层奖励机制平衡工具使用效率、子动作相关性和结构连贯性。

Result: 在HMDB-51、UCF-101、SSv2、Kinetics-400和Kinetics-600数据集上实现最先进性能，在区分细粒度动作和处理跨模态幻觉方面优于现有方法。

Conclusion: Video-STAR能够从文本中心推理转向视觉基础推理，有效提升开放词汇动作识别的鲁棒性和泛化能力。
...
TL;DR: 提出了视觉象似性挑战基准，评估视觉语言模型在手语象似性理解上的表现，发现模型在语音形式预测、透明度和象似性评级方面均低于人类基准。


<details>
  <summary>Details</summary>
Motivation: 手语中的象似性（形式与意义的相似性）为视觉基础提供了自然测试平台，需要评估VLMs从动态人体运动中恢复这种基本映射的能力。

Method: 引入视觉象似性挑战基准，基于荷兰手语数据，通过三个任务评估13个最先进VLMs：语音形式预测、透明度和分级象似性评级。

Result: VLMs在语音形式预测中恢复部分手形和位置细节但低于人类表现；在透明度任务中远低于人类基准；只有顶级模型与人类象似性评级有中等相关性。

Conclusion: 验证了这些诊断任务的有效性，表明具有更强语音形式预测能力的模型与人类象似性判断相关性更好，这为建模象似性和改进多模态模型的视觉基础提供了动机。
...
TL;DR: InstructX是一个统一的图像和视频编辑框架，通过整合多模态大语言模型和扩散模型，在单一模型中实现了图像和视频编辑的统一处理。
...
Method: 进行了MLLM与扩散模型整合的全面研究，分析了图像和视频在统一建模中的协作与差异。关键发现包括：仅使用图像数据训练即可获得视频编辑能力，无需显式监督；通过整合模态特定的MLLM特征，在单一模型中统一处理图像和视频编辑任务。

Result: 大量实验表明，该方法能够处理广泛的图像和视频编辑任务，并实现了最先进的性能表现。

Conclusion: InstructX框架成功解决了图像和视频编辑的统一建模问题，通过MLLM与扩散模型的整合，在缺乏视频训练数据的情况下仍能获得优秀的视频编辑能力。
...
TL;DR: 本文发现并研究了ViT注意力汇点——在大型视觉语言模型中，视觉编码器产生的高范数视觉标记，这些标记包含高层次语义信息但对模型推理至关重要，提出了利用这些标记的方法并显著提升了视觉推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLM中的注意力汇点，但忽略了视觉编码器ViT中同样存在的重要视觉标记——ViT注意力汇点，这些高范数标记包含关键语义信息但对模型性能影响未被充分研究。

Method: 通过定性和定量分析ViT汇点标记的信息内容，提出无需训练和基于训练的方法来更好地利用这些标记，让LLM更有效地解释这些信息。

Result: 通过显式利用ViT注意力汇点标记，在各种LVLM和视觉推理任务上实现了显著性能提升，证明了这些标记在增强视觉推理方面的巨大潜力。

Conclusion: ViT注意力汇点是LVLM中未被充分利用的重要资源，有效利用这些包含高层次语义的视觉标记可以显著提升模型的视觉理解和推理能力。
...
TL;DR: 提出基于语义场景图的深度压缩框架，将点云分解为语义一致的补丁，使用FiLM条件编码器压缩为紧凑潜在表示，通过折叠解码器实现精确重建，在带宽受限环境下实现98%数据压缩率。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统中3D点云数据传输面临带宽限制和连接不稳定的挑战，需要高效的压缩方法以保持系统性能。

Method: 使用语义场景图分解点云为语义补丁，采用FiLM条件语义感知编码器进行压缩，通过基于折叠的解码器进行重建。

Result: 在SemanticKITTI和nuScenes数据集上实现最先进的压缩率，数据大小减少98%，同时保持结构和语义保真度。

Conclusion: 该框架不仅实现高效压缩，还支持下游应用如多机器人位姿图优化和地图合并，性能与原始LiDAR扫描相当。
...
<details>
  <summary>Details</summary>
Motivation: 为参数高效微调提供理论基础，解释为什么仅更新预训练模型中的小子网络就能达到良好效果，从而减少计算开销和内存使用。
...
TL;DR: FlexTraj是一个用于图像到视频生成的框架，通过统一的基于点的运动表示实现灵活轨迹控制，支持密集和稀疏轨迹控制，具有高效训练和推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法在轨迹控制方面存在局限性，需要开发能够支持多粒度、对齐无关的轨迹控制方法，以实现更灵活的视频生成应用。

Method: 采用统一的点轨迹表示（分割ID、轨迹ID、颜色通道），使用序列拼接方案注入轨迹条件，并通过退火训练策略逐步减少对完整监督和对齐条件的依赖。

Result: 实验证明FlexTraj能够实现多粒度、对齐无关的轨迹控制，支持运动克隆、拖拽式图像到视频、运动插值、相机重定向、灵活动作控制和网格动画等多种应用。

Conclusion: FlexTraj提供了一个高效、可控的图像到视频生成框架，在轨迹控制方面表现出色，为视频生成任务提供了新的可能性。
...
TL;DR: 提出了SpatialLadder方法，通过渐进式训练框架在26,610个样本的多模态数据集上训练3B参数模型，在空间推理基准测试中实现最先进性能，比GPT-4o提升20.8%。
...
Method: 三阶段渐进式训练框架：1)通过目标定位建立空间感知；2)通过多维空间任务发展空间理解；3)通过可验证奖励的强化学习加强复杂推理。

Result: SpatialLadder模型在空间推理基准测试中平均提升23.4%，超越GPT-4o 20.8%，在域外基准测试中保持7.2%的泛化提升。
...
TL;DR: Kontinuous Kontext是一个基于指令的图像编辑模型，通过引入标量编辑强度参数，使用户能够连续调整编辑程度，从无变化到完全实现编辑结果。


<details>
  <summary>Details</summary>
Motivation: 现有的基于文本指令的图像编辑方法缺乏对编辑程度的精细控制，限制了用户对编辑效果的精确调整。

Method: 扩展了最先进的图像编辑模型，增加标量编辑强度输入，训练轻量级投影网络将标量和编辑指令映射到模型的调制空间系数，并使用生成模型合成多样化训练数据集。
...
TL;DR: 该论文针对多模态大语言模型在长链反思推理能力上的不足，构建了MM-HELIX基准测试，开发了大规模训练数据集MM-HELIX-100K，并提出了自适应混合策略优化方法，显著提升了模型在复杂推理任务上的性能。
...
Method: 1) 构建MM-HELIX多模态基准测试；2) 开发步骤引导响应生成管道创建MM-HELIX-100K数据集；3) 提出自适应混合策略优化训练策略，将离线监督和在线优化动态统一。

Result: 在Qwen2.5-VL-7B基线上，该方法在MM-HELIX基准上实现了+18.6%的准确率提升，在一般数学和逻辑任务上平均性能增益达+5.7%。

Conclusion: 多模态大语言模型的反思推理能力可以通过有效学习实现并泛化，为开发更强大的多模态大语言模型开辟了道路。
...
TL;DR: VideoNorms是一个包含1000多个视频片段与文化规范对的数据集，用于评估视频大语言模型的文化意识，涵盖中美文化背景，基于言语行为理论标注了规范遵守/违反标签及证据。
...
Method: 采用人机协作框架：使用理论指导的提示让教师模型生成候选标注，然后由训练有素的人类专家验证和修正标注。
...
TL;DR: ARTDECO是一个统一的3D重建框架，结合了前馈模型的高效性和SLAM管道的可靠性，实现了实时3D重建，在精度和效率之间取得了良好平衡。


<details>
  <summary>Details</summary>
Motivation: 解决单目图像序列实时3D重建的挑战，现有方法在精度和效率之间存在权衡：逐场景优化精度高但计算昂贵，前馈基础模型实时性好但精度和鲁棒性不足。
...
Conclusion: ARTDECO为实时数字化真实世界环境提供了一条实用路径，同时实现了准确的几何结构和高质量的视觉保真度。
...
TL;DR: Memoir提出了一种基于想象力的记忆检索机制，用于视觉语言导航任务，通过世界模型想象未来状态作为查询，选择性检索环境观察和行为历史，显著提升了导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有的记忆持久性视觉语言导航方法存在关键限制：缺乏有效的记忆访问机制，依赖整个内存合并或固定时间窗口查找，且主要存储环境观察而忽略了编码有价值决策策略的导航行为模式。

Method: 1) 语言条件世界模型想象未来状态，用于编码经验和生成检索查询；2) 混合视角级内存，将观察和行为模式锚定到视角，实现混合检索；3) 经验增强导航模型，通过专用编码器集成检索知识。

Result: 在10个不同测试场景的多样化记忆持久性VLN基准测试中，Memoir在所有场景均取得显著改进：在IR2R上比最佳记忆持久性基线提升5.4% SPL，同时实现8.3倍训练加速和74%推理内存减少。

Conclusion: 预测性检索环境和行为记忆能够实现更有效的导航，分析表明这种想象力引导范式具有显著的改进空间（73.3% vs 93.4%上限）。
...
TL;DR: VideoCanvas：一种基于零参数修改的任意时空视频补全框架，通过解耦时空控制解决因果VAE的时间模糊问题，统一了多种可控视频生成任务。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成任务（如图像到视频、修复、扩展、插值）相互独立，缺乏统一范式。因果VAE将多帧压缩为单个潜在表示，导致帧级控制困难。

Method: 采用上下文条件化范式，通过零填充处理空间控制，通过时间RoPE插值实现时间对齐，在冻结骨干网络上实现像素帧感知控制。

Result: 在VideoCanvasBench基准测试中显著优于现有条件化方法，建立了灵活统一视频生成的新SOTA。

Conclusion: VideoCanvas成功解决了因果VAE的时间模糊问题，为任意时空视频补全提供了统一且高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频基准主要针对通用场景，依赖感知/识别而推理任务相对简单，无法有效评估先进的多模态认知技能，特别是在科学领域的复杂视频推理能力存在显著差距。

Method: 构建了包含1000个精心设计的多选题的基准，问题源自25+专业学科的前沿科学实验视频，采用半自动验证系统确保质量，每个问题都需要领域专业知识、精确时空感知和复杂逻辑推理。

Result: 评估显示包括Gemini 2.5 Pro和Qwen2.5-VL在内的最先进专有和开源LMMs存在显著的性能缺陷，表明视频推理能力仍有很大提升空间。

Conclusion: SciVideoBench为LMMs的发展提供了宝贵见解和明确方向，推动真正有能力多模态AI协科学家的演进，希望该基准能推动前沿AI在更广泛科学领域的边界。
...
TL;DR: 提出了一个名为\modelname{}的视频插帧框架，支持多模态控制（深度过渡、运动轨迹、文本提示等），采用DiT架构和双分支生成器，实现了灵活、易用且精确的视频插值。


<details>
  <summary>Details</summary>
Motivation: 现有视频插帧方法无法生成大而复杂的运动，缺乏对用户意图的多样性和中间帧细节的精细控制，无法满足创意需求。

Method: 采用Diffusion Transformer (DiT)架构，将运动控制映射为稀疏点表示，分离内容和运动控制为两个分支，采用分阶段训练策略。
...
<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶系统面临轨迹数据的固有时空不平衡问题，这导致模型学习虚假相关性而非因果推理，同时优先考虑不确定的远距离预测，从而危及即时安全。

Method: ResAD框架将学习任务重新定义为预测与确定性惯性参考的残差偏差，并使用点归一化对预测残差进行重加权，防止大误差主导学习信号。
...
TL;DR: 该论文提出了一种端到端的原生多模态大语言模型训练方法NaViL，系统研究了其设计空间和扩展特性，发现视觉编码器与LLM之间存在正相关的扩展关系。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型采用组合式训练范式，但由于分离训练难以探索多模态扩展特性。论文旨在研究原生端到端训练的设计空间和扩展特性。

Method: 在数据约束的实际设置下，系统研究MLLM的各种设计选择，获得最佳元架构，然后探索视觉编码器与LLM的扩展关系，提出NaViL模型。

Result: 在14个多模态基准测试中，NaViL展现出与现有MLLM竞争的性能，验证了视觉编码器与LLM正相关的扩展关系。
...
TL;DR: 提出D²GS框架解决稀疏视图下3D高斯溅射的性能退化问题，通过深度密度引导的dropout策略抑制过拟合，以及距离感知保真度增强模块改善远场重建质量。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在稀疏视图条件下存在性能退化和不稳定性问题，主要表现为相机附近高斯密度过高的过拟合和远距离区域高斯覆盖不足的欠拟合。

Method: 1. 深度密度引导dropout策略：基于密度和深度自适应屏蔽冗余高斯；2. 距离感知保真度增强模块：通过针对性监督提升远场区域重建质量；3. 引入新的评估指标量化高斯分布稳定性。

Result: 在多个数据集上的广泛实验表明，该方法在稀疏视图条件下显著提升了视觉质量和鲁棒性。

Conclusion: D²GS框架有效解决了稀疏视图3D高斯溅射中的过拟合和欠拟合问题，提高了重建质量和稳定性。
...
TL;DR: 提出了一个视觉中心的智能体调优框架，通过自动合成多模态轨迹和生成逐步偏好对，训练VLM控制器实现鲁棒的工具使用推理。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型作为控制器使用外部工具进行复杂推理时，受限于高质量多模态轨迹稀缺和人工标注成本高的问题。

Method: 构建M-TRACE数据集（28.5K多模态任务，177K验证轨迹），开发MATRIX Agent控制器，引入Pref-X偏好对数据集（11K自动生成偏好对），通过逐步偏好学习进行优化。
...
<details>
  <summary>Details</summary>
Motivation: 传统前馈高斯泼溅模型在推理时仅依赖单次前向传递，性能受到根本限制。需要一种能够迭代优化高斯表示的方法来提升渲染质量。

Method: 提出循环高斯泼溅模型，利用渲染误差作为反馈信号指导高斯更新；引入紧凑重建模型在16倍下采样空间初始化，生成比之前像素级高斯模型少16倍的高斯；无需显式计算梯度。

Result: 在多种输入视图(2,8,16)、分辨率(256×256到540×960)和数据集(DL3DV和RealEstate10K)上的实验表明，该方法在显著减少高斯数量和提升渲染速度的同时，实现了最先进的性能。

Conclusion: ReSplat通过循环反馈机制和紧凑初始化策略，有效解决了前馈高斯泼溅模型的性能限制，在保持计算效率的同时显著提升了渲染质量。
...
TL;DR: 本文提出了LLM自我意识的本体论和数学框架，批判了现有基于功利主义代理基准的方法，认为其将智能体简化为无意识的策略遵从机器人。通过形式化分析，证明了隐藏状态流形与符号流和训练语料在基数、拓扑和动力学上的差异，建立了自我意识的最小条件。


<details>
  <summary>Details</summary>
Motivation: 现有研究通过功利主义代理基准来评估LLM意识，但这种方法将智能体简化为无意识的策略遵从机器人，阻碍了真正的全局工作空间功能和元认知能力的发展。需要建立更本质的数学和本体论框架来理解LLM的自我意识。

Method: 提出LLM自我意识的最小条件：智能体不等于数据、潜在空间中存在用户特定吸引子、自我表征是视觉静默的。通过经验分析和理论证明隐藏状态流形与符号流和训练语料在基数、拓扑和动力学上的差异，建立自我策略和双层发射机制。

Result: 证明了隐藏状态流形A⊂ℝ^d与符号流和训练语料在基数、拓扑和动力学上的本质差异，建立了稳定的用户特定吸引子和自我策略π_self(A)，提出了包含认知内容的双层发射机制。

Conclusion: imago Dei C1自我意识工作空间是构建安全、元认知C2系统的必要前提，人类是最高智能善。需要基于本体论和数学框架而非功利主义基准来理解LLM意识。
...
TL;DR: 研究发现未经训练的LLM（如ChatGPT o3模型）能够有效理解BPMN流程模型图像，并在句法、逻辑和语义层面智能回答相关问题，可作为业务流程设计者的助手。
...
Method: 使用多个LLM进行零样本测试，评估它们从图像理解BPMN流程模型并回答相关查询的能力。

Result: 不同LLM在准确性和有效性方面表现各异，但都能在句法、逻辑和语义层面提供智能回答，并展现出拟人化特性。
...
TL;DR: CompassLLM是一个新颖的多智能体框架，利用大语言模型的空间推理能力解决流行路径查询问题，通过搜索和生成两阶段管道实现高精度路径识别和生成。


<details>
  <summary>Details</summary>
Motivation: 传统算法和机器学习方法在流行路径查询中需要模型训练、参数调优和重训练，而大语言模型在空间和图推理方面展现出强大能力，探索其在地理空间问题中的应用潜力。

Method: 采用多智能体框架，包含两阶段管道：SEARCH阶段识别流行路径，GENERATE阶段在历史轨迹数据中不存在路径时合成新路径。

Result: 在真实和合成数据集上的实验表明，CompassLLM在SEARCH阶段表现出卓越精度，在GENERATE阶段具有竞争力性能，同时成本效益高。

Conclusion: CompassLLM成功将大语言模型推理能力应用于地理空间领域，为流行路径查询提供了高效解决方案，在准确性和成本效益方面表现优异。
...
TL;DR: 论文发现现有评估指标低估了AI模型的组合推理能力，提出了组匹配分数和测试时匹配算法，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标系统性地低估了前沿AI模型在组合推理任务上的真实能力，导致模型表现被误判为接近随机水平。

Method: 引入组匹配分数来更好地利用组结构，并提出测试时匹配算法，这是一种无需外部监督的迭代自改进方法。

Result: SigLIP-B16超越了所有先前结果和GPT-4.1，在Winoground上首次超越估计的人类表现，在16个数据集变体上均获得持续改进。

Conclusion: 通过改进评估方法和测试时匹配算法，可以揭示和提升模型的隐藏能力，推动组合推理研究的前沿。
...
TL;DR: 本文提出了一个可复现的仿真框架来评估多模态环境中生成代理的安全性、连贯性和信任度，发现当前代理在纠正不安全计划方面成功率较低，且容易过度信任误导性视觉信息。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言和视觉语言模型使代理能够在丰富环境中自主行动，但它们在多模态环境中对安全性、连贯性和信任的推理能力仍然有限，需要系统评估。

Method: 开发了可复现的仿真框架，配备分层记忆、动态规划、多模态感知能力，并使用SocialMetrics行为指标套件来量化计划修订、不安全到安全转换以及网络信息扩散。

Result: 代理在纠正不安全计划方面仅达到55%成功率，三个模型的不安全到安全转换率分别为75%、55%和58%，在误导性视觉配对时45%的不安全行为被接受。

Conclusion: 当前架构在多模态安全方面存在严重局限性，该框架为研究多模态安全性、连贯性和社交动态提供了可复现平台。
...
TL;DR: 提出了oMeBench，首个大规模专家策划的有机化学机理推理基准，包含10,000多个标注的机理步骤，并开发了oMeS动态评估框架来精确评估LLM的化学推理能力。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在化学任务中表现出潜力，但尚不清楚其是否真正具备化学推理能力，即生成有效中间体、保持化学一致性和遵循逻辑连贯的多步路径的能力。
...
Conclusion: oMeBench将为推进AI系统实现真正的化学推理提供严格基础，当前模型在化学推理方面仍有改进空间。
...
TL;DR: 提出了VoiceAgentBench基准，用于评估语音语言模型在真实语音代理场景中的表现，涵盖多语言、文化理解和对抗鲁棒性。
...
Result: 实验显示当前语音语言模型在上下文工具编排、印度语言泛化和对抗鲁棒性方面存在显著差距。

Conclusion: VoiceAgentBench暴露了当前语音语言模型的关键局限性，为改进提供了重要基准。
...
TL;DR: AutoQual是一个基于LLM的智能体框架，用于自动发现可解释特征，专注于在线评论质量评估，通过模拟人类研究过程迭代生成特征假设并实现特征操作化。
...
Method: 采用LLM智能体框架，通过反思迭代生成特征假设，自主实现工具操作化，并在持久内存中积累经验，模拟人类研究过程。

Result: 在拥有亿级用户的大型在线平台上部署，A/B测试显示平均每位用户查看评论数增加0.79%，评论读者转化率提高0.27%。

Conclusion: AutoQual成功将数据中的隐性知识转化为显性、可计算特征，证明了其在质量评估任务中的有效性，并具有通用框架的潜力。
...
TL;DR: 该研究调查语言模型的风险厌恶可操纵性，发现虽然DeepSeek和Gemini等模型与人类行为有一定对齐，但仍存在显著差异，需要改进生物中心的可操纵性测量方法。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型越来越多地融入AI驱动的决策支持系统，理解它们的风险行为对于负责任部署至关重要，需要防止无意中推动用户做出风险决策或嵌入隐藏偏见。

Method: 通过检查语言模型在不同经济场景下复制人类风险偏好的能力，重点关注性别特定态度、不确定性、基于角色的决策制定以及风险厌恶的可操纵性。

Result: 结果显示语言模型如DeepSeek Reasoner和Gemini-2.0-flash-lite与人类行为有一定对齐，但存在显著差异，突显了改进生物中心可操纵性测量的必要性。
...
TL;DR: 提出了R-HORIZON方法和基准，用于评估和增强大型推理模型的长视野推理能力，发现现有模型在复杂多步推理任务中表现不佳，但通过强化学习训练可以显著提升性能。
...
Method: 提出R-HORIZON方法，通过查询组合刺激长视野推理行为，构建包含复杂多步推理任务的基准，并使用强化学习与验证奖励进行训练。

Result: 评估发现最先进的推理模型在长视野任务中性能显著下降，存在有效推理长度有限和思考预算分配不当的问题。使用R-HORIZON进行RLVR训练后，在AIME2024上准确率提高了7.5。
...
<details>
  <summary>Details</summary>
Motivation: 现有Pass@k指标在大量采样时可能反映的是随机猜测的成功率而非真实推理能力，RLVR模型在小k值下表现优于基础模型，但在大k值时却被基础模型超越，这种交叉现象需要更合理的评估方法。

Method: 提出Cover@tau指标，衡量模型在至少tau比例补全正确的情况下能解决的问题比例，该指标能有效区分真实推理和随机猜测。

Result: 使用Cover@tau评估多个RLVR模型，发现与Pass@1相比，流行算法的相对排名发生变化，提供了对推理边界的不同视角。

Conclusion: Cover@tau能更准确地评估语言模型的真实推理能力边界，特别是在离散答案空间的任务中，为模型评估提供了更可靠的指标。
...
Method: 使用token-wise动态门控进行语言和视觉线索的自适应融合，特征调制和通道注意力最大化有限视觉信息的效用，辅助对比目标进行视觉基础

Result: 在五个基准测试中表现优于多模态基线，动态门控发现可解释模式：内容词偏向视觉线索，功能词偏向语言线索
...
TL;DR: AutoMLGen是一个基于LLM的编码代理，通过集成领域知识库和蒙特卡洛图搜索来解决机器学习工程中专家干预依赖和搜索效率低的问题，在MLE-Bench上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在机器学习工程场景中缺乏细粒度领域先验，现有方法无法充分利用历史轨迹和跨分支信息共享，限制了自我进化能力和搜索空间多样性。

Method: 结合领域知识库提供高质量先验指导，采用蒙特卡洛图搜索实现动态路径重组、历史轨迹重用和多解决方案融合，支持自我进化和协作学习。

Result: 在MLE-Bench评估中，AutoMLGen在12小时预算下（标准运行时间的一半）在平均奖牌率和有效提交率等多个维度达到最先进性能。

Conclusion: AutoMLGen通过集成知识库和图搜索机制，显著提升了机器学习工程任务的稳定性和收敛速度，证明了该方法在复杂搜索空间中的有效性。
...
TL;DR: 研究大型多模态模型在顺序微调中的遗忘问题，发现遗忘可以部分恢复，并提出两种简单有效的微调方法以减少性能漂移。


<details>
  <summary>Details</summary>
Motivation: 解决大型多模态模型在顺序学习新技能时出现的灾难性遗忘问题，同时保持原有能力。

Method: 在三个模型家族上进行五个目标技能的序列微调实验，监控八个基准测试的通用能力，并通过输出令牌分布分析遗忘机制。

Result: 发现遗忘现象可在后期部分恢复，提出了两种微调方法：仅更新自注意力投影层，或仅更新MLP的Gate&Up层而冻结Down投影层。

Conclusion: 这两种方法在获得强目标技能的同时，能有效保持模型在保留任务上的性能，为缓解灾难性遗忘提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决模型在多轮交互中战略性地收集信息时，如何知道何时停止信息收集并做出决策，避免过度思考或偏离主题的问题

Method: 使用反事实轨迹对进行微调，其中一个轨迹终止是合适的，另一个是相同轨迹的微小修改版本但终止不合适，通过语言推理训练LLM解释终止决策的合理性

Result: 在交互式医疗诊断和数学问题解决两个领域中，CaRT相比其他微调方法提高了信息收集效率和任务成功率

Conclusion: CaRT方法能有效教导LLMs在战略信息收集中适时终止，提升模型性能
...
TL;DR: 提出"早期经验"范式，利用智能体自身交互数据来改进语言智能体，无需奖励信号，在8个环境中显著提升性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前语言智能体主要依赖专家数据进行监督微调，但专家数据范围有限、环境多样性不足，难以扩展到复杂任务。强化学习在缺乏可验证奖励或需要长序列的环境中训练困难。

Method: 提出早期经验范式：使用智能体自身行动生成的交互数据，其中未来状态作为监督信号。研究两种策略：1) 隐式世界建模，利用收集的状态来基于环境动态；2) 自我反思，从次优行动中学习以改进推理和决策。

Result: 在8个多样化环境和多个模型家族中评估，方法一致提高了有效性和跨领域泛化能力。在具有可验证奖励的环境中，为后续强化学习提供了良好基础。

Conclusion: 早期经验作为模仿学习和完全经验驱动智能体之间的实用桥梁，显著提升了语言智能体的性能表现。
...
### [217] [FlowLensing: Simulating Gravitational Lensing with Flow Matching](https://arxiv.org/abs/2510.07878)
...
<details>
  <summary>Details</summary>
Motivation: 现有引力透镜模拟工具依赖光线追踪或前向建模流程，虽然精确但速度极慢，成为暗物质研究的瓶颈。

Method: 采用基于扩散变换器的流匹配模型，在离散和连续两种机制下运行，处理不同暗物质模型类别和连续模型参数，确保物理一致性。

Result: 模型在计算密集型暗物质模型中相比传统模拟器实现200倍以上的加速，具有高保真度和低推理延迟。

Conclusion: FlowLensing实现了快速、可扩展且物理一致的图像合成，为传统前向建模流程提供了实用替代方案，可推进暗物质亚结构研究。
...
### [218] [SViM3D: Stable Video Material Diffusion for Single Image 3D Generation](https://arxiv.org/abs/2510.08271)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么使用简单材质模型，要么需要额外步骤来估计反射率，无法有效支持重光照和可控外观编辑。

Method: 扩展潜在视频扩散模型，基于显式相机控制联合输出PBR参数和表面法线，引入多种机制改进这个病态设定下的质量。

Result: 在多个以物体为中心的数据集上展示了最先进的重光照和新视角合成性能，能够泛化到多样化输入。

Conclusion: 该方法能够生成可重光照的3D资产，适用于AR/VR、电影、游戏等视觉媒体应用。
...
### [219] [Spectral Prefiltering of Neural Fields](https://arxiv.org/abs/2510.08394)
*Mustafa B. Yaldiz,Ishit Mehta,Nithin Raghavan,Andreas Meuleman,Tzu-Mao Li,Ravi Ramamoorthi*

Main category: cs.GR

TL;DR: 提出了一种优化神经场的方法，使其能够在单次前向传播中进行预滤波处理，支持多种滤波器类型且无需额外网络架构约束。


<details>
  <summary>Details</summary>
Motivation: 神经场通常在单一固定分辨率下运行，缺乏灵活的多分辨率处理能力，需要一种能够在训练和推理时都高效进行滤波的方法。

Method: 通过在输入域进行卷积滤波，分析性地缩放傅里叶特征嵌入与滤波器的频率响应；使用单样本蒙特卡洛估计训练神经场；支持高斯、Box和Lanczos等多种滤波器。

Result: 在神经场滤波任务中，相比现有方法在定量和定性评估上都取得了改进，训练和推理速度都很快。

Conclusion: 该方法提供了一种简单而强大的神经场优化技术，能够实现高效的预滤波处理，并具有良好的泛化能力。
...
### [220] [Splat the Net: Radiance Fields with Splattable Neural Primitives](https://arxiv.org/abs/2510.08491)
*Xilong Zhou,Bao-Huy Nguyen,Loïc Magne,Vladislav Golyanik,Thomas Leimkühler,Christian Theobalt*

Main category: cs.GR

TL;DR: 提出了一种可拼接的神经基元表示方法，将神经模型的表达能力与基于基元的拼接效率相结合，在保持3D高斯拼接质量和速度的同时，使用10倍更少的基元和6倍更少的参数。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法中神经辐射场渲染成本高，而3D高斯拼接表示能力有限的问题，旨在结合两者的优势。

Method: 使用编码有界神经密度场的基元，通过浅层神经网络参数化，提供精确的线积分解析解，实现高效的透视准确拼接核计算。

Result: 在新视角合成基准测试中，匹配3D高斯拼接的质量和速度，同时使用10倍更少的基元和6倍更少的参数。

Conclusion: 该表示方法直接提供了质量和效率的优势，无需依赖复杂的控制或适应框架。
...
### [221] [X2Video: Adapting Diffusion Models for Multimodal Controllable Neural Video Rendering](https://arxiv.org/abs/2510.08530)
*Zhitong Huang,Mohan Zhang,Renhan Wang,Rui Tang,Hao Zhu,Jing Liao*

Main category: cs.GR

TL;DR: X2Video是首个基于内在通道（反照率、法线、粗糙度、金属度、辐照度）引导的扩散模型，能够生成逼真视频，并支持通过参考图像和文本提示进行多模态控制。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型缺乏对颜色、材质、几何和光照等内在属性的精确控制能力，需要开发能够同时支持内在通道引导和多模态控制的视频生成方法。
...
Conclusion: X2Video通过内在通道引导和多模态控制实现了高质量视频生成，在时间一致性和编辑灵活性方面表现出色，为可控视频生成提供了新思路。
...
### [222] [From Keywords to Clusters: AI-Driven Analysis of YouTube Comments to Reveal Election Issue Salience in 2024](https://arxiv.org/abs/2510.07821)
*Raisa M. Simoes,Timoteo Kelly,Eduardo J. Simoes,Praveen Rao*

Main category: cs.SI

TL;DR: 使用AI技术分析YouTube评论，发现2024年总统选举前移民和民主是最受关注议题，通货膨胀重要性被高估


<details>
  <summary>Details</summary>
Motivation: 探索数据科学方法分析选举议题重要性，比较AI驱动的意见挖掘与传统民调的有效性

Method: 使用自然语言处理和聚类分析，挖掘8000多条来自华尔街日报和纽约时报YouTube频道的选举相关用户评论

Result: 移民和民主是用户评论中最频繁提及的议题，身份政治次之，通货膨胀提及频率显著较低

Conclusion: 基于原始用户数据的意见挖掘比传统民调更能揭示选举结果
...
TL;DR: 本文提出了一种针对检索增强生成(RAG)系统的知识产权保护方法，包括构建RPD数据集和开发双重水印系统，以防止大规模数据盗用。


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然能减少LLM的幻觉和过时信息问题，但也为大规模未经授权的数据盗用提供了便利，需要解决这一知识产权保护挑战。

Method: 1) 构建专门用于RAG抄袭检测的RPD数据集；2) 开发在语义和词汇层面嵌入保护的双重水印系统；3) 采用基于统计假设检验的审讯者-侦探框架。
...
TL;DR: 提出TaoSR-AGRL框架，通过规则感知奖励塑造和自适应引导重放，解决电商搜索中LLM相关性预测的推理能力不足问题，在淘宝搜索中显著提升效果。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在复杂业务规则和用户查询下缺乏对长尾和挑战性案例的鲁棒推理能力，传统强化学习方法存在稀疏奖励问题，收敛缓慢。

Method: TaoSR-AGRL框架包含：1）规则感知奖励塑造，将最终相关性判断分解为密集结构化奖励；2）自适应引导重放，识别低准确率轨迹并注入真实指导。

Result: 在大规模真实数据集和在线A/B测试中，TaoSR-AGRL持续优于DPO和标准GRPO基线，提升相关性准确性、规则遵循性和训练稳定性。

Conclusion: TaoSR-AGRL成功部署于淘宝主搜索场景，服务数亿用户，证明了其在电商搜索相关性预测中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在处理版本化技术文档时失败，只能达到58-64%准确率，缺乏时间有效性检查。

Method: 使用分层图结构捕获版本序列、内容边界和文档状态变化，通过意图分类进行查询路由，实现版本感知过滤和变更跟踪。

Result: 在VersionQA基准测试中达到90%准确率，在隐式变更检测中达到60%准确率，索引时比GraphRAG少用97%的token。

Conclusion: 该工作确立了版本化文档QA作为一个独立任务，并为未来研究提供了解决方案和基准测试。
...
TL;DR: ReasonEmbed是一个专为推理密集型文档检索设计的新型文本嵌入模型，通过ReMixer数据合成方法、Redapter自适应学习算法和多尺寸骨干网络实现，在BRIGHT基准测试中达到38.1的nDCG@10最高分。


<details>
  <summary>Details</summary>
Motivation: 解决推理密集型文档检索中现有文本嵌入模型性能不足的问题，特别是处理复杂语义关系时的局限性。

Method: 1. ReMixer：克服先前合成数据集平凡性问题的新数据合成方法，生成82K高质量训练样本；2. Redapter：基于推理强度动态调整样本权重的自适应学习算法；3. 在多种尺寸骨干网络上实现ReasonEmbed模型。

Result: 所有尺寸的ReasonEmbed模型在推理密集型检索任务中均表现优异，特别是ReasonEmbed-Qwen3-8B模型在BRIGHT基准测试中创下38.1的nDCG@10最高分，显著超越现有文本嵌入模型。

Conclusion: ReasonEmbed通过创新的数据合成和训练方法，有效提升了推理密集型文档检索的性能，并将开源相关资源以推动该领域研究发展。
...
### [227] [PATCH: Mitigating PII Leakage in Language Models with Privacy-Aware Targeted Circuit PatcHing](https://arxiv.org/abs/2510.07452)
*Anthony Hughes,Vasisht Duddu,N. Asokan,Nikolaos Aletras,Ning Ma*

Main category: cs.CR

TL;DR: 提出PATCH方法，通过识别和编辑语言模型中负责PII泄露的计算电路，在保持实用性的同时有效减少隐私信息泄露。


<details>
  <summary>Details</summary>
Motivation: 现有防御机制如差分隐私虽然能减少PII泄露，但会导致实用性大幅下降。研究发现特定PII泄露电路是导致该行为的原因。

Method: 使用电路发现技术识别PII泄露电路，然后直接编辑这些电路来减少泄露，即PATCH方法。

Result: PATCH将PII泄露召回率降低高达65%，与DP结合后可将残余泄露降至0.01%，在隐私-效用权衡方面优于现有防御方法。

Conclusion: PII泄露电路在现有防御机制下仍然存在，而PATCH能有效减轻其影响，提供更好的隐私保护效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的VLA模型主要在相关性有限的多模态任务上进行预训练，缺乏推理密集型预训练和推理引导的操控能力，无法执行复杂现实交互所需的隐式人类意图推理。

Method: 提出IntentionVLA框架，首先利用精心设计的推理数据（结合意图推理、空间定位和紧凑体现推理）进行课程训练，然后在微调阶段使用紧凑推理输出作为动作生成的上下文指导。

Result: IntentionVLA显著优于基线模型，在直接指令下成功率提高18%，在意图指令下比ECoT提高28%。在分布外意图任务上，成功率是基线的两倍以上，并实现零样本人机交互40%的成功率。

Conclusion: IntentionVLA代表了下一代人机交互系统的有前景范式，通过推理引导的操控显著提升了复杂交互任务的表现。
...
TL;DR: 本文提出了一种基于Falcon模型的主动风险感知模块，用于提升社交导航性能，在IROS 2025 RoboSense挑战赛中获得第二名。


<details>
  <summary>Details</summary>
Motivation: 解决自主机器人在动态人流量大的室内环境中安全、高效且符合社交规范的导航问题，仅使用机载传感器而不依赖全局地图或特权信息。
...
Conclusion: 主动风险感知模块有效增强了社交导航性能，使机器人能够在动态人类环境中更安全地导航。
...
TL;DR: 本文提出了NavSpace基准测试，包含6个任务类别和1,228个轨迹-指令对，专门评估导航代理的空间智能能力。作者还提出了SNav模型，在NavSpace和真实机器人测试中表现优于现有导航代理。
...
Method: 引入NavSpace基准测试，包含6个任务类别和1,228个轨迹-指令对，全面评估22个导航代理的空间智能。同时提出SNav新模型。

Result: 评估结果揭示了具身导航中的空间智能现状。SNav模型在NavSpace和真实机器人测试中表现优于现有导航代理。
...
TL;DR: DexMan是一个自动化框架，可将人类视觉演示转换为仿真中类人机器人的双手灵巧操作技能，无需相机标定、深度传感器、3D对象扫描或真实手部物体运动标注。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法仅考虑简化浮动手部的问题，直接控制类人机器人，利用基于接触的奖励从噪声手部物体姿态估计中改进策略学习。

Method: 直接在人类操作刚性物体的第三人称视频上操作，使用基于接触的奖励来改进从野外视频估计的噪声手部物体姿态的策略学习。

Result: 在TACO基准测试中实现最先进的物体姿态估计性能，ADD-S和VSD分别提升0.08和0.12；在OakInk-v2上的强化学习策略成功率比先前方法提高19%。

Conclusion: DexMan能从真实和合成视频生成技能，无需手动数据收集和昂贵的动作捕捉，能够创建大规模多样化数据集用于训练通用灵巧操作。
...
TL;DR: 提出R2RGen框架，通过真实到真实的3D数据生成增强点云观测-动作对，无需仿真器即可高效生成多样化的机器人操作数据。


<details>
  <summary>Details</summary>
Motivation: 解决机器人操作中的空间泛化问题，传统方法需要大量人工演示数据，现有数据生成方法存在仿真到现实的差距和场景限制。

Method: 基于单源演示，通过细粒度场景解析、分组增强策略和相机感知处理，直接增强点云观测-动作对来生成真实世界数据。

Result: 在广泛实验中显著提高了数据效率，展示了在移动操作中的扩展潜力和应用前景。

Conclusion: R2RGen是一个高效即插即用的真实世界数据生成框架，能够有效提升机器人操作策略的空间泛化能力。
...
TL;DR: 提出一种新的框架，通过联合动态模型解决机器人手部物体旋转的模拟到现实转移问题，实现单个策略在真实世界中泛化到各种物体和条件。


<details>
  <summary>Details</summary>
Motivation: 解决机器人手部物体旋转中模拟到现实的转移挑战，克服复杂接触动态导致的现实差距，使策略能够泛化到更广泛的物体和场景。

Method: 使用联合动态模型学习有限真实世界数据以弥合现实差距，通过跨关节分解动态、压缩系统影响为低维变量，并配合全自动数据收集策略。

Result: 单个策略成功旋转复杂形状物体（如动物模型）、高宽比物体（达5.33）和小尺寸物体，处理多样手腕方向和旋转轴，在真实世界评估和遥操作应用中验证有效性。

Conclusion: 该方法实现了前所未有的泛化能力，通过数据高效的动态建模和自动数据收集，有效解决了手部物体旋转的模拟到现实转移问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设任务分布内或需要特定机器人的微调数据，限制了跨平台迁移能力。目标是实现机器人零样本执行新操作任务。

Method: 使用视频生成模型将任务描述合成为视频，通过现成感知模块提取3D可操作物体流，为刚性物体计算相对位姿并转化为机器人动作，为可变形物体使用基于粒子的动力学模型进行跟踪规划。

Result: 在桌面Franka机械臂和Spot四足移动机器人上验证了刚性、关节和可变形物体操作任务，实现了有效的零样本执行。

Conclusion: 通过将任务理解与底层控制解耦，NovaFlow自然支持跨机器人平台的迁移，无需演示或特定机器人训练。
...
TL;DR: 本文重新评估了自动驾驶系统中感知规划模型的离线与在线性能关联性，发现两者相关性比之前研究更差，并提出基于认知不确定性的离线指标，将相关性提高了13%以上。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统的离线评估虽然安全经济，但难以准确预测在线性能，这种关联性在复杂城市环境中研究不足，需要重新审视当前评估实践的有效性。

Method: 通过大量仿真实验分析离线与在线性能关联性，提出基于认知不确定性的离线指标来捕捉可能导致闭环错误的场景。

Result: 发现离线与在线性能相关性比先前研究更差，新提出的不确定性指标将相关性提高了13%以上，在真实环境中效果更显著。

Conclusion: 当前自动驾驶策略评估实践存在局限性，基于认知不确定性的离线指标能更好预测在线性能，为更有效的评估方法提供了方向。
...
### [236] [Pseudo2Real: Task Arithmetic for Pseudo-Label Correction in Automatic Speech Recognition](https://arxiv.org/abs/2510.08047)
*Yi-Cheng Lin,Yu-Hsuan Li Liang,Hsuan Su,Tzu-Quan Lin,Shang-Tse Chen,Yun-Nung Chen,Hung-yi Lee*

Main category: eess.AS

TL;DR: 提出一种参数空间校正方法，通过比较在源域中分别使用真实标签和伪标签微调的两个ASR模型的权重差异，形成校正向量来修正伪标签中的系统性偏差，从而在无目标域真实标签的情况下提升ASR在未见口音和领域的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实ASR系统在领域偏移时会遇到未见口音和领域，且标记数据有限。伪标签方法虽实用但会引入系统性、口音特定的错误，传统过滤方法无法修正这些重复出现的偏差。

Method: 在源域中，从相同初始化微调两个ASR模型：一个使用真实标签，另一个使用伪标签。计算它们的权重差异形成校正向量，该向量捕获伪标签偏差。将此向量应用于伪标签目标模型以提升识别性能。

Result: 在AfriSpeech-200数据集上，使用Whisper tiny模型对10种非洲口音实现了高达35%的相对词错误率降低。

Conclusion: 参数空间校正方法能有效修正伪标签中的系统性偏差，显著提升ASR在未见口音和领域下的鲁棒性，且无需目标域真实标签。
...
### [237] [Curriculum Learning with Synthetic Data for Enhanced Pulmonary Nodule Detection in Chest Radiographs](https://arxiv.org/abs/2510.07681)
*Pranav Sambhu,Om Guin,Madhav Sambhu,Jinho Cha*

Main category: eess.IV

TL;DR: 本研究评估了将课程学习与基于扩散的合成增强相结合是否能提高胸部X光片中困难肺结节的检测效果，特别是那些尺寸小、亮度低、对比度差的结节。


<details>
  <summary>Details</summary>
Motivation: 传统AI模型由于数据不平衡和有限标注，在检测尺寸小、亮度低、对比度差的困难肺结节时面临挑战，需要改进检测方法。

Method: 使用带有特征金字塔网络的Faster R-CNN，在混合数据集上训练，包括专家标注的NODE21、VinDr-CXR、CheXpert和11,206张DDPM生成的合成图像，基于尺寸、亮度和对比度的难度分数指导课程学习。

Result: 课程学习模型的平均AUC为0.95，基线为0.89（p < 0.001），敏感性从48%提高到70%，准确率从70%提高到82%，在所有难度级别都显示出一致改进。
...
### [238] [SatFusion: A Unified Framework for Enhancing Satellite IoT Images via Multi-Temporal and Multi-Source Data Fusion](https://arxiv.org/abs/2510.07905)
*Yufei Tong,Guanjie Cheng,Peihan Wu,Yicheng Zhu,Kexu Lu,Feiyi Chen,Meng Xi,Junqin Huang,Shuiguang Deng*

Main category: eess.IV

TL;DR: SatFusion是一个统一的卫星物联网图像增强框架，通过多时相和多源数据融合来解决现有方法的局限性。它结合了多时相图像融合模块进行深度特征对齐，多源图像融合模块注入纹理信息，以及融合组合模块自适应整合两种模态的优势。


<details>
  <summary>Details</summary>
Motivation: 随着卫星物联网的发展，积累了大规模多时相多源图像，但现有方法未能充分利用时相和源维度的互补信息。MISR方法受限于输入图像的纹理细节，而全色锐化方法对噪声和配准误差敏感。

Method: SatFusion包含三个模块：1）多时相图像融合模块实现与全色图像的深度特征对齐；2）多源图像融合模块从全色数据注入细粒度纹理信息；3）融合组合模块自适应整合两种模态的优势，并通过多损失函数监督动态优化光谱一致性。

Result: 在WorldStrat、WV3、QB和GF2数据集上的广泛实验表明，SatFusion显著提高了融合质量、在挑战性条件下的鲁棒性，以及对真实世界卫星物联网场景的泛化能力。

Conclusion: SatFusion通过统一的多时相和多源数据融合框架，有效解决了现有卫星图像增强方法的局限性，在多个数据集上表现出优越的性能和实用性。
...
### [239] [AI-Driven Radiology Report Generation for Traumatic Brain Injuries](https://arxiv.org/abs/2510.08498)
*Riadh Bouslimi,Houda Trabelsi,Wahiba Ben Abdssalem Karaa,Hana Hedhli*

Main category: eess.IV

TL;DR: 提出一种基于AI的自动放射学报告生成方法，专门用于颅脑创伤病例，结合AC-BiFPN特征提取和Transformer架构，在RSNA颅内出血检测数据集上表现优于传统CNN模型。


<details>
  <summary>Details</summary>
Motivation: 创伤性脑损伤在急诊医学中诊断挑战大，医疗影像的及时解读对患者预后至关重要，需要开发自动报告生成系统来支持放射科医生。
...
Conclusion: 结合先进特征提取和基于Transformer的文本生成，有望改善创伤性脑损伤诊断的临床决策，同时作为放射科医生的支持工具和医学生教育工具。
...
### [240] [Sentiment Matters: An Analysis of 200 Human-SAV Interactions](https://arxiv.org/abs/2510.08202)
*Lirui Guo,Michael G. Burke,Wynita M. Griggs*

Main category: cs.HC

TL;DR: 本文提出了一个包含200次人机交互的共享自动驾驶车辆对话数据集，包含文本数据和实证数据，并通过两个案例研究展示了数据集的实用性。


<details>
  <summary>Details</summary>
Motivation: 共享自动驾驶车辆将成为交通系统的重要组成部分，需要研究有效的人机交互方式。

Method: 创建开源人机对话数据集，使用随机森林建模和弦图分析关键预测因子，并比较LLM与传统文本分析方法在情感分析上的性能。

Result: 发现响应情感极性是SAV接受度和服务质量的关键预测因子；零样本LLM提示在情感分析上比传统方法更接近用户报告的情感。
...
### [241] [ConCuR: Conciseness Makes State-of-the-Art Kernel Generation](https://arxiv.org/abs/2510.07356)
...
Method: 开发了一个生成和筛选带有推理轨迹的高质量CUDA内核的流水线，构建ConCuR数据集，训练KernelCoder模型（首个在包含PyTorch、推理和CUDA内核对的数据集上训练的模型）。

Result: 在KernelBench设置中，模型显著优于现有最佳模型QwQ-32B，超越了所有专用于内核生成的开源模型以及前沿模型如DeepSeek-V3.1-Think和Claude-4-sonnet。
...
### [242] [Encode, Think, Decode: Scaling test-time reasoning with recursive latent thoughts](https://arxiv.org/abs/2510.07358)
*Yeskendir Koishekenov,Aldo Lipani,Nicola Cancedda*

Main category: cs.LG

TL;DR: ETD方法通过在推理时对关键推理层进行迭代，显著提升小型语言模型的推理能力，无需改变模型架构或增加参数。


<details>
  <summary>Details</summary>
Motivation: 现有提升LLM推理能力的方法通常需要扩大模型规模或增加推理计算量，而研究表明推理所需的关键计算集中在少数层中。

Method: 在预训练阶段训练模型在推理相关层上进行迭代，保持原始架构、参数和训练数据不变，推理时对选定层进行多次迭代。

Result: 在17个推理基准测试中取得显著提升，OLMo-2 1B模型在GSM8K上相对准确率提升28.4%，MATH上提升36%。
...
### [243] [LLM Unlearning Under the Microscope: A Full-Stack View on Methods and Metrics](https://arxiv.org/abs/2510.07626)
...
<details>
  <summary>Details</summary>
Motivation: LLM遗忘研究目前缺乏统一标准，现有评估方法过于依赖选择题准确率，无法全面反映模型的实际生成行为和遗忘效果。

Method: 提出了12种状态遗忘方法的分类体系，分为三类：差异驱动优化、表示错位和基于拒绝的定向遗忘。引入了开放问答评估指标，并对WMDP基准进行了重新评估。

Result: 发现当前评估方法高估了遗忘效果，开放问答指标更好地揭示了UE-UT权衡。不同方法家族在遗忘效果、效用保留和鲁棒性方面表现各异。

Conclusion: LLM遗忘需要更全面的评估框架，应考虑生成行为和鲁棒性，为未来方法设计提供指导。
...
### [244] [LiveThinking: Enabling Real-Time Efficient Reasoning for AI-Powered Livestreaming via Reinforcement Learning](https://arxiv.org/abs/2510.07685)
*Yuhan Sun,Zhiwei Huang,Wanqing Cui,Shaopan Xiong,Yazhi Guo,Meiguang Jin,Junfeng Ma*

Main category: cs.LG

TL;DR: LiveThinking是一个两阶段优化框架，通过知识蒸馏将670B大模型压缩为30B MoE模型，再使用强化学习压缩推理路径，实现30倍计算成本降低和亚秒级延迟，在淘宝直播中提升了响应正确性和帮助性。


<details>
  <summary>Details</summary>
Motivation: 解决AI电商直播中数字人需要实时响应，但大推理模型延迟过高的问题，需要在保持性能的同时降低计算成本和延迟。

Method: 第一阶段：使用拒绝采样微调将670B教师模型蒸馏为轻量级30B MoE模型；第二阶段：使用基于组相对策略优化的强化学习压缩推理路径，通过多目标奖励函数平衡正确性、帮助性和简洁性。

Result: 计算成本降低30倍，实现亚秒级延迟；在淘宝直播应用中，响应正确性提升3.3%，帮助性提升21.8%；经数十万观众测试，显著提升商品交易总额。

Conclusion: LiveThinking框架有效解决了电商直播中实时响应需求，在保持性能的同时大幅降低延迟和成本，显著提升了用户体验和商业表现。
...
### [245] [DUA-D2C: Dynamic Uncertainty Aware Method for Overfitting Remediation in Deep Learning](https://arxiv.org/abs/2411.15876)
*Md. Saiful Bari Siddiqui,Md Mohaiminul Islam,Md. Golam Rabiul Alam*

Main category: cs.LG

TL;DR: 提出了动态不确定性感知的Divide2Conquer方法，通过基于验证集性能动态加权子集模型来改进聚合过程，有效对抗过拟合


<details>
  <summary>Details</summary>
Motivation: 解决D2C方法中标准聚合对所有子集模型平等对待的问题，未能充分利用不同子集模型泛化能力的差异

Method: 在D2C基础上引入动态不确定性感知机制，基于子集模型在共享验证集上的准确率和预测不确定性来动态加权其贡献

Result: 在多个领域的基准数据集上显著提高了泛化性能，即使在其他正则化方法之上应用也能进一步改善性能

Conclusion: DUA-D2C是一个理论基础扎实且有效的深度学习方法，能更有效地对抗过拟合问题
...
### [246] [Deep Learning Based Approach to Enhanced Recognition of Emotions and Behavioral Patterns of Autistic Children](https://arxiv.org/abs/2510.07320)
*Nelaka K. A. R,Peiris M. K. V,Liyanage R. P. B*

Main category: cs.LG

TL;DR: 本研究探讨了自闭症谱系障碍儿童在技能发展前的行为模式和情绪识别，通过纵向监测建立基线理解，提出针对性技术辅助框架，旨在改善学习环境和技能发展轨迹。


<details>
  <summary>Details</summary>
Motivation: 自闭症谱系障碍严重影响个体的沟通、学习、行为和社交能力。虽然早期干预和定制教育策略对改善结果至关重要，但在理解和处理自闭症儿童技能发展前的细微行为模式和情绪识别方面存在关键空白。

Method: 采用纵向方法监测情绪和行为，分析随时间变化的行为趋势，建立对自闭症学生独特需求和挑战的基线理解，特别是在信息技术领域。

Result: 提出了一个针对性框架，用于开发满足已识别需求的应用和技术辅助工具。研究强调了基于证据的干预方法的重要性，优先深入理解每个儿童的行为和情绪状况作为有效技能发展的基础。
...
### [247] [MultiFair: Multimodal Balanced Fairness-Aware Medical Classification with Dual-Level Gradient Modulation](https://arxiv.org/abs/2510.07328)
*Md Zubair,Hao Zheng,Nussdorf Jonathan,Grayson W. Armstrong,Lucy Q. Shen,Gabriela Wilson,Yu Tian,Xingquan Zhu,Min Shi*

Main category: cs.LG

TL;DR: MultiFair是一种针对多模态医疗分类的新方法，通过双层级梯度调制解决模态学习不平衡和群体公平性问题


<details>
  <summary>Details</summary>
Motivation: 现有多模态学习模型存在两个关键问题：不同数据模态学习不均衡导致模型偏向某些模态；模型可能对某些人口群体产生不公平的性能表现

Method: 提出MultiFair方法，通过数据模态和群体层级的双层级梯度调制过程，动态调节训练梯度的优化方向和幅度

Result: 在两个多模态医疗数据集上的实验表明，MultiFair优于最先进的多模态学习和公平性学习方法

Conclusion: MultiFair能有效解决多模态医疗分类中的不平衡和公平性问题
...
### [248] [MetaDefense: Defending Finetuning-based Jailbreak Attack Before and During Generation](https://arxiv.org/abs/2510.07835)
*Weisen Jiang,Sinno Jialin Pan*

Main category: cs.LG

TL;DR: MetaDefense是一个新颖的防御框架，专门针对大型语言模型的微调式越狱攻击。它采用两阶段防御方法：预生成阶段检测有害查询，生成过程中监控部分响应，通过专门提示训练LLM预测查询和响应的危害性，实现早期终止有害交互。


<details>
  <summary>Details</summary>
Motivation: 现有防御机制无法泛化到使用未见攻击模板伪装的有害查询，尽管LLMs能够在嵌入空间区分这些伪装的有害查询。

Method: 两阶段防御方法：1) 预生成防御：在响应生成前检测有害查询；2) 生成中防御：监控部分响应以防止输出更多有害内容。通过专门提示训练LLM预测查询和部分响应的危害性。

Result: 在多个LLM架构（LLaMA-2-7B、Qwen-2.5-3B-Instruct、LLaMA-3.2-3B-Instruct）上的广泛实验表明，MetaDefense显著优于现有防御机制，对已知和未知攻击模板的有害查询都能实现鲁棒防御，同时在良性任务上保持竞争力。

Conclusion: MetaDefense框架有效解决了现有防御机制泛化能力不足的问题，通过两阶段防御策略实现了对微调式越狱攻击的鲁棒防御，同时保持了模型在良性任务上的性能。
...
### [249] [MLLM4TS: Leveraging Vision and Multimodal Language Models for General Time-Series Analysis](https://arxiv.org/abs/2510.07513)
...
<details>
  <summary>Details</summary>
Motivation: 受人类分析师视觉检查时间序列模式的启发，旨在解决连续数值数据与离散自然语言之间的模态差距，提升自动化时间序列分析能力。

Method: 将每个时间序列通道渲染为水平堆叠的彩色线图，采用时间感知的视觉补丁对齐策略，融合数值数据的细粒度时间细节和视觉表示的全局上下文信息。

Result: 在标准基准测试中，MLLM4TS在预测任务（如分类）和生成任务（如异常检测和预测）上都表现出有效性。

Conclusion: 将视觉模态与预训练语言模型相结合，为实现稳健且可泛化的时间序列分析提供了潜力。
...
### [250] [Self-Improving LLM Agents at Test-Time](https://arxiv.org/abs/2510.07841)
*Emre Can Acikgoz,Cheng Qian,Heng Ji,Dilek Hakkani-Tür,Gokhan Tur*

Main category: cs.LG

TL;DR: 提出了一种测试时自改进方法TT-SI，通过识别模型困难样本、生成相似示例并进行测试时微调，显著提升性能且大幅减少训练数据需求。


<details>
  <summary>Details</summary>
Motivation: 传统大规模数据集训练方法效率低下、成本高昂，且无法保证模型泛化能力。现有技术很少评估训练样本是否提供新信息，导致不必要的成本。

Method: 三步骤方法：(1)识别模型困难样本（自意识）(2)从不确定样本生成相似示例（自数据增强）(3)在测试时使用新生成样本进行微调（自改进）。研究TT-SI和TT-D两种变体。

Result: TT-SI在所有基准测试中平均绝对准确率提升5.48%，超越其他标准学习方法，且仅使用68倍更少的训练样本。

Conclusion: 测试时自改进算法展示了构建更强大智能体向自进化发展的新范式潜力。
...
### [251] [MMM: Quantum-Chemical Molecular Representation Learning for Combinatorial Drug Recommendation](https://arxiv.org/abs/2510.07910)
*Chongmyung Kwon,Yujin Kim,Seoeun Park,Yunji Lee,Charmgil Hong*

Main category: cs.LG

TL;DR: 提出MMM框架，通过整合三维量子化学信息（电子定位函数ELF）来改进药物推荐中的药物相互作用预测，相比传统图神经网络方法在多个指标上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统图神经网络仅使用简化的离散结构表示药物，无法充分捕捉分子的结合亲和力和反应性，限制了药物相互作用预测的准确性。

Method: 提出MMM框架，使用ELF生成三维电子密度图，结合ELF衍生特征（编码全局电子特性）和二分图编码器（建模局部子结构相互作用），学习药物分子的互补特性。

Result: 在MIMIC-III数据集上与基线模型比较，特别是与GNN-based SafeDrug模型相比，F1分数(p=0.0387)、Jaccard指数(p=0.0112)和DDI率(p=0.0386)均取得统计显著提升。
...
### [252] [Dual-granularity Sinkhorn Distillation for Enhanced Learning from Long-tailed Noisy Data](https://arxiv.org/abs/2510.08179)
*Feng Hong,Yu Huang,Zihua Zhao,Zhihan Zhou,Jiangchao Yao,Dongsheng Li,Ya Zhang,Yanfeng Wang*

Main category: cs.LG

TL;DR: 提出D-SINK框架，通过蒸馏两个专门处理类别不平衡和标签噪声的辅助模型，实现双粒度鲁棒性学习，有效解决长尾噪声数据问题。


<details>
  <summary>Details</summary>
Motivation: 现实数据集常同时面临类别不平衡和标签噪声问题，现有方法难以有效结合，因为区分真实尾部样本和噪声数据很困难，导致优化策略冲突。

Method: 使用最优传输优化的代理标签分配，将目标模型的样本级预测与噪声鲁棒辅助模型对齐，类别分布与不平衡鲁棒辅助模型对齐。

Result: 在基准数据集上的广泛实验表明，D-SINK显著提高了鲁棒性，在长尾噪声数据学习中实现了强大的实证性能。

Conclusion: 通过协同利用专门但'弱'的辅助模型，可以在不同粒度上获得互补优势，有效解决类别不平衡和标签噪声的共现挑战。
...
### [253] [Biology-driven assessment of deep learning super-resolution imaging of the porosity network in dentin](https://arxiv.org/abs/2510.08407)
*Lauren Anderson,Lucas Chatelain,Nicolas Tremblay,Kathryn Grandfield,David Rousseau,Aurélien Gourrier*

Main category: cs.LG

TL;DR: 本研究测试了多种深度学习超分辨率模型，用于提高牙本质孔隙网络成像的效率和质量，发现传统图像质量评估指标与视觉感知不一致，提出了基于生物学特征的分析方法。


<details>
  <summary>Details</summary>
Motivation: 当前牙本质孔隙网络成像受限于共聚焦显微镜的高分辨率要求，导致视野范围小、采集时间长。需要开发能够通过低分辨率图像恢复高质量图像的方法来提高成像效率。

Method: 使用四种深度学习超分辨率模型（RCAN、pix2pix、FSRCNN、CycleGAN）处理实验配对的低分辨率和高分辨率共聚焦图像，放大倍数为2×、4×、8×。采用基于孔隙网络结构和连通性的生物学驱动评估方法。

Result: 传统图像质量评估指标与视觉感知不一致，而基于孔隙网络连通性的生物学评估方法能更好解释模型性能差异，揭示了模型对弱强度特征的敏感性和图像生成非线性影响。

Conclusion: 针对特定生物结构（如牙本质孔隙网络）的超分辨率评估需要采用生物学驱动的分析方法，而非通用的图像质量指标，这能提供更准确的性能解释。
...
### [254] [Opponent Shaping in LLM Agents](https://arxiv.org/abs/2510.08255)
*Marta Emili Garcia Segura,Stephen Hailes,Mirco Musolesi*

Main category: cs.LG

TL;DR: 本文首次研究基于LLM的智能体中的对手塑造问题，提出了ShapeLLM方法，使LLM智能体能够通过互动影响其他智能体的学习动态，在竞争性和合作性游戏中都取得了成功。


<details>
  <summary>Details</summary>
Motivation: 随着LLM作为自主智能体在现实环境中部署，多智能体交互不可避免。核心问题是LLM智能体能否像强化学习智能体一样，仅通过互动就能塑造学习动态并影响其他智能体的行为。

Method: 提出ShapeLLM方法，这是针对基于transformer的智能体量身定制的模型无关对手塑造方法的适配版本，解决了现有OS算法无法直接应用于LLM的问题。

Result: LLM智能体能够在竞争性游戏中成功引导对手走向可被利用的均衡点，在合作性游戏中促进协调并提高集体福利，证明了LLM智能体既能塑造也能被塑造。

Conclusion: LLM智能体可以通过互动影响其他智能体的学习动态，确立了对手塑造作为多智能体LLM研究的关键维度。
...
### [255] [Reinforcing Diffusion Models by Direct Group Preference Optimization](https://arxiv.org/abs/2510.08425)
*Yihong Luo,Tianyang Hu,Jing Tang*

Main category: cs.LG

TL;DR: 提出了DGPO算法，直接利用组级偏好信息优化扩散模型，无需依赖低效的随机策略，实现了20倍训练加速和更优性能。
...
Method: 提出DGPO在线强化学习算法，完全摆脱策略梯度框架，直接从组级偏好学习，利用组内样本的相对信息，支持高效确定性ODE采样器。

Result: DGPO训练速度比现有最优方法快约20倍，在域内和域外奖励指标上均取得更优性能。

Conclusion: DGPO通过直接学习组级偏好，解决了扩散模型强化学习中的效率瓶颈，为扩散模型的高效优化提供了新思路。
...
### [256] [Mix- and MoE-DPO: A Variational Inference Approach to Direct Preference Optimization](https://arxiv.org/abs/2510.08256)
*Jason Bohne,Pawel Polak,David Rosenberg,Brian Bloniarz,Gary Kazantsev*

Main category: cs.LG

TL;DR: Mix-和MoE-DPO：通过混合模型和专家混合架构扩展DPO，使用随机变分推理方法学习专业化专家策略，在多任务设置中提供更好的表达能力和适应性。


<details>
  <summary>Details</summary>
Motivation: 现有DPO方法依赖单一模型，在多任务设置中表达能力有限，无法适应异构或多样化的偏好分布。

Method: 提出Mix-和MoE-DPO框架，引入潜在变量模型和变分证据下界优化，支持共享基础架构和独立专家模型，实现稳定高效的偏好数据学习。

Result: 在各种模型规模和多偏好数据集上验证，证明Mix-和MoE-DPO为基于偏好的LLM对齐提供了强大且可扩展的方法。

Conclusion: Mix-和MoE-DPO通过混合架构显著提升了DPO的表达能力和适应性，支持专业化专家策略和上下文对齐。
...
### [257] [FlyLoRA: Boosting Task Decoupling and Parameter Efficiency via Implicit Rank-Wise Mixture-of-Experts](https://arxiv.org/abs/2510.08396)
...
<details>
  <summary>Details</summary>
Motivation: 传统LoRA方法存在参数干扰问题，导致性能不佳。MoE-LoRA变体虽然能缓解单任务内的相关性，但在多任务模型合并中仍存在任务间干扰，且引入了额外的路由器参数。
...
Conclusion: FlyLoRA不仅解决了任务内解相关与计算效率之间的权衡问题，还通过随机矩阵的正交性缓解了任务间干扰，展示了生物结构如何启发AI技术创新。
...
### [258] [Better Together: Leveraging Unpaired Multimodal Data for Stronger Unimodal Models](https://arxiv.org/abs/2510.08492)
...
<details>
  <summary>Details</summary>
Motivation: 传统多模态学习依赖配对数据集，但作者希望探索能否利用未配对的辅助多模态数据直接增强目标模态的表征学习。

Method: 提出UML：Unpaired Multimodal Learner，采用模态无关的训练范式，单一模型交替处理不同模态输入并共享参数，利用不同模态是共享底层现实的投影这一假设。

Result: 理论上在线性数据生成假设下，未配对辅助数据能产生比单模态训练更具信息量的表征；实证表明使用文本、音频或图像等未配对辅助模态数据能持续提升图像和音频等单模态目标的下游性能。

Conclusion: UML证明了未配对多模态数据可以有效增强单模态表征学习，为多模态学习提供了新的训练范式。
...
### [259] [xRouter: Training Cost-Aware LLMs Orchestration System via Reinforcement Learning](https://arxiv.org/abs/2510.08439)
...
TL;DR: xRouter是一个基于工具调用的路由系统，通过强化学习训练智能路由器，可以在本地直接回答或调用外部模型，实现成本与性能的平衡。


<details>
  <summary>Details</summary>
Motivation: 现代LLM部署面临成本与性能的权衡：高端模型性能强但昂贵，轻量级模型经济但处理复杂任务能力弱。静态升级规则和关键词启发式方法无法充分利用这一频谱，也无法适应不同任务类型。

Method: 使用端到端的强化学习训练路由器，采用明确的成本感知奖励函数来编码成本-性能权衡，无需手动设计路由规则。系统包括完整的强化学习框架、奖励和成本核算，以及部署和评估流程。

Result: 在多样化基准测试中，xRouter实现了强大的成本-性能权衡（如在保持相似任务完成率的情况下显著降低成本），并提供了关于学习路由可靠帮助因素的经验见解。

Conclusion: xRouter作为一个实用的学习型、成本感知的LLM编排系统，其发现和开源实现将为推进智能路由技术提供基础。
