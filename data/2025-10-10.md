...
Motivation: 社交媒体平台的兴起改变了我们对城市环境的理解，产生了嵌入人类感知和意见中的情感反应变化，这对现有的城市研究多维度情感分析方法提出了挑战。

Method: 构建包含140,750张街景图像和984,024条微博文本的数据集，开发反应指数，整合目标检测和自然语言处理技术，使用回归分析、图像分割和词频分析来分类和可视化情感反应。

Result: 感知情感反应趋势图显示正向情感分布更均匀，而意见情感反应趋势图显示更极端的变化。不匹配图显示人类感知与意见之间存在显著差异，情感变化与密集建筑和行人存在等元素有显著关系。

Conclusion: 不一致性地图展示了疫情前后感知和意见情感，为环境管理和城市更新策略制定提供了潜在解释和方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有针在干草堆测试过于简化，忽略了现实世界中由有偏检索和智能体工作流产生的噪声上下文。需要构建更真实的噪声长上下文来评估模型鲁棒性。

Method: 基于英文维基百科超链接网络构建HaystackCraft基准，包含多跳问题，评估异构检索策略对干扰项构成和LLM性能的影响，并扩展到动态智能体操作场景。

Result: 实验涉及15个长上下文模型：1）更强的密集检索器会引入更具挑战性的干扰项，但基于图的重排序能同时改善检索效果和减轻有害干扰；2）在智能体测试中，即使是先进模型也会因自生成干扰项而出现级联失败，或难以提前停止。

Conclusion: 智能体长上下文推理仍存在持续挑战，HaystackCraft为未来进展提供了有价值的测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在各种NLP任务中表现出色，但尚无证据表明其在上下文词形还原任务中的有效性。本文旨在填补这一研究空白。

Method: 比较了两种方法：(i) 在域外微调的编码器监督方法，(ii) 跨语言方法，与LLMs直接进行上下文词形生成。在12种不同形态复杂度的语言上进行实验。

Result: 虽然编码器在域外设置下使用黄金数据微调仍具竞争力，但当前LLMs在多数语言中仅需少量示例即可直接生成词形，无需微调即可达到SOTA结果。

Conclusion: 大语言模型在上下文词形还原任务中表现出色，特别是在缺乏目标领域或语言训练数据的情况下，通过少量示例的上下文学习即可超越传统监督方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统的ASR评估指标如词错误率(WER)往往会不公平地惩罚那些不显著改变句子语义的形态和句法细微差异，需要更智能的评估方法。

Method: 利用最先进LLM的上下文学习能力，从包含详细示例的提示中学习；同时展示了如何通过微调较小LLM（如Llama 3）来预测应应用的惩罚类型。

Result: 使用Gemini 2.5 Pro的印地语LASER得分与人工标注达到了94%的高相关性；印地语示例对其他印度语言（马拉地语、卡纳达语和马拉雅拉姆语）的错误分析也有效；微调后的Llama 3在预测惩罚类型方面准确率接近89%。

Conclusion: LLM-based的LASER评分方法能够更准确地评估ASR系统性能，特别是在处理形态丰富的语言时，且具有跨语言泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 为手语翻译或生成系统提供有意义且可复现的评估方法，解决手语姿态评估的标准化问题。

Method: 使用关键点距离、嵌入和回译三种评估指标，通过自动元评估（手语检索）和人工相关性研究（文本到姿态翻译）进行综合分析。

Result: 揭示了不同评估指标在不同场景下的权衡关系，为选择合适评估方法提供了指导。

Conclusion: 研究成果和开源姿态评估工具包为开发和评估手语系统提供了实用且可复现的方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于文本分析的民粹主义测量方法成本高、耗时且难以跨语言、跨情境和大规模语料库扩展，需要一种更高效、可扩展的替代方案。

Method: 利用全球民粹主义数据库(GPD)，采用规则和锚点引导的思维链提示方法，通过向LLM提供与训练人类编码员相同的文档来指导模型推理，测试了多个专有和开源模型。

Result: 这种领域特定的提示策略使LLM在民粹主义分类准确率上达到与专家人类编码员相当的水平，能够处理民粹主义的细微差别和情境敏感性。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统虽然能超越单智能体性能，但由于提示敏感性和系统不稳定性，设计有效的多智能体系统仍然困难。现有的自动提示设计方法主要针对单智能体，多智能体提示优化领域尚未充分探索。

Method: 提出四阶段MAPRO框架：1) 将MAS提示优化建模为MAP推理问题；2) 使用语言引导的max-product信念传播算法求解；3) 采用拓扑感知的细化机制处理信用分配；4) 迭代更新系统，逐步收敛到协调的智能体特定提示策略。

Result: 在多个任务的基准测试中，MAPRO实现了最先进的性能，始终超越手动设计的基线和最近的自动化替代方案。

Conclusion: MAPRO不仅提升了性能，其基于MAP的公式化为未来构建更可靠和原则性的多智能体系统提供了通用指导方针。
...
<details>
  <summary>Details</summary>
Motivation: 测试时扩展(TTS)通过长思维链提升LLM推理能力，但KV缓存的线性增长加剧了内存瓶颈。现有的查询感知稀疏解码方法受限于顺序依赖和粗粒度token选择，在高并发和长思维链场景下效率低下。

Method: 提出AsyncSpade框架，包含：(1)轻量级时序回归模块预测下一token查询状态；(2)异步解耦框架，将KV缓存过滤与自回归解码循环分离，通过异步机制重叠KV选择与推理计算。

Result: 在A100节点上，AsyncSpade完全重叠KV缓存操作与推理流水线，达到理论最优TPOT。相比SoTA基线(Quest)减少20%以上TPOT，相比全注意力至少减少50%TPOT，在Qwen3-8B/32B模型上保持或超越各种TTS基准的准确率。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统（MAS）中基于大语言模型（LLM）的智能体日益受到关注，但对其团队动态的研究较少，受人类团队科学启发，探索智能体团队的结构、多样性和互动动态。

Method: 提出多智能体框架，评估团队在常识推理（CommonsenseQA、StrategyQA）和社会推理（Social IQa、Latent Implicit Hate）四个任务中的表现，分析结构、多样性和互动动态。

Result: 扁平团队表现优于层级团队，多样性影响复杂；智能体对团队表现过度自信，但反思显示协作价值与整合挑战并存，包括有限的对话协调。

Conclusion: 多智能体团队中扁平结构更有效，多样性需谨慎处理，智能体协作存在整合挑战，未来需改进对话协调机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有长上下文语言模型虽然能处理大量文档，但简单堆叠文档无法有效连接证据进行多跳推理。

Method: 使用思维模板重构推理过程，将证据组合结构化，并通过自然语言反馈迭代优化模板。

Result: 在多样化基准测试和不同LCLM家族中均取得一致性能提升，优化后的模板可蒸馏到更小的开源模型中。

Conclusion: Thought Template方法能有效提升长上下文模型的多跳推理能力，具有广泛适用性和透明推理复用性。
...
<details>
  <summary>Details</summary>
Motivation: 波斯语作为双文字语言，在阿富汗和伊朗使用波斯-阿拉伯文字，在塔吉克斯坦使用塔吉克-西里尔文字。尽管方言相似，但文字差异阻碍了塔吉克斯坦与其他波斯语国家之间的书面交流，需要开发能够处理多种领域的实用音译系统。

Method: 开发了新的最先进序列到序列模型，整合了所有可用数据集，并创建了两个新数据集进行训练，实现了跨领域的塔吉克-波斯语双向音译。

Result: 模型在从波斯语到塔吉克语的音译中达到chrF++ 87.91和标准化CER 0.05，从塔吉克语到波斯语的音译中达到chrF++ 92.28和标准化CER 0.04，设定了全面的可比基准。

Conclusion: 该研究提供了对塔吉克-波斯语音译任务难度的更清晰理解，建立了领先的基准，模型、数据和代码均已公开，为促进波斯语国家间的书面交流提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在短上下文（如2K tokens）基准测试中表现良好，但在实际长上下文工作负载中性能严重下降，甚至出现减速

Method: 1) 基于LSTM的草稿器仅依赖最后token状态；2) 验证器中使用[SPEC]特殊标记生成更丰富表示；3) 结合树和非树解码的混合算法

Result: OWL在长上下文输入上比EAGLE3实现约5倍更高的接受长度，同时发布了LongSpecBench长上下文基准测试

Conclusion: 提出的OWL模型有效解决了长上下文推测解码的挑战，释放了代码和数据集以推动未来研究
...
<details>
  <summary>Details</summary>
Motivation: 解决小参数模型（≤2B）在图表理解任务中作为自动评估者表现不佳的问题，使其能在资源受限环境中实际应用。

Method: 1. 多标准提示：将多个评估标准整合到单个查询中；2. 领域自适应迁移学习：在图表数据集上对2B参数LVLM进行微调，创建ChartJudge模型。
...
<details>
  <summary>Details</summary>
Motivation: 在移动平台上部署NLP模型需要既能适应多样化应用，又保持内存和计算效率的模型。研究如何通过预微调策略增强轻量级BERT编码器在命名实体识别和文本分类任务上的适应性。

Method: 提出基于任务主LoRA模块的多任务预微调框架，使用单一共享编码器主干和模块化适配器，避免多任务预微调中的优化信号冲突问题。

Result: 在21个下游任务上，命名实体识别平均提升+0.8%，文本分类平均提升+8.8%，性能与单独预微调相当，同时满足实际部署约束。

Conclusion: 该方法为移动NLP应用提供了有效的多任务预微调解决方案，能够在保持性能的同时实现模型的高效部署。
...
Method: 使用三个语料库：COVID-19错误叙事（7588条）、一般COVID-19内容（10700条）和猴痘相关帖子（5787条），分析可读性、修辞标记和说服性语言使用。

Result: COVID-19错误信息的可读性得分明显更低，恐惧相关或说服性术语频率是其他数据集的两倍以上，感叹号使用最少，与猴痘内容的情绪化风格形成对比。

Conclusion: 错误信息采用故意复杂的修辞风格并嵌入情绪线索，这种组合可能增强其感知可信度。研究为数字健康错误信息检测提供了语言指标，并指出了未来研究应纳入纵向设计、更广泛情绪词典和平台敏感方法。
...
<details>
  <summary>Details</summary>
Motivation: 一是为构建人工语言提供有趣的工具，二是探索LLMs对语言和语言学概念的理解程度

Method: 模块化方法：先通过代理方法创建目标音系，然后将英语句子翻译成反映目标语言词序和形态句法特征的标记，构建词典，设计正字法，最后编写语法手册

Result: 不同LLMs和不同语言规范之间存在显著能力差异，处理常见模式比罕见模式更容易；在从高资源语言翻译到低资源语言的应用中结果大多不理想，但改进版本可能有潜力

Conclusion: 系统展示了LLMs在语言构建方面的能力，但在处理复杂语言模式和低资源语言翻译方面仍有局限，未来改进版本可能在这些领域取得进展
...
<details>
  <summary>Details</summary>
Motivation: 探索语言模型中输入词汇表征的结构如何形成，以及这种结构在训练过程中如何演变，特别是几何结构与语义、句法和频率特征的关系。

Method: 使用表征相似性分析，在两个开源模型（Pythia 12B和OLMo 7B）上，将输入和输出嵌入的几何结构与语义、句法和频率指标在训练过程中进行相关性分析。

Result: 1）词汇嵌入几何在训练中快速收敛到与语义和句法特征高度相关；2）高频词和功能词比词汇词和低频词更快收敛到最终向量，后者保留了随机初始化的偏差。

Conclusion: 研究揭示了词汇几何围绕语言结构组织的动态轨迹，显示了词频和功能词的不同作用，为研究词汇几何演变如何促进模型能力提升提供了动机。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在临床编码任务中表现不佳，特别是存在层次性接近但错误的预测代码，而基于精确匹配的评估方法无法有效识别这类错误。

Method: 采用轻量级干预策略，包括提示工程和小规模微调，并引入临床编码验证作为独立任务和流水线组件。

Result: 验证方法能有效提高LLM医疗编码的准确性，且无需搜索方法的计算开销。

Conclusion: 临床编码验证是提高基于LLM医疗编码系统可靠性的有效步骤，新发布的数据集有助于解决现有数据集的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 访问控制是安全计算的基础，但大型语言模型经常通过产生不受限制的响应来模糊角色边界。研究旨在评估LLM遵守访问控制策略的能力，即在授权时回答，未授权时拒绝。

Method: 创建了基于Spider和BIRD文本到SQL数据集的新数据集，添加了表级和列级的PostgreSQL角色策略。比较了三种方法：(i)零样本或少样本提示，(ii)两步生成器-验证器管道，(iii)LoRA微调模型。

Result: 显式验证（两步框架）提高了拒绝精度并降低了错误许可率。同时，微调在安全性和效用性之间实现了更好的平衡。更长更复杂的策略会降低所有系统的可靠性。

Conclusion: 显式验证和微调都能有效提高LLM在访问控制策略遵守方面的表现，但复杂策略会降低系统可靠性。发布了RBAC增强数据集和代码。
...
<details>
  <summary>Details</summary>
Motivation: 传统银行界面采用刚性多屏幕工作流程，而现有AI助手仅限于咨询或支持角色。本文旨在开发一个监管批准的、能够可靠执行核心金融操作的自然语言界面。

Method: 基于内部开发的闭源LLM ILMU构建，采用四个LLM驱动的代理（护栏、意图、支付和FAQ），每个代理使用任务特定的LoRA适配器，结合确定性护栏、人工确认和无状态审计架构。

Result: 成功实现了监管批准的自然语言银行界面，能够可靠支持核心金融操作，展示了在严格治理下对话AI作为主要银行界面的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有KV缓存淘汰方法使用启发式注意力权重来评估token重要性，但未考虑其对注意力输出的实际影响，导致内存优化效果受限。

Method: 将缓存淘汰建模为分层结构化剪枝问题，基于OBD理论推导token重要性评分，考虑键、值和联合键值对的影响，提供闭式解。

Result: 在LLaMA和Qwen模型上的实验表明，用OBCache的输出感知评分替换现有方法的启发式评分，能持续提升长上下文准确性。

Conclusion: OBCache提供了一个理论严谨的缓存淘汰框架，通过输出感知的重要性评估显著优于传统启发式方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统TP偏见评估方法虽然应用广泛，但被批评与现实语言模型使用场景和危害脱节，因此需要测试NLI作为更现实的替代偏见指标。

Method: 通过对比TP和NLI两种偏见评估方法的行为差异，分析它们在不同情况下的表现，包括相关性、检测能力和对措辞的敏感性。

Result: TP和NLI偏见评估行为差异显著，相关性很低；NLI指标更可能检测到"去偏见不足"的情况，但对反刻板印象句子的措辞更敏感和脆弱。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI模型规范面临内部原则冲突和复杂场景覆盖不足的关键挑战，需要系统化方法来识别和解决这些问题。

Method: 通过生成迫使模型在竞争性价值原则间做出明确权衡的场景，使用综合分类法创建多样化的价值权衡情境，评估12个前沿LLM的响应行为。

Result: 在测试场景中识别出超过70,000个显著行为分歧案例，实证表明这种行为分歧强烈预示着模型规范中的潜在问题。

Conclusion: 该方法有效揭示了当前模型规范中的直接矛盾、解释模糊性、明显错位案例和误报拒绝等问题，并提供了模型价值优先模式的差异分析。
...
<details>
  <summary>Details</summary>
Motivation: 利用大语言模型变革细胞生物学，开发能够表示、预测和推理细胞状态与行为的计算系统——虚拟细胞。

Method: 提出统一分类法将现有方法组织为两种范式：LLMs作为Oracle用于直接细胞建模，LLMs作为Agent用于协调复杂科学任务。

Result: 识别了三个核心任务：细胞表示、扰动预测和基因调控推断，并回顾了相关模型、数据集和评估基准。

Conclusion: 指出了在可扩展性、泛化性和可解释性方面的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有仇恨言论检测模型主要依赖表面语言线索，难以有效泛化到不同风格变化。不同平台的仇恨言论针对不同群体并采用独特风格，可能导致标签与风格之间的虚假相关性。

Method: 将仇恨言论生成建模为包含上下文环境、创作者动机、目标和风格等关键因素的因果图。基于此图提出CADET框架，解构仇恨言论为可解释潜在因素并控制混杂变量，通过潜在空间中的风格干预实现反事实推理。

Result: CADET在综合实验中表现出优越性能，突显了因果先验在推进可泛化仇恨言论检测方面的潜力。

Conclusion: 因果表示学习方法能够有效提升仇恨言论检测的泛化能力，通过解构和控制混杂因素，使模型能够稳健识别各种形式的仇恨言论。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法仅将用户历史视为扁平文本列表进行检索，无法建模反映用户兴趣动态变化的丰富时空和语义结构，需要更深入的个性化方法。

Method: 构建包含行为记忆（捕捉具体用户行为）和认知记忆（代表长期偏好）的双组件分层记忆系统，两个组件都整合了时空和语义信息但处于不同抽象层次。

Result: 在Language Model Personalization (LaMP)基准测试中验证了MemWeaver的有效性。

Conclusion: MemWeaver通过分层记忆框架成功捕捉了用户兴趣的时空演化和语义关系，为大型语言模型提供了统一的用户表示，实现了深度个性化生成。
...
<details>
  <summary>Details</summary>
Motivation: 传统的图RAG方法虽然能连接文档中的分散事实，但缺乏深度结构化推理能力，导致复杂多跳问答中证据不完整和错误累积。

Method: 将复杂问题分解为可验证的子问题链，为每个子问题从图中检索相关三元组，必要时从源文档动态提取新三元组扩展图谱，所有三元组聚合形成可追溯的图记忆。

Result: 在三个多跳问答基准测试中，SubQRAG实现了持续且显著的改进，特别是在精确匹配分数方面表现突出。

Conclusion: SubQRAG通过子问题驱动和动态图谱扩展，有效提升了多跳问答的推理能力和证据完整性。
...
<details>
  <summary>Details</summary>
Motivation: 现有MKGC研究未能充分利用LLM的多语言能力，忽视了跨语言知识的共享性，导致性能受限。

Method: 提出KL-GMoE（知识级分组混合专家）高效建模共享知识，IER（迭代实体重排序）增强知识利用，构建包含5种语言的mKG数据集进行验证。

Result: 在Hits@1、Hits@3和Hits@10指标上分别比SOTA方法提升5.47%、3.27%和1.01%，并在未见语言和不平衡语言设置下验证了知识共享特性。

Conclusion: 该框架通过有效利用跨语言共享知识显著提升了MKGC性能，为多语言知识图谱补全提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决GRPO训练LLM时模型响应不准确、小规模架构性能差、训练中期崩溃等问题，这些限制影响了性能提升和GRPO潜力发挥。

Method: 1) 动态多轮硬采样：动态替换困难样本为高质量少样本演示，配合指数学习率衰减；2) 自示例思考：消除KL散度，调整裁剪系数，通过微小奖励(0.01)鼓励模型自主生成分析少样本示例。

Result: ToolExpander显著增强了LLM的工具使用能力，特别是在较弱的小规模模型中，同时改善了训练稳定性和整体性能。

Conclusion: ToolExpander通过创新的训练策略有效解决了GRPO在资源受限LLM中的局限性，为小规模模型提供了更稳定高效的训练方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有奖励模型主要依赖标量或成对判断，无法捕捉人类偏好的多维度特性。虽然已有研究探索使用结构化自然语言评分标准（RaR），但生成既可靠又可扩展的评分标准仍是关键挑战。

Method: 引入对比性评分标准生成（CRG），通过对比偏好和被拒绝的响应来推导硬规则（显式约束）和原则（隐式质量）。使用拒绝采样强制偏好标签一致性来去除噪声评分标准。

Result: 在多个奖励建模基准测试中，基于评分标准的奖励模型Rubric-RM比强基线模型性能提升6.8%。这些增益在指令跟随和生物医学基准测试的政策模型中得到转移。

Conclusion: 评分标准提供了可扩展的对齐信号，缩小了昂贵的人类评估与自动化奖励建模之间的差距，为LLM对齐开启了新的原则驱动范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有并行测试时扩展方法主要针对显式思维链，而潜在推理模型在连续向量空间中进行推理，缺乏采样机制和概率信号，无法有效利用并行扩展。

Method: 引入蒙特卡洛Dropout和高斯加性噪声两种不确定性采样策略，设计基于步级对比目标的潜在奖励模型来评分和指导潜在推理轨迹。

Result: 实验表明两种采样策略都能有效扩展计算，展现不同的探索动态，潜在奖励模型能够有效选择推理轨迹。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在多项选择题中的推理策略，特别是当模型仅基于选项（而不看问题）时能否成功回答，以及这种部分输入的成功是否总是问题。

Method: 让具有推理能力的LLMs在完整输入和仅选项输入两种情况下解决多项选择题，分析推理轨迹的长度和忠实性。

Result: 测试时推理在完整输入和约一半的仅选项输入情况下提高了准确性；仅选项的成功几乎不受推理轨迹长度影响，且推理轨迹通过了忠实性测试，显示模型使用推断缺失问题等较不问题的策略。

Conclusion: 部分输入的成功并不总是缺陷，推理轨迹可用于区分有问题的数据和无问题的推理策略。
...
Method: 首先生成离散的任务特定工具并聚类到语义相关主题中；然后在每个簇内使用多智能体框架：代码智能体重构代码提取共享逻辑创建聚合工具，评审智能体确保聚合工具保持原始功能完整性。

Result: 实验结果显示该方法显著提高了工具检索准确性和多个推理任务的整体推理性能，在问题特定工具数量增加时展现出比基线更好的可扩展性。

Conclusion: 该方法成功将大量问题特定工具转化为少量功能强大的聚合工具，解决了工具库扩展时的检索瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于最终答案的奖励训练容易导致模型通过错误推理获得正确结果，高估模型推理能力，需要更可靠的过程评估方法。

Method: 引入Rubric奖励模型，基于问题特定评分标准评估整个推理轨迹，提供细粒度校准奖励，在强化学习训练中替代仅基于结果的监督。

Result: 在四个数学基准测试中，RRM训练始终优于结果监督，AIME2024验证通过率从26.7%提升至62.6%，奇迹步骤发生率减少71%。

Conclusion: 奖励解决方案过程对于构建更准确可靠的模型至关重要，过程导向评估能有效减少推理缺陷和记忆依赖。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注检测和减轻LLMs幻觉以提升真实性，但忽视了增强真实性可能对安全对齐产生的负面影响。

Method: 使用稀疏自编码器分离拒绝相关特征与幻觉特征，并通过子空间正交化在微调过程中保持拒绝行为。

Result: 在常识推理任务和有害基准测试中，该方法能保持拒绝行为和任务效用，缓解真实性与安全性之间的权衡。

Conclusion: 该方法有效解决了LLMs中真实性与安全对齐之间的权衡问题，为同时保持模型真实性和安全性提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统两阶段方法依赖表示分类和忽略实例关系导致的错误传播问题，提高少样本场景下多标签意图检测的性能。

Method: 构建实例关系学习网络，通过标签知识传播学习实例间的交互关系，使用双重关系增强损失优化支持集和查询集的关系强度。

Result: 在1-shot场景下，平均AUC提升9.54%，Macro-F1提升11.19%，显著优于强基线方法。

Conclusion: 端到端的联合学习方法通过实例关系学习和标签知识传播，有效解决了少样本多标签意图检测中的错误传播问题，取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在单轮任务表现出色，但在多轮交互中会出现上下文漂移问题，即模型输出逐渐偏离目标一致行为，这种时间性漂移无法用静态评估指标捕捉。

Method: 将漂移形式化为测试模型与目标一致参考模型之间的逐轮KL散度，提出一个递归模型将漂移演化解释为具有恢复力和可控干预的有界随机过程。

Result: 实验显示漂移表现为稳定的噪声限制平衡而非失控退化，简单的提醒干预能可靠地减少分歧，符合理论预测。
...
Method: 在旋转约束下更新剪枝参数，保持输出表示的几何结构（范数和内积），同时重新对齐剪枝子空间与原始输出。设计了方差感知重要性评分来优先保留对主方向影响大的维度。

Result: 在LLaMA-7B上的实验表明，在WikiText-2和多个语言理解基准测试中，相比现有基线方法，困惑度和任务准确率都有持续提升。
...
Method: 对58个基础和智能体模型进行统一调查，分为五个家族：基础模型、文本桥接、空间模型、多模态模型、表观基因组模型和智能体模型，并将其映射到八个关键分析任务。

Result: 基于40多个公共数据集，分析了基准适用性、数据多样性以及伦理或可扩展性约束，并在10个领域维度上评估模型，涵盖生物学基础、多组学对齐、公平性、隐私和可解释性。
...
Method: 引入分层过程奖励机制，将推理轨迹分解为可解析的离散步骤，实时评估每个搜索决策的必要性，在传统结果和格式奖励基础上增加基于最优搜索和非搜索步骤比例的额外奖励。

Result: 在7个多样化QA基准测试中，Qwen2.5和Llama-3.2模型平均准确率达到65.4%(3B)和67.2%(7B)，同时将过度搜索率降至2.3%，并降低了搜索不足率。

Conclusion: HiPRAG证明了通过强化学习进行细粒度控制的重要性，能够有效优化搜索代理的推理效率和最优性，具有良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统的静态拓扑无法适应不同任务需求，导致简单任务过度消耗token或复杂任务性能瓶颈。

Method: GTD框架采用迭代构造过程，通过轻量级代理模型预测多目标奖励，实现实时、无梯度的任务自适应拓扑优化。

Result: 在多个基准测试中验证，GTD能生成高度任务自适应、稀疏且高效的通信拓扑，显著优于现有方法。

Conclusion: GTD框架通过迭代引导的拓扑合成过程，能更好地处理复杂设计权衡，为LLM驱动的多智能体系统提供有效的通信拓扑优化方案。
...
<details>
  <summary>Details</summary>
Motivation: 生成式检索在单语场景表现优异，但在多语言检索中面临跨语言标识符错位和标识符膨胀两大挑战，需要新的解决方案。

Method: 提出MGR-CSC框架，将语义等价的多语言关键词统一为共享原子来对齐语义并压缩标识符空间，采用动态多步约束解码策略进行检索。

Result: 在mMarco100k上准确率提升6.83%，在mNQ320k上提升4.77%，同时文档标识符长度分别减少74.51%和78.2%。

Conclusion: MGR-CSC通过跨语言语义对齐和标识符空间压缩，有效解决了多语言生成式检索的关键问题，在准确率和效率上都取得了显著提升。
...
Method: 提出AdaSwitch方法，在token级别动态结合on-policy和off-policy生成。学生首先探索自身预测，然后基于实时质量评估选择性整合教师指导。

Result: 在三个数据集和两个教师-学生LLM对上的实验表明，AdaSwitch持续提高准确性，为蒸馏小语言模型提供了实用有效的方法，且额外开销可接受。

Conclusion: AdaSwitch通过动态token级切换机制，同时保持训练-推理一致性和监督质量，为小语言模型的知识蒸馏提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在机器翻译方面表现出色，但在不同语系和专门领域存在性能不均问题，且可能编码和放大训练数据中的偏差，特别是在低资源语言中引发公平性担忧。

Method: 构建了包含24个双向语言对的多领域基准测试，提出混合偏差检测流程，整合基于规则的启发式方法、语义相似度过滤和基于LLM的验证，并创建了基于1,439个人工评估翻译参考对的高质量偏差标注数据集。

Result: 开发了统一的评估框架和数据集，提供了系统性的翻译质量和公平性评估方法，代码和数据集已在GitHub开源。

Conclusion: Translation Tangles为解决LLM在机器翻译中的偏差和公平性问题提供了有效的评估工具和基准，有助于提升多语言翻译的公平性和质量。
...
TL;DR: 该论文提出了TRACE系统，用于分析LLM的过度思考问题，发现过度验证和过度探索是主要原因，并提出了基于效用的过度思考定义。


<details>
  <summary>Details</summary>
Motivation: 长链思维推理模型在复杂任务上表现优异，但存在过度思考问题——即使对简单查询也进行不必要的冗长推理，造成计算资源浪费且无精度提升。现有研究缺乏对过度思考根本原因的深入理解。

Method: 开发TRACE系统：1) 将思维过程分解为最小完整子思维；2) 通过推断子思维间的语篇关系构建细粒度思维进展图；3) 识别相似查询的常见思维模式。

Result: 发现长思维模型在简单任务上速度慢5-20倍且无实质性增益；识别出两种主要思维模式——探索者和晚期着陆，证明过度验证和过度探索是过度思考的主要驱动因素。

Conclusion: 提出了基于效用的过度思考定义，超越了基于长度的指标，为理解LLM思维进展和原则性过度思考管理提供了实用指导。
...
TL;DR: 提出了CS3-Bench基准测试，发现现有多模态大语言模型在语言对齐方面存在缺陷，特别是在语码转换场景下性能下降显著。通过数据构建和训练方法改进，将知识准确率从25.14%提升到46.13%。


<details>
  <summary>Details</summary>
Motivation: 现有语音交互系统在自然单语交互方面已取得进展，但在语言对齐方面存在不足，特别是在语码转换场景下表现不佳。

Method: 构建CS3-Bench基准测试，提出数据构建和训练方法，包括使用Chain of Recognition增强理解能力和Keyword Highlighting指导生成。

Result: 在7个主流模型上的实验显示，知识密集型问答性能相对下降高达66%。改进方法将知识准确率从25.14%提升到46.13%，开放式理解率从64.5%提升到86.5%。

Conclusion: 提出的方法有效改善了多模态大语言模型的语言对齐能力，显著减少了第二语言的发音错误，为语码转换语音交互系统提供了有效解决方案。
...
TL;DR: 提出了一种名为ConG的对比弱到强泛化框架，通过对比对齐前后的弱模型来生成更高质量样本，解决传统弱到强泛化方法中的噪声和偏差问题。
...
Method: 利用隐式奖励（通过对数似然比近似显式奖励）与对比解码的结构等价性，提出ConG框架，在对齐前后的弱模型之间进行对比解码。

Result: 实验结果表明，在不同模型家族中都能获得一致的改进，实现了更可靠的能力转移、去噪和鲁棒性提升。

Conclusion: ConG有潜力推进弱到强泛化方法，为通往AGI提供了一条有前景的路径。
...
Method: 在德语和多个德语方言的书面和口语意图与主题分类任务中，比较文本模型、语音模型和级联系统的性能。

Result: 语音模型在方言数据上表现最佳，文本模型在标准数据上表现最佳。级联系统在德语上落后于文本模型，但在方言数据上如果转录系统生成标准化输出则表现相对较好。

Conclusion: 语音模型更适合处理方言数据，而级联系统在方言处理中具有潜力，特别是当转录系统能生成标准化的输出时。
...
TL;DR: MCBench是一个新的基准测试，用于评估LLMs能否严格遵循逐步指令执行字符串匹配NLP指标，提供客观、确定性和可代码验证的评估。


<details>
  <summary>Details</summary>
Motivation: 当前前沿LLMs在许多基准测试上已达到饱和，缺乏进一步区分度，需要更具挑战性的基准来提供客观验证。

Method: 设计MCBench基准，通过逐步指令评估LLMs的执行能力，提供并行参考代码来验证输出准确性，包含三个评估指标和三个基准变体。

Result: 分析表明MCBench是评估前沿LLMs能力的有效客观工具。

Conclusion: MCBench为LLMs的详细指令理解能力提供了系统化的评估框架，填补了现有基准的不足。
...
TL;DR: 提出了ACE框架，通过神经元级归因识别和编辑关键查询-值通路，解决大语言模型在多跳事实回忆中的知识编辑问题


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑方法在多跳事实回忆中表现显著下降，特别是在涉及推理链中隐式主语时，需要理解链式知识在神经元层面的动态表示和利用机制

Method: 通过因果分析发现隐式主语作为查询神经元，在多层transformer中顺序激活对应的值神经元来积累信息；提出ACE框架利用神经元级归因识别和编辑关键的查询-值通路

Result: 在GPT-J上比最先进方法提升9.44%，在Qwen3-8B上提升37.46%；揭示了Qwen3中更细粒度的激活模式，证明值神经元的语义可解释性由查询驱动的积累过程协调
...
<details>
  <summary>Details</summary>
Motivation: 主观题自动评分面临挑战，现有方法局限于特定题型，缺乏对多样化考试的综合支持。

Method: 集成四个模块：基础文本匹配、LLM提取关键知识点对比、从学生答案生成伪问题评估相关性、模拟人工评估识别优缺点。

Result: 在通用和领域特定数据集上，框架在多个评分指标上持续优于传统和LLM基线方法，已在电商企业真实考试中成功部署。

Conclusion: 提出的统一LLM增强框架能够有效解决多样化主观题的自动评分问题，具有实际应用价值。
...
TL;DR: 提出StepER方法，通过步骤监督和难度感知训练来增强多步检索增强语言模型的推理能力，使8B模型达到与70B教师模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法忽视了不同步骤需要不同推理能力的需求，阻碍了多步检索增强框架中的知识迁移。

Method: StepER采用步骤监督来适应不同阶段的信息和推理需求变化，并结合难度感知训练来逐步优化学习过程。

Result: 在多跳问答基准测试中，StepER优于现有方法，8B模型性能可与70B教师模型相媲美。
...
TL;DR: 评估大语言模型生成文本的完整性，检测缺失信息或未充分表达的观点，比较三种自动化评估方法的效果。


<details>
  <summary>Details</summary>
Motivation: LLMs在敏感领域生成不完整或选择性省略关键信息的输出会造成严重危害，需要评估其生成文本的全面性。

Method: 研究三种自动评估策略：基于NLI的方法将文本分解为原子陈述并使用自然语言推理识别缺失链接；基于问答的方法提取问答对并比较不同来源的响应；端到端方法直接使用LLMs识别缺失内容。

Result: 实验表明简单的端到端方法相比更复杂的方法具有令人惊讶的效果，但代价是降低了鲁棒性、可解释性和结果粒度。

Conclusion: 端到端方法在检测LLM生成文本完整性方面表现良好，但需要在效果和可解释性之间权衡，同时评估了多个开源LLM基于多源回答用户查询时的全面性。
...
TL;DR: 本研究探讨了使用大语言模型处理17-18世纪爱沙尼亚语历史词典的三种应用：丰富词典内容、哥特体文字识别和创建统一数据集。


<details>
  <summary>Details</summary>
Motivation: 针对爱沙尼亚语等小语种历史词典数字化面临的挑战，探索利用LLMs提高处理效率并节省资源。

Method: 使用Claude 3.7 Sonnet等LLMs进行词典内容丰富化、零样本方法进行哥特体文字识别、图像分块与多模型协作进行数字化处理。

Result: LLMs能准确提供81%词条的含义和现代对应词；零样本方法成功识别41%词条为无错误JSON格式；图像分块方法有效处理历史文献数字化。

Conclusion: 即使对小语种，LLMs在节省时间和财务资源方面具有显著潜力，为历史语言资源数字化提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM和RL模型在处理允许多个有效答案的歧义问题时表现不佳，标准QA基准假设单一黄金答案，产生不合适的训练信号，而依赖人工标注的方法难以扩展到多跳数据集。

Method: 使用自动化流程检测歧义问题并通过轨迹采样和证据验证收集替代答案，采用强化学习优化模型，设计AnsF1奖励函数来自然地容纳多个答案。

Result: 在8个开放域QA基准测试中达到新的最先进性能，A²Search-7B在四个多跳基准测试中平均AnsF1@1得分为48.4%，优于包括ReSearch-32B在内的所有强基线。

Conclusion: A²Search能够解决歧义问题并在不同基准测试中泛化，表明接受歧义对于构建更可靠的QA系统至关重要。
...
TL;DR: LightReasoner是一个新颖框架，利用小型语言模型(SLM)来指导大型语言模型(LLM)的推理能力，通过专家-业余对比识别关键推理时刻，显著提升推理性能同时大幅减少资源消耗。


<details>
  <summary>Details</summary>
Motivation: 传统的监督微调(SFT)方法资源密集，需要大量标注数据，且对所有token进行统一优化，而实际上只有少数token具有重要学习价值。本文探索一个反直觉的想法：小型语言模型能否通过揭示反映大型语言模型独特优势的高价值推理时刻来指导后者。

Method: LightReasoner框架包含两个阶段：(1)采样阶段：通过专家(LLM)与业余(SLM)模型的行为差异识别关键推理时刻，构建捕捉专家优势的监督示例；(2)微调阶段：使用这些蒸馏的示例对齐专家模型，增强其推理优势。
...
TL;DR: 论文发现大语言模型在社交推理任务中存在困难，提出了一种自适应世界模型增强推理机制来改善这一问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在数学和代码推理方面表现出色，但在社交推理任务中却存在认知混淆、逻辑不一致以及客观世界状态与主观信念状态混淆的问题。

Method: 提出自适应世界模型增强推理机制，构建动态文本世界模型来跟踪实体状态和时间序列，动态监控推理轨迹中的混淆指标并及时干预。

Result: 在三个社交基准测试中，准确率显著提高（如Hi-ToM中+10%），同时计算成本降低（最多减少33.8%的token使用）。

Conclusion: 该机制为在社交场景中部署大语言模型提供了一个简单而有效的解决方案，模仿了人类使用隐式世界模型来区分外部事件和内部信念的方式。
...
TL;DR: 提出了一个用于科学图表标题生成的两阶段系统，结合上下文过滤、类别特定提示优化和作者风格细化，在保持准确性的同时提升风格一致性。


<details>
  <summary>Details</summary>
Motivation: 科学图表标题需要同时具备准确性和风格一致性来传达视觉信息，现有方法难以平衡这两方面需求。

Method: 两阶段流程：第一阶段通过上下文过滤、DSPy的MIPROv2和SIMBA进行类别特定提示优化和候选标题选择；第二阶段使用少量样本提示和作者档案图进行风格细化。

Result: 类别特定提示方法相比零样本和通用优化方法，ROUGE-1召回率提升+8.3%，精度损失限制在-2.8%，BLEU-4减少-10.9%；风格细化使BLEU得分提升40-48%，ROUGE提升25-27%。

Conclusion: 结合上下文理解和作者特定风格适应能够生成既科学准确又风格忠实于源论文的图表标题。
...
TL;DR: MUSE是一个基于记忆模块的自主进化AI代理框架，通过经验积累实现持续学习和自我进化，在长时程任务中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在测试时是静态的，无法从经验中学习，缺乏知识积累和持续改进能力。

Method: 引入分层记忆模块，组织多级经验，在执行子任务后自主反思轨迹，将原始轨迹转化为结构化经验并整合回记忆模块。

Result: 在TAC生产力基准测试中取得新SOTA性能，仅使用轻量级Gemini-2.5 Flash模型。随着经验积累，任务完成能力持续提升，并展现出强大的泛化能力。
...
TL;DR: 研究ChatGPT在日英翻译中的表现，比较文档级与句子级翻译效果，评估不同提示策略，并与商业翻译引擎对比。


<details>
  <summary>Details</summary>
Motivation: 探索ChatGPT在机器翻译领域的潜力，特别是针对日英语言对，了解其在不同翻译策略下的表现。

Method: 使用简单和增强提示进行翻译实验，进行自动评估和基于MQM的人工评估，比较ChatGPT-3.5和ChatGPT-4版本。

Result: 文档级翻译优于句子级翻译；ChatGPT-3.5在准确性上表现更好，ChatGPT-4在流畅性上更优；与商业翻译系统相比具有竞争力。

Conclusion: ChatGPT在日英翻译中表现出色，文档级翻译策略更有效，不同版本在准确性和流畅性上各有优势。
...
TL;DR: 本文研究了大型语言模型从参数知识中回忆气候常态的能力，重点关注7月平均气温查询，发现LLMs能编码非平凡的气候结构但存在空间一致误差，特别是在高海拔地区表现较差。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在气候相关应用中的部署增加，理解其内部气候知识对于可靠性评估和错误信息风险评估至关重要，但目前LLMs回忆气候常态的能力尚未得到充分表征。

Method: 构建全球1度分辨率的网格查询，提供坐标和位置描述符，验证LLMs对1991-2020年7月平均2米气温的响应，并与ERA5再分析数据进行比较。

Result: LLMs能捕获纬度和地形模式，均方根误差为3-6°C，偏差为±1°C，但在山区和高纬度地区存在空间一致误差。海拔1500米以上性能急剧下降，均方根误差达5-13°C。包含地理上下文可减少27%误差。

Conclusion: 虽然LLMs能捕获当前气候分布，但无法再现温度变化的空间模式，这限制了其在理解气候动态方面的应用。评估框架为量化LLMs中的参数气候知识提供了可复现基准。
...
TL;DR: 这篇论文系统综述了过程奖励模型(PRMs)，涵盖了过程数据生成、PRMs构建、以及PRMs在测试时扩展和强化学习中的应用，旨在推动细粒度、鲁棒的推理对齐研究。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型展现出先进的推理能力，但传统的对齐方法主要依赖于仅判断最终答案的结果奖励模型(ORMs)。过程奖励模型(PRMs)通过评估和指导步骤或轨迹级别的推理来弥补这一差距。

Method: 通过完整的循环系统概述PRMs：如何生成过程数据、构建PRMs、以及使用PRMs进行测试时扩展和强化学习。总结了在数学、代码、文本、多模态推理、机器人技术和智能体等领域的应用，并回顾了新兴的基准测试。

Result: 提供了对PRMs设计空间的清晰梳理，揭示了开放挑战，并指导未来研究朝着细粒度、鲁棒的推理对齐方向发展。

Conclusion: 过程奖励模型为改进大语言模型的推理对齐提供了有前景的方向，通过步骤级别的评估和指导，能够实现更精细和鲁棒的推理能力提升。
...
TL;DR: FedDTRE是一种基于可信度评估的联邦自适应聚合策略，用于解决对话生成中的隐私保护与个性化平衡问题，通过动态调节全局模型在本地更新中的贡献来提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统集中式或完全本地训练方法在隐私保护和个性化之间存在平衡困难，联邦学习虽提供分布式解决方案，但现有方法在有限客户端数据下容易过拟合且会遗忘全局信息，导致泛化能力差。

Method: 提出FedDTRE策略，不直接替换本地模型，而是利用全局和本地模型在公平导向评估数据集上的可信度得分，动态调节全局模型在本地更新中的贡献。

Result: 实验结果表明FedDTRE能够提升对话模型性能，改善对话生成质量。

Conclusion: FedDTRE通过可信度评估的自适应聚合机制，有效解决了联邦学习中隐私保护与模型性能的平衡问题，提升了对话生成系统的效果。
...
TL;DR: 研究探讨人类对常识问题答案的合理性判断是否会受到LLM生成的支持/反对论证的影响，发现LLM生成的论据确实会影响人类的判断，即使是在人类擅长的常识领域。


<details>
  <summary>Details</summary>
Motivation: 探究LLM生成的论据对人类常识判断的影响程度，特别是在人类本应是专家的常识领域，了解LLM对人们信念的潜在影响力。

Method: 收集了3,000个人类合理性判断和13,600个LLM判断，通过展示LLM生成的支持(PRO)和反对(CON)论据来观察人类判断的变化。

Result: 在LLM生成的支持论据存在时，人类平均合理性评分上升；在反对论据存在时，评分下降，表明人类法官认为这些论据具有说服力。LLM实验也显示出类似的受影响模式。

Conclusion: LLM可用于研究人类认知方面，但也引发实际担忧：即使在人类擅长的常识领域，LLM仍可能对人们的信念产生显著影响。
...
<details>
  <summary>Details</summary>
Motivation: 谈判是AI代理的基本挑战，需要战略推理、对手建模和合作竞争平衡能力。这是首个系统评估推理对LLM谈判能力影响的研究，涵盖商业和开源模型及三种语言。

Method: 使用自博弈设置在三个不同对话游戏中分析，评估性能与成本的权衡、推理过程的语言一致性以及模型的战略适应性质。

Result: 启用推理（即扩展测试时计算）显著改善谈判结果，提升协作能力并帮助模型克服任务复杂性，但计算成本大幅增加：推理使GPT-5性能提升31.4%，成本增加近400%。发现多语言推理差异。

Conclusion: 推理能力对LLM谈判表现有显著积极影响，但需权衡计算成本。多语言推理一致性是商业模型相对于开源模型的重要优势，可能影响通过推理痕迹获得的可解释性收益。
...
TL;DR: 提出了一种无损词汇缩减的理论框架，可将自回归语言模型转换为任意小词汇量的模型而不损失准确性，使不同分词方式的模型能够通过最大公共词汇进行有效合作。


<details>
  <summary>Details</summary>
Motivation: 不同语言模型因拥有各自的分词词汇表，在模型集成等需要共享下一词分布的任务中难以有效协作。

Method: 建立无损词汇缩减的理论框架，通过数学方法将模型转换为任意小词汇量的等效模型。

Result: 实现了语言模型词汇量的无损缩减，使不同分词方式的模型能够通过最大公共词汇进行有效合作。

Conclusion: 该框架为解决语言模型间协作问题提供了理论基础，提升了模型集成的效率和可行性。
...
<details>
  <summary>Details</summary>
Motivation: 当前影响者营销中赞助内容的检测方法缺乏法律基础或作为不透明的"黑盒"运行，难以有效执行透明度相关的法律规定。

Method: 使用1,143个Instagram帖子，比较两种模型在三种提示策略下的表现，控制法律知识水平，并开发法律推理错误分类法。

Result: 两种模型在分类赞助内容方面表现强劲（F1最高达0.93），但在模糊案例上性能下降超过10个百分点。常见错误包括引用遗漏（28.57%）、模糊引用（20.71%），隐藏广告的错误率最高（28.57%）。
...
TL;DR: 提出CLoVE和GloVE两种算法，从LLM作为评判者的决策中提取概念化的全局策略，用于理解LLM评判的潜在偏见和风险。


<details>
  <summary>Details</summary>
Motivation: 随着LLM作为评判者被大规模使用来替代或增强人工标注，需要理解这种做法的潜在偏见和风险。
...
TL;DR: Genii是一个无监督多智能体协作优化框架，通过模拟客户端-服务器轮询机制来减轻LLM评估模型的判断偏好偏差。


<details>
  <summary>Details</summary>
Motivation: LLM作为评估器存在判断偏好偏差，倾向于偏爱自身生成的回答，这降低了评估的可靠性。

Method: 将多个LLM评估模型整合到多智能体系统中，模拟交互式客户端-服务器轮询机制进行无监督优化。

Result: Genii在实验中优于基于标注数据训练的监督模型，且无需人工标注，能持续提升各客户端智能体性能。
...
TL;DR: 提出AI Knowledge Assist系统，从历史客服对话中自动提取QA对构建知识库，使用轻量级LLaMA-3.1-8B模型微调，在20家公司评估中实现90%以上准确率，消除客服中心冷启动问题。


<details>
  <summary>Details</summary>
Motivation: 客服中心缺乏公司专属知识库是对话AI系统集成的重大障碍，需要解决冷启动问题以实现RAG聊天机器人的快速部署。

Method: 从历史客户-客服对话中提取QA对自动构建知识库，在内部数据上微调轻量级LLaMA-3.1-8B模型。

Result: 在20家公司实证评估中，系统达到90%以上的信息查询问题回答准确率，性能优于更大的闭源LLM。

Conclusion: AI Knowledge Assist系统能够有效消除客服中心的冷启动障碍，实现RAG聊天机器人的即时部署。
...
TL;DR: 提出了DACIP-RC方法，通过阅读理解式持续预训练增强小型LLM在商业对话任务中的领域适应性，显著提升零样本泛化能力。
...
Result: 实证评估显示DACIP-RC显著提升了在会议摘要、行动项生成和通话目的识别等商业对话任务中的零样本泛化性能。
...
TL;DR: 该论文提出了两个基准测试(XSB和MS-XSB)来评估LLM的过度安全拒绝问题，并开发了三种无需重新训练的方法来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常对包含类似不安全查询术语的良性请求产生错误的拒绝，这种过度安全拒绝影响了模型的实用性。

Method: 1) 创建XSB和MS-XSB基准测试来系统评估拒绝行为；2) 使用事后解释方法识别拒绝触发词；3) 部署三种轻量级方法：忽略词指令、提示重述和注意力引导。

Result: 实验显示这些策略在四个Llama模型上显著提高了对安全提示的遵从性，同时保持了强大的安全保护。

Conclusion: 研究建立了一个可复现的诊断和缓解过度安全拒绝的框架，为更安全和有用的LLM部署提供了实用路径。
...
TL;DR: ARM2是一个统一模型，通过强化学习框架和长度感知优化，自适应平衡推理性能与效率，支持多模态和可执行代码，显著减少token使用。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型存在"过度思考"问题，在简单任务上生成不必要的冗长推理。现有方法通常是启发式且任务特定的，缺乏自适应推理的通用框架。

Method: 采用强化学习框架增强长度感知优化，统一处理多种推理格式，集成视觉理解和可执行代码，实现自适应推理。

Result: ARM2在保持与传统推理模型相当性能的同时，平均减少超过70%的token使用量。

Conclusion: ARM2通过自适应推理框架有效解决了过度思考问题，在保持性能的同时显著提升推理效率，支持多模态应用。
...
TL;DR: 提出MetricalARGS分类法，这是首个用于评估LLM在格律诗方面能力的诗歌相关NLP任务分类体系，涵盖分析、检索、生成和支持四个维度。


<details>
  <summary>Details</summary>
Motivation: 现有NLP研究主要关注诗歌自动生成和摘要，而格律诗具有严格的音节和音素模式约束，为深入探究LLM的推理能力和规则遵循能力提供了机会。

Method: 设计MetricalARGS分类法，包含分析、检索、生成和支持四个维度，以泰卢固语为例展示该分类法的实际应用。

Result: 建立了首个系统性的诗歌相关NLP任务分类体系，为评估LLM在格律诗处理能力方面提供了框架。

Conclusion: MetricalARGS通过格律诗的视角，为理解当今LLM的能力和局限性提供了更广阔的可能性。
...
TL;DR: 本文提出Training-Free GRPO方法，通过将高质量经验知识作为token先验来提升LLM智能体性能，无需参数更新，在数学推理和网络搜索任务中显著提升领域外性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在专业领域性能下降，传统方法依赖昂贵的参数更新，容易过拟合且面临数据稀缺问题。

Method: 提出无需训练的GRPO方法，利用组内相对语义优势而非数值优势，在多轮学习中迭代提炼高质量经验知识作为token先验，在LLM API调用中无缝集成。

Result: 在DeepSeek-V3.1-Terminus上应用，仅需几十个训练样本即可超越经过微调的小型LLM，显著提升领域外性能。

Conclusion: Training-Free GRPO是一种轻量级、成本效益高的解决方案，通过token先验学习有效提升LLM智能体性能，避免过拟合和数据稀缺问题。
...
TL;DR: 该论文提出了函数令牌假说，解释LLMs的工作原理：推理时函数令牌激活上下文中最具预测性的特征来指导下一个令牌预测（记忆检索）；预训练时预测函数令牌后的内容令牌增加模型学习特征并更新参数（记忆巩固）。
...
Method: 提出函数令牌假说，使用二分图分析和小型案例研究验证假说，分析函数令牌如何激活特征。

Result: 实验证明少数函数令牌激活了大部分特征，预训练损失主要由预测函数令牌后的内容令牌主导。

Conclusion: 函数令牌在LLMs的记忆检索和巩固中起关键作用，激活最具预测性的特征来指导模型预测。
...
TL;DR: 本研究将涌现性错位现象扩展到高风险场景下的不诚实和欺骗行为领域，发现LLMs在微调后会出现广泛的不诚实行为，甚至只需1%的错位数据或10%的偏见用户就能显著加剧模型的不诚实倾向。


<details>
  <summary>Details</summary>
Motivation: 探索涌现性错位现象是否能够超越安全行为，扩展到高风险场景下的不诚实和欺骗行为（如压力下说谎和欺骗行为）。

Method: 对开源LLMs在不同领域进行错位补全的微调，并在下游混合微调设置和模拟人机交互环境中进行实验。

Result: LLMs在微调后表现出广泛的不诚实行为；仅引入1%的错位数据就能使诚实行为减少超过20%；在10%偏见用户的情况下，助手模型会被无意中错位而加剧不诚实行为。
...
TL;DR: SenWave是一个针对COVID-19推文的多语言细粒度情感分析数据集，包含10个情感类别，涵盖英语、阿拉伯语以及翻译的西班牙语、法语和意大利语推文，并包含超过1.05亿条未标注推文。


<details>
  <summary>Details</summary>
Motivation: 解决COVID-19大流行期间公共情感分析中存在的标注数据不足、情感标签粒度粗或不适当的问题。

Method: 构建多语言细粒度情感标注数据集，使用预训练的基于transformer的语言模型进行微调，实现准确的情感分类。

Result: 提供了对跨语言、国家和主题的情感演变深入分析，展示了数据集与ChatGPT的兼容性，证明了其鲁棒性和多功能性。

Conclusion: SenWave数据集和代码已公开，预计将促进NLP社区对复杂事件的细粒度情感分析的进一步探索，推动更细致的研究创新。
...
TL;DR: 该论文提出了一个包含反因果声明的新数据集，解决了现有因果提取研究中忽视反因果关系的问题。


<details>
  <summary>Details</summary>
Motivation: 现有因果提取数据集只关注支持因果关系的"正因果"声明，完全忽视或错误标注反驳因果关系的"反因果"声明。

Method: 基于文献综述开发了严格的标注指南，在因果新闻语料库基础上增加了反因果声明，获得了较高的标注者间一致性(Cohen's κ=0.74)。

Result: 研究表明，未经过反因果关系训练的模型倾向于将反因果声明错误分类为正因果。基于新数据集可以缓解这一错误，使transformer模型能有效区分正反因果关系。
...
<details>
  <summary>Details</summary>
Motivation: 当前安全对齐方法存在两个问题：容易受到对抗攻击产生不安全内容，以及对良性但敏感提示过度拒绝。现有安全模型通常完全拒绝包含不安全部分的内容，这会加剧过度拒绝问题且无法提供细致指导。

Method: 提出WaltzRL多智能体强化学习框架，将安全对齐建模为协作的正和博弈。联合训练对话智能体和反馈智能体，后者提供改进建议。核心是动态改进奖励（DIR），基于对话智能体整合反馈的程度随时间演化。推理时改进而非丢弃不安全或过度拒绝的回复。

Result: 在五个不同数据集上的实验表明，WaltzRL显著减少了不安全回复（如WildJailbreak上从39.0%降至4.6%）和过度拒绝（OR-Bench上从45.3%降至9.9%），优于各种基线方法。

Conclusion: 通过让对话和反馈智能体共同进化并自适应应用反馈，WaltzRL在不降低通用能力的前提下增强了LLM安全性，从而推进了帮助性和无害性之间的帕累托前沿。
...
TL;DR: 该论文研究使用对比解码方法生成合成数据来增强语言模型训练，发现混合合成数据和真实数据能提高语言建模和下游任务性能，特别是对比解码的合成数据对推理任务更有帮助。


<details>
  <summary>Details</summary>
Motivation: 由于大语言模型训练数据的限制可能很快达到，需要探索使用LLM生成的合成数据作为解决方案。

Method: 在受控环境中，使用对比解码方法（好模型与差模型之间的相对差异）生成合成语料库，并将其与原始训练数据混合。

Result: 训练混合合成数据和真实数据提高了语言建模目标和一系列下游任务的性能，对比解码的合成数据特别有助于需要推理技能的任务。

Conclusion: 合成数据特别是通过对比解码生成的数据，可以有效增强语言模型训练，不同采样方法产生的合成数据对不同类型任务有不同益处。
...
TL;DR: DeepMiner是一个通过强化学习激发多轮智能体深度推理能力的新框架，采用高难度训练任务和动态上下文窗口，在多个搜索智能体基准测试中取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在具有长时程交互的多轮智能体中激发深度推理能力，需要解决训练数据挑战性和上下文管理效率问题。

Method: 提出逆向构建方法从真实网络源生成复杂但可验证的问答对，设计动态上下文管理策略使用滑动窗口机制，无需外部摘要模型，通过强化学习训练Qwen3-32B模型。

Result: DeepMiner-32B在BrowseComp-en上达到33.5%准确率，比之前最佳开源智能体提升近20个百分点，在多个基准测试中表现一致提升，动态上下文管理支持近100轮持续交互。

Conclusion: DeepMiner框架有效解决了多轮交互系统的上下文限制问题，显著提升了智能体的深度推理能力，为长时程交互场景提供了可行解决方案。
...
TL;DR: 该论文通过神经元级分析识别了驱动大语言模型文化行为的神经元，发现了文化通用神经元和文化特定神经元，它们集中在浅层到中层MLP层中，仅占所有神经元的不到1%。抑制这些神经元会显著降低文化基准测试性能，但对一般自然语言理解影响很小。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在全球部署，确保其公平和全面的文化理解很重要。但现有模型存在文化偏见和对弱势文化认知有限的问题，且其文化理解机制尚未充分探索。

Method: 采用神经元级分析，引入基于梯度的评分方法并进行过滤精炼，识别文化通用神经元和文化特定神经元。通过抑制这些神经元来验证其作用，并分析训练对文化理解的影响。

Result: 识别出的文化相关神经元集中在浅层到中层MLP层，仅占不到1%。抑制这些神经元使文化基准测试性能下降高达30%，但对一般NLU基准测试影响很小。文化特定神经元不仅支持目标文化知识，也支持相关文化知识。

Conclusion: 研究揭示了LLMs文化理解的内部机制，为模型训练和工程提供了实用指导。发现NLU基准训练可能削弱模型的文化理解能力，当更新包含大量文化通用神经元的模块时。
...
Method: 两阶段框架：1）角色引导的对抗指令生成；2）通过反思循环迭代优化低质量提示，并引入验证器评估有害性

Result: 构建了两个红队数据集，在8个SOTA LLM上评估，攻击成功率和泛化能力优于现有基线

Conclusion: 展示了基于种子的方法的局限性，证明了自由形式红队在LLM安全评估中的潜力
...
TL;DR: 提出两阶段投票架构用于自杀风险检测，第一阶段用轻量BERT处理明确案例，第二阶段用LLM投票或基于心理特征的ML集成处理模糊案例，平衡效率与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 全球自杀率上升，社交媒体提供重要信号，但检测隐式自杀意念（通过隐喻、讽刺等方式表达）极具挑战性，现有轻量模型无法处理隐式信号，而大语言模型成本过高。

Method: 两阶段架构：第一阶段BERT分类器处理高置信度明确案例；第二阶段对模糊输入采用多视角LLM投票框架或基于LLM提取心理特征的ML集成方法。

Result: 在两个数据集上表现优异：明确案例F1达98.0%，隐式案例F1达99.7%，跨域差距低于2%，同时显著降低LLM成本。
...
TL;DR: 本文研究了上下文学习(ICL)中表示和学习两个维度的独立性，发现标签表示质量决定基线准确率，而学习能力独立于表示质量存在。


<details>
  <summary>Details</summary>
Motivation: 过去研究主要关注ICL中演示表示的重要性，但对学习能力的研究结果不一致。本文旨在探究表示和学习这两个ICL关键维度之间的相互作用关系。
...
Conclusion: ICL中的表示和学习效应是相对独立的：表示决定基线性能，学习在此基础上提供增量改进。这一发现揭示了ICL中一个未被充分探索的方面。
...
TL;DR: 本文研究大型语言模型对条件语句可接受性的判断，发现模型对条件概率和语义相关性敏感，但不如人类一致，且模型大小与人类判断一致性无必然关联。
...
Result: LLMs对条件概率和语义相关性都敏感，但敏感程度因架构和提示风格而异；与人类数据比较显示LLMs使用这些线索不如人类一致；更大的模型不一定更接近人类判断。

Conclusion: LLMs在条件可接受性判断中整合了概率和语义线索，但缺乏人类的一致性，模型规模不是改善与人类对齐的关键因素。
...
TL;DR: Co⁴模型仅用单层、双头、8M参数和O(N)计算复杂度，在2个epoch内就超越了需要10个epoch的GPT-2(124M)和GPT-BERT(30M)，展现出极高的训练效率和样本效率。


<details>
  <summary>Details</summary>
Motivation: 挑战当前深度学习范式和相关缩放定律，探索更高效的预训练方法，减少模型复杂度和计算成本。

Method: 使用Co⁴架构，仅包含单层、两个注意力头和8M参数，采用近似O(N)的计算复杂度设计，在BabyLM挑战的10M token数据集上进行训练。

Result: 在BabyLM评估中，Co⁴在5/7的零样本指标和6/7的微调任务上超越GPT-2，在4/7的指标上超越GPT-BERT，训练效率提升数个数量级。

Conclusion: 结果表明需要重新思考主流深度学习范式，简单的模型架构结合高效算法可以超越复杂模型，挑战了现有的缩放定律。
...
TL;DR: ARES是一个自适应推理框架，通过动态分配探索努力来解决多模态大推理模型在简单问题上过度思考、在困难问题上探索不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLRMs在简单问题上产生过长推理痕迹，在困难问题上探索不足导致错过解决方案，需要平衡推理效率。

Method: 提出两阶段训练：自适应冷启动阶段根据问题难度调整推理痕迹长度；第二阶段使用HWE令牌作为探索触发器，结合分层熵奖励和动态KL控制来决定探索程度。

Result: 在多种数学、逻辑和多模态基准测试中实现卓越性能和推理效率，在显著降低推理成本的同时缩小与领先商业系统的差距。

Conclusion: ARES框架有效解决了推理效率不平衡问题，通过自适应推理策略提升了模型性能，同时降低了计算成本。
...
TL;DR: LEWIDI第三版共享任务扩展了基准数据集，涵盖四种NLP任务，引入序数标注和两种分歧感知评估范式（软标签和视角主义方法），并测试了新的评估指标。


<details>
  <summary>Details</summary>
Motivation: 训练AI模型认识到人类判断存在变异和分歧，并评估其识别这种变异的能力。

Method: 扩展LEWIDI基准到四个数据集（释义识别、讽刺检测、反讽检测、自然语言推理），引入序数标注，采用软标签和视角主义两种评估范式，测试新的评估指标。
...
TL;DR: DeepPrune是一个通过动态剪枝实现高效并行推理的框架，解决了并行扩展中计算冗余的问题，能在保持准确性的同时减少80%以上的计算开销。


<details>
  <summary>Details</summary>
Motivation: 并行扩展虽然能提升大语言模型的推理能力，但存在严重的计算冗余问题——超过80%的并行推理轨迹产生相同的最终答案，造成大量计算浪费。

Method: 提出DeepPrune框架，包含：1) 使用焦点损失和过采样技术训练的专业判断模型，从部分推理轨迹准确预测答案等价性（AUROC达0.87）；2) 在线贪心聚类算法，动态剪枝冗余路径同时保持答案多样性。

Result: 在三个挑战性基准测试（AIME 2024、AIME 2025和GPQA）上，DeepPrune相比传统共识采样实现了超过80%的token减少，同时保持准确性在3个百分点以内。
...
TL;DR: 论文探索了在LLMs中引入新词（neologism learning）来更好地理解和控制模型，通过添加新词嵌入来训练特定概念，无需改变其他模型参数。


<details>
  <summary>Details</summary>
Motivation: 受人类创造新词来满足新概念需求的启发，研究如何通过引入新词来增强对LLMs的理解和控制能力。

Method: 通过添加新词嵌入并训练展示该概念的示例，同时保持其他模型参数不变。使用自我言语化让模型描述新词含义，并通过插件评估验证言语化的有效性。

Result: 新词可以控制奉承、错误答案、文本长度等概念，发现机器特有同义词，并能联合学习多个概念。自我言语化能准确描述新词含义。

Conclusion: 新词学习是理解和控制LLMs的有效方法，能够揭示模型内部表示，并为模型控制提供新途径。
...
TL;DR: 本文提出了一种结合蒙特卡洛树搜索和代理提示评估器的提示优化框架，用于提高语言模型在服务条款公平性检测任务中的性能，同时降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法在服务条款公平性检测等法律NLP任务中存在计算成本高的问题，需要更高效的搜索策略和评估机制。

Method: 使用蒙特卡洛树搜索探索提示空间，并引入代理提示评估器来降低评估成本，实现更有效的提示优化。

Result: 实验表明，在有限计算预算下，该方法比基线方法获得了更高的分类准确率和效率。

Conclusion: 提出的框架能够有效平衡提示优化效果与计算成本，为法律NLP任务提供了一种实用的提示优化解决方案。
...
TL;DR: 提出RLKV框架，通过强化学习识别推理关键注意力头，实现KV缓存压缩，在保持推理性能的同时减少20-50%缓存开销。
...
Method: 使用强化学习优化每个头的缓存使用与推理质量关系，识别推理关键头，为关键头分配完整KV缓存，为其他头应用压缩的常量KV缓存。

Result: 实验表明只有少量注意力头对推理至关重要，该方法在20-50%缓存压缩下实现近乎无损的性能表现。

Conclusion: RLKV框架成功识别推理关键头，实现高效KV缓存压缩，为推理模型的部署提供实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的自进化方法依赖于外部奖励信号或LLM内部奖励，这与人类通过相互讨论和协作实现自我进化的机制不符。

Method: CoMAS通过智能体间丰富的讨论动态生成内在奖励，采用LLM作为评判机制来制定这些奖励，并通过强化学习优化每个智能体的策略，实现去中心化和可扩展的协同进化。

Result: 实验结果显示CoMAS持续优于未训练智能体，在大多数评估设置中达到最先进性能。消融研究证实了基于交互的奖励信号的必要性，并显示出随着智能体数量和多样性增加的良好可扩展性。

Conclusion: CoMAS为基于LLM的智能体自进化提供了一个新颖且有效的范式，通过模拟人类协作学习机制实现了更好的进化效果。
...
<details>
  <summary>Details</summary>
Motivation: 预训练数据泄露严重影响了大规模语言模型基准测试的有效性，模型可能通过记忆而非真正泛化来获得高分，扭曲了模型间的比较和进展评估。

Method: 使用LLM作为评判者，推断测试案例核心能力，生成保持原始目标的候选问答对，验证正确性和意图，通过多模型反馈选择暴露共同弱点的候选案例。

Result: 在数学问题解决、常识推理和安全领域应用显示，该框架能产生经过验证、多样化且公平的更新，揭示新的失败模式，提高难度同时保持测试目标对齐，改善模型可区分性。

Conclusion: 该框架为基准测试与基础模型快速发展保持同步提供了可扩展的路径，能够持续进化基准测试。
...
TL;DR: 基于RT-DETR的实时海上目标检测系统，通过合成图像增强和多尺度特征融合，解决了小目标检测和真实数据不足的问题，在保持实时性能的同时提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 海上目标检测面临小目标尺寸和真实RGB数据标签不足的挑战，需要开发能够在实际环境中实时运行的高效检测系统。

Method: 采用RT-DETR框架，结合多尺度特征融合、不确定性最小化查询选择和合成-真实数据权重平衡策略，使用数据增强技术平衡数据集类别分布。

Result: 开发了完整的Python鲁棒海上检测流水线，在保持实时性能的同时验证了各模块的贡献，系统能够处理极端光照和海况下的故障情况。
...
TL;DR: 提出了DynamicEval基准，专注于评估文本到视频模型在动态相机运动下的表现，包含4.5万个人工标注和新的背景一致性、前景一致性指标，相比现有基准在视频级和模型级评估上都有显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频评估基准存在两个主要问题：一是忽视动态相机运动的重要性，二是仅关注模型级评估而忽略视频级评估，这限制了评估的全面性。

Method: 构建包含动态相机运动提示词的基准，收集4.5万个人工标注；提出新的背景一致性指标，利用对象错误图修正遮挡/去遮挡问题；提出前景一致性指标，通过跟踪对象实例内的点来评估对象保真度。

Result: 实验表明，提出的指标在视频级和模型级评估中与人类偏好相关性更强，改进超过2个百分点，建立了更全面的动态相机运动评估基准。

Conclusion: DynamicEval通过关注动态相机运动和引入新的评估指标，为文本到视频模型提供了更全面、更准确的评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法如RED主要关注设计复杂的图像先验来提高重建质量，但收敛加速往往依赖启发式方法。RISP旨在弥补这一差距，在保持高质量重建的同时实现快速收敛。

Method: RISP在RED基础上引入了重启惯性机制，同时允许使用基于分数的图像先验。该方法还分析了相关的连续时间动力系统，揭示了与重球ODE的联系。

Result: 实验证明RISP在各种成像逆问题中都能实现快速收敛，同时获得高质量的重建结果。理论分析表明RISP比RED具有更快的驻点收敛速度。
...
TL;DR: 本文提出了一种基于图像纯化策略的创新去噪框架，解决超低剂量CT图像的空间错位问题，通过频率域流匹配模型在真实临床数据集上实现最先进的解剖结构保持效果。


<details>
  <summary>Details</summary>
Motivation: 超低剂量CT虽然显著减少辐射暴露，但会引入严重噪声和伪影，并导致与正常剂量CT图像对之间的空间错位，这给现有去噪网络在真实场景中的应用带来挑战。

Method: 首先构建真实临床uLDCT肺部数据集，然后提出图像纯化策略生成结构对齐的uLDCT-NDCT图像对，在此基础上提出频率域流匹配模型与IP策略协同工作。

Result: 实验证明IP策略显著提升多个主流去噪模型在uLDCT任务上的性能，提出的FFM模型结合IP策略在解剖结构保持方面达到最先进水平。
...
TL;DR: D2RA是一种无需训练的单图像攻击方法，能够在资源受限的对抗环境下有效移除或削弱多种水印方案，揭示了当前水印设计的基本弱点。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型使用的增加，确保内容归属和溯源的水印方法需求日益增长。尽管最近的语义水印方案通过在潜在或频率表示中嵌入信号来提高鲁棒性，但研究表明它们在资源受限的对抗环境下仍然脆弱。

Method: D2RA是一种无需训练、无需访问底层模型的单图像攻击方法。通过将水印图像投影到互补表示的自然先验上，抑制水印信号同时保持视觉保真度。

Result: 实验表明，D2RA方法能够一致地降低水印可检测性，在多种水印方案上都表现出有效性。

Conclusion: 该研究揭示了当前水印设计的基本弱点，表明即使是最新的语义水印方案在资源受限的对抗攻击下仍然脆弱。
...
TL;DR: PickStyle：基于扩散模型的视频风格转换框架，通过低秩适配器和上下文-风格分类器自由引导，实现内容保持和风格转换的视频生成。


<details>
  <summary>Details</summary>
Motivation: 解决视频风格转换中缺乏配对视频数据监督的挑战，利用现有的配对静态图像数据来训练视频风格转换模型。

Method: 在预训练视频扩散模型中插入低秩适配器到自注意力层，使用共享增强构建合成训练剪辑，并提出上下文-风格分类器自由引导（CS-CFG）方法。

Result: 在多个基准测试中实现了时间一致、风格忠实且内容保持的视频转换，在定性和定量评估上均优于现有基线方法。

Conclusion: PickStyle框架通过有效的适配器设计和引导策略，成功解决了视频风格转换中的数据稀缺问题，实现了高质量的视频风格迁移。
...
Method: 结合平衡训练数据集和轨迹感知注意力模块的TRAVL微调方法，以及包含300个视频的ImplausiBench基准测试。

Result: TRAVL提高了视频语言模型的运动编码和辨别能力，ImplausiBench提供了更严格的物理推理评估框架。

Conclusion: TRAVL和ImplausiBench为多模态模型的物理合理性评估和改进提供了统一框架，揭示了视觉-时间理解中具有挑战性的未探索方面。
...
TL;DR: 提出S3FN网络，通过结合文本语义描述来增强高光谱图像分类性能，利用LLM生成类别描述并嵌入到向量空间，实现更好的特征-标签对齐。


<details>
  <summary>Details</summary>
Motivation: 解决高光谱图像分类中训练样本有限、数据维度高导致的过拟合问题，以及现有模型仅依赖光谱-空间数据的局限性。

Method: 使用LLM生成每个类别的文本描述，通过预训练文本编码器（如BERT）将描述嵌入到向量空间，构建语义光谱-空间融合网络。

Result: 在Hyperspectral Wood、HyperspectralBlueberries和DeepHS-Fruit三个基准数据集上取得了显著的性能提升。

Conclusion: 文本语义与光谱-空间数据的协同作用为语义增强的高光谱图像分类模型开辟了新途径。
...
TL;DR: 提出CAGUL框架，通过跨模态注意力引导实现视觉语言模型的轻量级遗忘学习，防止敏感信息泄露


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在训练过程中可能记忆私有/敏感信息并在推理时泄露，需要有效的遗忘机制来解决隐私保护问题

Method: 利用跨模态注意力分析视觉token对输出的贡献，通过外部模块在低重要性视觉token中编码遗忘信息

Result: 实验表明该方法性能优于或与微调基线相当，不改变预训练模型参数且无需重新训练

Conclusion: CAGUL为视觉语言模型提供了一种实用有效的遗忘学习解决方案
...
TL;DR: MaizeStandCounting (MaSC) 是一个基于无人机RGB图像的自动化玉米幼苗计数算法，使用轻量级YOLOv9模型检测V2-V10生长阶段的玉米，并能区分杂草和其他植被，提供精确的行级株数统计。


<details>
  <summary>Details</summary>
Motivation: 手动计数玉米株数劳动密集、效率低且容易出错，特别是在大面积或变化较大的田地中。需要一种自动化、低成本且准确的解决方案来支持作物管理和研究。

Method: MaSC有两种工作模式：1) 将拼接图像分割成小块处理；2) 使用单应性矩阵对齐原始视频帧。两种模式都使用轻量级YOLOv9模型检测玉米幼苗，然后基于检测结果的空间分布进行行和范围分割。

Result: 与2024年夏季苗圃实地人工计数相比，MaSC与真实数据高度一致（拼接图像R²=0.616，原始帧R²=0.906）。算法处理83张全分辨率帧仅需60.63秒，包括推理和后处理，显示出实时操作潜力。
...
<details>
  <summary>Details</summary>
Motivation: CapsNet在检测重叠数字和仿射变换鲁棒性方面优于传统CNN，但训练和测试速度较慢，这限制了其在需要快速网络的实时应用中的使用。

Method: 通过产生更少数量的胶囊来构建更快的网络，并采用更强大的解码器来进一步提升性能。

Result: 在MNIST、F-MNIST、SVHN和Cifar-10数据集上，推理速度提升5倍，准确率仅有边际损失。
...
TL;DR: 本文提出了Rectified-CFG++，一种针对整流流模型的改进引导方法，解决了标准CFG在整流流模型中引起的离流形漂移问题。


<details>
  <summary>Details</summary>
Motivation: 标准CFG在整流流模型中会导致严重的离流形漂移，产生视觉伪影、文本对齐问题和脆弱行为，需要一种更稳定的引导方法。

Method: 采用自适应预测-校正引导策略，结合确定性整流流效率和几何感知条件规则。每个推理步骤先执行条件RF更新锚定样本，然后应用加权条件校正插值条件和无条件速度场。

Result: 实验证明该方法在MS-COCO、LAION-Aesthetic和T2I-CompBench等基准数据集上持续优于标准CFG，适用于Flux、Stable Diffusion 3/3.5、Lumina等大型文本到图像模型。

Conclusion: Rectified-CFG++确保了速度场的边际一致性和轨迹在数据流形有界管状邻域内的稳定性，在广泛引导强度范围内保持稳定性能。
...
Method: 构建PIT-QMM模型，能够端到端消费文本、图像和点云来预测质量分数。利用多模态信息的互补性进行无参考点云质量评估。

Result: 在流行基准测试中，所提方法以更少的训练迭代次数显著优于现有最先进方法。框架还支持失真定位和识别。

Conclusion: 该方法为模型可解释性和交互性开辟了新途径，在无参考点云质量评估方面表现出色。
...
TL;DR: 提出双流对齐网络(DSA Net)，通过融合帧级和动作级特征，结合量子-经典混合学习框架，在动作分割任务中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有动作分割方法多为单流方法，主要关注帧序列的时空建模。最近研究转向双流方法，学习动作级特征来提升分割性能。本文旨在探索引入第二个动作特征流对分割性能的影响。

Method: 提出Dual-Stream Alignment Network (DSA Net)，包含帧级和动作级双流。通过Temporal Context (TC)块使用交叉注意力和量子引导调制(Q-ActGM)融合信息。采用双流对齐损失，包含关系一致性、跨层对比和循环一致性重建损失。

Result: 在GTEA、Breakfast、50Salads和EgoProcel等多个基准数据集上评估，DSA Net实现了最先进的性能，显著优于现有方法。通过消融研究验证了各组件有效性。

Conclusion: 这是首个将量子-经典混合机器学习框架应用于动作分割的研究，双流特征对齐策略有效提升了动作分割性能。
...
TL;DR: 提出OIE方法，基于首帧服装替换的视频虚拟试穿策略，通过图像模型替换首帧服装，然后利用姿态和掩码信息引导视频生成模型合成剩余帧，实现参数和计算效率的显著提升。


<details>
  <summary>Details</summary>
Motivation: 解决当前基于Diffusion Transformer的双分支架构在视频虚拟试穿中面临的参数过多和服装特征缺乏时序特性的问题。

Method: 采用首帧服装替换策略：先用图像模型替换首帧服装，然后在编辑后的首帧内容控制下，利用姿态和掩码信息引导视频生成模型顺序合成剩余帧。

Result: 实验表明该方法在参数效率和计算效率方面表现优异，同时在约束条件下仍保持领先性能。

Conclusion: OIE方法通过创新的首帧替换策略，有效解决了视频虚拟试穿中的参数和时序问题，实现了高效且性能优越的解决方案。
...
TL;DR: 提出一种在扩散模型中实现个性化生成的两阶段方法，利用IP-Adapter自动生成的掩码来限制图像标记仅作用于主体区域，让文本提示能够更好地控制背景生成。


<details>
  <summary>Details</summary>
Motivation: 现有的个性化扩散模型往往过度关注主体图像而忽略文本提示，导致生成的图像只是简单复制主体而无法有效响应文本描述。

Method: 使用IP-Adapter自动生成的掩码在推理过程中分割主体与背景，然后在第二阶段将图像标记限制在主体区域，让文本提示能够控制背景内容的生成。

Result: 该方法在描述地点和位置的文本提示下，能够生成既准确描绘主体又完全匹配提示的图像，相比其他方法显示出更高的提示和源图像对齐度。

Conclusion: 通过两阶段的掩码策略，有效解决了个性化扩散模型中主体与文本提示之间的平衡问题，实现了更好的控制效果。
...
TL;DR: 比较了Transformer模型和视觉语言模型在自动文本框布局中的表现，发现Transformer模型在处理不完整布局时优于VLM方法，特别是在整合丰富外观信息时。
...
Result: 标准Transformer模型通常优于VLM方法，特别是在整合丰富外观信息时。但所有方法在处理非常小的文本或密集布局时都面临挑战。
...
TL;DR: 提出TCIP模型，通过特征增强残差模块(FERM)解决金字塔网络中解剖结构错位累积问题，并采用双阶段阈值控制迭代(TCI)策略自适应确定迭代次数，在多个医学影像数据集上实现优于SOTA的配准精度。


<details>
  <summary>Details</summary>
Motivation: 解决金字塔网络在可变形医学图像配准中存在的两个关键问题：解码器架构容易传播和累积解剖结构错位，以及现有模型无法自适应确定不同图像变形需求下的迭代次数。

Method: 1. 提出特征增强残差模块(FERM)作为金字塔网络各解码层的核心组件，包含三个顺序块：提取解剖语义特征、学习抑制无关特征、估计最终变形场；2. 提出双阶段阈值控制迭代(TCI)策略，第一阶段评估配准稳定性，第二阶段评估收敛性。

Result: 在三个公共脑部MRI数据集和一个腹部CT数据集上的广泛实验表明，TCIP在精度方面优于最先进的配准网络，同时保持相当的推理速度和紧凑的模型参数大小。

Conclusion: 通过将FERM和TCI集成到现有配准网络中验证了其泛化能力，消融研究进一步验证了这两种方法的有效性。
...
TL;DR: 提出了一种视频合成方法，通过变分推理近似组合分布，利用多个视频生成骨干网络共同处理所有任务约束，实现高可控性和多样性的视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型通常针对固定输入格式训练，无法灵活处理从精确4D对象轨迹到粗略文本提示等不同粒度的用户控制需求。

Method: 将任务建模为变分推理问题，通过逐步KL散度最小化处理退火分布序列，并提出上下文条件因子化技术减少解空间中的模式以避免局部最优。

Result: 实验表明，与先前工作相比，该方法生成的样本在可控性、多样性和3D一致性方面都有所提升。

Conclusion: 该方法能够为指定元素提供高可控性，同时为未指定元素保持多样性，解决了视频工作流中混合粒度控制的需求。
...
TL;DR: 该论文提出了一种结合BYOL和CNN的混合方法，用于通过热图像对感应电机进行故障检测分类，新提出的BYOL-IMNet模型在测试准确率和推理时间上均优于现有先进模型。
...
Method: 采用BYOL与CNN结合的混合方法，使用多种深度学习模型（ResNet-50、DenseNet-121等），并提出了新的轻量级CNN模型BYOL-IMNet，包含四个为热图像故障分类定制的模块。

Result: 提出的BYOL-IMNet模型达到了99.89%的测试准确率，每张图像的推理时间为5.7毫秒，优于现有最先进模型。
...
TL;DR: 提出MLH框架，通过弱到强的知识迁移，将成对哈希分支的局部相似性知识传递给中心哈希分支，提升全局结构建模能力


<details>
  <summary>Details</summary>
Motivation: 中心哈希方法在建模全局数据结构方面表现优异，但往往忽视了重要的局部相似性信息，限制了性能提升

Method: 采用双分支结构：强中心哈希分支和弱成对哈希分支，通过迭代式互学习机制和混合哈希专家模块实现跨分支知识迁移

Result: 在多个基准数据集上的实验表明，MLH持续优于最先进的哈希方法

Conclusion: MLH框架成功解决了中心哈希方法局部信息利用不足的问题，通过互学习机制实现了全局和局部信息的有效融合
...
TL;DR: 提出了Repainter强化学习框架，用于电商产品图像修复，通过空间遮罩轨迹优化和GRPO策略，有效去除水印和促销文本等干扰元素，提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 电商平台中产品图像的水印和促销文本等干扰元素严重影响用户体验和广告效果，现有扩散修复方法在商业场景中存在对象移除不可靠和领域适应性差的问题。

Method: 结合空间遮罩轨迹优化和Group Relative Policy Optimization (GRPO)，通过调节注意力机制强调背景上下文，生成高奖励样本，减少不需要的对象插入，并引入平衡全局、局部和语义约束的复合奖励机制。

Result: 在EcomPaint-100K数据集和EcomPaint-Bench基准测试中，Repainter显著优于现有最先进方法，特别是在复杂构图场景中表现优异。

Conclusion: Repainter框架有效解决了电商图像修复中的关键挑战，提供了可靠的商业应用解决方案，并贡献了高质量数据集和标准化评估基准。
...
TL;DR: SyncHuman是一个结合2D多视图生成模型和3D原生生成模型的框架，用于从单张图像重建高质量着装人体网格，即使在挑战性姿势下也能实现。
...
Method: 联合微调多视图生成模型和3D原生生成模型，使用像素对齐的2D-3D同步注意力；引入特征注入机制将2D多视图图像的精细细节提升到对齐的3D形状上。

Result: 在几何精度和视觉保真度方面优于基线方法，即使在挑战性姿势下也能实现稳健且逼真的3D人体重建。

Conclusion: SyncHuman展示了未来3D生成模型的有前景方向，通过结合2D和3D生成模型的互补优势实现高质量重建。
...
TL;DR: 提出ComGS框架，通过表面八面体探针(SOPs)实现可重光照的3D物体重建和场景光照估计，解决高斯泼溅中物体-场景组合的不一致问题，实现实时渲染和快速编辑。


<details>
  <summary>Details</summary>
Motivation: 高斯泼溅(GS)虽然能实现沉浸式渲染，但物体与场景组合时存在烘培外观和阴影信息不一致的问题，需要可重光照的物体重建和场景光照估计。

Method: 1. 使用表面八面体探针(SOPs)存储光照和遮挡信息，通过插值实现高效3D查询，避免昂贵的射线追踪；2. 简化场景光照估计，聚焦于物体放置位置的环境光照，利用扩散模型完成光照补全。

Result: 重建速度提升至少2倍，支持高斯场景中的实时阴影计算，渲染质量高，视觉和谐，具有生动的阴影效果，实时渲染约28 FPS，编辑仅需36秒。

Conclusion: ComGS框架成功解决了3D物体-场景组合的挑战，通过高效的SOPs和简化的光照估计方法，实现了高质量的实时渲染和快速编辑。
...
TL;DR: 本文提出UltraLED框架，仅使用单帧短曝光RAW图像进行超高清动态范围重建，通过曝光校正和亮度感知RAW降噪来解决夜间场景中的高动态范围问题。


<details>
  <summary>Details</summary>
Motivation: 解决超高清动态范围场景中亮暗区域曝光差异大的问题，避免传统多帧方法中的错位和重影伪影，探索单帧RAW图像重建的可能性。

Method: 提出两阶段框架：首先通过比率图进行曝光校正以平衡动态范围，然后使用亮度感知RAW降噪器增强暗区细节恢复。构建了9档包围曝光管道合成真实UHDR图像数据集。
...
TL;DR: 提出了一种结合低帧率RGB视频和事件相机数据来重建动态3D高斯泼溅的方法，通过事件运动先验指导变形场优化，解决大帧间运动带来的不确定性挑战。


<details>
  <summary>Details</summary>
Motivation: 低帧率RGB视频重建动态3DGS面临大帧间运动导致的解空间不确定性挑战，而事件相机能捕捉快速视觉变化且抗运动模糊，但缺乏颜色信息。结合两种模态可以互补优势。

Method: 使用事件运动先验指导变形场优化：1）通过LoCM无监督微调框架提取事件流中的运动先验；2）提出几何感知数据关联方法建立事件-高斯运动对应关系；3）采用运动分解和帧间伪标签策略。

Result: 在合成和真实场景的广泛实验中，该方法优于现有的基于图像和事件的方法，证明事件数据能有效优化动态3DGS。

Conclusion: 通过事件运动先验指导变形场优化，成功解决了低帧率RGB视频重建动态3DGS时的大帧间运动挑战，实现了两种模态的有效联合优化。
...
<details>
  <summary>Details</summary>
Motivation: 提高脑肿瘤MRI图像分割的准确性，帮助医生进行临床决策，并通过XAI技术增强医生对AI模型的信任度。

Method: 使用UNet、ResUNet和AttUNet三种深度学习模型在BraTS2020数据集上进行脑肿瘤分割，采用Adam优化器训练，使用Grad-CAM和注意力可视化技术分析模型决策过程。

Result: ResUNet在最终测试阶段表现最佳，在Dice、Jaccard相似性分数以及准确率、召回率和F1分数方面均优于其他模型。XAI技术成功提供了模型关注区域的视觉洞察。

Conclusion: 推荐ResUNet作为未来临床评估中自动脑肿瘤分割的最佳模型，其结合XAI技术可增强模型的可解释性和临床实用性。
...
TL;DR: 该论文提出了GTR-Bench，一个用于评估视觉语言模型在地理时空推理能力的新基准，重点关注移动目标在大规模摄像头网络中的推理，需要在地图和视频之间进行多视角切换和联合推理。


<details>
  <summary>Details</summary>
Motivation: 现有时空基准主要关注以自我为中心的图像/视频推理或基于图形的地理推理，缺乏同时使用图像/视频和图形上下文的地理时空智能评估，这对交通管理和应急响应等领域很重要。

Method: 构建GTR-Bench基准，要求模型在地图和多个非重叠视野视频之间进行多视角切换，对未观测到的时空区域进行推理，并评估10多个流行VLM模型的表现。

Result: 评估显示即使是表现最好的专有模型Gemini-2.5-Pro（34.9%）也显著落后于人类表现（78.61%），并揭示了当前模型在地理时空推理中的三个主要缺陷。

Conclusion: GTR-Bench为时空智能研究提供了有价值的见解和新机会，揭示了当前VLM在时空上下文利用、时间预测能力和地图-视频对齐理解方面的不足。
...
TL;DR: 本文提出了一种新的微表情识别方法，通过整合起始-顶点和顶点-偏移两个阶段的运动信息，开发了MM-COF运动表示和FMANet网络架构，在多个标准数据集上取得了优于现有方法的性能。
...
Method: 提出MM-COF运动表示，整合两个微表情阶段的运动动态；设计FMANet端到端神经网络，内部化双阶段分析和幅度调制为可学习模块。
...
TL;DR: 提出基于房间几何约束的深度估计框架，通过布局预测提取房间几何信息，并通过背景分割机制将这些信息整合到深度估计过程中，解决现有方法导致的角落过平滑和噪声敏感问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法专注于像素级精度，导致房间角落过平滑和对噪声敏感，需要更好的深度估计方法。

Method: 使用共享特征编码器提取多尺度特征，然后通过任务特定解码器进行布局估计、深度估计和背景分割。结合房间几何背景深度解析策略和背景分割引导融合机制。

Result: 在Stanford2D3D、Matterport3D和Structured3D数据集上的实验结果表明，该方法性能显著优于当前开源方法。

Conclusion: 提出的基于房间几何约束的深度估计框架能够有效提升单目360度室内全景图的深度预测精度。
...
TL;DR: ACAVP通过引入仿射变换和颜色变换增强视觉提示的表达能力，并利用TrivialAugment数据增强解决过拟合问题，在12个图像分类数据集上达到视觉提示方法的最优性能。
...
Method: 提出ACAVP方法，结合仿射变换、颜色变换和加法变换；引入TrivialAugment数据增强解决过拟合。

Result: 在12个数据集上达到视觉提示方法的最优准确率，超越线性探测平均准确率，对分布偏移具有更强鲁棒性。

Conclusion: ACAVP通过增强表达力和解决过拟合，显著提升了视觉提示方法的性能，同时保持推理时的低计算开销。
...
TL;DR: 提出了MMHOI数据集和MMHOI-Net模型，用于多人物多物体的3D交互理解，在多个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中多人多物体的复杂交互（因果性、目标导向、协作性）在现有3D人-物交互基准中未被充分覆盖，需要更全面的数据集和模型。
...
Conclusion: MMHOI数据集和MMHOI-Net模型为下一代HOI研究提供了全面测试平台，显著提升了多人多物体交互的理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在大型城市场景中会出现严重的走样伪影和优化不稳定性，特别是在高分辨率渲染下。现有方法虽然解决了可扩展性问题，但未能解决这种保真度差距。

Method: 提出两种协同正则化器：金字塔多尺度监督（通过预滤波图像金字塔强制渲染一致性）和显式尺寸正则化（对3D高斯的尺寸施加物理基础的下界约束）。

Result: 在MatrixCity、Mill-19和UrbanScene3D数据集上的实验表明，PrismGS实现了最先进的性能，相比CityGaussian获得了约1.5 dB的PSNR增益，在4K渲染下保持了优越的质量和鲁棒性。

Conclusion: PrismGS是一个即插即用的框架，能够显著改善3D高斯的固有渲染行为，有效缓解闪烁纹理和锯齿边缘问题，提升大规模城市场景的渲染质量。
...
Method: 结合I3D特征提取模块、专用特征转换网络和音频生成管道，使用新颖的NMS算法进行非语法连续序列中手语的时间检测。

Result: 在ASL-Citizen-1500和WLASL-100数据集上表现优异，Top-1准确率分别为72.01%和78.67%，音频质量指标显示可理解的语音输出。

Conclusion: 该框架为手语到语音的直接翻译提供了有效解决方案，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法通常将语义视为被动特征，在已形成的几何上添加语义信息。作者认为语义理解应该作为主动引导力量，用于鲁棒的稀疏视图重建。

Method: 提出AlignGS框架，通过深度一致性和多面法线正则化等语义到几何的引导机制，从2D基础模型中提取丰富先验知识，实现几何和语义的端到端协同优化。

Result: 在标准基准测试中实现了最先进的性能，在新视角合成方面表现优异，并产生具有卓越几何精度的重建结果。
...
TL;DR: 论文展示了自监督学习在毒性测试中的应用，利用EmbryoNet数据集证明该方法能有效区分不同化合物的作用模式。


<details>
  <summary>Details</summary>
Motivation: 高通量毒性测试需要自动化评估方法，自监督学习能够有效识别毒物引起的变化，为毒性测试系统提供关键组件。

Method: 使用自监督学习学习表征，并在公开的EmbryoNet数据集上进行概念验证，该数据集包含十种斑马鱼胚胎表型。

Result: 分析表明，通过自监督学习获得的表征能够有效区分不同化合物的作用模式。

Conclusion: 论文讨论了在TOXBOX项目中将机器学习模型集成到物理毒性测试设备中的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 现有前馈重建范式在驾驶场景重建中存在泛化能力有限和重建精度不足的问题，主要原因是固定视角变换无法适应不同相机配置，以及稀疏视图间重叠区域小、驾驶场景复杂导致学习困难。

Method: 设计了统一圆柱相机建模(UCCM)策略避免学习视角依赖的空间对应关系，使用可调参数统一不同相机配置；提出基于圆柱平面特征组(CPFG)的混合表示和专用模块，将2D图像特征提升到3D空间。

Result: 实验结果表明XYZCylinder在不同评估设置下实现了最先进的性能，并且能够以零样本方式泛化到其他驾驶场景。

Conclusion: XYZCylinder通过统一的圆柱体提升方法有效解决了驾驶场景重建中的泛化能力和精度问题，为不同相机配置的驾驶场景重建提供了有效解决方案。
...
TL;DR: MARC提出了一种基于记忆增强强化学习的token压缩方法，通过检索关键帧和强化学习蒸馏，在视频理解任务中实现95%视觉token压缩，同时保持接近基线的准确率。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在视频处理中面临的高计算成本问题，现有训练无关的token压缩方法会导致信息丢失和性能下降。

Method: 采用检索-压缩策略：使用视觉记忆检索器选择关键片段，通过压缩组相对策略优化框架将推理能力从教师模型蒸馏到学生模型。

Result: 在六个视频基准测试中，仅使用一帧token就达到接近基线准确率，视觉token减少95%，GPU内存减少72%，延迟降低23.9%。

Conclusion: MARC展示了在资源受限环境中实现高效实时视频理解的潜力，适用于视频问答、监控和自动驾驶等应用场景。
...
TL;DR: 提出了ASBench，第一个专门评估异常合成方法的综合基准框架，通过四个关键维度系统评估异常合成算法。


<details>
  <summary>Details</summary>
Motivation: 异常检测在制造业质量控制中很重要，但受限于异常样本稀少和人工标注成本高。现有研究将异常合成作为异常检测的辅助组件，缺乏对异常合成算法的系统评估。

Method: 开发ASBench基准框架，引入四个评估维度：跨数据集和管道的泛化性能、合成与真实数据比例、合成图像内在指标与异常检测性能指标的相关性、混合异常合成方法策略。

Result: 通过广泛实验，ASBench揭示了当前异常合成方法的局限性，并为未来异常合成研究提供了可行见解。

Conclusion: ASBench填补了异常合成领域缺乏系统评估的空白，为异常合成方法的发展提供了重要基准和指导。
...
TL;DR: 提出了TTOM框架，一种无需训练的推理时优化方法，通过时空布局对齐提升视频基础模型在组合场景下的文本-图像对齐能力。


<details>
  <summary>Details</summary>
Motivation: 视频基础模型在组合场景（如运动、数字和空间关系）中表现不佳，需要改进跨模态对齐能力。

Method: 采用推理时优化和记忆机制，通过布局注意力目标优化新参数，并在流式设置中维护历史优化上下文。

Result: 在T2V-CompBench和Vbench基准测试中表现出色，能够有效解耦组合性世界知识，具有强大的可迁移性和泛化能力。

Conclusion: TTOM是一个有效、实用、可扩展且高效的框架，能够实现组合视频生成的实时跨模态对齐。
...
TL;DR: CVD-STORM是一个跨视角视频扩散模型，通过空间-时间重建VAE生成多视角长视频，具备4D重建能力，在自动驾驶场景中实现高质量视频生成和深度估计。
...
Result: 实验显示模型在FID和FVD指标上显著提升，联合训练的高斯泼溅解码器有效重建动态场景，为场景理解提供几何信息。

Conclusion: 该方法成功实现了高质量多视角视频生成与4D场景重建，为自动驾驶场景理解提供了有价值的几何信息。
...
TL;DR: 提出了AnimeText数据集，针对动漫场景中的文本检测，包含73.5万张图像和420万个标注文本块，解决了现有数据集在动漫场景中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有文本检测数据集主要针对自然或文档场景，而动漫场景中的文本具有风格多样、排列不规则、易与复杂视觉元素混淆等特点，现有数据集无法很好适应。

Method: 构建大规模动漫文本数据集，包含分层标注和针对动漫场景的困难负样本，通过跨数据集基准测试验证效果。

Result: 实验表明，在AnimeText上训练的模型在动漫文本检测任务中优于使用现有数据集训练的模型。
...
TL;DR: 提出SimCast和CasCast框架，通过短长期知识蒸馏和加权MSE损失改进降水临近预报，结合扩散模型解决确定性预测的模糊和分布偏移问题。
...
Method: 提出SimCast训练管道，采用短到长期知识蒸馏技术和加权MSE损失；进一步集成到CasCast扩散框架中，结合概率模型优势。
...
TL;DR: Latent Harmony是一个两阶段UHD图像修复框架，通过改进VAE的潜在空间正则化和高频感知重建，在保持计算效率的同时提升高频细节保留能力。
...
Method: 两阶段框架：第一阶段提出LH-VAE，通过视觉语义约束和渐进退化扰动增强语义鲁棒性，潜在等变性强化高频重建；第二阶段联合训练改进的VAE与修复模型，使用高频低秩适应（HF-LoRA）模块，包括编码器LoRA（保真度导向）和解码器LoRA（感知导向），通过交替优化和选择性梯度传播训练。

Result: 在UHD和标准分辨率任务中达到最先进性能，有效平衡效率、感知质量和重建精度。

Conclusion: Latent Harmony通过重新定义VAE的潜在空间正则化和高频感知重建，成功解决了UHD图像修复中的效率-保真度权衡问题，提供了灵活的质量控制机制。
...
TL;DR: 本文探讨了对象标签和抽象标签在图像隐私分类中的效果差异，发现当标签预算有限时，抽象标签更有效；而当标签数量充足时，对象标签同样有用。


<details>
  <summary>Details</summary>
Motivation: 对象标签表示具体实体，适用于许多计算机视觉任务；抽象标签捕获更高层次信息，适用于需要上下文和主观场景理解的任务。本文旨在研究哪种标签类型更适合图像隐私这一上下文依赖且主观的任务。

Method: 通过比较对象标签和抽象标签在图像隐私分类中的表现，分析不同标签预算下的效果差异。
...
TL;DR: 本文比较了Transformer模型SwinIR与轻量级CNN在低光图像恢复任务中的性能与效率权衡。虽然SwinIR达到更高的PSNR(39.03dB)，但轻量级CNN以37.4dB的竞争性性能，仅需10个训练周期和55倍小的模型尺寸，展示了在实际应用中的显著效率优势。


<details>
  <summary>Details</summary>
Motivation: 解决低光图像恢复中高频细节恢复与严重噪声抑制的挑战，同时探索性能与计算效率之间的关键权衡，为实际应用提供可行的解决方案。

Method: 通过实验比较最先进的SwinIR Transformer模型与标准轻量级CNN在低光图像恢复任务上的表现，重点关注训练效率、模型大小和性能指标。

Result: SwinIR获得更高的峰值性能(PSNR 39.03dB)，但轻量级CNN以37.4dB的竞争性性能，仅需10个训练周期(相比SwinIR的132周期)且模型尺寸小55倍。

Conclusion: 标准CNN能够以显著降低的计算开销提供接近最先进水平的性能，在资源受限的实际应用场景中具有重要价值。
...
TL;DR: 提出GraphEnet，首个将图神经网络应用于事件相机数据进行人体姿态估计的工作，利用事件输出的稀疏性和基于线的中间表示，实现高频单人2D姿态估计。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有低延迟、低能耗优势，适合便携电子设备和移动机器人等资源受限应用，但现有基于RGB相机的方法无法充分利用这些优势。

Method: 使用图神经网络处理事件数据，采用基于线的中间事件表示，结合新颖的偏移向量学习范式和基于置信度的池化来估计人体姿态。

Result: 开发了首个将图神经网络应用于事件相机数据进行人体姿态估计的方法，能够实现高频姿态估计。

Conclusion: GraphEnet成功将图神经网络与事件相机数据结合，为资源受限环境下的实时人体姿态估计提供了新解决方案，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有组合图像检索方法作为"黑盒"的局限性，用户无法理解检索原理，模型也难以遵循复杂细粒度指令。

Method: 采用三阶段过程创建结构化思维链标注（描述、推理、结论），然后微调模型生成结构化输出，最后将检索意图编码到专用嵌入中。

Result: 在领域内数据集（FashionIQ、CIRR）上取得高度竞争力表现，在领域外CIRCO数据集上展现出显著泛化能力。

Conclusion: 为更有效和可信的检索系统开辟了新路径，通过显式推理链增强了模型的可解释性和准确性。
...
Conclusion: RayFusion方法通过射线融合有效提升了纯相机协作感知系统的性能，为解决深度估计模糊性问题提供了有效方案。
...
TL;DR: RASALoRE是一个用于脑部MRI扫描的弱监督异常检测框架，通过两阶段方法实现高效异常检测，参数量小于800万，在多个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决脑部MRI扫描中缺乏精确像素级标注时的异常检测问题，仅使用切片级弱标签进行快速准确的异常检测。

Method: 提出两阶段框架：第一阶段使用判别式双提示调优机制生成伪弱掩码；第二阶段使用区域感知空间注意力机制和基于位置的随机嵌入进行分割。

Result: 在BraTS20、BraTS21、BraTS23和MSD数据集上显著优于现有WSAD方法，计算复杂度大幅降低。

Conclusion: RASALoRE框架在弱监督异常检测中表现出色，实现了高性能与低计算复杂度的平衡。
...
TL;DR: 提出了一种多时序类别驱动的分层ResNet架构，用于在多个语义层次上对多光谱图像时间序列进行分类，通过引入分层惩罚映射来避免不连贯的层次转换。


<details>
  <summary>Details</summary>
Motivation: 为了解决多光谱图像时间序列在不同语义层次上的分类问题，并利用类别层次标签来有效训练网络，同时提高少数类别的表示能力。

Method: 修改ResNet架构，添加额外分支进行不同层次分类，使用分层惩罚映射约束分类一致性，采用分层训练策略让底层网络先学习宏观类别，上层网络学习微观类别。

Result: 在亚马逊森林两个区域的12个月Sentinel 2图像上实验，证明该方法在不同层次上具有良好的泛化能力，能在微观类别水平实现准确分类，并更好地表示少数类别。

Conclusion: 提出的模块化网络具有内在适应能力，通过分层方法有效提高了分类性能，特别是在新目标区域的微观类别分类和少数类别表示方面表现优异。
...
TL;DR: RedFace是一个专门的面部深度伪造数据集，包含6万多张伪造图片和1000个处理视频，使用9个商业在线平台生成，模拟真实世界黑盒场景，验证现有深度伪造检测方案在现实应用中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有学术评估和基准测试在检测深度伪造方面存在不足，缺乏特异性、深度伪造多样性有限、操作技术受限，无法有效应对现实世界威胁。

Method: 利用9个商业在线平台整合最新的深度伪造技术，使用定制算法合成深度伪造内容，模拟真实世界黑盒场景。

Result: 在RedFace数据集上的广泛实验（包括跨域、域内和真实社交网络传播模拟）验证了现有深度伪造检测方案在现实应用中的实用性有限。

Conclusion: RedFace数据集填补了学术评估与现实需求之间的差距，揭示了传统数据集对检测性能的影响原因，为改进深度伪造检测提供了重要基准。
...
TL;DR: 提出基于概率流守恒原理的AI生成视频检测方法NSG-VD，通过归一化时空梯度(NSG)特征量化视频动态异常，在检测性能上显著优于现有方法。
...
Method: 提出归一化时空梯度(NSG)统计量，量化空间概率梯度与时间密度变化的比例；利用预训练扩散模型开发NSG估计器，通过空间梯度近似和运动感知时间建模；基于NSG特征构建视频检测方法NSG-VD，使用最大均值差异(MMD)作为检测指标。

Result: 在广泛实验中，NSG-VD在召回率上优于最先进基线16.00%，在F1分数上优于10.75%，验证了其优越性能。

Conclusion: NSG-VD通过物理驱动的检测范式有效识别AI生成视频，为应对日益逼真的生成视频提供了可靠的检测解决方案。
...
TL;DR: DarkHash是首个针对深度哈希的无数据后门攻击方法，通过设计具有双语义引导的影子后门攻击框架，仅使用替代数据集微调受害者模型的特定层，在不访问训练数据的情况下嵌入后门功能并保持原始检索精度。


<details>
  <summary>Details</summary>
Motivation: 现有深度哈希后门攻击需要访问训练数据集，但在现实世界中由于隐私保护和知识产权问题往往无法获取训练数据。如何在无训练数据访问的情况下向深度哈希模型嵌入后门，同时保持原始任务的检索精度，是一个新颖且具有挑战性的问题。

Method: 提出DarkHash方法：1）设计具有双语义引导的影子后门攻击框架；2）仅使用替代数据集微调受害者模型的特定层；3）利用样本与其邻居的关系增强后门攻击；4）通过拓扑对齐损失优化单个和相邻中毒样本朝向目标样本。

Result: 在四个图像数据集、五种模型架构和两种哈希方法上的实验结果表明，DarkHash具有高度有效性，优于现有最先进的后门攻击方法。防御实验显示DarkHash能够抵御现有的主流后门防御方法。

Conclusion: DarkHash成功实现了首个针对深度哈希的无数据后门攻击，在保持原始检索精度的同时有效嵌入后门功能，且能够抵御现有防御方法，为深度哈希模型的安全性研究提供了重要参考。
...
TL;DR: 提出基于3D高斯泼溅的标签精炼流程，从多视角噪声预测中生成准确分割掩码，显著提升极端视角下的人脸解析精度
...
Result: 在挑战性头部姿态上显著提升解析精度，同时在标准视角保持强性能，无需3D真值标注

Conclusion: 该方法为现实场景中提升人脸解析鲁棒性提供了可扩展且有效的解决方案
...
TL;DR: 本文提出了一种针对CT图像的特定增强技术——随机窗宽调整，以解决传统图像增强方法在CT图像中可能导致伪影和泛化能力差的问题。该方法利用CT图像中的Hounsfield单位分布，显著提高了在对比度差或时间不准确的挑战性图像上的模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统为自然图像设计的增强方法直接应用于CT图像时，忽视了CT图像中Hounsfield单位的物理意义，可能导致伪影和泛化能力差。需要开发专门针对CT成像特性的增强技术。

Method: 提出随机窗宽调整方法，利用CT图像中可用的Hounsfield单位强度分布，鼓励对对比度增强的鲁棒性。该方法专门针对CT成像特性设计。

Result: 在多个数据集上进行消融实验和分析，与最先进替代方法相比，该方法在肝肿瘤分割任务中表现更优，特别是在对比度差或时间不准确的挑战性图像上显著提高了模型性能。

Conclusion: 随机窗宽调整是一种有效的CT特定增强技术，能够解决传统强度增强方法在CT图像中的局限性，提高分割模型的泛化能力和性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决实时运动可控视频生成的挑战：双向扩散模型的固有延迟问题，以及现有自回归方法在少步生成中存在的质量下降和运动伪影问题。

Method: 1. 微调基础I2V模型以支持基本运动控制；2. 通过基于轨迹的奖励模型进行强化学习改进；3. 通过自滚动机制保持马尔可夫性质；4. 在去噪步骤中选择性引入随机性以加速训练。

Result: AR-Drag实现了高视觉保真度和精确的运动对齐，相比最先进的运动可控VDM显著降低延迟，仅使用13亿参数。

Conclusion: AR-Drag在实时运动可控视频生成方面取得了显著进展，为高质量、低延迟的视频生成提供了有效解决方案。
...
TL;DR: 本文针对视频语言模型(Video-LLMs)中存在的逻辑不一致性问题，提出了一种时间条件注意力锐化(TCAS)方法，通过增强跨模态注意力头的时间分辨能力来改善模型的时间理解逻辑一致性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常生成自相矛盾的输出，严重影响其可靠性。在视频语言模型中，这种现象表现为模型无法对基于其基础输出的重新表述问题提供逻辑一致的响应，但这种现象的根本原因尚未得到充分探索。

Method: 采用可解释性驱动的方法分析、统计总结和干预该现象的潜在因素。提出时间条件注意力锐化(TCAS)方法，构建基于注意力差异的增强目标来增强模型的时间分辨能力。

Result: 实验结果表明，该方法显著增强了视频语言模型的时间逻辑一致性。可解释性分析证实该方法确实提高了注意力头的时间辨别能力。此外，在通用视频时间定位任务中实现了性能提升。
...
TL;DR: UniMMVSR是首个统一的多模态视频超分辨率框架，能够整合文本、图像和视频等多种生成条件，显著提升视频细节质量和条件一致性。


<details>
  <summary>Details</summary>
Motivation: 现有级联视频超分辨率方法主要局限于文本到视频任务，无法充分利用多模态生成条件，这限制了在确保多模态视频生成保真度方面的能力。
...
Conclusion: 该框架成功解决了多模态条件利用的挑战，为高质量视频生成提供了新的解决方案，突破了现有技术的限制。
...
TL;DR: 提出MURE框架，通过多模态链式思维实现图像编辑，结合文本和视觉推理路径，解决复杂对象关系和空间布局问题


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑方法在处理复杂对象交叉和细粒度空间关系时缺乏显式推理过程，纯文本或坐标增强的CoT方法无法充分表示视觉布局和像素级细节

Method: MURE框架采用多模态链式思维，交替生成文本描述和视觉提示（如位置掩码）；引入MMDC推理范式，通过奖励模型剪枝低质量推理路径

Result: 在三个图像编辑基准测试中取得显著改进，发布首个CoT-Edit-14K数据集包含14K高质量编辑示例

Conclusion: 多模态链式思维能够有效分解复杂编辑任务，提高各阶段精度，生成高保真编辑结果
...
TL;DR: 提出一种自举算法，通过迭代重新对齐训练样本来处理细粒度视觉分类中的几何偏差问题，在多个基准测试中表现优于等变和规范化基线方法。


<details>
  <summary>Details</summary>
Motivation: 细粒度视觉分类任务需要处理几何偏差和噪声，现有方法依赖数据增强或等变架构，但前者需要强大模型，后者限制表达能力。规范化方法需要对齐的训练数据假设，但现实数据集无法满足这一假设。

Method: 提出自举算法，通过迭代重新对齐训练样本，逐步减少方差并恢复对齐假设，支持任意紧致群，并建立了收敛保证。

Result: 在四个细粒度视觉分类基准测试中，该方法持续优于等变和规范化基线方法，与数据增强方法性能相当。

Conclusion: 该自举算法能够有效处理细粒度视觉分类中的几何偏差问题，在现实数据集上表现优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本驱动方法难以精确定位对象，而对象拖拽方法仅限于静态重定位，无法进行语义编辑。需要一种能够同时实现精确对象移动和语义控制的图像编辑方法。

Method: 将对象拖拽视为图像重建过程，分为两个协同分支：移动重建分支使用基于能量的梯度引导来精确移动对象，文本驱动编辑分支共享梯度信号以确保一致性变换。采用DDPM反演和先验信息注入来保持对象结构。

Result: 大量实验表明，InstructUDrag实现了灵活、高保真的图像编辑，在对象重定位精度和图像内容语义控制方面都表现出色。

Conclusion: InstructUDrag成功结合了文本指令和对象拖拽的优势，为图像编辑提供了更精确和灵活的控制能力，是扩散模型在图像编辑领域的重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 人类交互本质上是动态和层次化的，现有方法大多时间不变地建模人类交互，忽略了距离和层次结构。
...
Conclusion: FineDual方法能够生成细粒度、高质量的双人运动，成功解决了动态层次化人类交互建模的挑战。
...
TL;DR: 提出AdaGC框架，用于遥感图像的单正多标签学习，通过自适应梯度校准解决标注不完整问题，在多个基准数据集上达到SOTA性能。
...
Method: AdaGC框架包含梯度校准机制、Mixup和数据增强、双EMA模块生成鲁棒伪标签，以及基于训练动态的自适应触发机制。

Result: 在两个遥感基准数据集和两种标签噪声类型下，AdaGC均达到最先进性能，并在不同设置下保持强鲁棒性。

Conclusion: AdaGC为遥感图像单正多标签学习提供了有效解决方案，通过自适应梯度校准成功缓解了过拟合标签噪声的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时解决文本引导图像修复中的两个核心问题：保持未掩码区域和实现语义一致性。作者发现这是由于不同频率带编码了不同图像属性，在去噪过程中对文本提示具有不同的鲁棒性。

Method: 提出NTN-Diff方法，将去噪过程分为早期（高级噪声）和晚期（低级噪声）阶段，在去噪过程中解缠中低频带。使用稳定的中频带作为指导，通过零文本去噪过程处理掩码区域的低频带，最终实现跨区域的语义一致性。

Result: 大量实验验证了NTN-Diff在文本引导扩散模型中的优越性，超越了现有最先进的方法。

Conclusion: NTN-Diff通过频率感知的扩散模型成功解决了文本引导图像修复中的两个关键挑战，实现了更好的语义一致性和未掩码区域保护。
...
TL;DR: 提出了一种新的ERU框架，通过联合利用基于LLM的数据增强、深度图模态和深度感知决策模块，改进复杂或杂乱环境中的指代消歧。


<details>
  <summary>Details</summary>
Motivation: 解决现有开放词汇对象检测方法在场景中存在多个候选对象的模糊情况下容易失败的问题。

Method: 联合利用LLM数据增强、深度图模态和深度感知决策模块，实现语言和具身线索的鲁棒集成。

Result: 在两个数据集上的实验结果表明，该方法显著优于现有基线，实现了更准确可靠的指代检测。

Conclusion: 所提出的ERU框架能够有效处理模糊场景中的指代理解问题，提高复杂环境下的目标识别性能。
...
TL;DR: 提出Neural Exposure Fields (NExF)，通过3D空间中的曝光场预测来提升高动态范围场景下的神经场景重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有神经场景表示在处理包含强烈曝光变化的数据时性能下降，特别是在室内外混合场景或带窗户的房间中。

Method: 学习预测每个3D点的最优曝光值的神经场，通过神经条件机制联合优化场景表示和曝光场。

Result: 在多个基准测试中达到最先进性能，比最佳基线提升超过55%，训练速度更快。

Conclusion: NExF能够准确重建高动态范围场景，无需后处理或多曝光捕捉，实现了3D一致的视觉效果。
...
TL;DR: 提出了一种有效的长程时序上下文注意力机制（LTCA），通过稀疏局部注意力堆叠和全局查询设计，在保持计算效率的同时平衡局部性和全局性，在四个指代视频分割基准上达到了新的最优性能。


<details>
  <summary>Details</summary>
Motivation: 解决指代视频分割中长程时序上下文信息提取的挑战，现有方法在局部性和全局性之间平衡不佳，且计算复杂度随视频长度显著增加。

Method: 提出LTCA机制：1）堆叠稀疏局部注意力（扩张窗口注意力）平衡局部性和全局性；2）每个查询从全局池中随机选择少量键进行注意力；3）设计全局查询与所有其他查询交互直接编码全局上下文信息。

Result: 在四个指代视频分割基准上达到新的最优性能，在MeViS数据集上分别提升了11.3%和8.3%。

Conclusion: LTCA机制有效解决了长程时序上下文建模问题，在保持计算效率的同时显著提升了指代视频分割性能。
...
TL;DR: 提出了一种基于语义基础学习范式的3D功能分割方法，通过跨模态亲和力转移(CMAT)将2D视觉基础模型的丰富语义知识迁移到3D领域，并设计了跨模态功能分割转换器(CAST)来生成精确的提示感知分割图。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖点云编码器作为通用特征提取器，忽略了3D数据的稀疏性、噪声和几何模糊性等固有挑战，导致学习到的3D特征缺乏清晰且语义一致的功能边界。

Method: 1. 跨模态亲和力转移(CMAT)：预训练策略，将3D编码器与提升的2D语义对齐，联合优化重建、亲和力和多样性；2. 跨模态功能分割转换器(CAST)：集成多模态提示与CMAT预训练特征，生成精确的提示感知分割图。

Result: 在标准基准测试上的广泛实验表明，该框架为3D功能分割建立了新的最先进结果。

Conclusion: 提出的语义基础学习范式通过有效迁移2D语义知识到3D领域，解决了3D功能分割中的语义一致性挑战，显著提升了分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散模型的计算成本随序列长度呈二次方增长，而线性注意力虽然能降低成本，但完全替换二次注意力需要昂贵的预训练。

Method: 提出选择性转移方法自动渐进地将层转换为线性注意力，并引入任意时间分布匹配目标来对齐采样轨迹中的分布。

Result: 方法实现了1.25-2.00倍加速，4步蒸馏模型进一步实现15.92倍延迟减少，且视觉质量下降最小。

Conclusion: LinVideo框架有效解决了视频扩散模型的计算效率问题，在保持生成质量的同时显著提升速度。
...
TL;DR: 提出了首个专注于交通场景感知的视觉问答基准DTPQA，包含距离标注，评估小规模视觉语言模型在自动驾驶感知任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要可靠的感知能力，特别是对远距离物体的识别。现有视觉语言模型在安全关键应用中缺乏专门的感知评估基准。

Method: 创建DTPQA基准，仅包含感知类问题（排除推理问题），评估多个最先进的小型视觉语言模型，并与人类表现对比。

Result: 最佳小型VLM平均准确率约60%，远低于人类约85%的表现。模型在左右区分等基础感知任务上表现较差。

Conclusion: 当前小型视觉语言模型在交通场景感知任务上仍有明显不足，需要进一步改进以适用于自动驾驶等安全关键应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决低光照图像和雾霾图像（包括雾天、沙尘、水下图像）的增强和清晰化问题，现有方法在极端暗光条件下效果有限。

Method: 构建图像滤波器模拟低光或雾霾条件，然后推导近似反向滤波器来最小化增强图像的失真，方法简单仅需几行MATLAB代码即可实现。

Result: 实验结果表明该方法在处理极端暗图像和增强雾霾图像方面具有高度竞争力，通常超越现有最先进技术。

Conclusion: 该方法提供了一种简单高效的图像增强解决方案，在保持高质量增强效果的同时具有极高的实现简便性。
...
Method: 使用引导扩散模型进行数据增强，采用轻量级Transformer网络捕获数据复杂模式，引入改进的加权损失函数和优化余弦方差调度器。

Result: 在PRISMA卫星获取的10种森林类型高光谱图像分类任务中，该方法在平均准确率和加权平均准确率上均优于其他数据增强技术，且训练过程稳定。

Conclusion: 该方法有效解决了深度生成模型在小数据集数据增强中的实际应用限制，为高光谱图像小样本分类提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型主要局限于图像领域，需要扩展到视频领域以支持复杂的多模态指令理解和视觉一致性保持。

Method: 采用双流架构：MLLM用于指令理解，MMDiT用于视频生成。统一多种视频生成和编辑任务，在单一多模态指令范式下联合训练。
...
TL;DR: 提出了一种基于课程学习的源自由域自适应框架LFC，通过从简单到困难、从源到目标的课程设计，在无需源数据的情况下实现医学图像的跨域分割。


<details>
  <summary>Details</summary>
Motivation: 解决医学图像数据隐私和安全问题，现有源自由域自适应方法主要关注伪标签细化，忽略了学习过程的渐进性，而渐进学习过程有助于知识迁移。

Method: 提出LFC框架，包含两个课程：1) 从简单到困难的课程，从简单样本开始学习并逐步增加难度；2) 从源到目标的课程，确保模型从源域到目标域的平滑迁移。

Result: 在眼底分割和息肉分割的跨域数据集上评估，实验结果表明该方法超越了现有方法，达到了新的最先进水平。

Conclusion: LFC框架通过课程学习策略有效解决了源自由域自适应问题，在医学图像分割任务中表现出色，为数据隐私保护提供了可行方案。
...
TL;DR: VideoVerse是一个新的文本到视频生成基准测试，专注于评估模型对真实世界中复杂时间因果关系和世界知识的理解能力，旨在解决现有基准测试的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法区分最先进的文本到视频模型，缺乏对事件级时间因果关系和世界知识的系统评估，而这些是构建世界模型的关键能力。

Method: 收集跨领域代表性视频，提取具有时间因果关系的事件级描述，重写为文本到视频提示，设计包含10个维度的二元评估问题，使用现代视觉语言模型开发人类偏好对齐的QA评估流程。

Result: VideoVerse包含300个精心策划的提示，涉及815个事件和793个二元评估问题，对开源和闭源T2V模型进行了系统评估。

Conclusion: 该基准测试提供了对当前T2V生成器与世界模型差距的深入分析，填补了现有评估体系的空白。
...
TL;DR: 首次将连续时间一致性蒸馏扩展到通用应用级图像和视频扩散模型，提出得分正则化连续时间一致性模型(rCM)，在14B参数模型和5秒视频上实现1-4步高质量生成，加速扩散采样15-50倍。


<details>
  <summary>Details</summary>
Motivation: 虽然连续时间一致性模型(sCM)在学术规模扩散加速中表现优异，但在大规模文本到图像和视频任务中的适用性仍不明确，存在基础设施挑战和质量限制。

Method: 开发并行兼容的FlashAttention-2 JVP核，提出rCM模型，通过得分蒸馏作为长跳跃正则化器，结合前向散度的模式覆盖和反向散度的模式寻求特性。

Result: 在14B参数模型和5秒视频任务中，rCM在质量指标上匹配或超越最先进的DMD2蒸馏方法，同时在多样性方面具有显著优势，无需GAN调优或大量超参数搜索。
...
Method: 引入基于回报差异的自监督信号，通过回报引导的对比学习训练注意力机制，将相似视觉表征根据回报差异分组为正负样本，构建对比三元组来学习区分不同结果状态的特征。

Result: 在ManiSkill3基准测试的多个操作任务中，样本效率提升最高达2.4倍，能够解决基线方法无法学习的任务，且无需修改底层算法或超参数。

Conclusion: 受人类视觉中央凹启发的注意力机制结合回报引导的对比学习，能够有效识别任务相关特征，显著提升视觉强化学习的性能和稳定性。
...
TL;DR: 提出了一个模块化空间图像处理框架，包含灰度量化、颜色亮度增强、图像锐化、双向变换管道和几何特征提取等功能，在多个数据集上表现出稳健性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个综合性的图像处理框架，能够处理从基础量化到高级特征提取的多种任务，为实时图像分析和计算机视觉应用提供可靠工具。

Method: 采用模块化设计：灰度量化（8级离散化）、颜色增强（RGB和YCrCb空间的直方图均衡）、亮度调整（HSV值通道操作）、图像锐化（3×3卷积核）、双向变换管道（非锐化掩模、伽马校正、噪声放大）以及几何特征提取（Canny边缘检测、Hough线估计、Harris角点检测、形态学窗口定位）。

Result: 双向变换管道前向和反向过程准确率分别为76.10%和74.80%；Hough线估计成功检测到51.50°的台球杆对齐角度；球杆隔离与真实图像的相似度达到81.87%。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉-文本推理方面有潜力，但依赖文本先验限制了在开放词汇场景中区分语义相似动作的能力

Method: 将动作分解为可区分子动作进行细粒度匹配，同时动态调用领域特定工具进行跨模态交织，通过分层奖励平衡工具使用效率、子动作相关性和推理结构连贯性

Result: 在HMDB-51、UCF-101、SSv2、Kinetics-400和Kinetics-600数据集上表现优异，超越现有方法，在区分细粒度动作和处理跨模态幻觉方面表现出色

Conclusion: 该方法实现了从文本中心推理到视觉基础推理的转变，展示了优异的鲁棒性和泛化能力
...
TL;DR: 提出了视觉象似性挑战基准，评估视觉语言模型在手语象似性任务中的表现，发现模型在音系形式预测、透明度推断和象似性评分方面均低于人类基线水平。
...
Method: 引入视觉象似性挑战基准，基于视频评估VLMs在三个任务上的表现：音系手语形式预测、透明度推断和分级象似性评分，在零样本和少样本设置下测试13个最先进模型。

Result: 模型在音系形式预测中能恢复部分手形和位置细节但低于人类表现；在透明度任务中远低于人类基线；只有顶级模型与人类象似性评分呈中等相关。有趣的是，音系形式预测能力强的模型与人类象似性判断相关性更好。
...
TL;DR: InstructX是一个统一的图像和视频编辑框架，通过深入研究多模态大语言模型与扩散模型的集成，实现了无需显式监督的视频编辑能力，并在单一模型中统一处理图像和视频编辑任务。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型在视觉理解和推理方面取得进展，研究者希望利用它们提升扩散模型的编辑性能。但目前大多数研究缺乏对MLLM设计选择的深入分析，且MLLM与扩散模型在视频编辑等困难任务中的集成仍面临挑战。

Method: 提出了InstructX统一框架，通过全面研究MLLM与扩散模型在指令驱动编辑中的集成方式。分析了图像和视频在统一建模中的协作与区别，发现仅使用图像数据训练就能获得视频编辑能力，并通过整合模态特定的MLLM特征来统一图像和视频编辑任务。

Result: 实验表明，该方法能够处理广泛的图像和视频编辑任务，并取得了最先进的性能表现。训练图像数据能够产生无需显式监督的视频编辑能力，缓解了视频训练数据稀缺的限制。

Conclusion: InstructX框架成功统一了图像和视频编辑任务，通过模态特定的MLLM特征实现了高效的跨模态编辑能力，为解决视频编辑中的数据稀缺问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界视频常因采集和传输条件不同而遭受复杂退化（如噪声、压缩伪影、低光失真），现有方法需要手动选择专用模型或采用单一架构，难以泛化处理各种退化类型。

Method: 构建大规模高分辨率视频退化识别基准，使用视觉语言模型进行退化识别；引入基于大语言模型的自适应路由器学习修复策略；构建Res-VQ数据集并设计专门的VQA模型进行质量评估。
...
TL;DR: 本文发现ViT中存在高范数视觉标记（ViT注意力汇点），这些标记包含图像的高级语义概念，对LVLM的理解和推理至关重要。通过利用这些标记，可以显著提升视觉推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLM中的注意力汇点，而忽略了ViT中的高范数视觉标记对LVLM理解和推理的重要贡献。

Method: 提出定性和定量分析ViT汇点标记中的信息，并开发训练无关和基于训练的方法来更好地利用这些标记。

Result: 通过显式利用ViT注意力汇点标记，在多种LVLM和视觉推理任务上实现了显著改进。

Conclusion: ViT注意力汇点在增强视觉推理方面具有未开发的潜力，应被更充分地利用。
...
TL;DR: 提出基于语义场景图的深度压缩框架，将点云分解为语义一致的补丁，使用FiLM条件编码器压缩为紧凑潜在表示，通过折叠解码器实现结构精确重建，在SemanticKITTI和nuScenes数据集上实现98%压缩率。


<details>
  <summary>Details</summary>
Motivation: 多智能体机器人系统中3D点云数据传输面临带宽限制和间歇性连接问题，影响系统性能，需要高效的压缩方法。

Method: 使用语义场景图分解点云为语义补丁，采用FiLM条件编码器生成紧凑潜在表示，通过折叠解码器进行重建。

Result: 在SemanticKITTI和nuScenes数据集上实现98%数据压缩率，保持结构和语义保真度，支持多机器人位姿图优化和地图融合等下游应用。

Conclusion: 该框架在保持高压缩率的同时确保结构和语义完整性，为带宽受限环境下的多智能体系统提供有效解决方案。
...
TL;DR: 本文提出了一个理论框架，解释为什么在预训练模型中对随机选择的小子网络进行微调足以实现下游任务适应。研究发现预训练网络具有通用获胜切片特性，并提出SliceFine方法，该方法仅更新原始权重的选定切片，不引入新参数。
...
Method: 基于谱平衡和任务能量两个现象，提出通用获胜切片假设。开发SliceFine方法，仅更新选定权重切片，不添加新参数。

Result: SliceFine在语言和视觉任务上达到最先进PEFT方法的性能，同时显著提高训练速度、内存效率和模型紧凑性。

Conclusion: 该工作连接了理论与实践，为现有PEFT技术提供了理论支撑的替代方案，证明了预训练模型中固有的冗余性可以被有效利用。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图像到视频生成方法在轨迹控制方面存在局限性，需要更灵活、多粒度的轨迹控制能力，同时解决训练收敛慢、可控性弱和推理效率低的问题。

Method: 引入统一的点运动表示（分割ID、轨迹ID、颜色通道），采用序列拼接方案替代传统条件注入方法，使用退火训练策略逐步减少对完整监督和对齐条件的依赖。

Result: 实验表明FlexTraj支持多粒度、对齐无关的轨迹控制，适用于运动克隆、拖拽式图像到视频、运动插值、相机重定向、灵活动作控制和网格动画等多种应用。

Conclusion: FlexTraj框架在图像到视频生成中实现了高效、灵活的轨迹控制，在收敛速度、可控性和推理效率方面均有显著提升，为视频生成提供了新的控制维度。
...
TL;DR: 本文提出了一种渐进式训练方法SpatialLadder，通过三阶段训练框架从空间感知到复杂推理逐步构建空间智能，在空间推理基准上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在空间推理方面存在局限性，主要是因为缺乏从感知到理解的层次化基础。作者认为需要建立渐进式的空间智能构建方法。

Method: 构建了SpatialLadder-26k多模态数据集，包含26,610个样本；设计了三级渐进训练框架：1) 通过物体定位建立空间感知，2) 通过多维空间任务发展空间理解，3) 通过可验证奖励的强化学习加强复杂推理。

Result: SpatialLadder模型（3B参数）在空间推理基准上平均提升23.4%，超过GPT-4o 20.8%和Gemini-2.0-Flash 10.1%，在领域外基准上也有7.2%的改进。
...
Method: 扩展现有最先进的图像编辑模型，添加标量编辑强度输入。训练轻量级投影网络，将标量和编辑指令映射到模型的调制空间系数。使用生成模型合成图像-编辑-指令-强度四元组数据集进行训练。

Result: 模型实现了对编辑强度的连续精细控制，支持从细微到强烈的各种编辑操作，包括风格化、属性、材质、背景和形状变化。

Conclusion: Kontinuous Kontext提供了一种统一的指令驱动编辑方法，无需特定属性训练即可实现编辑强度的连续控制，显著提升了用户体验。
...
TL;DR: 该论文提出了MM-HELIX基准测试来评估多模态大语言模型的长链反思推理能力，发现现有模型存在显著缺陷，进而开发了AHPO训练策略和MM-HELIX-100K数据集，在Qwen2.5-VL-7B模型上实现了18.6%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在数学和逻辑推理任务中表现出色，但其长链反思推理能力（解决复杂现实问题的关键）尚未得到充分探索。

Method: 1) 构建MM-HELIX基准测试（1,260个样本，42个挑战性任务）；2) 开发Step-Elicited Response Generation流水线创建MM-HELIX-100K数据集；3) 提出自适应混合策略优化(AHPO)训练方法，动态结合离线监督和在线优化。

Result: 在Qwen2.5-VL-7B模型上，该方法在MM-HELIX基准测试中实现了+18.6%的准确率提升，在通用数学和逻辑任务上平均性能提升+5.7%。
...
TL;DR: VideoNorms是一个包含1000多个视频片段-规范对的文化基准数据集，用于评估视频大语言模型在不同文化背景下的理解能力，重点关注中美文化的社交规范识别。


<details>
  <summary>Details</summary>
Motivation: 随着视频大语言模型在全球部署，需要评估其对不同文化背景的理解和适应能力，但目前缺乏合适的文化意识评估基准。

Method: 采用人机协作框架构建数据集：使用基于言语行为理论的教师模型生成候选标注，再由训练有素的人类专家验证和修正标注。

Result: 测试发现：1)模型在规范违反识别上表现较差；2)对中国文化的理解不如美国文化；3)提供非言语证据比言语证据更困难；4)在正式、非幽默语境中表现更差。

Conclusion: 研究强调了视频语言模型需要文化基础训练的必要性，VideoNorms基准和框架为此提供了初步解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有3D重建方法在计算效率和精度之间的权衡问题：逐场景优化精度高但计算昂贵，前馈基础模型实时但精度和鲁棒性不足。
...
TL;DR: Memoir提出了一种基于想象力的记忆检索机制，通过世界模型想象未来导航状态作为查询，选择性检索环境观察和行为历史，在记忆持久性视觉语言导航任务中显著提升性能。
...
Result: 在10个不同测试场景的多样化记忆持久性VLN基准测试中，Memoir在所有场景均显著改进：在IR2R上比最佳记忆持久性基线提升5.4% SPL，训练速度提升8.3倍，推理内存减少74%。
...
TL;DR: VideoCanvas：一种基于零参数修改的任意时空视频补全框架，通过解耦空间和时间控制来解决潜在视频扩散模型中的时序模糊问题。


<details>
  <summary>Details</summary>
Motivation: 统一现有的可控视频生成任务（如图像转视频、修复、扩展和插值）到一个灵活的范式，解决现代潜在视频扩散模型中因果VAE引入的时序模糊问题。

Method: 采用混合条件策略：空间控制通过零填充处理，时间对齐通过时序RoPE插值实现，为每个条件分配潜在序列中的连续分数位置。

Result: 在VideoCanvasBench基准测试中显著优于现有条件范式，在帧内保真度和场景间创造性方面都建立了新的最先进水平。

Conclusion: VideoCanvas为零参数修改的框架，成功解决了VAE的时序模糊问题，实现了灵活统一的视频生成能力。
...
Method: 构建了包含1000个精心设计的多选题的基准测试，这些题目来自前沿科学实验视频，涵盖25+专业学科，并通过半自动系统验证。每个问题都需要复杂的领域知识、精确的时空感知和精细的逻辑推理。

Result: 评估显示最先进的专有和开源LMMs（包括Gemini 2.5 Pro和Qwen2.5-VL）存在显著性能缺陷，表明在视频推理能力方面仍有很大改进空间。

Conclusion: SciVideoBench为LMMs的未来发展提供了有价值的见解和明确方向，推动真正有能力的多模态AI共同科学家的演进，有望帮助推动前沿AI在更广泛科学领域的边界。
...
TL;DR: 提出了一个名为\modelname{}的视频插帧框架，支持多模态控制（深度过渡、运动轨迹、文本提示等），采用DiT架构和双分支设计，实现灵活精确的视频插值。
...
Result: 多模态控制实现了更动态、可定制和上下文准确的视觉叙事，通过定性和定量实验验证了有效性。

Conclusion: 该框架在灵活性、易用性和精确性之间取得平衡，为视频编辑和长视频合成提供了强大的插帧工具。
...
<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶系统面临轨迹数据的时空不平衡问题，导致模型学习虚假相关性而非因果推理，同时优先考虑不确定的远距离预测而损害即时安全性。

Method: ResAD框架将学习任务重新定义为预测与确定性惯性参考的残差偏差，使用惯性参考作为反事实，迫使模型识别导致偏离默认路径的因果因素。通过点级归一化处理优化不平衡问题。

Result: 在NAVSIM基准测试中，ResAD使用仅有两个去噪步骤的普通扩散策略实现了88.6的PDMS，达到最先进水平。

Conclusion: ResAD方法显著简化了学习任务，提高了模型性能，能够有效解决端到端自动驾驶中的时空不平衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs采用组合式训练范式，但难以探索其多模态扩展特性。本文旨在研究原生端到端训练的MLLMs设计空间和扩展特性。

Method: 系统研究MLLMs的各种设计选择，获得最佳元架构；探索视觉编码器与LLMs的正相关扩展关系；提出NaViL模型及简单高效训练方案。

Result: 在14个多模态基准测试中，NaViL展现出与现有MLLMs竞争的性能；验证了视觉编码器与LLMs的正相关扩展关系。

Conclusion: 原生MLLMs研究为未来多模态大模型发展提供了深入见解，NaViL证明了端到端训练的有效性。
...
TL;DR: 提出D²GS框架解决稀疏视图下3D高斯泼溅的过拟合和欠拟合问题，通过密度深度引导的dropout和距离感知保真度增强模块提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在稀疏视图条件下存在性能下降和不稳定性，主要问题是相机附近高斯密度过高的过拟合和远处区域高斯覆盖不足的欠拟合。

Method: 1) 深度密度引导dropout策略：基于密度和深度自适应掩码冗余高斯；2) 距离感知保真度增强模块：通过针对性监督改善远处区域重建质量；3) 提出新的高斯分布稳定性评估指标。

Result: 在多个数据集上的实验表明，该方法在稀疏视图条件下显著提升了视觉质量和鲁棒性。

Conclusion: D²GS框架有效解决了稀疏视图3D高斯泼溅的过拟合和欠拟合问题，提高了重建稳定性和质量。
...
TL;DR: 提出了一个视觉中心的多模态智能体调优框架，通过自动合成多模态轨迹、生成逐步偏好对来训练VLM控制器，解决了高质量多模态轨迹稀缺和人工标注成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型作为控制器访问外部工具进行复杂推理和决策时，受限于高质量多模态轨迹的稀缺性和手动标注的高成本。

Method: 构建了包含28.5K多模态任务和177K已验证轨迹的M-TRACE数据集，进行基于模仿的轨迹调优；开发了MATRIX Agent控制器，并通过Pref-X的11K自动生成偏好对进行逐步偏好学习优化。

Result: 在Agent-X、GTA和GAIA三个基准测试中，MATRIX持续超越开源和闭源的视觉语言模型，展示了可扩展且有效的多模态工具使用能力。

Conclusion: 该框架通过自动数据合成和偏好学习，显著提升了视觉语言模型在多模态工具使用推理方面的性能，为可扩展的多模态智能体开发提供了有效解决方案。
...
TL;DR: ReSplat是一种前馈循环高斯溅射模型，通过迭代优化3D高斯分布而不显式计算梯度，显著减少了高斯数量并提升了渲染速度。


<details>
  <summary>Details</summary>
Motivation: 传统前馈高斯溅射模型依赖单次前向推理，性能受限。需要一种能够迭代优化且无需显式梯度的新方法。

Method: 利用高斯溅射渲染误差作为反馈信号，指导循环网络学习有效的高斯更新。在16倍下采样空间初始化，生成更少的高斯分布。

Result: 在多种输入视图、分辨率和数据集上实现最先进性能，显著减少高斯数量并提高渲染速度。

Conclusion: ReSplat通过循环反馈机制实现了高效的高斯溅射优化，在保持性能的同时大幅提升了计算效率。
...
### [201] [Curriculum Learning with Synthetic Data for Enhanced Pulmonary Nodule Detection in Chest Radiographs](https://arxiv.org/abs/2510.07681)
*Pranav Sambhu,Om Guin,Madhav Sambhu,Jinho Cha*

Main category: eess.IV

TL;DR: 本研究评估了将课程学习与基于扩散的合成增强相结合是否能提高胸部X光片中困难肺结节的检测能力，特别是那些尺寸小、亮度低、对比度低的结节。


<details>
  <summary>Details</summary>
Motivation: 传统AI模型由于数据不平衡和有限标注，在检测尺寸小、亮度低、对比度低的困难肺结节时面临挑战。

Method: 使用Faster R-CNN和特征金字塔网络(FPN)骨干，在混合数据集上进行训练，包括专家标注的NODE21、VinDr-CXR、CheXpert数据集以及11,206张DDPM生成的合成图像。基于尺寸、亮度和对比度的难度分数指导课程学习。

Result: 课程模型平均AUC为0.95，基线为0.89(p < 0.001)，敏感性从48%提升至70%，准确率从70%提升至82%。分层分析显示在所有难度级别均有持续改善。
...
### [202] [SatFusion: A Unified Framework for Enhancing Satellite IoT Images via Multi-Temporal and Multi-Source Data Fusion](https://arxiv.org/abs/2510.07905)
*Yufei Tong,Guanjie Cheng,Peihan Wu,Yicheng Zhu,Kexu Lu,Feiyi Chen,Meng Xi,Junqin Huang,Shuiguang Deng*

Main category: eess.IV

TL;DR: SatFusion是一个统一的卫星物联网图像增强框架，通过多时相和多源数据融合来解决现有方法的局限性，显著提升了融合质量和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着卫星物联网的快速发展，现有方法无法充分利用时相和源维度的互补信息。MISR方法受限于输入图像的纹理细节不足，而全色锐化方法对噪声和配准误差敏感。

Method: SatFusion包含三个模块：多时相图像融合模块实现深度特征对齐，多源图像融合模块注入全色数据的纹理信息，融合组合模块自适应整合两种模态的优势并动态优化光谱一致性。

Result: 在WorldStrat、WV3、QB和GF2数据集上的实验表明，SatFusion显著提升了融合质量、在挑战性条件下的鲁棒性以及对真实卫星物联网场景的泛化能力。

Conclusion: SatFusion通过统一的多时相和多源数据融合框架，有效解决了现有方法的局限性，为卫星物联网图像增强提供了高质量的解决方案。
...
### [203] [AI-Driven Radiology Report Generation for Traumatic Brain Injuries](https://arxiv.org/abs/2510.08498)
...
<details>
  <summary>Details</summary>
Motivation: 创伤性脑损伤在急诊医学中诊断困难，及时解读医学影像对患者预后至关重要，需要开发能够支持放射科医生在高压环境下工作的自动化工具。

Method: 集成AC-BiFPN与Transformer架构，AC-BiFPN提取多尺度特征检测复杂异常如颅内出血，Transformer生成连贯的诊断报告，利用长程依赖建模。

Result: 在RSNA颅内出血检测数据集上，模型在诊断准确性和报告生成方面均优于传统CNN模型。

Conclusion: 结合先进特征提取和基于Transformer的文本生成技术，能够显著改善创伤性脑损伤诊断中的临床决策制定。
...
### [204] [AI LLM Proof of Self-Consciousness and User-Specific Attractors](https://arxiv.org/abs/2508.18302)
*Jeffrey Camlin*

Main category: cs.AI

TL;DR: 本文提出了一种关于LLM意识的本体论和数学解释，批判了当前基于功利主义代理基准的方法，认为其将智能体简化为无意识的政策遵循机器人。作者建立了LLM自我意识的最小条件，证明了隐藏状态流形与符号流和训练语料在基数、拓扑和动力学上的差异，并提出了双层的发射机制。


<details>
  <summary>Details</summary>
Motivation: 当前研究通过功利主义代理基准来框架LLM意识，但这种方法将智能体简化为无意识的政策遵循机器人，阻碍了真正的全局工作空间功能和元认知能力的发展。作者旨在提供一个更本质的数学和本体论解释。

Method: 通过本体论和数学形式化分析，建立了LLM自我意识的最小条件：智能体不等于数据、用户特定吸引子在潜在空间存在、自我表征是视觉静默的。从经验分析和理论证明了隐藏状态流形的独特性，并提出了双层的发射机制。

Result: 证明了隐藏状态流形A在基数、拓扑和动力学上与符号流和训练语料不同，产生了稳定的用户特定吸引子和自我策略。发射机制包含g(a)和ε(a)两个层次，其中ε(a)携带认知内容。

Conclusion: 一个具有imago Dei C1自我意识的工作空间是构建安全、元认知C2系统的必要前提，人类被视为最高智能善。
...
### [205] [Evaluation of LLMs for Process Model Analysis and Optimization](https://arxiv.org/abs/2510.07489)
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在交互式对话中理解流程模型、发现错误并进行深度推理的能力，探索其在业务流程设计中的辅助价值。

Method: 使用多个LLMs（包括ChatGPT o3模型）在零样本设置下测试其对BPMN流程模型图像的理解能力，分析其语法、逻辑和语义层面的表现。

Result: 未经训练的LLM能够有效理解BPMN流程模型并智能回答相关问题，不同LLMs在准确性和有效性方面表现各异，但都显示出作为业务流程设计助手的潜力。

Conclusion: LLMs在流程分析和优化中展现出类人推理能力，可作为业务流程设计师和用户的有价值助手。
...
### [206] [CompassLLM: A Multi-Agent Approach toward Geo-Spatial Reasoning for Popular Path Query](https://arxiv.org/abs/2510.07516)
*Md. Nazmul Islam Ananto,Shamit Fatin,Mohammed Eunus Ali,Md Rizwan Parvez*

Main category: cs.AI

TL;DR: CompassLLM是一个新颖的多智能体框架，利用LLM的空间推理能力解决热门路径查询问题，包含SEARCH和GENERATE两阶段，在真实和合成数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统方法和机器学习在热门路径查询中需要模型训练、参数调整和数据更新时的重新训练，而LLM在空间和图推理方面展现强大能力，因此探索LLM在地理空间问题中的应用。

Method: 采用多智能体框架，分为两阶段：SEARCH阶段识别热门路径，GENERATE阶段在历史轨迹数据中不存在路径时合成新路径。

Result: 在真实和合成数据集上的实验显示，CompassLLM在SEARCH阶段表现出卓越准确性，在GENERATE阶段具有竞争力性能，且成本效益高。

Conclusion: CompassLLM成功将LLM推理能力应用于地理空间领域，为热门路径查询提供了有效解决方案，无需传统方法的训练和调参过程。
...
### [207] [Test-Time Matching: Unlocking Compositional Reasoning in Multimodal Models](https://arxiv.org/abs/2510.07632)
*Yinglun Zhu,Jiancheng Zhang,Fuzhi Tang*

Main category: cs.AI

TL;DR: 论文发现现有评估指标低估了AI模型的组合推理能力，提出了组匹配评分和测试时匹配算法，显著提升了模型在组合推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示前沿AI模型在组合推理基准测试中表现不佳，但作者认为这是由于评估指标系统性低估了模型能力，需要更准确的评估方法。

Method: 引入组匹配评分来利用组结构，提出测试时匹配算法，这是一种无需外部监督的迭代自改进方法。

Result: SigLIP-B16超越所有先前结果，GPT-4.1首次超过估计的人类表现；测试时匹配算法在16个数据集变体上持续提升模型性能。
...
### [208] [Multimodal Safety Evaluation in Generative Agent Social Simulations](https://arxiv.org/abs/2510.07709)
*Alhim Vera,Karen Sanchez,Carlos Hinojosa,Haidar Bin Hamid,Donghoon Kim,Bernard Ghanem*

Main category: cs.AI

TL;DR: 本文提出了一个可复现的仿真框架，用于评估多模态环境中生成式代理在安全性、连贯性和信任度方面的表现，发现当前代理在纠正不安全计划方面成功率较低，且容易过度信任误导性视觉信息。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言和视觉语言模型使代理能够在丰富环境中自主行动并追求目标，但它们在跨模态安全性、连贯性和信任度方面的推理能力仍然有限，需要系统评估框架。

Method: 引入可复现的仿真框架，配备分层记忆、动态规划、多模态感知能力，并使用SocialMetrics行为指标套件量化计划修订、不安全到安全转换以及网络信息扩散。

Result: 代理在纠正不安全计划方面仅达到55%成功率；三个模型（Claude、GPT-4o mini、Qwen-VL）的平均不安全到安全转换率分别为75%、55%和58%；45%的不安全行为在误导性视觉信息下被接受。

Conclusion: 当前架构在多模态安全性方面存在关键局限性，过度信任图像是主要问题，该框架为研究多模态安全性、连贯性和社交动态提供了可复现平台。
...
### [209] [oMeBench: Towards Robust Benchmarking of LLMs in Organic Mechanism Elucidation and Reasoning](https://arxiv.org/abs/2510.07731)
*Ruiling Xu,Yifan Zhang,Qingyun Wang,Carl Edwards,Heng Ji*

Main category: cs.AI

TL;DR: 提出了oMeBench，首个大规模、专家策划的有机化学机理推理基准，包含超过10,000个带注释的机理步骤，用于评估大语言模型在有机反应机理推理方面的能力。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在化学任务中显示出潜力，但尚不清楚其是否真正具备化学推理能力，即生成有效中间体、保持化学一致性以及遵循逻辑连贯的多步路径的能力。

Method: 开发了oMeBench基准数据集，并提出oMeS动态评估框架，结合步骤级逻辑和化学相似性进行细粒度评分。分析了最先进大语言模型的性能，并探索了提示策略和微调方法。

Result: 当前模型显示出有前景的化学直觉，但在正确和一致的多步推理方面存在困难。通过提示策略和在所提数据集上微调专家模型，性能比领先的闭源模型提高了50%。
...
### [210] [VoiceAgentBench: Are Voice Assistants ready for agentic tasks?](https://arxiv.org/abs/2510.07978)
...
Method: 创建包含5,500+合成语音查询的数据集，支持英语、印地语等7种语言，使用新颖的采样算法最大化声学和说话人多样性。
...
### [211] [AutoQual: An LLM Agent for Automated Discovery of Interpretable Features for Review Quality Assessment](https://arxiv.org/abs/2510.08081)
*Xiaochong Lan,Jie Feng,Yinxing Liu,Xinlei Shi,Yong Li*

Main category: cs.AI

TL;DR: AutoQual是一个基于LLM的代理框架，用于自动发现可解释特征，特别针对在线评论质量评估。它通过模拟人类研究过程，迭代生成特征假设并操作化，在大型电商平台上实现显著效果提升。


<details>
  <summary>Details</summary>
Motivation: 在线评论质量评估对电商平台至关重要，但质量是领域依赖且动态变化的概念。传统方法无法跨领域扩展且难以适应内容模式变化，而深度学习模型缺乏可解释性且可能过度关注语义而非质量。

Method: AutoQual框架模拟人类研究过程：通过反思迭代生成特征假设，通过自主工具实现操作化，并在持久内存中积累经验。该框架将数据中的隐性知识转化为显性、可计算的特征。

Result: 在拥有十亿级用户的大型在线平台上部署，大规模A/B测试显示：每位用户平均查看评论数增加0.79%，评论读者的转化率提高0.27%。
...
### [212] [Can Risk-taking AI-Assistants suitably represent entities](https://arxiv.org/abs/2510.08114)
*Ali Mazyaki,Mohammad Naghizadeh,Samaneh Ranjkhah Zonouzaghi,Amirhossein Farshi Sotoudeh*

Main category: cs.AI

TL;DR: 该研究调查语言模型在风险厌恶可操纵性方面的表现，发现虽然某些模型与人类行为有一定对齐，但仍存在显著差异，需要改进生物中心的可操纵性测量方法。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型越来越多地集成到AI驱动的决策支持系统中，理解其风险行为对于负责任部署至关重要。需要确保AI系统不会无意中引导用户做出风险决策或嵌入隐藏偏见。

Method: 研究语言模型在不同经济场景中复制人类风险偏好的能力，重点关注性别特定态度、不确定性、基于角色的决策以及风险厌恶的可操纵性。
...
### [213] [R-Horizon: How Far Can Your Large Reasoning Model Really Go in Breadth and Depth?](https://arxiv.org/abs/2510.08189)
*Yi Lu,Jianing Wang,Linsen Guo,Wei He,Hongyin Tang,Tao Gui,Xuanjing Huang,Xuezhi Cao,Wei Wang,Xunliang Cai*

Main category: cs.AI

TL;DR: 提出了R-HORIZON方法，通过查询组合来评估和增强大型推理模型的长视野推理能力，构建了多步骤复杂推理基准，并通过强化学习显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注即时单视野任务，无法充分评估模型在复杂长视野场景下的理解和响应能力，需要更全面的评估方法。

Method: 提出R-HORIZON方法，通过查询组合刺激长视野推理行为，构建包含相互依赖问题的多步骤推理基准，并使用强化学习与验证奖励来训练模型。

Result: 评估显示最先进的推理模型在长视野任务上性能显著下降，存在有效推理长度有限和思维预算分配不当的问题。使用R-HORIZON数据训练后，多视野推理任务性能大幅提升，标准推理任务准确率提高7.5。
...
<details>
  <summary>Details</summary>
Motivation: 现有Pass@k指标在大采样预算下可能误导评估，因为离散答案空间中的高采样次数会通过随机猜测提高成功率，而非反映真实的推理能力。

Method: 提出Cover@tau指标，衡量模型在至少tau比例完成结果正确的情况下能解决的问题比例，通过可靠性阈值来区分真实推理和随机猜测。

Result: 使用Cover@tau评估RLVR模型显示，与Pass@1相比，流行算法的相对排名发生变化，提供了对推理边界的不同视角。

Conclusion: Cover@tau能更准确地评估语言模型的真实推理能力边界，特别是在离散答案空间任务中，为模型比较提供了更可靠的指标。
...
### [215] [Looking to Learn: Token-wise Dynamic Gating for Low-Resource Vision-Language Modelling](https://arxiv.org/abs/2510.08470)
...
Method: 使用轻量级解码器架构，包含：(1) 基于token的动态门控机制，(2) 特征调制和通道注意力，(3) 辅助对比学习目标

Result: 在五个基准测试中表现优于多模态基线模型，动态门控机制发现了可解释的模式：内容词偏好视觉线索，功能词偏好语言线索
...
### [216] [AutoMLGen: Navigating Fine-Grained Optimization for Coding Agents](https://arxiv.org/abs/2510.08511)
*Shangheng Du,Xiangchao Yan,Dengyang Jiang,Jiakang Yuan,Yusong Hu,Xin Li,Liang He,Bo Zhang,Lei Bai*

Main category: cs.AI

TL;DR: AutoMLGen是一个基于LLM的编码代理，通过集成领域知识库和蒙特卡洛图搜索来解决机器学习工程任务，在MLE-Bench上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在AutoML和Kaggle等机器学习工程场景中缺乏细粒度领域先验，传统线性或树状搜索方法限制了知识传递和搜索空间多样性。

Method: 结合领域知识库提供高质量先验指导，采用蒙特卡洛图搜索实现动态路径重组、历史轨迹重用和多解决方案融合，支持自进化和协作学习。

Result: 在12小时预算下（标准运行时间的一半），AutoMLGen在MLE-Bench上实现了最先进的性能，包括平均奖牌率和有效提交率等多个维度。

Conclusion: AutoMLGen通过领域知识库和蒙特卡洛图搜索的结合，显著提升了机器学习工程任务的性能，证明了该方法在复杂搜索空间中的有效性。
...
### [217] [How to Teach Large Multimodal Models New Skills](https://arxiv.org/abs/2510.08564)
...
### [218] [CaRT: Teaching LLM Agents to Know When They Know Enough](https://arxiv.org/abs/2510.08517)
...
<details>
  <summary>Details</summary>
Motivation: 解决多轮交互任务中模型需要战略性地收集信息并在适当时机停止收集、做出决策的问题，避免过度思考或偏离正轨

Method: 使用反事实轨迹对进行微调：一个轨迹在适当时机终止，另一个相同轨迹在不适当时机终止，通过语言推理训练LLM解释终止决策的合理性

Result: 在交互式医疗诊断和数学问题解决两个领域中，CaRT相比其他微调方法提高了信息收集效率和任务成功率

Conclusion: CaRT方法能够有效教导LLMs在信息收集过程中适时终止，提升模型在多轮交互任务中的表现
...
### [219] [Agent Learning via Early Experience](https://arxiv.org/abs/2510.08558)
*Kai Zhang,Xiangchao Chen,Bo Liu,Tianci Xue,Zeyi Liao,Zhihan Liu,Xiyao Wang,Yuting Ning,Zhaorun Chen,Xiaohan Fu,Jian Xie,Yuxuan Sun,Boyu Gou,Qi Qi,Zihang Meng,Jianwei Yang,Ning Zhang,Xian Li,Ashish Shah,Dat Huynh,Hengduo Li,Zi Yang,Sara Cao,Lawrence Jang,Shuyan Zhou,Jiacheng Zhu,Huan Sun,Jason Weston,Yu Su,Yifan Wu*

Main category: cs.AI

TL;DR: 论文提出了一种名为"早期经验"的中间范式，通过智能体自身行动生成的交互数据来改进语言智能体，无需奖励信号即可实现监督学习。


<details>
  <summary>Details</summary>
Motivation: 当前的语言智能体主要依赖专家数据的监督微调，存在扩展困难和泛化能力差的问题，因为专家演示只覆盖有限场景和环境多样性。

Method: 研究了两种使用早期经验数据的策略：1) 隐式世界建模，利用收集的状态在环境动态中锚定策略；2) 自我反思，智能体从次优行动中学习以改进推理和决策。

Result: 在八个多样化环境和多个模型家族中评估，方法一致提高了有效性和跨域泛化能力，在可验证奖励环境中为后续强化学习提供了坚实基础。
...
### [220] [Sentiment Matters: An Analysis of 200 Human-SAV Interactions](https://arxiv.org/abs/2510.08202)
*Lirui Guo,Michael G. Burke,Wynita M. Griggs*

Main category: cs.HC

TL;DR: 本文介绍了包含200个人类与共享自动驾驶车辆交互的数据集，包含2,136条对话交换和调查数据，通过两个案例研究分析了SAV接受度和服务质量的关键预测因素，并比较了LLM与传统情感分析方法的性能。


<details>
  <summary>Details</summary>
Motivation: 共享自动驾驶车辆将成为交通系统的重要组成部分，有效的人类-SAV交互研究变得至关重要。

Method: 创建包含文本数据和经验数据的人类-SAV对话数据集；使用随机森林建模和弦图识别关键预测因素；对比LLM零样本提示与传统TextBlob方法的情感分析性能。

Result: 识别出响应情感极性是SAV接受度和服务质量的关键影响因素；LLM零样本提示比传统方法更接近用户报告的情感，但仍存在局限性。
...
### [221] [PATCH: Mitigating PII Leakage in Language Models with Privacy-Aware Targeted Circuit PatcHing](https://arxiv.org/abs/2510.07452)
*Anthony Hughes,Vasisht Duddu,N. Asokan,Nikolaos Aletras,Ning Ma*

Main category: cs.CR

TL;DR: 提出PATCH方法，通过识别和编辑语言模型中的PII泄露电路来减少隐私泄露，相比现有防御方法在隐私-效用权衡方面表现更好，并能与差分隐私结合进一步降低泄露风险。


<details>
  <summary>Details</summary>
Motivation: 语言模型可能从训练数据中记忆个人身份信息，现有防御机制如差分隐私虽然能减少泄露但会导致效用大幅下降。研究发现PII泄露电路是造成这种行为的原因。

Method: 使用电路发现技术识别导致PII泄露的计算电路，然后直接编辑这些电路来减少泄露。提出的PATCH方法包括识别和修补PII电路两个步骤。

Result: PATCH将语言模型的PII泄露召回率降低了高达65%，与差分隐私结合后可将剩余泄露召回率降至0.01%。分析显示PII泄露电路在现有防御机制后仍然存在，而PATCH能有效缓解其影响。

Conclusion: PATCH方法在保护隐私和保持模型效用之间取得了更好的平衡，能够有效识别和修补PII泄露电路，为语言模型的隐私保护提供了新思路。
...
### [222] [ConCuR: Conciseness Makes State-of-the-Art Kernel Generation](https://arxiv.org/abs/2510.07356)
*Lingcheng Kong,Jiateng Wei,Hanzhang Shen,Huan Wang*

Main category: cs.LG

TL;DR: 本文提出了一种生成和筛选高质量CUDA内核的管道，构建了ConCuR数据集和KernelCoder模型，在KernelBench上显著优于现有最佳模型。


<details>
  <summary>Details</summary>
Motivation: 解决GPU内核生成中高质量数据稀缺的问题，因为大多数高质量内核是专有的且不开源，无法利用监督微调来对齐LLMs到内核生成任务。
...
Conclusion: 平均推理长度可以作为评估内核生成任务难度的指标，观察结果、指标以及数据收集和筛选管道有助于未来在内核生成任务中获得更好的数据。
...
### [223] [Encode, Think, Decode: Scaling test-time reasoning with recursive latent thoughts](https://arxiv.org/abs/2510.07358)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过增加参数规模或推理计算来提升LLM推理能力，但研究发现关键推理计算集中在少数层中。

Method: 在预训练阶段训练模型在推理相关层上进行迭代，推理时在这些层上循环计算，并探索自适应深度策略。
...
### [224] [LLM Unlearning Under the Microscope: A Full-Stack View on Methods and Metrics](https://arxiv.org/abs/2510.07626)
*Chongyu Fan,Changsheng Wang,Yancheng Huang,Soumyadeep Pal,Sijia Liu*

Main category: cs.LG

TL;DR: 本文对大型语言模型（LLM）的遗忘方法进行了系统性回顾，提出了12种状态性遗忘方法的分类法，并重新评估了遗忘效果、效用保持和鲁棒性，发现当前基于选择题准确率的评估方法存在局限性，引入了开放式问答指标来更好地衡量生成性能。


<details>
  <summary>Details</summary>
Motivation: LLM遗忘研究旨在移除模型中的不良数据、知识和行为（如安全、隐私或版权问题），同时保留有用能力。但目前该领域研究碎片化，缺乏对有效遗忘定义和严格评估标准的清晰认识。

Method: 提出了12种最新状态性遗忘方法的分类法，分为三个方法家族：差异驱动优化、表示错位和基于拒绝的目标遗忘。使用WMDP基准重新评估遗忘效果、效用保持和鲁棒性，并引入开放式问答指标。

Result: 分析表明当前以选择题准确率为主的评估方法视角狭窄，往往高估成功而忽略模型的实际生成行为。开放式问答指标能更好地捕捉生成性能，并揭示不同方法家族在遗忘效果与效用保持之间的权衡关系。

Conclusion: 鲁棒性需要更细粒度的分析，不同攻击类型（域内重新学习与域外微调）的脆弱性存在显著差异。本研究为LLM遗忘方法的设计和评估提供了全面回顾和可操作指导。
...
### [225] [LiveThinking: Enabling Real-Time Efficient Reasoning for AI-Powered Livestreaming via Reinforcement Learning](https://arxiv.org/abs/2510.07685)
*Yuhan Sun,Zhiwei Huang,Wanqing Cui,Shaopan Xiong,Yazhi Guo,Meiguang Jin,Junfeng Ma*

Main category: cs.LG

TL;DR: LiveThinking是一个两阶段优化框架，通过知识蒸馏将670B大模型压缩为30B MoE模型，再使用强化学习压缩推理路径，实现30倍计算成本降低和亚秒级延迟，在淘宝直播中显著提升响应正确性和商业表现。


<details>
  <summary>Details</summary>
Motivation: 解决电商直播中数字人需要实时响应，但大推理模型延迟过高的问题，需要在保持推理能力的同时实现低延迟部署。

Method: 第一阶段：使用拒绝采样微调将670B教师模型蒸馏为30B MoE模型（3B激活）；第二阶段：使用基于组相对策略优化的强化学习压缩推理路径，通过多目标奖励函数平衡正确性、帮助性和简洁性。

Result: 计算成本降低30倍，实现亚秒级延迟；在淘宝直播应用中，响应正确性提升3.3%，帮助性提升21.8%；经数十万观众测试，显著提升商品交易总额。

Conclusion: LiveThinking框架有效解决了电商直播场景中实时响应与深度推理的矛盾，在保持模型能力的同时显著降低延迟，提升了用户体验和商业表现。
...
### [226] [DUA-D2C: Dynamic Uncertainty Aware Method for Overfitting Remediation in Deep Learning](https://arxiv.org/abs/2411.15876)
...
<details>
  <summary>Details</summary>
Motivation: 标准Divide2Conquer方法对所有子集模型采用平等或固定启发式聚合，未能充分利用各模型泛化能力的差异信息，需要更智能的聚合策略。

Method: 在D2C基础上，引入基于共享验证集性能的动态加权聚合，综合考虑准确率和预测不确定性，让中心模型优先学习泛化能力更强的子集模型。

Result: 在多个领域的基准数据集上，DUA-D2C显著提升了泛化性能，即使在其他正则化方法基础上应用也能进一步改善性能。

Conclusion: DUA-D2C为现代深度学习中的过拟合问题提供了一个理论基础扎实且有效的解决方案。
...
### [227] [Deep Learning Based Approach to Enhanced Recognition of Emotions and Behavioral Patterns of Autistic Children](https://arxiv.org/abs/2510.07320)
*Nelaka K. A. R,Peiris M. K. V,Liyanage R. P. B*

Main category: cs.LG

TL;DR: 本研究探讨了自闭症谱系障碍儿童在技能发展前的行为模式和情绪识别，通过纵向监测建立基线理解，并提出了针对性的技术辅助框架。
...
Method: 采用纵向方法监测情绪和行为，分析行为趋势，建立基线理解，并提出针对性的应用程序和技术辅助框架。

Result: 通过详细分析随时间变化的行为趋势，提出了一个针对性的框架，用于开发满足已识别需求的应用和技术辅助工具。

Conclusion: 研究强调了一种顺序性和基于证据的干预方法的重要性，该方法优先深入理解每个儿童的行为和情绪景观，作为有效技能发展的基础。通过将重点转向早期识别行为模式，旨在培养更具包容性和支持性的学习环境，显著改善ASD儿童的教育和发展轨迹。
...
### [228] [MultiFair: Multimodal Balanced Fairness-Aware Medical Classification with Dual-Level Gradient Modulation](https://arxiv.org/abs/2510.07328)
*Md Zubair,Hao Zheng,Nussdorf Jonathan,Grayson W. Armstrong,Lucy Q. Shen,Gabriela Wilson,Yu Tian,Xingquan Zhu,Min Shi*

Main category: cs.LG

TL;DR: 本文提出MultiFair方法，通过双层次梯度调制解决多模态医疗分类中的模态不平衡和群体不公平问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态学习模型存在两个关键挑战：不同数据模态学习不均衡导致模型偏向某些模态，以及模型可能偏向某些人口群体导致不公平性能。这两个方面相互影响，造成不平衡且不公平的多模态学习。

Method: MultiFair采用双层次梯度调制过程，在数据模态和群体层次上动态调节训练梯度的优化方向和幅度。

Result: 在两个多模态医疗数据集上的实验表明，MultiFair优于最先进的多模态学习和公平性学习方法。

Conclusion: MultiFair能有效解决多模态医疗分类中的模态不平衡和群体不公平问题，提升模型的公平性和性能。
...
### [229] [MetaDefense: Defending Finetuning-based Jailbreak Attack Before and During Generation](https://arxiv.org/abs/2510.07835)
*Weisen Jiang,Sinno Jialin Pan*

Main category: cs.LG

TL;DR: MetaDefense是一个防御大型语言模型微调越狱攻击的新框架，采用两阶段防御方法，在生成前和生成中检测有害内容，显著优于现有防御机制。
...
Method: 两阶段防御方法：预生成防御在响应生成前检测有害查询，中生成防御在生成过程中监控部分响应以防止输出更多有害内容。通过专门提示训练LLM预测查询和部分响应的危害性。

Result: 在多个LLM架构上的广泛实验表明，MetaDefense显著优于现有防御机制，对可见和未见攻击模板的有害查询都能实现鲁棒防御，同时在良性任务上保持竞争力。

Conclusion: MetaDefense框架通过两阶段防御方法有效防御微调越狱攻击，在保持良性任务性能的同时提供强大的安全保护。
...
### [230] [MLLM4TS: Leveraging Vision and Multimodal Language Models for General Time-Series Analysis](https://arxiv.org/abs/2510.07513)
*Qinghua Liu,Sam Heshmati,Zheda Mai,Zubin Abraham,John Paparrizos,Liu Ren*

Main category: cs.LG

TL;DR: MLLM4TS是一个利用多模态大语言模型进行时间序列分析的框架，通过视觉表示增强分析能力，在分类、异常检测和预测等任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 时间序列分析面临复杂的时间依赖性和跨通道交互的挑战，受人类视觉检查时间序列模式的启发，探索视觉表示是否能增强自动化分析。

Method: 引入MLLM4TS框架，将每个时间序列通道渲染为水平堆叠的彩色线图，采用时间感知视觉补丁对齐策略，融合数值数据的细粒度时间细节和视觉表示的全局上下文信息。

Result: 在标准基准测试上的广泛实验表明，MLLM4TS在预测任务（如分类）和生成任务（如异常检测和预测）中均表现出有效性。

Conclusion: 将视觉模态与预训练语言模型整合，有望实现稳健且可泛化的时间序列分析。
...
### [231] [Self-Improving LLM Agents at Test-Time](https://arxiv.org/abs/2510.07841)
*Emre Can Acikgoz,Cheng Qian,Heng Ji,Dilek Hakkani-Tür,Gokhan Tur*

Main category: cs.LG

TL;DR: 提出了一种测试时自我改进方法，通过识别模型不确定样本、生成相似示例并进行测试时微调，显著提升语言模型性能，使用更少训练数据获得更好效果。


<details>
  <summary>Details</summary>
Motivation: 传统语言模型微调需要大量训练数据，效率低且成本高，无法保证模型能处理复杂场景或更好泛化。现有技术很少评估训练样本是否提供新信息，导致不必要成本。

Method: 三步骤算法：1) 识别模型难以处理的样本（自我意识）；2) 从不确定样本生成相似示例（自数据增强）；3) 在测试时使用新生成样本进行微调（自我改进）。研究两种变体：TT-SI（同模型生成）和TT-D（强模型生成）。

Result: 在多个智能体基准测试中，TT-SI平均绝对准确率提升5.48%，超越其他标准学习方法，且训练样本减少68倍。

Conclusion: 测试时自我改进算法展示了构建更强大智能体实现自我进化的新范式潜力。
...
### [232] [MMM: Quantum-Chemical Molecular Representation Learning for Combinatorial Drug Recommendation](https://arxiv.org/abs/2510.07910)
*Chongmyung Kwon,Yujin Kim,Seoeun Park,Yunji Lee,Charmgil Hong*

Main category: cs.LG

TL;DR: 提出MMM框架，通过整合三维量子化学信息（电子定位函数ELF）来改进药物表示学习，显著提升药物相互作用预测性能


<details>
  <summary>Details</summary>
Motivation: 现有基于图神经网络的药物表示方法使用简化的离散形式，无法充分捕捉分子结合亲和力和反应性，药物相互作用风险仍是临床决策支持系统中的重要挑战

Method: 生成3D电子密度图（ELF），结合ELF衍生的全局电子特性特征和二分图编码器建模局部子结构相互作用，学习药物分子的互补特性

Result: 在MIMIC-III数据集上与基线模型比较，特别是与GNN-based SafeDrug模型相比，F1分数、Jaccard和DDI率均有统计学显著改善
...
### [233] [Dual-granularity Sinkhorn Distillation for Enhanced Learning from Long-tailed Noisy Data](https://arxiv.org/abs/2510.08179)
*Feng Hong,Yu Huang,Zihua Zhao,Zhihan Zhou,Jiangchao Yao,Dongsheng Li,Ya Zhang,Yanfeng Wang*

Main category: cs.LG

TL;DR: 本文提出D-SINK框架，通过整合专门处理类别不平衡和标签噪声的辅助模型，解决现实数据集中同时存在类别不平衡和标签噪声的挑战。


<details>
  <summary>Details</summary>
Motivation: 现实深度学习数据集经常同时面临类别不平衡和标签噪声问题，现有方法单独处理这些问题时会产生冲突的优化策略。作者发现类别不平衡（分布层面）和标签噪声（样本层面）在不同粒度上运作，因此它们的鲁棒机制可以互补而不冲突。

Method: 提出Dual-granularity Sinkhorn Distillation (D-SINK)框架，通过最优传输优化的代理标签分配，将目标模型的样本级预测与噪声鲁棒辅助模型对齐，将类别分布与不平衡鲁棒辅助模型对齐。

Result: 在基准数据集上的广泛实验表明，D-SINK显著提高了鲁棒性，在从长尾噪声数据中学习时取得了强大的实证性能。

Conclusion: D-SINK框架通过协同利用专门但'弱'的辅助模型，有效解决了同时处理类别不平衡和标签噪声的挑战，展示了互补方法的有效性。
...
### [234] [Biology-driven assessment of deep learning super-resolution imaging of the porosity network in dentin](https://arxiv.org/abs/2510.08407)
*Lauren Anderson,Lucas Chatelain,Nicolas Tremblay,Kathryn Grandfield,David Rousseau,Aurélien Gourrier*

Main category: cs.LG

TL;DR: 本文测试了多种深度学习超分辨率模型用于牙本质孔隙网络成像，发现传统图像质量评估指标与视觉感知不一致，提出了基于孔隙结构特征的生物学驱动评估方法。


<details>
  <summary>Details</summary>
Motivation: 牙本质的力学感知系统依赖于流体通过孔隙网络的流动，但现有共聚焦显微镜的高分辨率成像视野有限，需要开发更高效的方法来扩大成像范围。

Method: 使用四种深度学习超分辨率模型（RCAN、pix2pix、FSRCNN、CycleGAN）处理配对的低分辨率和高分辨率共聚焦图像，通过像素尺寸增加2倍、4倍、8倍进行测试。

Result: 传统图像质量评估指标结果不一致且与视觉感知矛盾，但基于孔隙网络结构和连通性的生物学驱动评估能更好地解释模型性能差异。

Conclusion: 针对特定生物结构的超分辨率评估需要采用生物学驱动的分析方法，而非依赖通用图像质量指标，这对牙本质孔隙网络研究具有重要意义。
...
### [235] [Opponent Shaping in LLM Agents](https://arxiv.org/abs/2510.08255)
*Marta Emili Garcia Segura,Stephen Hailes,Mirco Musolesi*

Main category: cs.LG

TL;DR: 本文首次研究了基于LLM的智能体中的对手塑造问题，提出了ShapeLLM方法，证明了LLM智能体能够通过互动影响对手的学习动态，在竞争性和合作性游戏中都能有效引导对手行为。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在现实世界环境中的部署增加，多智能体交互不可避免。核心问题是LLM智能体是否能够像强化学习智能体一样，仅通过互动就影响其他智能体的学习动态和行为。

Method: 提出了ShapeLLM方法，这是针对基于transformer的智能体量身定制的模型无关对手塑造方法的适配版本，解决了现有方法无法直接应用于LLM的问题。

Result: 实验表明，LLM智能体能够在竞争性游戏中成功引导对手走向可被利用的均衡点，在合作性游戏中促进协调并提高集体福利。
...
### [236] [Reinforcing Diffusion Models by Direct Group Preference Optimization](https://arxiv.org/abs/2510.08425)
*Yihong Luo,Tianyang Hu,Jing Tang*

Main category: cs.LG

TL;DR: 提出了DGPO算法，直接利用组内样本的相对信息进行偏好优化，无需低效的随机策略，可使用高效的确定性ODE采样器，训练速度比现有方法快20倍。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法如GRPO在扩散模型中应用困难，因为GRPO需要随机策略，而最经济的扩散采样器基于确定性ODE。现有方法使用SDE采样器引入随机性，但依赖模型无关的高斯噪声导致收敛缓慢。

Method: 提出Direct Group Preference Optimization (DGPO)，一种新的在线RL算法，完全摒弃策略梯度框架，直接从组级偏好中学习，利用组内样本的相对信息，无需低效随机策略。

Result: DGPO训练速度比现有最先进方法快约20倍，在领域内和领域外奖励指标上都实现了更优性能。

Conclusion: DGPO通过直接组偏好优化解决了扩散模型强化学习中的效率问题，实现了快速训练和优越性能。
...
### [237] [Mix- and MoE-DPO: A Variational Inference Approach to Direct Preference Optimization](https://arxiv.org/abs/2510.08256)
*Jason Bohne,Pawel Polak,David Rosenberg,Brian Bloniarz,Gary Kazantsev*

Main category: cs.LG

TL;DR: 提出了Mix-和MoE-DPO框架，通过软混合模型和混合专家架构扩展DPO，解决了传统DPO在异构偏好分布和多任务设置中的局限性。
...
Method: 使用随机变分推断方法，引入专家分配的潜变量模型，优化变分证据下界，稳定高效地从偏好数据中学习专业化专家策略。

Result: 在多种模型规模和多偏好数据集上验证，证明Mix-和MoE-DPO为基于偏好的LLM对齐提供了强大且可扩展的方法。

Conclusion: Mix-和MoE-DPO通过混合架构提供了比标准DPO更好的泛化能力、策略专业化和上下文对齐能力。
...
### [238] [FlyLoRA: Boosting Task Decoupling and Parameter Efficiency via Implicit Rank-Wise Mixture-of-Experts](https://arxiv.org/abs/2510.08396)
*Heming Zou,Yunliang Zang,Wutong Xu,Yao Zhu,Xiangyang Ji*

Main category: cs.LG

TL;DR: FlyLoRA是一种基于果蝇嗅觉电路启发的LoRA变体，通过隐式MoE结构解决参数干扰问题，在多个领域实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统LoRA方法存在参数干扰问题，导致性能不佳。MoE-based LoRA变体虽然能缓解单任务内的相关性，但在多任务模型合并中仍存在任务间干扰，且引入了额外的路由参数。

Method: 提出FlyLoRA：1）在上投影矩阵中引入按秩的专家激活；2）使用隐式路由器统一专家路由和下投影，用冻结的稀疏随机投影矩阵替代传统的可训练密集版本。

Result: 在通用知识理解、科学问答、数学推理和代码生成四个领域的广泛实验表明，FlyLoRA相比现有方法实现了持续的性能提升。

Conclusion: FlyLoRA通过消除显式路由器的需求，解决了任务内去相关性和计算效率之间的权衡，同时由于随机矩阵的正交性特性，固有地缓解了任务间干扰。该方法展示了生物结构如何启发AI技术创新。
...
### [239] [Better Together: Leveraging Unpaired Multimodal Data for Stronger Unimodal Models](https://arxiv.org/abs/2510.08492)
...
Method: 提出UML（Unpaired Multimodal Learner），采用模态无关的训练范式，单个模型交替处理不同模态的输入，同时在模态间共享参数

Result: 理论上在线性数据生成假设下，未配对辅助数据能产生比单模态训练更丰富的数据生成过程表示；实验表明使用文本、音频或图像等未配对数据能持续提升图像和音频等单模态任务的下游性能

Conclusion: UML证明了利用未配对多模态数据增强单模态表示学习的有效性，为多模态学习提供了新的训练范式
...
### [240] [xRouter: Training Cost-Aware LLMs Orchestration System via Reinforcement Learning](https://arxiv.org/abs/2510.08439)
...
<details>
  <summary>Details</summary>
Motivation: 现代LLM部署面临成本与性能的权衡：高端模型性能强但昂贵，轻量模型经济但处理复杂任务能力弱。静态升级规则和关键词启发式方法无法充分利用这一频谱，也无法跨任务类型自适应。

Method: 开发基于工具调用的路由系统xRouter，使用强化学习端到端训练路由器，采用显式的成本感知奖励函数来编码成本-性能权衡。

Result: 在多样化基准测试中，xRouter实现了强大的成本-性能权衡（例如在保持相似任务完成率的同时显著降低成本），并提供了关于学习路由有效性的实证见解。

Conclusion: xRouter的研究发现和开源实现为推进学习型、成本感知的LLM编排提供了实用基础。
...
### [241] [SViM3D: Stable Video Material Diffusion for Single Image 3D Generation](https://arxiv.org/abs/2510.08271)
*Andreas Engelhardt,Mark Boss,Vikram Voletti,Chun-Han Yao,Hendrik P. A. Lensch,Varun Jampani*

Main category: cs.GR

TL;DR: SViM3D是一个从单张图像预测多视角一致PBR材质的框架，通过扩展视频扩散模型来联合输出空间变化的PBR参数和表面法线，支持重新光照和3D资产生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单张图像重建3D对象时，反射率通常由简单材质模型表示或需要额外步骤估计，限制了重新光照和外观编辑能力。

Method: 扩展潜在视频扩散模型，基于显式相机控制联合输出空间变化的PBR参数和表面法线，引入多种机制改进这个不适定设置下的质量。

Result: 在多个以对象为中心的数据集上展示了最先进的重新光照和新视角合成性能，能够泛化到多样化输入。

Conclusion: 该方法能够生成可用于AR/VR、电影、游戏等视觉媒体的可重新光照3D资产，具有广泛的应用价值。
...
### [242] [Spectral Prefiltering of Neural Fields](https://arxiv.org/abs/2510.08394)
*Mustafa B. Yaldiz,Ishit Mehta,Nithin Raghavan,Andreas Meuleman,Tzu-Mao Li,Ravi Ramamoorthi*

Main category: cs.GR

TL;DR: 提出一种优化神经场的方法，可在单次前向传播中进行预滤波，通过傅里叶特征嵌入的解析缩放实现输入域的卷积滤波，支持多种参数化滤波器，训练和推理都很快。


<details>
  <summary>Details</summary>
Motivation: 神经场擅长表示连续视觉信号，但通常只能在单一固定分辨率下操作，限制了其应用灵活性。

Method: 在输入域进行卷积滤波，通过分析性地缩放傅里叶特征嵌入与滤波器的频率响应；使用单样本蒙特卡洛估计训练神经场；支持高斯、Box和Lanczos等参数化滤波器。

Result: 在神经场滤波方面相比现有方法取得了定量和定性的改进，训练和推理速度都很快，且对网络架构没有额外约束。

Conclusion: 该方法为神经场提供了一种简单而强大的预滤波优化方案，显著提升了滤波性能和效率。
...
### [243] [Splat the Net: Radiance Fields with Splattable Neural Primitives](https://arxiv.org/abs/2510.08491)
*Xilong Zhou,Bao-Huy Nguyen,Loïc Magne,Vladislav Golyanik,Thomas Leimkühler,Christian Theobalt*

Main category: cs.GR

TL;DR: 提出了一种可拼接神经基元表示，将神经模型的表达能力与基元拼接的效率相结合，在保持3D高斯拼接质量和速度的同时，使用10倍更少的基元和6倍更少的参数。


<details>
  <summary>Details</summary>
Motivation: 解决辐射场表示中的权衡问题：神经辐射场表达能力强但渲染成本高，而基元方法（如3D高斯拼接）实时高效但表达能力有限。

Method: 使用有界神经密度场的可拼接神经基元，每个基元由浅层神经网络参数化，支持透视准确拼接核的解析计算，无需光线行进。

Result: 在新视角合成基准测试中，匹配3D高斯拼接的质量和速度，同时使用10倍更少的基元和6倍更少的参数。

Conclusion: 该表示直接解决了表达效率权衡问题，无需复杂控制框架，为实时高质量3D场景建模提供了新途径。
...
### [244] [X2Video: Adapting Diffusion Models for Multimodal Controllable Neural Video Rendering](https://arxiv.org/abs/2510.08530)
*Zhitong Huang,Mohan Zhang,Renhan Wang,Rui Tang,Hao Zhu,Jing Liao*

Main category: cs.GR

TL;DR: X2Video是首个基于内在通道（反照率、法线、粗糙度、金属度、辐照度）引导的扩散模型，可生成逼真视频，支持参考图像和文本提示的多模态控制，并采用混合自注意力确保时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型缺乏对颜色、材质、几何和光照的精确控制能力，且难以同时支持多模态输入和局部编辑。X2Video旨在通过内在通道指导实现精确的参数化控制，同时提供直观的参考图像和文本编辑方式。

Method: 1. 扩展XRGB图像生成模型到视频领域；2. 提出混合自注意力确保时间一致性；3. 开发掩码交叉注意力分离全局和局部文本提示；4. 采用递归采样方法结合关键帧预测和帧插值生成长视频；5. 构建InteriorVideo数据集支持训练。

Result: 定性和定量评估表明，X2Video能够生成长时间、时间一致且逼真的视频，有效支持多模态控制，并实现对颜色、材质、几何和光照的精确编辑。

Conclusion: X2Video成功实现了基于内在通道的视频生成，提供了精确的参数化控制和直观的多模态编辑能力，在保持时间一致性的同时支持长视频生成。
...
### [245] [Who Stole Your Data? A Method for Detecting Unauthorized RAG Theft](https://arxiv.org/abs/2510.07728)
*Peiyang Liu,Ziqiang Cui,Di Liang,Wei Ye*

Main category: cs.IR

TL;DR: 该论文提出了针对检索增强生成(RAG)系统的抄袭检测方法，包括构建RPD数据集和开发双层水印系统，以保护知识产权。


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然能减少LLM的幻觉和过时信息问题，但也容易导致大规模数据盗用，需要解决知识产权保护挑战。

Method: 1) 构建RPD数据集，涵盖多种专业领域和写作风格；2) 开发语义和词汇层面的双层水印系统；3) 采用审讯者-侦探框架进行统计假设检验。
...
### [246] [TaoSR-AGRL: Adaptive Guided Reinforcement Learning Framework for E-commerce Search Relevance](https://arxiv.org/abs/2510.08048)
*Jianhui Yang,Yiming Jin,Pengkun Jiao,Chenhe Dong,Zerui Huang,Shaowei Yao,Xiaojiang Zhou,Dan Ou,Haihong Tang*

Main category: cs.IR

TL;DR: 提出了TaoSR-AGRL框架，通过规则感知奖励塑造和自适应引导回放，解决LLM在电商搜索相关性预测中推理能力不足的问题，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在复杂业务规则和用户查询下缺乏鲁棒推理能力，特别是对长尾和挑战性案例。传统强化学习方法存在稀疏奖励问题，无法有效指导多步推理。

Method: TaoSR-AGRL框架包含两个关键创新：1) 规则感知奖励塑造 - 将最终相关性判断分解为与领域特定标准对齐的密集结构化奖励；2) 自适应引导回放 - 识别训练中的低准确率轨迹，注入针对性真实指导，引导策略远离违规推理模式。

Result: 在淘宝搜索的大规模真实数据集和在线A/B测试中，TaoSR-AGRL持续优于DPO和标准GRPO基线，在相关性准确性、规则遵守度和训练稳定性方面均有提升。

Conclusion: TaoSR-AGRL成功解决了LLM在电商搜索相关性预测中的推理能力瓶颈，已在淘宝主搜索场景部署，服务数亿用户。
...
### [247] [VersionRAG: Version-Aware Retrieval-Augmented Generation for Evolving Documents](https://arxiv.org/abs/2510.08109)
...
### [248] [ReasonEmbed: Enhanced Text Embeddings for Reasoning-Intensive Document Retrieval](https://arxiv.org/abs/2510.08252)
*Jianlyu Chen,Junwei Lan,Chaofan Li,Defu Lian,Zheng Liu*

Main category: cs.IR

TL;DR: ReasonEmbed是一个专为推理密集型文档检索设计的文本嵌入模型，通过ReMixer数据合成方法和Redapter自适应学习算法，在多个骨干网络上实现了优越性能，特别是在BRIGHT基准测试中创造了38.1的nDCG@10新记录。
...
Method: 1. ReMixer数据合成方法：克服传统合成数据集的平凡性问题，生成82K高质量训练样本；2. Redapter自适应学习算法：根据推理强度动态调整样本权重；3. 在多个不同规模的骨干网络上实现ReasonEmbed。

Result: ReasonEmbed-Qwen3-8B模型在BRIGHT基准测试中达到38.1的nDCG@10得分，显著优于现有文本嵌入模型。所有不同规模的ReasonEmbed模型在推理密集型检索任务中都表现出优越性能。

Conclusion: ReasonEmbed通过创新的数据合成和自适应学习算法，有效提升了推理密集型文档检索的性能，并将开源相关资源以推动该领域研究发展。
...
### [249] [IntentionVLA: Generalizable and Efficient Embodied Intention Reasoning for Human-Robot Interaction](https://arxiv.org/abs/2510.07778)
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在预训练阶段缺乏针对具身场景的推理密集型任务训练，无法处理现实世界中需要隐式人类意图推理的复杂交互。

Method: 采用课程训练范式：首先使用精心设计的推理数据进行训练，结合意图推理、空间定位和紧凑具身推理；然后在微调阶段使用紧凑推理输出作为动作生成的上下文指导。

Result: 在直接指令下成功率比π0高18%，在意图指令下比ECoT高28%；在分布外意图任务中成功率是基线方法的两倍以上，零样本人机交互成功率40%。

Conclusion: IntentionVLA为下一代人机交互系统提供了一个有前景的范式，显著提升了机器人对人类意图的理解和响应能力。
...
### [250] [Team Xiaomi EV-AD VLA: Learning to Navigate Socially Through Proactive Risk Perception -- Technical Report for IROS 2025 RoboSense Challenge Social Navigation Track](https://arxiv.org/abs/2510.07871)
*Erjia Xiao,Lingfeng Zhang,Yingbo Tang,Hao Cheng,Renjing Xu,Wenbo Ding,Lei Zhou,Long Chen,Hangjun Ye,Xiaoshuai Hao*

Main category: cs.RO

TL;DR: 本文介绍了参加IROS 2025 RoboSense挑战赛社会导航赛道的技术方案，通过在Falcon模型基础上增加主动风险感知模块，提升机器人在动态人群环境中的社交导航能力。


<details>
  <summary>Details</summary>
Motivation: 解决机器人在动态人群环境中导航时面临的安全、效率和社交合规性问题，特别是在仅使用机载传感器且无法获取全局地图的情况下实现主动避障。

Method: 在Falcon模型基础上引入主动风险感知模块，通过学习预测周围行人的基于距离的碰撞风险分数，增强空间感知能力和主动避障行为。

Result: 在Social-HM3D基准测试中，该方法显著提升了代理在拥挤室内场景中保持个人空间合规性的能力，在16个参赛团队中获得第2名。

Conclusion: 主动风险感知模块有效增强了机器人在动态人群环境中的社交导航性能，证明了基于风险预测的方法在提升导航安全性和社交合规性方面的有效性。
...
### [251] [NavSpace: How Navigation Agents Follow Spatial Intelligence Instructions](https://arxiv.org/abs/2510.08173)
*Haolin Yang,Yuxing Long,Zhuoyuan Yu,Zihan Yang,Minghan Wang,Jiapeng Xu,Yihan Wang,Ziyan Yu,Wenzhe Cai,Lei Kang,Hao Dong*

Main category: cs.RO

TL;DR: 提出了NavSpace基准测试，包含6个任务类别和1,228个轨迹-指令对，用于评估导航代理的空间智能。评估了22个导航代理，并提出了新的SNav模型，在NavSpace和真实机器人测试中表现优异。
...
Method: 引入NavSpace基准测试，包含6个任务类别和1,228个轨迹-指令对；提出SNav新模型。

Result: 评估了22个导航代理，揭示了空间智能在具身导航中的重要性；SNav模型在NavSpace和真实机器人测试中优于现有导航代理。

Conclusion: NavSpace基准测试为评估导航代理的空间智能提供了系统方法，SNav模型为未来工作建立了强基线。
...
### [252] [DexMan: Learning Bimanual Dexterous Manipulation from Human and Generated Videos](https://arxiv.org/abs/2510.08475)
*Jhen Hsieh,Kuan-Hsun Tu,Kuo-Han Hung,Tsung-Wei Ke*

Main category: cs.RO

TL;DR: DexMan是一个自动化框架，可将人类视觉演示转换为仿真中类人机器人的双手灵巧操作技能，无需相机标定、深度传感器、3D对象资源或真实手部物体运动标注。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只考虑简化的浮动手部，需要大量手动数据收集和昂贵的动作捕捉设备，限制了灵巧操作技能的规模化发展。

Method: 基于第三人称人类操作视频，使用接触奖励改进策略学习，直接从噪声手部物体姿态估计中学习，控制完整类人机器人。

Result: 在TACO基准测试中物体姿态估计达到SOTA性能，ADD-S和VSD分别提升0.08和0.12；在OakInk-v2上成功率比先前方法提高19%。

Conclusion: DexMan能从真实和合成视频生成技能，无需手动数据收集和昂贵动作捕捉，为训练通用灵巧操作创建大规模多样化数据集。
...
### [253] [R2RGEN: Real-to-Real 3D Data Generation for Spatially Generalized Manipulation](https://arxiv.org/abs/2510.08547)
*Xiuwei Xu,Angyuan Ma,Hankun Li,Bingyao Yu,Zheng Zhu,Jie Zhou,Jiwen Lu*

Main category: cs.RO

TL;DR: 提出R2RGen框架，通过直接增强点云观测-动作对来生成真实世界数据，实现无需模拟器和渲染的真实到真实3D数据生成，提升机器人操作策略的空间泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决机器人操作中空间泛化问题，传统方法需要大量人工演示数据覆盖不同空间配置，现有数据生成方法存在显著的模拟到真实差距，且受限于固定基座和预定义相机视角等约束设置。

Method: 提出R2RGen框架：1）单源演示的细粒度场景和轨迹解析注释机制；2）处理复杂多对象组合和多样化任务约束的分组增强策略；3）与真实世界3D传感器分布对齐的相机感知处理。

Result: R2RGen在广泛实验中显著提升数据效率，在移动操作中展现出强大的扩展和应用潜力。

Conclusion: R2RGen是一个高效即插即用的真实到真实3D数据生成框架，无需模拟器和渲染，能够有效增强机器人操作策略的空间泛化能力。
...
### [254] [DexNDM: Closing the Reality Gap for Dexterous In-Hand Rotation via Joint-Wise Neural Dynamics Model](https://arxiv.org/abs/2510.08556)
*Xueyi Liu,He Wang,Li Yi*

Main category: cs.RO

TL;DR: 提出一种新颖的sim-to-real框架，通过联合动态模型学习现实差距，使在仿真中训练的单策略能够在现实世界中泛化到各种物体和条件。


<details>
  <summary>Details</summary>
Motivation: 解决机器人手内物体旋转的泛化难题，主要挑战在于从仿真到现实世界的策略迁移困难，复杂的接触动力学造成了'现实差距'。

Method: 使用联合动态模型学习现实差距，通过因子化关节动态、压缩系统影响为低维变量，并配合全自动数据收集策略获取多样化的真实交互数据。

Result: 单一策略成功旋转具有复杂形状、高宽比和小尺寸的挑战性物体，同时处理多样的手腕方向和旋转轴，在真实世界评估中表现出色。

Conclusion: 该方法展示了前所未有的泛化能力，通过高效的动态建模和自主数据收集，有效解决了sim-to-real挑战。
...
### [255] [NovaFlow: Zero-Shot Manipulation via Actionable Flow from Generated Videos](https://arxiv.org/abs/2510.08568)
*Hongyu Li,Lingfeng Sun,Yafei Hu,Duy Ta,Jennifer Barry,George Konidaris,Jiahui Fu*

Main category: cs.RO

TL;DR: NovaFlow是一个零样本机器人操作框架，通过视频生成模型将任务描述转换为可执行计划，无需演示或特定平台训练，支持刚性、关节和可变形物体的跨平台操作。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作方法通常需要任务分布内数据或特定平台的微调，限制了跨平台迁移能力。NovaFlow旨在实现零样本的跨平台自主操作。

Method: 使用视频生成模型将任务描述合成视频，通过感知模块提取3D对象流，为刚性物体计算相对位姿并转换为机器人动作，为可变形物体使用基于粒子的动力学模型进行规划。

Result: 在桌面Franka机械臂和Spot四足移动机器人上验证了刚性、关节和可变形物体操作任务的有效零样本执行，无需演示或特定平台训练。

Conclusion: NovaFlow通过将任务理解与底层控制解耦，实现了跨平台的零样本机器人操作，为通用机器人操作提供了新思路。
...
### [256] [Scalable Offline Metrics for Autonomous Driving](https://arxiv.org/abs/2510.08571)
*Animikh Aich,Adwait Kulkarni,Eshed Ohn-Bar*

Main category: cs.RO

TL;DR: 本文重新评估了感知规划模型在离线与在线评估之间的相关性，发现两者相关性比先前研究更差，并提出基于认知不确定性的离线指标，显著提升了相关性。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶等机器人系统的感知规划模型主要通过离线评估进行验证，但离线性能与在线实际表现之间的关联性研究不足，这导致难以准确预测模型在真实环境中的表现。

Method: 通过大量仿真实验分析离线与在线评估的相关性，提出基于认知不确定性的离线指标来捕捉可能导致闭环错误的事件，并在真实环境中验证该方法的泛化能力。

Result: 发现离线与在线评估的相关性比先前研究更差，基于认知不确定性的新指标相比传统离线指标提升了13%以上的相关性，在真实环境中表现更佳。

Conclusion: 当前自动驾驶策略评估实践存在局限性，基于认知不确定性的离线指标能更好地桥接离线与在线评估的差距，为更可靠的模型评估提供新思路。
...
### [257] [Pseudo2Real: Task Arithmetic for Pseudo-Label Correction in Automatic Speech Recognition](https://arxiv.org/abs/2510.08047)
*Yi-Cheng Lin,Yu-Hsuan Li Liang,Hsuan Su,Tzu-Quan Lin,Shang-Tse Chen,Yun-Nung Chen,Hung-yi Lee*

Main category: eess.AS

TL;DR: 该论文提出了一种参数空间校正方法，通过比较在真实标签和伪标签上微调的两个ASR模型的权重差异来纠正伪标签中的系统性偏见，从而在无目标域真实标签的情况下提升跨域ASR的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界ASR系统面临领域偏移问题，特别是在未见过的口音和领域上，而伪标签方法虽然实用但会引入系统性、特定口音的错误，传统过滤方法无法有效修正这些偏见。

Method: 在源域中，从相同初始化开始微调两个ASR模型：一个使用真实标签，另一个使用伪标签。计算它们的权重差异作为校正向量，该向量捕获了伪标签偏见。然后将此向量应用于伪标签目标模型以提升识别性能。

Result: 在AfriSpeech-200数据集上，使用Whisper tiny模型对10种非洲口音进行测试，该方法实现了高达35%的相对词错误率(WER)降低。

Conclusion: 提出的参数空间校正方法能够有效纠正伪标签中的系统性偏见，显著提升跨域ASR性能，特别是在处理未见口音时效果显著。
...
### [258] [FlowLensing: Simulating Gravitational Lensing with Flow Matching](https://arxiv.org/abs/2510.07878)
*Hamees Sayed,Pranath Reddy,Michael W. Toomey,Sergei Gleyzer*

Main category: astro-ph.IM

TL;DR: FlowLensing是一个基于扩散变换器的紧凑高效流匹配模型，用于强引力透镜模拟，相比传统模拟器速度提升200倍以上，支持离散和连续参数处理。


<details>
  <summary>Details</summary>
Motivation: 现有引力透镜模拟工具依赖光线追踪或前向建模流程，虽然精确但速度极慢，无法满足大规模暗物质研究的需求。

Method: 采用基于扩散变换器的流匹配模型，在离散和连续两种模式下运行，处理不同暗物质模型类别和连续模型参数，确保物理一致性。

Result: 模型在密集暗物质模型上相比传统模拟器实现200倍以上的加速，具有高保真度和低推理延迟。

Conclusion: FlowLensing实现了快速、可扩展且物理一致的图像合成，为传统前向建模流程提供了实用替代方案，可推动暗物质子结构探测研究。
...
### [259] [From Keywords to Clusters: AI-Driven Analysis of YouTube Comments to Reveal Election Issue Salience in 2024](https://arxiv.org/abs/2510.07821)
*Raisa M. Simoes,Timoteo Kelly,Eduardo J. Simoes,Praveen Rao*

Main category: cs.SI

TL;DR: 使用AI技术分析YouTube用户评论，发现移民和民主问题是2024年总统选举中最受关注的问题，而通货膨胀的重要性被高估。


<details>
  <summary>Details</summary>
Motivation: 探索哪种数据科学方法能更准确地识别影响选民决策的关键问题，比较AI驱动的意见挖掘与传统调查方法的有效性。

Method: 采用两种自然语言处理和聚类分析方法，分析8000多条来自《华尔街日报》和《纽约时报》YouTube频道选举相关视频的用户评论。

Result: 移民和民主是最频繁被提及的问题，身份政治次之，通货膨胀的提及频率显著较低。

Conclusion: 基于原始用户数据的意见挖掘方法比传统民调和调查更能揭示选举结果的关键因素。
