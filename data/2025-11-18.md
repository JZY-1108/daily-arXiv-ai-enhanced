...
TL;DR: TimeStampEval是一个用于从长文本记录中检索非逐字引用的精确时间戳的基准测试。通过两阶段方法显著提高检索准确率，同时降低90%以上的推理成本。


<details>
  <summary>Details</summary>
Motivation: 解决传统模糊匹配在处理语义相同但语法不同的引用时的失败问题，特别是在对齐官方书面记录和语音转文字记录的场景中。

Method: 采用两阶段方法：首先使用RapidFuzz进行预过滤，然后使用LLM在短片段上进行验证。优化提示设计，将查询放在文本之前并使用紧凑格式。

Result: 该方法将模糊匹配准确率提高了50个百分点，同时将延迟减半，每个正确结果的成本降低了96%。在弱设置下准确率从37%提升到77%，在强设置下超过90%。

Conclusion: 该方法对文本长度、词汇漂移和领域变化具有鲁棒性，在10个文本（50k-900k tokens）上保持95-100%的拒绝准确率。
...
TL;DR: MiroThinker v1.0是一个开源研究代理，通过交互式扩展作为模型性能提升的第三维度，实现深度工具调用和环境交互，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统方法仅通过增加模型大小或上下文长度来提升性能，但存在推理链过长时性能下降的风险。MiroThinker探索在模型层面进行交互扩展，利用环境反馈和外部信息获取来纠正错误和优化推理轨迹。

Method: 通过强化学习实现交互扩展，在256K上下文窗口下支持每个任务最多600次工具调用，实现持续多轮推理和复杂研究流程。

Result: 72B变体在GAIA、HLE、BrowseComp和BrowseComp-ZH四个基准测试中分别达到81.9%、37.7%、47.1%和55.6%的准确率，超越先前开源代理并接近GPT-5-high等商业模型。

Conclusion: 交互扩展与模型容量和上下文窗口一样，是构建下一代开源研究代理的第三个关键维度，交互深度展现出与模型大小和上下文长度类似的扩展行为。
...
TL;DR: 本文质疑语言模型是否真正具备推理能力，认为其输出只是统计模式匹配而非真正的逻辑推理机制


<details>
  <summary>Details</summary>
Motivation: 评估语言模型推理能力的定义，揭示当前NLP领域对"推理"概念的使用与语言模型实际工作原理之间的不一致性

Method: 假设基于transformer的语言模型实现了隐式有限阶马尔可夫核，将上下文映射到条件标记分布

Result: 语言模型类似推理的输出对应于学习核中的统计规律性和近似统计不变性，而非显式逻辑机制

Conclusion: 语言模型是"统计模式匹配器"而非真正的推理者，这一区分对于评估语言模型中的认知不确定性至关重要
...
TL;DR: 评估7个开源大语言模型(0.6B-70B参数)在水电许可文档信息提取中的性能与计算资源权衡，发现14B参数是有效验证的阈值，为监管文档的信息提取提供了首个全面的资源-性能映射指南。


<details>
  <summary>Details</summary>
Motivation: 监管文档信息提取在大语言模型应用中面临性能与计算资源的关键权衡问题，需要为实际部署提供实证指导。

Method: 使用7个不同参数规模(0.6B-70B)的开源大语言模型，在水电许可文档上进行信息提取评估，分析验证方法的有效性。

Result: 发现14B参数阈值：低于此阈值时验证方法无效(F1<0.15)，高于此阈值时验证可行(F1=0.64)。消费级模型通过适当验证可达64% F1，小模型停滞在51%，大规模模型接近77%但需企业基础设施。小模型存在系统性幻觉模式。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏验证LLM生成输出准确性的正式方法，特别是在将自然语言需求转换为结构化格式时。

Method: 使用简单的LLM自动形式化器，将自然语言需求和LLM生成输出都转换为形式逻辑，然后进行逻辑等价性验证。

Result: 实验一成功识别了两个不同表述的自然语言需求的逻辑等价性；实验二识别了自然语言需求与LLM生成输出之间的逻辑不一致性。
...
TL;DR: 提出一个分析电影剧本情感弧线并基于角色上下文进行扩展分析的框架，使用基于NRC-VAD数据集的定制词典进行情感分析，并通过Ward层次聚类技术聚类相似情感曲线。


<details>
  <summary>Details</summary>
Motivation: 故事理解和分析是自然语言理解中的长期挑战领域，需要深度计算语义表示和句法处理。大量叙事数据需要自动化语义分析和计算学习而非手动分析方法。

Method: 使用基于NRC-VAD数据集（Valence、Arousal、Dominance评分）构建的定制词典进行词典式情感分析，应用LabMTsimple storylab模块，并通过Ward层次聚类技术聚类相似情感曲线。

Result: 在电影数据集上的实验评估表明，该分析结果有助于消费者和读者在选择叙事或故事时做出决策。

Conclusion: 该框架能够提取叙事中传达的高层次和低层次概念，为故事分析提供了有效的自动化解决方案。
...
TL;DR: 该研究构建了一个包含6,393份放射学报告的标注语料库，用于评估各种模型在随访依从性检测任务上的性能，发现经过优化的GPT-4o和GPT-OSS-20B模型表现最佳，接近人类标注者水平。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏专门用于评估LLMs在放射学任务性能的领域特定数据集，需要构建一个高质量的标注语料库来系统比较传统机器学习方法和生成式LLMs在随访依从性检测任务上的表现。

Method: 使用6,393份放射学报告构建标注语料库，系统比较了逻辑回归、支持向量机、Longformer、全微调Llama3-8B-Instruct以及GPT-4o和GPT-OSS-20B等生成式LLMs。对生成式LLMs测试了基础配置和任务优化配置，并对GPT-OSS-20B进行了提示优化。

Result: GPT-4o（优化配置）表现最佳（F1=0.832），GPT-OSS-20B（优化配置）紧随其后（F1=0.828）。逻辑回归和支持向量机也表现良好（F1分别为0.776和0.775）。人类标注者间一致性很高（F1=0.846）。

Conclusion: 虽然通过提示优化，LLMs可以接近人类水平的标注一致性，但可解释且资源效率高的传统模型仍然是重要的基准方法。优化后的生成式LLMs在放射学随访依从性检测任务上表现出色。
...
TL;DR: MedPT是首个面向巴西葡萄牙语的大规模真实医疗问答数据集，包含38.4万条医患互动数据，通过多阶段筛选和LLM标注增强，支持医疗专科路由等任务，旨在解决医疗LLM在低资源语言中的发展障碍。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在医疗领域的发展主要集中于高资源语言，简单翻译无法捕捉独特的临床和文化细微差别（如地方性疾病），这为其他语言群体创造了关键障碍。

Method: 构建了38.4万条真实医患问答对的数据集，采用混合定量-定性分析的多阶段筛选协议过滤噪声，并通过LLM驱动的标注将问题分类为7种语义类型以捕捉用户意图。

Result: 数据集涵盖3,200个主题，展现了医患沟通的自然不对称性。在医疗专科路由任务中，微调1.7B参数模型在20类设置下达到了94%的F1分数。错误分析显示误分类反映了真实的临床模糊性。

Conclusion: MedPT数据集公开释放，旨在促进葡萄牙语世界更公平、准确和具有文化意识的医疗技术发展。
...
TL;DR: ClinStructor使用LLM将临床自由文本转换为结构化问答对，解决临床笔记中的偏见、泛化性和可解释性问题，在ICU死亡率预测任务中性能仅轻微下降2-3%。
...
Result: 在ICU死亡率预测任务中，相比直接微调，性能仅下降2-3%（AUC指标），同时显著提高了透明度和可控性。
...
TL;DR: 该研究通过监督微调和强化学习技术增强GPT-2的心理治疗对话生成能力，显著提升了情感准确性和专业响应质量。


<details>
  <summary>Details</summary>
Motivation: COVID-19加剧了心理健康服务的可及性挑战，预训练语言模型缺乏情境和情感意识，需要开发更合适的治疗对话系统。
...
TL;DR: CALM是一个可解释的临床文本分类框架，通过加法模型将预测结果分解为各语义组件的贡献，提供忠实解释和可视化。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在临床文本分类中预测不透明的问题，满足研究人员和医生需要了解患者记录中哪些部分驱动风险信号的需求。

Method: 使用加法LLM框架，将半结构化文本输入分解为语义组件，预测结果为各组件贡献的加和，实现忠实解释。

Result: CALM在保持与传统LLM分类器相当性能的同时，提高了可信度，支持质量检查，并在模型开发和审计中揭示临床有意义的模式。

Conclusion: CALM为临床文本分类提供了可解释的解决方案，平衡了性能和可解释性，有助于在研究和临床环境中的实际应用。
...
TL;DR: 提出了InData数据集来评估LLM的多步骤工具推理能力，发现当前LLM在复杂数据分析任务中表现不佳，特别是在多步骤推理方面存在明显短板。


<details>
  <summary>Details</summary>
Motivation: 解决LLM直接生成和执行代码访问敏感数据的安全风险，通过预定义的安全工具限制LLM的数据访问权限。

Method: 引入Indirect Data Engagement (InData)数据集，包含三个难度级别的数据分析问题，评估LLM使用工具进行多步骤推理的能力。

Result: 测试15个开源LLM发现，大型模型在简单任务上准确率达97.3%，但在困难任务上降至69.6%，表明当前LLM缺乏稳健的多步骤工具推理能力。
...
TL;DR: 本文提出LLM-KAT评估方法和实体匿名化技术，以改善大语言模型在知识图谱对话生成中对外部知识的利用能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在知识图谱对话生成任务中过度依赖内部知识，即使提供完美检索的知识图谱也难以有效利用外部知识，导致生成回复与知识图谱脱节。

Method: 1. 提出LLM-KAT评估程序来测量生成回复中的知识附着度；2. 设计简单有效的实体匿名化技术，鼓励LLM更好地利用外部知识。

Result: 在OpenDialKG数据集上的实验表明，该方法显著提高了LLM对外部知识的附着能力。

Conclusion: 通过实体匿名化和专门的评估方法，可以有效提升大语言模型在知识图谱对话生成中对外部知识的利用效果。
...
TL;DR: 本文研究语言模型的熵校准问题，发现模型存在校准错误，且随着模型规模增大，校准问题改善缓慢。理论分析表明校准改善速度取决于数据分布的幂律指数，实证研究显示从5亿到700亿参数的模型都存在相似的错误累积率。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型生成过程中熵校准问题，探索模型规模扩大是否能改善校准错误，以及是否存在无需牺牲多样性的校准方法。

Method: 首先在简化理论设置中分析校准错误随数据集规模的缩放行为，然后实证测量从0.5B到70B参数语言模型的校准错误。

Result: 发现校准错误的缩放指数接近0，意味着更大模型与较小模型以相似速率累积错误，这解释了为什么对更大模型仍需要相似的截断采样。

Conclusion: 理论上证明通过预测文本未来熵的黑盒模型可以减少熵同时保持对数损失，但实际中校准问题随规模改善缓慢，截断方法存在多样性损失。
...
TL;DR: 提出ReasoningNER框架，通过显式推理机制改进生成式LLM在命名实体识别中的性能，特别是在零样本和低资源场景下表现出色


<details>
  <summary>Details</summary>
Motivation: 生成式LLM在NER任务中通常通过指令调优提升性能，但缺乏显式、可验证的推理机制，导致在零样本和低资源场景下表现不佳

Method: 三阶段推理框架：1）生成带有NER导向思维链的数据集；2）使用思维链调优模型，在得出最终答案前生成连贯的推理过程；3）推理增强阶段，使用综合奖励信号优化推理过程

Result: 在零样本设置下达到SOTA性能，F1分数比GPT-4高出12.3个百分点，展现出在NER任务中的强大认知能力

Conclusion: ReasoningNER框架通过将提取范式从隐式模式匹配转向显式推理，证明了其在推理导向信息提取研究中的巨大潜力
...
TL;DR: CoT解释在道德场景中具有双重作用：既能提高透明度，也可能因用户只看结果而忽视错误推理，导致确认偏见。自信的语气会抑制错误检测但维持依赖。


<details>
  <summary>Details</summary>
Motivation: 研究CoT解释在道德场景中的双重作用，揭示解释如何影响用户信任和错误检测能力，特别是当推理存在缺陷时。

Method: 通过系统性地扰动推理链和操纵表达语气，分析视觉语言模型中的推理错误及其对用户信任和错误检测的影响。

Result: 发现两个关键效应：(1)用户将信任等同于结果一致性，即使推理有缺陷也持续依赖；(2)自信语气抑制错误检测但维持依赖，表明表达风格能凌驾于正确性之上。
...
TL;DR: 本文提出了一个用于评估大语言模型文化能力的新基准，通过真实情境上下文测试文化推理能力，并引入四个补充指标来全面评估响应质量。研究发现传统评估方法会高估文化能力，而新的厚评估方法能提供更稳定、可解释的文化理解信号。
...
Method: 开发了一套新的基准测试，通过呈现真实情境上下文来评估文化推理能力，除了标准精确匹配指标外，还引入了覆盖率、特异性、内涵和连贯性四个补充指标。

Result: 实证分析显示，传统薄评估会系统性高估文化能力且评估结果不稳定、方差大；而厚评估能暴露推理深度的差异，减少方差，提供更稳定可解释的文化理解信号。

Conclusion: 厚评估方法比传统评估更能准确衡量大语言模型的文化能力，为模型在多元文化环境中的部署提供了更可靠的评估框架。
...
TL;DR: 结合回译和微调技术在小规模日语语料上显著提升英日神经机器翻译质量，COMET分数从0.460提升至0.597。
...
Method: 首先使用回译技术生成合成数据，然后在真实的小型平行数据集上进行微调，最后将两种技术结合使用。

Result: 单独使用回译COMET=0.468，单独微调COMET=0.589，两者结合达到COMET=0.597，显著优于单独使用任一技术。

Conclusion: 回译和微调的协同使用为低资源语言对提供了一种轻量级但强大的翻译质量提升策略。
...
TL;DR: LLMLagBench是一个用于评估大语言模型训练数据时间边界的基准测试，通过检测模型对近期事件的认知来确定其知识的新鲜程度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在特定时间点前的文本数据上预训练，这形成了严格的知识边界。当这一限制被忽视时，模型可能在推理过程中无意间混合过时的时效性信息与通用知识，从而影响回答准确性。

Method: 引入LLMLagBench基准测试，通过评估模型对近期事件的知识来系统识别其训练数据的最早可能时间边界。该方法应用于大量LLM，包括明确声明和未声明训练截止时间的模型。

Result: 基准测试的可靠性通过人工验证和与公开的LLM预训练信息对比进行评估。

Conclusion: LLMLagBench提供了一种系统方法来识别LLM的知识时间边界，有助于理解模型的知识局限性和潜在的信息过时问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究存在伪多模态（仅源帖子包含视觉线索，评论被当作纯文本）和用户同质化（忽略个人特质对立场表达的影响）的局限性，需要更符合真实世界多模态交互的解决方案。
...
Conclusion: 用户中心的多模态方法和上下文基础推理对于现实立场理解至关重要，PRISM为解决多模态对话立场检测的关键挑战提供了有效方案。
...
TL;DR: 本文提出了AI-Salesman框架，通过构建真实电话销售数据集和双阶段架构，解决了目标驱动对话中的策略规划和事实保真度问题。


<details>
  <summary>Details</summary>
Motivation: 目标驱动的说服性对话（如电话销售）需要复杂的多轮规划和严格的事实保真度，但现有LLMs存在策略脆弱性和事实幻觉问题，且缺乏特定任务数据。

Method: 构建TeleSalesCorpus数据集；提出AI-Salesman双阶段框架：训练阶段使用贝叶斯监督强化学习从噪声对话中学习策略，推理阶段使用动态大纲引导代理(DOGA)结合脚本库提供动态策略指导。

Result: AI-Salesman在自动指标和人工评估中显著优于基线模型，在复杂说服场景中表现出色。

Conclusion: 该框架有效解决了目标驱动说服对话中的关键挑战，为类似应用提供了实用解决方案。
...
TL;DR: VBackChecker是一个基于"眼见为实"原则的无参考幻觉检测框架，通过像素级Grounding LLM验证MLLM生成响应与视觉输入的一致性，在R²-HalBench基准上达到SOTA性能。
...
Method: 利用具备推理和分割能力的像素级Grounding LLM，设计R-Instruct指令调优数据生成流程，包含丰富上下文描述、定位掩码和困难负样本。

Result: 在R²-HalBench基准上超越先前复杂框架，性能媲美GPT-4o，像素级定位任务提升超过10%。

Conclusion: VBackChecker为MLLM幻觉检测提供了有效且可解释的解决方案，在真实世界丰富场景中表现优异。
...
TL;DR: CriticSearch是一个细粒度信用分配框架，通过回顾性批评机制提供密集的回合级反馈，解决了搜索代理中稀疏奖励导致的低效探索和不稳定训练问题。


<details>
  <summary>Details</summary>
Motivation: 现有搜索代理管道依赖强化学习优化，但稀疏结果奖励导致探索效率低和训练不稳定，需要更密集的反馈机制。

Method: 使用冻结的非对称批评LLM，基于完整轨迹和黄金答案的优先信息回顾性评估每个回合，将评估转化为稳定的密集奖励来指导策略改进。

Result: 在多样化多跳推理基准测试中，CriticSearch始终优于现有基线方法，实现了更快的收敛速度、更高的训练稳定性和更好的性能。

Conclusion: CriticSearch通过密集的回合级反馈机制有效解决了搜索代理训练中的稀疏奖励问题，显著提升了模型性能和训练效率。
...
TL;DR: MME-RAG是一个多管理器-专家检索增强生成框架，通过类型级判断和跨度级提取的两阶段协调方法，解决细粒度实体识别中的领域适应性和检索可控性问题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在面向任务的对话中，细粒度实体识别仍面临领域适应性和检索可控性的挑战。

Method: 将实体识别分解为两个协调阶段：轻量级管理器进行类型级判断，专业专家进行跨度级提取，每个专家配备KeyInfo检索器注入语义对齐的少样本示例。

Result: 在CrossNER、MIT-Movie、MIT-Restaurant和新构建的多领域客服数据集上，MME-RAG在大多数领域表现优于基线方法。
...
TL;DR: CONFACTCHECK是一种高效的幻觉检测方法，通过检查生成文本中事实探针响应的一致性来检测幻觉，无需外部知识库，减少API调用和资源消耗。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成文本时经常产生事实错误的幻觉，这在医疗、金融等领域存在严重风险。现有方法需要多次API调用，增加了延迟和成本。

Method: 基于事实探针响应一致性的直觉，检查单个LLM内部和不同LLM之间对生成文本中事实探针的响应是否一致，无需外部知识库。

Result: 在多个数据集上的实验表明，CONFACTCHECK能以更少资源高效检测幻觉事实，在相似条件下比现有基线方法获得更高准确率。

Conclusion: CONFACTCHECK提供了一种资源高效的幻觉检测解决方案，特别适用于API受限或资源受限的环境。
...
TL;DR: 提出了ViConBERT框架，通过对比学习和基于词汇释义的蒸馏来学习越南语上下文嵌入，并创建了首个大规模越南语语义理解评估数据集ViConWSD。


<details>
  <summary>Details</summary>
Motivation: 越南语缺乏强大的语义理解模型和评估资源，而现有进展主要集中在英语等高资源语言。

Method: 结合对比学习(SimCLR)和基于词汇释义的蒸馏来学习越南语上下文嵌入，并构建合成数据集ViConWSD进行评估。

Result: ViConBERT在WSD任务上F1=0.87，在ViCon数据集上AP=0.88，在ViSim-400上Spearman's rho=0.60，表现优于基线模型。

Conclusion: ViConBERT能有效建模越南语的离散词义和分级语义关系，为低资源语言的语义理解提供了可行方案。
...
TL;DR: 提出使用小型LLM压缩大型LLM输入的新范式，开发了Cmprsr模型，在保持语义信息和遵循压缩率方面表现优异，可有效降低使用黑盒LLM的成本。


<details>
  <summary>Details</summary>
Motivation: 降低使用黑盒大型语言模型的高昂成本，通过压缩输入来减少计算开销。

Method: 使用小型LLM压缩大型LLM输入；通过Textgrad优化压缩元提示；对Qwen3-4B进行监督微调和GRPO训练，实现压缩率遵循和下游任务性能最大化。

Result: Cmprsr在MeetingBank、LongBench和GSM8k等数据集上优于提取式和普通抽象压缩方法，能精确控制压缩率，在不同输入长度和领域具有良好泛化性。

Conclusion: Cmprsr模型在保持语义完整性和压缩率控制方面表现优异，为LLM成本优化提供了有效解决方案。
...
TL;DR: 本文提出了一种利用现有抽象摘要自动生成抽取式摘要的轻量级方法，旨在为法律案例摘要研究社区创建包含两种摘要类型的增强数据集。


<details>
  <summary>Details</summary>
Motivation: 法律判决摘要对法律从业者认知负担重，自动摘要研究受关注。但神经方法生成的抽象摘要容易误解法律术语或忽略关键细节，因此抽取式摘要器使用趋势上升。人工标注抽取式摘要成本高，需要自动化解决方案。

Method: 设计轻量透明的流程，利用现有抽象摘要自动生成对应的抽取式摘要版本，确保专家意见从原始抽象摘要传递到转换后的抽取式摘要中。

Result: 计划增强七个现有案例摘要数据集，加入对应的抽取式摘要，创建丰富的法律案例摘要数据资源。通过结构、词汇和语义维度的广泛比较评估来确保质量。
...
TL;DR: 本研究探讨了LLMs与人类在抢答式测验中的难度差异，发现LLMs在维基百科未覆盖的问题和需要数值答案的问题上表现较差。


<details>
  <summary>Details</summary>
Motivation: 虽然LLMs在许多NLP任务中超越人类表现，但尚不清楚对人类困难的问题是否对LLMs同样困难。

Method: 收集日本测验数据（包括问题、答案和人类正确率），在多种设置下让LLMs回答测验，并从两个分析视角比较其与人类的正确率。

Result: 实验结果显示，相比人类，LLMs在维基百科未覆盖答案的问题和需要数值答案的问题上表现更差。

Conclusion: LLMs与人类在问题难度上存在差异，特别是在知识覆盖范围和数值计算方面。
...
TL;DR: 该研究探讨了大型语言模型中的讽刺反弹现象——否定指令反而会增加被禁止概念的可及性。通过两个实验发现，否定后立即出现反弹，语义干扰会强化反弹，而重复则支持抑制。极性分离程度与反弹持续性相关。


<details>
  <summary>Details</summary>
Motivation: 人类思维中的讽刺反弹现象（否定指令反而激活被禁止概念）可能在LLMs中同样存在，因为抑制概念需要内部激活它，这可能导致反弹而非避免。

Method: 两个实验：(1) 负载与内容：在否定指令后变化干扰文本类型（语义、句法、重复）并测量反弹强度；(2) 极性分离：测试模型是否能区分同一概念的中性与负面框架，以及这种分离是否预测反弹持续性。还进行了电路追踪分析。

Result: 否定后立即出现反弹，较长或语义干扰会强化反弹，而重复支持抑制。更强的极性分离与更持久的反弹相关。电路分析发现中间层注意力头放大被禁止token，而早期层抑制。

Conclusion: 研究将认知预测的讽刺反弹与长上下文干扰的机制洞察联系起来，并发布了ReboundBench数据集以支持未来研究。
...
TL;DR: ILAKKANAM是首个泰米尔语专用语言评估基准，基于820道斯里兰卡学校考试题目构建，评估显示LLMs在低年级表现良好但随着语言复杂度增加而下降，开源模型表现落后。


<details>
  <summary>Details</summary>
Motivation: 现有多语言基准依赖英语翻译数据集，无法捕捉泰米尔语等低资源、形态丰富语言的语言文化细微差别，需要专门评估基准。

Method: 使用820道斯里兰卡学校泰米尔语考试题目，由训练有素的语言学家按5个语言类别和1个事实知识类别标注，涵盖1-13年级，采用标准化评估框架评估闭源和开源LLMs。

Result: Gemini 2.5表现最佳，开源模型落后；所有模型在低年级题目表现良好，但随着语言复杂度增加明显下降；模型整体表现与识别语言类别能力无强相关性。

Conclusion: LLMs在泰米尔语的语言能力可能更多源于数据暴露而非真正理解，需要改进对形态丰富语言的语言基础能力。
...
TL;DR: 本文提出MRMBench基准和推理时探测方法，用于评估奖励模型在多维度偏好上的表现，发现奖励模型在捕捉多维度偏好方面存在困难，并证明该方法与LLM对齐性能强相关。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型评估方法通常在固定成对排序测试集上进行，但无法提供各偏好维度的性能信息，因此需要更细粒度的评估方法。

Method: 构建MRMBench基准（包含6个不同偏好维度的探测任务），并提出推理时探测方法用于识别奖励预测中使用的维度并增强可解释性。

Result: 实验表明MRMBench与大型语言模型对齐性能强相关，奖励模型在多维度偏好捕捉方面表现不佳，推理时探测方法能可靠评估奖励预测置信度。
...
TL;DR: SerenQA框架评估LLMs在科学知识图谱问答中发现意外见解的能力，重点关注药物重定位任务中的惊喜和新颖答案。


<details>
  <summary>Details</summary>
Motivation: 现有KGQA系统通常返回高度相关但可预测的答案，缺乏发现惊喜和意外洞察的能力，这是KGQA中缺失但期望的功能。

Method: 提出SerenQA框架，包括基于相关性、新颖性和惊喜度的严格惊喜度指标，以及从临床知识图谱构建的专家标注基准，涵盖知识检索、子图推理和惊喜探索三个子任务。

Result: 实验表明，最先进的LLMs在检索任务上表现良好，但在识别真正令人惊喜和有价值的发现方面仍存在困难。

Conclusion: LLMs在发现惊喜洞察方面仍有显著改进空间，SerenQA为评估和提升这一能力提供了资源和基准。
...
TL;DR: SGuard-v1是一个轻量级的大语言模型安全护栏系统，包含ContentFilter和JailbreakFilter两个专门模型，用于检测有害内容和筛选对抗性提示，支持12种语言，在保持轻量化的同时实现最先进的安全性能。
...
Method: 基于2B参数的Granite-3.3-2B-Instruct模型构建，通过指令调优训练两个专门组件：ContentFilter基于MLCommons危险分类学识别安全风险，JailbreakFilter通过精心设计的课程学习覆盖60种主要攻击类型。

Result: 在公共和专有安全基准测试中达到最先进的安全性能，同时保持轻量化部署，提供多类安全预测和二元置信度分数以提高可解释性。

Conclusion: SGuard-v1作为一个轻量级安全护栏系统，有效提升了LLM的安全性，并通过Apache-2.0许可证发布以促进进一步研究和实际部署。
...
TL;DR: QA-Noun是一个基于问答的框架，专门用于捕捉名词中心的语义关系，通过9个问题模板覆盖名词的显式句法和隐式上下文角色，与QA-SRL结合实现句子意义的统一分解。


<details>
  <summary>Details</summary>
Motivation: 现有的基于QA的语义方法主要关注谓词-论元关系，但对名词中心的语义关系处理不足，需要开发专门的方法来捕捉名词的语义角色。

Method: 定义9个问题模板来覆盖名词的显式和隐式语义角色，创建包含2000多个标注名词的数据集，并训练与QA-SRL集成的模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG方法由于输入上下文窗口限制而截断外部知识，导致信息丢失、响应幻觉和推理链断裂，同时检索的非结构化知识引入无关细节，影响准确推理。

Method: 提出意图驱动路由机制到领域特定提取模板，结合监督微调和基于强化学习的隐式提取机制，确保简洁、连贯且非冗余的知识集成。

Result: 在六个公共基准测试和一个真实业务基准测试（NowNewsQA）上，使用三个骨干模型进行评估，TAdaRAG在不同领域和长文本任务中均优于现有方法。

Conclusion: TAdaRAG展示了强大的泛化能力和实际有效性，能够有效解决传统RAG的知识截断和无关信息问题。
...
TL;DR: 提出一种因果框架来分析和缓解RLHF奖励模型中的长度偏见问题，通过反事实数据增强方法训练奖励模型，使其能够独立于冗长度评估内容质量。


<details>
  <summary>Details</summary>
Motivation: RLHF训练的奖励模型存在长度偏见，系统性地倾向于更长的回答，将冗长度与质量混为一谈。

Method: 使用反事实数据增强方法，构建长度不同但内容相似的响应对，以及长度相似但内容不同的响应对，用于训练奖励模型。
...
TL;DR: 本文提出了MMWOZ多模态对话数据集，扩展自MultiWOZ 2.3，包含GUI界面截图和操作指令，并提出了MATE多模态模型作为基线模型。


<details>
  <summary>Details</summary>
Motivation: 传统任务导向对话系统依赖定制后端API，但在实际应用中广泛存在前端GUI界面而缺乏API支持，这造成了应用障碍。

Method: 开发网页风格GUI前端，自动化脚本将对话状态和系统动作转换为GUI操作指令，收集网页快照和对应操作指令，提出MATE多模态模型。

Result: 创建了MMWOZ数据集，包含GUI界面和操作指令，MATE模型作为基线模型进行了全面实验分析。

Conclusion: 通过MMWOZ数据集和MATE模型，为构建实用的多模态任务导向对话系统提供了重要基础。
...
TL;DR: GAPO是一种基于GRPO的改进方法，通过组级奖励函数解决LLMs的模式崩溃问题，提升生成结果的多样性而不影响准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常出现模式崩溃，重复生成相同的少数回答，限制了在各种任务中的多样性。

Method: GAPO是GRPO的简单扩展，计算组级奖励，使用频率感知的奖励函数鼓励对有效补全的均匀采样。

Result: GAPO训练的模型产生有效且更多样化的响应，在标准基准测试中提高了响应多样性而不影响准确性。

Conclusion: GAPO能够推广到开放提示，并在不损害准确性的情况下改善LLMs的响应多样性。
...
TL;DR: Uni-MoE 2.0是基于Qwen2.5-7B架构的全开源多模态大模型，采用动态容量MoE设计、渐进式训练策略和多模态数据匹配技术，在语言中心的多模态理解、推理和生成方面取得显著进展。


<details>
  <summary>Details</summary>
Motivation: 推进Lychee Uni-MoE系列在多模态能力上的发展，构建能够理解多种模态并生成图像、文本和语音的全能模型。

Method: 采用动态容量MoE框架设计，包含共享、路由和空专家；使用Omni-Modality 3D RoPE确保跨模态对齐；实施渐进式监督微调策略，结合平衡数据组合和迭代GSPO-DPO方法稳定强化学习训练。

Result: 在85个基准测试中达到SOTA或极具竞争力的性能，在76个基准中超过50个优于Qwen2.5-Omni（使用1.2T token训练）。关键优势包括视频理解（+7%平均）、多模态理解（+7%平均）和视听推理（+4%）。

Conclusion: Uni-MoE 2.0展示了在计算效率和能力之间的良好平衡，通过创新的MoE架构和训练策略，在多模态理解和生成任务上取得了显著突破。
...
TL;DR: 提出了NOTAM语义解析任务，构建了Knots数据集，通过多智能体协作框架增强标注质量，在航空文本理解方面取得了显著改进。
...
Method: 构建Knots数据集（12,347个专家标注NOTAMs），采用多智能体协作框架进行字段发现，系统评估提示工程策略和模型适应技术。

Result: 在航空文本理解和处理方面取得了实质性改进，实验证明了所提方法的有效性。

Conclusion: 为自动NOTAM分析系统提供了有价值的见解，代码已开源。
...
TL;DR: Reason-KE++提出了一种SFT+RL框架，通过过程级监督解决LLM在复杂多跳推理任务中的忠实性问题，显著提升了事实准确性。


<details>
  <summary>Details</summary>
Motivation: 现有SFT方法存在"忠实性差距"，LLM会优先使用参数先验知识而忽略上下文事实，导致关键事实幻觉问题。

Method: 采用SFT+RL框架，核心是阶段感知奖励机制，为中间推理步骤提供密集监督，包括分解和子答案正确性等。

Result: 在MQUAKE-CF-3k上达到95.48%的新SOTA，相比之前提升5.28%，证明了过程对齐对构建可信LLM的重要性。
...
TL;DR: 提出了一种用于波斯语到英语语音翻译的直接语音到语音翻译系统，通过自监督预训练、离散语音单元和合成平行数据来解决低资源语言数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 直接语音到语音翻译系统需要大量平行语音数据，但波斯语等低资源语言缺乏这样的数据，因此需要解决数据稀缺问题。

Method: 使用三个组件：基于conformer的编码器、因果transformer解码器和基于单元的神经声码器。通过大语言模型翻译波斯语音转录文本，再用零样本文本到语音系统合成英语语音，构建合成平行语料库。

Result: 在CVSS语料库的波斯语-英语部分，使用合成数据比直接基线提高了4.6 ASR BLEU分数，可用平行语音数据量增加了约6倍。

Conclusion: 结合自监督预训练、离散语音单元和合成平行数据，能有效改善波斯语-英语等低资源语言对的直接语音到语音翻译性能。
...
TL;DR: EvoSynth是一个自主框架，通过进化合成方法生成全新的越狱攻击算法，而不是改进现有攻击策略，在Claude-Sonnet-4.5上达到85.5%的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有自动红队框架的越狱逻辑局限于选择、组合或改进现有攻击策略，无法自主发明全新的攻击机制，限制了创造力。
...
Conclusion: EvoSynth通过进化合成越狱方法开创了新方向，为未来研究提供了框架支持。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLMs在多轮对话中固定上下文窗口和简单内存策略的瓶颈问题，避免完整重放的高成本或静态摘要丢失关键安全细节的问题。

Method: 基于语义相似度、半衰期衰减权重和重要性分类，为每条历史消息分配三种保真度级别，在严格token预算下按时间顺序打包消息。

Result: 在涉及花生过敏用户规划泰国旅行的安全导向基准测试中，AFM在短中长度对话中保持过敏信息，安全性能与完整重放相当，平均token使用量减少66%。
...
TL;DR: LLMs在复杂推理任务上表现超人类，但在简单集合成员查询任务中却频繁失败，揭示其推理能力存在不可靠性和不可预测性


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在简单推理任务中的失败模式，以揭示其可靠性和可解释性问题

Method: 使用集合成员查询任务进行大规模系统评估，考察提示措辞、语义结构、元素顺序和模型选择等多个维度

Result: LLMs在这个基础任务上的表现持续脆弱且不可预测，表明模型对集合概念的理解是碎片化和复杂的

Conclusion: 通过简单问题的大规模实验可以全面映射和分析LLMs的失败模式，这为LLM评估提供了有价值的方法论
...
TL;DR: 研究发现语言模型训练中存在相变现象，即使在小型模型中也能观察到，可通过词汇统计方法在早期训练阶段直接检测到相变点。


<details>
  <summary>Details</summary>
Motivation: 探究相变现象是否仅限于大型语言模型，能否在线性训练空间中直接检测，以及相变是否在训练早期就出现。

Method: 训练小型GPT风格transformer模型，分析词汇使用演变，跟踪平均词长、正确/错误词数、词汇多样性变化，应用泊松和亚泊松统计量化词汇连接和重组。

Result: 发现了训练过程中明显的相变点，这些相变在标准损失曲线中不明显，但通过词汇统计方法变得可见。

Conclusion: 相变重组是语言模型训练的普遍特征，可在小型模型、线性训练空间中检测，并在训练早期出现，为理解语言模型训练的非线性动力学提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM对齐研究主要关注通过训练和提示来提高模型鲁棒性，但缺乏随着用户输入长度增加而有效增强对齐的方法。研究表明LLM越狱概率会随用户输入或对话长度增加而上升。

Method: 提出中断机制，即在用户输入中每x个token插入控制语句，这种方法可以推广到思维链过程以防止策略性行为。

Result: 论文提出了中断作为解决方案，但未提供具体的实验结果数据。

Conclusion: 中断机制是增强LLM对齐性的潜在有效方法，特别是对于长输入场景，可以防止模型越狱和策略性行为。
...
TL;DR: 评估大型语言模型在自动形式化任务中对语义相似但措辞不同的自然语言输入的鲁棒性，发现即使语义保持高度一致，模型输出仍存在显著差异。


<details>
  <summary>Details</summary>
Motivation: LLMs在自动形式化方面表现出色，但在生成可验证的形式化结果方面仍有不足。近期研究发现LLMs对自然语言输入的改写很敏感，本文旨在验证这一现象在自动形式化领域是否同样存在。

Method: 使用MiniF2F和Lean 4版本的ProofNet作为基准测试集，选取两个现代LLMs，生成语义相似但措辞不同的自然语言陈述，通过交叉评估测量语义有效性和编译有效性。

Result: 实验结果显示，在改写输入下模型性能存在显著变化，表明自然语言陈述的微小变化会显著影响模型输出质量。

Conclusion: LLMs在自动形式化任务中对自然语言输入的改写敏感，这揭示了模型鲁棒性的局限性，需要进一步改进以确保形式化结果的稳定性和可靠性。
...
TL;DR: BioMedJImpact是一个大规模生物医学期刊数据集，整合了文献计量指标、合作特征和基于LLM的AI参与度语义指标，用于分析合作强度和AI参与度如何共同影响科学影响力。


<details>
  <summary>Details</summary>
Motivation: 现有开放资源很少捕捉合作结构和AI研究如何共同塑造生物医学领域的期刊声望，需要构建一个综合数据集来推进期刊层面的科学影响力和AI参与度分析。

Method: 从PubMed Central的174万篇文章构建数据集，整合文献计量指标、合作特征，并通过提出的三阶段LLM流程提取AI参与度特征，分析合作强度和AI参与度对科学影响力的影响。
...
TL;DR: 通过目标激活工程技术，在LLaMA 3.1-8B模型中实现更人性化的情感表达，无需大量微调即可增强对话的情感特性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在对话流畅性上有所提升，但缺乏细腻、人性化的情感表达。现有对齐技术通常只解决表层输出或需要大量微调。

Method: 使用归因修补识别因果影响组件，通过对比文本对（正面vs负面情感示例）的激活差异推导情感表达向量，并将这些向量应用于新对话提示。

Result: 引导后的响应显示出更高的积极情感（如喜悦、信任）和更频繁的第一人称代词使用，表明更强的个人参与度。
...
TL;DR: 本文提出了一种新的稳定性度量方法，用于评估LDA主题模型的稳定性和准确性，发现LDA虽然内部一致性较好，但生成的主题与真实主题存在差异。


<details>
  <summary>Details</summary>
Motivation: 概率主题模型由于随机性在多次运行时会产生不一致的结果，影响模型的可复现性、可靠性和解释性，需要评估其是否真正捕捉到有意义的主题还是仅仅是噪声。

Method: 定义了一个结合准确性和一致性的新稳定性度量，利用LDA的生成特性生成带有真实主题的新语料库，并对这些语料库运行LDA 50次以评估输出变异性。

Result: LDA能够正确确定文档中的潜在主题数量，并且多次重运行时返回相似主题，表现出较好的内部一致性，但这些主题并非真实主题。

Conclusion: LDA在确定主题数量方面表现良好且具有内部一致性，但生成的主题与真实主题存在偏差，需要关注模型稳定性和准确性之间的平衡。
...
TL;DR: NeuroLex是一个专门针对临床脑电图报告训练的轻量级领域自适应语言模型，相比通用模型在脑电图文本处理上表现更优。


<details>
  <summary>Details</summary>
Motivation: 通用语言模型难以捕捉临床脑电图报告中的领域特定语言惯例，需要专门针对EEG报告文本的模型。

Method: 使用哈佛脑电图数据库的纯文本报告，通过span-corruption预训练和指令式微调（报告润色、段落摘要、术语问答）来学习EEG解释的语法和推理模式。

Result: 与同规模通用模型相比，NeuroLex实现了更低的困惑度、更高的提取和摘要准确性、更好的标签效率，以及对否定和事实幻觉的更强鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型在感知任务中的成功，提升其复杂推理能力成为关键研究方向。现有模型存在推理路径不透明和泛化能力不足等问题，而思维链推理在语言模型中已证明能增强推理透明度和输出可解释性，有望在多模态领域提升模型推理能力。
...
TL;DR: 本文比较了三种transformer架构(BERT、GPT-2、DeBERTa)在希望表达分类任务中的表现，发现BERT在准确率和计算效率方面表现最佳。
...
Method: 使用BERT、GPT-2和DeBERTa三种transformer架构进行二元分类(希望vs非希望)和多类分类(五个希望相关类别)。

Result: BERT表现最佳(二元准确率84.49%，多类72.03%)且计算效率最高；GPT-2准确率最低但擅长检测讽刺表达(召回率92.46%)；DeBERTa表现中等但计算成本最高。

Conclusion: 对于专业化的情感检测任务，架构的适用性可能比模型规模更重要，BERT在希望表达分类中提供了最佳平衡。
...
TL;DR: 通过系统算法审计发现Google搜索的AI概览和精选摘要功能在健康信息展示中存在严重质量问题，包括33%的信息不一致性和医疗安全措施严重缺乏。


<details>
  <summary>Details</summary>
Motivation: 评估AI生成内容在健康信息展示中的质量和一致性，因为用户依赖这些功能但无法控制其呈现方式。

Method: 对1,508个真实婴儿护理和怀孕相关查询进行系统算法审计，评估多个质量维度：答案一致性、相关性、医疗安全措施、来源类别和情感对齐。

Result: AI概览和精选摘要在同一搜索结果页面上有33%的信息不一致；医疗安全措施严重缺乏（仅11%的AI概览和7%的精选摘要包含）；健康网站是主要来源，但精选摘要也常链接商业来源。
...
TL;DR: 本文提出了视觉房间2.0基准，用于评估多模态大语言模型的感知-认知对齐，发现MLLMs的感知能力优于认知能力，且认知不依赖于感知推理。


<details>
  <summary>Details</summary>
Motivation: 基于塞尔中文房间思想扩展到多模态领域，质疑MLLMs是否能真正理解所见内容，即'看见不等于理解'。

Method: 构建包含350个多模态样本的分层基准，涵盖17个代表性任务，从低级到高级三个层次建模人类感知和认知过程。

Result: 评估10个SOTA MLLMs发现：感知能力比认知能力强8.0%；认知不因果依赖于感知推理；认知随模型规模扩展，但感知不随更大变体一致提升。
...
TL;DR: HCNR方法通过识别和恢复关键表达神经元，有效修复微调后LLM的诚实性，相比基线方法速度提升2.23倍且数据需求减少10倍以上。


<details>
  <summary>Details</summary>
Motivation: 监督微调严重削弱LLM的诚实性，但现有恢复方法假设模型知识边界识别能力被深度破坏，而实际上模型仍保留此能力，只是表达该意识的能力受损。

Method: 提出HCNR方法：识别并恢复关键表达神经元至预训练状态，同时通过Hessian引导的补偿机制与任务导向神经元协调。

Result: 在4个QA任务和5个LLM家族上的实验表明，HCNR有效恢复了33.25%受损的诚实性。

Conclusion: HCNR为可信LLM部署提供了实用解决方案，显著提升了恢复效率和数据效率。
...
TL;DR: AA-Omniscience是一个评估语言模型事实回忆和知识校准的基准，包含6000个问题，涵盖42个经济相关主题。评估结果显示大多数前沿模型在事实性和校准方面存在弱点，Claude 4.1 Opus表现最佳，只有三个模型得分高于零。
...
Method: 设计了AA-Omniscience基准，包含来自权威学术和行业来源的6000个问题，涵盖6个领域的42个主题，使用全知指数(-100到100)评估事实回忆和知识校准。

Result: Claude 4.1 Opus获得最高分(4.8)，只有三个模型得分高于零。不同模型在不同领域表现各异，三个不同研究实验室的模型在六个领域中各自领先。

Conclusion: 模型在事实性和校准方面存在持续弱点，性能因领域而异，应根据具体用例需求选择模型而非依赖通用性能指标。
...
TL;DR: 本文探讨了多语言嵌入模型与对齐单语模型在双语词典归纳任务中的表现对比，分析了BLI作为评估指标的优势和局限性，并提出了基于词干的新BLI方法和词汇剪枝技术。


<details>
  <summary>Details</summary>
Motivation: 随着多语言嵌入模型成为主流，需要评估其是否在所有方面都优于对齐的单语模型，以及高昂的计算成本是否合理。BLI作为评估嵌入空间对齐度的常用指标存在局限性，需要更准确的评估方法。

Method: 使用传统嵌入对齐技术、新型多语言模型和组合对齐技术，在高资源和低资源语言环境下进行BLI任务评估，分析语言家族的影响，提出基于词干的BLI方法和词汇剪枝技术。

Result: 组合嵌入对齐技术通常表现更好，但在某些情况下（主要是低资源语言）多语言嵌入表现更优。BLI在某些情况下不能真实反映对齐程度。

Conclusion: 多语言嵌入模型并非在所有方面都优于对齐单语模型，需要根据具体场景选择合适的方法。提出的基于词干的BLI和词汇剪枝技术能更准确地评估嵌入空间对齐度。
...
TL;DR: Spark-Prover-X1是一个7B参数模型，通过三阶段训练框架提升轻量级LLM的形式推理能力，在定理证明任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在自动定理证明中因缺乏多样化高质量形式语言数据而受限的问题。

Method: 三阶段框架：1) 在广泛数学语料上进行持续预训练，引入"思维链增强状态预测"任务；2) 在专家迭代循环中进行监督微调；3) 使用GRPO优化器针对难题进行强化训练。

Result: Spark-Prover-X1-7B在类似规模开源模型中达到SOTA，平均通过率37.0%(pass@32)，在PutnamBench上解决27个问题，CombiBench上达到24.0%。

Conclusion: 多样化的训练数据和渐进精炼的训练流程能有效增强轻量级LLM的形式推理能力。
...
TL;DR: BeDiscovER是一个评估现代LLM语篇理解能力的综合基准套件，包含5个语篇任务、52个数据集，涵盖词汇、句子和文档层面。评估发现前沿模型在时间推理方面表现良好，但在完整文档推理和微妙语义语篇现象方面仍有困难。


<details>
  <summary>Details</summary>
Motivation: 随着推理语言模型的发展，需要更新的综合基准来评估模型在语篇层面的理解能力，特别是涵盖从词汇到文档的多层次语篇现象。

Method: 整合5个公开可用的语篇任务，包括语篇词典、(多)句子和文档层面，总共52个数据集，评估开源LLM和前沿模型在语篇理解方面的表现。

Result: 最先进的模型在时间推理的算术方面表现出色，但在完整文档推理和某些微妙语义语篇现象（如修辞关系识别）方面表现不佳。

Conclusion: 现代LLM在语篇理解方面仍有局限性，特别是在文档级推理和复杂语义语篇现象处理上需要进一步改进。
...
TL;DR: 本研究评估了当代大型语言模型在零样本设置下识别随机对照试验对CONSORT 2010声明依从性的能力，发现模型整体表现一般，能够准确识别合规项目但难以检测不合规项目，目前尚不能替代人类专家。


<details>
  <summary>Details</summary>
Motivation: 手动验证CONSORT依从性是一个耗时费力的过程，构成了同行评审和证据合成的重要瓶颈，需要探索自动化解决方案。

Method: 构建了包含150篇已发表RCT的金标准数据集，在零样本设置下评估多个LLM模型，主要评估指标为宏平均F1分数，并辅以项目级性能指标和定性错误分析。

Result: 表现最佳的Gemini-2.5-Flash和DeepSeek-R1模型宏F1分数分别为0.634，Cohen's Kappa系数分别为0.280和0.282，仅与专家共识达成一般一致性。模型在识别合规项目时表现良好（F1>0.850），但在识别不合规和不适用项目时表现较差（F1<0.400）。

Conclusion: LLMs作为CONSORT检查的初步筛选助手具有潜力，能够识别报告良好的项目，但目前无法可靠检测报告遗漏或方法学缺陷，不适合替代人类专家进行试验质量的关键评估。
...
TL;DR: Agent-Event-Coder (AEC) 是一个多智能体框架，将零样本事件抽取视为软件工程过程，通过专门的检索、规划、编码和验证智能体协作生成结构化的代码式输出，显著提升了大型语言模型在零样本事件抽取任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在零样本事件抽取中面临的挑战：直接提示往往产生不完整或结构无效的输出，如触发器分类错误、参数缺失和模式违规。

Method: 采用多智能体框架，将事件抽取分解为四个专门子任务：检索、规划、编码和验证，每个任务由专门的LLM智能体处理，事件模式表示为可执行的类定义。

Result: 在五个不同领域和六个LLM上的实验表明，AEC始终优于先前的零样本基线方法，证明了将事件抽取视为代码生成方法的有效性。

Conclusion: AEC框架通过编程启发的方法实现了系统性的消歧和模式强制，使LLM能够在零样本设置中产生精确、完整和模式一致的事件抽取结果。
...
Method: 在Azerbaijani Sign Language Dataset和Word-Level American Sign Language数据集上实现并评估ConvLSTM和Vanilla Transformer两种模型。

Result: Vanilla Transformer在Top-1和Top-5准确率上均优于ConvLSTM，在AzSLD上达到76.8% Top-1准确率，在WLASL上达到88.3%。ConvLSTM计算效率更高但准确率较低。

Conclusion: 两种架构各有优势：Transformer在准确率和用户独立性方面表现更好，ConvLSTM在计算效率和时序建模方面有优势，选择取决于应用需求和资源限制。
...
TL;DR: 提出零样本语法能力评估框架，利用无标签数据和LLM生成伪标签，无需人工标注即可训练基于Transformer的语法评分模型
...
Method: 使用基于语法能力量表的提示词让LLM在无标签数据上生成预测作为伪标签，通过处理标签噪声的新训练框架训练Transformer模型

Result: 实验结果表明该方法能高精度估计语法能力分数，LLM选择和清洁-噪声样本比例对性能有重要影响

Conclusion: 该方法为可扩展、低资源语法评估系统铺平了道路，具有鲁棒性和可解释性
...
TL;DR: 该论文构建了首个公开的孟加拉语语料库，用于区分ASR转录中的重复性不流畅和形态学重叠现象，并通过多语言LLM和任务特定微调方法建立了强基准。


<details>
  <summary>Details</summary>
Motivation: 解决孟加拉语等低资源语言ASR转录中词重复的歧义问题：区分无意的重复性不流畅和故意的形态学重叠语法结构，避免标准不流畅校正错误删除有效语言信息。

Method: 1. 构建首个公开的20,000行手动标注孟加拉语语料库；2. 使用两种范式进行基准测试：多语言大语言模型的少样本提示和任务特定编码器模型的微调。

Result: 多语言LLM在少样本提示下达到82.68%准确率；微调方法表现更优，BanglaBERT模型达到84.78%准确率和0.677 F1分数。

Conclusion: 建立了强大的语言感知基准，为开发复杂的语义保留文本规范化系统提供了关键数据。
...
TL;DR: TCM-5CEval是一个针对中医药领域的更细粒度、更全面的LLM评估基准，涵盖5个关键维度：核心知识、经典文献、临床决策、中药学和临床非药物治疗。评估发现模型在基础知识方面表现良好，但在经典文本解释和推理稳定性方面存在显著弱点。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在中医药等专业文化领域的应用需要更严谨的评估，基于前期TCM-3CEval工作发现的知识差距和文化语境对齐问题，需要开发更全面的评估工具。

Method: 构建TCM-5CEval基准，包含5个评估维度：TCM-Exam（核心知识）、TCM-LitQA（经典文献）、TCM-MRCD（临床决策）、TCM-CMM（中药学）、TCM-ClinNPT（临床非药物治疗）。对15个主流LLM进行全面评估，包括排列一致性测试。

Result: 评估显示模型性能差异显著，deepseek_r1和gemini_2_5_pro表现最佳。模型在基础知识回忆方面表现良好，但在经典文本解释方面存在困难。排列一致性测试显示所有模型都存在推理脆弱性，对选项顺序敏感，存在位置偏见。

Conclusion: TCM-5CEval为中医药领域的LLM能力提供了更详细的诊断工具，揭示了模型推理稳定性的根本弱点。该基准已上传至Medbench平台，促进标准化比较和进一步研究。
...
TL;DR: 本文提出了一种量化评估翻译器性能的方法，通过分析翻译熵来衡量翻译器的表现。研究发现翻译熵在解码器块中增强，可用于对公开翻译器进行排名，并揭示互译熵的对称性。


<details>
  <summary>Details</summary>
Motivation: 在信息时代，翻译需求日益增长，但现有基于编码器-解码器架构的翻译器缺乏定量客观的性能评估方法，主要因为单语言的熵仍然未知。

Method: 通过给定翻译器，分析多个仅在一个选定标记上不同的句子产生相同翻译的现象，统计替换特定标记而保持翻译不变的概率，从而计算翻译熵。

Result: 该方法能够定量评估多个公开翻译器的性能，发现翻译熵在解码器块中增强，且双标记替换时翻译简并性呈乘积效应。

Conclusion: 翻译熵是可测量的属性，为人工翻译器提供了客观的基准测试方法，基于MarianMT、T5-Base和NLLB-200翻译器的实验结果验证了该方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 自动变音符号恢复对于处理像罗马尼亚语这样具有丰富变音符号的语言的文本处理至关重要。

Method: 使用包含GPT-3.5、GPT-4、GPT-4o、Gemini 1.0 Pro、Llama 2/3、Mixtral 8x7B等多种LLM，在全面语料库上测试从零样本到复杂多样本指令的多种提示模板。

Result: GPT-4o等模型实现了高精度的变音符号恢复，始终超过中性回显基线，而Meta的Llama系列模型表现出更大的变异性。

Conclusion: 研究结果强调了模型架构、训练数据和提示设计对变音符号恢复性能的影响，并为改进变音符号丰富语言的NLP工具指明了有前景的方向。
...
TL;DR: 本文评估了视觉语言模型在语音识别任务中的表现，发现即使经过微调，模型在识别语音频谱图和波形图方面的表现也仅略高于随机猜测。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型和视觉语言模型的发展，需要评估这些模型在融合视觉和语言模态任务中的能力，特别是在语音识别领域。

Method: 创建包含4000多个英语单词的数据集，通过多选任务测试模型从语音频谱图和波形图中识别正确音素或字素转录的能力。

Result: 零样本和微调模型的表现都很少超过随机水平，表明需要特定的参数知识来解释语音图形表示。

Conclusion: 仅凭配对样本不足以让视觉语言模型有效解释语音图形表示，需要专门的参数知识。
...
TL;DR: 提出SoCE方法，通过基准测试组合识别最优模型候选，使用非均匀加权平均来最大化性能，在多个领域提升模型表现


<details>
  <summary>Details</summary>
Motivation: LLM训练资源密集且耗时，模型融合（model souping）是一种有前景的技术，可以在不昂贵重新训练的情况下提升性能

Method: 利用基准类别间模型性能低相关性的观察，识别每个弱相关类别簇的"专家"模型，使用优化的加权平均而非均匀权重进行组合

Result: 在多个领域（多语言能力、工具调用、数学等）提升性能和鲁棒性，在伯克利函数调用排行榜上达到最先进结果

Conclusion: SoCE方法通过有原则的模型融合策略，有效提升了LLM的性能和鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 现有的EaaS水印方法只能抵抗部分攻击，无法提供全面保护。模型提取攻击可能导致模型提供商重大经济损失，需要更强大的版权保护方案。

Method: 使用秘密降维矩阵将文本嵌入投影到低维子空间，随机选择触发区域，在整个触发区域嵌入水印，并使用文本嵌入本身作为水印。

Result: 在多个数据集上的广泛实验表明，RegionMarker能有效抵抗不同攻击方法，包括重述攻击和维度扰动攻击。

Conclusion: RegionMarker通过区域触发机制和语义水印，为EaaS提供了全面的版权保护，难以被水印移除攻击规避检测。
...
TL;DR: 阿拉伯酒店业依赖客户反馈改进服务，推动阿拉伯语情感分析工具发展。该共享任务使用多方言酒店评论数据集，包含538条平衡情感评论，支持方言感知NLP系统开发。最佳系统F1分数达0.81。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯世界酒店业日益依赖客户反馈来塑造服务，需要先进的阿拉伯语情感分析工具。阿拉伯方言的多样性给情感分析带来挑战，特别是在酒店领域。

Method: 使用多方言、手动整理的数据集，包含从现代标准阿拉伯语翻译为沙特和摩洛哥方言的酒店评论。数据集包含538条情感平衡的评论（正面、中性、负面），翻译由母语者验证以确保方言准确性和情感保留。

Result: 超过40个团队注册共享任务，12个团队在评估阶段提交了系统。表现最佳的系统F1分数达到0.81，证明了跨阿拉伯方言情感分析的可行性。

Conclusion: 该研究展示了在阿拉伯方言中进行情感分析的可行性，同时指出了持续存在的挑战。开发的数据集支持方言感知NLP系统在客户体验分析等实际应用中的发展。
...
TL;DR: 该研究通过PEFT/LoRA方法系统分析了LLMs在任务和语言维度上的迁移效应，发现跨语言任务迁移稳定正向，而跨任务迁移常导致性能下降，揭示了任务-语言迁移的不对称性和稳定的供体-受体结构。


<details>
  <summary>Details</summary>
Motivation: 理解LLMs在不同任务和语言间的迁移效应，探究模型改进如何影响其他任务和语言组合，填补现有研究空白。

Method: 采用PEFT/LoRA方法，在多个开源LLM家族和尺寸上进行控制实验，将任务和语言作为迁移轴，在单一任务-语言源上微调模型，测量在所有其他任务-语言目标对上的迁移效果。

Result: 发现两个一致模式：1) 任务内跨语言迁移可靠正向，而跨任务迁移常导致性能下降；2) 跨语言和任务存在稳定的供体-受体结构（枢纽供体vs脆弱受体）。

Conclusion: 研究为风险感知微调和模型专业化提供了重要启示，揭示了任务-语言迁移的不对称性和稳定的迁移结构模式。
...
TL;DR: PEDIASBench评估框架显示，当前LLMs在儿科医学中表现出色于基础知识，但在复杂推理、动态诊疗和人文关怀方面存在局限，尚不能独立承担儿科诊疗工作。
...
Method: 开发PEDIASBench系统评估框架，从基础知识应用、动态诊疗能力、医疗安全与伦理三个维度，评估12个代表性模型在19个儿科亚专业和211种典型疾病上的表现。

Result: 顶尖模型在基础知识上表现良好（Qwen3-235B-A22B在执照级问题上准确率超90%），但随着任务复杂度增加性能下降约15%；在动态诊疗中DeepSeek-R1得分最高（平均0.58），但多数模型难以适应实时患者变化；在医疗伦理安全任务中Qwen2.5-72B表现最佳（92.05%准确率），但人文敏感性有限。

Conclusion: 儿科LLMs受限于有限的动态决策能力和未充分发展的人文关怀，未来应关注多模态整合和临床反馈-模型迭代循环，以增强安全性、可解释性和人机协作。虽然当前LLMs不能独立进行儿科诊疗，但在决策支持、医学教育和患者沟通方面具有潜力。
...
<details>
  <summary>Details</summary>
Motivation: 随着智能设备普及，需要能够理解用户特质的个性化对话助手，但现有方法难以处理长期交互和用户主观特征。

Method: 开发了基于LLM的多步合成流程创建PAL-Set数据集，并提出H²Memory分层异构记忆框架，结合检索增强生成技术。

Result: 在PAL-Bench和外部数据集上的实验证明了所提记忆框架的有效性。

Conclusion: PAL-Bench为评估个性化服务助手提供了新基准，H²Memory框架显著提升了长期交互中的个性化响应生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于MQM的线性评估方法在不同文本长度下存在偏差，短文本被过度惩罚而长文本被低估，与专家直觉不符。

Method: 建立了双参数对数模型 E(x) = a * ln(1 + b * x)，通过一维根查找步骤从两个容忍点进行校准，并集成到现有评估工作流中。

Result: 实证数据显示可接受错误数量随样本大小呈对数增长而非线性增长，模型提高了评估的可解释性、公平性和评分者间信度。

Conclusion: 该模型推进了翻译质量评估向更准确和可扩展的方向发展，为基于AI的文档级评估提供了更强的基础，并与人类判断保持一致。
...
TL;DR: 本文提出了一种基于方面的情感分析方法来解码泰国金融年报中的模糊情感，通过标注100多份财务报告并测试多种文本分类模型，展示了在情感分类方面的强大性能，并通过事件研究验证了分析结果对股票价格的实际影响。


<details>
  <summary>Details</summary>
Motivation: 理解金融文档中的情感对于洞察市场行为至关重要，但这类报告通常包含模糊语言，即使在基础条件不利时也呈现积极或中性展望，因此需要专门方法解码这种模糊情感。

Method: 开发了标注泰国金融年报中模糊情感的具体指南，标注了100多份财务报告，并对多种文本分类模型进行基准测试，同时进行事件研究来评估情感分析对股价的实际影响。

Result: 在情感分类任务上表现出强大性能，事件研究表明市场反应受到报告中特定方面的选择性影响。

Conclusion: 研究结果强调了金融文本情感分析的复杂性，并凸显了处理模糊语言以准确评估市场情感的重要性。
...
TL;DR: 提出利用大语言模型自动标注公共叙事的计算框架，在8个叙事和14个代码上达到平均F1分数0.80，接近人类专家水平，并扩展到22个故事和政客演讲的分析。


<details>
  <summary>Details</summary>
Motivation: 公共叙事是领导力发展和公民动员的关键工具，但由于主观解释和专家标注成本高，系统分析面临挑战。

Method: 开发了与领域专家共同制定的代码本，评估大语言模型在公共叙事定性标注中的表现，并与专家标注进行对比。

Result: 大语言模型能够达到接近人类专家的性能水平，平均F1分数为0.80，并成功扩展到更大数据集和政客演讲分析。
...
<details>
  <summary>Details</summary>
Motivation: 高资源语言的ASR发展受益于大量数据集，而匈牙利语等语言由于缺乏自发和对话语料库而代表性不足，需要填补这一空白。

Method: 利用匈牙利语语音语料库BEA中未处理的部分构建了两个数据集：BEA-Large（255小时自发语音）和BEA-Dialogue（85小时对话语音），并使用公开可用的ASR模型建立基线。

Result: 微调的Fast Conformer模型在自发语音上词错误率为14.18%，重复语音上为4.8%；说话人日志错误率在13.05%-18.26%之间。

Conclusion: 对话ASR仍然具有挑战性，特别是由于不流利、重叠和非正式语音模式。发布这些数据集和基线旨在推进匈牙利语语音技术，并为其他语言开发自发和对话基准提供方法框架。
...
TL;DR: 提出基于核心意图、语句类型、语法结构和关键动作的文本到SQL分类法，评估现有数据集局限性，并创建SQL-Synth数据集，验证分类法有效性并发现LLMs在多样化场景下的不足。


<details>
  <summary>Details</summary>
Motivation: 现有文本到SQL数据集覆盖范围有限，无法反映真实应用的多样性，需要更全面的分类方法和数据集来提升模型训练和评估效果。

Method: 提出新的文本到SQL分类法，基于该分类法评估现有数据集，然后使用分类法指导结合LLMs的数据集合成流程，创建SQL-Synth数据集。

Result: SQL-Synth数据集在多样性和覆盖范围上优于现有基准，实验显示现有LLMs在多样化场景下表现有限，但微调能显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在复杂环境中维持长期交互时面临上下文一致性和动态个性化挑战，传统记忆系统依赖语义分组检索，会忽略语义不相关但关键的用户信息并引入检索噪声。

Method: 提出O-Mem框架，基于主动用户画像动态提取和更新用户特征与事件记录，支持人物属性和主题相关上下文的分层检索。

Result: 在LoCoMo基准测试中达到51.76%（比LangMem提升近3%），在PERSONAMEM中达到62.99%（比A-Mem提升3.5%），同时在token和交互响应时间效率方面优于现有记忆框架。
...
TL;DR: 使用大型语言模型将机器学习检测到的虚假评论特征转化为人类可理解的语言现象，帮助人们识别虚假评论。


<details>
  <summary>Details</summary>
Motivation: 虚假评论误导消费者、损害商家利益并破坏在线市场信任。虽然机器学习分类器能有效区分虚假评论，但其学习到的特征往往难以被人类理解。

Method: 利用大型语言模型（LLMs）将机器学习学到的词汇线索翻译成人类可理解的语言现象，用于区分虚假评论和真实评论。

Result: 通过此方法获得的语言现象具有数据实证基础，在相似领域具有通用性，且比LLMs先验知识或上下文学习获得的现象更具预测性。
...
TL;DR: 提出了TAI框架，结合大型语言模型和潜在扩散模型，通过翻译和图像生成增强印度诗歌的可访问性，支持联合国可持续发展目标。


<details>
  <summary>Details</summary>
Motivation: 印度诗歌具有深厚的文化内涵和语言复杂性，但现有研究往往忽视印度语言诗歌，且其复杂的语法结构和文化典故给非母语读者带来理解困难。

Method: 使用TAI框架，包括：(1) 翻译模块采用比值偏好对齐算法准确翻译形态丰富的诗歌；(2) 图像生成模块使用语义图捕捉隐喻和含义的语义关系，生成视觉表示。

Result: 综合实验评估显示TAI Diffusion在诗歌图像生成任务中优于强基线方法，并发布了包含21种低资源印度语言的1,570首诗歌数据集。

Conclusion: 通过填补诗歌翻译和视觉理解的空白，该工作旨在扩大印度诗歌的可访问性，丰富读者体验。
...
TL;DR: Lang1模型系列在医疗EHR数据上预训练，通过真实医疗评估基准ReMedE验证，在微调后优于通用模型，显示专业LLM在医疗操作任务中的优势。
...
Method: 开发Lang1模型系列(100M-7B参数)，在NYU Langone Health的EHR数据和互联网数据混合语料上预训练，使用ReMedE基准评估五个关键医疗任务。

Result: 微调后的Lang1-1B模型在AUROC指标上比通用模型提升3.64%-6.75%，比零样本模型提升1.66%-23.66%，且能有效迁移到其他临床任务和外部医疗系统。

Conclusion: 医疗系统AI需要领域内预训练、监督微调和真实世界评估的结合，专业LLM在专业任务中能与通用模型竞争。
...
TL;DR: 该研究结合笔迹学和人工智能，通过分析学生手写考试卷来量化心理压力水平，使用OCR和基于Transformer的情感分析模型，提供超越传统评分系统的认知和情感状态洞察。


<details>
  <summary>Details</summary>
Motivation: 传统评分系统无法深入理解学生在考试期间的认知和情感状态，需要一种数据驱动的方法来量化心理压力，为学术取证提供更深入的洞察。

Method: 集成高分辨率图像处理、TrOCR和使用RoBERTa模型的情感熵融合，通过五模型投票机制和无监督异常检测生成数值化压力指数。

Result: 开发了一个创新的学术取证框架，能够通过手写分析量化学生的心理压力水平。

Conclusion: 该研究成功展示了人工智能与笔迹学融合在学术压力评估中的潜力，为教育心理学和学术取证提供了新的技术途径。
...
Method: 利用车辆搭载的传感器收集数据，使用SVM分类器进行坑洞检测

Result: 在包含26个坑洞的2公里路段测试中，检测准确率达到98.1%

Conclusion: 该方法能有效实时检测坑洞，为大规模道路管理提供有用数据
...
TL;DR: 本研究针对耕地生态系统缺乏标准化生境分类体系的问题，开发了一个包含15类耕地系统生境的超高清遥感图像数据集，并提出了动态加权特征融合网络(DWFF-Net)，在构建的数据集上取得了0.6979的mIoU和0.8049的F1分数。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏耕地生态系统的标准化生境分类体系，现有模型无法有效整合语义和纹理特征，导致多尺度生境分割精度不足和边界模糊。

Method: 使用冻结参数的DINOv3编码器提取基础特征，引入数据级自适应动态加权策略进行特征融合，解码器包含动态权重计算网络实现多层特征充分融合，采用混合损失函数优化训练。

Result: 在构建的数据集上，模型达到0.6979的mIoU和0.8049的F1分数，分别比基线网络提高0.021和0.0161。消融研究证实多层特征融合的互补性有效改善了田埂等微生境类别的IoU。

Conclusion: 本研究建立了基于自适应多层特征融合的耕地系统生境识别框架，能够以低成本实现亚米级精度的生境制图，为耕地景观的细粒度生境监测提供技术支持。
...
TL;DR: AGENet是一个用于医学图像分割的新型框架，通过边缘感知的测地距离学习结合空间关系，在有限标注数据下实现精确边界分割。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要大量标注数据，而现有少样本分割方法在医学图像边界分割上表现不佳，特别是当解剖结构相似且缺乏空间上下文时。

Method: 结合三个主要组件：(1)边缘感知测地距离学习模块，通过迭代快速行进细化解剖边界；(2)自适应原型提取，通过空间加权聚合捕获全局结构和局部边界细节；(3)自适应参数学习，自动适应不同器官特征。

Result: 在多个医学影像数据集上的广泛实验显示，相比现有最先进方法有显著改进，特别是在减少边界误差方面，同时保持计算效率。

Conclusion: 该方法在有限标注数据下实现了精确的医学图像分割，计算效率高，非常适合临床应用中需要精确分割但标注数据有限的场景。
...
TL;DR: EPSegFZ是一个无需预训练的少样本和零样本3D点云语义分割网络，通过原型增强注意力、双相对位置编码和语言引导原型嵌入模块，在S3DIS和ScanNet基准上分别比现有最佳方法提升5.68%和3.82%。


<details>
  <summary>Details</summary>
Motivation: 现有少样本3D点云分割方法依赖预训练阶段，限制了模型灵活性；同时忽视了支持集中的文本标注等有用信息，影响了模型性能和零样本能力。

Method: 提出EPSegFZ网络，包含三个关键组件：原型增强寄存器注意力模块、基于双相对位置编码的交叉注意力机制、语言引导原型嵌入模块，无需预训练即可实现准确的特征提取和查询-原型对应关系构建。

Result: 在S3DIS和ScanNet基准测试中，该方法分别比现有最优方法提升了5.68%和3.82%的性能。

Conclusion: EPSegFZ通过消除预训练依赖并充分利用文本信息，显著提升了少样本和零样本3D点云分割的性能和适应性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注对象级可操作性或将2D预测提升到3D，忽略了点云中的丰富几何结构信息且计算成本高。需要解决语义推理和空间基础化的挑战。

Method: TASA包含任务感知的2D可操作性检测模块来识别可操作点，指导任务相关视图选择；以及3D可操作性细化模块，将2D语义先验与局部3D几何信息集成，生成准确的空间一致的3D可操作性掩码。

Result: 在SceneFun3D数据集上的实验表明，TASA在场景级可操作性分割的准确性和效率方面显著优于基线方法。

Conclusion: TASA通过几何优化的框架成功解决了3D场景级可操作性分割的挑战，实现了语义推理和空间基础化的有效结合。
...
<details>
  <summary>Details</summary>
Motivation: CapsNet虽然比CNN有优势（如更好检测重叠类别图像和对变换图像更高准确率），但速度慢、资源消耗大、参数多且准确率不如CNN。

Method: 开发LE-CapsNet作为CapsNet的改进版本，使用380万权重，优化网络结构以提高效率和准确性。

Result: 在CIFAR-10数据集上获得76.73%准确率，推理速度比CapsNet快4倍；在AffNIST数据集上达到94.3%准确率，优于CapsNet的90.52%。

Conclusion: LE-CapsNet在保持CapsNet优势的同时，显著提高了效率、准确性和对仿射变换的鲁棒性。
...
TL;DR: 本文提出了Target-Balanced Score Distillation (TBSD)方法，通过多目标优化和自适应策略解决了Score Distillation Sampling (SDS)在3D资产生成中的纹理与形状权衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统的SDS方法存在过饱和和过平滑问题，而引入负提示的变体方法面临纹理优化有限或纹理增益但形状失真的关键权衡问题。

Method: 通过系统分析发现该权衡受负提示使用的影响，提出TBSD方法，将生成建模为多目标优化问题，并引入自适应策略来平衡纹理和形状。

Result: 大量实验表明TBSD显著优于现有最先进方法，生成具有高保真纹理和几何精确形状的3D资产。
...
<details>
  <summary>Details</summary>
Motivation: 深度CNN模型在微控制器和轻量级NPU上部署困难，现有低秩分解方法局部选择秩而忽略全局压缩与精度的权衡。

Method: 采用MicroNAS启发的方法，将秩选择作为全局搜索问题，使用快速精度估计器评估候选分解，在内存和精度约束下进行高效搜索。

Result: 在ImageNet上压缩ResNet-18达8倍，精度损失小于4%；在COCO上压缩YOLOv5s达2倍无精度损失，压缩YOLOv5n达2倍仅损失2.5%精度。

Conclusion: CompressNAS能有效平衡压缩与精度，并提出了新的压缩模型家族STResNet，性能优于其他高效模型。
...
TL;DR: AdaptFly是一个无需权重更新的提示引导测试时适应框架，通过轻量级token提示检索和梯度稀疏视觉提示优化两种模式，解决无人机在恶劣天气和视角变化下语义分割性能下降的问题，实现跨无人机知识共享和协作。


<details>
  <summary>Details</summary>
Motivation: 低空无人机网络需要鲁棒的语义分割作为分布式感知-通信-控制协同设计的基础，但现有分割基础模型在天气、光照和视角变化下性能快速下降。资源受限的无人机无法运行基于梯度的测试时适应，而资源充足的无人机独立适应会浪费共享经验。

Method: AdaptFly框架包含两种互补适应模式：针对资源受限无人机的轻量级token提示检索，从共享全局内存中获取提示；针对资源充足无人机的基于协方差矩阵自适应进化策略的梯度稀疏视觉提示优化。通过激活统计检测器触发适应，跨无人机知识池整合提示知识。

Result: 在UAVid和VDD基准测试以及真实无人机部署中，AdaptFly显著提高了分割精度和鲁棒性，优于静态模型和最先进的TTA基线方法。

Conclusion: AdaptFly为新兴低空经济中的弹性、通信高效感知提供了一条实用路径，通过无需权重更新的提示适应和跨无人机协作，解决了资源受限环境下的分割性能退化问题。
...
TL;DR: 提出一种基于人类视觉盲点的自监督视觉表示学习方法，用于从儿童视角数据中学习词汇-指称映射


<details>
  <summary>Details</summary>
Motivation: 儿童在6-9个月开始学习词汇，但首次遇到新词时存在无数可能的解释。需要一种生物合理的策略来学习视觉表示

Method: 使用基于掩码自编码器的视觉骨干网络，结合人类眼睛盲点知识定义新的掩码策略，采用对比学习的视频-文本模型

Result: 提出的生物合理掩码策略在从跨情境和时间扩展片段学习词汇-指称映射方面至少与随机掩码同样有效

Conclusion: 基于人类视觉系统的掩码策略为自监督学习提供了生物学上合理的替代方案，能够有效学习词汇-指称映射
...
TL;DR: GROVER是一个用于空间多组学数据自适应融合的新框架，通过图卷积网络和对比学习解决多模态异质性问题，在真实数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学、蛋白质组学和表观基因组学数据缺乏病理形态学背景，需要与组织病理图像整合以全面分析疾病组织。但多模态数据的异质性和分辨率不匹配带来了显著挑战。

Method: 使用基于Kolmogorov-Arnold网络的图卷积网络编码器捕捉模态间非线性依赖关系，采用点-特征对对比学习策略优化跨模态对应关系，设计动态专家路由机制自适应选择信息丰富的模态。
...
TL;DR: HSI-Detect是一个两阶段深度伪造检测方法，通过将RGB图像重建为31通道高光谱图像，在光谱域中检测伪造痕迹，相比RGB域方法有显著改进。


<details>
  <summary>Details</summary>
Motivation: 现代生成和扩散模型产生的逼真图像可能误导人类感知和自动化检测系统。大多数检测方法仅分析RGB三通道，而伪造痕迹在特定频段可能更明显。

Method: 提出两阶段管道：首先从标准RGB输入重建31通道高光谱图像，然后在光谱域进行检测。通过扩展输入表示到更密集的光谱带，放大RGB域中微弱或不可见的操作伪影。

Result: 在FaceForensics++数据集上的评估显示，相比仅使用RGB的基线方法，HSI-Detect取得了持续改进。

Conclusion: 光谱域映射在深度伪造检测中具有潜力，通过分析更丰富的光谱信息可以更有效地识别伪造痕迹。
...
TL;DR: 本文研究了点云神经网络是否保持对称感知距离的bi-Lipschitz等价性，发现现有网络不满足此性质，提出了改进方法并在点云对应任务中验证了优势。


<details>
  <summary>Details</summary>
Motivation: 受等变学习文献启发，研究点云网络是否保持对称感知距离的bi-Lipschitz等价性，这对模型性能有重要意义。

Method: 分析两种对称感知度量（Procrustes匹配和Hard Gromov Wasserstein距离）的bi-Lipschitz等价性，修改现有网络以获得bi-Lipschitz保证。

Result: 发现两种距离本身不bi-Lipschitz等价，现有网络也不满足此性质；改进后的bi-Lipschitz模型在点云对应任务中表现更优。

Conclusion: 对称感知距离的bi-Lipschitz等价性对点云网络设计很重要，提出的改进方法能提升模型在对应任务中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决现代视觉语言模型缺乏决策解释性、容易产生幻觉事实的问题，特别是面对分布外数据时。现有神经符号方法仅从任务标签提取符号，缺乏视觉基础。

Method: 1. 多模态概念生成器从训练图像中挖掘判别性视觉概念；2. 基于视觉概念进行符号发现，锚定真实图像统计；3. 大语言模型推理代理将符号组合成可执行的一阶规则；4. 推理时视觉验证代理量化符号存在程度并触发规则执行。

Result: 在五个基准测试（包括两个医学成像任务和三个代表性不足的自然图像数据集）上，系统平均提升最先进神经符号基线5%，同时将规则中的幻觉符号出现减少高达50%。

Conclusion: Concept-RuleNet通过视觉基础的概念挖掘和符号推理，成功增强了神经符号系统的解释性和准确性，有效减少了幻觉问题。
...
TL;DR: Image-POSER是一个基于强化学习的反射框架，通过编排预训练的文本到图像和图像到图像专家模型，动态分解长提示任务，并使用视觉语言模型进行结构化反馈监督，实现更好的图像生成效果。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在处理长组合提示时表现不佳，缺乏可靠执行创意工作流程的能力。

Method: 将图像合成和编辑建模为马尔可夫决策过程，通过强化学习框架动态编排多个预训练专家模型，利用视觉语言模型提供结构化反馈监督每个步骤的对齐。

Result: 在行业标准和自定义基准测试中，Image-POSER在一致性、保真度和美学方面均优于基线模型和前沿模型，在人类评估中持续获得偏好。

Conclusion: 强化学习可以赋予AI系统自主分解、重新排序和组合视觉模型的能力，向通用视觉助手迈进。
...
<details>
  <summary>Details</summary>
Motivation: 解决在遮挡、尺度变化和时间漂移下保持时序一致性的挑战，这些因素会破坏实时感知所需的时序连贯性。

Method: 采用最小常量内存时序transformer，包含真实值引导的存储器和燃烧锚点损失来稳定初始化，单一轻量级时序注意力层跨帧优化嵌入。

Result: 在Mini-LaSOT基准测试中达到76.3 AUC和53.7 FPS（4.3GB VRAM），在快速运动、尺度变化和遮挡情况下优于TrackFormer和MOTRv2等transformer基线模型。

Conclusion: SOTFormer通过统一的端到端框架成功实现了稳定的身份传播和实时性能，在挑战性场景下表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态图学习方法在加工工艺规划中虽然能捕捉时空依赖关系，但无法融入零件的三维几何信息，缺乏领域感知能力。

Method: 利用StereoLithography表面网格表示每个加工操作后零件的3D几何，通过边界表示方法处理初始3D设计，使用注意力机制集成几何信息。

Result: 在合成数据集上评估，相比最先进方法，在主操作和子操作预测准确率上分别提升了24%和36%。

Conclusion: MP-GFormer通过集成3D几何信息，显著提升了加工操作序列预测的准确性，解决了现有方法缺乏几何感知的问题。
...
<details>
  <summary>Details</summary>
Motivation: 预训练模型和开源仓库的快速扩散使得模型合并成为方便但有风险的做法，未经授权的模型合并侵犯知识产权并破坏模型所有权和问责制。

Method: 第一阶段通过L2正则化优化重新分布任务相关信息，第二阶段注入结构化扰动来错位任务子空间，破坏损失景观中的曲率兼容性。

Result: 在视觉(ViT-L-14)和语言(Llama2、Gemma2、Mistral)模型上的广泛实验表明，MergeGuard将合并模型准确率降低高达90%，而保护模型性能损失不到1.5%。
...
TL;DR: FocusSDF是一种基于符号距离函数(SDF)的新型损失函数，通过自适应地为靠近病灶或器官边界的像素分配更高权重，使网络专注于边界区域，从而解决医学图像分割中边界保持的挑战。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割是医学图像分析的重要组成部分，但大多数分割模型没有明确编码边界信息，导致边界保持成为持续挑战。

Method: 引入FocusSDF损失函数，基于符号距离函数(SDF)自适应地为靠近边界的像素分配更高权重，使网络边界感知。

Result: 在涵盖脑动脉瘤、中风、肝脏和乳腺肿瘤分割任务的多个数据集上，与五种最先进的分割模型和四种基于距离的损失函数进行比较，FocusSDF始终表现出优越性能。

Conclusion: FocusSDF在医学图像分割中优于现有的基于距离变换的损失函数，能有效提升边界保持能力。
...
TL;DR: 本研究探讨了使用合成图像补充有限训练数据，以提高在零样本和少样本设置下对麝牛的检测性能。研究发现合成图像能有效提升检测精度，特别是在数据稀缺的情况下。


<details>
  <summary>Details</summary>
Motivation: 传统野生动物调查方法资源密集且受限于后勤挑战，而深度学习目标检测模型在稀疏分布物种（如麝牛）的小数据集上效果有限。需要寻找替代方案来改善野生动物监测。

Method: 比较基线模型（仅使用真实图像）与5个零样本和5个少样本模型，这些模型在训练集中逐步加入更多合成图像。评估不同模型在精度、召回率和F1分数上的表现。

Result: 零样本模型中，添加合成图像提高了检测性能，但随着合成图像超过基线模型训练数据集的100%，性能增长趋于平缓。少样本模型中，真实图像与合成图像结合使用可获得更好的召回率和略高的整体准确率。

Conclusion: 合成图像在数据稀缺时能有效训练准确的目标检测模型，为监测稀有或难以接近的物种提供了重要视角，并可增加监测频率。该方法可在没有真实数据的情况下启动模型，并随着时间推移获取真实图像进行优化。
...
TL;DR: 开发了Harpia CUDA处理库，为Annotat3D提供可扩展的交互式3D分割工作流，支持超出单GPU内存容量的大型数据集处理


<details>
  <summary>Details</summary>
Motivation: 高分辨率体积成像技术产生的大型数据集挑战现有处理工具的效率，需要支持HPC和远程访问环境下的可扩展交互式分割

Method: 基于CUDA开发Harpia处理库，具有严格内存控制、原生分块执行和GPU加速的过滤、标注和量化工具套件

Result: 相比NVIDIA cuCIM和scikit-image等框架，在处理速度、内存效率和可扩展性方面显著提升
...
TL;DR: 本文探索了在医学视觉语言系统中应用自动提示优化技术，通过DSPy框架在放射学、胃肠病学和皮肤病学的五个医学成像任务中实现了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 视觉语言基础模型在医学基准测试中表现不佳，传统方法如微调需要大量领域数据和计算资源，手动提示工程难以推广。因此需要开发能够利用模型嵌入知识且不依赖人工设计提示的可扩展方法。

Method: 采用DSPy框架进行结构化自动提示优化，在五个医学成像任务中评估了10个开源视觉语言模型和四种提示优化技术。

Result: 优化后的管道在零样本提示基线基础上实现了53%的中位数相对改进，在零样本性能较低的任务中最大改进达到300%至3400%。

Conclusion: 自动提示优化在医学AI系统中具有巨大潜力，能够显著提升临床图像解释的准确性，同时减少对提示设计的依赖，让临床医生更专注于患者护理和临床决策。
...
TL;DR: PI-NAIM是一个双路径架构，根据缺失复杂度动态路由样本到优化的插补方法，结合统计插补和神经网络的优势，在医学影像和多模态临床数据中实现最先进的性能。
...
TL;DR: QTSplus是一个轻量级视觉token选择模块，通过动态选择与文本查询最相关的视觉证据，解决了长视频理解中视觉token数量线性增长导致的注意力成本、内存和延迟问题。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在长视频理解中面临的视觉token数量线性增长问题，这会导致注意力成本爆炸、内存占用过大和延迟增加。

Method: QTSplus通过(i)跨注意力机制对视觉token评分，(ii)基于查询复杂度预测实例特定的保留预算，(iii)在训练时使用可微分直通估计器选择Top-n token，在推理时使用硬门控。同时使用小型重编码器保持时间顺序。

Result: 在Qwen2.5-VL中集成QTSplus，视觉流压缩达89%，端到端延迟减少28%。在8个长视频理解基准测试中，整体准确率接近原始模型，在TempCompass方向和顺序准确率上分别提升20.5和5.6个百分点。

Conclusion: QTSplus是一种有效且通用的机制，能够在保留任务相关证据的同时，将多模态大语言模型扩展到现实世界长视频场景。
...
TL;DR: 首次使用事件相机进行图像去雾，通过事件引导的扩散模型将事件的高动态范围信息传输到RGB图像中，在严重雾霾条件下实现清晰成像。


<details>
  <summary>Details</summary>
Motivation: 传统基于RGB的去雾方法受限于有限的动态范围，容易丢失结构和光照细节。事件相机具有更高的动态范围和微秒级延迟，更适合雾霾场景，但真实配对数据稀缺。

Method: 提出事件引导的扩散模型，设计事件引导模块将稀疏的高动态范围事件特征映射到扩散潜在空间，在生成过程中提供精确的结构指导。

Result: 在两个基准测试和自建的无人机数据集上取得了最先进的结果，特别是在严重雾霾条件下表现优异。

Conclusion: 事件相机结合扩散模型为图像去雾提供了新的解决方案，能够有效传输高动态范围信息，提高视觉真实性和减少语义漂移。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在评估不同注意力机制对考古遗产数字保护的语义分割效果，特别针对巴西考古遗址的岩画岩刻。

Method: 比较三种U-Net架构：(1)BEGL-UNet带边界增强高斯损失；(2)Attention-Residual BEGL-UNet结合残差块和门控注意力；(3)Spatial Channel Attention BEGL-UNet使用空间-通道注意力模块。所有方法都采用BEGL损失函数，在巴西Poço da Bebidinha考古遗址图像上进行5折交叉验证。

Result: Attention-Residual BEGL-UNet表现最佳：Dice分数0.710，验证损失0.067，召回率0.854。Spatial Channel Attention BEGL-UNet表现相当：Dice分数0.707，召回率0.857。基线BEGL-UNet Dice分数0.690。注意力机制相比基线提升2.5-2.9%。

Conclusion: 注意力机制在考古遗产数字保护的语义分割中具有显著效果，特别是Attention-Residual BEGL-UNet架构表现最优，为文化遗产保护提供了有效的技术方案。
...
TL;DR: 该研究评估了视觉语言模型在数据受限条件下进行细粒度肾小球亚型分类的表现，发现病理学专业化的视觉语言骨干网络配合标准微调是最有效的起点，即使每个亚型只有4-8个标注样本也能显著提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 肾活检中细粒度肾小球亚型分类至关重要，但临床上有价值的标注数据稀缺且难以获取。现有计算方法主要在完全监督下进行粗粒度疾病分类，不清楚视觉语言模型如何在数据约束下适应临床有意义的亚型分类。

Method: 将细粒度肾小球亚型分类建模为临床现实的少样本问题，系统评估病理学专业化和通用视觉语言模型在此设置下的表现，分析分类性能、特征表示几何结构、图像-文本嵌入对齐以及肾小球亚型可分性。

Result: 病理学专业化视觉语言骨干网络配合标准微调是最有效的起点，即使每个亚型只有4-8个标注样本也能显著提升区分度和校准性能，正负样本区分与图像-文本对齐同等重要。

Conclusion: 监督水平和适应策略共同影响诊断性能和多模态结构，为模型选择、适应策略和标注投入提供指导，病理学专业化模型在少样本设置下表现出色。
...
TL;DR: 本文提出了一种突破面部特写限制的IPPG方法，通过双线推理管道、身份自适应融合和身份聚合预置模块，实现了身份保真度和场景语义创造的协同优化。
...
Method: 设计了身份-语义分离的双线推理管道，提出身份自适应融合策略将ID-语义融合推迟到噪声预测阶段，并引入身份聚合预置模块来聚合ID信息替换随机初始化。

Result: 实验验证该方法在超越面部特写的IPPG任务中实现了稳定有效的性能，无需手动掩码或微调即可高效生成。

Conclusion: 该方法可作为即插即用组件快速部署到现有IPPG框架中，解决了对面部特写的过度依赖，促进了电影级角色-场景创作，为相关领域提供了更丰富的个性化生成能力。
...
TL;DR: 本文针对基于变换的黑盒攻击中参数优化的盲点问题，提出了动态参数优化方法(DPO)，通过经验研究发现迁移性的三种动态模式，并基于上升-下降模式将参数优化复杂度从O(mn)降低到O(nlogm)。


<details>
  <summary>Details</summary>
Motivation: 现有基于变换的攻击方法存在三个主要问题：1) 只考虑低迭代设置，无法反映高迭代下的性能；2) 对不同代理模型、迭代次数和任务使用统一参数；3) 传统网格搜索复杂度高，限制了参数优化。

Method: 首先通过经验研究揭示变换参数强度与迁移性之间的三种动态模式，提出同心衰减模型(CDM)解释这些模式，然后基于上升-下降模式设计高效的动态参数优化(DPO)方法。

Result: 在不同代理模型、迭代次数和任务上的综合实验表明，DPO方法能显著提升基于变换攻击的迁移性。

Conclusion: 本文提出的动态参数优化方法有效解决了变换攻击中的参数优化问题，显著提升了黑盒攻击的迁移能力。
...
TL;DR: 提出LithoSeg，一种用于光刻SEM图像分割的粗到细网络，通过人机协作的SAM引导和1D回归方法实现高精度分割和测量


<details>
  <summary>Details</summary>
Motivation: 现有方法在光刻SEM图像分割中缺乏足够的精度和鲁棒性，限制了实际应用，需要更精确的分割方法来确保工艺控制和半导体制造良率

Method: 采用粗到细两阶段方法：粗阶段使用人机协作的SAM引导方案获得鲁棒性；细阶段将2D分割转化为1D回归问题，使用轻量级MLP进行点级精化

Result: LithoSeg在分割精度和计量精度方面均优于先前方法，同时需要更少的监督，为实际应用提供了良好前景

Conclusion: LithoSeg通过创新的粗到细网络架构和人机协作方法，有效解决了光刻SEM图像分割的精度和鲁棒性问题
...
TL;DR: 提出SIT-ADDA-Auto框架，仅调整卷积网络早期层而非整个网络，实现显微镜图像跨仪器和设置的可靠域适应，无需目标标签。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在不同显微镜仪器或采集设置下性能下降的问题，传统域适应方法会破坏已学习的语义表示。

Method: SIT-ADDA-Auto框架：仅适应最早卷积层并冻结深层，结合浅层对抗对齐和预测不确定性自动选择适应深度。

Result: 在曝光、光照变化、跨仪器迁移和多种染色条件下，相比全编码器适应和非对抗基线，SIT-ADDA改进了重建和分割性能，减少语义特征漂移。

Conclusion: 为显微镜无标签域适应提供了设计规则，代码已公开。
...
TL;DR: 提出基于多摄像头计算机视觉的实时交通安全评估框架，通过后侵入时间(PET)计算在交叉路口进行风险分析，实现高分辨率实时监测。


<details>
  <summary>Details</summary>
Motivation: 传统基于事故的交通安全分析受限于数据稀疏性和延迟问题，需要实时、连续的安全评估方法。

Method: 使用四个同步摄像头覆盖交叉路口，在NVIDIA Jetson AGX Xavier设备上运行YOLOv11分割进行车辆检测，通过单应性矩阵将车辆多边形转换为统一鸟瞰图，开发像素级PET算法进行精确风险测量。

Result: 系统能够以亚秒级精度识别高风险区域，在边缘设备上实现实时处理，平均2.68 FPS生成800x800像素对数热力图，精度达3.3平方厘米。

Conclusion: 验证了去中心化视觉PET分析在智能交通系统中的可行性，为高分辨率、实时、可扩展的交叉路口安全评估提供了可复现的方法论。
...
TL;DR: 本文提出LIHE框架解决弱监督广义指代表达理解(WGREC)问题，通过两阶段方法处理零个或多个目标的表达，结合双曲几何和欧几里得相似度来避免语义崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督指代表达理解方法受限于一对一映射假设，无法处理现实场景中对应零个或多个目标的表达，需要更实用的WGREC范式。

Method: 提出LIHE框架：第一阶段指称解耦预测目标数量并分解复杂表达；第二阶段指称定位使用HEMix混合相似度模块，结合欧几里得精确对齐和双曲几何层次建模优势。

Result: 在gRefCOCO和Ref-ZOM上建立了首个有效的弱监督WGREC基线，HEMix在标准REC基准上IoU@0.5提升达2.5%。

Conclusion: LIHE成功解决了WGREC中的监督信号模糊和语义表示崩溃问题，为弱监督广义指代表达理解提供了有效解决方案。
...
TL;DR: 提出NSDD方法，通过蒸馏迭代DDNM+求解器的零空间组件，在无配对监督下实现快速、逼真的无透镜相机重建


<details>
  <summary>Details</summary>
Motivation: 现有无透镜相机重建方法依赖配对监督会导致域不匹配偏差，而通用扩散先验方法在无透镜去卷积的噪声、高度复用和病态设置下表现不佳

Method: NSDD方法：单次通过的学生网络，蒸馏迭代DDNM+求解器的零空间组件，以无透镜测量和范围空间锚点为条件

Result: 在Lensless-FFHQ和PhlatCam数据集上，NSDD是第二快的方法（仅次于Wiener），达到接近教师模型的感知质量（第二好LPIPS，低于DDNM+），优于DPS和经典凸优化基线

Conclusion: NSDD为实现快速、无需真实标签、逼真的无透镜成像提供了一条实用路径
...
TL;DR: VL-SurgPT是首个大规模多模态手术点跟踪数据集，结合视觉跟踪与文本描述，包含908个手术视频片段，涵盖组织和器械跟踪，并提出TG-SurgPT文本引导跟踪方法，在视觉挑战条件下显著提升跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 现有手术跟踪数据集缺乏语义上下文，无法理解跟踪失败机制。复杂手术环境中的烟雾遮挡、镜面反射和组织变形等挑战使得准确点跟踪仍然困难。

Method: 构建VL-SurgPT数据集（908个手术视频，包含754个组织跟踪和154个器械跟踪），建立8种最先进跟踪方法的基准，并提出TG-SurgPT文本引导跟踪方法，利用语义描述提升鲁棒性。

Result: 实验结果表明，结合点状态信息显著提高了跟踪准确性和可靠性，特别是在视觉挑战场景下，传统纯视觉方法表现不佳时效果更明显。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在长视频理解中的固有标记限制和长期时间依赖捕捉困难的问题，现有方法难以捕获全局上下文和复杂事件关系。

Method: 提出GCAgent框架，核心创新是示意图和叙事情景记忆，将事件及其因果和时间关系结构化建模为有组织的上下文，采用多阶段感知-行动-反思循环，使用记忆管理器检索相关情景上下文。

Result: 在Video-MME Long分割上相比强基线MLLM提升23.5%准确率，在7B规模MLLM中达到最先进性能，Long分割准确率73.4%，总体平均71.9%。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖视觉线索，常违反物理约束（如穿透或非接触），而现有物理推理方法通常依赖后优化或不可微分物理引擎，损害视觉一致性和端到端可训练性

Method: 1) 视觉-物理线索联合学习：训练模型提取2D视觉线索和3D物理线索；2) 候选姿态聚合：通过扩散生成多个候选姿态，利用视觉和物理预测进行聚合，得到最终估计

Result: 在姿态精度和物理合理性方面显著优于现有最先进方法

Conclusion: 提出的联合视觉和物理线索的框架能够产生视觉一致且物理合理的手-物体3D姿态估计结果
...
TL;DR: KA-MIG是一个知识增强的掩码图像生成框架，通过引入三种先验知识图来增强模型对语义依赖关系的理解，从而提高生成质量。
...
Method: 提出KA-MIG框架，引入三种先验知识图：共现图、语义相似度图和位置-标记不兼容图，设计图感知编码器学习token和位置感知表示，并通过轻量级融合机制集成到现有MIG方法中。
...
TL;DR: 本文提出CalMRL方法，通过表示级插补解决多模态学习中模态缺失导致的锚点偏移问题，使现有方法能够处理不完整模态数据。


<details>
  <summary>Details</summary>
Motivation: 现有多模态学习方法需要所有模态同时存在，无法有效利用普遍存在的缺失模态数据集。从锚点偏移理论角度分析，缺失模态会导致局部锚点偏离最优锚点。

Method: 提出CalMRL方法，利用模态先验和内在联系在表示层面对缺失模态进行建模插补，采用双步学习方法和共享潜在变量的后验分布闭式解解决优化困境。

Result: 理论验证了CalMRL能够缓解锚点偏移并保证收敛，实验证明其优越性，为现有先进方法提供了处理缺失模态数据的灵活性。

Conclusion: CalMRL通过校准缺失模态引起的对齐偏差，有效解决了多模态表示学习中的模态缺失问题，扩展了方法的适用性。
...
TL;DR: SRSplat是一个前馈式3D重建框架，能从少量低分辨率视图重建高分辨率3D场景，通过结合外部参考图像和内部纹理线索来补偿纹理信息不足。
...
Method: 构建场景特定参考图库，使用多模态大语言模型和扩散模型生成；引入参考引导特征增强模块对齐融合低分辨率输入和参考图像特征；训练解码器预测高斯基元；使用纹理感知密度控制自适应调整高斯密度。
...
TL;DR: 提出FedSDA方法，通过扩散模型和染色分离技术对齐联邦学习中非IID组织病理图像的染色分布，缓解分布偏移问题


<details>
  <summary>Details</summary>
Motivation: 联邦学习中非IID数据分布是主要挑战，特别是在组织病理图像中，特征分布偏移严重影响模型性能

Method: 基于扩散模型拟合数据分布，利用染色分离提取关键特征，在联邦学习框架下对齐各客户端的染色分布到目标分布
...
TL;DR: DCMM-Transformer是一种新颖的医学图像分析ViT架构，通过将度校正混合成员模型作为自注意力中的加性偏置，解决了标准ViT无法利用潜在解剖结构的问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像存在潜在解剖分组（如器官、组织、病理区域），但标准ViT无法有效利用这些结构。现有方法如SBM-Transformer存在不可微分、训练不稳定和无法建模复杂社区结构的问题。

Method: 提出DCMM-Transformer，将度校正混合成员模型作为自注意力中的加性偏置引入，完全可微分且可解释，避免了乘性掩码和二元采样。

Result: 在多种医学影像数据集（脑部、胸部、乳腺、眼部）上的综合实验表明，该方法具有优越的性能和泛化能力。

Conclusion: 该方法不仅提升了性能，还通过学习的组结构和结构化注意力调制显著增强了可解释性，产生了解剖学意义和语义连贯的注意力图。
...
TL;DR: 提出DeiTFake方法，使用DeiT-based transformer和两阶段渐进式训练策略进行深度伪造检测，在OpenForensics数据集上达到99.22%准确率
...
Method: 采用DeiT-based transformer架构，结合两阶段渐进式训练策略：第一阶段使用标准数据增强进行迁移学习，第二阶段使用高级仿射变换和深度伪造特定增强进行微调

Result: 在OpenForensics数据集（190,335张图像）上，第一阶段达到98.71%准确率，第二阶段达到99.22%准确率和0.9997 AUROC，优于最新基线方法

Conclusion: 该方法通过知识蒸馏模型捕捉细微篡改痕迹，增强检测模型的鲁棒性，为面部深度伪造检测提供了实用基准
...
<details>
  <summary>Details</summary>
Motivation: 监督方法依赖大量成对标注限制了可扩展性，而无监督方法由于跨视角域差距导致伪标签噪声问题，需要解决这些限制。

Method: 提出双阶段框架：1) 视角感知对抗桥接(VAAB)建模视角不变特征增强伪标签鲁棒性；2) 异构图过滤校准(HGFC)构建双视图间结构图来精化跨视角关联。

Result: 在University-1652数据集上Satellite→Drone AP提升+10.63%，在SUES-200数据集上提升+16.73%，甚至超过了监督基线方法。

Conclusion: UniABG通过结合对抗学习和图结构建模，在无监督跨视角地理定位中实现了显著性能提升，展示了其有效性和实用性。
...
TL;DR: 提出PipeDiT框架，通过流水线并行化技术加速视频生成，在OpenSoraPlan和HunyuanVideo上实现1.06-4.02倍加速


<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)模型在视频生成中表现出色，但实际部署受限于推理速度慢和内存消耗高的问题

Method: 1. 设计序列并行流水线算法(PipeSP)实现GPU间计算与通信流水化；2. 提出DeDiVAE将扩散模块与VAE模块解耦到不同GPU组；3. 提出注意力协同处理(Aco)方法优化VAE组GPU资源利用

Result: 在8-GPU系统上测试，在多种常见分辨率和时间步配置下，相比OpenSoraPlan和HunyuanVideo实现1.06-4.02倍加速

Conclusion: PipeDiT框架通过创新的流水线并行化技术有效解决了视频生成模型的推理延迟和内存消耗问题
...
TL;DR: MovSemCL是一个基于运动语义对比学习的轨迹相似度计算框架，通过运动语义特征提取、分层表示和物理合理的增强策略，解决了现有方法在语义建模、计算效率和增强合理性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的轨迹相似度计算方法存在三个主要问题：(1) 对轨迹语义和层次结构建模不足；(2) 逐点编码导致计算成本高；(3) 使用物理上不合理的增强方法扭曲轨迹语义。

Method: 首先将原始GPS轨迹转换为运动语义特征并分割成补丁，然后使用内部和跨补丁注意力编码局部和全局轨迹模式，实现高效的分层表示。还包含曲率引导的增强策略，保留信息丰富的段并屏蔽冗余段。

Result: 在真实世界数据集上的实验表明，MovSemCL在相似性搜索任务中达到接近理想值1的平均排名，在启发式近似方面提升高达20.3%，同时推理延迟降低高达43.4%。

Conclusion: MovSemCL能够超越现有最先进方法，在保持物理合理性的同时显著提高轨迹相似度计算的性能和效率。
...
TL;DR: DCA-LUT是首个基于深度学习的紫边去除框架，通过色差感知坐标变换模块分离紫边到专用维度，并使用5D查找表进行高效色彩校正。


<details>
  <summary>Details</summary>
Motivation: 传统紫边去除方法依赖昂贵的消色差镜头硬件和手工特征提取，忽视了数据驱动方法。紫边是由镜头色散导致的RGB通道空间错位引起的。

Method: 提出色差感知坐标变换模块学习图像自适应色彩空间，将紫边分离到专用维度；使用5D查找表进行非线性色彩映射；构建大规模合成紫边数据集PF-Synth。

Result: 在合成和真实数据集上的广泛实验表明，该方法在紫边去除方面达到了最先进的性能。

Conclusion: DCA-LUT框架通过深度学习方法有效解决了紫边问题，实现了高质量的图像恢复。
...
TL;DR: VAEmotionLLM是一个两阶段框架，通过有限音频预训练教会视觉语言模型跨模态理解情感，在艺术情感基准上达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有工作多为单模态或人类中心，忽视了艺术作品有意表达的情感，且当前音视频语言模型需要大规模音频预训练，限制了可扩展性

Method: 第一阶段VG-Align通过视觉引导音频对齐，将冻结的视觉通路蒸馏到新的音频通路；第二阶段轻量级EmoAdapter通过情感增强器和情感监督器注入情感敏感残差并应用情感监督

Result: 在ArtEmoBenchmark上达到最先进结果，优于音频、视觉和音视频基线，消融实验显示各组件具有互补性

Conclusion: 提出的框架能够以有限音频预训练实现跨模态情感理解，为音视频语言模型提供了更可扩展的解决方案
...
Method: 采用多模态提示驱动量化框架，利用预训练模型的文本嵌入作为原型先验，构建双约束量化空间（紧凑性和分离性正则化），使用Gumbel-Softmax实现可微分离散化。

Result: 在ModelNet40和ScanObjectNN数据集上的广泛实验证明了该方法的优越有效性。

Conclusion: 所提出的多模态提示驱动量化框架能够有效解决视觉-语言语义鸿沟问题，生成同时编码几何和语义信息的混合表示。
...
Method: 使用改进的ResNet-101架构，结合概率推理来模拟标签依赖关系和不确定性，提升预测准确性。

Result: 在COCO-2014数据集上达到0.794 mAP，优于ResNet-SRN（0.771）和Vision Transformer基线（0.785）。

Conclusion: 将概率推理集成到深度学习模型中，能够有效解决多标签场景中的挑战，实现最先进的分类性能。
...
TL;DR: 提出SemanticStitch框架，通过融入前景对象的语义先验来改善图像拼接质量，解决传统方法忽视语义信息导致的前景连续性破坏问题。


<details>
  <summary>Details</summary>
Motivation: 图像拼接因拍摄角度、位置差异和物体移动导致错位和视觉差异，传统接缝雕刻方法忽视语义信息，破坏前景连续性。

Method: 基于深度学习的框架，融入前景对象的语义先验，提出新颖的损失函数强调显著对象的语义完整性。

Result: 实验结果显示相比传统技术有显著改进，为实际应用提供有力支持。

Conclusion: SemanticStitch框架通过语义先验有效提升图像拼接质量，解决了传统方法的局限性。
...
TL;DR: 提出了一种分层分组提示调优方法用于持续学习，通过层分组共享提示和根提示生成子提示来减少层间独立性，提高模型稳定性并缓解灾难性遗忘。
...
Method: 分层分组提示调优方法：1) 同组层共享大致相同的提示，通过位置编码调整；2) 使用单一任务特定根提示生成各层组的子提示，增强子提示间的协同性。

Result: 在四个基准测试上的广泛实验表明，该方法相比多个最先进方法取得了优越性能。

Conclusion: 提出的分层分组提示调优方法通过增强层间协同性和保持预训练模型内在特征关系，有效提高了持续学习中的模型稳定性。
...
TL;DR: PACE是首个面向SNN和事件视觉的数据集蒸馏框架，通过压缩大型训练数据集为紧凑的合成数据集，显著降低SNN训练成本。


<details>
  <summary>Details</summary>
Motivation: SNN由于时间编码导致训练成本高昂，限制了实际部署。事件相机与SNN的生物启发特性相契合，但训练效率问题阻碍了其应用。

Method: 采用两个核心模块：ST-DSM使用残差膜电位来稠化基于尖峰的特征，并进行细粒度的时空匹配；PEQ-N提供即插即用的概率整数量化器。

Result: 在多个数据集上超越现有方法，特别是在动态事件流和低IPC条件下表现优异。在N-MNIST上达到84.4%准确率，训练时间减少50倍以上，存储成本降低6000倍。

Conclusion: PACE能够实现分钟级的SNN训练和高效的边缘部署，为事件视觉和SNN的实际应用提供了可行的解决方案。
...
TL;DR: SpikeNM是首个面向SNN的半结构化(N:M)剪枝框架，通过M路基对数参数化和可微分top-k采样器，线性化复杂度并实现更激进的稀疏化，同时引入神经科学启发的资格蒸馏来稳定高稀疏度下的搜索。


<details>
  <summary>Details</summary>
Motivation: 解决SNN深层架构参数和计算成本膨胀的问题，现有剪枝方法要么难以硬件加速(非结构化)，要么缺乏灵活性且精度下降(结构化)，需要一种兼顾硬件友好性和精度的半结构化剪枝方案。

Method: 采用M路基对数参数化和可微分top-k采样器线性化复杂度，提出资格启发蒸馏(EID)将时间累积信用转换为块级软目标，对齐掩码概率与脉冲动态。

Result: 在2:4稀疏度下，SpikeNM在主流数据集上保持甚至提升了性能，同时产生硬件友好的稀疏模式，补充了内在的脉冲稀疏性。

Conclusion: SpikeNM成功实现了SNN的半结构化剪枝，在保持精度的同时获得硬件友好的稀疏模式，为边缘部署提供了有效的解决方案。
...
TL;DR: 提出DGCF框架，结合冻结的自监督DINOv3 Transformer和可训练的CNN编码器-解码器，通过可学习的交叉融合模块分层融合全局表示和局部特征，实现局部外观和上下文表示的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有CNN模型缺乏全局语义理解，而Transformer在小规模医学数据集上容易过拟合。需要解决这些限制以实现高效的放射剂量规划和自适应放疗。

Method: 使用冻结的DINOv3 Transformer与可训练CNN编码器-解码器结合，通过交叉融合模块分层融合特征，并引入多级DINOv3感知损失。

Result: 在SynthRAD2023骨盆数据集上，DGCF在MRI→CT和CBCT→CT转换任务中，在MS-SSIM、PSNR和基于分割的指标上达到最先进性能。
...
TL;DR: 提出Adaptive Begin-of-Video Tokens (ada-BOV)方法，用于改进自回归视频扩散模型，解决现有方法在生成长视频时的连贯性和动态质量不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在生成长视频时存在两种主要范式：基于块的扩展和流式去噪。前者存在去噪延迟和误差累积问题，后者在连贯性和运动动态方面表现不佳。

Method: 提出自适应BOV令牌，通过类似自适应层归一化的调制吸收去噪的前帧；提出流式去噪细化策略，分离采样轨迹长度与注意力窗口约束；提出扰动增强训练噪声调度。

Result: 在多个指标上取得了令人信服的定性和定量结果，显著提升了长视频生成的连贯性和动态质量。

Conclusion: ada-BOV方法有效解决了自回归视频扩散模型在生成长视频时的关键挑战，在保持全局一致性的同时提升了局部动态质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉模型依赖有限充分原因进行预测，对分布偏移敏感。模型关键区域被遮挡后模型会误分类，而人类仍能识别，表明模型学习依赖关系不够因果充分

Method: 基于LIMA归因方法开发Counterfactual LIMA识别最小空间区域集，用自然背景替换这些区域进行数据增强，联合训练原始和增强样本

Result: 在多个ImageNet变体上实验显示，SS-CA提高了ID测试数据泛化能力，在ImageNet-R和ImageNet-S等OOD基准上表现更优，在噪声扰动下也展现增强的泛化能力

Conclusion: SS-CA有效利用可解释性见解纠正模型缺陷，提升性能和鲁棒性
...
TL;DR: BdSL-SPOTER是一个基于姿态的transformer框架，用于准确高效地识别孟加拉手语，在BdSLW60基准测试上达到97.92%的Top-1验证准确率，比Bi-LSTM基线提升22.82%。


<details>
  <summary>Details</summary>
Motivation: 为低资源区域手语（如孟加拉手语）开发一个实用且高效的识别框架，解决数据有限和计算成本高的问题。

Method: 扩展SPOTER范式，结合文化特定的预处理、紧凑的四层transformer编码器、优化的可学习位置编码，并使用课程学习来增强泛化能力和加速收敛。

Result: 在BdSLW60基准上实现97.92%的Top-1验证准确率，参数数量减少，FLOPs更低，FPS更高，计算成本保持较低水平。
...
TL;DR: TEMPO是一个基于深度学习从高分辨率卫星图像生成的全球时间分辨建筑密度和高度数据集，提供2018年第一季度至2025年第二季度的季度更新数据。


<details>
  <summary>Details</summary>
Motivation: 需要大规模监测全球发展模式和气候影响，为全球韧性和适应工作提供支持，但现有方法计算成本高。

Method: 使用现有建筑足迹和高度数据与季度PlanetScope卫星图像配对，训练多任务深度学习模型预测37.6米分辨率的建筑密度和高度。

Result: 验证显示F1分数在85%-88%之间，时间稳定性高（五年趋势一致性得分0.96），能以较低计算成本捕获季度变化。

Conclusion: TEMPO数据集为大规模监测建成区变化提供了高效解决方案，支持全球发展模式和气候影响研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于DINOv2的深度伪造检测方法将其视为通用二分类问题，忽略了不同伪造方法产生的独特伪影特征，需要更精细化的检测方法。

Method: 在DINOv2的每个transformer块中集成轻量级多头LoRA模块，构建共享分支将细粒度伪造方法线索传播到真实性检测头，实现多任务协同优化。

Result: 仅使用3.5M可训练参数，该方法在检测精度上达到或超越了当前复杂的先进方法。

Conclusion: DFF-Adapter通过参数高效的方式实现了对深度伪造的细粒度检测，证明了利用伪造方法特定知识能够显著增强真实性判别能力。
...
TL;DR: 提出了MEMR-Seg任务，通过多轮对话和实体级推理进行医学图像分割，构建了MR-MedSeg数据集，并开发了MediRound模型来解决多轮分割中的错误传播问题。
...
Method: 构建MR-MedSeg数据集，提出MediRound基线模型，引入轻量级的判断与校正机制来缓解多轮分割中的错误传播。

Result: 实验结果表明该方法有效解决了MEMR-Seg任务，并优于传统的医学参考分割方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决稀疏和噪声雷达点导致运动感知不精确的问题，提升自动驾驶系统在光学传感器性能下降时的感知能力。

Method: 使用两帧连续雷达回波信号，在统一架构中联合建模雷达目标检测和运动估计，设计基于多普勒频移和回波强度的自监督损失函数。

Result: 在公共数据集上的实验表明，RadarMP在各种天气和光照条件下实现了可靠的运动感知，优于基于雷达的解耦运动感知方法。

Conclusion: RadarMP增强了全场景自动驾驶系统的感知能力，特别是在恶劣天气条件下表现出色。
...
TL;DR: 提出了OAD-Promoter方法，通过缓解语言偏见和提升领域迁移鲁棒性来增强基于LLM的视觉问答系统。


<details>
  <summary>Details</summary>
Motivation: LLM在视觉问答中面临语言偏见和领域外泛化能力不足的问题，限制了其在少样本或零样本场景下的可靠性。

Method: 包含三个模块：OEG模块生成全局描述和对象集中样本，MKA模块从存储示例中检索相关知识，OAD提示整合前两个模块的输出优化LLM推理。

Result: 实验表明OAD-Promoter显著提升了LLM-based VQA方法在少样本或零样本设置下的性能，达到了新的最先进水平。

Conclusion: OAD-Promoter有效解决了LLM在视觉问答中的语言偏见和领域泛化问题，为知识密集型视觉问答提供了可靠解决方案。
...
TL;DR: 本文提出了一个轻量级的基于C的SNN推理运行时，通过优化数据布局、预分配和稀疏性剪枝，在边缘设备上实现高效推理，速度提升约10倍，内存大幅减少。


<details>
  <summary>Details</summary>
Motivation: SNN具有事件驱动特性，在时序处理和能效方面有优势，但在资源受限的边缘设备上训练和部署仍面临挑战。

Method: 将SNNTorch训练的模型转换为紧凑的C表示，采用静态缓存友好的数据布局和预分配，避免解释器和分配开销；利用稀疏脉冲活动剪枝非活跃神经元和突触。

Result: 在N-MNIST和ST-MNIST数据集上实现与Python基线功能对等，在桌面CPU上速度提升约10倍，剪枝带来额外增益，内存大幅减少，可在Arduino Portenta H7等微控制器上部署。
...
TL;DR: MAVIS是首个评估多模态来源归因系统的基准，包含157K视觉问答实例，答案带有事实级引用。研究发现多模态RAG比单模态RAG生成更丰富流畅的答案，但在图像文档的接地性方面较弱，且存在信息丰富性与接地性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注纯文本场景，忽视了多模态在来源归因中的作用。需要开发能够理解视觉问题意图、检索多模态证据并生成带引用的长文本答案的系统。

Method: 构建包含157K视觉问答实例的数据集，每个答案都标注了指向多模态文档的事实级引用。开发了基于信息丰富性、接地性和流畅性三个维度的自动评估指标。

Result: 多模态RAG比单模态RAG生成更丰富流畅的答案，但对图像文档的接地性较弱；不同提示方法在信息丰富性和接地性之间存在权衡；缓解图像文档解释中的上下文偏见是未来关键方向。

Conclusion: MAVIS基准揭示了多模态来源归因系统的重要发现，特别是在图像文档接地性方面的挑战，为未来研究提供了关键方向。
...
TL;DR: TMKT框架通过时间步混合策略和模态感知目标，实现从RGB到DVS的跨模态知识迁移，提升脉冲神经网络在事件相机数据上的分类性能。


<details>
  <summary>Details</summary>
Motivation: 事件相机和脉冲神经网络结合具有能效优势，但事件数据稀缺和DVS输出稀疏性阻碍有效训练。现有RGB到DVS的知识迁移方法因模态分布差异大而表现不佳。

Method: 提出时间步混合知识转移框架，包含概率性时间步混合策略，在序列内插值RGB和DVS输入创建平滑课程；引入两个轻量级模态感知目标：每帧源监督的模态感知指导和序列级混合比感知。

Result: 在多种基准测试和多个SNN骨干网络上进行广泛实验，证明该方法能实现更平滑的知识迁移，缓解训练中的模态不匹配，在脉冲图像分类任务中取得优越性能。

Conclusion: TMKT通过时间步混合和模态感知目标，有效解决了RGB到DVS跨模态知识迁移的挑战，为事件相机和脉冲神经网络的结合提供了有效的训练框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于流的免反演方法虽然高效，但缺乏有效的源信息整合，导致背景保留差、空间不一致和过度编辑问题。

Method: 提出频率交互注意力机制，包含频率表示交互模块（在自注意力中交换源和目标特征的频率分量）和特征注入模块（在交叉注意力中显式引入源侧查询、键、值和文本嵌入）。

Result: 在RTX 4090上每张512*512图像仅需约6秒，在视觉质量、背景保真度和可控性方面优于现有方法，并成功应用于医学图像出血合成，提升下游出血分类性能。
...
TL;DR: 提出Center-Reassigned Hashing (CRH)框架，通过动态重新分配预设码本中的哈希中心来优化深度哈希学习，避免了两阶段方法的复杂性和性能损失。
...
Method: CRH采用端到端框架，从预设码本中动态重新分配哈希中心，同时联合优化哈希函数，无需显式的中心优化阶段。还引入了多头机制来增强哈希中心的表示能力。

Result: 在三个基准数据集上的广泛实验表明，CRH能够学习到具有语义意义的哈希中心，并在检索任务中优于最先进的深度哈希方法。

Conclusion: CRH通过动态哈希中心重新分配和端到端优化，有效解决了现有方法的局限性，实现了更好的语义保持和检索性能。
...
TL;DR: 提出了一种新的点云补全范式Completion-by-Correction，通过预训练图像到3D模型生成拓扑完整的形状先验，然后进行特征空间校正以对齐部分观测，相比传统的Completion-by-Inpainting方法能更好地保持结构一致性。


<details>
  <summary>Details</summary>
Motivation: 传统基于修复的点云补全方法由于几何和语义约束有限，容易产生结构不一致和拓扑伪影问题，需要更鲁棒的补全范式。

Method: 提出PGNet多阶段框架：1) 双特征编码以锚定生成先验；2) 合成结构对齐的粗糙骨架；3) 通过分层校正逐步细化几何细节。

Result: 在ShapeNetViPC数据集上，PGNet在平均Chamfer距离上比现有最优方法降低23.5%，F-score提高7.1%。

Conclusion: Completion-by-Correction范式将补全任务从无约束合成转向引导式细化，能够实现结构一致且与观测对齐的重建效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归方法使用离散token会丢失细粒度信息，而连续空间建模虽然质量更高但面临效率挑战。

Method: 提出MixAR框架，使用离散token作为先验指导连续自回归预测，探索了DC-SA、DC-CA、DC-Mix等混合策略，并引入TI-Mix来弥合训练-推理差距。

Result: 实验表明DC-Mix策略在计算效率和生成保真度之间取得良好平衡，TI-Mix带来一致改进。

Conclusion: MixAR通过混合离散-连续表示有效提升了自回归图像生成的质量和效率。
...
TL;DR: MMRINet：一种轻量级脑肿瘤分割架构，使用线性复杂度的Mamba状态空间模型替代二次复杂度的注意力机制，在资源受限环境中实现高效的体积上下文建模。


<details>
  <summary>Details</summary>
Motivation: 在资源受限环境中，深度3D网络计算成本过高，难以实现多参数MRI中的自动脑肿瘤分割。

Method: 使用线性复杂度的Mamba状态空间模型替代注意力机制；提出双路径特征精炼模块最大化特征多样性；采用渐进式特征聚合实现有效的多尺度融合。

Result: 在BraTS-Lighthouse SSA 2025中，仅使用约250万参数，平均Dice分数达到0.752，平均HD95为12.23。

Conclusion: MMRINet在低资源临床环境中实现了高效且准确的脑肿瘤分割，适合资源受限的医疗环境部署。
...
TL;DR: 提出了一种两阶段跨视角、跨模态的无监督领域自适应框架，用于解决驾驶员活动识别中的视角变化和领域偏移问题，显著提升了模型在真实部署中的性能。
...
Method: 两阶段框架：第一阶段使用对比学习在多视角数据中学习视角不变和动作区分性特征；第二阶段使用信息瓶颈损失进行跨模态领域自适应，无需新领域的标注数据。

Result: 使用Video Swin和MViT视频变换器在Drive&Act数据集上评估，相比监督对比学习方法在RGB视频数据上提升近50%的top-1准确率，比仅使用无监督领域自适应方法提升5%。

Conclusion: 提出的联合框架有效解决了驾驶员活动识别中的跨视角和跨模态挑战，为模型在多样化车辆配置中的鲁棒部署提供了可行方案。
...
TL;DR: 提出了一种分层语义学习框架来解决跨域少样本分割中的粒度差距问题，通过双风格随机化和分层语义挖掘模块学习多粒度语义特征，提升模型在目标域中对新类别的语义识别能力。


<details>
  <summary>Details</summary>
Motivation: 现有跨域少样本分割方法主要关注源域和目标域之间的风格差异，而忽略了分割粒度差距，导致模型在目标域中对新类别的语义判别能力不足。

Method: 提出HSL框架，包含双风格随机化模块模拟目标域风格差异，分层语义挖掘模块利用多尺度超像素挖掘不同粒度下的类内一致性和类间区分性，以及原型置信度调制阈值模块缓解分割模糊问题。

Result: 在四个流行的目标域数据集上进行广泛实验，结果表明该方法达到了最先进的性能。

Conclusion: 所提出的分层语义学习框架有效解决了跨域少样本分割中的粒度差距问题，显著提升了模型在目标域中的语义识别能力。
...
TL;DR: OmniSparse是一个训练感知的细粒度稀疏注意力框架，专门为长视频多模态大语言模型设计，通过动态token预算分配在训练和推理中实现高效加速。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏注意力方法主要针对推理时加速，使用预定义稀疏模式选择关键token，但存在训练-推理差距，缺乏跨查询、键值对和注意力头的细粒度token选择能力，导致性能次优和加速效果有限。

Method: 包含三个自适应互补机制：1) 通过懒-主动分类进行查询选择，保留捕捉广泛语义相似性的主动查询；2) 基于最平坦头的共享预算进行头级动态预算分配的KV选择；3) 根据头级解码查询模式选择性获取视觉KV缓存的KV缓存瘦身。

Result: 实验结果显示OmniSparse在保持全注意力性能的同时，在预填充阶段实现最高2.7倍加速，在解码阶段实现2.4倍内存减少。

Conclusion: OmniSparse框架有效解决了现有稀疏注意力方法的局限性，通过训练感知的细粒度token选择实现了显著的性能保持和加速效果。
...
TL;DR: LSS3D是一种高质量图像到3D生成方法，通过可学习空间位移技术解决多视图不一致性和非正面输入视角问题，实现更完整的几何细节和清晰纹理的3D生成。
...
Method: 为每个视图分配可学习空间位移参数，在重建网格引导下调整视图实现空间一致性，并加入输入视图作为额外约束增强对非正面角度的鲁棒性。

Result: 在几何和纹理评估指标上均取得领先结果，特别对提升视角等非正面输入表现出更强的鲁棒性。

Conclusion: LSS3D方法能有效处理多视图不一致性，在更灵活的输入视角下实现高质量的3D生成，并提供了全面的量化评估流程。
...
<details>
  <summary>Details</summary>
Motivation: 多视图图像生成在3D重建、虚拟现实等领域有重要应用价值，现有基于单图像扩展的方法在保持跨视图一致性和生成高分辨率输出方面面临计算挑战。

Method: 设计了多视图几何信息提取模块（深度图、法线图、前景分割掩码），解耦几何增强注意力机制，自适应学习策略，迭代精炼过程和动态几何信息强度调节机制。

Result: 生成的图像在跨视图一致性和细节丰富度方面表现优异，能够确保形状和结构一致性，同时保持高质量细节。

Conclusion: 该模型通过几何引导有效解决了多视图图像生成的关键挑战，为3D重建等应用提供了高质量的解决方案。
...
TL;DR: 基于YOLOv8和EasyOCR的AI交通违规检测系统，可自动识别头盔佩戴违规、摩托车后视镜缺失，并进行车牌识别，提升交通执法效率。


<details>
  <summary>Details</summary>
Motivation: 传统人工交通执法资源密集且不一致，需要自动化解决方案来提高道路安全执法效率。

Method: 使用YOLOv8进行目标检测，EasyOCR进行车牌识别，基于自定义标注数据集，采用Streamlit构建实时监控界面，并应用高级图像预处理技术。

Result: 模型整体精度0.9147，召回率0.886，mAP@50为0.843，mAP@50:95为0.503，在严格IoU阈值下仍保持良好检测能力。

Conclusion: 该系统为自动化交通规则执法提供了实用有效的解决方案，具备实际部署潜力。
...
TL;DR: MoS是一种新型的多模态扩散模型融合范式，通过基于状态的可学习路由器实现模态间灵活交互，在文本到图像生成和编辑任务中达到最先进性能，参数效率极高。


<details>
  <summary>Details</summary>
Motivation: 传统多模态融合方法通常采用固定策略，无法根据扩散轨迹和输入内容动态调整模态交互，限制了模型性能和效率。

Method: 提出可学习的token-wise路由器，基于去噪时间步和输入内容稀疏选择前k个隐藏状态，采用ε-greedy训练策略，实现模态间精确对齐。

Result: MoS-Image和MoS-Editing在文本到图像生成和编辑任务中达到SOTA，仅用3B-5B参数即可匹配或超越4倍大的模型。

Conclusion: MoS为多模态扩散模型提供了一种灵活且计算高效的扩展范式，显著提升了参数利用效率。
...
TL;DR: FaNe是一个语义增强的医学视觉语言预训练框架，通过语义感知正样本挖掘、文本条件稀疏注意力池化和硬负样本感知对比损失，解决了现有方法中的假阴性和细粒度跨模态对齐不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言预训练方法存在两个主要限制：语义相似文本导致的假阴性问题，以及细粒度跨模态对齐不足。
...
Conclusion: FaNe框架通过解决假阴性和细粒度对齐问题，有效提升了医学视觉语言预训练的性能，在多个下游任务中表现出色。
...
TL;DR: SRF是一种无需训练的方法，通过分析特征协方差结构来抑制视觉语言模型的幻觉，通过特征分解识别幻觉模式，并在深层投影权重中应用软谱滤波器来减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型经常产生幻觉，描述图像中不存在的对象、属性或关系，这是由于过度依赖语言先验和不精确的跨模态对齐造成的。

Method: 引入谱表示过滤(SRF)，通过分析模型表示的协方差结构来抑制幻觉。该方法识别低秩幻觉模式，并在深层vLLM层的投影权重中应用软谱滤波器来衰减这些模式。

Result: 在LLaVA-1.5、MiniGPT-4和mPLUG-Owl2等VLM上，SRF在MSCOCO、POPE-VQA等基准测试中持续降低幻觉率，在不降低描述质量的情况下达到最先进的忠实度。

Conclusion: SRF是一种轻量级、无需训练的后处理方法，无需推理开销或架构修改，能有效抑制VLM的幻觉问题。
...
TL;DR: DHMI是首个针对深度哈希的扩散模型反演框架，能在黑盒设置下成功重构高分辨率、高质量的原始训练图像，揭示了深度哈希系统的严重隐私风险。


<details>
  <summary>Details</summary>
Motivation: 深度哈希虽然提高了检索效率，但引入了严重的隐私风险——从哈希码重构原始训练数据可能导致生物特征伪造和隐私泄露。然而针对深度哈希的模型反演攻击尚未被探索。

Method: DHMI首先对辅助数据集进行聚类得到语义哈希中心作为代理锚点，然后引入代理引导的去噪优化方法，使用融合分类一致性和哈希邻近度的新攻击度量来动态选择候选样本，通过代理模型集群指导候选样本的细化。

Result: 在多个数据集上的实验表明，DHMI即使在最具挑战性的黑盒设置下也能成功重构高分辨率、高质量的图像，在黑盒场景下优于现有的最先进模型反演攻击方法。

Conclusion: DHMI证实了深度哈希系统固有的严重隐私风险，其方法在实际效果上表现出色，为深度哈希系统的安全性评估提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: Video Browser Showdown (VBS)需要在严格时间限制下提供准确结果，因此需要开发快速高效且易用的视频检索系统。
...
Conclusion: Fusionista2.0被证实为大规模视频搜索的竞争性且用户友好的系统。
...
TL;DR: 提出基于MedSigLIP的提示条件框架，通过FiLM和多尺度池化注入文本先验，在LDCTIQA2023挑战中取得优异性能


<details>
  <summary>Details</summary>
Motivation: 通过文本提示将临床意图条件化到补丁令牌特征中，实现数据高效学习和快速适应

Method: 使用特征级线性调制(FiLM)和多尺度池化，结合全局、局部和纹理感知池化，通过轻量级MLP融合，使用成对排序损失训练

Result: 在LDCTIQA2023数据集上，PLCC=0.9575，SROCC=0.9561，KROCC=0.8301，超越已发表的最佳挑战提交结果

Conclusion: 提示引导方法在低剂量CT图像质量评估中表现出有效性
...
TL;DR: 提出了一种新颖的双阶段疾病感知框架，用于胸部X光报告生成，通过疾病感知语义标记和视觉-疾病注意力融合，显著提升了临床准确性和语言质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在医学图像分析中缺乏足够的疾病感知能力和视觉-语言对齐，导致忽略关键病理特征且难以生成临床准确的报告。
...
TL;DR: CrossVid是首个专门评估多模态大语言模型在跨视频推理中时空推理能力的基准测试，包含4个高层维度和10个具体任务，提供5,331个视频和9,015个问答对。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准主要关注单视频分析，无法评估MLLMs同时推理多个视频的能力，而现有的多视角视频基准任务有限，无法全面评估真实世界中的跨视频推理场景。

Method: 构建CrossVid基准，涵盖广泛的层次化任务，包括四个高层维度和十个具体任务，提供多种视频和问答格式（单选、多选、开放式）。

Result: 在各类开源和闭源MLLMs上的实验表明，Gemini-2.5-Pro表现最佳，平均准确率为50.4%。大多数MLLMs在跨视频推理任务上表现不佳，主要原因是无法整合或比较分布在多个视频中的证据。

Conclusion: CrossVid基准能够指导未来增强MLLMs跨视频推理能力的研究，当前MLLMs在整合多视频证据方面仍存在显著挑战。
...
TL;DR: 提出了ZoomEarth框架，通过主动感知范式处理超高分辨率遥感图像，使用自适应裁剪缩放和区域引导奖励机制，在LRS-GRO基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有动态分辨率和令牌剪枝方法受限于被动感知范式，在获取更精细视觉输入时会产生冗余。需要探索主动感知范式，使模型能够重新访问信息丰富区域。

Method: 提出ZoomEarth自适应裁剪缩放框架，包含区域引导奖励机制。通过监督微调(SFT)和组相对策略优化(GRPO)进行训练。

Result: 在LRS-GRO基准测试中达到最先进性能，在零样本设置下在三个公共UHR遥感基准测试中表现优异。

Conclusion: ZoomEarth框架具有强大的通用性和可扩展性，可通过简单工具接口与下游模型无缝集成，用于云去除、去噪、分割和图像编辑等任务。
...
TL;DR: TM-UNet是一种轻量级医学图像分割框架，通过多尺度令牌-记忆块将2D空间特征转换为令牌序列，利用矩阵记忆单元选择性保留和传播上下文信息，在显著降低计算成本的同时实现高效分割。


<details>
  <summary>Details</summary>
Motivation: 基于transformer的方法在医学图像分割中取得了显著成果，但高计算成本阻碍了临床部署，需要开发更高效的轻量级框架。
...
Conclusion: TM-UNet通过创新的令牌-记忆机制实现了高效的全局推理和长程依赖建模，为临床部署提供了可行的轻量级解决方案。
...
Method: 使用决策器引导的动态token合并方法，在去噪步骤中基于前一步生成的token构建重要性图，保留最显著的token并通过相似性聚合合并冗余token，采用动态变化的合并比例

Result: 实验表明D³ToM能够显著加速推理速度，同时在同等计算预算下保持竞争性性能

Conclusion: D³ToM是一种即插即用的模块，可有效解决扩散MLLMs推理速度慢的问题，为实际应用提供了可行的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶等复杂视觉系统中，精确的多传感器对齐至关重要。现有方法通常依赖分离的成对标定，无法实现一次性联合标定。

Method: 设计并构建了新型3D标定靶标，包含平面特征、ChArUco图案和主动LED模式，分别针对LiDAR、RGB相机和事件相机的特性。实现一次性联合外参标定过程。

Result: 在定制数据集上进行了广泛实验评估，使用先进自动驾驶传感器设置记录，验证了方法的准确性和鲁棒性。

Conclusion: 该方法能够准确标定复杂视觉系统，为自动驾驶应用提供了可靠的多传感器对齐解决方案。
...
TL;DR: 本文提出频率重校准方法(FreRec)来解决医学AI生成数据增强中的频率失准问题，通过统计高频替换和重建高频映射来改善生成图像质量，提升下游分类任务性能。


<details>
  <summary>Details</summary>
Motivation: 医学AI开发面临数据稀缺问题，生成数据增强(GDA)是解决方案，但存在偏见风险，特别是频率失准可能导致不可靠的GDA，损害下游任务。

Method: 提出FreRec方法，包含：(1)统计高频替换(SHR)粗略对齐高频分量；(2)重建高频映射(RHM)增强图像质量并重建高频细节。

Result: 在脑部MRI、胸部X光、眼底图像等多种医学数据集上的实验表明，FreRec显著提升了下游医学图像分类性能。

Conclusion: FreRec是一个独立的后处理步骤，兼容任何生成模型，可无缝集成到常见医学GDA流程中，有效解决频率失准问题。
...
TL;DR: LiDAR-GS++是一种基于高斯泼溅的LiDAR重建方法，通过扩散先验增强，解决了单次扫描重建不完整导致的伪影问题，实现了实时高保真重模拟。


<details>
  <summary>Details</summary>
Motivation: 现有的基于高斯泼溅的LiDAR渲染方法在单次扫描重建不完整的情况下，外推新视角合成时会出现伪影，需要解决这一限制。

Method: 引入可控的LiDAR生成模型，基于粗略外推渲染生成额外的几何一致扫描，并采用有效的蒸馏机制进行扩展重建。
...
TL;DR: 提出了一种简单有效的框架，为前馈分类器添加时序推理能力，无需修改模型架构或引入循环模块。通过SEQ学习范式将训练数据组织成时序连贯的轨迹，学习类特定时序原型并使用可微soft-DTW损失对齐预测序列。


<details>
  <summary>Details</summary>
Motivation: 现实世界的视觉数据通常随时间逐渐演变，但传统分类器假设时间独立性，无法捕捉这种动态特性。

Method: 使用Support-Exemplar-Query学习范式构建时序连贯轨迹，学习类特定时序原型，通过可微soft-DTW损失对齐预测序列，多目标函数促进语义一致性和时序平滑性。

Result: 在细粒度和超细粒度图像分类中提升性能，在视频异常检测中提供精确且时序一致的预测。

Conclusion: 该方法通过损失设计引入强时序归纳偏置，以模块化和数据高效的方式桥接静态和时序学习，仅需在预提取特征上使用简单分类器。
...
TL;DR: 提出了一种无需训练的框架，将否定建模为联合嵌入空间中的子空间而非单点，通过构建球形区域来改进视觉语言模型的否定理解能力。
...
Method: 将否定建模为嵌入空间中的子空间，构建围绕A和N嵌入的两个球形区域，通过接近A且远离N的区域中心方向来评分图像。

Result: 在检索、多项选择和文本到图像任务中，否定理解能力平均提高约30%，缩小了肯定和否定提示之间的差距。

Conclusion: 该方法在保持零样本性能的同时显著提升了否定理解能力，无需重新训练模型。
...
TL;DR: 通过将基础设施摄像头检测到的车辆反投影到地面平面进行3D坐标分析，可以提高转向运动计数的准确性。单摄像头系统使用反投影可获得更准确的轨迹分类和转向计数，多摄像头系统的弱融合能进一步提升精度。


<details>
  <summary>Details</summary>
Motivation: 交叉口的准确转向运动计数对于信号控制、交通管理和城市规划至关重要。传统计算机视觉系统通常在图像平面进行分析，但在地面平面分析交通可能具有优势。

Method: 将基础设施摄像头检测到的车辆反投影到地面平面，在真实世界3D坐标中进行分析。研究了单摄像头系统和多摄像头系统的弱融合方法。

Result: 单摄像头系统使用反投影可获得更准确的轨迹分类和转向计数，多摄像头系统的弱融合能进一步提升精度。
...
TL;DR: 提出CLAReSNet混合架构，结合多尺度卷积和transformer注意力，通过自适应潜在瓶颈解决高光谱图像分类中的高维度、复杂光谱-空间相关性和样本不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类面临高光谱维度、复杂光谱-空间相关性、训练样本有限且类别不平衡等关键挑战。CNN擅长局部特征提取，transformer能捕捉长程依赖，但单独使用效果不佳。

Method: CLAReSNet集成多尺度卷积提取与transformer注意力，采用多尺度卷积主干、增强卷积块注意力模块、双向RNN结合多尺度光谱潜在注意力(MSLA)，MSLA通过自适应潜在token分配将复杂度从O(T²D)降至O(Tlog(T)D)。

Result: 在Indian Pines和Salinas数据集上达到最先进性能，总体准确率分别为99.71%和99.96%，显著超越HybridSN、SSRN和SpectralFormer。

Conclusion: 学习到的嵌入表现出优异的类间可分性和紧凑的类内聚类，验证了CLAReSNet在有限样本和严重类别不平衡情况下的有效性。
...
TL;DR: 提出了XAIGID-RewardBench基准，用于评估多模态大语言模型在判断AI生成图像检测解释质量方面的能力，发现当前最佳模型与人类表现仍有明显差距。


<details>
  <summary>Details</summary>
Motivation: 传统基于分类的AI生成图像检测方法无法提供人类专家可理解的解释，降低了检测工具的可信度。虽然使用MLLMs生成解释成为趋势，但MLLMs在评估自身或其他MLLMs生成的解释质量方面的能力尚未得到充分研究。

Method: 构建包含约3000个标注三元组的基准数据集，涵盖多种图像生成模型和MLLMs作为检测器，评估MLLMs作为奖励模型（评判者）的能力。

Result: 当前最佳奖励模型在该基准上得分88.76%，而人类标注者间一致性达到98.30%，表明当前MLLMs的推理能力与人类水平仍有可见差距。

Conclusion: 需要进一步改进MLLMs的推理能力，以更好地评估AI生成图像检测解释的质量，并分析了这些模型常见的错误类型。
...
TL;DR: DT-R1是一个基于强化学习的框架，通过训练大语言模型构建视觉输入的数字孪生表示，实现统一的视觉推理方法。
...
Conclusion: DT-R1开辟了通过数字孪生表示进行强化学习实现视觉推理的新方向。
...
TL;DR: FastReasonSeg通过数字孪生表示和两阶段蒸馏方法，实现了高效的推理分割，在保持性能的同时大幅减少模型参数和计算需求，适合边缘设备部署。


<details>
  <summary>Details</summary>
Motivation: 现有推理分割方法需要数十亿参数的多模态大模型，超出边缘设备的计算能力，需要开发高效的蒸馏方法来压缩模型同时保持推理能力。

Method: 使用数字孪生表示将感知与推理解耦，采用两阶段蒸馏：首先基于教师模型生成推理链进行监督微调，然后通过联合奖励评估分割准确性和推理质量进行强化微调。

Result: 在四个基准测试中达到最先进性能，0.6B参数变体超越20倍参数模型，实现7.79 FPS吞吐量和仅2.1GB内存消耗。

Conclusion: FastReasonSeg实现了高效的实时推理分割，能够在资源受限环境中部署，为具身智能系统提供可行的解决方案。
...
TL;DR: 提出首个姿态无关、无标签、保证多视角一致性的在线场景变化检测方法，在10+FPS下达到新SOTA，甚至超越最佳离线方法


<details>
  <summary>Details</summary>
Motivation: 现有在线场景变化检测方法准确性远低于离线方法，需要开发能在无约束视角下实时检测相关变化的高精度在线方法

Method: 引入自监督融合损失从多线索推断场景变化，基于PnP的快速姿态估计，以及针对3D高斯泼溅场景表示的快速变化引导更新策略

Result: 在复杂真实世界数据集上的广泛实验表明，该方法超越了在线和离线基线方法

Conclusion: 该方法首次实现了姿态无关、无标签、多视角一致的在线场景变化检测，在保持实时性的同时达到了最先进性能
...
TL;DR: 该论文提出了推理式文本到视频检索新范式，通过数字孪生表示和LLM推理处理隐式查询，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频检索方法只能处理显式查询，无法处理需要推理的隐式查询，因此需要开发能够进行推理的检索系统。

Method: 采用两阶段框架：首先通过数字孪生表示进行组合对齐识别候选视频，然后使用LLM推理配合即时细化机制填补信息缺口。

Result: 在ReasonT2VBench-135上达到81.2% R@1，比最强基线提升50多个百分点；在扩展配置上保持81.7% R@1，并在三个传统基准测试中取得SOTA结果。
...
TL;DR: AGGRNet框架通过提取信息性和非信息性特征来改善复杂医学图像分析任务中的分类性能，特别是在区分细微类别方面。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析面临类别间视觉模式相似、标记数据稀缺和专家解释差异等挑战，现有注意力模型难以有效区分细微类别。

Method: 提出AGGRNet框架，提取信息性和非信息性特征，以更好地理解细粒度视觉模式。

Result: 在多个医学影像数据集上达到最先进性能，在Kvasir数据集上比SOTA模型提升高达5%。

Conclusion: AGGRNet能有效改善复杂医学图像分类任务，特别是在区分视觉模式相似的类别方面表现优异。
...
TL;DR: 本研究开发了一种混合量子-经典框架，使用CT图像诊断和区分肾结石、囊肿和肿瘤，结合预训练的ResNet50编码器和量子卷积神经网络，在8量子比特和12量子比特配置下均达到99%的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 利用量子计算增强医学诊断性能，特别是针对肾脏CT图像中肾结石、囊肿和肿瘤的自动诊断和分类问题。

Method: 采用混合量子-经典框架：1）使用去噪和对比度受限自适应直方图均衡化预处理肾脏图像；2）通过数据增强和加权采样解决类别不平衡；3）使用预训练ResNet50编码器提取特征；4）通过角度编码将特征转换为量子比特；5）使用量子卷积神经网络处理量子特征。

Result: 模型在8量子比特和12量子比特配置下均快速收敛，训练和验证性能高度一致。测试准确率达0.99，12量子比特配置在囊肿检测中实现完美召回率，肿瘤F1分数达0.9956，混淆矩阵显示所有类别分类可靠，误分类极少。

Conclusion: 将经典预处理和深度特征提取与量子电路相结合，能够显著提升医学诊断性能，量子辅助诊断在肾脏疾病分类中表现出色。
...
TL;DR: 提出了Uncertainty-Guided Inference-Time Selection框架，在深度特征空间中解耦偶然不确定性和认知不确定性，无需采样、集成或额外前向传播，实现了60%的计算节省和更紧密的预测区间。
...
Method: 使用正则化全局密度模型估计偶然不确定性，认知不确定性由三个互补组件组成：局部支持不足、流形谱崩溃和跨层特征不一致性。这些组件经验正交，无需采样、集成或额外前向传播。

Result: 在MOT17上实现约60%的计算节省，精度损失可忽略；不确定性分解校准产生显著更紧密的预测区间；消融实验显示正交不确定性分解在所有MOT17序列上持续获得更高计算节省，比总不确定性基线提高13.6个百分点。

Conclusion: 该框架实现了实用的自调节视觉推理，通过解耦不确定性类型实现了高效的计算资源分配和更可靠的推理决策。
...
TL;DR: DenseAnnotate是一个音频驱动的在线标注平台，通过语音注释和区域标记实现图像和3D资产的密集细粒度标注，显著提升了多语言、文化对齐和3D空间能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的训练数据存在稀疏标注问题，传统文本标注方法表达受限、速度慢，难以捕捉图像的细微视觉特征，特别是在多元文化图像和3D资产标注等专业领域。

Method: 开发音频驱动的在线标注平台，标注者通过语音描述观察内容，同时将语音短语与图像区域或3D场景部分同步关联，结合语音转文字和注意力区域标记技术。

Result: 收集了3,531张图像、898个3D场景和7,460个3D对象的多模态数据集，包含20种语言的音频对齐密集标注。基于该数据集训练的模型在多项指标上显著提升：多语言能力提升5%，文化对齐能力提升47%，3D空间能力提升54%。

Conclusion: DenseAnnotate平台为未来视觉语言研究提供了可行的密集标注方法，可应用于各种任务和数据类型，解决了传统标注方法的局限性。
...
TL;DR: MSLoRA是一种参数高效的自适应方法，通过重新加权特征响应而非重新调优主干网络，统一适用于CNN和ViT架构，仅需不到5%的参数即可提升分类、检测和分割任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的低秩自适应方法主要局限于视觉变换器，难以跨架构通用化。需要一种能够统一适应卷积神经网络和视觉变换器的轻量级方法。

Method: 结合低秩线性投影和多尺度非线性变换，通过点乘和残差连接融合，联合调制空间和通道注意力，保持预训练权重冻结。

Result: 在分类、检测和分割任务上持续提升迁移性能，参数使用量少于主干网络的5%，实现稳定优化、快速收敛和强跨架构泛化能力。

Conclusion: MSLoRA通过重新加权而非重新调优，为冻结视觉主干网络的高效自适应提供了简单通用的解决方案。
...
Conclusion: 该框架为自动化室内设计提供了有效的解决方案，在质量和效率方面均表现出色
...
Method: 利用冻结的视觉语言模型进行开放世界检测和分割，通过Q-Former瓶颈聚合视觉特征，引入视觉-动作对比学习对齐视觉语言和动作嵌入

Result: 在真实机器人平台上实验表明，在非结构化、未见环境中具有强泛化和探索性能，即使数据有限

Conclusion: VLA-R框架成功整合开放世界感知与视觉-动作检索，为端到端自动驾驶提供了有效的开放世界解决方案
...
TL;DR: PROBE是一个自监督框架，通过视觉探测目标域无需标签，使用自监督提示增强模块从无标签数据中获取缺陷感知提示来指导ViT骨干网络，并通过域感知提示对齐目标实现跨域泛化。


<details>
  <summary>Details</summary>
Motivation: 解决自动化路面缺陷检测中跨域泛化差的问题，避免监督方法需要重新标注的高成本，同时克服标准自监督方法对域偏移的脆弱性。

Method: 提出自监督提示增强模块(SPEM)从无标签目标数据中提取缺陷感知提示，以及域感知提示对齐(DAPA)目标来对齐源域和目标域的表示。

Result: 在四个挑战性基准测试中，PROBE始终优于强监督、自监督和适应基线方法，实现鲁棒的零样本迁移、对域变化的强韧性以及少样本适应的高数据效率。
...
TL;DR: 本文提出了一种基于旋转流形的旋转优化框架，通过将平移表示为旋转的函数，在重投影误差基础上实现了仅旋转的优化方法，在双视图和多视图场景中均表现出色。


<details>
  <summary>Details</summary>
Motivation: 基于姿态成像几何学中平移可以用旋转表示的关键发现，希望将成像几何表示压缩到旋转流形上，从而开发更精确、高效和可靠的3D视觉计算方法。

Method: 提出了一种基于重投影误差的旋转优化框架，将平移表示为旋转的函数，在旋转流形上进行优化，适用于双视图和多视图场景。

Result: 实验结果显示该方法在旋转估计精度和鲁棒性方面优于当前最先进方法，甚至可与多次束调整迭代结果相媲美。

Conclusion: 这项基于旋转流形的旋转优化框架为3D视觉计算提供了更准确、高效和可靠的解决方案，有望推动该领域的进一步发展。
...
TL;DR: DHGM是一个基于扩散模型的高频引导模型，用于同时进行去雨和超分辨率处理，解决传统级联方法中恢复与增强之间的冲突问题。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像常受恶劣天气影响而退化，但现有的天气恢复方法可能会牺牲对分析小物体至关重要的高频细节。简单的去雨和超分辨率级联方法存在内在冲突：去雨旨在去除高频噪声，而超分辨率则要基于现有细节生成高频纹理。

Method: 提出DHGM模型，集成预训练扩散先验与高通滤波器，同时去除雨滴伪影并增强结构细节。

Result: 大量实验表明，DHGM在性能上优于现有方法，且成本更低。

Conclusion: DHGM能够有效解决去雨和超分辨率之间的冲突，生成清晰的高分辨率图像。
...
TL;DR: MFI-ResNet通过压缩-扩展策略，使用MeanFlow模块替代ResNet中的多层结构，在显著减少参数的同时提升分类准确率。
...
Method: 采用压缩-扩展策略：压缩阶段将每个ResNet阶段的多层结构简化为1-2个MeanFlow模块构建轻量级元模型；扩展阶段对前三阶段应用选择性孵化策略，扩展至与基线ResNet相同的残差块配置，最后一阶段保持MeanFlow形式并进行微调。
...
TL;DR: RedVTP是一种针对扩散视觉语言模型(DVLMs)的响应驱动视觉token剪枝策略，通过利用推理动态来提升模型效率，在保持甚至提升准确率的同时显著提高推理速度。


<details>
  <summary>Details</summary>
Motivation: 扩散视觉语言模型虽然支持并行token解码，但大量视觉token仍然严重阻碍推理效率。视觉token剪枝在自回归VLMs中已有研究，但在DVLMs中仍未被充分探索。

Method: 提出RedVTP策略，利用DVLMs的推理动态来估计视觉token重要性。通过掩码响应token的注意力来评估重要性，并在第一步推理后剪枝不重要的视觉token。

Result: 实验显示RedVTP将LLaDA-V和LaViDa的token生成吞吐量分别提升186%和28.05%，推理延迟分别降低64.97%和21.87%，且准确率未受影响甚至有所提升。

Conclusion: RedVTP有效解决了DVLMs的推理效率问题，为高效多模态推理提供了新思路。
...
TL;DR: 提出UP-Fusion框架，通过通道扰动和预训练知识集成来解决多模态图像融合中的梯度冲突问题，提升融合质量和泛化能力


<details>
  <summary>Details</summary>
Motivation: 统一模型在多模态图像融合中因模态差异导致梯度冲突，而模态特定编码器虽提升质量但降低泛化性，需要平衡两者

Method: 使用语义感知通道剪枝模块(SCPM)过滤冗余特征，几何仿射调制模块(GAM)保持模态区分性，文本引导通道扰动模块(TCPM)减少对模态特定通道的依赖

Result: 在多种多模态图像融合任务和下游任务中表现优于现有方法

Conclusion: UP-Fusion框架有效解决了多模态融合中的梯度冲突和泛化问题，实现了更好的融合性能
...
<details>
  <summary>Details</summary>
Motivation: 长途驾驶容易导致疲劳，而疲劳驾驶对驾驶员和其他道路使用者构成生命威胁，需要实时检测系统来预防事故

Method: 使用实时摄像头采集驾驶员面部图像，通过OpenCV分析面部特征（眼睛开合度和打哈欠动作），利用预训练的DCNN模型检测疲劳状态

Result: 在NTHU-DDD和Yawn-Eye数据集上的疲劳检测分类准确率分别达到99.6%和97%

Conclusion: 该技术提供了一种非侵入式、低成本且有效的疲劳检测方法，有望在智能汽车技术中挽救生命
...
TL;DR: CoTBox-TTT是一种测试时训练方法，通过视觉思维链信号识别问题相关区域，仅更新少量连续软提示来适应视觉语言模型，提高医学视觉问答在领域转移下的可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉问答系统在领域转移时可靠性不足，产生与图像证据弱相关的答案，且重新训练或添加标签在部署时不实用。

Method: 采用证据优先的测试时训练方法，在推理时适应视觉语言模型但保持所有主干网络冻结，仅更新少量连续软提示，通过视觉思维链识别问题相关区域，并鼓励原始图像和局部裁剪区域间答案一致性。

Result: 在医学VQA实验中，该方法对实际部署实用，例如在LLaVA上添加CoTBox-TTT可使pathVQA上的封闭式准确率提高12.3%。

Conclusion: CoTBox-TTT是一种无需标签、即插即用的方法，能有效提高医学视觉问答系统在领域转移下的可靠性和准确性。
...
TL;DR: MOON2.0是一个动态模态平衡的多模态表示学习框架，针对电商产品理解中的模态不平衡、对齐关系利用不足和数据噪声问题提出了解决方案。


<details>
  <summary>Details</summary>
Motivation: 解决电商多模态理解中的三个关键挑战：模态混合训练导致的模态不平衡、产品内视觉和文本信息内在对齐关系的利用不足，以及电商多模态数据中的噪声处理限制。

Method: 包含三个核心组件：1) 模态驱动的专家混合模块进行自适应处理；2) 双级对齐方法利用产品内语义对齐特性；3) 基于MLLM的图像-文本协同增强策略结合动态样本过滤。

Result: 在MBE2.0基准测试和多个公共数据集上实现了最先进的零样本性能，注意力热图可视化提供了改进多模态对齐的定性证据。

Conclusion: MOON2.0通过动态模态平衡和协同增强策略，有效提升了电商产品多模态理解的性能，为解决电商多模态学习的关键挑战提供了有效方案。
...
TL;DR: 本文提出MaskAnyNet方法，将图像掩码区域视为辅助知识而非丢弃信息，通过重新学习机制同时利用可见和掩码信息，提升语义多样性并保留细粒度细节。


<details>
  <summary>Details</summary>
Motivation: 传统图像掩码存在两个问题：丢弃像素未充分利用导致上下文信息丢失；掩码可能移除细小或关键特征。而MIM研究表明掩码区域可以从部分输入重建，显示不完整数据仍具有强上下文一致性，掩码区域可作为语义多样性来源。

Method: 提出MaskAnyNet，结合掩码和重新学习机制，利用可见和掩码信息。可轻松扩展到任何模型，通过额外分支联合学习重组掩码区域。

Result: 在CNN和Transformer骨干网络上的实验显示，在多个基准测试中均获得一致提升。进一步分析证实该方法通过重用掩码内容提高了语义多样性。

Conclusion: 将掩码内容视为辅助知识而非忽略信息的方法有效，MaskAnyNet通过利用掩码区域的语义多样性来丰富特征并保留细粒度细节。
...
TL;DR: 提出了C3DFusion模块，通过显式对齐当前和历史帧的3D提升点特征，生成隐藏区域感知的3D特征几何，解决了现有方法难以重建ego-vehicle侧面关键外帧区域的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于摄像头的3D语义场景补全方法主要关注增强帧内区域，但难以重建ego-vehicle侧面附近的关键外帧区域，尽管历史帧通常包含这些不可见区域的宝贵上下文信息。

Method: 提出Current-Centric Contextual 3D Fusion模块，通过历史上下文模糊化和当前中心特征致密化两种互补技术进行增强的时间融合：通过衰减尺度抑制不准确扭曲历史点特征的噪声，并通过增加体积贡献增强当前点特征。

Result: 在SemanticKITTI和SSCBench-KITTI-360数据集上显著优于最先进方法，并展现出强大的泛化能力，在其他基线模型上也能获得显著的性能提升。

Conclusion: C3DFusion模块简单集成到标准SSC架构中，在3D语义场景补全任务中表现出强大的有效性，特别是在重建外帧区域方面取得了显著改进。
...
TL;DR: 提出一种新的结构重定位范式，通过神经网络直接从图像观测映射到可见场景结构，避免传统图像检索或搜索启发式方法。


<details>
  <summary>Details</summary>
Motivation: 传统基于结构的方法在大场景中需要复杂的搜索启发式或图像检索，导致管道复杂且存储需求随观测数量增长。

Method: 训练一个可见结构检索网络，通过前向传播直接从查询图像获取地图中可见的3D结构点子集。

Result: 该方法在定位精度上可与最先进方法相媲美，同时计算和存储需求更低。

Conclusion: 提出的新范式使基于结构的重定位更易处理，无需依赖图像检索或复杂搜索启发式。
...
TL;DR: 提出基于师生知识蒸馏的模糊鲁棒AI生成图像检测框架，通过冻结教师模型(DINOv3)向处理模糊图像的学生模型传递特征和逻辑响应，提升在运动模糊条件下的检测性能。
...
Method: 采用师生知识蒸馏框架，冻结在清晰图像上训练的DINOv3教师模型，将其特征和逻辑响应蒸馏给处理模糊图像的学生模型，使学生能在运动模糊下产生一致的表征。

Result: 在运动模糊和清晰条件下均达到最先进性能，表现出改进的泛化能力和真实世界适用性。

Conclusion: 该方法有效解决了AIGI检测在运动模糊下的性能下降问题，为真实世界应用提供了鲁棒的解决方案。
...
TL;DR: 提出MdaIF框架，通过大语言模型驱动，解决恶劣天气条件下红外与可见光图像融合问题，采用专家混合系统和视觉语言模型来适应多种退化场景。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法处理恶劣天气下可见光图像的退化问题，且网络架构固定，难以适应多样化的退化场景。

Method: 使用预训练视觉语言模型提取语义先验，提出退化感知通道注意力模块，结合专家混合系统进行自适应特征融合。

Result: 大量实验验证了MdaIF的有效性，性能优于现有最先进方法。

Conclusion: MdaIF框架能够有效处理多种退化场景下的图像融合问题，具有优越的性能表现。
...
TL;DR: 提出D²-VPR框架，通过知识蒸馏和可变形聚合器在保持视觉基础模型强大特征提取能力的同时，显著减少模型参数和计算开销，实现更好的性能-效率平衡。
...
Result: 与最先进方法相比性能相当，同时参数减少约64.2%，FLOPs减少约62.6%（相比CricaVPR）。

Conclusion: D²-VPR框架在保持竞争力的视觉地点识别性能的同时，显著提升了模型效率，更适合资源受限环境部署。
...
<details>
  <summary>Details</summary>
Motivation: 视频理解需要选择既信息丰富又具有因果决定性的关键帧，但现有方法在有限输入token和视频帧时间稀疏性的限制下效果有限

Method: 提出因果信息瓶颈(CIB)原则，使用可学习策略网络从候选帧中选择关键帧确保预测充分性，通过反事实干预评估因果必要性，设计复合奖励进行强化学习

Result: 在NExT-QA、EgoSchema和Video-MME数据集上的实验表明，ReaSon在有限帧设置下持续优于现有最先进方法
...
TL;DR: HiGFA是一种用于细粒度视觉分类的数据增强方法，通过分层引导策略在扩散模型采样过程中结合文本、轮廓和细粒度分类器指导，生成高保真度的合成图像。


<details>
  <summary>Details</summary>
Motivation: 解决生成扩散模型在细粒度任务中难以准确捕捉细微类别特征的问题，避免标准文本引导方法生成误导性样本导致分类器性能下降。

Method: 采用分层引导策略：早期采样阶段使用固定强度的文本和轮廓引导建立整体场景和结构；后期采样阶段激活细粒度分类器引导，并基于预测置信度动态调制所有引导信号的强度。

Result: 在多个细粒度视觉分类数据集上的实验证明了HiGFA的有效性。

Conclusion: HiGFA通过分层、置信度驱动的引导协调，能够生成多样化且忠实于原始类别的合成图像，在全局结构形成和精确细节细化之间实现智能平衡。
...
TL;DR: EmoVerse是一个大规模开源数据集，通过多层次、知识图谱启发的注释实现可解释的视觉情感分析，将情感分解为背景-属性-主体三元组，并提供词级和主体级情感推理。


<details>
  <summary>Details</summary>
Motivation: 现有视觉情感分析研究缺乏开源和可解释数据集，大多数研究为整张图像分配单一离散情感标签，无法揭示视觉元素如何贡献情感。

Method: 通过将情感分解为背景-属性-主体三元组，并将每个元素定位到视觉区域；采用多阶段标注流程确保高可靠性；引入可解释模型将视觉线索映射到维度情感空间表示。

Result: 构建了包含超过21.9万张图像的数据集，包含分类情感状态和维度情感空间的双重标注，支持离散和连续情感表示的统一。
...
Method: 提出SEMC框架：1）语义-结构融合模块(SSFM)利用多尺度结构信息并增强细粒度结构感知；2）专家混合对比识别模块(MCRM)使用MoE机制进行分层对比学习和分类。
...
TL;DR: 提出一种结合信号处理和机器学习的新方法，通过遮挡的森林植被重建地表温度，用于无人机自主监测野火，实现烟雾或火焰可见前的地面火灾早期检测。


<details>
  <summary>Details</summary>
Motivation: 实现全自动无人机野火监测，在烟雾或火焰可见前早期检测地面火灾。合成孔径感知虽能减轻树冠和阳光遮挡，但会引入热模糊，掩盖实际地表温度。

Method: 训练视觉状态空间模型从模糊数据中恢复部分遮挡土壤和火灾热点的细微热信号。使用潜在扩散模型和矢量量化生成大量真实地表温度模拟数据，并通过温度增强和程序化热森林模拟扩展数据集。

Result: 在模拟数据上，相比传统热成像和未校正SA成像，RMSE降低了2-2.5倍。在高温热点现场实验中，改进更显著：相比传统热成像RMSE提升12.8倍，相比未校正SA图像提升2.6倍。

Conclusion: 该方法能重建火灾和人体特征的完整形态，而传统成像因部分遮挡而失效。模型还能泛化到其他热信号，如用于搜救的人体特征检测。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型能够从社交媒体图像中推断用户地理位置，造成严重的隐私泄露威胁。现有的对抗性扰动方法需要较强的图像扭曲，会显著降低图像质量。
...
<details>
  <summary>Details</summary>
Motivation: 解决长视频生成中保持时间一致性和高效合成的挑战，传统方法在生成长视频时难以同时保证时间连贯性和计算效率。

Method: 首先生成低帧率视频作为粗粒度蓝图，然后逐步提高帧率来细化视觉细节和运动连续性，采用双向注意力机制和跨帧率自回归方法。

Result: 在长视频生成任务中达到了新的最先进水平，在视觉质量和时间质量方面都表现出色。

Conclusion: TempoMaster通过帧率渐进式生成策略，成功解决了长视频生成中的时间一致性难题，同时实现了高效的并行合成。
...
TL;DR: 提出了CountIHC框架，通过排名感知的教师选择策略和视觉语言对齐方法，实现IHC图像中多类细胞的高精度计数


<details>
  <summary>Details</summary>
Motivation: IHC图像细胞计数对癌症诊断至关重要，但面临染色重叠、形态多样等挑战。现有回归方法很少支持端到端多类计数，且基础模型潜力未充分挖掘

Method: 使用排名感知教师选择策略评估基础模型的计数能力，通过视觉语言对齐将多类计数任务重新表述，利用结构化文本提示生成语义锚点指导密度图回归

Result: 在12种IHC生物标志物和5种组织类型上超越现有最优方法，与病理学家评估高度一致，在H&E染色数据上也表现有效

Conclusion: CountIHC框架成功解决了IHC图像多类细胞计数问题，展示了良好的可扩展性和临床实用性
...
TL;DR: 提出TopoFG框架，通过细粒度查询和边界点拓扑推理，精确建模复杂车道结构，在OpenLane-V2基准测试中达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有方法使用单一查询表示每条车道，基于查询相似性推断拓扑连接，难以准确建模复杂车道结构，导致拓扑预测不可靠

Method: 提出三阶段框架：1) HPE从BEV掩码提取全局空间先验和车道内关键点序列提取局部序列先验；2) RFD构建细粒度查询，在RoI区域采样参考点并与BEV特征交叉注意力；3) RBTR基于边界点查询特征建模车道连接性，采用拓扑去噪策略减少匹配模糊性

Result: 在OpenLane-V2基准测试中，TopoFG在subsetA上OLS达到48.0%，subsetB上达到45.4%，创下新的最先进性能

Conclusion: 通过将空间和序列先验集成到细粒度查询中，并对边界点拓扑推理应用去噪策略，该方法能精确建模复杂车道结构并提供可信的拓扑预测
...
TL;DR: Seg-VAR将分割任务重新定义为条件自回归掩码生成问题，通过多阶段训练策略学习分割掩码的潜在表示，在多个分割任务上超越之前的判别式和生成式方法。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归建模在图像生成中表现出色，但其在需要精确空间感知的分割任务中的潜力尚未被探索。受Mask2Former多尺度建模启发，研究分割作为条件自回归掩码生成问题的可能性。

Method: 包含三个核心组件：图像编码器生成潜在先验、空间感知的seglat编码器将分割掩码映射为离散潜在token、解码器从潜在重建掩码。采用多阶段训练策略：先学习seglat表示，然后精炼潜在变换，最后对齐图像编码器潜在与seglat分布。
...
TL;DR: 提出一种基于师生框架的单图像形态攻击检测方法，使用CNN教师模型优化ViT学生模型，并集成LoRA进行微调以提高效率。
...
Method: 使用CNN教师模型和ViT学生模型的师生框架，集成LoRA进行高效微调，在三个公开人脸数据集上构建形态数据集，包含十种不同的形态生成算法。

Result: 与六种最先进的S-MAD技术相比，该方法在检测性能和计算效率方面表现优越。

Conclusion: 提出的方法在形态攻击检测方面具有卓越性能和高效性，为人脸识别系统的安全性提供了有效保障。
...
TL;DR: 本文介绍了SoccerNet-GAR数据集，这是一个基于2022年世界杯足球赛的多模态数据集，包含同步的广播视频和球员追踪数据，用于群体活动识别。研究比较了视频和追踪两种模态的性能，发现基于追踪的方法在准确性和效率上都优于视频方法。


<details>
  <summary>Details</summary>
Motivation: 目前群体活动识别主要基于视频模态，而追踪模态（位置和轨迹数据）相对未被充分探索。缺乏标准化的多模态基准使得无法公平比较这两种模态在群体活动识别中的表现。

Method: 构建了SoccerNet-GAR数据集，包含94,285个同步标注的群体活动。提出了两种单模态方法：竞争性视频分类器和基于图神经网络的追踪分类器，其中追踪模型采用角色感知图架构，通过位置边和时间注意力直接编码战术结构。

Result: 追踪模型达到67.2%的平衡准确率，而最佳视频基线为58.1%。追踪模型训练速度快4.25倍，参数数量少438倍（19.7万 vs 8630万）。

Conclusion: 追踪模态在群体活动识别中优于视频模态，强调了模态选择和角色感知建模的重要性。这项研究为像素和位置在群体活动识别中的相对优势提供了新见解。
...
TL;DR: 提出分层阶梯网络和注意力仿射网络来解决测试时适应中的OOD样本问题，通过特征提取和自适应注意力机制提升模型在域偏移下的性能


<details>
  <summary>Details</summary>
Motivation: 现有TTA方法在面对未见类别(OOD)样本时性能显著下降，这些样本被误分类为已知类别不仅影响预测精度，还会损害适应过程

Method: 使用分层阶梯网络从所有Transformer层提取OOD特征，结合注意力仿射网络自适应调整自注意力机制，并采用加权熵机制动态抑制低置信度样本

Result: 在基准数据集上的实验结果表明，该方法显著提升了最常用分类数据集的性能

Conclusion: 所提出的方法有效解决了TTA中的OOD样本问题，通过分层特征提取和自适应注意力机制增强了模型在域偏移下的鲁棒性
...
TL;DR: 提出了一个统一的端到端框架，将物体检测和姿态估计与灵活的物体注册过程相结合。该系统支持基于3D CAD模型或从多视角图像重建神经表示的物体注册，使用CNOS检测器定位物体，并通过新型姿态估计模块OPFormer推断6D姿态。


<details>
  <summary>Details</summary>
Motivation: 为了解决物体检测和姿态估计在实际应用中的灵活性需求，特别是在缺乏传统3D模型的情况下，需要一种能够同时处理模型驱动和模型无关场景的统一框架。

Method: 1) 物体注册阶段：从3D CAD模型或通过多视角图像快速重建神经表示（NeRF）生成物体表示；2) 检测阶段：使用CNOS检测器定位目标物体；3) 姿态估计：OPFormer模块基于transformer架构，利用基础模型进行特征提取，结合多模板视图和NOCS几何先验，建立2D-3D对应关系。

Result: 在BOP基准测试中，集成系统在准确性和效率之间取得了良好平衡，展示了在模型驱动和模型无关场景中的实际适用性。

Conclusion: 该框架提供了一种灵活且高效的解决方案，能够适应不同的物体表示形式，在复杂场景下实现准确的6D姿态估计，具有实际应用价值。
...
TL;DR: C3Net是一个专门用于伪装目标检测的双路径解码器架构，通过边缘细化路径和上下文定位路径协同工作，解决了伪装目标检测的六大核心挑战，在多个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 伪装目标检测面临传统分割方法和现代基础模型的严重失败，主要由于伪装对象与背景在颜色、纹理和模式上的高度相似性。作者识别出六个根本性挑战：内在相似性、边缘破坏、极端尺度变化、环境复杂性、上下文依赖性和显著-伪装目标区分。

Method: 提出C3Net双路径解码器架构：1) 边缘细化路径使用梯度初始化边缘增强模块从早期特征恢复精确边界；2) 上下文定位路径采用基于图像的上下文引导机制实现内在显著性抑制；3) 注意力融合模块通过空间门控协同结合两条路径。
...
TL;DR: MDiTFace是一个基于扩散变换器的多模态人脸生成框架，通过统一标记化策略处理语义掩码和文本输入，采用解耦注意力机制显著降低计算开销，在面部保真度和条件一致性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统的多模态特征融合方法难以实现有效的跨模态交互，导致生成效果不理想。需要解决异质模态表示之间的差异问题。

Method: 提出统一的标记化策略处理语义掩码和文本输入；设计堆叠的多变量变换器块实现同步条件处理；引入解耦注意力机制，将内部计算分离为动态和静态路径，可缓存和重用静态路径特征。

Result: 将掩码条件引入的计算开销降低了94%以上，同时保持性能；在面部保真度和条件一致性方面显著优于其他竞争方法。
...
TL;DR: 提出Denoising-VAE，通过频谱自正则化策略抑制高维潜在空间中的冗余高频噪声，同时保持重建质量，使扩散模型收敛速度提升约2倍，在ImageNet 256×256基准上达到SOTA重建质量和竞争性生成性能。


<details>
  <summary>Details</summary>
Motivation: 传统VAE在高维潜在空间中存在优化困境：高维空间能提高重建保真度但会损害生成性能。现有方法使用外部视觉基础模型进行正则化，但高维潜在如何影响生成模型优化仍不明确。

Method: 1. 提出频谱自正则化策略抑制冗余高频噪声；2. 开发Denoising-VAE（基于ViT的自编码器，不依赖VFMs）；3. 引入频谱对齐策略优化基于Denoising-VAE的生成模型训练。

Result: 扩散模型收敛速度比SD-VAE快约2倍，在ImageNet 256×256上达到：重建质量rFID=0.28、PSNR=27.26（SOTA），生成性能gFID=1.82（竞争性）。

Conclusion: 揭示了高维潜在空间中冗余高频分量阻碍扩散模型训练收敛的关键问题，提出的Denoising-VAE通过频谱正则化有效解决了这一优化困境，实现了重建与生成性能的双重提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示调优方法无法精确区分不同类型的医学概念，缺乏针对特定疾病的跨模态特征。大型语言模型具有丰富的医学知识，可以弥补这一缺陷。

Method: 提出CILMP方法，从LLMs提取疾病特定表征，在低秩线性子空间进行干预，并加入条件机制生成实例自适应提示，将医学知识转移到VLM提示中。

Result: 在多个医学图像数据集上的广泛实验表明，CILMP始终优于最先进的提示调优方法。

Conclusion: CILMP通过桥接LLMs和VLMs，有效将医学知识转移到提示调优过程中，提高了医学图像分类的性能和适应性。
...
TL;DR: DPVO-QAT++是一个用于深度视觉里程计的分层量化优化框架，通过可学习尺度参数化、前端FP16/FP32和后端全精度的异构精度设计，以及GPU原生核融合技术，显著降低了内存占用并提高了处理速度，同时保持了原始模型的轨迹精度。


<details>
  <summary>Details</summary>
Motivation: 基于深度学习的视觉SLAM系统虽然具有出色的几何推理能力，但其巨大的计算开销严重限制了在资源受限的自主平台上的部署。

Method: 采用分层量化优化框架，包括可学习尺度参数化、前端浮点伪量化和后端全精度的异构精度设计，以及GPU原生核融合技术。
...
Conclusion: DPVO-QAT++有效弥合了高精度深度视觉里程计与实际部署效率需求之间的差距，为在真实嵌入式平台上的应用提供了可行的工程范式。
...
TL;DR: 提出FSTS框架，通过分析真实篡改行为参数，构建层次化建模框架来合成逼真的文本图像篡改数据，解决现有方法因合成数据与真实篡改分布差异导致的泛化能力差问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本图像伪造定位方法因真实世界数据集规模有限，且合成数据无法捕捉真实篡改复杂性，导致泛化能力差。

Method: 收集16,750个真实篡改实例，通过结构化管道记录编辑痕迹，分析参数和行为模式，构建基于傅里叶级数启发的层次化建模框架，使用基函数和权重进行可解释逼近。

Result: 在四个评估协议上的广泛实验表明，使用FSTS数据训练的模型在真实世界数据集上实现了显著改进的泛化能力。

Conclusion: FSTS框架能够合成多样且逼真的训练数据，更好地反映真实世界伪造痕迹，有效提升模型泛化性能。
...
TL;DR: 提出了一个高保真、实时的对话数字人系统，结合了视觉逼真的3D虚拟形象、基于角色的表情语音合成和基于知识的对话生成，通过异步执行管道实现低延迟多模态交互。


<details>
  <summary>Details</summary>
Motivation: 当前数字人类在交互应用中难以同时实现视觉真实性和实时响应性，需要开发既能保持高保真视觉效果又能实时响应的对话系统。

Method: 采用异步执行管道协调多模态组件，结合检索增强方法（历史增强和基于意图的路由），支持唤醒词检测、情感表达韵律和上下文感知响应生成。

Result: 构建了一个集成的数字人系统，能够实现响应迅速且可信的交互体验，适用于通信、教育和娱乐等沉浸式应用。

Conclusion: 该系统成功解决了数字人类在视觉真实性和实时响应性之间的平衡问题，为沉浸式交互应用提供了可行的解决方案。
...
TL;DR: 提出了一种基于非因果选择性状态空间的实时光学流和视差估计模型，用于密集感知任务，在保持高精度的同时减少推理时间。


<details>
  <summary>Details</summary>
Motivation: 解决实时应用中光学流和视差估计的计算效率问题，需要在保持准确性的同时实现快速推理和低GPU使用。

Method: 使用非因果Mamba块构建模型，融合成对输入图像，在非因果选择性状态空间中进行密集感知任务处理。

Result: 模型在真实场景中验证有效，能够实现统一、实时且准确的3D密集感知估计，同时保持低推理时间和GPU使用。

Conclusion: 提出的模型适用于实时准确的3D密集感知估计任务，代码和模型已在GitHub开源。
...
TL;DR: 提出了一个任务感知的新视角合成评估框架，使用Zero123模型特征和轻量调优来改进评估指标，包含基于参考和无参考的评分方法。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标难以准确评估生成图像是否既真实又忠实于源视图和视角变换，标准指标经常错误排名不正确的结果。

Method: 利用强大的NVS基础模型Zero123的特征，结合轻量调优步骤来增强判别能力，提出了基于参考的D_PRSIM和无参考的MMD_PRSIM两个互补评估指标。

Result: 两个指标都能可靠识别不正确的生成结果，并与人类偏好研究一致地排名模型，在三个基准测试中产生清晰稳定的模型排名。

Conclusion: 该框架为新视角合成评估提供了原则性和实用的方法，为更可靠的进展铺平了道路。
...
TL;DR: BridgeEQA是一个用于开放词汇具身问答的基准数据集，包含2200个问题-答案对，基于200个真实桥梁场景的专业检查报告，要求模型在多图像间进行视觉证据综合并与NBI条件评级对齐。


<details>
  <summary>Details</summary>
Motivation: 解决在真实世界环境中部署具身代理进行问答的困难，特别是缺乏能够忠实反映实际操作条件的基准测试。桥梁检查领域自然需要多尺度推理、长距离空间理解和复杂语义关系。

Method: 提出BridgeEQA基准数据集，包含2200个开放词汇问题-答案对，基于200个真实桥梁场景的专业检查报告。同时提出EMVR方法，将检查建模为基于图像场景图的顺序导航：图像作为节点，代理通过马尔可夫决策过程遍历视图、比较证据和推理。

Result: 对最先进的视觉语言模型的评估显示在情景记忆EQA设置下存在显著的性能差距。提出的EMVR方法在基线上表现出强大的性能。

Conclusion: 桥梁检查是开放词汇具身问答的一个有前景的领域，BridgeEQA基准和EMVR方法为解决这一挑战提供了有效的解决方案，数据集和代码已公开发布。
...
TL;DR: R²Seg是一个无需训练的两阶段框架，通过推理-拒绝机制提升医学图像分割在分布外肿瘤上的鲁棒性，显著减少假阳性并改善分割性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割基础模型在分布外数据上表现不佳，容易产生碎片化假阳性结果，需要提升模型对OOD肿瘤的鲁棒性。

Method: 采用两阶段方法：1) 推理阶段使用LLM引导的解剖推理定位器官锚点并生成多尺度ROI；2) 拒绝阶段在ROI内对基础模型生成的候选区域进行双样本统计检验，只保留与正常组织显著不同的候选区域。

Result: 在多中心多模态肿瘤分割基准测试中，R²Seg在Dice系数、特异性和敏感性方面显著优于强基线方法和原始基础模型。

Conclusion: R²Seg提供了一种无需参数更新的训练免费框架，有效提升OOD肿瘤分割的鲁棒性，避免灾难性遗忘，并与零更新测试时增强兼容。
...
Result: 评估显示幻觉可检测性在密集视觉标记化模型中最高，VASE度量与嵌入聚类组合效果最佳，提示设计对检测效果有显著影响。
...
TL;DR: 提出了一个基于可控性的可解释性框架，用于理解视觉状态空间模型（SSMs）如何处理空间信息，通过量化输入序列对内部状态动态的影响。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏类似Transformer中透明的注意力机制，理解视觉SSMs如何处理空间信息具有挑战性，需要开发新的可解释性方法。

Method: 提出了两种互补方法：基于雅可比矩阵的方法适用于任何SSM架构，测量通过状态传播链的影响；基于格拉姆矩阵的方法针对对角SSMs，通过闭式解析解实现更快的速度。两种方法都在单次前向传播中运行，具有线性复杂度。

Result: 在三种不同医学成像模态上的实验表明，SSMs自然实现了从早期层中扩散的低级纹理到深层中聚焦的临床有意义模式的分层特征精炼。分析揭示了与诊断标准对齐的领域特定可控性特征、跨网络层次的空间选择性进展，以及扫描策略对注意力模式的显著影响。

Conclusion: 该框架确立了可控性分析作为跨所有领域的SSMs的统一、基础性可解释性范式，在计算机视觉、自然语言处理和跨领域任务中具有广泛应用。
...
TL;DR: CountOCC是一个用于遮挡场景下物体计数的框架，通过多模态引导显式重建被遮挡物体特征，在多个数据集上显著提升了遮挡条件下的计数准确率。


<details>
  <summary>Details</summary>
Motivation: 现有物体计数方法在遮挡场景下表现不佳，因为主干网络会编码遮挡表面而非目标物体，导致特征表示被破坏。

Method: 通过整合可见片段的空间上下文与文本和视觉嵌入的语义先验，在多个金字塔层级合成完整的类判别特征；引入视觉等价目标确保遮挡和非遮挡视图产生空间对齐的注意力图。

Result: 在FSC 147数据集上验证集和测试集的MAE分别减少26.72%和20.80%；在CARPK和CAPTUREReal数据集上分别实现49.89%和28.79%的MAE减少。

Conclusion: CountOCC通过互补机制在遮挡场景下保持了判别性特征，实现了跨视觉领域的鲁棒计数性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有驾驶员注意力模型依赖大规模标注数据集，但这类数据收集和整理成本高昂。研究旨在开发在数据受限场景下仍能有效工作的可解释驾驶员注意力系统。

Method: 采用双路径架构，分别处理空间注意力预测和描述生成，通过跨模态对齐保持语义一致性，实现少样本学习。

Result: 在注意力预测方面表现竞争力，能生成连贯且上下文感知的解释，并在多个驾驶基准测试中展示出强大的零样本泛化能力。

Conclusion: 研究表明在有限监督下也能实现有效的注意力条件生成，为数据受限场景中可解释驾驶员注意力系统的实际部署开辟了新可能性。
...
TL;DR: TrAP是首个针对开放词汇目标检测器的多模态后门攻击方法，通过联合优化图像和文本模态的提示参数及视觉触发器，在不重新训练基础模型权重的情况下植入后门。


<details>
  <summary>Details</summary>
Motivation: 随着开放词汇目标检测器在机器人、自动驾驶等高风险应用中的普及，理解其安全风险变得至关重要。本研究首次探讨了OVODs的后门攻击，并揭示了提示调优引入的新攻击面。

Method: 提出TrAP多模态后门注入策略，联合优化图像和文本模态的提示参数以及视觉触发器；采用基于课程学习的训练策略逐步缩小触发器尺寸；无需重新训练基础模型权重。

Result: 在多个数据集上的实验表明，TrAP在目标误分类和目标消失攻击中均实现了高攻击成功率，同时在下游数据集上的干净图像性能相比零样本设置有所提升。

Conclusion: TrAP展示了OVODs对后门攻击的脆弱性，强调了在部署前进行安全评估的重要性，并为开发防御机制提供了基础。
...
TL;DR: 论文提出了一种新的注意力监督损失函数KLAL，通过直接监督视觉标记的注意力分布来改善视觉语言模型在视觉任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型中，与查询最相关的视觉标记在LLM模块的最终层几乎不被答案标记关注，这导致视觉问答错误。标准的下一标记预测损失无法有效指导对视觉标记的注意力。

Method: 提出KL注意力损失(KLAL)，通过KL散度对齐视觉标记的注意力分布与真实注意力图。真实注意力图来自合成案例的任务几何或真实图像的基础标注，无需新标签。

Result: KLAL与NTP结合后，在几何任务、指向和指代表达理解等任务上取得显著改进，在合成和真实数据集上均表现优异。

Conclusion: 直接监督视觉标记的注意力分布能有效提升VLMs在视觉任务中的性能，特别是在需要精确视觉定位的任务中。
...
TL;DR: 本研究探讨了从体素化的LiDAR点云数据推断体素内目标占用百分比的可能性，使用KPConv和多目标回归方法处理类别不平衡问题，并通过敏感性分析发现较大体素尺寸（2米）误差较低，而较小体素尺寸（0.25-0.5米）在树冠区域误差较高。


<details>
  <summary>Details</summary>
Motivation: 体素化是降低LiDAR数据处理计算成本的有效方法，但会导致细尺度结构信息丢失。本研究旨在探索是否可以从高级体素化LiDAR点云数据中推断低级的体素内容信息（目标占用百分比）。

Method: 提出基于KPConv的多目标回归方法，采用密度相关性的成本敏感学习处理类别不平衡，使用加权MSE、Focal回归和正则化来优化KPConv。对体素尺寸（0.25-2米）进行敏感性分析。

Result: 敏感性分析显示较大体素尺寸（如2米）由于变异性降低而误差较小，而较小体素尺寸（如0.25或0.5米）在变异性最大的树冠区域误差较高。树皮和叶片目标在较小体素尺寸数据集中的误差显著高于较大体素尺寸数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有显著性引导方法依赖内部模型机制可能不可靠，且仅局限于图像空间。需要将人类显著性先验更可靠地整合到模型训练中。

Method: 使用对比嵌入方法，对输入应用显著性保留和显著性退化信号增强，通过对比三元组损失引导模型关注显著特征、忽略非显著特征，并对logit分布进行完整性检查。

Result: 在开放集和封闭集场景下，相比最先进的显著性方法，分类性能均有提升，在不同骨干网络和任务上展现出广泛泛化能力。

Conclusion: SAGE通过潜在空间对比嵌入有效整合人类显著性先验，是提升模型泛化能力和性能的有效方法。
...
TL;DR: 论文发现Stable Diffusion的CLIP嵌入具有置换不变性，可视为Wasserstein空间中的点云。通过最优传输方法进行嵌入插值，能产生更平滑的图像过渡。


<details>
  <summary>Details</summary>
Motivation: 传统矩阵插值方法在Stable Diffusion中产生不自然的图像过渡，需要更符合嵌入空间几何特性的插值方法。

Method: 将嵌入插值重构为最优传输问题，计算Wasserstein空间中的测地线路径，利用点云视角而非矩阵视角。

Result: 基于最优传输的插值方法相比标准方法产生更平滑、更连贯的中间图像。

Conclusion: 将CLIP嵌入视为点云而非矩阵能更好地反映和利用嵌入空间的几何特性，最优传输方法提供了更自然的插值路径。
...
TL;DR: 本文介绍了RoCoISLR数据集，这是首个用于罗马尼亚孤立手语识别的大规模标准化语料库，包含9000多个视频样本和近6000个标准化词汇。通过评估7种最先进的视频识别模型，发现基于Transformer的架构优于卷积基线，Swin Transformer达到34.1%的Top-1准确率。


<details>
  <summary>Details</summary>
Motivation: 当前手语识别研究主要集中于美国手语，罗马尼亚孤立手语识别缺乏大规模标准化数据集，限制了该领域的研究进展。

Method: 创建RoCoISLR数据集，包含9000多个视频样本和近6000个标准化词汇。在一致的实验设置下评估7种视频识别模型：I3D、SlowFast、Swin Transformer、TimeSformer、Uniformer、VideoMAE和PoseConv3D，并与WLASL2000语料库进行性能比较。

Result: 基于Transformer的架构优于卷积基线，Swin Transformer获得最高Top-1准确率34.1%。研究突出了低资源手语中长尾类分布带来的挑战。

Conclusion: RoCoISLR为罗马尼亚孤立手语识别研究提供了初步基础，填补了该领域数据集的空白，并为未来系统化研究奠定了基础。
...
TL;DR: 提出了一种轻量级的颜色协调方法MKL-Harmonizer，用于增强现实中的实时颜色协调，支持设备端推理。


<details>
  <summary>Details</summary>
Motivation: 增强现实中需要实时颜色协调算法，但现有方法缺乏实时解决方案，无法集成到AR流程中。

Method: 利用经典最优传输理论，训练紧凑编码器预测Monge-Kantorovich传输映射。

Result: 在真实AR合成图像上，该方法获得了最佳综合评分，超越了现有最先进方法。

Conclusion: 开发了轻量级颜色协调算法，并发布了专门的AR合成图像数据集和采集工具包。
...
TL;DR: 提出了一个不确定性感知的脑肿瘤分割框架，在nnUNet基础上增加体素级不确定性通道，同时结合正常脑结构和肿瘤分割，为临床决策提供肿瘤位置、周围解剖结构和不确定性信息。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习方法缺乏不确定性估计，无法提供分割错误的置信度，且不能同时分割肿瘤和周围健康脑结构，限制了临床手术规划的应用。

Method: 在nnUNet架构中增加不确定性通道，使用BraTS2023数据集训练；开发统一模型结合正常和癌症数据集，实现全脑结构和肿瘤的同时分割。

Result: 不确定性预测相关性0.750，RMSD 0.047，不影响肿瘤分割精度；全脑分割DSC为0.81（脑结构）和0.86（肿瘤），关键区域表现稳健。

Conclusion: 首个输出肿瘤自然周围环境叠加不确定性图的方法，不确定性可视化有助于评估预测质量、识别错误，为AI辅助外科决策提供关键信息。
...
TL;DR: 提出了一种多尺度递归网络(MSRNet)用于伪装目标检测，通过金字塔视觉Transformer提取多尺度特征，结合注意力机制和递归反馈解码策略，在复杂场景下有效检测小型和多个伪装目标。


<details>
  <summary>Details</summary>
Motivation: 当前伪装目标检测方法在复杂场景中（特别是小型和多个目标）仍存在困难，需要改进检测精度。伪装目标与环境高度相似，且面临低光照、遮挡、小尺寸等挑战。

Method: 使用金字塔视觉Transformer作为骨干网络提取多尺度特征，通过注意力尺度集成单元选择性融合特征，解码器采用多粒度融合单元递归优化特征，并开发了递归反馈解码策略增强全局上下文理解。

Result: 在两个基准数据集上达到最先进性能，在另外两个数据集上排名第二，能够成功检测小型和多个伪装目标。

Conclusion: 通过联合利用多尺度学习和递归特征优化，提出的方法在伪装目标检测任务中取得了性能提升，证明了该方法的有效性。
...
TL;DR: SAGA是首个用于AI生成视频源归属的综合框架，通过多粒度归因识别生成模型，仅需0.5%的标注数据即可达到全监督性能，并提出了可解释的时序注意力签名方法。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的普及导致超逼真合成视频泛滥，传统二元真伪检测已无法应对滥用风险，迫切需要能识别具体生成模型的源归属方法。

Method: 采用新颖的视频transformer架构，利用鲁棒视觉基础模型特征捕获时空伪影；提出数据高效的预训练-归因策略；引入时序注意力签名(T-Sigs)进行可解释性分析。

Result: 在公共数据集上的广泛实验表明，SAGA在合成视频溯源方面设定了新基准，在跨域场景下表现优异，仅用0.5%的源标注数据即可匹配全监督性能。

Conclusion: SAGA为法证和监管应用提供了关键的可解释洞察，通过多粒度源归属框架有效解决了AI生成视频的溯源挑战。
...
TL;DR: 本文提出视觉思维链(vCoT)方法，通过生成帧间过渡事件描述来增强多模态大语言模型的视频理解能力。研究发现视频微调模型已隐含掌握时序推理能力，并能将此能力迁移到静态图像推理任务中。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解中往往只是简单拼接帧token，缺乏对帧间时序关系的显式建模。本文旨在探究视频微调对多模态LLMs带来的具体价值。

Method: 提出视觉思维链(vCoT)方法，生成连续帧之间的过渡事件描述作为显式推理过程。系统比较仅图像模型与视频微调模型在有无过渡线索情况下的表现。

Result: vCoT显著提升仅图像模型在长视频问答任务中的性能，但对视频微调模型仅带来边际增益。视频模型能将时序推理能力迁移到静态视觉推理任务，超越图像模型基线。

Conclusion: 视频微调模型已隐含掌握帧间过渡推理能力，无需显式vCoT提示。这种时序推理能力可有效迁移到静态场景，提升关系视觉推理性能。
...
TL;DR: 提出ViCoKD框架，通过跨模态知识蒸馏解决部分重叠多视角动作识别问题，让模态和标注受限的学生模型从多模态教师模型学习知识


<details>
  <summary>Details</summary>
Motivation: 现实世界多传感器系统中，动作通常只在部分视角可见，且输入模态有限、只有序列级标注而非密集帧级标注，现有方法对此研究不足

Method: 使用跨模态适配器和跨模态注意力让学生模型利用多模态相关性；提出视角感知一致性模块处理视角不对齐问题，通过人类检测掩码和置信度加权的Jensen-Shannon散度强制共可见动作的预测对齐

Result: 在真实世界MultiSensor-Home数据集上，ViCoKD在多个骨干网络和环境中始终优于竞争性蒸馏方法，在受限条件下甚至超越了教师模型

Conclusion: ViCoKD框架有效解决了部分重叠多视角动作识别中的模态和标注限制问题，通过知识蒸馏和视角一致性机制实现了优越性能
...
TL;DR: 本文提出EgoLoc方法，用于在自我中心视频中定位手与物体接触和分离的时间戳，无需对象掩码和动词-名词分类法，实现零样本通用化。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注交互动作的行为范式（"如何交互"），但捕捉手与目标物体接触和分离关键时刻（"何时交互"）的挑战性细粒度问题仍未充分探索，这对混合现实中的沉浸式交互体验和机器人运动规划至关重要。

Method: 提出EgoLoc零样本方法，引入手动力学引导采样生成高质量视觉提示，利用视觉语言模型识别接触/分离属性、定位特定时间戳，并提供闭环反馈进行进一步优化。

Result: 在公共数据集和新基准上的综合实验表明，EgoLoc在自我中心视频中实现了可信的时间交互定位，并有效促进了自我中心视觉和机器人操作任务中的多个下游应用。

Conclusion: EgoLoc消除了对对象掩码和动词-名词分类法的需求，实现了可泛化的零样本实现，为自我中心视觉中的时间交互定位提供了有效解决方案。
...
TL;DR: 提出一个数据驱动的自动绘画创造力评估框架，通过多模态多任务学习同时预测创造力分数、分类内容类型和提取风格特征，实现可解释的创造力评估。


<details>
  <summary>Details</summary>
Motivation: 当前绘画创造力评估依赖专家主观评分，既费时又主观。受认知科学启发，将创造力重新解释为内容（画什么）和风格（怎么画）两个互补维度的函数。

Method: 首先扩展现有创造力标注数据集，增加内容类别标注。然后提出多模态多任务学习框架，引入条件学习机制，根据绘画的风格和语义线索动态调整视觉特征提取。

Result: 实验表明该模型相比现有回归方法达到最先进性能，并提供与人类判断一致的可解释可视化结果。

Conclusion: 该框架能够自动、可解释地评估绘画创造力，代码和标注将公开。
...
TL;DR: ActVAR是一种动态激活框架，通过双重稀疏化（权重和token序列）提升VAR模型效率，在ImageNet 256×256基准测试中实现21.2% FLOPs减少且性能损失最小。


<details>
  <summary>Details</summary>
Motivation: 解决VAR模型随着序列长度增加计算成本急剧上升的问题，避免静态剪枝方法永久移除权重或token导致的性能下降和预训练依赖破坏。

Method: 将前馈网络分解为轻量级专家子网络，使用可学习路由器动态选择token特定专家子集；同时通过门控token选择器识别高更新潜力token进行计算，重构未选中token以保持全局上下文和序列对齐；采用两阶段知识蒸馏策略训练。

Result: 在ImageNet 256×256基准测试中实现高达21.2%的FLOPs减少，同时保持最小性能下降。

Conclusion: ActVAR通过动态激活框架有效平衡了VAR模型的效率和性能，在不牺牲模型容量的前提下显著降低了计算成本。
...
TL;DR: 提出了首个直接从原生HDR数据重建3D场景的方法NH-3DGS，通过新颖的亮度-色度分解技术，在重建流程中保持完整动态范围，显著优于现有方法。
...
Method: 提出了NH-3DGS方法，采用新颖的亮度-色度分解颜色表示，支持直接从原生HDR相机数据优化，在重建流程中保持完整动态范围。

Result: 在合成和真实多视角HDR数据集上，NH-3DGS在重建质量和动态范围保持方面显著优于现有方法，能够直接从原生HDR捕获实现专业级3D重建。

Conclusion: NH-3DGS是首个直接从原生HDR观测建模的3D场景重建方法，通过亮度-色度分解技术实现了专业级的重建效果，代码和数据集将公开。
...
TL;DR: 本文提出了一种基于频域分解预处理（FDP）的脑MRI无监督异常检测方法，通过频域分析发现异常具有独特频率模式，FDP能同时抑制病理信号并保留正常解剖结构，显著提升现有方法的检测性能。


<details>
  <summary>Details</summary>
Motivation: 脑MRI解剖结构多样且标注数据稀缺，现有无监督异常检测方法使用模拟噪声缺乏生物物理保真度和形态复杂性，需要更有效的异常检测方法。

Method: 提出频率分解预处理（FDP）框架，通过频域重建同时实现病理抑制和解剖结构保留，可与现有异常模拟技术无缝集成。

Result: FDP显著提升异常检测性能，与LDM结合时DICE分数提高17.63%，在多个基线方法上保持稳健改进。

Conclusion: FDP是首个利用频域重建的无监督异常检测方法，通过频域分析有效提升脑MRI异常检测性能，具有良好应用前景。
...
TL;DR: DeepSport是首个端到端训练的多模态大语言模型框架，专为多任务、多运动的视频理解设计，通过主动迭代推理和专门帧提取工具实现"用视频思考"。


<details>
  <summary>Details</summary>
Motivation: 现有体育视频理解方法存在局限性：要么专注于单一运动，要么局限于特定任务，或者依赖缺乏稳健学习推理过程的免训练范式。

Method: 提出数据蒸馏管道从10个不同数据源合成高质量思维链轨迹，创建7.8万训练数据；采用两阶段训练策略：监督微调后接强化学习，使用新颖的门控工具使用奖励来优化推理过程。

Result: 在6.7k问题的测试基准上，DeepSport实现了最先进的性能，显著优于专有模型和开源模型的基线。

Conclusion: 这项工作为领域特定的视频推理建立了新基础，以应对多样化运动的复杂性。
...
TL;DR: 提出CASL框架，通过曲率增强的自监督学习实现3D异常检测，无需特定任务设计即可达到领先性能，且能泛化到其他3D理解任务。
...
Method: 基于U-Net架构，引入多尺度曲率提示来指导解码器预测点空间坐标，通过重建范式进行曲率增强的自监督学习。

Result: 仅使用点曲率作为异常分数就超越多个经典模型，CASL框架在异常检测上达到领先性能，且学习到的表示能很好泛化到点云分类等任务。

Conclusion: 曲率在3D异常检测中起关键作用，CASL框架证明了无需专用异常检测机制也能实现高性能，为通用3D理解模型提供了新思路。
...
TL;DR: 提出了一种通过注入有益随机噪声的多模态大语言模型微调方法MuNG，仅需调整1-2%参数即可超越全参数微调和其他现有方法
...
Method: 从变分推断角度重新表述MLLMs推理过程，设计多模态噪声生成器动态分析图像-文本对的跨模态关系，生成任务自适应的有益噪声

Result: 在QwenVL和LLaVA上的实验表明，该方法超越了全参数微调和其他现有微调方法

Conclusion: 通过注入有益噪声有效抑制无关语义成分，显著改善跨模态表示对齐，提升下游任务性能
...
TL;DR: CoordAR是一个用于单参考视图6D姿态估计的自回归框架，通过将3D-3D对应关系建模为离散标记图，解决了现有方法在对称性和遮挡场景下的局限性。
...
TL;DR: CineCtrl是首个提供专业相机参数精细控制的视频电影化编辑框架，通过解耦交叉注意力机制实现摄影效果与相机运动的独立控制，解决了生成视频模型中摄影效果控制的难题。


<details>
  <summary>Details</summary>
Motivation: 电影叙事深受景深、曝光等摄影元素的艺术操控影响，但现有生成视频模型大多仅限于相机运动控制，难以实现专业摄影参数的精细控制。

Method: 提出解耦交叉注意力机制分离相机运动和摄影输入，开发综合数据生成策略结合模拟摄影效果和真实世界采集流程，构建大规模训练数据集。

Result: 模型能生成具有精确控制、用户指定摄影相机效果的高保真视频，在广泛实验中表现出色。

Conclusion: CineCtrl框架成功实现了视频生成中专业摄影参数的精细控制，为电影化视频编辑提供了新的可能性。
...
TL;DR: 提出统一文本驱动的交通场景图像生成与编辑框架，通过可控掩码机制整合两任务，利用多视角数据增强几何多样性，采用两阶段训练策略提升文本-图像对齐和细节质量。


<details>
  <summary>Details</summary>
Motivation: 解决智能交通系统中文本驱动图像生成存在的语义丰富度不足、视角有限、视觉保真度低、文本-内容对齐差等问题。

Method: 使用可控掩码机制统一图像生成与编辑；整合车端和路端多视角数据；采用两阶段训练（概念学习+细粒度微调）；引入掩码区域加权损失增强小尺度交通元素生成质量。

Result: 在交通场景的文本驱动图像生成和编辑任务中取得了领先性能。

Conclusion: 所提方法有效解决了交通场景文本驱动图像生成的多个挑战，在生成质量和文本对齐方面表现优异。
...
TL;DR: PFAvatar是一种从"每日穿搭"照片重建高质量3D虚拟形象的新方法，通过两阶段流程：姿势感知扩散模型微调和神经辐射场3D虚拟形象提取，相比现有方法在重建质量、细节保留和处理遮挡方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有方法将图像分割为服装、配饰等组件进行3D组装容易导致不一致性，且难以处理"每日穿搭"照片中的多样姿势、遮挡和复杂背景问题。

Method: 两阶段方法：1) 使用ControlNet进行姿势估计和条件先验保持损失，端到端学习全身外观；2) 基于NeRF的虚拟形象表示，通过规范SMPL-X空间采样和多分辨率3D-SDS优化。

Result: 方法在5分钟内完成个性化，比之前方法快48倍，在重建保真度、细节保留和处理遮挡/截断方面优于最先进方法。

Conclusion: PFAvatar推进了从真实世界"每日穿搭"相册生成实用3D虚拟形象的能力，支持虚拟试穿、动画和人体视频重演等下游应用。
...
TL;DR: ProtoAnomalyNCD是一个基于原型学习的框架，用于发现和分类多种未见异常类型，通过结合Grounded SAM定位物体区域和异常图引导注意力机制来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有工业异常检测方法主要判断是否存在异常，但实际应用需要发现和分类多种异常类型。工业异常语义细微且现有方法未充分利用图像先验，直接聚类效果不佳。

Method: 1) 使用Grounded SAM和文本提示定位物体区域作为先验；2) 设计异常图引导注意力块，包含区域引导因子区分背景、物体和异常区域；3) 在统一原型学习框架下发现未见异常类别并实现多类型异常分类。

Result: 在MVTec AD、MTD和Real-IAD数据集上优于最先进方法，并能检测未见异常值，实现任务级统一。

Conclusion: ProtoAnomalyNCD通过结合物体定位和异常图先验，有效提升了多类型异常发现和分类性能，为工业异常检测提供了更全面的解决方案。
...
TL;DR: 提出了一种基于半监督学习的高动态范围图像重建方法，通过教师模型生成伪HDR标签，学生模型学习伪标签，并引入不确定性掩码机制来过滤不可靠区域，仅需6.7%的HDR真实标签即可达到全监督方法的性能。


<details>
  <summary>Details</summary>
Motivation: 基于学习的HDR图像重建方法需要LDR-HDR图像对，但这些配对数据难以获取，因此需要研究如何在有限HDR真实标签下实现可比较性能的注释高效方法。

Method: 采用半监督学习框架，教师模型为无标签的LDR样本生成伪HDR标签，学生模型学习伪标签。提出基于不确定性的掩码过程，在像素和补丁级别丢弃伪标签中的不可靠部分，学生只学习可信区域。

Result: 该方法不仅优于之前的注释高效算法，而且仅使用6.7%的HDR真实标签就能达到最新全监督方法的可比性能。

Conclusion: 通过不确定性掩码机制有效缓解了确认偏差问题，证明了半监督学习在HDR图像重建中的有效性，大幅减少了所需的标注数据量。
...
TL;DR: 提出RAD框架，通过将LSTM集成到扩散变换器中，解决长视频生成中的记忆遗忘和时空不一致问题，实现帧级自回归记忆更新与检索。
...
Method: 在扩散变换器框架中引入RNN（LSTM），提出RAD框架，在训练和推理时一致执行帧级自回归记忆更新与检索。
...
TL;DR: DiffSign是一个基于文本到图像生成的新型交通标志识别对抗攻击框架，通过CLIP损失和掩码提示等技术，生成物理鲁棒、高效、可迁移且隐蔽的外观攻击。
...
Method: 提出DiffSign框架，整合CLIP损失和掩码提示提高攻击聚焦性和可控性，开发两种风格定制方法指导视觉外观，提升跨域攻击泛化能力和隐蔽性。

Result: 在多种真实场景下（不同距离、角度、光照条件和标志类别）评估，平均物理世界攻击成功率达到83.3%，具有很高的攻击迁移性。

Conclusion: DiffSign能够生成物理鲁棒、高效、可迁移、实用且隐蔽的对抗攻击，显著优于现有方法。
...
TL;DR: EndoSight AI是一个深度学习系统，用于实时检测和分割胃肠道息肉，在GPU上达到35fps的实时推理速度，检测mAP为88.3%，分割Dice系数为69%。
...
Result: 系统在息肉检测方面达到88.3%的平均精度(mAP)，分割方面Dice系数最高达69%，在GPU硬件上实时推理速度超过35帧/秒。

Conclusion: 这个集成AI解决方案设计用于无缝部署到内窥镜工作流程中，有望提高胃肠道医疗的诊断准确性和临床决策能力。
...
TL;DR: CalibrateMix是一种针对半监督学习模型的校准方法，通过识别易学和难学样本进行定向mixup，在保持分类准确率的同时显著改善模型校准性能。


<details>
  <summary>Details</summary>
Motivation: 现有半监督学习方法存在校准不良问题，模型会产生过度自信的预测。虽然mixup在监督学习中能改善校准，但在半监督学习中直接混合伪标签会因伪标签的不可靠性而面临挑战。

Method: 利用训练动态识别易学和难学样本，然后在易学样本和难学样本之间进行定向mixup，避免直接混合不可靠的伪标签。

Result: 在多个基准图像数据集上的实验表明，该方法实现了更低的期望校准误差(ECE)和更高的准确率。

Conclusion: CalibrateMix能有效改善半监督学习模型的校准性能，同时维持或提升分类准确率。
...
TL;DR: GrOCE是一种无需训练的概念擦除框架，通过图语义推理实现精确、自适应的有害内容移除，同时保留非目标语义。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法要么依赖昂贵的微调，要么进行粗糙的语义分离，往往会退化无关概念且缺乏对动态概念集的适应性。

Method: 将概念及其相互关系建模为动态语义图，包含三个组件：动态拓扑图构建、自适应聚类识别和选择性边切断。

Result: 在概念相似度和Fréchet Inception距离指标上达到最先进性能，提供高效、准确且稳定的概念擦除。

Conclusion: GrOCE无需重新训练即可实现精确的概念擦除，具有良好的适应性和语义保留能力。
...
TL;DR: HiFusion是一个深度学习框架，通过分层建模和跨尺度融合从H&E染色图像预测空间转录组基因表达，在多个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 空间转录组技术面临临床采用障碍，现有方法难以捕捉斑点内的生物异质性，且易受形态学噪声影响。

Method: 包含两个互补组件：分层斑点内建模模块通过多分辨率子块分解提取细粒度形态表征；上下文感知跨尺度融合模块使用交叉注意力选择性整合相关区域上下文。
...
TL;DR: MCAQ-YOLO是一种形态复杂度感知的量化框架，通过五种形态学指标实现空间自适应比特分配，在目标检测任务中相比均匀量化获得更好的精度和收敛效率。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络量化方法在空间区域上采用统一的比特精度，忽略了视觉数据的异构结构和纹理复杂性。

Method: 使用分形维度、纹理熵、梯度方差、边缘密度和轮廓复杂度五种形态学指标来表征局部视觉形态，并指导空间自适应比特分配；采用基于课程的量化感知训练方案逐步增加量化难度。

Result: 在安全装备数据集上达到85.6% mAP@0.5，平均4.2比特和7.6倍压缩比，比4位均匀量化高3.5个百分点，每张图像仅增加1.8毫秒运行时间；在COCO和Pascal VOC数据集上验证了性能一致性。
...
TL;DR: ArtiWorld是一个自动化管道，能够从文本场景描述中定位可关节化对象，并重建可执行的URDF模型，同时保持原始几何形状。


<details>
  <summary>Details</summary>
Motivation: 现有的3D资产大多是刚性的，手动将其转换为关节化对象非常耗时耗力，需要自动化的解决方案来构建交互式模拟器和机器人学习环境。

Method: 使用Arti4URDF，结合3D点云、大型语言模型先验知识和URDF导向的提示设计，快速将刚性对象转换为基于URDF的关节化对象。

Result: 在3D模拟对象、完整3D模拟场景和真实世界扫描场景三个层面评估，该方法始终优于现有方法，达到最先进性能，同时保持对象几何形状和正确捕捉对象交互性。

Conclusion: 这为直接从现有3D资产构建交互式、机器人就绪的模拟环境提供了一条实用路径。
...
TL;DR: 提出CCI方法，利用CLIP的patch嵌入进行聚类分析，识别模型预测中的虚假相关性，特别是背景过度依赖问题，并在COVAR基准上评估了18个CLIP变体的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 对比视觉语言模型如CLIP在零样本识别中表现良好，但容易受到虚假相关性（特别是背景过度依赖）的影响，需要新的解释性方法来诊断和改善这一问题。

Method: 提出CCI方法：使用CLIP的patch嵌入将空间patch聚类成语义连贯的簇，通过掩码这些簇并评估模型预测的相对变化来分析概念重要性。结合GroundedSAM自动分类预测为前景驱动或背景驱动。

Result: CCI在忠实性基准测试中达到新SOTA，在MS COCO检索的deletion-AUC指标上获得超过两倍的改进。通过COVAR基准系统性地评估了18个CLIP变体，揭示了除背景相关性外的其他误差来源。

Conclusion: CCI提供了关键的诊断能力，结合COVAR基准能够全面评估VLM的鲁棒性，为开发更稳健的视觉语言模型指明了方向。
...
TL;DR: UNSEEN是一个基于泛化视角的数据集剪枝框架，通过使用未见过样本的模型进行评分，解决了传统方法中评分密集化问题，并在多步骤场景中优化核心集质量。


<details>
  <summary>Details</summary>
Motivation: 传统数据集剪枝方法依赖训练阶段的模型性能评分，导致样本评分密集分布在狭窄范围内，降低了样本区分度。需要从泛化角度重新思考数据集剪枝。

Method: 提出UNSEEN框架：1）使用未在样本上训练过的模型进行评分；2）扩展到多步骤场景，通过在不同核心集上训练的模型进行增量选择；3）动态优化核心集质量。

Result: 在CIFAR-10、CIFAR-100和ImageNet-1K上显著优于现有SOTA方法。在ImageNet-1K上，减少30%训练数据仍能达到无损性能。

Conclusion: 从泛化角度进行数据集剪枝比传统拟合中心方法更有效，UNSEEN框架为数据集剪枝提供了新的视角和实用解决方案。
...
Method: 使用加权语义图减少非语义特征单元计算，采用自适应候选编辑序列确定最优计算顺序。

Result: 实验证明该方法性能优越，能提供更清晰深入的视觉反事实解释理解。

Conclusion: WSAE-Net方法有效解决了语义相关性问题，提升了视觉反事实解释的质量和效率。
...
TL;DR: PerTouch是一个基于扩散模型的图像润色框架，支持语义级图像润色并保持全局美学。通过参数映射和语义感知机制实现细粒度控制，结合VLM驱动的智能体处理用户指令，在个性化图像润色方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决图像润色中平衡可控性和主观性的挑战，满足用户个性化审美偏好，同时支持语义级别的精细控制。

Method: 使用包含特定语义区域属性值的参数映射作为输入，构建显式的参数到图像映射；引入语义替换和参数扰动机制提升语义边界感知；开发VLM驱动的智能体处理用户指令，具备反馈驱动重思考和场景感知记忆机制。

Result: 广泛实验证明各组件有效性，PerTouch在个性化图像润色方面表现出色。

Conclusion: PerTouch框架成功实现了语义级图像润色与全局美学的平衡，通过参数映射和智能体机制更好地满足用户意图和长期偏好。
...
TL;DR: Medal S是一个医学分割基础模型，支持原生分辨率空间和文本提示的端到端训练框架，在BiomedSegFM数据集上支持多达243个类别的多模态医学图像分割。


<details>
  <summary>Details</summary>
Motivation: 解决现有文本方法缺乏空间感知能力的问题，通过通道对齐避免分辨率不匹配导致的误差，实现更精确的医学图像分割。

Method: 采用通道对齐机制将体积提示与文本嵌入对齐，使用轻量级3D卷积模块进行体素空间细化，支持纯文本和混合两种提示模式，并提出动态重采样等技术优化性能。

Result: 在验证集上，Medal S在五个模态上的平均DSC为75.44（vs SAT的69.83），NSD为77.34（vs 71.06），F1为38.24（vs 24.88），并行空间提示比顺序提示减少90%以上的推理时间。

Conclusion: Medal S通过协调空间精度与语义文本指导，在多类医学分割任务中展现出卓越的效率和准确性，优于基于顺序提示的方法。
...
TL;DR: Infinite-Story是一个无需训练的文本到图像生成框架，专注于多提示词故事场景的一致性生成，解决了身份和风格不一致问题，推理速度比现有最快模型快6倍以上。
...
Method: 采用尺度自回归模型，结合身份提示替换、自适应风格注入和同步指导适应的统一注意力指导机制。

Result: 在身份和风格一致性方面达到最先进水平，每张图像推理时间仅1.72秒，比现有最快模型快6倍以上。
...
<details>
  <summary>Details</summary>
Motivation: CLIP模型存在多模态虚假偏见问题，即模型倾向于依赖虚假特征（如背景）而非核心特征进行推理，这严重影响了在分布外数据上的鲁棒性

Method: 提出SAGE方法，通过探索提示模板空间并选择能诱导最大类别语义分离的提示，无需训练、微调或外部标注

Result: 在4个真实基准数据集和5个主流骨干模型上的实验表明，SAGE持续提升零样本性能和泛化能力，优于现有零样本方法

Conclusion: SAGE是一种简单有效的零样本方法，能够缓解多模态虚假偏见，提升模型在分布外数据上的鲁棒性
...
TL;DR: DTGS是一个统一框架，将Retinex启发的光照分解与热成像引导的3D高斯泼溅相结合，用于极端低光条件下的新视角合成，解决了标准3DGS在曝光不足输入时的失败问题。


<details>
  <summary>Details</summary>
Motivation: 在极端低光条件下，新视角合成在几何、颜色一致性和辐射稳定性方面严重退化。标准3DGS直接应用于曝光不足输入时会失败，因为跨视图的独立增强会导致光照不一致和几何失真。

Method: DTGS通过循环增强-重建机制，在增强、几何和热监督之间进行联合优化。包含热监督分支动态平衡增强、结构和热损失，以及嵌入3DGS循环的Retinex分解模块进行物理可解释的反射-光照分离。

Result: 在构建的RGBT-LOW多视图低光热数据集上的实验表明，DTGS显著优于现有的低光增强和3D重建基线方法，在极端光照下实现了优异的辐射一致性、几何保真度和颜色稳定性。

Conclusion: DTGS通过紧密耦合光照分解与热引导的3D高斯泼溅，成功解决了极端低光条件下的新视角合成问题，为低光3D重建提供了有效的解决方案。
...
TL;DR: BP-FPN是一种基于反向传播的特征金字塔架构，专门用于移动红外小目标检测，通过梯度隔离低层捷径和定向梯度正则化来提升特征表示能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法主要关注时空特征聚合，但性能提升有限，根本瓶颈在于单帧特征表示的模糊性而非时空建模本身。

Method: 提出BP-FPN架构，包含梯度隔离低层捷径(GILS)避免捷径学习，以及定向梯度正则化(DGR)确保层次特征一致性，理论严谨且计算开销小。

Result: 在多个公开数据集上的广泛实验表明，BP-FPN持续实现了最先进的性能。

Conclusion: 这是首个完全从反向传播视角设计的专门用于红外小目标检测的特征金字塔网络，为相关任务提供了新的设计思路。
...
TL;DR: GeoUniPS是一个通用光度立体网络，通过结合合成监督和从大规模3D重建模型获取的几何先验，解决了传统方法在复杂野外场景中的局限性。
...
Result: 在多个数据集上实现了最先进的性能，特别是在复杂野外场景中，定量和定性评估都表现出色。

Conclusion: GeoUniPS通过整合3D重建模型的几何先验，显著提升了通用光度立体在复杂场景中的表现，证明了视觉-几何基础模型的有效性。
...
TL;DR: 提出了一种在表示自编码器潜在空间中训练MeanFlow的高效方法，通过一致性中期训练和两阶段方案解决梯度爆炸问题，显著降低了训练和采样成本，在ImageNet上取得了优异的生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统的MeanFlow在SD-VAE潜在空间中训练计算量大且不稳定，推理时解码器成本高，需要复杂的引导超参数。

Method: 在RAE潜在空间中训练MeanFlow，采用一致性中期训练进行轨迹感知初始化，使用两阶段方案：从预训练流匹配教师蒸馏加速收敛，然后可选的自举阶段进一步减少偏差。

Result: 在ImageNet 256上1步FID达到2.03，优于vanilla MF的3.43，采样GFLOPS减少38%，总训练成本降低83%；在ImageNet 512上1步FID达到3.23，在所有基线中GFLOPS最低。

Conclusion: 该方法消除了引导需求，简化了训练配置，在训练和采样中都减少了计算量，实现了高效且稳定的生成。
...
TL;DR: SpectralAdapt是一个半监督域适应框架，通过光谱密度掩码和光谱端元表示对齐技术，解决医疗领域高光谱图像重建中的数据稀缺和域差异问题。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像在医疗领域潜力巨大，但获取成本高且技术复杂。现有通用领域数据集丰富，但人体高光谱数据稀缺，限制了医疗应用的发展。

Method: 提出SpectralAdapt框架，包含：1）光谱密度掩码(SDM) - 基于光谱复杂性自适应掩码RGB通道；2）光谱端元表示对齐(SERA) - 从标注像素提取物理可解释端元作为域不变锚点。

Result: 在基准数据集上的实验表明，该方法在光谱保真度、跨域泛化能力和训练稳定性方面均有显著提升。

Conclusion: 半监督域适应是解决医疗高光谱成像数据稀缺问题的有效方案，SpectralAdapt框架成功缓解了域偏移、光谱退化和数据稀缺等挑战。
...
TL;DR: REVISOR是一个用于长视频理解的多模态反思框架，通过文本和视觉模态的协同反思增强推理能力，无需额外监督微调就能在多个基准测试中取得显著效果。
...
Method: 提出REVISOR框架，支持多模态协作反思；设计DADR奖励机制，确保在强化学习中准确回顾与问题相关的视频片段；集成到GRPO训练策略中，强制模型推理与所选视频证据之间的因果对齐。

Result: 在VideoMME、LongVideoBench、MLVU和LVBench四个基准测试中取得显著成果，显著提升了MLLMs的长视频理解能力。

Conclusion: REVISOR框架通过跨模态反思机制有效解决了长视频理解的挑战，在不依赖额外监督微调或外部模型的情况下实现了性能提升。
...
TL;DR: Ocean是一个基于视觉的3D语义场景补全框架，采用对象中心化方法，通过实例分割、3D语义分组注意力、全局相似性引导注意力和实例感知局部扩散模块，在复杂环境中实现更准确的语义占用预测。


<details>
  <summary>Details</summary>
Motivation: 现有的自中心化方法在复杂环境中往往忽视细粒度对象级细节，导致语义和几何模糊。需要一种对象中心化的方法来提升语义场景补全的准确性。

Method: 1. 使用MobileSAM从输入图像提取实例掩码；2. 3D语义分组注意力模块用线性注意力聚合对象中心特征；3. 全局相似性引导注意力模块处理分割错误和缺失实例；4. 实例感知局部扩散模块通过生成过程改进实例特征并在BEV空间细化场景表示。

Result: 在SemanticKITTI和SSCBench-KITTI360基准测试中达到最先进性能，mIoU分数分别为17.40和20.28。

Conclusion: Ocean通过对象中心化分解场景，有效解决了复杂环境中的语义和几何模糊问题，在3D语义场景补全任务中表现出色。
...
Method: 引入统一交互体积(UIV)作为体积表示，将异构交互实体编码到共享空间场中，通过关节级概率预测在UIV上进行运动生成。
...
Method: 冻结预训练的图像编码器和多语言文本编码器，仅训练紧凑的1.7M参数投影模块，使用对比损失以英语表示为语义锚点进行多语言对齐。

Result: 在多个多语言检索基准测试中取得显著效果，在五个代表性不足语言上表现大幅提升，现有模型通常在这些语言上表现不佳。

Conclusion: 基于枢轴的参数高效对齐策略对于包容性多模态学习具有显著效果，证明了轻量级方法的有效性。
...
TL;DR: 提出MGCA-Net网络，通过多粒度类别感知解决开放词汇时序动作定位问题，在基类和未见类动作上均实现优越性能


<details>
  <summary>Details</summary>
Motivation: 现有方法大多在单一粒度上识别动作类别，这会降低基类和未见类动作类别的识别准确率

Method: 包含定位器、动作存在预测器、传统分类器和粗到细分类器的多粒度类别感知网络，通过不同粒度处理基类和未见类动作

Result: 在THUMOS'14和ActivityNet-1.3基准测试中达到最先进性能，在零样本时序动作定位设置下也取得最佳结果

Conclusion: 多粒度类别感知能有效增强定位性能，为开放词汇时序动作定位提供了有效解决方案
...
TL;DR: DiffPixelFormer是一种用于RGB-D室内场景分割的差分像素感知Transformer，通过改进模态内表示和模态间交互来提升分割精度。
...
Method: 提出IIMIB模块，通过自注意力捕获模态内长程依赖，使用DSIM模块建模模态间交互以分离模态特定和共享线索，实现像素级跨模态对齐，并采用动态融合策略平衡模态贡献。

Result: 在SUN RGB-D和NYUDv2基准测试中，DiffPixelFormer-L分别达到54.28%和59.95%的mIoU，比DFormer-L分别提升1.78%和2.75%。

Conclusion: DiffPixelFormer通过增强模态内表示和精细建模模态间交互，有效提升了RGB-D室内语义分割性能。
...
TL;DR: 提出Pretext-GRPO自监督强化学习算法和ViSS-R1框架，通过视觉转换任务强制模型深入处理视频信息，提升MLLM在复杂视频推理中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前基于R1的方法在视频任务中过度依赖文本推理，未能充分利用丰富的视觉信息，容易导致捷径学习和幻觉问题。

Method: 1. Pretext-GRPO：在标准R1流程中引入自监督强化学习，对转换后的视觉输入正确解决前置任务给予奖励；2. ViSS-R1：将前置任务自监督学习直接集成到MLLM的R1后训练范式中，同时处理前置问题和真实用户查询。

Result: 在六个广泛使用的视频推理和理解基准测试中表现出有效性和优越性。

Conclusion: 提出的方法能够促进更稳健的视觉中心视频理解，代码和模型将公开。
...
TL;DR: MonoUnc是一种免BEV的单目3D车道线检测器，通过局部车道结构建模随机不确定性，在ONCE-3DLanes和OpenLane数据集上优于现有方法。
...
Conclusion: 提出的MonoUnc方法通过显式建模局部结构不确定性，有效提升了单目3D车道线检测性能，并提出了新的评估指标。
...
TL;DR: 提出一种将Segment Anything 2 (SAM2)从自然图像领域迁移到电子显微镜图像领域的框架，通过特征引导注意力模块和双亲和度解码器，在神经元分割任务中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜图像中的神经元分割面临形态复杂、信噪比低和标注稀缺等挑战，现有方法在准确性和泛化性上存在局限。希望利用视觉基础模型在大量自然图像上学到的先验知识来解决这些问题。

Method: 使用SAM2提取通用特征，引入特征引导注意力模块利用SAM2的语义线索指导轻量级精细编码器关注困难区域，最后通过双亲和度解码器生成粗粒度和精炼的亲和度图。

Result: 在SAM2权重冻结的情况下，性能与最先进方法相当；在电子显微镜数据上微调后，显著超越现有最先进方法。

Conclusion: 研究表明，将自然图像预训练的表征与针对性的领域自适应指导相结合，能够有效解决神经元分割中的特定挑战。
...
TL;DR: RobustGait是一个用于评估基于外观的步态识别系统鲁棒性的框架，通过四个维度（扰动类型、轮廓提取方法、模型架构能力、部署场景）进行细粒度评估，揭示了系统对真实世界损坏和轮廓变化的敏感性。


<details>
  <summary>Details</summary>
Motivation: 现有基于外观的步态识别方法在受控数据集上表现良好，但缺乏对其在真实世界损坏和轮廓变化下鲁棒性的系统性评估。

Method: 提出了RobustGait评估框架，在CASIA-B、CCPG和SUSTech1K数据集上引入15种损坏类型和5个严重级别，评估了6种最先进的步态系统，并在MEVID上进行野外验证。

Result: 发现：1）RGB级别的噪声能更好反映真实世界退化；2）步态精度对轮廓提取器偏差高度敏感；3）鲁棒性取决于扰动类型和架构设计；4）噪声感知训练和知识蒸馏能提高性能。

Conclusion: RobustGait框架揭示了步态识别系统在真实世界部署中的关键鲁棒性问题，并展示了通过噪声感知训练和知识蒸馏可以提升系统性能，推动系统向可部署状态发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有模块化自动驾驶架构过度依赖自车状态，导致泛化能力受限和场景理解不鲁棒。根本原因是自车状态在BEV编码器中过早融合，成为下游规划模块的捷径。
...
Conclusion: AdaptiveAD通过架构层面的解决方案有效解决了自动驾驶系统中自车状态过度依赖的问题，为构建更鲁棒和泛化的端到端自动驾驶系统提供了新思路。
...
TL;DR: 提出了RFxG分类法，从参考框架和粒度两个维度组织显著性解释，揭示了现有评估指标过度关注点式忠实性而忽视对比推理和语义粒度的问题，并提出了四个新的忠实性指标来系统评估解释质量。
...
Method: 引入RFxG分类法框架，从参考框架（点式vs对比）和粒度（细粒度类级vs粗粒度组级）两个维度组织显著性解释，并提出四个新的忠实性指标来系统评估解释质量。

Result: 通过RFxG视角揭示了现有评估指标的关键局限性，开发了全面的评估框架，应用于十个最先进的显著性方法、四个模型架构和三个数据集。
...
TL;DR: MergeSlide是一个用于WSI终身学习的框架，将终身学习视为模型合并问题，利用视觉语言病理学基础模型，通过正交持续合并策略和任务到类别提示对齐推理来缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: WSI终身学习旨在按顺序训练或微调统一模型处理癌症相关任务，减少数据传输和处理所需的资源和努力，特别是考虑到WSI的千兆字节大小。

Method: 1) 使用类别感知提示定义新任务；2) 使用无MLP的主干网络进行少量轮次的微调；3) 采用正交持续合并策略将模型合并到统一模型中；4) 引入任务到类别提示对齐推理来处理类别增量学习设置。

Result: 在六个TCGA数据集流上的实验表明，MergeSlide优于基于排练的持续学习和视觉语言零样本基线方法。

Conclusion: MergeSlide是一个简单而有效的框架，通过模型合并方法成功实现了WSI上的终身学习，在保持性能的同时缓解了灾难性遗忘问题。
...
TL;DR: CapeNext通过层次化跨模态交互和双流特征优化，解决了传统类别无关姿态估计中静态关节嵌入的跨类别歧义和细粒度差异识别不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有CAPE方法使用固定文本关键点描述作为语义先验，但存在两个局限：(1)多义词导致的跨类别歧义（如'腿'在人类和家具中的不同视觉表现）；(2)对细粒度类别内变化（如姿势和毛发差异）的区分能力不足。

Method: 提出新框架，创新性地整合层次化跨模态交互与双流特征优化，通过文本描述和特定图像中的类别级和实例特定线索来增强关节嵌入。

Result: 在MP-100数据集上的实验表明，无论使用何种网络骨干，CapeNext都大幅优于最先进的CAPE方法。

Conclusion: 提出的方法有效解决了静态关节嵌入的局限性，通过动态融合多模态信息显著提升了类别无关姿态估计的性能。
...
TL;DR: PlugTrack是一个新颖的多目标跟踪框架，通过自适应融合卡尔曼滤波器和数据驱动的运动预测器来提升跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界跟踪场景同时包含线性和非线性运动模式，但现有方法要么使用计算效率高但无法处理非线性运动的卡尔曼滤波器，要么使用能捕捉复杂动态但泛化能力有限且计算开销大的数据驱动方法。
...
Conclusion: PlugTrack是首个通过自适应融合桥接经典和现代运动预测范式的MOT框架，展示了两种方法的互补性。
...
TL;DR: 提出了首个用于医学图像增强的低级数据集蒸馏方法，通过解剖先验和个性化生成模块解决低级任务中像素级保真度的挑战，同时保护患者隐私。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法主要针对高级任务，而低级任务需要像素级保真度，导致信息压缩困难。医学图像增强需要处理多对多映射关系，现有方法无法满足需求。

Method: 利用患者间解剖相似性构建共享解剖先验，通过结构保持个性化生成模块整合患者特定信息，使用梯度对齐方法将患者特定知识注入蒸馏数据。

Result: 提出的方法能够有效压缩医学图像增强所需的数据集规模，同时保持像素级保真度，保护患者隐私。

Conclusion: 该方法为医学图像增强提供了一种高效的数据集蒸馏解决方案，解决了低级任务中的信息压缩和隐私保护问题。
...
TL;DR: 提出DGS-Net框架，通过梯度空间分解解决CLIP模型微调时的灾难性遗忘问题，提升AI生成图像检测的泛化能力


<details>
  <summary>Details</summary>
Motivation: 生成模型快速发展导致AI生成图像泛滥，引发虚假信息、隐私侵犯等问题。CLIP等预训练模型在微调时存在灾难性遗忘，限制了跨域泛化能力

Method: 提出DGS-Net框架，通过梯度空间分解分离优化过程中的有害和有益下降方向，将任务梯度投影到有害方向的正交补空间，并与冻结CLIP编码器提取的有益方向对齐

Result: 在50个生成模型上的实验表明，该方法平均优于现有方法6.6%，在多样生成技术上实现了优越的检测性能和泛化能力

Conclusion: DGS-Net通过梯度手术有效保留了预训练先验知识，同时抑制任务无关组件，为AI生成内容检测提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂场景时难以平衡非均匀雾分布细粒度特征表示与全局一致性建模，且需要更好地学习雾在空间变化中的共同退化表示。

Method: 1. 基于Kolmogorov-Arnold表示定理，结合通道独立和通道依赖机制增强非线性依赖学习；2. 设计隐式神经表示将雾退化建模为连续函数；3. 设计密集残差增强模块学习雾特征的隐式表示。

Result: 在多个公共和真实世界数据集上取得了具有竞争力的去雾性能，在复杂场景中实现了良好的视觉感知效果。

Conclusion: 该方法能够有效处理复杂场景中的图像去雾问题，通过隐式神经表示和增强模块实现了高质量图像恢复。
...
<details>
  <summary>Details</summary>
Motivation: 现有去雨方法在处理语义和空间细节保真度方面存在不足，影响计算机视觉系统在雨天环境下的性能。
...
Conclusion: MPHM网络通过有效整合异质先验和分层特征表示，实现了state-of-the-art的图像去雨性能。
...
TL;DR: 提出了一种旋转不变特征(RIF)框架用于3D异常检测，通过点坐标映射和轻量级卷积变换特征网络解决点云方向和位置变化带来的特征不一致问题。
...
Result: 在Anomaly-ShapeNet数据集上平均P-AUROC提升17.7%，在Real3D-AD数据集上平均P-AUROC提升1.6%，与传统特征提取方法结合验证了强泛化能力。

Conclusion: RIF框架在3D异常检测中表现出色，具有强泛化能力和工业应用潜力，有效解决了点云旋转和位置变化带来的特征不一致问题。
...
TL;DR: CloseUpShot是一个基于扩散的框架，通过点条件视频扩散从稀疏输入合成特写新视角，解决了特写场景下像素扭曲条件稀疏和背景泄漏的问题。


<details>
  <summary>Details</summary>
Motivation: 从稀疏输入视图重建3D场景和合成新视角具有挑战性。现有方法主要针对适度的视角变化，在特写场景中难以捕捉细节，因为输入信息严重受限。

Method: 提出分层扭曲和遮挡感知噪声抑制来增强条件图像质量；引入全局结构引导，利用密集融合点云为扩散过程提供一致几何上下文。
...
TL;DR: RePo是一种联合编码区域级和点级特征的轨迹相似性计算方法，通过捕捉空间上下文和细粒度移动模式，在准确率上比现有最佳方法平均提升22.2%。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法虽然降低了轨迹相似性计算的计算复杂度，但未能充分利用轨迹信息的全面频谱进行相似性建模。

Method: 将GPS轨迹映射为网格序列，通过结构特征和视觉特征捕获空间上下文；使用三个轻量级专家网络从密集GPS序列中提取局部、相关和连续移动模式；通过路由网络自适应融合点级特征，再与区域级特征通过交叉注意力结合生成最终轨迹嵌入。

Result: 在所有评估指标上，RePo相比现有最佳基线方法平均准确率提升22.2%。

Conclusion: RePo通过联合编码区域级和点级特征，有效捕捉了轨迹的空间上下文和细粒度移动模式，显著提升了轨迹相似性计算的准确性。
...
TL;DR: VEIL是一种针对文本到视频(T2V)模型的越狱攻击框架，通过中性场景锚点、潜在听觉触发器和风格调制器的模块化提示设计，利用跨模态关联模式诱导模型生成语义不安全但表面良性的视频。


<details>
  <summary>Details</summary>
Motivation: 现有T2V模型越狱攻击通常向明显不安全的提示添加对抗性扰动，容易被检测和防御。本文旨在探索如何通过表面良性的提示，利用模型内在的跨模态关联来诱导生成违反安全策略的视频。

Method: 提出VEIL框架，包含三个模块化组件：中性场景锚点（提供表面场景描述）、潜在听觉触发器（利用音频-视觉共现先验）、风格调制器（增强触发效果）。通过约束优化和引导搜索平衡隐蔽性和有效性。

Result: 在7个T2V模型上的实验表明，该方法在商业模型中的平均攻击成功率提升了23%。

Conclusion: VEIL展示了T2V模型对表面良性但包含丰富隐含线索的提示的脆弱性，揭示了现有安全防护的盲点，需要更深入理解模型跨模态关联机制来改进防御。
...
TL;DR: 本文提出了基于室内照明的对抗攻击框架ILA，通过操纵全局光照来干扰视觉语言导航(VLN)代理，揭示了VLN代理对现实室内光照变化的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有对抗评估通常依赖不常见的纹理扰动，这些条件在实际室内环境中很少遇到。而室内光照是影响导航的重要场景属性，但被现有研究忽视。

Method: 提出ILA黑盒攻击框架，包含两种攻击模式：SILA（静态照明攻击，光照强度恒定）和DILA（动态照明攻击，在关键时刻开关灯造成光照突变）。

Result: 在两个最先进的VLN模型和三个导航任务上的评估显示，ILA显著提高了失败率并降低了轨迹效率。

Conclusion: ILA揭示了VLN代理对现实室内光照变化的先前未被认识的脆弱性，表明光照变化是影响导航性能的重要因素。
...
TL;DR: 提出了MedGEN-Bench基准测试，用于评估医学AI系统的多模态生成能力，包含6422个专家验证的图像-文本对，涵盖6种成像模态和16个临床任务。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉基准存在局限性：查询模糊、简化诊断推理为封闭式捷径、忽视图像生成能力。需要能评估上下文交织指令和跨模态推理的基准。

Method: 构建包含VQA、图像编辑和上下文多模态生成三种格式的基准，采用三层评估框架（像素级指标、语义文本分析、专家临床相关性评分）。

Result: 系统评估了10个组合框架、3个统一模型和5个VLMs，证明了基准的有效性。

Conclusion: MedGEN-Bench为医学AI研究提供了全面的多模态评估标准，推动系统从文本诊断向图像生成能力发展。
...
TL;DR: WinMamba是一种基于Mamba的3D特征编码骨干网络，通过窗口尺度自适应模块和窗口移位策略，在保持计算效率的同时增强长距离空间依赖捕获能力，在KITTI和Waymo数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决3D目标检测中同时最大化计算效率和捕获长距离空间依赖的根本挑战，现有Mamba方法在固定窗口内进行轴对齐扫描会丢失空间信息。

Method: 提出WinMamba块组成的骨干网络，包含窗口尺度自适应模块补偿不同分辨率下的体素特征，以及带有可学习位置编码和窗口移位策略的WinMamba层来丰富上下文信息。

Result: 在KITTI和Waymo数据集上的大量实验表明，WinMamba显著优于基线方法，消融研究验证了WSF和AWF模块对检测精度的独立贡献。

Conclusion: WinMamba通过创新的窗口尺度自适应和窗口移位策略，有效平衡了3D目标检测的计算效率和长距离依赖捕获能力，代码将公开。
...
TL;DR: 使用计算机视觉技术进行道路病害分割，评估GAN生成合成数据对模型训练的影响，比较CNN和MaskFormer模型性能，发现GAN数据提升模型表现且MaskFormer优于CNN。


<details>
  <summary>Details</summary>
Motivation: 美国基础设施状况不佳，道路系统评级为D。传统道路检测方法效率低下且成本高，可利用自动驾驶车辆提供的实时视觉数据，应用计算机视觉技术进行先进道路监测。

Method: 1. 评估GAN生成的合成数据对模型训练的有用性；2. 应用CNN进行道路病害分割；3. 研究基于transformer的MaskFormer模型。

Result: GAN生成的数据能提高模型性能；MaskFormer在mAP50和IoU两个指标上优于CNN模型。

Conclusion: 计算机视觉方法在道路病害分割方面具有潜力，GAN合成数据和MaskFormer模型能有效提升道路监测效率。
...
TL;DR: 提出了CSIP-ReID框架，首次使用骨架序列作为多模态预训练方式，通过对比学习和原型融合更新器，将运动线索与外观特征结合，在视频行人重识别任务上取得SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本的多模态预训练方法存在两个根本限制：缺乏真正的多模态预训练，以及文本难以捕捉细粒度的时间运动信息。而运动线索对于视频中区分身份至关重要。

Method: 两阶段方法：第一阶段使用对比学习对齐骨架和视觉特征；第二阶段引入动态原型融合更新器(PFU)来优化多模态身份原型，并提出骨架引导时间建模(SGTM)模块将骨架的时间线索整合到视觉特征中。

Result: 在标准视频ReID基准测试(MARS、LS-VID、iLIDS-VID)上达到新的SOTA结果，在骨架only的ReID任务(BIWI、IAS)上表现出强大的泛化能力，显著优于先前方法。
...
TL;DR: SOMA是一个SAR与光学图像密集配准框架，通过集成结构梯度先验和混合匹配策略，显著提高了配准精度。


<details>
  <summary>Details</summary>
Motivation: SAR与光学图像由于成像机制和视觉特征的根本差异，像素级配准仍具挑战性。传统梯度信息在手工描述符中很重要，但在深度学习框架中未被有效利用。

Method: 提出特征梯度增强器(FGE)将多尺度多方向梯度滤波器嵌入特征空间，以及全局-局部仿射流匹配器(GLAM)结合仿射变换和基于流的细化，采用从粗到精的架构。

Result: 在SEN1-2数据集上CMR@1px提高12.29%，在GFGE_SO数据集上提高18.50%，表现出强鲁棒性和良好泛化能力。

Conclusion: SOMA通过有效利用梯度信息和混合匹配策略，显著提升了SAR-光学图像配准性能，具有实际应用价值。
...
TL;DR: THIR是一个基于拓扑数据分析的无监督医学图像检索框架，使用Betti数和持久同调来表征组织病理学图像的结构模式，无需训练即可高效检索相似图像。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球女性主要死因，早期诊断和准确临床决策至关重要。现有深度学习方法需要大量标注数据和GPU资源，限制了临床应用。

Method: 使用立方体持久性从RGB组织病理学图像中提取拓扑指纹，将环的演化编码为紧凑的特征向量，通过计算拓扑描述符距离进行相似性检索。

Result: 在BreaKHis数据集上，THIR超越了现有监督和无监督方法，在标准CPU上20分钟内处理完整数据集，提供快速可扩展的解决方案。

Conclusion: THIR提供了一个无需训练、快速、可扩展的临床图像检索方案，特别适合资源受限的医疗环境。
...
TL;DR: 提出HDW-SR方法，通过小波分解和残差扩散专注于高频信息恢复，在图像超分辨率中显著提升细节恢复能力


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的单图像超分辨率方法在高频域指导不足，导致恢复的细节模糊

Method: 使用小波分解替代传统U-Net，仅在残差图上进行扩散；引入小波下采样实现多尺度频率分解；设计动态阈值块优化高频选择；利用小波变换可逆性进行低损失特征重建

Result: 在合成和真实数据集上均取得有竞争力的超分辨率性能，特别是在恢复细粒度图像细节方面表现出色

Conclusion: HDW-SR通过高频引导扩散网络有效解决了现有方法细节模糊的问题，为图像超分辨率提供了新的解决方案
...
TL;DR: GenTract是首个用于全局纤维追踪的生成模型，将纤维追踪构建为生成任务，直接从dMRI生成完整且解剖学上合理的流线。相比传统方法，在精度上显著提升，特别是在低分辨率和噪声数据上表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决传统局部纤维追踪方法容易积累误差和高假阳性率的问题，以及全局方法计算成本高的问题。

Method: 将纤维追踪构建为生成任务，学习从dMRI到完整流线的直接映射，比较了基于扩散和流匹配的两种范式。

Result: GenTract的精度比次优方法TractOracle高2.1倍，在低分辨率和噪声设置下优势更明显，比最接近的竞争对手高一个数量级。

Conclusion: GenTract在保持研究级数据高精度的同时，在低质量数据上也保持可靠性，是全局纤维追踪的有前景解决方案。
...
TL;DR: 本文提出了ViXML框架，通过结合解码器模型和视觉信息来改进极端多标签分类(XMC)，在保持计算效率的同时显著提升性能。
...
Method: 开发ViXML框架：1) 使用数十亿参数的解码器模型；2) 通过单嵌入池化高效集成视觉基础模型；3) 将现有文本数据集扩展为多模态版本。

Result: 在四个公开数据集上验证，ViXML在最大数据集上P@1指标提升达8.21%，小编码器+视觉模型优于纯文本解码器。

Conclusion: 视觉信息在XMC中具有巨大价值，ViXML框架成功平衡了性能与效率，为多模态XMC提供了新方向。
...
TL;DR: 提出Object-Centric 3D Rollout (OCR)方法，通过结构化扰动3D几何来增强多模态大语言模型的视频空间推理能力，解决现有方法的查询锁定推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在视频空间推理中存在查询锁定问题，即模型仅关注提示中明确提到的对象而忽略关键上下文线索，需要提升对动态3D场景中对象位置、方向和关系的整体理解能力。

Method: OCR方法在训练中对选定对象的3D几何施加结构化扰动，通过退化对象特定视觉线索并将改变的几何投影到2D空间，迫使模型进行整体场景推理。设计了基于rollout的训练流程，联合利用原始视频和区域噪声视频来优化空间推理轨迹。

Result: 3B参数模型在VSI-Bench上达到47.5%的准确率，优于多个7B基线模型。消融实验证实OCR优于之前的rollout策略（如T-GRPO、NoisyRollout）。

Conclusion: OCR方法有效提升了多模态大语言模型的视频空间推理能力，通过结构化扰动3D几何解决了查询锁定问题，实现了更全面的场景理解。
...
TL;DR: 提出一个可微分的笔触重建框架，统一了绘画、风格化纹理和涂抹操作，能够真实再现人类绘画-涂抹循环过程。
...
Method: 使用可微分笔触重建框架，通过并行可微分绘画渲染器优化单色和双色贝塞尔笔触，结合风格生成模块合成几何条件纹理，并引入可微分涂抹算子实现自然颜色混合和阴影。

Result: 在油画、水彩、水墨和数字绘画上的广泛实验表明，该方法能产生逼真且富有表现力的笔触重建、平滑的色调过渡和丰富的风格化外观。

Conclusion: 该方法为富有表现力的数字绘画创作提供了一个统一的模型。
...
TL;DR: MonoDLGD是一个基于难度感知标签引导去噪的单目3D物体检测框架，通过自适应扰动和重建真实标签来提供显式几何监督，在KITTI基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 单目3D物体检测由于深度线索的固有模糊性而存在根本性问题。现有DETR方法虽然通过全局注意力和辅助深度预测来缓解，但仍受限于不准确的深度估计，且忽略了实例级检测难度（如遮挡、距离、截断），导致检测性能不佳。

Method: 提出难度感知标签引导去噪框架MonoDLGD：1）根据检测不确定性自适应扰动和重建真实标签；2）对简单实例应用更强扰动，对困难实例应用更弱扰动；3）通过联合优化标签重建和3D物体检测，促进几何感知表示学习。

Result: 在KITTI基准测试上的广泛实验表明，MonoDLGD在所有难度级别上都达到了最先进的性能。

Conclusion: MonoDLGD通过难度感知的标签扰动和重建机制，有效提供了显式几何监督，提高了对不同复杂度物体的鲁棒性，在单目3D物体检测任务中表现出色。
...
TL;DR: 提出一种自监督管道，从超声显示器照片中提取超声图像，绕过DICOM传输瓶颈，实现快速算法测试和原型开发。
...
Conclusion: 该方法成功绕过DICOM瓶颈，为快速测试和开发新算法提供了可行方案。
...
TL;DR: RefineVAD是一个弱监督视频异常检测框架，通过模仿人类感知异常的双重过程，联合建模时间运动模式和语义结构，解决现有方法将异常视为单一类别的问题。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督视频异常检测方法将异常事件简化为单一类别，忽略了真实世界异常在语义和时间特征上的多样性。受人类感知异常方式的启发，需要同时理解时间运动模式和不同异常类型的语义结构。

Method: 提出RefineVAD框架，包含两个核心模块：MoTAR模块通过运动感知时间注意力和重校准来估计运动显著性并动态调整时间关注；CORE模块通过跨注意力将段级特征与可学习类别原型对齐，注入软异常类别先验。

Result: 在WVAD基准测试上的广泛实验验证了RefineVAD的有效性，并强调了整合语义上下文来引导特征向异常相关模式细化的重要性。

Conclusion: 通过联合利用时间动态和语义结构，RefineVAD能够显式建模运动演化方式和语义类别相似性，为弱监督视频异常检测提供了更全面的解决方案。
...
TL;DR: PAVE-Net是首个用于多人视频姿态估计的端到端框架，通过空间编码器和时空姿态解码器消除传统两阶段方法中的启发式操作，在PoseTrack2017上实现6.0 mAP提升。


<details>
  <summary>Details</summary>
Motivation: 现有多人视频姿态估计方法依赖检测、RoI裁剪和NMS等启发式操作，限制了准确性和效率。需要开发完全端到端的框架来消除这些限制。

Method: 提出PAVE-Net框架，包含空间编码器建模帧内关系，时空姿态解码器捕获跨帧全局依赖。采用姿态感知注意力机制，使每个姿态查询能选择性地聚合跨帧对应同一人的特征。

Result: 在PoseTrack2017上比基于图像的端到端方法提升6.0 mAP，与最先进的两阶段视频方法精度相当，同时效率显著提升。

Conclusion: PAVE-Net证明了端到端方法在多人视频姿态估计中的有效性，既提高了准确性又提升了效率。
...
TL;DR: 3DAlign-DAER是一个统一框架，通过动态注意力策略和高效检索策略来对齐文本和3D几何，在细粒度语义对齐和大规模检索方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有3D-文本跨模态对齐方法难以对齐细粒度文本语义与详细几何结构，且在大规模3D数据库上性能显著下降。

Method: 提出动态注意力策略（DAP），使用分层注意力融合模块表示可学习的细粒度token到点注意力，并通过蒙特卡洛树搜索动态校准注意力权重；在推理阶段采用高效检索策略（ERS）进行分层搜索。

Result: 在多个基准测试上表现出优越性能，构建了包含200万文本-3D对的Align3D-2M数据集。

Conclusion: 3DAlign-DAER在细粒度文本-3D对齐和大规模检索任务中表现优异，将发布代码、模型和数据集。
...
TL;DR: 提出HARL框架，通过混合域自适应表示学习解决基于外观的视线估计中的跨域性能下降问题，利用多源数据集学习鲁棒视线表示。


<details>
  <summary>Details</summary>
Motivation: 现有基于外观的视线估计方法在跨域评估中性能显著下降，主要受表情、佩戴物和图像质量等与视线无关因素的干扰。

Method: 提出混合域自适应表示学习框架，通过无监督域适应方式对齐高质量近眼图像特征，分离视线相关表示；设计稀疏图融合模块探索视线方向与头部姿态的几何约束。

Result: 在EyeDiap、MPIIFaceGaze和Gaze360数据集上分别达到5.02°、3.36°和9.26°的最先进精度，跨数据集评估表现优异。

Conclusion: HARL框架能有效学习鲁棒视线表示，显著提升跨域视线估计性能，无需额外计算或推理成本。
...
Method: 结合K空间降级模拟物理一致的uLF图像，使用v预测和分类器自由引导的稳定图像生成，采用SNR加权的3D感知损失保持解剖结构保真度，基于注意力UNet架构进行体素级图像转换。

Result: 在新生儿队列测试中，MRIQT在PSNR指标上比现有最佳方法提升1.78%，医生评估85%的输出图像质量良好且病理清晰可见。
...
TL;DR: 提出了MMD-Thinker框架，通过自适应多维思维进行多模态虚假信息检测，解决了通用MLLMs在检测中推理不足和推理偏差的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息在AIGC时代快速演变，低成本高欺骗性对社会构成重大威胁。现有基于通用MLLMs的检测方法存在推理不足和推理偏差两个关键局限。

Method: 两阶段框架：1) 设计针对多模态虚假信息检测的思维模式；2) 通过任务特定指令调优将定制思维模式注入通用MLLMs；3) 使用混合优势函数的强化学习策略增强推理能力。构建了包含8K+图像-文本对的MMR数据集。

Result: 在领域内和领域外基准数据集上实现了最先进的性能，同时保持了灵活的推理和token使用。

Conclusion: MMD-Thinker通过自适应多维思维有效提升了多模态虚假信息检测能力，解决了通用MLLMs的推理局限。
...
TL;DR: 提出RFMNet用于参考伪装目标检测，通过多阶段特征融合和重叠窗口交叉注意力机制，在Ref-COD基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将参考图像转换为1D提示符，我们探索通过丰富的显著图像特征和伪装目标特征的多上下文融合来提升性能。

Method: 提出RFMNet：1）在对应编码阶段进行参考显著图像多阶段特征与伪装特征的交互融合；2）重叠窗口交叉注意力机制关注局部信息匹配；3）参考特征聚合模块逐步解码分割伪装目标。

Result: 在Ref-COD基准上的大量实验表明，我们的方法达到了最先进的性能。

Conclusion: 通过多阶段特征融合和局部注意力机制，RFMNet在参考伪装目标检测任务中表现出色，验证了所提方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型多模态模型在多种任务中表现出色，但其在跨视角地理定位和姿态估计领域的能力尚未得到探索，而这些能力对导航、自动驾驶、户外机器人等应用具有重要价值。

Method: 构建包含128个城市、49个国家的10,859个全景-卫星图像对数据集，并创建755,976个问答对，其中42,900个用于基准测试。评估了25个最先进的大型多模态模型，并探索了指令调优的效果。

Result: 当前大型多模态模型在地理定位任务中表现良好，但在更复杂的姿态估计任务中效果显著下降。通过GeoX-Bench训练数据进行指令调优可以显著提升模型的跨视角地理感知能力。
...
TL;DR: 本文提出了以自我为中心的程序AI助手（EgoProceAssist）概念，旨在通过第一人称视角为日常程序性任务提供逐步支持。研究识别了三个核心任务：自我中心程序错误检测、自我中心程序学习和自我中心程序问答，并建立了新的分类法。


<details>
  <summary>Details</summary>
Motivation: 受视觉语言模型和自我中心感知研究的进展驱动，需要开发专门针对第一人称视角日常程序性任务的AI助手，以提供逐步支持。

Method: 进行了全面的技术回顾、相关数据集和评估指标分析，并通过对代表性VLM方法的实验来澄清现有AI助手与EgoProceAssist之间的差距。

Result: 研究识别了三个核心任务并建立了新的分类法，对现有方法进行了全面评估，揭示了当前技术的局限性。

Conclusion: 讨论了未来挑战和研究方向，并建立了持续更新的公开存储库来收集最新研究成果。
...
TL;DR: SymGS是一个基于对称感知的3D高斯泼溅压缩框架，通过引入可学习镜像来消除局部和全局反射冗余，在保持渲染质量的同时实现108倍压缩比。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术在渲染速度和真实感方面表现出色，但内存占用随场景复杂度快速增长。现有压缩方法主要利用基元级冗余，但无法充分利用场景中的对称性。

Method: 提出SymGS框架，引入可学习镜像来识别和消除场景中的镜像对称冗余，可作为插件增强现有压缩方法（如HAC），实现进一步压缩。

Result: 相比HAC方法，在基准数据集上实现1.66倍压缩（大规模场景可达3倍），平均实现108倍压缩，同时保持渲染质量。

Conclusion: 对称感知压缩能有效突破现有压缩方法的极限，SymGS框架为3D高斯泼溅技术提供了高效的内存压缩解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在无人机场景中的空间智能能力未被充分探索，存在在动态环境中导航和解释的局限性。

Method: 开发了包含13个子类别的SpatialSky-Bench基准测试，构建了包含100万样本的SpatialSky-Dataset数据集，并设计了专门用于无人机空间推理的Sky-VLM模型。

Result: 主流视觉语言模型在复杂无人机导航场景中表现不佳，而Sky-VLM在所有基准任务中达到了最先进的性能。
...
TL;DR: 提出使用视频级监督检测监控视频中罕见多样异常的双主干框架，结合卷积和Transformer表示，在UCF-Crime数据集上达到90.7% AUC


<details>
  <summary>Details</summary>
Motivation: 解决在仅使用视频级监督的情况下检测监控视频中罕见且多样异常物的挑战

Method: 双主干框架结合卷积和Transformer表示，通过top-k池化整合特征

Result: 在UCF-Crime数据集上达到90.7%的AUC性能

Conclusion: 该双主干框架能有效检测监控视频中的罕见异常
...
TL;DR: SF-Recon是一种直接从多视角图像重建轻量级建筑表面的方法，无需后处理网格简化，通过3D高斯场优化和深度约束三角化实现高效重建。
...
Method: 首先训练3D高斯场获得视图一致表示，然后通过法向梯度引导的高斯优化选择与建筑边界对齐的基元，进行多视角边缘一致性剪枝，最后通过深度约束Delaunay三角化生成轻量网格。

Result: 在SF数据集上的实验表明，SF-Recon能够直接重建轻量级建筑模型，显著减少面和顶点数量，同时保持计算效率。

Conclusion: SF-Recon方法成功实现了从多视角图像直接重建轻量级建筑表面，避免了传统流程的繁琐步骤，在保持结构忠实度的同时提高了效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有单人体网格伪真值生成方法缺乏场景级一致性，导致同一图像中个体深度和尺度冲突。

Method: 提出深度条件平移优化(DTO)方法，利用人体高度先验和单目深度估计器，在MAP框架下联合优化所有个体的相机空间平移；开发Metric-Aware HMR网络，通过相机分支和相对度量损失直接估计度量尺度的人体网格。

Result: 创建DTO-Humans数据集(56万张高质量场景一致图像，平均每图4.8人)；在相对深度推理和人体网格恢复方面达到最先进性能。

Conclusion: DTO方法有效解决多人网格恢复中的场景一致性问题，Metric-Aware HMR网络成功实现度量尺度的人体网格估计，为密集人群分析提供重要工具。
...
TL;DR: TabFlash是一个高效的多模态大语言模型，通过渐进式问题条件化、剪枝策略和令牌聚焦训练，解决了表格图像理解中的冗余和效率问题，在性能提升的同时显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 表格图像理解面临两个主要挑战：问题特定关注的需求和冗余背景区域的存在。现有MLLM方法往往忽略这些特性，导致视觉表示不具信息性和冗余。

Method: 1. 渐进式问题条件化：将问题信息以递增频率注入Vision Transformer各层；2. 剪枝策略：丢弃背景令牌提高效率；3. 令牌聚焦：训练策略促使模型将关键信息集中在保留的令牌中。

Result: TabFlash在表格理解任务上达到最先进性能，优于开源和专有MLLM，同时相比第二好的MLLM减少了27%的FLOPs和30%的内存使用。

Conclusion: 通过结合渐进式问题条件化、剪枝和令牌聚焦，TabFlash实现了高效且有效的表格理解，在性能和效率方面都表现出色。
...
TL;DR: SkyReels-Text是一个字体可控的海报文本编辑框架，能够在保持非编辑区域视觉外观的同时，对多个文本区域进行精确的字体样式编辑，无需字体标签或推理时微调。


<details>
  <summary>Details</summary>
Motivation: 解决现有图像编辑模型在细粒度、字体感知的文本操作方面的不足，满足专业设计工作流程中对字体样式精确控制的需求。

Method: 提出字体可控框架，用户只需提供裁剪的字形补丁即可控制所需字体样式，支持同时编辑多个不同字体样式的文本区域。

Result: 在多个数据集上实现最先进的文本保真度和视觉真实感，提供对字体家族和风格细微差别的精确控制。

Conclusion: 该工作弥合了通用图像编辑与专业级字体设计之间的差距，为海报设计等专业应用提供了强大的文本编辑工具。
...
TL;DR: 提出CorrectAD系统，利用扩散视频生成模型DriveSora和PM-Agent，为端到端自动驾驶规划器自动生成故障案例数据以提升鲁棒性
...
Method: 使用PM-Agent制定数据需求，DriveSora生成与3D布局对齐的时空一致视频，构建自纠正代理系统CorrectAD

Result: 在nuScenes和内部数据集上分别纠正62.5%和49.8%的故障案例，碰撞率降低39%和27%

Conclusion: CorrectAD提供模型无关的端到端自纠正管道，能有效提升任何端到端规划器的鲁棒性
...
TL;DR: DriveLiDAR4D是一个新颖的LiDAR生成流水线，能够生成时间一致的LiDAR场景，具有高度可控的前景对象和逼真的背景，在nuScenes数据集上性能超越现有最佳方法37.2%。


<details>
  <summary>Details</summary>
Motivation: 现有3D LiDAR点云生成方法缺乏序列生成能力，无法产生精确定位的前景对象和逼真背景，限制了实际应用。

Method: 提出DriveLiDAR4D流水线，包含多模态条件和新型序列噪声预测模型LiDAR4DNet，以端到端方式实现完整场景操作的序列生成。

Result: 在nuScenes和KITTI数据集上评估，nuScenes数据集上FRD得分为743.13，FVD得分为16.96，分别比SOTA方法UniScene提升37.2%和24.1%。

Conclusion: 这是首个以端到端方式解决具有完整场景操作能力的LiDAR场景序列生成的工作，显著提升了LiDAR生成的性能。
...
TL;DR: 提出基于计算机视觉的群体活动检测框架，结合深度学习模型和图关系推理，通过Mask R-CNN定位演员、多骨干网络提取特征、构建演员关系图，使用图卷积网络预测个体动作和群体活动。


<details>
  <summary>Details</summary>
Motivation: 多人场景中的群体活动检测面临复杂人际交互、遮挡和时间外观变化的挑战，需要有效建模个体间关系。

Method: 使用Mask R-CNN进行演员定位，多骨干网络提取特征，RoIAlign保持空间对齐，融合掩码信息获得精炼特征，构建演员关系图编码外观相似性和位置关系，应用图卷积网络进行关系推理。

Result: 在Collective Activity数据集上的实验表明，基于掩码的特征精炼、鲁棒相似性搜索和图神经网络推理的组合在拥挤和非拥挤场景下都能提高识别性能。
...
TL;DR: 提出基于YOLOv9-T的混合专家框架，通过自适应路由实现动态特征专业化，相比单一模型提升了mAP和AR指标


<details>
  <summary>Details</summary>
Motivation: 为了提升目标检测性能，需要解决单一模型在复杂场景下特征表达能力有限的问题

Method: 采用混合专家框架，集成多个YOLOv9-T专家模型，通过自适应路由机制实现动态特征选择

Result: 相比单一YOLOv9-T模型，该框架在平均精度(mAP)和平均召回率(AR)指标上均有所提升

Conclusion: 混合专家框架通过动态特征专业化能够有效提升目标检测性能
...
TL;DR: 提出一种混合半监督学习方法，结合整体质量人工标签和质量细节伪标签，在多任务框架下改进视网膜图像质量评估，无需大量人工标注即可获得更可解释的模型。


<details>
  <summary>Details</summary>
Motivation: 现有视网膜图像质量评估工具大多只分类整体图像质量，无法指示采集缺陷来指导重新采集，主要原因是详细标注成本高昂。

Method: 使用在小型数据集上训练的教师模型生成伪标签，然后在多任务设置中微调预训练模型，采用ResNet-18骨干网络。

Result: 在EyeQ和DeepDRiD数据集上，多任务模型性能优于单任务基线（F1：0.875 vs 0.863，0.778 vs 0.763），匹配或超越现有方法。在新标注的EyeQ子集上表现与专家相当。

Conclusion: 半监督方法不仅改进了整体质量评估，还提供了关于采集条件（光照、清晰度、对比度）的可解释反馈，以零额外人工标注成本增强可解释性，并提供临床可操作的输出来指导图像重新采集。
...
TL;DR: 提出了广义去噪扩散压缩模型(gDDCM)，将DDCM扩展到主流的扩散模型及其变体，包括DDPM、基于分数的模型、一致性模型和整流流，在图像压缩任务上取得了改进的性能。


<details>
  <summary>Details</summary>
Motivation: DDCM虽然利用DDPM实现了图像压缩，但无法应用于DDPM之外的其他扩散模型方法。

Method: 提出gDDCM，将DDCM扩展到多种主流扩散模型，包括DDPM、基于分数的模型、一致性模型和整流流，在反向过程中使用特定集合的噪声采样。

Result: 在CIFAR-10和LSUN Bedroom数据集上的实验结果表明，该方法成功将DDCM推广到上述模型，并实现了性能提升。

Conclusion: gDDCM有效扩展了DDCM的应用范围，使其能够适用于多种扩散模型变体，并在图像压缩任务中表现出更好的性能。
...
TL;DR: 提出了DTPQA基准测试，专门用于评估视觉语言模型在交通场景中的感知能力，特别是关注物体距离对感知性能的影响。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶领域需要可靠的感知系统，但现有视觉语言模型在复杂交通场景中的感知能力，特别是对远距离物体的感知能力尚未得到充分评估。

Method: 创建包含合成数据和真实数据的DTPQA基准测试，每个样本包含图像、问题、真实答案和物体距离信息，用于分析VLM性能随距离增加而下降的情况。

Result: 开发了完整的DTPQA数据集和生成工具，能够系统评估VLM在交通场景中的感知能力，特别是距离相关的性能变化。

Conclusion: DTPQA为评估VLM在自动驾驶领域的感知能力提供了专门工具，有助于提升模型在安全关键应用中的可靠性。
...
TL;DR: TripleFDS是一个用于场景文本编辑的新框架，通过解耦文本样式、内容和背景三个属性，实现更灵活和一致的文本编辑。
...
Method: 提出SCB Synthesis数据集和TripleFDS框架，通过SCB Group构造实现三特征解耦，采用组间对比正则化和组内多特征正交性确保语义准确性和减少冗余。

Result: 在主流STE基准测试中达到最先进的图像保真度（SSIM 44.54）和文本准确率（ACC 93.58%），支持样式替换和背景转移等新操作。

Conclusion: TripleFDS通过三特征解耦实现了更灵活的场景文本编辑，在保持视觉一致性的同时提升了编辑能力。
...
TL;DR: 该论文提出了一个名为"What Color Is It"的数据集，用于测试多模态大模型在颜色感知方面的视觉幻觉问题，并分析了其原因和潜在解决方案。
...
Method: 通过构建"What Color Is It"数据集，使用简单方法触发多模态大模型中的单模态视觉幻觉，并基于此分析幻觉原因。

Result: 验证了多模态大模型在颜色感知方面存在视觉幻觉问题，并识别了导致这些问题的潜在原因。
...
TL;DR: DelAnyFlow是一种用于大规模农田边界制图的分辨率无关方法，结合DelAny实例分割模型和结构化后处理流程，能够快速生成拓扑一致的矢量边界，在乌克兰应用中6小时内完成60.3万平方公里区域的处理。


<details>
  <summary>Details</summary>
Motivation: 现有农田边界识别方法存在边界不完整、相邻田地合并、难以扩展等问题，需要开发可扩展的解决方案来支持土地管理和作物监测。

Method: 基于YOLOv11骨干网络构建DelAny实例分割模型，在FBIS 22M数据集上训练，结合结构化后处理、合并和矢量化流程生成矢量边界。

Result: DelAny模型比SAM2准确率提高100%以上，推理速度快400倍；在乌克兰应用中识别出375万（5米分辨率）和515万（2.5米分辨率）个田地，远超现有产品。
...
TL;DR: 本文提出VOPE方法，用于评估大型视觉语言模型在自愿想象任务中的幻觉问题，发现现有模型在想象任务中存在严重幻觉，且现有缓解方法效果有限。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注禁止输出图像中不存在内容的描述任务，但忽视了自愿想象任务（如故事创作）中的幻觉问题，这些任务需要模型生成超出图像的新内容。

Method: 提出VOPE方法，通过重新检查式问题评估LVLM如何解释其响应中想象对象的存在性，根据模型解释与图像中对象存在的一致性来判断是否产生幻觉。

Result: 应用VOPE评估主流LVLM和幻觉缓解方法发现：大多数LVLM在自愿想象中严重幻觉，对想象对象的存在性评估表现差；现有缓解方法在自愿想象任务中效果有限。

Conclusion: 自愿想象任务中的幻觉问题需要特别关注，这是未来研究的重要方向，现有方法在此类任务中效果不佳。
...
<details>
  <summary>Details</summary>
Motivation: 开发计算高效且支持跨表示形式（点云、网格、SDF、体数据）的3D形状映射方法，避免传统方法对大规模训练数据的依赖

Method: 将3D形状表示为从固定锚分布通过连续可逆流映射诱导的概率分布，通过逆流（源到锚）和前向流（锚到目标）的组合实现形状间连续映射，使用点级任务定制嵌入编码形状

Result: 在多样化基准测试和挑战性设置中一致实现高覆盖率和准确率，在形状匹配、UV映射和人体点云扫描配准等任务中表现出色

Conclusion: 该方法提供了可逆且模态无关的形状映射表示，在多种3D形状处理任务中展现出优越性能
...
<details>
  <summary>Details</summary>
Motivation: 现有图像伪造检测方法泛化能力有限且解释性不足，而MLLM具有强大的跨任务泛化潜力但需要大量训练资源。

Method: 提出训练免费的MLLM管道，采用类型先验驱动策略和灵活特征检测器模块专门处理复制移动操作。

Result: 在多种伪造类型上实现优于现有方法的定位精度和更丰富的文本解释，展现出更强的泛化能力。

Conclusion: Foresee成功释放了原始MLLM在取证领域的潜力，无需额外训练即可实现高性能的图像伪造分析。
...
TL;DR: SliDer是一个使用视觉语言模型将幻灯片图像转换为可编辑SVG格式的框架，解决了现有方法无法保持文档高层次结构的问题。


<details>
  <summary>Details</summary>
Motivation: 多媒体文档通常以静态栅格格式分发，限制了编辑和定制能力。现有几何栅格-矢量转换方法无法保持复杂文档的高层次结构，导致语义信息丢失。

Method: 使用视觉语言模型检测和提取栅格输入中的图像和文本元素属性，将其组织成连贯的SVG格式，并在推理过程中迭代优化预测。

Result: SliDer实现了0.069的重建LPIPS，在82.9%的情况下被人类评估者认为优于最强的零样本VLM基线。

Conclusion: SliDer能够有效地将幻灯片图像转换为可编辑的SVG表示，保持了文档的语义结构，并引入了Slide2SVG数据集促进未来研究。
...
TL;DR: InterMoE是一个基于动态时间选择性专家混合的新型框架，用于生成高质量的人类交互动作，通过路由机制结合高级文本语义和低级运动上下文，在保持个体特征的同时确保高语义保真度。
...
Method: 采用动态时间选择性专家混合框架，核心是路由机制，协同使用高级文本语义和低级运动上下文来将时间运动特征分配给专门专家，专家动态确定选择能力并关注关键时间特征。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在零样本任务中表现良好，但其对受控语言扰动的响应可靠性尚不清楚，需要评估模型对语义保留改写和语义改变翻转的鲁棒性。

Method: 使用4万张MS COCO图像及其人工标注，自动生成语义保留的改写和基于规则的语义翻转（改变物体类别、颜色或数量），通过不变性误差、语义敏感性差距和阳性率统计来总结模型行为。

Result: EVA02-CLIP和大型OpenCLIP变体在不变性-敏感性边界上表现最佳，而SigLIP和SigLIP2显示出较大的不变性误差，且经常偏好翻转后的描述而非原始人工描述。

Conclusion: LGIP提供了一个模型无关的诊断工具，能够评估视觉语言模型的语言鲁棒性，这些失败在标准检索指标中往往不可见。
...
TL;DR: 本研究提出基于深度学习的框架，通过多时相遥感影像语义分割监测中国城中村的时空变化，分析拆迁后土地利用情况，揭示城中村改造的复杂性和非线性特征。


<details>
  <summary>Details</summary>
Motivation: 中国城中村大规模拆迁改造后缺乏对土地再利用效果的系统评估，需要评估当前改造实践的有效性和可持续性。

Method: 使用多时相遥感影像语义分割绘制城中村边界变化，将拆迁后土地利用分为六类：不完全拆迁、闲置土地、建筑工地、建筑物、绿地和其他。选取四个经济区域代表城市作为研究区。

Result: 1) 城中村改造过程经常拖延；2) 改造主要发生在城市外围，城市核心区相对稳定；3) 揭示了三种时空转型路径：同步改造、延迟改造和渐进优化。

Conclusion: 城中村改造具有碎片化、复杂性和非线性特征，需要分层级和因地制宜的规划策略，为更包容、高效和可持续的城市更新提供实证支持。
...
TL;DR: 提出了一种渐近极小极大方法用于多目标共形预测，为多目标成像逆问题提供紧密的预测区间并确保联合边际覆盖。


<details>
  <summary>Details</summary>
Motivation: 在病态成像逆问题中，不确定性量化是一个基本挑战，特别是在安全关键应用中。现有方法只能处理标量估计目标，而实际应用往往涉及多个目标。

Method: 提出渐近极小极大多目标共形预测方法，可应用于多指标盲图像质量评估、多任务不确定性量化和多轮测量采集。

Result: 数值实验证明该方法相对于现有多目标共形预测方法具有优势，在合成数据和MRI数据上都表现出色。

Conclusion: 该方法为多目标成像逆问题提供了有效的不确定性量化解决方案，确保联合边际覆盖的同时提供紧密的预测区间。
...
TL;DR: 本文揭示了一种新的攻击类型，通过联邦学习中的微小颜色扰动来破坏模型可解释性而不影响准确性，挑战了正确预测意味着忠实解释的假设。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型在安全关键领域部署，可解释性技术变得至关重要。本文旨在揭示模型可解释性本身可能成为攻击面的新漏洞。

Method: 提出色度扰动模块攻击框架，通过系统性地改变前景和背景之间的颜色对比度来制作对抗样本，在联邦学习中累积扰动，毒化全局模型的内部特征归因。

Result: 攻击将Grad-CAM解释中的峰值激活重叠减少高达35%，同时在所有评估数据集上保持96%以上的分类准确率。

Conclusion: 标准训练流程不足以检测或缓解解释退化，特别是在联邦学习环境中，细微的颜色扰动更难被发现，可解释性本身可以成为攻击面。
...
TL;DR: BootOOD是一个完全自监督的OOD检测框架，通过ID数据合成伪OOD特征，利用神经坍缩特性，使用基于特征范数的轻量级辅助头进行OOD检测。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测器在处理语义相似的OOD样本时表现不佳，需要一种能有效处理语义挑战性OOD样本的检测方法。

Method: 通过ID特征变换合成伪OOD特征，利用神经坍缩特性，设计基于特征范数的轻量级辅助分类头，将OOD检测与主分类器解耦。

Result: 在CIFAR-10、CIFAR-100和ImageNet-200上，BootOOD优于现有后处理方法，超越无离群点暴露的训练方法，与最先进的离群点暴露方法竞争，同时保持或提高ID准确率。

Conclusion: BootOOD提供了一种有效的自监督OOD检测解决方案，特别适用于语义相似的OOD样本，在保持ID性能的同时显著提升检测效果。
...
<details>
  <summary>Details</summary>
Motivation: 单目高度估计在遥感3D感知中很重要，但现有深度学习方法受限于标注数据的稀缺性和获取成本高的问题，需要利用大量未标记数据来提升模型性能。

Method: 采用教师-学生-考试网络的自训练框架：教师网络生成伪标签，学生网络在未标记数据上训练，考试网络作为学生网络的时间集成以稳定性能。教师网络结合回归和分类分支，使用分层双切策略处理高度值的长尾分布。

Result: 在三个不同分辨率和成像模式的数据集上进行了评估，证明了方法的有效性。

Conclusion: TSE-Net通过半监督学习框架有效解决了单目高度估计中标注数据稀缺的问题，提高了模型的泛化能力和预测性能。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在新视角合成中表现优异，但其核心优化存在陷入局部最优和收敛质量不足的问题。

Method: 采用两阶段优化：探索阶段使用自适应加权随机梯度Langevin动力学增强全局搜索；开发阶段使用局部拟牛顿方向引导的Adam优化器利用曲率信息实现精确收敛。

Result: 在多个基准数据集上的广泛实验表明，Opt3DGS在不改变底层表示的情况下实现了最先进的渲染质量。

Conclusion: Opt3DGS通过改进3DGS的优化过程，有效提升了渲染质量，为解决3D高斯泼溅的优化挑战提供了有效方案。
...
TL;DR: 提出了一种名为分层提示学习（HPL）的统一框架，通过任务感知提示建模联合优化图像到图像（I2I）和文本到图像（T2I）的行人重识别任务。


<details>
  <summary>Details</summary>
Motivation: 现有的方法通常将I2I和T2I任务分开处理，这可能导致表示纠缠和性能不佳。I2I强调判别性身份学习，而T2I需要准确的跨模态语义对齐，两者共享检索目标但面临不同挑战。

Method: 1. 任务路由Transformer：在共享视觉编码器中引入双分类令牌，分别路由I2I和T2I分支的特征。2. 分层提示生成：集成身份级可学习令牌和实例级伪文本令牌。3. 跨模态提示正则化：在提示令牌空间中强制语义对齐。

Result: 在多个ReID基准测试上的广泛实验验证了该方法的有效性，在I2I和T2I任务上均达到了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像块的方法能捕捉详细核形态但缺乏组织上下文，且可用的人类标注通常粗糙且分布不均，难以获得细粒度的亚型级监督。
...
TL;DR: 提出VVS框架，通过部分验证跳过加速视觉自回归模型的推理，减少目标模型前向传递次数2.8倍，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型的next-token-prediction范式引入显著推理延迟，传统推测解码的"一步草稿、一步验证"范式无法直接减少前向传递次数，限制了加速潜力。

Method: 基于视觉令牌可互换性，提出VVS框架：1) 动态截断的验证自由令牌选择器；2) 令牌级特征缓存和重用；3) 细粒度跳过步骤调度。

Result: 相比原始AR解码，VVS将目标模型前向传递次数减少2.8倍，同时保持竞争力的生成质量。

Conclusion: VVS在传统推测解码框架上提供了优越的速度-质量权衡，具有重塑推测解码范式的强大潜力。
...
TL;DR: 提出ICLR框架，通过双流交互增强模块和协方差修正损失，解决HVI色彩空间中色度和亮度分支交互的分布差异问题，提升低光图像增强效果。


<details>
  <summary>Details</summary>
Motivation: HVI色彩空间在低光图像增强中取得进展，但色度和亮度分支间的分布差异限制了互补特征提取，亮度误差通过非线性参数传播到色度通道，传统像素级损失在弱相关区域导致梯度冲突。

Method: 提出ICLR框架，包含双流交互增强模块(DIEM)从融合和增强两个维度改进互补信息提取，以及协方差修正损失(CCL)利用亮度残差统计惩罚色度误差并约束色度分支协方差。

Result: 在多个数据集上的实验结果表明，提出的ICLR框架优于现有最先进方法。

Conclusion: ICLR框架通过改进色度和亮度分支的交互机制，有效解决了分布差异和梯度冲突问题，显著提升了低光图像增强性能。
...
TL;DR: 提出一种基于卷积配准神经网络的机器学习框架，能够根据特定属性（如年龄、性别）高效生成条件化可变形模板，并可在有分割标注时生成对应的解剖分割图。


<details>
  <summary>Details</summary>
Motivation: 现有模板开发计算成本高，可用模板数量少，导致研究常使用不具代表性的次优模板，特别是在群体变异大的情况下。

Method: 使用卷积配准神经网络学习一个函数，该函数根据特定属性输出条件化模板，并利用分割标注生成模板的解剖分割图。

Result: 在3D脑MRI数据集上验证，该方法能学习到高质量且具有代表性的群体模板，带标注的条件模板比无标注模板配准效果更好，优于其他模板构建方法。

Conclusion: 该方法能高效生成条件化模板，提高医学图像分析的准确性和代表性。
...
TL;DR: TAND是一种新型组织感知细胞核检测框架，通过组织掩码条件化增强点级监督，实现联合细胞核检测和分类，在PUMA基准测试中达到最先进性能。
...
Result: 在PUMA基准测试中超越组织无关基线和掩码监督方法，在组织依赖性细胞类型（如上皮、内皮和基质）上表现显著提升。
...
TL;DR: 开发基于面部特征和眼动分析的驾驶员疲劳检测系统，使用Eye Aspect Ratio方法和MediaPipe Face Mesh实时监测眨眼频率，通过声音警报提醒疲劳驾驶员。


<details>
  <summary>Details</summary>
Motivation: 驾驶员疲劳是导致道路事故的主要原因之一，每年造成大量伤亡，需要开发实时监测系统来提高道路安全。

Method: 使用标准网络摄像头，结合OpenCV图像处理和MediaPipe Face Mesh框架识别面部关键点，通过Eye Aspect Ratio方法分析眼动特征，检测长时间闭眼和低眨眼频率等疲劳迹象。

Result: 实验分析表明系统具有高准确性和快速响应能力，能够有效识别疲劳状态并及时发出警报。

Conclusion: 该系统提供了一种高性能、低成本的驾驶员监控解决方案，可作为高级驾驶辅助系统的组成部分，有效提升道路安全。
...
TL;DR: 本文提出了两种基于α-散度的边际损失函数：Q-Margin和A3M，用于提升人脸和说话人验证性能，特别是在低误接受率下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的α-散度损失函数虽然能诱导稀疏解，但难以直接集成角度边际，而角度边际对于验证任务至关重要。

Method: 通过两种不同方式集成角度边际：在参考度量中（Q-Margin）和在logits中（A3M），并针对A3M的训练不稳定性提出了原型重初始化策略。

Result: 在IJB-B、IJB-C人脸验证基准和VoxCeleb说话人验证上取得了显著性能提升，特别是在低误接受率下明显优于基线方法。

Conclusion: 提出的两种边际化α-散度损失函数有效提升了验证性能，特别适用于需要高安全性的实际应用场景。
...
TL;DR: CacheFlow是一个无需训练的视频问答系统，通过动态令牌丢弃和压缩长期记忆机制，显著减少处理token数量（最多87%），同时保持答案准确性，特别适合实时流媒体视频问答。
...
Method: 结合动态令牌丢弃（基于余弦相似度在线修剪每帧的patch令牌）和压缩长期记忆（使用小型循环编码器总结关键信息，同时将完整KV对卸载并在生成时重新加载）。

Result: 在离线和流媒体视频问答基准测试中，CacheFlow优于当前强基线方法，同时处理token数量减少高达87%。
...
TL;DR: Part-X-MLLM是一个原生3D多模态大语言模型，通过结构化可执行语法将多样3D任务统一为程序，能够从RGB点云和自然语言提示生成包含部件级边界框、语义描述和编辑命令的连贯令牌序列。


<details>
  <summary>Details</summary>
Motivation: 解决3D任务中缺乏统一接口的问题，通过语言原生前端控制几何引擎，实现符号规划与几何合成的解耦。

Method: 使用双编码器架构进行预训练以分离结构与语义，在大型部件中心数据集上进行指令微调，生成结构化输出驱动下游几何感知模块。

Result: 模型在生成高质量结构化规划方面表现出色，在基于问答、组合生成和局部编辑等任务中实现最先进性能。

Conclusion: Part-X-MLLM通过单一统一接口成功实现了3D多模态任务的统一处理，为几何引擎控制提供了语言原生解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法大多忽略物理和关节属性，限制了在具身AI中的应用。需要开发能直接用于仿真的物理3D生成技术。

Method: 提出首个基于VLM的物理3D生成模型，使用新型3D表示方法将几何token数量减少193倍，并构建了PhysX-Mobility数据集扩展物体类别多样性。

Result: 在PhysX-Mobility数据集和真实图像上的实验显示，PhysX-Anything具有强大的生成性能和鲁棒泛化能力，生成的资产可直接用于接触密集的机器人策略学习。

Conclusion: PhysX-Anything能够显著增强下游应用，特别是在具身AI和基于物理的仿真领域。
...
TL;DR: DMDR结合强化学习与蒸馏过程，通过动态分布引导和动态重噪采样策略，突破传统蒸馏性能上限，使少步生成器性能超越多步教师模型。


<details>
  <summary>Details</summary>
Motivation: 传统蒸馏方法中少步模型的性能受限于多步教师模型，需要突破这一性能瓶颈。

Method: 将强化学习融入蒸馏过程，使用DMD损失作为正则化，设计动态分布引导和动态重噪采样训练策略。

Result: DMDR在视觉质量、提示一致性方面领先于其他少步方法，甚至性能超过多步教师模型。

Conclusion: DMDR框架成功解锁了少步生成器的潜力，实现了性能突破。
...
Result: 在24个任务中的15个任务上获得最佳嵌入性能，在29个任务中的19个任务上通过完整微调获得最佳性能，优于其他12个基础模型。

Conclusion: OlmoEarth作为端到端平台的核心，为非营利组织和NGO提供前沿基础模型和强大数据管理工具，以解决全球重大问题。
...
TL;DR: GS-Light是一个基于3D高斯溅射的高效文本引导3D场景重光照系统，通过训练免费的扩散模型扩展和多视图输入处理，实现高质量的重光照效果。


<details>
  <summary>Details</summary>
Motivation: 解决现有3D场景重光照方法在准确反映用户期望（特别是光照方向）方面的不足，提供更精确的文本引导重光照能力。

Method: 使用大型视觉语言模型解析文本提示为光照先验，结合几何和语义估计器计算光照图，生成初始潜在代码指导扩散模型进行多视图重光照，最后微调3D高斯溅射场景。

Result: 在室内外场景测试中，GS-Light在定量指标（多视图一致性、图像质量、美学评分、语义相似度等）和定性评估（用户研究）上均优于现有基线方法。

Conclusion: GS-Light提供了一种高效准确的文本引导3D场景重光照解决方案，能够生成高质量且符合用户期望的重光照结果。
...
TL;DR: TiViBench是一个专门评估图像到视频生成模型推理能力的层次化基准，包含4个维度的24个任务场景。研究发现商业模型推理能力更强，开源模型潜力未充分开发，并提出无需额外训练的VideoTPO策略来提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型评估主要关注视觉保真度和时间一致性，缺乏对高阶推理能力的评估。需要专门基准来评估模型是否具备类似大语言模型的推理能力。

Method: 提出TiViBench基准，系统评估结构推理、空间视觉模式推理、符号逻辑推理、行动规划4个维度的推理能力。同时提出VideoTPO策略，通过LLM自分析生成候选视频来识别优劣，无需额外训练即可提升推理性能。

Result: 商业模型（如Sora 2、Veo 3.1）展现出更强的推理潜力，开源模型因训练规模和数据多样性限制潜力未充分开发。VideoTPO策略显著提升了推理性能。
...
TL;DR: FFSE是一个3D感知的自回归框架，能够在真实图像上进行直观、物理一致的对象编辑，通过建模为3D变换序列实现平移、缩放、旋转等操作，同时保持背景效果和场景一致性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型在语义图像编辑方面有显著进展，但大多数方法在3D感知对象操作方面表现不足，无法实现直观的物理一致性编辑。

Method: FFSE将编辑建模为学习的3D变换序列，引入3DObjectEditor混合数据集，支持多轮动态条件下的有效训练，实现任意对象操作。

Result: 大量实验表明，FFSE在单轮和多轮3D感知编辑场景中显著优于现有方法。

Conclusion: FFSE框架成功实现了在真实图像上进行3D感知的对象编辑，保持了物理一致性和场景全局一致性，为多轮编辑提供了有效解决方案。
...
TL;DR: UnSAMv2是一个无需人工标注即可实现任意粒度分割的模型，通过发现丰富的掩码-粒度对和引入粒度控制嵌入，显著提升了SAM-2的分割能力。
...
Result: 仅使用6K未标注图像和0.02%额外参数，UnSAMv2显著提升了SAM-2，在11个基准测试中改善了NoC90、1-IoU和AR1000指标。
...
TL;DR: 本文提出SAAS模型解决多镜头半监督视频目标分割问题，通过过渡模拟数据增强和跨镜头理解，在YouMVOS和Cut-VOS基准上实现最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有VOS方法主要针对单镜头视频，难以处理镜头间的不连续性，限制了实际应用。多镜头标注数据稀缺也是重要挑战。

Method: 提出过渡模拟数据增强策略(TMA)和SAAS模型，能够检测和理解镜头转换，利用单镜头数据实现跨镜头泛化。

Result: 在YouMVOS和Cut-VOS基准上的大量实验表明，SAAS通过有效模拟、理解和分割复杂转换实现了最先进的性能。

Conclusion: SAAS模型能够有效处理多镜头视频分割，提出的Cut-VOS基准为未来研究提供了支持，代码和数据集已开源。
...
TL;DR: SenseNova-SI项目通过构建800万样本数据集，在多模态基础模型中培养空间智能，在多个基准测试中取得突破性表现，并分析了数据扩展、泛化能力等关键问题。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态基础模型取得显著进展，但在空间智能方面仍存在明显不足，需要专门提升这方面的能力。

Method: 基于Qwen3-VL、InternVL3和Bagel等多模态基础模型，系统构建包含800万样本的SenseNova-SI-8M数据集，采用严格的空间能力分类体系。
...
Conclusion: SenseNova-SI项目展示了通过大规模多样化数据训练可以显著提升多模态模型的空间智能，并发现了早期涌现的泛化能力，该项目将持续更新并向研究社区开放。
...
TL;DR: 本文提出JiT方法，使用大块Transformer直接在像素层面预测干净图像，而非传统扩散模型预测噪声。该方法基于流形假设，让网络在低维流形上操作，在ImageNet上取得竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型预测噪声而非干净图像，但根据流形假设，自然数据位于低维流形而噪声数据不在。直接预测干净数据可以让网络在低维流形上有效操作。

Method: 使用简单的大块Transformer直接在像素层面预测干净图像，无需分词器、预训练或额外损失函数，称为JiT方法。

Result: 在ImageNet 256×256和512×512分辨率上，使用16和32的大块尺寸取得了竞争性结果，而预测高维噪声的方法会失败。

Conclusion: 通过让网络映射回流形基础，JiT为基于Transformer的扩散模型提供了一个自包含的范式，回归到原始自然数据的基础。
...
### [382] [TIMERIPPLE: Accelerating vDiTs by Understanding the Spatio-Temporal Correlations in Latent Space](https://arxiv.org/abs/2511.12035)
*Wenxuan Miao,Yulin Sun,Aiyue Chen,Jing Lin,Yiwu Yao,Yiming Gan,Jieru Zhao,Jingwen Leng,Mingyi Guo,Yu Feng*

Main category: cs.AR

TL;DR: 提出了一种基于时空相关性的轻量级自适应重用策略，通过重用空间或时间相关token的部分注意力分数来加速视频扩散变换器中的自注意力计算。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型主要基于视频扩散变换器(vDiT)，但由于自注意力机制导致推理延迟严重。先前研究主要关注减少自注意力中的冗余计算，但忽略了视频流中固有的时空相关性。

Method: 利用潜在空间中的时空相关性，提出轻量级自适应重用策略，通过重用空间或时间相关token的部分注意力分数来近似注意力计算。

Result: 在4个vDiT模型上实现了显著更高的计算节省(85%)，同时保持几乎相同的视频质量(VBench上损失<0.06%)。

Conclusion: 该方法通过有效利用视频的时空相关性，在保持视频质量的同时大幅提升了vDiT的计算效率。
...
### [383] [Neo: Real-Time On-Device 3D Gaussian Splatting with Reuse-and-Update Sorting Acceleration](https://arxiv.org/abs/2511.12930)
*Changhun Oh,Seongryong Oh,Jinwoo Hwang,Yoonsung Kim,Hardik Sharma,Jongse Park*

Main category: cs.AR

TL;DR: Neo提出了一种重用和更新排序算法，利用连续帧间高斯排序的时间冗余性，并设计了针对该算法的硬件加速器，显著提升了3D高斯泼溅在资源受限设备上的实时渲染性能。


<details>
  <summary>Details</summary>
Motivation: 在资源受限设备上实现3D高斯泼溅的实时渲染对于提供沉浸式AR/VR体验至关重要，但现有解决方案难以实现高帧率，特别是高分辨率渲染。

Method: 提出了重用和更新排序算法，利用连续帧间高斯排序的时间冗余性，通过跟踪和更新高斯深度排序而非从头重新排序，显著减少冗余计算和内存带宽压力，并设计了针对该算法的硬件加速器。
...
### [384] [QUILL: An Algorithm-Architecture Co-Design for Cache-Local Deformable Attention](https://arxiv.org/abs/2511.13679)
*Hyunwoo Oh,Hanning Chen,Sanggeon Yun,Yang Ni,Wenjun Huang,Tamoghno Das,Suyeon Jang,Mohsen Imani*

Main category: cs.AR

TL;DR: QUILL是一个针对可变形注意力机制的硬件加速器，通过距离排序查询和预取技术将不规则内存访问转换为缓存友好的单次计算，显著提升检测任务的性能和能效。
...
Method: 采用距离排序查询(DOOQ)按空间邻近性排序查询，结合预取技术形成调度感知预取循环；融合MSDeformAttn引擎单次执行插值、Softmax、聚合和投影；小张量片上存储，密集层使用集成GEMM。

Result: 相比RTX 4090实现7.29倍吞吐量和47.3倍能效提升；相比现有加速器实现3.26-9.82倍吞吐量和2.01-6.07倍能效提升；混合精度量化下精度损失≤0.9 AP。
...
### [385] [Live-SWE-agent: Can Software Engineering Agents Self-Evolve on the Fly?](https://arxiv.org/abs/2511.13646)
*Chunqiu Steven Xia,Zhe Wang,Yan Yang,Yuxiang Wei,Lingming Zhang*

Main category: cs.SE

TL;DR: Live-SWE-agent是首个能够在运行时自主持续进化的软件代理，从基础bash工具开始，在解决真实世界软件问题时自主演化其脚手架实现，在SWE-bench Verified基准测试中达到75.4%的解决率。
...
Method: 从仅具备bash工具的基础代理脚手架开始，在解决真实软件问题时自主演化其脚手架实现，实现on-the-fly的持续进化。

Result: 在SWE-bench Verified基准测试中达到75.4%的解决率，优于所有现有开源软件代理；在SWE-Bench Pro基准测试中达到45.8%的最佳已知解决率。

Conclusion: Live-SWE-agent证明了软件代理能够在运行时自主持续进化的可行性，在多个基准测试中取得了最先进的性能。
...
### [386] [Characterizing and Understanding Energy Footprint and Efficiency of Small Language Model on Edges](https://arxiv.org/abs/2511.11624)
*Md Romyull Islam,Bobin Deng,Nobel Dhar,Tu N. Nguyen,Selena He,Yong Shi,Kun Suo*

Main category: cs.DC

TL;DR: 评估五种小型语言模型在边缘设备上的能效表现，发现Jetson Orin Nano GPU配置能效最高，Llama 3.2在准确性和能效间取得最佳平衡，TinyLlama适合低功耗环境。


<details>
  <summary>Details</summary>
Motivation: 边缘设备部署小型语言模型具有延迟低、不依赖网络的优势，但受限于计算资源和能耗预算，需要评估实际能效表现。

Method: 在Raspberry Pi 5、Jetson Nano和Jetson Orin Nano（CPU和GPU配置）上评估Llama 3.2、Phi-3 Mini、TinyLlama和Gemma 2五种代表性SLM的功率效率。

Result: Jetson Orin Nano GPU加速实现最高能效比，显著优于CPU设置；Llama 3.2准确性和能效平衡最佳；TinyLlama适合低功耗但准确性较低；Phi-3 Mini能耗最高但准确性高。

Conclusion: GPU加速、内存带宽和模型架构是优化推理能效的关键因素，为AI、智能系统和移动平台在能耗受限环境中权衡准确性、推理延迟和能效提供实用指导。
...
### [387] [A Structure-Agnostic Co-Tuning Framework for LLMs and SLMs in Cloud-Edge Systems](https://arxiv.org/abs/2511.11678)
*Yuze Liu,Yunhan Wang,Tiehua Zhang,Zhishu Shen,Cheng Peng,Libing Wu,Feng Xia,Jiong Jin*

Main category: cs.DC

TL;DR: Co-PLMs是一个新颖的云边协同调优框架，通过结构无关的相互学习实现异构语言模型间的知识交换，解决了跨域部署和架构异构带来的性能挑战。


<details>
  <summary>Details</summary>
Motivation: 随着LLM应用的激增，带宽受限的云服务器难以实时处理大量工作负载且保护用户数据隐私。云边联盟结合服务器LLM和移动边缘SLMs成为解决方案，但跨域部署和SLMs架构异构性对性能提升构成挑战。

Method: 提出Co-PLMs框架，使用蒸馏代理模型作为桥梁，实现异构服务器LLM与设备SLMs间的协同训练，同时保留各设备的领域特定知识。采用结构无关的相互学习方法促进知识交换。

Result: 实验结果显示Co-PLMs优于最先进方法，在Rouge-L和EM指标上分别平均提升5.38%和4.88%。

Conclusion: Co-PLMs通过异构语言模型的协同训练有效提升了推理性能，为云边智能应用提供了可行的解决方案。
...
### [388] [Range Asymmetric Numeral Systems-Based Lightweight Intermediate Feature Compression for Split Computing of Deep Neural Networks](https://arxiv.org/abs/2511.11664)
*Mingyu Sung,Suhwan Im,Vikas Palakonda,Jae-Mo Kang*

Main category: cs.DC

TL;DR: 提出了一种轻量级压缩框架，利用rANS编码结合非对称整数量化和稀疏张量表示，显著减少边缘设备与云服务器间的传输开销，适用于带宽受限环境。


<details>
  <summary>Details</summary>
Motivation: 解决分割计算中中间特征传输导致的通信瓶颈问题，在保持模型性能的同时减少传输带宽需求。

Method: 采用非对称整数量化与稀疏表示技术，结合rANS编码，无需复杂概率建模或网络修改；优化张量重塑维度以最大化压缩效率；GPU加速实现亚毫秒级编码/解码延迟。

Result: 在多种神经网络架构（ResNet、VGG16、MobileNetV2等）和数据集（CIFAR100、ImageNet）上保持接近基线精度；在NLP任务（Llama2 7B/13B）和多个基准测试中也验证了有效性。

Conclusion: 该框架有效解决了带宽受限环境中部署复杂AI系统的根本瓶颈，在计算机视觉和自然语言处理领域都具有广泛适用性。
...
### [389] [How Far Do SSL Speech Models Listen for Tone? Temporal Focus of Tone Representation under Low-resource Transfer](https://arxiv.org/abs/2511.12285)
*Minu Kim,Ji Sub Um,Hoirin Kim*

Main category: eess.AS

TL;DR: 研究四种有声调语言（缅甸语、泰语、老挝语、越南语）在自监督学习语音模型中的声调感知和迁移机制，发现下游任务对声调时间跨度有显著影响。


<details>
  <summary>Details</summary>
Motivation: 声调在多种语言中至关重要，但在自监督学习语音模型中研究不足，特别是在非汉语语言中。研究旨在探索这些模型如何感知声调以及在低资源条件下的迁移机制。

Method: 使用探针分析和梯度分析研究微调后的SSL模型，比较不同下游任务（语音识别、韵律、语音相关任务）对声调时间跨度的影响。

Result: 声调迁移因下游任务而异：语音识别微调使时间跨度与语言特定声调线索对齐，而韵律和语音相关任务则偏向过长的时间跨度。缅甸语和泰语的声调线索时间跨度约为100毫秒，老挝语和越南语约为180毫秒。

Conclusion: 声调迁移受下游任务影响显著，任务特性决定了模型在声调建模中的时间聚焦范围。
...
### [390] [VoiceCraft-X: Unifying Multilingual, Voice-Cloning Speech Synthesis and Speech Editing](https://arxiv.org/abs/2511.12347)
*Zhisheng Zheng,Puyuan Peng,Anuj Diwan,Cong Phuoc Huynh,Xiaohang Sun,Zhu Liu,Vimal Bhat,David Harwath*

Main category: eess.AS

TL;DR: VoiceCraft-X是一个统一多语言语音编辑和零样本文本转语音合成的自回归神经编解码语言模型，支持11种语言，使用Qwen3大语言模型进行无音素跨语言文本处理。


<details>
  <summary>Details</summary>
Motivation: 解决多语言语音编辑和文本转语音合成任务需要统一框架的问题，克服传统方法在不同语言和任务间的分割限制。

Method: 采用自回归神经编解码语言模型，结合Qwen3大语言模型进行跨语言文本处理，引入新颖的令牌重排序机制处理时间对齐的文本和语音令牌。

Result: 模型生成高质量、自然的语音，在多样语言环境中表现稳健，即使每种语言数据有限也能良好工作。

Conclusion: 统一的自回归方法在推进复杂多语言语音应用方面具有强大潜力，VoiceCraft-X展示了这种方法的有效性。
...
### [391] [EduAgentQG: A Multi-Agent Workflow Framework for Personalized Question Generation](https://arxiv.org/abs/2511.11635)
*Rui Jia,Min Zhang,Fengrui Liu,Bo Jiang,Kun Kuang,Zhongxiang Dai*

Main category: cs.CY

TL;DR: EduAgentQG是一个多智能体协作框架，通过五个专门智能体的迭代反馈循环，生成高质量、多样化的个性化问题，在教育目标和问题质量方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 手动设计问题耗时且难以满足多样化学习需求，现有自动问题生成方法存在质量不稳定、多样性不足、与教育目标对齐不够的问题。

Method: 包含五个专门智能体的协作框架：Planner生成结构化设计计划，Writer基于计划生成候选问题，Solver和Educator进行多维度二元评分，Checker进行最终验证，通过迭代反馈循环优化问题质量。
...
### [392] [Automatic generation of DRI Statements](https://arxiv.org/abs/2511.11655)
*Maurice Flechtner*

Main category: cs.CY

TL;DR: 本文提出了一种利用NLP和LLM自动生成DRI陈述的创新方法，显著减少了群体审议评估中的人工工作量。


<details>
  <summary>Details</summary>
Motivation: DRI是评估群体推理质量的重要指标，但其陈述生成过程复杂耗时，限制了实际应用。

Method: 开发了基于先进自然语言处理和大语言模型的自动化DRI陈述生成框架。

Result: 该方法大幅降低了进行综合审议过程评估的门槛，为生成式AI融入社会科学研究方法提供了可复制的模板。

Conclusion: 自动化DRI陈述生成方法有效解决了传统方法的效率问题，推动了群体审议研究的可扩展性。
...
### [393] [Generative AI as a Linguistic Equalizer in Global Science](https://arxiv.org/abs/2511.11687)
*Dragan Filimonovic,Christian Rutzer,Jeffrey Macher,Rolf Weder*

Main category: cs.CY

TL;DR: 研究表明生成式AI正在成为全球科学中的语言平等器，通过减少非英语国家研究人员的语言障碍来重塑科学交流。


<details>
  <summary>Details</summary>
Motivation: 英语在科学界的长期主导地位为非母语者创造了障碍，生成式AI的出现为解决这一不平等问题提供了技术可能性。

Method: 分析2021-2024年间565万篇科学论文，使用SciBERT文本嵌入模型测量非英语国家作者在GenAI辅助下与美式科学写作风格的相似度变化。

Result: ChatGPT发布后，GenAI辅助的论文显示出显著且不断增长的语言风格趋同效应，对与英语语言距离较远的国家团队效果最明显。

Conclusion: 生成式AI正在通过降低语言障碍来重塑全球科学交流，为非英语国家研究人员提供语言平等机会。
...
### [394] [Leveraging Large Language Models for Career Mobility Analysis: A Study of Gender, Race, and Job Change Using U.S. Online Resume Profiles](https://arxiv.org/abs/2511.12010)
*Palakorn Achananuparp,Connie Xu,Yao Lu,Xavier Jayaraj Siddarth Ashok,Ee-Peng Lim*

Main category: cs.CY

TL;DR: 基于在线简历的大规模分析显示，公司内部职位变动对职业上升最有利，女性和非裔大学毕业生从工作变动中获得的回报显著低于男性和白人同行。


<details>
  <summary>Details</summary>
Motivation: 研究性别、种族和工作变动选择如何影响大学学历美国工人的职业上升流动性，解决现有研究中关于工作变动对职业上升影响以及不同性别种族群体结果差异的关键问题。

Method: 使用在线简历数据，开发基于大语言模型的FewSOC职业分类方法，分析228,710条职业轨迹，采用多层次敏感性分析验证结果稳健性。

Result: 公司内部职位变动对职业上升促进作用最强，其次是跨公司职位变动和跨公司平级调动；女性和非裔毕业生从工作变动中获得的回报显著较低。

Conclusion: 工作变动类型对职业上升有不同影响，且存在显著的性别和种族差异，这些差异在考虑群体异质性后依然稳健，揭示了交叉性模式。
...
### [395] [Understanding the Representation of Older Adults in Motion Capture Locomotion Datasets](https://arxiv.org/abs/2511.11713)
*Yunkai Yu,Yingying Wang,Rong Zheng*

Main category: cs.CY

TL;DR: 该论文调查了41个公开运动捕捉数据集，发现老年人运动数据严重不足，仅有8个数据集包含真实老年人运动，4个包含标注为老年风格的年轻演员运动。研究提出定量指标评估老年风格行走动作的保真度，发现这些动作往往过于控制化，无法真实反映衰老特征。


<details>
  <summary>Details</summary>
Motivation: 现有运动捕捉数据集对老年人这一重要医疗人群的代表性不足，需要系统评估数据集中老年人运动的覆盖情况和质量。

Method: 调查41个公开数据集，识别包含老年人运动的数据集；引入定量指标评估老年风格行走动作的保真度，使用对年龄敏感、抗噪声且对数据稀缺具有鲁棒性的步态参数。

Result: 老年人仅占总体参与者的小部分，提供老年人全身运动数据的数据集很少；老年风格行走动作通常表现出过度控制的模式，无法真实表征衰老特征。

Conclusion: 需要在运动数据集中改进老年人的代表性，并建立定量评估老年风格行走动作质量的方法。
...
### [396] [Accepted with Minor Revisions: Value of AI-Assisted Scientific Writing](https://arxiv.org/abs/2511.12529)
*Sanchaita Hazra,Doeun Lee,Bodhisattwa Prasad Majumder,Sachin Kumar*

Main category: cs.HC

TL;DR: 研究通过随机对照试验发现，AI生成的论文摘要经过少量修改后可以达到与人类撰写摘要相当的接受度，但作者编辑行为主要受AI作者身份感知而非客观质量驱动，揭示了来源披露在科学写作中的重要性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在各领域应用广泛，但其作为科学写作辅助工具的有效性尚未充分了解。科学写作需要精确性、多模态综合和领域专业知识，本研究旨在探索LLMs在支持领域专家进行科学写作（特别是摘要撰写）方面的潜力。

Method: 设计了激励性随机对照试验，采用假设会议设置，将相关领域专家分为作者和审稿人两组。采用2x2组间设计，考察摘要的隐含来源（AI生成vs人类撰写）和来源披露两个维度。通过新颖的激励结构鼓励作者将提供的摘要编辑到同行评审可接受的质量。

Result: 作者在编辑人类撰写摘要时比编辑无来源归属的AI生成摘要做出更多修改，这通常受到AI生成文本更高可读性感知的引导。当披露来源信息后，两种来源处理的编辑量趋于一致。审稿人决策不受摘要来源影响，但与编辑数量显著相关。在存在来源信息的情况下，对AI生成摘要进行仔细的文体编辑能提高接受机会。

Conclusion: AI生成的摘要经过最小修改后具有达到与人类撰写摘要相当接受水平的潜力，但观察到的编辑行为主要由AI作者身份感知而非客观质量驱动。研究结果强调了来源披露在协作科学写作中的重要性。
...
### [397] [Enhancing XR Auditory Realism via Multimodal Scene-Aware Acoustic Rendering](https://arxiv.org/abs/2511.11930)
...
TL;DR: SAMOSA是一个在设备上运行的XR空间音频渲染系统，通过融合实时估计的房间几何、表面材料和语义驱动的声学上下文，动态适应物理环境来合成高度逼真的房间脉冲响应，提升XR听觉真实感。


<details>
  <summary>Details</summary>
Motivation: 现有XR空间音频渲染方法难以实时适应不同的物理场景，导致视觉和听觉线索不匹配，破坏用户沉浸感。

Method: 利用多模态场景表示融合实时房间几何、表面材料和语义声学上下文，通过场景先验进行高效声学校准，合成逼真的房间脉冲响应。

Result: 技术评估显示SAMOSA在各种房间配置和声音类型下都能有效合成房间脉冲响应，专家评估(N=12)验证了系统在增强XR听觉真实感方面的可行性和有效性。

Conclusion: SAMOSA系统通过动态适应物理环境，成功解决了XR中听觉真实感不足的问题，为创造更逼真的虚拟体验提供了有效解决方案。
...
### [398] [Trust in Vision-Language Models: Insights from a Participatory User Workshop](https://arxiv.org/abs/2511.13458)
*Agnese Chiatti,Lara Piccolo,Sara Bernardini,Matteo Matteucci,Viola Schiaffonati*

Main category: cs.HC

TL;DR: 该论文通过用户工作坊探索用户对视觉语言模型(VLMs)信任度的形成与演变，为未来研究提供信任度量和参与策略的初步见解。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型的广泛应用，需要理解用户何时应该信任这些系统，但目前缺乏对用户信任构建过程的研究，且过度依赖AI模型作为实验验证存在局限性。

Method: 采用以用户为中心的方法，与潜在VLM用户进行工作坊，收集初步数据。

Result: 工作坊提供了关于用户信任度构建的初步见解，为未来研究设计合适的信任度量和参与策略奠定了基础。

Conclusion: 用户工作坊是理解VLM信任问题的有效方法，为后续研究提供了重要参考框架，强调需要更贴近用户实际体验的研究方法。
...
### [399] [Slow - Motion Video Synthesis for Basketball Using Frame Interpolation](https://arxiv.org/abs/2511.11644)
*Jiantang Huang*

Main category: eess.IV

TL;DR: 该研究开发了一个实时慢动作合成系统，通过微调RIFE网络在篮球视频上，实现了高质量的4倍慢动作生成，在PSNR和SSIM指标上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统篮球广播视频帧率较低（30-60 fps），限制了观众欣赏快速动作（如扣篮和变向）的能力，需要开发专门的慢动作合成技术。

Method: 在SportsSloMo数据集的篮球子集上微调RIFE网络，采用人类感知的随机裁剪策略，构建端到端的4倍慢动作生成系统。

Result: 微调后的RIFE模型达到平均PSNR 34.3 dB和SSIM 0.949，分别比Super SloMo高2.1 dB，比基线RIFE高1.3 dB，在RTX 4070 Ti Super上实现约30 fps的实时性能。

Conclusion: 任务特定的适配对于体育慢动作合成至关重要，RIFE在精度和速度之间提供了良好的平衡，适合消费级应用。
...
### [400] [A Deep Learning Framework for Thyroid Nodule Segmentation and Malignancy Classification from Ultrasound Images](https://arxiv.org/abs/2511.11937)
...
<details>
  <summary>Details</summary>
Motivation: 解决甲状腺结节超声风险分层任务中观察者间变异性的问题，同时提供可解释的深度学习模型，避免传统黑盒模型的问题。

Method: 两阶段框架：第一阶段使用TransUNet自动分割甲状腺结节，第二阶段将分割后的结节区域输入ResNet-18分类器进行恶性预测。

Result: 在349张临床图像数据集上通过5折交叉验证获得0.852的F1分数，优于基于手工形态特征的随机森林基线（F1分数0.829）。

Conclusion: 这是首个完全自动化的端到端管道，能够同时检测甲状腺结节并预测其恶性程度，证明从局部结节学习的隐式视觉特征比显式形状特征更具预测性。
...
### [401] [Recursive Threshold Median Filter and Autoencoder for Salt-and-Pepper Denoising: SSIM analysis of Images and Entropy Maps](https://arxiv.org/abs/2511.12212)
*Petr Boriskov,Kirill Rudkovskii,Andrei Velichko*

Main category: eess.IV

TL;DR: 本文研究使用中值滤波器和简单三层自编码器在递归阈值算法中去除图像椒盐噪声，提出两种可扩展方案并验证SSIMMap指标对模糊评估的实用价值。


<details>
  <summary>Details</summary>
Motivation: 研究如何有效去除图像中的椒盐噪声，特别是针对不同噪声水平和图像分辨率的情况，并探索更敏感的评估指标来补充传统SSIM。

Method: 采用中值滤波器(MF)和简单三层自编码器(AE)结合递归阈值算法，提出两种方案：2MF（使用两个不同窗口大小的MF和最终阈值处理）和MFs-AE（通过AE聚合多个MF的特征）。

Result: 递归阈值MF在强噪声(50-60%)下仍能稳健恢复图像，而简单AE仅能处理低噪声(<30%)图像。2MF方案在低分辨率下有效突出局部细节，MFs-AE在高分辨率下有利于恢复整体场景结构。

Conclusion: MF因其简单性和计算效率在资源受限平台上更受青睐，而AE在没有预去噪的情况下表现不佳。SSIMMap在客观模糊评估和去噪参数调优方面具有实用价值。
...
### [402] [Deep Unfolded BM3D: Unrolling Non-local Collaborative Filtering into a Trainable Neural Network](https://arxiv.org/abs/2511.12248)
*Kerem Basim,Mehmet Ozan Unal,Metin Ertas,Isa Yildirim*

Main category: eess.IV

TL;DR: 提出了深度展开BM3D（DU-BM3D）框架，将传统BM3D算法展开为可训练架构，用可学习的U-Net去噪器替代固定协作滤波，结合非局部自相似性先验和深度学习灵活性。


<details>
  <summary>Details</summary>
Motivation: BM3D依赖固定参数，而深度模型如U-Net缺乏可解释性且在不同噪声机制下泛化能力差。需要结合两者优势。

Method: 将BM3D算法展开为可训练架构，用U-Net替代固定协作滤波，实现端到端优化，同时保留非局部结构先验。

Result: 在低剂量CT去噪任务中，DU-BM3D在不同噪声水平下均优于传统BM3D和独立U-Net，尤其在强噪声条件下PSNR和SSIM指标更高。

Conclusion: DU-BM3D成功结合了传统算法先验知识和深度学习灵活性，在CT图像去噪中表现出优越性能。
...
### [403] [Multimodal RGB-HSI Feature Fusion with Patient-Aware Incremental Heuristic Meta-Learning for Oral Lesion Classification](https://arxiv.org/abs/2511.12268)
*Rupam Mukherjee,Rajkumar Daniel,Soujanya Hazra,Shirin Dasgupta,Subhamoy Mandal*

Main category: eess.IV

TL;DR: 提出了一种用于口腔病变筛查的统一四分类器，结合了深度RGB嵌入、高光谱重建、手工特征和人口统计元数据，通过增量启发式元学习器提高在低资源环境下的检测性能。
...
Method: 使用微调的ConvNeXt-v2编码器处理口腔图像，进行RGB到HSI重建生成31波段高光谱立方体，提取血红蛋白敏感指数、纹理特征和光谱形状度量，并与深度特征和临床特征融合，采用增量启发式元学习器结合校准的基础分类器。
...
### [404] [RAA-MIL: A Novel Framework for Classification of Oral Cytology](https://arxiv.org/abs/2511.12269)
*Rupam Mukherjee,Rajkumar Daniel,Soujanya Hazra,Shirin Dasgupta,Subhamoy Mandal*

Main category: eess.IV

TL;DR: 首个弱监督深度学习框架用于口腔细胞学全玻片图像的病人级诊断，通过多示例学习建模空间关系，在未见测试集上达到72.7%准确率


<details>
  <summary>Details</summary>
Motivation: 手动检查口腔细胞学全玻片图像速度慢、主观性强且依赖专家病理学家，需要可靠的AI辅助数字病理解决方案

Method: 提出区域亲和力注意力多示例学习(RAA-MIL)框架，将每个病人病例表示为细胞学图像块的集合，建模切片内区域间的空间关系

Result: RAA-MIL在未见测试集上达到平均准确率72.7%，加权F1分数0.69，优于基线方法

Conclusion: 本研究建立了首个口腔细胞学病人级弱监督基准，推动了可靠的AI辅助数字病理发展
...
### [405] [MTMed3D: A Multi-Task Transformer-Based Model for 3D Medical Imaging](https://arxiv.org/abs/2511.12373)
*Fan Li,Arun Iyengar,Lanyu Xu*

Main category: eess.IV

TL;DR: 提出了MTMed3D，首个基于Transformer的多任务学习模型，在3D医学影像中同时执行检测、分割和分类任务，显著降低计算成本并提升推理速度。
...
Method: 使用Transformer作为共享编码器生成多尺度特征，然后使用CNN任务特定解码器，在BraTS数据集上评估。

Result: 在三个任务上都取得了有希望的结果，特别是在检测任务上优于先前工作，多任务模型相比单任务变体显著降低了计算成本。

Conclusion: 这是首个在3D医学影像中同时覆盖检测、分割和分类任务的Transformer多任务学习工作，展示了提升诊断流程的潜力。
...
### [406] [DEMIST: \underline{DE}coupled \underline{M}ulti-stream latent d\underline{I}ffusion for Quantitative Myelin Map \underline{S}yn\underline{T}hesis](https://arxiv.org/abs/2511.12396)
...
TL;DR: DEMIST使用3D潜在扩散模型从标准T1w和FLAIR图像合成qMT成像的PSR图，通过三种互补条件机制实现快速、高质量的合成。


<details>
  <summary>Details</summary>
Motivation: 传统qMT成像需要20-30分钟的专业扫描，耗时较长。研究旨在从标准MRI序列快速合成PSR图，简化多发性硬化评估流程。

Method: 两阶段方法：1) 训练PSR和解剖图像的自编码器学习对齐潜在表示；2) 在冻结扩散基础模型上训练条件扩散模型，使用语义token、空间残差提示和自适应LoRA三种条件机制。

Result: 在163个扫描的5折交叉验证中，DEMIST在多个指标上优于VAE、GAN和扩散基线，产生更清晰的边界和更好的定量一致性。

Conclusion: DEMIST能够从标准MRI序列高效合成高质量的PSR图，为多发性硬化评估提供了更便捷的解决方案。
...
### [407] [Improving the Generalisation of Learned Reconstruction Frameworks](https://arxiv.org/abs/2511.12730)
*Emilien Valat,Ozan Öktem*

Main category: eess.IV

TL;DR: 提出GLM混合神经网络架构，结合图卷积和网格卷积处理CT投影数据，解决CNN在CT成像中泛化能力差和参数过多的问题。


<details>
  <summary>Details</summary>
Motivation: 传统CNN在CT逆问题中泛化能力不足，因为CNN将网格卷积应用于正弦图（本质上是线流形），无法理解几何关系，需要大量参数且性能受限。

Method: 引入图数据结构表示CT采集几何和断层数据，提出GLM混合架构，结合图卷积和网格卷积处理断层数据。

Result: GLM在结构相似性和峰值信噪比方面优于CNN，仅使用少量可训练参数，训练时间和内存需求显著减少，且能泛化到未见过的采集几何变化。

Conclusion: GLM架构在CT成像中比CNN更高效、泛化能力更强，为数据驱动的逆问题求解提供了更好的解决方案。
...
### [408] [BrainNormalizer: Anatomy-Informed Pseudo-Healthy Brain Reconstruction from Tumor MRI via Edge-Guided ControlNet](https://arxiv.org/abs/2511.12853)
*Min Gu Kwak,Yeonju Lee,Hairong Wang,Jing Li*

Main category: eess.IV

TL;DR: BrainNormalizer是一个基于扩散模型的框架，能够直接从肿瘤MRI扫描重建伪健康MRI，为脑肿瘤患者的治疗规划提供解剖学参考。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤会引发显著的解剖结构变形，但在临床实践中无法获得无肿瘤影响的个体化参考图像，这给诊断和治疗规划带来困难。

Method: 采用边界引导的扩散框架，通过两阶段训练策略：首先进行基于修复的微调，然后训练边缘图引导的ControlNet分支，在推理时利用镜像对侧边缘图和错位策略进行重建。

Result: 在BraTS2020数据集上，BrainNormalizer实现了强大的定量性能，并在肿瘤区域产生解剖学上合理的重建，同时保持整体结构一致性。

Conclusion: BrainNormalizer为治疗规划提供临床可靠的解剖学参考，并支持反事实建模和肿瘤诱导变形分析的新研究方向。
...
### [409] [Inertia-Informed Orientation Priors for Event-Based Optical Flow Estimation](https://arxiv.org/abs/2511.12961)
*Pritam P. Karmokar,William J. Beksi*

Main category: eess.IV

TL;DR: 提出一种基于对比度最大化的混合方法，结合视觉和惯性运动线索来估计事件相机的光流，通过方向图引导优化过程提高鲁棒性和收敛性。


<details>
  <summary>Details</summary>
Motivation: 事件相机直接编码场景运动，但事件的时间密集空间稀疏特性给光流估计带来挑战。对比度最大化方法虽然有效，但仍是高度非凸优化问题。

Method: 引入生物启发的混合方法，使用从相机3D速度导出的方向图作为先验来指导对比度最大化过程，约束运动轨迹空间。

Result: 在MVSEC、DSEC和ECD数据集上的评估显示，该方法在准确性上优于现有最先进方法。

Conclusion: 方向图引导的对比度最大化方法显著提高了事件相机光流估计的鲁棒性和收敛性能。
...
### [410] [TR-Gaussians: High-fidelity Real-time Rendering of Planar Transmission and Reflection with 3D Gaussian Splatting](https://arxiv.org/abs/2511.13009)
*Yong Liu,Keyang Ye,Tianjia Shao,Kun Zhou*

Main category: cs.GR

TL;DR: TR-Gaussians是一种基于3D高斯的新型表示方法，用于平面透射和反射的高保真渲染，结合了3D高斯和可学习反射平面，通过多阶段优化框架实现实时高质量新视角合成。
...
Method: 将3D高斯与可学习反射平面结合，透射分量由3D高斯建模，反射分量由镜像高斯建模，基于菲涅尔效应的视角相关权重混合，采用多阶段优化框架包含颜色几何约束和透明度扰动机制。

Result: 在不同数据集上的实验表明，TR-Gaussians在包含平面透射和反射的场景中实现了实时高保真新视角合成，在定量和定性评估中都优于最先进方法。

Conclusion: TR-Gaussians能够有效建模平面透射和反射，为室内场景渲染提供了高质量的解决方案。
...
### [411] [Predicting upcoming visual features during eye movements yields scene representations aligned with human visual cortex](https://arxiv.org/abs/2511.12715)
*Sushrut Thorat,Adrien Doerig,Alexander Kroner,Carmen Amme,Tim C. Kietzmann*

Main category: q-bio.NC

TL;DR: 本文提出Glimpse Prediction Networks (GPNs)，通过自监督学习从主动视觉的时间规律中学习场景表示，该模型能够预测人类扫描路径中的下一个视觉片段，产生的场景表示与人类fMRI反应高度对齐。


<details>
  <summary>Details</summary>
Motivation: 场景是复杂的结构化部分集合，需要统一的场景表示来关联场景部分的位置和共现关系。假设这种结构可以通过主动视觉的时间规律自监督学习得到。

Method: 使用Glimpse Prediction Networks (GPNs) - 循环模型，训练预测自然场景中人类扫描路径上的下一个视觉片段的特征嵌入，利用共现和眼跳条件空间规律。

Result: GPNs成功学习共现结构，对空间排列敏感，能够将多个视觉片段信息整合为统一场景表示。这些表示与人类fMRI反应高度对齐，优于语义目标训练的对照组和现代视觉基线。

Conclusion: 主动视觉中的下一个视觉片段预测是生物可信的自监督途径，能够从自然视觉经验中学习与大脑对齐的场景表示。
...
### [412] [Towards Mitigating Systematics in Large-Scale Surveys via Few-Shot Optimal Transport-Based Feature Alignment](https://arxiv.org/abs/2511.11787)
*Sultan Hassan,Sambatra Andrianomena,Benjamin D. Wandelt*

Main category: astro-ph.IM

TL;DR: 提出一种特征对齐方法，通过优化预训练模型提取的特征对齐损失来对齐分布内和分布外样本的特征，以解决系统误差导致的分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 系统误差会污染观测数据，导致与理论模拟信号相比出现分布偏移，这对使用预训练模型标记此类观测数据构成重大挑战。由于系统误差通常难以理解和建模，直接完全消除不可行。

Method: 通过优化预训练ID模型提取的特征对齐损失来对齐ID和OOD样本的学得特征，实验验证了包括均方误差和最优传输在内的可能对齐损失。

Result: 结果表明，在ID和OOD样本对等性未知的情况下，最优传输特别有效地对齐OOD特征，即使在数据有限的情况下也能模拟从大规模调查中提取信息的真实条件。

Conclusion: 提出的特征对齐方法能够有效处理系统误差导致的分布偏移问题，最优传输在未知对等性情况下表现优异，适用于大规模调查中的信息提取。
...
### [413] [Preference Learning from Physics-Based Feedback: Tuning Language Models to Design BCC/B2 Superalloys](https://arxiv.org/abs/2511.12036)
*Satanu Ghosh,Collin Holgate,Neal R. Brodnik,Doug Downey,Samantha Daly,Tresa M. Pollock,Samuel Carton*

Main category: cs.CE

TL;DR: 该论文应用偏好学习于语言模型引导的结构合金设计，首次使用基于热力学相计算的物理基础反馈来优化语言模型，针对BCC/B2高温合金的可合成性进行多目标设计。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注生成稳定的无机晶体，而本研究针对BCC/B2高温合金这一具有极端环境应用潜力但研究不足的结构类别，旨在开发基于物理基础的模型优化方法。

Method: 使用三个开源模型（LLaMA-3.1、Gemma-2和OLMo-2），通过直接偏好优化（DPO）方法，利用热力学相计算得到的统一奖励信号来优化语言模型的多重设计目标。

Result: 研究表明语言模型可以通过基于热力学计算的物理基础反馈进行偏好调优，为结构合金设计提供科学依据，避免了传统启发式或人工反馈的高成本问题。
...
### [414] [LLM-Generated Negative News Headlines Dataset: Creation and Benchmarking Against Real Journalism](https://arxiv.org/abs/2511.11591)
*Olusola Babalola,Bolanle Ojokoh,Olutayo Boyinbode*

Main category: cs.AI

TL;DR: 本研究探讨了使用LLM生成的合成新闻标题作为真实世界数据替代方案的可行性，特别关注负面情感文本在情感分析中的应用。


<details>
  <summary>Details</summary>
Motivation: 解决NLP任务中数据获取困难和真实数据隐私问题，探索LLM生成数据在负面情感分析中的潜力。

Method: 使用定制提示创建负面新闻标题语料库，通过专家评审和嵌入空间分析验证合成数据，采用多种指标与真实新闻标题进行基准比较。

Result: 生成的标题在内容、语调、长度和风格上与真实标题高度匹配，仅在POS分析中的专有名词得分存在明显差异。

Conclusion: LLM生成的合成数据集可以作为真实数据的有效替代品，在负面情感分析任务中表现出良好的适用性。
...
### [415] [CLINB: A Climate Intelligence Benchmark for Foundational Models](https://arxiv.org/abs/2511.11597)
...
TL;DR: CLINB是一个评估大语言模型处理气候变化专业知识的基准，发现前沿模型具有博士级别的知识综合能力，但在证据基础和引用方面存在严重幻觉问题。
...
Method: 引入CLINB基准，基于真实用户问题和气候科学家制定的评估标准，采用模型驱动的评估流程来测试多个前沿模型在开放式、多模态问答任务中的表现。

Result: 前沿模型展现出卓越的知识综合能力，甚至超过专家辅助的混合答案质量，但在证据基础方面存在显著问题，引用和图像存在高幻觉率。

Conclusion: 在知识综合与可验证归因之间建立桥梁对于AI在科学工作流程中的部署至关重要，需要像CLINB这样的可靠基准来构建可信赖的AI系统。
...
### [416] [SynBullying: A Multi LLM Synthetic Conversational Dataset for Cyberbullying Detectio](https://arxiv.org/abs/2511.11599)
...
<details>
  <summary>Details</summary>
Motivation: 传统网络欺凌数据收集存在伦理问题和规模限制，需要可扩展且安全的替代方案来研究多轮对话中的欺凌行为。

Method: 利用大语言模型生成模拟真实欺凌互动的多轮对话数据，提供对话结构、上下文感知标注和细粒度标签。

Result: 数据集在对话结构、词汇模式、情感/毒性、角色动态、伤害强度和欺凌类型分布等五个维度上进行了评估，并测试了其作为训练数据和增强源的效用。
...
### [417] [Do LLMs Really Struggle at NL-FOL Translation? Revealing their Strengths via a Novel Benchmarking Strategy](https://arxiv.org/abs/2511.11816)
*Andrea Brunello,Luca Geatti,Michele Mignani,Angelo Montanari,Nicola Saccomanno*

Main category: cs.AI

TL;DR: 本文提出了一种新的评估协议来准确评估大语言模型在自然语言到一阶逻辑翻译中的能力，发现对话导向的LLMs具有强大的逻辑理解能力，而嵌入中心模型表现较差。


<details>
  <summary>Details</summary>
Motivation: 现有NL-FOL翻译评估存在局限性，可能低估LLMs的实际能力，需要更准确的方法来区分真正的语义级逻辑理解与表面模式识别。

Method: 提出新颖评估协议，专门设计用于区分真正的语义级逻辑理解与表面模式识别、记忆和数据集污染。

Result: 最先进的对话导向LLMs展现出强大的NL-FOL翻译技能和真正的句子级逻辑理解能力，而嵌入中心模型表现明显更差。

Conclusion: 通过适当的评估方法，对话导向的LLMs确实具备强大的NL-FOL翻译能力和逻辑理解能力，这为逻辑推理应用开辟了新途径。
...
### [418] [Forgetting-MarI: LLM Unlearning via Marginal Information Regularization](https://arxiv.org/abs/2511.11914)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI模型在扩展数据集上训练，需要从训练模型中移除特定数据的影响以满足隐私保护和法规合规要求，避免从头重新训练资源密集型模型。

Method: 引入Forgetting-MarI框架，通过惩罚边际信息来仅移除待遗忘数据贡献的额外信息，同时保留待保留数据支持的信息。

Result: 在广泛实验中，该方法优于当前最先进的遗忘方法，提供可靠的遗忘效果和更好的通用模型性能保留。

Conclusion: 这一进展是使AI系统更可控、更符合隐私和版权法规的重要步骤，同时不损害其有效性。
...
### [419] [WebCoach: Self-Evolving Web Agents with Cross-Session Memory Guidance](https://arxiv.org/abs/2511.12997)
*Genglin Liu,Shijie Geng,Sha Li,Hejie Cui,Sarah Zhang,Xin Liu,Tianyi Liu*

Main category: cs.AI

TL;DR: WebCoach是一个模型无关的自进化框架，为多模态LLM驱动的网页浏览代理提供跨会话持久记忆，通过记忆存储和检索机制提升长期规划和持续学习能力，无需重新训练即可提高任务成功率。


<details>
  <summary>Details</summary>
Motivation: 当前多模态LLM代理在网页导航中存在重复错误问题，缺乏跨会话学习能力，限制了长期鲁棒性和样本效率，需要一种能够从历史经验中学习的方法。

Method: WebCoach包含三个核心组件：WebCondenser标准化导航日志为摘要、External Memory Store组织完整轨迹作为经验、Coach基于相似性和时效性检索相关经验并通过运行时钩子注入任务特定建议。

Result: 在WebVoyager基准测试中，WebCoach显著提升了三种不同LLM骨干网络的性能，38B模型任务成功率从47%提升至61%，同时减少或维持平均步骤数。较小基础模型配合WebCoach可达到与GPT-4o相当的性能。

Conclusion: WebCoach通过赋予网页浏览代理持久跨会话记忆能力，实现了自进化和持续学习，显著提升了复杂浏览任务的鲁棒性和性能，为智能代理的长期发展提供了有效解决方案。
...
### [420] [PragWorld: A Benchmark Evaluating LLMs' Local World Model under Minimal Linguistic Alterations and Conversational Dynamics](https://arxiv.org/abs/2511.13021)
*Sachin Vashistha,Aryan Bibhuti,Atharva Naik,Martin Tutek,Somak Aditya*

Main category: cs.AI

TL;DR: 本文评估了语言模型在对话中构建和维护世界模型的能力，发现模型在语言变化下难以保持准确率，并提出了一种双视角可解释性框架和层正则化微调策略。
...
Method: 对流行数据集的对话应用七种最小语言变化，构建两个是-否问答基准，评估多种开源和闭源语言模型，并提出双视角可解释性框架识别有用/有害的transformer层。

Result: 语言模型在语言变化下难以保持稳健准确率，特别是在跟踪实体等关键细节方面表现不佳。有害层通常编码虚假信号或依赖捷径。

Conclusion: 语言模型在对话中构建和维护世界模型的能力有限，提出的层正则化微调策略可以抑制有害层的影响，提高模型性能。
...
### [421] [STEP: Success-Rate-Aware Trajectory-Efficient Policy Optimization](https://arxiv.org/abs/2511.13091)
*Yuhan Chen,Yuxuan Liu,Long Zhang,Pengzhi Gao,Jian Luan,Wei Liu*

Main category: cs.AI

TL;DR: STEP框架通过成功率感知的轨迹高效策略优化，解决了多轮交互强化学习中轨迹级优化的低效问题，实现了基于任务难度的动态采样分配和步骤级优化。


<details>
  <summary>Details</summary>
Motivation: 传统轨迹级优化方法存在效率低下和学习信号误导问题：对任务难度不敏感、惩罚失败轨迹中的正确中间动作、采样成本高。

Method: STEP框架包含三个核心组件：基于平滑成功率记录的自适应轨迹重采样、成功率加权优势计算、步骤级GRPO增强优化。

Result: 在OSWorld和AndroidWorld上的实验表明，STEP相比轨迹级GRPO显著提高了采样效率和训练稳定性，在相同采样预算下收敛更快且泛化能力更强。
...
### [422] [Dropouts in Confidence: Moral Uncertainty in Human-LLM Alignment](https://arxiv.org/abs/2511.13290)
*Jea Kwon,Luiz Felipe Vecchietti,Sungwon Park,Meeyoung Cha*

Main category: cs.AI

TL;DR: 该研究探讨了AI模型在道德困境中的不确定性，发现在电车问题中模型间的不确定性差异大于道德维度间的差异，通过引入推理时dropout机制可增加互信息并改善人类-LLM道德对齐。


<details>
  <summary>Details</summary>
Motivation: 人类在道德困境中表现出显著不确定性，但AI系统的不确定性研究不足。随着AI越来越多地参与伦理决策，理解其道德推理和不确定性对构建可靠AI系统至关重要。

Method: 分析32个开源模型在9个道德维度上的电车问题响应，使用二元熵量化不确定性，通过推理时dropout引入随机性，测量总熵、条件熵和互信息的变化。

Result: 模型间置信度方差大于道德维度间方差；dropout机制主要增加互信息而条件熵基本不变；该机制显著改善人类-LLM道德对齐，互信息与对齐分数变化相关。

Conclusion: 通过有意识地调节不确定性和降低LLM在道德复杂场景中的置信度，可以更好地对齐模型决策与人类偏好。
...
### [423] [Value-Aligned Prompt Moderation via Zero-Shot Agentic Rewriting for Safe Image Generation](https://arxiv.org/abs/2511.11693)
*Xin Zhao,Xiaojun Chen,Bingshan Liu,Zeyao Liu,Zhendong Zhao,Xiaoyan Gu*

Main category: cs.AI

TL;DR: VALOR是一个模块化、零样本的代理框架，通过分层提示分析和人类价值观对齐推理，实现更安全、更有帮助的文本到图像生成。


<details>
  <summary>Details</summary>
Motivation: 生成式视觉语言模型如Stable Diffusion在创意媒体合成方面表现出色，但在对抗性提示下可能产生不安全、冒犯或文化不适当的内容。现有防御方法难以在不牺牲生成质量或增加高成本的情况下使输出与人类价值观对齐。

Method: VALOR整合了分层提示分析：多级NSFW检测器过滤词汇和语义风险；文化价值对齐模块识别社会规范、合法性和代表性伦理违规；意图消歧器检测微妙或不安全暗示。检测到不安全内容时，在动态角色特定指令下由大语言模型选择性重写提示。如果生成图像仍不安全，可选择进行风格化再生以引导输出到更安全的视觉领域。

Result: 在对抗性、模糊性和价值敏感提示上的实验表明，VALOR将不安全输出显著减少高达100.00%，同时保持提示的有用性和创造性。

Conclusion: VALOR作为可扩展且有效的方法，可在开放世界环境中部署安全、对齐且有用的图像生成系统。
...
### [424] [TopoPerception: A Shortcut-Free Evaluation of Global Visual Perception in Large Vision-Language Models](https://arxiv.org/abs/2511.11831)
...
Method: 利用拓扑性质构建评估基准，因为拓扑依赖于图像全局结构且对局部特征不变，从而实现无捷径的全局感知评估。

Result: 所有最先进模型在最粗粒度感知上都表现不佳，准确率不高于随机猜测。更强大的模型反而准确率更低。

Conclusion: 仅扩大模型规模不足以解决全局视觉感知缺陷，可能需要新的训练范式或架构。TopoPerception揭示了当前LVLMs的关键瓶颈并指明了改进方向。
...
### [425] [End to End AI System for Surgical Gesture Sequence Recognition and Clinical Outcome Prediction](https://arxiv.org/abs/2511.11899)
...
TL;DR: F2O系统将手术视频转化为手势序列，通过分析手势频率、持续时间和转换模式来预测患者术后结果，特别是勃起功能恢复情况。


<details>
  <summary>Details</summary>
Motivation: 精细分析术中行为对患者结果的影响是一个长期挑战，需要自动化的方法来理解手术表现与临床结果之间的关系。

Method: 使用基于transformer的空间和时间建模，结合逐帧分类，在机器人辅助前列腺切除术中检测连续短手势（约2秒）。

Result: F2O在手势检测上表现优异（AUC：0.80帧级；0.81视频级），预测术后结果的准确性与人工标注相当（0.79 vs 0.75），并识别出与勃起功能恢复相关的关键模式。

Conclusion: F2O通过自动可解释的评估，为数据驱动的手术反馈和前瞻性临床决策支持奠定了基础。
...
### [426] [Adaptive Diagnostic Reasoning Framework for Pathology with Multimodal Large Language Models](https://arxiv.org/abs/2511.12008)
*Yunqi Hong,Johnson Kao,Liam Edwards,Nein-Tzu Liu,Chung-Yen Huang,Alex Oliveira-Kowaleski,Cho-Jui Hsieh,Neil Y. C. Lin*

Main category: cs.AI

TL;DR: RECAP-PATH是一个可解释的病理AI框架，通过自学习范式让多模态大语言模型从被动模式识别转向基于证据的诊断推理，无需大量标注数据即可生成癌症诊断。


<details>
  <summary>Details</summary>
Motivation: 当前病理AI系统缺乏人类可读的推理过程，限制了临床采用。需要能够审计决策和防止错误的可解释AI工具。

Method: 采用两阶段自学习过程：多样化阶段扩展病理学风格的解释，优化阶段为准确性精炼这些解释。无需白盒访问或权重更新。

Result: 在乳腺和前列腺数据集上，RECAP-PATH生成的推理与专家评估一致，诊断准确性相比基线方法有显著提升。

Conclusion: RECAP-PATH通过结合视觉理解和推理，提供了临床可信赖的AI，展示了实现基于证据解释的通用路径。
...
### [427] [AURA: Development and Validation of an Augmented Unplanned Removal Alert System using Synthetic ICU Videos](https://arxiv.org/abs/2511.12241)
*Junhyuk Seo,Hyeyoon Moon,Kyu-Hwan Jung,Namkee Oh,Taerim Kim*

Main category: cs.AI

TL;DR: AURA是一个基于视觉的意外拔管风险检测系统，完全使用合成ICU视频数据集开发，通过姿态估计识别手部进入气道管附近区域（碰撞）和关键点速度（躁动）两种高风险模式。


<details>
  <summary>Details</summary>
Motivation: ICU中意外拔管是严重的安全问题，但由于伦理和隐私限制，获取带标注的ICU视频数据困难，阻碍了实时检测系统的开发。

Method: 利用文本到视频扩散技术生成多样且临床真实的ICU场景合成视频数据集，应用姿态估计识别碰撞（手部进入气道管附近空间区域）和躁动（追踪解剖关键点速度）两种高风险运动模式。
...
### [428] [Yanyun-3: Enabling Cross-Platform Strategy Game Operation with Vision-Language Models](https://arxiv.org/abs/2511.12937)
*Guoyan Wang,Yanyan Huang,Chunlin Chen,Lifeng Wang,Yuxiang Sun*

Main category: cs.AI

TL;DR: Yanyun-3是一个通用智能体框架，首次实现了在三个异构策略游戏环境中的自主跨平台操作，通过结合视觉语言推理和精确执行能力，在减少63%推理时间的同时将BLEU-4分数提升12.98倍。
...
Method: 集成Qwen2.5-VL的视觉语言推理能力和UI-TARS的精确执行能力，采用屏幕捕获-模型推理-动作执行的闭环流程，研究多模态数据组合策略（静态图像、多图像序列、视频）。

Result: 混合策略（融合多图像和视频数据，同时混合静态图像）显著优于完全融合：推理时间减少63%，BLEU-4分数从4.81%提升至62.41%（约12.98倍提升）。
...
### [429] [MEGA-GUI: Multi-stage Enhanced Grounding Agents for GUI Elements](https://arxiv.org/abs/2511.13087)
*SeokJoo Kwak,Jihoon Kim,Boyoun Kim,Jung Jae Yoon,Wooseok Jang,Jeonghoon Hong,Jaeho Yang,Yeong-Dae Kwon*

Main category: cs.AI

TL;DR: MEGA-GUI是一个多阶段GUI定位框架，通过分离粗粒度ROI选择和细粒度元素定位来解决视觉杂乱和语义模糊问题，在多个基准测试中超越了现有方法。
...
Conclusion: 模块化结构利用不同视觉尺度下视觉语言模型的互补优势，比单一方法获得更高准确率。
...
### [430] [MM-Telco: Benchmarks and Multimodal Large Language Models for Telecom Applications](https://arxiv.org/abs/2511.13131)
...
TL;DR: MM-Telco是一个针对电信领域的多模态基准测试套件和模型，旨在克服LLMs在电信应用中的领域特定挑战，提升网络优化、故障排除等任务的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在电信领域有巨大潜力，但面临领域特定挑战，需要专门适配才能有效应用于网络优化、自动化故障排除等任务。
...
Conclusion: MM-Telco为电信领域LLMs的发展提供了重要基准，指导进一步的研究和开发方向。
...
### [431] [DAP: A Discrete-token Autoregressive Planner for Autonomous Driving](https://arxiv.org/abs/2511.13306)
...
<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶中数据和模型规模扩展时的可持续性能提升挑战，自回归模型在规划任务中表现出数据扩展效率，但仅预测自车轨迹存在监督稀疏和场景演化约束弱的问题。

Method: 提出离散令牌自回归规划器，联合预测BEV语义和自车轨迹；结合基于强化学习的微调，保留监督行为克隆先验的同时注入奖励引导的改进。
...
### [432] [Attention Grounded Enhancement for Visual Document Retrieval](https://arxiv.org/abs/2511.13415)
*Wanqing Cui,Wei Huang,Yazhi Guo,Yibo Hu,Meiguang Jin,Junfeng Ma,Keping Bi*

Main category: cs.IR

TL;DR: AGREE框架通过利用多模态大语言模型的跨模态注意力作为局部监督信号，结合全局监督联合优化检索器，显著提升了视觉文档检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索器仅使用粗粒度的全局相关性标签进行训练，无法识别支持匹配的具体文档区域，导致依赖表面线索且难以处理非抽取式查询。

Method: 提出AGREE框架，利用多模态大语言模型的跨模态注意力作为代理局部监督，指导识别相关文档区域，将局部信号与全局信号结合联合优化检索器。

Result: 在ViDoRe V2基准测试中显著优于仅使用全局监督的基线方法，定量和定性分析表明AGREE促进了查询术语与文档区域之间更深层次的对齐。

Conclusion: AGREE框架能够超越表面级匹配，实现更准确和可解释的检索，使检索器不仅学习文档是否匹配，还学习驱动相关性的具体内容。
...
### [433] [Exploring Multi-Table Retrieval Through Iterative Search](https://arxiv.org/abs/2511.13418)
*Allaa Boutaleb,Bernd Amann,Rafael Angarita,Hubert Naacke*

Main category: cs.IR

TL;DR: 提出了一种迭代式贪婪连接感知检索算法，用于数据湖中的多表检索，在保证检索性能的同时大幅提升效率


<details>
  <summary>Details</summary>
Motivation: 解决多表检索中语义相关性和结构连贯性的平衡问题，传统MIP方法计算复杂度过高，而简单贪心算法又难以找到可连接的连贯表集合

Method: 将多表检索构建为迭代搜索过程，提出贪婪连接感知检索框架，综合考虑相关性、覆盖率和可连接性

Result: 在5个NL2SQL基准测试中，与MIP方法相比达到竞争性检索性能，同时速度提升4-400倍
...
### [434] [LLM Architecture, Scaling Laws, and Economics: A Quick Summary](https://arxiv.org/abs/2511.11572)
*William H. Press*

Main category: cs.GL

TL;DR: 本文总结了当前大型语言模型的标准QKV自注意力架构和Transformer结构，提供了计算和内存的扩展规律，以及2025年不同规模LLM参数的成本估算，并讨论了DeepSeek是否应被视为特例。


<details>
  <summary>Details</summary>
Motivation: 虽然这些内容并非全新，但似乎缺乏现成的总结形式，作者希望提供一个系统性的总结来填补这一空白。

Method: 采用总结性分析方法，系统梳理LLM的标准架构、扩展规律和成本估算。

Result: 提供了Transformer架构的简明概述、计算和内存的扩展规律，以及2025年不同规模LLM参数的成本估算数据。

Conclusion: 该文为理解LLM架构和扩展规律提供了有价值的总结性资源，并探讨了DeepSeek在其中的定位。
...
### [435] [Tactile Data Recording System for Clothing with Motion-Controlled Robotic Sliding](https://arxiv.org/abs/2511.11634)
*Michikuni Eguchi,Takekazu Kitagishi,Yuichi Hiroi,Takefumi Hiraki*

Main category: cs.RO

TL;DR: 开发基于机械臂的系统，通过模拟指尖滑动运动收集服装触觉数据，创建运动标记的多模态触觉数据库，提升服装触觉感知的表征能力。
...
Method: 使用机械臂系统对完整服装进行触觉数据采集，通过模拟指尖进行抚摸测量，精确控制速度和方向，创建运动标记的多模态触觉数据库。
...
### [436] [Image-based Morphological Characterization of Filamentous Biological Structures with Non-constant Curvature Shape Feature](https://arxiv.org/abs/2511.11639)
*Jie Fan,Francesco Visentin,Barbara Mazzolai,Emanuela Del Dottore*

Main category: cs.RO

TL;DR: 提出了一种基于图像的几何方法，使用3D分段回旋曲线模型来重建触须在机械刺激后的形状变化，揭示了触须顶端区域更高的响应性。


<details>
  <summary>Details</summary>
Motivation: 研究攀援植物触须的形态变化与机械刺激触发事件及接触位置之间的关系，这一关系目前仍难以提取和分析。

Method: 采用基于图像的几何方法，使用3D分段回旋曲线模型来重建触须在机械摩擦后的空间配置。

Result: 重建方法显示出高鲁棒性和可靠性，精度R2 > 0.99。分析发现触须顶端区域具有更高的响应性，可能对应该区域更高的敏感性和组织灵活性。
...
### [437] [Large Language Models and 3D Vision for Intelligent Robotic Perception and Autonomy: A Review](https://arxiv.org/abs/2511.11777)
*Vinit Mehta,Charu Sharma,Karthick Thiyagarajan*

Main category: cs.RO

TL;DR: 这篇综述论文分析了大型语言模型与3D视觉融合在机器人感知技术中的应用，涵盖方法、挑战和未来方向。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能和机器人技术的快速发展，LLMs与3D视觉的融合成为提升机器人感知能力的重要途径，旨在弥合语言智能与空间感知之间的差距。

Method: 介绍了LLMs和3D数据表示的基础原理，深入分析了机器人3D感知技术，探讨了场景理解、文本到3D生成、物体定位和具身智能体等关键进展。

Result: 综述了零样本3D分割、动态场景合成和语言引导操作等前沿技术，以及整合触觉、听觉和热输入的多模态LLMs，增强了环境理解和机器人决策能力。

Conclusion: 识别了自适应模型架构、增强跨模态对齐和实时处理能力等关键挑战和未来研究方向，为更智能、上下文感知和自主的机器人感知系统铺平道路。
...
### [438] [PIGEON: VLM-Driven Object Navigation via Points of Interest Selection](https://arxiv.org/abs/2511.13207)
...
TL;DR: PIGEON是一种基于兴趣点引导的物体导航方法，使用视觉语言模型选择兴趣点并提高决策频率，在零样本迁移中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前方法在决策频率与智能性之间难以平衡，导致决策缺乏前瞻性或动作不连续。

Method: 使用PIGEON-VL视觉语言模型选择探索过程中形成的兴趣点，然后由低级规划器输出动作，提高决策频率。基于兴趣点的决策还能生成适用于模拟器的强化学习验证奖励数据。

Result: 在经典物体导航基准测试中，零样本迁移方法达到最先进性能，RLVR进一步增强了模型的语义引导能力。

Conclusion: PIGEON方法实现了实时导航中的深度推理，平衡了决策频率与智能性。
...
### [439] [Evolving Prompts for Toxicity Search in Large Language Models](https://arxiv.org/abs/2511.12487)
*Onkar Shelar,Travis Desell*

Main category: cs.NE

TL;DR: ToxSearch是一个黑盒进化框架，通过进化提示词来测试LLM的安全性，发现词汇替换是最有效的攻击方法，并观察到跨模型毒性转移现象


<details>
  <summary>Details</summary>
Motivation: 大型语言模型即使在安全对齐后仍然容易受到对抗性提示的攻击，需要系统性的红队测试方法来评估模型安全性

Method: 使用同步稳态循环进化提示词，包含词汇替换、否定、回译、改写和两种语义交叉算子，通过审核oracle提供适应度指导

Result: 词汇替换在产出和方差之间达到最佳平衡，语义相似性交叉作为精确的低吞吐量插入器，全局改写方差高但拒绝成本也高。跨模型毒性转移存在但有所衰减

Conclusion: 小的可控扰动是系统性红队测试的有效工具，防御措施应预期对抗性提示的跨模型重用，而不仅仅是单模型加固
...
### [440] [A Content-Preserving Secure Linguistic Steganography](https://arxiv.org/abs/2511.12565)
*Lingyun Xiang,Chengfu Ou,Xu He,Zhongliang Yang,Yuling Liu*

Main category: cs.CR

TL;DR: 提出了一种内容保留的语言隐写方法CLstega，通过可控分布变换嵌入秘密信息而不修改原始文本，实现完美安全的隐蔽通信。


<details>
  <summary>Details</summary>
Motivation: 现有语言隐写方法通过内容变换隐藏秘密信息，但会在正常文本和隐写文本之间产生微妙差异，存在安全风险。

Method: 使用增强掩码策略定位嵌入位置，设计动态分布隐写编码策略，通过微调MLM模型实现分布变换，直接提取秘密信息。

Result: 实验显示CLstega能实现100%提取成功率，在安全性上优于现有方法，有效平衡嵌入容量和安全性。

Conclusion: CLstega方法确保了秘密信息的完美安全性，同时完全保留了原始文本的完整性，为隐蔽通信提供了新范式。
...
### [441] [AutoMalDesc: Large-Scale Script Analysis for Cyber Threat Research](https://arxiv.org/abs/2511.13333)
...
TL;DR: AutoMalDesc是一个自动化的静态分析总结框架，通过自定步调学习流水线生成恶意软件检测的自然语言解释，无需大量人工标注即可扩展运行。


<details>
  <summary>Details</summary>
Motivation: 尽管恶意软件检测系统已有显著进展，但为威胁检测生成全面的自然语言解释仍是网络安全研究中的开放问题。

Method: 利用迭代的自定步调学习流水线，通过合成数据生成和验证循环逐步提高输出质量，无需大量手动数据标注。

Result: 在5种脚本语言的3,600个多样化样本上评估显示，迭代间有统计显著改进，总结质量和分类准确性均有持续提升。

Conclusion: 结合定量指标和人工专家与LLM评估的综合验证方法证实了生成总结的技术精确性和语言连贯性，发布了完整数据集以促进该领域研究。
...
### [442] [ForgeDAN: An Evolutionary Framework for Jailbreaking Aligned Large Language Models](https://arxiv.org/abs/2511.13548)
*Siyang Cheng,Gaotian Liu,Rui Mei,Yilin Wang,Kejia Zhang,Kaishuo Wei,Yuqi Yu,Weiping Wen,Xiaojie Wu,Junhua Liu*

Main category: cs.CR

TL;DR: ForgeDAN是一个新颖的进化框架，通过多策略文本扰动、可解释语义适应度评估和双维度越狱判断，有效生成对抗性提示来攻击对齐的大型语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有自动越狱生成方法（如AutoDAN）存在突变多样性有限、适应度评估浅层和基于关键字的检测脆弱等问题，需要更有效的解决方案。

Method: 1. 在字符、单词和句子级别引入多策略文本扰动以增强攻击多样性；2. 使用基于文本相似度模型的可解释语义适应度评估来指导进化过程；3. 集成双维度越狱判断，利用基于LLM的分类器联合评估模型合规性和输出危害性。

Result: 评估显示ForgeDAN在保持自然性和隐蔽性的同时实现了高越狱成功率，优于现有最先进解决方案。

Conclusion: ForgeDAN通过增强的突变多样性、语义导向的进化过程和更准确的检测机制，有效解决了现有自动越狱方法的局限性。
...
### [443] [BackWeak: Backdooring Knowledge Distillation Simply with Weak Triggers and Fine-tuning](https://arxiv.org/abs/2511.12046)
*Shanmin Wang,Dongdong Zhao*

Main category: cs.CR

TL;DR: BackWeak是一种简单有效的知识蒸馏后门攻击方法，通过使用弱触发器微调良性教师模型，无需代理学生模型即可实现高攻击成功率


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏后门攻击方法复杂且计算量大，使用代理学生模型和类似通用对抗扰动的触发器，缺乏隐蔽性且具有强对抗性

Method: 提出BackWeak攻击范式，仅使用弱触发器以极小学习率微调良性教师模型，无需代理学生模型，在受害者标准蒸馏过程中可靠地将后门转移到不同学生架构

Result: 在多个数据集、模型架构和知识蒸馏方法上的广泛实验表明，BackWeak比之前复杂方法更高效、简单且通常更隐蔽

Conclusion: 该工作呼吁研究者在研究知识蒸馏后门攻击时特别关注触发器的隐蔽性及其潜在的对抗特性
...
### [444] [AttackVLA: Benchmarking Adversarial and Backdoor Attacks on Vision-Language-Action Models](https://arxiv.org/abs/2511.12149)
*Jiayu Li,Yunhan Zhao,Xiang Zheng,Zonghuan Xu,Yige Li,Xingjun Ma,Yu-Gang Jiang*

Main category: cs.CR

TL;DR: 提出了AttackVLA统一框架来评估VLA模型的安全漏洞，并开发了BackdoorVLA后门攻击方法，能够在触发条件出现时强制VLA执行攻击者指定的长时序动作序列。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在集成感知、语言和控制时引入新的安全漏洞，但缺乏统一的评估框架，且现有攻击方法大多未在真实场景验证，无法进行公平比较。

Method: 构建AttackVLA统一框架，涵盖数据构建、模型训练和推理全流程，实现多种攻击方法，并专门提出BackdoorVLA后门攻击来执行目标长时序动作。

Result: 在仿真和真实机器人场景中评估，BackdoorVLA平均目标成功率58.4%，在特定任务上可达100%，显著优于现有攻击方法。

Conclusion: 提供了标准化VLA漏洞评估框架，证明了精确对抗操纵的可能性，推动VLA系统安全研究发展。
...
### [445] [SafeGRPO: Self-Rewarded Multimodal Safety Alignment via Rule-Governed Policy Optimization](https://arxiv.org/abs/2511.12982)
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在文本-图像交互中引入了新的组合安全风险，即使单个输入是良性的，跨模态耦合也可能产生不安全语义，暴露当前MLLMs脆弱的安全意识。

Method: 提出SafeGRPO框架，在构建的SafeTag-VL-3K数据集基础上，集成规则治理的奖励构建到GRPO中，执行步骤引导的安全思考来强制执行结构化推理和行为对齐。

Result: 显著提高了多模态安全意识、组合鲁棒性和推理稳定性，在多样化基准测试中表现出色，且不牺牲通用能力。

Conclusion: SafeGRPO通过自奖励的安全对齐框架，有效解决了多模态模型中的组合安全风险，提供了可解释和可验证的推理安全优化方案。
...
### [446] [Scalable Vision-Guided Crop Yield Estimation](https://arxiv.org/abs/2511.12999)
*Harrison H. Li,Medhanie Irgau,Nabil Janmohamed,Karen Solveig Rieckmann,David B. Lobell*

Main category: stat.AP

TL;DR: 提出基于预测驱动推理的方法，结合田间照片补充传统作物收割测量，提高作物产量估计精度和不确定性量化


<details>
  <summary>Details</summary>
Motivation: 传统作物收割测量方法耗时且成本高，需要更高效的产量估计方法支持农业监测和决策

Method: 训练计算机视觉模型从照片预测产量，学习控制函数重新校准预测，结合空间坐标信息，使用近20,000个真实作物收割数据和照片训练模型

Result: 点估计在渐近无偏的同时不增加方差，在仅有20个田地的区域也能显著改进基线，水稻有效样本量增加73%，玉米增加12-23%

Conclusion: 低成本图像有潜力使基于区域的作物保险更经济实惠，从而促进可持续农业实践投资
...
### [447] [Bregman geometry-aware split Gibbs sampling for Bayesian Poisson inverse problems](https://arxiv.org/abs/2511.12257)
*Elhadji Cisse Faye,Mame Diarra Fall,Nicolas Dobigeon,Eric Barat*

Main category: stat.CO

TL;DR: 提出了一种用于泊松逆问题的贝叶斯框架，通过蒙特卡洛采样算法处理非欧几何结构，利用Bregman散度进行数据增强，结合Gibbs采样和Hessian黎曼朗之万蒙特卡洛算法，在去噪、去模糊和PET重建中表现出色。


<details>
  <summary>Details</summary>
Motivation: 泊松逆问题面临非Lipschitz梯度和正性约束等挑战，传统方法难以有效处理这些几何结构问题。

Method: 使用基于Burg熵的Bregman散度进行数据增强，构建增强后验分布，采用Gibbs采样和Hessian黎曼朗之万蒙特卡洛算法进行高效采样。

Result: 在去噪、去模糊和PET重建实验中，该方法在重建质量方面与基于优化和采样的方法相比具有竞争力。

Conclusion: 所提出的贝叶斯框架能够有效处理泊松逆问题的几何结构，通过结合数据增强和几何感知采样算法实现了高质量重建。
...
### [448] [Decoupling Positional and Symbolic Attention Behavior in Transformers](https://arxiv.org/abs/2511.11579)
*Felipe Urrutia,Jorge Salas,Alexander Kozachinskiy,Cristian Buc Calderon,Hector Pasten,Cristobal Rojas*

Main category: cs.LG

TL;DR: 本文深入分析了RoPE位置编码在Transformer中的位置信息与符号信息编码机制，证明了注意力头的位置行为和符号行为是互斥的，并开发了量化这些行为的指标。研究发现注意力头的行为与频率使用密切相关，通过控制频率访问可以控制模型性能。


<details>
  <summary>Details</summary>
Motivation: 理解RoPE位置编码成功的原因，特别是其如何分别使用大频率和小频率来编码鲁棒的位置信息和语义信息，以及探索注意力头在位置与符号行为上的二分法。

Method: 从理论和实证层面分析注意力头的位置与符号行为，提供这两种行为的通用定义并证明其互斥性，开发量化指标，在基于RoPE的Transformer LLMs上应用分析框架，并设计纯位置和纯符号的规范任务。

Result: 发现所有注意力头的行为与频率使用存在强相关性，通过控制注意力头可访问的频率可以因果性地控制Transformer在规范任务上的性能表现。

Conclusion: 本研究提供了对RoPE位置编码的详细理解，阐明了其特性与模型行为之间的关系，证明了频率控制在调节模型性能中的因果作用。
...
### [449] [The Anatomy of a Triton Attention Kernel](https://arxiv.org/abs/2511.11581)
*Burkhard Ringlein,Jan van Lunteren,Radu Stoica,Thomas Parnell*

Main category: cs.LG

TL;DR: 开发了一个基于Triton的跨平台LLM推理系统，通过优化的分页注意力内核实现了在NVIDIA和AMD GPU上的最佳性能，将通用Triton注意力内核性能从19.7%提升到105.9%。


<details>
  <summary>Details</summary>
Motivation: 长期以来业界和学术界都希望开发一个可移植到不同硬件架构的LLM推理平台，无需底层手动调优，同时保持最佳效率。

Method: 使用Triton领域特定即时编译语言开发了最先进的分页注意力内核，结合高层方法、关键算法和系统级改进、参数自动调优，并集成到流行的推理服务器中。

Result: 实现了在NVIDIA和AMD GPU上的最佳性能，将通用Triton注意力内核性能从19.7%提升到105.9%，超越了现有最佳方案。

Conclusion: 开源领域特定语言可以被用来解锁模型在不同GPU厂商间的可移植性，证明了可移植、高效的跨平台LLM推理是可行的。
...
### [450] [Small Vocabularies, Big Gains: Pretraining and Tokenization in Time Series Models](https://arxiv.org/abs/2511.11622)
...
Method: 通过经验训练实验和理论分析，系统评估不同分词器配置（缩放和量化策略）与预训练/随机初始化的组合效果。

Result: 预训练模型能更有效地利用设计良好的分词器，特别是在小词汇量情况下；而错位的分词会削弱甚至逆转预训练的好处。

Conclusion: 在时间序列建模中，精心设计的分词器至关重要，特别是在多模态预测场景中，将小型高效词汇表与预训练权重结合特别有利。
...
### [451] [H-Model: Dynamic Neural Architectures for Adaptive Processing](https://arxiv.org/abs/2511.11669)
*Dmytro Hospodarchuk*

Main category: cs.LG

TL;DR: 提出了一种能够根据输入数据动态调整内部结构的神经网络架构，通过路由机制实现迭代和自适应计算。


<details>
  <summary>Details</summary>
Motivation: 探索可适应且可能更可解释的网络架构，不追求与最先进语言模型竞争性能，而是提出能够学习计算结构本身的概念原型。

Method: 引入路由机制，使每一层能够影响其输出在网络中的传播方式，实现基于数据和内部状态的条件化信息流。

Result: 由于计算资源和数据限制，这是一个初步研究，但初步观察显示有潜力，需要在更有利的计算条件下进一步评估。

Conclusion: 提出了一个概念性架构框架，为探索适应性网络开辟了新方向，未来在更好计算条件下有望发挥其全部潜力。
...
### [452] [Reasoning: From Reflection to Solution](https://arxiv.org/abs/2511.11712)
*Zixi Li*
...
<details>
  <summary>Details</summary>
Motivation: 在LLMs在推理基准上取得超人表现的时代，需要区分系统是真正学会了推理，还是仅仅在推理轨迹上进行模式匹配。

Method: 提出推理的理论定义，开发OpenOperator理论框架，并构建OpenLM架构实现迭代算子应用。

Result: 在OpenXOR问题上达到76%准确率，而现有最先进LLMs为0%。

Conclusion: 推理需要特定的架构支持，而不仅仅是模式匹配，OpenLM证明了这一理论框架的有效性。
...
### [453] [Better LLM Reasoning via Dual-Play](https://arxiv.org/abs/2511.11881)
*Zhengxin Zhang,Chengyu Huang,Aochong Oliver Li,Claire Cardie*

Main category: cs.LG

TL;DR: PasoDoble是一个无需监督的LLM对抗训练框架，通过提案者和解答者两个角色的相互对抗来提升模型推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM训练严重依赖外部监督，而对抗学习特别是双角色自博弈可以减少对外部监督的依赖，但存在奖励破解和训练不稳定问题。

Method: 从同一基础模型初始化提案者和解答者，提案者生成有挑战性的问题，解答者尝试解决；通过知识增强确保问题质量，采用联合更新避免奖励破解，并提供离线训练模式增强稳定性。

Result: 实验结果显示PasoDoble能够有效提升LLMs的推理性能。

Conclusion: PasoDoble证明了无监督对抗训练在提升LLM推理能力方面的有效性，为减少对外部监督的依赖提供了可行方案。
...
### [454] [Computational Measurement of Political Positions: A Review of Text-Based Ideal Point Estimation Algorithms](https://arxiv.org/abs/2511.13238)
*Patrick Parschan,Charlott Jakob*

Main category: cs.LG

TL;DR: 本文首次系统综述了无监督和半监督的基于文本的理想点估计算法(CT-IPE)，这些方法旨在从文本数据推断潜在政治立场。


<details>
  <summary>Details</summary>
Motivation: 过去20年CT-IPE算法的发展紧跟NLP趋势，从词频模型发展到大型语言模型，但导致领域碎片化，缺乏系统比较和应用指导。

Method: 通过系统文献回顾识别25种CT-IPE算法，进行手动内容分析，并引入区分文本方差生成、捕获和聚合的概念框架。

Result: 识别出四种方法家族：词频、主题建模、词嵌入和LLM方法，评估了它们的假设、可解释性、可扩展性和局限性。

Conclusion: 提供了算法发展的结构化综合，为应用研究者提供实用指导，强调算法间估计差异本身具有信息价值，需要系统基准测试。
...
### [455] [P1: Mastering Physics Olympiads with Reinforcement Learning](https://arxiv.org/abs/2511.13612)
...
TL;DR: P1系列开源物理推理模型通过强化学习训练，在物理奥林匹克竞赛中表现卓越，其中P1-235B-A22B获得IPhO 2025金牌，并在13个国际/区域物理竞赛中赢得12枚金牌。


<details>
  <summary>Details</summary>
Motivation: 推动大型语言模型从谜题解决向科学级推理发展，物理作为连接符号与现实的基础学科，是检验这一转变的最佳测试场。

Method: 完全通过强化学习训练开源物理推理模型家族P1，并配备智能代理框架PhysicsMinions。

Result: P1-235B-A22B在IPhO 2025获得金牌，在13个物理竞赛中赢得12枚金牌；P1-235B-A22B+PhysicsMinions在IPhO 2025总体排名第一；P1模型在数学和编程等其他推理任务上也表现出色。

Conclusion: P1系列模型展示了卓越的物理推理能力和泛化性能，为推进物理研究和科学级AI推理提供了有力工具。
...
### [456] [Learning with Preserving for Continual Multitask Learning](https://arxiv.org/abs/2511.11676)
*Hanchen David Wang,Siwoo Bae,Zirong Chen,Meiyi Ma*

Main category: cs.LG

TL;DR: 提出了LwP框架，通过保持共享表示空间的几何结构来解决持续多任务学习中的灾难性遗忘问题，无需重放缓冲区。


<details>
  <summary>Details</summary>
Motivation: 关键领域中的AI系统需要持续学习新任务而不遗忘旧能力，现有方法因学习碎片化的任务特定特征而失败。

Method: 引入动态加权距离保持损失，通过正则化潜在数据表示之间的成对距离来防止表示漂移。

Result: 在时间序列和图像基准测试中，LwP不仅缓解了灾难性遗忘，还持续优于最先进的基线方法。

Conclusion: LwP是唯一超越强单任务学习基线的方法，在现实动态环境中表现出优越的鲁棒性。
...
### [457] [A neural optimization framework for free-boundary diffeomorphic mapping problems and its applications](https://arxiv.org/abs/2511.11679)
*Zhehao Xu,Lok Ming Lui*

Main category: cs.LG

TL;DR: 提出了SBN-Opt框架，通过神经代理网络SBN嵌入LSQC能量，优化自由边界微分同胚映射，在密度均衡映射和不一致表面配准中表现优异


<details>
  <summary>Details</summary>
Motivation: 自由边界微分同胚优化在表面映射问题中至关重要，但传统数值算法需要地标条件且无法应用于基于梯度的优化

Method: 提出Spectral Beltrami Network (SBN)作为神经代理，将LSQC能量嵌入多尺度网格谱架构；然后提出SBN-Opt优化框架优化自由边界微分同胚

Result: 在密度均衡映射和不一致表面配准的广泛实验中，SBN-Opt优于传统数值算法

Conclusion: SBN-Opt框架能够有效优化自由边界微分同胚，同时显式控制局部几何畸变
...
### [458] [Probabilistic Wildfire Susceptibility from Remote Sensing Using Random Forests and SHAP](https://arxiv.org/abs/2511.11680)
*Udaya Bhasker Cheerala,Varun Teja Chirukuri,Venkata Akhil Kumar Gummadi,Jintu Moni Bhuyan,Praveen Damacharla*

Main category: cs.LG

TL;DR: 本研究使用随机森林算法结合SHAP可解释AI方法，开发了加利福尼亚州的全面野火风险地图，识别了不同生态系统中的关键风险驱动因素。


<details>
  <summary>Details</summary>
Motivation: 野火对全球生态系统构成重大威胁，加利福尼亚因气候、地形、植被和人类活动等因素经常发生火灾，需要开发全面的风险评估方法。

Method: 应用随机森林算法，结合SHAP可解释人工智能方法，采用空间和时间验证策略评估模型性能。

Result: RF模型表现出强大的预测性能，对草地和森林的AUC接近完美。SHAP分析识别出森林中土壤有机碳、树木覆盖和NDVI是关键驱动因素，而草地中地表温度、海拔和植被健康指数占主导地位。

Conclusion: RF-SHAP框架为评估野火风险提供了稳健、可理解和适应性强的方法，有助于制定有针对性的减灾策略。
...
### [459] [MPCM-Net: Multi-scale network integrates partial attention convolution with Mamba for ground-based cloud image segmentation](https://arxiv.org/abs/2511.11681)
*Penghui Niu,Jiashuai She,Taotao Cai,Yajuan Zhang,Ping Zhang,Junhua Gu,Jianxin Li*

Main category: cs.LG

TL;DR: 提出MPCM-Net网络，结合部分注意力卷积和Mamba架构，用于地面云图像分割，在CSRC数据集上实现精度和推理速度的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法存在三个主要局限：1)依赖空洞卷积提取多尺度上下文，缺乏部分特征有效性和通道间互操作性；2)基于注意力的特征增强实现忽视精度-吞吐量平衡；3)解码器修改未能建立层次局部特征间的全局依赖关系，限制推理效率。

Method: 提出MPCM-Net网络，编码器包含MPAC模块（MPC块和MPA块），分别实现多尺度云形成的全局空间交互和低计算复杂度的判别特征提取；解码器使用M2B模块，通过SSHD在保持线性复杂度的同时实现空间和尺度维度的深度特征聚合。

Result: 在CSRC数据集上的广泛实验表明，MPCM-Net优于现有最先进方法，实现了分割精度和推理速度的最佳平衡。

Conclusion: MPCM-Net通过集成部分注意力卷积和Mamba架构，有效解决了现有方法的局限性，同时贡献了CSRC数据集作为清晰标签的细粒度分割基准。
...
### [460] [Stratified Knowledge-Density Super-Network for Scalable Vision Transformers](https://arxiv.org/abs/2511.11683)
...
<details>
  <summary>Details</summary>
Motivation: 训练和部署多个ViT模型用于不同资源约束成本高且效率低，需要一种能够灵活提取不同大小子网络的解决方案。

Method: WPAC通过加权PCA压缩注意力层知识到关键权重；PIAD通过渐进重要性感知dropout促进知识分层组织。

Result: WPAC在知识集中方面优于现有剪枝标准，与PIAD结合为模型压缩和扩展提供了强大替代方案。

Conclusion: 该方法能够有效组织ViT中的知识分层，实现灵活的子网络提取，为多资源约束下的模型部署提供了高效解决方案。
...
### [461] [Hierarchical Schedule Optimization for Fast and Robust Diffusion Model Sampling](https://arxiv.org/abs/2511.11688)
*Aihua Zhu,Rui Su,Qinglin Zhao,Li Feng,Meng Shen,Shibo He*

Main category: cs.LG

TL;DR: HSO是一种新型的双层优化框架，通过分层优化策略在极低函数评估次数下显著加速扩散模型采样，无需重新训练即可实现高效采样。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型采样方法在低函数评估次数下难以同时满足有效性、自适应性、实用鲁棒性和计算效率四个核心原则，需要更先进的解决方案。

Method: 提出分层调度优化器(HSO)，采用双层优化框架：上层全局搜索最优初始化策略，下层局部优化调度细化，结合中点误差代理和间距惩罚适应度函数。

Result: 在极低NFE(如5次)下，HSO在LAION-Aesthetics数据集上达到11.94的FID分数，优化成本仅需不到8秒。

Conclusion: HSO为训练自由的扩散模型加速提供了高效实用的新范式，在极低函数评估次数下实现了最先进的采样性能。
...
### [462] [Doubly Debiased Test-Time Prompt Tuning for Vision-Language Models](https://arxiv.org/abs/2511.11690)
*Fei Song,Yi Li,Rui Wang,Jiahuan Zhou,Changwen Zheng,Jiangmeng Li*

Main category: cs.LG

TL;DR: 提出双重去偏测试时提示调优方法，通过动态检索增强调制和可靠性感知提示优化，缓解视觉语言模型在无标签测试数据上的提示优化偏差问题


<details>
  <summary>Details</summary>
Motivation: 现有测试时提示调优方法仅基于无标签测试数据调整可学习提示，可能引发提示优化偏差，导致下游任务性能下降
...
Conclusion: 所提出的双重去偏方法能有效缓解提示优化偏差，提升视觉语言模型的泛化性能
...
### [463] [AnchorDS: Anchoring Dynamic Sources for Semantically Consistent Text-to-3D Generation](https://arxiv.org/abs/2511.11692)
*Jiayin Zhu,Linlin Yang,Yicong Li,Angela Yao*

Main category: cs.LG

TL;DR: 本文提出AnchorDS方法，通过将文本到3D优化重新表述为动态源分布到固定目标分布的映射，解决了传统SDS方法中的语义过平滑问题。
...
Method: 将问题投射到双条件潜空间，引入AnchorDS机制提供状态锚定指导，设计轻量级过滤策略和微调策略来优化锚点。
...
### [464] [Toward Dignity-Aware AI: Next-Generation Elderly Monitoring from Fall Detection to ADL](https://arxiv.org/abs/2511.11696)
*Xun Shao,Aoba Otani,Yuto Hirasuka,Runji Cai,Seng W. Loke*

Main category: cs.LG

TL;DR: 本文提出下一代老年人监控系统，从跌倒检测转向更广泛的日常活动识别，旨在开发隐私保护、边缘部署和联邦学习的AI系统，支持老年人在智能家居环境中保持独立生活。


<details>
  <summary>Details</summary>
Motivation: 当前老年人监控系统主要关注跌倒检测，但需要向更全面的日常活动识别发展，以更好地支持老龄化社会的独立生活和尊严。

Method: 使用SISFall数据集及其GAN增强变体进行可行性验证，将跌倒检测作为代理任务，在非IID条件下进行联邦学习实验，并在Jetson Orin Nano设备上进行嵌入式部署。

Result: 获得了联邦学习在非IID条件下的初步结果，并成功在边缘设备上部署系统，证明了技术可行性。

Conclusion: 本文为从单任务检测向全面日常活动识别的过渡提供了早期证据和技术路线图，指出了领域偏移、数据稀缺和隐私风险等开放挑战，为可持续、以人为本的老年人护理AI指明了方向。
...
### [465] [Simple Vision-Language Math Reasoning via Rendered Text](https://arxiv.org/abs/2511.11704)
*Matvey Skripkin,Elizaveta Goncharova,Andrey Kuznetsov*

Main category: cs.LG

TL;DR: 提出了一种轻量级但有效的训练流程，通过将LaTeX公式渲染为图像并配合结构化思维链提示，使紧凑的多模态架构在数学问题上达到最先进的推理准确率。


<details>
  <summary>Details</summary>
Motivation: 解决数学问题的传统方法通常依赖复杂的模型架构，本文旨在探索通过简单的文本到视觉增强方法，使紧凑的多模态模型也能在数学推理任务上取得优异表现。

Method: 将LaTeX编码的数学公式渲染成图像，并与结构化的思维链提示配对，构建训练数据。通过系统消融实验分析渲染保真度和提示设计对性能的影响。

Result: 该方法在多个基准测试中一致匹配或超越了开源和专有的数学视觉语言求解器，同时在MMMU、ChartQA和DocVQA等任务上取得了高达20%的性能提升，同时保持了广泛的通用领域能力。

Conclusion: 尽管方法简单，但渲染保真度和提示设计是性能的主要驱动力，证明了通过适当的视觉增强，紧凑的多模态架构也能在数学推理任务上实现卓越性能。
...
### [466] [Multimodal ML: Quantifying the Improvement of Calorie Estimation Through Image-Text Pairs](https://arxiv.org/abs/2511.11705)
*Arya Narang*

Main category: cs.LG

TL;DR: 研究探讨了在菜品热量估算中，结合菜品名称的文本信息能否显著提升基于图像的估算准确度。


<details>
  <summary>Details</summary>
Motivation: 当前基于图像的热量估算模型存在精度不足的问题，希望通过引入菜品名称的文本信息来改进估算效果。

Method: 使用TensorFlow库和Nutrition5k数据集，分别训练仅使用图像的CNN模型和结合图像与文本的多模态CNN模型。

Result: 多模态模型将热量估算的平均绝对误差从84.76千卡降低到83.70千卡，减少了1.06千卡（1.25%的改进）。

Conclusion: 短文本输入（菜品名称）能够轻微但显著地改善热量估算的准确性。
...
### [467] [Context-Aware Multimodal Representation Learning for Spatio-Temporally Explicit Environmental modelling](https://arxiv.org/abs/2511.11706)
*Julia Peters,Karin Mora,Miguel D. Mahecha,Chaonan Ji,David Montero,Clemens Mosig,Guido Kraemer*

Main category: cs.LG

TL;DR: 提出了一个统一的多模态地球观测表示学习框架，能够在10米空间分辨率和Sentinel-2无云采集频率下整合不同传感器数据，解决现有模型固定时空尺度限制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有地球观测基础模型通常在固定时空尺度下运行，限制了需要精细空间细节和高时间保真度的生态分析应用。需要开发能够整合不同观测模态的统一特征空间。

Method: 采用两阶段设计：首先独立建模每个传感器以捕获其特定特征，然后将表示组合到共享模型中。使用Sentinel-1和Sentinel-2作为代表性模态，在10米分辨率和无云采集频率下构建潜在空间。

Result: 学习到的嵌入在异质景观中表现出高度的空间和语义一致性，在总初级生产力建模的定量评估中编码了生态学意义模式，并保持了足够的时间保真度以支持精细尺度分析。

Conclusion: 该框架为需要不同空间和时间分辨率的环境应用提供了灵活、分析就绪的表示学习方法，能够捕获互补的遥感数据并保持跨时空的一致性。
...
### [468] [Fast 3D Surrogate Modeling for Data Center Thermal Management](https://arxiv.org/abs/2511.11722)
...
TL;DR: 开发基于视觉的替代建模框架，直接在3D体素化数据中心表示上运行，实现实时温度预测，比传统CFD求解器快20,000倍，可节省7%能源。


<details>
  <summary>Details</summary>
Motivation: 减少数据中心能耗和碳排放，传统热CFD求解器计算昂贵且需要专家构建网格，不适用于实时应用。

Method: 使用3D CNN U-Net变体、3D傅里叶神经算子和3D视觉变换器等多种架构，将热输入映射到高保真热图。

Result: 替代模型在不同数据中心配置中泛化良好，实现高达20,000倍加速（数百毫秒vs数小时），准确估计热点和温度分布。

Conclusion: 快速准确的温度预测可实现实时冷却控制和负载重新分配，带来显著节能（7%）和碳足迹减少。
...
### [469] [Optimizing Input of Denoising Score Matching is Biased Towards Higher Score Norm](https://arxiv.org/abs/2511.11727)
*Tongda Xu*

Main category: cs.LG

TL;DR: 本文揭示了在扩散模型中优化条件输入时，去噪分数匹配与精确分数匹配之间的等价性被破坏，导致偏差和高分数范数问题。


<details>
  <summary>Details</summary>
Motivation: 许多近期工作使用去噪分数匹配来优化扩散模型的条件输入，但作者发现这种方法存在理论偏差。

Method: 通过理论分析和实验验证，展示了优化条件输入时去噪分数匹配与精确分数匹配不等价的问题。

Result: 发现这种偏差会导致更高的分数范数，并在数据分布优化时也观察到类似偏差。

Conclusion: 该偏差影响了多个领域的工作，包括自回归生成的MAR、图像压缩的PerCo和文本到3D生成的DreamFusion等。
...
### [470] [Improving a Hybrid Graphsage Deep Network for Automatic Multi-objective Logistics Management in Supply Chain](https://arxiv.org/abs/2511.11753)
*Mehdi Khaleghi,Nastaran Khaleghi,Sobhan Sheykhivand,Sebelan Danishvar*

Main category: cs.LG

TL;DR: 本文提出了一种混合GraphSAGE网络（H-GSN）用于供应链物流管理的多任务预测，包括货物类型、物流状态、交通状况、物流ID和物流延迟等目标，在多个数据集上取得了97.8%-100%的高准确率。


<details>
  <summary>Details</summary>
Motivation: 供应链物流系统、运输设施和仓储信息对供应链盈利发展至关重要。需要自动预测货物类型、物流延迟和交通状况的方法来提高供应链管理效率，增强供应链的韧性和可持续性。

Method: 提出混合GraphSAGE网络（H-GSN）用于供应链物流管理的多任务预测，利用三个不同的数据集（DataCo、Shipping和Smart Logistics）进行训练和评估。
...
### [471] [Coordinate Descent for Network Linearization](https://arxiv.org/abs/2511.11781)
*Vlad Rakhlin,Amir Jevnisek,Shai Avidan*

Main category: cs.LG

TL;DR: 本文提出了一种基于坐标下降的离散优化方法，直接在离散域中减少ReLU激活函数数量，以解决私有推理中的延迟瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: ReLU激活函数是基于ResNet网络的私有推理中的主要瓶颈，因为它们会导致显著的推理延迟。现有方法基于平滑近似，但在最后的硬阈值步骤中通常会出现较大的性能损失。
...
### [472] [Transformers vs. Recurrent Models for Estimating Forest Gross Primary Production](https://arxiv.org/abs/2511.11880)
...
TL;DR: 该研究比较了GPT-2和LSTM两种深度学习模型在预测森林CO2吸收（GPP）方面的表现，发现LSTM总体精度更高，但GPT-2在极端事件中表现更好，揭示了模型架构、上下文长度和多模态输入对GPP预测性能的共同影响。


<details>
  <summary>Details</summary>
Motivation: 监测森林CO2吸收的时空动态是生态系统研究的关键挑战。传统EC塔空间覆盖有限，遥感方法大多依赖单传感器和统计模型，难以捕捉GPP的复杂时间动态。深度学习和数据融合为改进植被过程时间动态表征提供了新机会。

Method: 使用两种代表性深度学习模型：1）GPT-2（transformer架构）和2）LSTM（循环神经网络），采用多元输入进行GPP预测，并分析时间上下文长度和特征重要性。

Result: 两种模型达到相似精度，但LSTM总体表现更好，GPT-2在极端事件中表现更优。LSTM使用更短的输入窗口就能达到类似精度，揭示了准确性-效率权衡。辐射是最重要的预测因子，其次是Sentinel-2、MODIS地表温度和Sentinel-1。

Conclusion: 模型架构、上下文长度和多模态输入共同决定了GPP预测性能，为未来开发监测陆地碳动态的深度学习框架提供了指导。
...
### [473] [A Systematic Analysis of Out-of-Distribution Detection Under Representation and Training Paradigm Shifts](https://arxiv.org/abs/2511.11934)
*C. César Claros Olivares,Austin J. Brockmeier*

Main category: cs.LG

TL;DR: 系统比较了不同OOD检测方法在CLIP分层机制下的表现，发现特征空间决定OOD检测效果，概率分数在ID检测中占优，几何感知分数在强分布偏移下表现更好。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏对OOD检测方法在CLIP分层机制下的系统性比较，需要为不同分布偏移下的方法选择提供统计依据。

Method: 使用AURC和AUGRC作为主要指标，比较CNN和ViT两种表示范式，采用多重比较控制的基于排名的统计流程。

Result: 特征空间决定OOD检测效果，概率分数主导ID检测，几何感知分数在强偏移下表现更好，ViT中GradNorm和KPCA重构误差表现稳定。

Conclusion: 支持基于表示的OOD检测观点，为分布偏移下的方法选择提供统计指导，PCA投影可提升多个检测器性能。
...
### [474] [Selecting Fine-Tuning Examples by Quizzing VLMs](https://arxiv.org/abs/2511.12002)
...
<details>
  <summary>Details</summary>
Motivation: 在微调文本到图像扩散模型时，选择高质量训练图像至关重要。使用质量参差不齐的图像集（如维基共享资源）往往会产生糟糕的输出结果

Method: 提出QZLoRA框架，利用QuizRank方法自动对图像进行排序，将图像视为'教育干预'并通过视觉语言模型进行'测验'来选择最佳训练样本

Result: QZLoRA能够用更少样本生成更好对齐、更逼真的图像，并且这些微调后的模型也能生成具有代表性的风格化图像（如插画）

Conclusion: 将自动化视觉推理与参数高效微调相结合，为主题自适应生成建模展现了良好前景
...
### [475] [Variation-Bounded Loss for Noise-Tolerant Learning](https://arxiv.org/abs/2511.12143)
*Jialiang Wang,Xiong Zhou,Xianming Liu,Gangfeng Hu,Deming Zhai,Junjun Jiang,Haoliang Li*

Main category: cs.LG

TL;DR: 提出一种新的鲁棒损失函数属性——变异比，并基于此构建了变异有界损失函数家族，通过理论分析和实验验证了其在噪声标签下的有效性。


<details>
  <summary>Details</summary>
Motivation: 缓解噪声标签对监督学习的负面影响是一个长期问题，鲁棒损失函数是解决该问题的普遍方法。

Method: 引入变异比作为损失函数鲁棒性的新属性，提出变异有界损失函数家族，对常用损失函数进行变异有界重构。

Result: 在各种数据集上的实验验证了该方法的有效性和灵活性。

Conclusion: 变异比提供了放松对称条件的可行方法，为达到非对称条件提供了更简洁的路径。
...
### [476] [Calibrated Adversarial Sampling: Multi-Armed Bandit-Guided Generalization Against Unforeseen Attacks](https://arxiv.org/abs/2511.12265)
*Rui Wang,Zeming Wei,Xiyue Zhang,Meng Sun*

Main category: cs.LG

TL;DR: 提出了一种名为校准对抗采样（CAS）的高效微调方法，通过多臂老虎机框架动态设计奖励并平衡探索与利用，以解决现有对抗训练方法仅针对有限攻击类型的问题。


<details>
  <summary>Details</summary>
Motivation: 现有对抗训练框架主要关注单一或有限攻击类型，导致DNN在实际中仍面临未在训练中处理的其他攻击类型的威胁。

Method: 从多臂老虎机优化角度，动态设计奖励并平衡探索与利用，考虑多个鲁棒性维度的动态和相互依赖特性。

Result: 在基准数据集上的实验表明，CAS在保持高清洁精度的同时实现了优越的整体鲁棒性。
...
### [477] [BSO: Binary Spiking Online Optimization Algorithm](https://arxiv.org/abs/2511.12502)
*Yu Liang,Yu Yang,Wenjie Wei,Ammar Belatreche,Shuai Wang,Malu Zhang,Yang Yang*

Main category: cs.LG

TL;DR: 提出BSO和T-BSO两种在线训练算法，显著降低二元脉冲神经网络训练内存需求，通过翻转信号直接更新权重，无需存储潜在权重。


<details>
  <summary>Details</summary>
Motivation: 二元脉冲神经网络具有效率优势，但传统训练算法需要大量内存存储潜在权重和处理时序数据。

Method: BSO通过翻转信号在线更新权重，当梯度动量与权重的乘积超过阈值时触发翻转。T-BSO利用时序动态特性，跨时间步捕获梯度信息进行自适应阈值调整。

Result: 实验证明BSO和T-BSO相比现有方法获得更优的优化性能，理论分析建立了收敛保证和遗憾边界。

Conclusion: BSO和T-BSO是高效的二元脉冲神经网络在线训练算法，显著降低内存需求并保持良好性能。
...
### [478] [Linear time small coresets for k-mean clustering of segments with applications](https://arxiv.org/abs/2511.12564)
*David Denisov,Shlomi Dolev,Dan Felmdan,Michael Segal*

Main category: cs.LG

TL;DR: 本文提出了首个能够处理任意输入线段的k-means聚类核心集构造方法，在常数k和ε下生成大小为O(log²n)的核心集，计算时间为O(nd)，实验验证了方法的实用性和理论保证。


<details>
  <summary>Details</summary>
Motivation: 研究线段集合的k-means聚类问题，旨在为实时视频跟踪等应用开发高效的近似算法，需要处理任意输入线段并支持流式、分布式或并行计算。

Method: 构建ε-核心集，这是一个加权子集，能够在任意k个中心集下以1±ε因子近似原始线段集合的聚类目标函数，核心集大小为O(log²n)。

Result: 提出的核心集构造方法在常数k和ε下具有理论保证，实验显示在视频跟踪等应用中实现了显著加速，且聚类精度损失最小。

Conclusion: 该方法首次实现了对任意输入线段的高效核心集构造，在理论和实践层面均表现出色，为线段聚类问题提供了有效的解决方案。
...
### [479] [Functional Mean Flow in Hilbert Space](https://arxiv.org/abs/2511.12898)
*Zhiqi Li,Yuchen Sun,Greg Turk,Bo Zhu*

Main category: cs.LG

TL;DR: FMF是一种在无限维希尔伯特空间中定义的一步生成模型，将Mean Flow框架扩展到函数域，提供理论公式和实际实现。


<details>
  <summary>Details</summary>
Motivation: 将一步Mean Flow框架扩展到函数域，为函数数据生成任务提供高效的一步流匹配方法。

Method: 提出函数流匹配的理论公式和实际实现，引入x1预测变体以提高稳定性。

Result: 开发出实用的函数流匹配框架，适用于时间序列、图像、PDE和3D几何等多种函数数据生成任务。

Conclusion: FMF是一个通用且高效的一步函数流匹配方法，在多个函数数据生成领域具有应用潜力。
...
### [480] [Angular Gradient Sign Method: Uncovering Vulnerabilities in Hyperbolic Networks](https://arxiv.org/abs/2511.12985)
*Minsoo Jo,Dongyoon Yang,Taesup Kim*

Main category: cs.LG

TL;DR: 提出了一种针对双曲网络的几何感知对抗攻击方法，通过在双曲空间的切空间中分解梯度，仅使用角度方向生成对抗样本


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法如FGSM和PGD未考虑双曲空间的几何结构，可能导致低效或几何不一致的攻击，需要重新评估非欧几何中的攻击策略

Method: 在双曲空间的切空间中计算损失函数梯度，将其分解为径向（深度）和角度（语义）分量，仅使用角度方向分量生成扰动

Result: 在图像分类、跨模态检索任务和网络架构上的实验表明，该方法比传统对抗攻击获得更高的欺骗率，同时产生具有高影响力的扰动

Conclusion: 这项工作强调了在弯曲表示空间中几何感知对抗策略的重要性，为攻击分层嵌入提供了原则性框架
...
### [481] [Real-time prediction of breast cancer sites using deformation-aware graph neural network](https://arxiv.org/abs/2511.13082)
*Kyunghyun Lee,Yong-Min Shin,Minwoo Shin,Jihun Kim,Sunghwan Lim,Won-Yong Shin,Kyungho Yoon*

Main category: cs.LG

TL;DR: 开发基于图神经网络的实时变形预测模型，用于乳腺癌活检中准确预测肿瘤位置变化，解决间接MRI引导活检中实时变形建模的挑战。


<details>
  <summary>Details</summary>
Motivation: 直接MRI引导活检时间长、成本高，间接MRI引导活检面临实时变形建模不准确的挑战，需要开发能够实时预测乳腺变形和肿瘤位移的方法。

Method: 结合MRI图像结构信息构建个体特异性有限元模型模拟变形行为，使用图神经网络处理表面位移和距离图数据，预测整体组织位移包括肿瘤区域变形。

Result: 在模型和真实患者数据集上验证，肿瘤节点位移RMSE小于0.2毫米，与实际癌变区域空间重叠DSC达0.977，计算速度比传统有限元模拟快4000倍以上。

Conclusion: 提出的变形感知图神经网络模型为乳腺癌活检中的实时肿瘤位移预测提供了高精度和实时能力的解决方案，有望显著提升乳腺癌诊断的精确性和效率。
...
### [482] [Uncovering and Mitigating Transient Blindness in Multimodal Model Editing](https://arxiv.org/abs/2511.13243)
*Xiaoqi Han,Ru Li,Ran Yi,Hongye Tan,Zhuomin Liang,Víctor Gutiérrez-Basulto,Jeff Z. Pan*

Main category: cs.LG

TL;DR: 提出多模态模型编辑的全面局部性评估框架，解决现有方法因依赖低相似度或随机输入而高估成功率的问题，并引入De-VQA动态评估揭示瞬时盲视现象。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本模型编辑的多模态评估方法存在缺陷，通过低相似度或随机输入高估编辑成功率，掩盖了过拟合问题。

Method: 提出包含三个关键维度的局部性评估框架（随机图像、无图像、一致图像局部性），通过七种数据类型进行结构化分析；引入De-VQA动态评估；提出局部性感知对抗损失来平衡跨模态表示。

Result: 方法在减少瞬时盲视和改善局部性方面平均提升17%，持续优于现有基线。

Conclusion: 提出的评估框架和方法有效解决了多模态模型编辑中的过拟合问题，显著提升了编辑质量和局部性。
...
### [483] [Tuning for Two Adversaries: Enhancing the Robustness Against Transfer and Query-Based Attacks using Hyperparameter Tuning](https://arxiv.org/abs/2511.13654)
*Pascal Zimmer,Ghassan Karame*

Main category: cs.LG

TL;DR: 本文首次系统分析了优化超参数对对抗攻击鲁棒性的影响，发现学习率对迁移攻击和查询攻击的鲁棒性存在相反作用，并探索了联合增强两种鲁棒性的超参数设计空间。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏对优化超参数如何影响对抗攻击鲁棒性的系统研究，特别是在不同训练设置下对迁移攻击和查询攻击的差异化影响。

Method: 通过理论分析和实验验证，研究了学习率、权重衰减、动量和批大小等超参数在集中训练、集成学习和分布式训练等实际部署场景中的影响。

Result: 发现学习率对两种攻击的鲁棒性存在显著差异：降低学习率可提升迁移攻击鲁棒性达64%，而提高学习率可提升查询攻击鲁棒性达28%。分布式模型通过超参数调优能同时有效缓解两种攻击。

Conclusion: 优化超参数是增强对抗鲁棒性的重要手段，分布式训练模型通过适当的超参数调优能实现最佳的鲁棒性权衡，为实际部署提供了实用指导。
