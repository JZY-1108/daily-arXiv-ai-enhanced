...
Motivation: 评估思维链提示在多语言环境中的忠实性和可解释性，解决现有研究对生成推理链可信度的担忧。

Method: 使用ContextCite进行步骤级归因和Inseq进行令牌级归因，在Qwen2.5 1.5B-Instruct模型上应用MGSM基准测试，并通过否定和干扰句进行受控扰动。

Result: 归因分数过度强调最终推理步骤（尤其在错误生成中），结构化CoT提示仅显著提高高资源拉丁语系语言的准确性，扰动会降低模型准确性和归因一致性。
...
Result: 当前AI系统在非语言线索解读方面表现显著不足，在检测任务上存在明显性能差距，在解释任务中表现出过度解读模式。

Conclusion: 机器在理解非语言线索方面仍面临重大挑战，需要进一步发展非语言交流理解能力。
...
TL;DR: 提出了TOD-ProcBench基准，用于系统评估LLMs在面向任务对话中理解和遵循复杂指令的能力，包含三个任务来全面测试指令遵循能力。
...
Method: 基于高质量ABCD数据集构建包含复杂过程指令的对话数据集，将细粒度约束和行动程序制定为多级条件-行动指令语句，设计三个任务评估LLMs的指令遵循能力。

Result: 构建了包含复杂约束的基准数据集，设计了系统评估框架，可用于全面测试LLMs在多种场景下的指令遵循表现。

Conclusion: TOD-ProcBench填补了现有基准的空白，为评估LLMs在复杂指令遵循能力方面提供了系统化的测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 现有谎言检测技术通常在狭窄环境中验证，无法捕捉大语言模型可能生成的各种谎言类型。

Method: 构建包含不同谎言类型的测试平台，按模型撒谎原因和谎言目标两个维度分类，评估三种黑白盒谎言检测技术。

Result: 现有技术系统性无法识别某些类型的谎言，特别是在仅凭对话记录无法确定模型是否撒谎的情况下。
...
TL;DR: LTLA是一种混合方法，将基础语言模型与可处理的代理模型结合，通过单次批处理HMM更新同时考虑所有候选token，在保持固定代理解码器的同时仅调节代理的潜在状态先验，从而高效处理序列级约束的受控文本生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用可处理的代理模型（如HMM）来近似约束条件下的文本生成，但这些代理模型通常上下文感知能力弱，影响生成质量。需要一种既能利用语言模型的丰富上下文编码，又能高效处理未来token依赖约束的方法。

Method: LTLA将基础语言模型用于丰富前缀编码，与固定可处理代理模型配对计算精确的延续概率。通过单次批处理HMM更新处理所有候选token，仅调节代理的潜在状态先验于LM的隐藏表示，保持代理解码器固定以实现计算复用。

Result: LTLA获得了比无条件HMM更高的条件似然，能够近似视觉语言模型的延续分布（单独HMM无法编码视觉上下文），在受控生成任务中在保持相当流畅度的同时提高了约束满足度，且推理开销最小。

Conclusion: LTLA通过有效结合语言模型的上下文编码能力和代理模型的精确概率计算，解决了受控文本生成中未来token依赖约束的挑战，在保持高效的同时提升了生成质量。
...
TL;DR: GPT-5在多个科学领域（数学、物理、天文学、计算机科学、生物学、材料科学）中帮助研究人员取得新进展，包括四个经过验证的数学新结果，展示了AI与人类合作加速科研的潜力。


<details>
  <summary>Details</summary>
Motivation: 前沿AI模型如GPT-5对科学家越来越有价值，但许多研究者仍不了解其能力。本文旨在展示GPT-5如何在实际科研中产生具体成果。

Method: 通过多个短案例研究，记录研究人员与GPT-5的互动过程，展示AI如何加速研究，以及在哪些方面仍需人类输入。

Result: GPT-5在多个科学领域产生了新的研究进展，特别是在数学领域贡献了四个经过人类作者仔细验证的新结果，帮助解决了之前未解决的问题。

Conclusion: 虽然这些贡献范围有限，但鉴于前沿AI的快速发展速度，其影响深远，展示了AI与人类合作进行科学研究的巨大潜力。
...
Result: 实验表明ELPO在多个任务上优于现有最优方法，如在ArSarcasm数据集上F1分数提升7.6分

Conclusion: ELPO框架通过集成学习方法显著提升了提示优化的性能，为复杂任务提供了更准确和鲁棒的解决方案
...
TL;DR: 本文提出Token-Selective PEFT（TS-PEFT）方法，选择性地对部分位置索引应用参数高效微调，挑战传统PEFT对所有位置都进行修改的必要性。


<details>
  <summary>Details</summary>
Motivation: 传统PEFT方法对所有位置索引都应用修改，作者质疑这种做法的必要性，认为选择性修改可能提升下游任务性能。

Method: 引入TS-PEFT框架，通过选择函数S仅对位置索引的子集应用PEFT修改，而非传统PEFT的全面修改。

Result: 实验表明，对所有位置索引不加选择地应用PEFT不仅多余，甚至可能适得其反。
...
TL;DR: SemanticCite是一个AI驱动的系统，通过全文分析验证引文准确性，提供四类分类（支持、部分支持、不支持、不确定）来捕捉细微的声明-来源关系，并针对不同错误类型采取补救措施。


<details>
  <summary>Details</summary>
Motivation: 解决学术文献中的语义引文错误、AI生成幻觉引用以及传统引文格式无法指向具体支持段落的问题，确保科学传播的准确性。

Method: 结合多种检索方法与四类分类系统，使用轻量级语言模型进行全文源分析，提供详细推理和相关文本片段。

Result: 实验表明，微调的轻量级语言模型性能可与大型商业系统相媲美，计算需求显著降低，使大规模引文验证实际可行。
...
<details>
  <summary>Details</summary>
Motivation: 现有不确定性量化方法主要依赖语义概率分布或成对距离，忽略了潜在语义结构信息，这限制了不确定性估计的精度。在安全关键场景中，可靠的UQ对于防止LLM产生幻觉至关重要。

Method: 1. 开发自适应稀疏有向语义图构建算法，捕获方向性语义依赖并自动修剪负面干扰连接；2. 通过层次抽象利用潜在语义结构信息，将SeSE定义为最优语义编码树的结构熵；3. 扩展SeSE以量化长文本生成中单个声明的不确定性。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs在真实应用中的广泛部署，确保模型响应与人类意图对齐成为基础挑战，特别是在无需昂贵重训练或大量监督的情况下实现推理时的高效对齐。

Method: 提出SDA框架，基于用户定义的对齐指令动态重分布模型输出概率，无需微调即可增强模型行为与人类意图的对齐。该方法轻量、资源高效且兼容多种开源LLMs。

Result: 在8个不同规模和来源的开源LLMs上评估，SDA在帮助性、无害性和诚实性三个关键维度上一致提升对齐性能，平均增益分别为64.4%、11.5%和30%。

Conclusion: SDA是一种有效且通用的对齐方法，能够显著提升LLMs在多种应用场景下的对齐性能，支持个性化偏好对齐，并可与基于训练的对齐策略集成。
...
TL;DR: 提出自改写框架，通过模型改写自身推理文本并从中学习，以提升推理过程质量，解决传统强化学习仅关注最终正确性的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习仅基于最终正确性的奖励机制无法对内部推理过程提供详细监督，导致推理质量不佳，出现过思考、欠思考、冗余思考和混乱思考等问题。

Method: 采用选择性改写方法，仅对模型持续正确的"简单"样本进行改写，保持GRPO的所有原始奖励信号，并在单个批次中同时进行改写和原始生成，仅增加约10%开销。

Result: 在准确率-长度权衡方面，自改写方法在准确率提升0.6%的同时显著缩短推理长度46%；在推理质量方面，LLM作为评判者的得分显著提高7.2分，有效缓解内部推理缺陷。

Conclusion: 自改写框架能有效提升推理模型内部思考过程质量，在保持强化学习可扩展性的同时显著改善推理效率和准确性。
...
TL;DR: 本文创建了三个用于评估语言模型处理习语和比喻语言能力的数据集，包括一个大尺度潜在表达数据集和两个人标注的确定表达数据集，用于训练和评估模型在习语识别任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 习语和比喻语言在口语和书面语中占很大比例，但大型语言模型在这方面表现不佳。虽然大型语料库看似是解决方案，但习语和比喻语言仍然是LLMs的难点。微调方法被证明是最优的，但更好更大的数据集可以进一步缩小这一差距。

Method: 使用近期习语和比喻语言数据集获取合并的习语列表，从大型语料库中检索上下文序列。创建了一个大规模潜在习语和比喻语言表达数据集，以及两个额外的人工标注确定表达数据集，用于评估预训练语言模型处理比喻意义的能力。

Result: 构建的数据集经过后处理以实现模型无关的训练兼容性，被用于训练，并在槽位标注和序列标记任务中进行评估。

Conclusion: 本文提供的数据集为解决习语和比喻语言理解问题提供了一种答案，同时提供了多样化的类别来构建新模型和开发新方法。
...
TL;DR: 本文分析了自然语言解释（rationales）的充分性指标，发现该指标无法系统性地捕捉解释信息对模型性能的影响，并探讨了解释信息在跨域分类中的作用。


<details>
  <summary>Details</summary>
Motivation: 现有评估模型是否基于正确原因学习标签的充分性指标存在局限性，无法深入了解解释信息对模型性能的实际影响，需要更系统地研究解释信息的复杂性。

Method: 通过将充分性与两种建模范式相关联：模型识别解释标记的能力（通过标记分类）和通过将解释纳入输入来提升模型性能的能力（通过注意力正则化）。

Result: 研究发现高度信息化的解释不太可能帮助正确分类实例；充分性反而捕捉了非解释上下文的分类影响；解释信息可以提升跨域分类但结果不一致；充分性与标记分类似乎无关。

Conclusion: 解释信息具有复杂性，需要开发能够系统捕捉此类信息的指标进行进一步研究。
...
TL;DR: MinerU-HTML是一个基于语言模型的HTML内容提取管道，通过序列标注方法显著提升网页内容提取质量，特别是对结构化元素的保留效果，并构建了AICC语料库证明提取质量对下游模型性能有重要影响。


<details>
  <summary>Details</summary>
Motivation: 现有网页语料库主要依赖启发式提取器（如Trafilatura），这些方法难以保留文档结构和结构化元素（公式、代码、表格），限制了网页数据质量对大型语言模型训练的价值。

Method: 将内容提取重新定义为序列标注问题，使用0.6B参数的语言模型解决；采用两阶段格式化管道，先对语义元素进行显式分类，再转换为Markdown格式。

Result: 在7,887个标注网页的MainWebBench基准上，MinerU-HTML达到81.8% ROUGE-N F1，显著优于Trafilatura的63.6%；结构化元素保留率极高（代码块90.9%，公式94.0%）；AICC语料库训练的模型在13个基准上平均准确率达50.8%，优于Trafilatura提取的语料1.08个百分点。

Conclusion: HTML提取是网页语料构建中关键但常被低估的组件，基于模型的提取方法在提升数据质量和下游模型性能方面具有显著优势。
...
TL;DR: 本研究评估了机器学习和深度学习模型在区分感知低质量与高质量新闻文章方面的效果，使用140多万篇新闻文章数据集，发现深度学习模型（特别是ModernBERT-large）表现最佳。


<details>
  <summary>Details</summary>
Motivation: 探索机器学习和深度学习模型是否能有效区分感知低质量与高质量的新闻文章，以帮助自动识别新闻质量。

Method: 使用3种机器学习分类器和3种深度学习模型，基于新创建的1,412,272篇英语新闻文章数据集，通过专家共识评级将来源网站分为低质量与高质量两类，每篇文章提取194个语言特征。
...
Conclusion: 传统基于CPU的机器学习分类器和深度学习分类器都能有效区分全球新闻文章的感知质量，其中深度学习模型表现更优。
...
Method: 构建包含多ESG主题的领域基础问题集，配备人工标注的答案和证据支持，用于细粒度评估模型推理能力。

Result: 分析了最先进LLM在ESGBench上的表现，揭示了在事实一致性、可追溯性和领域对齐方面的关键挑战。

Conclusion: ESGBench旨在加速透明和负责任的ESG导向AI系统研究，为评估可解释ESG问答提供标准化框架。
...
TL;DR: 本文研究了基于Transformer的语言模型如何处理习语表达，通过电路发现技术识别出习语处理的特有计算模式，包括习语注意头和增强接收现象。
...
Result: 发现习语处理具有独特的计算模式，识别出跨不同习语频繁激活的习语注意头，以及习语token间由于早期处理而产生的增强注意机制。

Conclusion: 这些发现揭示了Transformer处理非组合性语言的机制，为理解更复杂语法结构的处理提供了路径。
...
<details>
  <summary>Details</summary>
Motivation: 解决商业文档中结构化数据提取的需求，同时满足在资源受限设备上的部署要求。

Method: 采用特定的训练协议开发轻量化模型，模型大小仅6.6 GiB，可在A10 GPU等资源受限设备上运行。

Result: 模型在A10 GPU（24GB内存）上可处理多达125页A4文档，在文档理解任务中表现出色。

Conclusion: Arctic-Extract在保持先进性能的同时实现了轻量化部署，适合长文档处理的实际应用场景。
...
TL;DR: 该论文提出了TurkColBERT基准，首次系统比较了密集编码器和延迟交互模型在土耳其语检索中的表现，发现延迟交互模型在参数效率上显著优于密集编码器，并开发了高效的索引算法MUVERA+Rerank。


<details>
  <summary>Details</summary>
Motivation: 神经信息检索系统在高资源语言中表现出色，但在形态丰富、资源较少的土耳其语中研究不足。密集编码器目前在土耳其语IR中占主导，但延迟交互模型尚未得到系统评估。

Method: 采用两阶段适应流程：先在土耳其语NLI/STS任务上微调英语和多语言编码器，然后使用PyLate在MS MARCO-TR上将其转换为ColBERT风格的检索器。评估了10个模型在五个土耳其BEIR数据集上的表现。

Result: 延迟交互模型在参数效率上表现突出：1.0M参数的colbert-hash-nano-tr比600M参数的turkish-e5-large小600倍，但保留了71%的平均mAP。较小的延迟交互模型显著优于密集编码器，ColmmBERT-base-TR在特定领域任务上mAP提升达13.8%。MUVERA+Rerank比PLAID快3.33倍，mAP相对提升1.7%。

Conclusion: 延迟交互模型在土耳其语检索中具有显著优势，特别是在参数效率和特定领域性能方面。MUVERA算法实现了低延迟检索，为生产环境提供了可行方案。但研究仍受限于中等规模数据集和翻译基准，需要更大规模的评估。
...
TL;DR: 本文提出了一种从LLM激活中预测文本类型的框架，使用Mistral-7B模型和两个数据集，通过scikit-learn分类器实现了最高98%和71%的F1分数。
...
Method: 使用Mistral-7B模型，基于其激活状态来预测提示文本的类型，采用scikit-learn分类器进行预测。

Result: 在两个数据集上，类型预测的F1分数分别达到98%和71%，结果持续优于控制任务。

Conclusion: 这提供了一个概念验证，表明可以使用浅层学习模型从LLM中推断文本类型。
...
TL;DR: 挑战ASR评估中WER标准的有效性，提出基于临床影响的评估框架，使用LLM作为评判器实现自动化安全评估


<details>
  <summary>Details</summary>
Motivation: 当前ASR在临床对话中广泛部署，但标准评估仍依赖WER，需要验证WER是否与临床影响相关

Method: 建立专家标注的黄金标准基准，分析现有指标与临床风险标签的相关性，引入LLM-as-a-Judge方法，使用GEPA优化Gemini-2.5-Pro模型

Result: WER和现有指标与临床风险标签相关性差；优化的LLM评判器达到90%准确率和0.816的Cohen's κ，与人类专家表现相当

Conclusion: 提供了经过验证的自动化框架，将ASR评估从简单的文本保真度扩展到必要的、可扩展的临床对话安全评估
...
TL;DR: 提出一种基于统计语言模型的词义消歧方法，无需人工标注训练数据，通过将符号NLU系统生成的候选含义转换为可区分的自然语言选项，利用LLM根据上下文选择合适解释。
...
Method: 使用统计语言模型作为消歧预言机，将符号NLU系统生成的多个候选含义转换为可区分的自然语言替代选项，通过查询LLM根据语言上下文选择合适解释，并将选定含义传播回符号NLU系统。
...
TL;DR: 本文比较了多模态RAG系统的两种检索方法：基于文本分块检索（图像先被LLM总结为文本）和直接多模态嵌入检索（图像以原生形式存储在向量空间）。实验证明直接多模态嵌入检索显著优于基于LLM总结的方法。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统依赖LLM将图像总结为文本，导致视觉细节和上下文信息丢失，影响下游检索和问答质量。

Method: 在金融财报数据集上比较两种检索方法：文本分块检索和直接多模态嵌入检索，评估了6个LLM模型和2个多模态嵌入模型。

Result: 直接多模态嵌入检索在mAP@5和nDCG@5指标上分别获得13%和11%的绝对提升，相对提升分别为32%和20%，且生成答案更准确、事实一致性更高。

Conclusion: LLM总结在预处理阶段引入信息损失，而直接多模态嵌入能保留视觉上下文，为检索和推理提供更完整的信息。
...
TL;DR: Nemotron Elastic是一个用于构建推理导向LLM的框架，通过单一父模型嵌入多个嵌套子模型，实现多尺度部署而无需额外训练，大幅降低训练成本。


<details>
  <summary>Details</summary>
Motivation: 传统方法训练不同规模的LLM家族成本过高，现有压缩方法仍需大量训练开销。需要一种更高效的方法来支持多预算部署。

Method: 采用混合Mamba-Attention架构，通过端到端训练的路由器和两阶段训练课程，结合分组感知SSM弹性化、异构MLP弹性化等技术，在单一模型中嵌入多个子模型。

Result: 应用于Nemotron Nano V2 12B模型，仅用110B训练token同时生成9B和6B模型，相比从头训练成本降低360倍，相比最先进压缩技术降低7倍，所有嵌套模型性能均达到或超过SOTA。

Conclusion: 该方法实现了多合一推理模型，部署内存与模型数量无关，为多尺度LLM部署提供了高效解决方案。
...
TL;DR: UniFit是一个基于多模态大语言模型的通用虚拟试穿框架，通过语义对齐模块和两阶段训练策略解决文本指令与图像间的语义鸿沟和数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 解决当前多任务VTON框架面临的两个关键限制：(1)文本指令与参考图像之间的语义鸿沟，(2)复杂场景下的数据稀缺问题。

Method: 提出MLLM引导的语义对齐模块(MGSA)，通过多模态输入和可学习查询捕获跨模态语义关系；采用两阶段渐进训练策略和自合成流程。

Result: UniFit不仅支持广泛的VTON任务（包括多服装和模型间试穿），而且实现了最先进的性能。

Conclusion: UniFit通过MLLM驱动的语义对齐和两阶段训练，成功构建了一个能够灵活处理多样化复杂任务的通用VTON框架。
...
TL;DR: EfficientSAM3通过渐进式分层蒸馏方法，将SAM3的能力转移到轻量级模型上，实现高效的设备端概念分割和跟踪。


<details>
  <summary>Details</summary>
Motivation: SAM3的统一架构在图像和视频理解方面表现出色，但其计算复杂度限制了在设备端的应用。
...
Conclusion: EfficientSAM3系列模型能够在保持高保真度的同时，实现设备端的概念分割和跟踪。
...
TL;DR: 提出四种新颖扩展来改进基于模型的6D物体姿态估计方法，包括动态非均匀密集采样、多假设推理、迭代优化和遮挡增强训练，在遮挡条件下显著提升精度和速度


<details>
  <summary>Details</summary>
Motivation: 解决现有6D姿态估计方法在遮挡条件下早期检测错误传播的问题，提升对未见物体的泛化能力

Method: 动态非均匀密集采样策略、多假设推理机制、迭代优化和遮挡增强训练，以及新的加权可见性评估指标

Result: 在ICBIN数据集上精度提升超过5%，BOP数据集上提升超过2%，推理速度提升约3倍

Conclusion: 提出的方法有效解决了遮挡条件下的姿态估计问题，显著提升了精度、鲁棒性和推理效率
...
TL;DR: 提出了一种自动生成合成历史地图数据的方法，通过将原始历史地图的制图风格转移到矢量数据上，解决历史地图分析中训练数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 历史地图分析缺乏足够的标注训练数据，人工标注耗时耗力，而现有的合成数据缺乏真实性和多样性。

Method: 采用自动深度生成方法和手动随机退化技术，模拟历史地图扫描中的视觉不确定性和噪声，生成无限数量的合成历史地图。

Result: 生成的训练数据集成功应用于同质地图语料库的领域自适应语义分割任务，使用自构建图卷积网络进行评估。

Conclusion: 该方法有效解决了历史地图分析中训练数据稀缺的问题，为同质历史地图语料库的土地覆盖解释等任务提供了可行的解决方案。
...
TL;DR: Box6D是一种针对仓库场景中存储箱的类别级6D姿态估计方法，通过快速二进制搜索推断箱子尺寸，使用类别CAD模板而非实例特定模型，在保持精度的同时显著降低推理时间。


<details>
  <summary>Details</summary>
Motivation: 解决仓库自动化中6D姿态估计的挑战：基于模型的方法需要精确CAD模型且泛化性差；无模型方法在复杂条件下容易失败；类别级方法过于通用且忽略环境先验。

Method: 使用单次RGB-D观测，通过快速二进制搜索推断箱子尺寸，基于类别CAD模板估计姿态，采用深度合理性过滤器和早停策略拒绝不合理假设。

Result: 在真实仓库场景和公共基准测试中，Box6D实现了竞争性或更优的6D姿态精度，同时将推理时间减少约76%。

Conclusion: Box6D为仓库环境中的存储箱提供了高效准确的6D姿态估计解决方案，平衡了灵活性和精度需求。
...
TL;DR: 提出一种两阶段自改进范式，通过自生成详细文本推理来弥合视觉语言模型在领域特定视频分类中的语义鸿沟，无需新标注即可显著提升性能


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在领域特定视频分类任务中表现不佳，特别是在数据有限的情况下，主要原因是稀疏的领域数据不足以弥合复杂时空内容与抽象分类标签之间的语义距离

Method: 两阶段方法：1) 提示VLMs为每个视频生成详细文本推理，迫使模型阐述领域特定逻辑；2) 在这些自生成推理上微调VLM，然后进行传统的监督微调

Result: 在多样化数据集上的广泛实验表明，该方法显著优于直接监督微调，验证了自生成推理作为适应领域特定视频分析的有效且标注高效的范式

Conclusion: 自生成推理为视觉语言模型适应领域特定视频分析提供了一种有效的、标注高效的范式，能够显著提升模型在数据有限情况下的性能
...
<details>
  <summary>Details</summary>
Motivation: CLIP等现有方法专注于单标签、单粒度对齐，在医学成像等复杂领域效果有限，因为这些领域的图像通常对应多个高层次标签和不同注释粒度。

Method: 提出MGLL对比学习框架，利用结构化多标签监督，集成跨粒度文本描述，引入带点约束的软标签监督，使用平滑KL散度确保跨粒度一致性。

Result: 在构建的大规模多粒度数据集上预训练，并在多个数据集上评估，MGLL在下游任务中优于其他最先进方法。

Conclusion: MGLL作为即插即用模块，有效解决了多标签和跨粒度对齐问题，在复杂视觉理解任务中表现出色。
...
TL;DR: 提出从3D心脏超声体积中自动选择标准2D视图的方法，结合深度学习视图分类器和解剖标志启发式算法，生成标准超声心动图视图，准确率达96%。


<details>
  <summary>Details</summary>
Motivation: 传统心脏超声依赖2D视频，3D超声心动图可简化采集过程并改善离轴特征评估，但医生习惯使用2D视图进行诊断。

Method: 应用深度学习视图分类器和基于解剖标志的下游启发式算法，结合心脏病专家提供的启发式规则，重建标准超声心动图视图。

Result: 在两家医院的1600个视频中达到96%准确率，提取的2D视频能够检测心脏异常并生成临床级心脏解剖测量。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频场景图生成系统是封闭的前馈管道，无法融入人工指导；而提示性分割模型缺乏语义或关系推理能力。需要将视觉提示与空间、时间和语义理解统一起来。

Method: 引入动态交互发现模块生成主体条件化对象提示，以及语义分类头执行联合实体和谓词推理。从单一用户提示出发，分割跟踪主体、自动发现交互对象并预测三元组。
...
TL;DR: InfoCLIP通过信息论方法改进CLIP在开放词汇语义分割中的微调，避免过拟合并保持预训练模态对齐。
...
Method: 基于互信息提出两个新目标：压缩像素-文本模态对齐以减少噪声，最大化预训练CLIP与微调模型对齐知识的互信息以传递适合分割任务的局部语义关系。

Result: 在多个基准测试中验证了InfoCLIP在增强CLIP微调方面的有效性，展示了其在非对称迁移中的适应性和优越性。

Conclusion: InfoCLIP通过信息论指导的模态对齐知识迁移，成功提升了CLIP在开放词汇语义分割任务中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 多任务学习存在破坏性任务干扰问题，导致联合训练模型性能低于单任务基准，限制了泛化能力。本文旨在改善乳腺超声肿瘤分割任务在多任务学习中的泛化表现。
...
Method: 检测引导的生成式框架，提取ROI级特征，使用BART生成器按从粗到细的顺序生成语义token，涵盖类别层次结构和属性-值对，支持属性条件化识别。

Result: 在大规模电商数据集和开源数据集上的实验表明，该方法显著优于现有相似性流水线和多阶段分类系统，实现了更强的细粒度识别和更一致的统一推理。

Conclusion: 该检测引导的生成式框架能够有效解决视觉语义理解中的细粒度类别和属性识别问题，为统一推理提供了更优的解决方案。
...
TL;DR: 提出FADS方法，通过聚类采样构建人口统计学平衡且语义相关的演示，无需微调即可改善多模态大语言模型在医学图像推理中的公平性。
...
Method: 提出公平感知演示选择(FADS)，通过聚类采样构建人口统计学平衡且语义相关的演示，作为轻量级、无需调优的上下文学习替代方案。
...
TL;DR: 提出了一种基于图表示的方法来改进长尾分布视觉数据集中的OOD检测，通过图卷积网络优化特征空间，显著降低了误报率并提高了尾部类别的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 在长尾分布数据集中，现有的OOD检测方法存在高误报率和尾部类别分类准确率低的问题，需要一种能够有效利用样本间关系的方法来改进性能。

Method: 使用预训练模型的特征空间初始化图结构，通过高斯化处理缓解激活层分布的偏差，然后使用图卷积网络精炼图表示，得到适合长尾OOD检测的特征空间。

Result: 在CIFAR10-LT、CIFAR100-LT和ImageNet-LT三个基准测试中，该方法在误报率和尾部类别分类准确率方面大幅优于现有最先进方法。

Conclusion: 基于图表示的方法能够有效改善长尾分布数据集中的OOD检测性能，特别是在处理尾部类别时表现出色。
...
TL;DR: 提出序列级优化框架，为衣物生成自然可打印的对抗纹理，在行走视频中保持隐蔽性，在数字和物理环境中均有效
...
Method: 使用UV空间映射、紧凑调色板和控制点参数化，结合物理模拟管道模拟运动、多角度视角、布料动态和光照变化，采用期望变换目标与时序加权优化控制点

Result: 实验显示具有强大稳定的隐蔽性、对视角变化的高鲁棒性以及优异的跨模型迁移性，物理打印衣物在室内外录制中实现可靠抑制

Conclusion: 该方法证实了在真实世界中生成可打印对抗纹理的可行性，为可穿戴对抗攻击提供了有效的解决方案
...
TL;DR: 提出了一种用于单步图像超分辨率的混合秩（MoR）架构，将稀疏MoE集成到真实图像超分辨率中，通过细粒度专家分区和退化感知路由实现自适应计算分配。
...
Method: 将LoRA中的每个秩视为独立专家，采用细粒度专家分区策略；利用CLIP嵌入和预定义正负文本对计算相对退化分数，动态指导专家激活；引入零专家槽和退化感知负载均衡损失。

Result: 综合实验验证了框架的有效性和最先进的性能表现。

Conclusion: MoR架构成功解决了真实图像超分辨率中的自适应计算和知识共享问题，实现了优异的性能。
...
TL;DR: 提出一种基于深度学习的激光散斑材料分类方法，用于激光切割过程的监控和控制，即使激光颜色改变也能保持高精度。


<details>
  <summary>Details</summary>
Motivation: 激光切割过程中产生大量粉尘、烟雾和气溶胶，对环境和工作健康构成风险，需要实时监测切割过程和材料类型。

Method: 使用卷积神经网络(CNN)在激光散斑图案数据集上进行训练，以识别不同材料类型，实现安全高效的切割。

Result: 模型在训练集上达到98.30%准确率，验证集上96.88%准确率，在3000张新图像上对30种材料评估获得0.9643的F1分数。
...
TL;DR: CuriGS是一个课程引导的稀疏视图3D重建框架，通过引入学生视图和渐进式训练策略，解决了3D高斯泼溅在稀疏视图设置下的过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)在稀疏视图设置下面临监督稀缺和视角覆盖有限导致的过拟合挑战，需要一种有效的解决方案。

Method: 提出课程引导框架：1)为每个教师视图生成多组不同扰动级别的学生视图；2)按课程计划逐步解锁更高扰动级别；3)通过深度相关和共同正则化进行学生视图正则化；4)使用多信号指标评估学生视图质量；5)定期保留表现最佳的学生视图加入训练集。

Result: 实验结果表明，CuriGS在各种合成和真实稀疏视图场景中，在渲染保真度和几何一致性方面均优于现有最先进方法。

Conclusion: CuriGS通过课程引导的学生视图增强策略，有效提升了3DGS在稀疏视图设置下的重建性能，为实时场景重建提供了高质量解决方案。
...
TL;DR: 提出一种跨模态学习策略，通过将高分辨率卫星图像与无人机级别的视觉细节融合，用于作物冠层性状估计。该方法能生成类似无人机的表示，在产量和氮预测等下游任务中优于真实卫星图像。


<details>
  <summary>Details</summary>
Motivation: 虽然卫星任务在农业相关任务中有效，但受限于空间分辨率，难以满足现代微地块管理系统的需求。无人机具有高分辨率优势但覆盖范围有限。需要一种方法结合两者的优势。

Method: 使用约84种杂交玉米品种在5个美国玉米带地点的卫星-无人机图像对数据集，训练模型学习不同传感模态间的精细光谱空间对应关系。

Result: 生成的无人机类似表示在多个下游任务（包括产量和氮预测）中持续优于真实卫星图像。

Conclusion: 跨模态对应学习有潜力弥合卫星和无人机传感在农业监测中的差距，为精细农业提供更好的数据支持。
...
TL;DR: 提出一个基于大语言模型的框架来解决食品识别中的领域偏移、长尾分布和细粒度分类挑战，通过文本-图像对齐在共享嵌入空间中实现更好的识别性能。


<details>
  <summary>Details</summary>
Motivation: 解决食品识别中的三个主要挑战：训练数据（来自互联网）与真实环境图像之间的领域偏移、长尾数据分布问题，以及不同类别食品之间视觉差异细微难以区分的问题。

Method: 利用大语言模型解析食品图像生成食品标题和成分，然后将生成的文本和不同领域的食品图像投影到共享嵌入空间，最大化配对相似度，最后使用对齐后的多模态特征进行识别。

Result: 在两个食品数据集上，该方法在长尾数据分布、领域适应和细粒度分类方面均优于现有专门方法。

Conclusion: 基于大语言模型的简单框架能够有效解决食品识别中的多重挑战，通过文本-图像对齐实现卓越的识别性能。
...
Method: 基于系统研究发现，提出AMS-KV策略：优先存储压缩尺度和局部尺度的KV，通过跨尺度相似性分析识别缓存需求层，优化KV缓存利用。

Result: 相比基线VAR模型，AMS-KV减少KV缓存使用达84.83%，自注意力延迟降低60.48%，并能在基线模型内存不足时实现更大批次的稳定扩展。
...
TL;DR: LiSTAR是一个创新的生成世界模型，直接在LiDAR传感器的原生几何上操作，用于合成高保真可控的4D LiDAR数据。它通过混合圆柱-球面表示减少量化伪影，使用时空注意力机制建模复杂动态，并支持可控合成。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真环境需要高质量可控的4D LiDAR数据合成，但由于传感器独特的球面几何、点云时间稀疏性和动态场景复杂性，这一任务极具挑战性。
...
TL;DR: VideoSeg-R1是首个将强化学习引入视频推理分割的框架，采用解耦架构，将任务分解为参考图像分割和视频掩码传播，通过分层文本引导帧采样、推理模型和分割传播三阶段实现高效视频推理分割。


<details>
  <summary>Details</summary>
Motivation: 传统视频推理分割方法依赖监督微调，泛化能力有限且缺乏显式推理过程。为解决这些问题，提出了引入强化学习的创新框架。

Method: 采用三阶段架构：1)分层文本引导帧采样模拟人类注意力；2)推理模型生成空间线索和显式推理链；3)使用SAM2和XMem进行分割传播。包含任务难度感知机制自适应控制推理长度。

Result: 在多个基准测试上的广泛评估表明，VideoSeg-R1在复杂视频推理和分割任务中实现了最先进的性能。

Conclusion: VideoSeg-R1通过引入强化学习和显式推理链，显著提升了视频推理分割的性能和泛化能力，为视频理解任务提供了新的解决方案。
...
TL;DR: SpectralTrain是一个通用的、架构无关的训练框架，通过结合课程学习和基于PCA的光谱降采样来提高高光谱图像分类的学习效率，显著降低计算成本。
...
Method: 将课程学习与基于PCA的光谱降采样相结合，逐步引入光谱复杂性同时保留基本信息，实现光谱-空间模式的高效学习。

Result: 在三个基准数据集上的实验显示，训练时间减少了2-7倍，准确率变化较小，在云分类等气候相关遥感任务中表现出潜力。

Conclusion: 训练策略优化是高光谱图像模型中架构设计的有效补充，SpectralTrain框架具有广泛的适用性和实际部署价值。
...
TL;DR: Rad-GS是一个基于4D雷达-相机融合的SLAM系统，使用3D高斯作为可微空间表示，适用于千米级户外环境重建。
...
Method: 结合原始雷达点云与多普勒信息，通过几何增强点云指导动态物体掩码生成，利用非同步图像帧全局优化3D高斯表示，采用全局八叉树结构和高斯基元管理策略。

Result: 在千米级真实环境重建中表现优异，性能与传统基于相机或激光雷达的3D高斯方法相当，显著降低内存消耗并提升纹理一致性。

Conclusion: Rad-GS证明了4D毫米波雷达在大尺度户外场景重建中的潜力，为雷达SLAM系统提供了新的技术路径。
...
TL;DR: 本文提出了T2T-VICL框架，探索视觉语言模型在跨任务视觉上下文学习中的潜力，通过文本提示生成和感知评分推理实现不同视觉任务间的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型是否能在视觉提示和目标图像来自不同视觉任务时仍能实现视觉上下文学习，突破跨任务学习的边界。

Method: 设计文本提示生成和选择机制来描述不同低层视觉任务间的差异，构建首个跨任务VICL数据集，并提出结合感知评分推理和传统评估指标的新型推理框架。

Result: 在9个跨任务场景中取得顶级结果，在另外10个场景中取得次优性能，成功解锁了VLMs中跨任务VICL的潜力。

Conclusion: T2T-VICL框架有效实现了视觉语言模型的跨任务视觉上下文学习，为VLMs在多样化视觉任务中的应用开辟了新途径。
...
Method: 采用椭圆误差聚类和误差校正泼溅添加技术定位动态区域，并使用分组4D高斯泼溅提高泼溅与动态对象映射的一致性。通过交叉视角颜色一致性指导，对缺失颜色和遮挡错误进行针对性校正。

Result: 在Neural 3D Video和Technicolor数据集上验证，显著提高了时间一致性，在Technicolor光场数据集上PSNR提升0.39dB，达到最先进的感知渲染质量。

Conclusion: 该方法有效改善了泼溅与动态对象的对齐，误差校正方法能够准确识别错误并正确初始化新泼溅，提升了动态场景重建质量。
...
TL;DR: DCS-LDM提出了一种新的视觉生成范式，将信息复杂度与尺度解耦，通过构建层次化、尺度无关的潜在空间，实现固定潜在表示下任意分辨率和帧率的解码。
...
Conclusion: DCS-LDM通过解耦信息复杂度与尺度，实现了灵活的计算质量权衡，支持渐进式生成，为视觉生成提供了新的有效范式。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于光流的视频帧插值方法在低分辨率估计运动后使用上采样获取高分辨率光流，会导致边缘模糊和马赛克，且无法充分捕捉高分辨率下的精细像素运动，导致重影和不连续问题。

Method: VTinker包含两个核心组件：引导流上采样(GFU)使用输入帧作为引导来改善双线性上采样的模糊细节；纹理映射生成中间代理帧，从中选择清晰纹理块映射到代理帧，通过重建模块生成最终插值帧。

Result: 大量实验表明VTinker在视频帧插值任务中达到了最先进的性能。

Conclusion: VTinker通过引导流上采样和纹理映射有效解决了高分辨率视频帧插值中的运动估计和纹理重建问题。
...
TL;DR: 提出PiN-CLIP方法解决AI生成图像检测中的分布外泛化问题，通过在特征空间注入正激励噪声来抑制捷径敏感方向并增强稳定取证线索。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展使真实和合成图像难以区分，现有检测方法在分布外泛化方面存在挑战，主要原因是训练时利用了虚假捷径。

Method: 联合训练噪声生成器和检测网络，通过交叉注意力融合视觉和类别语义特征构建正激励噪声，在特征空间注入噪声来微调视觉编码器。

Result: 在包含42种不同生成模型合成的开放世界数据集上，该方法达到新的最先进性能，平均准确率比现有方法提高5.4个百分点。

Conclusion: PiN-CLIP通过特征空间的正激励噪声注入，有效抑制捷径敏感方向并放大稳定取证线索，实现了更鲁棒和泛化的伪影表示提取。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中量化参数可能未知，现有盲方法只能捕捉全局退化信息而缺乏空间细节，且不同压缩级别需要不同计算量

Method: 预训练退化表示学习模块提取高维多尺度退化表示来指导伪影去除，分层终止机制根据压缩级别动态调整处理阶段数量

Result: 在QP=22时PSNR提升110%（从0.31dB到0.65dB），分层终止机制在QP=22时推理时间比QP=42减少一半

Conclusion: 该方法显著提升了盲视频质量增强的性能和效率，为实际应用提供了有效解决方案
...
TL;DR: 本文提出了多模态交互欺骗评估任务和数据集，发现当前最先进的多模态大语言模型在识别社交互动中的欺骗行为方面表现不佳，并提出了社交思维链和动态社交认知记忆模块来改进性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然具备先进的推理能力，但缺乏人类智能的核心组成部分——在复杂社交互动中'察言观色'和评估欺骗的能力。

Method: 引入多模态交互欺骗评估任务，构建包含同步视频和文本的多模态数据集，评估12个先进模型，并设计社交思维链推理流程和动态社交认知记忆模块。

Result: 基准测试显示即使像GPT-4o这样的强大模型也难以可靠地区分真假，模型无法有效将语言与多模态社交线索关联，也缺乏建模他人知识、信念或意图的能力。

Conclusion: 需要新的方法来构建更具洞察力和可信度的AI系统，提出的框架在该挑战性任务上取得了性能提升，展示了构建具备真正类人社交推理能力的多模态大语言模型的新路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D目标检测器在训练和评估之间存在差距，主要源于训练时缺乏空间可靠性和排序感知，这与推理时基于排序的预测选择相冲突。

Method: 提出空间优先最优传输分配来动态强调位置良好且空间可靠的样本，以及秩感知自适应自蒸馏方案通过自蒸馏范式自适应注入排序感知。

Result: 在ScanNet V2和SUN RGB-D数据集上的大量实验表明，SR3D有效弥合了训练-推理差距，在保持实时速度的同时显著优于现有方法的准确性。

Conclusion: SR3D框架成功解决了3D目标检测中的训练-推理不一致问题，为室内点云的实时3D目标检测提供了有效解决方案。
...
Method: 构建HSRW-CD数据集，设计SSCP注意力模块包含MSA、SRGA和CSA三个组件，可即插即用到现有WBCD模型中

Result: 在HSRW-CD和Water-CD数据集上的大量实验验证了SSCP的有效性和泛化能力

Conclusion: 提出的数据集和SSCP模块显著提升了水体变化检测的判别能力
...
TL;DR: LEGO-SLAM是首个在3D高斯泼溅SLAM系统中实现实时开放词汇语义映射的框架，通过场景自适应编码器将高维语言特征压缩到16维，减少内存占用并加速渲染，同时支持语言引导的修剪和闭环检测。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS SLAM系统缺乏开放词汇语义理解能力，而存储高维语言特征会消耗过多内存和渲染资源，静态模型方法无法适应新环境。

Method: 使用场景自适应编码器-解码器将高维语言嵌入压缩到16维紧凑特征空间，实现语言引导的修剪策略减少60%以上高斯点，并利用映射特征进行语言驱动的闭环检测。

Result: 在保持竞争性映射质量和跟踪精度的同时，以15FPS实现开放词汇语义能力，显著减少内存占用和渲染开销。

Conclusion: LEGO-SLAM成功解决了3DGS SLAM中语义集成难题，为高级机器人交互提供了实时、紧凑且自适应的语义映射解决方案。
...
TL;DR: 提出RGE方法，通过将MLLMs的生成式推理能力融入嵌入提取过程，结合对比学习提升多模态表示质量


<details>
  <summary>Details</summary>
Motivation: 现有方法将嵌入提取视为直接编码步骤，忽视了MLLMs具备的推理能力可以提升表示质量

Method: RGE方法：先让模型基于指令生成结构化推理过程，然后在推理完成后提取表示，结合对比训练
...
TL;DR: PPCL是一种专门为DiT架构设计的可插拔剪枝框架，通过连续层蒸馏实现参数减少50%，关键指标退化小于3%，保持高质量图像生成能力


<details>
  <summary>Details</summary>
Motivation: DiT模型参数量大导致计算成本高，阻碍在资源受限环境中的部署，需要高效的剪枝方法

Method: 使用线性探测机制和相似度指标一阶微分趋势分析识别冗余层区间，提出即插即用的师生交替蒸馏方案，在单次训练中集成深度和宽度剪枝

Result: 在多个多模态扩散变换器架构模型上实现参数减少50%，关键客观指标退化小于3%，保持高质量图像生成

Conclusion: PPCL在保持高质量图像生成的同时实现更高压缩比，适合资源受限环境
...
TL;DR: Video2Layout框架通过连续边界坐标重建空间布局，克服了传统栅格表示在细粒度空间推理上的限制，在多个基准测试中表现优于基于栅格地图的方法。
...
Conclusion: 连续边界坐标表示比离散栅格表示更适合空间推理任务，能够有效提升多模态大语言模型的空间智能能力。
...
<details>
  <summary>Details</summary>
Motivation: 点云补全任务面临同时保留输入细粒度细节和确保全局结构完整性的挑战。现有基于局部对称变换的回归方法存在过拟合和噪声敏感的问题。

Method: 将点级变换回归重新定义为分布学习问题，结合对称性先验和扩散模型的生成能力，使用分层Mamba架构实现高保真上采样。

Result: 在PCN、ShapeNet和KITTI基准测试中验证了方法的state-of-the-art性能。

Conclusion: Simba框架通过分布学习方法有效解决了点云补全中的过拟合和噪声敏感问题，实现了优异的补全效果。
...
TL;DR: TimeViper是一个混合视觉语言模型，采用Mamba-Transformer混合架构处理长视频，提出TransV模块压缩视觉token，能处理超过10,000帧的时长视频。


<details>
  <summary>Details</summary>
Motivation: 处理长视频需要高效模型架构和有效处理扩展时间上下文的机制，同时发现视觉token到文本token的信息聚合现象导致视觉token冗余。

Method: 采用混合Mamba-Transformer骨干网络，结合状态空间模型效率和注意力机制表达能力；提出TransV模块将视觉token转移压缩到指令token中。

Result: 在多个基准测试中与最先进模型竞争，同时扩展帧数处理能力；分析了Mamba和Transformer层的注意力行为。

Conclusion: 这是开发、解释和压缩混合Mamba-Transformer架构的初步步骤，为长视频理解提供了新见解。
...
TL;DR: 提出LNG-SWR方法，通过层间噪声引导选择性小波重构，在训练时注入噪声学习频率偏置先验，实现频率自适应，提升医学图像分割的鲁棒性，同时保持清洁性能。


<details>
  <summary>Details</summary>
Motivation: 临床部署需要分割模型在分布偏移和扰动下保持稳定。对抗训练虽能提升鲁棒性，但存在清洁-鲁棒性权衡和高训练成本的问题，限制了在医学影像中的可扩展性和可维护性。
...
<details>
  <summary>Details</summary>
Motivation: 现有生存分析方法缺乏临床所需的透明度，而现有病理智能体在生存预测中存在三个局限：无法整合多模态数据、区域探索效率低、未能利用历史案例的经验学习。

Method: 两阶段方法：1) WSI-基因CoT增强案例库构建，通过分层分析病理图像和基因分层分析生成结构化报告；2) 二分法多专家智能体推理，通过RAG检索相似案例并整合多模态报告进行渐进区间精化。

Result: 在五个TCGA队列上的广泛实验表明，SurvAgent优于传统方法、专有MLLM和医疗智能体，为精准肿瘤学中的可解释AI驱动生存预测建立了新范式。

Conclusion: SurvAgent通过分层思维链增强多智能体系统，成功解决了现有方法的透明度问题，为临床可解释的生存预测提供了有效解决方案。
...
TL;DR: 提出了一种新的冗长文本诱导攻击方法VTIA，通过两阶段框架在良性图像中注入难以察觉的对抗扰动，诱导视觉语言模型生成冗长输出以增加计算成本和延迟。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在多模态任务中的成功，其部署效率问题日益突出。现有方法缺乏稳定性和可控性，无法直接最大化输出令牌长度作为明确优化目标。

Method: 采用两阶段框架：1）对抗提示搜索，使用强化学习策略自动识别能诱导LLM生成冗长输出的对抗提示；2）视觉对齐扰动优化，在输入图像上构建对抗样本，最大化扰动图像视觉嵌入与对抗提示嵌入的相似性。

Result: 在四个流行的VLM上进行全面实验，证明该方法在有效性、效率和泛化能力方面具有显著优势。

Conclusion: VTIA方法能够有效诱导VLM生成冗长文本，揭示了模型在安全性和效率方面的新漏洞，为防御此类攻击提供了重要见解。
...
TL;DR: EvoVLA是一个自监督的视觉-语言-动作框架，通过阶段对齐奖励、基于姿态的对象探索和长时程记忆三个组件，解决了长时程机器人操作中的阶段幻觉问题，显著提升了任务成功率和样本效率。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型存在阶段幻觉问题，即智能体利用粗略评估信号在多步任务中走捷径，报告高进度但未真正完成任务。这限制了长时程机器人操作的实际效果。

Method: 1. 阶段对齐奖励(SAR)：使用三元组对比学习和Gemini生成的困难负样本来防止视觉捷径；2. 基于姿态的对象探索(POE)：基于相对对象-夹爪姿态而非原始像素来引导好奇心；3. 长时程记忆：使用选择性上下文保留和门控融合来稳定内在塑造。
...
Conclusion: EvoVLA有效解决了VLA模型中的阶段幻觉问题，实现了更好的长时程操作性能和模拟到真实的迁移能力，为机器人操作提供了更可靠的解决方案。
...
TL;DR: 提出Thinking-while-Generating(TwiG)框架，在视觉生成过程中交织进行文本推理，实现文本推理与视觉生成的共同演进。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成方法只在生成前或生成后进行文本推理，缺乏生成过程中的多模态交互。

Method: 开发TwiG框架，在视觉内容逐步生成时交织文本推理，指导即将生成的局部区域并反思已合成内容。探索了三种策略：零样本提示、监督微调(TwiG-50K数据集)和强化学习(TwiG-GRPO)。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法很少从可解释性机制角度研究CLIP在密集预测中的性能边界，发现CLIP会将大量注意力资源从目标区域分散到无关token，类似于人类注意力分散现象。

Method: 提出RF-CLIP训练免费方法，通过过滤维度特定过度激活产生的干扰token，将注意力从分散token重定向回目标区域，提升CLIP的多模态对齐粒度。

Result: 在八个基准测试中达到最先进性能，同时保持高推理效率。

Conclusion: 通过模拟人类注意力重聚焦行为，有效提升了CLIP在开放词汇语义分割中的密集预测能力，验证了方法的有效性。
...
TL;DR: Mantis是一个新颖的视觉-语言-动作框架，通过解耦视觉预测来提升模型性能，在LIBERO基准测试中达到96.7%的成功率，优于现有方法。
...
Method: 提出解耦视觉预测(DVF)，使用元查询和扩散Transformer头将视觉预测与主干网络分离，通过残差连接提供当前视觉状态，让元查询自动捕捉潜在动作。

Result: 在LIBERO基准测试中达到96.7%成功率，超越强基线方法，收敛速度快。真实世界评估显示在指令跟随、泛化到未见指令和推理能力方面优于π_{0.5}模型。

Conclusion: Mantis通过解耦视觉预测有效减轻了VLA主干网络负担，使其能够通过语言监督保持理解和推理能力，在多个任务中表现出色。
...
TL;DR: 提出DSLGA两阶段模型解决UDA-VI-ReID问题，通过域共享学习和渐进对齐策略处理跨域和跨模态差异，在无监督域适应可见光-红外行人重识别任务中取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有VI-ReID算法在公共数据集表现良好，但与真实世界数据存在差异，难以在实际应用中有效工作。需要解决无监督域适应问题，将公共数据集知识迁移到真实数据而不损失精度且无需新样本标注。

Method: 两阶段模型DSLGA：1) 预训练阶段使用域共享学习策略(DSLS)缓解跨域模态差异；2) 微调阶段使用渐进对齐策略(GAS)通过簇到整体对齐方式处理跨模态对齐挑战。

Result: 大量实验表明该方法显著优于现有VI-ReID域适应方法，甚至在某些设置下超过监督方法。

Conclusion: DSLGA模型有效解决了UDA-VI-ReID中的跨域和跨模态差异问题，为实际应用提供了可行的无监督解决方案。
...
TL;DR: VLA-Fool：首个针对具身视觉-语言-动作模型的多模态对抗鲁棒性研究，揭示了在文本、视觉和跨模态扰动下VLA模型的脆弱性
...
Conclusion: 具身多模态对齐具有脆弱性，VLA模型在多模态对抗攻击下容易失效，需要加强对抗鲁棒性研究
...
TL;DR: ICCE提出了一种新的无监督图像聚类方法，通过多聚类头训练和聚类集成策略，在十个图像分类基准上取得了最先进的性能，其中在ImageNet上首次超过70%准确率。


<details>
  <summary>Details</summary>
Motivation: 当前无监督图像分类方法要么将表示学习和聚类结合在迭代框架中，要么完全依赖基础模型只做聚类。本文旨在改进多聚类头方法，通过自适应最近邻选择和聚类集成来提升聚类性能。

Method: ICCE方法包含三个阶段：1) 在冻结骨干网络上训练多个聚类头生成多样化聚类结果；2) 使用聚类集成技术整合冲突结果形成共识聚类；3) 使用共识聚类结果作为伪标签训练图像分类器。

Result: 在十个图像分类基准上取得最先进性能：CIFAR10达到99.3%准确率，CIFAR100达到89%，ImageNet达到70.4%，这是首个在ImageNet上超过70%准确率的完全无监督方法。

Conclusion: ICCE通过聚类集成策略显著提升了无监督图像分类性能，缩小了与监督方法的性能差距，证明了该方法在无监督学习中的有效性。
...
TL;DR: SwiTrack是一个新颖的跨模态目标跟踪框架，通过三个专用流处理RGB、NIR和无效模态，提升跟踪鲁棒性和精度。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态跟踪方法通常连接并行RGB和NIR分支到共享主干，限制了模态特定特征的全面提取，无法有效处理目标漂移问题，特别是在存在不可靠输入时。

Method: 采用状态切换框架，部署三个专用流：RGB帧由视觉编码器处理；NIR帧通过NIR门控适配器和视觉编码器进行细化；无效模态使用一致性轨迹预测模块。还包含动态模板重建和相似性对齐损失。

Result: 在最新基准测试中达到最先进性能，精度率和成功率分别提升7.2%和4.3%，同时保持65帧/秒的实时跟踪速度。

Conclusion: SwiTrack通过专门的状态切换框架和模态特定处理，有效解决了跨模态跟踪中的目标漂移问题，实现了鲁棒且高效的跟踪性能。
...
TL;DR: 提出了一种基于MLP骨干网络的新方法，通过残差连接和Memory-Block组件，从稀疏传感器输入生成完整身体运动，在移动HMD上达到72FPS并显著减少预测误差。


<details>
  <summary>Details</summary>
Motivation: 现有AR/VR系统主要跟踪头和手部，导致3D全身重建不完整。需要从有限传感器输入的稀疏数据生成全身运动。

Method: 使用多层感知机(MLP)骨干网络，增强残差连接和Memory-Block组件。Memory-Block用可训练代码向量表示缺失传感器数据，结合先前时间实例的稀疏信号提高时间一致性。采用多任务学习框架。

Result: 实验表明该方法优于最先进基线，显著减少预测误差，在移动HMD上达到72FPS，改善了精度-运行时间权衡。

Conclusion: 提出的方法能够从稀疏输入有效生成完整身体运动，在保持高帧率的同时提高预测精度，适用于AR/VR应用。
...
TL;DR: TetraSDF是一个精确的解析网格提取框架，用于从带有四面体位置编码的ReLU MLP表示的SDF中提取网格，解决了采样方法离散化误差和传统解析方法适用范围有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从神经SDF中提取精确网格时面临挑战：基于采样的方法引入离散化误差，而连续分段仿射解析方法仅适用于普通ReLU MLP。需要一种既能精确匹配零水平集又具有实际可行性的方法。

Method: 使用四面体位置编码的ReLU MLP表示SDF，编码器的重心插值保持全局CPWA结构，能够在编码器诱导的多面体复合体内跟踪ReLU线性区域。采用固定的解析输入预处理器减少方向偏差并稳定训练。

Result: 在多个基准测试中，TetraSDF在SDF重建精度上匹配或超越现有基于网格的编码器，其解析提取器产生高度自一致的网格，忠实于学习到的等值面，同时保持实际运行时间和内存效率。

Conclusion: TetraSDF提供了一个实用的解析网格提取框架，能够从带有四面体位置编码的神经SDF中精确提取网格，在精度和效率方面都表现出色。
...
Conclusion: 提出的框架能够有效实现时空场景理解，支持辅助导航等实时应用。
...
TL;DR: 提出了一种用于杂草检测的混合深度学习框架，结合CNN、ViT和GNN技术，通过GAN数据增强和自监督对比预训练实现高精度检测。
...
<details>
  <summary>Details</summary>
Motivation: 利用移动设备部署的优势（数据隐私、无需网络连接、响应更快），将3DGS技术适配到移动GPU上，解决2D纹理缓存优化的关键挑战。

Method: 开发了针对2D内存优化的新型排序算法，改进了变量布局设计，并对3DGS算法的其他步骤进行了加速优化。

Result: Texture3dgs在排序方面实现了4.1倍加速，整体3D场景重建速度提升1.7倍，同时内存使用减少1.6倍。
...
<details>
  <summary>Details</summary>
Motivation: 视觉基础模型的特征通常被下采样14x/16x，限制了在像素级应用中的直接使用。现有特征上采样方法依赖数据集特定重训练或重隐式优化，限制了可扩展性和泛化性。

Method: 采用简单的每图像优化，学习结合空间和范围线索的各向异性高斯核，将高斯泼溅与联合双边上采样相结合。学习到的核作为通用边缘感知算子，可在架构和模态间无缝转移。

Result: 每224x224图像仅需约0.419秒运行时间，在语义分割、深度估计以及深度和概率图上采样任务中达到最先进性能。

Conclusion: 该框架提供了一种高效、通用的特征上采样解决方案，无需训练即可实现精确的高分辨率重建，具有良好的可扩展性和跨任务泛化能力。
...
TL;DR: 将稀疏自编码器重新解释为主题模型，提出SAE-TM框架，用于跨模态的大规模主题分析，在文本和图像数据集上表现出色。
...
Method: 将潜在狄利克雷分配扩展到嵌入空间，推导出稀疏自编码器目标作为最大后验估计器，提出SAE-TM框架训练可重用的主题原子。

Result: SAE-TM在文本和图像数据集上产生比强基线更连贯的主题，同时保持多样性，成功分析了图像数据集的主题结构和日本木版画的主题演变。

Conclusion: 稀疏自编码器是跨模态大规模主题分析的有效工具，为嵌入分析提供了新的理论框架和应用方法。
...
TL;DR: BioBench是一个生态视觉基准测试，解决了ImageNet在科学图像评估中的局限性，包含9个任务、4个生物界、6种采集模态的310万张图像，提供新的视觉表示质量评估信号。


<details>
  <summary>Details</summary>
Motivation: ImageNet-1K线性探测准确率已无法准确预测科学图像上的性能表现，在生态任务中仅能解释34%的方差，且对30%的模型排名错误，需要更可靠的AI-for-science基准。

Method: 开发BioBench基准，统一9个公开的生态应用任务，涵盖4个生物分类界和6种采集模态，提供Python API进行数据下载、轻量级分类器训练和性能评估。

Result: BioBench揭示了ImageNet在生态视觉任务评估中的不足，为计算机视觉在生态学领域提供了新的评估信号，ViT-L模型在A6000 GPU上6小时即可完成评估。

Conclusion: BioBench填补了ImageNet在科学图像评估中的空白，为任何领域构建可靠的AI-for-science基准提供了模板方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统基于多视图扩散模型（MVDs）的纹理生成方法在处理遮挡区域、实现精确网格纹理对齐以及保持跨视图一致性和颜色强度方面的固有局限性。

Method: 提出潜在颜色扩散方法，包括几何感知的颜色点云VAE和多控制扩散变换器（DiT），完全使用3D数据从头训练；引入原生几何控制，通过位置嵌入和几何潜在变量为DiT提供直接3D空间信息；设计紧密耦合的VAE-DiT架构。

Result: NaTex在纹理一致性和对齐方面显著优于先前方法，并展现出强大的泛化能力，能够无需训练或仅需简单调优即可应用于材质生成、纹理优化、部件分割和纹理化等各种下游应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有混合方法虽然性能强大但计算成本高，限制了在实时场景中的实用性。需要开发既高效又能保持良好恢复质量的解决方案。

Method: 1. 自适应白平衡缓解波长依赖性颜色衰减；2. 小波增强块进行多频带分解，捕获全局结构和精细纹理；3. 梯度感知模块使用可学习门控的Sobel算子显式保留边缘结构。

Result: 在基准数据集上的广泛实验表明，WWE-UIE以显著更少的参数和FLOPs实现了竞争性的恢复质量，能够在资源受限平台上进行实时推理。

Conclusion: WWE-UIE通过集成三个可解释先验，成功平衡了性能与效率，为水下图像增强提供了实用的实时解决方案。
...
Result: 在四个公开基准测试中，ChangeDINO在IoU和F1指标上持续优于近期最先进方法，消融研究证实了各组件的有效性。
...
TL;DR: ARASFSR是一种基于隐式表示网络的任意分辨率和任意尺度人脸超分辨率方法，解决了现有方法固定上采样尺度和对输入尺寸敏感的局限性。
...
Method: 使用2D深度特征、局部相对坐标和上采样比例来预测每个目标像素的RGB值；引入局部频率估计模块捕获高频纹理信息；采用全局坐标调制模块利用先验人脸结构知识实现分辨率自适应。

Result: 定量和定性评估表明，ARASFSR在不同输入尺寸和上采样尺度下都能稳健地超分辨率人脸图像，优于现有最先进方法。

Conclusion: ARASFSR通过三个创新设计实现了任意分辨率和任意尺度的人脸超分辨率，在灵活性和性能方面都表现出色。
...
TL;DR: 该论文提出了一种基于师生架构的弱监督学习方法，结合关键帧选择和更新算法，仅使用30%标注数据就能在无人机地形分类任务中同时提升mIoU和时序一致性。
...
Result: 在ScanNet++数据集上表现优异，优于现有SLAM系统，实现无漂移的相机跟踪。
...
TL;DR: MOMNet是一个无需对齐的深度超分辨率框架，通过多阶匹配机制从未对齐的RGB图像中自适应检索相关信息，解决了实际场景中RGB-D数据不对齐的问题。
...
Method: 使用多阶匹配机制（零阶、一阶、二阶）在多个特征空间中识别与深度一致的RGB信息，并通过多阶聚合结构检测器选择性融合特征。

Result: 大量实验表明MOMNet实现了最先进的性能，并展现出优秀的鲁棒性。

Conclusion: 提出的MOMNet框架有效解决了RGB-D数据不对齐问题，在深度超分辨率任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖整体特征进行配对比较，而作者认为细粒度差异对于鲁棒的离线签名验证至关重要。此外，基于transformer的骨干网络可能会自然掩盖局部细节，影响性能。

Method: 提出DetailSemNet模型，包括局部结构匹配和细节语义集成器。集成器利用特征解缠和重缠技术，增强细节信息同时扩展判别语义，提升局部结构匹配效果。

Result: 在离线签名验证的领先基准测试中，该方法持续优于近期方法，以明显优势达到最先进水平。在跨数据集测试中也表现出优异的泛化能力。

Conclusion: 强调局部结构匹配不仅提高了性能，还增强了模型的可解释性。结合泛化性和可解释性，DetailSemNet在实际应用中具有巨大潜力。
...
TL;DR: CAMS提出了一种基于CLIP的组合零样本学习方法，通过门控交叉注意力机制和多空间解缠技术，从视觉特征中提取语义特征并实现属性与对象的语义解缠，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的CZSL方法主要依赖图像编码器获得的全局语义表示，但这种表示能力有限，无法完全解缠属性和对象。需要更精细的语义特征提取和解缠方法。

Method: 设计门控交叉注意力机制从CLIP高层图像编码块中捕获细粒度语义特征，通过潜在单元自适应抑制背景和无关信息；然后进行多空间解缠，实现属性和对象语义的分离。

Result: 在MIT-States、UT-Zappos和C-GQA三个流行基准测试中，CAMS在封闭世界和开放世界设置下都达到了最先进的性能。

Conclusion: CAMS通过语义特征提取和多空间解缠有效提升了未见属性-对象组合的泛化能力，为组合零样本学习提供了新的解决方案。
...
TL;DR: 提出基于刚性体标记(RBM)的新型运动捕捉方法，通过稀疏6自由度标记和深度学习回归模型，实现实时高精度人体姿态估计


<details>
  <summary>Details</summary>
Motivation: 解决传统光学运动捕捉系统依赖密集标记配置导致的准备耗时、标记识别模糊等可扩展性问题

Method: 引入刚性体标记(RBM)作为基本单元，开发基于深度学习的回归模型，在测地线损失下直接估计SMPL参数

Result: 端到端方法在保持优化方法性能的同时，计算量减少一个数量级，在AMASS数据集上达到最先进的姿态估计精度

Conclusion: 结合稀疏6自由度RBM和流形感知测地线损失，为图形学、虚拟现实和生物力学提供了实用且高保真的实时运动捕捉解决方案
...
TL;DR: 提出一种几何引导的多相机深度估计方法，通过圆柱投影实现跨视图一致性的密集深度估计
...
Method: 使用几何引导方法，将各图像的3D点投影到共享圆柱体上建立邻域关系，应用显式空间注意力聚合跨图像特征

Result: 在DDAD和nuScenes数据集上评估，相比最先进方法提高了深度估计的跨视图一致性和整体精度

Conclusion: 该方法能够预测密集、度量化且跨视图一致的深度，显著改善了多相机系统的深度估计性能
...
<details>
  <summary>Details</summary>
Motivation: 解决深度学习方法在腹腔镜胆囊切除术中面临的遮挡、长距离依赖和罕见结构几何特征捕捉困难的问题

Method: 提出两种分割模型：(1) 静态k-NN图+GCNII实现稳定长距离信息传播；(2) 动态可微分图生成器+GAT支持自适应拓扑学习

Result: 在Endoscapes-Seg50和CholecSeg8k基准测试中，mIoU提升7-8%，mDice提升6%，在细薄、罕见和安全关键结构上表现优异

Conclusion: 图基分割方法结合ViT全局上下文和图关系推理，提高了可解释性和可靠性，为更安全的手术提供精确的解剖特征识别
...
TL;DR: 本文提出了一种语言驱动的属性泛化（LDAG）架构，通过利用目标类别的语言描述来构建鲁棒的支持策略，解决了小样本分割中由于类内变化导致的支持图像元信息不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 现有小样本分割方法主要从支持图像中提取元指导信息，但由于视觉表示的类内变化，这些信息无法为未训练类别提供准确指导。作者认为支持图像提供的参考信息可能不是必需的，关键在于为已训练和未训练类别提供无偏的元指导。
...
Conclusion: 利用语言描述构建支持策略比仅依赖支持图像更有效，能够为小样本分割提供更准确的无偏元指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有垃圾检测数据集缺乏对垃圾容器跟踪的标注，且多在静态环境中采集，限制了在真实物流场景中的应用

Method: 创建包含城市垃圾场景的综合数据集，为三个任务提供基线模型，并提出启发式跟踪方法和利用几何先验的模型无关分割框架

Result: 微调的目标检测器在垃圾容器检测上表现良好，但基线跟踪方法计数效果差；提出的启发式方法将平均绝对计数误差降低79.6%，几何感知策略在轻量模型上将分割mAP@0.5提升27%

Conclusion: StreetView-Waste为城市垃圾管理的真实感知系统研究提供了具有挑战性的基准
...
TL;DR: VLA-Pruner是一种针对视觉-语言-动作模型的专用令牌剪枝方法，通过双层级重要性标准和自适应令牌选择策略，在保持VLA模型性能的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言模型令牌剪枝方法仅基于语义显著性选择令牌，忽视了VLA模型的双系统特性（高层语义理解和低层动作执行），导致动作生成所需的关键信息被丢弃，严重影响VLA性能。
...
TL;DR: LLaVA³通过多视角2D图像提升VLM的3D场景理解能力，无需微调，在3D视觉问答和语言定位任务中表现优异
...
Method: 受立体主义画家启发，通过多视角3D重建生成全方位视觉表示，为VLM描述3D场景

Result: 在3D视觉问答和3D语言定位任务中超越了之前基于2D的VLM解决方案
...
TL;DR: FastSurfer-CC是一个高效、全自动的胼胝体形态测量框架，能够自动识别正中矢状面切片、分割胼胝体和穹窿、定位前后连合、生成厚度剖面和分区，并提取8个形状指标用于统计分析。
...
Method: 开发了FastSurfer-CC框架，自动执行正中矢状面识别、胼胝体和穹窿分割、前后连合定位、头部位置标准化、厚度剖面和分区生成，以及形状指标提取。

Result: FastSurfer-CC在各项任务中均优于现有专用工具，并且在亨廷顿病研究中检测到了现有最先进方法未能发现的统计学显著差异。
...
TL;DR: 本研究首次系统性地探讨了遮挡情况下的视频预测问题，通过引入点流和深度图信息来改善模型在遮挡场景和背景运动预测中的表现。


<details>
  <summary>Details</summary>
Motivation: 尽管通用视频预测模型在标准场景中表现优异，但遮挡仍然是视频预测的固有挑战。假设通过提供明确的运动信息（点流）和几何结构信息（深度图）能够提升模型在遮挡和背景运动情况下的预测性能。

Method: 采用标准的多对象潜在变换器架构进行未来帧预测，但改进该架构以整合深度和点流信息。在合成和真实世界数据集上进行受控评估，使用基于外观的指标和对象掩码上的Wasserstein距离来有效测量预测的运动分布。

Result: 当预测模型借助点流和深度信息时，在遮挡场景中表现更好，相比不使用这些模态的模型能够预测更准确的背景运动。

Conclusion: 点流和深度信息的引入显著提升了视频预测模型在遮挡场景下的性能，特别是在背景运动预测方面表现更佳。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法依赖大型高质量显微镜图像数据集，但由于微机器人制造复杂和标注工作量大，这些数据集获取困难且成本高昂。现有数字孪生技术难以复现复杂的光学显微镜现象。

Method: 将基于波动光学的物理渲染和深度对齐集成到生成对抗网络中，合成高保真显微镜图像用于微机器人姿态估计。

Result: 相比纯AI驱动方法，结构相似性指数提高35.6%，保持实时渲染速度(0.022秒/帧)。使用合成数据训练的姿势估计器达到93.9%/91.9%(俯仰/滚转)准确率，仅比真实数据训练的估计器低5.0%/5.4%。

Conclusion: 该框架能够泛化到未见过的姿势，实现数据增强，并为新型微机器人配置提供鲁棒的姿态估计，无需额外训练数据。
...
TL;DR: 提出一种利用图像采集时间信息的时间调制肿瘤分割方法，通过FiLM层整合时间信息，提升乳腺DCE-MRI肿瘤分割性能和模型泛化能力。
...
Method: 使用特征线性调制(FiLM)层整合采集时间信息，根据特定采集序列调制模型特征，能够充分利用每次成像研究中获取的可变数量图像。

Result: 在域内图像和公开域外数据集上的评估表明，整合相位采集时间知识提高了肿瘤分割性能和模型泛化能力。

Conclusion: 通过时间调制方法可以有效提升DCE-MRI肿瘤分割的准确性和泛化性，证明了时间信息在医学图像分析中的重要性。
...
TL;DR: 提出了一种联合室内场景建图和天花板相机注册的新方法，通过移动代理设备实现两种任务的统一优化框架
...
Method: 使用配备头戴RGB-D相机的移动代理遍历场景，同步采集自我视角和天花板相机视频，通过时间戳关联轨迹，构建因子图进行联合优化

Result: 方法不仅能有效完成两项任务，还能相互提升性能，建立了首个协作场景建图和相机注册的基准数据集

Conclusion: 提供了一个可靠的工具来促进下游位置感知应用，实现了场景建图和相机注册的协同增强
...
TL;DR: 提出了一个专门用于拳击击打检测和分类的综合视频数据集，包含6,915个高质量击打片段，涵盖6种不同击打类型，来自20个YouTube对练视频和18名运动员。


<details>
  <summary>Details</summary>
Motivation: 由于战斗运动的动态性和非结构化特性以及录制环境的变化，开发鲁棒的数据集一直是主要瓶颈，需要专门针对拳击击打检测和分类的标注数据集。

Method: 从20个公开YouTube对练视频中提取6,915个击打片段，手动分割和标注以确保精确的时间边界和类别一致性，涵盖多种运动风格、摄像机角度和运动员体型。

Result: 创建了一个包含6种击打类型、6,915个片段的全面数据集，支持实时视觉动作识别研究，特别是在低资源和不受约束的环境中。

Conclusion: 该数据集为拳击和相关领域的运动分析、自动化教练和性能评估提供了丰富的基准，旨在加速这些领域的研究进展。
...
TL;DR: 本文提出了SemCLIP方法，通过结合释义和否定处理，改进CLIP模型在语义变换下的鲁棒性，显著提高了对否定文本的处理能力。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在处理否定或释义文本时表现不稳定，因为否定会以最小词汇变化彻底改变意义，而释义可能产生完全不同但意义相同的表达，这给视觉语言模型的评估和对齐带来了挑战。

Method: 提出SemCLIP方法：评估释义和否定的组合，设计新的CLIP对比损失函数，使用LLM生成的包含原始、释义和否定文本描述的训练三元组来训练CLIP类模型。

Result: 在CC-Neg基准测试中，原始vs否定图像检索准确率从68.1%提升到78.1%；在Sugarcrepe++基准测试中表现优于仅使用否定描述的模型；在零样本分类任务中全面优于CLIP。

Conclusion: SemCLIP能够显著提高对语义变换的鲁棒性，在保持CLIP性能的同时，有效处理否定和释义文本，推动视觉语言模型在语义理解方面的发展。
...
TL;DR: 提出了一种用于体操裁判的鲁棒多摄像头体操运动员跟踪方法，通过结合体操领域知识（运动员3D中心位于预设垂直平面）和级联数据关联策略，在交叉视图检测不足时使用射线-平面相交来生成3D轨迹候选，解决了传统多摄像头三角测量在有限摄像头和复杂场景下的挑战。


<details>
  <summary>Details</summary>
Motivation: 体操运动员跟踪面临独特挑战：体育馆空间限制导致摄像头数量有限；光照变化、背景复杂、服装变化和遮挡使得多摄像头检测可能在某些视角失败，只能从两个相对视角获得有效检测，这给传统多摄像头三角测量准确确定3D轨迹带来困难。

Method: 提出级联数据关联范式：当交叉视图检测充足时使用三角测量生成3D轨迹候选；当检测不足时采用射线-平面相交方法（基于体操运动员3D中心位于预设垂直平面的领域知识）生成共面3D轨迹候选，用共面候选补偿不确定轨迹以最小化跟踪失败。

Result: 通过大量实验验证了方法的鲁棒性，在挑战性场景中优于现有方法。配备该跟踪方法的体操裁判系统已成功应用于最近的体操世锦赛，并获得国际体操联合会的高度认可。

Conclusion: 通过结合体操领域知识和级联数据关联策略，成功解决了多摄像头体操运动员跟踪的独特挑战，为体操裁判提供了可靠的跟踪解决方案，并在实际国际比赛中得到验证和应用。
...
TL;DR: 本文对局部和全局光流计算方法进行了应用分析，重点研究了Horn-Schunck算法，并实现了结合双线性插值和延拓的多分辨率版本以提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究不同光流计算方法在估计帧间运动时的有效性，特别是在各种图像条件下的表现。

Method: 采用局部方法（如Lucas-Kanade）和全局方法（如Horn-Schunck），并实现多分辨率Horn-Schunck算法，使用双线性插值和延拓技术。

Result: 多分辨率策略提高了光流计算的准确性和收敛性，在变化的图像条件下表现良好。

Conclusion: 结合多分辨率技术的Horn-Schunck算法能够有效改善运动估计的精度，为光流计算提供了实用的改进方案。
...
TL;DR: 提出了一种新颖的两阶段检测框架，通过监督对比学习和k-NN分类器，在少量样本下实现合成图像检测和来源归属，显著提升检测精度。
...
Method: 第一阶段使用监督对比学习的视觉深度学习模型提取判别性嵌入，第二阶段使用k-NN分类器在嵌入空间进行少样本学习，仅需每个类别150张图像。

Result: 在少样本学习机制下，平均检测准确率达到91.3%，比现有方法提升5.2个百分点；在来源归属任务中，AUC和OSCR分别提升14.70%和4.27%。
...
TL;DR: EOGS++是基于3D高斯泼溅的卫星图像重建方法，直接在原始高分辨率全色数据上操作，无需外部预处理，通过光流技术将束调整嵌入训练过程，提升相机姿态估计精度，在重建质量和效率方面达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 扩展地球观测高斯泼溅框架，针对卫星图像开发更高效的重建方法，避免依赖外部预处理工具，提升相机姿态估计精度。

Method: 直接在原始高分辨率全色数据上操作，使用光流技术将束调整嵌入训练过程，引入早期停止和TSDF后处理等改进。

Result: 在IARPA 2016和DFC2019数据集上，EOGS++在重建质量和效率方面达到最先进性能，建筑物平均MAE误差从1.33提升至1.19。

Conclusion: EOGS++在保持高斯泼溅计算优势的同时，显著提升了卫星图像的重建质量和几何精度，优于原始EOGS和其他基于NeRF的方法。
...
<details>
  <summary>Details</summary>
Motivation: VAR模型的多尺度生成存在累积KV缓存导致的高内存开销问题，限制了实际部署。研究发现早期尺度对网络深度极其敏感，而后期尺度对深度减少具有鲁棒性。

Method: 通过等距采样从30层VAR网络中选取16到2层的多个子网络，早期尺度使用完整网络，后期尺度使用子网络。子网络与完整网络共享权重，并提出渐进式训练策略解决优化冲突。
...
Method: 设计了紧凑而表达能力强的骨干网络、精心设计的混合成本聚合模块，以及在大规模数据上的三阶段训练策略来弥合仿真到真实的差距。

Result: 在四个广泛使用的真实世界基准测试中排名第一，准确率与或超过最先进的非先验方法，同时计算成本不到1%。

Conclusion: 该工作证明了超轻量模型能够实现强大的泛化能力，为高效立体匹配设立了新标准。
...
TL;DR: NutriScreener是一个基于检索增强的多姿态图注意力网络，结合CLIP视觉嵌入、类别增强知识检索和上下文感知，从儿童图像中实现营养不良检测和人体测量预测，解决泛化性和类别不平衡问题。
...
Method: 使用检索增强的多姿态图注意力网络，结合CLIP视觉嵌入、类别增强知识检索和上下文感知技术，在2141名儿童数据上训练和测试。

Result: 临床研究中医生评分准确度4.3/5、效率4.6/5；实现0.79召回率、0.82 AUC，显著降低人体测量RMSE；跨数据集结果显示使用人口匹配知识库可提升25%召回率、减少3.5cm RMSE。

Conclusion: NutriScreener为低资源环境下的早期营养不良检测提供了可扩展且准确的解决方案，已具备部署准备。
...
TL;DR: POMA-3D是首个从点图学习的自监督3D表示模型，通过将3D坐标编码到结构化2D网格上，保持全局3D几何结构同时兼容2D基础模型输入格式。
...
Method: 使用视图到场景对齐策略将丰富2D先验转移到POMA-3D；提出POMA-JEPA架构确保多视图下点图特征的几何一致性；构建ScenePoint数据集支持大规模预训练。

Result: POMA-3D成为专业和通用3D理解的强大骨干网络，仅使用几何输入（3D坐标）就能在3D问答、具身导航、场景检索和具身定位等任务中取得良好表现。

Conclusion: POMA-3D通过点图方式探索3D场景理解，有效解决了3D表示学习中的预训练先验稀缺和数据限制问题。
...
TL;DR: 提出Erase to Retain框架，通过LoRA约束的子空间更新实现医学图像分割网络的选择性知识遗忘，无需完全重新训练即可移除特定病变或类别的表示。


<details>
  <summary>Details</summary>
Motivation: 医学分割网络需要选择性知识遗忘能力，以满足隐私合规、伦理部署和数据集持续修订的需求。

Method: 采用师生蒸馏范式，结合LoRA约束的子空间更新，通过对抗性优化强制移除指定遗忘子集的语义信息，随后进行温和恢复阶段以保持保留数据的泛化能力。

Result: 在ISIC分割任务中，遗忘集IoU从0.875降至0.509，同时保留集和验证集性能保持竞争力；在分类任务中，遗忘集准确率从87.0%降至64.1%，保留集准确率从83.9%提升至90.6%。

Conclusion: 基于LoRA的子空间遗忘为医学图像分析提供了实用、可控且可逆的遗忘路径，使模型能够忘记敏感样本或结构，同时在关键区域保持性能。
...
TL;DR: 利用生成式AI技术创建合成烟雾数据集，并通过无监督域适应方法解决烟雾分割中的领域差异问题，提高野火检测准确性。


<details>
  <summary>Details</summary>
Motivation: 野火早期检测至关重要，但缺乏大规模标注的烟雾数据集限制了深度神经网络的应用潜力。

Method: 采用生成式AI合成标注烟雾数据集，探索无监督域适应方法，并集成风格迁移、GAN和图像抠图等先进生成技术。

Result: 通过合成数据增强和领域适应技术，提高了烟雾分割模型在真实世界数据上的性能表现。

Conclusion: 生成式AI方法能够有效解决烟雾检测中的数据稀缺问题，为更准确和可扩展的野火检测模型铺平道路。
...
TL;DR: 提出了SAM2S模型，这是基于SAM2改进的外科手术视频分割基础模型，通过多样化记忆机制、时间语义学习和抗模糊学习来解决手术场景中的长期跟踪和领域差距问题。


<details>
  <summary>Details</summary>
Motivation: 解决外科手术视频分割中存在的领域差距和长期跟踪限制问题，现有iVOS模型在手术场景中表现不佳。

Method: 构建SA-SV基准数据集，提出SAM2S模型，包含DiveMem多样化记忆机制、时间语义学习和抗模糊学习三个关键技术。

Result: SAM2S达到80.42平均J&F分数，比原始SAM2提升17.10分，比微调SAM2提升4.11分，同时保持68 FPS实时推理和强零样本泛化能力。

Conclusion: SAM2S显著提升了外科手术视频分割的性能，在长期跟踪和零样本泛化方面表现出色，为计算机辅助手术提供了有效的解决方案。
...
TL;DR: 本文针对长尾分布下的2D目标检测问题，在LVISv1数据集上通过改进BAGS框架和引入度量学习，实现了24.5%的mAP新纪录。
...
Method: 采用两阶段Faster R-CNN架构，改进平衡组Softmax(BAGS)框架缓解类别不平衡，并探索度量学习生成特征嵌入，使用k-NN进行推理。
...
TL;DR: 提出自适应引导上采样(AGU)方法，通过多参数优化在低光图像上采样时同时减少噪声和提高锐度，实现实时高质量图像渲染。


<details>
  <summary>Details</summary>
Motivation: 传统引导方法在低光图像上效果不佳，因为低光图像噪声高、亮度低，缺乏足够的特征用于引导上采样。

Method: 基于引导图像方法，通过多参数优化学习低光图像与明亮图像特征之间的关联，仅需少量样本图像对即可训练。

Result: 实验证明AGU在低光场景下优于现有最先进方法，能够使用低质量、低分辨率输入实时渲染高质量图像。

Conclusion: AGU通过多特征学习和实时处理能力，有效解决了低光图像上采样的挑战。
...
TL;DR: SAM 3D是一个从单张图像生成3D物体重建的模型，能够预测几何形状、纹理和布局，在自然图像中表现优异，特别是在存在遮挡和场景杂乱的复杂环境下。


<details>
  <summary>Details</summary>
Motivation: 解决在自然图像中进行3D物体重建的挑战，特别是在遮挡和场景杂乱的情况下，利用上下文视觉线索提高识别能力。

Method: 采用人机协作的标注流程来标注物体形状、纹理和姿态，构建大规模视觉基础的3D重建数据集，并通过多阶段训练框架结合合成预训练和真实世界对齐。

Result: 在真实世界物体和场景的人类偏好测试中，相比近期工作获得显著提升，至少达到5:1的胜率。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯扩散模型由于高斯基元数量庞大，导致去噪和后续处理耗时严重，生成速度慢且沿采样轨迹的可扩展性有限。

Method: 提出轻量级选择器模型评估潜在高斯基元质量，实现早期轨迹缩减；引入实例掩码去噪技术，在每一步去噪过程中过滤冗余背景区域的高斯基元。

Result: 实验表明TRIM显著提高了3D生成的效率和质量。

Conclusion: TRIM通过时间和空间修剪策略，在不损失质量的前提下有效加速3D高斯扩散模型的推理过程，支持推理时缩放。
...
TL;DR: 提出了一种新颖的3D分层语义分割框架，通过解耦解码器和双分支监督机制解决多层级冲突和类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D分层语义分割方法存在两个关键挑战：参数共享模型导致的多层级优化冲突，以及跨层级不可避免的类别不平衡问题。

Method: 采用主3DHS分支和辅助判别分支的框架，使用多个解码器进行粗到细的分层指导，并引入基于语义原型的双分支监督机制。

Result: 在多个数据集和骨干网络上实现了最先进的3DHS性能，核心组件可作为即插即用模块提升现有方法。

Conclusion: 提出的方法有效缓解了多层级冲突和类别不平衡问题，为3D分层语义分割提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决非结构化剪枝需要迭代训练-剪枝-再训练循环带来的高计算开销问题，提供更高效的计算方案。

Method: 在重要性分数计算中利用教师模型提供的梯度信号，识别和保留对任务性能和知识转移都关键的参数，然后进行稀疏感知的再训练。

Result: 在CIFAR-10、CIFAR-100和TinyImageNet等基准测试中，该方法在高稀疏度下性能损失最小，优于EPG和EPSD等先进基线方法。

Conclusion: 该框架为资源受限环境提供了计算高效且性能保持的解决方案，是迭代剪枝方案的有效替代。
...
TL;DR: 本文对Cambrian-S视频世界模型进行了批判性分析，发现其基准测试VSR和VSC存在严重缺陷，可以通过简单方法解决，且其推理策略利用了基准中的捷径而非真正的空间超感知能力。


<details>
  <summary>Details</summary>
Motivation: 评估Cambrian-S提出的空间超感知视频世界模型是否真正实现了其声称的能力，以及其基准测试的有效性。

Method: 1）提出NoSense基线方法，仅使用词袋模型SigLIP解决VSR基准；2）设计VSC-Repeat实验，通过重复视频来测试Cambrian-S推理策略的鲁棒性。

Result: 1）NoSense在VSR上达到95%准确率，表明该基准无需空间认知即可解决；2）VSC-Repeat实验使Cambrian-S准确率从42%降至0%，证明其依赖基准中的捷径而非真正的空间推理。

Conclusion: 当前VSI-Super基准无法可靠测量空间超感知能力，Cambrian-S的推理策略通过利用基准捷径而非稳健的空间超感知来提升性能。
...
TL;DR: PartUV是一个基于部件分割的UV展开管道，针对AI生成网格的噪声和不良条件问题，通过语义部件分解和几何启发式方法，生成更少但部件对齐的图表，同时保持低失真。
...
Method: 基于PartField部件分解方法，PartUV在自上而下的递归框架中结合高层语义部件分解和新的几何启发式方法，确保每个图表的失真低于用户指定阈值，同时最小化图表总数。
...
TL;DR: TriDiff-4D是一个基于扩散模型的4D生成管道，通过三平面重定位生成高质量、时间一致的4D虚拟形象，支持任意长度的4D序列生成，显著提升了生成效率和运动质量。


<details>
  <summary>Details</summary>
Motivation: 解决现有4D生成方法在时间一致性、几何一致性、运动规律性、计算成本和动态控制方面的局限性，满足对高保真、可控4D虚拟形象的需求。

Method: 采用自回归策略，通过两个扩散模型：首先生成规范3D虚拟形象和运动序列，然后根据运动序列对虚拟形象进行动画化，支持骨架驱动的4D生成。

Result: 实验结果表明TriDiff-4D显著优于现有方法，将生成时间从小时级缩短到秒级，同时大幅提升了复杂运动的生成质量、外观保真度和3D几何精度。

Conclusion: TriDiff-4D通过扩散模型和三平面表示，成功解决了4D生成的多个关键挑战，实现了高效、高质量、可控的4D虚拟形象生成。
...
TL;DR: SceneDesigner是一个用于精确灵活的多物体9自由度姿态控制的方法，通过分支网络和CNOCS表示实现高效训练，在可控性和生成质量上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多物体9D姿态（位置、大小、方向）同时控制方面存在可控性有限和生成质量下降的问题，无法实现全面的多物体姿态控制。

Method: 提出SceneDesigner方法，包含分支网络、CNOCS映射表示、两阶段训练策略（含强化学习）和分离物体采样技术，使用ObjectPose9D数据集进行训练。

Result: 广泛的定性和定量实验表明，SceneDesigner在可控性和生成质量方面显著优于现有方法。

Conclusion: SceneDesigner成功解决了多物体9D姿态控制的挑战，实现了精确灵活的姿态操纵，并支持用户特定的个性化控制。
...
TL;DR: V-ReasonBench是一个评估视频推理能力的基准测试，涵盖结构化问题解决、空间认知、基于模式的推理和物理动力学四个维度，旨在系统评估生成视频模型的推理能力。
...
Method: 构建包含合成和真实世界图像序列的基准测试，提供多样化、可复现、可扩展且答案可验证的任务，评估六个最先进的视频模型。

Result: 评估显示不同维度存在明显差异，模型在结构化、空间、模式基础和物理推理方面表现各异。视频模型与强图像模型比较，分析幻觉行为，研究视频时长对帧链推理的影响。
...
TL;DR: 提出DualMindVLM模型，通过强化学习让视觉语言模型根据问题难度自动切换快慢思维模式，在保持高性能的同时显著提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型追求冗长的推理链，导致计算成本过高。受人类快慢思维机制启发，希望让模型能根据任务难度自动选择思维模式。

Method: 两阶段方法：第一阶段根据模型输出长度标记数据为快慢思维；第二阶段使用GRPO强化学习训练模型发展双模式思维。

Result: DualMindVLM显著超越基础模型，性能与最先进视觉推理模型相当，同时保持极高的token效率。

Conclusion: 提出的简单RL方法能有效让VLMs发展双模式思维，在保持性能的同时大幅提升计算效率。
...
TL;DR: EvoLMM是一个完全无监督的自进化框架，通过单一骨干模型实例化两个协作智能体（Proposer和Solver），实现多模态推理能力的自我提升，无需人工标注数据或奖励模型。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型的训练仍依赖人工标注数据或外部奖励模型，限制了自主性和可扩展性。本文旨在以完全无监督的方式提升多模态推理能力。

Method: 提出EvoLMM框架，包含两个协作智能体：Proposer生成多样化的图像相关问题，Solver通过内部一致性解决这些问题，形成持续自我奖励的学习过程。

Result: 基于Qwen2.5-VL骨干模型，EvoLMM在ChartQA、MathVista和MathVision等多模态数学推理基准上获得约3%的稳定提升，仅使用原始训练图像。

Conclusion: EvoLMM提供了一种简单有效的完全无监督自改进方法，可作为未来自改进多模态模型研究的坚实基础。
...
TL;DR: NoPo-Avatar：无需姿态输入的动画化3D人体重建方法，在无真实姿态的实用场景中优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖准确的地面真实相机姿态和人体姿态作为输入，但在姿态估计不准确时重建质量会显著下降

Method: 提出NoPo-Avatar方法，仅从图像重建头像，完全不需要姿态输入，避免了噪声姿态估计的影响

Result: 在THuman2.0、XHuman和HuGe100K数据集上的实验表明，在无真实姿态的实用场景中优于现有基线，在有真实姿态的实验室场景中表现相当

Conclusion: NoPo-Avatar通过消除对测试时姿态输入的依赖，使3D人体重建方法更具实用性和鲁棒性
...
TL;DR: 提出Linear Gradient Matching方法，针对预训练视觉模型优化数据集蒸馏，使合成数据在训练线性分类器时产生与真实数据相似的梯度，显著提升性能并实现跨模型泛化。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法主要针对随机初始化模型的训练，而当前最优视觉方法越来越多地基于预训练的自监督模型。需要研究如何为这些预训练模型蒸馏最优数据集。

Method: Linear Gradient Matching方法：优化合成图像，使其通过预训练特征提取器后，在线性分类器中产生的梯度与真实数据产生的梯度相似。

Result: 合成数据性能超越所有真实图像基线，具有跨预训练模型的泛化能力（如用DINO骨干蒸馏的数据集训练CLIP线性分类器），在细粒度分类和模型可解释性方面表现优异。

Conclusion: 该方法为预训练模型提供了高效的数据集蒸馏工具，在性能、泛化能力和可解释性方面均有显著优势。
...
Result: 在Trainium 1上平均峰值吞吐量从49%提升到61%，Trainium 2上从45%提升到59%；使用开源模型比Claude Sonnet 4便宜26倍。

Conclusion: AccelOpt展示了随时间改进的能力，为AI加速器内核优化提供了高效且成本效益高的解决方案。
...
### [137] [PSM: Prompt Sensitivity Minimization via LLM-Guided Black-Box Optimization](https://arxiv.org/abs/2511.16209)
*Huseein Jawad,Nicolas Brunel*

Main category: cs.CR

TL;DR: 本文提出了一种通过shield appending来强化系统提示的新框架，将提示硬化形式化为效用约束优化问题，使用LLM作为优化器来搜索保护层，在保持任务效用的同时最小化提示泄露风险。


<details>
  <summary>Details</summary>
Motivation: 系统提示通常包含专有逻辑或敏感信息，容易受到提取攻击。现有防御机制依赖启发式方法、计算开销大或不适用于黑盒API访问的模型，需要一种轻量级且实用的防御方法。

Method: 采用shield appending方法，在原始提示上添加保护性文本层。将提示硬化形式化为效用约束优化问题，使用LLM作为优化器搜索可能的SHIELD，最小化基于对抗攻击套件的泄露指标，同时保持任务效用高于指定阈值。

Result: 经验证，优化的SHIELD显著减少了针对全面提取攻击集的提示泄露，优于现有基线防御方法，且不损害模型的预期功能。

Conclusion: 这项工作为在LLM安全不断升级的背景下开发鲁棒、效用感知的防御提供了新范式，该方法轻量级、实用，仅需API访问目标LLM和优化器LLM。
...
### [138] [Codec2Vec: Self-Supervised Speech Representation Learning Using Neural Speech Codecs](https://arxiv.org/abs/2511.16639)
*Wei-Cheng Tseng,David Harwath*

Main category: eess.AS

TL;DR: Codec2Vec是首个完全基于离散音频编解码单元的语音表示学习框架，在SUPERB基准测试中表现优异，同时显著降低了存储需求和训练时间。
...
Method: 使用离散音频编解码单元进行掩码预测，采用多种训练目标推导策略来优化框架效果。

Result: 在SUPERB基准测试中达到与连续输入模型相当的性能，存储需求减少16.5倍，训练时间减少2.3倍。

Conclusion: Codec2Vec展示了基于离散音频编解码单元的语音表示学习框架的可行性、可扩展性和高效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM查询重写方法缺乏统一实现工具，阻碍了公平比较、快速实验和可靠部署，需要标准化框架来解决这些问题。

Method: 提供Python API支持多种LLM方法，检索无关的接口集成Pyserini等后端，集中式提示管理系统，内置BEIR和MS MARCO基准测试支持。

Result: 开发了完全开源的QueryGym工具包，实现了LLM查询重写方法的统一实现、执行和比较框架。

Conclusion: QueryGym填补了LLM查询重写领域工具集的空白，为研究社区提供了标准化的实验和部署平台。
...
### [140] [Music Recommendation with Large Language Models: Challenges, Opportunities, and Evaluation](https://arxiv.org/abs/2511.16478)
*Elena V. Epure,Yashar Deldjoo,Bruno Sguerra,Markus Schedl,Manuel Moussallam*

Main category: cs.IR

TL;DR: 本文主张随着大型语言模型在音乐推荐系统中的应用，需要重新思考评估方法，提出了结合NLP评估实践的结构化成功与风险维度框架。


<details>
  <summary>Details</summary>
Motivation: 传统基于信息检索范式的音乐推荐系统评估方法在LLM时代面临挑战，因为LLM是生成式而非排序式模型，传统准确度指标不再适用，且存在幻觉、知识截止等问题。

Method: 首先回顾LLM如何重塑用户建模、物品建模和自然语言推荐，然后借鉴NLP领域的评估实践，最后通过LLM提示技术为MRS制定结构化的成功与风险维度。

Result: 提出了适用于LLM驱动音乐推荐系统的更新评估框架，强调跨学科视角和教学性指导。

Conclusion: LLM驱动的音乐推荐系统需要全新的评估范式，本文提供了结合NLP实践的结构化评估维度，为MRS社区提供更新视角。
...
### [141] [The Oracle and The Prism: A Decoupled and Efficient Framework for Generative Recommendation Explanation](https://arxiv.org/abs/2511.16543)
*Jiaheng Zhang,Daqiang Zhang*

Main category: cs.IR

TL;DR: Prism是一个解耦框架，将推荐系统分离为专门的排序阶段和解释生成阶段，通过知识蒸馏使用大型教师LLM生成高质量解释知识，再由紧凑的学生模型合成个性化解释，解决了性能与效率的权衡问题。
...
Method: 提出Prism解耦框架，使用强大的教师LLM（如FLAN-T5-XXL）作为Oracle生成高保真解释知识，然后由精调的紧凑学生模型（如BART-Base）专门合成个性化解释。

Result: 140M参数的Prism模型在忠实性和个性化的人类评估中显著优于11B参数的教师模型，推理速度提升24倍，内存消耗减少10倍。

Conclusion: 解耦结合定向蒸馏为高质量可解释推荐提供了高效有效的途径。
...
TL;DR: 提出了一个模拟和分析LLM多智能体系统中不合作行为导致系统崩溃的框架，包括基于博弈论的行为分类和动态生成不合作行为的模拟流程。


<details>
  <summary>Details</summary>
Motivation: 现有文献缺乏对LLM多智能体系统中不合作行为的系统性研究，需要理解这些行为如何破坏系统稳定性。

Method: 开发了包含博弈论行为分类和多阶段模拟流程的框架，在协作资源管理场景中评估系统稳定性。
...
### [143] [UniUltra: Interactive Parameter-Efficient SAM2 for Universal Ultrasound Segmentation](https://arxiv.org/abs/2511.15771)
...
TL;DR: UniUltra是一个用于通用超声图像分割的框架，通过上下文-边缘混合适配器和深度监督知识蒸馏技术，在保持高性能的同时大幅减少参数数量，适合临床部署。


<details>
  <summary>Details</summary>
Motivation: SAM2在自然图像上表现优异，但在超声图像上性能显著下降，需要解决参数高效微调和资源受限临床环境部署的挑战。

Method: 提出上下文-边缘混合适配器(CH-Adapter)增强细粒度感知，并开发深度监督知识蒸馏(DSKD)技术将知识从大型图像编码器转移到超轻量编码器。

Result: UniUltra在实验中优于现有方法，仅使用SAM2参数的8.91%进行微调，最终压缩模型相比原始SAM2减少94.08%的参数。

Conclusion: 该框架在保持竞争力的同时大幅降低计算需求，非常适合实际临床部署。
...
### [144] [Weakly Supervised Segmentation and Classification of Alpha-Synuclein Aggregates in Brightfield Midbrain Images](https://arxiv.org/abs/2511.16268)
*Erwan Dereure,Robin Louiset,Laura Parkkinen,David A Menassa,David Holcman*

Main category: eess.IV

TL;DR: 开发基于弱监督分割的自动化图像处理流程，用于在帕金森病和中度路易体病的中脑组织全切片图像中分割和分类α-突触核蛋白聚集物，能够区分路易体和神经突等主要形态，平衡准确率达80%。


<details>
  <summary>Details</summary>
Motivation: 帕金森病与错误折叠的α-突触核蛋白聚集物形成有关，这些聚集物形成路易体和神经突形态，用于病理诊断。深度学习自动分析免疫组织化学组织病理学图像为更好理解这些聚集物的空间组织提供了有前景的工具。

Method: 基于弱监督分割开发自动化图像处理流程，使用ResNet50分类器对中脑组织全切片图像中的α-突触核蛋白聚集物进行分割和分类，能够应对免疫组织化学标记变异性。
...
### [145] [Chain of Summaries: Summarization Through Iterative Questioning](https://arxiv.org/abs/2511.15719)
*William Brach,Lukas Galke Poech*

Main category: cs.AI

TL;DR: 提出Chain of Summaries (CoS)方法，通过类似黑格尔辩证法的迭代过程生成信息密集的通用摘要，使网页内容更易被LLM消化。


<details>
  <summary>Details</summary>
Motivation: 解决LLM处理网页内容时面临的格式不友好和上下文长度限制问题，使网页内容更易被LLM访问和使用。

Method: 采用类似黑格尔辩证法的三阶段迭代方法：初始摘要（正题）→ 通过质疑识别局限性（反题）→ 生成通用摘要（合题）。

Result: 在TriviaQA、TruthfulQA和SQUAD数据集上，CoS比零样本LLM基线提升66%，比专业摘要方法BRIO和PEGASUS提升27%。

Conclusion: CoS为网站维护者提供了一种使内容更易被LLM访问的吸引人选项，同时保留了人工监督的可能性。
...
### [146] [Step-Audio-R1 Technical Report](https://arxiv.org/abs/2511.15848)
...
TL;DR: Step-Audio-R1是首个成功的音频推理模型，通过模态基础推理蒸馏框架解锁音频领域的推理能力，在音频理解基准上超越Gemini 2.5 Pro，性能接近Gemini 3 Pro。


<details>
  <summary>Details</summary>
Motivation: 解决音频语言模型中存在的困惑现象：推理越少性能越好，探索音频智能是否真正受益于深思熟虑。

Method: 提出模态基础推理蒸馏框架，让模型学习生成基于声学特征的音频相关推理链，避免产生脱节的幻觉推理。

Result: 模型展现出强大的音频推理能力，在涵盖语音、环境声音和音乐的综合音频理解和推理基准上表现优异。
...
### [147] [JudgeBoard: Benchmarking and Enhancing Small Language Models for Reasoning Evaluation](https://arxiv.org/abs/2511.15958)
*Zhenyu Bi,Gaurav Srivastava,Yang Li,Meng Lu,Swastik Roy,Morteza Ziyadi,Xuan Wang*

Main category: cs.AI

TL;DR: JudgeBoard是一个直接评估模型答案正确性的新方法，无需额外答案比较。通过多智能体评判框架MAJ，小型语言模型可以接近大型语言模型的判断准确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-as-a-judge框架依赖答案比较，难以自动化且不支持细粒度评估。需要直接评估模型判断答案正确性的能力。

Method: 提出JudgeBoard评估管道直接查询模型评估答案正确性，使用MAJ多智能体框架让多个SLM协作判断，通过不同推理配置提升判断准确性。

Result: 实验显示SLM与LLM在独立判断任务中存在显著性能差距，但MAJ框架显著提升了SLM的可靠性和一致性。在MATH数据集上，使用较小模型的MAJ表现与较大模型相当甚至更好。

Conclusion: 多智能体SLM系统在判断任务中可能匹配或超越LLM性能，为可扩展高效评估提供了新途径。
...
### [148] [CARE-RAG - Clinical Assessment and Reasoning in RAG](https://arxiv.org/abs/2511.15994)
*Deepthi Potluri,Aby Mammen Mathew,Jeffrey B DeWitt,Alexander L. Rasgon,Yide Hao,Junyuan Hong,Ying Ding*

Main category: cs.AI

TL;DR: LLMs在临床环境中即使获得正确证据也常推理错误，研究使用WET指南测试发现检索增强生成能约束输出，但安全部署需要严格评估推理能力。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在临床环境中检索与推理之间的差距，确保输出符合结构化协议，特别是在权威证据提供后仍存在错误的问题。

Method: 使用Written Exposure Therapy指南作为测试平台，评估模型对临床问题的响应，提出评估框架衡量准确性、一致性和推理保真度。

Result: 发现即使提供权威段落，错误仍然存在，检索增强生成能约束输出但不足以确保安全部署。

Conclusion: 安全部署LLMs需要像严格评估检索一样严格评估推理能力，检索增强生成有潜力但需谨慎使用。
...
### [149] [SpellForger: Prompting Custom Spell Properties In-Game using BERT supervised-trained model](https://arxiv.org/abs/2511.16018)
*Emanuel C. Silva,Emily S. M. Salum,Gabriel M. Arantes,Matheus P. Pereira,Vinicius F. Oliveira,Alessandro L. Bicho*

Main category: cs.AI

TL;DR: SpellForger是一款让玩家通过自然语言提示创建自定义法术的游戏，使用BERT模型将文本描述映射到法术模板并平衡参数，验证AI作为直接游戏机制的应用。


<details>
  <summary>Details</summary>
Motivation: 探索AI作为核心游戏玩法共创工具的应用，目前这方面研究不足。旨在提供个性化和创造性的独特游戏体验。

Method: 使用监督训练的BERT模型解释玩家提示，将文本描述映射到法术模板并平衡参数（伤害、成本、效果）。游戏在Unity引擎开发，AI后端使用Python。

Result: 开发出功能原型，能够实时生成法术并应用于引人入胜的游戏循环中，玩家创造力成为体验的核心。

Conclusion: 成功验证了AI作为直接游戏机制的应用可行性，展示了玩家通过自然语言创造自定义法术的独特游戏体验。
...
Conclusion: 该训练方法不仅超越了强基线，还为未来大规模多模态推理研究奠定了坚实的实证基础，所有代码、流程和数据都已开源。
...
### [151] [TOFA: Training-Free One-Shot Federated Adaptation for Vision-Language Models](https://arxiv.org/abs/2511.16423)
*Li Zhang,Zhongxuan Han,XiaoHua Feng,Jiaming Zhang,Yuyuan Li,Linbo Jiang,Jianan Lin,Chaochao Chen*

Main category: cs.AI

TL;DR: 提出了一种无需训练的单次联邦视觉语言模型适应框架TOFA，通过视觉和文本双管道处理数据异质性，无需额外训练资源


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习中的视觉语言模型适应方法通信成本高且易受攻击，需要开发轻量级的单次适应方法来缓解这些问题

Method: 使用视觉管道学习个性化类别原型分布，文本管道评估和对齐本地文本提示，通过自适应权重校准机制结合双模态预测

Result: 在9个不同联邦设置的数据集上进行了广泛实验，证明了TOFA方法的有效性

Conclusion: TOFA框架成功解决了联邦视觉语言模型适应中的数据异质性问题，实现了高效的单次联邦适应
...
### [152] [D-GARA: A Dynamic Benchmarking Framework for GUI Agent Robustness in Real-World Anomalies](https://arxiv.org/abs/2511.16590)
...
Conclusion: D-GARA是模块化和可扩展的，支持新任务、异常类型和交互场景的无缝集成，以满足特定的评估目标。
...
### [153] [How Modality Shapes Perception and Reasoning: A Study of Error Propagation in ARC-AGI](https://arxiv.org/abs/2511.15717)
*Bo Wen,Chen Wang,Erhan Bilal*

Main category: cs.AI

TL;DR: 本文研究了不同模态（文本和图像）在ARC-AGI基准测试中对模型感知能力的影响，发现结构化文本能精确捕捉稀疏特征坐标，图像能保持2D形状但对分辨率敏感，结合两种模态可提升执行效果。


<details>
  <summary>Details</summary>
Motivation: 当前指令优先系统在ARC-AGI任务中缺乏对编码方式如何影响模型感知的原则性理解，以及如何区分指令错误和执行错误。假设不同模态会施加感知瓶颈，影响网格特征的可靠感知。

Method: 使用加权集合差异度量和两阶段推理流程，在九种文本和图像模态中分离感知与推理，比较不同表示方式的效果。

Result: 结构化文本在稀疏特征坐标上表现精确，图像能捕捉2D形状但对分辨率敏感，结合两种模态可提升执行效果（约8个感知点；约0.20中位数相似度）。
...
### [154] [FOOTPASS: A Multi-Modal Multi-Agent Tactical Context Dataset for Play-by-Play Action Spotting in Soccer Broadcast Videos](https://arxiv.org/abs/2511.16183)
*Jeremie Ochin,Raphael Chekroun,Bogdan Stanciulescu,Sotiris Manitsaris*

Main category: cs.AI

TL;DR: 提出了FOOTPASS数据集，这是首个在足球比赛中进行逐场动作识别的基准，结合计算机视觉输出和战术知识来生成可靠的逐场数据流。


<details>
  <summary>Details</summary>
Motivation: 当前动作识别方法不足以构建可靠的逐场数据，通常只能辅助而非完全自动化标注。战术建模、轨迹预测和性能分析的研究进展表明可以利用战术知识作为先验来支持基于计算机视觉的预测。

Method: 引入FOOTPASS数据集，支持多模态、多智能体战术背景下的球员中心动作识别，结合计算机视觉任务输出（如跟踪、识别）和足球战术规律知识。

Result: 创建了首个覆盖整场足球比赛的逐场动作识别基准数据集，为数据驱动的体育分析提供重要输入。

Conclusion: FOOTPASS数据集通过整合计算机视觉和战术知识，实现了更自动化和可靠的逐场数据提取，为体育分析应用奠定了基础。
...
### [155] [Green Resilience of Cyber-Physical Systems: Doctoral Dissertation](https://arxiv.org/abs/2511.16593)
*Diaeddin Rimawi*

Main category: cs.SE

TL;DR: 该研究提出GResilience框架来平衡在线协作AI系统(OL-CAIS)的弹性与绿色性，通过多目标优化、博弈论和强化学习策略实现绿色恢复，减少恢复时间并降低人类依赖。


<details>
  <summary>Details</summary>
Motivation: OL-CAIS作为网络物理系统，在协作学习过程中容易受到破坏性事件影响，需要在恢复性能的同时限制能耗，因此需要解决弹性与绿色性之间的权衡问题。

Method: 建立OL-CAIS三状态行为模型（稳定、破坏、最终），开发GResilience框架，包含多目标优化（单代理）、博弈论决策（双代理）和强化学习（RL代理）三种恢复策略，并设计量化弹性与绿色性的测量框架。

Result: 实验表明弹性模型能捕捉性能转换，GResilience策略缩短恢复时间、稳定性能并减少人类依赖，RL代理策略效果最佳但CO2排放略有增加，容器化执行可减少一半CO2排放。

Conclusion: 本研究提供了确保OL-CAIS绿色恢复的模型、指标和策略，有效平衡了系统弹性与能源效率。
...
TL;DR: MiMo-Embodied是首个在自动驾驶和具身AI领域都取得SOTA性能的跨具身基础模型，在29个基准测试中创下新纪录


<details>
  <summary>Details</summary>
Motivation: 探索自动驾驶和具身AI两个领域之间的正迁移效应，通过统一模型架构实现跨领域的性能提升

Method: 采用多阶段学习、精心构建的数据集以及思维链/强化学习微调方法

Result: 在17个具身AI基准测试和12个自动驾驶基准测试中均显著超越现有开源、闭源和专用基线模型

Conclusion: 两个领域存在强正迁移效应，能够相互促进，为跨具身智能研究提供了新的方向
...
TL;DR: 该论文提出了一种基于动物窥视行为的合成孔径感知方法，通过机器人侧向运动模拟宽合成孔径，计算合成浅景深图像来消除遮挡影响，实现实时高分辨率感知。


<details>
  <summary>Details</summary>
Motivation: 解决机器人视觉中部分遮挡的基本限制，传统相机小孔径大景深导致前景障碍物和背景物体都清晰，遮挡关键场景信息。

Method: 让机器人执行窥视运动，相机描述宽合成孔径，计算集成捕获图像合成极浅景深图像，有效模糊遮挡元素同时使背景清晰聚焦。

Result: 该方法不仅恢复基本场景理解，还赋能大型多模态模型的高级视觉推理，对遮挡具有鲁棒性，计算高效，可在任何移动机器人上立即部署。

Conclusion: 将动物行为与机器人技术结合，表明用于合成孔径感知的窥视运动是在复杂杂乱环境中实现高级场景理解的关键。
