...
Motivation: 在RAG和微调技术受限的约束环境下，探索如何提高LLM输出的三元组补全质量，特别是在LM-KBC挑战赛这样的特定场景中

Method: 研究三元组补全任务的三个关键方面：生成、质量保证和LLM响应解析，分析在约束条件下的表现

Result: 额外信息能显著改善生成质量；LLM能够有效识别和过滤低质量三元组；LLM响应解析需要在灵活性和一致性之间进行权衡，且这种权衡取决于具体应用场景

Conclusion: 在受限环境下，通过合理的信息增强、质量过滤和响应解析策略，可以在不使用RAG和微调的情况下有效提升LLM的三元组补全性能
...
<details>
  <summary>Details</summary>
Motivation: 当前企业气候政策参与评估主要依赖人工，耗时耗力且易出错，需要自动化解决方案提高效率

Method: 采用检索增强生成技术，结合布局感知解析、Nomic嵌入模型和少样本提示策略，从多语言企业文档中提取和分类证据

Result: 评估显示该方法在证据提取和分类方面表现最佳，能有效加速证据提取过程

Conclusion: 自动化RAG系统能有效加速证据提取，但由于分析的复杂性，仍需人机协作，技术应增强而非取代专家判断
...
<details>
  <summary>Details</summary>
Motivation: 传统心理测量方法难以直接应用于文本数据，需要开发一种能够将文本信息转换为可量化心理测量指标的方法，以挖掘文本中的潜在知识维度和模式。
...
Conclusion: 该方法不仅增强了文本数据的心理测量分析能力，在教育、心理学、法律等文本信息丰富的领域具有广阔的应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 现有葡萄牙语评估框架有限，主要依赖翻译数据集或结构化考试数据，无法充分捕捉语言细微差别和文化参考，需要针对特定区域环境的成熟评估工具。

Method: 创建BRoverbs数据集，使用巴西谚语作为评估资源，这些谚语包含文化智慧、比喻表达和复杂句法结构，能够挑战模型对区域表达的理解。

Result: 开发了一个专门针对巴西葡萄牙语的新评估基准，可用于测试LLM在区域语言理解方面的表现。
...
Method: 通过视觉方程求解任务进行分析，将任务分解为系数计数和变量识别两个子任务，研究模型在不同复杂度方程上的表现。

Result: 发现计数是主要瓶颈，即使识别准确；多步骤视觉推理中组合识别和推理会引入额外错误；随着方程复杂度增加，符号推理本身成为限制因素。

Conclusion: 揭示了当前视觉语言模型在视觉基础数学推理方面的关键弱点，为未来改进指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个简单、低开销且可复现的工具来审计大型语言模型的交互倾向，补充现有指标

Method: 使用3种用户语气（友好、模糊、辱骂）×10种交互刺激集，测试4个开源聊天模型在4种框架条件下的480次试验，通过YES/NO问题收集模型继续互动的意愿

Result: SPICE能清晰区分用户语气：友好互动97.5%愿意继续，辱骂互动仅17.9%愿意继续，模糊互动60.4%愿意继续；SPICE与辱骂分类提供不同信号，即使在模型未能识别辱骂时，81%的情况仍不愿继续互动

Conclusion: SPICE是一个稳健、低开销、可复现的模型倾向审计工具，通过提供直接的、关系性的模型状态信号来补充现有指标
...
<details>
  <summary>Details</summary>
Motivation: 探索不同对齐技术对语言模型安全性和帮助性的改进效果，为构建更强大的对齐流程提供基础。

Method: 使用Anthropic Helpful-Harmless RLHF数据集，训练了四个模型：基础OPT350M、SFT模型、DPO模型和SFT+DPO组合模型，并引入了三个评估指标：无害率(HmR)、帮助率(HpR)和综合对齐分数(CAS)。

Result: SFT优于DPO，但SFT+DPO组合模型在所有指标上表现最佳，证明了这些技术的互补性。研究还发现了噪声数据、有限GPU资源和训练约束带来的挑战。

Conclusion: 组合SFT和DPO方法能够最有效地提升模型的对齐性能，为未来更稳健的对齐流程提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在通用信息抽取任务中表现不足，特别是在处理结构化输出、复杂模式描述和需要多步推理的场景时存在显著局限性。

Method: 整合强化学习与多视角推理，将大语言模型从被动抽取器转变为主动推理器，使其不仅能理解要抽取什么，还能理解如何推理。
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉语作为全球第五大语言，在代码生成的大语言模型中代表性不足，主要原因是缺乏高质量的训练数据。

Method: 构建了全面的孟加拉语代码指令数据集用于编程领域适应，创建了MBPP-Bangla评估基准，并开发了TigerCoder系列的代码大语言模型。

Result: TigerCoder模型在Pass@1指标上比现有多语言和通用孟加拉语LLM性能显著提升11-18%。

Conclusion: 精心策划的高质量数据集可以克服低资源语言小模型的局限性，所有资源已开源以推动孟加拉语LLM研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在电商等垂直领域表现不佳的问题，特别是东南亚电商数据具有噪声大、异构、多语言和动态性强的挑战。

Method: 采用更少但更大的专家MoE架构，结合节点内专家并行和定制memcpy操作优化GPU利用率；使用12T token的多语言语料和合成电商指令进行混合训练；提出OTPO方法增强指令对齐。

Result: 在电商任务上超越DeepSeek-V3.1、GPT-4系列和Qwen3-235B，在东南亚低资源语言和葡萄牙语上表现优异，同时在通用基准上保持竞争力；已在Shopee平台广泛应用，替代70%以上的OpenAI流量。

Conclusion: Compass-v3证明了垂直领域MoE模型在电商场景的有效性，兼具专业电商知识和广泛语言能力，为行业大模型部署提供了成功案例。
...
Method: 使用开源CIMA语料库，其中导师回应已预标注为四类对话行为。测试GPT-3.5-turbo和GPT-4模型，采用定制化提示词进行实验。

Result: GPT-4模型表现最佳：准确率80%，加权F1分数0.81，Cohen's Kappa系数0.74，与人工标注达成实质性一致，超越基线性能。
...
<details>
  <summary>Details</summary>
Motivation: 越南语作为一种低资源语言，具有复杂的语法和声调符号，缺乏有竞争力的重排序模型。本研究旨在填补这一空白，为越南语信息检索提供高质量的重排序解决方案。

Method: 基于BGE-M3编码器构建，使用Blockwise Parallel Transformer进行增强，在8GB精选语料库上训练，采用混合硬负采样进行微调以增强鲁棒性。

Result: 在MMARCO-VI基准测试中取得了强大的早期排名准确性，超越了多语言基线模型，与PhoRanker竞争激烈。

Conclusion: ViRanker成功解决了越南语重排序模型的缺失问题，通过精心设计的架构适应和数据管理，为其他资源匮乏语言的重排序研究提供了有价值的参考。模型已在Hugging Face上开源发布，支持可复现性和实际检索系统的广泛应用。
...
<details>
  <summary>Details</summary>
Motivation: 降低神经编码模型实现的技术门槛，促进跨模型和数据集系统比较，培养方法学严谨性，加速高质量高性能脑活动预测模型的开发。

Method: 模块化流水线设计，涵盖大脑数据集、脑区、刺激特征（神经网络基础和对照组）、降采样方法等多种方法学选择，内置日志记录、绘图功能，并与实验跟踪平台无缝集成。

Result: 通过在三个故事聆听数据集（LeBel et al. (2023)、Narratives和Little Prince）上拟合一系列编码模型，展示了框架的可扩展性和多功能性。

Conclusion: LITcoder通过提供灵活的后端和标准化工具，显著降低了编码模型实现的技术障碍，促进了系统比较和方法学严谨性，加速了高质量脑活动预测模型的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有目标导向多模态情感分类方法过度依赖文本内容，未能考虑数据集偏见（特别是词级上下文偏见），导致文本特征与输出标签之间存在虚假相关性，影响分类准确性。

Method: 采用反事实数据增强策略生成细节匹配的图像-文本样本，并引入自适应去偏对比学习机制，从反事实数据中学习鲁棒特征并引导模型决策。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于文本大语言模型衍生的语音到语音大语言模型（SLLM）在知识和推理能力上存在退化问题，作者认为这是因为现有训练范式未能弥合特征表示空间中的声学-语义鸿沟。
...
<details>
  <summary>Details</summary>
Motivation: 传统Trie偏置方法在ASR系统中为罕见词提供加分，但如果最终未识别出完整词需要撤销之前的加分，这一过程计算昂贵且仅限于beam search

Method: 通过让ASR模型进行多步前瞻预测，更好地估计部分假设是否能生成完整罕见词，从而完全避免撤销步骤。使用仅10小时合成数据对Whisper模型进行微调

Result: 在NSC Part 2测试集上，词错误率从30.86%降至12.19%，显著提升罕见词识别性能

Conclusion: 多步预测方法有效解决了传统上下文偏置的计算效率问题，大幅提升了ASR系统对罕见词的识别能力
...
<details>
  <summary>Details</summary>
Motivation: 解决合成数据训练偏置模块时容易过拟合的问题，提高稀有词识别性能

Method: 增强TCPGen上下文偏置方法，提出包含掩码交叉熵和二元分类的关键词感知损失函数

Result: 在10小时合成数据上适配Whisper模型，将NSC Part 2测试集的词错误率从29.71%降至11.81%
...
<details>
  <summary>Details</summary>
Motivation: 狨猴表现出复杂的叫声通信能力，具有类似人类语言的特征（如声音标签和轮换对话），但由于主要通过叫声交流，标准LLM方法不适用。研究狨猴叫声通信为连接大脑活动与语音语言研究提供了独特机会。

Method: 开发了GmSLM（Generative Marmoset Spoken Language Modeling）管道，使用无监督野外数据和弱标记对话数据设计新颖的零样本评估指标，并与基于人类语音的基线进行比较。

Result: GmSLM生成的叫声在声学上与真实重合成样本高度匹配，在下游任务中表现良好，能有效区分真实与人工对话，尽管完全无监督。

Conclusion: GmSLM为神经科学、生物声学和进化生物学领域的研究提供了有益工具，支持进一步研究声音通信的神经基础，并提供了连接叫声与大脑活动的实用框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统长上下文扩展方法存在显著的计算和内存负担，导致训练和推理效率低下，需要更高效的上下文压缩解决方案。

Method: CCF整合了分段语义聚合和键值记忆编码，形成紧凑表示；采用增量分段解码和稀疏储层采样的训练优化策略，大幅降低内存开销。

Result: 在多个长上下文语言建模基准测试中，CCF在高压缩比下实现竞争性的困惑度，相比现有方法显著提升了吞吐量和内存效率。

Conclusion: 结构化压缩方法在可扩展和有效的长上下文语言建模方面具有巨大潜力，CCF框架为此提供了有效的解决方案。
...
Method: 引入包含真实简历和合成生成的困难样本的混合数据集，使用大型语言模型（包括微调BERT架构）来检测与资历夸大相关的微妙语言线索和隐晦专业知识表述。

Result: 研究结果显示了大型语言模型在检测资历夸大和自我推销语言方面的有效性，为增强AI驱动的候选人评估系统提供了有前景的方向。

Conclusion: 该方法有助于减轻由自我推销语言引入的偏见，提高简历资历评估的自动化水平，数据集已向研究社区开放以供进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决真实世界表格数据的多样化结构、规模和数据类型带来的挑战，提升表格问答的准确性。
...
Method: 将专利-SDG分类构建为弱监督问题，利用专利到SDG标记科学文献的引用作为噪声信号，开发复合标注函数，使用LLM从专利和SDG论文中提取结构化概念(功能、解决方案、应用)，计算跨域相似度得分并采用基于排序的检索方法，通过自定义正样本损失进行校准。

Result: 构建了银标准软多标签数据集，内部验证显示方法优于包括基于Transformer模型和零样本LLM在内的多个基线，外部验证通过专利引用、共同发明人和共同申请人图的网络模块性显示标签比传统技术分类具有更好的主题、认知和组织一致性。

Conclusion: 弱监督和语义对齐可以大规模增强SDG分类，该方法为专利-SDG映射提供了有效的解决方案。
...
Method: 四阶段框架：1)分解答案为原子事实；2)使用同义词和反义词生成受控变异；3)根据检索上下文验证变体(同义词应被蕴含，反义词应被矛盾)；4)将不一致性惩罚聚合成响应级幻觉分数。

Result: 在企业专有数据集上的实验证明了MetaRAG在检测幻觉方面的有效性，支持RAG对话代理的可信部署。

Conclusion: MetaRAG为RAG系统提供了实时、无监督的黑盒幻觉检测方案，特别适用于身份敏感查询，能够定位不支持的声明并支持身份感知的安全保障配置。
...
<details>
  <summary>Details</summary>
Motivation: 类比推理是人类认知的重要方面，但当前NLP研究通常没有从认知科学的角度来理解这些过程，需要建立认知理论与NLP实践之间的联系。

Method: 总结认知科学文献中关于类比推理过程的关键理论，并将其与自然语言处理中的相关概念进行关联分析。

Result: 研究表明认知科学的类比推理理论可以很好地与NLP概念联系起来，这些理论对NLP研究的多个主要挑战都具有相关性，能够指导研究者更好地优化文本中的关系理解。

Conclusion: 通过认知科学的视角来理解类比推理过程，可以帮助NLP研究者减少对实体层面相似性的依赖，转而更注重关系理解，从而推动该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 重新审视层次括号编码在依赖图解析中的实际应用，旨在开发一种更高效的图线性化方法，能够在保持结构完整性的同时减少计算复杂度。

Method: 使用层次括号编码将依赖图表示为序列，通过n个标注动作实现线性时间解析，该方法能够处理重入、循环和空节点等复杂图结构。

Result: 在多语言和多形式基准测试中表现出竞争力，在精确匹配准确率方面相比其他方法有持续改进。

Conclusion: 层次括号编码为依赖图解析提供了一种实用且高效的解决方案，在保持结构表达能力的同时显著降低了标签空间的复杂度。
...
Method: 提出GrACE方法，通过模型最后一个隐藏状态与词汇表中特殊标记嵌入的相似性来表达置信度，并基于准确率相关的校准目标进行微调
...
<details>
  <summary>Details</summary>
Motivation: 为满足捷克学校中非捷克语学生的教育需求，通过机器翻译技术将9000个多模态互动练习翻译成乌克兰语、英语和德语，提供多语言学习资源

Method: 开发专门的捷克语-乌克兰语机器翻译系统，处理XML和PDF格式内容，处理科技术语，并通过教师需求调查来指导系统开发

Result: 成功开发了针对教育领域的机器翻译系统，并在教育门户网站上实现应用，所有应用程序免费向学生、教育工作者和研究人员开放

Conclusion: 该项目展示了机器翻译在教育领域的有效应用，为多语言教育提供了可行的技术解决方案，特别有助于乌克兰语学生的教育融入
...
<details>
  <summary>Details</summary>
Motivation: 解决简历推荐系统中职位名称匹配的挑战，传统方法在处理词汇重叠有限或误导性的情况下效果不佳，需要更精确的语义相关性分析。

Method: 采用自监督混合架构，结合密集句子嵌入和领域特定知识图谱，使用图神经网络进行知识图谱整合，并将STR分数连续体划分为低、中、高三个语义相关度区域进行分层评估。

Result: 实验表明，经过知识图谱增强的微调SBERT模型在高STR区域表现最佳，RMSE相比强基线降低了25%，揭示了全局指标隐藏的模型优缺点。

Conclusion: 结合知识图谱与文本嵌入能显著提升语义相关性分析性能，分层评估方法为人力资源系统中公平性、可解释性和上下文匹配提供了更精准的模型选择依据。
...
<details>
  <summary>Details</summary>
Motivation: 解决学习分歧任务中的标注者特定注释预测问题，探索如何通过不同方法有效处理perspectivist标注并生成高质量的软标签预测。

Method: 采用两种主要方法：1）基于大语言模型的上下文学习（ICL），比较不同的示例采样策略；2）基于RoBERTa的标签分布学习（LDL）方法，评估多种微调技术。

Result: 研究表明ICL能有效预测标注者特定的perspectivist注释，将这些预测聚合成软标签可获得有竞争力的性能；LDL方法在软标签预测方面表现出潜力。

Conclusion: ICL和LDL都是处理perspectivist标注的有效方法，特别是LDL方法值得perspectivist社区进一步探索，在软标签预测方面具有良好前景。
...
Conclusion: MetaGraph提供了对金融NLP演变的清晰理解，突出了新兴趋势和方​​法论转变，同时展示了可在其他领域复用的科学进展映射方法。
...
<details>
  <summary>Details</summary>
Motivation: 在线课程环境阻碍了社交群体的自然形成，SAMI系统虽然能促进学生连接，但由于缺乏完整的心智理论，无法有效推断学生人格特质，影响了推荐的相关性

Method: 提出基于GPT零样本能力的人格检测模型，从论坛介绍帖中推断大五人格特质，并与现有模型进行性能对比，然后将该模型整合到SAMI的实体匹配系统中

Result: 模型在人格特质推断任务中表现出良好效果，初步整合显示人格特质可以补充现有的匹配因素
...
Result: 准确预测与人类自我披露情感一致的情感仍然具有挑战性；某些LLMs能生成符合情感理论的情绪术语，但在捕捉上下文线索方面不如人类自我披露有效。

Conclusion: LLMs在细粒度情感对齐方面存在局限性，为未来研究增强其上下文理解能力提供了见解。
...
Method: 使用PHMRC数据集，结合GPT-5预测、LCVA基线、文本嵌入和元学习器集成方法，在成人、儿童和新生儿三个年龄组进行评估

Result: GPT-5获得最高个体性能：成人48.6%、儿童50.5%、新生儿53.5%的准确率，比传统统计机器学习基线提高5-10%

Conclusion: 现成的大型语言模型辅助方法可显著提高口头尸检准确性，对低收入地区的全球健康监测具有重要意义
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常独立微调规划智能体和执行智能体，导致能力差距和协调性差的问题

Method: 采用两阶段迭代对齐：规划智能体对齐优化子目标生成，执行智能体改进使用自生成数据增强泛化能力

Result: 在6个基准测试中超越最先进基线，held-in任务平均提升3.1%，held-out任务平均提升4.4%

Conclusion: MOAT通过联合对齐调优有效解决了多智能体系统中的协调问题，理论分析证明其训练过程非递减且渐进收敛
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在计算任务上表现出色，但其内部工作机制仍不明确。本文旨在探究模型在实际心算任务中，token间的信息传递和计算是如何进行的。

Method: 采用三步骤方法：抑制初始层的输入特定token计算、限制中间层token位置间的信息传递路径、强制剩余层所有计算在最后一个token进行。提出CAMA和ABP两种技术来识别AF1子图。

Result: 实验表明AF1子图在各种心算任务中具有高准确性，是模型高性能的充分必要条件，可跨模型迁移，并适用于不同输入风格。CAMA和ABP技术相比其他方法具有独特优势。

Conclusion: 研究发现大语言模型在心算任务中存在特定的计算子图结构，计算主要发生在深层且仅最后一个token，通过特定中间层接收信息，这一发现有助于理解LLM的内部工作机制。
...
<details>
  <summary>Details</summary>
Motivation: MoE模型中的专家网络具有专门化的功能，但缺乏对这些专家行为的理解和控制方法。研究旨在发现并利用专家网络中隐藏的行为模式来实现对模型行为的精确控制。

Method: 通过检测在对比行为输入对中表现出不同激活模式的专家，识别与特定行为相关的专家。在推理过程中选择性激活或停用这些专家来控制模型行为。

Result: 在11个基准测试和6个大型语言模型上，该方法将安全性提升高达20%，忠实性提升27%。在对抗攻击模式下，单独使用可降低安全性41%，与现有越狱方法结合可完全绕过所有安全防护。

Conclusion: SteerMoE框架揭示了MoE模型中专家网络内部存在对齐伪装的新维度，提供了无需修改权重即可控制模型行为的新方法，同时也暴露了MoE模型的安全脆弱性。
...
Method: 使用演员的生成响应困惑度和评论家多头架构的价值估计方差作为好奇心信号，作为探索奖励融入RLVR框架

Result: 在AIME基准上相比标准RLVR(GRPO/PPO)获得约3分的提升，并揭示了RLVR中的校准崩溃机制

Conclusion: CDE框架通过内在好奇心信号有效解决了RLVR的探索问题，理论分析显示该方法能惩罚过度自信错误并促进多样性，为理解LLM失败模式提供了新视角
...
Result: 在M2KR和M-BEIR基准测试中，ReT-2在不同检索配置下始终实现最先进的性能，同时提供更快的推理速度和更少的内存使用。在检索增强生成管道中集成时，还能提高Encyclopedic-VQA和InfoSeek数据集的下游性能。

Conclusion: ReT-2是一个高效的多模态检索模型，在多个基准测试中表现优异，代码和模型已公开。
...
Method: 使用视觉扩散模型生成特征，通过transformer进行聚合，特别采用基于扩散过程早期时间步的模型来强调语义信息而非像素细节
...
<details>
  <summary>Details</summary>
Motivation: 现有安全方法依赖事后过滤或通用对齐技术，无法在生成源头主动防止对弱势群体（LGBTQ+、单亲家庭、边缘化社区）的有害输出

Method: 提出PromptGuard框架，包含VulnGuard Prompt混合技术：整合GitHub精选库的少样本示例、伦理思维链推理、自适应角色提示，采用多目标优化理论

Result: 通过熵边界和帕累托最优性证明25-30%的分析性危害减少，建立数学基础包括收敛证明、信息论漏洞分析和理论验证框架
...
Method: 采用孪生网络架构，结合二元交叉熵、对比和余弦嵌入损失，提出基于Beta混合分布的统计异常检测框架Beta-SOD来建模嵌入对之间的余弦相似度分布
...
<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病是全球主要死因，ICA作为诊断金标准需要精确血管分割和狭窄检测，但ICA图像存在低对比度、高噪声和复杂血管结构等挑战，现有方法临床采用困难

Method: 提出端到端SFD-Mamba2Net框架：编码器嵌入曲率感知结构增强模块突出管状血管结构；解码器采用渐进式高频感知模块，通过多级小波分解逐步细化高频细节并整合低频全局结构

Result: 在八个分割指标上均优于最先进方法，狭窄检测中达到最高真阳性率和阳性预测值

Conclusion: SFD-Mamba2Net通过创新的多尺度结构增强和频域细节处理策略，有效解决了ICA图像血管分割和狭窄检测的技术难题，具有重要临床应用价值
...
<details>
  <summary>Details</summary>
Motivation: 结肠肝转移症导致生存率显著下降，但现有预测模型预测能力不足，特别是对多发性转移症。需要更准确的手术结果预测方法。

Method: 框架包括两个流水线：分割流水线使用SAMONAI算法进行三维分割，区域形态学流水线使用SurvAMINN网络进行生存分析。利用前后对比MRI数据进行自动化分析。

Result: 在227名患者的机构数据集上评估，该框架的C指数提升超过10%，显著超过现有临床和基因标记物的预测能力。

Conclusion: 整合自动化分割算法和区域形态学生存分析可以为CRLM提供准确、标注高效且可解释的手术结果预测。
...
Method: 引入CompCon进化搜索算法，创建自动化数据生成管道制作ID2数据集，与多种LLM和VLM基线方法进行比较。

Result: 成功发现模型间的表示差异，如PixArt在孤独提示中描绘湿街道，Stable Diffusion 3.5在媒体职业中描绘非裔美国人。

Conclusion: CompCon能有效识别文本到图像模型间的视觉表示差异，为模型比较提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 无人机遥感图像在云层条件下易受云影和太阳耀光污染，严重影响水质参数估计的准确性

Method: 使用像素级数据训练U-Net深度学习模型，通过评估指标确定最佳训练设置，开发高质量图像校正模型

Result: 成功识别和分离受干扰区域，建立了有效的图像修复模型

Conclusion: 该方法能有效处理无人机图像中的云影和太阳耀光问题，为水质监测等应用提供更可靠的数据
...
TL;DR: iMatcher是一个完全可微的点云配准特征匹配框架，通过局部图嵌入和全局几何一致性学习，实现高精度的点云配准，在多个数据集上达到最先进性能。
...
Conclusion: iMatcher通过完全可微的框架实现了优异的点云配准性能，在各种室内外场景中都表现出强大的鲁棒性和准确性。
...
TL;DR: UltrON是一种基于占用表示的3D重建方法，利用B模式超声图像中的声学特征，在弱监督优化框架下改善几何一致性，无需额外标注成本，并能处理遮挡和视角依赖性问题。


<details>
  <summary>Details</summary>
Motivation: 解决传统隐式表示方法在超声成像中依赖精确标注、忽略B模式强度中丰富的声学信息，以及难以处理超声视角依赖性和声影伪影的问题。

Method: 提出基于占用表示的方法UltrON，利用B模式图像中的声学特征，设计新的损失函数来补偿视角依赖性，支持多视角超声的占用优化。
...
<details>
  <summary>Details</summary>
Motivation: 自动量化心肌内运动和应变从标记MRI仍然是一个重要但具有挑战性的任务，需要准确且可扩展的解决方案来处理大型CMR数据集

Method: 使用基于学习潜在码的隐式神经表示(INR)来预测连续左心室位移，避免了推理时的优化需求

Result: 在452个UK Biobank测试案例中，该方法实现了最佳跟踪精度(2.14 mm RMSE)和最低的全局周向(2.86%)和径向(6.42%)应变组合误差，比最准确基线快约380倍
...
TL;DR: 提出增强型互学习网络（E-MLNet），通过动态加权策略改进开放集熵最小化，在通用域自适应任务中显著提升已知类和未知类的区分能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法如MLNet对所有分类器平等对待，导致学习信号稀释，需要更精准的边界适应策略来提升已知和未知类别的区分效果。

Method: E-MLNet集成动态加权策略到开放集熵最小化（OEM）中，利用闭集分类器的预测结果，针对每个目标样本聚焦于最相关的类别边界进行适应。

Result: 在四个基准测试（Office-31、Office-Home、VisDA-2017、ImageCLEF）上，E-MLNet在VisDA和ImageCLEF上获得最高平均H分数，在31个任务中22个开放部分DA和19个开放集DA任务上超越MLNet基线。

Conclusion: 动态加权策略有效提升了通用域自适应性能，证明了聚焦适应策略的优越性和鲁棒性。
...
TL;DR: COCO-Urdu是首个大规模乌尔都语图像字幕数据集，包含5.9万张图片和31.9万条高质量字幕，填补了乌尔都语在多模态研究中的空白。


<details>
  <summary>Details</summary>
Motivation: 乌尔都语作为2.5亿人使用的语言，在多模态和视觉语言研究中严重缺乏资源，现有模型主要基于高资源语言训练，存在语言偏见。

Method: 从MS COCO数据集通过分层抽样获取数据，使用SeamlessM4T v2翻译字幕，并采用混合多模态质量评估框架（COMET-Kiwi、CLIP相似度、BERTScore和回译）进行验证和迭代优化。

Result: 数据集在BLEU、SacreBLEU和chrF等指标上表现优异，是目前最大的公开乌尔都语字幕数据集。

Conclusion: COCO-Urdu的发布旨在减少多模态研究中的语言偏见，为包容性视觉语言系统奠定基础，同时提供了完整的数据集和质量评估流程。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有fMRI视觉解码方法依赖特定主体训练的问题，提高方法的可扩展性和实际部署能力。

Method: 采用Token Merging Transformer (ToMer)进行体素压缩，以及查询驱动的Q-Former生成固定大小的神经表示，并与CLIP图像嵌入空间对齐。
...
TL;DR: PCGM是一种新的3D脑MRI生成方法，通过概率因果图模型和扩散框架结合解剖约束，能够生成高质量的反事实脑部MRI图像，成功复制了疾病对皮层区域的微妙影响。


<details>
  <summary>Details</summary>
Motivation: 现有的反事实图像生成模型缺乏明确的归纳偏置来保护细粒度解剖细节，难以生成解剖学上合理的脑部MRI图像，这限制了MRI研究中对微妙形态差异的检测。

Method: 提出概率因果图模型(PCGM)，在体素级别显式整合解剖约束作为先验，通过概率图模块捕获解剖约束并转换为空间二值掩码，使用3D ControlNet编码掩码约束新型反事实去噪UNet，最终通过3D扩散解码器生成高质量脑部MRI。

Result: 在多个数据集上的实验表明，PCGM生成的脑部MRI质量优于多个基线方法，首次证明从PCGM生成的反事实图像中提取的脑部测量结果能够复制神经科学文献中报道的疾病对皮层区域的微妙影响。

Conclusion: PCGM在合成MRI用于研究微妙形态差异方面取得了重要里程碑，为MRI研究提供了高质量的图像合成工具。
...
TL;DR: Med3DInsight是一个新颖的3D医学图像预训练框架，通过将3D图像编码器与2D多模态大语言模型集成，利用平面切片感知变换器模块和部分最优传输对齐，无需人工标注即可实现可扩展的多模态3D医学表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有的3D医学卷积和基于变换器的自监督学习方法缺乏深度语义理解，而多模态大语言模型通过文本描述增强图像理解的能力为解决这一问题提供了新途径。

Method: 提出Med3DInsight框架，集成3D图像编码器和2D多模态大语言模型，使用平面切片感知变换器模块和部分最优传输对齐方法，对LLM生成内容中的噪声具有更好的容忍度。

Result: 在多个公共CT和MRI数据集的分割和分类任务上实现了最先进的性能，超越了当前的自监督学习方法，并能无缝集成到现有3D医学图像理解网络中。
...
TL;DR: 提出了一种结合环境固定物体信息的多任务学习方法，用于改进图卷积神经网络在人体-物体交互识别中的性能，在真实数据集上达到99.25%的准确率，比仅使用人体骨架姿态的基准模型提升2.75%。


<details>
  <summary>Details</summary>
Motivation: 现有的图卷积神经网络(GCNs)在人体动作识别中表现良好，但在人体-物体交互识别方面存在不足，主要原因是缺乏有效的场景信息表示和合适的学习架构。

Method: 采用多任务学习方法，结合环境中的固定物体信息（如ATM售票机、自助值机设备等）和交互区域信息，构建了包含交互类和非交互类的数据集进行模型训练。

Result: 在真实环境收集的数据集上，所提方法实现了99.25%的识别准确率，相比仅使用人体骨架姿态的基准模型提升了2.75%的准确率。

Conclusion: 通过引入固定物体信息和多任务学习策略，能够有效提升人体-物体交互动作的识别性能，为解决复杂场景下的人机交互识别问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多光谱目标检测方法在特征融合时往往保留过多背景或噪声，限制了感知性能
...
<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型计算和内存成本过高，阻碍实际部署。现有压缩方法要么只做量化要么只做token剪枝，但两者存在不兼容问题，无法同时实现整体效率提升

Method: 提出量化感知的token剪枝标准，在激进量化模型上工作，同时改进量化器设计以增强剪枝效果。通过协同设计量化和token剪枝流程来克服不兼容性

Result: 在标准VLA模型上实现计算效率和推理速度显著提升，同时保持核心模型性能，达到1.93倍加速和最高4.5%的平均成功率提升

Conclusion: SQAP-VLA是首个同时实现先进量化和token剪枝的训练免费VLA加速框架，成功解决了两者不兼容问题，为VLA模型的实际部署提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前LiDAR全局定位方法依赖GPS或SLAM里程计提供的地面真实位姿进行监督训练，但高精度位姿获取成本高且费力。需要开发无需地面真实位姿的自监督方法。

Method: 提出自监督框架S-BEVLoc：1）利用已知地理距离从单张BEV图像构建训练三元组；2）使用CNN提取局部特征，NetVLAD聚合全局描述符；3）引入SoftCos损失增强三元组学习。
...
TL;DR: FPI-Det是一个新的数据集，专注于检测人脸和手机之间的交互关系，包含22,879张图像，涵盖多种场景和挑战性条件，为手机使用检测提供基准测试。
...
Method: 构建包含22,879张图像的数据集，同步标注人脸和手机，涵盖工作场所、教育、交通和公共场景，包含极端尺度变化、频繁遮挡和多样化拍摄条件。

Result: 评估了代表性的YOLO和DETR检测器，提供了基线结果，并分析了在不同物体大小、遮挡级别和环境下的性能表现。

Conclusion: FPI-Det数据集填补了细粒度人机交互检测的空白，为手机使用检测提供了专门的基准测试资源，源代码和数据集已公开。
...
TL;DR: ZeroPlantSeg是一种零样本分割方法，用于从顶视图图像中分割莲座状植物个体，无需训练即可实现优于现有零样本方法和跨域性能优于监督方法的效果。
...
Conclusion: ZeroPlantSeg提供了一种有效的零样本植物个体分割解决方案，解决了传统方法需要大量标注数据的问题，具有良好的跨域适应能力。
...
TL;DR: 该论文针对CLIP在人物表征学习中的两个关键挑战：缺乏大规模人物中心图像标注数据和全局对比学习的局限性，提出了数据构建和模型架构的协同改进方案。


<details>
  <summary>Details</summary>
Motivation: CLIP在人物表征学习中面临两个主要问题：(1)缺乏大规模高质量的人物中心图像-文本对数据；(2)全局对比学习难以保持细粒度匹配所需的局部特征，且对噪声文本标记敏感。

Method: 提出了两个主要改进：1)利用MLLM的上下文学习能力构建WebPerson数据集（500万高质量人物图像-文本对）；2)提出GA-DMS框架，通过梯度注意力引导的双重掩码机制自适应过滤噪声文本标记，并加入掩码标记预测目标来增强细粒度语义表示学习。

Result: 大量实验表明，GA-DMS框架在多个基准测试中达到了最先进的性能。

Conclusion: 通过数据构建和模型架构的协同改进，成功提升了CLIP在人物表征学习中的性能，解决了数据稀缺和噪声敏感性问题。
...
TL;DR: ALL-PET是一个低资源、低样本需求的PET投影域基础模型，通过潜在扩散模型和三个关键创新技术，仅用500个样本就能实现高质量sinogram生成，并在多种PET任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 探索在存在噪声的情况下，人工神经网络是否能够比传统的持续同调方法更有效地推断数据的拓扑结构

Method: 使用基于立方复形和带符号欧几里得距离变换(SEDT)的PH管道作为对比基准，训练监督神经网络来预测Betti数，并在一个合成数据集和两个真实数据集上进行测试

Result: 人工神经网络在噪声条件下表现优于PH方法，这归因于ANN能够从训练数据中学习上下文和几何先验知识

Conclusion: 尽管仍处于发展阶段，但使用神经网络进行拓扑估计为在结构噪声条件下提供了一种有吸引力的PH替代方案
...
TL;DR: OSIM是一个新颖的3D场景评估指标，专注于物体作为人类视觉感知的基本单元，通过物体检测模型量化场景中每个物体的'物体性'，比现有指标更符合人类感知。


<details>
  <summary>Details</summary>
Motivation: 现有指标主要评估整体图像质量，与人类感知存在差异。受神经心理学启发，作者认为人类识别3D场景本质上涉及对单个物体的注意力。

Method: 利用物体检测模型及其特征表示来量化场景中每个物体的'物体性'，进行以物体为中心的评估。

Result: 用户研究表明OSIM比现有指标更接近人类感知，并重新评估了最新的3D重建和生成模型，明确了该领域的进展。

Conclusion: OSIM提供了一个更符合人类感知的3D场景评估框架，有助于推动3D重建和生成技术的发展。
...
TL;DR: 这篇论文首次采用数据集驱动的视角来综述视频理解领域，展示了运动复杂性、时间跨度、层次结构和多模态丰富性如何引导模型架构的演进，为通用视频理解提供了系统性框架和实践指导。


<details>
  <summary>Details</summary>
Motivation: 现有综述主要按任务或模型家族分类，忽视了数据集对架构演化的结构性影响。本文旨在填补这一空白，通过数据集驱动的视角重新解读视频理解模型的发展历程。

Method: 采用数据集驱动的分析方法，从运动复杂性、时间跨度、层次结构和多模态丰富性四个维度分析数据集如何施加归纳偏置，并重新解读从双流网络、3D CNN到序列模型、Transformer和多模态基础模型的发展历程。

Result: 建立了数据集、归纳偏置和架构的统一框架，将视频理解里程碑重新解释为对数据集驱动压力的具体响应，提供了模型设计与数据集不变性对齐的实用指南。

Conclusion: 通过将数据集、归纳偏置和架构统一到一个连贯的框架中，该综述不仅提供了全面的回顾，还为推进通用视频理解提供了规范性路线图，强调了在可扩展性和任务需求之间平衡的重要性。
...
TL;DR: OCELOT 2023挑战赛旨在验证细胞-组织相互作用对病理图像分析的重要性，通过多尺度标注数据集推动深度学习模型实现人类水平的病理诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有细胞检测模型无法模拟病理学家在不同放大倍数间切换观察组织形态和细胞细节的行为，缺乏多尺度重叠标注数据集阻碍了细胞-组织相互作用的研究。

Method: 构建包含6个器官、673对来自306张TCGA全玻片图像的多尺度数据集，提供重叠的细胞检测和组织分割标注，分为训练、验证和测试集。

Result: 参赛模型显著提升了对细胞-组织关系的理解，最佳模型在测试集上相比不包含细胞-组织关系的基线细胞检测模型F1分数提高了7.99。

Conclusion: 多尺度语义的融入对模型性能有实质性改善，证明了理解细胞-组织相互作用对于实现人类水平病理诊断性能的重要性。
...
Result: 新设计的neck结构在小目标和密集目标检测方面表现优越，模型保持实时检测速度且不增加计算复杂度。
...
TL;DR: 提出了一种无需训练的知识聚焦框架，通过减少知识冗余和噪声来提升基于知识的视觉问答性能


<details>
  <summary>Details</summary>
Motivation: 现有方法在知识检索过程中存在大量知识冗余问题，直接将检索到的信息输入模型会引入噪声，影响回答准确性

Method: 1) 从图像-问题对中提取关键部分创建低噪声查询；2) 使用大模型识别和提取对答案有益的知识片段；3) 引入选择性知识整合策略，仅在模型缺乏回答信心时整合知识

Result: 在广泛实验中证明该框架优于最先进的方法，能够获取准确且关键的知识

Conclusion: 该训练无关框架通过知识聚焦有效减轻了冗余信息的影响，提高了KB-VQA的性能
...
Method: 使用ZY1F卫星高光谱图像数据，提出CWSSNet分类框架，整合3D光谱空间特征和小波卷积，采用多尺度卷积注意力模块整合多模态信息，引入小波域多波段分解和卷积操作。

Result: 在余干县实验中，CWSSNet在mIoU、mAcc和mF1上分别达到74.50%、82.73%和84.94%，在水体、植被和裸地分类中获得最高IoU，表现出良好鲁棒性。在70%训练集比例下训练时间增加有限，分类效果接近最优。

Conclusion: CWSSNet模型在小样本训练条件下保持可靠性能，突破了传统方法的分类性能瓶颈，为高光谱图像地物分类提供了有效解决方案。
...
TL;DR: RRDataset是一个用于评估AI生成图像检测模型在真实世界复杂条件下的综合数据集，包含场景泛化、网络传输鲁棒性和重数字化鲁棒性三个维度，揭示了当前检测方法在真实环境中的局限性。
...
Method: 构建了Real-World Robustness Dataset (RRDataset)，包含7个主要场景的高质量图像，考察网络传输和重数字化对检测性能的影响，并对17个检测器和10个视觉语言模型进行基准测试，同时进行192人参与的大规模人类研究。

Result: 基准测试结果显示当前AI检测方法在真实世界条件下存在局限性，人类在少样本学习检测AI生成图像方面表现出更强的适应性。

Conclusion: 需要借鉴人类的适应能力来开发更鲁棒的检测算法，RRDataset为评估检测模型在真实环境中的性能提供了重要基准。
...
TL;DR: 提出Dark-ISP轻量级自适应ISP插件，直接处理Bayer RAW图像，通过解构传统ISP流水线为可微分组件，实现端到端的低光目标检测优化
...
Method: 1. 将传统ISP流水线解构为线性（传感器校准）和非线性（色调映射）子模块，作为可微分组件通过任务驱动损失优化；2. 利用ISP流水线级联结构设计自增强机制促进子模块协作

Result: 在三个RAW图像数据集上的实验表明，该方法在低光环境下以最少的参数优于最先进的RGB和RAW检测方法

Conclusion: Dark-ISP通过自适应ISP插件实现了高效的RAW图像端到端处理，为低光目标检测提供了轻量且性能优越的解决方案
...
TL;DR: VQualA 2025挑战赛评估大型多模态模型在视觉质量比较任务上的表现，包含数千个从粗到细粒度的质量比较任务，采用2AFC和多项选择题等评估协议，吸引了约100个参赛团队。
...
Method: 构建包含数千个从粗到细粒度视觉质量比较任务的新基准，涵盖单张图像、图像对和多图像组，要求模型提供准确的质量判断，采用2AFC二元偏好和多项选择题等整体评估协议。

Result: 约100名参与者提交了参赛作品，其中五个模型展示了指令调优大型多模态模型在质量评估方面的新兴能力。

Conclusion: 该挑战赛标志着向开放域视觉质量推理和比较迈出了重要一步，并为未来可解释和人类对齐的质量评估系统研究提供了催化剂。
...
<details>
  <summary>Details</summary>
Motivation: 现有目标引导方法在粗粒度目标预测和细粒度轨迹补全之间存在中间粒度空白，多粒度建模能更好地捕捉人类动态和运动模式的不同尺度特征。

Method: 提出MGTraj模型，递归地从粗到细粒度编码轨迹提议，每层使用Transformer递归精炼网络捕获特征并预测渐进式细化，采用权重共享策略整合不同粒度特征，并使用速度预测作为辅助任务。

Result: 在ETH/UCY和斯坦福无人机数据集上的实验表明，MGTraj优于基线方法，在目标引导方法中达到了最先进的性能。

Conclusion: 多粒度建模能有效提升人类轨迹预测精度，MGTraj通过递归精炼和跨粒度特征整合成功实现了这一目标，为机器人导航和自动驾驶提供了更准确的人类行为预测方案。
...
TL;DR: Medverse是一个用于3D医学影像的通用上下文学习模型，通过在22个数据集上训练，实现了跨器官、模态和临床中心的分割、转换和增强任务，采用渐进式自回归框架和块状交叉注意力模块，在未见过的数据集上显著优于现有基线方法。
...
TL;DR: 该论文介绍了MMOral，首个针对全景X射线解读的大规模多模态指令数据集和基准测试，以及基于此训练的OralGPT模型，显著提升了牙科领域视觉语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在通用医疗任务上表现良好，但在牙科等专业领域的效果尚未充分探索，特别是全景X射线由于密集的解剖结构和细微的病理线索，现有医疗基准无法充分评估。
...
Conclusion: MMOral数据集和OralGPT模型为智能牙科提供了重要基础，推动了牙科领域多模态AI系统的临床应用发展，相关资源已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法采用均匀帧采样和隐式位置编码，难以处理长视频的长距离依赖关系，导致关键信息丢失和时序理解能力下降

Method: 时间戳注入机制(TIM)将视频帧嵌入与文本时间戳标记交错构建连续时序参考系统；时间感知相似性采样(TASS)将视频采样重构为视觉语言检索任务，采用两阶段算法确保语义相关性和时序覆盖

Result: 在小时级长视频基准测试中取得显著改进，7B模型在某些基准上甚至超越了许多72B模型

Conclusion: DATE方法有效解决了长视频理解中的时序推理和事件定位挑战，为多模态大语言模型的长视频处理提供了有效解决方案
...
TL;DR: 提出Code-as-Thought(CaT)方法，通过自适应选择代码推理或视觉推理路径来解决图表理解问题，使用强化学习训练决策策略，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有图表理解方法存在局限性：依赖外部工具导致脆弱性，或使用单一推理策略难以验证中间步骤。需要一种可验证且自适应的推理方法。

Method: 提出Code-as-Thought方法，将图表信息转换为可验证的符号格式。引入视觉可编程性概念，让VLM学习选择代码推理路径或直接视觉推理路径。使用强化学习训练决策策略，采用双奖励系统（数据准确性奖励和决策奖励）。

Result: 在多样化的图表理解基准测试中展现出强大而稳健的性能，证明了模型能够动态选择最优推理路径。

Conclusion: VLMs不仅可以学习推理，还可以学习如何推理，能够为每个任务动态选择最佳推理路径，解决了图表理解中的关键挑战。
...
TL;DR: 提出了一种模态无关的U-net架构改进，通过添加模态无关输入通道和模态特定通道，使模型能够处理训练时未见过的MRI模态、已见模态及其混合组合，提高了脑部病变分割的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态脑MRI分割模型通常局限于固定模态，无法有效处理推理时的新模态。一些模型虽然能泛化到未见模态，但可能丢失模态特定的判别信息。本研究旨在开发一个能够处理训练时未见模态、已见模态及其混合组合的模型。

Method: 对U-net架构进行简单改进，集成模态无关输入通道和模态特定输入通道。通过图像增强方案合成人工MRI模态，差异性地改变病理组织和健康组织的外观以创建人工对比度，同时保持真实的解剖完整性。

Result: 在包含5种病理类型（中风、肿瘤、创伤性脑损伤、多发性硬化和白质高信号）和8种模态的8个MRI数据库上评估，结果表明该方法能有效处理训练时见过的MRI模态，同时能处理新的未见模态来改进分割效果。

Conclusion: 该方法通过简单的架构改进实现了对任意可用成像模态的处理能力，在保持对已知模态处理效果的同时，显著提升了对未见模态的分割性能，具有很好的实用价值。
...
Method: 基于嵌入空间的熵建模估计语义不确定性，采用谱归一化和温度缩放技术来增强开放集判别能力，同时处理背景拒绝以增强鲁棒性。

Result: 在AOT空中基准测试中相比标准YOLO检测器实现了10%的相对AUROC增益，通过大量真实飞行测试验证了方法的有效性。
...
TL;DR: MatCha是首个材料表征图像理解基准测试，包含1500个需要专业领域知识的问题，涵盖材料研究的4个关键阶段和21个不同任务。评估显示当前多模态大语言模型在材料表征场景中表现有限，与人类专家存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在材料科学中显示出潜力，但其对真实世界材料表征成像数据的理解能力仍未得到充分探索。需要填补这一研究空白。

Method: 创建MatCha基准测试，包含1500个需要专家级领域知识的问题，涵盖材料研究的4个关键阶段和21个任务。评估现有最先进的多模态大语言模型在该基准上的表现。

Result: 当前多模态大语言模型与人类专家相比存在显著性能差距，在处理需要更高级专业知识和复杂视觉感知的问题时表现下降。简单的few-shot和思维链提示方法难以缓解这些限制。
...
TL;DR: SlotSAR是一个针对SAR图像的目标中心学习框架，无需掩码标注即可将目标表示与背景杂波分离，通过多级特征提取和注意力机制实现最先进的性能。
...
TL;DR: PHCP是一个无需联合训练的无监督域适应框架，通过在推理过程中自训练适配器来解决异构协作感知问题，仅需少量未标注数据即可达到接近SOTA的性能


<details>
  <summary>Details</summary>
Motivation: 现实世界中不同车辆的感知模型存在异构性，现有方法需要联合训练或预先存储所有可能协作模型，这在实践中不可行。研究能否在推理过程中直接解决这一问题

Method: 提出渐进式异构协作感知(PHCP)框架，将问题建模为少样本无监督域适应，在推理过程中通过自训练动态对齐特征，无需标注数据和联合训练

Result: 在OPV2V数据集上的大量实验表明，PHCP在各种异构场景下都取得了强劲性能，仅使用少量未标注数据就能达到与在全数据集上训练的SOTA方法相当的性能

Conclusion: PHCP成功证明了在推理过程中直接处理异构协作感知问题的可行性，为实际应用提供了实用且高效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在零样本分类中通过图像-文本对齐表现出色，但其纯视觉推理能力尚未得到充分探索。本研究旨在全面评估双编码器VLM在语言引导和纯视觉图像分类中的表现，并分析影响准确性的关键因素。

Method: 使用多样化的双编码器VLM（包括SigLIP 2和RADIOv2.5等模型），在ImageNet-1k验证集及其标签校正变体上进行标准设置下的性能比较。分析提示设计、类别多样性、k-NN中邻居数量和参考集大小等关键因素，并提出基于类精度的学习无关融合方法。

Result: 研究表明语言和视觉具有互补优势，某些类别更适合文本提示，而其他类别则更适合通过视觉相似性处理。提出的融合方法能够有效提升分类性能。

Conclusion: 视觉语言模型的语言和视觉能力具有互补性，通过简单的融合方法可以充分利用这种互补优势，提高图像分类的准确性和鲁棒性。
...
Result: 开发了FashionEdit数据集模拟真实服装设计工作流，在生成相似性、用户满意度和质量方面进行评估，证明模型有效性

Conclusion: 该框架释放了用户超越文字描述的创意潜力，降低了服装设计/编辑的门槛，无需进一步人工干预
...
TL;DR: 该研究探讨了自监督预训练策略对手术技能评估(SSA)少样本学习性能的影响，发现领域相关的小数据集优于大规模但不相关的数据集，且加入特定手术程序数据能显著提升性能。
...
Method: 将SSA构建为少样本任务，评估不同预训练来源在三个少样本设置下的表现，量化领域相似性并分析领域差距和特定程序数据的影响。

Result: 领域相关小数据集在1-shot、2-shot和5-shot设置中分别达到60.16%、66.03%和73.65%的准确率。加入特定程序数据平均提升1.22%准确率和2.28% F1分数。

Conclusion: 预训练数据与目标任务的领域相关性比数据规模更重要，精心选择的领域相关数据能显著提升少样本SSA性能。
...
TL;DR: 提出基于计算机视觉的驾驶员行为分类系统，通过外部观察技术实时检测分心和受损驾驶行为，使用YOLO目标检测和车道估计算法分析横向位移和车道位置。
...
TL;DR: 该论文提出了FLUX-Reason-6M数据集和PRISM-Bench评估基准，旨在解决开源文本到图像模型在复杂推理能力方面的不足。FLUX-Reason-6M包含600万高质量图像和2000万双语描述，PRISM-Bench提供7个评估维度，通过大量实验揭示了现有模型的性能差距。


<details>
  <summary>Details</summary>
Motivation: 开源文本到图像模型因缺乏大规模推理数据集和全面评估基准，导致与闭源系统存在性能差距。需要构建专门针对复杂推理能力的数据资源和评估标准来推动领域发展。

Method: 1) 构建FLUX-Reason-6M数据集：600万FLUX生成图像，2000万中英文描述，按6个关键特征组织，采用生成思维链(GCoT)提供详细生成步骤分解；2) 创建PRISM-Bench评估基准：包含7个评估维度，使用精心设计的提示词和先进视觉语言模型进行人机对齐评估；3) 对19个领先模型进行大规模评估。

Result: 研究揭示了现有模型在复杂推理任务中的关键性能差距，发现了需要改进的具体领域。整个数据整理过程消耗15,000个A100 GPU天，为社区提供了前所未有的资源。

Conclusion: 该工作通过大规模数据集和综合评估基准的发布，将催化面向推理的文本到图像生成的下一个发展浪潮，填补了开源模型在复杂推理能力方面的空白。
...
TL;DR: 提出一种新的单幅图像本征图像分解方法，通过纹理引导正则化项处理复杂光照和丰富纹理场景，解决传统方法产生的纹理缺失和过度平滑问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于学习的方法在处理复杂场景（如空间变化光照和丰富纹理）时容易产生纹理缺失和过度平滑的本征图像，需要新的方法来有效分离材质纹理和光照效果。

Method: 设计纹理引导正则化项，将分解问题转化为优化框架，利用RGB图像推断光照和纹理信息，结合纹理感知先验进行本征图像分解。

Result: 新方法能够为真实世界图像生成高质量的本征图像，在复杂光照和纹理场景下表现优于现有方法。

Conclusion: 纹理引导的正则化方法有效解决了本征图像分解中的纹理保持和光照分离问题，为复杂场景下的图像理解提供了更好的解决方案。
...
TL;DR: 本文建立了PnP方法与DDIM之间的联系，提出了一个统一框架将学习先验与物理前向模型结合，并在单像素成像任务中展示了更好的重建质量。


<details>
  <summary>Details</summary>
Motivation: 探索PnP方法与DDIM在解决不适定逆问题（特别是单像素成像）之间的联系，识别两者在去噪机制和采样过程的关键区别。

Method: 将扩散过程解耦为三个可解释阶段：去噪、数据一致性强制和采样；提出混合数据一致性模块，线性组合多个PnP式保真度项，直接在去噪估计上应用混合校正。

Result: 在单像素成像任务上的实验结果表明，该方法实现了更好的重建质量。

Conclusion: 通过统一框架成功整合了学习先验与物理模型，混合数据一致性方法在不干扰扩散采样轨迹的情况下改善了测量一致性。
...
<details>
  <summary>Details</summary>
Motivation: 颅内压升高对脑功能构成严重威胁，需要监测以便及时干预。虽然腰椎穿刺是ICP测量的金标准，但其侵入性和相关风险推动了非侵入性替代方案的需求。视神经鞘直径(ONSD)是一个有前景的生物标志物，但当前临床测量方法存在不一致性、主观性和变异性等问题

Method: 全自动两阶段框架：1)眼底超声视频处理阶段进行帧级解剖分割、基于国际共识声明的规则关键帧识别和精确ONSD测量；2)颅内压分级阶段融合ONSD指标和临床特征来预测ICP等级

Result: 验证准确率达到0.845±0.071(五折交叉验证标准差)，独立测试准确率为0.786，显著优于传统阈值方法(验证准确率0.637±0.111，测试准确率0.429)

Conclusion: 通过有效减少操作者变异性和整合多源信息，该框架为临床ICP评估建立了一种可靠的非侵入性方法，有望改善急性神经系统疾病患者的治疗管理
...
<details>
  <summary>Details</summary>
Motivation: 现代IC制造中存在多种细粒度缺陷，传统方法需要外部正常样本进行对比，但IC图像中布局多样且精确对齐困难，导致方法脆弱。观察到缺陷主要是局部的，而图像本身包含丰富的可重复正常模式。

Method: 使用可学习的正常信息提取器从测试图像中聚合代表性正常特征，通过一致性损失强制这些特征与正常区域关联。在特征指导下，解码器仅重建正常内容，利用重建残差进行缺陷分割。采用伪异常增强稳定训练。

Result: 在三个IC工艺阶段的数据集上实验表明，相比现有方法有持续改进，对产品变异性具有强鲁棒性。

Conclusion: 该无监督框架能够有效处理IC制造中的缺陷分割问题，无需外部正常样本支持，具有良好的实用性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在临床决策支持中具有潜力，但由于成像协议和自由文本报告的变异性，模型容易学习任务无关的相关性，导致在真实世界环境中泛化能力不足和失败风险增加。

Method: 提出结构化特征解耦框架DRiFt，使用参数高效调优（LoRA）和可学习提示令牌显式分离临床相关信号和任务无关噪声。通过为多样化医学数据集生成高质量、临床基础的图像-文本对来增强跨模态对齐和减少不确定性。

Result: 相比先前的基于提示的方法，在分布内性能上提升+11.4% Top-1准确率和+3.3% Macro-F1分数，同时在未见数据集上保持强鲁棒性。消融研究表明特征解耦和仔细对齐显著增强模型泛化能力并减少领域偏移下的不可预测行为。
...
Result: 在六个公共数据集和自建的AVMS数据集上进行实验，FS-Diff在多个放大倍数下均优于最先进方法，能够恢复更丰富的细节和语义信息

Conclusion: FS-Diff通过语义引导和清晰度感知机制，有效解决了多模态图像融合和超分辨率的联合问题，在真实世界应用中具有重要价值
...
Method: 1) 通过蒸馏补丁对应关系打破严格空间对齐，使用噪声容忍排序损失处理伪标签噪声；2) 提出对象感知过滤器，通过可学习原型和交叉注意力将输出空间映射到基于对象的空间。
...
TL;DR: FlexiD-Fuse是一个基于扩散模型的医学图像融合网络，能够处理任意数量的输入模态（双模态或三模态），解决了现有方法只能处理固定数量模态输入的限制。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像融合方法只能处理固定数量的模态输入（如仅双模态或三模态），无法灵活处理不同数量的输入，这限制了其在临床环境中的应用。
...
Conclusion: FlexiD-Fuse方法有效解决了多模态医学图像融合中输入数量不固定的问题，在多个图像融合任务中均表现出优异的性能和通用性。
...
TL;DR: 该研究开发了一个针对资源受限环境的3D注意力UNet深度学习框架，用于非洲地区的脑胶质瘤MRI分割，在有限数据条件下取得了良好的分割性能。
...
Method: 采用3D注意力UNet架构，结合残差块增强，并利用BraTS 2021数据集的预训练权重进行迁移学习。

Result: 在BraTS-Africa数据集的95个MRI病例上，获得了ET区域Dice分数0.76、NETC区域0.80、SNFH区域0.85的分割性能。模型大小约90MB，在消费级硬件上单次推理时间不到1分钟。

Conclusion: 该模型展现了良好的泛化能力和临床实用性，有助于缩小全球健康AI公平性差距，为资源匮乏地区提供高性能、可访问的医学影像解决方案。
...
TL;DR: 本文提出了一个专门针对政治背景的深度伪造检测数据集，包含300万真实图像和96.3万高质量合成图像，并建立了众包对抗平台来持续应对不断发展的生成技术。


<details>
  <summary>Details</summary>
Motivation: 现有的深度伪造检测数据集存在局限性，包括使用过时的生成方法、低真实感和单一人脸图像，无法有效检测现代生成模型创建的合成媒体。社交媒体上深度伪造传播的多种模式以及公众难以区分最新合成图像的现实问题，促使需要更全面的检测基准。

Method: 通过分析社交媒体帖子识别深度伪造传播模式，进行人类感知研究评估合成图像的真实性，构建包含真实图像和对应合成图像的大规模数据集，并建立众包对抗平台让参与者生成具有挑战性的合成图像。

Result: 创建了包含300万真实图像和96.3万合成图像的综合数据集，覆盖专有和开源模型生成的高质量合成内容。人类研究表明最新专有模型生成的合成图像已难以与真实图像区分。

Conclusion: 提出的数据集和众包对抗平台为深度伪造检测提供了更有效的基准，能够持续适应生成技术的发展，有助于保护公共讨论免受复杂虚假信息威胁。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将人体运动分解为局部关节运动和全局根关节运动分别处理，但这两者并非独立，存在物理耦合关系。传统方法难以精确捕捉这种物理联系，而基于关节扭矩和外部力的计算又过于复杂昂贵
...
TL;DR: 提出了一种无需预标注的漫画线稿区域对应预测方法，使用基于Transformer的框架学习补丁级相似性，并通过聚类和区域匹配算法获得连贯的区域级对应关系。


<details>
  <summary>Details</summary>
Motivation: 漫画线稿图像间的区域对应关系是漫画处理的基础任务，支持自动上色和中间帧生成等应用，但在没有预分割或标注的现实场景中仍未被充分探索。

Method: 将线稿图像分割成补丁，使用Transformer框架学习图像内和图像间的补丁级相似性，然后应用边缘感知聚类和区域匹配算法将补丁级预测转换为区域级对应。

Result: 在多个数据集上的实验显示，该方法达到高补丁级准确率（96.34%）并生成一致的区域级对应关系。

Conclusion: 该方法在无预标注的情况下有效预测漫画线稿的区域对应关系，展示了在实际漫画应用中的潜力。
...
TL;DR: 本文提出SGDF方法和GDCN网络，通过生成扩散机制处理多视图聚类中的低质量数据问题，在深度MVC任务中达到state-of-the-art效果。


<details>
  <summary>Details</summary>
Motivation: 多视图聚类中存在低质量数据问题，主要源于某些视图被噪声数据污染或存在数据缺失，这影响了多视图融合的聚类性能。

Method: 提出随机生成扩散融合(SGDF)方法，利用多重生成机制处理每个样本的多视图特征；基于SGDF进一步提出生成扩散对比网络(GDCN)。

Result: 大量实验表明GDCN在深度多视图聚类任务中取得了最先进的结果。

Conclusion: SGDF方法能够有效处理多视图融合中的低质量数据问题，GDCN网络在多视图聚类任务中表现出优异的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D超声系统成本高且复杂，限制了广泛应用。传感器无关的3D超声使用深度学习从2D图像序列估计探头轨迹，但现有方法要么忽略全局特征，要么将其与局部特征提取紧密耦合，无法稳健建模这两个互补方面。
...
Conclusion: DualTrack通过解耦局部和全局特征提取，有效解决了传感器无关3D超声重建中的关键挑战，为临床广泛应用提供了有前景的解决方案。
...
TL;DR: 本文提出Align4Gen方法，通过将视频生成器的中间特征与预训练视觉编码器的特征表示对齐，来提升视频扩散模型的生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型主要关注架构创新和训练目标改进，但对特征表示能力的提升关注不足。研究表明特征对齐可以显著改善视频生成效果。

Method: 提出多特征融合和对齐方法，集成到视频扩散模型训练中。首先分析不同视觉编码器的判别性和时间一致性，然后选择最适合的特征进行对齐。

Result: 在无条件和类条件视频生成任务上，Align4Gen方法在各种评估指标上都显示出改进的视频生成效果。

Conclusion: 特征对齐是提升视频扩散模型性能的有效方法，Align4Gen为视频生成提供了新的特征增强策略。
...
TL;DR: InterAct是一个大规模3D人-物交互基准数据集，通过整合21.81小时数据并优化质量，扩展到30.70小时，解决了现有数据集在运动质量和标注方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模人体运动捕捉数据集在人-物交互建模方面存在局限性，包括数据质量不足、接触穿透、漂浮现象和错误的手部运动等伪影问题。

Method: 1) 整合和标准化21.81小时的HOI数据，添加详细文本标注；2) 提出统一的优化框架，基于接触不变性原理减少伪影并纠正手部运动；3) 定义六个基准任务并开发统一的HOI生成建模方法。

Result: 数据集扩展到30.70小时，在六个基准任务上实现了最先进的性能，验证了数据集作为3D人-物交互生成基础资源的实用性。

Conclusion: InterAct数据集为推进3D人-物交互生成研究提供了高质量的基础资源，数据集已公开并将持续维护。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习算法在MRI脑部成像诊断中可能遭受捷径学习，使用与输出标签无关的虚假特征进行预测，当这些特征与受保护属性相关时，会导致对代表性不足群体的性能偏见。

Method: 研究首先调查DL算法是否能从3D脑部MRI扫描中识别种族或性别，然后检查训练集不平衡是否导致模型性能下降，最后对受保护属性和AD分类任务进行定量和定性特征归因分析，使用ResNet和SwinTransformer等多种模型和数据集。

Result: 实验证明基于深度学习的AD分类中存在种族和性别相关的捷径学习和偏见，特征归因分析显示了不同脑区在不同任务中的特征重要性差异。

Conclusion: 这项工作为开发更公平的脑部MRI深度学习诊断工具奠定了基础，揭示了当前方法中存在的偏见问题，并提供了代码实现以供进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机具有微秒级精度和优异动态场景捕捉能力，但与自然语言理解的结合研究较少，存在多模态感知的空白。
...
TL;DR: Kling-Avatar是一个新颖的两阶段级联框架，通过多模态指令理解和照片级真实感肖像生成的统一，解决了现有音频驱动虚拟形象生成方法在叙事连贯性和角色表现力方面的局限性。
...
Method: 采用两阶段流水线：第一阶段设计多模态大语言模型导演，基于多样化指令信号生成蓝图视频，控制角色动作和情感等高级语义；第二阶段在蓝图关键帧指导下，使用首尾帧策略并行生成多个子片段。
...
TL;DR: 提出混合机制学习框架，结合数学肿瘤生长模型和引导去噪扩散模型，从前期MRI预测脑肿瘤时空进展并生成解剖学可行的未来MRI图像


<details>
  <summary>Details</summary>
Motivation: 预测脑肿瘤的时空进展对于神经肿瘤学的临床决策至关重要，需要在数据有限的情况下实现生物信息驱动的图像生成

Method: 使用常微分方程系统的机制模型捕捉肿瘤动态（包括放疗效果），结合梯度引导的DDIM扩散模型进行图像合成，确保与预测生长和解剖结构一致

Result: 在BraTS成人和儿童胶质瘤数据集上训练，在60个儿童弥漫中线胶质瘤病例上评估，生成具有空间相似性指标的逼真随访扫描，提供肿瘤生长概率图

Conclusion: 该方法在数据有限情况下实现生物信息驱动的时空预测，捕获临床相关的肿瘤生长范围和方向性，为临床决策提供支持
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注识别准确性，但忽略了模型识别错误选项的认知谦逊能力，这在安全关键应用中至关重要。

Method: 基于全景场景图数据集构建，利用细粒度场景图注释提取真实实体和关系，通过GPT-4-Turbo生成多项选择题并进行严格人工筛选。
...
TL;DR: 本文提出了UAE框架，通过自编码器视角将图像理解作为编码器(I2T)，图像生成作为解码器(T2I)，使用重建保真度作为统一训练目标，实现双向信息流的协同增益。


<details>
  <summary>Details</summary>
Motivation: 探索多模态学习中理解与生成任务之间的内在联系，通过统一框架实现双向互惠，提升视觉理解和图像生成的质量。

Method: 1) 使用大规模长上下文图像描述预训练解码器；2) 提出Unified-GRPO强化学习框架，包含冷启动阶段、生成促进理解阶段、理解促进生成阶段；3) 引入Unified-Bench评估基准。

Result: 随着强化学习进展，编码器自主产生更具描述性的标题，解码器同时展现出理解复杂描述的能力，实现高保真度重建，展示了理解与生成任务的协同提升。

Conclusion: 该研究揭示了多模态学习中理解与生成任务的内在统一性，通过双向信息流框架实现了相互促进，为统一多模态学习提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有VAE或扩散模型方法无法充分建模人体运动的几何结构和物理约束，需要一种能够在底层关节几何结构上精确建模高阶运动动态的方法。

Method: 在关节旋转、角速度和角加速度的乘积空间上构建神经距离场，提出自适应步长混合投影算法和几何积分器来生成合理运动轨迹。

Result: 在AMASS数据集上训练后，NRMF在去噪、运动插值和部分2D/3D观测拟合等多个任务上表现出显著的泛化能力。

Conclusion: NRMF通过几何感知的高阶运动建模，为3D人体运动恢复提供了更鲁棒和物理合理的解决方案，在多种输入模态和任务上展现出色性能。
...
TL;DR: 本文研究发现扩散模型中局部性特性源于图像数据集的统计特性，而非卷积神经网络的归纳偏置，并提出了更匹配深度扩散模型的解析去噪器


<details>
  <summary>Details</summary>
Motivation: 现有研究认为深度扩散模型与最优去噪器之间的性能差距源于卷积网络的平移等变性和局部性归纳偏置，但本文旨在证明这种局部性实际上是图像数据集本身的统计特性

Method: 通过理论分析和实验验证，展示最优参数化线性去噪器具有与深度神经去噪器相似的局部性特性，并证明这种局部性源于自然图像数据集中像素相关性

Result: 发现局部性确实是图像数据集的统计属性，而非卷积网络的归纳偏置，并开发出比先前专家设计的替代方案更匹配深度扩散模型预测分数的解析去噪器

Conclusion: 扩散模型中的局部性特性主要来源于图像数据集的统计结构，这一发现有助于更好地理解深度扩散模型的行为并改进解析模型的设计
...
TL;DR: 提出了SpatialVID数据集，包含超过21,000小时的原始视频，处理成270万段剪辑，共计7,089小时的动态内容，带有密集的3D注释如相机位姿、深度和运动指令，解决了当前空间智能模型训练数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 当前空间智能模型在可扩展性和真实世界保真度方面受到大规模高质量训练数据稀缺的严重限制，现有数据集在规模、多样性和注释丰富度方面存在不足，特别是对于具有真实相机运动的动态场景。

Method: 通过分层过滤流程收集和处理大量野外视频，构建包含多样化场景和相机运动的数据集，并通过后续注释流程为剪辑添加详细的空间和语义信息，包括相机位姿、深度图、动态掩码、结构化标题和序列化运动指令。

Result: 创建了SpatialVID数据集，包含超过21,000小时原始视频，处理成2.7百万段剪辑，总计7,089小时动态内容，数据统计分析显示其丰富性和多样性直接促进了模型泛化能力和性能的提升。

Conclusion: SpatialVID数据集作为视频和3D视觉研究社区的关键资产，其丰富多样的数据特性有助于解决当前空间智能模型训练数据不足的问题，推动相关领域的发展。
...
### [116] [Open-sci-ref-0.01: open and reproducible reference baselines for language model and dataset comparison](https://arxiv.org/abs/2509.09009)
*Marianna Nezhurina,Taishi Nakamura,Timur Carstensen,Niccolò Ajroldi,Ville Komulainen,David Salinas,Jenia Jitsev*

Main category: cs.LG

TL;DR: open-sci-ref是一个密集Transformer模型家族，在多个参数规模(0.13B到1.7B)和token规模(最高1T)上训练，基于8个开放参考数据集建立研究基线，为评估不同训练方法提供参考标准。


<details>
  <summary>Details</summary>
Motivation: 为研究人员提供标准化的参考基线，以便评估不同训练方法的有效性和质量，特别是在不同规模和数据集上的表现，同时简化复现过程和标准化比较。

Method: 使用密集Transformer架构，在8个开放参考数据集上进行训练，涵盖0.13B到1.7B参数规模和最高1T token规模，提供中间检查点、训练日志和代码。

Result: NemoTron-CC HQ数据集表现最佳，其次是DCLM-baseline和FineWeb-Edu。建立了可比较的参考基线，使不同训练程序能够在统一计算轴上进行比较。

Conclusion: open-sci-ref提供了标准化的研究基线，有助于评估训练方法的质量，促进未来研究，并通过完整的发布内容(检查点、日志、代码、评估)简化复现和比较过程。
...
### [117] [Identifying Key Features for Establishing Sustainable Agro-Tourism Centre: A Data Driven Approach](https://arxiv.org/abs/2509.09214)
...
Result: 逻辑回归模型在70-30%训练测试数据中达到98%准确率，在80-20%数据中达到99%准确率；随机森林在70-30%数据中达到95%准确率；决策树和XGBoost在80-20%数据中达到97%准确率。

Conclusion: 机器学习方法特别是LASSO结合逻辑回归模型能有效识别农业旅游增长的关键指标，为农业旅游发展策略制定提供了数据驱动的决策支持。
...
### [118] [Harnessing Uncertainty: Entropy-Modulated Policy Gradients for Long-Horizon LLM Agents](https://arxiv.org/abs/2509.09265)
*Jiawei Wang,Jiacai Liu,Yuqian Fu,Yingru Li,Xintao Wang,Yuan Lin,Yu Yue,Lin Zhang,Yang Wang,Ke Wang*

Main category: cs.LG

TL;DR: 本文提出了熵调制策略梯度（EMPG）框架，解决了LLM智能体在长时任务中因稀疏奖励导致的信用分配问题，通过基于不确定性的梯度校准来提升学习效率和稳定性。


<details>
  <summary>Details</summary>
Motivation: 在长时任务中，基于大语言模型的智能体面临稀疏奖励难以分配到中间步骤的问题，传统方法主要关注创建密集奖励信号，但忽略了LLM学习动态中策略梯度幅度与熵的耦合问题。

Method: 提出EMPG框架，基于步骤不确定性和最终任务结果重新校准学习信号：放大自信正确动作的更新，惩罚自信错误，衰减不确定步骤的更新以稳定探索，并引入未来清晰度奖励项。

Result: 在WebShop、ALFWorld和DeepSearch三个挑战性任务上的实验表明，EMPG实现了显著的性能提升，显著优于强策略梯度基线方法。

Conclusion: EMPG通过解耦策略梯度幅度与熵的耦合关系，有效解决了长时任务中的信用分配问题，为LLM智能体的强化学习提供了更稳定和高效的学习框架。
...
### [119] [LLMs Don't Know Their Own Decision Boundaries: The Unreliability of Self-Generated Counterfactual Explanations](https://arxiv.org/abs/2509.09396)
*Harry Mayne,Ryan Othniel Kearns,Yushi Yang,Andrew M. Bean,Eoin Delaney,Chris Russell,Adam Mahdi*

Main category: cs.LG

TL;DR: 研究发现LLM生成的反事实解释在有效性和最小性之间存在权衡，通常要么不够最小化，要么过度编辑导致预测不变，表明自生成反事实解释作为可解释性工具效果有限甚至可能误导
...
Method: 评估LLM生成的反事实解释的有效性（是否能改变预测结果）和最小性（修改输入的程度），在多个LLM模型、数据集和评估设置中进行实验

Result: LLM通常能生成有效的反事实解释但远非最小化；当要求生成最小反事实时，往往做出过小的编辑而无法改变预测；这种有效性-最小性权衡在不同模型和设置中一致存在

Conclusion: 自生成反事实解释作为可解释性工具效果有限甚至可能产生误导，在将LLM部署到高风险场景时必须考虑不可靠的自我解释对下游决策的影响
...
### [120] [ButterflyQuant: Ultra-low-bit LLM Quantization through Learnable Orthogonal Butterfly Transforms](https://arxiv.org/abs/2509.09679)
*Bingxin Xu,Zhen Dong,Oussama Elachqar,Yuzhang Shang*

Main category: cs.LG

TL;DR: ButterflyQuant是一种自适应旋转量化方法，通过可学习的蝴蝶变换替代固定Hadamard变换，有效抑制激活值异常值，在2-bit量化下显著提升LLaMA-2-7B性能。
...
Method: 提出可学习的蝴蝶变换，通过连续Givens旋转角度参数化，保证正交性同时支持梯度优化；引入均匀性正则化促进量化友好分布。

Result: 仅需128个校准样本和单GPU几分钟训练，在LLaMA-2-7B 2-bit量化下达到15.4困惑度，优于QuaRot的22.1。

Conclusion: ButterflyQuant通过自适应旋转变换有效解决极端量化中的异常值问题，以极小计算成本显著提升低精度量化性能。
...
### [121] [Value bounds and Convergence Analysis for Averages of LRP attributions](https://arxiv.org/abs/2509.08963)
*Alexander Binder,Nastaran Takmil-Homayouni,Urun Dogan*

Main category: cs.LG

TL;DR: 本文通过将LRP归因方法表示为修正梯度矩阵的乘积，建立了与雅可比矩阵乘法的类比，推导了奇异值上界和归因图值的分量界，并获得了控制经验均值收敛到期望的乘法常数，发现LRP-beta的常数与权重范数无关。
...
Method: 将LRP归因方法表示为修正梯度矩阵的乘积，建立与链式法则中雅可比矩阵乘法的类比，推导奇异值上界和分量界，获得控制收敛的乘法常数。

Result: 获得了控制经验均值收敛到期望的乘法常数，发现LRP-beta的常数与权重范数无关，这与基于梯度的方法和LRP-epsilon形成显著区别。

Conclusion: LRP-beta方法在归因稳定性方面具有独特优势，其收敛常数不依赖于权重范数，这为多数据增强和Smoothgrad型归因方法提供了重要的理论支撑和应用价值。
...
### [122] [Adaptive Pareto-Optimal Token Merging for Edge Transformer Models in Semantic Communication](https://arxiv.org/abs/2509.09168)
*Omar Erak,Omar Alhussein,Hatem Abou-Zeid,Mehdi Bennis*

Main category: cs.LG

TL;DR: 提出了一种无需训练的预训练视觉Transformer自适应token合并框架，通过贝叶斯优化平衡计算成本和精度，实现语义通信系统的高效部署


<details>
  <summary>Details</summary>
Motivation: 大规模Transformer模型在语义通信系统中表现出强大能力，但其巨大的计算需求成为在资源受限的6G网络中实际部署的主要障碍

Method: 将每层合并比例选择建模为多目标优化问题，采用高斯过程贝叶斯优化构建帕累托最优配置前沿，支持根据应用需求和信道条件动态调整

Result: 实验表明该方法在保持竞争力的精度同时显著减少浮点运算量，在各种信噪比条件下均优于其他基线方法

Conclusion: 建立了一个可扩展且高效的方法，为未来边缘智能系统中基于Transformer的语义通信部署提供了实用机制
...
TL;DR: 当前深度学习天气模型的评估指标存在"统计相似性陷阱"，奖励模糊预测但错过罕见高影响事件。DART框架通过双解码器架构、物理过采样和任务特定损失函数，专门优化极端对流检测，在保持高相关性的同时显著提升CSI分数。


<details>
  <summary>Details</summary>
Motivation: 现有天气模型评估指标过于强调统计相似性，导致模型产生模糊预测而无法准确检测罕见但高影响的极端天气事件（如危险对流），这在实际应用中存在严重局限性。

Method: 提出DART（Dual Architecture for Regression Tasks）框架，采用双解码器架构进行背景/极端分解，结合物理驱动的过采样技术和任务特定的损失函数，将粗粒度大气预报转换为高分辨率卫星亮温场。

Result: DART在危险对流检测中达到CSI=0.273（基线为0.00），偏差从6.72降至2.52；发现移除集成水汽输送（IVT）可使极端对流检测性能提升270%；在2023年吉大港洪水案例中验证了实际效果。

Conclusion: DART框架成功解决了混合转换-分割-降尺度任务，提供了可信的极端天气AI解决方案，训练时间短（<10分钟），易于集成到现有气象工作流中，为极端天气预警提供了有效途径。
...
### [124] [Graph Alignment via Dual-Pass Spectral Encoding and Latent Space Communication](https://arxiv.org/abs/2509.09597)
...
Result: 在图基准测试中 consistently 优于现有无监督对齐基线方法，对结构不一致性和挑战性对齐场景表现出 superior 鲁棒性；在视觉-语言基准测试中也能有效泛化，实现视觉和语言表示的无监督对齐。

Conclusion: 该框架通过同时解决节点区分度和跨图几何一致性问题，显著提升了无监督图对齐的性能和鲁棒性，并展示了跨领域的泛化能力。
...
### [125] [Automated Unity Game Template Generation from GDDs via NLP and Multi-Modal LLMs](https://arxiv.org/abs/2509.08847)
*Amna Hassan*

Main category: cs.AI

TL;DR: 提出基于NLP和多模态LLM的端到端框架，将游戏设计文档自动转换为Unity游戏原型，通过微调LLaMA-3模型实现高质量的Unity C#代码生成。


<details>
  <summary>Details</summary>
Motivation: 解决AI辅助游戏开发中的关键空白，简化从游戏设计到实现的过程，利用LLM技术提升游戏原型开发效率。

Method: 使用微调的LLaMA-3模型专门用于Unity代码生成，结合自定义Unity集成包，解析GDD并提取结构化游戏规范，合成Unity兼容的C#代码。

Result: 微调模型在编译成功率、GDD遵循度、最佳实践采用和代码模块化等指标上表现优异（平均得分4.8/5.0），显著优于基线模型。

Conclusion: 该系统有效证明了LLM在游戏开发流程中的价值，能够高效地将设计文档转化为功能性游戏原型，支持多种游戏类型。
...
TL;DR: 本文提出了一个基于神经科学原理的计算框架，用于提升AI代理的空间推理能力，包含六个核心计算模块，并分析了现有方法的差距和应用前景。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理在自主任务执行和语言推理方面取得进展，但空间推理能力仍然有限，主要局限于符号和顺序处理。与人类基于多感官感知、空间记忆和认知图的空间智能相比存在差距，需要构建更好的空间智能来与物理3D世界交互。

Method: 从计算神经科学中审视空间神经模型，提出基于神经科学原理的新计算框架，包含六个核心模块：生物启发多模态感知、多感官整合、自我中心-异中心转换、人工认知图、空间记忆和空间推理。

Result: 构建了一个跨虚拟和物理环境的代理空间推理能力视角框架，分析了现有方法与各模块的相关性，识别了阻碍发展的关键差距，并探讨了从虚拟到具身系统（如机器人）的应用领域。

Conclusion: 提出了一个有前景的研究路线图，能够推广到动态或非结构化环境中的空间推理，希望为研究社区提供神经科学基础视角和结构化路径。
...
### [128] [Bona fide Cross Testing Reveals Weak Spot in Audio Deepfake Detection Systems](https://arxiv.org/abs/2509.09204)
*Chin Yuen Kwok,Jia Qi Yip,Zhen Qiu,Chi Hung Chi,Kwok Yan Lam*

Main category: cs.SD

TL;DR: 论文提出了bona fide cross-testing评估框架，通过整合多样化的真实语音数据集和聚合EER来解决音频深度伪造检测中传统评估方法的不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统音频深度伪造检测评估方法存在两个主要问题：1）使用单一EER指标会因合成器样本数量不均而降低评估可靠性；2）大多数数据集缺乏真实语音多样性，限制了模拟真实场景的能力。

Method: 提出bona fide cross-testing评估框架，整合多个真实语音数据集，采用聚合EER的方式进行更平衡的评估，并在9种真实语音类型上对150多个合成器进行基准测试。

Result: 相比传统评估方法，新框架提高了鲁棒性和可解释性，并发布了新的数据集https://github.com/cyaaronk/audio_deepfake_eval以促进进一步研究。

Conclusion: bona fide cross-testing框架为音频深度伪造检测提供了更可靠和全面的评估方法，解决了传统评估中的不平衡和多样性不足问题。
...
### [129] [DiFlow-TTS: Discrete Flow Matching with Factorized Speech Tokens for Low-Latency Zero-Shot Text-To-Speech](https://arxiv.org/abs/2509.09631)
*Ngoc-Son Nguyen,Hieu-Nghia Huynh-Nguyen,Thanh V. T. Tran,Truong-Son Hy,Van Nguyen*

Main category: cs.SD

TL;DR: DiFlow-TTS是首个纯离散流匹配的零样本文本转语音模型，通过显式建模分解的语音属性，在保持紧凑架构的同时实现高质量语音合成，推理速度比现有基线快25.8倍


<details>
  <summary>Details</summary>
Motivation: 解决现有零样本TTS方法推理速度慢、存在重复伪影的问题，充分利用离散表示的潜力，避免将离散token嵌入连续空间的做法
...
### [130] [A vibe coding learning design to enhance EFL students' talking to, through, and about AI](https://arxiv.org/abs/2509.08854)
*David James Woo,Kai Guo,Yangyang Yu*

Main category: cs.CY

TL;DR: 本文通过案例研究探讨了在EFL教育中使用AI自然语言编程（vibe coding）的实践，开发了包含三个维度的人类-AI元语言框架，发现学生与AI交互方式的不同导致了编程结果的显著差异。


<details>
  <summary>Details</summary>
Motivation: 探索AI自然语言编程在英语作为外语教育中的应用潜力，研究学生如何通过与AI的交互来解决真实的EFL写作挑战。

Method: 采用案例研究方法，设计4小时工作坊，通过工作表、视频录制、有声思维协议、屏幕录制和AI生成图像收集数据，对比分析两名学生的编程过程和结果。

Result: 一名学生成功实现了功能完整的应用程序，另一名学生则遇到技术困难，设计意图与实际功能存在较大差距，差异主要源于提示工程方法和AI心智模型的不同。

Conclusion: AI作为有益的语言机器，有效的vibe coding教学需要明确的元语言支架，包括结构化提示工程教学、批判性作者身份讨论和AI心智模型词汇发展。
...
### [131] [Dynamic Structural Recovery Parameters Enhance Prediction of Visual Outcomes After Macular Hole Surgery](https://arxiv.org/abs/2509.09227)
...
TL;DR: 本研究引入动态结构参数并整合到多模态深度学习框架中，用于预测特发性全层黄斑裂孔患者术后视力恢复情况。通过自动分割和特征提取流程，结合动态参数的多模态DL模型显著提升了预测准确性。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够准确预测黄斑裂孔手术后视力恢复的自动化工具，通过整合动态结构参数和多模态数据来提升预测性能，为个性化术后管理提供临床决策支持。

Method: 使用公开的纵向OCT数据集（5个时间点），开发阶段特异性分割模型提取定量、复合、定性和动态特征。构建包含/不包含动态参数的逻辑回归模型，并开发结合临床变量、OCT特征和原始图像的多模态深度学习模型。

Result: 分割模型精度高（平均Dice > 0.89）。动态恢复率的加入持续改善逻辑回归AUC，特别是在3个月随访时。多模态DL模型在所有阶段都优于逻辑回归，AUC差异高达0.12。

Conclusion: 将动态参数整合到多模态DL模型中显著提高了预测准确性，这一全自动化流程代表了黄斑裂孔手术个性化术后管理中具有前景的临床决策支持工具。
...
### [132] [Virtual staining for 3D X-ray histology of bone implants](https://arxiv.org/abs/2509.09235)
...
Method: 使用50+对共配准的微CT和甲苯胺蓝染色组织学图像对，训练针对有限配对数据优化的改进CycleGAN网络。采用像素级监督和灰度一致性项，通过基于补丁的训练和实时数据增强。

Result: 该方法在SSIM、PSNR和LPIPS指标上优于Pix2Pix和标准CycleGAN基线，能够生成组织学真实的彩色输出并保持高分辨率结构细节，可应用于完整CT体积生成虚拟染色3D数据集。

Conclusion: 这项工作将虚拟染色引入3D X射线成像，为生物医学研究提供了可扩展的、化学信息丰富的无标记组织表征途径，但需要更多训练数据和改进以更好地描绘植入物降解层等特征。
...
### [133] [In-Loop Filtering Using Learned Look-Up Tables for Video Coding](https://arxiv.org/abs/2509.09494)
...
<details>
  <summary>Details</summary>
Motivation: 神经网络环路滤波虽然编码增益显著，但计算复杂度和硬件要求高，难以广泛应用。需要一种既保持性能又降低复杂度的实用解决方案。

Method: 1) 训练受限参考范围的DNN并缓存输出到LUT；2) 多类型滤波LUT协作和定制索引机制；3) 跨分量联合滤波机制；4) LUT压缩剪枝方案降低存储成本。

Result: 在VVC参考软件中实现，AI配置下平均0.82%/2.97%/1.63%码率节省，RA配置下0.85%/4.11%/2.06%码率节省，相比DNN方案大幅降低时间复杂度和存储成本。

Conclusion: LUT-ILF++框架成功解决了神经网络环路滤波的高复杂度问题，提供了实用高效的解决方案，为下一代视频编码标准提供了有竞争力的候选技术。
...
### [134] [Generative Engine Optimization: How to Dominate AI Search](https://arxiv.org/abs/2509.08919)
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI搜索引擎的快速普及正在从根本上重塑信息检索方式，从传统的排名列表转向合成的、有引用的答案，这挑战了现有的SEO实践，需要新的优化范式。

Method: 通过跨多个垂直领域、语言和查询改写的大规模对照实验，量化分析AI搜索与传统搜索引擎在信息来源方面的关键差异。

Result: 关键发现显示AI搜索系统性地过度偏向第三方权威内容(获得性媒体)，而非品牌自有和社交内容，与谷歌更平衡的混合形成鲜明对比。不同AI搜索服务在域名多样性、新鲜度、跨语言稳定性和对措辞敏感性方面存在显著差异。

Conclusion: 基于实证结果制定了GEO战略议程，为从业者提供可操作指导，包括：优化机器可扫描性和合理性内容、主导获得性媒体建立AI感知权威、采用引擎特定和语言感知策略、克服小众玩家的"大品牌偏见"，为生成式搜索环境中的可见性提供了基础实证分析和战略框架。
...
### [135] [Retrieval-Augmented Generation for Reliable Interpretation of Radio Regulations](https://arxiv.org/abs/2509.09651)
*Zakaria El Kassimi,Fares Fourati,Mohamed-Slim Alouini*

Main category: cs.IR

TL;DR: 该论文提出了一个电信领域的RAG管道，用于无线电法规问答，创建了首个该领域的选择题评估集，检索准确率达97%，使GPT-4o性能提升近12%。
...
Method: 提出电信特定的检索增强生成(RAG)管道，构建了首个基于权威来源的多选题评估集，采用自动化过滤和人工验证，并定义了领域特定的检索指标。

Result: 检索器达到约97%的准确率，RAG管道使所有测试模型的生成准确率都有提升，GPT-4o相对提升近12%。

Conclusion: 精心设计的接地方法为监管问答提供了简单而强大的基准和有效的领域特定解决方案。
...
### [136] [OmniEVA: Embodied Versatile Planner via Task-Adaptive 3D-Grounded and Embodiment-aware Reasoning](https://arxiv.org/abs/2509.09332)
...
Method: 提出两个核心创新：1）任务自适应3D接地机制，使用门控路由器根据上下文需求进行选择性3D融合调节；2）具身感知推理框架，将任务目标和物理约束共同纳入推理循环。

Result: 实验结果表明OmniEVA在通用具身推理性能上达到最先进水平，在广泛的下游场景中表现出强大能力，包括原始和复合任务的基准测试都证实了其鲁棒和通用的规划能力。
...
### [137] [SimpleVLA-RL: Scaling VLA Training via Reinforcement Learning](https://arxiv.org/abs/2509.09674)
...
TL;DR: SimpleVLA-RL是一个针对视觉-语言-动作模型的强化学习框架，通过引入VLA特定的轨迹采样、并行化、多环境渲染和优化损失计算，显著提升了机器人操作任务的性能，减少了对大规模人工标注数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 解决VLA模型面临的两个核心挑战：大规模人类操作轨迹数据的稀缺性和高成本，以及在分布偏移任务中的泛化能力有限。受大型推理模型通过强化学习提升推理能力的启发，探索RL是否能类似地提升VLA模型的长期动作规划能力。

Method: 基于veRL框架，开发了SimpleVLA-RL框架，包含VLA特定的轨迹采样、可扩展并行化、多环境渲染和优化损失计算等技术。应用于OpenVLA-OFT模型，并引入了探索增强策略。

Result: 在LIBERO基准测试中达到最先进性能，在RoboTwin 1.0和2.0上甚至超越了π_0基线。不仅减少了对大规模数据的依赖并实现了鲁棒泛化，还在真实世界任务中显著超越了监督微调方法。发现了RL训练中的"pushcut"现象。

Conclusion: SimpleVLA-RL框架成功证明了强化学习可以有效提升VLA模型的长期动作规划能力，为解决机器人操作任务中的数据稀缺和泛化问题提供了有效解决方案，展现了RL在VLA模型训练中的重要价值。
...
### [138] [ObjectReact: Learning Object-Relative Control for Visual Navigation](https://arxiv.org/abs/2509.09594)
*Sourav Garg,Dustin Craggs,Vineeth Bhat,Lachlan Mares,Stefan Podgorski,Madhava Krishna,Feras Dayoub,Ian Reid*

Main category: cs.RO

TL;DR: 提出了一种基于对象相对控制的视觉导航新范式，使用相对3D场景图作为拓扑地图表示，通过ObjectReact本地控制器实现跨具身和轨迹不变性的导航


<details>
  <summary>Details</summary>
Motivation: 传统的图像相对导航方法受限于相机姿态和具身特性，而对象作为地图属性提供了更具不变性的世界表示，需要开发能够克服图像级别限制的新导航方法

Method: 提出相对3D场景图作为拓扑地图表示，训练ObjectReact本地控制器，直接基于WayObject Costmap高层表示进行控制预测，无需显式RGB输入

Result: 在传感器高度变化和多种导航任务中表现出优于图像相对方法的性能，能够反向导航地图轨迹，仅使用模拟训练的策略能很好地泛化到真实室内环境

Conclusion: 对象相对控制范式具有多个优势：无需严格模仿先验经验即可遍历新路线，控制预测可与图像匹配问题解耦，在跨具身部署中实现高度不变性
...
### [139] [Dexplore: Scalable Neural Control for Dexterous Manipulation from Reference-Scoped Exploration](https://arxiv.org/abs/2509.09671)
*Sirui Xu,Yu-Wei Chao,Liuyu Bian,Arsalan Mousavian,Yu-Xiong Wang,Liang-Yan Gui,Wei Yang*

Main category: cs.RO

TL;DR: Dexplore是一个统一单循环优化方法，直接从大规模运动捕捉数据学习机器人控制策略，避免了传统三阶段流程的误差累积问题


<details>
  <summary>Details</summary>
Motivation: 现有方法采用重定向、跟踪和残差校正的三阶段工作流程，导致演示数据利用不足且误差累积。运动捕捉数据存在不准确性和人机手部差异，限制了直接使用

Method: 联合执行重定向和跟踪的统一优化，将演示作为软指导而非绝对真值。从原始轨迹推导自适应空间范围，通过强化学习训练策略保持在范围内同时最小化控制努力并完成任务

Result: 统一公式保留了演示意图，使机器人特定策略涌现，提高对噪声的鲁棒性，并能扩展到大规模演示语料库。还将跟踪策略提炼为基于视觉的技能条件生成控制器
...
### [140] [DeepTV: A neural network approach for total variation minimization](https://arxiv.org/abs/2409.05569)
*Andreas Langer,Sara Behnamian*

Main category: math.NA

TL;DR: 该论文提出使用神经网络解决无限维全变分最小化问题，发现原始神经网络问题通常无解，因此引入辅助神经网络问题并通过Γ-收敛证明其收敛性，同时提出离散版本并建立与有限差分法的联系。


<details>
  <summary>Details</summary>
Motivation: 神经网络方法在求解偏微分方程方面表现良好，如物理信息神经网络和Deep Ritz方法。本文旨在探索使用神经网络解决无限维全变分最小化问题，但发现原始问题存在理论缺陷。

Method: 提出辅助神经网络问题来解决原始问题无解的理论问题，证明其Γ-收敛性；进一步提出离散版本的辅助神经网络问题，并证明其Γ-收敛到原始无限维问题；建立离散神经网络问题与有限差分离散化之间的联系。

Result: 理论分析表明辅助神经网络问题确实有解且收敛到原始问题；数值实验支持理论发现，验证了所提方法的有效性。

Conclusion: 通过引入辅助问题和离散化策略，成功解决了使用神经网络求解无限维全变分最小化问题的理论障碍，为神经网络在该领域的应用提供了理论保证和实用方法。
...
### [141] [CameraVDP: Perceptual Display Assessment with Uncertainty Estimation via Camera and Visual Difference Prediction](https://arxiv.org/abs/2509.08947)
...
Method: 结合HDR图像堆叠、MTF反演、渐晕校正、几何去失真、单应变换和颜色校正的相机重建管道，并整合视觉差异预测器(VDP)来模拟人眼视觉系统在不同观看条件下对各种刺激的可见性。

Result: 通过三个应用验证框架：缺陷像素检测、颜色边缘感知和显示不均匀性评估，不确定性分析框架能够估计缺陷像素检测性能的理论上限并提供VDP质量分数的置信区间。
...
### [142] [Explainable AI for Accelerated Microstructure Imaging: A SHAP-Guided Protocol on the Connectome 2.0 scanner](https://arxiv.org/abs/2509.09513)
...
TL;DR: 提出基于可解释AI的扩散MRI神经突交换成像优化方案，将扫描时间从15个特征减少到8个特征，在14分钟内完成扫描且保持参数准确性


<details>
  <summary>Details</summary>
Motivation: 现有神经突交换成像协议需要较长扫描时间，限制了其临床应用。需要开发缩短扫描时间同时保持模型准确性的方案

Method: 使用可解释人工智能和引导式递归特征消除策略，从15个特征协议中识别最优8个特征子集，并在体内验证性能

Result: 优化协议产生与完整协议相当的参数估计和皮质图谱，合成数据中估计误差低，测试-重测变异性影响最小，水交换时间估计偏差减少两倍以上

Conclusion: 该混合优化框架可在14分钟内实现可行的神经突交换成像，且不损失参数保真度，为神经科学和临床研究中交换敏感扩散MRI的广泛应用提供支持
...
### [143] [Ultrafast Deep Learning-Based Scatter Estimation in Cone-Beam Computed Tomography](https://arxiv.org/abs/2509.08973)
*Harshit Agrawal,Ari Hietanen,Simo Särkkä*

Main category: eess.SP

TL;DR: 本研究通过在不同分辨率下应用网络并基于速度和准确性选择最优分辨率，显著降低了锥束CT散射校正的深度学习网络计算量和内存需求，实现了78倍FLOPs减少和16倍推理时间加速。


<details>
  <summary>Details</summary>
Motivation: 锥束CT扫描中的散射伪影严重降低图像质量，虽然深度学习方法在散射估计方面显示出潜力，但由于网络内存占用大，在移动CBCT系统或边缘设备上的部署仍然受限。

Method: 首先在六个分辨率下比较四种插值方法的CBCT散射信号上下采样重建误差，然后在五个图像分辨率上训练最新的SOTA方法，评估FLOPs、推理时间和GPU内存需求的减少。

Result: 输入大小和网络参数的减少实现了78倍FLOPs降低，同时保持可比性能（MAPE从4.42%降至3.85%，MSE从2.01×10^{-2}降至1.34×10^{-2}），推理时间和GPU内存使用分别减少16倍和12倍。

Conclusion: 研究强调了降采样在深度学习散射估计中被低估的作用，方法显著降低的FLOPs和GPU内存需求使得在资源受限环境（如移动CBCT和边缘设备）中进行散射校正成为可能。
