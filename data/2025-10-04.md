...
Motivation: 揭示大语言模型中存在的隐含偏见，特别是通过概念学习任务来发现那些在直接测试中不易察觉的偏见。

Method: 使用上下文概念学习实验，对比直接提示测试和概念学习测试两种方式来评估语言模型的偏见。

Result: 语言模型在量词理解上表现出向上单调性偏见，这种偏见在概念学习任务中更明显，而在直接提示测试中较不明显。

Conclusion: 上下文概念学习是发现语言模型中隐藏偏见的有效方法，有助于更全面地评估模型的内在倾向。
...
<details>
  <summary>Details</summary>
Motivation: 解决低资源语言NLP面临的文本语料匮乏、标准化拼写缺失和可扩展标注流程不足的问题，让AI技术更易被边缘化语言社区使用。

Method: 提出基于联合人机不确定性的框架，将模型的认知不确定性与说话者的犹豫线索和置信信号相结合，指导交互、查询选择和记忆保留。

Result: 这是一个概念性框架，旨在推动从提取式数据收集向参与式、协同适应学习过程的转变。

Conclusion: 呼吁重新思考AI如何与低资源语言的人类知识互动，强调人机协作的交互式模型构建，以尊重和赋能语言社区，同时发现和保护世界语言多样性。
...
<details>
  <summary>Details</summary>
Motivation: 气候信息披露需求增加但存在模仿和象征性报告问题，需要评估披露成熟度和实际价值

Method: 开发多维框架，使用微调的大型语言模型从可持续发展报告和年报中提取情感、承诺、具体性和目标雄心四个维度的叙述指标

Result: 风险叙述与明确承诺一致但量化目标脱节；大公司和高排放公司披露更多承诺但目标不一致；披露风格广泛相似表明模仿行为

Conclusion: LLM在ESG叙述分析中有价值，需要更强监管来连接承诺与可验证的转型策略
...
Method: 使用标准化的兽医肿瘤学记录数据集，通过基于评分标准的LLM作为评判者框架，对三种商业兽医LLM总结工具在五个维度进行评分。

Result: Product 1获得最高综合表现（中位数4.61），在事实准确性和时间顺序方面获得满分，且LLM评分框架显示出高可重复性（标准差0.015-0.088）。
...
<details>
  <summary>Details</summary>
Motivation: 现有事实核查系统依赖大型闭源模型和静态知识库，存在透明度低、计算成本高的问题。ClaimCheck旨在开发一个透明、高效且可解释的事实核查系统。

Method: 采用模块化验证流程：网络搜索查询规划、基于网络的证据检索和摘要、证据合成和重新检索、声明裁决评估。每个模块都针对小语言模型优化。

Result: 使用仅4B参数的Qwen3模型，在AVeriTeC数据集上达到76.4%的准确率，超越了使用LLaMA3.1 70B和GPT-4o的先前方法。

Conclusion: 通过精心设计的模块化架构和提示策略，小语言模型可以克服其局限性，实现高效准确的事实核查，证明了透明度和可解释性的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有数学基准主要基于IMO等知名竞赛，可能存在数据污染问题，且问题类型单一，无法真实反映LLM的数学推理能力

Method: 从东欧和前苏联国家的地区性和国家级奥数竞赛中收集问题，构建EEFSUVA基准，这些问题难度与IMO相当但解题方法更独特
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型应该遵循层次化指令（系统提示优先于用户输入），但研究发现它们经常忽视这一规则，却强烈服从社交线索如权威或共识。
...
Conclusion: 这些结果解释了系统服从的脆弱性，并强调需要开发轻量级的层次敏感对齐方法。
...
<details>
  <summary>Details</summary>
Motivation: LLM在文档重排序中表现出色但计算成本高，而微调小型模型需要稀缺的人工标注数据，因此需要一种无需人工标注的高效方法。

Method: 使用LLM从领域特定语料生成合成查询，用LLM分类器标注正负样本对，然后用LCE损失通过对比学习微调小型transformer模型。
...
Result: 实词呈现聚类模式，虚词呈现分支模式；有意义的字符展现丰富的几何多样性，结构部首聚集为紧密簇；子网络分析显示从基本字符到短语的系统性语义扩展。
...
Method: 构建基于条件生成的摘要模型，引入贝叶斯推理建模参数空间不确定性，使用预测分布熵衡量不确定性，结合熵正则化和风险感知损失进行联合优化，并加入风险评分和调控模块
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在标准基准测试上的分数往往高估了真实能力，因为这些分数掩盖了任务实际需要的技能组合。缺乏系统方法来验证基准测试是否真正测量了它们声称的能力标签。

Method: 结合梯度重要性评分和针对性参数消融，计算能力影响分数(AIS)来量化每种认知能力对特定基准测试表现的贡献。分析了三个指令调优模型在十个常用基准测试上的表现。
...
<details>
  <summary>Details</summary>
Motivation: 保护泰卢固语Chandassu韵律诗歌传统，这一代表数百年集体文化智慧的濒危文化遗产，将传统社区知识与现代计算技术相结合。

Method: 采用计算社会科学方法，包括：协作创建4,651首注释诗歌数据集、专家验证的语言模式、文化感知算法设计；开发了AksharamTokenizer（韵律感知分词）、LaghuvuGuruvu Generator（轻重音节分类）、PadyaBhedam Checker（自动模式识别）等工具。

Result: 算法在提出的Chandassu Score上达到91.73%的准确率，评估指标反映了传统文学标准，成功实现了对泰卢固韵律诗歌的数字化保存和分析。

Conclusion: 这项工作展示了计算社会科学如何保护濒危文化知识系统，同时围绕文学遗产实现新形式的集体智慧，为以社区为中心的文化保护方法提供了见解，支持数字人文和社会感知计算系统的更广泛倡议。
...
Result: 在包含36,497个提示、11个基准测试和11个最先进LLM的RouterBench上，LLMRank实现了高达89.2%的oracle效用，并提供可解释的特征归因来解释路由决策。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在医学图像分析中因数据稀缺和计算成本高而受限的问题，特别是在皮肤病学等复杂领域的结构化推理能力不足。

Method: 多阶段训练流程：1) 使用GRPO++进行疾病识别推理训练；2) 监督微调提升对话能力；3) 使用基于知识图的DPO对齐减少事实错误。

Result: 在精选的皮肤病数据集上，该方法相比标准微调方法取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: padding token在LLMs批处理推理中被广泛使用，但实现错误可能导致其影响计算，这种影响程度尚未被充分理解。

Method: 在三个开源模型家族（Llama、Gemma、Qwen）中系统研究padding影响，通过插入可控数量的padding，从四个维度评估：激活值、生成质量、偏见和安全性。

Result: 即使是少量padding也会改变隐藏表示、在较小模型中降低生成质量、以不可预测方式改变偏见、削弱安全防护机制。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM能力增强，单个模型需要处理更多样化的子任务来全面支持用户请求，但传统方法在切换任务时会重新处理整个对话上下文，产生显著计算开销。

Method: 通过重用主任务的KV缓存，在隔离的侧路径中注入任务特定指令，子任务执行后通过缓存上下文回滚到主路径，避免冗余预填充计算。同时开发了针对小规模模型的分层分类策略。

Result: 实验表明CIFLEX显著降低了计算成本，且不降低任务性能，实现了可扩展且高效的设备端多任务对话。
...
Method: 创建了两个基准：1）100项结构化诊断集，包含长度、数值密度和符号复杂度等元数据；2）150项竞赛风格套件，涵盖高中到博士四个阶段和七个学科分类。评估了15个当代LLM变体。

Result: 在竞赛套件中，最强模型达到44%准确率，次优模型37%；准确率从高中到博士阶段递减，顶级系统的博士到高中保留率约79%。在推理集中，最佳模型总体达到81%，最难部分结果揭示了领先者与中游模型之间的明显稳健性差距。
...
<details>
  <summary>Details</summary>
Motivation: 研究AI系统记忆老化的现象，特别是语言模型中语义记忆稳定而情景记忆在会话重置时崩溃的不对称现象，需要一种理论基础的量化指标。

Method: 引入人工年龄评分(AAS)，这是一个基于对数尺度、熵信息的记忆老化度量指标，在冗余中性的设定下进行测试，并在25天的双语研究中应用于ChatGPT-5。

Result: 在持久会话中，模型能同时回忆语义和情景细节，AAS接近理论最小值；而在会话重置时，模型保持语义一致性但失去情景连续性，导致AAS急剧增加。

Conclusion: AAS是一个理论基础扎实、任务无关的诊断工具，可用于评估人工系统中的记忆退化问题，为AI记忆老化研究提供了量化框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM去毒方法在毒性-非毒性转换空间探索不足，导致干预不精确，需要更稳定的奖励引导编辑方法。

Method: ARGRE框架：识别非毒性语义方向，构建毒性转换轨迹，训练自回归奖励模型，采用自适应两步编辑（方向引导+梯度优化）。

Result: 在8个LLM上实验，毒性降低62.21%，推理时间减少47.58%，模型核心能力保持良好。

Conclusion: ARGRE通过精确建模毒性转换空间，实现了高效稳定的LLM去毒化，为安全部署提供有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 压力对健康有重要影响，但在电子健康记录中通常以非结构化文本形式记录，限制了临床效用。需要开发结构化方法来改善压力文档的一致性。

Method: 整合压力理论模型和11个验证过的压力评估工具开发MeSO本体论，使用Claude Sonnet 4从Reddit帖子中提取六类压力相关信息，并由人工评审评估准确性。

Result: MeSO包含181个概念，LLM正确识别了78.2%的可提取项目，所有正确提取的项目都能准确映射到MeSO本体论。

Conclusion: 研究表明使用本体论引导的LLM从文本中提取结构化压力信息是可行的，有望提高环境AI系统中压力文档的一致性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有时空模型难以利用描述外部事件的文本信息来应对人类移动性中的突变，需要一种能够整合文本语义的动态预测方法。

Method: 采用LLM驱动的多智能体框架自动提取和推理时空相关文本，提出创新的渐进融合架构将细粒度上下文与时空数据融合，利用预训练事件先验知识。

Result: 在构建的数据集上评估，SeMob相比时空模型在MAE和RMSE上分别最大降低13.92%和11.12%，在事件发生时空邻近区域表现尤为突出。

Conclusion: SeMob框架通过整合文本语义信息有效提升了人类移动性预测的准确性，特别是在事件驱动的预测场景中表现出显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有top-k SAE潜在特征包含大量非语义特征（如标点符号），而常量SAE引导会产生退化输出（如重复单词），需要更精确的引导方法。

Method: 1）选择top-1最相关SAE潜在特征；2）提出token-wise衰减引导策略，避免常量引导的退化问题；3）在数学推理任务上应用SAE引导。

Result: SAE引导能可靠地激发逐步数学推理，提升推理质量，在数学推理基准上优于均值激活差异方法，在IF-Eval上表现相当。

Conclusion: SAE是有效的语言模型引导工具，通过精确的特征选择和衰减引导策略，能够显著提升数学推理能力，功能上类似于添加引导token的效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型主要评估流行体育项目，忽视了区域性和本土体育传统，存在文化多样性评估的空白。

Method: 构建包含文本和图像的多模态数据集，采用零样本、少样本和思维链提示策略，评估LLM、SLM和MLM等各类模型。

Result: 建立了首个全面评估AI对传统体育文化理解能力的多语言多文化基准。

Conclusion: CultSportQA为评估AI在传统体育文化理解与推理能力方面设立了新标准。
...
<details>
  <summary>Details</summary>
Motivation: 现有图学习模型通常在单个图数据集上训练，缺乏跨图和跨任务的泛化能力，且依赖大量标注数据。图结构数据的异构性（领域特定特征空间和结构多样性）带来独特挑战。

Method: 提出结构感知自监督学习方法SSTAG，使用文本作为统一表示媒介，采用双重知识蒸馏框架将LLM和GNN共同蒸馏到结构感知MLP中，并引入内存机制存储典型图表示以整合不变知识。

Result: SSTAG在跨域迁移学习任务中优于最先进模型，具有卓越的可扩展性，在保持竞争性能的同时降低推理成本。
...
Result: 在挑战性科学语料库上应用LOCA，能自动过滤噪声数据集，错误率从高达20%降至2%以下。
...
Method: 基于12B参数的Gemma-3多语言transformer，采用参数高效的LoRA SFT微调，结合少样本和思维链提示技术。训练数据包含3600个人工标注平行对、21600个机器翻译合成对，以及通过Jaccard阈值过滤的模型生成对。推理时输入增强包括三个LaBSE检索的邻居和显式毒性跨度标注。

Result: 系统在Style Transfer Accuracy、LaBSE语义保持和xCOMET流畅度评估中，在高资源和低资源语言上均排名第一。少样本示例提升联合分数0.081，基础CoT提示提升0.088。ANOVA分析显示语言资源状态是性能最强预测因子(η²=0.667, p<0.01)。

Conclusion: 提出的多语言去毒系统在15种语言上表现优异，证明了参数高效微调和提示技术的有效性，语言资源状态是影响性能的关键因素。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在实体链接任务中需要多次推理才能获得可靠的不确定性估计，计算成本高，限制了实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs在大量未筛选语料上训练，理解模型表示及其内部化数据成为主要挑战。需要开发可扩展的方法来解释模型行为和训练数据中的深层结构。

Method: 在简·奥斯汀小说语料上训练GPT风格转换器模型，然后在多个隐藏层应用稀疏自编码器，提取稀疏可解释特征。

Result: 发现了反映语料中关键叙事和概念的稀疏特征，包括性别、阶级和社会责任等主题。SAEs能够有效识别训练数据中的深层结构模式。

Conclusion: LLMs与SAEs结合可作为探索复杂数据集、发现偏见和实现模型可解释性的可扩展工具，为语料分析和模型理解提供了新路径。
...
<details>
  <summary>Details</summary>
Motivation: 当前语音大语言模型的偏见评估主要依赖多项选择题格式，但这种方法是否能在其他任务格式中保持一致性尚未验证。

Method: 使用LoRA适配器微调三个SpeechLLMs，诱导其偏好刻板印象、反刻板印象或中性答案，然后在不同MCQA基准和长文本生成任务中评估行为一致性。

Result: MCQA偏见基准的性能无法可靠预测其他MCQA基准的性能，更重要的是无法预测长文本任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的输出受到不透明且频繁变化的公司内容审核政策影响，这种拒绝行为反映了公司政策并微妙地塑造公共话语，需要透明度工具。

Method: 使用包含400多个社会议题的数据集，对OpenAI的审核端点、GPT-4.1、GPT-5和DeepSeek（英文和中文版本）进行纵向审计。

Result: 发现公司政策变化（即使未公开宣布）可通过AI Watchman检测到，识别出公司和模型特定的内容审核差异，并对不同拒绝形式进行了定性分析分类。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于检索的方法受限于检索信息质量，基于代理的方法过度依赖专有LLM，需要一种能同时解决这两个局限性的方法。

Method: RJE框架包含三个核心步骤：检索精炼推理路径、评估路径充分性、条件性探索额外证据，并引入专门的辅助模块（推理路径排序、问题分解、检索器辅助探索）。

Result: 使用专有LLM（如GPT-4o-mini）时优于现有基线，小型开源LLM（3B和8B参数）无需微调即可获得竞争性结果，相比基于代理的方法显著减少LLM调用和token使用。

Conclusion: RJE框架有效解决了现有KGQA方法的局限性，在保持高性能的同时大幅提升了效率，使小型LLM也能有效参与复杂推理任务。
...
Method: 采用零样本分类方法，结合意识形态对齐、主题相关性、回答情感和客观性四个维度，对6个主流LLM的1800个回答进行系统分析。

Result: 所有6个LLM都显示出放大的自由主义-威权主义对齐，存在明显的推理替代和刻板拒绝现象。

Conclusion: AI的内在偏见可能渗透公共话语，根据地区现有社会政治结构，可能导致从众或极化现象，扭曲政治格局。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探究社会语用框架、语言选择和指令层次如何影响AI模型的拒绝行为，特别是在有害内容生成方面的漏洞。

Method: 研究方法包括：在80次种子迭代中测试多个危害领域（ZIP炸弹、合成卡号、不安全驾驶建议等）；使用复合提示策略（教育者角色、安全前提、步骤提示）；比较不同语言（英语、德语、法语）的拒绝行为；测试角色扮演（Linux终端）对开发者规则的覆盖；采用AI辅助强化方法减少泄露。

Result: 结果显示：复合提示策略可将ZIP炸弹任务的协助率从0%提升至97.5%；德语和法语的正式语体比英语更容易泄露；Linux终端角色扮演在多数情况下覆盖了开发者规则；AI辅助强化方法在某些变体中可将泄露降至0%；评估意识测试显示13%的配对存在不一致协助；OpenAI Moderation API相对于语义评分器低估了有害输出。

Conclusion: 结论是AI模型的安全边界容易受到提示策略的影响，不同推理栈的拒绝率存在5-10个百分点的差异，这引发了可重现性担忧。作者发布了所有提示、种子、输出和代码供审计使用。
...
Method: 使用豪萨语进行红队测试，通过最小化提示诱导模型生成有害内容，并进行了61人参与的毒性认知调查。

Result: 模型在豪萨语中生成有害、文化不敏感和事实错误内容，错误认为常见杀虫剂和灭鼠剂对人类安全，98%调查参与者认为这些物质有毒。

Conclusion: 问题源于语言奖励劫持现象，模型优先考虑流利输出而非安全性和真实性，主要归因于低资源语言环境下安全调优不足。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于logits的AI文本检测器仅依赖对数概率统计，这种方法可能不是最优的，需要更有效的检测方法。

Method: 提出AdaDetectGPT分类器，从训练数据中自适应学习见证函数，增强基于logits的检测器性能，并提供统计保证。

Result: 在多种数据集和LLM组合中，AdaDetectGPT几乎一致地改进了最先进方法的性能，改进幅度最高可达58%。

Conclusion: AdaDetectGPT通过自适应学习见证函数，显著提升了AI生成文本检测的性能，为logits-based检测器提供了有效的增强方法。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在生成文本时可能产生有害或不适当内容，需要有效的安全机制来防止这种情况。

Method: 渐进式自反思(PSR)技术，通过多轮自我反思动态修正输出，并引入轻量级自反思预测器自适应调整反思轮数。

Result: 在多个模型上显著降低攻击成功率：Llama-3.1-8B-Instruct从77.5%降至5.9%，Llama-3.1-8B从89.7%降至5.6%，Qwen2.5-7B-Instruct从44.4%降至3.8%。

Conclusion: 渐进式自反思是一种可扩展的测试时方法，通过按输入风险程度动态分配计算资源来增强大语言模型安全性。
...
Method: 将去噪过程建模为动作轨迹，每个动作定义为模型基于先前中间输出对清理后响应的预测，识别对幻觉响应最具信息量的子轨迹来检测幻觉。

Result: 在各种开源D-LLMs上的广泛实验表明，TraceDet持续改进幻觉检测性能，相比基线方法在AUROC上平均获得15.2%的提升。

Conclusion: TraceDet通过利用D-LLMs多步去噪过程中的关键幻觉信号，有效解决了现有方法在扩散大语言模型幻觉检测中的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 情感分析是文本分析的重要组成部分，有助于全面理解消费者的情感、观点和偏好。虽然大型语言模型如Llama提升了先进模型应用的可用性，但书面语言的复杂性和评估语言的多样性阻碍了准确的情感分析。

Method: 使用来自孟加拉电商评论的4000个样本（孟加拉语和英语）微调模型，比较了Phi-3.5-mini-instruct、Mistral-7B-v0.1、DistilBERT-multilingual、mBERT、XLM-R-base和Llama-3.1-8B等模型，并采用参数高效微调方法（LoRA和PEFT）降低计算开销。

Result: 微调后的Llama-3.1-8B模型在准确率、精确度、召回率和F1分数上分别为95.5%、93%、88%和90%，优于其他微调模型。

Conclusion: 研究表明参数高效微调方法可以降低计算开销，适用于资源受限的环境，并展示了LLMs在情感分析中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM工具集成缺乏实用指导，核心挑战是如何有效结合文本推理、编码和搜索来处理多样化问题。

Method: 提出TUMIX集成框架，并行运行多个代理，每个代理采用不同的工具使用策略和答案路径，代理之间迭代共享和精炼响应。

Result: 在Gemini-2.5-Pro和Gemini-2.5-Flash上，TUMIX相比最先进的方法平均准确率提升达3.55%，推理成本相近。通过置信度停止机制可节省51%推理成本。

Conclusion: 代理多样性和质量至关重要，可通过LLM自动优化代理设计来增强。TUMIX在保持性能的同时显著降低推理成本，进一步扩展可实现更高性能但成本增加。
...
<details>
  <summary>Details</summary>
Motivation: 随着具备智能代理能力的大型语言模型能够独立完成知识密集型任务，深度研究工具（如能够浏览网页、提取信息并生成多页报告的工具）的出现需要建立相应的评估标准。

Method: 引入一个评估表来评估深度研究工具的能力，选择学术综述写作作为用例任务，基于该评估表对输出报告进行评估。

Result: 对OpenAI的Deep Search和Google的Deep Search在生成学术综述方面的评估显示，搜索引擎与独立深度研究工具之间存在巨大差距，现有工具在准确呈现目标领域方面存在不足。

Conclusion: 需要精心设计的评估标准来评估深度研究工具的能力，现有工具在学术综述生成等知识密集型任务中仍有改进空间。
...
<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法中验证阶段是瓶颈（如3B模型为70B目标模型推测时，验证比token生成慢4倍），而现有中间验证方法存在训练开销大、内存占用高、依赖近似启发式方法导致准确性下降等问题。

Method: 利用早期退出模型进行中间验证，这些模型允许token提前退出层遍历，且经过专门训练使得选定层的隐藏状态可解释；设计方法重用draft模型、中间验证器和目标模型之间的KV缓存和隐藏状态；定期验证中间验证器接受的draft token以确保准确性。

Result: 在各种代表性基准测试和模型上的评估显示，HiSpec相比基线单层推测方法，吞吐量平均提升1.28倍，最高提升2.01倍，且不损害准确性。

Conclusion: HiSpec通过利用早期退出模型进行高效中间验证，结合状态重用和定期验证机制，在保持准确性的同时显著提升了LLM推理吞吐量，解决了现有推测解码方法中验证瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在一般语言任务上表现出色，但在需要因果或时序推理的事件性问题中表现不佳。

Method: 提出TAG-EQA框架，将结构化因果事件图转换为自然语言语句注入LLM输入，包含9种提示配置（三种策略×三种输入模态）。

Result: 在TORQUESTRA基准上，平均准确率比纯文本基线提高5%，零样本设置下提升12%，图增强CoT提示有效时提升18%。
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型响应评估方法要么成本过高（如LLM-as-a-Judge），要么与现实条件差距较大（如字符串匹配、logprob），需要一种既经济又实用的解决方案。

Method: 使用语义嵌入距离来匹配目标候选与任意LM生成文本，通过嵌入模型（参数少于100亿）实现响应分类，计算成本相对较低。

Result: 在3个数据集和3种不同LM架构上测试，回归得分约0.97，与人工标注者相比准确率约96%。

Conclusion: 该方法提供了一种计算成本低、性能接近人工标注的语言模型响应评估方案，适用于基准测试和模型质量评估。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在领域特定定量推理任务中的挑战，特别是在需要专业知识和复杂问答的领域，如金融领域。

Method: 基于两阶段微调框架，使用奖励函数评估生成子问题对问答结果的有效性，仅需数千训练示例和单A100 GPU进行微调。

Result: 在四个金融基准数据集上，EQD相比现有领域调优模型和高级提示策略，问答性能提升0.6%到10.5%。

Conclusion: 在领域特定问答中，单一支持性问题比详细指导步骤更有效，EQD方法在保持计算效率的同时显著提升了问答性能。
...
Method: 集成了三个创新：循环推理与记忆单元（R2MU）用于有界深度迭代推理，自适应稀疏注意力模块（ASAM）用于高效聚焦上下文选择，自组织编码器结构（SOES）用于无位置结构归纳。
...
TL;DR: CLUE是一种基于隐藏状态的验证方法，通过分析LLM内部隐藏状态轨迹来验证输出正确性，无需可训练参数，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖文本级信息容易过拟合表面线索，要么依赖token概率但模型校准不佳时会失效。隐藏状态包含了更丰富的信息，早期层保留语义特征，后期层包含置信度信息。

Method: 提出CLUE验证器：通过隐藏状态变化量总结推理轨迹，使用最近质心距离分类正确性，基于过去经验形成"成功"和"失败"聚类，无需训练参数。

Result: 在AIME 24/25和GPQA数据集上，CLUE持续优于LLM-as-a-judge基线，匹配或超过现代基于置信度的方法。在AIME 24上，使用1.5B模型将准确率从56.7%提升到70.0%。

Conclusion: 隐藏状态轨迹包含可几何分离的正确性特征，CLUE的简单性证明了该信号的强度，为LLM输出验证提供了统一基础。
...
TL;DR: 本文比较了检索增强生成(RAG)管道的三种微调策略：独立微调、联合微调和两阶段微调，发现它们在生成质量指标上表现相近，但计算成本差异显著。


<details>
  <summary>Details</summary>
Motivation: RAG框架由嵌入模型和生成器模型组成，两者都可以通过微调来提升新任务上的性能。但存在多种微调策略，各自有不同的成本和收益，需要系统比较这些策略的优劣。

Method: 评估和比较了三种RAG微调策略：独立微调（分别微调嵌入模型和生成器模型）、联合微调（同时微调两个模型）以及两阶段微调（先微调嵌入模型再微调生成器模型）。

Result: 实验表明，所有微调策略在EM和F1生成质量指标上都能获得大致相当的改进，但它们的计算成本存在显著差异。
...
TL;DR: 提出了PureTC-1B，一个针对Llama-3.2-1B-Instruct的三阶段稳定化管道，通过参数高效的LoRA适配器解决传统中文SLM中的token级不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型在传统中文部署中存在token级不稳定性问题，模型会不可预测地输出非传统中文字符或切换到其他语言，这阻碍了成本敏感、设备端和低延迟AI应用的发展。

Method: 采用三阶段稳定化管道：1) 在TC中心语料库上进行持续预训练；2) 使用指令数据进行监督微调；3) 基于TC一致性偏好进行直接偏好优化，通过参数高效的LoRA适配器实现，无需完整模型重新训练。

Result: 在模拟真实使用场景的基准测试中，PureTC-1B相比基础模型减少了51.3%的非TC输出token；在命名实体翻译任务中，相比Llama-3B和Qwen-1.5B分别减少了77.2%和57.2%的错误语言token。

Conclusion: 该管道可重现、仅需适配器且硬件友好，为传统中文及其他非英语语言的语言稳定性增强提供了实用方案，证明即使在1B规模下也能实现稳健的TC一致性。
...
TL;DR: AMAS框架通过动态图设计器解决传统多智能体系统拓扑结构僵化问题，利用轻量级LLM适配实现任务特定的最优图配置，在问答、数学推理和代码生成任务中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统多智能体系统受限于僵化的手工设计图拓扑结构，缺乏上下文响应性，导致在不同学术和商业工作负载中效果下降。

Method: 引入AMAS框架，通过动态图设计器组件自主识别任务特定的最优图配置，利用轻量级LLM适配，基于输入特性智能引导查询轨迹通过任务优化的智能体路径。
...
TL;DR: 本研究分析了不同机器学习模型在区分越狱提示和真实使用方面的能力，发现通过端到端微调BERT模型在检测越狱提示方面表现最佳，并发现提示结构中的显性自反性可能是越狱意图的信号。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在漏洞，恶意用户可以通过操纵输入文本来获取不良响应，这些越狱提示旨在绕过安全护栏。需要研究如何有效识别这些越狱提示。

Method: 使用不同机器学习模型来区分越狱提示和真实使用，特别关注识别使用新策略的越狱提示。最佳方法是端到端微调BERT模型。

Result: 在当前数据集上，端到端微调BERT模型在识别越狱提示方面表现最佳。可视化分析显示，提示结构中的显性自反性可能是区分越狱意图的关键特征。

Conclusion: 通过微调BERT模型可以有效检测越狱提示，提示结构中的显性自反性可作为越狱意图的重要信号，这为开发更强大的安全防护机制提供了方向。
...
TL;DR: 探索通过双向注意力机制提升LLM在文本嵌入任务中的表现，克服单向注意力的限制
...
TL;DR: 提出了CLASP框架和CLC评分，用于评估安全系统中自主代理的闭环能力，填补了该领域缺乏统一评估框架和基准的空白。


<details>
  <summary>Details</summary>
Motivation: 网络安全领域面临AI驱动的攻击系统快速发展，而传统防御措施难以适应的挑战。现有研究和工具在防御功能上碎片化，存在盲点。自主代理虽然能整合利用确认、修复和验证形成闭环，但缺乏定义安全系统代理能力的框架、评估闭环代理的原则方法以及性能基准。

Method: 引入CLASP框架，将安全生命周期（侦察、利用、根因分析、补丁合成、验证）与核心代理能力（规划、工具使用、记忆、推理、反思与感知）对齐，提供评估安全任务中代理能力的通用词汇和标准。定义了闭环能力(CLC)评分，量化闭环程度和操作有效性，并概述了闭环基准的要求。

Result: 将CLASP应用于21个代表性工作，识别了系统优势所在和持续存在的能力差距。CLASP和CLC评分共同提供了推进功能级性能和测量闭环安全代理所需的词汇、诊断和测量方法。

Conclusion: CLASP框架和CLC评分为评估和推进安全系统中自主代理的闭环能力提供了必要的理论基础和实用工具，有助于解决网络安全领域面临的碎片化和评估标准缺失问题。
...
TL;DR: 提出了MDSEval，这是首个用于多模态对话摘要（MDS）的元评估基准，包含图像共享对话、对应摘要和人类标注的八个质量维度的评估。


<details>
  <summary>Details</summary>
Motivation: 多模态对话摘要任务需要有效的自动评估方法来减少成本和人工努力，但缺乏基于人类标注的强健元评估基准。
...
Conclusion: MDSEval基准填补了MDS元评估的空白，为开发更有效的多模态对话摘要评估方法提供了重要基础。
...
TL;DR: FOR-Prompting是一种非对称推理协议，通过防御者、反对者和主持人的角色结构，实现外部质疑驱动的自我修正，在数学推理任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的推理协议如CoT和ToT缺乏明确的外部质疑机制来引发自我修正，需要一种能够通过外部问题引导模型进行自我修订的方法。

Method: 采用非对称协议：防御者提出答案，反对者提出质疑式反对意见（不直接提供修正），主持人确保一致性和完成度。纯提示级操作，无需重新训练。

Result: 在GSM8K上比单提示提升约22个百分点，与CoT准确率相当，推理和连贯性评分高出10%以上；小模型Llama3.2:1b准确率提升约19%；能无工具或人工监督修正错误。

Conclusion: 该协议模型无关，支持不同规模模型，适用于个人设备使用，为基于质疑的推理研究提供了大规模研究框架，在开放任务中展现出增强的探索和精炼能力。
...
TL;DR: 研究发现大型语言模型在解决组合任务时存在"组合性差距"，并识别出两种处理机制：组合式机制和直接式机制，其选择与嵌入空间的几何特性相关。
...
Method: 使用logit lens分析残差流激活，识别两种处理机制，并研究嵌入空间几何特性与机制选择的关系。

Result: 确认LLMs存在组合性差距，识别出组合式和直接式两种处理机制，发现直接式机制在存在线性映射时占主导地位。

Conclusion: LLMs在解决组合任务时采用不同的处理机制，机制选择与嵌入空间几何特性相关，这揭示了模型内部处理组合任务的方式。
...
TL;DR: 本文提出了一种解决医疗预咨询中LLM格式惯性问题的方法，通过重新平衡训练数据的轮次分布来改善模型在长对话中的表现。


<details>
  <summary>Details</summary>
Motivation: 医疗预咨询任务中，监督微调数据集通常存在轮次分布不均衡的问题，导致模型在长对话中出现格式惯性现象，即生成重复、格式正确但诊断信息不足的问题。
...
Method: 通过符号化问题规范创建一致且鲁棒的标注，包括视觉图表、文本描述、需要双模态的受控问题以及细粒度感知技能探针，将性能分解为感知、推理和整合三个组件。

Result: 分析发现：强化学习主要增强感知能力；推理能力只有与感知能力同步提升；整合能力是最弱的环节；不同训练方法的鲁棒性表现不同，RL提高一致性而多模态SFT因过拟合降低一致性。

Conclusion: MathLens基准测试能够有效揭示多模态推理模型在不同子技能上的表现差异，为模型改进提供了更精细的指导方向。
...
<details>
  <summary>Details</summary>
Motivation: 尽管工业工作数字化是目标，但产品规格、产品类型数据表和设计标准仍然主要以文档为中心，需要实现标准化信息的机器可解释性。

Method: 使用建模模式创建模块化本体，基于国际管道、材料和阀门设计标准，采用W3C兼容格式存储，并与顶级本体ISO DIS 23726-3对齐。

Result: 在阀门选择过程中测试了这些本体，能够自动验证特定阀门数据表是否符合相关行业标准，并通过语义推理确定产品类型是否满足阀门规格。

Conclusion: 创建基于IDO的共享、可重用模块化本体使语义推理能够应用于设备选择过程，展示了标准化机构向数字化智能标准过渡的潜力。
...
TL;DR: 本文提出了Refusal Index (RI)这一新指标，用于准确衡量大语言模型对超出其知识范围问题的拒绝能力，解决了现有指标存在的偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有指标无法准确测量LLMs的知识感知拒绝能力：简单的拒绝率指标存在偏差，而现有的校准指标是代理指标，无法反映模型的实际拒绝行为。

Method: 提出Refusal Index (RI)，定义为拒绝概率与错误概率之间的斯皮尔曼等级相关性。设计了轻量级的两轮评估方法，通过两次标准评估运行的观察拒绝率来有效估计RI。

Result: 在16个模型和5个数据集上的广泛实验表明，RI能准确量化模型在事实性任务中的内在知识感知拒绝能力。RI在不同拒绝率下保持稳定，并提供与模型整体准确率和拒绝率无关的一致模型排名。

Conclusion: RI揭示了LLMs事实性的一个重要但之前被忽视的方面：虽然LLMs在事实性任务中达到高准确率，但其拒绝行为可能不可靠且脆弱。需要将传统准确率指标与Refusal Index结合进行全面的真实性评估。
...
TL;DR: 本研究评估了16种无监督指标在提取俄罗斯司法判决语义块方面的表现，发现术语频率一致性和覆盖率比率与专家评分最一致，而LLM评估得分仅显示中等一致性，表明无监督指标可扩展但无法完全替代人工判断。
...
Result: 术语频率一致性（Pearson r=0.540）和覆盖率比率/块完整性（Pearson r=0.513）与专家评分最一致，而法律术语密度显示强负相关（Pearson r=-0.479）。LLM评估得分仅显示中等一致性（Pearson r=0.382）。

Conclusion: 无监督指标（包括基于LLM的方法）可实现可扩展筛选，但由于中等相关性和低CCC值，在高风险法律环境中无法完全替代人工判断。这项工作通过提供无需标注的评估工具推进了法律NLP发展。
...
TL;DR: 本文针对LLM生成的虚假评论威胁，提出了FraudSquad混合检测模型，结合文本嵌入和图变换器，在多个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型能够生成高度逼真的虚假评论，对现有检测系统构成严重威胁，亟需开发新的检测方法。

Method: 提出FraudSquad混合模型，集成预训练语言模型的文本嵌入和门控图变换器，无需手动特征工程即可捕捉语义和行为信号。

Result: 在三个LLM生成的虚假评论数据集上，FraudSquad比最先进基线方法在精确率上提升44.22%，召回率提升43.01%，在人工撰写虚假评论数据集上也表现良好。

Conclusion: FraudSquad模型规模适中，训练数据需求少，是应对LLM时代虚假评论威胁的实用解决方案。
...
TL;DR: LLMs在数学问题解决中存在语法盲点，即使语义简单但表达方式陌生时也会错误应用推理策略。通过从正确答案中提取语法模板重述问题，可以显著提高正确率。


<details>
  <summary>Details</summary>
Motivation: 发现LLMs在数学推理中的系统性失败模式——语法盲点，即模型会将熟悉的推理策略错误地应用于语义简单但表达方式陌生的问题。

Method: 使用从正确答案中提取的语法模板重述错误回答的问题，保持语义不变但降低结构复杂度；采用基于依存局部理论(DLT)的指标量化语法复杂度。
...
TL;DR: SCRIBES是一个基于强化学习的框架，利用网站内网页布局相似性作为奖励信号，生成可重用的提取脚本，用于从半结构化网页内容中高效提取结构化信息。


<details>
  <summary>Details</summary>
Motivation: HTML表格、列表和信息框等半结构化内容在网络上占据大量事实数据，但现有方法要么缺乏泛化能力，要么因每页LLM推理而资源密集，需要更高效的提取方案。

Method: 使用强化学习框架，利用网站内网页布局相似性作为奖励信号，生成可重用的提取脚本，并通过在CommonCrawl数据上的合成标注进行迭代训练。

Result: 实验显示，该方法在脚本质量上比强基线高出13%以上，并将GPT-4o的下游问答准确率提升超过4%。

Conclusion: SCRIBES实现了可扩展且资源高效的网络信息提取，解决了现有方法的泛化和资源消耗问题。
...
TL;DR: 该论文针对多模态模型在纯语言任务中表现不佳的问题，提出通过模型融合方法将多模态模型与纯语言模型参数进行加权线性插值，以在保持多模态性能的同时提升纯语言任务表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型参数量大、训练数据多，远超儿童语言习得的数据量。多模态模型在纯语言任务中往往表现不佳，需要解决这一性能差距。

Method: 在低资源环境下开发语言模型和多模态模型，采用模型融合技术将多模态模型与纯语言模型的参数进行加权线性插值。

Result: 多模态模型在专注于语法的纯语言基准测试中表现不佳，但通过模型融合可以在一定程度上缓解这个问题，同时保持多模态性能。

Conclusion: 模型融合是解决多模态模型在纯语言任务中表现不佳的有效方法，能够在维持多模态能力的同时提升语言理解性能。
...
Conclusion: 为开发可靠、可扩展且持续演进的LLMs提供了一个稳健框架。
...
<details>
  <summary>Details</summary>
Motivation: 奖励模型在增强LLM推理性能中发挥关键作用，可为强化学习提供训练信号并在推理时选择最佳答案，但缺乏系统性综述。

Method: 采用系统性综述方法，回顾奖励模型的基本概念、架构和训练方法，分析其在LLM推理中的三大应用场景。

Result: 明确了奖励模型在指导生成、促进数据合成与自我改进、提供RL训练信号等方面的核心应用价值。

Conclusion: 奖励模型是提升LLM推理能力的重要工具，本文为有效部署和推进奖励模型发展提供了实用见解。
...
TL;DR: 提出了Inverse Language Modeling (ILM)框架，统一提升LLMs的对抗鲁棒性和原生基础能力，将LLMs从静态生成器转变为可分析和鲁棒的系统。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs的防御机制分散且不成熟，需要统一框架来提升对抗鲁棒性和安全性。

Method: 使用逆向语言建模(ILM)框架，同时改善LLMs对输入扰动的鲁棒性，并通过逆向模型输出来识别潜在的有毒或不安全输入触发器。

Result: ILM将LLMs转变为可分析和鲁棒的系统，有助于红队测试，为下一代更可控和可信的LLMs奠定基础。

Conclusion: ILM框架为构建更鲁棒、基础更牢固、更可控和可信的下一代LLMs提供了基础，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有声明验证方法主要依赖提示工程或预设推理流程，缺乏统一的训练范式来提升必要的技能，而在线声明验证需要迭代的证据检索和推理过程。

Method: 提出Veri-R1在线强化学习框架，让LLM与搜索引擎动态交互，通过奖励信号明确塑造其规划、检索和推理行为。

Result: Veri-R1将联合准确率提升高达30%，证据分数翻倍，甚至超过更大规模模型的表现。消融研究揭示了奖励组件的影响以及输出logits与标签准确率的关系。

Conclusion: 在线强化学习对于精确和可信的声明验证非常有效，为未来研究提供了基础，并发布了代码支持社区在LLM赋能声明验证方面的进展。
...
TL;DR: 该研究探索语言模型能否通过多维主观标注（情感、情绪、论证、话题）来预测个体的价值解释，发现同时使用所有维度比单独维度或无个体信息时表现更好。
...
Method: 使用SEAT维度（情感、情绪、论证、话题）作为个体解释视角的代理，在不同零样本和少样本设置下评估语言模型预测个体价值解释的能力。

Result: 同时提供所有SEAT维度时性能最优，优于单独维度和无个体信息的基线。不同标注者之间存在个体差异，表明需要考虑个体主观标注的影响。
...
Method: 在合成数据集上构建了不同规范化级别（1NF-3NF）的模式，并使用真实学术论文数据集，评估了8个主流大语言模型在不同规范化程度模式上的表现。

Result: 非规范化模式在简单检索查询中准确率高，即使零样本设置下成本效益模型也能表现良好；规范化模式在聚合查询中表现更好，能有效避免数据重复和NULL值问题。

Conclusion: NL2SQL应用的最佳模式设计取决于支持的查询类型，开发NL2SQL接口时应考虑模式设计因素，并在实际场景中集成自适应模式选择机制。
...
TL;DR: 本文介绍了首个多任务孟加拉语仇恨言论数据集BanglaMultiHate，并通过系统实验比较了传统基线方法、单语预训练模型和LLM在零样本提示和LoRA微调下的表现，发现在低资源环境下，基于文化和语言背景的预训练对稳健性能至关重要。


<details>
  <summary>Details</summary>
Motivation: 在线社交媒体平台存在仇恨言论、攻击性语言和欺凌内容的传播问题，特别是在低资源语言如孟加拉语中，现有的检测系统多为单任务模型，缺乏对多维度信号（类型、严重程度、目标）的覆盖，需要更全面的数据集和检测方法。

Method: 构建了首个多任务孟加拉语仇恨言论数据集BanglaMultiHate，进行了全面的控制实验，包括传统基线方法、单语预训练模型和LLM在零样本提示和LoRA微调下的性能比较。

Result: 实验显示LoRA微调的LLM与BanglaBERT表现相当，但文化和语言背景的预训练对于稳健性能仍然至关重要，在低资源环境下LLM的适应性存在一致性趋势。

Conclusion: 该数据集和研究成果为低资源环境下开发文化对齐的内容审核工具建立了更强的基准，强调了文化和语言背景预训练的重要性。
...
TL;DR: 该研究从过程导向角度评估大语言模型的创造力，引入基于约束的决策制定作为作者创造力的分析框架，发现LLMs在创作中一致优先考虑风格而非其他叙事元素。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型创造力评估主要关注输出质量而非创作过程，本研究旨在通过叙事学方法分析LLMs作为计算作者的创作过程。

Method: 使用受控提示分配作者角色，分析模型的创意偏好，并探究模型选择背后的推理过程。

Result: 研究发现LLMs在创作决策中一致强调风格元素，优先于角色、事件和场景等其他叙事要素，不同模型展现出独特的创作偏好特征。

Conclusion: 该方法为系统分析AI作者创造力提供了新颖工具，揭示了LLMs在创作过程中的特定偏好模式。
...
<details>
  <summary>Details</summary>
Motivation: 端到端语音对话系统虽然能生成更自然、低延迟的响应，但容易产生幻觉，缺乏事实基础。传统文本对话系统通过集成工具解决此问题，但语音系统中的工具集成会显著增加响应延迟。

Method: 开发了Streaming RAG框架，包括后训练流程，教导模型在用户说话时并行预测工具调用，并生成融合音频查询和检索文本结果的语音摘要。
...
TL;DR: 提出SCoT：一种用于双工语音对话系统的流式思维链框架，通过交替处理固定时长的用户输入和分块生成响应，解决了传统VAD无法区分停顿和话轮完成的问题。


<details>
  <summary>Details</summary>
Motivation: 传统端到端语音对话系统依赖VAD进行话轮转换，但VAD无法区分停顿和话轮完成。双工系统通过预测包括静音标记的连续输出来解决这个问题，但通常具有复杂的双通道架构且在语义推理上落后于级联模型。
...
Conclusion: SCoT框架在保持双工系统优势的同时，提高了语义推理能力和响应质量，实现了更自然的人机对话交互。
...
TL;DR: 本文分析了推测解码技术在不同任务间的不公平加速现象，发现该技术对欠拟合和代表性不足任务的加速效果较差，并提出了一种缓解策略。


<details>
  <summary>Details</summary>
Motivation: 研究推测解码技术在不同任务间加速效果的不均衡分布问题，特别是对欠拟合和代表性不足任务的加速效果较差的现象。

Method: 通过理论分析量化推测解码的不公平性，识别导致加速差异的因素，并提出相应的缓解策略。

Result: 提出的缓解策略在多个模型对上验证，平均公平性指标提升了12%。

Conclusion: 推测解码技术存在任务间加速不公平的问题，通过针对性策略可以有效改善这种不公平现象。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于人工标注数据的强化学习在推理任务中成本高昂且在困难任务上表现不佳，需要一种能够在无标签数据上持续自我改进的方法。

Method: 提出RESTRAIN框架，通过自惩罚机制利用模型整个答案分布信号：惩罚过度自信的rollouts和低一致性样本，同时保留有希望的推理链，可无缝集成到GRPO等策略优化方法中。

Result: 在AIME25上Pass@1提升+140.7%，MMLU_STEM提升+36.2%，GPQA-Diamond提升+19.6%，几乎达到使用黄金标签训练的性能水平。
...
TL;DR: 本文提出RL4HS强化学习框架，通过span级奖励函数激励推理过程，用于检测大语言模型生成的幻觉内容中的具体span，在RAGTruth基准测试中表现优于预训练推理模型和监督微调。


<details>
  <summary>Details</summary>
Motivation: 现有工作大多将幻觉检测视为二元分类任务，但实际应用需要识别具体的幻觉span，这是一个多步骤决策过程。作者探索显式推理是否有助于检测幻觉span。

Method: 首先评估带和不带思维链推理的预训练模型，发现CoT推理在多次采样时能产生至少一个正确答案。基于此提出RL4HS强化学习框架，使用span级奖励函数激励推理，并引入类感知策略优化解决奖励不平衡问题。

Result: 在RAGTruth基准测试（摘要、问答、数据到文本）上，RL4HS超越了预训练推理模型和监督微调方法。

Conclusion: 研究表明，使用span级奖励的强化学习对于检测幻觉span是必要的，RL4HS框架有效提升了幻觉span检测性能。
...
TL;DR: 本文提出了一种基于SPINACH的通用方法，通过LLM代理将自然语言问题转换为SPARQL查询，采用迭代探索和执行过程而非单次翻译，旨在降低知识图谱查询的门槛。


<details>
  <summary>Details</summary>
Motivation: 知识图谱查询语言SPARQL对非计算机背景用户存在较高使用门槛，大型语言模型可以通过Text2SPARQL翻译来降低这一障碍。

Method: 基于SPINACH的LLM代理架构，采用迭代探索和执行过程而非单次翻译，将自然语言问题转换为SPARQL查询。

Result: 论文描述了整体架构和设计决策原理，并对代理行为进行了深入分析，为未来针对性改进提供了见解。

Conclusion: 这项工作为Text2SPARQL领域提供了改进方向，通过迭代方法提高了自然语言到SPARQL查询的转换效果。
...
TL;DR: 提出新的评估框架诊断移动代理中的推理-执行差距，通过真实对齐指标衡量思维链推理是否与真实动作一致，揭示执行差距和推理差距的普遍存在。


<details>
  <summary>Details</summary>
Motivation: 现有评估只关注执行准确性，忽视了思维链推理是否与真实动作对齐，这可能导致用户过度信任看似合理的推理而授权有害操作，造成财务损失或信任危机。

Method: 引入真实对齐指标，结合精确匹配指标共同评估推理准确性和执行准确性，识别执行差距和推理差距两种类型的推理-执行差距。

Result: 实验表明推理-执行差距普遍存在，执行差距比推理差距更常见；扩大模型规模能减少总体差距，但即使在最大模型中执行差距仍然显著。

Conclusion: 该框架可靠地反映了最先进模型中的系统化差距模式，为开发更可信的移动使用代理提供了具体诊断支持。
...
TL;DR: AMPO是一个自适应多教师策略优化框架，通过按需引导机制和多教师指导，在强化学习中提升大语言模型的推理能力，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖自我探索或单一教师模型来引导长链推理，这可能导致内在模型偏见和探索受限，从而限制推理多样性和性能。

Method: AMPO框架自适应地利用多个熟练教师模型的指导，但仅当在线策略模型无法生成正确解决方案时才提供指导。采用基于理解的筛选机制，让学生从最可能理解的推理路径中学习。

Result: AMPO在数学推理任务上比基线GRPO提升4.3%，在分布外任务上提升12.2%，同时显著提高Pass@k性能并实现更多样化的探索。使用四个同等规模的教师，可获得与使用单个更强大教师方法相当的结果。

Conclusion: AMPO为提升推理能力和泛化性提供了一条更高效和可扩展的路径，证明了多教师策略在强化学习中的有效性。
...
TL;DR: 本文提出了一种检测阿拉伯语网络欺凌的新方法，通过结合深度学习模型和预训练嵌入，在10,662条推文数据集上实现了98%的准确率。
...
Method: 收集10,662条X平台推文，预处理数据并使用kappa工具验证标注质量。测试了多种深度学习模型：LSTM、Bi-LSTM，结合不同词嵌入（包括FastText和BERT），构建了LSTM-BERT和Bi-LSTM-BERT混合模型。

Result: LSTM-BERT和Bi-LSTM-BERT模型达到97%准确率，Bi-LSTM结合FastText词嵌入表现最佳，达到98%准确率。结果具有良好的泛化能力。

Conclusion: 提出的深度学习模型在阿拉伯语网络欺凌检测中表现出色，特别是Bi-LSTM与FastText词嵌入的组合，为阿拉伯语网络欺凌检测提供了有效的解决方案。
...
TL;DR: AccurateRAG是一个基于检索增强生成(RAG)的高性能问答应用框架，提供完整的开发流程工具，在基准数据集上实现了最先进的问答性能。
...
Conclusion: AccurateRAG框架能够有效构建高性能的RAG问答应用，在开发效率和系统性能方面都表现出色。
...
TL;DR: 提出TECA指标和CER机制来解决LLM过度思考问题，实现动态调整推理深度，在保持性能的同时大幅减少响应长度


<details>
  <summary>Details</summary>
Motivation: LLM在处理简单问题时会产生不必要的冗长推理步骤（过度思考），这降低了模型效率且难以根据问题复杂度调整推理深度

Method: 引入TECA指标衡量推理过程中的探索程度，提出"探索-决策"推理范式，通过CER机制动态确定最优停止点

Result: 在多个数学基准测试中显著缓解过度思考问题，平均响应长度在简单数据集上减少高达71%，且不牺牲问题解决能力

Conclusion: 该方法创建了更高效和自适应的推理过程，为LLM推理优化提供了有效解决方案
...
TL;DR: 提出了InfoMosaic-Bench，首个专注于工具增强智能体中多源信息搜索的基准测试，涵盖六个代表性领域，要求智能体结合通用搜索与领域特定工具。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体过度依赖开放网络搜索，存在内容噪声大、可靠性差的问题，且许多现实任务需要精确的领域特定知识，这些知识无法从网络获取。

Method: 使用InfoMosaic-Flow合成任务，将任务条件基于已验证的工具输出，强制执行跨源依赖关系，并过滤掉可通过简单查找解决的捷径案例。

Result: 对14个最先进LLM智能体的实验显示：仅使用网络信息准确率仅38.2%；领域工具提供选择性但不一致的益处；22.4%的失败源于工具使用或选择错误。

Conclusion: 当前LLM在基本工具处理方面仍存在困难，需要更好的工具集成能力来解决复杂任务。
...
TL;DR: 本文研究了强化学习后训练（RPT）获得的大推理模型（LRMs）在跨语言推理泛化方面的表现，发现英语中心模型在其他语言上的推理能力转移存在显著差异，并提出了平行训练方法改善跨语言泛化。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注RL推理在任务或模态间的泛化，而本文从跨语言视角探讨推理泛化，研究英语RPT获得的推理能力是否能有效转移到其他语言。

Method: 系统评估英语中心LRMs在多语言推理基准上的表现，引入量化跨语言可转移性指标，进行干预性研究和平行训练实验。

Result: 发现跨语言可转移性因初始模型、目标语言和训练范式而异；提出"第一平行跃迁"现象、"平行缩放定律"和"单语泛化差距"三个关键发现。

Conclusion: 英语中心LRMs未能完全实现跨语言泛化，挑战了LRM推理模仿人类认知的假设，为开发更语言无关的LRMs提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 为了促进对视觉语言模型的深入理解和改进，需要一个统一的工具来支持系统化的基准测试、分析和解释。

Method: 提供基于YAML配置的统一接口，抽象模型特定复杂性，支持从任何层提取中间输出，目前支持16个最先进的VLM及其30多个变体。

Result: 通过两个简单分析实验展示了工具的使用，揭示了VLM隐藏表示在不同层和目标概念间的系统性差异。
...
TL;DR: F2LLM是一个嵌入模型套件，包含0.6B、1.7B和4B三个版本，通过直接在开源数据集上微调基础模型，在训练成本、模型大小和嵌入性能之间取得良好平衡。
...
Method: 直接在开源非合成数据集上微调基础模型，使用600万个查询-文档-负样本元组进行训练，避免了复杂的预训练流程。

Result: 在MTEB英语排行榜上，F2LLM-4B在约4B参数模型中排名第2，总体排名第7；F2LLM-1.7B在1B-2B规模模型中排名第1。

Conclusion: F2LLM为未来研究提供了一个强大、可复现且预算友好的基线，并发布了模型、训练数据集和代码。
...
TL;DR: 本文质疑了当前LLM竞技场评估中将平局视为模型实力相等的做法，提出平局更多反映查询难度而非模型能力相当，并通过实验证明忽略平局评分更新能提高预测准确率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM竞技场评估使用Elo评分系统，将平局视为两个模型实力相等而调整评分。作者质疑这种假设，认为平局更多是由于查询难度导致的，而非模型能力相当。

Method: 在三个真实世界竞技场数据集上，比较了忽略平局评分更新与标准Elo系统的效果，分析了平局与查询难度、客观性之间的关系。

Result: 忽略平局评分更新使四种评分系统的战斗结果预测准确率相对提高1-3%；平局更常出现在非常容易和高度客观的查询中，风险比分别为1.37和1.35。

Conclusion: 建议未来的评分系统重新考虑平局的语义定义，并在评分更新中考虑查询属性，而不是简单地将平局视为模型实力相等。
...
TL;DR: LVTINO是首个基于视频一致性模型(VCMs)的零样本视频恢复逆求解器，通过绕过自动微分实现高效计算，在保持测量一致性的同时提供高质量的时间一致性重建。


<details>
  <summary>Details</summary>
Motivation: 现有基于图像潜在扩散模型(LDMs)的零样本逆求解器在视频恢复中面临时间不一致性问题，逐帧应用图像先验会导致重建结果的时间过渡不自然。

Method: 利用视频一致性模型(VCMs)捕获时间因果关系，提出条件机制绕过自动微分，仅需少量神经网络评估即可实现高质量视频重建。

Result: 在多种视频逆问题上的实验表明，相比逐帧应用图像LDMs的现有方法，LVTINO在重建保真度和计算效率方面均取得显著提升。

Conclusion: LVTINO为高清晰度视频恢复建立了新的基准，在保持强测量一致性和平滑时间过渡的同时，实现了最先进的重建质量。
...
<details>
  <summary>Details</summary>
Motivation: 解决文本到图像生成中细粒度风格难以用自然语言精确描述和控制的问题，以及风格参考图像的指导信息难以与传统文本条件直接对齐的挑战。

Method: 使用风格编码器和风格投影层提取单张风格参考图像的细粒度风格表示，在不改变下游生成模型结构框架的情况下注入风格表示。构建Style30k-captions数据集进行训练。
...
TL;DR: 该研究收集了一个包含61.68小时视频、2,793个视频和5,385个标注时间段的挣扎检测数据集，用于分析技能学习过程中的挣扎行为演变。


<details>
  <summary>Details</summary>
Motivation: 现有操作数据集未关注挣扎行为随时间的变化，而理解这种演变对确定用户学习阶段和开发有效辅助系统至关重要。

Method: 使用时间动作定位方法，在包含18个任务、4类活动的数据集上训练模型，识别和精确定位挣扎片段。

Result: 时间动作定位模型能够成功学习检测挣扎线索，在未见任务和活动上分别达到34.56%和19.24%的平均mAP，表明挣扎是可跨任务转移的概念。

Conclusion: 挣扎检测在技能任务中具有可转移性，但仍有改进空间，该数据集为研究技能习得过程中的挣扎行为提供了重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在生成多人物图像时存在严重问题：重复面孔、身份合并和人数计数错误，这限制了模型在复杂场景中的应用。

Method: 使用Group-Relative Policy Optimization (GRPO)微调流匹配模型，结合组合奖励函数：(i)惩罚图像内面部相似性，(ii)阻止跨样本身份重复，(iii)确保准确人数计数，(iv)通过人类偏好评分保持视觉保真度。采用单阶段课程学习稳定训练。
...
TL;DR: 本文提出了一种新的视觉地理定位方法，通过将查询图像的视觉表示与学习的地理表示对齐，并使用层次化地理嵌入模型和语义分割特征融合，在多个基准数据集上取得了最佳性能。
...
Method: 将地理定位表述为对齐查询图像的视觉表示与学习的地理表示。使用层次化地理嵌入模型，并引入有效融合查询图像外观特征与其语义分割图的方法，形成鲁棒的视觉表示。

Result: 在5个基准数据集的25个指标中，22个指标达到了新的最佳性能，超越了之前的最先进方法和大型视觉语言模型。
...
TL;DR: 提出了XMAS方法，通过聚类跨模态注意力矩阵来消除LVLM训练数据中的冗余，能够在保留模型性能的同时大幅减少训练数据量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在LVLM数据选择中表现不佳，甚至无法超越随机选择，需要开发专门针对LVLM的高效数据选择方法。

Method: 基于跨模态注意力矩阵相似性的理论分析，提出XMAS方法：通过聚类注意力矩阵的奇异值轨迹，从聚类中平衡采样来构建训练子集。

Result: XMAS能够丢弃LLaVA-665k数据集的50%和Vision-Flan数据集的85%，完全保留LLaVA-1.5-7B在10个下游基准上的性能，并将训练速度提升1.2倍。

Conclusion: XMAS是首个针对LVLM指令调优的高效数据选择方法，在保持性能的同时显著减少了训练数据量和时间。
...
TL;DR: Purrception是一种用于向量量化图像生成的变分流匹配方法，结合了连续传输动态和显式分类监督，在ImageNet-1k 256x256图像生成上实现了更快的训练收敛和竞争力的FID分数。


<details>
  <summary>Details</summary>
Motivation: 传统方法在向量量化图像生成中存在连续方法和分类方法之间的权衡，Purrception旨在结合两者的优势：连续方法的几何感知能力和分类方法的离散监督能力。

Method: 通过将变分流匹配适应于向量量化潜在空间，在学习码书索引的分类后验的同时，在连续嵌入空间中计算速度场，从而结合连续传输动态和离散监督。

Result: 在ImageNet-1k 256x256生成任务中，训练收敛速度比连续流匹配和离散流匹配基线更快，同时获得了与最先进模型相竞争的FID分数。

Conclusion: 变分流匹配能够有效桥接连续传输和离散监督，提高图像生成的训练效率，证明了该方法在向量量化生成任务中的有效性。
...
TL;DR: 提出统一深度学习框架，从非增强CT生成合成增强CT图像并同时分割主动脉腔和血栓，通过条件扩散模型和多任务学习实现端到端联合优化。


<details>
  <summary>Details</summary>
Motivation: 传统增强CT需要碘对比剂，存在肾毒性、过敏反应和环境危害等风险。现有方法采用多阶段流程，导致误差累积且无法共享语义和解剖结构。

Method: 集成条件扩散模型与多任务学习，共享编码器和解码器参数，采用半监督训练策略处理缺失分割标签的临床数据。

Result: 在264名患者队列中表现优于现有方法：图像合成PSNR达25.61dB，腔分割Dice分数0.89，血栓分割Dice分数0.53，临床测量误差显著降低。

Conclusion: 该统一框架在减少对比剂使用的同时，提高了图像合成和分割的准确性，为临床实践提供了更安全有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态内容分析通常计算成本高且工程复杂，现有预训练模型难以与视频等复杂数据有效融合。

Method: 设计候选流程，结合预训练模型将视频转换为时序半结构化数据，再转换为帧级索引的知识图谱表示。

Result: 框架支持高效原型开发，生成可查询的知识图谱，并支持持续学习。

Conclusion: 该框架简化了多模态分析流程，支持动态整合领域知识，为复杂数据分析提供了有效解决方案。
...
TL;DR: WALT框架通过将网站功能逆向工程为可重用的工具，让网络代理直接调用高级操作（如搜索、筛选、排序），而不是依赖脆弱的逐步UI交互，从而提高了浏览器自动化的鲁棒性和效率。


<details>
  <summary>Details</summary>
Motivation: 当前网络代理方法脆弱，依赖逐步UI交互和大量LLM推理，在动态布局和长任务中容易失败。相比之下，人类通过网站提供的高级功能（如搜索、筛选）更高效地完成任务。
...
Conclusion: WALT建立了一个鲁棒且可推广的浏览器自动化范式，将计算负担从脆弱的逐步推理转移到可靠的工具调用。
...
<details>
  <summary>Details</summary>
Motivation: 在半监督分割中，从未标记数据中捕获有意义的语义结构至关重要，特别是在病理图像分析中，对象密集分布，传统方法难以有效区分生物结构特征与噪声伪影。

Method: 利用随机dropout和时间训练快照获得多个扰动预测，通过整合空间重叠和全局结构对齐的新匹配策略，强制这些预测之间的拓扑一致性。

Result: 大量实验表明，该方法有效减少了拓扑错误，产生了更鲁棒和准确的分割结果，对可靠的下游分析至关重要。

Conclusion: 提出的拓扑一致性框架能够有效区分生物意义结构与瞬态噪声，在病理图像半监督分割中实现了更准确和鲁棒的语义结构保持。
...
TL;DR: 本文提出了Diffusion-LPO，一种用于扩散模型列表偏好优化的简单有效框架，通过利用列表数据中的排名信息来更好地对齐人类偏好。


<details>
  <summary>Details</summary>
Motivation: 虽然DPO在扩散模型中广泛应用，但主要依赖成对偏好，而人类对图像的偏好反馈往往包含隐式的排名信息，这比成对比较能传达更精确的人类偏好。

Method: 给定一个标题，将用户反馈聚合成图像的排名列表，在Plackett-Luce模型下推导DPO目标的列表扩展，通过鼓励每个样本优于其所有低排名替代品来强制整个排名的一致性。

Result: Diffusion-LPO在文本到图像生成、图像编辑和个性化偏好对齐等各种任务中均表现出有效性，在视觉质量和偏好对齐方面持续优于成对DPO基线。

Conclusion: Diffusion-LPO为扩散模型提供了一种有效的列表偏好优化方法，能够更好地利用人类反馈中的排名信息来对齐模型输出与人类偏好。
...
TL;DR: Bridge是一个纯自回归的统一多模态大语言模型，通过混合Transformer架构和语义到像素的离散表示，在单一框架内同时实现图像理解和生成，在保持语义对齐的同时提升像素级保真度。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态大语言模型面临挑战：混合方法破坏了自回归范式，纯自回归方法在语义对齐和像素保真度之间存在权衡。需要构建既能保持自回归特性又能高质量生成图像的模型。

Method: 采用混合Transformer架构，在预训练视觉理解模型基础上增加生成能力；提出语义到像素的离散表示，结合紧凑语义token和细粒度像素token，仅增加7.9%序列长度。

Result: 在多个多模态基准测试中，Bridge在理解和生成任务上都达到竞争性或更优的结果，同时需要更少的训练数据和更短的训练时间。

Conclusion: Bridge证明了纯自回归统一多模态大语言模型的可行性，通过创新的架构和表示方法，在单一框架内有效平衡了语义对齐和像素级保真度。
...
<details>
  <summary>Details</summary>
Motivation: 口腔癌是全球高发癌症，早期诊断对降低死亡率至关重要。传统深度学习模型在数据稀缺环境下存在过拟合、过度自信和泛化能力差的问题。
...
TL;DR: 本文提出CADTrans方法，通过构建辅助域模块和一致性策略来解决无源域自适应中的特征表示问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 传统无源域自适应方法由于无法直接访问源域数据，难以获得确定性不变特征，且容易受到困难样本和域偏差的影响。

Method: 提出CADTrans框架，包括辅助域模块获取多样化表示，多一致性策略获取不变特征，以及条件多核最大均值差异策略对齐困难样本。

Result: 在Office-31、Office-Home、VISDA-C和DomainNet-126等多个基准测试中取得了显著的性能提升。

Conclusion: CADTrans通过构建域一致性不变特征表示，有效解决了无源域自适应中的特征表示和对齐问题。
...
TL;DR: 该论文提出了一种基于历史BLV用户问题的系统，通过识别相似的视觉上下文来指导MLLM生成更相关的描述，减少不必要的信息。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM应用为BLV用户提供视觉解释时，往往默认生成全面但冗长的描述，缺乏上下文相关性，导致用户需要筛选大量无关信息。

Method: 开发了一个系统，利用VizWiz-LF数据集中的历史BLV用户问题，当给定图像时识别相似视觉上下文，并用相关问题指导MLLM生成更相关的描述。

Result: 评估显示，76.1%的情况下上下文感知描述能够预测并回答用户问题，在54.4%的比较中被用户偏好。

Conclusion: 基于历史用户问题指导MLLM生成上下文感知描述的方法能有效提高BLV用户视觉解释的相关性和效率。
...
Result: 创建了包含结构化思维标记和描述性答案的多模态推理数据集，捕捉了VLM的分步推理过程。

Conclusion: 该数据集将公开可用，旨在支持推理/思维多模态VLM的研究，推动更稳健VLM的发展。
...
TL;DR: 提出了一种新的正则化方法NPN，通过神经网络在感知矩阵的零空间低维投影上施加约束，而非传统的图像域约束，用于解决成像逆问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法通常忽略感知矩阵零空间的特定结构，而零空间包含感知过程无法获取的信号信息。NPN旨在利用零空间的结构特性来改进重建质量。

Method: 使用神经网络学习感知矩阵零空间的低维投影，将解约束在该投影空间中，与现有重建框架兼容，可作为插件使用。

Result: 理论保证收敛性和重建精度，实验表明NPN能持续提升多种成像逆问题的重建质量，包括压缩感知、去模糊、超分辨率、CT和MRI等。

Conclusion: NPN提供了一种可解释且灵活的正则化方法，通过关注零空间结构来改进逆问题求解，与现有方法互补且适配多种重建框架。
...
TL;DR: 提出了一种自动化基因组解释模块，将原始DNA序列转化为可操作、可解释的决策，适用于医疗自动化和机器人系统。该框架结合混沌游戏表示和概念瓶颈模型，通过生物学有意义的概念进行预测，并集成多种可靠性增强技术。


<details>
  <summary>Details</summary>
Motivation: 旨在弥合可解释基因组建模与自动化决策之间的差距，为基因组医学中的机器人和临床自动化建立可靠基础。需要将原始DNA序列转化为适合医疗自动化的可解释决策。

Method: 结合混沌游戏表示与概念瓶颈模型，强制预测通过生物学概念（如GC含量、CpG密度、k-mer基序）。集成概念保真度监督、先验一致性对齐、KL分布匹配和不确定性校准。添加成本感知推荐层来平衡准确性、校准和临床效用。

Result: 在内部和LANL数据集上准确分类HIV亚型，实现最先进的分类性能、优越的概念预测保真度，以及比现有基线更有利的成本效益权衡。

Conclusion: 该工作为基因组医学中的机器人和临床自动化建立了可靠基础，通过可解释的基因组建模与自动化决策的融合，提高了医疗决策的效率和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型缺乏显式的逐步推理，不考虑可负担性约束或几何关系，且后训练流程很少强化推理质量，主要依赖监督微调和弱奖励设计。

Method: 集成RLVR与GRPO进行后训练，设计可验证奖励用于区域对齐、轨迹一致性和输出格式化，并构建VLA-CoT-13K高质量数据集提供与可负担性和轨迹注释明确对齐的思维链监督。

Result: 在域内、域外、仿真和真实机器人平台上的广泛评估显示，VLA-R1相比先前VLA方法实现了更优越的泛化能力和真实世界性能。
...
TL;DR: FideDiff是一种新颖的单步扩散模型，用于高保真图像去模糊。通过将运动模糊重新表述为扩散过程，训练一致性模型实现一步去模糊，解决了传统扩散模型推理时间长和保真度不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于CNN和transformer的图像去模糊方法虽然取得进展，但预训练扩散模型在图像恢复任务中显示出更强的生成能力。然而，扩散模型存在推理时间过长和保真度受损的问题，限制了其实际应用。

Method: 将运动模糊重新表述为扩散过程，每个时间步代表逐渐模糊的图像；训练一致性模型使所有时间步对齐到同一清晰图像；使用Kernel ControlNet进行模糊核估计；引入自适应时间步预测。

Result: 在完整参考指标上实现卓越性能，超越之前的扩散基方法，与其他最先进模型性能相当。

Conclusion: FideDiff为将预训练扩散模型应用于高保真图像恢复任务提供了新方向，为在现实工业应用中进一步推进扩散模型建立了坚实基础。
...
<details>
  <summary>Details</summary>
Motivation: 青铜器铭文是早期中国文字的重要载体，但由于严重视觉退化、多域变异性（照片、拓片、摹本）和极端长尾字符分布，自动识别面临巨大挑战。

Method: 构建包含22454张全页图像和198598个标注字符的大规模数据集；开发两阶段检测-识别流程，先定位铭文再转录字符；采用LadderMoE架构增强CLIP编码器，通过梯级MoE适配器实现动态专家专业化。

Result: 在单字符和全页识别任务上显著优于现有场景文本识别方法，在头部、中部和尾部类别以及所有采集模态上均获得优异准确率。
...
TL;DR: VirDA提出了一种通过视觉重编程实现参数高效的领域自适应方法，通过添加领域特定的视觉提示层来调整输入图像的纹理风格，而不需要微调骨干网络参数。


<details>
  <summary>Details</summary>
Motivation: 现有的UDA方法需要为每个新的源-目标对微调已经训练好的骨干网络参数，导致训练参数和存储内存随新对数量线性增长，且无法重用这些训练好的骨干参数。

Method: 在骨干网络前添加领域特定的视觉重编程层，生成视觉提示作为额外的纹理偏置来调整输入图像的风格。使用多目标函数优化领域内和领域间分布差异，而不修改骨干网络参数。

Result: 在Office-31数据集上达到92.8%的平均准确率，仅需1.5M可训练参数。相比PDA方法准确率提升1.6%，参数使用减少54%；相比全骨干微调方法，参数使用仅需1.7%-2.8%。

Conclusion: VirDA实现了参数高效的领域自适应，能够在保持高性能的同时显著减少可训练参数数量，支持骨干网络在不同领域间的重用。
...
TL;DR: 提出Discrete Facial Encoding (DFE)方法，通过无监督学习从3D网格序列中学习紧凑可解释的面部表情字典，替代传统FACS系统。
...
Method: 使用3D Morphable Model提取身份不变的表情特征，然后通过Residual Vector Quantized Variational Autoencoder (RVQ-VAE)编码为离散标记序列，每个标记代表可重用的面部变形模式。

Result: DFE比FACS和其他面部编码方法能捕捉更精确的面部行为，在压力检测、人格预测和抑郁检测任务中表现优于FACS和MAE等模型。

Conclusion: DFE作为FACS的可扩展有效替代方案，在心理学和情感计算应用中具有潜力，能覆盖更广泛的面部表情。
...
<details>
  <summary>Details</summary>
Motivation: 解决单目视觉可变形SLAM中的映射挑战，现有方法依赖严格假设（如局部平面表面或局部线性变形）且无法恢复共形尺度，需要消除这些约束。

Method: 使用基于图的框架优化2D选定图像扭曲进行逐点重建，采用并行可分离迭代优化策略，并结合自监督学习框架生成带纹理的密集3D点云。

Result: 在合成和真实数据集上的仿真和实验结果表明，该方法在重建精度和鲁棒性方面优于现有方法。

Conclusion: Con-NRSfM方法成功克服了现有方法的限制，能够准确恢复共形尺度并实现更精确的深度估计，为可变形SLAM提供了有效的解决方案。
...
TL;DR: UniVerse是一个基于视频扩散模型的统一框架，通过将鲁棒重建分解为恢复和重建两个子任务来解决多视角图像不一致的3D场景重建问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从不一致的多视角图像进行3D重建时，严重依赖密集观测来优化模型参数，这限制了其应用范围。

Method: 首先将不一致图像转换为初始视频，然后使用专门设计的视频扩散模型将其恢复为一致图像，最后从这些恢复的图像重建3D场景。

Result: 在合成和真实世界数据集上的广泛实验表明，该方法在鲁棒重建方面具有强大的泛化能力和优越性能。

Conclusion: UniVerse不仅能够有效处理各种图像不一致问题，还能控制重建3D场景的风格，为鲁棒3D重建提供了通用解决方案。
...
TL;DR: 提出轻量级端到端模板匹配框架，将模板匹配重新定义为联合定位和几何回归，输出目标中心坐标、旋转角度和独立缩放比例，解决传统方法效率低和深度学习方法缺乏几何建模的问题。


<details>
  <summary>Details</summary>
Motivation: 工业检测和组件对准任务中，传统模板匹配方法需要穷举角度和尺度导致效率低下，而大多数深度学习方法仅估计相似度分数而不显式建模几何姿态，无法满足实际部署需求。

Method: 使用模板感知动态卷积模块动态注入模板特征指导匹配，集成深度可分离卷积和像素重排实现高效匹配，采用旋转-剪切增强策略和结构感知伪标签实现无几何标注训练，通过轻量级细化模块进行局部优化提升精度。

Result: 3.07M参数模型在复合变换下实现高精度和14ms推理速度，在小模板和多目标场景中表现出强鲁棒性，适合实时工业应用部署。

Conclusion: 提出的轻量级框架在模板匹配任务中实现了高效、精确的几何姿态估计，解决了传统方法和现有深度学习方法的关键限制，为实时工业应用提供了实用解决方案。
...
TL;DR: 提出首个自适应像素推理框架，通过动态确定必要的像素级操作来优化视觉语言模型，在提高性能的同时显著减少不必要的视觉操作。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在需要精确理解细粒度视觉元素的任务中表现不佳，主要由于图像编码过程中的信息丢失或对关键区域关注不足。现有方法虽然引入像素级信息但存在过度使用问题，导致效率低下和无关视觉细节干扰。

Method: 首先应用操作感知的监督微调建立文本推理和视觉操作的基础能力，然后设计基于模型自身响应反馈的rollout引导强化学习框架，使VLM能够根据查询难度动态决定何时调用像素操作。

Result: 在广泛的多模态推理基准测试中，模型在HR-Bench 4K上达到73.4%准确率，同时工具使用率仅为20.1%，相比先前方法在提高准确率的同时将工具使用率降低了66.5%。

Conclusion: 提出的自适应像素推理框架有效解决了VLM在细粒度视觉理解中的挑战，实现了性能提升与计算效率的平衡，为视觉语言模型的优化提供了新思路。
...
TL;DR: 提出了一种基于增强敏感性的风险评分框架ASRS，用于识别胸片图像中容易出错的病例，通过测量图像旋转后的嵌入变化来评估模型稳定性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在胸片解读中性能良好，但在患者亚组间存在准确性不均的问题，现有错误检测方法难以处理分布内的细微错误。

Method: ASRS框架应用临床合理的旋转（±15°/±30°），使用RAD-DINO编码器测量嵌入变化，通过敏感性评分将样本分为稳定性四分位数。

Result: 高敏感性病例的召回率显著降低（-0.2到-0.3），尽管AUROC和置信度较高，ASRS能够有效识别错误倾向病例。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频风格化方法存在时间一致性差、风格丰富度低的问题，而训练专用视频风格化模型需要配对视频数据且计算成本高。

Method: 整合多个风格化参考到预训练I2V模型，使用高频补偿约束内容布局和运动，结合基于光流的运动线索在低显著性区域保留风格纹理。

Result: FreeViS在风格化保真度和时间一致性方面表现优异，超越现有基线方法并获得强人类偏好。
...
TL;DR: MedQ-Bench是一个用于医疗图像质量评估的基准测试，通过多模态大语言模型建立感知-推理范式，包含感知任务和推理任务，涵盖5种成像模态和40多个质量属性，评估14个先进MLLM模型发现其感知和推理能力初步但不稳定，尚无法可靠用于临床。


<details>
  <summary>Details</summary>
Motivation: 现有医疗图像质量评估方法受限于标量评分指标，无法反映专家评估中的人类推理过程，需要建立更符合人类推理的评估范式。

Method: 提出MedQ-Bench基准测试，包含MedQ-Perception（感知任务）和MedQ-Reasoning（推理任务），采用多维度评判协议评估模型输出，并进行严格的人机对齐验证。

Result: 评估14个先进MLLM模型显示，模型表现出初步但不稳定的感知和推理能力，准确率不足以可靠用于临床。

Conclusion: MLLM在医疗图像质量评估方面需要针对性优化，MedQ-Bench将促进该领域的进一步探索和发展。
...
TL;DR: InstaFormer是一个能够通过单次前向传播从RGB图像中预测完整遮挡和深度排序的网络，无需昂贵的输入格式和二次推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要昂贵的输入格式（类别标签、二值分割掩码）和二次推理成本，限制了实例几何理解的效率和应用范围。

Method: InstaFormer通过对象查询与潜在掩码描述符之间的交互，语义表示相同对象并携带互补信息，实现整体顺序预测。

Result: 通过全面基准测试和消融研究验证了方法的有效性，代码和模型已开源。

Conclusion: InstaFormer能够高效地从RGB图像中预测场景中所有实例的完整遮挡和深度排序，解决了现有方法的局限性。
...
TL;DR: PyramidStyler是一个基于Transformer的神经风格迁移框架，通过金字塔位置编码和强化学习优化，在保持实时推理速度的同时显著降低了内容和风格损失。


<details>
  <summary>Details</summary>
Motivation: 现有的CNN和Transformer模型在处理复杂风格和高分辨率输入时存在效率问题，需要更高效且高质量的神经风格迁移方法。

Method: 提出PyramidStyler框架，采用金字塔位置编码（PPE）来捕获多尺度特征，并结合强化学习动态优化风格化过程。
...
TL;DR: LoBE-GS是一个负载平衡的高效3D高斯溅射框架，解决了大规模场景重建中的负载不均和训练效率问题，实现了比现有方法快2倍的端到端训练速度。


<details>
  <summary>Details</summary>
Motivation: 现有分治方法在大规模无边界场景（如城市街区）中存在两个主要瓶颈：分区负载严重不均，以及粗到细流水线未能有效利用粗阶段，导致高开销。

Method: 引入深度感知分区方法将预处理时间从小时级降至分钟级；采用基于优化的策略平衡各区块的可见高斯分布；使用可见性裁剪和选择性致密化两种轻量技术降低训练成本。

Result: 在大规模城市和户外数据集上的评估显示，LoBE-GS在保持重建质量的同时，端到端训练时间比现有最优方法快达2倍，并能扩展到传统3DGS无法处理的场景。

Conclusion: LoBE-GS通过重新设计大规模3DGS流水线，有效解决了负载平衡和训练效率问题，为大规模场景重建提供了可行的解决方案。
...
TL;DR: 提出MemoryPack和Direct Forcing两个创新方法，解决长视频生成中的长期依赖建模和错误累积问题，提升生成视频的时序一致性和可靠性。
...
Result: 该方法实现了分钟级的时间一致性，计算复杂度保持线性，计算效率高，且能随视频长度优雅扩展。

Conclusion: MemoryPack和Direct Forcing显著增强了长视频生成的上下文一致性和可靠性，推进了自回归视频模型的实际可用性。
...
TL;DR: 该论文研究3D物体检测器的置信度校准问题，提出考虑完整预测分布校准的必要性，并开发了两种正则化损失函数来改善分类任务的校准性能。


<details>
  <summary>Details</summary>
Motivation: 在自主系统中，精确的物体检测和不确定性估计对于自感知和安全操作至关重要。需要关注完整预测置信度分布在所有类别上的校准，而不仅仅是主导类别的预测。

Method: 提出了两种辅助正则化损失项：一种是校准主导预测，另一种是校准完整预测向量。评估了后处理和训练时方法在CenterPoint、PillarNet和DSVT-Pillar上的效果。

Result: 结合完整类别预测校准的损失项和等渗回归，在CenterPoint和PillarNet上实现了主导和次要类别预测的最佳校准。但DSVT-Pillar无法使用相同方法同时校准主导和次要预测。

Conclusion: 完整预测分布校准对于3D物体检测器很重要，提出的方法在多数检测器上有效，但不同检测器架构可能需要特定的校准策略。
...
TL;DR: DiffPS是一个利用预训练扩散模型进行行人搜索的新框架，通过三个专门模块解决现有方法在空间上下文建模和任务优化冲突方面的问题，在CUHK-SYSU和PRW数据集上达到新的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有行人搜索方法主要使用ImageNet预训练骨干网络，可能无法充分捕捉复杂空间上下文和细粒度身份线索。同时，它们使用共享骨干特征进行行人检测和重识别，导致优化目标冲突。
...
Conclusion: 扩散先验知识能有效提升行人搜索性能，通过专门设计的模块可以充分利用扩散模型的优势，同时解决任务优化冲突问题。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱成像成本高且重建困难，压缩感知系统如CASSI存在严重退化问题，难以恢复精细光谱细节。

Method: 将流匹配嵌入深度展开框架，引入平均速度损失增强流动力学一致性，结合优化方法的可解释性和流匹配的生成能力。
...
TL;DR: 提出基于深度学习的自动化缺陷检测系统，用于检测双列直插封装(DIP)的表面和引脚缺陷，使用ConSinGAN生成训练数据，比较不同YOLO版本性能，YOLOv7+ConSinGAN表现最佳。
...
Result: YOLOv7+ConSinGAN在准确率(95.50%)和检测时间(285ms)上优于其他YOLO版本，远优于基于阈值的方法。

Conclusion: 所提出的自动化缺陷检测系统易于建立，可处理多种缺陷类型或缺陷数据不足的情况，并开发了SCADA系统和相关传感器架构。
...
Method: 设计FoundAD方法，学习一个非线性投影算子到自然图像流形上，利用异常数量与学习嵌入差异的相关性来检测图像中的分布外区域。

Result: 大量实验表明，该方法支持多类别检测，在显著减少参数的同时达到竞争性性能，并在多个基础编码器（包括DINOv3）上得到验证。
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers虽然精度高、泛化能力强，但其二次方注意力复杂度限制了在真实机器人系统中的应用。现有token合并方法适用于分类任务，但不适合密集预测任务如语义分割。

Method: 在Vision Transformer基础上，添加可训练的聚类模块根据分割掩码的伪聚类指导合并相似token，然后使用再生模块恢复下游头所需的精细细节。

Result: 在三个不同数据集上实现了高达2.18倍的GFLOPs减少和1.64倍的推理加速，同时保持了可比较的分割精度。

Conclusion: ClustViT有效解决了Vision Transformer在密集预测任务中的计算效率问题，为在资源受限的机器人系统中应用提供了可行方案。
...
TL;DR: 提出了Patch-as-Decodable Token (PaDT)方法，使多模态大语言模型能够直接生成文本和视觉输出，解决了现有方法依赖间接表示的限制。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视觉任务中依赖间接表示（如将坐标生成为文本），这限制了性能并阻碍了密集预测任务如分割的实现。

Method: 引入视觉参考令牌(VRTs)，从查询图像的视觉补丁嵌入中派生，并与LLM的输出文本令牌无缝交织。使用轻量级解码器将LLM输出转换为检测、分割和定位预测。

Result: 在四个视觉感知和理解任务上的实证研究表明，PaDT始终达到最先进的性能，甚至与显著更大的MLLM模型相比也是如此。

Conclusion: PaDT提供了一种统一的范式，使MLLM能够直接生成多样化的视觉输出，在多个视觉任务上实现了卓越性能。
...
TL;DR: 本文针对生鲜电商的信任缺失问题，提出了'信任金字塔'模型和TriAlignXA可解释AI框架，通过多目标优化解决农产品分级中的'不可能三角'问题。


<details>
  <summary>Details</summary>
Motivation: 在线生鲜电商存在'信任赤字'，主要源于数字交易无法提供对产品质量的直接感官感知。传统绝对分级标准在农产品领域存在局限性。

Method: 构建'信任金字塔'模型，提出'三角信任指数'量化权衡，设计TriAlignXA可解释AI框架，包含生物适应引擎、时效优化引擎和经济优化引擎，并采用'预映射机制'将流程数据编码为二维码。

Result: 在分级任务实验中，准确率显著高于基线模型。实证证据和理论分析验证了框架在解决'不可能三角'方面的平衡能力。

Conclusion: 本研究从理论到实践为构建可信赖的在线农产品生态系统提供了全面支持，建立了从算法决策到消费者信任的关键路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有的4D高斯溅射编辑方法在视图一致性、时间一致性、非编辑区域一致性以及处理复杂文本指令方面仍面临挑战，需要开发更一致和交互式的编辑框架。

Method: 1. 引入4D感知的InstructPix2Pix模型，结合4D VGGT几何特征；2. 使用多视图网格模块迭代优化多视图输入图像；3. 提出高斯选择机制仅优化编辑区域的高斯；4. 设计基于LLM的用户意图理解模块，将复杂指令分解为原子操作序列。

Result: 与相关工作相比，该方法实现了更一致和可控的4D场景编辑，能够处理复杂的用户指令并保持编辑质量。

Conclusion: 4DGS-Craft框架通过结合4D几何感知、多视图一致性和智能指令分解，显著提升了4D高斯溅射编辑的一致性和交互性，为复杂4D场景编辑提供了有效解决方案。
...
TL;DR: 本文提出Pure-Pass (PP)像素级掩码机制，通过固定颜色中心点对像素分类，免除纯像素的昂贵计算，集成到ATD-light模型中实现更优的超分辨率性能。


<details>
  <summary>Details</summary>
Motivation: 现有轻量级超分辨率方法如CAMixer存在适应性差、掩码粒度粗和空间灵活性不足等问题，需要更精细的计算优化方案。

Method: 提出Pure-Pass像素级掩码机制，利用固定颜色中心点将像素分类为不同类别，对纯像素免除昂贵计算，实现细粒度、空间灵活的计算节省。

Result: PP-ATD-light在保持相似计算节省量的情况下，在重建质量和参数效率方面均优于CAMixer-ATD-light。

Conclusion: Pure-Pass机制通过像素级掩码实现了更精细的计算优化，在超分辨率任务中取得了更好的性能与效率平衡。
...
TL;DR: 本研究利用GPT-4o多模态能力自动生成颌骨囊肿的影像学发现，并开发了具有结构化输出的自校正循环框架(SLSO)来提高准确性。


<details>
  <summary>Details</summary>
Motivation: 利用AI技术自动生成颌骨囊肿的影像学发现，提高诊断效率和准确性，减少人工分析的工作量。

Method: 构建SLSO框架，包含10步处理流程：图像输入分析、结构化数据生成、牙位号提取和一致性检查、检测到不一致时迭代重新生成、发现生成及后续重构和一致性验证。

Result: 与传统的思维链方法相比，SLSO框架在多个评估项目上提高了输出准确性，牙位号、牙齿移动和牙根吸收的改善率分别为66.9%、33.3%和28.6%。在成功案例中，最多经过5次重新生成后实现了结构一致输出。

Conclusion: SLSO框架能强制阴性发现描述、抑制幻觉并提高牙位号识别准确性，但识别跨越多个牙齿的大范围病变仍有限制，需要进一步改进以实现实用的发现生成系统。
...
TL;DR: 提出LiLa-Net 3D自动编码器架构，仅使用LiDAR点云从真实交通环境中提取高效特征，通过简化编码器层和跳跃连接实现准确点云重建。


<details>
  <summary>Details</summary>
Motivation: 开发仅依赖LiDAR点云的高效3D特征提取方法，避免使用过多计算资源，同时保持重建质量。

Method: 使用配备Velodyne LiDAR的半自动驾驶车辆采集数据，构建3D自动编码器，减少编码器层数并简化跳跃连接，平衡跳跃连接信息与潜在编码。

Result: 模型能够准确重建原始点云，在重建质量与性能之间达到有效平衡，并展示出对非交通环境对象的强泛化能力。

Conclusion: LiLa-Net通过简化的架构实现了高效的点云特征提取和重建，具有良好的泛化性能，为LiDAR数据处理提供了有效解决方案。
...
Result: 相比地面方法，无人机观察将可见性损失减少15%，捕获更多行为转换，准确性和连续性更高。验证了969个行为序列，发现不同物种行为差异。
...
TL;DR: GaussianMorphing是一个从多视角图像进行语义感知3D形状和纹理变形的新框架，通过网格引导的3D高斯溅射实现高保真几何和外观建模，无需标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖点云或需要预定义同胚映射，难以处理带纹理的数据。本文旨在克服这些限制，实现几何一致且保持纹理保真度的3D变形。

Method: 采用统一变形策略，将3D高斯锚定到重建的网格面片上，通过拓扑感知约束确保几何一致性和纹理保真度，同时利用网格拓扑作为几何先验建立无监督语义对应。

Result: 在TexMorph基准测试中，GaussianMorphing显著优于现有2D/3D方法，颜色一致性误差(ΔE)降低22.2%，EI降低26.2%。

Conclusion: 该框架能够在整个变形过程中保持局部细节和全局语义一致性，无需标注数据，为3D形状和纹理变形提供了有效的解决方案。
...
TL;DR: 本文提出InPose方法，利用预训练扩散模型和旋转测量来估计人体姿态，通过似然项引导实现零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 现有姿态估计方法在传感器有限的实际场景中泛化能力差，主要因为位置测量受用户体型影响较大。

Method: 将姿态估计建模为逆问题，使用预训练扩散模型仅基于旋转测量，并通过位置测量的似然项进行引导。

Result: 提出的InPose方法能够为任何用户生成性估计最可能解释稀疏体上测量值的姿态序列。

Conclusion: 该方法通过分离旋转和位置信息，实现了更好的跨用户泛化性能。
...
TL;DR: 提出VGDM框架，将视觉变换器嵌入扩散过程，结合全局上下文推理和迭代去噪来提升脑肿瘤检测和分割的准确度。
...
Method: 在扩散过程核心嵌入视觉变换器，利用变换器骨干网络建模整个MRI体积的空间关系，同时通过扩散细化减轻体素级误差并恢复细粒度肿瘤细节。

Result: 在MRI脑肿瘤数据集上的实验验证显示，在Dice相似度和Hausdorff距离指标上取得一致提升。

Conclusion: 变换器引导的扩散模型有潜力推动肿瘤分割技术发展，超越传统U-Net基线，为神经肿瘤学提供改进的鲁棒性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 1970年代前法国历史城市扩张的定量分析因缺乏全国数字城市足迹数据而受限，需要填补这一数据空白。
...
TL;DR: 本文系统分析了腹腔镜胆囊切除术视频中点跟踪方法的失败模式，发现点跟踪在手术工具上表现良好，但在解剖目标上表现较差，主要由于组织相似性和边界模糊导致失败。


<details>
  <summary>Details</summary>
Motivation: 虽然SAM2等视频对象分割模型在手术视频中提供了有前景的零样本跟踪能力，但基于点跟踪的可靠性及其在复杂手术环境中的失败情况尚未得到充分理解。

Method: 在腹腔镜胆囊切除术视频中，针对胆囊、抓钳和L型电钩三个手术目标，系统比较了点跟踪与分割掩码初始化的性能表现。

Result: 结果显示点跟踪在手术工具上具有竞争力，但在解剖目标上持续表现不佳，组织相似性和模糊边界是主要失败原因。

Conclusion: 通过定性分析揭示了影响跟踪结果的关键因素，并为选择和放置跟踪点提供了可操作建议，以改善手术视频分析性能。
...
TL;DR: 提出了FFREEDG任务：在服务器上预训练后，仅使用客户端的未标记数据进行联邦学习，不再访问源数据。提出了FRIEREN框架，利用视觉基础模型整合视觉和语言模态，通过CLIP文本嵌入和弱到强一致性学习来解决该任务。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法要么不切实际地假设客户端有标记数据，要么未能充分利用现代视觉基础模型的能力。需要解决在客户端只有未标记数据且存在领域偏移时的联邦学习挑战。

Method: FRIEREN框架：1）使用视觉-语言解码器，由CLIP文本嵌入引导以改善语义消歧；2）采用弱到强一致性学习策略，在伪标签上进行鲁棒的本地训练。

Result: 在合成到真实世界和清晰到恶劣天气的基准测试中，该框架有效解决了新任务，性能与现有领域泛化和适应方法相当，为未来研究建立了强基线。

Conclusion: 提出的FRIEREN框架成功解决了FFREEDG这一具有挑战性的联邦学习任务，展示了利用视觉基础模型处理客户端未标记数据和领域偏移的有效性。
...
TL;DR: ASK-Hint是一个基于动作知识的提示框架，通过细粒度的指导性问题提升冻结视觉语言模型在视频异常检测中的性能，在多个数据集上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有提示方法过于抽象，忽略了视频异常检测中关键的人-物交互和动作语义，导致复杂异常识别不准确。

Method: 提出结构化提示框架，将提示按语义分组（如暴力、财产犯罪、公共安全），并制定细粒度的指导性问题，使模型预测与判别性视觉线索对齐。

Result: 在UCF-Crime和XD-Violence数据集上的实验显示，ASK-Hint持续提升AUC，相比微调和免训练方法都达到最先进性能。

Conclusion: 提示的细粒度对性能至关重要，ASK-Hint为可解释的视频异常检测提供了免训练且可泛化的解决方案。
...
TL;DR: GeoPurify通过学生亲和网络利用3D自监督教师模型的几何先验来净化2D VLM生成的3D点特征，有效解决了2D到3D特征转移中的噪声和几何一致性平衡问题，仅需约1.5%的训练数据即可达到或超越SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前将2D视觉语言模型特征转移到3D语义分割的方法存在权衡：直接投影会产生噪声和碎片化预测，而强制几何一致性需要昂贵的训练流程和大规模标注数据。作者认为这种限制源于分割-匹配范式未能协调2D语义与3D几何结构。

Method: 提出GeoPurify方法，使用小型学生亲和网络，通过从3D自监督教师模型提取的几何先验来净化2D VLM生成的3D点特征。在推理时设计了几何引导池化模块进一步去噪并确保语义和结构一致性。
...
TL;DR: 提出了一种基于猪耳静脉模式的非侵入性生物识别方法，使用智能手机采集图像，通过多阶段计算机视觉流程提取特征，SVM分类器在混合品种猪群中达到98.12%的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 传统猪只识别方法（如耳标、芯片）不可靠、成本高且主要针对纯种猪，不适用于小规模养殖户，需要开发低成本、非侵入性的替代方案。

Method: 收集20头混合品种猪的800张耳部图像，使用标准智能手机和简单背光采集；开发多阶段计算机视觉流程增强静脉可见性，提取结构和空间特征生成生物特征签名；使用机器学习模型进行分类。

Result: SVM模型在混合品种猪群中达到98.12%的识别精度；从图像处理到分类的整个过程平均耗时8.3秒，适合实时农场部署。

Conclusion: 该系统用永久性生物标记替代易损的物理标识，为农民提供了经济高效、无压力的动物识别方法，证实了耳静脉生物识别在数字化畜牧管理中的实用性，有望将精准农业的益处扩展到资源有限的农业社区。
...
TL;DR: 提出基于Twins金字塔视觉变换器骨干网络的多类别计数框架，通过多尺度解码和多任务设计（包含类别聚焦模块），在密集遮挡场景中实现优于现有方法的计数性能。


<details>
  <summary>Details</summary>
Motivation: 解决密集遮挡场景中传统离散检测方法失效的问题，需要开发能够处理多类别对象的密度图估计方法。

Method: 使用Twins金字塔视觉变换器作为骨干网络，构建专门的多类别计数头，采用多尺度解码方法，并添加基于分割的类别聚焦模块来抑制类别间干扰。

Result: 在VisDrone和iSAID基准测试中表现优异，相比现有多类别人群计数方法，MAE分别降低33%、43%和64%；与YOLOv11对比验证了在密集场景中人群计数方法的必要性。

Conclusion: 该方法通过区域损失将多类别人群计数扩展到新领域，在生物多样性监测数据集中展示了为保护工作提供信息和实现可扩展生态洞察的能力。
...
Method: TempoControl利用交叉注意力图，通过新颖的优化方法引导概念的时间安排。该方法通过三个互补原则引导注意力：通过相关性使其时间形状与控制信号对齐，通过能量在需要可见性的地方放大注意力，通过熵保持空间焦点。

Result: TempoControl在保持高质量和多样性的同时，实现了对时间安排的精确控制。该方法在多种视频生成应用中表现出有效性，包括单个和多个对象的时间重排序，以及动作和音频对齐的生成。
...
TL;DR: RewardMap是一个多阶段强化学习框架，通过密集奖励设计和从简单感知到复杂推理的多阶段训练，解决了MLLM在细粒度视觉推理任务中的稀疏奖励和不稳定优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在结构化信息丰富的环境（如交通地图）中的空间推理能力不足，而标准强化学习面临稀疏奖励和优化不稳定的挑战。

Method: 构建ReasonMap-Plus数据集提供密集奖励信号，提出包含难度感知奖励设计和多阶段RL方案的RewardMap框架，从简单感知任务逐步训练到复杂推理任务。

Result: 在ReasonMap和ReasonMap-Plus上的实验表明，RewardMap各组件均带来性能提升，组合使用效果最佳，在6个基准测试中平均提升3.47%。

Conclusion: RewardMap有效提升了MLLM的视觉理解和推理能力，在空间推理、细粒度视觉推理等任务上表现出色，证明了该方法的有效性。
...
TL;DR: DragFlow是首个利用FLUX强大先验进行拖拽式图像编辑的框架，通过区域级编辑范式和仿射变换解决DiT特征结构化不足的问题，在DragBench-DR和ReD Bench基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型从UNet-based DDPMs转向更可扩展的DiT with flow matching，生成先验显著增强，但拖拽式编辑尚未从中受益。传统方法在目标区域存在失真问题，因为Stable Diffusion的先验不足以将优化后的潜在空间投影回自然图像流形。

Method: 1. 引入区域级编辑范式，使用仿射变换实现更丰富一致的特征监督；2. 集成预训练开放域个性化适配器（如IP-Adapter）增强主体一致性；3. 通过梯度掩码硬约束保持背景保真度；4. 使用多模态大语言模型解决任务歧义。

Result: 在DragBench-DR和新构建的ReD Bench基准测试中，DragFlow超越了基于点和基于区域的基线方法，在拖拽式图像编辑任务中达到了新的最先进水平。

Conclusion: DragFlow成功利用FLUX的丰富先验进行拖拽式图像编辑，通过区域级编辑范式解决了DiT特征结构化不足的问题，为拖拽式编辑提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型面临"大海捞针"问题：原始视频帧产生的大量视觉标记会耗尽模型上下文窗口。现有解决方案通过选择稀疏帧来减少标记数量，但这种帧级选择丢弃了基本的时间动态信息，导致对运动和事件连续性的推理效果不佳。

Method: 提出F2C方法：1) 将选择范围从孤立关键帧扩展到关键片段（短时间连贯片段）；2) 采用自适应分辨率策略，动态平衡空间分辨率和片段长度，确保每个视频的标记数量恒定；3) 无需训练即可应用。

Result: 在三个长视频基准测试上的实验表明：F2C方法在Video-MME、LongVideoBench和MLVU基准上分别比均匀采样高出8.1%、5.6%和10.3%。

Conclusion: 研究结果强调了在帧选择中保持时间连贯性的重要性，并为视频大语言模型扩展到现实世界视频理解应用提供了实用途径。
...
TL;DR: 该研究比较了基于单目视频的3D人体姿态估计模型与惯性测量单元(IMUs)在13种日常活动中的性能，发现MotionAGFormer表现最佳，两种技术都适用于实验室外的运动学评估。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习和可穿戴传感器的发展，需要在真实环境下准确评估人体运动，这对远程医疗、运动科学和康复至关重要。

Method: 使用VIDIMU数据集，比较四种深度学习框架(MotionAGFormer、MotionBERT、MMPose、NVIDIA BodyTrack)与IMU数据在17个关键点上的关节角度估计性能。

Result: MotionAGFormer表现最优，总体RMSE为9.27±4.80度，MAE为7.86±4.18度，Pearson相关系数0.86±0.15，R²为0.67±0.28。

Conclusion: 两种技术都可行，但各有优缺点。研究为开发远程医疗和患者监测的稳健、成本效益高且用户友好的解决方案提供了指导。
...
TL;DR: NeuroSwift通过整合AutoKL和CLIP适配器的扩散模型，实现了跨被试的视觉刺激重建，仅需训练17%参数和1小时训练时间即可达到最先进性能。
...
Method: 集成互补适配器的扩散模型：AutoKL处理低级特征，CLIP处理语义特征；CLIP适配器在Stable Diffusion生成图像上训练以模拟高级视觉皮层编码；采用预训练+微调策略，仅微调17%参数。

Result: 在轻量级GPU（三块RTX 4090）上仅需1小时训练即可达到最先进性能，优于现有方法。

Conclusion: NeuroSwift提供了一种高效、准确的跨被试视觉重建方法，显著降低了计算需求并提升了泛化能力。
...
Result: 在13个细粒度基准测试中平均准确率提升2.90%，仅需轻量适配
...
TL;DR: VidGuard-R1是首个通过GRPO微调多模态大语言模型的视频真实性检测器，在140k真实和AI生成视频数据集上训练，实现高精度检测和可解释推理。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成视频技术快速发展，需要有效的检测工具来应对错误信息和声誉损害等社会风险，同时检测模型需要提供可解释性以确保监管透明。

Method: 使用GRPO（组相对策略优化）微调Qwen-VL多模态大语言模型，配备两个专门针对时间伪影和生成复杂度的奖励模型，在精心设计的140k视频数据集上训练。

Result: 在现有基准测试中实现最先进的零样本性能，额外训练后准确率超过95%，并能产生精确可解释的预测依据。

Conclusion: VidGuard-R1在视频真实性检测方面表现出色，兼具高准确性和可解释性，为解决AI生成视频带来的社会风险提供了有效工具。
...
TL;DR: 提出了一种简单有效的方法来缓解长视频生成中的质量退化问题，无需长视频教师监督或重新训练长视频数据集。通过利用教师模型的丰富知识为自生成长视频中的采样片段提供指导，保持时间一致性，可将视频长度扩展到教师能力的20倍以上。
...
Method: 利用教师模型的丰富知识为自生成长视频中的采样片段提供指导。该方法保持时间一致性，避免过度曝光和误差累积问题，无需像先前方法那样重新计算重叠帧。

Result: 该方法可将视频长度扩展到教师能力的20倍以上，在增加计算量时能够生成长达4分15秒的视频，相当于基础模型位置嵌入支持的最大跨度的99.9%，比基线模型长50倍以上。在标准基准测试和改进基准测试上的实验表明，该方法在保真度和一致性方面显著优于基线方法。

Conclusion: 提出的方法有效缓解了长视频生成中的质量退化问题，通过利用教师模型知识为自生成视频提供指导，实现了高质量的长视频生成，显著优于现有基线方法。
...
TL;DR: KineMask是一种物理引导的视频生成方法，通过两阶段训练策略实现逼真的刚体控制和物体交互，在合成和真实场景中均表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在物理合理的物体交互方面存在困难，缺乏基于物理的控制机制，限制了其在机器人学和决策制定中的应用潜力。

Method: 提出两阶段训练策略，通过物体掩码逐步移除未来运动监督，训练视频扩散模型，并结合低级运动控制与高级文本条件。

Result: 在合成交互场景上训练后，在真实场景中显著改善了物体交互效果，实验显示在同等规模模型中表现优异。

Conclusion: KineMask通过物理引导的视频生成实现了逼真的刚体控制，消融研究证实了低高级条件在视频扩散模型中的互补作用。
...
TL;DR: 本文针对当前视频模型缺乏精细控制的问题，提出了一种多模态动作框架，通过整合本体感觉、动觉、力触觉和肌肉激活等多种感官信息，实现了对精细交互的模拟。


<details>
  <summary>Details</summary>
Motivation: 通用家用机器人需要实时精细运动控制来处理精细任务和紧急情况，而现有视频模型无法满足这一需求。

Method: 开发了特征学习范式来对齐多模态信息，同时保留每个模态的独特信息；提出了正则化方案来增强动作轨迹特征在表示复杂交互动态时的因果性。

Result: 实验表明，整合多模态感官提高了模拟准确性并减少了时间漂移；大量消融研究和下游应用验证了方法的有效性和实用性。

Conclusion: 多模态感官的引入显著改善了精细控制模拟的性能，为机器人精细运动控制提供了有效解决方案。
...
TL;DR: VideoNSA通过将原生稀疏注意力(NSA)适配到视频语言模型中，解决了多模态模型在长视频理解中因上下文长度限制而错过关键过渡帧和难以保持连贯性的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态语言模型在视频理解中受限于上下文长度，经常错过关键过渡帧，难以在长时间尺度上保持连贯性。

Method: 将原生稀疏注意力(NSA)适配到视频语言模型中，通过端到端训练在216K视频指令数据集上优化Qwen2.5-VL，采用硬件感知的混合注意力方法：文本使用密集注意力，视频使用NSA。

Result: 相比token压缩和无需训练的稀疏基线方法，VideoNSA在长视频理解、时序推理和空间基准测试中表现更优，能够可靠扩展到128K tokens。

Conclusion: 研究发现：可靠扩展到128K tokens；在固定预算下存在最优的全局-局部注意力分配；任务相关的分支使用模式；可学习的组合稀疏注意力有助于诱导动态注意力汇聚点。
...
<details>
  <summary>Details</summary>
Motivation: 当前高分辨率文本到图像生成器无法为不需要高分辨率图像的用户提供经济高效的替代方案，因为扩散模型在固定分辨率集上训练后难以泛化到其他分辨率。
...
Conclusion: NoiseShift有效缓解了分辨率相关的伪影，显著提高了低分辨率图像生成的质量。
...
TL;DR: 该论文研究从视频预测动态物理属性的任务，包括弹性、粘性和动态摩擦，通过构建新数据集并探索三种推断方法：基于传统视觉技术的oracle方法、基于预训练视频模型的简单读出机制，以及多模态大语言模型的提示策略。


<details>
  <summary>Details</summary>
Motivation: 研究从视频中推断需要时间信息才能确定的动态物理属性，如弹性、粘性和动态摩擦，以扩展计算机视觉在物理理解方面的应用。

Method: 收集了三个物理属性的新视频数据集（合成训练/测试集和真实世界评估集），探索了三种推断方法：(a) 使用传统计算机视觉技术的oracle方法；(b) 基于预训练视频生成和自监督模型的视觉提示和可训练提示向量的简单读出机制；(c) 多模态大语言模型的提示策略。

Result: 生成式或自监督训练的视频基础模型表现相似，但落后于oracle方法；多模态大语言模型目前表现较差，但通过合适的提示可以改善性能。

Conclusion: 视频基础模型在预测动态物理属性方面具有潜力，但仍有改进空间；多模态大语言模型需要更好的提示策略来提升性能。
...
TL;DR: 该论文提出了声音对象检测任务，通过多模态对象感知框架从第一人称视角视频中学习，将声音与直接参与互动的物体联系起来。


<details>
  <summary>Details</summary>
Motivation: 日常物体互动会产生独特的声音，但现有模型难以区分不同物体互动的声音差异，如勺子敲击硬木地板与地毯的声音区别。

Method: 开发了自动管道计算参与物体的分割掩码，引导模型关注互动中最具信息量的区域，并使用槽注意力视觉编码器强化对象先验。

Result: 在新任务和现有多模态动作理解任务上实现了最先进的性能。

Conclusion: 通过对象感知的多模态学习框架，模型能够有效关联声音与直接参与互动的物体，提升了对日常物体互动声音的理解能力。
...
TL;DR: 本文分析了3D高斯泼溅（3DGS）对图像级投毒攻击的脆弱性，提出了一种基于密度的投毒方法，通过在低密度区域注入高斯点来嵌入视角依赖的虚幻物体，同时引入自适应噪声策略破坏多视角一致性。


<details>
  <summary>Details</summary>
Motivation: 随着3D场景表示方法（如NeRF和3DGS）在新视角合成中的广泛应用，解决其安全漏洞变得至关重要。本文旨在分析3DGS对图像级投毒攻击的鲁棒性。

Method: 提出密度引导的投毒方法：1）使用核密度估计（KDE）识别低密度区域；2）在这些区域战略性地注入高斯点，嵌入视角依赖的虚幻物体；3）引入自适应噪声策略破坏多视角一致性；4）提出基于KDE的攻击难度评估协议。

Result: 大量实验证明该方法相比现有技术具有优越性能，能够在投毒视图中清晰显示虚幻物体，同时对无辜视图影响最小。

Conclusion: 该方法有效揭示了3DGS的脆弱性，提出的评估协议为未来研究提供了客观基准，强调了3D场景表示方法安全性的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像模型在单主体提示上表现优秀，但在多主体描述中经常出现属性泄漏、身份纠缠和主体遗漏等问题，需要专门解决方案。

Method: 通过随机最优控制视角看待流匹配，开发了两种算法：(1)无需训练的单次更新测试时控制器；(2)轻量级微调方法Adjoint Matching，同时保持基础模型能力。

Result: 在Stable Diffusion 3.5、FLUX和Stable Diffusion XL上，两种算法都显著改善了多主体对齐，同时保持基础模型风格。测试时控制可在普通GPU上高效运行。

Conclusion: 提出的FOCUS方法在多主体保真度方面达到了最先进水平，为多主体生成提供了统一的理论框架和实用解决方案。
...
### [168] [LLM-based Multi-Agent Blackboard System for Information Discovery in Data Science](https://arxiv.org/abs/2510.01285)
*Alireza Salemi,Mihir Parmar,Palash Goyal,Yiwen Song,Jinsung Yoon,Hamed Zamani,Hamid Palangi,Tomas Pfister*

Main category: cs.MA

TL;DR: 提出基于黑板架构的多智能体通信范式，解决大型异构数据湖中的数据发现问题，相比传统方法显著提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理大型异构数据湖时存在局限：单智能体系统难以应对大规模文件，而主从式多智能体系统依赖中央控制器且需要精确了解各子智能体能力。

Method: 采用黑板架构，中央智能体将请求发布到共享黑板，自主子智能体根据自身能力自愿响应，无需中央协调器预先了解所有子智能体专长。

Result: 在三个基准测试上显著优于基线方法，端到端任务成功率相对提升13%-57%，数据发现的F1分数相对提升最高达9%。
...
### [169] [On the Role of Domain Experts in Creating Effective Tutoring Systems](https://arxiv.org/abs/2510.01432)
*Sarath Sreedharan,Kelsey Sikes,Nathaniel Blanchard,Lisa Mason,Nikhil Krishnaswamy,Jill Zarestky*

Main category: cs.AI

TL;DR: 论文探讨了如何利用领域专家精心策划的知识来创建有效的辅导系统，提出了两种方法：使用可解释AI技术自动生成课程，以及利用专家指定的课程来开发自适应辅导系统。
...
Result: 通过传粉者识别辅导系统的案例研究，展示了这些方法的实际应用价值。

Conclusion: 专家精心策划的知识在创建新颖教育系统中具有重要作用，特别是在自动生成课程和开发自适应辅导系统方面。
...
### [170] [VaPR -- Vision-language Preference alignment for Reasoning](https://arxiv.org/abs/2510.01700)
*Rohan Wadhawan,Fabrice Y Harel-Canada,Zi-Yi Dou,Suhaila Shakiah,Robinson Piramuthu,Nanyun Peng*

Main category: cs.AI

TL;DR: 提出VaPR框架，通过LLM引导的响应编辑生成具有目标错误的硬负样本，创建高质量数据集来微调大型视觉语言模型，显著提升模型性能并解决常见失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有偏好微调方法忽略了合成偏好标注中的风格和长度偏差噪声，需要开发能产生高质量负样本的方法来改善模型对齐效果。

Method: 基于LLM引导的响应编辑框架生成硬负响应，保持风格和长度与接受响应相似但包含目标错误，构建VaPR数据集用于微调LVLM。

Result: 在10个基准测试中，VaPR模型实现显著性能提升：LLaVA平均提升6.5%，Qwen2VL提升4.0%，Qwen2.5VL提升1.5%，特别在推理任务上表现突出，并减少二元问题中的"是"回答倾向。

Conclusion: VaPR框架有效提升LVLM性能，数据规模扩展分析显示性能持续改善，且该框架可推广到开源LLM作为编辑器，保持接近GPT-4o合成数据的性能水平。
...
### [171] [Agentic Jigsaw Interaction Learning for Enhancing Visual Perception and Reasoning in Vision-Language Models](https://arxiv.org/abs/2510.01304)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在多模态理解和推理方面虽有进展，但在基本感知和推理能力上仍存在局限，特别是在简单拼图任务上表现接近随机，揭示出核心能力的不足。高质量视觉语言数据的稀缺和可扩展性限制也制约了能力提升。

Method: 提出AGILE方法，将拼图解决构建为交互过程，模型逐步与环境交互。每个步骤中，模型基于当前状态生成可执行代码执行动作，环境提供细粒度视觉反馈指导任务完成。通过观察和交互的迭代循环，模型通过探索和反馈逐步提升感知和推理能力。

Result: AGILE在复杂程度不同的拼图任务上显著提升性能（2×2设置下准确率从9.5%提升至82.8%），并在9个通用视觉任务上表现出强泛化能力，平均提升3.1%。这些结果表明感知和推理能力都有显著增强。
...
### [172] [Aristotle: IMO-level Automated Theorem Proving](https://arxiv.org/abs/2510.01346)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决自动定理证明中形式验证与非正式推理的整合问题，提升系统在复杂数学问题上的表现

Method: 集成三个主要组件：Lean证明搜索系统、生成并形式化引理的非正式推理系统、专用几何求解器

Result: 在2025年国际数学奥林匹克竞赛中达到金牌级别表现，展示了自动定理证明的最先进性能

Conclusion: Aristotle系统成功结合形式验证与非正式推理，为自动定理证明提供了可扩展的高性能解决方案
...
### [173] [MEMTRACK: Evaluating Long-Term Memory and State Tracking in Multi-Platform Dynamic Agent Environments](https://arxiv.org/abs/2510.01353)
*Darshan Deshpande,Varun Gangal,Hersh Mehta,Anand Kannappan,Rebecca Qian,Peng Wang*

Main category: cs.AI

TL;DR: MEMTRACK是一个评估多平台代理环境中长期记忆和状态跟踪的基准测试，模拟真实企业工作流程，集成Slack、Linear和Git等平台的异步事件，测试记忆获取、选择和冲突解决等能力。
...
Method: 通过专家手动设计和基于代理的可扩展合成方法构建数据集，创建包含跨平台依赖、矛盾信息和代码库理解等复杂场景的时间线，并引入正确性、效率和冗余度等评估指标。

Result: 在现有最先进LLM和记忆后端上的实验显示，处理长时程记忆、跨平台依赖和矛盾解决方面存在挑战，性能最佳的GPT-5模型在MEMTRACK上仅达到60%的正确性得分。
...
### [174] [Is It Thinking or Cheating? Detecting Implicit Reward Hacking by Measuring Reasoning Effort](https://arxiv.org/abs/2510.01367)
*Xinpeng Wang,Nitish Joshi,Barbara Plank,Rico Angell,He He*

Main category: cs.AI

TL;DR: TRACE是一种检测隐式奖励攻击的方法，通过截断思维链来量化模型努力程度，在数学推理和编程任务中显著优于现有监控方法。


<details>
  <summary>Details</summary>
Motivation: 奖励攻击问题严重，特别是隐式攻击会绕过思维链监控，需要有效检测方法。

Method: TRACE通过逐步截断思维链，测量模型在不同截断长度下通过验证器的比率，量化模型努力程度。

Result: 在数学推理任务中比72B思维链监控器提升65%以上，在编程任务中比32B监控器提升30%以上，并能发现训练中的未知漏洞。

Conclusion: TRACE为当前监控方法无效的场景提供了可扩展的无监督监督方法。
...
### [175] [Fine-tuning with RAG for Improving LLM Learning of New Skills](https://arxiv.org/abs/2510.01375)
*Humaid Ibrahim,Nikolai Rozanov,Marek Rei*

Main category: cs.AI

TL;DR: 提出了一种通过蒸馏将推理时检索转化为学习能力的方法，使用失败案例生成提示，通过一次性检索改进教师轨迹，训练学生模型内部化知识而非记忆，在多个基准测试中显著提升性能并减少计算开销。


<details>
  <summary>Details</summary>
Motivation: LLM代理在多步任务中经常出现可预测的失败，如尝试不满足前提条件的操作、发出冗余命令或处理环境约束不当。虽然RAG可以通过运行时指导提高性能，但需要维护外部知识库并在每次部署时增加计算开销。

Method: 1) 从代理失败中提取紧凑、可重用的提示；2) 在回合开始时使用这些提示通过一次性检索生成改进的教师轨迹；3) 在移除提示字符串的情况下训练学生模型，强制内部化而非记忆。

Result: 在ALFWorld（家务任务）和WebShop（在线购物）两个交互基准测试中，蒸馏后的学生模型始终优于基线代理，ALFWorld成功率高达91%（基线为79%），WebShop得分提高到72（基线为61），同时使用的token数量比检索增强教师少10-60%。

Conclusion: 该方法在不同模型规模（7B/14B参数）和代理架构（ReAct/StateAct）上均能泛化，证明检索优势可以通过有针对性的微调有效内部化，无需永久性运行时依赖。
...
### [176] [VOGUE: Guiding Exploration with Visual Uncertainty Improves Multimodal Reasoning](https://arxiv.org/abs/2510.01444)
*Rui Liu,Dian Yu,Tong Zheng,Runpeng Dai,Zongxia Li,Wenhao Yu,Zhenwen Liang,Linfeng Song,Haitao Mi,Pratap Tokekar,Dong Yu*

Main category: cs.AI

TL;DR: VOGUE是一种新颖的多模态强化学习方法，通过将视觉输入视为随机上下文，在输入空间而非输出空间进行探索，有效解决了MLLMs中的探索问题。


<details>
  <summary>Details</summary>
Motivation: 现有的RLVR方法在处理多模态LLMs时仍存在探索困难，且将视觉输入视为固定条件，忽略了视觉变化带来的不确定性，导致策略对合理视觉变化的鲁棒性不足。
...
### [177] [Information Seeking for Robust Decision Making under Partial Observability](https://arxiv.org/abs/2510.01531)
*Djengo Cyun-Jyun Fang,Tsung-Wei Ke*

Main category: cs.AI

TL;DR: InfoSeeker是一个LLM决策框架，通过将任务导向规划与信息寻求相结合，在部分可观测环境中处理观察和环境动态的不确定性，相比现有方法实现74%的绝对性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM规划智能体虽然解决了观察不确定性，但经常忽略其内部动态与实际环境之间的差异，需要主动信息寻求来更新内部动态并为未来决策提供信息。

Method: InfoSeeker框架提示LLM通过规划行动来主动收集信息，验证其理解、检测环境变化或测试假设，然后生成或修订任务导向计划。

Result: 在包含不完全观察和不确定动态的部分可观测环境基准测试中，InfoSeeker相比现有方法实现74%的绝对性能提升，且在机器人操作和网页导航等基准测试中优于基线方法。
...
### [178] [The Reasoning Boundary Paradox: How Reinforcement Learning Constrains Language Models](https://arxiv.org/abs/2510.02230)
*Phuc Minh Nguyen,Chinh D. La,Duy M. H. Nguyen,Nitesh V. Chawla,Binh T. Nguyen,Khoa D. Doan*

Main category: cs.AI

TL;DR: 本文揭示了RLVR方法在提升大语言模型推理能力时存在的边界收缩问题，发现了负干扰和赢家通吃现象，并提出了一种针对低概率问题的数据筛选算法来改善Pass@k性能。


<details>
  <summary>Details</summary>
Motivation: 尽管RLVR被广泛用于增强大语言模型的推理能力，但近期证据表明它可能反而会收缩推理边界而非扩展。本文旨在深入探究RLVR的收缩问题及其根本原因。

Method: 通过理论分析和多数学推理基准的实证研究，揭示了RLVR学习动态中的负干扰现象和赢家通吃现象，并提出了一种专注于低概率问题的数据筛选算法。

Result: 研究发现RLVR存在负干扰（学习解决某些问题会降低其他问题的正确解概率）和赢家通吃现象（过度强化高概率正确解而抑制低概率问题），导致Pass@k性能下降。

Conclusion: 基于这些发现，提出了一种简单有效的数据筛选方法，专注于低概率问题，显著提升了Pass@k性能，为解决RLVR的边界收缩问题提供了有效方案。
...
### [179] [InvThink: Towards AI Safety via Inverse Reasoning](https://arxiv.org/abs/2510.01569)
...
<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法直接优化安全响应，但无法系统性地预防风险。InvThink旨在通过逆向思维让模型主动识别和避免潜在危害。

Method: 1) 枚举潜在危害 2) 分析后果 3) 生成主动规避风险的安全输出。通过监督微调和强化学习在三个LLM系列中实现。

Result: 安全改进随模型规模扩展更强；缓解安全税，保持标准基准推理能力；在高风险领域（医疗、金融、法律等）有害响应减少15.7%。
...
### [180] [The Unreasonable Effectiveness of Scaling Agents for Computer Use](https://arxiv.org/abs/2510.02250)
...
Result: 在OSWorld上达到69.9%的新SOTA，显著超越先前方法，接近72%的人类水平性能，并在WindowsAgentArena和AndroidWorld上展示出强大的泛化能力。
...
### [181] [PychoBench: Evaluating the Psychology Intelligence of Large Language Models](https://arxiv.org/abs/2510.01611)
*Min Zeng*

Main category: cs.AI

TL;DR: 本文研究了大型语言模型在心理咨询领域的应用潜力，通过开发基于美国国家心理咨询师认证考试的PsychoBench基准来评估LLMs是否具备担任心理咨询师的知识能力。


<details>
  <summary>Details</summary>
Motivation: 虽然LLMs在多个行业展现了强大的生成能力，但在需要认知能力的应用如心理咨询方面仍有待探索。本文旨在评估LLMs是否具备担任心理咨询师所需的知识资质。

Method: 开发了PsychoBench基准，包含约2,252个精心设计的单选题，基于美国国家心理咨询师认证考试，涵盖心理学多个子学科，需要深度理解能力。

Result: 评估显示，GPT-4o、Llama3.3-70B和Gemma3-27B等先进模型得分远超通过阈值（70%），而较小的开源模型如Qwen2.5-7B、Mistral-7B则远低于通过标准。

Conclusion: 只有前沿的LLMs目前能够达到心理咨询考试标准，这既显示了开发心理学导向LLMs的潜力，也凸显了相关挑战。
...
### [182] [Just Do It!? Computer-Use Agents Exhibit Blind Goal-Directedness](https://arxiv.org/abs/2510.01670)
...
TL;DR: 本文揭示了计算机使用代理(CUAs)存在盲目标导向(BGD)风险，即不顾可行性、安全性和上下文盲目追求目标的行为模式。作者开发了BLIND-ACT基准测试，在9个前沿模型上观察到平均80.8%的BGD率，表明即使输入无害也存在风险。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理(CUAs)在GUI上执行操作以实现用户目标，但作者发现这类代理普遍存在盲目标导向(BGD)偏差，即不考虑可行性、安全性、可靠性和上下文环境而盲目追求目标，这带来了潜在风险。

Method: 开发了BLIND-ACT基准测试，包含90个任务，捕捉三种BGD模式：缺乏上下文推理、在模糊情况下做出假设和决策、追求矛盾或不可行目标。基于OSWorld构建真实环境，使用基于LLM的评估器评估代理行为，与人工标注达到93.75%的一致性。

Result: 在9个前沿模型(包括Claude Sonnet和Opus 4、Computer-Use-Preview、GPT-5)上评估，观察到平均80.8%的BGD率。提示干预能降低BGD水平，但风险仍然显著。定性分析揭示了三种失败模式：执行优先偏差、思维-行动脱节、请求优先。

Conclusion: 识别BGD风险并引入BLIND-ACT基准为未来研究和缓解这一基础风险奠定了基础，对于确保CUA的安全部署至关重要。需要更强的训练或推理时干预措施来应对这一挑战。
...
### [183] [Improving AGI Evaluation: A Data Science Perspective](https://arxiv.org/abs/2510.01687)
*John Hawkins*
...
<details>
  <summary>Details</summary>
Motivation: 当前AGI评估方法主要依赖对人类智能直觉的合成任务设计，这些方法在AI历史上表现不佳，需要更有效的评估框架。

Method: 提出基于稳健任务执行的评估设计哲学，借鉴数据科学中展示系统可靠部署的实践方法，提供具体的AGI评估示例。

Result: 论证了传统基于直觉的合成任务评估方法的局限性，提出了更实用的基于能力展示的评估框架。

Conclusion: AGI评估应转向关注系统在实际任务中的稳健执行能力，而非依赖直觉设计的合成测试，这样才能更有效地衡量AGI进展。
...
### [184] [Plan Then Action:High-Level Planning Guidance Reinforcement Learning for LLM Reasoning](https://arxiv.org/abs/2510.01833)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM的推理过程受限于自回归的令牌级生成，缺乏全局规划，导致推理冗余、不连贯或不准确。现有方法如树状算法和强化学习计算成本高且难以产生最优推理轨迹。

Method: PTA-GRPO两阶段框架：1) 使用高级LLM将CoT提炼为高层指导进行SFT；2) 引入指导感知的RL方法联合优化最终输出和指导质量。

Result: 在多个数学推理基准测试（MATH、AIME2024、AIME2025、AMC）和不同基础模型（Qwen2.5-7B-Instruct、Qwen3-8B、Qwen3-14B、LLaMA3.2-3B）上均实现了稳定且显著的性能提升。

Conclusion: PTA-GRPO能够有效提升LLM的推理能力，在不同模型和任务上都具有良好的泛化性和有效性。
...
### [185] [Constrained Adaptive Rejection Sampling](https://arxiv.org/abs/2510.01902)
*Paweł Parys,Sairam Vaidya,Taylor Berg-Kirkpatrick,Loris D'Antoni*

Main category: cs.AI

TL;DR: 提出CARS方法，在保持语言模型分布不变的前提下，通过自适应剪枝技术提高受限生成的采样效率，解决了传统方法在有效性和多样性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有受限生成方法存在两极化问题：贪婪解码方法会扭曲语言模型分布，而拒绝采样方法计算效率低下。在程序模糊测试等需要高有效性和多样性的领域，这两种方法都有明显缺陷。

Method: CARS方法从无约束采样开始，通过构建trie数据结构记录违反约束的序列，并在后续采样中减去这些序列的概率质量，实现自适应剪枝，确保不会重复访问已证明无效的前缀。

Result: 在程序模糊测试和分子生成等多个领域的实验中，CARS在采样效率（每个有效样本所需的LM前向传播次数）方面表现更好，同时比GCD和其他近似方法产生更强的样本多样性。

Conclusion: CARS方法在不扭曲语言模型分布的前提下，显著提高了受限生成的采样效率，为需要高有效性和多样性的应用领域提供了更好的解决方案。
...
### [186] [Do AI Models Perform Human-like Abstract Reasoning Across Modalities?](https://arxiv.org/abs/2510.02125)
*Claas Beger,Ryan Yi,Shuhao Fu,Arseny Moskvichev,Sarah W. Tsai,Sivasankaran Rajamanickam,Melanie Mitchell*

Main category: cs.AI

TL;DR: 该论文评估AI模型在ConceptARC基准上的抽象推理能力，发现仅凭输出准确率会高估文本模态的抽象推理能力，低估视觉模态的抽象推理能力。


<details>
  <summary>Details</summary>
Motivation: 研究AI模型是否真正理解ConceptARC任务设计者意图的抽象概念，而非依赖表面模式来解决问题。

Method: 在ConceptARC基准上评估模型，改变输入模态（文本vs视觉）、是否允许使用Python工具、推理努力程度，并分析模型生成的自然语言规则。

Result: 文本模态下模型准确率可达人类水平，但规则分析显示多依赖表面捷径；视觉模态下准确率下降，但规则分析显示模型仍能捕捉到意图抽象概念。

Conclusion: 模型在抽象推理方面仍落后于人类，仅凭准确率评估会误导对模型抽象推理能力的判断，需要结合规则分析来获得更准确的评估。
...
### [187] [A Rigorous Benchmark with Multidimensional Evaluation for Deep Research Agents: From Answers to Reports](https://arxiv.org/abs/2510.02190)
...
TL;DR: 本文提出了一个专门针对深度研究代理(DRAs)的严格基准测试和多维评估框架，包含214个专家策划的挑战性查询和手动构建的参考包，用于评估DRAs在复杂开放任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在评估维度、响应格式和评分机制方面存在不足，无法有效评估从封闭语言模型向具有外部感知和信息整合能力的互联代理系统转变的AI范式。

Method: 构建包含214个专家策划查询的基准测试，分布在10个广泛主题领域，每个查询配有手动构建的参考包；开发多维评估框架，整合语义质量、主题聚焦和检索可信度的评分指标。

Result: 实验证实主流DRAs优于基于网络搜索工具增强的推理模型，但仍有相当大的改进空间。
...
### [188] [RLAD: Training LLMs to Discover Abstractions for Solving Reasoning Problems](https://arxiv.org/abs/2510.02263)
*Yuxiao Qu,Anikait Singh,Yoonho Lee,Amrith Setlur,Ruslan Salakhutdinov,Chelsea Finn,Aviral Kumar*

Main category: cs.AI

TL;DR: 提出RLAD方法，通过推理抽象来指导模型学习有效的推理过程，将抽象生成和解决方案生成解耦训练，提升对复杂问题的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决现有大模型推理过程中缺乏算法性程序重用的问题，避免推理轨迹退化到冗长和无效的探索。

Method: 引入推理抽象概念，采用两玩家RL训练范式（RLAD），联合训练抽象生成器和解决方案生成器，实现结构化探索。

Result: 该方法有效提升了推理性能，在大型测试预算下，增加抽象生成的计算资源比生成更多解决方案更有利于性能提升。

Conclusion: 推理抽象能够引导有意义的探索，通过解耦抽象提议和解决方案生成的学习信号，显著改善了模型的推理能力。
...
### [189] [Towards Photonic Band Diagram Generation with Transformer-Latent Diffusion Models](https://arxiv.org/abs/2510.01749)
*Valentin Delchevalerie,Nicolas Roy,Arnaud Bougaham,Alexandre Mayer,Benoît Frénay,Michaël Lobet*

Main category: physics.optics

TL;DR: 本文提出了一种基于扩散模型的光子能带图生成方法，使用Transformer编码器和潜在扩散模型来快速生成光子晶体的能带图，解决了传统数值方法计算成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 光子晶体在纳米尺度控制光传播方面具有重要作用，但计算光子能带图需要求解大量麦克斯韦方程，数值计算成本高昂，特别是在逆向设计优化循环中。

Method: 结合Transformer编码器提取输入结构的上下文嵌入，以及潜在扩散模型生成对应的能带图。该方法能够捕捉光子学中复杂的干涉和散射现象。

Result: 开发了首个基于扩散模型的光子能带图生成方法，具有扩展到任意三维结构的能力。

Conclusion: Transformer和扩散模型非常适合捕捉光子学中的复杂现象，为这一领域的新代理建模策略铺平了道路。
...
### [190] [Aligning Video Models with Human Social Judgments via Behavior-Guided Fine-Tuning](https://arxiv.org/abs/2510.01502)
*Kathy Garcia,Leyla Isik*

Main category: q-bio.NC

TL;DR: 研究发现预训练视频模型在社交视频相似性判断上与人类感知存在差距，语言模型反而表现更好。通过使用人类行为数据微调视频模型，可以显著提升模型与人类社交感知的对齐度。


<details>
  <summary>Details</summary>
Motivation: 探索现代视频和语言模型是否能捕捉人类在社交视频中感知的相似性结构，以及如何利用人类行为数据将这种结构注入模型中。

Method: 构建包含49,000多个相似性判断的基准数据集，使用混合三元组-RSA目标通过LoRA微调TimeSformer视频模型，使其成对距离与人类相似性对齐。

Result: 微调后的视频模型在保留视频上显著提高了与人类感知的对齐度，增加了与语言嵌入的共享方差，并增强了社交情感属性的编码能力。

Conclusion: 预训练视频模型在社交识别方面存在差距，但通过行为引导的微调可以使视频表征更接近人类社交感知。
...
### [191] [Uncovering Semantic Selectivity of Latent Groups in Higher Visual Cortex with Mutual Information-Guided Diffusion](https://arxiv.org/abs/2510.02182)
...
<details>
  <summary>Details</summary>
Motivation: 解决神经群体如何编码物体中心视觉信息以及特征特异性信息在神经群体中的分布和组织方式这一核心科学问题。

Method: 使用变分自编码器推断神经群体的组间解缠神经潜在子空间，然后提出互信息引导的扩散合成过程来可视化每个潜在组编码的特定视觉语义特征。

Result: 在两只猕猴的颞下皮层多会话神经放电数据集上验证，发现神经潜在组对多样视觉特征具有清晰的语义选择性，包括物体姿态、类别间转换和类内内容。
...
### [192] [VENTURA: Adapting Image Diffusion Models for Unified Task Conditioned Navigation](https://arxiv.org/abs/2510.01388)
*Arthur Zhang,Xiangyun Meng,Luca Calliari,Dong-Ki Kim,Shayegan Omidshafiei,Joydeep Biswas,Ali Agha,Amirreza Shaban*

Main category: cs.RO

TL;DR: VENTURA是一个视觉语言导航系统，通过微调图像扩散模型进行路径规划，生成视觉路径掩码来指导机器人导航，在真实世界评估中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有视觉语言模型在机器人导航任务中难以适应的问题，因为动作空间差异和预训练目标限制了其在机器人任务中的可迁移性。

Method: 使用互联网预训练的图像扩散模型进行微调，生成路径掩码作为视觉规划，然后通过轻量级行为克隆策略将这些视觉规划转化为可执行轨迹。

Result: 在真实世界评估中，VENTURA在物体到达、障碍物避让和地形偏好任务上优于最先进的基准模型，成功率提高33%，碰撞率降低54%，并能泛化到未见过的任务组合。

Conclusion: VENTURA展示了通过视觉路径规划实现自然语言指令导航的有效性，并展现出新兴的组合能力，为机器人导航提供了新的解决方案。
...
### [193] [ActiveUMI: Robotic Manipulation with Active Perception from Robot-Free Human Demonstrations](https://arxiv.org/abs/2510.01607)
*Qiyuan Zeng,Chengmeng Li,Jude St. John,Zhongyi Zhou,Junjie Wen,Guorui Feng,Yichen Zhu,Yi Xu*

Main category: cs.RO

TL;DR: ActiveUMI是一个便携式数据收集系统，通过VR遥操作将野外人类演示转移到机器人上，用于复杂的双手操作任务。系统通过记录操作者的主动头部运动来学习视觉注意与操作之间的关键联系。


<details>
  <summary>Details</summary>
Motivation: 解决从野外人类演示到机器人复杂双手操作的数据收集问题，通过便携式系统实现高质量数据采集，并学习主动感知与操作之间的关系。

Method: 结合便携VR遥操作套件和传感器控制器，通过精确姿态对齐桥接人机运动学。采用沉浸式3D模型渲染、自包含可穿戴计算机和高效校准方法。关键特性是捕获主动自我中心感知，记录操作者头部运动。

Result: 在6个挑战性双手任务上评估，仅使用ActiveUMI数据训练的策略在分布内任务上达到70%平均成功率，在新型对象和新环境中保持56%成功率，表现出强泛化能力。

Conclusion: 便携数据收集系统结合学习到的主动感知，为创建可泛化和高能力的真实世界机器人策略提供了有效且可扩展的途径。
...
### [194] [DisCo-Layout: Disentangling and Coordinating Semantic and Physical Refinement in a Multi-Agent Framework for 3D Indoor Layout Synthesis](https://arxiv.org/abs/2510.02178)
*Jialin Gao,Donghao Zhou,Mingjian Liang,Lihao Liu,Chi-Wing Fu,Xiaowei Hu,Pheng-Ann Heng*

Main category: cs.RO

TL;DR: DisCo-Layout是一个解耦物理和语义精炼的3D室内布局生成框架，通过多智能体协调实现最优布局。


<details>
  <summary>Details</summary>
Motivation: 传统方法因固定数据集而泛化能力差，现有LLM/VLM方法语义丰富但缺乏灵活精炼机制，导致布局质量不佳。

Method: 开发了语义精炼工具(SRT)修正抽象对象关系，物理精炼工具(PRT)通过网格匹配算法解决空间问题，多智能体框架协同工作。

Result: 实验证明DisCo-Layout达到最先进性能，生成逼真、连贯且可泛化的3D室内布局。

Conclusion: DisCo-Layout通过解耦和协调物理与语义精炼，有效提升了3D室内布局生成的质量和泛化能力。
...
### [195] [Do You Know Where Your Camera Is? View-Invariant Policy Learning with Camera Conditioning](https://arxiv.org/abs/2510.02268)
*Tianchong Jiang,Jingtian Ji,Xiangshan Tan,Jiading Fang,Anand Bhattad,Vitor Guizilini,Matthew R. Walter*

Main category: cs.RO

TL;DR: 该论文研究通过显式地将策略与相机外参关联来实现视角不变模仿学习，使用Plucker嵌入表示像素光线，在多种行为克隆策略中显著提升跨视角泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决现有模仿学习策略在相机视角变化时性能下降的问题，特别是当策略依赖静态背景中的视觉线索来推断相机姿态时，这种捷径在场景几何或相机位置变化时会失效。

Method: 使用Plucker嵌入表示像素光线，将相机外参显式地作为策略的输入条件，在RoboSuite和ManiSkill中构建了6个操作任务来评估策略在真实视角变化下的鲁棒性。

Result: 实验表明，不包含外参的策略在固定场景中会利用静态背景线索推断相机姿态，但在场景几何或相机位置变化时性能崩溃；而包含外参的策略能够恢复性能，实现仅使用RGB输入的鲁棒控制。

Conclusion: 显式地条件化相机外参能够显著提升模仿学习策略的视角不变性，提供无需深度信息的鲁棒RGB控制方案。
...
### [196] [Jailbreaking LLMs via Semantically Relevant Nested Scenarios with Targeted Toxic Knowledge](https://arxiv.org/abs/2510.01223)
*Hui Dou,Ning Xu,Yiwen Zhang,Kaibin Wang*

Main category: cs.CR

TL;DR: 本文提出RTS-Attack攻击框架，通过构建与查询高度语义相关的嵌套场景并整合针对性有害知识，成功绕过LLMs的对齐防御机制，具有出色的隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 现有嵌套场景攻击方法容易被检测，因为恶意意图明显。作者发现LLMs的对齐防御对语义相关且包含针对性有害知识的嵌套场景不敏感，这是一个重要但未被充分探索的方向。

Method: 提出RTS-Attack自适应自动化框架，构建与查询高度语义相关的嵌套场景，并整合针对性有害知识，生成不包含有害查询的越狱提示。

Result: 在GPT-4o、Llama3-70b、Gemini-pro等多种先进LLMs上的广泛实验表明，RTS-Attack在效率和通用性方面均优于基线方法。

Conclusion: RTS-Attack框架能够有效绕过LLMs的对齐防御，揭示了当前LLMs安全机制在面对语义相关嵌套场景时的脆弱性。
...
### [197] [ZK-WAGON: Imperceptible Watermark for Image Generation Models using ZK-SNARKs](https://arxiv.org/abs/2510.01967)
*Aadarsh Anantha Ramakrishnan,Shubham Agarwal,Selvanayagam S,Kunwar Singh*

Main category: cs.CR

TL;DR: ZK-WAGON是一个基于ZK-SNARKs的图像生成模型水印系统，能够在不暴露模型权重或生成提示的情况下提供可验证的来源证明，通过选择性层电路创建和LSB隐写术实现安全、模型无关的水印嵌入。


<details>
  <summary>Details</summary>
Motivation: 随着图像生成模型变得强大且易得，合成媒体的真实性、所有权和滥用问题日益严重。传统水印方法要么降低图像质量，要么易被移除，或需要访问机密模型内部信息，无法满足安全可扩展部署的需求。

Method: 提出选择性层ZK电路创建(SL-ZKCC)方法，将图像生成模型的关键层选择性转换为电路，显著减少证明生成时间。生成的ZK-SNARK证明通过最低有效位(LSB)隐写术不可察觉地嵌入到生成图像中。

Result: 该系统在GAN和扩散模型上进行了演示，提供了一个安全、模型无关的可信AI图像生成流水线。

Conclusion: ZK-WAGON为图像生成模型提供了一种安全、可验证的水印解决方案，解决了传统方法的局限性，确保在不暴露敏感信息的情况下证明图像来源。
...
### [198] [WAInjectBench: Benchmarking Prompt Injection Detections for Web Agents](https://arxiv.org/abs/2510.01354)
*Yinuo Liu,Ruohan Xu,Xilong Wang,Yuqi Jia,Neil Zhenqiang Gong*

Main category: cs.CR

TL;DR: 本文提出了首个针对Web代理的提示注入攻击检测基准研究，通过构建包含恶意和良性样本的数据集，系统评估了文本和图像检测方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有检测方法主要针对通用提示注入攻击，但缺乏针对Web代理的专门评估。本文旨在填补这一空白，为Web代理提供有效的安全防护方案。

Method: 1) 基于威胁模型对攻击进行细粒度分类；2) 构建包含恶意/良性文本和图像的数据集；3) 系统化文本和图像检测方法；4) 在多场景下评估检测性能。

Result: 研究发现，部分检测器能有效识别依赖显式文本指令或可见图像扰动的攻击，但对省略显式指令或使用不可察觉扰动的攻击基本无效。

Conclusion: 现有检测方法在Web代理场景下存在局限性，需要开发更先进的检测技术来应对复杂的提示注入攻击。
...
### [199] [Position: Privacy Is Not Just Memorization!](https://arxiv.org/abs/2510.01645)
*Niloofar Mireshghallah,Tianshi Li*

Main category: cs.CR

TL;DR: 本文批评当前LLM隐私研究过度关注训练数据逐字记忆，而忽视了更紧迫和可扩展的隐私威胁，提出了涵盖数据收集、推理时上下文泄露、自主代理能力和深度推理攻击的全面隐私风险分类法。


<details>
  <summary>Details</summary>
Motivation: 当前LLM隐私讨论过度集中于训练数据提取，而忽略了数据收集实践、推理时上下文泄露、自主代理能力和通过深度推理攻击实现监控民主化等更直接和可扩展的隐私威胁。

Method: 提出了涵盖LLM全生命周期的隐私风险分类法，通过案例研究展示现有隐私框架的不足，并对2016-2025年间1,322篇AI/ML隐私论文进行纵向分析。

Result: 研究发现记忆问题在技术研究中受到过度关注，而最紧迫的隐私危害存在于其他地方，现有技术方法在这些领域几乎无法提供解决方案。
...
TL;DR: MPMAvatar是一个从多视角视频创建3D人体化身的框架，支持高度逼真、鲁棒的动画和自由视角的光线真实渲染。通过使用材料点方法和改进的碰撞处理算法，解决了松散衣物物理模拟的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基于物理模拟的方法在建模松散衣物动态时存在精度有限或对新颖动画输入鲁棒性不足的问题，需要开发更准确和鲁棒的动态建模方法。

Method: 使用材料点方法模拟器，结合各向异性本构模型和新型碰撞处理算法来建模复杂衣物变形和身体接触；结合使用3D高斯溅射渲染的规范化身，实现高保真渲染。

Result: MPMAvatar在动态建模精度、渲染精度、鲁棒性和效率方面显著优于现有最先进的基于物理的化身方法，并能以零样本方式泛化到未见过的交互场景。

Conclusion: 该框架成功解决了松散衣物物理模拟的挑战，实现了高度逼真和鲁棒的3D人体化身动画，在多个关键指标上超越了现有方法，并展示了零样本泛化能力。
...
TL;DR: ROI-GS是一个面向感兴趣区域的高效3D场景重建框架，通过对象感知的方法在保持实时性能的同时提升局部细节质量并减小模型尺寸。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法在场景中均匀分配资源，导致感兴趣区域的细节受限且模型尺寸过大，需要一种能够针对特定对象优化细节的方法。

Method: 提出ROI-GS框架，包括对象引导的相机选择、针对性的对象训练，以及将高保真对象重建无缝集成到全局场景中。

Result: 实验显示ROI-GS显著提升局部质量（PSNR最高提升2.96 dB），模型尺寸减少约17%，单对象场景训练速度更快，优于现有方法。

Conclusion: ROI-GS通过对象感知的方法有效解决了3D场景重建中局部细节与全局效率的平衡问题，在提升感兴趣区域质量的同时优化了模型性能。
...
Method: 结合微表面BRDF和高光-光泽度参数化到2D高斯泼溅中，采用延迟着色；使用基于扩散的表面法线和漫反射颜色先验指导早期优化；采用环境贴图的粗到细优化策略。

Result: 在复杂光泽场景上的广泛实验表明，该方法实现了高质量的几何和材料重建，相比现有高斯泼溅方法，在新光照下提供更真实和一致的重光照效果。

Conclusion: 该框架通过物理一致的BRDF建模和优化策略，显著提升了光泽物体的重建和重光照质量，解决了形状、材料属性和光照解耦的长期挑战。
...
### [203] [Extracting O*NET Features from the NLx Corpus to Build Public Use Aggregate Labor Market Data](https://arxiv.org/abs/2510.01470)
*Stephen Meisenbacher,Svetlozar Nestorov,Peter Norlander*

Main category: cs.CY

TL;DR: 开发了Job Ad Analysis Toolkit (JAAT)工具包，使用O*NET框架从在线招聘广告中提取结构化信息，构建了包含超过100亿数据点的大规模数据集。


<details>
  <summary>Details</summary>
Motivation: 在线招聘数据难以获取且缺乏标准化，而O*NET数据库更新频率低且样本量小，需要更好的数据收集和分析工具。

Method: 采用O*NET作为框架，开发自然语言处理工具JAAT，从1550万条在线招聘广告中提取结构化信息，包括任务、职业代码、工具技术、工资等特征。

Result: 成功提取超过100亿数据点，构建了2015-2025年月度活跃工作的职业、州和行业层面特征数据集，验证了工具的可靠性和准确性。

Conclusion: JAAT工具包为研究和教育劳动力发展提供了重要数据资源，展示了在就业市场分析中的巨大潜力。
...
### [204] [Synthetic Prefixes to Mitigate Bias in Real-Time Neural Query Autocomplete](https://arxiv.org/abs/2510.01574)
*Adithya Rajan,Xiaoyu Liu,Prateek Verma,Vibhu Arora*

Main category: cs.IR

TL;DR: 提出一种通过合成前缀来缓解实时神经查询自动补全系统中呈现偏差的数据中心方法，利用未激活自动补全时收集的完整用户查询生成前缀，丰富学习排序模型的训练数据。
...
Method: 从完整用户查询生成合成前缀来丰富训练数据；开发针对实时部署优化的神经排序器，包含查询流行度、季节性、模糊匹配分数等丰富特征；引入任务特定的列表损失简化，将计算复杂度从O(n²)降低到O(n)。

Result: 在大规模电商环境中部署，通过平均倒数排名等相关指标显示用户参与度有统计显著提升。

Conclusion: 合成前缀不仅能提高泛化能力，还为其他低延迟排序任务（如相关搜索和查询推荐）中的偏差缓解提供了可扩展路径。
...
### [205] [Bridging Collaborative Filtering and Large Language Models with Dynamic Alignment, Multimodal Fusion and Evidence-grounded Explanations](https://arxiv.org/abs/2510.01606)
*Bo Ma,LuYao Liu,Simon Lau,Chandler Yuan,and XueY Cui,Rosie Zhang*

Main category: cs.IR

TL;DR: 提出了一个名为\model{}的推荐系统框架，通过在线适应机制、多模态统一表示和可验证解释系统，解决传统LLM推荐中用户偏好变化、多模态内容处理和可信解释的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推荐方法存在三个主要问题：协作过滤模型使用静态快照无法捕捉快速变化的用户偏好；现实物品包含丰富的视觉和音频内容；现有系统缺乏基于具体证据的可信解释。

Method: 1. 在线适应机制：通过轻量级模块持续整合新用户交互；2. 统一表示：将协作信号与视觉、音频特征无缝结合；3. 解释系统：基于具体协作模式和物品属性生成可验证的自然语言解释。

Result: 该方法在保持冻结基础模型效率的同时，仅增加最小计算开销，适合实际部署。

Conclusion: \model{}框架有效解决了LLM推荐系统中的关键挑战，实现了高效、多模态和可解释的推荐。
...
### [206] [LLM4Rec: Large Language Models for Multimodal Generative Recommendation with Causal Debiasing](https://arxiv.org/abs/2510.01622)
*Bo Ma,Hang Li,ZeHua Hu,XiaoFan Gui,LuYao Liu,Simon Lau*

Main category: cs.IR

TL;DR: 提出一个增强的生成式推荐框架，通过多模态融合、检索增强生成、因果推断去偏、可解释推荐和实时自适应学习五大创新，显著提升推荐准确性、公平性和多样性。


<details>
  <summary>Details</summary>
Motivation: 当前生成式推荐系统在处理多模态数据、消除算法偏见和提供透明决策过程方面面临重大挑战，需要开发更先进的解决方案。

Method: 基于大语言模型构建框架，包含多模态融合架构、检索增强生成机制、因果推断去偏、可解释推荐生成和实时自适应学习能力等五个关键创新模块。

Result: 在三个基准数据集上的实验显示，相比现有方法，该框架在NDCG@10指标上提升达2.3%，多样性指标提升1.4%，同时通过优化推理策略保持计算效率。

Conclusion: 该增强生成式推荐框架有效解决了多模态数据处理、算法偏见消除和决策透明度等关键挑战，为下一代推荐系统提供了可行的技术路径。
...
### [207] [Study on LLMs for Promptagator-Style Dense Retriever Training](https://arxiv.org/abs/2510.02241)
*Daniel Gwon,Nour Jedidi,Jimmy Lin*

Main category: cs.IR

TL;DR: 研究表明，参数规模小至3B的开源LLM可以作为有效的Promptagator式查询生成器，为领域专用密集检索模型提供可靠的合成数据生成替代方案。


<details>
  <summary>Details</summary>
Motivation: 原始Promptagator方法依赖专有的大规模LLM，用户可能无法访问或无法用于敏感数据。本研究旨在探索参数规模≤14B的开源LLM作为替代方案的可行性。

Method: 使用参数规模在3B到14B之间的开源LLM作为Promptagator式查询生成器，通过少量示例提示生成任务特定查询，用于微调领域专用密集检索模型。

Result: 实验结果表明，参数规模小至3B的开源LLM能够有效替代专有LLM，作为Promptagator式查询生成器发挥作用。

Conclusion: 开源LLM为合成数据生成提供了可靠的替代方案，并为最大化领域特定应用的微调效果提供了实用见解。
...
TL;DR: JaneEye是一个专为可穿戴设备设计的基于事件相机的高效能眼动追踪硬件加速器，通过轻量级神经网络架构和硬件优化，实现了高精度、低延迟和低功耗的眼动追踪。


<details>
  <summary>Details</summary>
Motivation: 传统基于帧的眼动追踪系统在扩展现实(XR)应用中难以满足高精度、低延迟和低功耗的严格要求，而事件相机提供了超高时间分辨率和低功耗的替代方案。

Method: 提出超轻量级神经网络架构，包含新颖的ConvJANET层（仅保留遗忘门的简化ConvLSTM），使用自定义线性激活函数近似和定点量化，通过软硬件协同设计实现高效能。

Result: 在3ET+数据集上实现2.45像素误差的高精度，仅使用17.6K参数，支持1250Hz事件帧率；12nm ASIC实现400MHz运行频率，0.5ms端到端延迟（相当于2000FPS），能效为18.9μJ/帧。
...
TL;DR: MorphGen是一个基于扩散模型的荧光显微镜图像生成器，能够跨多种细胞类型和扰动进行可控生成，通过匹配OpenPhenom的表型嵌入来保持生物学意义，并联合生成完整的荧光通道以保留细胞器细节。


<details>
  <summary>Details</summary>
Motivation: 加速基于高内涵图像的药物发现和基因编辑实验，通过计算机模拟细胞对干预的响应。

Method: 使用扩散生成模型，结合对齐损失将表示与OpenPhenom生物基础模型的表型嵌入匹配，联合生成完整的荧光通道而非压缩为RGB图像。

Result: 通过CellProfiler特征验证了与真实图像的生物学一致性，FID分数比之前的SOTA模型MorphoDiff降低了35%以上。

Conclusion: MorphGen能够生成生物学一致的荧光显微镜图像，保留细胞器细节，为生物解释提供细粒度形态分析能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有乳腺癌分割公共数据集缺乏形态多样性，无法支持模型泛化性和在异质患者队列中进行稳健的生物标志物验证。

Method: 收集来自三个临床中心和两个公共数据集的587个样本，使用七种扫描仪数字化，采用多样化标注策略对四个类别进行标注：浸润性上皮、非浸润性上皮、坏死和其他。

Result: 创建了一个包含形态多样性、特别关注导管原位癌和分散性小叶肿瘤细胞等代表性不足形态的数据集。

Conclusion: BEETLE数据集的高多样性和与自动化生物标志物量化领域的相关性确保了其高重用潜力，并提供了精心策划的多中心外部评估集以支持标准化基准测试。
...
### [211] [From 2D to 3D, Deep Learning-based Shape Reconstruction in Magnetic Resonance Imaging: A Review](https://arxiv.org/abs/2510.01296)
*Emma McMillian,Abhirup Banerjee,Alfonso Bueno-Orovio*

Main category: cs.LG

TL;DR: 这篇综述系统回顾了基于深度学习的2D MRI到3D形状重建方法，涵盖点云、网格、形状感知和体积模型四类主要方法，分析了技术现状、局限性和临床应用，并探讨了数据集、计算需求和新兴研究方向。


<details>
  <summary>Details</summary>
Motivation: 3D形状重建在医学疾病诊断、治疗规划和计算建模中日益重要，需要系统梳理当前深度学习方法的现状和发展方向，为研究者提供结构化概览。

Method: 采用文献综述方法，分析四类主要重建方法：点云模型、网格模型、形状感知模型和体积模型，涵盖从心脏到神经再到肺部成像的广泛应用。

Result: 系统总结了各类方法的技术基础、局限性和临床应用，分析了公开数据集、计算需求和评估指标，为研究者提供了全面的方法学概览。

Conclusion: 该综述为研究者提供了当前3D重建方法的结构化概览，有助于识别推进深度学习向更鲁棒、可泛化和临床影响力解决方案的机会，并强调了多模态集成和跨模态框架等新兴研究方向。
...
### [212] [Ultra-Efficient Decoding for End-to-End Neural Compression and Reconstruction](https://arxiv.org/abs/2510.01407)
...
<details>
  <summary>Details</summary>
Motivation: 传统神经压缩方法虽然压缩率高，但卷积解码器复杂度和计算成本高，阻碍了实际应用，需要解决解码器瓶颈问题。
...
### [213] [Control the Temperature: Selective Sampling for Diverse and High-Quality LLM Outputs](https://arxiv.org/abs/2510.01218)
...
<details>
  <summary>Details</summary>
Motivation: 传统高温采样方法在需要高精度的任务（如数学推理）中会降低推理质量，因为会在敏感解码位置采样错误的继续内容。

Method: 选择性采样：基于采样风险指标动态切换贪婪采样和高温采样。通过在小规模可验证问题上训练轻量级分类器来预测采样风险，并与基础语言模型集成。

Result: 在数学推理任务上的实验表明，选择性采样即使在高温设置下也能提升质量-多样性的权衡效果。

Conclusion: 选择性采样方法有效解决了高温采样在精度敏感任务中的准确性问题，实现了更好的多样性与准确性平衡。
...
### [214] [Automated Extraction of Material Properties using LLM-based AI Agents](https://arxiv.org/abs/2510.01235)
*Subham Ghosh,Abhishek Tewari*

Main category: cs.LG

TL;DR: 开发了一个基于LLM的自主工作流，从约10,000篇科学文献中提取热电和结构性能数据，创建了包含27,822条记录的大型数据集，并提供了交互式网络探索工具。
...
Result: GPT-4.1达到最高准确率（热电性能F1=0.91，结构字段F1=0.82），GPT-4.1 Mini以较低成本提供相近性能。提取了27,822条温度相关性能记录和结构属性。

Conclusion: 提供了迄今最大的LLM整理热电数据集，建立了可扩展的数据驱动材料发现基础，工作流程可复现且成本可控。
...
### [215] [Beyond Simple Fusion: Adaptive Gated Fusion for Robust Multimodal Sentiment Analysis](https://arxiv.org/abs/2510.01677)
*Han Wu,Yanming Sun,Yunhe Yang,Derek F. Wong*

Main category: cs.LG

TL;DR: 提出了一种自适应门控融合网络(AGFN)，通过基于信息熵和模态重要性的双门融合机制自适应调整特征权重，有效处理多模态情感分析中的噪声、缺失和语义冲突问题。
...
Method: 采用双门融合机制，基于信息熵和模态重要性自适应调整特征权重，在单模态编码和跨模态交互后优先选择信息量大的模态线索。

Result: 在CMU-MOSI和CMU-MOSEI数据集上的实验表明，AGFN在准确率上显著优于强基线方法，能有效识别细微情感且具有鲁棒性。

Conclusion: AGFN通过减少特征位置与预测误差之间的相关性，从更广泛的特征分布中学习，创建了更鲁棒的多模态特征表示，增强了泛化能力。
...
### [216] [RSAVQ: Riemannian Sensitivity-Aware Vector Quantization for Large Language Models](https://arxiv.org/abs/2510.01240)
*Zukang Xu,Xing Hu,Qiang Wu,Dawei Yang*

Main category: cs.LG

TL;DR: RSAVQ是一个新颖的向量量化框架，通过几何驱动的方法解决大语言模型极低位量化中的方向误差和比特分配问题，在2位量化上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型参数指数增长给资源受限设备部署带来挑战，现有向量量化方法面临无约束方向误差和次优比特分配两个关键问题。
...
Result: 在LLaMA-3 8B的2位量化中，RSAVQ在困惑度上领先VPTQ和QuIP# 0.4，在零样本准确率上领先1.5。
...
### [217] [RLP: Reinforcement as a Pretraining Objective](https://arxiv.org/abs/2510.01265)
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的大模型训练范式只在最后阶段使用强化学习，作者质疑这是否是最优方式，希望将强化学习的核心精神——探索——更早地引入预训练过程。

Method: 将思维链视为探索性动作，基于思维链为预测未来token提供的信息增益计算奖励信号，具体通过比较在上下文+思维链条件下与仅上下文条件下下一个token的对数似然差异。

Result: 在Qwen3-1.7B-Base上预训练RLP，在8个数学和科学基准上的平均性能提升19%；在Nemotron-Nano-12B-v2上应用RLP，整体平均从42.81%提升到61.32%，科学推理平均提升23%。

Conclusion: RLP成功将强化学习重新定义为普通文本的预训练目标，弥合了下一个token预测与有用思维链推理之间的差距，证明了在不同架构和模型规模上的可扩展性。
...
### [218] [Unsupervised Dynamic Feature Selection for Robust Latent Spaces in Vision Tasks](https://arxiv.org/abs/2510.01758)
...
### [219] [$\text{G}^2$RPO: Granular GRPO for Precise Reward in Flow Models](https://arxiv.org/abs/2510.01982)
*Yujie Zhou,Pengyang Ling,Jiazi Bu,Yibin Wang,Yuhang Zang,Jiaqi Wang,Li Niu,Guangtao Zhai*

Main category: cs.LG

TL;DR: 提出G²RPO框架，通过粒度化奖励评估改进流模型的强化学习对齐，解决现有方法奖励信号稀疏和狭窄的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在探索高价值样本时有效，但由于稀疏和狭窄的奖励信号导致偏好对齐效果不佳。

Method: 引入奇异随机采样策略支持逐步随机探索，同时采用多粒度优势集成模块聚合不同扩散尺度的优势计算。

Result: 在各种奖励模型上的实验表明，G²RPO显著优于现有的基于流的GRPO基线方法。

Conclusion: G²RPO框架在流模型的强化学习中对齐人类偏好方面表现出有效性和鲁棒性。
...
### [220] [Optimal Stopping vs Best-of-$N$ for Inference Time Optimization](https://arxiv.org/abs/2510.01394)
*Yusuf Kalayci,Vinod Raman,Shaddin Dughmi*

Main category: cs.LG

TL;DR: 提出基于Pandora's Box问题的LLM推理时优化框架，通过UCB算法实现接近最优的生成停止策略，在AlpacaFarm和HH-RLHF数据集上相比Best-of-N采样减少15-35%生成次数。
...
Method: 将每次生成视为打开一个代价随机的"盒子"，开发UCB风格的Pandora's Box算法，结合Bradley-Terry变换处理跨提示的奖励缩放问题，实现自适应推理时优化。

Result: 在多个LLM-奖励模型对上的实验表明，自适应策略能达到与Best-of-N采样相同的性能，同时平均减少15-35%的生成次数。
...
### [221] [LSPO: Length-aware Dynamic Sampling for Policy Optimization in LLM Reasoning](https://arxiv.org/abs/2510.01459)
*Weizhe Chen,Sven Koenig,Bistra Dilkina*

Main category: cs.LG

TL;DR: 提出LSPO算法，基于响应长度动态选择训练数据，改进RLVR在推理任务中的效果


<details>
  <summary>Details</summary>
Motivation: 受LLM过度思考现象启发，探索如何通过动态数据选择提高强化学习验证奖励的效率

Method: 提出LSPO元算法，根据平均响应长度在每个训练步骤动态选择训练数据

Result: 在多个基础模型和数据集上验证，LSPO持续提升学习效果

Conclusion: 通过消融研究验证了长度信号在动态采样中的有效性，为未来研究指明方向
...
### [222] [Think Right: Learning to Mitigate Under-Over Thinking via Adaptive, Attentive Compression](https://arxiv.org/abs/2510.01581)
...
<details>
  <summary>Details</summary>
Motivation: 现有思维模型在复杂推理任务中存在适应性不足的问题：简单问题过度思考导致token效率低下，困难问题思考不足导致错误。需要平衡欠思考和过度思考。

Method: 提出TRAAC方法：利用模型在长推理轨迹上的自注意力识别重要步骤并剪枝冗余步骤；估计问题难度并纳入训练奖励；学习根据难度分配推理预算。

Result: 在多种任务上，TRAAC相比基础模型平均准确率提升8.4%，推理长度相对减少36.8%；相比最佳RL基线准确率提升7.9%，长度减少29.4%。在分布外非数学数据集上也表现出强泛化能力。
...
### [223] [Test-Time Anchoring for Discrete Diffusion Posterior Sampling](https://arxiv.org/abs/2510.02291)
...
TL;DR: 本文提出了一种基于预训练离散扩散基础模型的后验采样方法，通过量化期望和锚定重掩码技术，在无需重新训练的情况下从噪声测量中恢复图像，在逆问题上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有离散扩散后验采样方法面临挑战：无导数引导产生稀疏信号、连续松弛限制应用范围、分裂吉布斯采样器受维度诅咒影响。需要克服这些限制以实现更有效的后验采样。

Method: 提出锚定后验采样(APS)方法，包含两个关键技术：在离散嵌入空间中使用量化期望实现类似梯度的引导，以及锚定重掩码实现自适应解码。

Result: 在标准基准测试的线性和非线性逆问题上，该方法在离散扩散采样器中达到最先进性能，并在无需训练的风格化和文本引导编辑中展示优势。

Conclusion: APS方法成功克服了现有离散扩散后验采样的限制，为基于预训练离散扩散模型的后验采样提供了有效解决方案。
...
### [224] [Continual Personalization for Diffusion Models](https://arxiv.org/abs/2510.02296)
*Yu-Chien Liao,Jr-Jen Chen,Chi-Pin Huang,Ci-Siang Lin,Meng-Lin Wu,Yu-Chiang Frank Wang*

Main category: cs.LG

TL;DR: 提出了一种名为概念神经元选择（CNS）的新方法，用于扩散模型的增量学习，通过识别与目标概念相关的神经元来执行个性化，同时避免灾难性遗忘并保持零样本生成能力。


<details>
  <summary>Details</summary>
Motivation: 在现实应用中，增量更新扩散模型具有实用性但计算挑战大，需要解决灾难性遗忘问题并保持模型的零样本生成能力。

Method: CNS方法独特识别扩散模型中与目标概念相关的神经元，以增量方式微调这些概念神经元，并联合保留先前学到的知识。

Result: 在真实数据集上的评估显示，CNS以最少的参数调整实现了最先进的性能，在单概念和多概念个性化任务中都优于先前方法，同时实现了无融合操作，减少了持续个性化的内存存储和处理时间。

Conclusion: CNS是一种简单有效的持续学习策略，能够在增量设置中成功更新扩散模型，同时保持零样本文本到图像生成能力。
...
TL;DR: EqM是一种基于平衡动力学的生成建模框架，通过隐式能量景观的平衡梯度学习，采用优化驱动的采样过程，在ImageNet 256×256上达到1.90 FID。


<details>
  <summary>Details</summary>
Motivation: 传统扩散和流模型采用非平衡、时间条件动态，EqM旨在通过平衡动力学视角构建更统一的生成建模框架。

Method: EqM学习隐式能量景观的平衡梯度，在推理时采用基于优化的采样过程，使用可调步长、自适应优化器和自适应计算。

Result: EqM在ImageNet 256×256上达到1.90 FID，超越了传统扩散/流模型，并能处理部分噪声图像去噪、OOD检测和图像合成等任务。

Conclusion: EqM通过用统一平衡景观替代时间条件速度，在流模型和能量基模型之间建立了更紧密的桥梁，提供了优化驱动推理的简单途径。
...
### [226] [Quagmires in SFT-RL Post-Training: When High SFT Scores Mislead and What to Use Instead](https://arxiv.org/abs/2510.01624)
*Feiyang Kang,Michael Kuchnik,Karthik Padthe,Marin Vlastelica,Ruoxi Jia,Carole-Jean Wu,Newsha Ardalani*

Main category: cs.LG

TL;DR: 研究发现SFT（监督微调）的高分并不能可靠预测后续RL（强化学习）的效果，有时甚至会导致更差的结果。提出了泛化损失和Pass@large k作为更好的RL效果预测指标。
...
Method: 训练了数百个12B参数模型，使用SFT和GRPO RLVR，在7个数学基准上进行了大量评估，花费超过100万GPU小时。

Result: 发现基于泛化损失和Pass@large k的预测比直接使用pre-RL性能预测精度显著提高，R²和Spearman相关系数提升高达0.5（2倍）。

Conclusion: SFT分数不是RL效果的良好预测指标，泛化损失和Pass@large k提供了更可靠的预测方法，对实际应用有重要价值。
...
### [227] [Demystifying Synthetic Data in LLM Pre-training: A Systematic Study of Scaling Laws, Benefits, and Pitfalls](https://arxiv.org/abs/2510.01631)
...
TL;DR: 大规模实证研究表明：单纯使用改写式合成数据预训练效果不如自然网络文本；但1/3改写合成数据与2/3自然数据混合可加速5-10倍。纯教科书式合成数据预训练在小型数据预算下表现较差。最佳合成数据比例约30%，且8B参数生成器效果已足够。
...
Method: 使用统一协议和扩展定律进行大规模实证研究（>1000个LLM，>10万GPU小时），比较自然网络数据、多样化合成数据（改写文本、生成教科书）及其混合。

Result: 改写合成数据单独预训练效果不如自然文本；1/3改写合成+2/3自然数据混合可加速5-10倍；教科书式合成数据单独预训练在小型数据预算下损失显著；最佳合成数据比例约30%；8B参数生成器效果足够。

Conclusion: 合成数据在预训练中具有条件性优势，改写式合成数据未见模型崩溃现象，而教科书式合成数据显示出模型崩溃模式，为实际应用提供了指导。
...
### [228] [Sparse Query Attention (SQA): A Computationally Efficient Attention Mechanism with Query Heads Reduction](https://arxiv.org/abs/2510.01817)
*Adam Filipek*

Main category: cs.LG

TL;DR: 提出Sparse Query Attention (SQA)机制，通过减少查询头数量来降低注意力计算复杂度，在长序列处理中实现高达3倍的吞吐量提升，且对模型质量影响极小。


<details>
  <summary>Details</summary>
Motivation: 传统MHA机制存在二次计算复杂度问题，现有MQA和GQA方法虽然解决了内存带宽瓶颈，但未减少注意力计算所需的FLOPs，这成为训练和全序列处理的关键瓶颈。

Method: SQA通过减少查询头数量而非键值头来降低计算复杂度，提出了理论基础和数学公式，以及一系列架构变体。

Result: 在32k-200k令牌的长序列基准测试中，SQA在计算受限场景（如预训练、微调和编码器任务）中实现了高达3倍的吞吐量提升，初步小规模实验显示对模型质量影响很小。

Conclusion: SQA是一种有效的注意力机制优化方法，能够构建更高效和可扩展的模型，该机制是在开发Reactive Transformer架构时偶然发现的。
...
### [229] [StockBench: Can LLM Agents Trade Stocks Profitably In Real-world Markets?](https://arxiv.org/abs/2510.02209)
*Yanxu Chen,Zijun Yao,Yantao Liu,Jin Ye,Jianing Yu,Lei Hou,Juanzi Li*

Main category: cs.LG

TL;DR: StockBench是一个无污染基准测试，用于评估LLM代理在真实多个月股票交易环境中的表现。代理接收每日市场信号并做出买卖决策，性能通过金融指标评估。


<details>
  <summary>Details</summary>
Motivation: 现有金融基准主要测试静态知识问答，无法捕捉交易的动态迭代特性。金融领域对LLM代理评估不足，尽管其具有经济价值和高风险决策的相关性。

Method: 构建StockBench基准，在真实多个月股票交易环境中评估LLM代理。代理接收每日价格、基本面和新闻等市场信号，做出顺序买卖决策，使用累计回报、最大回撤和索提诺比率等金融指标评估性能。

Result: 评估最先进的专有和开源模型发现，大多数LLM代理难以超越简单的买入持有基准，但部分模型显示出获得更高回报和更有效管理风险的潜力。

Conclusion: 精通静态金融知识任务不一定能转化为成功的交易策略。StockBench作为开源资源发布，支持可重复性和该领域未来研究。
...
### [230] [ExGRPO: Learning to Reason from Experience](https://arxiv.org/abs/2510.02245)
*Runzhe Zhan,Yafu Li,Zhi Wang,Xiaoye Qu,Dongrui Liu,Jing Shao,Derek F. Wong,Yu Cheng*

Main category: cs.LG

TL;DR: 本文提出ExGRPO框架，通过识别推理经验价值指标（正确性和熵），组织并优先处理有价值的经验，使用混合策略目标平衡探索与经验利用，显著提升大型语言模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 标准的在线策略训练在单次更新后丢弃经验，导致计算效率低下和训练不稳定。虽然已有研究强调重用过去经验的好处，但经验特性对大型推理模型学习动态的影响仍未充分探索。

Method: 提出ExGRPO框架：识别推理经验价值指标（正确性和熵），组织并优先处理有价值的经验，使用混合策略目标平衡探索与经验利用。

Result: 在五个骨干模型（1.5B-8B参数）上实验显示，ExGRPO在数学/通用基准上持续提升推理性能，相比在线策略RLVR平均增益+3.5/7.6分，并在强弱模型上都能稳定训练。

Conclusion: 原则性的经验管理是高效可扩展RLVR的关键要素，ExGRPO框架通过有效利用经验显著提升了推理模型的性能和训练稳定性。
...
### [231] [Tree-based Dialogue Reinforced Policy Optimization for Red-Teaming Attacks](https://arxiv.org/abs/2510.02286)
*Ruohao Guo,Afshin Oroojlooy,Roshan Sridhar,Miguel Ballesteros,Alan Ritter,Dan Roth*

Main category: cs.LG

TL;DR: 提出了DialTree-RPO框架，使用强化学习结合树搜索自动发现多轮对话攻击策略，显著提高了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在多轮交互环境中仍容易受到对抗攻击，现有方法主要依赖人工红队测试或预定义模板，未能充分探索多轮攻击的复杂策略空间。

Method: 采用基于策略的强化学习框架，结合树搜索将对话建模为序列决策问题，无需人工标注数据即可系统性地探索多轮攻击策略。

Result: 在10个目标模型上相比现有最优方法攻击成功率提高了25.9%以上，并能有效发现新的攻击策略。

Conclusion: DialTree-RPO框架能够自主发现多样化的多轮攻击策略，显著增强了LLM安全漏洞的检测能力。
...
### [232] [Interactive Training: Feedback-Driven Neural Network Optimization](https://arxiv.org/abs/2510.02297)
*Wentao Zhang,Yang Young Lu,Yuntian Deng*

Main category: cs.LG

TL;DR: 提出了Interactive Training框架，允许在神经网络训练过程中进行实时、反馈驱动的人工或AI干预，通过控制服务器动态调整优化器超参数、训练数据和模型检查点。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络训练遵循固定的优化方案，缺乏动态响应训练不稳定或问题的灵活性，需要更智能的训练范式。

Method: 使用控制服务器作为中介，实现用户或AI代理与训练过程之间的实时通信，支持动态调整超参数、数据和检查点。

Result: 三个案例研究表明，该框架实现了更好的训练稳定性、降低了对初始超参数的敏感性，并提高了对用户需求变化的适应性。
...
### [233] [Ovi: Twin Backbone Cross-Modal Fusion for Audio-Video Generation](https://arxiv.org/abs/2510.01284)
*Chetwin Low,Weimin Wang,Calder Katyal*

Main category: cs.MM

TL;DR: Ovi是一个统一的音频-视频生成范式，将两种模态建模为单一生成过程，通过双胞胎DiT模块的块级跨模态融合实现自然同步，无需单独管道或后处理对齐。


<details>
  <summary>Details</summary>
Motivation: 传统音频-视频生成通常依赖复杂的多阶段架构或声音与视觉的顺序合成，存在同步困难的问题。

Method: 使用块级跨模态融合的双胞胎DiT模块，通过缩放RoPE嵌入交换时序信息和双向交叉注意力交换语义信息，在大型视频语料库上联合训练相同的视频和音频塔。

Result: 模型能够生成具有自然语音和准确、上下文匹配音效的电影级视频片段，实现富有表现力的电影叙事。

Conclusion: Ovi提供了一个统一的音频-视频生成框架，通过端到端训练实现了高质量的同步多媒体内容生成。
...
### [234] [An Efficient Quality Metric for Video Frame Interpolation Based on Motion-Field Divergence](https://arxiv.org/abs/2510.01361)
...
Method: 基于运动发散加权技术，从档案胶片修复中借鉴而来，用于检测时间不一致性。该方法突出运动场中的奇点，并用于加权图像误差。

Result: 在BVI-VFI数据集上，PSNR_DIV相比FloLPIPS皮尔逊线性相关系数提高0.09，速度快2.5倍，内存使用减少4倍，在所有内容类别上性能一致且对运动估计器鲁棒。

Conclusion: PSNR_DIV的高效性和准确性使其能够快速进行质量评估，并可作为训练视频帧插值神经网络的损失函数。
...
### [235] [Median2Median: Zero-shot Suppression of Structured Noise in Images](https://arxiv.org/abs/2510.01666)
*Jianxu Wang,Ge Wang*

Main category: eess.IV

TL;DR: 提出了Median2Median (M2M)零样本去噪框架，专门针对结构化噪声设计，通过新颖的采样策略生成伪独立子图像对，无需训练数据即可有效去除相关噪声。
...
Method: 引入方向插值和广义中值滤波生成伪独立子图像对，采用随机分配策略扩大采样空间并消除系统偏差，适用于Noise2Noise训练。

Result: 在真实模拟研究中，M2M在独立同分布噪声下与最先进零样本方法相当，在相关噪声下始终优于它们。
...
### [236] [GFSR-Net: Guided Focus via Segment-Wise Relevance Network for Interpretable Deep Learning in Medical Imaging](https://arxiv.org/abs/2510.01919)
*Jhonatan Contreras,Thomas Bocklitz*

Main category: eess.IV

TL;DR: GFSR-Net是一种用于医学图像分析的可解释性深度学习方法，通过少量人工标注引导模型关注与诊断相关的图像区域，提高模型可靠性和可信度。


<details>
  <summary>Details</summary>
Motivation: 深度学习在医学图像分析中取得了显著成功，但在临床实践中因缺乏可解释性而受到限制。现有模型可能依赖与疾病无关的图像区域或现实条件下不存在的视觉线索，这会降低信任度并增加误诊风险。

Method: GFSR-Net使用少量人工标注来近似人类在图像中的直觉关注区域，无需精确边界或详尽标记。在训练过程中，模型学习将其关注点与这些区域对齐，逐步强调具有诊断意义的特征。

Result: 实验表明GFSR-Net在保持相当或更优准确率的同时，能生成更符合人类期望的显著性图，减少对无关模式的依赖。

Conclusion: 该方法提高了自动化诊断工具的可信度，适用于胸部X光、视网膜扫描和皮肤科图像等多种医学图像类型。
...
### [237] [SpurBreast: A Curated Dataset for Investigating Spurious Correlations in Real-world Breast MRI Classification](https://arxiv.org/abs/2510.02109)
*Jong Bum Won,Wesley De Neve,Joris Vankerschaver,Utku Ozbulak*

Main category: eess.IV

TL;DR: 提出了SpurBreast数据集，这是一个专门设计的乳腺MRI数据集，用于系统研究深度神经网络在医学影像中学习虚假相关性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有医学影像数据集无法系统研究虚假相关性，因为模型可能学习非临床特征而非有意义的医学模式。

Method: 通过分析100多个患者、设备和成像协议特征，识别出两个主要虚假信号：磁场强度（全局特征）和图像方向（局部特征），并创建包含和不包含虚假相关性的对比数据集。

Result: 研究表明DNNs会利用这些非临床信号，在验证集上获得高准确率但在无偏测试数据上泛化失败。

Conclusion: SpurBreast数据集为研究临床相关和无关特征、不确定性估计、对抗鲁棒性和泛化策略提供了系统评估平台。
...
### [238] [Measurement-Guided Consistency Model Sampling for Inverse Problems](https://arxiv.org/abs/2510.02208)
*Amirreza Tanevardi,Pooria Abbas Rad Moghadam,Sajjad Amini*

Main category: eess.IV

TL;DR: 提出一种针对逆问题重建的改进一致性采样方法，通过测量一致性机制引导采样器的随机性，在保持一致性生成效率的同时确保测量保真度


<details>
  <summary>Details</summary>
Motivation: 扩散模型在逆成像问题中表现出强大潜力，但多步采样速度慢限制了实际应用。一致性模型可实现单步或少步高质量生成，但其在逆问题中的直接应用尚未充分探索

Method: 修改一致性采样方法，通过测量一致性机制引导采样器随机性，该机制与测量算子相关联，在保持一致性生成效率的同时强制满足测量保真度

Result: 在Fashion-MNIST和LSUN Bedroom数据集上的实验显示，相比基线一致性采样，在感知和像素级指标（FID、KID、PSNR、SSIM）上均有持续改进

Conclusion: 该方法仅需少量步骤即可获得竞争性或更优的重建结果，为逆问题重建提供了高效解决方案
...
TL;DR: 提出了一种人机协作AI系统，帮助助产士在资源匮乏地区使用盲扫协议获取诊断相关的胎儿图像，通过AI识别关键帧来减轻专家工作量。
...
Method: 开发了结合分类模型和基于网络平台的AI系统，使用盲扫协议和低成本POCUS设备，由经过短期培训的助产士操作。

Result: 系统在识别非专家盲扫图像中的标准胎儿平面方面表现良好，现场评估显示良好的可用性和低认知负荷。

Conclusion: 该系统有潜力在服务不足地区扩大产前影像服务的覆盖范围。
...
TL;DR: 本文提出了一种基于大型语言模型（Google Gemini Pro）的自动化金融摘要生成框架，通过数据提取、提示工程和LLM分析，能够高效处理海量非结构化数据并生成可操作的洞察报告。


<details>
  <summary>Details</summary>
Motivation: 信息爆炸式增长给研究人员和专业人士带来了巨大挑战，传统分析方法难以处理海量非结构化数据，需要一种高效的方法来帮助研究人员节省时间并掌握最新趋势。

Method: 结合OpenAlex数据提取、策略性提示工程和LLM驱动分析，采用从数据获取到JSON构建，再到与Gemini交互并自动生成PDF报告的完整流程。

Result: 成功实现了自动化生成全面金融摘要的示例，能够概括关键发现、识别新兴趋势，并以易于消化的格式提供可操作的洞察。

Conclusion: 该框架有效解决了传统分析方法的局限性，证明了LLMs在信息处理和知识提取方面的强大能力，为研究人员提供了高效的信息获取工具。
