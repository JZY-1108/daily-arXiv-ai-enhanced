...
Motivation: 现有代码生成方法（如监督微调和基于规则的后处理）依赖人工标注或脆弱的启发式方法，难以确保代码质量（如安全性和可维护性），限制了其实际应用。

Method: REAL结合程序分析（检测安全或可维护性缺陷）和单元测试（确保功能正确性），通过强化学习框架激励模型生成高质量代码。

Result: 实验表明，REAL在功能和代码质量的综合评估中优于现有方法。

Conclusion: REAL填补了快速原型设计和生产级代码之间的差距，使大型语言模型能够兼顾速度和代码质量。
...
Result: 发现检索器定位答案段落的能力是性能瓶颈，并强调Weight Quantization等技术在碳报告中的优势。

Conclusion: 提出透明碳报告的重要性，并展示了RAG方法在气候信息披露问答中的潜力。
...
Method: 采用课程学习策略，包括前向课程（从单令牌预测逐步过渡到多令牌预测）和反向课程（相反顺序）。

Result: 前向课程提升了小语言模型的下游任务性能和生成质量，同时保留了自推测解码的优势；反向课程虽性能更强，但无自推测解码优势。

Conclusion: 前向课程更适合小语言模型的多令牌预测训练，平衡了性能和效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有语音基础模型（如Whisper和SeamlessM4T）的封闭性导致可复现性和公平评估困难，而其他领域已通过开放模型取得进展。

Method: FAMA基于15万+小时的开放语音数据训练，并引入16k小时的清洗和伪标注语音数据集。

Result: FAMA性能与现有模型相当，速度提升高达8倍。

Conclusion: FAMA及其所有资源（代码、数据集、模型）均以开放许可发布，推动语音技术的开放研究。
...
<details>
  <summary>Details</summary>
Motivation: 句子重音在语音中传达隐含意图，但现有SLMs在评估和开发中忽视了其重要性。

Method: 引入StressTest基准和Stress17k合成数据集，通过数据生成和微调优化模型。

Result: 微调后的StresSLM在重音推理和检测任务中显著优于现有模型。

Conclusion: 合成数据和微调策略有效提升了SLMs对句子重音的理解能力。
...
Method: 使用PERSUADE语料库，结合两种反馈注释：拼写与语法错误标记和论证成分标记。采用生成式语言模型和基于编码器的标记分类器生成注释。

Result: 通过整合注释，基于编码器的大型语言模型在评分性能上有所提升。

Conclusion: 反馈驱动的注释能有效提升自动作文评分的准确性，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 研究口语和书面语在句法结构上的差异，以揭示实时互动和书面表达的不同需求对句法组织的影响。

Method: 采用自下而上的归纳方法，从英语和斯洛文尼亚语的Universal Dependencies树库中提取去词汇化的依赖子树，分析其大小、多样性和分布。

Result: 口语语料库的句法结构更少且多样性较低，与书面语的句法结构重叠有限，表现出模态特定的偏好。
...
<details>
  <summary>Details</summary>
Motivation: 现有聊天机器人评估数据集静态、过时且缺乏多语言覆盖，无法捕捉细微的语言和文化差异，阻碍了其进一步发展。

Method: 利用多代理框架和先进LLMs生成多语言对话，通过GPT-4.1进行多维分析，并基于大规模评估结果优化多语言基准。

Result: 发现当前LLMs在检测细微问题（如共情和推理）方面表现不佳，并揭示了显著的跨语言性能差异。

Conclusion: MEDAL框架为对话评估提供了更全面和多样化的基准，但LLMs在评估开放域对话时仍需改进。
...
<details>
  <summary>Details</summary>
Motivation: 随着科学文献的指数增长，利用LLMs自动化生成SR的需求增加，但其在证据评估和多文档推理方面的能力尚不明确。

Method: 研究提出了MedEvidence基准，评估了24种LLMs（包括推理和非推理模型、医学专用模型及不同规模的模型）在匹配专家SR结论方面的表现。

Result: 研究发现推理能力不一定提升表现，模型规模与性能无直接关联，知识微调反而降低准确性，且模型普遍缺乏科学怀疑态度。

Conclusion: LLMs目前尚无法可靠匹配专家SR的结论，仍需进一步研究。研究团队公开了代码和基准以促进相关研究。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中新型关系与已知关系混合存在，传统OpenRE方法无法处理这种分布。

Method: MixORE采用关系分类和聚类的两阶段框架，联合学习已知和新型关系。

Result: 在三个基准数据集上，MixORE在已知关系分类和新型关系聚类中表现优于基线。
...
<details>
  <summary>Details</summary>
Motivation: 探索LLM代理在非直接交互场景下的应用潜力，特别是在监听人类对话中提供帮助的可能性。

Method: 使用大型多模态音频-语言模型作为偷听代理，辅助《龙与地下城》游戏中的地下城主，并通过人类评估验证其有效性。

Result: 研究发现某些大型音频-语言模型具备通过隐式音频线索完成偷听代理任务的涌现能力。

Conclusion: 偷听代理范式具有研究价值，作者开源了相关代码以支持进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有自然语言解释（NLEs）未能忠实反映模型推理过程，需改进解释的忠实性。

Method: 采用自我批评和迭代优化框架（SR-NLE），结合自然语言反馈和基于特征归因的新反馈机制。

Result: 实验表明SR-NLE显著降低不忠实率，最佳方法平均不忠实率降至36.02%，比基线降低18.79%。

Conclusion: 大语言模型可通过反馈引导自我优化解释，无需额外训练或微调。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLM生成评估基准的能力，以降低成本并提高效率，同时探讨其局限性。

Method: 通过两个案例研究（CondaQA和DROP数据集），比较LLM生成的基准与人工创建的基准在有效性和挑战性上的差异。

Result: LLM生成的基准在有效性上接近人工基准，但挑战性较低。

Conclusion: 需谨慎评估LLM生成基准的适用性，避免因挑战性不足而影响评估效果。
...
Method: 提出贝叶斯注意力机制（BAM），将位置编码建模为概率先验，并引入广义高斯位置先验。

Result: BAM在500倍训练上下文长度下实现了准确的信息检索，优于现有方法，同时保持较低的困惑度和参数增量。
...
<details>
  <summary>Details</summary>
Motivation: 现有NLI数据集中标注者的推理差异被忽视，尤其是标签内差异。作者希望通过LITEX分类法系统化理解这一问题，并改进模型生成解释的能力。

Method: 提出LITEX分类法，用于标注e-SNLI数据集中的自由文本解释，验证分类法的可靠性，并分析其与NLI标签、高亮和解释的关联。进一步评估分类法在解释生成中的应用。

Result: LITEX分类法能有效捕捉标签内差异，且基于LITEX生成的解释在语言学上更接近人类解释，优于仅依赖标签或高亮的生成方法。

Conclusion: LITEX不仅揭示了NLI中的标签内差异，还为模型生成更接近人类推理的解释提供了有效工具，优于现有方法。
...
Method: 方法包括在XReasoning基准上全面评估两种主流LRMs家族，并尝试通过提示干预和少量针对性训练改善多语言推理能力。

Result: 结果显示，即使最先进的模型也常回归英语或产生碎片化推理。干预措施提高了可读性但降低了准确性，而针对性训练部分缓解了这一问题。

Conclusion: 结论指出当前LRMs的多语言推理能力有限，并提出了未来改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLM偏见研究多集中于肖像图像和性别-职业关联，忽视了更广泛的社会刻板印象及其潜在危害。

Method: 通过30M+图像的VQA框架，结合社会心理学理论，分析VLMs在情境化设置中对身份的解读。

Result: 研究发现VLMs存在微妙、多面且令人惊讶的刻板模式，揭示了模型如何从输入中构建社会意义。

Conclusion: VIGNETTE为评估和解决VLMs中的偏见提供了新视角，强调了更全面的社会刻板印象研究的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 研究归因理论在LLMs中的应用，揭示其如何基于人口统计学因素归因事件结果，对公平性有重要影响。

Method: 提出一个认知基础的偏见评估框架，分析模型推理差异如何导致对特定人口群体的偏见。

Result: 揭示了LLMs在归因事件结果时可能存在的偏见，尤其是基于人口统计学因素的差异。

Conclusion: 研究强调了认知基础方法在评估LLMs偏见中的重要性，为未来公平性研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有医疗问答任务评估依赖昂贵的人工标注且未能全面反映临床推理或工作流，尤其在急诊室这一高风险场景。

Method: ER-Reason整合了25,174份去标识化临床笔记，设计了涵盖ER工作流各阶段的任务，并收集了72份医师撰写的推理过程。

Result: 评估显示LLM生成的临床推理与医师撰写的内容存在差距。

Conclusion: 未来研究需缩小LLM与医师临床推理的差距。
...
<details>
  <summary>Details</summary>
Motivation: 传统语言模型在处理长期依赖时存在上下文丢失和语义漂移问题，需改进其对语义信息的保留和检索能力。

Method: 模型集成了显式记忆单元、门控写入机制和注意力读取模块，并引入遗忘函数动态更新记忆内容。通过联合训练目标优化记忆操作。

Result: 模型在文本生成一致性、多轮问答稳定性和跨上下文推理准确性方面表现优异，尤其在长文本任务和复杂问答场景中展现了强语义保留和上下文连贯性。

Conclusion: 实验证实了记忆机制在语言理解中的关键作用，证明了所提方法在架构设计和性能上的可行性与有效性。
...
Method: 提出OSRM方法，在微调前约束LoRA子空间，确保任务更新不影响其他任务输出。

Result: 在八个数据集上的实验表明，OSRM提升了合并性能并保持了单任务准确性，且对超参数更鲁棒。

Conclusion: OSRM为LoRA模型合并提供了一种即插即用的解决方案，强调了数据-参数交互的重要性。
...
Method: 采用DistilBERT架构，结合数据增强和超参数调优，进行模型优化。

Result: 模型在验证集上F1分数为0.6536，推理时间为0.1208秒/问题，优于规则基线和完整BERT模型。

Conclusion: 该模型在CPU推理场景下表现出色，适合实时应用，并提供了数据增强和超参数优化的实用见解。
...
Method: 采用规则强化学习框架（R1-style），设计结构化奖励函数评估输出格式和动作正确性。

Result: 在WorkArena基准测试中，WorkForceAgent-R1比SFT基线提升10.26-16.59%，接近GPT-4o性能。

Conclusion: WorkForceAgent-R1通过强化学习显著提升网页导航任务的推理和鲁棒性，适用于企业环境。
...
<details>
  <summary>Details</summary>
Motivation: 预训练多模态表示（如CLIP）虽然表现出色，但存在组合性漏洞，导致反直觉判断。研究旨在揭示并改善这些漏洞。

Method: 引入MAC基准，利用LLMs生成欺骗性文本样本，提出自训练方法，结合拒绝采样微调和多样性促进过滤。

Result: 使用较小语言模型（如Llama-3.1-8B），方法在多模态表示（图像、视频、音频）中表现出更高的攻击成功率和样本多样性。
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在多语言和跨语言环境下的记忆能力，尤其是对非英语语言或翻译内容的记忆和回忆能力。

Method: 使用OWL数据集（包含10种语言的3.15万条对齐摘录），通过直接探测、名称填空和前缀探测三种任务评估模型记忆能力。

Result: LLMs能够跨语言回忆内容，GPT-4o在新翻译摘录中识别作者和标题的准确率为69%，预测掩码实体的准确率为6%。扰动（如掩码字符、打乱单词）对准确率影响较小。

Conclusion: 研究揭示了LLMs的跨语言记忆能力，并提供了不同模型之间的差异分析。
...
<details>
  <summary>Details</summary>
Motivation: 评估VLMs对否定的理解能力，因为否定是语言中的基本现象，可能完全改变句子含义，而VLMs在高风险应用中的部署需要这种能力。

Method: 利用大型语言模型从现有VQA数据集中生成否定版本的问题，构建包含7,379个二选一问题的NegVQA基准。

Result: 评估20个最先进的VLMs，发现它们在否定问题上表现显著下降，并观察到U型扩展趋势，即模型大小增加初期性能下降随后改善。

Conclusion: NegVQA揭示了VLMs在否定理解上的关键缺陷，为未来VLM发展提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型（LLM）在零样本摘要中难以建模文档结构和识别长文本关键信息的问题。

Method: 提出StrucSum框架，包含三种策略：邻居感知提示（NAP）、中心性感知提示（CAP）和中心性引导掩码（CGM），通过结构信号增强提示。

Result: 在ArXiv、PubMed和Multi-News数据集上，StrucSum显著优于无监督基线和普通提示方法，特别是在ArXiv上FactCC和SummaC分别提升19.2和9.7分。

Conclusion: 结构感知提示是一种简单有效的零样本抽取摘要方法，无需额外训练或任务特定调整。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索LLM在检测和分析争议性话题（如堕胎）中预定义论点的能力，填补其在特定主题论点挖掘中的研究空白。

Method: 方法包括对四种LLM在三个论点挖掘任务上的定量评估，使用了超过2000条涉及六个争议话题的评论数据集。

Result: 结果显示，大型和经过微调的LLM在任务中表现优异，但存在环境成本高、对长文本和情感化语言处理不足的问题。

Conclusion: 结论指出LLM在自动化论点分析中具有潜力，但仍需解决其在复杂语言环境下的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 探索LLM在HSE领域特定知识和结构化法律推理中的潜力，填补现有研究的空白。

Method: 构建HSE-Bench数据集，采用IRAC推理流程评估LLM，并提出RoE提示技术以模拟专家推理。

Result: LLM表现良好但依赖语义匹配，缺乏系统性法律推理能力。RoE技术显著提升推理准确性。

Conclusion: 研究揭示了LLM在HSE合规性评估中的推理缺陷，RoE技术为改进提供了方向，鼓励进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在说服任务中缺乏对手心智建模能力，导致多样性和对手意识不足。

Method: 引入两个心智理论模块，通过预测对手立场和强化学习生成更有效的论点。

Result: ToMAP在3B参数规模下，性能优于GPT-4o等更大模型，相对提升39.4%。

Conclusion: ToMAP展示了对手意识和多样性的提升，适用于长对话和逻辑性强的说服任务。
...
<details>
  <summary>Details</summary>
Motivation: 探索EHR数据中的缩放规律，以填补其在自然语言领域外的研究空白，并为临床预测任务提供资源高效的训练策略。

Method: 使用MIMIC-IV数据库中的患者时间线数据，训练不同规模和计算预算的Transformer架构，分析缩放模式。

Result: 发现EHR模型具有抛物线IsoFLOPs曲线和计算、模型参数、数据量与临床效用之间的幂律关系。

Conclusion: EHR模型的缩放行为与LLMs相似，为开发强大的EHR基础模型奠定了基础，有望推动个性化医疗的发展。
...
Method: VeGraph分三阶段：图表示（将声明分解为结构化三元组）、实体消歧（与知识库交互消歧）、验证（完成事实核查）。

Result: 实验显示VeGraph在Meta-Llama-3-70B上优于基线模型，验证效果显著。
...
Conclusion: DyePack是一种高效且可靠的检测方法，适用于多种任务，能有效防止误报。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在工业应用中需平衡灵活对话能力与服务约束，但二者存在冲突，亟需解决方案。

Method: 提出一种方法并设计策略，通过电商对话机器人的案例研究展示实现流程与优化。

Result: 研究为学术与工业应用搭建桥梁，提出了可扩展、可控且可靠的AI驱动代理框架。

Conclusion: 论文成功解决了LLMs在工业应用中的挑战，为实际部署提供了可行方案。
...
Method: 通过参考模型输出与样本响应的差异筛选可疑样本，再通过TF-IDF聚类根据类内距离识别真实中毒样本。

Result: 在两个机器翻译数据集和一个QA数据集上的实验表明，RFTC在检测后门和模型性能上优于基线方法。

Conclusion: RFTC方法有效解决了隐蔽后门样本检测问题，参考过滤机制也被证明有效。
...
<details>
  <summary>Details</summary>
Motivation: 现有KE评估通常忽略前置上下文对知识编辑的影响，导致实际应用中编辑效果不佳。

Method: 开发CHED基准评估KE方法的上下文鲁棒性，并提出CoRE方法以减少隐藏状态的上下文敏感方差。

Result: CHED显示现有KE方法在前置上下文下表现不佳，而CoRE显著提高了编辑成功率并保持模型能力。

Conclusion: CoRE方法有效提升了KE的上下文鲁棒性，为实际应用提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究假设文本本身在图像-标题数据集中已包含足够信号来准确估计可想象性和具体性，无需依赖多模态模型或并行视觉语义空间。

Method: 提出NSM（邻域稳定性度量），量化语义嵌入空间中单词邻域的峰值锐度，作为无监督、分布无关的度量。

Result: NSM与真实评分的相关性优于现有无监督方法，且能有效分类可想象性和具体性。

Conclusion: NSM是一种高效的无监督方法，可用于估计文本的心理语言学特性，代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 评估通用临床NLP工具在儿科胸片报告标注任务中的适用性，填补独立评估的空白。

Method: 分析95,008份儿科胸片报告，比较四种NLP系统和两种专用工具在实体提取和断言检测上的表现，使用Fleiss Kappa和准确率评估。

Result: SP表现最佳（76%准确率），AWS最差（50%），专用工具准确率为56%。实体提取数量差异显著。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索人类与AI持续互动如何塑造MFE，以及这种语言现象对沟通效率和语言丰富性的影响。

Method: 研究方法包括基于双语（韩语/英语）语音和文本产品测试的定性观察，以及使用自然语言声明提示（NLD-P）进行反思性起草。

Result: 研究识别了五种常见特征（冗余清晰性、指令性语法、受控词汇、扁平化韵律和单一意图结构），这些特征提高了执行准确性但压缩了表达范围。

Conclusion: 结论强调了MFE发展中沟通效率与语言丰富性之间的张力，并提出了对话界面设计和多语言用户教学方面的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体评论的自由性和多样性给NLP任务（如聚类、摘要和意见分析）带来挑战，需要更细粒度的处理。

Method: 使用多语言大语言模型进行监督微调，生成评论方面术语（CAT-G），并通过DPO优化模型预测。

Result: 方法在两项NLP任务中提升了社交媒体评论的理解，并贡献了首个多语言CAT-G测试集。

Conclusion: CAT-G方法有效解决了社交媒体评论的多样性问题，多语言测试集为跨语言性能分析提供了基础。
...
Method: 1. 设计基于任务分解的多阶段集成学习流程；2. 提出新的span级句子相似度算法优化ICL演示检索；3. 引入自验证机制减少集成噪声。

Result: EL4NER在多个NER数据集上表现优于大多数闭源大参数LLM方法，部分数据集达到SOTA性能。

Conclusion: EL4NER展示了在小参数LLM上实现高效ICL的可行性，为NER任务提供了更经济的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有路由方法在RAG场景中表现不佳，因为它们依赖静态知识表示，而检索文档会动态影响LLM的回答能力。

Method: 提出RAGRouter，结合文档嵌入和RAG能力嵌入，通过对比学习捕捉知识表示变化，实现智能路由。

Result: RAGRouter在多样化知识密集型任务中平均优于最佳单LLM 3.61%，优于现有路由方法3.29%-9.33%。

Conclusion: RAGRouter通过动态路由和高效机制，显著提升了RAG场景下的性能与效率权衡。
...
<details>
  <summary>Details</summary>
Motivation: 探索小模型是否具备通过自我反思改进代码生成的能力，填补研究空白。

Method: 提出CoCoS方法，采用在线强化学习目标，设计累积奖励函数和细粒度奖励，优化多轮修正。

Result: 在1B规模模型上，CoCoS在MBPP和HumanEval上分别提升35.8%和27.7%。

Conclusion: CoCoS有效提升小模型的多轮代码修正能力，验证了强化学习在此类任务中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 随着社交媒体中视觉与文本内容的融合，需要评估大模型的多模态能力以提升用户体验和平台智能。

Method: 设计了包含8种多模态任务的基准测试，涵盖4,001个多模态问答对，并评估了25种先进的多模态大模型。

Result: 研究发现多模态社交语境理解仍存在挑战。

Conclusion: 希望SNS-Bench-VL能推动未来研究，发展更强大、上下文感知且符合人类需求的多模态智能。
...
<details>
  <summary>Details</summary>
Motivation: 文档级文本生成任务比句子级任务更具挑战性，因为需要理解更长的上下文。MBR解码在句子级任务中表现良好，但在文档级任务中效果有限，因为现有效用函数主要针对句子设计。

Method: 提出MBR-OT方法，利用Wasserstein距离结合句子级效用函数计算文档的效用。
...
<details>
  <summary>Details</summary>
Motivation: 直接使用LLM生成的关系抽取训练样本结构相似度高，表达方式单一，影响了任务性能。

Method: 1. 通过ICL提示直接生成多样化样本；2. 使用DPO微调LLM以生成多样性样本。

Result: 实验表明两种方法均能提升生成样本质量，且用这些样本训练的非LLM模型性能优于直接使用LLM。

Conclusion: 提升LLM生成样本的多样性对关系抽取任务有显著帮助，且非LLM模型可能更适合此类任务。
...
Method: 使用GPT-4o作为LLM对齐代理，计算对齐分数并构建基于均值和方差的数据地图。

Result: 实验表明，仅使用33%高质量数据（高均值、低方差区域）即可达到或超越全数据集性能。

Conclusion: Alignment Data Map可显著提升数据收集效率，并有效诊断现有偏好数据集中的低效或错误样本。
...
<details>
  <summary>Details</summary>
Motivation: 医学领域的多模态推理模型应用尚未充分探索，临床决策需要跨多源证据的迭代推理。

Method: 分两阶段：1）用2000个文本样本微调模型以激发推理行为；2）用1500个多模态医学案例增强推理能力。

Result: MedE²显著提升医学多模态模型的推理性能，在多个基准测试中优于基线。

Conclusion: MedE²具有稳健性和实用性，适用于更大模型和推理扩展。
...
Conclusion: ContextQFormer和TMDialog为多轮多模态对话研究提供了有效工具。
...
TL;DR: 论文提出了一种简单有效的方法，通过获取上下文敏感单元（CSUs）并应用语义聚焦，增强大语言模型（LLMs）的机器翻译能力，解决了CSUs导致的翻译问题。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在机器翻译中因上下文敏感单元（如多义词）导致的翻译不准确或失败问题。

Method: 动态分析并识别翻译挑战，以结构化方式将CSUs融入LLMs，避免信息扁平化导致的误译。

Result: 在基准数据集上，该方法表现优于多个开源基线模型，适用于多种语言对。
...
TL;DR: 本文提出了一种新的双语词典归纳任务，利用通用领域和目标领域的单语语料库提取领域特定的双语词典，并通过预训练模型改进词嵌入。


<details>
  <summary>Details</summary>
Motivation: 传统双语词典归纳方法在专业领域表现不佳，主要由于专业领域数据量小、词频低，且静态词嵌入无法充分捕捉上下文影响。

Method: 结合预训练模型改进词嵌入，并首次在跨领域双语词典归纳任务中引入Code Switch策略，以匹配不同上下文。

Result: 实验结果显示，该方法在三个特定领域上平均提升了0.78分，优于传统方法。

Conclusion: 该方法在专业领域双语词典归纳中表现更优，但仍需进一步验证其适用性。
...
TL;DR: Retell是一种针对文学文本的主题建模方法，通过生成语言模型将叙事内容转化为高层概念，再结合LDA提升主题建模效果。


<details>
  <summary>Details</summary>
Motivation: 传统方法（如LDA）难以处理文学文本，因其注重感官细节而非抽象描述。Retell旨在解决这一问题。

Method: 利用生成语言模型将叙事内容转化为高层概念，再对转化结果运行LDA，以获取更精确的主题。

Result: 在高中英语教材的案例研究中，Retell生成的主题比单独使用LDA或直接询问语言模型更精确。

Conclusion: Retell为文学文本的主题建模提供了一种简单有效的方法，适用于文化分析等领域。
...
TL;DR: ZIPA是一系列高效的语音模型，在多语言音素识别任务中表现优异，但仍有社会语音多样性建模的挑战。
...
Method: 利用大规模多语言语音数据集IPAPack++，结合高效的Zipformer架构（ZIPA-T和ZIPA-CR），并通过噪声学生训练进一步扩展。

Result: ZIPA在音素识别任务中优于现有系统，且参数更少；但社会语音多样性建模仍存在局限性。

Conclusion: ZIPA在多语言音素识别中表现优异，但需进一步研究以解决社会语音多样性问题。
...
TL;DR: 论文提出了一种名为Map&Make的方法，通过将文本分解为命题原子语句，提取潜在模式并生成表格，显著改进了文本到表格生成的性能。


<details>
  <summary>Details</summary>
Motivation: 当前文本到表格生成方法在提取复杂信息和推断数据方面存在不足，需要一种更有效的方法。

Method: Map&Make方法将文本分解为命题原子语句，提取潜在模式并填充表格，同时纠正幻觉错误。

Result: 在Rotowire和Livesum数据集上表现优异，显著提升了生成表格的准确性和可解释性。

Conclusion: Map&Make方法在结构化摘要任务中具有实用性和优越性能。
...
TL;DR: Infinite-Instruct是一种自动化框架，用于合成高质量的问答对，旨在提升大语言模型（LLMs）的代码生成能力。通过反向构建和反馈构建等方法，增强问题的逻辑性和代码质量，实验显示性能显著提升。
...
Method: 采用反向构建将代码片段转化为编程问题，通过反馈构建将关键词结构化以增强逻辑性，最后通过跨语言静态代码分析过滤无效样本。

Result: 在主流代码生成基准测试中，7B和32B参数模型的性能分别提升21.70%和36.95%，且使用数据量仅为十分之一。
...
TL;DR: 该论文研究了利用语言模型可解释性和不确定性量化来高效识别翻译错误的方法，替代了传统昂贵的大模型提示或人工标注数据训练。


<details>
  <summary>Details</summary>
Motivation: 现代词级质量评估（WQE）技术成本高昂，需要大型语言模型或大量人工标注数据，因此探索更高效的替代方法。

Method: 通过语言模型的可解释性和不确定性量化技术，从翻译模型的内部机制中识别翻译错误。

Result: 在12种翻译方向的14个指标评估中，发现无监督指标的潜力、监督方法在标签不确定性下的不足，以及单标注者评估的脆弱性。

Conclusion: 无监督指标具有潜力，监督方法受标签不确定性影响，单标注者评估不可靠。
...
TL;DR: 论文提出了一种多智能体跨任务经验学习框架（MAEL），通过图结构协作网络和显式经验积累，提升LLM驱动智能体在复杂任务中的协作效率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常孤立处理任务，导致计算冗余和跨任务泛化能力有限。MAEL旨在通过跨任务学习和经验积累解决这一问题。

Method: 基于图结构多智能体协作网络，量化任务解决流程中的步骤质量，存储奖励、输入和输出到个体经验池，推理时检索高奖励经验作为少样本示例。
...
TL;DR: 本文提出了一种自主经验转移框架，使大语言模型（LLMs）能够从现有任务中自主转移经验到新任务，减少人工和时间成本，并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖大量人工或时间收集任务解决经验，难以应对LLMs任务类型的多样性，因此需要一种更高效的方法。

Method: 设计了一个自主经验转移框架，模拟人类认知智能，将经验从源任务转移到目标任务。

Result: 在13个数据集上的实验表明，该框架显著提升了LLMs的性能。

Conclusion: 该框架为LLMs的泛化提供了新路径，并详细分析了框架中各模块的作用。
...
TL;DR: MMBoundary提出了一种新框架，通过校准推理步骤的置信度来提升多模态大语言模型的知识边界意识，显著减少了校准错误并提高了任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在评估模型置信度时忽略了推理步骤的置信度，导致幻觉问题累积。

Method: 结合文本和跨模态自奖励信号估计推理步骤置信度，并通过监督微调和强化学习校准。

Result: 在多种数据集和指标上显著优于现有方法，平均减少7.5%的校准错误，任务性能提升8.3%。

Conclusion: MMBoundary通过步骤级置信度校准有效提升了多模态推理的准确性和可靠性。
...
TL;DR: MCTSr-Zero是一种针对开放式对话的MCTS框架，通过“领域对齐”和“再生”机制，解决了心理辅导等主观任务中的对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有MCTS方法在开放式对话（如心理辅导）中因缺乏明确正确性标准而产生不匹配响应，需改进以符合主观需求。

Method: 提出MCTSr-Zero框架，引入“领域对齐”调整搜索目标，并采用“再生”和“元提示适应”机制扩展探索范围。

Result: 实验证明MCTSr-Zero生成的对话数据能微调LLM（PsyLLM），在PsyEval基准上达到最优性能。

Conclusion: MCTSr-Zero有效解决了LLM在复杂心理标准下的对齐问题，为以人为本的领域提供了高质量对话数据。
...
TL;DR: ChartMind是一个新的复杂图表问答（CQA）基准，支持多语言和开放域输出，提出了一种上下文感知的框架ChartLLM，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有CQA评估过于依赖固定输出格式和客观指标，忽略了实际图表分析的复杂需求。

Method: 提出ChartMind基准和ChartLLM框架，专注于提取关键上下文元素并减少噪声。

Result: ChartLLM在多个基准测试中显著优于现有CQA范式。

Conclusion: 灵活的图表理解对实际CQA至关重要，为未来研究提供了新方向。
...
TL;DR: 本文提出了一种基于多维度的方法管理框架，通过语言模式识别和树结构相似性度量，实现了高效的方法查询和分类。


<details>
  <summary>Details</summary>
Motivation: 科学论文中方法的查询和管理耗时且缺乏组织，需要一种高效的多维度管理框架。

Method: 采用自上而下的方式识别方法模式，并通过语义、话语、句法和词汇四个语言层次进行细化；提出树结构表示步骤，并基于句法相似性度量方法相似性；使用自下而上的聚类算法构建多维度方法空间。

Result: 构建了五维方法空间，查询结果表明该方法空间能确保查询结果的高相关性和快速缩小搜索范围。
...
TL;DR: 该论文全面研究了阿拉伯语机器生成文本，通过多种生成策略和模型架构，揭示了机器生成文本的语言特征，并开发了高效的检测模型。
...
Method: 研究采用多种生成策略（如仅标题生成、内容感知生成和文本优化）和模型架构（ALLaM、Jais、Llama、GPT-4），通过风格计量分析区分人工与机器生成文本。

Result: 研究发现机器生成的阿拉伯语文本具有可检测的特征，并开发了BERT检测模型，在正式语境中表现优异（F1分数高达99.9%）。
...
TL;DR: Sentinel是一种轻量级句子级压缩框架，通过注意力机制实现上下文过滤，无需训练专用压缩模型，性能接近大型模型。
...
Result: 在LongBench基准测试中，Sentinel实现5倍压缩，性能与7B规模的压缩系统相当。
...
TL;DR: 论文提出了一个新的知识编辑（KE）基准ScEdit，结合了反事实和时间编辑，并扩展了传统的事实型评估到行动型评估，发现现有KE方法在性能上有所下降。


<details>
  <summary>Details</summary>
Motivation: 当前KE任务过于简单，缺乏实际应用场景的整合，需要更全面的评估框架。

Method: 引入ScEdit基准，结合反事实和时间编辑，采用词级和文本级评估方法。

Result: 所有KE方法在现有指标上表现下降，文本级指标面临挑战。

Conclusion: ScEdit为KE任务提供了更全面的评估，揭示了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLMs生成文本长度与事实准确性之间的关系，以解决现有研究中未充分探索的问题。

Method: 提出自动双层长文本事实性评估框架，并进行控制实验验证长度偏差。

Result: 长文本响应的事实准确性较低，主要原因是知识耗尽。

Conclusion: 知识耗尽是导致事实准确性下降的主要原因，而非错误传播或长上下文。
...
TL;DR: 介绍了EmoBench-UA，首个乌克兰语情感分类数据集，并评估了多种方法，强调了乌克兰语情感分类的挑战。


<details>
  <summary>Details</summary>
Motivation: 乌克兰语情感分类缺乏公开基准，填补这一空白。

Method: 通过众包平台Toloka.ai创建高质量数据集，评估了语言学基线、合成数据和大型语言模型。

Result: 发现乌克兰语等非主流语言情感分类的挑战，需开发更多本地化模型和资源。

Conclusion: 呼吁进一步开发乌克兰语专用模型和训练资源。
...
TL;DR: 论文提出了一种结合高效分类算法和降维技术的方法，以减少幻觉检测框架的训练数据需求，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型和检索增强生成系统中幻觉检测的数据标注瓶颈问题。

Method: 结合高效分类算法与降维技术，减少Lookback Lens和探测方法对训练数据的需求。

Result: 在标准化问答基准测试中，仅需250个训练样本即可达到与强基线相当的性能。
...
TL;DR: 论文提出EC-GCD任务，解决长文本和类别不平衡问题，并提出PaMA框架，利用LLM优化聚类与分类对齐，显著提升性能。
...
Method: 提出PaMA框架，利用LLM提取事件模式并优化对齐，采用排名-过滤-挖掘流程平衡类别表示。

Result: 在EC-GCD基准测试中，PaMA性能提升高达12.58% H-score，并在基础GCD数据集上表现良好。

Conclusion: PaMA有效解决了EC-GCD的挑战，显著优于现有方法。
...
TL;DR: 研究通过分类任务和KWOCCE损失函数提升AES评分可靠性，实现高CEFR一致性。


<details>
  <summary>Details</summary>
Motivation: 解决AES评分可靠性问题，确保分数仅在满足高可靠性标准时发布。

Method: 将置信度估计建模为分类任务，利用分数分箱和KWOCCE损失函数。

Result: 最佳模型F1分数0.97，47%分数100%符合CEFR，99%至少95%符合。

Conclusion: KWOCCE方法显著提升AES评分的可靠性和CEFR一致性。
...
TL;DR: 论文分析了直接对齐方法（如DPO）在优化大型语言模型时的局限性，提出了PRO方法以解决似然不确定性问题。


<details>
  <summary>Details</summary>
Motivation: 直接对齐方法虽然能调整模型以匹配偏好，但会降低响应似然，导致输出偏离预期模式。

Method: 通过重新表述DPO损失函数，发现其隐含简化了正则化项，提出PRO方法以完整正则化项解决不确定性问题。

Result: PRO方法在成对、二元和标量反馈场景中优于现有方法。
...
TL;DR: 本文提出了一种关于LLM能力的中间立场，认为其通过上下文指导的外推（context-directed extrapolation）从训练数据中提取信息，既非随机模仿也非具有不可预测的高级推理能力。


<details>
  <summary>Details</summary>
Motivation: 批判性地审视LLM能力的极端观点，提出更现实的中间立场，以消除对LLM不可控涌现能力的担忧。

Method: 通过文献支持，提出“上下文指导的外推”机制，解释LLM如何从训练数据中提取并应用信息。

Result: LLM的能力虽超越随机模仿，但可预测、可控，且不具人类高级认知能力或无限扩展性。

Conclusion: 未来研究应聚焦于上下文指导的外推机制及其与训练数据的交互，探索不依赖LLM固有高级推理的增强技术。
...
TL;DR: 本文提出了一种名为Q-RM的令牌级奖励模型，通过将奖励建模与语言生成解耦，解决了生成语言建模与奖励建模之间的冲突，提升了复杂推理任务中LLM的性能和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有的过程奖励模型（PRMs）在令牌级粒度上存在生成语言建模与奖励建模的冲突，导致不稳定性和信用分配不准确。本文旨在解决这一问题。

Method: 通过优化判别策略（Q函数）推导出令牌级奖励模型（Q-RM），无需依赖细粒度注释即可从偏好数据中学习令牌级Q函数。

Result: Q-RM在多个基准测试中表现优于基线方法，例如在数学推理任务中显著提升了Pass@1分数，并大幅提高了训练效率。

Conclusion: Q-RM通过解耦奖励建模与语言生成，有效解决了冲突问题，显著提升了LLM在复杂任务中的性能和训练效率。
...
TL;DR: 论文提出了一种基于LLM的管道方法，利用语言基础的分段器从CoTs中提取支持与反对陈述，并提出了基于排名的HLV评估框架，优于直接生成方法。


<details>
  <summary>Details</summary>
Motivation: 研究如何利用LLMs生成的CoTs更好地理解人类标签变异，改进模型预测与人类标签分布的匹配。

Method: 提出一种LLM管道，结合语言分段器提取CoTs中的支持与反对陈述，并设计基于排名的HLV评估框架。

Result: 方法在三个数据集上优于直接生成方法和基线，排名方法更符合人类标注。

Conclusion: 该方法有效提升了模型预测与人类标签分布的对齐能力。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的越狱攻击威胁其安全性和可靠性，因此需要研究针对不同理解能力的LLMs的自适应攻击策略。

Method: 将LLMs分为Type I和Type II两类，根据其语义理解能力设计定制化的越狱策略。

Result: 实验表明，自适应策略显著提高了越狱成功率，对GPT-4o的成功率高达98.9%。

Conclusion: 提出的自适应越狱策略能有效针对不同LLMs的弱点，为AI安全提供了重要参考。
...
TL;DR: 论文研究了预训练语言模型中事实知识提取的机制，重点关注微调数据对事实性的影响，并提出推理阶段的提示方法可以弥补微调数据的不足。


<details>
  <summary>Details</summary>
Motivation: 探讨监督微调数据对大型语言模型事实性的影响机制，尤其是已知与未知知识之间的差距。

Method: 通过系统实验分析微调数据的影响，重点关注知识图谱视角下的理论证明。

Result: 研究发现推理阶段的提示方法（如少样本学习和思维链）可以显著减少事实性差距。

Conclusion: 研究表明上下文学习提示可以有效弥补微调数据的不足，并建议重新评估微调数据选择方法的有效性。
...
TL;DR: 论文探讨了大规模语音到文本（S2T）训练中的学习率（LR）预热策略，发现次指数预热优于传统方法，且高初始LR加速收敛但不提升最终性能。


<details>
  <summary>Details</summary>
Motivation: 大规模S2T训练中，传统学习率调整方法不足以应对复杂模型（如Conformer、Branchformer）的收敛问题，需研究更优的预热策略。

Method: 比较不同学习率预热策略，包括双线性预热和次指数预热，分析其对初始收敛和最终性能的影响。

Result: 次指数预热更适合大规模S2T训练；高初始LR加速收敛，但对最终性能无显著提升。

Conclusion: 大规模S2T训练应采用次指数学习率预热策略，高初始LR仅用于加速初期收敛。
...
TL;DR: 论文提出了一个双语数据集UAQFact，用于评估LLMs在处理不可回答问题时的能力，并定义了两个新任务来测试LLMs利用内外知识的能力。实验表明，即使LLMs具备相关知识，其表现仍不稳定，且外部知识的使用效果有限。


<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏事实知识支持，限制了评估LLMs在处理不可回答问题时的能力，尤其是利用事实知识的能力。

Method: 构建了基于知识图谱的双语数据集UAQFact，并定义了两个新任务来测试LLMs利用内外知识的能力。

Result: 实验结果显示，LLMs在UAQFact上表现不稳定，即使具备相关知识，也无法充分利用外部知识，可能导致错误回答。

Conclusion: UAQFact为评估LLMs在处理不可回答问题时的能力提供了新基准，但LLMs在利用知识方面仍有改进空间。
...
TL;DR: 研究评估了28个大语言模型（LLMs）在神经外科考试题目上的表现，发现部分模型能通过考试，但性能易受干扰信息影响。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在神经外科知识上的能力，并测试其对干扰信息的鲁棒性。

Method: 使用2,904个神经外科考试题目测试28个LLMs，并引入干扰框架评估模型脆弱性。

Result: 6个模型通过考试，但干扰信息导致性能显著下降（最高下降20.4%）。

Conclusion: LLMs在神经外科考试中表现良好，但需提升对干扰信息的鲁棒性以用于临床。
...
TL;DR: 论文提出了一种减少大型语言模型（RLLMs）过度思考的方法，通过量化分析自我怀疑现象，并设计了一种提示方法以减少对输入问题的过度依赖。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂任务中表现出色，但存在过度思考问题，即在不必要的推理步骤上浪费时间。现有研究多基于定性分析，本文则从自我怀疑的角度进行量化分析。

Method: 提出了一种简单的提示方法：首先让模型质疑输入问题的有效性，然后根据评估结果简洁回答。

Result: 在三个数学推理任务和四个数据集上的实验表明，该方法显著减少了答案长度，并在广泛使用的RLLMs上取得了显著改进。

Conclusion: 该方法有效减少了推理步骤和自我怀疑，为优化大型语言模型的推理效率提供了新思路。
...
TL;DR: 研究了语音语言模型（SLM）中码本大小和单元粗糙度（持续时间）对性能的影响，发现粗糙单元在某些任务中表现更好，而DPDP方法能有效生成粗糙单元。


<details>
  <summary>Details</summary>
Motivation: 探索码本大小和单元粗糙度对SLM性能的影响，填补这一未充分研究的领域。

Method: 使用持续时间惩罚动态规划（DPDP）方法调整码本大小和单元粗糙度，并在不同语言层级进行分析。

Result: 在句子重合成任务中，粗糙单元表现更好；在词汇和句法任务中，粗糙单元在低比特率下准确率更高。

Conclusion: 粗糙单元并非总是更好，但DPDP方法能简单高效地为需要粗糙单元的任务生成合适单元。
...
TL;DR: 论文提出KGQAGen框架，通过结合结构化知识、LLM引导生成和符号验证，解决现有KGQA数据集的低质量问题，并构建了KGQAGen-10k基准测试。
...
Method: 提出KGQAGen框架，结合结构化知识、LLM引导生成和符号验证，生成高质量QA实例。

Result: 构建了KGQAGen-10k基准测试，实验表明即使是SOTA模型在该测试上表现不佳，凸显了其挑战性。

Conclusion: KGQAGen为KGQA评估提供了可扩展的框架，并呼吁更严格的基准测试构建。
...
TL;DR: 论文提出三种模型架构用于企业ESG报告中的承诺验证任务，结合语言特征和多目标学习，最终模型表现优于基线。


<details>
  <summary>Details</summary>
Motivation: 解决企业ESG报告中承诺验证的复杂任务，包括识别、证据评估、清晰度评价和时间验证。

Method: 1. ESG-BERT加任务特定分类头；2. 结合语言特征的改进模型；3. 多任务注意力池化与元数据增强的联合模型。

Result: 联合模型在ML-Promise数据集上得分0.5268，优于基线0.5227。
...
TL;DR: 论文提出PCPO框架，通过双重定量指标优化LLM的数学推理能力，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法忽视内部逻辑一致性，需改进。

Method: 提出PCPO框架，结合答案正确性和概率一致性。

Result: PCPO在多种LLM和基准测试中表现优于现有方法。

Conclusion: PCPO有效提升LLM数学推理能力，代码已开源。
...
TL;DR: LLMs的翻译能力可能源于预训练数据中的两种类型，而非专门的翻译目标训练。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在没有专门翻译目标训练的情况下，为何能表现出卓越的翻译能力。

Method: 通过分析预训练数据和指令调优的作用，提出“双重性”假设。

Result: LLMs的翻译能力可能与预训练数据中的“偶然双语”和语义对齐有关。

Conclusion: 研究为重新定义深度学习时代的翻译（人机）提供了新视角。
...
TL;DR: 论文通过稀疏自编码器研究指令调优LLM中的拒绝行为，揭示了拒绝行为的潜在特征及其因果影响。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型中拒绝行为的内部机制，以增强模型安全性和对抗性攻击的理解。

Method: 使用稀疏自编码器识别拒绝行为的潜在特征，并在开源聊天模型上进行干预实验。

Result: 验证了拒绝特征对生成行为的影响，并展示了其在分类任务中对对抗样本的泛化能力。

Conclusion: 研究为拒绝行为的机制提供了细粒度分析，并提出了增强模型安全性的潜在方法。
...
TL;DR: 研究探讨了使用开放权重的大型语言模型（LLMs）识别YouTube上的阴谋论视频，发现文本模型召回率高但精度低，多模态模型表现不如纯文本模型。RoBERTa在未标注数据上表现接近更大参数量的LLMs。


<details>
  <summary>Details</summary>
Motivation: YouTube作为全球领先平台易传播有害内容，需有效识别阴谋论视频以减少负面影响。

Method: 使用标注数据集评估多种LLMs的零样本性能，并与微调RoBERTa基线对比，测试未标注数据的实际应用效果。

Result: 文本LLMs召回率高但精度低，多模态模型表现较差；RoBERTa在未标注数据上接近LLMs性能。

Conclusion: 当前LLM方法在有害内容检测中表现有限，需开发更精准、鲁棒的系统。
...
<details>
  <summary>Details</summary>
Motivation: 解决小参数语言模型在真实软件工程任务中表现不佳的问题，同时避免高成本监督微调和测试时扩展的过度采样。
...
Conclusion: EvoScale为小参数模型提供高效优化路径，减少资源消耗，性能显著提升。
...
TL;DR: 本文首次研究了表格推理任务中的推理时间扩展，提出了两种后训练策略：基于前沿模型推理轨迹的蒸馏和基于可验证奖励的强化学习（RLVR）。Table-R1-Zero模型在多种任务中表现优异，甚至超越GPT-4.1和DeepSeek-R1。


<details>
  <summary>Details</summary>
Motivation: 探索如何在推理时扩展表格推理任务的性能，以提升小规模语言模型的表现。

Method: 1. 蒸馏：利用DeepSeek-R1生成的大规模推理轨迹数据集微调模型（Table-R1-SFT）。2. RLVR：设计任务特定的可验证奖励函数，使用GRPO算法训练模型（Table-R1-Zero）。

Result: Table-R1-Zero模型在7B参数规模下，性能匹配或超越GPT-4.1和DeepSeek-R1，并展现出强大的跨领域泛化能力。

Conclusion: 指令微调、模型架构选择和跨任务泛化对表格推理能力提升至关重要，RL训练中涌现出关键推理技能。
...
TL;DR: 本文研究了固定精度Transformer的表达能力，证明其在严格未来掩码和软注意力机制下等同于一种线性时序逻辑片段，并与形式语言理论、自动机理论和代数关联。


<details>
  <summary>Details</summary>
Motivation: 探索Transformer语言模型的理论表达能力，弥补现有研究中对实际实现与理想模型之间差距的理解不足。

Method: 通过固定精度、严格未来掩码和软注意力机制，将Transformer的表达能力与线性时序逻辑片段关联，并分析其与形式语言理论的关系。

Result: 证明Transformer在特定条件下等同于仅包含过去操作符的线性时序逻辑片段，且实验表明其能完美泛化在其理论能力范围内的语言。

Conclusion: 为Transformer的表达能力提供了一个统一的理论框架，并验证了理论与实验的一致性。
...
TL;DR: AutoSchemaKG是一个无需预定义模式的自主知识图谱构建框架，利用大语言模型从文本中提取知识三元组并生成模式，构建了包含9亿节点和59亿边的ATLAS知识图谱。
...
Result: 构建了ATLAS知识图谱，在多跳QA任务中优于基线方法，模式生成与人工模式的语义对齐率达95%。
...
<details>
  <summary>Details</summary>
Motivation: NLP中的文本数据常因使用男性通用词而存在性别偏见，GeNRe旨在通过性别中性改写技术缓解这一问题。

Method: 开发了基于规则的系统（RBS）和两个微调的语言模型，并探索了基于指令的模型（如Claude 3 Opus）的性能。

Result: RBS表现优异，Claude 3 Opus结合词典的结果接近RBS。
...
TL;DR: 大型推理模型（LRMs）在复杂任务中表现优异，但其在事实寻求任务中的幻觉问题尚存争议。本文从三个角度探讨了LRMs的幻觉问题，并提出了缓解方法。


<details>
  <summary>Details</summary>
Motivation: 研究LRMs在事实寻求任务中的幻觉问题，以明确其推理能力是否能减少幻觉。

Method: 通过全面评估、行为分析和模型不确定性机制研究，探讨LRMs的幻觉问题。

Result: 发现完整后训练流程（SFT和可验证奖励RL）可减少幻觉，而蒸馏或RL训练未冷启动微调会增加幻觉。行为分析揭示了两种关键认知行为与幻觉相关。

Conclusion: 初步揭示了LRMs的幻觉机制，为未来研究提供了方向。
...
TL;DR: 论文研究了指令调优的大型语言模型（LLMs）在摘要生成中是否充分保留关键论点角色，并提出ARC框架评估其表现。


<details>
  <summary>Details</summary>
Motivation: 在法律等高风险领域，论点角色对摘要至关重要，但LLMs是否保留这些信息尚不明确。

Method: 引入Argument Representation Coverage (ARC)框架，分析三个开源LLMs在法律意见和科学文章中的摘要表现。

Result: LLMs能部分覆盖关键论点角色，但信息常被遗漏，尤其是论点分散时。还发现位置偏差和角色偏好影响论点覆盖。

Conclusion: 需开发更具论点意识的摘要策略，以提升关键信息的保留。
...
TL;DR: 提出了一种新的解码策略ActLCD，通过强化学习优化生成内容的真实性，减少LLM的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有解码方法在长文本生成中仍易产生幻觉，需改进。

Method: ActLCD通过强化学习策略动态选择对比层，优化生成过程。

Result: 在五个基准测试中表现优于现有方法，有效减少幻觉。

Conclusion: ActLCD是一种有效的解码策略，显著提升生成内容的真实性。
...
TL;DR: ToolHaystack是一个用于评估大语言模型在长期交互中使用工具能力的基准测试，发现现有模型在标准多轮对话中表现良好，但在长期鲁棒性方面存在显著不足。
...
Method: 引入ToolHaystack基准，包含多任务执行上下文和连续对话中的噪声，测试模型在长期交互中的表现。

Result: 14个先进LLMs在ToolHaystack中表现不佳，揭示了长期鲁棒性的关键缺陷。

Conclusion: 当前模型在长期工具使用中仍有显著改进空间，ToolHaystack为未来研究提供了重要基准。
...
TL;DR: LoLA是一种低秩线性注意力方法，通过稀疏缓存和三种记忆形式解决长上下文信息召回问题，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer在长序列推理中的二次复杂度问题，同时弥补线性注意力对softmax注意力近似不足的缺陷。

Method: 结合滑动窗口注意力、稀疏全局缓存和递归隐藏状态，分别存储不同类型的键值对。

Result: 在8K上下文长度任务中，准确率从0.6%提升至97.4%，缓存大小仅为Llama-3.1的1/4.6。

Conclusion: LoLA是一种轻量级高效方法，适用于长上下文任务，且可在消费级GPU上实现。
...
TL;DR: 论文展示了wav2vec2.0模型能高精度自动分类英语和日语中的爆破音存在，且适用于不同语料库。


<details>
  <summary>Details</summary>
Motivation: 当前语音研究中缺乏针对多变语音现象的自动标注工具，而预训练的自监督模型（如wav2vec2.0）在语音分类任务中表现优异。

Method: 利用wav2vec2.0模型训练自动分类爆破音存在，并在英语和日语的不同语料库中测试其鲁棒性。

Result: 自动标注结果与人工标注高度一致，且能复现爆破音实现的变异性模式。

Conclusion: 预训练语音模型有望成为语音数据自动标注的强大工具，助力语音研究规模扩展。
...
TL;DR: 研究发现，儿童导向语言（CDL）在语言模型训练中并未普遍优于维基百科数据，且现有评测方法存在不足。


<details>
  <summary>Details</summary>
Motivation: 验证CDL在不同语言、模型和评测设置中的通用性，并改进评测方法。

Method: 比较CDL与维基百科训练的模型，采用两种目标、三种语言和三个句法评测基准，并提出新评测方法FIT-CLAMS。

Result: CDL在多数情况下表现不如维基百科模型，且评测方法需控制词频影响。

Conclusion: CDL并未显著提升句法学习能力，评测时需注意词频控制。
...
TL;DR: 论文指出，基于最终答案的评估指标在数学应用题中常被用作推理能力的代理，但实际上混淆了抽象表达和算术计算两个子技能。研究发现，Llama-3和Qwen2.5的算术计算是瓶颈，而思维链（CoT）主要帮助计算而非抽象表达。


<details>
  <summary>Details</summary>
Motivation: 揭示基于最终答案的评估指标在评估大型语言模型（LLM）数学推理能力时的局限性，并探讨抽象表达和算术计算两个子技能的独立作用。

Method: 通过GSM8K和SVAMP数据集对Llama-3和Qwen2.5（1B-32B）进行解耦评估，分析思维链（CoT）的作用，并通过因果修补验证抽象表达的存在和机制。

Result: 研究发现算术计算是瓶颈，CoT主要帮助计算而非抽象表达；抽象表达和计算在单次前向传递中通过“先抽象后计算”机制结合。

Conclusion: 解耦评估能更准确评估LLM的推理能力，并为未来改进提供指导。
...
TL;DR: 论文介绍了SocialMaze，一个评估大语言模型（LLMs）社交推理能力的新基准，填补了现有评估框架的不足。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架对LLMs的社交推理能力评估过于简化，缺乏系统性，无法挑战先进模型。

Method: SocialMaze通过六项任务评估三个核心挑战：深度推理、动态交互和信息不确定性，结合自动化和人工验证确保数据质量。

Result: 评估发现：模型在动态交互和信息整合能力上差异显著；具备链式推理能力的模型表现更优；不确定性下模型推理能力显著下降。针对性微调可提升模型表现。

Conclusion: SocialMaze为LLMs社交推理能力提供了系统性评估工具，揭示了模型在复杂社交场景中的表现及改进方向。
...
TL;DR: 论文提出了一种半自动标注方法，并发布了针对九种低资源语言的多义词句子数据集，以支持跨语言迁移研究。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言中高质量评估数据集的缺乏问题，以促进跨语言迁移技术的发展。

Method: 采用半自动标注方法创建多义词句子数据集，并通过WiC格式实验评估跨语言迁移效果。

Result: 实验结果表明，针对性的数据集创建和评估对低资源语言中的多义消歧和迁移研究至关重要。
...
TL;DR: 论文提出PCBench评估框架，测试大语言模型（LLMs）对输入前提的批判能力，发现现有模型依赖显式提示，批判能力有限，且与推理能力不直接相关。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注LLMs在理想环境下的推理能力，忽视了其对错误前提的脆弱性，因此需评估其前提批判能力。

Method: 设计了PCBench，包含四种错误类型和三个难度级别，对15种代表性LLMs进行系统评估。

Result: 发现模型依赖显式提示，批判能力与难度和错误类型相关，推理能力与批判能力不一致，错误前提导致过度思考。

Conclusion: 提升LLMs对输入有效性的主动评估能力是开发可靠系统的关键，前提批判能力应作为基础能力。
...
TL;DR: DEER方法通过利用训练标签的token级统计信息，改进了上下文学习（ICL）在命名实体识别（NER）中的表现，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的ICL方法在NER任务中通常仅基于语义相似性选择演示样本，忽略了训练标签，导致性能不佳。

Method: DEER通过标签引导的token检索器改进样本选择，并提示LLM重新审视易错token进行针对性修正。

Result: 在五个NER数据集和四种LLM上的实验表明，DEER优于现有ICL方法，接近监督微调的性能。

Conclusion: DEER在可见和不可见实体上均表现优异，且在低资源环境下具有鲁棒性。
...
TL;DR: 论文提出了一种基于学习的大语言模型（LLM）代理框架，通过在线强化学习（RL）优化机器学习（ML）任务，显著提升了自主ML的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理依赖手动提示工程，无法根据实验经验自适应优化，因此探索基于学习的代理范式。

Method: 提出包含三个关键组件的框架：探索增强微调、逐步RL训练和ML专用奖励模块。

Result: 7B规模的ML-Agent在仅9个ML任务上训练后，性能超过671B规模的DeepSeek-R1，并展现出持续改进和跨任务泛化能力。

Conclusion: 学习型LLM代理框架在自主ML中具有显著优势，为未来研究提供了新方向。
...
TL;DR: SITAlign是一个推理时间框架，通过最大化主要目标并满足次要标准的阈值约束，解决了对齐的多面性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将人类偏好反馈视为多目标优化问题，但忽视了人类实际决策方式（如满意策略）。

Method: 提出SITAlign框架，结合满意策略，最大化主要目标并满足次要标准的阈值约束。

Result: 在PKU-SafeRLHF数据集上，SITAlign在保持无害性阈值的同时，帮助性奖励的GPT-4胜率比现有方法高22.3%。

Conclusion: SITAlign通过满意策略有效解决了对齐问题，优于现有方法。
...
TL;DR: 论文提出了ATLAS，一种高容量的长期记忆模块，解决了现代循环神经网络在长上下文理解和序列外推任务中的不足。ATLAS通过优化当前和过去令牌的记忆，克服了长期记忆模型的在线更新限制，并在多个任务中表现优于Transformer和线性循环模型。


<details>
  <summary>Details</summary>
Motivation: Transformer在长序列任务中的二次复杂度限制了其应用，而现代循环神经网络在长上下文理解和外推任务中表现不佳。论文旨在通过改进记忆容量、更新方式和记忆管理来解决这些问题。

Method: 提出ATLAS，一种高容量长期记忆模块，优化基于当前和过去令牌的记忆更新。进一步提出DeepTransformers，作为Transformer架构的严格泛化。

Result: 实验表明，ATLAS在语言建模、常识推理、召回密集和长上下文理解任务中优于Transformer和线性循环模型，并在BABILong基准测试中实现了+80%的准确率提升。

Conclusion: ATLAS通过改进记忆模块的设计，显著提升了长上下文任务的性能，为序列建模提供了更高效的解决方案。
...
TL;DR: DeepTheorem是一个利用自然语言增强大型语言模型（LLM）数学推理的非正式定理证明框架，包含大规模数据集和强化学习策略，显著提升LLM的定理证明性能。


<details>
  <summary>Details</summary>
Motivation: 传统自动定理证明（ATP）方法依赖形式化证明系统，与LLM通过预训练获得的非正式自然语言知识不匹配，限制了其推理能力。

Method: 提出DeepTheorem框架，包括121K高质量非正式定理和证明的数据集，以及专为非正式定理证明设计的强化学习策略（RL-Zero）。
...
<details>
  <summary>Details</summary>
Motivation: Rebus谜题对VLMs提出了独特挑战，需要多模态抽象、符号推理和文化语言双关的理解，而传统任务无法涵盖这些能力。

Method: 构建了一个手工生成和标注的多样化Rebus谜题基准，评估不同VLMs的表现。

Result: VLMs在解码简单视觉线索时表现出色，但在抽象推理、横向思维和视觉隐喻理解方面表现较差。

Conclusion: 当前VLMs在解决需要复杂推理和文化语言理解的Rebus谜题时仍有显著局限性。
...
TL;DR: 论文提出了一种新任务——聚合问答（Aggregative Question Answering），旨在通过分析大规模用户与聊天机器人的对话数据，回答聚合性问题。
...
Method: 提出了聚合问答任务，并构建了一个基准数据集WildChat-AQA，包含6,027个聚合问题，源自182,330条真实聊天记录。

Result: 实验表明，现有方法在有效推理或计算成本方面表现不佳，凸显了需要新方法从大规模对话数据中提取集体洞察。

Conclusion: 论文强调了从大规模对话数据中提取集体洞察的重要性，并提出了新的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前头部检测与跟踪方法计算成本高，导致延迟和资源占用问题，需优化性能与效率的平衡。
...
Method: 设计了一个自适应DCT预处理模块，结合ViT-B16和ResNet50提取频率和空间特征，并通过交叉级融合策略整合特征，最后使用贝叶斯线性分类器进行预测。

Result: 在自建的50类野生动物数据集上，该方法优于传统CNN和固定频带DCT方法，在样本稀缺情况下达到最优准确率。

Conclusion: 该方法通过自适应频率选择和多模态特征融合，显著提升了稀有动物图像分类的性能。
...
TL;DR: HiDream-I1是一个17B参数的图像生成基础模型，通过稀疏扩散变换器（DiT）和动态MoE架构实现高质量快速生成，并提供三种变体。HiDream-E1支持指令编辑，HiDream-A1整合生成与编辑功能。
...
Result: 实现了秒级高质量图像生成，并扩展了指令编辑和交互式图像创作功能。

Conclusion: HiDream系列模型为多模态AIGC研究提供了高效、灵活的工具，并开源了代码和模型。
...
TL;DR: MIAS-SAM是一种用于医学图像异常区域分割的新方法，利用基于补丁的记忆库和SAM编码器提取特征，无需阈值即可生成异常图并实现精确分割。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要手动设置阈值来从异常图中获取分割结果，MIAS-SAM旨在消除这一需求，提高分割的准确性和自动化程度。

Method: 使用SAM编码器从正常数据中提取特征并存储到记忆库中，推理时通过比较特征生成异常图，并通过计算重心提示SAM解码器完成分割。

Result: 在三种不同模态的医学图像数据集（脑MRI、肝CT和视网膜OCT）上实验，显示其具有较高的异常分割能力（DICE评分）。
...
TL;DR: Rhet2Pix是一个解决修辞语言生成图像问题的框架，通过多步策略优化和双层MDP扩散模块，显著优于现有SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型难以理解修辞语言的隐含意义，导致生成图像偏离语义意图。
...
TL;DR: 现代视觉语言模型（VLM）在文化能力评估中表现不佳，需系统性框架分析图像中的文化维度。


<details>
  <summary>Details</summary>
Motivation: 由于VLM应用的多样性，理解其如何编码文化细微差别变得重要，但目前缺乏系统性方法。

Method: 借鉴视觉文化研究（文化研究、符号学、视觉研究）的方法论，提出五个文化维度框架。

Result: 提出五个文化维度框架，为VLM的文化能力分析提供更全面的方法。

Conclusion: 结合视觉文化研究方法论，系统性分析VLM的文化能力是必要的。
...
TL;DR: 论文提出了一种独立于轨迹的模型基MPI重建算法，并开发了零样本PnP算法，用于解决去卷积问题，展示了在不同扫描场景下的强大重建能力。


<details>
  <summary>Details</summary>
Motivation: 传统MPI重建依赖耗时校准或模型模拟，且受限于特定扫描轨迹。本文旨在开发一种灵活、通用的模型基重建方法。

Method: 采用零样本PnP算法，结合先进去噪器，无需针对MPI数据重新训练，实现了轨迹无关的模型基重建。

Result: 在公开数据集和自定义数据上验证了算法的强大重建能力，适用于不同扫描场景。
...
TL;DR: VidText是一个新的视频文本理解基准，填补了现有视频理解和OCR基准的不足，支持多语言内容，并引入分层评估框架和感知推理任务。实验表明当前模型表现不佳，仍有改进空间。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准忽视文本信息，OCR基准局限于静态图像，无法捕捉动态视觉上下文与文本的交互。

Method: 提出VidText基准，覆盖多场景和多语言，采用分层评估框架（视频级、片段级、实例级任务）和感知推理任务。

Result: 实验显示18种先进多模态模型在多数任务中表现不佳，输入分辨率、OCR能力等内部因素及辅助信息使用等外部因素影响显著。
...
<details>
  <summary>Details</summary>
Motivation: IMTS数据由于多通道信号未对齐和大量缺失值，现有方法难以捕捉可靠时间模式，视觉MAE在稀疏多通道数据处理上表现优异。

Method: VIMTS将IMTS沿时间线分割为等间隔特征补丁，利用跨通道依赖补全缺失值，结合视觉MAE进行补丁重建，并通过粗到细技术生成预测。

Result: 实验表明VIMTS性能优越，具备少样本学习能力，推动了视觉基础模型在更广泛时间序列任务中的应用。

Conclusion: VIMTS成功将视觉MAE应用于IMTS预测，解决了缺失值问题，提升了预测精度。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在区分视觉相似但对应不同指代表达的对象时表现不佳，需要一种更鲁棒的表示学习方法。

Method: 提出C-REX框架，利用监督对比学习增强判别性表示，避免图像-文本对比学习的对齐问题，并提供更多负样本。

Result: C-REX在REC任务中达到SOTA，MAE提升22%，RMSE提升10%，并在类无关计数任务中表现优异。

Conclusion: C-REX通过对比学习显著提升REC性能，并展示了其通用性，适用于其他类似任务。
...
<details>
  <summary>Details</summary>
Motivation: 尽管高斯泼溅（GS）在3D场景渲染中表现出色，但对其实现复杂风格迁移仍具挑战性。

Method: 直接在GS基元上操作，通过联合优化颜色和几何形状，并集成到现有GS流程中。

Result: 在风格保真度和一致性上表现优异，支持多模态风格迁移。

Conclusion: CLIPGaussians是一种通用且高效的解决方案，适用于多模态风格迁移。
...
<details>
  <summary>Details</summary>
Motivation: 开放世界的自我中心活动识别由于未约束性，模型需从部分观察的广阔空间中推断未见活动，挑战巨大。

Method: 结合结构化常识先验构建语义一致的搜索空间，利用视觉语言模型（VLMs）自适应优化预测，采用随机搜索机制高效定位高似然活动标签。

Result: 在多个开放级别（L0-L3）上系统评估，ProbRes在多个基准数据集（GTEA Gaze、GTEA Gaze+、EPIC-Kitchens和Charades-Ego）上达到最先进性能。

Conclusion: ProbRes不仅性能优越，还建立了开放世界识别的清晰分类，为自我中心活动理解提供了方法论进展。
...
<details>
  <summary>Details</summary>
Motivation: 解决4D-SLAM中非刚性运动的高维优化问题，填补现有研究空白。

Method: 使用高斯表面基元SLAM方法，结合MLP变形场和新型相机位姿估计技术。

Result: 实现精确表面重建，并推出首个开放合成数据集用于评估。
...
Method: 引入AGFM模块动态调整蛋白质特征分布，结合RCFE模块捕获残基交互，并集成3D结构编码器施加几何约束。
...
Method: SHTC框架包含基于KLT的基础层和稀疏编码的增强层，通过线性变换和ISTA算法重构残差，设计可解释且参数少。

Result: SHTC显著提升了率失真性能，同时减少了参数和计算开销。
...
TL;DR: 论文提出了一种基于物理特性的材料分类法，并构建了一个包含图像和深度图的多样化数据集。通过图注意力网络实现分层材料识别，性能优异，且能适应恶劣成像条件和少样本学习。
...
Method: 利用图注意力网络，结合分类法和数据集，实现分层材料识别。

Result: 模型在分类任务中达到最优性能，并能适应恶劣成像条件和少样本学习。
...
TL;DR: 提出了一种多模态CAD重建模型，结合点云、图像和文本输入，利用VLM和LLM训练范式，通过监督微调和强化学习优化性能，在多个数据集上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有CAD重建方法通常仅支持单一输入模态，限制了通用性和鲁棒性。希望通过多模态输入和先进训练方法提升性能。

Method: 采用两阶段训练：1) 监督微调（SFT）在大规模程序生成数据上；2) 强化学习（RL）微调，使用在线反馈（如GRPO算法）。

Result: SFT模型在DeepCAD基准测试中优于单模态方法；RL微调后，模型在三个数据集上达到SOTA。
...
TL;DR: 论文提出Re-ttention方法，通过利用扩散模型的时间冗余性，实现高稀疏注意力机制，显著降低计算开销，同时保持视觉生成质量。
...
Method: Re-ttention通过基于历史softmax分布重塑注意力分数，解决注意力机制中的概率归一化偏移问题。

Result: 实验表明，Re-ttention仅需3.1%的token，性能优于FastDiTAttn等方法，并在H100 GPU上实现显著延迟降低。

Conclusion: Re-ttention是一种高效且高质量的稀疏注意力解决方案，适用于视觉生成任务。
...
TL;DR: 研究探讨扩散模型生成的合成图像是否能提升蛋白质亚细胞定位的多标签分类，发现合成数据虽在验证中表现良好，但测试泛化性差，传统方法更稳定。
...
Method: 采用简化的类条件去噪扩散概率模型（DDPM）生成标签一致的样本，并通过混合训练策略（Mix Loss和Mix Representation）整合真实数据。

Result: 混合策略在验证中表现良好，但测试泛化性差；传统ResNet分类器更稳定且表现更好。

Conclusion: 生成式增强在生物医学图像分类中需更真实的数据生成和鲁棒的监督方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统中值滤波算法存在图像位深、滤波器核大小和形状的限制，导致实际应用受限。

Method: 提出一种新方法，支持任意位深、任意核大小和任意凸核形状（包括圆形）的高效处理。

Result: 新方法能够高效处理各种图像数据，避免了传统算法的局限性。

Conclusion: 该方法为中值滤波提供了更灵活和高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过独立模块处理不同类型的运动，缺乏统一性。本文旨在提供一种更灵活、一致的运动控制方案。

Method: 通过轻量级运动注入器将用户定义的轨迹投影到预训练模型的潜在空间，实现多类型运动的统一控制。

Result: 实验表明，该方法在运动控制任务中表现优异，可控性和视觉质量显著优于现有方法。

Conclusion: 该框架为视频生成提供了高效、灵活的运动控制方案，兼容多种先进模型。
...
TL;DR: 论文提出了一种用于提升世界模型空间一致性的数据集和基准测试，通过Minecraft环境中的导航视频数据支持模型学习长距离空间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏对空间一致性的明确要求，且多数基准测试仅关注视觉连贯性或生成质量，忽略了长距离空间一致性的需求。

Method: 构建了一个包含150个不同位置的Minecraft导航视频数据集（20百万帧），采用课程设计逐步增加序列长度，并评估了四种代表性世界模型基线。

Result: 数据集和基准测试支持模型学习空间一致性，且数据收集流程易于扩展到新环境。

Conclusion: 该研究填补了空间一致性数据集的空白，为未来研究提供了开源支持。
...
TL;DR: 论文提出了一种新的数据集和基准测试（Open-HyperMotionX Dataset和HyperMotionX Bench），用于评估和改进复杂人体运动条件下的姿态引导图像动画模型，并提出了一种基于DiT的视频生成基线方法和空间低频增强RoPE模块。


<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂人体运动（Hypermotion）生成中存在明显局限性，且缺乏高质量评估基准。
...
TL;DR: SHARE是一种无需精确相机位姿的3D高斯泼溅框架，通过联合形状和相机光线估计解决位姿不准确问题。


<details>
  <summary>Details</summary>
Motivation: 在真实场景中，精确相机位姿难以获取，导致几何对齐问题。SHARE旨在解决这一问题。

Method: SHARE构建位姿感知的规范体积表示，整合多视角信息，并通过锚对齐高斯预测优化局部几何。

Result: 在多样化真实数据集上，SHARE在无需位姿的情况下表现出鲁棒性能。

Conclusion: SHARE为无需精确位姿的高斯泼溅提供了一种有效解决方案。
...
TL;DR: 提出了一种无需训练的多对象视频生成方法，利用扩散模型和LLM的开放世界知识，显著提升了多对象生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有扩散基T2V模型在多对象视频生成中表现不佳，难以准确捕捉复杂对象交互和生成多个独立对象。

Method: 使用LLM作为对象轨迹的“导演”，通过噪声重新初始化和注意力机制优化实现精确控制和特征分离。

Result: 实验表明，该方法在运动动态和对象生成准确性上提升了42%，同时保持高保真度和运动平滑性。

Conclusion: 提出的训练免费方法显著提升了现有视频扩散模型的多对象生成能力。
...
Method: 通过从Common Crawl中提取匈牙利语文档，手动或合成生成VQA数据集，并进行质量过滤和去重。

Result: 微调Llama 3.2 11B Instruct模型后，在HuDocVQA上的准确率提高了7.2%。

Conclusion: 提出的数据集和代码将公开，以促进多语言文档VQA的进一步研究。
...
TL;DR: 论文提出了一种语义引导的超分辨率框架（SeG-SR），利用视觉语言模型（VLM）提取语义知识，指导遥感图像超分辨率重建，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有遥感图像超分辨率方法主要关注像素空间低层特征，忽略了高层语义理解，导致重建结果语义不一致。本文探索高层语义知识对提升超分辨率性能的作用。

Method: 提出SeG-SR框架，包括语义特征提取模块（SFEM）、语义定位模块（SLM）和可学习调制模块（LMM），通过VLM提取语义知识并指导超分辨率过程。
...
TL;DR: 论文提出了一种新的空间-时间关节密度（STJD）测量方法，用于量化骨骼序列中动态与静态关节的交互作用，并通过对比学习策略（STJD-CL）和重建框架（STJD-MP）提升动作分类性能。
...
Method: 提出STJD量化动态与静态关节的交互，并基于此设计STJD-CL对比学习策略和STJD-MP重建框架。

Result: 在NTU RGB+D 60、120和PKUMMD数据集上，STJD-CL和STJD-MP的性能优于现有对比方法，尤其在NTU RGB+D 120上提升3.5和3.6个百分点。

Conclusion: STJD方法有效挖掘了动态与静态关节的交互潜力，显著提升了动作分类性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有细粒度分类方法依赖实例级标签，不适用于隐私敏感场景（如医学图像分析），因此需要一种无需直接访问标签的方法。

Method: 提出LHFGLP框架，结合分层稀疏字典学习和分层比例损失，通过袋级标签进行训练。

Result: 在三个细粒度数据集上实验表明，LHFGLP优于现有LLP方法。

Conclusion: LHFGLP为隐私保护的细粒度分类提供了有效解决方案，代码和数据集将开源。
...
TL;DR: 论文提出了一种基于CLIP的改进方法，结合多种深度模型和联邦学习技术，用于医学图像分类，同时保护数据隐私并提升泛化能力。
...
Method: 1) 提出CLIP变体，使用4种CNN和8种ViT作为图像编码器；2) 结合12种深度模型与联邦学习技术；3) 引入传统机器学习方法提升泛化能力。

Result: MaxViT在HAM10000数据集中表现最佳（AVG=87.03%），ConvNeXt_L在联邦学习中F1分数达83.98%，SVM提升Swin Transformer系列性能约2%。

Conclusion: 改进方法在医学图像分类中表现优异，同时解决了数据隐私和泛化问题。
...
TL;DR: 本文探讨了统一视觉语言模型（VLM）中理解与生成任务的相互增强效果，发现混合训练能带来显著的双向提升，且数据量增加时效果更佳。


<details>
  <summary>Details</summary>
Motivation: 现有统一VLM研究未充分探索理解与生成任务的相互增强假设，本文旨在填补这一空白。

Method: 设计了贴近现实场景的数据集，评估多种统一VLM架构，验证理解与生成任务的相互影响。

Result: 混合训练能双向提升任务表现，数据量增加效果更佳；多模态输入输出对齐提升泛化能力；生成任务知识可迁移至理解任务。

Conclusion: 统一理解与生成对VLM至关重要，为未来模型设计与优化提供了重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在压缩语义特征时牺牲了表达能力，且像素级预测导致冗余和高内存开销。

Method: 采用双场语义表示（粗粒度特征场和细粒度低维特征场）和选择性高斯机制，消除冗余高斯。

Result: 实验表明，参数减少60%，性能优于现有方法。

Conclusion: SpatialSplat以更紧凑的高斯表示实现了更准确的语义3D重建。
...
TL;DR: 本文探讨了视觉问答（VQA）中的多源组合泛化（MSCG）问题，提出了一种检索增强训练框架，通过学习跨模态的统一表示来提升模型的MSCG能力。


<details>
  <summary>Details</summary>
Motivation: 由于视觉与语言（V&L）任务的多模态特性，组合中的基元来自不同模态，形成多源新组合。然而，多源组合泛化能力尚未被充分研究。

Method: 提出检索增强训练框架，通过检索语义等效的基元并聚合其特征，学习跨模态的统一表示。

Result: 实验结果表明该框架有效提升模型的MSCG能力，并基于GQA数据集构建了GQA-MSCG评估数据集。

Conclusion: 该研究填补了多源组合泛化研究的空白，提出的框架和数据集为未来研究提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 部分视角3D重建在稀疏视角和单图像重建中研究较多，但在密集视角仅覆盖特定角度范围的情况下仍未被充分探索。

Method: 提出了一种融合策略，在DDIM采样中对齐多源先验，生成多视角一致的图像，并通过迭代细化策略利用几何结构提升重建质量。

Result: 在多个数据集上的实验表明，该方法在不可见区域的表现优于现有技术。

Conclusion: \method通过融合局部观测和多源先验，显著提升了部分视角3D重建的生成一致性和质量。
...
TL;DR: 论文提出了一种统一的多状态感知模型URWKV，用于低光图像增强和去模糊，通过自适应归一化和选择性融合模块显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有低光图像增强和去模糊模型难以处理动态耦合的退化问题，需要更灵活的解决方案。

Method: 设计了URWKV模型，包含Luminance-adaptive Normalization（LAN）和State-aware Selective Fusion（SSF）模块，利用多状态视角分析退化。

Result: URWKV在多个基准测试中表现优于现有模型，且参数和计算资源需求更低。

Conclusion: URWKV通过多状态感知和动态融合机制，有效解决了低光图像增强和去模糊中的复杂退化问题。
...
TL;DR: GeoMan是一种新架构，通过结合图像模型和视频扩散模型，从单目视频中生成准确且时间一致的3D人体几何估计，解决了数据稀缺和深度估计的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法在单图像优化时存在时间不一致性和动态细节捕捉不足的问题，GeoMan旨在解决这些局限性。

Method: GeoMan使用图像模型估计首帧深度和法线，再通过视频扩散模型生成后续帧，将任务转化为图像到视频生成问题，并引入根相对深度表示以保留人体尺度细节。
...
TL;DR: 论文提出了一种轻量级语义分割方法LeMoRe，通过结合显式和隐式建模，平衡计算效率与表示精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在特征建模复杂度上难以平衡效率与性能，且依赖参数密集型设计和计算密集的Vision Transformer框架。

Method: 结合显式建模的笛卡尔方向和隐式推断的中间表示，通过嵌套注意力机制高效捕获全局依赖。

Result: 在ADE20K、CityScapes、Pascal Context和COCO-Stuff等数据集上验证了LeMoRe在性能与效率上的平衡。

Conclusion: LeMoRe通过显式与隐式建模的结合，实现了轻量级语义分割的高效与高性能。
...
TL;DR: 论文提出CURVE方法，利用CLIP模型和强化学习优化低光图像增强，兼顾效果与效率。
...
Method: 采用基于Bézier曲线的简单图像处理模块调整全局色调，并通过强化学习迭代估计参数，奖励设计使用CLIP文本嵌入。
...
TL;DR: 论文提出了一种名为EEG Adapter（EAD）的灵活框架，用于学习EEG信号的嵌入表示，解决了不同设备采集的EEG数据通道数不一致的问题，并在多个数据集上取得了高准确率。


<details>
  <summary>Details</summary>
Motivation: 传统EEG信号分类方法依赖于特定任务的数据采集和预处理，且对不同设备采集的数据适应性差，因此需要一种统一的框架来学习适用于不同设备的EEG嵌入表示。

Method: 论文提出了EEG Adapter（EAD）框架，基于一种EEG基础模型进行改进，用于学习鲁棒的EEG信号表示，并支持零样本分类。

Result: 在EEG-ImageNet和BrainLat数据集上分别取得了99.33%和92.31%的准确率，展示了框架在多种EEG任务中的有效性。

Conclusion: EAD框架能够有效处理不同设备采集的EEG数据，并在多种任务中表现出优异的性能和泛化能力。
...
TL;DR: 提出了一种个性化认知测试方案，通过识别个体特定的认知障碍模式，为痴呆早期检测提供更高效的工具。


<details>
  <summary>Details</summary>
Motivation: 传统全面认知测试耗时且难以大规模应用，而痴呆进展中的认知障碍模式因人而异，需要个性化解决方案。

Method: 采用两步法：先通过群体聚类学习认知障碍模式，再用特征选择和聚类分析识别这些模式。

Result: 识别出的模式与临床认可的轻度认知障碍（MCI）变体一致，可用于预测无症状人群的认知障碍路径。

Conclusion: 该方法为痴呆早期检测提供了高效且个性化的工具，适用于大规模人群的定期评估。
...
TL;DR: 该论文提出了一种基于扩散模型的生成方法，用于从视觉输入预测3D占用网格，解决了传统判别方法在噪声数据、不完整观测和复杂3D结构中的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前判别方法在3D占用网格预测中存在噪声敏感性和对复杂场景结构处理不足的问题，因此需要一种更鲁棒的方法。

Method: 采用扩散模型作为生成建模工具，学习数据分布并融入3D场景先验，以提高预测一致性和噪声鲁棒性。

Result: 实验表明，扩散模型在3D占用预测中优于现有判别方法，尤其在遮挡或低可见度区域表现更优。

Conclusion: 该方法不仅提升了预测质量，还对下游规划任务有显著帮助，具有实际应用价值。
...
TL;DR: TextSR是一种针对多语言场景文本图像超分辨率的多模态扩散模型，通过结合文本检测、OCR和UTF-8编码，解决了现有扩散模型在文本区域定位和字符形状建模上的不足。
...
Method: TextSR利用文本检测器和OCR提取多语言文本，通过UTF-8编码和交叉注意力将字符转换为视觉形状，并结合两种创新方法提升模型鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法仅依赖音频信号难以捕捉大幅手势动作，导致视频中出现明显伪影和失真。现有方法通过引入额外先验信息解决，但限制了实际应用。

Method: 提出MMGT网络，分为两阶段：第一阶段（SMGA）从音频生成高质量姿态视频和运动掩码；第二阶段（MM-HAA）结合稳定扩散视频生成模型，提升细节控制和运动生成。

Result: 实验表明，该方法在视频质量、唇同步和手势生成上均有提升。

Conclusion: MMGT通过两阶段设计，实现了高质量、细节丰富的上半身视频生成，优于传统方法。
...
TL;DR: HMAD框架通过结合BEV轨迹生成与多标准评分，解决了自动驾驶中轨迹多样性和路径选择的挑战，显著提升了驾驶评分。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶在生成多样化、合规轨迹及通过多维度评估选择最优路径方面存在挑战。

Method: HMAD采用BEVFormer和可学习锚点查询生成轨迹，并通过模拟监督评分模块评估轨迹。

Result: HMAD在CVPR 2025测试集上实现了44.5%的驾驶评分。

Conclusion: HMAD展示了轨迹生成与安全评分分离对高级自动驾驶的益处。
...
TL;DR: PhotoArtAgent是一个结合视觉语言模型和自然语言推理的智能系统，模拟专业艺术家的创作过程，提供透明且交互式的照片润色服务。


<details>
  <summary>Details</summary>
Motivation: 解决非专业用户依赖自动化工具时缺乏解释深度和交互透明度的问题，同时模拟专业艺术家的创意过程。

Method: 结合视觉语言模型（VLMs）和自然语言推理，进行艺术分析、制定润色策略，并通过API输出参数到Lightroom，迭代优化图像。

Result: 在用户研究中超越现有自动化工具，结果接近专业人类艺术家水平。

Conclusion: PhotoArtAgent通过透明解释和迭代优化，实现了高质量且用户可控的照片润色。
...
Method: 方法分为两个阶段：Zero阶段通过编辑锚帧作为参考图像，并利用原始帧的对应关系引导注意力机制；Hero阶段学习条件生成模型进行视频修复。

Result: 方法在PSNR上比最佳基线提高了2.6 dB，并在处理大运动对象时表现出更强的鲁棒性。

Conclusion: Zero-to-Hero方法在视频编辑的准确性和时间一致性方面表现优异，但注意力机制的干预可能导致图像退化。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学AI模型忽略诊断推理的序列性，导致与临床场景脱节、推理缺乏上下文和错误不可追溯。

Method: 构建CXRTrek数据集（428,966样本，11百万Q&A对），并提出CXRTrekNet模型，融入临床推理流程。

Result: CXRTrekNet在CXRTrek基准上优于现有医学VLLM，并在多个外部数据集上表现优异。

Conclusion: CXRTrek数据集和模型填补了医学AI在诊断推理建模上的空白，具有临床实用性和泛化能力。
...
TL;DR: FlowAlign是一种基于流的无反转图像编辑框架，通过流匹配损失提升编辑轨迹的稳定性和源一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于流的无反转图像编辑方法（如FlowEdit）虽避免了精确潜在反转，但常导致编辑轨迹不稳定和源一致性差。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖离线处理且模型庞大，难以满足实时需求，因此需要一种在线解析方法。

Method: 提出PreFM框架，包括预测多模态未来建模和模态无关的鲁棒表示，以提升上下文理解和实时性能。
...
TL;DR: 提出了一种基于3D高斯泼溅的层次化LOD方法，优化实时渲染性能与内存占用。
...
Method: 采用层次化LOD表示，结合深度感知平滑、重要性剪枝与动态空间分块加载。

Result: 在户外和室内数据集上实现高性能渲染，降低延迟与内存需求。

Conclusion: 该方法在保持视觉质量的同时显著提升了渲染效率。
...
TL;DR: CLIP模型无需解码器或训练即可实现图像合成，通过频率感知隐式神经表示和多种稳定技术解锁了生成能力。


<details>
  <summary>Details</summary>
Motivation: 探索CLIP模型的潜在生成能力，无需额外训练或解码器。
...
<details>
  <summary>Details</summary>
Motivation: 模仿学习在机器人操作中很重要，但大规模真实数据收集成本高，仿真数据又存在sim-to-real差距，因此需要一种高效的数据合成方法。

Method: RoboTransfer整合多视角几何和场景组件控制，通过跨视角特征交互和全局深度/法线条件确保几何一致性。

Result: 实验表明，RoboTransfer生成的多视角视频几何一致性和视觉保真度更高，训练的策略在DIFF-OBJ和DIFF-ALL场景中分别提升33.3%和251%的成功率。

Conclusion: RoboTransfer为机器人数据合成提供了一种高效且可控的解决方案，显著提升了策略训练效果。
...
<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs在视觉理解方面表现出色，但在复杂现实场景（如密集人群）中的细粒度感知仍有限。受强化学习（RL）在LLMs和MLLMs中的成功启发，作者探索如何利用RL提升MLLMs的视觉感知能力。

Method: 开发了DIP-R1框架，通过三种规则化奖励模型（标准推理奖励、方差引导观察奖励和加权精确召回奖励）指导MLLMs进行详细视觉场景检查。

Result: DIP-R1在多种细粒度目标检测数据上表现优异，显著优于现有基线模型和监督微调方法。

Conclusion: 研究表明，将RL整合到MLLMs中具有巨大潜力，可显著提升复杂现实场景中的感知能力。
...
TL;DR: 论文提出新任务FS2RG，通过结合平面草图和文本指导生成逼真服装图像，并提出了HiGarment框架解决其挑战。


<details>
  <summary>Details</summary>
Motivation: 填补服装生产过程中基于扩散模型的合成任务研究空白，解决文本指导不足和模态冲突问题。
...
<details>
  <summary>Details</summary>
Motivation: 随着文本生成图像（T2I）模型的普及，身份滥用和AIGC检测器的鲁棒性问题日益突出，需要研究对抗性攻击方法。

Method: 利用语法树结构和蒙特卡洛树搜索算法，系统探索语义提示空间，生成多样且可控的对抗性提示。

Result: 该方法能有效规避开源和商业AIGC检测器，并在实际竞赛中排名第一。
...
TL;DR: 论文提出了一种名为LANGO的语言引导对象检测框架，旨在解决航空图像中因光照和视角变化导致的多样性和对象外观变化问题。


<details>
  <summary>Details</summary>
Motivation: 航空图像中的多样性和对象外观变化使得对象检测变得复杂，论文通过模拟人类理解场景语义的方式来解决这一问题。

Method: 设计了视觉语义推理器来理解图像场景的视觉语义，并提出了关系学习损失来处理实例级变化。

Result: 实验表明，该方法显著提高了检测性能。
...
TL;DR: WTEFNet是一个专为低光场景设计的实时目标检测框架，通过增强低光区域、提取多级小波特征和自适应融合语义与光照特征，显著提升了低光条件下的检测性能。
...
Method: WTEFNet包含三个核心模块：低光增强（LLE）、小波特征提取（WFE）和自适应融合检测（AFFD），并结合新数据集GSN进行训练和评估。

Result: 在多个数据集（BDD100K、SHIFT、nuScenes、GSN）上达到最先进的低光检测精度，并在嵌入式平台上验证了实时性。
...
TL;DR: 提出了一种完全基于3D点云的多模态融合方法，使用双分支Transformer模型学习几何和光谱特征，并通过跨注意力机制增强特征融合。


<details>
  <summary>Details</summary>
Motivation: 现有方法多将3D数据栅格化为2D格式，未能充分利用3D数据的潜力，限制了模型学习原始点云的3D空间特征和生成3D预测的能力。

Method: 提出了一种3D点云多模态融合方法，采用双分支Transformer模型和跨注意力机制，直接在3D点云上学习特征。

Result: 在多个数据集上的实验表明，3D融合方法性能与2D方法相当，且能生成3D预测，灵活性更高。

Conclusion: 3D融合方法不仅性能优越，还能提供2D方法无法实现的3D预测能力，为多模态遥感数据分类提供了新思路。
...
TL;DR: FlexMerge是一种新颖的无数据模型合并框架，通过灵活生成不同大小的合并模型，平衡精度与成本。


<details>
  <summary>Details</summary>
Motivation: 解决单模型合并的精度损失与多模型部署的高成本问题。

Method: 将微调模型视为顺序块集合，逐步合并，支持多种无数据合并算法。

Result: 实验显示，适度增大的合并模型能显著提升精度。

Conclusion: FlexMerge为多样化部署提供了灵活、高效的无数据解决方案。
...
TL;DR: SAMamba是一种新型红外小目标检测框架，结合了SAM2的分层特征学习和Mamba的选择性序列建模，解决了现有方法的信息丢失和全局上下文建模效率低的问题。
...
Method: SAMamba通过FS-Adapter实现自然到红外的域适应，CSI模块实现高效的全局上下文建模，DPCF模块平衡多尺度特征融合。

Result: 在NUAA-SIRST、IRSTD-1k和NUDT-SIRST数据集上，SAMamba显著优于现有方法，尤其在复杂背景和多尺度目标场景中。

Conclusion: SAMamba通过域适应、细节保留和高效长距离依赖建模，成功解决了ISTD的核心挑战。
...
TL;DR: UniTEX提出了一种新颖的两阶段3D纹理生成框架，通过直接在统一的3D功能空间中操作，避免了UV映射的限制，生成高质量且一致的3D纹理。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖UV映射修复纹理，存在拓扑模糊性问题。UniTEX旨在通过3D功能空间直接生成纹理，解决这一问题。

Method: 1. 提出Texture Functions（TFs）将纹理生成提升到3D空间；2. 使用基于Transformer的大规模纹理模型（LTM）直接从图像和几何输入预测TFs；3. 采用LoRA策略优化2D先验，实现高质量多视图纹理合成。

Result: 实验表明，UniTEX在视觉质量和纹理完整性上优于现有方法，提供了可扩展的自动化3D纹理生成方案。

Conclusion: UniTEX通过创新的3D功能空间方法，为3D纹理生成提供了高效且通用的解决方案。
...
TL;DR: 论文提出了一种结合数据和方法的完整解决方案，以提升多模态大语言模型（MLLMs）在医学图像筛选中的美学推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在图像筛选任务中表现不佳，主要由于数据缺乏和美学推理能力不足。

Method: 构建了一个包含1500+样本的医学图像数据集，并提出了一种基于强化学习的方法DPA-GRPO。

Result: 实验显示，即使最先进的闭源MLLMs（如GPT-4o）在美学推理中表现接近随机猜测，而作者的方法显著优于这些模型。

Conclusion: 该研究为图像美学推理提供了一种新的配置方法，并有望在未来成为标准。
...
TL;DR: 提出了一种结合视频帧和文本转录的多模态RL框架，用于视频摘要和高光检测，优于仅依赖视觉内容的方法。


<details>
  <summary>Details</summary>
Motivation: 视频消费需求高，但完整观看耗时；现有方法未结合多模态和RL框架。

Method: 多模态管道结合视频帧和转录文本，通过RL框架训练，奖励多样性和代表性摘要。

Result: 实验表明，结合转录文本的方法优于仅依赖视觉内容的方法。

Conclusion: 多模态RL框架能有效提升视频摘要和高光检测性能，适用于大规模未标注数据。
...
TL;DR: LADA（Label-specific ADApter）是一种针对CLIP模型的持续学习方法，通过添加轻量级标签特定记忆单元，避免参数划分错误，并利用特征蒸馏防止灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP方法在持续学习中需要划分参数，容易导致推理错误和性能下降。

Method: LADA在冻结的CLIP图像编码器后添加标签特定记忆单元，通过特征蒸馏保护已学类别特征。
...
TL;DR: 论文探讨多模态大模型（MLMs）是否能真正“理解”图像，提出“视觉房间”论点，指出模型可能仅通过算法规则处理图像而缺乏真正理解。通过感知与认知两层次评估框架，发现MLMs在感知任务表现良好，但在讽刺理解上仍有显著差距。


<details>
  <summary>Details</summary>
Motivation: 挑战当前假设，即感知能力等同于真正理解，提出“视觉房间”论点，质疑MLMs是否具备真实理解能力。

Method: 引入两层次评估框架（感知与认知），构建高质量多模态讽刺数据集，评估8种SoTA MLMs。

Result: MLMs在感知任务表现良好，但讽刺理解平均错误率达16.1%，主要因情感推理、常识推断和上下文对齐不足。

Conclusion: 为“视觉房间”论点提供实证支持，提出新的MLMs评估范式，揭示感知与理解的差距。
...
Method: MixGS通过视图感知表示整合相机位姿和高斯属性，结合混合操作保留全局一致性和局部细节。

Result: 实验表明MixGS在渲染质量和速度上达到最优，且计算需求显著降低。

Conclusion: MixGS为大规模场景重建提供了一种高效且高质量的解决方案。
...
TL;DR: 论文介绍了RSFAKE-1M数据集，用于检测基于扩散模型的伪造遥感图像，并验证了现有检测方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 遥感图像在环境监测等领域至关重要，但现有伪造检测方法主要针对GAN生成图像或自然图像，缺乏对扩散模型生成的伪造遥感图像的关注。

Method: 构建了包含50万伪造和50万真实遥感图像的RSFAKE-1M数据集，伪造图像由10种扩散模型生成，涵盖6种生成条件。

Result: 实验表明，现有检测方法对扩散模型生成的伪造遥感图像效果不佳，而基于RSFAKE-1M训练的模型表现出更好的泛化性和鲁棒性。

Conclusion: RSFAKE-1M为遥感图像伪造检测领域提供了重要基础，推动了下一代检测方法的发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决GenCAD生成CAD模型时约10%设计不可行的问题，提升AI驱动CAD生成的实用性。

Method: 结合扩散引导的潜在空间去噪过程和回归校正机制，优化不可行CAD命令序列。

Result: 成功将基线方法中三分之二的不可行设计转化为可行设计，显著提升可行性率并保持几何精度。

Conclusion: 该方法显著提升CAD生成的可行性，扩展高质量训练数据，增强AI在制造、建筑和产品设计中的应用。
...
TL;DR: FUSS是首个完全去中心化、无标签的联邦学习语义图像分割框架，通过特征和原型空间的一致性优化，在异构数据分布下表现优异。


<details>
  <summary>Details</summary>
Motivation: 探索联邦学习在无监督语义图像分割中的应用，解决分布式客户端特征和聚类中心对齐的挑战。

Method: 提出FUSS框架，引入新的联邦策略，联合优化本地分割头和共享语义中心。

Result: 在基准和真实数据集上，FUSS优于本地训练和经典联邦学习算法。
...
TL;DR: TRACE是一种新方法，通过轨迹约束注意力概念擦除技术，从扩散模型中删除特定概念，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型中生成不良内容（如色情、敏感身份、版权风格）的隐私、公平和安全问题。

Method: 结合理论框架和微调程序，通过更新交叉注意力层和轨迹感知微调目标，在扩散过程中抑制特定概念。

Result: 在多个基准测试中表现优异，超越现有方法如ANT、EraseAnything和MACE。

Conclusion: TRACE在概念擦除效果和输出质量上达到最先进水平。
...
TL;DR: 本文提出了首个针对行人属性识别（PAR）的对抗攻击与防御框架，包括全局和局部攻击方法，并设计了一种语义偏移防御策略。
...
Method: 基于CLIP的PAR框架，采用多模态Transformer融合视觉和文本特征，并通过对抗语义和标签扰动生成噪声（ASL-PAR）。
...
<details>
  <summary>Details</summary>
Motivation: 探索训练好的高维视频生成模型是否能有效支持低维任务（如图像生成），以挖掘视频模型的潜力。
...
TL;DR: 论文研究了使用强化学习（RL）微调预训练生成模型的方法，特别是应用GRPO优化视觉自回归模型（VAR），显著提升了图像质量并实现了生成风格的控制。


<details>
  <summary>Details</summary>
Motivation: 通过RL微调生成模型，可以更好地对齐复杂的人类偏好，尤其是在视觉生成任务中。

Method: 采用Group Relative Policy Optimization（GRPO）微调VAR模型，结合美学预测器和CLIP嵌入的奖励信号。

Result: 实验表明，该方法能显著提升图像质量，并实现生成风格的精确控制，还能让模型生成预训练数据中未包含的风格。

Conclusion: RL微调对VAR模型高效且有效，其快速推理速度使其在在线采样中优于扩散模型。
...
TL;DR: DSAGL是一种新型弱监督分类框架，结合教师-学生架构和双流设计，通过多尺度注意力伪标签解决实例级模糊性和包级语义一致性，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 全切片图像（WSIs）因其超高分辨率和丰富语义内容对癌症诊断至关重要，但数据量大且细粒度标注稀缺，传统监督学习面临挑战。

Method: 提出DSAGL框架，结合教师-学生架构与双流设计，利用VSSMamba轻量编码器建模长程依赖，FASA模块增强稀疏诊断区域关注，并引入混合损失确保双流一致性。

Result: 在CIFAR-10、NCT-CRC和TCGA-Lung数据集上，DSAGL显著优于现有MIL基线，表现出优越的判别性能和弱监督下的鲁棒性。

Conclusion: DSAGL通过创新设计有效解决了WSIs分类中的弱监督问题，为医学图像分析提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型因采样轨迹的随机性导致样本质量不稳定，现有方法计算成本高且依赖外部奖励信号，限制了广泛应用。

Method: 通过分析去噪轨迹中条件与无条件分数的累积差异（ASD），提出CFG-Rejection方法，早期过滤低质量样本。

Result: 实验证明CFG-Rejection显著提升了图像生成的人类偏好分数（HPSv2, PickScore）和挑战性基准（GenEval, DPG-Bench）。
...
TL;DR: Flow Matching (FM) 框架通过向量场将样本从源分布传输到目标分布。传统方法使用随机耦合导致路径交叉和非直线轨迹，而基于最优传输 (OT) 的方法虽减少交叉但未与模型偏好对齐。作者提出 Model-Aligned Coupling (MAC)，结合几何距离和模型预测误差对齐耦合，显著提升生成质量和效率。


<details>
  <summary>Details</summary>
Motivation: 传统 FM 方法使用随机耦合导致路径交叉和低效轨迹，而 OT 方法虽减少交叉但未与模型偏好对齐，导致学习困难。

Method: 提出 MAC 方法，结合几何距离和模型预测误差选择耦合，并采用 top-k 策略减少计算成本。

Result: 实验表明 MAC 在少步生成中显著优于现有方法，提升质量和效率。

Conclusion: MAC 通过对齐模型偏好优化耦合选择，有效提升 FM 框架的性能。
...
TL;DR: KRCapVLM是一种基于知识回放的图像描述生成框架，通过结合视觉语言模型和束搜索解码，生成更具多样性和连贯性的描述，同时提升知识识别能力。
...
Method: 采用知识回放框架，结合束搜索解码、注意力模块增强特征表示，并使用训练调度器优化训练稳定性。

Result: 模型在知识识别准确性和描述质量上均有显著提升，能够更好地泛化到未见过的知识概念。

Conclusion: KRCapVLM有效提升了生成知识丰富、上下文相关的图像描述的能力。
...
TL;DR: 论文介绍了VideoReasonBench，一个用于评估视觉中心复杂视频推理的基准测试，发现现有多模态大语言模型在复杂视频推理上表现不佳，而扩展思考预算对提升性能至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理基准缺乏深度推理需求，无法展示长链思维推理的优势，且任务多为知识驱动而非视觉内容依赖。

Method: 设计VideoReasonBench，包含视觉丰富且高复杂度的视频推理任务，评估模型在三个逐步升级的推理技能上的表现。

Result: 18个先进多模态大语言模型中，大多数表现不佳（如GPT-4o仅6.9%准确率），而Gemini-2.5-Pro以56.0%显著领先。

Conclusion: 扩展思考预算对提升VideoReasonBench性能至关重要，而现有视频基准无法体现这一优势。
...
Method: MCFNet包含正则化融合模块和混合注意力机制，通过多模态决策分类模块联合利用模态间相关性和单模态判别特征。
...
<details>
  <summary>Details</summary>
Motivation: PAN和MS图像融合中由于传感器放置、采集时间和分辨率差异导致的跨模态错位问题，传统深度学习方法假设完美对齐，导致光谱失真和模糊。

Method: 提出PAN-Crafter框架，包括模态自适应重建（MARs）和跨模态对齐感知注意力（CM3A），联合重建HRMS和PAN图像，并双向对齐特征。

Result: 在多个基准数据集上，PAN-Crafter在所有指标上均优于最新方法，推理速度快50.11倍，内存占用减少0.63倍，且在未见卫星数据集上表现稳健。

Conclusion: PAN-Crafter通过模态一致对齐和自适应特征细化，有效解决了跨模态错位问题，具有高效性和强泛化能力。
...
TL;DR: UniRL是一种自改进的后训练方法，通过模型生成图像作为训练数据，无需外部图像数据，同时提升生成和理解任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态大语言模型依赖大规模数据和计算，且后训练方法常需外部数据或局限于任务定制。UniRL旨在解决这些问题。

Method: 采用自生成图像作为训练数据，结合监督微调（SFT）和组相对策略优化（GRPO）优化模型，实现生成与理解任务的相互增强。

Result: 在Show-o和Janus上评估，GenEval得分分别为0.77和0.65。

Conclusion: UniRL无需外部数据，减少任务间不平衡，且仅需少量额外训练步骤，具有显著优势。
...
TL;DR: 提出VModA框架，显著提升NSFW内容检测准确率，适应多样化规则和复杂语义内容。


<details>
  <summary>Details</summary>
Motivation: NSFW内容在社交网络中泛滥，现有检测方法难以应对复杂语义和多样化规则。

Method: 提出VModA框架，适应多样化规则，处理复杂语义NSFW内容。

Result: VModA在NSFW检测中准确率提升54.3%，并展示强适应性。

Conclusion: VModA有效解决NSFW检测问题，并在实际场景中验证其效果。
...
TL;DR: 提出了一种基于YOLOv11n检测器和FUSegNet分割模型的轻量级管道，仅需500个标注框即可实现跨部位伤口分割，无需微调。


<details>
  <summary>Details</summary>
Motivation: 现有模型（如FUSegNet）在非足部伤口上泛化能力差，需高效标注方法提升通用性。

Method: 结合YOLOv11n检测器和预训练FUSegNet，仅用500个标注框实现零微调跨域分割。

Result: 在多个伤口测试集上，平均IoU提升23%，DESIGN-R尺寸估计准确率从71%增至94%。

Conclusion: 该方法仅需少量标注即可泛化至不同伤口类型，支持临床自动化评分，减少标注依赖。
...
TL;DR: VecFormer提出了一种基于线基表示的新方法，用于CAD图纸中的全景符号识别，解决了现有方法的高计算成本和几何信息丢失问题，并通过分支融合细化模块提升预测一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在CAD图纸的全景符号识别中存在高计算成本、通用性有限和几何信息丢失的问题，需要一种更高效且保留几何结构的方法。

Method: VecFormer采用线基表示原始图形，保留几何连续性，并引入分支融合细化模块整合实例和语义预测。

Result: 实验表明，VecFormer在PQ指标上达到91.1，Stuff-PQ分别提升9.6和21.2分，优于现有方法。

Conclusion: 线基表示是矢量图形理解的有效基础，VecFormer在性能和效率上均表现优异。
...
TL;DR: BriGeS是一种融合几何与语义信息的方法，通过Bridging Gate和Attention Temperature Scaling技术提升单目深度估计性能，显著减少资源需求。


<details>
  <summary>Details</summary>
Motivation: 结合几何与语义信息的互补优势，提升复杂场景下的单目深度估计效果。

Method: 利用预训练基础模型，仅训练Bridging Gate，结合Attention Temperature Scaling平衡注意力机制。

Result: 在多个数据集上表现优于现有方法，尤其在复杂结构和重叠物体场景中。

Conclusion: BriGeS通过高效整合几何与语义信息，显著提升了单目深度估计的泛化能力和性能。
...
TL;DR: EdiDub是一个新颖的视觉配音框架，通过内容感知编辑任务解决现有方法的局限性，显著提升了身份保留和唇同步效果。
...
Conclusion: 内容感知编辑方法在保留复杂视觉元素和实现准确唇同步方面优于传统生成或修复方法。
...
TL;DR: UrbanCraft通过分层语义几何表示解决外推视图合成问题，结合粗粒度场景级先验和细粒度实例级先验，提出HSG-VSD方法提升性能。
...
Method: 利用部分可观测场景重建语义和几何基元，结合场景级和实例级先验，提出HSG-VSD方法整合约束。

Result: 定性和定量实验验证了UrbanCraft在外推视图合成问题上的有效性。

Conclusion: UrbanCraft通过分层语义几何表示和HSG-VSD方法，显著提升了外推视图合成的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有强增强方法主要基于强度扰动，对语义掩码影响有限，而空间增强在SSSS中被忽视。作者希望验证空间增强的有效性并解决其带来的掩码不一致问题。

Method: 提出自适应空间增强（ASAug），基于熵动态调整每张图像的增强策略，作为可插拔模块集成到现有方法中。

Result: 在PASCAL VOC 2012、Cityscapes和COCO等基准数据集上取得SOTA结果，验证了ASAug的有效性。

Conclusion: 空间增强在半监督语义分割中具有潜力，自适应策略能进一步提升模型性能。
...
TL;DR: 本文提出了一种基于非刚性配准的虚拟试穿方法（VITON-DRR），通过双金字塔结构特征提取器和变形模块，提高了服装细节保留和变形准确性。


<details>
  <summary>Details</summary>
Motivation: 虚拟试穿在电商和时尚行业有巨大应用潜力，但现有方法在服装细节保留和变形准确性上存在不足。
...
<details>
  <summary>Details</summary>
Motivation: 冷冻电镜（cryo-EM）的高质量标注数据稀缺，限制了模型的开发。现有合成数据方法难以同时捕捉生物样本的结构多样性和复杂噪声。

Method: CryoCCD通过生物物理建模和生成技术（如条件扩散模型）生成多尺度图像，并结合循环一致性和对比学习优化噪声模拟。

Result: 实验表明，CryoCCD生成的图像结构准确，在下游任务（如粒子挑选和重建）中优于现有方法。

Conclusion: CryoCCD为冷冻电镜数据合成提供了更高效的解决方案，提升了图像质量和下游任务性能。
...
TL;DR: 论文提出了一种反向因果框架（RcSGG），通过重构因果链结构来消除场景图生成（SGG）中的虚假相关性，解决了现有方法中的偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有两阶段SGG框架因因果链结构导致虚假相关性，引发尾部关系被预测为头部关系、前景关系被预测为背景关系的偏差。

Method: 提出RcSGG框架，包括主动反向估计（ARE）和最大信息采样（MIS），通过干预混杂变量估计反向因果关系。

Result: 在多个基准测试和不同SGG框架上实现了最先进的平均召回率。

Conclusion: RcSGG有效缓解了SGG中的虚假相关性，消除了偏差，提升了性能。
...
TL;DR: 论文提出LAFR方法，通过潜在空间适配器对齐低质量图像的潜在分布，实现高质量人脸修复，同时减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在低质量图像编码时存在语义不匹配问题，传统方法需重新训练VAE编码器，计算成本高。

Method: 提出LAFR方法，通过潜在空间适配器对齐潜在分布，并引入多级修复损失和轻量级扩散先验微调。

Result: 实验表明LAFR在合成和真实数据集上均能高效实现高质量、身份保持的人脸修复。

Conclusion: LAFR是一种高效且有效的人脸修复方法，显著降低了计算成本。
...
TL;DR: 本文探讨了深度学习中加权风险函数与校准误差的关系，提出正则化AURC优化方法，证明其优于焦点损失，并引入SoftRank技术实现梯度优化。


<details>
  <summary>Details</summary>
Motivation: 现有加权风险函数（如焦点损失和逆焦点损失）的校准性质存在争议，需建立其与校准误差的理论联系。

Method: 通过选择性分类范式分析校准误差，提出正则化AURC优化方法，并采用SoftRank技术实现可微分优化。

Result: 实验表明，正则化AURC在多数据集和模型架构中表现优于焦点损失，校准性能更优。

Conclusion: 正则化AURC为校准目标提供了更灵活的优化框架，支持逆焦点损失的加权策略，而焦点损失在此目标下不够理想。
...
TL;DR: DACPO提出了一种分而治之的策略，用于大规模非封闭3D场景的点云定向，通过分块处理和全局优化实现高效定向。
...
Method: DACPO将点云分割为小块，通过随机贪婪法估计初始法线方向，并利用改进的泊松曲面重建进行细化；通过无向图建模块间关系，全局优化采用0-1整数约束问题。
...
Result: TimePoint比标准DTW更快且更准确，且在合成数据上训练后能泛化到真实数据，进一步微调可提升性能。
...
TL;DR: PhysicsNeRF通过引入物理约束改进NeRF，在稀疏视图下实现更优的3D重建。


<details>
  <summary>Details</summary>
Motivation: 解决标准NeRF在稀疏视图下性能不足的问题，提升3D重建的物理一致性和泛化能力。

Method: 结合深度排序、RegNeRF一致性、稀疏先验和跨视图对齐四种约束，采用0.67M参数架构。

Result: 仅用8视图即达到21.4 dB PSNR，优于现有方法，并揭示了稀疏重建的局限性（5.7-6.2 dB泛化差距）。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准在细粒度评估上不足，尤其是对视频生成至关重要的时空细节捕捉。

Method: 引入VCapsBench基准，包含5,677视频和109,796问答对，覆盖21个细粒度维度，并提出三个指标（AR、IR、CR）及自动化评估流程。

Result: 基准通过对比问答对分析验证字幕质量，为字幕优化提供可操作建议。

Conclusion: VCapsBench有助于推动稳健的文本到视频模型发展，数据集和代码已开源。
...
TL;DR: 论文提出了R2I-Bench基准，用于评估文本到图像生成模型的推理能力，并设计了R2IScore指标进行细粒度评估。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型在推理能力方面表现不足，缺乏系统性评估工具。

Method: 构建R2I-Bench基准，涵盖多种推理类别，并设计R2IScore指标评估文本图像对齐、推理准确性和图像质量。

Result: 实验显示16种代表性模型在推理能力上表现有限，表明需要更强大的推理感知架构。
...
TL;DR: VAU-R1是一个基于多模态大语言模型（MLLMs）的数据高效框架，通过强化微调（RFT）提升视频异常推理能力，并提出了首个视频异常推理基准VAU-Bench。
...
Method: 提出VAU-R1框架，利用MLLMs和RFT增强异常推理，并建立VAU-Bench基准，包含多选题、详细解释、时间标注和描述性标题。
...
TL;DR: OmniEarth-Bench是一个全面的多模态基准测试，涵盖地球科学的六个领域及其交互，提供100个专家评估维度，现有MLLM模型表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有地球科学多模态学习基准在覆盖范围和评估维度上存在局限，无法全面反映地球系统的复杂性。

Method: 整合卫星和实地观测数据，通过专家和众包标注，构建包含29,779个注释的四个层次评估体系。

Result: 测试9种先进MLLM模型，表现均不理想，最高准确率低于35%，部分任务中GPT-4o准确率为0%。
...
Method: 结合视觉语言预训练和分类预训练协作增强，引入音频感知丰富上下文边界信息，采用自监督跨视角学习范式。
...
TL;DR: 论文提出了一种基于人类偏好对齐的扩散框架，用于生成高度动态和逼真的肖像动画，解决了唇音同步、自然表情和高保真身体运动动力学的挑战。


<details>
  <summary>Details</summary>
Motivation: 生成逼真的肖像动画需要解决唇音同步、自然表情和高质量身体运动的难题，现有方法在这些方面仍有不足。

Method: 采用人类偏好优化和时序运动调制技术，前者通过人类偏好数据集对齐生成结果，后者通过时间通道重分布和比例特征扩展解决时空分辨率不匹配问题。

Result: 实验表明，该方法在唇音同步、表情生动性和身体运动连贯性上优于基线方法，并在人类偏好指标上有显著提升。
...
TL;DR: 论文提出了一种结合神经网络与语义技术的Metadata Enrichment Model (MEM)，用于丰富文化遗产数字化收藏的元数据，并通过实际案例验证其潜力。


<details>
  <summary>Details</summary>
Motivation: 解决文化遗产数字化收藏中元数据不足导致的访问性、互操作性和跨机构合作问题。

Method: 提出MEM框架，结合计算机视觉模型、大语言模型和知识图谱，通过Multilayer Vision Mechanism (MVM)动态检测嵌套特征。

Result: 在Jagiellonian数字图书馆的案例中验证了MEM的潜力，并发布了手动标注的数据集。
...
TL;DR: Qwen-LA通过引入视觉-文本反思过程，减少视觉语言推理模型（VLRM）中的幻觉问题，提升视觉注意力。


<details>
  <summary>Details</summary>
Motivation: 长推理过程会稀释视觉信息，导致注意力分散和幻觉问题，仅文本反思不足以解决。
...
Conclusion: 视觉-文本反思能有效提升视觉注意力，减少幻觉，Qwen-LA为VLRM提供了新解决方案。
...
TL;DR: VScan是一个两阶段视觉令牌减少框架，通过全局和局部扫描结合令牌合并，以及在语言模型中间层引入剪枝，显著加速推理并保持高性能。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型（LVLMs）因视觉令牌序列较长导致计算成本高，难以实时部署，需优化令牌冗余问题。

Method: 提出VScan框架，包括视觉编码阶段的全局和局部扫描与令牌合并，以及语言模型中间层的剪枝。

Result: 在四个LVLMs上验证，VScan显著加速推理（LLaVA-NeXT-7B提速2.91倍，FLOPs减少10倍），性能保留95.4%。

Conclusion: VScan通过两阶段令牌减少有效解决冗余问题，优于现有方法，适用于实时部署。
...
TL;DR: Uni-MuMER利用预训练视觉语言模型（VLM）进行手写数学表达式识别（HMER），通过数据驱动的任务集成，实现了新的SOTA性能。
...
Method: 完全微调VLM，结合Tree-CoT、EDL和SC三个任务，注入领域知识。

Result: 在CROHME和HME100K数据集上，性能超越SSAN和Gemini2.5-flash。

Conclusion: Uni-MuMER展示了通用框架在HMER任务中的潜力，并开源了资源。
...
Method: 基于WCBnet生成多视角特征图，结合预训练分割模型（如DeepLab、SegmentAnything和PSPnet）的分割信息，通过贝叶斯推断优化定位结果。

Result: 实验证明该方法能有效定位图像篡改区域，且无需依赖像素级标注。

Conclusion: 该方法为弱监督条件下的图像篡改定位提供了可行解决方案。
...
TL;DR: 论文研究了基于规则的多模态大语言模型（MLLMs）在视觉强化学习中的应用，以拼图任务为实验框架，发现MLLMs通过微调能显著提升性能并泛化到复杂任务，且强化学习比监督微调更具泛化优势。


<details>
  <summary>Details</summary>
Motivation: 探索多模态大语言模型在视觉任务中的表现，特别是基于规则的强化学习方法在感知密集型任务中的独特挑战和潜力。

Method: 使用拼图任务作为结构化实验框架，通过微调和强化学习训练MLLMs，并分析其性能和泛化能力。

Result: MLLMs在拼图任务中从接近随机猜测提升到近乎完美准确率，并能泛化到复杂任务；强化学习比监督微调更具泛化优势。

Conclusion: 研究为基于规则的视觉强化学习提供了重要见解，展示了其在多模态学习中的潜力，但结果可能因任务而异。
...
TL;DR: DeepChest是一种动态任务加权框架，用于多标签胸部X光分类，通过性能驱动的权重机制提高训练效率和准确性。
...
Method: 提出DeepChest框架，基于任务特定损失趋势动态调整权重，无需梯度访问，降低内存使用并加速训练。
...
TL;DR: PerceptiveNet结合对数Gabor卷积层和宽感受野的骨干网络，显著提升了树冠语义分割的精度，并在多个数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决树冠语义分割中因阴影、复杂背景、尺度变化和光谱差异带来的挑战，传统方法和现有深度学习模型难以应对这些复杂性。

Method: 提出PerceptiveNet，包含可训练对数Gabor卷积层和宽感受野的骨干网络，通过实验比较不同卷积层效果，并进行消融研究和混合CNN-Transformer模型评估。

Result: PerceptiveNet在树冠数据集和多个基准数据集上超越现有最优模型，表现出显著的性能提升和跨领域泛化能力。

Conclusion: PerceptiveNet通过创新设计有效解决了树冠分割的复杂性问题，为相关领域提供了高性能解决方案。
...
TL;DR: EndoBench是一个全面的内窥镜多模态大语言模型（MLLMs）评估基准，涵盖多种内窥镜场景和临床任务，旨在填补现有评估的不足。


<details>
  <summary>Details</summary>
Motivation: 当前的内窥镜评估基准覆盖范围有限，无法反映真实世界的多样性和临床工作流程的需求，因此需要更全面的评估工具。

Method: EndoBench包含4种内窥镜场景、12项临床任务及12项子任务、5种视觉提示粒度，共6,832个验证过的VQA对。通过多维度评估框架（如解剖识别、病变分析等）全面测试MLLMs能力。

Result: 实验表明：1）专有MLLMs优于开源和医学专用模型，但仍不及人类专家；2）医学领域监督微调显著提升任务准确性；3）模型性能受提示格式和任务复杂度影响。

Conclusion: EndoBench为内窥镜MLLMs评估设立了新标准，揭示了当前模型与专家临床推理之间的差距，并公开了基准和代码。
...
TL;DR: 论文提出了一种基于物体轨迹的视频标记化方法TrajViT，显著减少了冗余标记并提升了性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于时空块的方法标记视频时会产生过多标记且计算效率低，而最佳标记减少策略在相机移动时效果不佳。

Method: 提出基于全景子对象轨迹的标记化方法TrajViT，通过对比学习训练，提取物体轨迹并转换为语义标记。

Result: TrajViT在多个视频理解任务中显著优于ViT3D，如视频-文本检索任务中top-5召回率提升6%，标记减少10倍。

Conclusion: TrajViT是首个在多样化视频分析任务中始终优于ViT3D的高效编码器，具有鲁棒性和可扩展性。
...
Method: 将彩色图像集表示为四元数Grassmannian上的点，并计算最短距离构建分类框架。
...
Method: 通过聚合持久条形码或拼接拓扑特征向量两种方法构建特征，并在多数据集上比较分类性能。
...
TL;DR: 论文提出了一种基于三角形的推理时优化框架，用于表示场景的几何和外观，相比高斯样条，三角形能提供更丰富的颜色插值和更好的下游任务支持。


<details>
  <summary>Details</summary>
Motivation: 当前广泛使用的高斯样条在3D场景表示中存在局限性，三角形作为一种更灵活的基元，能够更好地支持颜色插值和表面连续性。

Method: 开发了一种针对三角形汤（不连接的半透明三角形基元）的场景优化算法，并在优化过程中引入连接力以鼓励表面连续性。

Result: 在代表性3D重建数据集上的实验表明，该方法在光度和几何结果上具有竞争力。

Conclusion: 三角形基元在3D场景表示中具有潜力，能够提供更好的表达能力和下游任务支持。
...
TL;DR: 论文提出VideoREPA框架，通过Token Relation Distillation（TRD）损失将物理知识注入T2V模型，显著提升视频生成的物理合理性。
...
Method: 提出VideoREPA框架，利用TRD损失从视频理解基础模型中蒸馏物理知识，对齐token级关系。

Result: VideoREPA显著提升了基线方法CogVideoX的物理常识，在相关基准测试中表现优异。

Conclusion: VideoREPA是首个针对T2V模型微调并注入物理知识的REPA方法，能生成更符合直觉物理的视频。
...
TL;DR: D-AR将图像扩散过程重新定义为标准的自回归过程，通过离散化图像为序列令牌，实现与扩散去噪步骤的直接对应。


<details>
  <summary>Details</summary>
Motivation: 探索一种统一的自回归架构，利用扩散模型的特性简化视觉合成任务，并利用大型语言模型潜力。

Method: 设计令牌化器将图像转换为离散令牌序列，利用扩散的粗到细顺序特性，直接应用标准的下一个令牌预测方法。

Result: 在ImageNet基准测试中，使用775M Llama主干和256离散令牌，达到2.09 FID。

Conclusion: D-AR展示了自回归架构在视觉合成中的潜力，为未来研究提供了统一框架的启发。
...
<details>
  <summary>Details</summary>
Motivation: 受统一模型学习实践的启发，旨在简化训练复杂度并提升多模态任务的表现。

Method: 采用可学习查询和轻量级Transformer连接器，结合现有多模态大语言模型和扩散模型。

Result: OpenUni能生成高质量图像并在多个标准基准测试中表现优异，仅需1.1B和3.1B激活参数。
...
TL;DR: ViGoRL是一种通过强化学习训练的视觉语言模型，能够将推理步骤锚定到具体视觉坐标，显著提升视觉推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉推理任务需要模型具备视觉注意力、感知输入解释和空间证据抽象推理能力，而现有方法缺乏显式的地面机制。

Method: ViGoRL采用多轮强化学习框架，动态聚焦任务相关区域，并结合视觉反馈进行细粒度探索。

Result: 在多个视觉推理基准测试中，ViGoRL表现优于监督微调和传统强化学习方法，尤其在视觉搜索和小GUI元素定位任务中表现突出。

Conclusion: 视觉地面强化学习是一种强大的范式，能够赋予模型通用视觉推理能力。
...
TL;DR: 论文提出了VF-Eval基准，用于评估MLLMs在AIGC视频上的能力，发现现有模型表现不佳，并展示了RePrompt实验如何改进视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注自然视频，忽略了AIGC视频的评估，且MLLMs在AIGC视频上的能力尚未充分探索。

Method: 提出VF-Eval基准，包含四项任务，评估13种前沿MLLMs，并进行RePrompt实验。

Result: GPT-4.1表现最佳但仍不理想，显示基准的挑战性；RePrompt实验表明对齐人类反馈可改进视频生成。

Conclusion: VF-Eval填补了AIGC视频评估的空白，并为改进视频生成提供了实用方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究未深入探索视觉提示（prompts）与图像标记之间的相关性和分布关系，本文旨在填补这一空白。

Method: 提出Distribution Aware Visual Prompt Tuning (DA-VPT)框架，通过学习类别相关语义数据的距离度量来指导提示分布。

Result: 在识别和分割任务的主流基准测试中，DA-VPT通过利用语义信息指导提示学习，显著提升了ViT模型的微调效果。

Conclusion: DA-VPT通过优化提示分布，有效提升了ViT模型在下游视觉任务中的性能，证明了语义信息在提示学习中的重要性。
...
TL;DR: 论文提出CLDTracker，一种结合视觉与语言模型的双分支架构，用于解决视觉目标跟踪中的动态变化和语义理解问题，并在多个基准测试中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 传统跟踪器在复杂场景中表现不佳，而视觉语言模型（VLMs）在语义理解方面有潜力，但直接应用于视觉目标跟踪（VOT）存在文本表示不足、特征融合效率低和缺乏时间建模等问题。

Method: 提出CLDTracker，采用双分支架构（文本分支和视觉分支），利用CLIP和GPT-4V等VLMs生成丰富的文本描述，并结合语义和上下文信息。

Result: 在六个标准VOT基准测试中，CLDTracker实现了最优性能。

Conclusion: CLDTracker通过结合视觉与语言模型，解决了VOT中的关键问题，展示了其在复杂场景中的有效性。
...
TL;DR: SLIMP通过嵌套对比学习方法结合图像和元数据，提升皮肤病变分类任务的表现。


<details>
  <summary>Details</summary>
Motivation: 仅依赖图像的黑色素瘤检测和皮肤病变分类面临成像条件和临床背景缺失的挑战，SLIMP受临床医生综合考虑患者病史和病变外观的启发。

Method: SLIMP结合皮肤病变的外观、元数据及患者级别的医疗记录，通过嵌套对比学习捕捉复杂关系。
...
<details>
  <summary>Details</summary>
Motivation: 传统神经渲染方法需要已知相机姿态和逐场景优化，而现有前馈方法在密集视图下计算负担重。AnySplat旨在解决这些问题，实现高效、高质量的视角合成。
...
TL;DR: FMG-Det提出了一种简单高效的方法，用于在噪声标注下训练目标检测模型，结合多实例学习和基础模型预处理，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 目标检测任务中标注边界的主观性导致数据质量难以保证，噪声标注会显著降低模型性能，尤其在少样本场景下。

Method: 结合多实例学习（MIL）框架和基于基础模型的预处理流程，修正标注后再训练，同时对检测头进行微调。

Result: 在多个数据集上实现了最先进的性能，适用于标准及少样本场景，且方法更简单高效。

Conclusion: FMG-Det通过预处理和模型调整，有效解决了噪声标注问题，提升了目标检测模型的鲁棒性和性能。
...
TL;DR: PixelThink通过结合任务难度和模型不确定性调节推理生成，提高了推理效率和分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在泛化性和推理效率上表现不足，需改进以应对复杂场景。

Method: 提出PixelThink，整合外部任务难度和内部模型不确定性，调节推理生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有前馈3D高斯泼溅模型因编码器容量有限，难以处理多视角输入，导致性能下降或内存消耗过大。

Method: 提出ZPressor模块，将多视角分为锚点和支持集，利用交叉注意力压缩支持集信息到锚点，形成潜在状态Z。

Result: ZPressor使模型能处理100+视角输入，在DL3DV-10K和RealEstate10K基准测试中提升性能和鲁棒性。

Conclusion: ZPressor有效解决了3D高斯泼溅模型的扩展性问题，显著提升了多视角输入下的表现。
...
Method: 使用文本到图像或GPT-4生成关键帧，通过图优化问题合成舞蹈，并利用视频扩散模型生成中间帧。

Result: 仅需6个输入关键帧即可生成长达30秒的动物舞蹈视频，适用于多种动物和音乐。
...
TL;DR: LayerPeeler是一种新的分层图像矢量化方法，通过渐进式简化解决遮挡区域的挑战，生成完整路径和连贯层结构的矢量图形。


<details>
  <summary>Details</summary>
Motivation: 现有图像矢量化工具在遮挡区域处理上表现不佳，生成不完整或碎片化的形状，限制了编辑灵活性。
...
TL;DR: MAGREF是一个基于掩码引导的统一框架，用于多参考视频生成，通过动态掩码机制和像素级通道拼接实现高质量、一致的多主体视频合成。
...
Method: 提出区域感知动态掩码机制和像素级通道拼接机制，支持灵活处理多种主体并保持外观特征。

Result: 模型在多主体场景中表现出色，生成质量优于现有开源和商业基线。

Conclusion: MAGREF为可扩展、可控、高保真的多主体视频合成提供了有效解决方案。
...
TL;DR: 提出了一种基于预训练生成扩散模型的新框架，用于增强低光照原始图像，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有回归模型在低光照条件下导致的图像过度平滑或色彩失真的问题。

Method: 利用预训练的生成扩散模型，结合相机ISP，增强低光照原始图像。

Result: 在三个低光照原始图像基准测试中，表现优于现有技术。

Conclusion: 该方法在感知质量上显著提升，为低光照摄影提供了更优解决方案。
...
TL;DR: SOYO是一个轻量级框架，通过高斯混合压缩器和域特征重采样器改进参数隔离域增量学习（PIDIL）中的域选择问题，并在多任务中验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络（DNNs）在动态变化的真实世界数据分布中表现不佳，现有PIDIL方法在参数选择准确性上存在不足，尤其是在多域和多类情况下。
...
Conclusion: SOYO通过改进域选择和特征提取，显著提升了PIDIL的性能，适用于多种任务，代码已开源。
...
TL;DR: TrustVLM是一个无需训练的框架，旨在提升视觉语言模型（VLM）预测的可信度，通过利用图像嵌入空间的特性改进误分类检测。


<details>
  <summary>Details</summary>
Motivation: VLM在零样本和迁移学习中表现优异，但在安全关键领域容易产生高置信度的错误预测，存在严重风险。

Method: 提出一种基于图像嵌入空间的置信度评分函数，利用模态间隙和概念的区分性表示。
...
TL;DR: Spatial-MLLM是一个新框架，通过纯2D观察实现视觉空间推理，无需依赖3D或2.5D数据。


<details>
  <summary>Details</summary>
Motivation: 现有3D MLLMs依赖额外3D或2.5D数据，限制了其在仅有2D输入场景中的应用。

Method: 采用双编码器架构：一个预训练的2D视觉编码器提取语义特征，一个空间编码器提取3D结构特征，并通过连接器整合。提出空间感知帧采样策略。
...
TL;DR: MMSI-Bench是一个专注于多图像空间智能的VQA基准测试，旨在评估MLLMs在复杂物理世界中的表现。现有基准仅测试单图像关系，而MMSI-Bench通过1,000个挑战性问题填补了这一空白。实验显示，当前MLLMs表现远低于人类水平，揭示了未来研究的巨大空间。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法评估多图像空间推理能力，而这是MLLMs在现实世界中应用的关键。因此，需要一个新的基准来填补这一空白。

Method: 研究人员耗时300多小时，从12万张图像中精心设计1,000个多选问题，并配以干扰项和逐步推理过程。评估了34个开源和专有MLLMs。

Result: 最强开源模型准确率约30%，OpenAI的o3模型达40%，而人类为97%。结果凸显了MMSI-Bench的挑战性和未来研究的潜力。

Conclusion: MMSI-Bench为多图像空间智能研究提供了重要工具，通过错误分析揭示了四大失败模式，为未来改进指明了方向。
...
TL;DR: ThinkGeo是一个用于评估LLM驱动代理在遥感任务中工具使用能力的基准测试，涵盖多种实际应用场景。


<details>
  <summary>Details</summary>
Motivation: 现有评估多关注通用或多模态场景，缺乏针对复杂遥感任务的领域特定基准。

Method: 设计ThinkGeo基准，包含人类策划的查询，采用ReAct式交互循环，评估开源和闭源LLM。
...
TL;DR: Rooms from Motion (RfM) 是一种基于物体中心的3D目标检测方法，通过未定位图像生成全局语义3D物体地图，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有3D目标检测方法依赖全局视角和已知相机位姿，而RfM旨在通过未定位图像实现定位与地图构建。

Method: 使用基于3D框的物体中心匹配器替代传统2D关键点匹配，估计相机位姿和物体轨迹，生成全局3D物体地图。

Result: 在CA-1M和ScanNet++上表现优于基于点云或多视图的方法，地图质量更高。

Conclusion: RfM提供了一种稀疏且参数化的物体中心表示，适用于场景级3D目标检测。
...
TL;DR: Impromptu VLA数据集解决了自动驾驶中Vision-Language-Action模型在非结构化场景中的不足，通过8万多个精心标注的视频片段提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在非结构化极端场景中表现不佳，缺乏针对性数据集。

Method: 构建了包含8万多个视频片段的Impromptu VLA数据集，基于四类非结构化场景分类，并标注了规划导向的问答和动作轨迹。

Result: 使用该数据集训练的模型在NeuroNCAP和nuScenes等基准测试中性能显著提升，接近SOTA水平。
...
Method: 利用扩散变换器中概念特定特征的早期空间激活特性，生成解耦的潜在掩码，并在特定区域内混合LoRA权重。

Result: 实验表明，LoRAShop在身份保留方面优于基线方法，且无需重新训练或外部约束。

Conclusion: LoRAShop为个性化扩散模型提供了实用的编辑工具，推动了视觉叙事和创意迭代的发展。
...
TL;DR: 论文提出两种针对草图数据的组件，通过跨模态知识蒸馏和基于RL的画布选择器，显著降低计算量（FLOPs减少99.37%），同时保持精度。


<details>
  <summary>Details</summary>
Motivation: 现有高效轻量级模型专为照片设计，不适用于草图数据，因此需要针对草图的高效推理方法。

Method: 1. 跨模态知识蒸馏网络，将照片高效网络适配草图；2. 基于RL的动态画布选择器，根据草图抽象程度调整计算量。

Result: FLOPs从40.18G降至0.254G（减少99.37%），精度几乎不变（33.03% vs 32.77%）。
...
TL;DR: Argus是一种新型视觉注意力机制，通过对象中心化接地提升多模态大语言模型在视觉中心任务中的表现。
...
Result: 在多种基准测试中，Argus在多模态推理和对象接地任务中表现优异。

Conclusion: Argus验证了语言引导的视觉兴趣区域参与对MLLMs的重要性，推动了视觉中心多模态智能的发展。
...
TL;DR: TextRegion结合图像文本模型和SAM2，生成文本对齐区域标记，提升详细视觉理解能力，适用于多种下游任务。


<details>
  <summary>Details</summary>
Motivation: 现有图像文本模型在详细视觉理解上表现不足，而SAM2能提供精确空间边界，结合两者优势以提升性能。

Method: 提出TextRegion框架，无需训练，结合图像文本模型和SAM2生成文本对齐区域标记。

Result: 在多项任务中表现优于或媲美现有免训练方法，兼容多种图像文本模型。

Conclusion: TextRegion简单有效，扩展性强，适用于开放词汇视觉任务。
...
### [255] [Decomposing Elements of Problem Solving: What "Math" Does RL Teach?](https://arxiv.org/abs/2505.22756)
*Tian Qin,Core Francisco Park,Mujin Kwun,Aaron Walsman,Eran Malach,Nikhil Anand,Hidenori Tanaka,David Alvarez-Melis*

Main category: cs.AI

TL;DR: 论文提出将数学推理任务分解为计划、执行和验证三个基本能力，发现GRPO主要提升执行能力，但RL训练模型在新问题上遇到‘覆盖墙’问题。通过合成任务验证RL主要增强执行稳健性，并探索了克服覆盖墙的条件。


<details>
  <summary>Details</summary>
Motivation: 现有准确性指标无法细粒度评估LLM的推理能力，需分解问题解决能力以更深入理解。

Method: 将数学问题解决分解为计划、执行和验证能力，并通过合成任务验证RL的作用。

Result: GRPO主要提升执行能力（温度蒸馏现象），但RL模型在新问题上因计划能力不足遇到覆盖墙。合成任务证实RL增强执行稳健性，并探索了克服覆盖墙的条件。

Conclusion: RL能增强LLM的执行稳健性，但需改进探索和泛化能力以克服覆盖墙，为未来研究提供了方向。
...
### [256] [Enhancing Study-Level Inference from Clinical Trial Papers via RL-based Numeric Reasoning](https://arxiv.org/abs/2505.22928)
*Massimiliano Pronesti,Michela Lorandi,Paul Flanagan,Oisin Redmon,Anya Belz,Yufang Hou*

Main category: cs.AI

TL;DR: 论文提出了一种基于定量推理的方法，通过提取结构化数值证据并应用领域知识逻辑，改进了医学系统综述中的证据提取和结论推断。


<details>
  <summary>Details</summary>
Motivation: 自动化医学系统综述中的证据提取和结论推断存在瓶颈，现有方法依赖浅层文本线索，未能捕捉专家评估背后的数值推理。

Method: 开发了一个数值推理系统，包括数值数据提取模型和效应估计组件，采用监督微调和强化学习训练模型。

Result: 在CochraneForest基准测试中，最佳方法（强化学习训练的小规模数值提取模型）比检索系统和大型语言模型性能提升显著。
...
### [257] [Be.FM: Open Foundation Models for Human Behavior](https://arxiv.org/abs/2505.23058)
...
TL;DR: Be.FM是一种基于开源大语言模型的行为基础模型，用于理解和预测人类行为，并在多项任务中表现出色。
...
Method: 基于开源大语言模型，通过多样化行为数据微调构建Be.FM。
...
### [258] [Infi-MMR: Curriculum-based Unlocking Multimodal Reasoning via Phased Reinforcement Learning in Multimodal Small Language Models](https://arxiv.org/abs/2505.23091)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型（LLMs）在推理能力上取得显著进展，但多模态小语言模型（MSLMs）面临数据集稀缺、视觉处理导致推理能力下降以及强化学习可能产生错误推理等问题。

Method: 设计了一个三阶段框架：1）基础推理激活，利用高质量文本数据集；2）跨模态推理适应，使用带字幕的多模态数据；3）多模态推理增强，通过无字幕多模态数据减少语言偏见。

Result: Infi-MMR-3B在多模态数学推理（如MathVerse、MathVision、OlympiadBench）和通用推理（MathVista）任务上达到最先进水平。
...
### [259] [Socratic-PRMBench: Benchmarking Process Reward Models with Systematic Reasoning Patterns](https://arxiv.org/abs/2505.23474)
*Xiang Li,Haiyang Yu,Xinghua Zhang,Ziyang Huang,Shizhu He,Kang Liu,Jun Zhao,Fei Huang,Yongbin Li*

Main category: cs.AI

TL;DR: 论文介绍了Socratic-PRMBench，一个用于系统评估过程奖励模型（PRMs）在六种推理模式下的新基准。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注逐步正确性，缺乏对PRMs在不同推理模式下系统评估的需求。

Method: 提出Socratic-PRMBench，包含2995条带有缺陷的推理路径，覆盖六种推理模式。

Result: 实验发现现有PRMs在多种推理模式下的评估能力存在显著不足。
...
### [260] [ZeroGUI: Automating Online GUI Learning at Zero Human Cost](https://arxiv.org/abs/2505.23762)
...
TL;DR: ZeroGUI是一种在线学习框架，通过自动任务生成和奖励估计，无需人工标注即可训练GUI代理，显著提升性能。
...
Result: 在UI-TARS和Aguvis上验证，ZeroGUI显著提升了OSWorld和AndroidLab环境中的性能。
...
### [261] [Conversational Alignment with Artificial Intelligence in Context](https://arxiv.org/abs/2505.22907)
...
Conclusion: 需要进一步研究以克服LLM在对话对齐中的限制。
...
### [262] [SWE-bench Goes Live!](https://arxiv.org/abs/2505.23419)
...
TL;DR: SWE-bench-Live是一个动态更新的基准测试，旨在解决现有静态基准测试的局限性，通过自动化流程和实时数据支持LLM在软件修复任务中的评估。
...
Method: 提出SWE-bench-Live，包含1,319个实时GitHub问题任务，配备Docker镜像和自动化流程（\method），实现可扩展和持续更新。

Result: 实验显示，现有LLM和代理框架在动态基准上的表现显著低于静态基准，揭示了性能差距。

Conclusion: SWE-bench-Live为动态、真实的软件开发环境提供了抗污染、多样化的评估工具，推动LLM研究的严谨性。
...
### [263] [Identity resolution of software metadata using Large Language Models](https://arxiv.org/abs/2505.23500)
*Eva Martín del Pico,Josep Lluís Gelpí,Salvador Capella-Gutiérrez*

Main category: cs.SE

TL;DR: 本文探讨了研究软件的重要性，并评估了大型语言模型在软件元数据身份解析任务中的表现，以支持生命科学领域研究软件的FAIR化。


<details>
  <summary>Details</summary>
Motivation: 研究软件在科研中至关重要，但其重要性常被忽视。本文旨在通过整合和分析软件元数据，提升研究软件的可见性和可持续性。

Method: 利用指令调优的大型语言模型进行软件元数据身份解析，并通过人工标注的黄金标准对模型进行基准测试。

Result: 提出的基于一致性的代理方法实现了高精度和统计稳健性，同时揭示了当前模型的局限性。

Conclusion: 尽管自动化语义判断在跨注册表和存储库的FAIR化软件元数据中面临挑战，但本文的方法为研究软件的整合和监控提供了可行路径。
...
### [264] [GSO: Challenging Software Optimization Tasks for Evaluating SWE-Agents](https://arxiv.org/abs/2505.23671)
*Manish Shetty,Naman Jain,Jinjian Liu,Vijay Kethanaboyina,Koushik Sen,Ion Stoica*

Main category: cs.SE

TL;DR: GSO是一个用于评估语言模型在高性能软件开发中能力的基准测试，通过自动化流程生成并执行性能测试，发现当前SWE-Agents成功率低于5%。


<details>
  <summary>Details</summary>
Motivation: 开发高性能软件需要专业知识，因此需要评估语言模型在此任务中的能力。

Method: 开发自动化管道生成性能测试，分析代码库提交历史以识别优化任务，并提供代码库和性能测试作为规范。

Result: 领先的SWE-Agents成功率低于5%，且改进有限。定性分析揭示了失败模式，如低级语言困难和瓶颈定位问题。

Conclusion: GSO基准测试揭示了当前语言模型在高性能软件开发中的局限性，并提供了未来研究的资源。
...
### [265] [Physiology-Informed Generative Multi-Task Network for Contrast-Free CT Perfusion](https://arxiv.org/abs/2505.22673)
...
TL;DR: 论文提出了一种名为MAGIC的深度学习框架，通过生成式人工智能和生理信息，将非对比CT图像映射为多模态无对比CT灌注图像，解决了传统CT灌注成像中对比剂带来的副作用和高成本问题。


<details>
  <summary>Details</summary>
Motivation: 传统CT灌注成像（CTP）依赖对比剂，可能导致过敏反应和高昂成本，因此需要一种无对比剂的替代方案。

Method: MAGIC框架结合生成式AI和生理信息，将非对比CT图像转换为多模态无对比CT灌注图像，并通过生理特征增强图像保真度。

Result: MAGIC在视觉质量和诊断准确性上表现优异，优于传统对比剂灌注成像，且对脑灌注异常具有鲁棒性。
...
### [266] [TailorSQL: An NL2SQL System Tailored to Your Query Workload](https://arxiv.org/abs/2505.23039)
...
TL;DR: TailorSQL利用历史查询负载信息提升NL2SQL的准确性和效率，相比现有技术提升2倍执行准确率。


<details>
  <summary>Details</summary>
Motivation: 现有NL2SQL技术忽略了历史查询负载中的有用信息（如常见连接路径和表列语义），而这些信息能显著提升翻译准确性和效率。

Method: TailorSQL通过结合数据库模式和历史查询负载信息，利用预训练大语言模型生成更准确的SQL查询。

Result: TailorSQL在标准化基准测试中实现了高达2倍的执行准确率提升。

Conclusion: TailorSQL证明了利用历史查询负载信息可以显著改进NL2SQL的性能，为智能数据应用提供了更高效的解决方案。
...
### [267] [Quality assessment of 3D human animation: Subjective and objective evaluation](https://arxiv.org/abs/2505.23301)
*Rim Rekik,Stefanie Wuhrer,Ludovic Hoyet,Katja Zibrek,Anne-Hélène Olivier*

Main category: cs.GR

TL;DR: 论文提出了一种基于数据驱动的虚拟人动画质量评估方法，通过用户研究生成数据集并训练线性回归模型，其性能优于现有深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 虚拟人动画在虚拟和增强现实中有广泛应用，但非参数化生成的动画质量评估尚未解决。

Method: 生成带有主观真实感评分的虚拟人动画数据集，并训练线性回归模型预测感知评分。

Result: 线性回归模型在数据集上达到90%的相关性，优于现有深度学习方法。

Conclusion: 提出的数据驱动框架为虚拟人动画质量评估提供了有效解决方案。
...
### [268] [ConnectomeDiffuser: Generative AI Enables Brain Network Construction from Diffusion Tensor Imaging](https://arxiv.org/abs/2505.22683)
*Xuhang Chen,Michael Kwok-Po Ng,Kim-Fung Tsang,Chi-Man Pun,Shuqiang Wang*

Main category: q-bio.NC

TL;DR: 提出了一种名为ConnectomeDiffuser的新型扩散框架，用于从DTI数据中自动构建脑网络，解决了现有方法的局限性，并提高了诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在构建结构脑网络时存在主观性、工作流程繁琐及无法捕捉复杂拓扑特征等问题，需要一种更高效、自动化的解决方案。

Method: 结合模板网络、扩散模型和图卷积网络分类器，从DTI数据中提取拓扑特征并生成全面的脑网络。

Result: 实验验证表明，该方法在捕捉结构连接性和病理信息方面表现优越，显著优于其他脑网络方法。

Conclusion: ConnectomeDiffuser为神经退行性疾病的诊断和治疗监测提供了更准确、通用的测量框架。
...
### [269] [CF-DETR: Coarse-to-Fine Transformer for Real-Time Object Detection](https://arxiv.org/abs/2505.23317)
*Woojin Shin,Donghwa Kang,Byeongyun Park,Brent Byunghoon Kang,Jinkyu Lee,Hyeongboo Baek*

Main category: eess.SY

TL;DR: CF-DETR通过粗到细的Transformer架构和实时调度框架NPFP**，解决了多DETR任务在自动驾驶中的实时性和准确性挑战。
...
Method: 提出CF-DETR，采用粗到细推理、选择性细推理和多级批量推理策略，结合NPFP**调度框架动态调整资源分配。

Result: 在服务器、GPU嵌入式平台和实际AV平台上，CF-DETR成功满足严格的时间保证，并显著提高了检测精度。

Conclusion: CF-DETR通过动态调整和专用调度，有效平衡了实时性和准确性，适用于自动驾驶场景。
...
### [270] [AgentAlign: Navigating Safety Alignment in the Shift from Informative to Agentic Large Language Models](https://arxiv.org/abs/2505.23020)
*Jinchuan Zhang,Lu Yin,Yan Zhou,Songlin Hu*

Main category: cs.CR

TL;DR: 论文提出了AgentAlign框架，通过抽象行为链合成安全对齐数据，显著提升LLM代理的安全性，同时保持其有用性。


<details>
  <summary>Details</summary>
Motivation: LLM代理能力的提升使其易受恶意利用，现有方法在安全对齐上存在不足。

Method: 利用抽象行为链在模拟环境中生成真实可执行的指令，同时合成良性指令以平衡安全性和有用性。

Result: 在AgentHarm评估中，安全性提升35.8%至79.5%，且对有用性影响极小或有所提升。

Conclusion: AgentAlign框架有效解决了LLM代理的安全对齐问题，代码和数据集已开源。
...
### [271] [Disrupting Vision-Language Model-Driven Navigation Services via Adversarial Object Fusion](https://arxiv.org/abs/2505.23266)
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击未考虑服务计算环境中的可靠性和服务质量（QoS），而大型语言模型（LLM）和视觉语言模型（VLM）的集成在提升导航系统的同时引入了漏洞。

Method: AdvOF通过精确聚合和对齐2D和3D空间中的目标对象位置，定义并渲染对抗性对象，并通过物理属性和VLM感知的协同优化生成对抗性对象。
...
### [272] [FlashFormer: Whole-Model Kernels for Efficient Low-Batch Inference](https://arxiv.org/abs/2505.22758)
*Aniruddha Nrusimha,William Brandon,Mayank Mishra,Yikang Shen,Rameswar Panda,Jonathan Ragan-Kelley,Yoon Kim*

Main category: cs.LG

TL;DR: FlashFormer是一种针对低批量推理优化的Transformer内核，显著提升了单批量推理速度。


<details>
  <summary>Details</summary>
Motivation: 现代大语言模型的计算需求引发了对专用内核的兴趣，但现有内核主要针对大批量训练和推理优化，而低批量推理在边缘部署和延迟敏感应用中仍很重要。

Method: 提出FlashFormer，一种针对单批量推理优化的概念验证内核。

Result: 在不同模型大小和量化设置下，FlashFormer相比现有最优推理内核实现了显著加速。

Conclusion: FlashFormer为低批量推理场景提供了一种高效解决方案，适用于边缘部署和延迟敏感应用。
...
### [273] [DenoiseRotator: Enhance Pruning Robustness for LLMs via Importance Concentration](https://arxiv.org/abs/2505.23049)
*Tianteng Gu,Bei Liu,Bo Xiao,Ke Zeng,Jiacheng Liu,Yanmin Qian*

Main category: cs.LG

TL;DR: 论文提出了一种新方法DenoiseRotator，通过重新分配参数重要性来增强模型剪枝的鲁棒性，显著减少了性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝方法主要关注单个权重的重要性估计，难以保留模型的关键能力，尤其是在半结构化稀疏约束下性能下降明显。

Method: 通过最小化归一化重要性分数的信息熵，将重要性集中在更小的权重子集上，并结合可学习的正交变换（DenoiseRotator）实现。

Result: 在LLaMA3、Qwen2.5和Mistral模型上，DenoiseRotator显著提升了困惑度和零样本准确率，例如在LLaMA3-70B上，困惑度差距减少了58%。

Conclusion: DenoiseRotator是一种模型无关的方法，可与现有剪枝技术无缝结合，显著提升剪枝后的模型性能。
...
### [274] [MAP: Revisiting Weight Decomposition for Low-Rank Adaptation](https://arxiv.org/abs/2505.23094)
*Chongjie Si,Zhiyi Shi,Yadao Wang,Xiaokang Yang,Susanto Rahardja,Wei Shen*

Main category: cs.LG

TL;DR: MAP提出了一种新的参数高效微调框架，通过将权重矩阵分解为方向和大小，显著提升了现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA和DoRA）在方向定义上缺乏几何基础，限制了其灵活性和可解释性。
...
### [275] [Does Machine Unlearning Truly Remove Model Knowledge? A Framework for Auditing Unlearning in LLMs](https://arxiv.org/abs/2505.23270)
...
TL;DR: 论文提出了一种全面的机器遗忘评估框架，包括基准数据集、遗忘算法和审计方法，并引入了一种基于中间激活扰动的新技术。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的训练数据可能包含敏感或受版权保护的内容，而现有遗忘算法的有效性难以评估。

Method: 提出了一个包含三个基准数据集、六种遗忘算法和五种基于提示的审计方法的框架，并开发了一种基于中间激活扰动的新技术。

Result: 通过多种审计算法评估了不同遗忘策略的有效性和鲁棒性，新技术弥补了仅依赖输入输出审计的局限性。

Conclusion: 该框架为机器遗忘算法的评估提供了全面工具，新技术提升了审计的准确性。
...
### [276] [Rethinking Regularization Methods for Knowledge Graph Completion](https://arxiv.org/abs/2505.23442)
*Linyu Li,Zhi Jin,Yuanpeng He,Dongming Jin,Haoran Duan,Zhengwei Tao,Xuan Zhang,Jiandong Li*

Main category: cs.LG

TL;DR: 本文重新思考了知识图谱补全（KGC）中正则化方法的应用，提出了一种新的稀疏正则化方法（SPR），通过选择性惩罚显著特征组件，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有KGC模型未能充分利用正则化的潜力，本文旨在探索正则化在提升模型性能中的作用。

Method: 提出了一种稀疏正则化方法（SPR），选择性惩罚嵌入向量中的显著特征组件，忽略噪声。

Result: 实验表明，SPR优于其他正则化方法，显著提升了KGC模型的性能上限。

Conclusion: SPR方法通过优化正则化策略，为KGC模型提供了更高的性能突破空间。
...
### [277] [Domain-Aware Tensor Network Structure Search](https://arxiv.org/abs/2505.23537)
*Giorgos Iacovides,Wuyang Zhou,Chao Li,Qibin Zhao,Danilo Mandic*

Main category: cs.LG

TL;DR: 提出了一种基于大型语言模型（LLM）的TN-SS框架tnLLM，结合领域信息直接预测最优张量网络结构，减少计算开销并提升透明度。
...
Method: 利用LLM的推理能力，通过领域感知提示管道预测张量网络结构，并结合领域信息优化目标函数。

Result: tnLLM在较少函数评估下达到与SOTA算法相当的优化效果，并能加速其他方法的收敛。
...
### [278] [Segment Policy Optimization: Effective Segment-Level Credit Assignment in RL for Large Language Models](https://arxiv.org/abs/2505.23564)
*Yiran Guo,Lijie Xu,Jie Liu,Dan Ye,Shuang Qiu*

Main category: cs.LG

TL;DR: 论文提出了一种名为SPO的新强化学习框架，通过中粒度段级优势估计，平衡了精细信用分配与估计效率，显著提升了大型语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在优势估计粒度上存在两极分化：Token级方法（如PPO）因训练准确批评模型困难而估计不准确；轨迹级方法（如GRPO）仅依赖最终奖励导致信用分配不精确。

Method: SPO框架包含三个创新策略：灵活段划分、准确段优势估计和基于段优势的策略优化（包括概率掩码策略）。具体实例化为SPO-chain（短链式思维）和SPO-tree（长链式思维）。

Result: SPO-chain在GSM8K上比PPO和GRPO准确率提升6-12个百分点；SPO-tree在MATH500上比GRPO提升7-11个百分点，且显著降低MC估计成本。

Conclusion: SPO通过中粒度优势估计，有效平衡了信用分配精度与计算效率，显著提升了语言模型的推理能力，代码已开源。
...
Method: 提出OPO算法，强调精确的在线策略训练，并引入最优奖励基线以减少梯度方差。
...
### [280] [Differential Information: An Information-Theoretic Perspective on Preference Optimization](https://arxiv.org/abs/2505.23761)
*Yunjae Won,Hyunji Lee,Hyeonbin Hwang,Minjoon Seo*

Main category: cs.LG

TL;DR: 本文通过Differential Information Distribution（DID）理论填补了Direct Preference Optimization（DPO）中log-ratio奖励参数化的理论空白，揭示了其最优性，并分析了偏好数据与策略行为的关系。


<details>
  <summary>Details</summary>
Motivation: DPO在语言模型对齐人类偏好方面表现优异，但其理论基础不完整，尤其是log-ratio奖励参数化的合理性尚未充分解释。

Method: 利用DID理论，分析偏好标签如何编码从参考策略到目标策略的差异信息，并推导log-ratio奖励的最优性。同时研究熵对策略分布的影响。

Result: 发现偏好数据需编码差异信息，且与log-margin有序策略的隐式假设相关。低熵差异信息强化策略分布，高熵差异信息平滑策略。

Conclusion: 研究为DPO目标、偏好数据结构和策略行为提供了统一的理论视角，并验证了高熵差异信息对通用指令跟随的重要性。
...
### [281] [Test-time augmentation improves efficiency in conformal prediction](https://arxiv.org/abs/2505.22764)
*Divya Shanmugam,Helen Lu,Swami Sankaranarayanan,John Guttag*

Main category: cs.LG

TL;DR: 测试时增强（TTA）可减少保形分类器生成的预测集大小，提高效率。


<details>
  <summary>Details</summary>
Motivation: 保形分类器生成的预测集通常过大且缺乏信息，需改进其效率。

Method: 结合测试时增强（TTA）技术，无需重新训练模型，适用于多种保形评分方法。

Result: 平均减少预测集大小10%-14%，在多种数据集、模型和分布偏移下验证有效性。
...
### [282] [Number of Clusters in a Dataset: A Regularized K-means Approach](https://arxiv.org/abs/2505.22991)
*Behzad Kamgar-Parsi,Behrooz Kamgar-Parsi*

Main category: cs.LG

TL;DR: 论文探讨了正则化k-means算法中如何确定关键超参数λ的严格界限，假设聚类为理想情况，并分析了加性和乘性正则化对解的影响。


<details>
  <summary>Details</summary>
Motivation: 在无标签数据集中找到有意义的聚类数量是许多应用中的重要问题，但目前缺乏设置正则化超参数λ的指导原则。

Method: 假设聚类为理想情况（d维球体），推导λ的严格界限，并比较加性和乘性正则化k-means算法的解。

Result: 实验表明加性正则化常产生多解，而乘性正则化与加性正则化的共识可减少解的模糊性。

Conclusion: 论文为λ的设置提供了理论支持，并通过实验验证了正则化k-means算法在非理想情况下的性能。
...
### [283] [Diverse Prototypical Ensembles Improve Robustness to Subpopulation Shift](https://arxiv.org/abs/2505.23027)
...
<details>
  <summary>Details</summary>
Motivation: 子群体分布偏移会显著降低机器学习模型的性能，而现有方法依赖于对子群体数量和性质的假设以及群体成员注释，这些在实际数据中往往不可得。

Method: 提出用多样化的原型分类器集合替代标准线性分类层，每个分类器专注于不同的特征和样本，以自适应地捕捉子群体风险。

Result: 在九个真实数据集上的实验表明，DPEs在最差群体准确率上优于现有方法。

Conclusion: DPEs是一种无需依赖子群体假设的有效方法，能够显著提升模型在子群体分布偏移下的性能。
...
### [284] [Pseudo Multi-Source Domain Generalization: Bridging the Gap Between Single and Multi-Source Domain Generalization](https://arxiv.org/abs/2505.23173)
*Shohei Enomoto*

Main category: cs.LG

TL;DR: 论文提出了一种名为PMDG的新框架，通过风格迁移和数据增强从单源域生成伪多源域，以解决多源域泛化（MDG）在实际应用中的局限性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在部署到与训练数据分布不同的环境时性能下降，而多源域泛化（MDG）因多域数据集构建成本高而难以实用。

Method: 提出PMDG框架，通过风格迁移和数据增强从单源域生成伪多源域，模拟多域数据集，用于现有MDG算法。
...
### [285] [Buffer-free Class-Incremental Learning with Out-of-Distribution Detection](https://arxiv.org/abs/2505.23412)
*Srishti Gupta,Daniele Angioni,Maura Pintor,Ambra Demontis,Lea Schönherr,Battista Biggio,Fabio Roli*

Main category: cs.LG

TL;DR: 论文提出了一种无需记忆缓冲区的后验OOD检测方法，用于开放世界中的类增量学习，性能与基于缓冲区的方法相当或更优。


<details>
  <summary>Details</summary>
Motivation: 解决开放世界场景中类增量学习的挑战，尤其是避免使用记忆缓冲区带来的隐私、可扩展性和训练时间问题。
...
### [286] [Network Inversion for Uncertainty-Aware Out-of-Distribution Detection](https://arxiv.org/abs/2505.23448)
...
Method: 通过引入一个“垃圾”类，初始填充高斯噪声表示离群输入，结合网络反演和迭代训练优化分类器。

Result: 模型能有效检测并拒绝OOD样本，同时提供预测置信度用于不确定性估计。
...
### [287] [Muddit: Liberating Generation Beyond Text-to-Image with a Unified Discrete Diffusion Model](https://arxiv.org/abs/2505.23606)
...
TL;DR: Muddit是一种统一的离散扩散Transformer，通过结合预训练文本到图像骨干和轻量级文本解码器，实现快速并行生成文本和图像。
...
Method: 采用离散扩散Transformer，结合预训练视觉先验和轻量级文本解码器。

Result: 在质量和效率上优于或接近更大的自回归模型。
...
### [288] [Merge-Friendly Post-Training Quantization for Multi-Target Domain Adaptation](https://arxiv.org/abs/2505.23651)
*Juncheol Shin,Minsang Seok,Seonggon Kim,Eunhyeok Park*

Main category: cs.LG

TL;DR: 研究提出了一种新型后训练量化方法HDRQ，用于解决量化对模型合并的挑战，确保量化过程对多目标域适应的模型合并影响最小。


<details>
  <summary>Details</summary>
Motivation: 量化在目标特定数据上的应用限制了兴趣域并引入离散化效应，使得模型合并变得复杂。

Method: 通过误差屏障分析量化影响，提出HDRQ方法，结合Hessian和远距离正则化量化，优化量化过程。

Result: HDRQ方法在量化过程中最小化对源预训练模型的偏离，并平滑损失表面以促进模型合并。

Conclusion: HDRQ是首个针对量化模型合并的研究，实验证实其有效性。
...
### [289] [REOrdering Patches Improves Vision Models](https://arxiv.org/abs/2505.23751)
*Declan Kutscher,David M. Chan,Yutong Bai,Trevor Darrell,Ritwik Gupta*

Main category: cs.LG

TL;DR: 论文提出了一种名为REOrder的两阶段框架，用于发现任务最优的补丁顺序，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现代长序列变换器对补丁顺序敏感，而固定顺序（如行优先）可能影响性能，因此需要探索更优的补丁排列方式。

Method: REOrder框架：1）通过评估补丁序列的可压缩性生成信息论先验；2）使用REINFORCE优化Plackett-Luce策略学习排列策略。

Result: 在ImageNet-1K上，REOrder比行优先顺序提升3.01%的top-1准确率；在Functional Map of the World上提升13.35%。

Conclusion: REOrder通过优化补丁顺序显著提升了模型性能，证明了补丁顺序对变换器模型的重要性。
...
### [290] [Large Language Models for Depression Recognition in Spoken Language Integrating Psychological Knowledge](https://arxiv.org/abs/2505.22863)
*Yupei Li,Shuaijie Shao,Manuel Milling,Björn W. Schuller*

Main category: cs.HC

TL;DR: 论文提出了一种结合音频特征和心理知识的LLM多模态抑郁症检测方法，显著提升了诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 现有DNN和LLM在抑郁症检测中效果有限，缺乏对非文本线索和心理知识的整合。

Method: 使用Wav2Vec提取音频特征，结合LLM处理，并引入心理知识问答集增强模型。

Result: 在DAIC-WOZ数据集上，MAE和RMSE显著优于基线。

Conclusion: 多模态结合心理知识的方法有效提升了抑郁症检测性能。
...
### [291] [Human Empathy as Encoder: AI-Assisted Depression Assessment in Special Education](https://arxiv.org/abs/2505.23631)
*Boning Zhao*

Main category: cs.HC

TL;DR: 论文提出了一种名为HEAE的人本AI框架，通过结合学生叙述文本和教师衍生的‘共情向量’，提升抑郁严重程度评估的透明度和社会责任。


<details>
  <summary>Details</summary>
Motivation: 在特殊教育等敏感环境中评估学生抑郁具有挑战性，现有方法未能充分整合教师的共情理解。

Method: HEAE框架融合学生叙述文本与教师提供的9维‘共情向量’，通过多模态融合优化分类架构。

Result: 实验显示，该方法在7级抑郁严重程度分类中达到82.74%的准确率。

Conclusion: HEAE为情感计算提供了一种更负责任和伦理的路径，通过嵌入人类共情提升AI评估效果。
...
### [292] [MAC-Gaze: Motion-Aware Continual Calibration for Mobile Gaze Tracking](https://arxiv.org/abs/2505.22769)
...
<details>
  <summary>Details</summary>
Motivation: 移动视线跟踪在用户姿势和设备方向变化时准确性下降，传统一次性校准方法无法适应动态条件。
...
### [293] [Nosey: Open-source hardware for acoustic nasalance](https://arxiv.org/abs/2505.23339)
*Maya Dewhurst,Jack Collins,Justin J. H. Lo,Roy Alderton,Sam Kirkham*

Main category: cs.SD

TL;DR: Nosey是一种低成本、可定制的开源硬件系统，用于记录声学鼻音数据，与商业设备相比表现相似但成本更低。


<details>
  <summary>Details</summary>
Motivation: 开发低成本、可定制的鼻音测量系统，以替代昂贵的商业设备。

Method: 设计并3D打印开源硬件系统Nosey，与商业设备进行性能对比。

Result: Nosey的鼻音评分高于商业设备，但两者在语音环境对比上表现一致。
...
### [294] [Wav2Sem: Plug-and-Play Audio Semantic Decoupling for 3D Speech-Driven Facial Animation](https://arxiv.org/abs/2505.23290)
*Hao Li,Ju Dai,Xin Zhao,Feng Zhou,Junjun Pan,Lei Li*

Main category: cs.SD

TL;DR: 论文提出Wav2Sem模块，通过语义解耦改善3D语音驱动面部动画中音近字异导致的平均效应。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用自监督音频模型编码时，音近字异音节在特征空间中耦合严重，导致唇形生成的平均效应。

Method: 提出Wav2Sem模块，提取音频序列的语义特征，解耦特征空间中的音频编码。

Result: 实验表明Wav2Sem有效解耦音频特征，显著提升唇形生成的精确度和自然度。

Conclusion: Wav2Sem模块能有效解决音近字异问题，提升面部动画质量。
...
### [295] [Semantics-Aware Human Motion Generation from Audio Instructions](https://arxiv.org/abs/2505.23465)
*Zi-An Wang,Shihao Zou,Shiyao Yu,Mingyuan Zhang,Chao Dong*

Main category: cs.SD

TL;DR: 本文提出了一种利用音频信号生成语义对齐动作的新任务，通过掩码生成变换器和记忆检索注意力模块提升效果。


<details>
  <summary>Details</summary>
Motivation: 音频信号比文本更自然直观，但现有方法多关注节奏匹配而非语义关联。

Method: 采用端到端框架，结合掩码生成变换器和记忆检索注意力模块处理稀疏长音频输入。

Result: 实验证明框架有效，音频指令能传达类似文本的语义且更实用。

Conclusion: 音频作为输入在语义表达和交互友好性上优于文本。
...
### [296] [ZeroSep: Separate Anything in Audio with Zero Training](https://arxiv.org/abs/2505.23625)
...
<details>
  <summary>Details</summary>
Motivation: 当前监督深度学习方法需要大量标注数据且难以泛化到真实世界复杂声学场景，受生成基础模型启发，探索预训练扩散模型是否可克服这些限制。

Method: 通过将混合音频反转到扩散模型的潜在空间，利用文本条件引导去噪过程恢复单个源，无需任务特定训练或微调。

Result: ZeroSep在多个分离基准上表现优异，甚至超越监督方法。

Conclusion: 预训练文本引导音频扩散模型可用于零样本音频源分离，支持开放场景，性能优越。
...
### [297] [IRS: Incremental Relationship-guided Segmentation for Digital Pathology](https://arxiv.org/abs/2505.22855)
...
Result: 实验表明IRS能有效处理多尺度病理分割，精确分割肾脏结构和OOD病变，显著提升领域泛化能力。
...
### [298] [iHDR: Iterative HDR Imaging with Arbitrary Number of Exposures](https://arxiv.org/abs/2505.22971)
*Yu Yuan,Yiheng Chi,Xingguang Zhang,Stanley Chan*

Main category: eess.IV

TL;DR: 论文提出了一种名为iHDR的新框架，用于动态高动态范围（HDR）成像，通过迭代融合多张低动态范围（LDR）图像生成高质量HDR图像。


<details>
  <summary>Details</summary>
Motivation: 现有学习型HDR成像方法通常针对固定数量的输入设计，无法灵活处理不同数量的输入帧。

Method: iHDR框架包括一个无重影的双输入HDR融合网络（DiHDR）和一个基于物理的域映射网络（ToneNet），通过迭代融合逐步生成HDR图像。

Result: 实验表明，该方法在输入帧数量灵活的情况下，优于现有HDR去重影技术。

Conclusion: iHDR框架为动态HDR成像提供了一种灵活且高效的解决方案。
...
### [299] [Proximal Algorithm Unrolling: Flexible and Efficient Reconstruction Networks for Single-Pixel Imaging](https://arxiv.org/abs/2505.23180)
...
<details>
  <summary>Details</summary>
Motivation: 解决PnP方法在重建精度和速度上的局限性，以及unrolling方法在压缩比变化时需要微调或重新训练的问题。

Method: 设计高效的深度图像恢复器（DIR）用于HQS和ADMM的展开，并提出通用的近端轨迹（PT）损失函数来训练网络。

Result: 实验表明，提出的近端展开网络不仅能灵活处理不同压缩比，还在重建精度和速度上优于之前的CR特定展开网络。

Conclusion: 该方法成功整合了PnP和unrolling的优势，为单像素成像逆问题提供了更高效的解决方案。
...
### [300] [Advancing Image Super-resolution Techniques in Remote Sensing: A Comprehensive Survey](https://arxiv.org/abs/2505.23248)
*Yunliang Qi,Meng Lou,Yimin Liu,Lu Li,Zhen Yang,Wen Nie*

Main category: eess.IV

TL;DR: 本文系统综述了遥感图像超分辨率（RSISR）方法，包括监督、无监督和质量评估三类，并指出了现有方法在保留细粒度纹理和几何结构方面的不足，提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 遥感图像超分辨率（RSISR）任务缺乏系统综述，本文旨在填补这一空白，帮助研究者了解当前趋势与挑战。

Method: 对RSISR方法进行分类分析，包括监督、无监督和质量评估三类，并评估其方法、数据集和评价指标。

Result: 发现现有方法在大规模退化下难以保留细粒度纹理和几何结构，存在显著局限性。

Conclusion: 未来需开发领域专用架构和鲁棒评价协议，以缩小合成与真实场景的差距。
...
### [301] [Can Large Language Models Challenge CNNS in Medical Image Analysis?](https://arxiv.org/abs/2505.23503)
...
Result: CNN在某些情况下优于多模态方法，但LLM结合额外过滤可显著提升性能。
...
### [302] [PCA for Enhanced Cross-Dataset Generalizability in Breast Ultrasound Tumor Segmentation](https://arxiv.org/abs/2505.23587)
*Christian Schmidt,Heinrich Martin Overhoff*

Main category: eess.IV

TL;DR: 论文提出了一种基于PCA的预处理方法，用于提升医学超声图像分割模型的外部有效性，实验表明该方法显著提高了召回率和Dice分数。
...
Method: 通过PCA预处理降噪并保留90%的数据方差，生成PCA重建数据集，训练U-Net模型并与其他数据集对比。

Result: PCA方法显著提升了召回率（0.57→0.70）和Dice分数（0.50→0.58），并将外部验证的召回率下降减少了33%。
...
### [303] [ImmunoDiff: A Diffusion Model for Immunotherapy Response Prediction in Lung Cancer](https://arxiv.org/abs/2505.23675)
*Moinak Bhattacharya,Judy Huang,Amna F. Sher,Gagandeep Singh,Chao Chen,Prateek Prasanna*

Main category: eess.IV

TL;DR: 提出了ImmunoDiff模型，通过合成治疗后CT扫描并结合临床数据，显著提高了NSCLC免疫治疗响应预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有模型仅依赖治疗前影像预测免疫治疗响应，无法捕捉治疗引起的复杂形态变化，亟需改进。

Method: 结合解剖学先验和临床数据，提出ImmunoDiff扩散模型和cbi-Adapter模块，优化CT合成和预测。

Result: 在NSCLC队列中，响应预测平衡准确率提升21.24%，生存预测c指数增加0.03。

Conclusion: ImmunoDiff通过多模态数据整合显著提升了免疫治疗响应预测性能。
...
### [304] [MRI Image Generation Based on Text Prompts](https://arxiv.org/abs/2505.22682)
...
<details>
  <summary>Details</summary>
Motivation: 解决真实MRI数据集获取的高成本、罕见病例样本有限和隐私问题。

Method: 使用预训练的Stable Diffusion模型，通过3T fastMRI和0.3T M4Raw数据集进行微调，生成不同磁场强度的脑部T1、T2和FLAIR图像。

Result: 定量评估显示图像质量和语义一致性提升，合成图像能有效增强训练数据集并改善MRI对比分类任务性能。
...
### [305] [DeepMultiConnectome: Deep Multi-Task Prediction of Structural Connectomes Directly from Diffusion MRI Tractography](https://arxiv.org/abs/2505.22685)
...
Conclusion: DeepMultiConnectome为生成多分区方案下的个体化连接组提供了快速、可扩展的解决方案。
...
### [306] [Plug-and-Play Posterior Sampling for Blind Inverse Problems](https://arxiv.org/abs/2505.22923)
*Anqi Li,Weijie Gan,Ulugbek S. Kamilov*

Main category: eess.IV

TL;DR: Blind-PnPDM是一种解决盲逆问题的新框架，无需显式先验或单独参数估计，通过交替高斯去噪方案进行后验采样。
...
Method: 利用两个扩散模型作为学习先验：一个捕捉目标图像分布，另一个表征测量算子参数。
...
### [307] [Synthetic Generation and Latent Projection Denoising of Rim Lesions in Multiple Sclerosis](https://arxiv.org/abs/2505.23353)
...
TL;DR: 该论文提出了一种生成合成定量磁化率图的方法，用于改善多发性硬化症中边缘病变的检测和分类性能，并引入了一种新颖的去噪方法。


<details>
  <summary>Details</summary>
Motivation: 多发性硬化症中的边缘病变（PRLs）是一种新兴的生物标志物，但由于其稀少性，检测和分类面临类别不平衡问题。

Method: 通过生成合成定量磁化率图，扩展多通道对比度，并利用生成网络的投影能力提出去噪方法。

Result: 合成数据和去噪方法显著提高了边缘病变的检测性能，并更好地逼近真实分布。

Conclusion: 该方法在多发性硬化症病变检测中具有临床可解释性，并公开了代码和生成数据。
...
### [308] [NGPU-LM: GPU-Accelerated N-Gram Language Model for Context-Biasing in Greedy ASR Decoding](https://arxiv.org/abs/2505.22857)
*Vladimir Bataev,Andrei Andrusenko,Lilit Grigoryan,Aleksandr Laptev,Vitaly Lavrukhin,Boris Ginsburg*

Main category: eess.AS

TL;DR: NGPU-LM提出了一种高效的统计n-gram语言模型数据结构，支持GPU优化推理，显著提升并行计算效率，适用于多种ASR模型。


<details>
  <summary>Details</summary>
Motivation: 现有统计n-gram语言模型在ASR任务中因并行化不足导致计算效率低下，限制了工业应用。

Method: 重新设计数据结构，支持快速并行操作，并引入可定制贪婪解码方法，适用于多种ASR模型。

Result: 在计算开销增加不到7%的情况下，显著缩小贪婪解码与束搜索的准确率差距（减少50%以上），同时避免束搜索的显著减速。

Conclusion: NGPU-LM为ASR任务提供了一种高效且通用的解决方案，其开源实现进一步推动了工业应用。
...
### [309] [Anomalies by Synthesis: Anomaly Detection using Generative Diffusion Models for Off-Road Navigation](https://arxiv.org/abs/2505.22805)
*Siddharth Ancha,Sunshine Jiang,Travis Manderson,Laura Brandt,Yilun Du,Philip R. Osteen,Nicholas Roy*

Main category: cs.RO

TL;DR: 论文提出了一种基于生成扩散模型的像素级异常检测方法，通过分析合成图像与原始图像的差异来识别异常。
...
Method: 使用生成扩散模型合成去除异常的图像，并通过分析图像修改部分检测异常。提出了一种新的引导扩散推理方法。

Result: 方法无需重新训练或微调，可直接集成到现有工作流程中，实现了准确的异常检测。
...
### [310] [TrackVLA: Embodied Visual Tracking in the Wild](https://arxiv.org/abs/2505.23189)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在目标识别和轨迹规划分离时的性能瓶颈，提升在严重遮挡和高动态场景下的跟踪能力。
...
### [311] [Autoregressive Meta-Actions for Unified Controllable Trajectory Generation](https://arxiv.org/abs/2505.23612)
*Jianbo Zhao,Taiyu Ban,Xiyang Wang,Qibin Zhou,Hangning Zhou,Zhihao Liu,Mu Yang,Lei Liu,Bin Li*

Main category: cs.RO

TL;DR: 论文提出了一种自回归元动作方法，用于解决自动驾驶系统中元动作与轨迹时间不对齐的问题，通过分解元动作为帧级动作，提升轨迹预测的精确性和任务一致性。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶系统依赖固定时间间隔的元动作，导致元动作与实际行为轨迹时间不对齐，影响任务连贯性和模型性能。

Method: 提出自回归元动作方法，将长间隔元动分解为帧级元动作，实现元动作预测与轨迹生成的序列交互，并采用分阶段预训练分离基础运动动态与高级决策控制的学习。

Result: 实验验证了方法的有效性，提升了轨迹的自适应性和动态决策响应能力。

Conclusion: 自回归元动作方法解决了元动作与轨迹的时间对齐问题，显著提升了自动驾驶系统的性能。
...
### [312] [Mobi-$π$: Mobilizing Your Robot Learning Policy](https://arxiv.org/abs/2505.23692)
*Jingyun Yang,Isabella Huang,Brandon Vu,Max Bajracharya,Rika Antonova,Jeannette Bohg*

Main category: cs.RO

TL;DR: 论文提出了一种解决视觉运动策略在新环境中泛化能力不足的方法，通过优化机器人基座姿态以适配策略，而无需重新训练策略。
...
Conclusion: 政策动员方法通过优化基座姿态，显著提升了策略在新环境中的泛化能力。
