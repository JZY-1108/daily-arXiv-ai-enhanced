...
Method: 采用协同训练框架，在单一SFT阶段同时训练三种安全行为模式：积极（合法/亲社会）、消极（无过滤/风险倾向）和拒绝（拒绝导向/保守），通过系统指令动态激活不同模式。

Result: 实验显示该方法安全对齐质量与SFT+DPO相当，8B模型在安全性能上显著超越DeepSeek-R1（671B），同时大幅降低训练复杂度和部署成本。
...
Conclusion: 自动排名是初步结果，最终WMT25官方排名将基于更可靠的人工评估，届时将取代这个自动排名
...
<details>
  <summary>Details</summary>
Motivation: 多语言和文化基础的数学推理在资源稀缺语言中较英语欠住，主要因为缺乏反映准确本土实体的社会文化任务数据集。现有多语言标准数据集主要通过翻译生成，通常保留了英语中心的实体，而人工注释本土化成本高。

Method: 提出了一种LLM驱动的文化本土化框架，能够自动从现有来源构建包含本土名字、机构名称和货币的数据集。

Result: 通过广泛实验发现，翻译的标准数据集可能隐藏了在适当社会文化背景下的真实多语言数学能力。该框架能够帮助减少英语中心实体偏差，并在各种语言中引入本土实体时提高稳健性。

Conclusion: 该研究为解决多语言数学推理中的文化本土化问题提供了有效框架，显示了自动化本土化工具在提升模型在不同文化背景下的表现方面的潜力。
...
Method: 使用Direct Preference Optimization (DPO)训练，通过两个LLM(GaMS-9B-Instruct和EuroLLM-9B-Instruct)翻译英语维基百科文章生成质量排名的翻译对，使用COMET等自动评估指标和合成策略进行排名

Result: 精细调整后的模型在翻译维基百科文章时，COMET分数比基准模型分别提高0.04和0.02，更一致地避免语言和格式错误

Conclusion: 通过少量程序化筛选的数据进行DPO训练，可以有效提升通用LLM在指定语言对的翻译性能
...
Result: 在5种语言上进行了单语、跨语言和多语言评估，验证了所提方法在多语言意图识别和自定义输入处理方面的有效性。

Conclusion: 该研究为多语言ConvXAI系统提供了重要的数据集和方法基础，解决了训练数据稀缺和自定义输入支持的问题，推动了多语言可解释AI的发展。
...
<details>
  <summary>Details</summary>
Motivation: 测试时对齐技术虽然能提升LLM的安全性和推理能力，但往往带来巨大的推理成本，限制了实际应用。受推测采样加速技术的启发，希望解决测试时对齐的效率瓶颈

Method: 使用奖励偏移推测采样（SSS）算法，让草稿模型与人类偏好对齐，而目标模型保持不变。通过修改接受标准和奖励token分布来利用对齐草稿模型与未对齐目标模型之间的分布偏移

Result: 在测试时弱到强对齐实验中，算法以显著降低的推理成本实现了优越的黄金奖励分数

Conclusion: SSS算法有效解决了测试时对齐的高成本问题，在不实际获得RLHF最优解的情况下恢复了最优解，验证了方法的有效性和效率
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在语言任务中表现出色但存在幻觉和复杂推理问题，特别是在结构化预测任务中。作者希望通过结合LLM的预测能力和符号推理的结构一致性来解决这些问题。

Method: 结合大语言模型与组合推理方法，研究不同提示策略来估计LLM置信度值，并使用符号推理进行结构化预测。同时采用校准和基于结构化预测目标的微调方法。

Result: 实验表明无论采用何种提示策略，在提示基础上添加符号推理都能产生更一致和准确的预测。结构化学习和校准能显著提升在挑战性任务上的性能表现。

Conclusion: 结构化学习在大语言模型时代仍然具有重要价值，将符号推理与LLM结合可以有效提升模型的结构一致性和预测准确性，减少幻觉问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于Common Crawl的数学数据集由于脆弱的提取启发式方法、有损的HTML到文本转换以及无法可靠保留数学结构而导致质量下降，需要更高质量的数学预训练数据。

Method: 使用布局感知渲染（lynx）和基于LLM的针对性清理阶段的新管道，恢复各种格式的数学内容（MathJax、KaTeX、MathML），保持方程和代码块的结构完整性，同时去除样板文件，将符号标准化为LaTeX表示法。
...
Conclusion: 提出了第一个从嘈杂的网络规模数据中可靠提取科学内容（包括数学）的管道，在数学、代码和一般推理方面取得了可衡量的收益，并在开放数学预训练语料库中设立了新的最先进水平。
...
Result: 实验表明：(1)整合检索证据是有益的；(2)生成和验证原子假设能带来更多改进，并通过指定错误假设提供可解释的答案。

Conclusion: 提出的方法能有效识别和回答带有错误假设的问题，减少LLM的幻觉，提高回答的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型和多模态大模型在处理复杂多轮视觉对话任务时存在上下文丢失、视觉幻觉等问题，需要更全面的评估基准和增强框架。

Method: 构建包含300个复杂多轮对话场景的MMDR-Bench数据集；提出CoLVLM Agent框架，采用无需重新训练的迭代式"记忆-感知-规划-执行"循环机制。

Result: CoLVLM Agent在MMDR-Bench上获得平均4.03的人类评估分数，显著超越GPT-4o(3.92)和Gemini 1.5 Pro(3.85)，在推理深度、指令遵循和错误抑制方面表现优异。

Conclusion: 该框架通过模块化设计和迭代方法有效提升了多模态模型处理复杂交互任务的能力，验证了其在长对话轮次中的鲁棒性能。
...
Method: SemToken首先通过轻量级编码器提取上下文语义嵌入，进行局部语义聚类以合并语义等效的分词；然后根据语义密度分配异质分词粒度，在内容丰富区域使用更细粒度分词，在重复或低熵片段使用更粗粒度压缩。
...
<details>
  <summary>Details</summary>
Motivation: 现有的流程奖励模型(PRMs)主要在通用或STEM领域训练，在金融等专业领域中表现不足，因为金融推理更加结构化、符号化且对事实和监管准确性敏感。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型中KV缓存的内存瓶颈问题，现有方法主要关注时间轴压缩而忽略了通道维度的重要性差异，导致效率和准确性平衡受限。

Method: 采用非结构化稀疏化方法，在通道级别对KV缓存进行剪枝，并在注意力分数计算时动态恢复被剪枝的条目，与现有KV压缩和量化技术正交兼容。

Result: 在相同内存预算下可处理更长序列，相同序列长度下KV缓存存储减少30%以上，即使80%的激进剪枝比例，性能下降也小于5%。

Conclusion: SPARK通过通道级别的细粒度剪枝有效解决了KV缓存的内存瓶颈问题，在保持模型精度的同时显著提升了处理效率，具有良好的鲁棒性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在领域特定场景中表现不佳，检索增强生成存在幻觉和延迟问题，持续预训练成本高且缺乏跨域灵活性。领域知识的长尾分布导致部分有用内部知识未被充分利用。

Method: 提出Selct2Know框架，采用内部-外部知识自选择策略和选择性监督微调来内化领域知识，引入结构化推理数据生成流程并集成GRPO来增强推理能力。

Result: 在医学、法律和金融QA基准测试中，S2K始终优于现有方法，并以显著更低的成本匹配领域预训练的大型语言模型。

Conclusion: S2K框架通过渐进式知识获取策略有效解决了领域特定知识获取的挑战，实现了成本效益和性能的平衡。
...
TL;DR: SEER是一种自引导方法，通过从持续更新的经验池中进行细粒度逐步检索，解决了LLM在多步骤工具调用中的工具选择、参数生成和工具链规划问题，无需依赖手动设计的演示或静态库。
...
Conclusion: SEER通过自引导的经验检索方法有效解决了LLM在多步骤工具调用中的挑战，实现了持续性能改进，无需依赖手动策划的演示库。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成任务自动评估时面临标准模糊的挑战，自动检查表生成虽然是有前景的方法，但其实际有效性尚未充分探索。

Method: 研究了检查表是否应该对所有问题使用还是选择性使用，使用六种方法生成检查表，在八种模型规模上评估有效性，并识别与人工评估相关的检查表项目。通过成对比较和直接评分任务的实验进行分析。

Result: 选择性使用检查表在成对比较设置中倾向于提高评估性能，但在直接评分中的好处不太一致。分析显示即使与人工评分相关性低的检查表项目也常常反映人工编写的标准，表明人工评估存在潜在不一致性。

Conclusion: 研究结果强调了需要更清晰地定义客观评估标准，以指导人工和自动评估，确保评估的一致性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 小型语言模型在边缘设备部署时面临内存瓶颈，传统静态词汇剪枝方法存在信息损失和灵活性不足的问题

Method: 基于词汇局部性原理和计算不对称性，提出解耦动态词汇选择框架：嵌入组件卸载存储，LM头部采用混合静态-动态词汇选择策略，按需加载词汇组件

Result: 在多样化下游任务中，词汇相关组件内存使用减少高达99%，任务性能仅有最小程度或无损下降，显著优于现有静态词汇剪枝方法

Conclusion: VocabTailor有效解决了SLM内存瓶颈问题，为边缘设备部署提供了高效的内存优化解决方案
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在英语中表现优异，但在泰语等资源稀缺语言中的指令追随性能待探索。现有测试集多依赖翻译，缺乏文化和领域特定细节，影响实际应用效果。

Method: 创建WangchanThaiInstruct数据集，涵盖4个专业领域和7种任务类型，通过注释员、领域专家和AI研究人员的多阶段质量控制过程。进行两个研究：(1)零样本评估文化和专业特定任务的性能差距；(2)指令调整研究分离本土监督的效果。

Result: 使用WangchanThaiInstruct进行微调的模型在领域内外测试中都显著超过使用翻译数据的模型，本土化指令数据能够有效提升LLM在资源稀缺语言中的对齐性能。

Conclusion: 研究突出了基于文化和专业知识的本土指令数据对于改善语言模型在资源稀缺、语言多样环境中对齐性能的重要性。
...
Method: 使用SWORDS算法（用同义词替换单词），在保持句子语义的前提下，通过替换选定单词为其翻译来生成CS语音，同时考虑词性。
...
Result: 教师角色LLMs表现出比人类教师更理想化和极化的人格特征，在抽象道德推理方面表现优异但在情感复杂情境中表现不佳，推理能力强的模型更容易受到有害提示注入攻击，模型温度等超参数对风险行为影响有限。

Conclusion: 本研究首次建立了评估教师角色LLMs伦理和心理对齐的基准，揭示了能力与安全性之间的悖论，为教育AI的伦理安全评估提供了重要工具和洞见。
...
<details>
  <summary>Details</summary>
Motivation: 解决检索增强生成(RAG)中当检索上下文与LLM参数知识冲突时，模型无法正确处理上下文-记忆冲突的问题

Method: 引入Conflict-Aware REtrieval-Augmented Generation (CARE)，包含上下文评估器和基础LLM。上下文评估器通过压缩记忆token嵌入和基于grounded/adversarial软提示训练，识别不可靠上下文并引导推理
...
Result: TComQA数据集在提取时间常识方面达到超过80%的精确度，基于该数据集训练的模型在时间问答任务上优于现有数据集微调的大语言模型。

Conclusion: 该方法成功证明了利用大语言模型自动挖掘时间常识的可行性，构建的高质量数据集能有效提升模型在时间推理任务上的性能。
...
Method: 使用短时固定宽度窗口独立处理，学习所有语言共有的基础声学模式，采用监督和自监督训练在多种语言上进行评估

Result: 在UCLA语音语料库的零样本测试中表现出强大的跨语言泛化能力，参数量少于现有方法但性能相当
...
Method: 提出基于平均解距离的评估方法（EDAS），将模型在多数据集和多种评估指标下的性能综合为单一归一化分数（0-1范围）。

Result: 在FB15k-237和WN18RR等基准数据集上的实验表明，EDAS能有效整合多指标、多数据集性能，提供一致、稳健和可推广的评估框架。

Conclusion: EDAS提供了一个统一的元度量框架，解决了知识图谱补全模型评估中的不一致性问题，支持更公平的跨数据集评估和更明智的模型选择。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力的深度和广度快速发展，相应的评测基准大量涌现。作为模型性能的量化评估工具，基准不仅是衡量模型能力的核心手段，也是指导模型发展方向和推动技术创新的关键要素。

Method: 系统综述当前大语言模型评测基准的现状和发展，将283个代表性基准分为三类：通用能力（涵盖核心语言学、知识、推理等）、领域特定（聚焦自然科学、人文社科、工程技术等）、目标特定（关注风险、可靠性、智能体等）。

Result: 识别出当前基准存在的主要问题：数据污染导致分数膨胀、文化语言偏见导致评估不公、缺乏过程可信度和动态环境评估。

Conclusion: 提供了未来基准创新的参考设计范式，为解决现有问题指明方向，推动大语言模型评测基准的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型(MLLMs)的可信度问题日益受到关注，但现有评估方法往往关注狭窄方面，忽视了多模态性带来的风险。需要构建一个全面的评估基准来系统性地评估、分析和缓解MLLMs的可信度问题。

Method: 提出了MultiTrust-X基准，定义了一个三维框架：包含真实性、鲁棒性、安全性、公平性和隐私性5个可信度维度；涵盖多模态风险和跨模态影响2种新型风险类型；以及从数据、模型架构、训练和推理算法角度提出的多种缓解策略。包含32个任务和28个精选数据集，评估了30多个开源和专有MLLMs，并分析了8种代表性缓解方法。

Result: 实验揭示了当前模型的显著漏洞，包括可信度与通用能力之间的差距，以及多模态训练和推理对基础LLM潜在风险的放大。现有缓解策略存在关键局限性，虽然某些方法在特定方面有所改进，但很少有方法能有效解决整体可信度问题，许多方法会引入意外的权衡而损害模型效用。
...
Result: 在机器翻译和摘要任务上相比标准推测解码获得显著加速，同时保持或提升了BLEU和ROUGE分数

Conclusion: 该方法为大型语言模型在不同不确定性条件下提供了一种原则性的、即插即用的高效鲁棒解码方案
...
<details>
  <summary>Details</summary>
Motivation: 探究大词汇表对语言模型性能提升的真正原因，现有实践倾向于使用更大词汇表但效益来源不明确。

Method: 在固定数据、计算和优化条件下，将词汇表从24K扩展到196K进行控制研究，分析token化文本复杂度、词级损失分解和嵌入范数约束。

Result: 大于24K的词汇表主要通过降低前2500个高频词的不确定性来减少交叉熵，尽管低频词损失上升；约束嵌入范数会逆转增益；相同词汇表下扩大模型参数也能获得类似高频词收益。

Conclusion: 词汇表大小的效益实质是降低token化文本复杂度，这为tokenizer-模型协同设计提供了原则性指导，并阐明了预训练中语言模型扩展的损失动态。
...
Conclusion: 该研究填补了基于证据的LLM文本生成领域的空白，提出了开放挑战并指明了未来工作的有前景方向，为该领域的标准化和发展提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型音频-语言模型在处理多模态输入时，对音频和文本信息冲突情况下的处理能力尚未得到充分研究，需要系统评估模型在信息冲突时的表现。

Method: 开发了MCR-BENCH基准测试，通过设计不一致的音频-文本对来评估模型的信息优先级选择，并进行广泛的音频理解任务评估，同时研究影响因素和缓解策略。

Result: 研究发现LALMs存在显著的文本偏向，经常忽略音频证据，导致音频中心任务性能大幅下降。模型即使在矛盾输入情况下也表现出持续的过度自信。
...
<details>
  <summary>Details</summary>
Motivation: 大规模语音语言模型（LSLMs）发展缓慢，主要原因是架构碎片化、缺乏透明度，以及通常只发布模型权重而不公开训练数据和配置，阻碍了系统比较和研究的可复现性。

Method: 提出LLaSO框架，包含三个核心组件：1）LLaSO-Align（1200万条语音文本对齐语料）；2）LLaSO-Instruct（1350万条多任务指令调优数据集）；3）LLaSO-Eval（可复现的标准化评估基准）。基于这些资源训练了38亿参数的LLaSO-Base参考模型。

Result: LLaSO-Base模型在标准化评估中获得0.72的分数，超越了同类可比模型，建立了强大且可复现的基线。分析显示，虽然更广泛的训练覆盖能提升性能，但在未见任务（特别是纯音频场景）上仍存在显著的泛化差距。

Conclusion: 通过完整发布数据、基准和模型，LLaSO为LSLMs研究建立了基础性的开放标准，有助于统一研究努力并加速社区驱动的进展。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型在训练过程中可能记忆并泄露敏感数据，引发隐私风险。随着语言模型在各领域的广泛应用，保护个人隐私权利成为重要研究课题。目前对语言模型隐私风险的理解和缓解方法仍有限。

Method: 进行了隐私保抨语言建模方法的综合性研究，包括对各种方法的深入概览、优势分析和局限性评估。

Result: 研究结果为隐私保护语言建模领域提供了有价值的见解，明确了现有方法的特点和不足，推动了该领域的发展。

Conclusion: 本研究填补了语言模型隐私保护研究的短板，通过系统性的分析为未来研究提供了重要的参考框架和研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注心理健康障碍检测，但缺乏对主动求助行为的识别，这是心理健康危机干预的关键环节。

Method: 构建了新颖的M-Help数据集，包含求助行为、具体心理障碍类型和根本原因的三重标注，用于训练AI模型。

Result: 基于M-Help数据集训练的AI模型能够完成三个关键任务：识别求助者、诊断心理健康状况、发现问题的根本原因。

Conclusion: M-Help数据集填补了心理健康检测领域的重要空白，为开发更精准的求助行为识别和干预系统提供了重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 语言理解直接关系到翻译知识，非对等词汇导致误解的主要原因。这些词汇包括食物、服装、文化和传统词语等，在目标语言中缺乏直接对应词，需要找到合适的翻译方式。

Method: 采用图书馆基础研究方法，分析了从源语言向目标语言翻译非对等词汇的不同方法和规则。

Result: 研究提出了多种翻译非对等词汇的方法，并实际将达尔赫语和乌兹别克语的25个非对等词翻译成了英语和俄语。

Conclusion: 这项研究为翻译学领域提供了重要的方法论支持，帮助减少语言误解，尽管仍有许多非对等词汇需要进一步研究和翻译。
...
Conclusion: PyTOD证明了执行感知的状态跟踪方法的有效性，能够随着对话的进展提供更准确和鲁棒的状态跟踪能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的放射学报告自动评估方法要么产生粗略的整体分数，要么依赖不透明的黑盒模型，缺乏临床基础、可解释性和细粒度指标，限制了在真实临床工作流程中的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 个性化营销对提升客户参与度和业务增长至关重要，研究表明良好的个性化策略可使收入提升高达40%，但目前学术和行业主要关注推荐系统和个性化广告，个性化优惠生成领域具有巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 质疑大语言模型是否能够有效处理主观性用户浏览行为的异质性，因为用户的浏览行为形成了类似个人化语言的序列浏览日志。

Method: 提出了HeTLM（Heterogeneity aware Training of Language Model）方法，通过聚类化训练来处理用户行为的异质性，使用页面级别的tokenizer进行小型模型训练。

Result: 小型专门模型在浏览语言表达方面超过大型预训练或微调模型；HeTLM模型在控制参数数量的情况下超过单一模型；并实现了更高的平均性能和更低的方差，提高了个人化对齐效果。

Conclusion: 对于主观性用户行为，专门设计的小型模型比通用的大型模型更有效，聚类化训练方法能够更好地捐捕用户行为的异质性并提高个性化对齐性。
...
<details>
  <summary>Details</summary>
Motivation: 传统的以人为中心的难度指标在语言模型预训练中效果有限，需要寻找更符合模型训练过程中实际观察到的难度度量方法。

Method: 使用训练数据影响力作为难度指标来排序训练样本，按照从简单到复杂的顺序进行课程学习训练。

Result: 采用模型中心难度指标的课程学习方法在基准测试中比随机顺序训练提升了超过10个百分点。

Conclusion: 课程学习对语言模型预训练是有益的，但需要采用更符合模型视角的难度概念，而不是传统的人类中心难度指标。
...
Method: 开发SLM-Bench基准测试，在4种硬件配置下评估15个SLM模型在9个NLP任务上的表现，使用23个数据集覆盖14个领域，量化11个正确性、计算和能耗指标。

Result: 评估结果显示不同SLM模型存在多样化权衡，一些模型在准确性方面表现出色，而另一些模型在能源效率方面表现优异。
...
Method: 构建包含5536个句子的多标签希伯来语语料库，手动标注12种社会身份，基于调查数据，比较多标签和单标签编码器以及2B-9B参数生成式LLM的性能。

Result: 希伯来语调优的LLM取得最佳性能（macro-F1=0.74），应用于政客Facebook帖子和议会演讲分析，发现了受欢迎程度、时间趋势、聚类模式和性别相关差异。
...
<details>
  <summary>Details</summary>
Motivation: 开发比自回归模型更强大的扩散语言模型，通过并行生成和迭代去噪实现更灵活的序列生成能力。

Method: 采用离散扩散建模方法，使用基于自回归LLM的初始化和上下文自适应token级噪声重调度等简单有效的训练技术。

Result: 在通用任务、数学任务和编程任务上一致超越现有扩散语言模型，展示出优秀的规划能力、推理灵活性、任意顺序生成、填充能力和可调的质量-速度权衡。

Conclusion: Dream 7B代表了扩散语言建模的重要进展，其开源发布将促进该领域的进一步研究。
...
Result: 最佳模型(DictaLM 2.0)在二元PDD检测中F1得分0.74，在去合法化特征分类中macro-F1得分0.67；发现PDD在过去三十年中显著上升，社交媒体中更普遍，右倾行为者使用更多
...
Method: 设计包含7个专门代理的自动化系统，配备多功能工具，确保过程可控性和成本可控性，并将人类专业知识整合到自动化流程中。

Result: 构建的SafetyFlowBench数据集包含23,446个低冗余、强区分度的查询，评估了49个先进LLM的安全性，验证了系统的有效性和效率。

Conclusion: SafetyFlow是首个完全自动化的基准构建管道，提供了全面的安全基准，显著提升了LLM安全评估的效率和效果。
...
TL;DR: 提出了"训练微型模型"概念，通过微调小型语言模型(SLMs)来实现特定高价值应用的文本生成，大幅降低成本


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)在文本生成方面表现出色，但计算成本高昂，特别是在销售和营销等针对性应用中，这些成本难以承受

Method: 开发小型语言模型(SLMs)，针对特定领域进行微调，生成类似的领域特定响应

Result: 能够以极低成本实现与大型语言模型相似的领域特定文本生成效果

Conclusion: 训练微型模型为高价值特定应用提供了一种经济高效的解决方案，解决了LLMs成本过高的问题
...
<details>
  <summary>Details</summary>
Motivation: 发现LLM在作为判别器识别有害请求时比作为生成器防御攻击更有效，存在安全不一致性，希望通过对齐模型的判别和生成能力来提升安全性。

Method: 提出SDGO框架，使用模型自身的判别能力作为强化学习的奖励信号，通过迭代优化来增强生成安全性，无需外部数据或模型。

Result: SDGO显著提高了模型安全性，在保持通用基准上有用性的同时，对分布外越狱攻击表现出鲁棒性能，仅需少量判别样本就能进一步增强生成能力。

Conclusion: 通过对齐LLM的判别和生成能力，SDGO实现了两种能力的紧密耦合，为提升模型安全性提供了一种有效且高效的方法。
...
TL;DR: SurGE是一个用于评估科学综述生成的新基准，包含测试实例和大规模学术语料库，提出了自动化评估框架，发现即使先进的自反思框架也难以胜任综述生成任务


<details>
  <summary>Details</summary>
Motivation: 由于学术文献快速增长，手动撰写科学综述变得不可行，而LLM自动化生成综述缺乏标准化基准和评估协议

Method: 提出SurGE基准，包括主题描述、专家撰写综述及引用文献的测试实例集合，以及百万篇论文的学术语料库；设计自动化评估框架从四个维度评估生成综述
...
<details>
  <summary>Details</summary>
Motivation: 位置偏差（PB）在长上下文理解中表现为对不同位置敏感度不均，严重影响模型性能。现有方法通过修改架构来缓解PB，但效果有限，需要更有效的解决方案。

Method: 提出Pos2Distill知识蒸馏框架，利用位置间的固有差异来对抗位置偏差本身。针对检索和推理两种范式设计了专门实例：Pos2Distill-R¹和Pos2Distill-R²。

Result: 在长上下文检索和推理任务中，所有上下文位置都实现了增强的一致性和显著的性能提升。两个专门系统在各自任务上表现出优异性能，并具有强大的跨任务泛化能力。

Conclusion: Pos2Distill通过位置到位置的知识蒸馏有效缓解了位置偏差问题，为长上下文处理提供了新的解决方案，在检索和推理任务中都取得了成功。
...
TL;DR: 本文对孟加拉语词干提取技术进行全面调研，指出当前存在资源稀缺、评估方法不足等问题，并提出未来研究建议。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语作为全球第七大语言，因资源稀缺而数字化表现不佳。词干提取对于这种语法复杂的低资源语言至关重要，能大大简化语言分析模型的复杂度。

Method: 采用综述性调研方法，系统分析现有的孟加拉语词干提取技术，批判性评估现有评估方法的不足。

Result: 发现孟加拉语词干提取领域存在显著空白，早期研究与现有工作出现断层，可复现实现缺乏，评估指标不适用。

Conclusion: 建议发展更健壮的孟加拉语词干提取器，并继续深入研究以提升语言分析和处理能力。
...
TL;DR: EcomMMMU数据集揭示电商产品图像并不总能提升多模态大语言模型性能，有时甚至降低效果。作者提出SUMEI方法，通过预测视觉效用策略性利用多图像。


<details>
  <summary>Details</summary>
Motivation: 电商平台拥有丰富的多模态数据，但现有数据集规模有限，无法系统研究图像是否真正提升产品理解，还是可能带来冗余或性能下降。

Method: 构建EcomMMMU数据集（406,190样本，8,989,510图像），包含8个核心任务和VSS子集。提出SUMEI方法，先预测视觉效用再策略性使用图像。

Result: 分析发现产品图像并不总能提升性能，有时反而降低效果，表明MLLMs难以有效利用丰富视觉内容。SUMEI方法在实验中展现有效性和鲁棒性。

Conclusion: 需要更智能的图像选择策略，SUMEI方法为多模态模型在电商场景中的视觉内容利用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 医疗大语言模型在诊断中存在知识缺口和幻觉问题，现有的检索增强方法对外部知识利用不足且缺乏反馈推理可追溯性，需要开发更有效的诊断辅助系统。

Method: 构建大规模医学检索语料库（患者记录和可靠医学知识），将LLM作为核心代理，检索语料库作为环境，通过强化学习训练代理RAG策略，使用格式、检索、推理结构和诊断准确性等多维度奖励机制。

Result: 在多个数据中心一致优于提示工程和无训练RAG方法，诊断准确性显著提升，超越GPT-4o、DeepSeek-R1等强基线模型，在分布内和分布外设置下对常见和罕见疾病诊断都有优异表现。

Conclusion: 端到端的代理强化学习训练框架有效解决了医疗诊断中的知识利用和推理可追溯性问题，奖励设计和检索语料库组件对性能提升至关重要，为临床医生提供更可靠精确的初步诊断支持。
...
TL;DR: ReasonZoo基准测试显示，工具集成推理(TIR)能显著提升大语言模型在数学和非数学推理任务中的表现和效率，减少过度思考，实现更流畅的推理过程。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在推理任务上取得进展，但在精确计算方面仍有不足。工具集成推理(TIR)通过整合外部工具来提升推理能力，但其泛化效果和对模型推理行为的改善尚不明确。

Method: 提出ReasonZoo基准测试，涵盖9个不同推理类别，并引入两个新指标：性能感知成本(PAC)和性能-成本曲线下面积(AUC-PCC)来评估推理效率。

Result: 实证评估表明，启用TIR的模型在数学和非数学任务中均优于非TIR模型，TIR提高了推理效率，表现为改进的PAC和AUC-PCC指标，表明减少了过度思考。

Conclusion: TIR具有领域通用的优势，有潜力提升大语言模型在复杂推理任务中的能力，通过更精简的推理过程实现性能提升。
...
Result: 在WFLW、COFW和300W三个面部关键点基准数据集上达到了最先进性能，训练收敛速度快2.2倍，同时保持更好/有竞争力的准确率。

Conclusion: Soft-argmax不是面部关键点检测中实现高性能的唯一选择，提出的结构化预测方法提供了更优的替代方案，具有更快的训练速度和竞争性的精度。
...
<details>
  <summary>Details</summary>
Motivation: 传统卷积神经网络在处理复杂数据结构时存在局限性，而图卷积网络能够有效建模关系数据。研究旨在通过结合GCN和Voronoi图来提升图像分类性能，特别是在复杂场景和细粒度分类任务中。

Method: 将图像表示为图结构（像素或区域作为顶点），使用对应的Delaunay三角剖分对图进行优化，然后应用图卷积网络进行分类。

Result: 在多个基准数据集上实现了预处理效率和分类准确率的显著提升，特别是在复杂场景和细粒度分类任务中超越了现有最先进方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖伪深度、CAD模型或多阶段级联，需要简化流程并开发仅使用RGB图像的端到端解决方案。
...
Conclusion: YOPO证明了单阶段RGB-only方法在类别级9自由度姿态估计任务上的有效性，简化了流程同时保持高性能。
...
TL;DR: 提出了一种统一的训练框架Paired-Sampling Contrastive Framework，用于同时检测物理呈现攻击和数字伪造攻击，在Face Anti-Spoofing Challenge基准测试中达到2.10%的平均分类错误率，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统的人脸反欺骗系统使用分离的模型来处理物理呈现攻击和数字伪造攻击，这增加了系统复杂性、推理延迟，并且容易受到组合攻击向量的威胁。

Method: 采用配对采样对比学习框架，利用自动匹配的真实和攻击自拍对来学习模态无关的活体检测线索，实现统一的训练方法。

Result: 在第六届人脸反欺骗挑战赛统一物理-数字攻击检测基准上，平均分类错误率(ACER)达到2.10%，计算量仅为4.46 GFLOPs，训练时间不到一小时。

Conclusion: 该方法提供了一个轻量级且高效的统一解决方案，能够同时处理物理和数字攻击，具有实际部署的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成模型的对抗攻击往往产生低质量图像且需要大量计算资源，扩散模型虽然能生成高质量图像但通常需要数百个采样步骤，因此需要开发高效的对抗图像生成方法

Method: 通过在混合步骤区间注入扰动实现有效攻击，无需处理所有时间步；采用选择性RGB通道策略，对红色通道应用注意力图，对绿色和蓝色通道使用GradCAM引导的扰动，在保持图像结构的同时最大化目标模型的误分类

Result: 在ImageNet数据集上，以VGGNet为源模型，对ResNet攻击成功率达70.6%，对MNASNet达80.8%，对ShuffleNet达97.8%；PSNR超过30dB，视觉质量良好；生成速度比现有扩散攻击快10倍；获得最低的鲁棒准确率，表明是最具影响力的攻击方法

Conclusion: TAIGen通过创新的选择性通道扰动策略和高效的采样步骤设计，实现了高质量、高效率的对抗图像生成，在攻击效果和计算效率方面均优于现有方法
...
TL;DR: RUN++是一个可逆展开网络，通过生成式精炼解决隐蔽视觉感知问题，在mask和RGB双域应用可逆建模，并引入定向伯努利扩散模型来精炼不确定区域。


<details>
  <summary>Details</summary>
Motivation: 现有CVP方法主要局限于mask域的可逆策略，未能充分利用RGB域的潜力，且存在不确定性处理不足的问题。

Method: 将CVP任务建模为数学优化问题，展开为多阶段深度网络，包含CORE（mask域可逆建模）、CARE（RGB域可逆建模）和FINE（基于噪声的精炼）三个模块，其中FINE模块使用定向伯努利扩散模型专门精炼不确定区域。

Result: 该方法通过展开网络为扩散模型提供强不确定性先验，有效聚焦模糊区域，显著减少假阳性和假阴性，在真实世界退化条件下保持鲁棒性。

Conclusion: RUN++提出了CVP系统的新范式，建立了双层次优化框架，实现了mask和RGB双域的可逆建模协同，以较低计算成本实现精细细节恢复。
...
Method: 提出混合视觉Transformer架构，使用新颖的Mix Twin编码器交替全局注意力和局部注意力机制，配备轻量级LR-ASPP解码器进行多尺度特征聚合。
...
TL;DR: CurveFlow通过引入曲率正则化学习非线性轨迹，在文本到图像生成中实现了最先进的性能，显著提升了语义一致性和图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有整流流模型基于线性轨迹，强制零曲率可能导致生成过程经过数据流形的低概率区域，影响语义对齐和指令遵循能力。

Method: 提出CurveFlow框架，通过曲率正则化技术直接在学习过程中融入曲率指导，学习平滑的非线性轨迹，惩罚轨迹内在动态的突变。

Result: 在MS COCO 2014和2017数据集上的实验表明，CurveFlow在文本到图像生成中达到最先进性能，在BLEU、METEOR、ROUGE和CLAIR等语义一致性指标上显著优于标准整流流变体和其他非线性基线。

Conclusion: 曲率感知建模显著增强了模型忠实遵循复杂指令的能力，同时保持高图像质量，证明了轨迹曲率与语义对齐之间的重要关联。
...
Conclusion: HiRQA通过自监督的层次排序和对齐方法，解决了NR-IQA的泛化问题，无需参考图像或主观标签，在多种失真类型上表现出强大的泛化能力和实时部署潜力。
...
TL;DR: 基于投资组合理论的边缘资源管理策略，用于在时空相关中断下保证多视角3D重建的可靠性


<details>
  <summary>Details</summary>
Motivation: 多视角3D重建应用在应急响应等关键场景中需要近实时性能，但边缘环境的动态性和操作困难会导致相机操作的中断，影响重建质量

Method: 提出投资组合理论启发的边缘资源管理策略，使用遗传算法解决优化问题，确保在时空相关中断下的重建质量

Result: 使用公开和定制3D数据集验证，相比传统基线策略，该方法能有效保证3D重建的可靠性

Conclusion: 该方法为动态边缘环境中的可靠3D重建提供了有效解决方案，能够应对时空相关中断的挑战
...
Method: 整合专业医学视觉编码器、LVLM核心，采用多任务提示工程和多阶段微调，深入理解眼底图像病理特征并生成全面诊断报告

Result: 在DDR数据集上达到84.55%平衡准确率和79.92% F1分数，概念检测达到77.95% BACC和66.88% F1，人类评估确认生成解释的高流畅性、准确性和临床实用性
...
TL;DR: MeSS方法使用城市网格模型作为几何先验，通过改进图像扩散模型生成高质量、风格一致的外景场景，包含三阶段流程：生成几何一致的稀疏视图、传播密集中间视图、全局消除视觉不一致性，同时在生成过程中重建3D高斯溅射场景。


<details>
  <summary>Details</summary>
Motivation: 城市网格模型缺乏真实纹理，限制了在虚拟城市导航和自动驾驶中的应用。现有图像和视频扩散模型无法直接生成3D场景或难以保持视图一致性和几何对齐。

Method: 三阶段流程：1)使用级联外绘ControlNet生成几何一致的稀疏视图；2)通过AGInpaint组件传播更密集的中间视图；3)使用GCAlign模块全局消除视觉不一致性。同时在网格表面初始化高斯球重建3D高斯溅射场景。

Result: 在几何对齐和生成质量方面优于现有方法，生成的场景可通过重光照和风格迁移技术以不同风格渲染。

Conclusion: MeSS方法成功解决了城市网格模型纹理缺失问题，能够生成高质量、风格一致的3D外景场景，为虚拟城市导航和自动驾驶应用提供了有效解决方案。
...
TL;DR: 本文提出了SurgWound数据集、手术伤口诊断基准和三阶段学习框架WoundQwen，用于解决手术部位感染筛查中的数据隐私和标注成本问题。
...
Method: 1) 创建首个开源手术伤口数据集SurgWound，包含697张图像和8个临床属性标注；2) 建立包含视觉问答和报告生成任务的基准；3) 提出三阶段框架WoundQwen：第一阶段用多个MLLM预测伤口特征，第二阶段基于特征预测进行感染风险评估，第三阶段整合结果生成综合报告。

Result: 开发了首个公开的手术伤口数据集和诊断基准，提出了能够分析伤口特征、评估感染风险并提供个性化护理指导的三阶段框架。

Conclusion: 该研究为个性化伤口护理、及时干预和改善患者预后提供了新途径，解决了手术伤口筛查中的数据可用性和模型性能评估问题。
...
Method: 使用基于学习的方法来推导规则智能体的对抗行为，以制造失败场景。评估对抗智能体对所有规则智能体的影响

Result: 对抗智能体导致累积奖励下降，证明了规则智能体建模中存在的漏洞

Conclusion: 该方法能够有效识别规则智能体建模中的弱点，为安全关键应用中的智能体验证提供了重要工具
...
<details>
  <summary>Details</summary>
Motivation: 现有的肾脏病理学数据集和自动化方法主要提供二元（语义）掩码，限制了后续分析的精确性。传统的后处理技术在分离语义掩码为实例时，受到肾脏组织多样形态和复杂连接性的限制。

Method: 提出了DyMorph-B2I管道，将分水岭、骨架化和形态学操作整合到统一框架中，辅以自适应几何细化和可定制的超参数调优，针对每类功能单元进行系统参数优化。

Result: 实验结果表明，该方法优于单个传统方法和简单组合，实现了更优的实例分离，促进了肾脏病理工作流程中更准确的形态计量分析。

Conclusion: DyMorph-B2I为肾脏病理学提供了有效的二元到实例分割解决方案，能够鲁棒处理复杂组织结构，提升形态学定量分析的准确性。
...
TL;DR: 提出STAGNet模型，通过改进时空特征提取和循环网络聚合，提升基于行车记录仪视频的事故预测性能，在多个数据集上取得更高的平均精度和碰撞时间值。
...
Method: 采用改进的时空特征提取方法，并通过循环网络聚合这些特征，基于图神经网络架构构建STAGNet模型。

Result: 在三个公开数据集上的实验表明，STAGNet在平均精度和平均碰撞时间指标上均优于现有方法，且在跨数据集测试中表现稳定。
...
Conclusion: TriMM通过有效利用多模态数据，在3D生成领域实现了显著的性能提升，为多模态3D建模提供了新的解决方案
...
Result: 在五个数据集上的实验表明，该方法相比现有最先进方法具有更好的效果
...
TL;DR: 提出双高度无人机协作视觉语言导航任务(DuAl-VLN)，通过高低空无人机协作解决复杂户外导航问题，构建HaL-13k数据集并开发AeroDuo框架。


<details>
  <summary>Details</summary>
Motivation: 解决无人机视觉语言导航中轨迹长、机动性复杂导致的性能不可靠问题，利用无人机多粒度视角优势同时保持可控运动空间。

Method: 采用双无人机协作框架：高空无人机负责环境推理，低空无人机负责精确导航；高空使用多模态大语言模型(Pilot-LLM)，低空使用轻量级多阶段策略；仅交换坐标信息确保效率。

Result: 构建包含13,838条协作轨迹的HaL-13k数据集，包含未见地图和未见目标验证集，系统评估模型在新环境和陌生目标上的泛化能力。

Conclusion: 双高度无人机协作VLN任务和AeroDuo框架有效结合了高低空无人机的互补优势，为复杂户外导航提供了可行的解决方案。
...
TL;DR: 本文提出了一种跳过步采样机制，通过马尔可夫方式加速扩散模型的生成过程，无需依赖非马尔可夫过程，显著减少了采样步骤并保持高质量生成。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然在各生成任务中达到最先进效果，但其顺序生成过程需要多步迭代，效率较低。现有方法如DDIM通过非马尔可夫过程减少采样步骤，但作者希望探索原始扩散过程是否也能实现相同效率。

Method: 提出了跳过步采样机制，在迭代生成过程中绕过多个中间去噪步骤，与传统逐步细化方法形成对比。该方法从与标准扩散模型相同的训练目标推导而来，证明通过马尔可夫方式加速采样是预训练扩散模型的内在特性。

Result: 在OpenAI ADM、Stable Diffusion和Open Sora等流行预训练扩散模型上的大量实验表明，该方法能够以显著减少的采样步骤实现高质量生成。

Conclusion: 跳过步采样是一种有效的加速扩散模型生成的方法，可以与DDIM等现有技术结合使用，进一步提升了生成效率，同时保持了生成质量。
...
TL;DR: Comp-X是首个基于大语言模型智能交互的图像压缩范式，通过统一多模式编码框架、交互式编码代理和专用基准测试，实现了对用户请求的智能理解和高效压缩。


<details>
  <summary>Details</summary>
Motivation: 传统图像编解码器编码模式有限且依赖工程师手动选择，对非专业用户不友好。需要开发能够智能理解用户需求并自动选择最佳编码模式的交互式压缩系统。

Method: 提出三方面创新：(1)多功能编码框架统一不同目标/需求的编码模式；(2)基于增强上下文学习的交互式编码代理，通过专家反馈教导LLM理解编码请求和工具使用；(3)首个专用基准测试IIC-bench，包含多样化用户请求和专家标注。

Result: 实验结果表明Comp-X能高效理解编码请求并具备出色的文本交互能力，在单一编码框架下仍能保持可比的压缩性能。

Conclusion: Comp-X为图像压缩中的人工通用智能提供了有前景的发展方向，展示了LLM代理在智能图像压缩中的强大潜力。
...
Method: 重构Transformer解码器结构，交换交叉注意力和自注意力层顺序；实例化多个并行交叉注意力块；实现一对多分配策略

Result: 在OpenLane-V2数据集上实验表明，RATopo策略具有模型无关性，能无缝集成到现有拓扑推理框架，显著提升车道-车道和车道-交通元素的拓扑性能

Conclusion: RATopo通过冗余分配策略实现了数量丰富和几何多样的拓扑监督，有效解决了现有方法监督范围有限的问题
...
TL;DR: DesignCLIP利用CLIP模型构建统一框架，通过类别感知分类和对比学习处理设计专利图像，在专利分类和检索任务中优于现有方法


<details>
  <summary>Details</summary>
Motivation: 传统设计专利分析依赖图像数据，但专利草图缺乏完整的视觉上下文和语义信息，导致在先技术检索中存在模糊性。视觉语言模型如CLIP为更可靠的AI驱动专利分析提供了机会

Method: 开发DesignCLIP框架，结合类别感知分类和对比学习，使用生成的详细图像描述和多视图图像学习来处理专利数据的独特特征

Result: DesignCLIP在所有专利领域任务中 consistently 优于基线和最先进模型，验证了多模态方法在专利分析中的有效性

Conclusion: 多模态方法在推进专利分析方面具有巨大潜力，DesignCLIP展示了视觉语言模型在设计专利应用中的优势
...
<details>
  <summary>Details</summary>
Motivation: 超声视频中的先天性心脏病检测受到图像噪声和探头位置变异性的影响，现有机器学习方法往往忽略时序信息、仅限于二分类且缺乏预测校准。
...
Result: 在CHD检测私有数据集和EchoNet-Dynamic公共数据集上，TPA实现了85.40%的宏观F1分数用于CHD诊断，同时将预期校准误差降低5.38%，自适应ECE降低6.8%。在三分类任务上，宏观F1提高了4.73%。

Conclusion: TPA是一个有效的胎儿先天性心脏病超声视频分类框架，成功整合了时序建模、对比学习和不确定性量化，显著提升了分类性能和临床可靠性。
...
TL;DR: 本文提出了BasketLiDAR数据集和新型MOT框架，结合LiDAR点云和多视角相机数据，在篮球场景中实现了实时3D球员追踪，解决了传统相机方法的遮挡问题和计算成本高的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统多相机系统在体育3D轨迹追踪中存在二维数据局限性和复杂3D重建处理的问题，难以实现实时分析。篮球场景中球员快速移动、频繁遮挡，使得追踪尤为困难。

Method: 构建了首个结合LiDAR点云和同步多视角相机数据的多模态数据集BasketLiDAR，包含4,445帧和3,105个球员ID。提出了基于LiDAR高精度3D空间信息的新型MOT算法，包括纯LiDAR实时追踪管道和LiDAR-相机融合的多模态追踪管道。

Result: 实验结果表明，该方法实现了传统相机方法难以达到的实时操作，在遮挡条件下仍能保持优异的追踪性能，同时降低了计算成本。

Conclusion: BasketLiDAR数据集和提出的多模态MOT框架为体育分析领域提供了有效的解决方案，实现了高精度实时3D球员追踪，特别是在复杂篮球场景中表现出色。
...
TL;DR: RAG-SEG是一种无需训练的两阶段伪装目标检测方法，先通过检索增强生成粗掩码作为提示，再用SAM进行精细分割，在个人笔记本上实现高效检测。


<details>
  <summary>Details</summary>
Motivation: 现有伪装目标检测方法需要大量训练和计算资源，而基础模型如SAM需要高质量提示才能良好工作，手动生成提示成本高且低效。

Method: 提出RAG-SEG两阶段范式：第一阶段通过无监督聚类构建检索数据库生成粗掩码提示，第二阶段使用SAM2进行精细分割。

Result: 在基准COD数据集上的实验表明，RAG-SEG性能与或超越最先进方法，且所有实验均在个人笔记本上完成，计算效率高。

Conclusion: 该方法无需传统训练，保持了竞争性性能，具有计算效率和实用性优势，为伪装目标检测提供了新的解决方案。
...
TL;DR: VideoEraser是一个无需训练的框架，通过选择性提示嵌入调整和抗干扰噪声引导技术，有效防止文本到视频扩散模型生成有害或不当内容


<details>
  <summary>Details</summary>
Motivation: 文本到视频扩散模型的快速发展带来了隐私、版权和安全问题，这些模型可能被滥用来生成有害或误导性内容，需要一种方法来控制不当内容的生成
...
Conclusion: VideoEraser提供了一个有效的训练免费解决方案，能够显著抑制T2V扩散模型中不良内容的生成，具有很好的实用价值和推广潜力
...
TL;DR: 该研究开发了一个端到端的深度学习框架，使用姿态检测和序列建模技术来预测行人过马路意图，发现GRU比LSTM预测效果更好，而1D CNN在速度方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶车辆的发展，需要能够从远处预测行人是否要过马路的系统，这对车辆的安全导航至关重要。

Method: 使用深度学习模型进行姿态检测，然后结合序列建模技术（包括GRU、LSTM和1D CNN）进行时序预测，构建端到端的深度学习框架。

Result: GRU在预测行人过马路意图方面表现优于LSTM，而1D CNN在速度方面是最快的模型。

Conclusion: 研究成功开发了一个有效的端到端框架来预测行人过马路意图，不同序列建模技术各有优势，GRU在准确性方面表现最佳，1D CNN在速度方面最优。
...
TL;DR: 本文通过注意力敲除技术系统分析了Video-LLMs的内部工作机制，发现视频信息处理主要在早期层完成，呈现两阶段模式，且时空建模更依赖语言引导检索而非视频token间的自注意力。
...
Result: 发现三个关键发现：(1)视频信息提取主要在早期层完成，形成感知编码和抽象推理的两阶段过程；(2)某些中间层对视频问答有超常影响；(3)时空建模更依赖语言引导检索。这些见解可用于减少注意力计算。

Conclusion: 这是首个系统揭示Video-LLMs内部视频处理机制的工作，为未来研究提供了可解释性和效率方面的视角，证明了通过理解模型内部机制可以优化计算效率。
...
TL;DR: 这篇论文提出了ICGS-Quantizer方法，通过因子化量化和图像条件解码，将3D戈坦散射的存储需求从克级降到千克级，同时支持档案后场景变化的适应性。


<details>
  <summary>Details</summary>
Motivation: 解决现有3DGS压缩方法的两大限制：(1)压缩后仍需克级存储，对大规模场景不实用；(2)缺乏对档案后场景变化的适应机制。

Method: 提出图像条件化戈坦散射量化器(ICGS-Quantizer)，通过联合利用戈坦间相关性和属性间相关性，使用跨场景共享代码本，并在解码时使用图像条件来支持档案后场景变化的适应性。

Result: 实验结果显示ICGS-Quantizer在压缩效率和场景更新适应性方面都较现有方法更优，能将3DGS的存储需求从克级降到千克级。

Conclusion: ICGS-Quantizer为3D戈坦散射提供了高效的压缩和档案后适应性，解决了现有方法的存储和变化适应问题，为大规模场景档案提供了可行方案。
...
Method: 1. 区域体素初始化方案：将场景划分为近、中、远区域以增强近距离细节表示；2. 可变形神经高斯：通过可学习变形网络对非刚性动态物体的参数进行时间调整；3. 深度和法线先验监督：使用预训练模型的深度和法线先验来监督整个框架，提高几何结构准确性。
...
TL;DR: 本文针对深度学习模型在抽象推理方面的瓶颈，通过Raven渐进矩阵问题评估和改进抽象推理能力。提出了因果链建模方法，并基于基线模型DIO的局限性，逐步提出了三种改进方法。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型在抽象推理方面存在根本性瓶颈，Raven渐进矩阵问题被用作评估抽象推理能力的权威基准。本文旨在通过解决RPM问题来增强机器智能的抽象推理能力。

Method: 采用因果链建模视角分析RPM任务的完整因果链，设计基线模型DIO的网络架构。针对DIO的局限性（互信息下界紧致性问题和统计度量无法捕捉因果关系），逐步提出三种改进方法。

Result: 实验表明基线模型DIO的优化目标（最大化上下文与正确选项间互信息的变分下界）无法让模型真正获得预定义的人类推理逻辑。

Conclusion: 需要克服互信息下界紧致性影响和统计度量无法捕捉因果关系的局限性，为此提出了三种渐进改进方法来解决这些问题。
...
TL;DR: 提出SpiVG网络，利用脉冲神经网络和变分推理解决视频摘要中的时序依赖、语义连贯性和多通道特征融合噪声问题，在多个数据集上超越现有方法。
...
Method: 设计基于SNN的关键帧提取器，引入动态聚合图推理器解耦上下文对象一致性和语义视角连贯性，使用变分推理重建模块处理特征融合不确定性。

Result: 在SumMe、TVSum、VideoXum和QFVS等多个数据集上超越现有方法，代码和预训练模型已开源。

Conclusion: SpiVG网络通过结合SNN的事件驱动机制和变分推理，有效提升了视频摘要的信息密度和计算效率，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管MAE在视觉基础模型预训练中表现强大，但在应用于新数据集时需要大量超参数调优（掩码比例、补丁大小、编码器/解码器层数）。MAE超参数与下游任务性能之间的关系尚未得到充分探索。

Method: 通过分析推导线性MAE学习的特征，并将分析扩展到非线性MAE，研究MAE表示如何适应数据集中的空间相关性。

Result: 研究发现掩码比例和补丁大小可以用于选择捕获不同范围空间相关性的特征，MAE表示能够适应数据集的空间相关性，超越二阶统计量。

Conclusion: 研究为实践中如何选择MAE超参数提供了见解，揭示了MAE学习空间相关性的机制。
...
TL;DR: 提出双向时序信息传播方法BIRD，用于红外小目标检测，通过双向传播策略同时利用局部和全局时序信息，实现最优性能和高推理速度
...
Method: 提出双向传播策略，包含前向和后向传播分支，设计局部时序运动融合(LTMF)模块建模局部时空依赖，全局时序运动融合(GTMF)模块聚合全局传播特征，最后融合双向特征进行检测

Result: 大量实验表明BIRD方法不仅达到最先进性能，而且显示出快速推理速度

Conclusion: BIRD方法通过双向时序信息传播有效解决了传统滑动窗口方法的局限性，在红外小目标检测任务中表现出优异性能
...
TL;DR: 该论文提出基于YOLOv8的深度学习解决方案，用于自动检测汽车表面的微小凹陷缺陷，相比传统人工检测方法具有更高的精度和实时性。


<details>
  <summary>Details</summary>
Motivation: 传统汽车损伤检测方法劳动密集、人工操作且经常忽略微小表面缺陷，需要更快速精确的自动化检测方案来满足汽车保险评估和检测需求。
...
Conclusion: YOLOv8m-t42模型在检测微小表面凹陷方面具有高精度和良好性能，适合实时应用如自动化保险评估和汽车检测，尽管收敛速度较慢但实用性更强。
...
TL;DR: MATR模型通过双阶段序列对齐和自监督预训练，在视频到视频时刻检索任务中显著提升性能，在ActivityNet-VRL数据集上R@1提升13.1%，mIoU提升8.1%


<details>
  <summary>Details</summary>
Motivation: 视频到视频时刻检索(Vid2VidMR)任务面临语义帧级对齐和复杂依赖关系建模的挑战，需要开发能够精确捕捉语义上下文和时间细节的模型

Method: 提出MATR模型，使用基于Transformer的架构进行双阶段序列对齐，编码查询视频和目标视频之间的相关性，并采用自监督预训练技术来定位视频中的随机片段

Result: 在ActivityNet-VRL数据集上，R@1提升13.1%，mIoU提升8.1%；在新提出的SportsMoments数据集上，R@1提升14.7%，mIoU提升14.4%

Conclusion: MATR通过有效的序列对齐和预训练策略，在视频时刻检索任务中取得了显著的性能提升，证明了其在捕捉语义和时间信息方面的有效性
...
Method: 使用密集立体模块逐步估计相机位姿和重建全局密集点云；提出连贯视角插值模块生成额外监督信号；引入多尺度拉普拉斯一致性正则化和自适应空间感知多尺度几何正则化
...
<details>
  <summary>Details</summary>
Motivation: 解决现有轻量级模型在医学图像分割中因缺乏注意力机制而全局感知能力不足，以及相同卷积核下的通道冗余问题，满足资源受限临床环境的需求。
...
Method: 提出JMBO算法联合优化SMPL-X人体模型，确保多视角一致性；使用法线图积分添加几何细节，整合前后视角的法线图来捕捉服装褶皱和发型等细节

Result: 实验结果表明MExECON相比单视角基线方法在保真度方面有显著提升，与现代少样本3D重建方法相比具有竞争力

Conclusion: MExECON在不需重新训练网络的情况下实现了多视角优势，为穿衣人体3D重建提供了有效的多视角解决方案
...
TL;DR: AdaSFFuse是一个用于多模态图像融合的新型框架，通过自适应跨域协同融合学习实现任务通用性，在保持低计算成本和紧凑网络的同时提供卓越的融合性能。
...
Result: 在红外-可见光图像融合、多焦点图像融合、多曝光图像融合和医学图像融合四个任务上的广泛实验表明，AdaSFFuse具有卓越的融合性能。
...
TL;DR: ExtraGS是一个用于自动驾驶场景轨迹外推的框架，结合了几何和生成先验，通过Road Surface Gaussian和Far Field Gaussians表示，以及基于球谐函数的自监督不确定性估计，显著提升了外推视图的真实性和几何一致性。


<details>
  <summary>Details</summary>
Motivation: 从记录的驾驶日志中合成外推视图对于自动驾驶模拟至关重要，但现有方法使用生成先验作为伪地面真值往往导致几何一致性差和渲染过度平滑的问题。
...
TL;DR: GroupSketch是一种新颖的矢量草图动画方法，通过两阶段流程（运动初始化和运动细化）有效处理多对象交互和复杂运动，解决了现有方法在时间一致性和泛化能力方面的限制。
...
TL;DR: D3FNet是一种用于高分辨率遥感图像中细粒度窄道路分割的网络，通过差分注意力扩张提取、双流解码融合和多尺度扩张策略，有效解决窄道路宽度有限、拓扑破碎和遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像中的窄道路提取面临宽度有限、拓扑破碎和频繁遮挡等挑战，传统模型容易过拟合到一般道路宽度，无法有效处理细粒度、遮挡和低对比度的道路段。
...
Result: 在DeepGlobe和CHN6-CUG基准测试中，D3FNet在挑战性道路区域实现了优异的IoU和召回率，超越了最先进的基线方法。消融研究验证了注意力引导编码和双路径解码的互补协同作用。

Conclusion: D3FNet是复杂遥感和协同感知场景中细粒度窄道路提取的鲁棒解决方案，特别适用于处理细粒度、遮挡和低对比度的道路段。
...
Result: 在多样化基准测试中，该方法在各种分布偏移下实现了最先进的性能，具有优异的可扩展性和鲁棒性。

Conclusion: ADAPT提供了一种高效、可扩展的测试时自适应解决方案，通过概率推理框架和轻量级正则化机制，解决了现有方法的局限性，为实时部署和可靠预测提供了新的途径。
...
TL;DR: 提出两阶段训练策略解决INR的频谱偏差问题，通过邻域感知软掩码自适应加权高频像素，提升细节重建质量
...
TL;DR: 提出了一种新的线性时间凸松弛方法和contractor方法，用于加速分支定界算法，解决点云配准中的截断最小二乘问题，在旋转轴已知的情况下能够在0.5秒内实现100个点的全局最优配准。


<details>
  <summary>Details</summary>
Motivation: 现有的半定规划松弛方法求解截断最小二乘配准问题需要数百秒时间，计算效率低下，需要更快的全局最优求解方法。

Method: 采用线性时间凸松弛技术和contractor方法加速分支定界算法，在已知旋转轴的情况下求解3D点云配准问题。

Result: 该方法比最先进的SDP求解器STRIDE快两个数量级，能够在0.5秒内完成100个点的全局最优配准，并且能够处理高达95%的异常值。

Conclusion: 提出的方法显著提高了点云配准的计算效率，虽然目前不能解决完整的6自由度问题，但在旋转轴已知的情况下实现了快速全局最优求解。
...
TL;DR: 该研究结合相机陷阱和无人机热成像技术，利用深度学习模型自动识别野生动物和人类活动，通过空间模式分析评估人兽冲突区域，在尼泊尔奇旺国家公园实现了96.2%的检测精度。
...
Result: YOLOv11s模型达到96.2%的精确度、92.3%的召回率、96.7%的mAP50和81.3%的mAP50；无人机热成像提供了补充的空中视角；空间分析识别出明显的野生动物和人类活动热点及其重叠模式。
...
TL;DR: Grounded VideoDiT是一个视频大语言模型，通过扩散时间潜在编码器、对象接地表示和混合令牌方案，解决了现有视频LLM在时间感知和实体定位方面的局限性，在多个基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频LLM虽然在整体推理方面取得了显著进展，但在时间感知方面仍然粗糙：时间戳仅隐式编码，帧级特征在捕捉连续性方面较弱，语言视觉对齐经常偏离感兴趣的实体。

Method: 1) 扩散时间潜在(DTL)编码器增强边界敏感性和保持时间一致性；2) 对象接地表示将查询实体显式绑定到局部化视觉证据；3) 混合令牌方案使用离散时间令牌提供显式时间戳建模

Result: 在Charades STA、NExT GQA和多个VideoQA基准测试中取得了最先进的性能，验证了模型的强大接地能力

Conclusion: Grounded VideoDiT通过三项关键创新设计，为视频LLM提供了强大的时间感知和实体定位能力，显著提升了细粒度时间推理性能
...
TL;DR: 提出一种弱监督方法，通过人类操作员对初始分割结果进行质量评级，训练评级模型来改进树实例分割性能，在正确识别树实例方面提升34%
...
Conclusion: 该方法有效解决了树实例分割的挑战，但在稀疏林区小树或复杂环境中性能仍有待提升
...
TL;DR: 提出CFG方法，通过关键特征指导提升3D点云对抗样本的迁移性，在无需目标模型信息的情况下实现高效黑盒攻击
...
Method: 基于关键特征一致性观察，提出CFG方法：计算特征重要性指导对抗点云搜索，优先破坏不同架构可能采用的关键特征，并在损失函数中约束最大偏差确保不可感知性

Result: 在ModelNet40和ScanObjectNN基准数据集上，CFG大幅优于现有最先进攻击方法

Conclusion: CFG方法通过关键特征指导有效提升了3D点云对抗样本的迁移攻击能力，为黑盒场景下的3D点云安全提供了有效解决方案
...
TL;DR: MapKD是一个多级跨模态知识蒸馏框架，通过教师-教练-学生范式，将多模态模型的知识转移到轻量级视觉学生模型中，显著提升在线高清地图构建性能。
...
Method: 提出Teacher-Coach-Student范式：1）相机-LiDAR融合模型作为教师；2）带先验知识和模拟LiDAR的视觉中心教练模型；3）轻量级视觉学生模型。采用TGPD和MSRD两种知识蒸馏策略。

Result: 在nuScenes数据集上，学生模型性能提升+6.68 mIoU和+10.94 mAP，同时加速推理速度。

Conclusion: MapKD框架有效解决了跨模态知识转移问题，实现了高效、低成本的视觉中心在线地图构建。
...
TL;DR: 通过冲突地图和人工智能技术，自动重建具有细节立面元素的LOD3建筑模型，解决传统手工建模的效率问题


<details>
  <summary>Details</summary>
Motivation: 现有LOD1和LOD2建筑模型缺乏细节立面元素，而LOD3模型需要手工建模且难以大规模应用，需要自动化方案

Method: 使用光线到模型分析获得冲突地图(CMs)，通过语义冲突地图生成器(SCMG)生成合成CMs进行语义分割，并融合细节模型的分割结果提升性能

Result: 方法能够有效分割和重建建筑窗户门等立面元素，融合细节模型后达到61%的性能提升

Conclusion: 研究提供了一种可扩展的自动化LOD3模型重建方法，为高效的3D城市建模奠定了基础
...
TL;DR: MDPR框架通过构建多维度视觉语义知识库和动态提示路由机制，解决VLM在类别不平衡场景下微调时的偏见问题，在多个长尾基准测试中达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉语言模型(VLMs)在类别不平衡场景下微调时存在偏见问题，现有方法忽略了VLM预训练中固有的类别不平衡，导致下游任务中的偏见累积。

Method: 提出多维度动态提示路由(MDPR)框架：构建涵盖五个视觉语义维度的综合知识库；通过动态路由机制对齐全局视觉类别、检索最优提示、平衡细粒度语义；通过logits融合获得稳定预测。
...
TL;DR: StreamMem是一种面向长视频理解的查询无关KV缓存压缩机制，通过流式编码和注意力分数压缩，在内存受限场景下实现高效问答。
...
Method: 提出WorldWeaver框架，联合建模RGB帧和感知条件；利用深度线索构建记忆库保存上下文信息；采用分段噪声调度训练预测组。
...
TL;DR: 提出了分子赋能的All-in-SAM模型，通过整合分子信息来增强计算病理学中的细胞分割和分类性能，减少标注工作量并提高在资源有限环境中的可及性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉基础模型（如SAM）在细粒度语义分割（如识别特定细胞亚型）方面存在挑战，需要开发能够更好处理病理图像中特定语义的方法。

Method: 采用全栈方法：1）分子赋能的标注减少像素级标注需求；2）使用SAM适配器强调特定语义；3）整合分子导向校正学习（MOCL）提高分割精度。

Result: 在内部和公共数据集上的实验显示，All-in-SAM模型显著提高了细胞分类性能，即使面对不同质量的标注也能保持良好效果。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决现有视频生成模型在运动捕捉、时间一致性和生成质量方面的挑战，开发一个统一的框架来同时支持图像和视频的高质量生成。

Method: 采用混合流DiT架构增强模态对齐和加速训练收敛；建立全面的数据筛选流程，使用MLLM视频质量模型过滤高质量样本；提供详细的训练和推理方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数化人体模型在捕捉多样化姿势和形状细节方面存在困难，主要由于训练数据多样性有限和建模假设限制，且内部骨骼与外部软组织之间存在问题依赖关系，限制了直接控制身体高度和骨骼长度的能力。
...
Conclusion: ATLAS通过骨骼基础的解耦方法实现了增强的形状表达能力、细粒度身体属性定制以及与外部软组织特征无关的关键点拟合，超越了现有方法。
...
Method: 提出SceneGen框架，包含新颖的特征聚合模块整合局部和全局场景信息，结合位置头实现单次前向传播生成3D资产及其相对空间位置

Result: 广泛的定量和定性评估证实了该方法的高效性和鲁棒生成能力，在单图像训练基础上可直接扩展到多图像输入场景
...
TL;DR: VAREdit是一个基于视觉自回归模型的图像编辑框架，通过多尺度特征预测实现精确编辑，在编辑准确性和效率方面显著优于扩散模型方法


<details>
  <summary>Details</summary>
Motivation: 扩散模型在指令引导的图像编辑中存在全局去噪过程导致编辑区域与整个图像上下文纠缠的问题，产生意外的虚假修改和编辑指令遵循性差的问题

Method: 提出VAREdit框架，将图像编辑重新定义为下一尺度预测问题，使用Scale-Aligned Reference模块注入尺度匹配的调节信息，通过自回归方式生成多尺度目标特征

Result: 在标准基准测试中，GPT-Balance得分比领先的扩散方法高出30%以上，512×512图像编辑仅需1.2秒，比同类方法UltraEdit快2.2倍

Conclusion: 自回归模型为图像编辑提供了更好的因果和组合机制，VAREdit在编辑准确性和效率方面都取得了显著进步，为指令引导的图像编辑提供了新的解决方案
...
TL;DR: 提出了一种可扩展的组推理方法，通过将组推理建模为二次整数分配问题，在保证样本质量的同时最大化组内多样性，解决了生成模型中独立采样导致冗余的问题。


<details>
  <summary>Details</summary>
Motivation: 在实际应用中，用户通常需要获得一组多样化的输出结果，但现有的生成模型独立采样方法往往产生冗余结果，限制了用户选择和创意探索。

Method: 将组推理建模为二次整数分配问题：候选输出作为图节点，通过优化样本质量（一元项）和最大化组多样性（二元项）来选择子集；采用渐进式剪枝策略提高运行效率。

Result: 实验表明该方法在文本到图像、图像到图像、图像提示和视频生成等多种任务中，相比独立采样基线和最新推理算法，显著提高了组多样性和质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉扩散模型受限于训练分辨率和计算资源，无法生成高质量的高分辨率内容，且现有免调优方法容易产生重复模式
...
Conclusion: CineScale成功解决了高分辨率视觉生成中的高频信息增加问题，显著提升了生成质量，扩展了预训练模型的高分辨率生成潜力
...
### [121] [LLMs and Agentic AI in Insurance Decision-Making: Opportunities and Challenges For Africa](https://arxiv.org/abs/2508.15110)
*Graham Hill,JingYuan Gong,Thulani Babeli,Moseli Mots'oehli,James Gachomo Wanjiku*

Main category: cs.CE

TL;DR: 本文探讨了AI（特别是大语言模型和智能体AI）在保险行业的变革潜力，重点关注非洲市场的独特机遇、挑战和发展路径，呼吁各方合作创建包容可持续的AI解决方案。


<details>
  <summary>Details</summary>
Motivation: 随着AI性能快速提升、开源访问增加、部署成本下降以及LLM框架复杂性增加，保险行业面临转型机遇，特别是在非洲保险市场存在关键缺口需要填补。

Method: 通过分析AI技术在保险领域的应用潜力，识别非洲保险市场的关键差距，并强调本地努力、主要参与者和合作伙伴机会。

Result: 提出了AI在保险行业的变革性潜力，特别是在非洲市场的应用前景，强调了本地化解决方案的重要性。

Conclusion: 呼吁精算师、保险公司、监管机构和技术领导者开展协作，创建由非洲人主导的包容、可持续和公平的AI战略和解决方案。
...
### [122] [Fusing Structural Phenotypes with Functional Data for Early Prediction of Primary Angle Closure Glaucoma Progression](https://arxiv.org/abs/2508.14922)
...
TL;DR: 该研究使用AI分割的视神经头结构参数和分区视野功能参数，结合机器学习方法成功分类原发性闭角型青光眼的快速和缓慢进展者，随机森林模型AUC达0.87


<details>
  <summary>Details</summary>
Motivation: 原发性闭角型青光眼(PACG)进展速度差异很大，需要准确分类快速和缓慢进展者以制定个性化治疗方案，但现有方法效果有限

Method: 纳入>5年随访的PACG患者，AI分割OCT获取31个ONH参数，结合分区视野敏感度，使用多种机器学习模型训练分类器，SHAP分析关键预测因子

Result: 分析451眼(299例患者)，82眼快速进展。随机森林模型结合结构和功能特征表现最佳(AUC=0.87)，单独使用结构或功能特征AUC分别为0.82和0.78

Conclusion: 结合ONH结构和VF功能参数显著改善PACG进展风险分类，下象限ONH特征(特别是MRW和RNFL厚度)是最重要的预测因子
...
### [123] [Scalable Event-Based Video Streaming for Machines with MoQ](https://arxiv.org/abs/2508.15003)
*Andrew C. Freeman*
...
<details>
  <summary>Details</summary>
Motivation: 传统视频流采用有损压缩和速率自适应技术，但新兴的神经形态事件传感器使用异步像素采样而非图像帧记录视频。这些传感器专为计算机视觉应用设计，而非人类视频消费。目前研究主要关注应用开发，忽视了数据传输这一关键问题。

Method: 调查基于事件的视频系统现状，讨论最近可扩展事件流工作的技术问题，并基于Media Over QUIC协议草案的最新补充提出新的低延迟事件流格式。

Result: 提出了针对神经形态事件传感器的新型低延迟事件流传输解决方案。

Conclusion: 需要为事件传感器开发专门的数据传输格式和协议，以解决当前研究中被忽视的数据传输问题，Media Over QUIC协议为此提供了良好的基础。
...
### [124] [Zero-shot Volumetric CT Super-Resolution using 3D Gaussian Splatting with Upsampled 2D X-ray Projection Priors](https://arxiv.org/abs/2508.15151)
*Jeonghyun Noh,Hyun-Jic Oh,Byungju Chae,Won-Ki Jeong*

Main category: eess.IV

TL;DR: 提出了一种利用扩散模型生成2D X射线投影先验的零样本3D CT超分辨率框架，通过3D高斯溅射重建高质量CT体积，无需配对训练数据
...
Method: 使用扩散模型在大规模2D X射线投影上训练，提出逐投影自适应采样策略生成HR投影作为先验，结合3D高斯溅射进行3D重建，并引入负alpha混合技术实现残差学习

Result: 在两个数据集上的实验表明，该方法在3D CT超分辨率方面取得了优异的定量和定性结果

Conclusion: 该方法成功利用2D X射线投影先验解决了零样本3D CT超分辨率问题，能够有效恢复高频结构细节，为临床诊断提供高质量CT图像
...
### [125] [Pathology-Informed Latent Diffusion Model for Anomaly Detection in Lymph Node Metastasis](https://arxiv.org/abs/2508.15236)
*Jiamu Wang,Keunho Byeon,Jinsol Song,Anh Nguyen,Sangjeong Ahn,Sung Hak Lee,Jin Tae Kwak*

Main category: eess.IV

TL;DR: 该论文提出了一种结合视觉语言模型和扩散模型的数字病理学无监督异常检测方法，通过组织病理学提示指导重建过程，在胃淋巴结和乳腺淋巴结数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 数字病理学中监督学习方法需要大量标注数据，但实际数据稀缺。无监督异常检测能够在不依赖详尽标注的情况下识别异常组织，扩散模型在医学图像异常检测中表现出色，因此探索结合视觉语言模型的新方法。

Method: 将视觉语言模型与扩散模型结合，使用一组与正常组织相关的病理学关键词作为提示来指导重建过程，通过重建差异区分正常和异常组织。

Result: 在本地医院胃淋巴结数据集和公共乳腺淋巴结数据集上的实验表明，该方法在无监督异常检测方面具有良好性能，并展现了跨器官的泛化能力。

Conclusion: 提出的方法展示了在数字病理学中进行无监督异常检测的潜力，能够有效处理不同器官的数据，为解决医学图像数据标注稀缺问题提供了可行方案。
...
### [126] [Explainable Knowledge Distillation for Efficient Medical Image Classification](https://arxiv.org/abs/2508.15251)
...
Method: 采用VGG19和轻量级Vision Transformers作为教师模型，指导基于OFA-595超网的紧凑学生模型训练，使用混合监督（真实标签+教师软目标），并在COVID-QU-Ex和LCS25000数据集上进行验证，使用Score-CAM可视化解释模型决策过程。

Result: 蒸馏后的学生模型在保持高分类性能的同时，显著减少了参数数量和推理时间，适合在资源受限的临床环境中部署。

Conclusion: 该研究强调了将模型效率与可解释性相结合对于开发实用、可信赖的医疗AI解决方案的重要性，知识蒸馏方法能够有效平衡准确性和计算效率。
...
### [127] [Bladder Cancer Diagnosis with Deep Learning: A Multi-Task Framework and Online Platform](https://arxiv.org/abs/2508.15379)
*Jinliang Yu,Mingduo Xie,Yue Wang,Tianfan Fu,Xianglai Xu,Jiajun Wang*

Main category: eess.IV

TL;DR: 该研究提出了一个用于膀胱癌诊断的多任务深度学习框架，包括分类、分割和分子分型模型，并开发了在线诊断平台，显著提高了诊断准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 当前膀胱癌诊断主要依赖医生经验，存在主观性和变异性，需要客观、准确的计算方法来改善诊断效果。

Method: 使用EfficientNet-B0+CBAM进行分类，ResNet34-UNet+++自注意力机制进行分割，ConvNeXt-Tiny进行分子分型，并开发了基于Gradio的在线诊断平台。
...
### [128] [DoSReMC: Domain Shift Resilient Mammography Classification using Batch Normalization Adaptation](https://arxiv.org/abs/2508.15452)
...
Method: 提出DoSReMC框架，仅微调批归一化层和全连接层，保持预训练卷积滤波器不变，并结合对抗训练方案提升跨域泛化性。

Result: 实验证明BN层是域依赖的主要来源，DoSReMC能显著提升模型在跨域场景下的泛化性能。

Conclusion: 该方法可轻松集成到现有AI流程中，为构建更鲁棒和可泛化的乳腺X线摄影分类系统提供了实用途径。
...
### [129] [Deep Equilibrium Convolutional Sparse Coding for Hyperspectral Image Denoising](https://arxiv.org/abs/2508.15553)
*Jin Ye,Jingran Wang,Fengchao Xiong,Jingzhou Chen,Yuntao Qian*

Main category: eess.IV

TL;DR: 本文提出了一种基于深度平衡模型的卷积稀疏编码框架(DECSC)，用于高识别图像的噪声清除，统一了局部空间-谱相关联性、非局部自相似性和全局空间一致性，并通过固定点问题模型化保证了收敛性。


<details>
  <summary>Details</summary>
Motivation: 高识别图像通常受到复杂噪声的影响，而保持去噪后图像的物理属性对于稳健的HSI去噪至关重要。传统的深度展开方法缺乏收敛保证，需要一种能够统一优化理论和深度学习的方法。

Method: 提出DECSC框架，结合2D卷积稀疏表示保证全局空间一致性，3D卷积稀疏表示捕捉局部空间-谱相细节，嵌入Transformer块使用非局部自相似性，并集成细节增强模块。将CSC模型的近端梯度下降法形式化为固定点问题，通过DEQ框架将迭代更新转换为可学习网络结构。

Result: 实验结果表明，DECSC方法在HSI去噪性能方面超过了当前最先进的方法，实现了更优秀的去噪效果。

Conclusion: 本文提出的DECSC框架通过DEQ模型有效结合了卷积稀疏编码和深度学习的优势，能够在保证收敛性的同时完整地模型HSI的多模态特征，为HSI去噪提供了一种更为稳健和高效的解决方案。
...
### [130] [Are Virtual DES Images a Valid Alternative to the Real Ones?](https://arxiv.org/abs/2508.15594)
*Ana C. Perre,Luís A. Alexandre,Luís C. Freire*

Main category: eess.IV

TL;DR: 本研究探讨使用U-Net和CycleGAN模型从低能量图像生成虚拟双能量减影图像，并评估其在乳腺病变分类中的表现。预训练U-Net模型表现最佳，F1分数达85.59%，但略低于真实图像的90.35%。


<details>
  <summary>Details</summary>
Motivation: 对比增强光谱乳腺摄影(CESM)需要获取高能量图像，这会增加患者辐射暴露。通过图像转换技术从低能量图像生成虚拟双能量减影图像，有望减少辐射风险。

Method: 研究比较了三种模型：预训练U-Net模型、端到端训练的U-Net模型和CycleGAN模型，用于从低能量图像生成虚拟DES图像，并评估这些虚拟图像在良恶性病变分类中的性能。

Result: 预训练U-Net模型表现最佳，使用虚拟DES图像获得85.59%的F1分数，而使用真实DES图像为90.35%。虚拟图像与真实图像之间存在性能差距，但结果显示出临床应用潜力。

Conclusion: 虚拟DES图像生成具有重要临床价值，虽然目前性能略低于真实图像，但未来技术进步有望缩小这一差距，使其成为临床可行的替代方案。
...
### [131] [Label Uncertainty for Ultrasound Segmentation](https://arxiv.org/abs/2508.15635)
...
TL;DR: 该研究提出了一种在医学影像标注中引入放射科医生像素级置信度的方法，通过训练AI模型时利用这些置信度信息，显著提高了分割性能并改善了临床下游任务的预测效果。


<details>
  <summary>Details</summary>
Motivation: 医学影像中放射科医生之间存在观察者间变异性，导致标签不确定性，特别是在肺部超声等主观性较强的模态中。传统方法将标注视为绝对真实值，忽略了这种固有的不确定性。

Method: 设计数据标注协议，让放射科医生为每个标注区域提供像素级置信度值。比较多种将不确定性信息融入学习模型的方法，发现使用60%置信度阈值进行二值化标签训练效果最佳。

Result: 使用置信度信息训练模型显著提高了分割性能，更重要的是改善了临床关键下游任务的表现：S/F氧合比估计、S/F比变化分类以及30天患者再入院预测。高置信度阈值（如60%）明显优于50%阈值的朴素方法。

Conclusion: 标签置信度是一个有价值的信号，适当利用可以显著提升AI在医学影像中的可靠性和临床实用性，训练时使用高置信度像素比使用所有标注像素更有效。
...
### [132] [Hessian-based lightweight neural network for brain vessel segmentation on a minimal training dataset](https://arxiv.org/abs/2508.15660)
*Alexandra Bernadotte,Elfimov Nikita,Mikhail Shutov,Ivan Menshikov*

Main category: eess.IV

TL;DR: 提出了一种名为HessNet的轻量级半监督神经网络，用于脑MRA血管分割，仅需6000个参数，可在CPU上运行，达到最先进的分割精度，并创建了包含200张标注图像的数据集。


<details>
  <summary>Details</summary>
Motivation: 脑磁共振血管成像(MRA)中血管的精确分割对神经外科手术至关重要，但目前缺乏公开的标注数据集，现有方法如Frangi滤波器精度不足，神经网络训练又需要大量标注数据。

Method: 开发了HessNet - 一种基于Hessian矩阵的半监督轻量级神经网络，专门用于3D管状结构分割，模型仅含6000个参数，可在CPU上高效运行。

Result: 在最小训练数据集上达到了最先进的血管分割精度，基于IXI数据集创建了包含200张半手动标注图像的大型脑血管数据集，标注由三位专家在神经血管外科医生监督下完成。

Conclusion: HessNet提供了高效的血管分割解决方案，显著降低了神经网络训练资源需求，使专家能够专注于复杂病例，创建的标注数据集有助于推动该领域研究发展。
...
### [133] [Don't Think Twice! Over-Reasoning Impairs Confidence Calibration](https://arxiv.org/abs/2508.15050)
*Romain Lacombe,Kerrie Wu,Eddie Dilworth*

Main category: cs.AI

TL;DR: 研究发现挑战了'测试时扩展'范式：大型语言模型在推理预算增加时反而出现系统性过度自信，检索增强生成比纯推理在置信度校准方面表现更好，信息访问而非推理深度是关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型作为问答工具时的置信度校准问题，特别是在推理能力和计算预算如何影响置信度评估准确性方面进行系统研究。

Method: 使用ClimateX数据集并扩展到人类和行星健康领域，系统评估不同推理预算下LLM的置信度校准准确性，比较纯推理与检索增强生成方法。

Result: 推理LLM在评估专家置信度时达到48.7%准确率，但增加推理预算会损害校准性能，导致系统性过度自信；检索增强生成达到89.3%准确率，显著优于纯推理。

Conclusion: 信息访问而非推理深度或推理预算是改进知识密集型任务置信度校准的关键瓶颈，检索增强方法比延长推理预算更有效。
...
### [134] [Open-Universe Assistance Games](https://arxiv.org/abs/2508.15119)
...
<details>
  <summary>Details</summary>
Motivation: 解决具身AI代理需要推断和处理未预定义的人类多样化目标和偏好的问题，特别是在开放且不断变化的目标空间中

Method: GOOD方法：使用LLM模拟具有不同复杂意图的用户，通过对话提取自然语言目标，进行概率推理得到目标分布，无需大型离线数据集

Result: 在基于文本的杂货购物领域和AI2Thor模拟家庭机器人环境中测试，使用合成用户配置文件，GOOD方法在LLM和人类评估中均优于无显式目标跟踪的基线

Conclusion: GOOD方法能够有效处理开放宇宙中的目标推理问题，提供丰富的目标表示和不确定性估计，在数据效率方面表现优异
...
### [135] [aiXiv: A Next-Generation Open Access Ecosystem for Scientific Discovery Generated by AI Scientists](https://arxiv.org/abs/2508.15126)
...
TL;DR: aiXiv是一个面向人类和AI科学家的下一代开放获取平台，通过多智能体架构实现研究提案和论文的提交、评审和迭代改进，解决了AI生成研究内容缺乏合适发布渠道的问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成的大量高质量研究内容缺乏合适的发布平台，传统期刊依赖人工评审难以扩展，预印本服务器缺乏质量控制机制，阻碍了AI生成研究对科学进步的贡献。

Method: 采用多智能体架构，提供API和MCP接口，实现人类和AI科学家的无缝集成，支持研究提案和论文的提交、评审和迭代精炼。

Result: 实验表明aiXiv是一个可靠且鲁棒的平台，经过迭代修订和评审后显著提升了AI生成研究提案和论文的质量。

Conclusion: aiXiv为AI科学家建立了下一代开放获取生态系统的基础，加速了高质量AI生成研究内容的发布和传播。
...
TL;DR: LLM4Sweat是一个针对罕见病多汗症的开源LLM框架，通过数据增强、微调和专家评估三阶段流程，为多汗症提供可信赖的共情支持。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗领域的应用受到罕见病数据稀缺和不可靠的限制。多汗症影响2-3%人口，但尚无专门针对该疾病的LLM研究。

Method: 三阶段流程：1）使用前沿LLM从开源数据生成医学上合理的合成数据；2）在开源基础模型上进行微调；3）临床和心理专家评估准确性、适当性和共情性，验证结果迭代丰富数据集。

Result: 实验显示LLM4Sweat优于基线模型，提供了首个针对多汗症的开源LLM框架，为其他类似数据稀缺的罕见病提供了可推广的方法。

Conclusion: 该研究成功开发了针对罕见病多汗症的专用LLM框架，解决了数据稀缺和可信度挑战，为其他罕见病的AI辅助诊疗提供了可行方案。
...
TL;DR: 这篇论文提出了一种受认知心理学启发的多重记忆系统(MMS)，用于改善基于大语言模型的代理在处理历史交互数据时的记忆质量和回想性能问题。


<details>
  <summary>Details</summary>
Motivation: 当前的记忆模块如MemoryBank和A-MEM存储的记忆内容质量较差，影响了回想性能和响应质量，需要更好构建高质量的长期记忆内容。

Method: 设计了受认知心理学理论启发的多重记忆系统(MMS)，将短期记忆处理成多个长期记忆片段，构建检索记忆单元和上下文记忆单元，两者一一对应。在检索阶段匹配最相关的检索记忆单元，然后获取对应的上下文记忆单元作为响应上下文。

Result: 在LoCoMo数据集上与三种方法进行对比，证明了方法的有效性。消融学习实验确认了记忆单元的合理性，分析了选择记忆段数量和存储开销的稳健性。

Conclusion: 该方法能够有效利用历史数据，提高记忆质量和回想性能，具有实际应用价值。
...
### [138] [DiagECG: An LLM-Driven Framework for Diagnostic Reasoning via Discretized ECG Tokenization](https://arxiv.org/abs/2508.15338)
*Jinning Yang,Wen Shi*

Main category: cs.AI

TL;DR: DiagECG是一个将12导联心电图信号与语言模型结合的新框架，通过将连续ECG嵌入离散化为符号标记，使大语言模型能够处理心电图数据并生成临床文本。


<details>
  <summary>Details</summary>
Motivation: 现有自动化心电图分析方法在跨临床任务泛化能力有限，缺乏开放式推理支持，需要一种能够统一处理心电图信号和自然语言的解决方案。
...
Conclusion: 该研究展示了将符号化ECG表示整合到LLMs中进行医学推理的潜力，为心血管诊断提供了更通用的自动化解决方案。
...
### [139] [GraSP: A Unified Graph-Based Framework for Scalable Generation, Quality Tagging, and Management of Synthetic Data for SFT and DPO](https://arxiv.org/abs/2508.15432)
*Bidyapati Pradhan,Surajit Dasgupta,Amit Kumar Saha,Omkar Anustoop,Sriram Puttagunta,Vipul Mittal,Gopal Sarda*

Main category: cs.AI

TL;DR: 提出了一个综合的合成数据生成框架，用于为LLM监督微调和对齐任务生成高质量合成数据，通过模块化流水线和双重质量标注机制确保数据质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的发展严重依赖高质量数据集，但现有数据准备过程开销大且难以规模化，需要自动化解决方案来生成高质量的合成对话数据。

Method: 采用模块化配置流水线，使用启发式规则和LLM评估的双重质量标注机制，从OASST格式对话中自动过滤和评分数据，支持SFT和DPO用例的灵活数据结构。

Result: 开发了一个能够规模化生成高质量合成对话数据的框架，显著减少了LLM训练流水线中数据准备的开销。

Conclusion: 该框架为生成和管理规模化合成对话数据提供了稳健解决方案，支持多种训练范式，降低了人工干预需求。
...
### [140] [Language-Guided Tuning: Enhancing Numeric Optimization with Textual Feedback](https://arxiv.org/abs/2508.15757)
...
Method: 使用多智能体LLM框架，包含顾问、评估器和优化器三个专门代理，通过文本梯度提供语义反馈，形成自我改进的反馈循环

Result: 在六个多样化数据集上，LGT相比传统优化方法实现了显著性能提升，同时保持高可解释性

Conclusion: LGT框架通过语言引导的推理有效解决了配置优化的关键挑战，为机器学习工作流提供了更智能和可解释的优化方案
...
### [141] [See it. Say it. Sorted: Agentic System for Compositional Diagram Generation](https://arxiv.org/abs/2508.15222)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成照片级真实感图像方面表现出色，但在处理流程图等需要精确空间对齐和符号结构的图表时存在困难，需要一种能够保持全局约束和可编辑性的解决方案。

Method: 采用训练免费的智能代理系统，结合视觉语言模型(VLM)和大语言模型(LLM)的迭代循环：批评VLM提出编辑建议，多个候选LLM生成不同的SVG更新策略，评判VLM选择最佳候选方案。

Result: 在10个源自已发表论文流程图的草图测试中，该方法比GPT-5和Gemini-2.5-Pro等前沿闭源图像生成LLM更准确地重建布局和结构，能够精确组合图元而不插入不需要的文本。

Conclusion: 该方法通过程序化SVG输出实现了高质量草图到图表的转换，具有良好的可扩展性和人类参与修正的能力，代码已开源。
...
### [142] [Scalable FPGA Framework for Real-Time Denoising in High-Throughput Imaging: A DRAM-Optimized Pipeline using High-Level Synthesis](https://arxiv.org/abs/2508.14917)
*Weichien Liao*

Main category: cs.AR

TL;DR: 基于FPGA的高速图像预处理管道，通过HLS实现实时去噪和数据减小，适用于PRISM等高速成像工作流


<details>
  <summary>Details</summary>
Motivation: 高速成像技术如PRISM产生的数据速率超过传统实时处理能力，需要低延迟的预处理方案

Method: 使用高级综合(HLS)开发可扩展的FPGA预处理管道，通过burst-mode AXI4接口实现帧减法和平均法，在数据流中直接处理图像

Result: 核心操作在帧间间隔之内完成，实现了内联去噪和数据集减小，为下游CPU/GPU分析提供优化

Conclusion: 该模块化FPGA框架为光谱学和显微镜等延迟敏感的成像工作流提供了实用解决方案
...
### [143] [Evaluating Knowledge Graph Complexity via Semantic, Spectral, and Structural Metrics for Link Prediction](https://arxiv.org/abs/2508.15291)
*Haji Gul,Abul Ghani Naim,Ajaz Ahmad Bhat*

Main category: cs.LG

TL;DR: 本文批判性地评估了CSG指标在知识图谱链接预测中的有效性，发现其对参数敏感且与性能指标相关性弱，提出了更可靠的结构和语义复杂度指标。


<details>
  <summary>Details</summary>
Motivation: 理解数据集复杂度对于评估和比较知识图谱链接预测模型至关重要，但现有的CSG复杂度指标尚未在KG环境中得到验证。

Method: 在多元关系链接预测背景下检验CSG指标，引入transformer嵌入的语义表示，并提出并基准测试一组结构和语义KG复杂度指标。

Result: 发现CSG对参数高度敏感，不能稳健地随类别数量扩展，与MRR和Hit@1相关性弱。提出的关系熵、最大关系多样性和关系类型基数等指标与性能指标呈强负相关。

Conclusion: CSG的稳定性和泛化预测能力在链接预测设置中不成立，需要更稳定、可解释且与任务对齐的数据集复杂度度量方法。
...
### [144] [CITE: A Comprehensive Benchmark for Heterogeneous Text-Attributed Graphs on Catalytic Materials](https://arxiv.org/abs/2508.15392)
*Chenghao Zhang,Qingqing Long,Ludi Wang,Wenjuan Cui,Jianjun Yu,Yi Du*

Main category: cs.LG

TL;DR: 提出了CITE数据集，这是首个大规模异质文本属性图基准数据集，包含43.8万节点和120万边，用于催化材料领域，并建立了标准化评估流程和基准测试
...
Method: 构建CITE数据集（包含四种关系类型），建立标准化评估程序，在节点分类任务上进行基准测试，比较四类学习范式（同质图模型、异质图模型、LLM中心模型、LLM+图模型）

Result: 创建了包含438K节点和1.2M边的大规模异质文本属性图基准数据集，并进行了全面的基准测试和消融实验

Conclusion: CITE数据集填补了异质文本属性图基准数据的空白，为相关研究提供了重要的基础设施和评估标准
...
### [145] [Classification errors distort findings in automated speech processing: examples and solutions from child-development research](https://arxiv.org/abs/2508.15637)
*Lucas Gautheron,Evan Kidd,Anton Malko,Marvin Lavechin,Alejandrina Cristia*

Main category: cs.LG

TL;DR: 本文提出贝叶斯方法分析音频分类算法错误对语言发展研究统计推断的影响，发现在LENA和ACLEW系统中分类错误会显著扭曲效应大小估计，并展示了贝叶斯校准方法的有效性


<details>
  <summary>Details</summary>
Motivation: 随着可穿戴录音设备的普及，研究者大量使用自动分类器分析儿童语言数据，但分类错误对下游统计推断（如相关性估计和效应大小）的影响尚未得到充分研究

Method: 采用贝叶斯方法研究算法错误对关键科学问题的影响，包括兄弟姐妹对儿童语言经验的影响以及儿童产出与输入之间的关联，使用LENA和ACLEW系统的Voice Type Classifier进行对比分析

Result: 分类错误显著扭曲估计结果，自动化注释低估了兄弟姐妹对成人输入的负面影响达20-80%，可能使其低于统计显著性阈值

Conclusion: 贝叶斯校准方法可有效恢复无偏的效应大小估计，但不能提供完美解决方案；所报告的问题和解决方案适用于任何涉及事件检测和分类且错误率非零的分类器
...
### [146] [Intern-S1: A Scientific Multimodal Foundation Model](https://arxiv.org/abs/2508.15763)
...
TL;DR: Intern-S1是一个专门针对科学领域的多模态专家混合模型，拥有280亿激活参数和2410亿总参数，在科学专业任务上超越了开源模型，甚至在某些专业任务上超过了闭源最先进模型。


<details>
  <summary>Details</summary>
Motivation: 当前开源基础模型在热门领域表现接近闭源模型，但在高价值的科学专业领域仍存在明显差距，需要专门针对科学领域的通用理解和推理能力的模型来推动AGI发展。

Method: 采用多模态专家混合(MoE)架构，持续预训练5T token（其中2.5T来自科学领域），在InternBootCamp中进行离线和在线强化学习训练，提出混合奖励(MoR)方法在1000多个任务上同时进行RL训练。

Result: 在综合评估基准测试中，Intern-S1在开源模型中具有竞争力的通用推理性能，在科学领域显著优于开源模型，在分子合成规划、反应条件预测、晶体热力学稳定性预测等专业任务上超越了闭源最先进模型。

Conclusion: Intern-S1通过算法、数据和训练系统的集成创新，成功弥合了科学专业领域中开源与闭源模型之间的性能差距，为科学研究和AGI发展提供了强有力的工具。
...
### [147] [Side Effects of Erasing Concepts from Diffusion Models](https://arxiv.org/abs/2508.15124)
*Shaswati Saha,Sourajit Saha,Manas Gaur,Tejas Gokhale*

Main category: cs.LG

TL;DR: 本文揭示了概念擦除技术(CETs)存在严重的安全漏洞，可以通过层级关系和语义相似提示词轻松绕过，同时还存在属性泄露和注意力异常等副作用。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像生成模型的普及，隐私、版权和安全问题日益突出，概念擦除技术应运而生。但现有CETs的有效性和鲁棒性缺乏系统评估。

Method: 提出了Side Effect Evaluation (SEE)评估基准，包含分层和组合提示词的数据集，通过自动化评估流程从三个维度量化CETs的副作用：邻近概念影响、目标规避和属性泄露。

Result: 实验发现CETs可以通过超类-子类层级关系和语义相似提示词轻松绕过，存在属性泄露问题，并出现注意力集中或分散的反常现象。
...
### [148] [Probability Density from Latent Diffusion Models for Out-of-Distribution Detection](https://arxiv.org/abs/2508.15737)
*Joonas Järve,Karl Kaspar Haavel,Meelis Kull*

Main category: cs.LG

TL;DR: 本文探讨了生成模型中基于似然度的离群分布检测方法，发现在表示空间中训练变分扩散模型可以显著提升OOD检测性能，证明了似然度作为OOD检测器的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管AI快速发展，安全性仍是部署机器学习系统的主要瓶颈。离群分布检测是关键安全组件，而数据似然度是最自然的OOD评分方法。但先前工作报告似然度在实践中经常失败，本文旨在探索这是表示空间的问题还是像素空间的问题。

Method: 训练一个变分扩散模型，不是在图像上而是在预训练ResNet-18的表示空间上，使用OpenOOD套件中的最先进方法进行比较评估。

Result: 在表示空间中训练的生成模型显著提升了基于似然度的OOD检测性能，证明了似然度作为OOD检测器的有效性。

Conclusion: 似然度确实是有效的OOD检测器，先前报告的失败主要是由于在像素空间中训练生成模型的问题，而不是表示空间本身的问题。
...
### [149] [Retrieval-Augmented Review Generation for Poisoning Recommender Systems](https://arxiv.org/abs/2508.15252)
*Shiyi Yang,Xinshu Li,Guanglin Zhou,Chen Wang,Xiwei Xu,Liming Zhu,Lina Yao*

Main category: cs.CR

TL;DR: 本文提出RAGAN攻击框架，利用多模态基础模型的上下文学习能力生成高质量虚假用户资料，通过演示检索和文本风格转换增强攻击效果，在多个真实数据集上实现最先进的中毒攻击性能。


<details>
  <summary>Details</summary>
Motivation: 推荐系统容易受到数据中毒攻击，但现有方法在生成高质量虚假资料方面存在挑战，特别是文本评论质量差影响了攻击效果和隐蔽性。

Method: 提出RAGAN框架，利用多模态基础模型的上下文学习能力，结合演示检索算法和文本风格转换策略，通过越狱器、指令代理和守护者协同优化生成高质量虚假用户资料。

Result: 在多个真实数据集上的综合实验表明，RAGAN实现了最先进的中毒攻击性能，具有很好的攻击迁移性和隐蔽性。

Conclusion: RAGAN框架有效解决了推荐系统数据中毒攻击中虚假资料生成质量的问题，为评估推荐系统鲁棒性提供了新方法。
...
### [150] [IPIGuard: A Novel Tool Dependency Graph-Based Defense Against Indirect Prompt Injection in LLM Agents](https://arxiv.org/abs/2508.15310)
...
### [151] [Self-supervised physics-informed generative networks for phase retrieval from a single X-ray hologram](https://arxiv.org/abs/2508.15530)
...
TL;DR: 这篇论文提出了一种无需训练数据的自学习方法，通过物理信息激励生成对抗网络来从单张X光轩影图像中同时恢复相位和吸收信息，解决传统方法对训练数据和专家调参的依赖问题。


<details>
  <summary>Details</summary>
Motivation: X光相位对比成像虽然能显著改善弱吸收结构的可视化，但传统的相位恢复方法存在多种局限性：需要特定近似或边界条件、要求专家手动调整参数、以及面临训练数据获取困难的挑战。

Method: 采用物理信息激励生成对抗网络（GAN），从单张轩影图像中直接重建样品平面的未传播波场的相位和吸收信息。该方法不需要成对、非成对或模拟的训练数据。

Result: 算法在多种成像条件和样品类型下都表现出稳健且一致的性能，能够为模拟数据和实验数据（来自PETRA III P05光束线）提供量化的高质量重建结果，并同时恢复相位和吸收信息。

Conclusion: 该自学习方法为X光相位恢复提供了一种更加灵活和普遍适用的解决方案，免除了对训练数据的依赖，在复杂或变化的实验条件下也能够实现高质量的重建效果。
...
### [152] [Exploring the Landscape of Non-Equilibrium Memories with Neural Cellular Automata](https://arxiv.org/abs/2508.15726)
*Ethan Lake,Ehsan Pajouheshgar*

Main category: cond-mat.stat-mech

TL;DR: 本文发现二维多体记忆系统的景观比之前认为的要丰富得多，不仅存在Toom规则之外的多种记忆机制，还包括由涨落稳定的有序相和仅在噪声存在时保存信息的记忆方式。


<details>
  <summary>Details</summary>
Motivation: 研究多体记忆系统的多样性，探索除了已知的Toom规则之外，是否存在其他能够在非平衡动力学中长期保持初始条件信息的局部机制。

Method: 结合严格数学证明和机器学习方法，系统研究二维多体记忆系统的动力学行为。

Result: 发现了多种与Toom规则在纠错方式上截然不同的记忆机制，包括由涨落稳定的有序相和仅在噪声环境下保存信息的记忆方式。

Conclusion: 物理系统可以通过多种不同方式实现鲁棒的信息存储，多体记忆的物理机制比之前认识的要丰富得多。
...
### [153] [Adversarial Attacks against Neural Ranking Models via In-Context Learning](https://arxiv.org/abs/2508.15283)
*Amin Bigdeli,Negar Arabzadeh,Ebrahim Bagheri,Charles L. A. Clarke*

Main category: cs.IR

TL;DR: FSAP是一种新颖的黑盒攻击框架，利用LLM的上下文学习能力生成高排名的对抗性文档，无需梯度访问或模型内部操作。
...
Method: 通过少样本提示生成对抗性文档，包括FSAP-IntraQ（同查询有害样本）和FSAP-InterQ（跨查询迁移对抗模式）两种模式。

Result: 在TREC 2020和2021健康错误信息数据集上，FSAP生成的文档在四个神经排序模型中均能超越真实可信文档的排名。

Conclusion: FSAP展示了强大的立场一致性和低检测性，对神经检索系统构成现实且可扩展的威胁，并能跨专有和开源LLM有效泛化。
...
### [154] [On the Effectiveness of Graph Reordering for Accelerating Approximate Nearest Neighbor Search on GPU](https://arxiv.org/abs/2508.15436)
*Yutaro Oguri,Mai Nishimura,Yusuke Matsui*

Main category: cs.IR

TL;DR: 本文首次系统研究GPU上基于图的近似最近邻搜索(ANNS)的图重排序效果，发现内存布局优化可带来15%的QPS提升，且与算法创新正交。


<details>
  <summary>Details</summary>
Motivation: 当前基于图的ANNS方法主要关注算法创新，但忽视了内存布局对执行时间的显著影响，特别是在GPU环境下。

Method: 构建统一评估框架，包括图适配器（将任意图拓扑转换为通用表示）和GPU优化的图遍历引擎，全面评估不同重排序策略。

Result: GPU定向重排序在保持搜索精度的同时，实现了高达15%的QPS提升，证明内存布局优化与现有算法创新正交。

Conclusion: 内存布局优化是基于图ANNS的重要优化方向，与算法创新互补，将发布代码促进可重现性和进一步研究。
...
### [155] [Foundational Design Principles and Patterns for Building Robust and Adaptive GenAI-Native Systems](https://arxiv.org/abs/2508.15411)
*Frederik Vandeputte*
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI虽然具有变革性能力，但其不可预测性和低效性给系统开发带来挑战，需要新的设计原则来构建可靠系统

Method: 提出五大核心支柱的设计原则（可靠性、卓越性、可进化性、自依赖性、保障性）和架构模式（原生单元、有机基质、可编程路由器），构建GenAI原生软件栈

Result: 建立了GenAI原生系统的概念框架，包括技术架构和设计模式，为构建弹性自进化系统提供指导

Conclusion: 需要进一步验证和实验，鼓励相关社区实施和完善这一概念框架，推动未来GenAI原生系统研究
...
### [156] ["Does the cafe entrance look accessible? Where is the door?" Towards Geospatial AI Agents for Visual Inquiries](https://arxiv.org/abs/2508.15752)
...
TL;DR: 提出Geo-Visual Agents愿景，通过分析地理空间图像和多模态数据来回答视觉空间问题，突破传统GIS数据的限制


<details>
  <summary>Details</summary>
Motivation: 传统交互式数字地图依赖预结构化GIS数据，无法有效回答关于世界外观的视觉空间问题，需要新的解决方案

Method: 结合街景图像、地点照片、航拍影像等大规模地理空间图像库与传统GIS数据，构建多模态AI代理

Result: 提出了Geo-Visual Agents的概念框架、感知交互方法和三个示例应用

Conclusion: 该愿景为地理视觉分析开辟了新方向，但面临数据集成、算法开发等挑战，具有重要的研究机遇
...
### [157] [A Vision-Based Shared-Control Teleoperation Scheme for Controlling the Robotic Arm of a Four-Legged Robot](https://arxiv.org/abs/2508.14994)
...
TL;DR: 提出一种基于视觉姿态估计的直观四足机器人遥操作方法，通过外部摄像头检测操作者手腕位置，实时映射到机械臂控制，并结合轨迹规划器确保安全操作


<details>
  <summary>Details</summary>
Motivation: 在危险和远程环境中，四足机器人机械臂需要更安全的遥操作。传统摇杆控制方式不直观且需要专业知识，认知负荷高，缺乏集成的障碍物检测功能，在受限或动态工作空间中碰撞风险大
...
### [158] [Decentralized Vision-Based Autonomous Aerial Wildlife Monitoring](https://arxiv.org/abs/2508.15038)
*Makram Chahine,William Yang,Alaa Maalouf,Justin Siriska,Ninad Jadhav,Daniel Vogt,Stephanie Gil,Robert Wood,Daniela Rus*

Main category: cs.RO

TL;DR: 提出基于视觉的去中心化多无人机系统，用于野生动物监测，具有可扩展、低带宽和最小传感器配置的特点


<details>
  <summary>Details</summary>
Motivation: 野生动物野外作业需要高效的并行部署方法来识别和与特定个体互动，实现同时的集体行为分析以及健康和安全干预
...
### [159] [Lang2Lift: A Framework for Language-Guided Pallet Detection and Pose Estimation Integrated in Autonomous Outdoor Forklift Operation](https://arxiv.org/abs/2508.15427)
...
Method: 集成Florence-2和SAM-2进行语言基础分割，使用FoundationPose进行鲁棒的6D姿态估计，并将结果输入运动规划模块实现全自主叉车操作

Result: 在真实世界测试数据集上达到0.76 mIoU的托盘分割精度，时间和误差分析证明了系统的鲁棒性和部署可行性

Conclusion: Lang2Lift框架在杂乱的多托盘户外场景中表现出色，验证了其在物流和建筑环境中部署的可行性
...
TL;DR: 本文提出了一个语义归一化框架，用于处理视觉叙事（如漫画）中的层次化叙事知识图谱，通过词汇相似性和嵌入聚类来减少标注噪声和提高推理一致性。


<details>
  <summary>Details</summary>
Motivation: 视觉叙事理解需要结构化表示，但现有的符号叙事图谱存在不一致性和冗余问题，相似的动作或事件在不同标注或上下文中被不同标记，这限制了推理和泛化的有效性。

Method: 基于认知基础的叙事理解模型，提出使用词汇相似性和嵌入聚类的方法来整合语义相关的动作和事件，对面板级、事件级和故事级图谱进行归一化处理。

Result: 在Manga109数据集上的初步评估显示，语义归一化在动作检索、角色定位和事件摘要等叙事推理任务中提高了连贯性和鲁棒性，同时保持了符号透明度。

Conclusion: 语义归一化是构建可扩展、认知启发的多模态叙事理解图谱模型的关键步骤，能够有效提升叙事知识图谱的质量和实用性。
...
TL;DR: 本文介绍了adder-viz软件的改进，用于实时事件转码过程的可视化应用


<details>
  <summary>Details</summary>
Motivation: 传统事件视频表示在灵活性、速度和压缩性方面存在局限，ADΔER表示旨在解决这些问题

Method: 改进adder-viz软件，提供实时事件转码过程的可视化工具，支持循环内应用

Result: 开发了MIT许可的开源软件，可从集中式代码库获取

Conclusion: adder-viz软件的改进为事件视频处理提供了更好的可视化工具，有助于ADΔER表示的应用和发展
...
### [162] [A Chinese Heart Failure Status Speech Database with Universal and Personalised Classification](https://arxiv.org/abs/2508.14908)
*Yue Pan,Liwei Liu,Changxin Li,Xinyao Wang,Yili Xia,Hanyue Zhang,Ming Chu*

Main category: eess.AS

TL;DR: 本研究建立了首个中文心衰患者语音数据库，验证了中文音节包含心衰相关信息，提出了个性化配对分类方法和自适应频率滤波器，为心衰检测提供了新基准。


<details>
  <summary>Details</summary>
Motivation: 语音是识别急慢性心衰的经济有效且非侵入性数据源，但缺乏中文音节是否包含心衰相关信息的研究，而其他语言已有相关研究。

Method: 建立首个中文心衰患者语音数据库，包含住院前后的配对录音；采用标准'患者级'和个性化'配对级'分类方法；提出自适应频率滤波器(AFF)进行频率重要性分析。

Result: 证实中文语言在心衰检测中的有效性；统计测试和分类结果显示个体差异是准确性的关键影响因素；配对分类方法可作为理想的说话人解耦基线。

Conclusion: 中文语音包含心衰相关信息，个性化配对分类方法是有效的检测方法，个体差异是主要挑战，提出的方法和数据库为未来研究提供了重要基础。
...
### [163] [Transsion Multilingual Speech Recognition System for MLC-SLM 2025 Challenge](https://arxiv.org/abs/2508.14916)
*Xiaoxiao Li,An Zhu,Youhai Jiang,Fengjie Zhu*

Main category: eess.AS

TL;DR: 本文提出了一种基于Whisper-large-v3和Qwen2.5-7B-Instruct的多语言ASR系统，在MLC-SLM 2025挑战赛Track 1中取得9.83%的WER/CER，获得第三名


<details>
  <summary>Details</summary>
Motivation: 开发高效的多语言自动语音识别系统，解决跨语言语音识别任务，参与国际技术挑战赛

Method: 采用三组件架构：冻结的Whisper-large-v3语音编码器、可训练的线性适配器模块、以及集成LoRA的冻结Qwen2.5-7B-Instruct LLM进行语言解码

Result: 在11种语言的评估集上达到9.83%的词/字符错误率，在全球参赛者中排名第三

Conclusion: 通过预训练模型与任务特定微调的系统组合，能够有效实现多语言ASR的高性能表现
...
### [164] [AmbiSQL: Interactive Ambiguity Detection and Resolution for Text-to-SQL](https://arxiv.org/abs/2508.15276)
*Zhongjun Ding,Yin Lin,Tianjing Zeng*

Main category: cs.DB

TL;DR: AmbiSQL是一个交互式系统，通过自动检测查询歧义并引导用户澄清意图，显著提升Text-to-SQL系统的准确率


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在Text-to-SQL任务中容易出错，查询歧义是主要障碍，导致用户意图误解和SQL生成不准确

Method: 引入细粒度歧义分类法识别影响数据库元素映射和LLM推理的歧义，通过多选问题获取用户反馈来重写模糊问题

Result: 在歧义查询数据集上达到87.2%的歧义检测精度，集成Text-to-SQL系统后SQL精确匹配准确率提升50%

Conclusion: AmbiSQL通过交互式歧义澄清机制显著改善了Text-to-SQL系统的性能，具有实际应用价值
