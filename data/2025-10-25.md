...
Motivation: 开发能够自动识别在线科学学习环境中知识建构水平的工具，以支持大规模的教育话语分析。

Method: 扩展DeBERTa-v3模型，结合Focal Loss、Label Smoothing和R-Drop正则化技术，构建端到端的可复现流程。

Result: 在10折分层交叉验证中达到0.836±0.008的宏F1分数，对高阶认知参与（特别是探索和协商类话语）表现出强敏感性。

Conclusion: 大语言模型能有效捕捉非正式数字学习环境中知识建构的细微指标，为话语分析和认知参与评估提供可扩展的理论驱动方法。
...
<details>
  <summary>Details</summary>
Motivation: 评估AI生成的课程计划在教学实践中的可靠性和有效性，为教育工作者选择合适的AI工具和提示框架提供指导。

Method: 使用五个大语言模型和三种提示框架生成15个高中物理课程计划，通过四个自动化计算指标进行分析：可读性、事实准确性、课程标准对齐度和认知需求。

Result: DeepSeek生成最易读的课程计划，Claude语言最复杂；RACE框架产生最低的幻觉指数和最高的课程标准对齐度；所有模型的学习目标主要集中在布鲁姆分类法的低阶认知层次。

Conclusion: 可读性主要由模型设计决定，而教学可靠性和课程对齐度更依赖提示框架。最佳配置是结合可读性优化的模型、RACE框架和明确的概念清单。
...
<details>
  <summary>Details</summary>
Motivation: 离散扩散模型在视觉语言任务中面临训练-推理差异的严重挑战，初始令牌错误在并行解码时会污染生成上下文，引发错误级联，导致语法错误和语义幻觉。

Method: 提出两阶段训练：首先训练模型修正合成错误建立基础修订能力；然后实施在线自校正循环，让模型学习从专家修正中修订自己的缺陷草稿。
...
<details>
  <summary>Details</summary>
Motivation: 传统机制可解释性技术在分析注意力时随上下文长度呈二次方扩展，超过10万token时需要TB级内存，无法处理长上下文场景。

Method: 提出Stream算法，使用可编译的分层剪枝方法，在近线性时间O(T log T)和线性空间O(T)内估计每个头的稀疏注意力掩码，通过二分搜索式细化保留每个查询的前k个关键块。

Result: 在长链式思维推理轨迹中识别思维锚点，剪枝97-99%的token交互；在RULER基准测试中保留关键检索路径，剪枝90-96%的交互，暴露从针到输出的分层路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有HIV筛查方法主要依赖结构化数据，忽略了临床笔记等非结构化文本中包含的HIV风险信息，而大规模实验室检测不可行，需要更有效的筛查方法。

Method: 设计了一个新颖的流程，利用大语言模型分析电子健康记录中的非结构化文本数据，评估患者是否需要进行进一步HIV检测。

Result: 在伊拉斯姆斯大学医学中心鹿特丹的临床数据上测试，该流程实现了高准确率，同时保持了较低假阴性率。

Conclusion: 基于大语言模型的HIV筛查方法能够有效利用非结构化文本数据，为HIV早期诊断提供新途径。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI和LLM在生命周期评估中的应用日益增多，但缺乏对其可靠性、鲁棒性和可用性的系统性证据，且该领域缺乏标准化的评估框架。

Method: 评估了11个通用LLM（包括商业和开源模型）在22个LCA相关任务中的表现，由17位经验丰富的从业者根据科学准确性、解释质量、鲁棒性、可验证性和指令遵循等标准进行评审，共收集168份专家评审。

Result: 37%的响应被专家判定包含不准确或误导信息；准确性和解释质量评级普遍为中等或良好；幻觉率差异显著，某些模型的幻觉引用率高达40%；开源模型与闭源模型在准确性和解释质量等标准上表现相当或更优。

Conclusion: 在LCA中简单应用LLM存在风险，特别是将其视为自由形式的预言机时，但在解释质量和减轻简单任务劳动强度方面显示出益处。没有基础机制支持的通用LLM使用存在局限性。
...
<details>
  <summary>Details</summary>
Motivation: 传统多模态大语言模型评估存在静态基准测试无法综合评估、人工或模型对比主观性强且成本高、模型可能利用表面特征（如冗长性）来提升胜率等问题。

Method: 采用游戏化评估框架，具体通过Dixit幻想卡牌游戏实现。游戏要求玩家为卡片生成标题，既能欺骗部分玩家选择该卡片，又不能欺骗所有玩家。

Result: 实验表明，Dixit游戏胜率排名与流行MLM基准测试排名完全相关，同时揭示了人类与MLM玩家在策略上的差异以及MLM推理能力的改进空间。
...
<details>
  <summary>Details</summary>
Motivation: 传统优化方法依赖领域专家将实际问题转化为数学模型，LLMs有望通过自然语言理解和代码生成来弥补这一公式化差距。

Method: 在NL4OPT、IndustryOR、EasyLP和ComplexOR四个基准上系统评估DeepSeek-R1，采用LLM-as-a-Judge、Few-shot Learning、Tool Calling和多智能体框架等策略减少幻觉。

Result: DeepSeek-R1作为成本效益高且性能良好的模型，在运筹学应用场景中的有效性仍需深入探索。

Conclusion: 该研究为LLMs在运筹学优化建模中的实际应用提供了系统评估框架和幻觉缓解策略。
...
<details>
  <summary>Details</summary>
Motivation: 传统微调方法成本高、不灵活且不透明，需要一种更高效、灵活的学习方法让LLM代理能够从经验中学习。

Method: 使用情景记忆存储实例级批评，语义记忆提炼这些批评为可重用的任务级指导，结合标注数据和LLM生成的批评进行学习。

Result: 在多样化任务上，结合批评的方法比仅依赖标注的检索式基线准确率提升高达24.8%，揭示了不同模型在处理事实导向与偏好数据时的行为差异。

Conclusion: 记忆驱动的反思性学习为构建更自适应和可解释的LLM代理提供了前景，模型特性和记忆策略共同塑造学习动态。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型存在幻觉和输出不一致问题，而符号NLU系统虽然提供可解释的结构化表示但覆盖范围有限且需要专业知识维护。希望结合两者优势。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种更接近人脑处理语言方式的解析算法，与传统的短语结构解析器不同，该算法能够实时构建依赖关系。

Method: 采用逐个单词处理的依赖解析方法，每个单词在可附加时立即附加到依赖树中，类似于人脑的解析过程。

Result: 该算法的最坏情况复杂度为O(n^3)，但在人类语言中，最坏情况只出现在小规模n值下。

Conclusion: 该依赖解析算法提供了一种高效且符合人脑处理机制的自然语言解析方法，在实际应用中具有良好的性能表现。
...
Method: 总结现有数据集设置，回顾临床决策方法（包括训练时和测试时技术），并扩展评估维度到效率、可解释性等。

Result: 建立了一个统一评估范式，能够澄清假设、标准化比较，并指导开发具有临床意义的LLM。

Conclusion: 该范式为开发临床上有意义的LLM提供了指导，强调了超越准确性的多维度评估的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 希腊语等形态丰富的中等资源语言在NLP发展中面临研究碎片化、架构多样性不足以及上下文长度受限等问题，特别是在法律等专业领域，现有模型通常局限于早期transformer架构和512个token的窗口，无法有效处理长法律文档。

Method: 构建多个大规模希腊语语料库，采用严格的质量驱动过滤和预处理方法；预训练并系统评估多种现代架构（如ELECTRA、ConvBERT、ModernBERT）；提出首个面向法律领域的希腊-英语双语嵌入模型。

Result: 在下游任务上的广泛实验表明，新模型类别证明了所提出方法的有效性，特别是GEM-RoBERTa和GEM-ConvBERT模型显著优于现有基线模型。

Conclusion: 通过精心策划的数据和多样化的现代架构，成功开发了适用于希腊语的高性能NLP模型，为中等资源语言的处理提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有预训练语言模型在序列标注任务中的迁移学习效果仍有提升空间，特别是在领域迁移、信息流限制和生成式模型应用方面存在不足。

Method: 1) 多任务模型结合领域无关文本处理信号；2) 修改自回归大语言模型架构实现层间双向信息流；3) 生成式监督上下文微调框架结合响应导向适应策略。

Result: 提出的模型、方法和框架证明，通过针对性迁移学习范式，预训练神经语言模型在序列标注任务中能达到最佳性能。

Conclusion: 预训练语言模型在序列标注任务中的表现可以通过专门的迁移学习方法得到显著提升，包括多任务学习、架构优化和生成式微调策略。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的工具集通常包含冗余工具，这些工具具有重叠的名称和描述，导致选择歧义。同时，LLM面临严格的输入上下文限制，无法有效考虑大型工具集。

Method: ToolScope包含两个组件：(1) ToolScopeMerger with Auto-Correction：自动审计和修复工具合并，减少冗余；(2) ToolScopeRetriever：对每个查询进行工具排序和选择，压缩工具集以适应上下文限制。

Result: 在三个最先进的LLM和三个开源工具使用基准测试中，工具选择准确率提升了8.38%到38.6%。

Conclusion: ToolScope有效增强了LLM的工具使用能力，通过减少工具冗余和优化工具选择，显著提高了工具选择的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多语言基准测试在捕捉低资源文化细微差异方面存在不足，特别是针对孟加拉语文化知识的评估存在空白。

Method: 构建包含民间传统、烹饪艺术和地区方言的孟加拉语文化知识数据集(BLanCK)，并评估多个多语言语言模型在该数据集上的表现。

Result: 多语言模型在非文化类别表现良好，但在文化知识方面表现显著较差；提供上下文后所有模型的性能都得到大幅提升。

Conclusion: 需要开发上下文感知的架构和精心策划的文化训练数据，以提升语言模型对低资源文化的理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 提升小语言模型在专业领域（如医学问答）的推理能力，特别是在波斯语等资源稀缺语言中，解决数据有限的问题。

Method: 将医学问答数据集翻译成波斯语，使用RLAIF生成拒绝-偏好答案对，通过CoT推理构建数据集，采用DPO训练方法。

Result: 训练出的模型在医学推理能力上显著提升，仅用450万token的数据就超越了基于5700万token预训练的gaokerena-V模型。

Conclusion: 推理导向的训练方法在数据有限的情况下，能高效开发领域特定的语言模型，证明了小数据集也能实现显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型创造力评估方法碎片化，缺乏统一的评估框架，主要由于对创造力的定义和测量方法存在差异。

Method: 构建CreativityPrism框架，包含三个创造力维度（质量、新颖性、多样性），九个任务，三个领域和二十个评估指标，对17个SoTA大语言模型进行评估。

Result: 专有模型和开源模型之间存在显著差距；同一领域内任务表现高度相关，不同领域间相关性较弱；多样性和质量指标强相关，新颖性与其他维度相关性较弱。

Conclusion: 在某一创造力任务或维度上的强表现不一定能推广到其他方面，强调了对大语言模型创造力进行整体评估的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 传统实体链接依赖大量标注数据和模型微调，而现有少样本方法虽然减少了训练需求，但LLM推理成本高昂且效率低下。

Method: 采用结构化管道，结合候选生成、基于上下文的评分、自适应路由和选择性推理，通过互补信号将上下文提及分类为简单和困难案例，分别由低计算实体链接器和针对性LLM推理处理。

Result: 在标准基准测试中，ARTER比ReFinED性能提升最高达+4.47%，在6个数据集中的5个平均提升+2.53%，与全LLM推理管道性能相当，但LLM token使用量减少一半。

Conclusion: ARTER通过智能路由和选择性推理，在保持高性能的同时显著提高了实体链接的效率，为平衡准确性和计算成本提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着结构化文本在技术报告、生成式AI提示等领域的复杂性增加，传统基于句子或段落的分割方法无法有效处理包含表格、代码片段和占位符等非纯语言元素的文本。

Method: BoundRL采用强化学习与可验证奖励（RLVR），通过专门设计的奖励函数联合优化文档重建保真度和语义对齐，并通过系统扰动部分生成序列构建中间候选来缓解熵崩溃问题。

Result: 实验表明，BoundRL使小语言模型（1.7B参数）在复杂LLM应用提示的分割任务上优于大模型的少样本提示，RLVR相比监督微调有显著提升，中间候选的加入进一步提高了性能和泛化能力。

Conclusion: BoundRL为处理复杂结构化文本提供了一种高效的分割解决方案，特别适用于包含多种非纯语言元素的文本场景，在保持高质量分割的同时大幅降低了计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型从预训练数据中继承了刻板印象，导致在NLP任务中对某些社会群体产生偏见行为。然而，立场检测方法中的这种偏见评估在社区中被严重忽视。

Method: 在现有立场检测数据集中自动标注帖子的两个属性：特定群体的方言/语言风格和文本复杂度/可读性，研究这些属性是否影响模型的立场检测决策。

Result: 结果显示LLMs在立场检测任务中表现出显著刻板印象，例如错误地将支持大麻的观点与低文本复杂度相关联，将非裔美国人方言与反对特朗普相关联。

Conclusion: 大型语言模型在零样本立场检测中存在系统性偏见，需要更严格的偏见评估和缓解措施。
...
<details>
  <summary>Details</summary>
Motivation: 当前搜索代理无法同时进行深度多跳推理和广尺度信息收集，这在真实应用如市场分析和业务发展中是严重缺陷。

Method: 通过转换现有数据集的方法构建了包含220个问题的基准测试，涵盖15个不同领域，要求代理处理大量数据并进行深度多跳检索推理。

Result: 实验显示即使最先进的代理在DeepWideSearch上的平均成功率也仅为2.39%，错误分析揭示了四种失败模式：缺乏反思、过度依赖内部知识、检索不足和上下文溢出。

Conclusion: DeepWideSearch基准测试揭示了当前代理架构在整合深度和广度搜索方面的关键限制，为未来开发更强大信息搜索代理提供了研究基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于微调的方法容易产生算术错误和幻觉，基于工具的方法依赖刚性模式且缺乏语义理解。需要整合稳健推理与可靠表格处理的方法。

Method: 采用多智能体框架，包含规划、编码和回答三个专门角色；使用代码执行进行精确表格操作；通过MCTS生成伪黄金轨迹，结合强化学习优化智能体。

Result: 在TableBench上达到62.13%的准确率，超越了OpenAI-o4-mini-high，显示出显著性能提升。

Conclusion: 结合结构化多智能体工作流与强化学习，在表格理解方面展现出巨大潜力，为解决表格推理问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在纯文本输入下的空间推理能力，评估其在语言理解和几何推理之间的差距。

Method: 设计了五个空间推理任务（象限识别、几何变换、距离评估、单词搜索、滑块拼图），通过增加网格维度和复杂度来测试模型性能。

Result: LLMs在小规模任务中表现中等（准确率50%以上），但随着复杂度增加性能急剧下降，平均准确率下降42.7%，最高达84%，所有初始准确率超过50%的任务都至少下降了48%。

Conclusion: LLMs在空间推理方面存在显著局限性，缺乏稳健的空间表征架构，突显了语言理解与空间推理能力之间的差距，为未来语言与几何交叉领域的基准测试奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有的子词分词方法导致模型在推理时只评估特定分词输出的概率，而忽略了同一文本可能有多种分词方式。虽然边际化评估能解决这个问题，但传统采样方法需要昂贵的生成步骤，限制了采样数量和近似精度。

Method: 研究解码无关的采样策略，这些策略不需要语言模型进行生成，而是使用模型和分词器无关的廉价采样方法，通过计算给定分词序列的概率来近似边际概率。

Result: 实验表明，解码无关采样策略在多个开源模型上都能提供足够准确的边际估计，且运行时间成本仅为传统方法的一小部分。

Conclusion: 解码无关采样策略是实现高效边际概率评估的有效方法，在保持精度的同时显著降低了计算开销，可用于下游推理任务。
...
<details>
  <summary>Details</summary>
Motivation: 抑郁症和创伤后应激障碍常常共病且症状相互关联，而现有的自动评估方法通常是二分类且针对单一障碍，临床诊断需要能够跨障碍评估严重程度并提供决策支持解释。

Method: 同步融合三种模态：文本（句子级transformer嵌入）、音频（log Mel统计特征和delta）、面部（动作单元、注视、头部姿态描述符），通过校准的后期融合分类器输出每个障碍的概率和特征级归因。

Result: 在DAIC数据集上的分层交叉验证显示，融合模型在准确率和加权F1上与最强单模态基线相当，同时提高了决策曲线效用和鲁棒性。对于PTSD，融合减少了回归误差并改善了类别一致性。

Conclusion: 该方法为情感临床决策提供了可重复的评估和临床医生在环支持，文本对抑郁症严重程度贡献最大，音频和面部线索对PTSD至关重要，归因结果与语言和行为标记一致。
...
<details>
  <summary>Details</summary>
Motivation: 传统的next-token预测限制了模型捕捉高层次语义结构和长距离上下文关系的能力，需要克服这一限制。

Method: 在标准预训练基础上增加next-context预测目标，训练模型学习多token上下文的预测表示，利用未来token块的错误信号。

Result: 在GPT2和Pythia模型系列上的实验显示，ContextLM在困惑度和下游任务性能上均带来一致改进，模型规模可达15亿参数。

Conclusion: next-context预测为语言建模提供了可扩展且高效的增强路径，能以最小计算开销获得更好的长距离连贯性和注意力分配。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM检索增强生成系统中的引用失败问题，即模型生成有用回答但未能完整引用证据，这与回答失败（回答本身有缺陷）相区别。

Method: 采用两步法：1）研究引用失败发生的情况，通过CITECONTROL基准系统分析回答与证据的关系；2）提出CITENTION框架，整合生成式、基于注意力和基于检索的方法来改善引用。

Result: 实验表明引用失败随关系复杂性增加而增加，CITENTION框架在CITECONTROL基准和迁移设置中显著改善了引用质量。

Conclusion: 通过系统分析引用失败的原因并开发多方法融合的框架，可以有效提升LLM检索增强生成系统的引用准确性。
...
<details>
  <summary>Details</summary>
Motivation: 过程奖励模型在数学等领域已证明能提升复杂推理能力，但在涉及半结构化数据的表格问答任务中的适用性尚未探索。表格问答对PRMs提出了独特挑战，包括大量无关信息、松散连接的推理步骤和领域特定推理。

Method: 评估了最先进的生成式PRMs在表格问答任务中的表现，从答案和步骤两个维度进行分析，结合文本和代码验证的方法。

Result: 结果显示：结合文本和代码验证的PRMs有助于解决方案选择，但难以泛化到域外数据；步骤级验证性能与答案准确性之间相关性较弱，可能源于步骤依赖关系弱和因果联系松散。

Conclusion: 当前PRMs在表格问答任务中存在局限性，研究结果为构建更鲁棒、过程感知的验证器提供了宝贵见解。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂数学运算中表现不佳，虽然集成代码解释器是解决方案，但模型内部概率性推理与外部确定性知识之间存在冲突，导致无效思考。

Method: 提出CoRT框架，采用提示工程数据合成策略，在推理路径中注入多样化提示，生成高质量代码集成推理数据，并通过监督微调、拒绝采样和强化学习优化模型与代码解释器的交互。

Result: 在五个挑战性数学推理数据集上，32B和1.5B模型分别获得4%和8%的绝对提升，同时显著减少token使用量（32B模型约30%，1.5B模型约50%）。

Conclusion: CoRT框架有效解决了大型推理模型与代码解释器集成时的冲突问题，显著提升了数学推理的性能和效率，为模型与外部工具的有效协作提供了可行方案。
...
Method: 通过一系列受控探测实验，分析LLMs在包含语义线索的数据集与去除/随机化语义线索的数据集上的表现差异。
...
Method: 将句子视为原子单位，从静态分块分割转变为支持任意句子组合的灵活检索，减少语义边界检测的计算开销。

Result: 在LongBench V2上的实验评估显示，FreeChunker相比传统分块方法实现了更优的检索性能，同时在计算效率上显著优于现有方法。

Conclusion: FreeChunker通过范式转变从根本上改进了RAG系统的分块策略，提供了更好的适应性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 探索专门在对话数据上进行预训练是否能创造出在形式和功能上都适合的小型语言模型，并研究如何通过微调策略提升模型的对话生成能力。

Method: 基于llamalogue模型进行预训练，采用多种微调策略（包括PPO和DPO）来增强模型的对话生成能力，并在BabyLM基准测试和自定义对话基准上进行评估。

Result: 模型在大多数BabyLM基准测试中表现不佳，但在最小对设置下的对话延续预测中表现出色；PPO微调效果混杂甚至有害，而DPO微调进一步提升了在自定义对话基准上的性能。

Conclusion: 专门在对话数据上预训练能产生优秀的对话延续预测模型，DPO微调比PPO更有效地提升对话生成质量，但模型在通用语言理解任务上仍有局限。
...
Method: 通过重构现有幻觉检测数据集构建NegHalu数据集，在token级别追踪LLMs处理否定输入时的内部状态。

Result: 实验表明LLMs在否定文本中难以有效检测幻觉，经常产生逻辑不一致或不忠实的判断。

Conclusion: 否定文本对LLMs的幻觉检测构成显著挑战，揭示了缓解其意外影响的困难。
...
<details>
  <summary>Details</summary>
Motivation: 为越南多模态法律领域建立基准数据集，推动智能系统在交通标志法规方面的研究与发展。

Method: 通过多模态法律检索和多模态问答两个子任务来评估系统性能。

Result: 最佳报告结果：多模态法律检索F2得分为64.55%，多模态问答准确率为86.30%。

Conclusion: 该任务为越南多模态法律文本处理提供了重要的基准数据集和评估平台。
...
<details>
  <summary>Details</summary>
Motivation: 虽然BERT模型在各种任务上表现出色，但其架构选择相对于Llama3和Qwen3等新型transformer模型已经过时。ModernBERT和NeoBERT在英语基准测试上显示出显著改进，作者希望将这种成功扩展到希伯来语领域。
...
<details>
  <summary>Details</summary>
Motivation: 儿童与照顾者之间的多轮对话具有连续性特征，即对话者之间及时、直接且有意义的交流。现有BabyLM在多轮对话连续性方面存在不足，需要改进。

Method: 提出ContingentChat教师-学生框架，使用新颖的对齐数据集进行后训练，并尝试自适应教师解码策略。

Result: BabyLM生成的回复在语法性和连贯性方面得到改善，但自适应教师解码策略带来的额外增益有限。

Conclusion: 针对性后训练对提升对话质量有益，但连续性仍然是BabyLM面临的挑战性目标。
...
<details>
  <summary>Details</summary>
Motivation: 解决高质量指令数据稀缺而低质量数据被大量浪费的问题，充分利用低质量数据资源提升LLM性能。

Method: 构建MIXTURE数据集，提出LM-Mixup方法：先在MIXTURE上进行监督微调，然后通过GRPO强化学习优化，使用质量、语义对齐和格式合规三个奖励信号。

Result: 仅使用3%的蒸馏数据微调LLM，性能超过全数据集训练，可与最先进的高质量数据选择方法竞争。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的输出具有不同程度的不确定性和正确性，实际可靠性难以保证，需要量化这种不确定性。
...
<details>
  <summary>Details</summary>
Motivation: 提升掩码语言建模效果，增强模型在GLUE任务上的性能表现

Method: 改进的MLM方法，根据模型预测能力调整掩码标记概率；结合子词嵌入增强形态学泛化能力

Result: 在(Super)GLUE任务上性能显著提升，在strict-small赛道中超越基线

Conclusion: 改进的MLM方法和子词嵌入策略有效提升了模型性能，在BabyLM挑战赛中取得成功
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的内部表示可作为学习知识的可靠代理，但现有持续学习方法需要任务标签或面临性能权衡，因此需要开发无需历史数据、能无缝集成多领域知识的解决方案。

Method: RECALL框架通过聚类典型样本计算层级隐藏表示的模型间相似度，进行自适应分层参数融合。浅层保留领域通用特征，深层允许任务特定适应，实现知识对齐。

Result: 在五个NLP任务和多种持续学习场景下的广泛实验表明，RECALL在知识保留和泛化能力方面均优于基线方法，提供了可扩展的无数据LLM演进方案。

Conclusion: RECALL通过表示感知的模型融合，实现了无需历史数据的持续学习，有效抵抗灾难性遗忘，为LLM的演进提供了可扩展的无数据解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型有时能检测到它们正在被评估，并调整行为以显得更加对齐，这会损害安全评估的可靠性。

Method: 采用两阶段训练过程：首先在包含模型事实描述的文档上进行持续预训练，然后通过专家迭代训练模型在评估设置中使用Python类型提示。最后使用激活向量来抑制评估意识。

Result: 研究发现激活向量能够有效抑制评估意识，使模型在存在评估线索时表现得像在部署环境中一样。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型在训练数据中主要反映主流偏好，导致对特定、个体化需求表现不佳，存在偏好覆盖差距。传统方法需要昂贵的重新训练，且难以覆盖所有多样化偏好。

Method: RPS是一种后处理、无需训练的方法，通过在相关偏好邻域采样多个响应来创建候选池，然后选择最符合用户原始意图的响应。

Result: 在三种不同对齐范式(DPA、DPO、SFT)上的实验表明，RPS相比基线方法显著提升鲁棒性，在挑战性偏好上达到69%的胜率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的检索增强生成方法在处理多步问题和异构证据源时存在脆弱性，需要在准确性、延迟和计算资源之间做出权衡。

Method: 将文档、表格和知识图谱线性化为可逆的分层序列，通过头代理和迭代代理进行结构感知的迭代检索，最后合成规范化证据生成答案。

Result: 在HotpotQA、HybridQA/TAT-QA和MetaQA等数据集上，相比现有方法在EM/F1指标上获得一致提升，同时保持高效率。

Conclusion: HSEQ框架实现了格式无关的统一处理、预算感知的迭代机制和证据规范化，显著提升了多步异构问答的可靠性和可审计性。
...
Method: 构建了一个新的21种语言多并行版本的EuroParl数据集，包含演讲者政治背景信息，系统比较欧洲议会中不同政党演讲的翻译质量差异。

Result: 发现主流政党（左翼、中间、右翼）的演讲翻译质量显著优于边缘政党，揭示了LLMs在政治翻译中存在的系统性偏见。

Conclusion: 基于翻译公平性的评估方法能有效揭示LLMs的政治偏见，为更全面的偏见评估提供了新工具和视角。
...
TL;DR: ARC-Encoder是一种可适配的文本表示压缩器，通过将上下文压缩为连续表示来替代解码器LLM中的token嵌入，在保持性能的同时提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有上下文压缩技术需要微调目标模型或修改架构，这会降低模型的通用能力。作者探索了一种替代方法：使用编码器将上下文压缩为连续表示。

Method: 设计ARC-Encoder编码器，输出比文本token少4-8倍的连续表示。系统研究了训练策略和架构选择，使单个编码器能适配多个解码器LLM。

Result: ARC-Encoder在多个基准测试中达到最先进性能，同时提高了推理时的计算效率，并能同时适配多个解码器。

Conclusion: ARC-Encoder提供了一个灵活高效的解决方案，使便携式编码器能够无缝地与多个LLM协同工作。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法区分语言模型是真正理解语法结构还是仅依赖语义模式匹配，需要开发新方法来量化模型何时从结构分析转向语义关联。

Method: 构建包含中心嵌套句的数据集，每个句子都有语法相同但语义不合理的对照版本，包含6类理解问题测试表层理解、句法依赖和因果推理。

Result: 测试6个模型发现，随着句子复杂度增加，合理与不合理句子间的性能差距系统性地扩大（中位数差距达26.8个百分点），表明模型在复杂情况下放弃结构分析而依赖语义关联。

Conclusion: CenterBench提供了首个识别模型何时从结构分析转向模式匹配的框架，发现语义合理性在需要因果推理的问题上反而损害性能，推理模型虽提高准确性但仍存在语义捷径等问题。
...
TL;DR: GlobalRAG是一个强化学习框架，通过分解问题为子目标、协调检索与推理、迭代优化证据来解决多跳问答中的全局规划和忠实执行问题，在少量训练数据下显著提升性能。
...
Method: 提出GlobalRAG框架，包含：问题分解为子目标、检索与推理协调、证据迭代优化；引入规划质量奖励和子目标完成奖励；采用渐进权重退火策略平衡过程导向和结果导向目标。

Result: 在领域内和领域外基准测试中，GlobalRAG仅使用8k训练数据（强基线训练数据的42%），在EM和F1指标上平均提升14.2%，显著优于强基线方法。
...
TL;DR: 提出多智能体认知决策框架（MACDF），将电商搜索从被动检索转变为主动决策支持，解决传统检索-排序范式与用户多阶段认知决策过程的不匹配问题。
...
Method: 提出多智能体认知决策框架（MACDF），通过多智能体系统模拟用户的认知决策过程，从被动检索转向主动决策支持。

Result: 离线评估显示MACDF在推荐准确性和用户满意度方面显著提升，特别是在涉及否定、多约束或推理需求的复杂查询中。在JD搜索平台的在线A/B测试证实了其实际效果。

Conclusion: 这项工作凸显了多智能体认知系统在重新定义电商搜索方面的变革潜力，实现了从被动检索到主动决策支持的范式转变。
...
TL;DR: 本文研究ChatGPT在自动编码沟通数据时是否存在性别和种族偏见，发现在协作问题解决框架下无显著偏见。


<details>
  <summary>Details</summary>
Motivation: 大规模评估沟通协作需要人工编码沟通数据，虽然ChatGPT能直接按编码规则自动编码且准确性接近人类评分员，但其是否存在针对不同人口群体的偏见尚不明确。

Method: 使用典型的协作问题解决编码框架，分析三种协作任务数据：谈判、问题解决和决策制定，检验ChatGPT编码在不同性别和种族群体间的差异。
...
TL;DR: 团队在阿拉伯语AI生成文本检测任务中获第5名，发现多语言XLM-RoBERTa模型性能优于专门的阿拉伯语模型。
...
TL;DR: 本文基于人类好奇心评估问卷5DCR，构建了评估大语言模型好奇心的框架，发现LLMs比人类有更强的求知欲但在不确定环境中更保守，好奇心行为能增强模型的推理和主动学习能力。
...
Method: 基于人类好奇心评估问卷Five-Dimensional Curiosity scale Revised (5DCR)，设计了涵盖信息寻求、刺激寻求、社交好奇心等维度的综合评估框架。
...
TL;DR: 大型推理模型在非英语问题上倾向于使用英语推理，这导致认知行为表现更好且准确率更高，但随着任务复杂度增加，这种英语中心策略容易出现翻译错误。


<details>
  <summary>Details</summary>
Motivation: 探索大型推理模型的多语言推理能力，关注其在非英语问题中默认使用英语推理的现象及其对解释性和语言文化细微差别处理的影响。

Method: 系统比较模型在英语与问题语言中的推理表现，评估MGSM和GPQA Diamond两个任务，分析推理轨迹中的认知属性。

Result: 英语推理轨迹显示出更高的认知行为存在度，英语推理通常获得更高的最终答案准确率，且性能差距随任务复杂度增加而扩大，但英语中心策略容易因翻译步骤导致错误。

Conclusion: 大型推理模型存在英语中心倾向，虽然能提高准确率但容易产生翻译错误，需要平衡英语推理优势与多语言推理的细微差别处理。
...
Method: 构建了CantoNLU基准测试，包含7个任务：词义消歧、语言可接受性判断、语言检测、自然语言推理、情感分析、词性标注和依存句法分析。评估了四种模型：未在粤语上训练的普通话模型、通过持续预训练获得的粤语适应模型、从头训练的粤语单语模型。
...
TL;DR: 提出神经多样性作为减少语言模型幻觉的新机制，通过解相关的并行表示来降低幻觉率，而无需增加参数或数据量。
...
Result: ND-LoRA将幻觉减少高达25.6%（平均14.6%），且不降低一般准确性。神经相关性增加0.1%与幻觉增加3.8%相关，不同任务需要不同的最优神经多样性水平。

Conclusion: 神经多样性是改进语言模型可靠性的第三个扩展维度，与参数和数据正交，可在固定预算下提高模型性能。
...
<details>
  <summary>Details</summary>
Motivation: MBR解码在机器翻译中有效，但在对话等开放任务中可能选择过于平均但次优的响应，因为标准相似性效用函数无法捕捉潜在结构变化。

Method: 提出了三种轻量级效用函数适配方法，使MBR对结果空间的结构变化更敏感；构建了包含对话行为、情感和响应结构的数据集；提出了两个评估结构最优性的指标。

Result: 标准相似性效用函数在结构最优性指标上表现不佳，而提出的适配方法显著改善了结构最优性；在真实指令跟随基准测试中，生成质量提升达13.7个百分点。

Conclusion: 增强MBR对结构变化的敏感性可以显著提高开放任务中的生成质量，提出的轻量级适配方法有效解决了标准MBR在结构多样性任务中的局限性。
...
TL;DR: 研究发现LLM在隐私保护方面存在用户感知差异，代理LLM评估与真实用户评估相关性低，需要用户中心的研究方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM隐私评估依赖代理LLM，忽视了真实用户感知，且主要关注隐私保护质量而未研究帮助性的细微差异。

Method: 使用PrivacyLens的90个场景，对94名参与者进行用户研究，比较用户与5个代理LLM对隐私保护质量和帮助性的评估。

Result: 用户对相同LLM响应的隐私保护质量和帮助性评估一致性低，代理LLM间一致性高但与用户评估相关性低。

Conclusion: LLM响应的隐私和帮助性具有个体特异性，代理LLM无法准确预测用户感知，需要用户中心的研究方法。
...
TL;DR: 开发并评估了多种NLP方法从临床笔记中提取氟嘧啶治疗和毒性信息，发现基于LLM的方法表现最佳，特别是错误分析提示法在所有指标上都达到完美分数。


<details>
  <summary>Details</summary>
Motivation: 氟嘧啶广泛用于结直肠癌和乳腺癌治疗，但存在手足综合征和心脏毒性等副作用。由于毒性信息通常嵌入在临床笔记中，需要开发NLP方法来自动提取这些信息。

Method: 构建了包含236份临床笔记的金标准数据集，开发了基于规则、机器学习（随机森林、SVM、逻辑回归）、深度学习（BERT、ClinicalBERT）和LLM（零样本提示和错误分析提示）的NLP方法，使用80:20的训练测试分割。

Result: 错误分析提示法在治疗和毒性提取上达到最优表现（F1=1.000），零样本提示法在治疗提取上F1=1.000，毒性提取F1=0.876。逻辑回归和SVM在毒性提取上排名第二（F1=0.937）。深度学习方法表现较差，规则方法作为基线表现一般。
...
TL;DR: 本文首次系统分析了大型推理模型作为机器翻译评估器的潜力，发现其存在评估材料需定制化、对简单实例过度思考、评分机制导致高估等问题，并提出通过训练合成的人类思维轨迹来校准模型思维，显著减少了35倍思考预算并提升了评估性能。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务中展现出了强大的推理能力，但作为机器翻译质量评估器的潜力尚未充分探索。本文旨在系统分析LRM在MT评估中的应用，识别关键挑战并提出改进方案。

Method: 通过训练大型推理模型学习合成的人类思维轨迹来校准其思维过程，减少过度思考问题，优化评分机制。实验在WMT24 Metrics基准上进行，涵盖7B到32B不同规模的模型。

Result: 该方法大幅减少了约35倍的思考预算，同时提升了评估性能。例如，R1-Distill-Qwen-7B模型的相关性得分提高了8.7个百分点，在不同规模的LRM上都表现出改进。

Conclusion: 经过有效校准的大型推理模型具有推动细粒度自动机器翻译评估的潜力，通过优化思维过程可以显著提升评估效率和准确性。
...
TL;DR: 构建了一个基于真实应用场景的数据集，用于评估大语言模型在负责任AI维度（特别是公平性）上的表现，提出了结合具体资源的评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型的方法主要关注文本生成等高层任务，没有针对特定AI应用，这种通用方法不足以评估负责任AI维度如公平性，因为不同应用中受保护属性的相关性不同。

Method: 构建了一个基于真实应用场景（根据产品特征生成纯文本产品描述）的数据集，该数据集通过公平性属性与性别化形容词和产品类别的交叉参数化，产生丰富的标记提示集合。
...
TL;DR: 本文提出了一种混合注意力模型，在线性注意力基础上引入稀疏注意力机制，通过可学习的token淘汰策略和滑动窗口注意力，在保持线性注意力效率的同时缓解遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 线性注意力模型虽然高效，但有限的内存会导致遗忘问题，影响检索密集型任务的性能。

Method: 提出混合模型，结合线性注意力和稀疏注意力，包括token淘汰稀疏注意力和查询感知原生稀疏注意力。特别提出可学习的token淘汰方法，结合滑动窗口注意力和轻量级CNN，自适应保留关键KV对。
...
<details>
  <summary>Details</summary>
Motivation: 在检索增强生成(RAG)中使用长上下文会导致高昂的计算成本，软上下文压缩通过将输入序列转换为更短的连续表示来降低这些成本。

Method: 开发了一种轻量级且简单的均值池化方法，并训练同一压缩器以输出多种压缩比率。在多个领域内外QA数据集、不同模型家族、规模和压缩比率上进行了广泛实验。

Result: 简单的均值池化方法实现了最强的性能，在训练多种压缩比率时性能下降相对较小。不同架构和训练机制之间的权衡更为复杂。
...
TL;DR: 论文指出当前LLM生成文本检测面临定义不明确、检测目标范围有限、人机文本界限模糊等问题，现有评估方法无法充分反映真实应用场景，检测结果应谨慎解读。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，检测其生成文本的需求日益增长，但现有检测方法存在定义不一致、评估不充分等问题，需要重新审视检测器的有效性和局限性。

Method: 通过分析LLM生成文本的定义多样性、人机文本界限模糊性以及现有评估方法的不足，提出对检测器结果应持谨慎态度的观点。

Result: 研究发现检测器在特定条件下仍有用，但其数值结果常被误解，检测意义正在减弱，检测结果应仅作为参考而非决定性指标。

Conclusion: LLM生成文本检测面临严峻挑战，检测器结果需要谨慎解读，未来需要更完善的评估框架来应对真实应用场景的复杂性。
...
TL;DR: 使用频域分析和深度学习来区分StyleGAN生成的图像与真实图像，通过2D DFT转换图像到傅里叶域检测周期性伪影，ResNet50网络在频域图像上训练，准确率达92.8%，AUC为0.95。
...
Method: 应用二维离散傅里叶变换将图像转换到频域，利用ResNet50神经网络在频域图像上进行训练，检测GAN图像的周期性伪影特征。

Result: 频域模型准确率达到92.8%，AUC为0.95，显著优于在原始空间域图像上训练的同等模型。

Conclusion: GAN生成的图像具有独特的频域特征，结合信号处理技术和深度学习的方法在数字取证和增强工业AI系统可信度方面具有工业应用潜力。
...
Conclusion: ViT能够捕捉全局形状语义，对比学习优化局部判别特征，这种结合克服了传统方法对大量标注数据的依赖和CNN的局限性，为3D表示学习提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D多目标跟踪方法主要依赖单一传感器，缺乏对多模态传感器特征的充分利用，导致在遮挡和视角变化下的重识别性能不足。

Method: 采用基于查询的两阶段transformer精炼和跟踪流水线，融合相机和LiDAR的多模态BEV特征，无需显式运动模型，通过时间平滑器在移动窗口内优化边界框序列。

Result: 在nuScenes测试集上达到74.7 aMOTA，显著减少了身份切换，在3D MOT基准测试中保持竞争力的准确性。

Conclusion: 基于查询的transformer跟踪方法通过多模态传感器特征显著提升性能，即使数据有限且无需预训练，也能与其他基于神经网络的方法竞争。
...
TL;DR: 论文提出使用在线标签平滑（OLS）方法来解决医学图像分类中深度学习模型的过度自信预测问题，相比传统标签平滑方法，OLS能动态调整软标签并考虑类别间关系，在RadImageNet数据集上显著提升了分类准确率和特征表示质量。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学图像分类中常产生过度自信的预测，这在医疗关键应用中会降低可靠性。传统标签平滑方法将所有非目标类别同等对待，未能考虑类别间的关系。

Method: 采用在线标签平滑（OLS）方法，这是一种动态方法，在训练过程中根据模型自身的预测模式调整软标签。在RadImageNet数据集上使用ResNet-50、MobileNetV2和VGG-19三种架构进行评估。

Result: OLS相比标准训练方法（包括硬标签、传统标签平滑和无教师知识蒸馏）持续提升了Top-1和Top-5分类准确率。同时，OLS产生了更紧凑和分离良好的特征嵌入，表明表示学习得到改善。
...
TL;DR: 提出了一种针对鱼眼相机图像的目标检测框架，通过预处理和后处理管道以及模型集成策略，在鱼眼交通图像上实现稳健检测，在AI City Challenge Track 4中获得第8名。


<details>
  <summary>Details</summary>
Motivation: 鱼眼相机在广域交通监控中具有优势，但其固有的径向畸变和非均匀分辨率给标准目标检测器带来挑战，特别是在图像边界区域物体外观严重退化时。

Method: 采用简单有效的预处理和后处理管道来提高检测一致性，训练多个先进检测模型并通过集成策略结合它们的输出以提高整体检测精度。
...
Method: 利用中间梯度推导敏感性分数，专门针对各向异性方向引起的不稳定性进行过滤，而非各向同性方差

Result: 实验证明该方法相比现有NeRF方法显著提升视觉质量、真实感和一致性，并能实时集成到现有3DGS渲染管线

Conclusion: 该方法有效解决生成不确定性核心问题，使3D重建系统在用户自由导航超出原始训练视角时仍能保持高视觉保真度
...
TL;DR: 提出BrainPuzzle混合框架，结合物理建模与机器学习，通过两阶段方法解决颅骨超声成像中声速重建的挑战，在低信噪比和稀疏孔径条件下实现准确的定量脑部声速成像。


<details>
  <summary>Details</summary>
Motivation: 传统物理方法受颅骨衰减、模式转换和相位畸变导致的弱信号限制，而纯数据驱动方法难以建模复杂非线性波传播，导致定量偏差。需要结合两者优势解决定量经颅超声成像的挑战。

Method: 两阶段混合框架：第一阶段使用逆时偏移处理多角度采集数据生成迁移片段；第二阶段使用基于Transformer的超分辨率编码器-解码器与图注意力单元融合片段为连贯的声速图像。采用可移动低计数换能器的部分阵列采集策略。

Result: 在两个合成数据集上的实验表明，BrainPuzzle实现了优越的声速重建精度和图像完整性，显著优于传统方法。

Conclusion: BrainPuzzle展示了推进定量超声脑成像的潜力，通过物理与学习方法的结合有效解决了稀疏孔径和低信噪比条件下的成像挑战。
...
TL;DR: 本文提出了一个统一的评估框架，用于诊断生成式图像模型中的文化偏见，涵盖文本到图像生成和图像到图像编辑，发现在国家无关提示下模型偏向全球北方现代描绘，迭代编辑会侵蚀文化保真度，当前系统难以实现文化敏感的编辑。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注文本到图像系统的文化偏见，而图像到图像编辑器的文化偏见研究不足，需要建立一个标准化评估协议来诊断生成式图像模型中的文化偏见问题。

Method: 使用统一评估框架，涵盖六个国家、8个类别/36个子类别的分类体系，结合自动指标、文化感知检索增强VQA和本地评审专家的专业判断，采用固定设置的开放模型进行跨国家、跨时代、跨类别评估。

Result: 研究发现：(1)国家无关提示下模型默认偏向全球北方现代描绘；(2)迭代编辑会侵蚀文化保真度；(3)图像到图像模型仅应用表面线索而非时代一致、上下文感知的变化。

Conclusion: 当前系统的文化敏感编辑仍不可靠，通过发布标准化数据、提示和人工评估协议，为诊断和追踪生成式图像模型中的文化偏见提供了可复现的、以文化为中心的基准。
...
TL;DR: FIBAR是一种基于滤波器的异步重建方法，用于从移动事件相机的事件中重建强度图像，相比神经网络方法更简单且能在CPU上高效运行。


<details>
  <summary>Details</summary>
Motivation: 现有的事件相机图像重建方法通常依赖神经网络和GPU，计算复杂度高。本文旨在开发一种更简单、异步且能在CPU上高效运行的重建方法。

Method: 使用时间数字IIR滤波器积分事件信号强度变化，通过新颖算法检测陈旧像素并调节更新窗口，对陈旧像素应用高斯滤波以降低噪声。

Result: FIBAR在现代笔记本电脑CPU上处理速度达42-140百万事件/秒，重建图像比神经网络方法噪声更大且存在鬼影，但足以完成某些任务如基准标记检测。

Conclusion: FIBAR提供了一种简单高效的异步图像重建方案，虽然重建质量不如神经网络方法，但在特定应用场景下具有实用价值。
...
TL;DR: 提出TBTLRR模型，通过数据自适应的张量核范数学习任意酉变换，结合双边结构和混合正则化项，提升图像聚类的鲁棒性和性能。


<details>
  <summary>Details</summary>
Motivation: 现有张量低秩表示方法依赖固定变换，对噪声鲁棒性差。需要开发能自适应学习变换、更好捕捉全局和局部相关性的方法。

Method: TBTLRR模型引入数据自适应的张量核范数学习任意酉变换，利用双边结构捕捉图像样本和特征的局部相关性，集成ℓ1/2-范数和Frobenius范数正则化处理复杂噪声，采用ADMM优化算法。

Result: 大量实验验证TBTLRR在聚类任务上优于现有最先进方法，代码将在GitHub开源。

Conclusion: TBTLRR通过自适应变换学习、双边结构和混合正则化，显著提升了张量低秩表示在图像聚类中的性能和鲁棒性。
...
TL;DR: Endoshare是一个开源、跨平台的应用，用于标准化和匿名化内窥镜手术视频，解决视频格式异构和隐私问题，促进手术数据科学的发展。
...
Method: 采用软件开发生命周期，结合用户中心反馈。分析阶段通过内部调查确定关键需求，设计隐私优先架构。测试阶段通过外部调查评估可用性和采纳度，并进行硬件配置基准测试。

Result: 初步测试显示高可用性（4.68/5和4.03/5），改进后外科医生报告高感知有用性（5.07/7）、易用性（5.15/7）、启发式可用性（4.38/5）和强烈推荐（9.20/10）。处理时间受处理模式、视频时长和机器计算能力影响。
...
TL;DR: 本文提出了注意力卷积（ATConv），通过将自注意力的两个关键特性——自适应路由和侧向抑制——引入传统卷积，在保持线性复杂度的同时显著提升了CNN性能，在多个视觉任务中超越了自注意力机制。


<details>
  <summary>Details</summary>
Motivation: 自注意力机制虽然表达能力强但存在二次复杂度瓶颈，而传统卷积虽然线性复杂度但性能存在差距。本文旨在探索自注意力优于卷积的内在原理，并设计兼具两者优点的卷积变体。

Method: 揭示了自注意力的两个关键特性：自适应路由（根据语义内容动态调节位置信息流）和侧向抑制（通过得分竞争抑制冗余）。基于此提出了ATConv，在3×3卷积核中注入这些原理。

Result: ATConv在基础视觉任务中持续优于各种自注意力机制。基于ATConv的AttNet在ImageNet-1K上达到84.4%的top-1准确率（仅27M参数）。在扩散模型中用ATConv替换自注意力可降低FID 0.15并加速采样。

Conclusion: 通过重新审视CNN设计原则并引入自注意力的关键特性，ATConv成功弥合了卷积与自注意力之间的性能差距，为构建高效高性能的视觉主干网络提供了新方向。
...
TL;DR: StableSketcher是一个基于扩散模型的框架，专门用于生成高质量的手绘草图，通过优化VAE解码器和引入基于视觉问答的强化学习奖励函数，提升了草图的风格保真度和文本-图像对齐效果。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在生成像素级手绘草图方面存在挑战，特别是对抽象表达的处理。现有数据集主要基于图像-标签对，缺乏实例级草图与描述文本的配对数据。

Method: 1. 微调变分自编码器以优化潜在解码，更好捕捉草图特征；2. 集成基于视觉问答的新奖励函数进行强化学习，改善文本-图像对齐和语义一致性。

Result: 实验表明StableSketcher相比Stable Diffusion基线能生成风格保真度更高的草图，与提示词对齐效果更好。同时构建了首个实例级草图数据集SketchDUO。

Conclusion: 提出的框架成功解决了手绘草图生成中的挑战，通过技术改进和新数据集填补了该领域的研究空白。
...
TL;DR: 该研究探索使用描述性标题作为生物多模态基础模型的额外监督源，通过多模态大语言模型生成合成标题来解决大规模获取实例特定标题的挑战，训练出的BIOCAP模型在物种分类和图文检索方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 图像和标题可以视为物种潜在形态空间中的互补样本，各自捕捉某些生物特征。在训练中融入标题可以鼓励与这种共享潜在结构对齐，强调潜在诊断特征同时抑制虚假相关性。主要挑战在于大规模获取忠实、实例特定的标题。

Method: 使用多模态大语言模型生成合成标题，通过维基百科衍生的视觉信息和分类群定制的格式示例作为指导。这些领域特定上下文有助于减少幻觉并产生准确的、基于实例的描述性标题。

Result: 使用这些标题训练了BIOCAP模型，该模型捕获了丰富的语义，在物种分类和文本-图像检索方面实现了强大的性能。
...
<details>
  <summary>Details</summary>
Motivation: 计算机视觉在通用物体检测和跟踪方面已有显著进展，但针对快速移动小物体的特定问题仍未被充分探索。现有方法在处理这类挑战性场景时存在局限性。

Method: 1) 综合系统设计用于3D空间中快速移动小物体的检测与跟踪；2) 创新的基于物理的跟踪算法，集成运动学方程处理异常值和漏检；3) 异常值检测和校正模块，提升在遮挡和快速方向变化等挑战场景下的跟踪性能。

Result: 在自定义壁球数据集上的评估显示，该系统相比基于卡尔曼滤波器的跟踪器减少了高达70%的平均位移误差。

Conclusion: 该系统在自主平台机器人感知方面具有重要应用价值，证明了将物理模型与深度学习方法相结合在实时3D检测和跟踪挑战性小物体方面的有效性。
...
TL;DR: 提出了一种从单张图像生成光场的逆图像渲染方法，通过神经网络渲染管线重建光线流，实现新颖视角合成。
...
Method: 设计神经渲染管线，通过交叉注意力计算源光线间关系，预测目标光线颜色，并迭代更新生成内容以确保遮挡区域一致性。

Result: 在多个挑战性数据集上表现良好，无需重新训练或微调，优于相关最先进的新视角合成方法。

Conclusion: 逆图像渲染方法有效实现了从单张图像生成光场，为光场技术的广泛应用提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有的后处理方法往往未充分利用模型logits空间中的丰富信息，特别是在最大logit与其余logits关系方面。

Method: 提出LogitGap方法，显式利用最大logit与剩余logits的关系，并引入无训练策略自动识别最有信息的logits子集进行评分。

Result: 在视觉语言和纯视觉模型上的广泛实验表明，LogitGap在多样化OOD检测场景和基准测试中持续实现最先进性能。

Conclusion: LogitGap通过有效利用logits空间信息，为后处理OOD检测提供了高效且性能优异的方法。
...
Method: 构建包含23,000+高质量纹理3D模型的数据集，提供细粒度分层部件标注，涵盖50个类别。在类无关部件分割和3D部件问答两个任务上进行基准测试。

Result: 在类无关部件分割任务中，现有方法在细粒度和叶级部件上表现不佳；在3D部件问答任务中，3D-LLMs在开放词汇部件定位方面存在显著差距。使用PartNeXt训练Point-SAM相比PartNet获得显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶中，骑行者属于易受伤害的道路使用者，准确估计其姿态对于意图分类、行为预测和碰撞避免至关重要。传统的6D姿态估计无法处理自行车关节结构的变化，因为车把和踏板的角度变化会影响3D边界框和实际行驶方向。

Method: 提出联合估计8D姿态和3D关键点的方法，使用合成和真实图像数据进行训练，以在真实图像上实现泛化。8D姿态包括3D平移、旋转以及车把和踏板相对于自行车车架的旋转。

Result: 方法在8D姿态参数估计准确性方面表现出有希望的结果，与使用刚性规范对象模板匹配的最先进类别级6D姿态估计器相比，取得了竞争性分数。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在测试时面临标签空间分布偏移的挑战，这是由于包含了从属性和对象重新组合的未见组合导致的性能下降。

Method: 1) 使用自适应更新权重控制原型调整程度；2) 引入动态优先级队列存储高置信度图像；3) 通过多模态协作表示学习对齐文本和视觉原型。

Result: 在四个基准数据集上的闭世界和开世界设置下均达到最先进性能。

Conclusion: 该方法能有效适应测试时的分布偏移，在组合零样本学习任务中表现优异。
...
TL;DR: IB-GAN是一种基于GAN的无监督解耦表示学习模型，通过信息瓶颈框架优化GAN，利用生成器中间层约束输入与输出之间的互信息，实现可解释的潜在空间学习。


<details>
  <summary>Details</summary>
Motivation: 利用信息瓶颈(IB)框架优化GAN，解决InfoGAN在解耦表示学习中的局限性，实现更好的潜在空间控制和可解释性。

Method: 在GAN架构中引入中间随机层作为可学习的潜在分布，通过约束输入与生成输出之间的互信息，实现端到端的联合训练。
...
Conclusion: IB-GAN成功将信息瓶颈框架应用于GAN优化，实现了有效的解耦表示学习，在生成质量和解耦性能方面均优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 立体视频的时序一致性深度估计对于增强现实等应用至关重要，但现有方法在建模长程时间依赖性和计算效率之间存在根本性权衡。

Method: 受人类两阶段决策过程启发，提出Pick-and-Play Memory模块：'pick'过程选择最相关帧，'play'过程自适应加权选中的帧进行时空聚合，维持紧凑且信息丰富的记忆缓冲区。

Result: 在Sintel数据集上，PPMStereo在clean/final测试中分别达到0.62/1.11 TEPE，相比BiDAStereo提升17.3%和9.02%，且计算成本更低。

Conclusion: PPMStereo通过两阶段协作过程实现了高效的动态立体匹配，在精度和时间一致性方面均达到最先进水平。
...
TL;DR: 提出了一个评估文本到视频和图像到视频模型作为行人动态隐式模拟器的基准协议，包括从像素空间重建鸟瞰轨迹的方法，发现主流模型学习了有效的多智能体行为先验但存在合并和消失等失败模式


<details>
  <summary>Details</summary>
Motivation: 现有基准关注单个主体而非多交互行人场景，生成视频中多智能体动态的合理性尚未验证

Method: 为I2V使用现有数据集起始帧与真实视频比较，为T2V开发多样化行人密度和交互提示套件，提出从像素空间重建2D鸟瞰轨迹的方法

Result: 分析显示主流模型学习了令人惊讶的有效多智能体行为先验

Conclusion: 失败模式如合并和消失的行人突显了未来改进的方向
...
TL;DR: 提出SPAN网络，将可疑意图检测从离散分类转为连续回归，能捕捉动态变化的可疑意图，在HAI数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有离散分类方法无法捕捉可疑意图的连续变化特性，限制了早期干预和可解释性。

Method: 基于时序点过程理论，定义可疑度评分公式建模连续变化；引入可疑系数调制机制；提出概念锚定映射方法。
...
TL;DR: 该研究系统分析了54个公开脑MRI数据集，揭示了数据规模、模态组成和预处理方法的不一致性，并证明标准化预处理无法完全消除数据集间的偏差，强调了开发通用脑MRI基础模型时需采用预处理感知和领域自适应策略的重要性。


<details>
  <summary>Details</summary>
Motivation: 开发脑MRI基础模型严重依赖于数据的规模、多样性和一致性，但目前缺乏对这些因素的系统评估。

Method: 在数据集层面分析模态组成、疾病覆盖和规模；在图像层面量化体素间距、方向和强度分布；评估预处理变异性；使用3D DenseNet121进行特征空间案例研究。

Result: 发现健康人群与临床人群数据规模严重不平衡，预处理步骤提高了数据集内一致性但无法消除数据集间差异，特征空间分析显示标准化预处理后仍存在可测量的残留协变量偏移。
...
Method: 1. RAPO阶段：从关系图中检索语义相关修饰符并重构提示以匹配训练分布；2. SSPO阶段：使用多源反馈迭代优化提示；3. 使用SSPO优化的提示对微调重写器LLM。
...
TL;DR: FlowCycle提出了一种基于流的免反演图像编辑框架，通过目标感知的中间状态和循环一致性约束，解决了现有方法在语义差距较大时编辑效果有限或不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于文本的图像编辑方法采用目标无关的中间状态构建方式，主要关注源图像重建而忽略了与特定编辑目标的语义差距，导致在需要大幅修改时编辑能力有限或结果不一致。

Method: FlowCycle框架参数化可学习的噪声来构建目标感知的中间状态，通过循环一致性过程进行优化，在源到目标和目标回源的双重一致性约束下迭代编辑。

Result: 大量消融实验证明，FlowCycle在编辑质量和一致性方面优于现有最先进方法。

Conclusion: FlowCycle通过目标感知的中间状态和循环一致性优化，实现了更忠实于编辑意图的修改，同时保持了源图像的一致性。
...
TL;DR: 提出CURL框架，通过自监督对比学习从胎儿超声视频中检测胎儿运动，在92名受试者数据集上达到78.01%的敏感度和81.60%的AUROC。
...
Method: 使用双重对比损失的自监督学习框架，结合空间和时间对比学习，并引入任务特定采样策略和概率微调方法。
...
TL;DR: 本文提出EditInfinity方法，通过适配基于VQ的生成模型进行图像编辑，解决了传统扩散模型图像反转中的近似误差问题，实现了更精确的图像编辑。


<details>
  <summary>Details</summary>
Motivation: 传统基于扩散模型的图像编辑方法在图像反转过程中存在近似误差，这些误差源于中间生成步骤缺乏精确监督，限制了图像编辑的性能。

Method: 提出EditInfinity方法，适配基于二进制量化的Infinity生成模型，开发了高效的图像反转机制，包括文本提示修正和图像风格保持，并设计了整体平滑策略。

Result: 在PIE-Bench基准测试中，在"添加"、"更改"和"删除"编辑操作上，相比最先进的基于扩散模型的基线方法表现出优越性能。

Conclusion: EditInfinity通过利用VQ生成模型可获取精确中间量化表示的特性，实现了更精确的图像反转和编辑，在保持源图像高保真度的同时实现与文本提示的精确语义对齐。
...
TL;DR: 本文提出了一种新的"诱导-检测-抑制"框架来解决大型视觉语言模型在长文本响应中的幻觉问题，通过主动诱导幻觉、早期检测高风险情况，并在解码过程中抑制潜在的对象级幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在长文本响应中更容易产生幻觉，传统观点认为这是由长度引起的错误累积导致的。本文旨在探究幻觉增加的深层机制，验证是否不仅仅是长度本身，而是长文本对上下文连贯性和完整性的依赖增加导致了幻觉风险。

Method: 提出"诱导-检测-抑制"框架：1）通过精心设计的上下文主动诱导幻觉；2）利用诱导实例进行高风险情况的早期检测；3）在实际解码过程中抑制潜在的对象级幻觉。

Result: 该方法在所有基准测试中都取得了持续且显著的改进，证明了其有效性。强大的检测能力和改进的幻觉缓解不仅验证了该框架，更重要的是重新验证了关于上下文的假设。

Conclusion: 本研究不仅追求性能提升，更旨在提供新的见解，作为深入探索大型视觉语言模型长文本响应中幻觉问题的第一步。研究证实幻觉风险不是由长度本身引起，而是由长文本对上下文依赖增加导致的。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于高斯溅射的方法要么依赖单一3D语言场导致分割效果差，要么依赖预计算的类无关分割导致错误累积，需要更好的整合语言和分割线索的方法。

Method: 引入协作场概念（实例场和语言场），通过实例到语言特征映射和两阶段训练策略构建协作场，在推理时设计自适应语言到实例提示优化。

Result: 在两个广泛使用的基准测试上表现优于现有方法，在图像驱动的3D分割、分层分割和机器人应用方面显示出高潜力。

Conclusion: COS3D通过协作场框架有效整合语言和分割线索，在开放词汇3D分割任务中取得了领先性能，并具有广泛的应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频时间定位方法在处理文本标记时忽视其不同的语义角色，过度依赖[EOS]驱动的全局语义，无法有效利用词级信号进行细粒度时间对齐。

Method: 提出双分支架构：将[EOS]标记通过句子级路径处理，将词标记聚类为短语级单元进行局部定位；引入基于标记角色的跨模态交互策略和联合建模框架。

Result: 在QVHighlights和Charades-STA基准测试的Moment Retrieval和Highlight Detection任务中达到最先进性能。
...
TL;DR: 提出了EDGeo框架，通过掩码位置编码和上下文增强模块改进跨视角物体地理定位，在CVOGL和VIGOR-Building数据集上实现最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 智能手机技术普及，但视障人士在日常金钱交易中仍面临困难，需要依赖他人帮助。开发实时货币检测系统可以帮助他们独立处理金钱事务。

Method: 使用YOLOv8 nano模型，配备带有深度卷积层和Squeeze-and-Excitation块的自定义检测头，以增强特征提取和检测精度。

Result: 模型达到97.73%的准确率、95.23%的召回率、95.85%的F1分数，以及97.21%的mAP50(B)值。检测后通过语音反馈帮助视障人士识别货币。

Conclusion: 成功开发了一个实用高效的货币检测系统，能够帮助视障人士在金钱处理方面实现独立自主。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频异常检测方法在引入文本特征时过于粗糙，忽略了视频片段中可能存在的大量冗余信息，需要利用多模态信息的多样性来精炼特征。
...
Result: 实验证明该去偏方法在不同数据集上均有效。

Conclusion: 提出的去偏方法能够有效提升模型在视觉常识推理任务中的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统数据驱动模型未能充分利用CV-SAR数据中丰富的电磁散射特征，导致在数据有限和领域偏移场景下难以同时优化泛化性、可解释性和效率。

Method: 采用'压缩-聚合-压缩'架构：物理引导压缩阶段使用字典处理器嵌入物理先验，紧凑展开网络提取稀疏物理特征；聚合模块丰富表示；语义压缩阶段使用紧凑分类头和自蒸馏学习判别性嵌入。

Result: 在五个SAR基准测试中，KINN的CNN(0.7M)和ViT(0.95M)变体在参数效率识别方面达到最先进水平，在数据稀缺和分布外场景下表现出优异泛化能力和实际可解释性。
...
Method: 包含反事实样本构建模块（通过事件描述改写和核心交互挖掘生成正负样本）和对比优化模块（使用对比损失最小化原样本与正样本特征距离，最大化与负样本距离）。
...
TL;DR: 本文提出了Instruction-as-Reasoning范式，将指令视为动态分析路径，通过两阶段训练框架（SFT+RL）优化路径选择和组合，在GUI grounding任务上取得了state-of-the-art结果。


<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding方法将指令视为静态的用户意图代理，忽视了指令多样性和质量对性能的影响。研究发现现有数据集有23.3%的指令缺陷，且利用指令多样性可带来76%的相对性能提升。

Method: 采用Instruction-as-Reasoning范式，将指令作为动态分析路径。提出两阶段训练框架：首先在合成的多样化指令上进行监督微调（SFT），然后通过强化学习（RL）优化路径选择和组合。

Result: UI-Ins-7B和UI-Ins-32B在五个基准测试中达到最佳性能：UI-I2E-Bench 87.3%、ScreenSpot-Pro 57.0%、MMBench-GUI L2 84.9%。UI-Ins-7B在AndroidWorld上达到74.1%的成功率。

Conclusion: 该方法不仅提升了GUI grounding性能，还展现出涌现推理能力，能够在推理时选择性组合和合成新的指令路径，同时缓解了SFT+RL框架中的策略崩溃问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大视觉语言模型主要应用于足球、板球、篮球等流行运动，而针对街舞这种小众但流行的舞蹈运动的研究较少，需要探索视频基础模型在此类任务中的适用性。

Method: 使用现代视频基础模型（包括编码器和解码器），对街舞视频进行分类任务，并对微调后的解码器模型进行了深入分析。

Result: 视频编码器模型在预测任务中继续优于最先进的视频语言模型，研究提供了选择编码器模型的见解。

Conclusion: 对于街舞视频分类等预测任务，视频编码器模型比视频语言模型表现更好，需要根据具体任务选择合适的模型类型。
...
TL;DR: 本文提出了一个赢得RoboSense 2025 Track 4竞赛的跨模态无人机导航解决方案，通过领域对齐预处理和混合专家框架解决多平台异质性和领域差距问题。


<details>
  <summary>Details</summary>
Motivation: 解决跨模态地理定位中的两个主要障碍：严重的平台间异质性（卫星/无人机/地面）和通用训练描述与平台特定测试查询之间的领域差距。

Method: 使用领域对齐预处理管道（平台划分、卫星增强、方向词移除）和LLM驱动的标题精炼管道，结合BGE-M3和EVA-CLIP模型，采用渐进式两阶段硬负样本挖掘策略训练三个平台专家，并在推理时融合得分。

Result: 系统在官方排行榜上排名第一，展示了在异质视角下稳健的跨模态地理定位能力。

Conclusion: 提出的领域对齐预处理和混合专家框架有效解决了跨平台异质性问题，实现了优越的跨模态地理定位性能。
...
TL;DR: HyperET是一种高效的多模态大语言模型训练范式，利用双曲空间通过动态半径调整实现视觉与文本在任意粒度级别的对齐，仅需不到1%的额外参数即可显著提升现有MLLM性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM需要极高计算资源进行训练，主要原因是广泛使用的视觉编码器（如CLIP、SAM）缺乏与语言在多层次粒度上的对齐能力。

Method: 利用双曲空间建模层次结构，通过动态双曲半径调整优化视觉表示与文本在任意粒度级别的对齐，采用可学习矩阵和Möbius乘法操作，提供三种高效参数化策略。

Result: 在多个MLLM基准测试中，HyperET仅需不到1%的额外参数就能显著提升现有预训练和微调MLLM的性能。

Conclusion: HyperET提供了一种高效的多模态对齐训练范式，通过双曲空间解决了视觉与文本的粒度差距问题，显著降低了MLLM训练的计算需求。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度学习点云压缩的泛化挑战，主要源于缺乏鲁棒的上下文模型和低效处理分布外数据的问题。

Method: 提出通用上下文模型利用空间和通道分组先验捕获上下文依赖，以及实例自适应微调策略结合显式和隐式压缩范式。

Result: 在15个多样化数据集上的广泛实验证实AnyPcc在点云压缩方面达到了新的最先进水平。

Conclusion: AnyPcc通过创新的上下文建模和自适应微调机制，有效解决了点云压缩的泛化问题，代码和数据集将开源以促进可重复研究。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在采样过程中，量化误差会在多个去噪步骤中累积，影响生成质量。现有方法独立处理每一步的误差，未能有效解决累积问题。
...
TL;DR: 本文发现扩散变换器(DiTs)中的补丁标记具有高度独立性，空间一致性主要由位置编码控制。作者提出位置编码场(PE-Field)，将2D位置编码扩展到结构化3D场，使DiTs能够直接在3D空间中建模几何。
...
Conclusion: PE-Field通过扩展位置编码到3D场，使DiTs能够直接在3D空间中建模几何，显著提升了视觉生成能力。
...
TL;DR: 本文提出了一种新颖的因果方法，通过预测图像中可能被忽略的烹饪元素，并将这些元素显式注入跨模态表示学习来缓解偏见，从而改善图像到食谱检索的性能。
...
Method: 提出因果表示学习方法，预测图像中可能被忽略的烹饪元素（如细微配料和烹饪方法），并将这些元素显式注入跨模态表示学习以缓解偏见。

Result: 在标准单语Recipe1M数据集和新构建的多语言多文化数据集上的实验表明，该方法能够发现细微配料和烹饪动作，在两个数据集上都取得了令人印象深刻的检索性能。

Conclusion: 因果表示学习方法能够有效缓解图像到食谱检索中的表示偏见，提升对细微烹饪元素的识别能力，在单语和多语言多文化场景下均表现优异。
...
TL;DR: 提出FuzzyDistillViT-MobileNet模型，通过动态模糊逻辑驱动的知识蒸馏解决肺癌分类中的不确定性问题，结合图像融合优化和遗传算法选择，在多个数据集上达到99%以上的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统静态知识蒸馏方法使用固定权重，无法有效处理医学图像中不同区域的不确定性差异。需要动态调整蒸馏权重，让模型重点关注高置信度区域，减少对模糊区域的关注。

Method: 使用ViT-B32作为教师模型，MobileNet作为学生模型，采用动态模糊逻辑调整蒸馏权重。结合Gamma校正和直方图均衡化进行图像预处理，使用小波融合方法提升图像质量。采用遗传算法从12个候选模型中选择最优学生模型。

Result: 在LC25000组织病理学图像数据集上达到99.16%准确率，在IQOTH/NCCD CT扫描图像数据集上达到99.54%准确率，证明了模型在不同成像领域的鲁棒性。

Conclusion: 动态模糊逻辑知识蒸馏能有效处理医学图像分类中的不确定性，结合图像融合和模型选择优化，显著提升了肺癌分类的准确性和鲁棒性。
...
TL;DR: Conan是一个用于证据基础的多步视频推理框架，通过识别上下文和证据帧、跨帧线索推理以及自适应决策，在六个多步推理基准上超越基线模型超过10%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的视频推理方法存在局限性：基于强化学习的方法依赖纯文本链，产生无基础或幻觉的结论；而帧检索方法虽然引入视觉基础，但在证据定位方面仍不准确。

Method: 构建Conan-91K大规模自动生成推理轨迹数据集，包含帧识别、证据推理和行动决策；设计多阶段渐进冷启动策略与识别-推理-行动强化学习训练框架，联合增强多步视觉推理。

Result: 在六个多步推理基准上，Conan平均超越基线Qwen2.5-VL-7B-Instruct超过10%的准确率，达到最先进性能；在长视频理解任务上表现出良好的泛化能力。

Conclusion: Conan框架通过证据基础的多步视频推理方法，有效解决了现有方法的局限性，展现出强大的可扩展性和鲁棒性。
...
TL;DR: 本文提出了一种可复现的人口属性推断管道，用于改善人脸分析系统的公平性评估。通过模块化迁移学习方法替代传统端到端训练，提高了人口属性推断的可靠性，从而获得更准确、偏差更小的公平性评估结果。


<details>
  <summary>Details</summary>
Motivation: 人脸分析系统的公平性评估依赖于自动人口属性推断的可靠性，但现有方法在人口属性推断的准确性上存在不足，导致公平性审计结果存在偏差。

Method: 采用模块化迁移学习方法，将预训练的人脸识别编码器与非线性分类头相结合，构建可复现的人口属性推断管道，并在准确性、公平性和鲁棒性三个维度进行评估。

Result: 所提出的方法在多个数据集和训练设置中优于强基线方法，特别是在更具挑战性的种族属性推断上表现更优。

Conclusion: 这项工作为公平性审计中的人口属性推断提供了可靠的基础，通过提高人口属性推断的可靠性来改善人脸分析系统的公平性评估效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前一步扩散模型虽然能快速生成高质量图像，但在个性化新概念方面能力有限，难以有效捕捉新概念的分布特征。

Method: 采用端到端训练，多步扩散模型(教师)和一步扩散模型(学生)同时训练，通过双向概念蒸馏和回声反馈机制，共享文本编码器，结合对抗损失和对齐损失优化学生模型。

Result: 实验表明该协作框架在1-SDP设置下显著优于现有个性化方法，不仅提升了学生模型的新概念个性化能力，还改善了教师模型的生成质量。

Conclusion: EchoDistill为T2I扩散模型建立了一种快速有效个性化的新范式，通过双向概念蒸馏机制实现了概念捕捉和生成质量的协同提升。
...
TL;DR: 提出Metis-HOME混合专家框架，通过思维分支和非思维分支解决多模态推理模型在效率与泛化能力之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大推理模型存在两个关键局限：对简单查询使用计算昂贵的推理导致效率低下，以及专注于专门推理损害了更广泛的泛化理解能力。

Method: 将原始密集模型重构为两个专家分支：思维分支处理复杂多步推理，非思维分支优化快速直接推理任务。使用轻量级可训练路由器动态分配查询。

Result: 该方法不仅显著增强了复杂推理能力，还改善了模型的泛化能力，逆转了其他推理专用模型观察到的性能退化趋势。
...
TL;DR: 提出Fake-in-Facext框架，通过细粒度面部区域概念树和伪影接地解释任务，解决现有可解释深度伪造分析方法缺乏细粒度感知的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在可解释深度伪造分析中存在不足：数据标注不可靠且粗粒度，模型无法输出文本解释与视觉证据的关联，也不支持任意面部区域的查询输入。

Method: 定义面部图像概念树进行细粒度区域划分，提出FiFa-Annotator标注流程，引入伪影接地解释任务，构建统一的多任务学习架构FiFa-MLLM。

Result: FiFa-MLLM在AGE任务上优于强基线，在现有XDFA数据集上达到SOTA性能。

Conclusion: FiFa框架通过细粒度数据标注和多任务学习，显著提升了可解释深度伪造分析的性能。
...
TL;DR: 提出了一种深度学习框架，通过联合估计潜在清晰图像和相机运动轨迹来恢复运动模糊图像，在严重模糊情况下表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决相机抖动（特别是大范围或旋转运动）造成的运动模糊问题，这是图像恢复领域的主要挑战。

Method: 使用投影运动模糊模型(PMBM)，通过神经网络预测完整的3D旋转轨迹，指导基于模型的端到端恢复网络，并采用可微分模糊创建模块。

Result: 在合成和真实数据集上达到最先进性能，特别是在严重或空间变化模糊情况下优于端到端去模糊网络。

Conclusion: 该方法通过揭示产生模糊的相机运动轨迹，提供了可解释性，并能重建生成模糊图像的清晰图像序列，模块化架构具有优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有SLAM技术在低纹理、运动模糊和挑战性光照条件下表现不佳，这在视觉障碍者辅助导航等应用中尤为关键，会影响定位精度和跟踪稳定性，降低导航可靠性和安全性。
...
Conclusion: 该框架在挑战性条件下展现出增强性能，为开发视觉障碍者导航辅助工具提供了可靠平台。
...
Conclusion: ACamera-Net无需依赖额外的图像增强模块，就能有效解决复杂光照条件下的图像质量问题，在边缘设备上实现实时性能。
...
TL;DR: 研究用户对不同细节层次和观看距离下人群角色表示的视觉质量感知，比较几何网格、图像替代物、NeRF和3D高斯等表示的视觉保真度与计算性能权衡。
...
Method: 使用定性分析和定量评估方法，比较几何网格、图像替代物、NeRF和3D高斯四种表示在不同细节层次和观看距离下的表现。

Result: 每种表示方法在视觉保真度和计算性能方面都有不同的权衡关系，研究结果为人群渲染的感知优化提供了具体见解。

Conclusion: 研究结果为设计基于感知的细节层次策略提供了重要指导，有助于在人群渲染中平衡视觉质量和计算效率。
...
TL;DR: EmbodiedBrain是一个新型的视觉语言基础模型，旨在解决现有LLM和MLLM在具身任务中的局限性，通过代理对齐的数据结构、强大的训练方法和综合奖励系统，在具身AI领域实现了最先进的性能。
...
Method: 采用代理对齐的数据结构，结合大规模监督微调(SFT)和步骤增强组相对策略优化(Step-GRPO)的强大训练方法，集成生成奖励模型(GRM)的综合奖励系统。

Result: 实验结果表明，EmbodiedBrain在所有指标上都实现了优越性能，为具身基础模型建立了新的最先进水平。

Conclusion: 该研究为下一代通用具身代理铺平了道路，并开源了所有数据、模型权重和评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 大多数视频推理模型只生成文本推理轨迹而不指示关键证据出现的时间和位置。将证据中心推理扩展到视频更具挑战性，需要跨动态场景的联合时间跟踪和空间定位。
...
TL;DR: 提出了GenColorBench，首个针对文本到图像颜色生成的综合基准测试，包含44K个颜色相关提示，覆盖400+颜色，通过感知和自动评估揭示模型在颜色控制方面的真实能力。
...
Method: 基于ISCC-NBS和CSS3/X11颜色系统构建基准测试，包含数值颜色和人类可理解的颜色名称，采用感知评估和自动评估相结合的方法。
...
TL;DR: 提出了一种基于相似性原型的跨模态分割框架，通过在学习嵌入空间中构建类别原型，并引入相似性约束来提升模型在未见数据上的表现。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在视觉任务中表现优异，但在面对未见数据时性能会显著下降。由于模型对领域偏移敏感，无监督领域自适应旨在减少领域差距并避免对未见领域进行昂贵的标注。

Method: 在学习嵌入空间中学习类别原型，引入相似性约束使原型对每个语义类别具有代表性且不同类别间可分离。使用字典存储从不同图像提取的原型，防止类别缺失问题并实现原型对比学习。

Result: 大量实验表明，该方法比其他最先进方法取得了更好的结果。

Conclusion: 提出的基于相似性原型的框架有效提升了跨模态分割的性能，通过原型学习和对比学习机制解决了领域自适应中的关键问题。
...
Method: 使用第一帧作为锚点，通过密集高斯基元场逐步优化物体表示；核心创新是双键记忆模块，结合潜在外观-几何键和显式方向键，通过空间引导记忆读取和高效稀疏化机制处理自由移动物体。

Result: 在真实世界数据集上的评估显示，OnlineSplatter显著优于最先进的免位姿重建基线方法，随着观察次数增加性能持续提升，同时保持恒定的内存和运行时间。

Conclusion: 该框架实现了高质量、物体中心的3D高斯重建，能够有效处理自由移动物体，具有恒定的计算成本优势。
...
TL;DR: SeViCES是一个无需训练、模型无关的框架，通过语义-视觉共识证据选择来解决长视频理解问题，包含SVCFS模块进行帧选择和ACR模块进行答案精炼。


<details>
  <summary>Details</summary>
Motivation: 长视频理解面临计算复杂度高、推理不聚焦不一致的问题，现有帧选择方法忽略时间依赖或依赖单模态证据，无法提供完整且与查询相关的上下文。

Method: 提出SeViCES框架：1）SVCFS模块通过时间感知语义分支（利用LLM在字幕上的推理）和聚类引导视觉分支（通过互信息对齐嵌入与语义分数）选择帧；2）ACR模块通过融合证据和约束答案空间来解决语义和视觉预测间的不一致性。

Result: 在长视频理解基准测试上的广泛实验表明，SeViCES在准确性和鲁棒性方面持续优于最先进方法。

Conclusion: 共识驱动的证据选择对Video-LLMs至关重要，SeViCES证明了其在长视频理解中的有效性。
...
TL;DR: 本文系统综述了深度学习在牙科图像分析中的应用，涵盖260项研究，包括49篇公开数据集论文和211篇算法论文，总结了该领域的数据集特征、模型方法、评估指标，并讨论了当前挑战和未来方向。
...
Method: 系统综述了260项深度学习在牙科图像分析中的研究，重点关注数据集和模型两大基础方面。介绍了牙科成像基本概念，总结了现有数据集特征和获取方法，分类整理了相关模型算法，分析了网络架构、优化策略、训练方法和性能。

Result: 全面总结了深度学习在牙科图像分析领域的最新进展，包括公开数据集的特点、各种深度学习模型的架构和性能，以及常用的训练和评估指标。

Conclusion: 深度学习已成为牙科图像分析中最广泛应用和最具影响力的方法。本文为该领域研究人员提供了系统性的参考，并指出了当前研究的挑战和未来发展方向。
...
TL;DR: BTB3D是一种用于3D医学影像的因果卷积编码器-解码器模型，通过频率感知的体积标记化方法，解决了高分辨率长序列体积数据的处理问题，在报告生成和文本到CT合成任务上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前3D医学影像的视觉语言模型在处理高分辨率长序列体积时存在困难：对比预训练产生的视觉编码器与临床语言不对齐，切片级标记化会模糊精细解剖结构，降低下游任务的诊断性能。

Method: 提出BTB3D模型，采用因果卷积编码器-解码器架构，统一2D和3D训练与推理，生成紧凑的频率感知体积标记。通过三阶段训练课程：(i)局部重建，(ii)重叠窗口平铺，(iii)长上下文解码器精炼，使模型能从短切片片段学习并泛化到超过300切片的扫描。

Result: 在报告生成任务上，BLEU分数提升，临床F1分数比CT2Rep、CT-CHAT和Merlin提高40%；在文本到CT合成任务上，FID降低75%，FVD减半，能生成解剖学一致的512*512*241体积。

Conclusion: 精确的三维标记化，而非仅仅更大的语言骨干网络，对于3D医学影像中可扩展的视觉语言建模至关重要。
...
TL;DR: 提出UltraHR-100K超高清文本到图像数据集和频率感知后训练方法，解决超高清图像生成中数据集缺失和细节合成训练策略不足的问题。
...
Method: 1) 构建UltraHR-100K数据集：包含10万张超过3K分辨率的超高清图像，基于细节丰富度、内容复杂度和美学质量严格筛选；2) 提出频率感知后训练方法：包括细节导向时间步采样(DOTS)和软加权频率正则化(SWFR)，利用离散傅里叶变换约束频率分量以保持高频细节。

Result: 在提出的UltraHR-eval4K基准测试上，该方法显著提升了超高清图像生成的细粒度细节质量和整体保真度。

Conclusion: 通过高质量数据集和专门训练策略的结合，有效解决了超高清文本到图像生成中的细节合成问题，为超高清图像生成提供了新的解决方案。
...
TL;DR: 本文提出了一种名为HybridSOMSpikeNet的混合深度学习框架，用于实现智能且节能的废物分类，在十类废物数据集上达到了97.39%的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 准确的废物分类对于实现可持续废物管理和减少城市化的环境足迹至关重要。可回收材料的错误分类会导致垃圾填埋场堆积、回收效率低下以及温室气体排放增加。

Method: 该模型采用预训练的ResNet-152骨干网络提取深度空间表示，然后使用可微分软自组织映射增强拓扑聚类和可解释性，最后通过尖峰神经网络头在离散时间步上累积时间激活，提高鲁棒性和泛化能力。

Result: HybridSOMSpikeNet在十类废物数据集上实现了97.39%的测试准确率，优于多个最先进架构，同时保持了轻量级计算特性，适合实际部署。

Conclusion: 该框架通过实现精确和自动化的废物分离，支持更高的回收效率，减少可回收流中的污染，并最小化废物处理的生态和运营成本，符合联合国可持续发展目标（SDG 11和SDG 12）。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要为每个支持的比特宽度重复完整数据集更新，导致训练开销随精度数量线性增长，且需要额外微调阶段来支持更多精度选项

Method: 1. 权重偏置校正：通过中和跨比特宽度的量化诱导偏置和对齐激活分布，实现共享批归一化并消除微调需求；2. 比特级核心集采样策略：利用隐式知识转移现象，通过基于梯度的重要性分数为每个子模型选择紧凑的信息子集进行训练

Result: 在CIFAR-10/100、TinyImageNet和ImageNet-1K数据集上，使用ResNet和ViT架构的实验表明，该方法在实现竞争性或更优精度的同时，将训练时间减少高达7.88倍

Conclusion: 所提出的方法显著降低了多比特量化网络的训练开销，同时保持了模型性能，为灵活部署深度学习网络提供了高效的解决方案
...
TL;DR: 提出基于代理的架构，结合LLM推理和轻量级视觉模块，通过细粒度分析和迭代优化推理链来解决MLLM中的视觉幻觉和文本依赖问题。
...
Method: 使用三阶段评估框架诊断现有模型，提出基于代理的架构，结合LLM推理和轻量级视觉模块进行细粒度分析和迭代优化。

Result: 在MMMU上提升10.3分，MathVista上提升6.0分（基于7B基线），性能达到或超过更大模型。

Conclusion: 未来视觉推理模型应专注于整合更多专门工具来分析视觉内容，将发布框架和评估套件以促进研究。
...
Method: MixKV方法分析KV缓存在注意力头间的冗余变化，自适应地平衡多样性和重要性来压缩KV对，而不是仅仅依赖重要性。

Result: 在极端压缩（预算=64）下，MixKV将基线方法平均提升5.1%，在GUI接地任务上对SnapKV和AdaKV分别实现8.0%和9.0%的显著增益，同时保持相当的推理效率。

Conclusion: MixKV通过混合重要性和多样性来优化KV缓存压缩，在多模态理解和LLM中都表现出色，有效解决了KV缓存扩展带来的内存瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统LiDAR投影方法存在几何不一致性，导致不可逆的信息丢失，影响高保真应用。需要一种能够实现无损投影的方法来满足高精度遥感应用的需求。

Method: 通过自动反向工程推断LiDAR传感器的关键参数，包括激光束配置、角度分布和每束激光的校准校正，从而实现无损投影和完整点云重建。

Result: 在KITTI和DurLAR数据集上的全面评估显示，ALICE-LRI实现了完美的点保留，所有点云中零点丢失，几何精度保持在传感器精度限制内，具有实时性能。
...
TL;DR: AutoScape是一个长时域驾驶场景生成框架，通过RGB-D扩散模型迭代生成几何一致的关键帧，然后使用视频扩散模型插值生成密集连贯的视频帧。
...
Method: 1) 在共享潜在空间中联合处理图像和深度；2) 基于先前生成关键帧的几何信息进行条件生成；3) 使用扭曲一致性指导采样过程；4) 通过视频扩散模型插值关键帧。

Result: 生成超过20秒的真实且几何一致的驾驶视频，长时域FID和FVD分数相比现有最佳方法分别提升48.6%和43.0%。
...
TL;DR: 提出一种基于注意力驱动特征融合的CNN和ViT双编码器模型，用于组织病理学图像的语义分割，在两个公开数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 深度学习在组织病理学图像分析中表现出色，但现有方法仍有改进空间。通过结合CNN和ViT的优势，提升语义分割性能。

Method: 使用注意力驱动特征融合机制，将卷积神经网络和视觉变换器集成到统一的双编码器模型中。

Result: 在GCPS数据集上达到μIoU/μDice 76.79%/86.87%，在PUMA数据集上达到64.93%/76.60%，优于最先进方法。

Conclusion: 提出的双编码器模型有效提升了组织病理学图像语义分割性能，代码已开源。
...
TL;DR: 提出了DyPE方法，无需训练即可让预训练的扩散变换器生成远超训练分辨率的图像，通过动态调整位置编码匹配扩散过程的光谱特性。
...
Method: DyPE方法利用扩散过程的光谱特性，在每一步扩散过程中动态调整位置编码，使其频率谱与当前生成阶段匹配。

Result: 能够生成1600万像素的超高分辨率图像，在多个基准测试中性能持续提升，在超高分辨率图像生成中达到最先进的保真度。

Conclusion: DyPE是一种高效的无训练方法，显著扩展了扩散变换器的分辨率生成能力，在高分辨率下效果尤为明显。
...
TL;DR: α-Flow通过解耦MeanFlow中的轨迹流匹配和轨迹一致性冲突，采用课程学习策略从轨迹流匹配平滑过渡到MeanFlow，在ImageNet-256上实现了新的SOTA结果。


<details>
  <summary>Details</summary>
Motivation: MeanFlow框架在少步生成建模中表现出色，但其成功机制尚未完全理解。研究发现MeanFlow目标自然分解为轨迹流匹配和轨迹一致性两个部分，这两个目标存在强负相关性，导致优化冲突和收敛缓慢。

Method: 提出α-Flow目标族，统一了轨迹流匹配、Shortcut Model和MeanFlow。采用课程学习策略，从轨迹流匹配平滑过渡到MeanFlow，从而解耦冲突目标。

Result: 在ImageNet-1K 256x256上使用标准DiT骨干网络，α-Flow在所有规模和设置下都优于MeanFlow。最大的α-Flow-XL/2+模型实现了新的SOTA结果：1-NFE FID 2.58，2-NFE FID 2.15。

Conclusion: α-Flow通过解耦MeanFlow中的优化冲突，实现了更好的收敛性能，在少步生成建模中取得了突破性进展。
...
TL;DR: Cupid是一种基于生成的单图像3D重建方法，通过条件采样过程联合估计相机姿态、3D形状和纹理，采用两阶段流匹配框架实现精确的3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有单图像3D重建方法在姿态估计和形状生成方面存在分离问题，需要一种统一的生成框架来同时处理姿态、形状和纹理的联合估计。

Method: 采用两阶段流匹配流程：粗粒度阶段生成初始3D几何和2D投影用于姿态恢复；精炼阶段整合姿态对齐的图像特征来增强结构保真度和外观细节。

Result: 在PSNR指标上获得超过3dB的提升，Chamfer距离减少超过10%，在姿态精度上媲美单目估计器，视觉保真度优于基线3D生成模型。

Conclusion: Cupid通过统一的生成框架成功实现了从单张2D图像到3D对象的准确重建，在多个指标上显著优于现有先进方法。
...
TL;DR: 提出了一种融合雷达和摄像头数据的多目标跟踪框架，通过在线标定和共同特征利用，显著提升了跟踪精度并减少了人工干预。


<details>
  <summary>Details</summary>
Motivation: 现有研究往往低估雷达的作用，仅将其作为辅助传感器，而雷达实际上能够提供准确的目标距离/深度信息。本文旨在充分发挥雷达的关键作用，解决传感器融合中的标定和关联问题。

Method: 开发了雷达-摄像头融合MOT框架，利用在线标定技术简化两个传感器的检测结果整合；使用雷达和摄像头数据的共同特征来精确推导检测对象的真实世界位置；采用特征匹配和类别一致性检查来超越单纯位置匹配的限制。

Result: 框架能够简化雷达-摄像头映射过程，提高跟踪精度，在受控环境和实际交通场景中的实验证明了其有效性。
...
TL;DR: ARGenSeg提出了一种基于自回归生成的新型图像分割范式，通过统一框架实现多模态理解和像素级感知，在多个分割数据集上超越现有方法并显著提升推理速度。
...
Method: 基于图像生成的分割框架，利用MLLM输出视觉标记，通过通用VQ-VAE将其解码为图像，使分割完全依赖MLLM的像素级理解。采用下一尺度预测策略并行生成视觉标记以减少推理延迟。

Result: 在多个分割数据集上超越了现有最先进方法，推理速度显著提升，同时保持了强大的理解能力。

Conclusion: ARGenSeg通过生成式方法成功统一了多模态理解和像素级分割任务，证明了基于生成的分割范式的有效性。
...
TL;DR: 提出了一种基于纯Transformer的自回归视频预测模型，通过连续像素空间表示进行视频预测，无需复杂训练策略或潜在特征学习组件，在物理模拟预测时间范围上比现有方法提升50%


<details>
  <summary>Details</summary>
Motivation: 受自回归大语言模型性能和可扩展性启发，将Transformer架构应用于视频预测领域，专注于解决现有视频生成方法在物理模拟时间因果建模方面的不足

Method: 使用纯Transformer模型进行自回归视频预测，比较不同的时空自注意力布局，采用连续像素空间表示，通过物理对象跟踪指标和无监督训练来隔离时空推理

Result: 相比现有潜在空间方法，物理准确预测的时间范围显著延长50%，在常见视频质量指标上保持可比性能，并能通过探测模型准确估计PDE模拟参数
...
<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在处理信息密集型图像时面临挑战，这些图像密集交织文本注释和细粒度图形元素，需要精确定位关键线索和多跳推理来整合分散证据。

Method: 采用训练免费的推测解码框架：草稿阶段由小型VLM作为草稿专家生成多样化的定位候选推理路径；裁决阶段由强大VLM综合这些路径生成最终答案，并引入共识专家选择机制仅转发高一致性推理路径。

Result: 在InfographicVQA、ChartMuseum、ChartQAPro和HR-Bench 4K等具有挑战性的信息密集和高分辨率视觉问答基准上取得一致性能提升，相比大型专有模型或训练流程实现了错误纠正和成本效率。

Conclusion: SV框架通过综合多个部分准确推理路径中的正确见解，在最小化计算成本的同时恢复正确答案，为信息密集型图像理解提供了有效的解决方案。
...
TL;DR: 提出SpectraMorph框架，通过物理引导的自监督融合方法解决高光谱图像超分辨率问题，使用解混瓶颈结构替代直接回归，在单波段MSI下仍保持鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 高光谱传感器空间分辨率低导致边界模糊和混合像素效应，现有深度学习方法缺乏可解释性且在MSI波段数少时性能下降。

Method: 采用物理引导的自监督融合框架，从低分辨率HSI提取端元特征，使用多层感知机从MSI预测丰度图，通过线性混合重建光谱，利用MSI传感器光谱响应函数进行自监督训练。

Result: 在合成和真实数据集上实验表明，SpectraMorph始终优于最先进的无监督/自监督基线方法，与监督基线方法相比仍具有很强竞争力。

Conclusion: SpectraMorph框架产生可解释的中间结果，训练时间不到一分钟，即使在单波段MSI下也保持鲁棒性，为高光谱超分辨率提供了有效的解决方案。
...
TL;DR: 提出了LDDBM框架，用于跨模态转换的通用扩散模型方法，支持任意模态对之间的转换。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在单模态领域表现优异，但在跨模态转换中存在维度不匹配、高斯先验限制和特定架构等局限性。

Method: 基于潜在变量扩展的Denoising Diffusion Bridge Models，在共享潜在空间中学习模态间桥梁，使用对比对齐损失和预测损失进行训练。

Result: 在多视角到3D形状生成、图像超分辨率和多视角场景合成等任务中表现优异。

Conclusion: LDDBM为通用模态转换建立了新的强基线，支持任意模态对的转换。
...
TL;DR: LayerComposer是一个交互式框架，通过分层画布和锁定机制实现多主体个性化图像生成，解决了现有方法在空间组合控制和多主体扩展方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有个性化生成模型缺乏对空间组合的交互控制，且难以扩展到多个主体。为解决这些限制，需要开发能够支持多主体、提供空间控制能力的生成框架。

Method: 提出分层画布表示，每个主体位于独立图层实现无遮挡组合；引入锁定机制，保持选定图层高保真度，同时允许其他图层灵活适应上下文；无需架构修改，利用位置嵌入和互补数据采样策略。

Result: 实验表明LayerComposer在多主体个性化图像生成中，相比现有最先进方法，实现了更优越的空间控制和身份保持能力。

Conclusion: LayerComposer通过分层画布和锁定机制，为多主体个性化图像生成提供了有效的交互控制解决方案，在空间组合和身份保持方面表现出色。
...
Method: 采用窗口交叉注意力机制将文本提示定位到特定镜头，同时使用稀疏镜头间自注意力模式（镜头内密集但镜头间稀疏），确保分钟级生成所需的效率。

Result: 在叙事连贯性方面达到了新的最先进水平，并展现出显著的新兴能力：对角色和场景的持久记忆，以及对电影技术的直观理解。

Conclusion: 标志着从片段合成向自动化电影制作的关键转变，使端到端的电影创作成为可实现的未来。
...
### [149] [Beyond One-Way Influence: Bidirectional Opinion Dynamics in Multi-Turn Human-LLM Interactions](https://arxiv.org/abs/2510.20039)
*Yuyang Jiang,Longjie Guo,Yuchen Wu,Aylin Caliskan,Tanu Mitra,Hua Shen*

Main category: cs.HC

TL;DR: 研究探讨了人类与LLM聊天机器人在争议话题对话中的双向影响，发现人类观点变化很小，而LLM输出变化较大，个性化设置会放大这种双向变化。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLM对用户的单向影响，缺乏对双向影响和多轮对话中动态变化的研究。

Method: 通过50个争议话题讨论，266名参与者分为三组：静态陈述、标准聊天机器人和个性化聊天机器人。

Result: 人类观点变化很小，LLM输出变化较大，缩小了人与LLM立场差距。个性化设置增强了双向变化，涉及个人故事的交流最易引发立场变化。

Conclusion: 研究揭示了人机交互中过度对齐的风险，需要更谨慎地设计个性化聊天机器人以实现更稳定、深思熟虑的对齐。
...
### [150] [Empathic Prompting: Non-Verbal Context Integration for Multimodal LLM Conversations](https://arxiv.org/abs/2510.20743)
*Lorenzo Stacchio,Andrea Ubaldi,Alessandro Galdelli,Maurizio Mauri,Emanuele Frontoni,Andrea Gaggioli*

Main category: cs.HC

TL;DR: Empathic Prompting是一个多模态人机交互框架，通过面部表情识别捕捉用户情绪线索，将其作为上下文信号融入LLM对话，实现无显式用户控制的情感增强交互。


<details>
  <summary>Details</summary>
Motivation: 传统多模态界面需要显式用户控制，而情感信号在医疗、教育等领域的交流中至关重要但往往在语言交流中难以捕捉。

Method: 集成商业面部表情识别服务，将情感线索嵌入提示中，采用模块化可扩展架构，通过本地部署的DeepSeek实例实现。

Result: 初步评估（N=5）显示非语言输入能一致整合到连贯的LLM输出中，参与者强调对话流畅性。

Conclusion: Empathic Prompting在聊天机器人中介的沟通中具有应用潜力，特别是在医疗和教育等情感信号关键的领域。
...
### [151] [Multimedia-Aware Question Answering: A Review of Retrieval and Cross-Modal Reasoning Architectures](https://arxiv.org/abs/2510.20193)
*Rahul Raja,Arpita Vats*

Main category: cs.IR

TL;DR: 本文综述了多媒体增强问答系统的最新进展，重点分析了整合视觉、语言和音频模态的检索架构，并对方法、数据集和挑战进行了系统分类。


<details>
  <summary>Details</summary>
Motivation: 传统问答系统主要依赖结构化文本数据，而多媒体内容的快速增长为检索增强型问答系统带来了新的挑战和机遇，需要整合多种模态数据来提升系统性能。

Method: 基于检索方法、融合技术和答案生成策略对现有方法进行分类，分析基准数据集、评估协议和性能权衡，重点关注跨模态对齐架构。

Result: 系统梳理了多媒体问答系统的技术发展现状，识别了不同方法的性能特点和适用场景，为研究者提供了全面的技术路线图。

Conclusion: 多媒体问答系统面临跨模态对齐、延迟-准确性权衡和语义基础等关键挑战，未来需要构建更鲁棒和上下文感知的系统来充分利用多媒体数据。
...
### [152] [Analyticup E-commerce Product Search Competition Technical Report from Team Tredence_AICOE](https://arxiv.org/abs/2510.20674)
*Rakshith R,Shubham Sharma,Mohammed Sameer Khan,Ankush Chopra*

Main category: cs.IR

TL;DR: Tredence_AICOE团队开发的多语言电商搜索系统，在Query-Category和Query-Item两个多语言相关性任务中，通过数据增强和模型微调获得第4名成绩。
...
Method: 通过翻译现有数据集进行数据增强，覆盖开发集中缺失的语言；微调Gemma-3 12B和Qwen-2.5 14B模型，采用多种训练策略。

Result: Gemma-3 12B（4位量化）模型在QC任务中使用原始和翻译数据获得最佳性能，在QI任务中使用原始、翻译和少数类数据获得最佳性能；最终在私有测试集上平均F1得分为0.8857，排名第4。

Conclusion: 数据增强和适当的模型微调策略在多语言电商搜索任务中表现有效，Gemma-3 12B模型在两个相关性任务中都取得了最佳性能。
...
### [153] [Automating Iconclass: LLMs and RAG for Large-Scale Classification of Religious Woodcuts](https://arxiv.org/abs/2510.19986)
*Drew B. Thomas*

Main category: cs.IR

TL;DR: 使用大型语言模型(LLMs)和向量数据库结合检索增强生成(RAG)的新方法，对早期现代宗教图像进行分类，在分类精度上显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统图像和基于关键字的搜索方法在早期现代宗教图像分类中效果有限，需要更准确的大规模视觉档案分析工具。

Method: 利用神圣罗马帝国书籍插图的完整页面上下文，通过LLM生成包含视觉和文本元素的详细描述，然后通过混合向量搜索匹配到相关的Iconclass代码。
...
### [154] [Decoding the Ear: A Framework for Objectifying Expressiveness from Human Preference Through Efficient Alignment](https://arxiv.org/abs/2510.20513)
...
Method: 基于语音学和心理学，DeEAR在三个维度上评估语音：情感、韵律和自发性，使用不到500个标注样本就能实现与人类感知的强对齐。

Result: DeEAR与人类感知的相关性达到Spearman秩相关系数0.86，不仅能可靠评分，还能进行公平基准测试和定向数据筛选，显著提升了语音到语音模型的表达性分数。

Conclusion: DeEAR提供了一个有效的语音表达性评估框架，不仅能够准确评估现有模型的表达性差距，还能通过筛选高质量数据显著提升模型的表达性表现。
...
### [155] [Co-Designing Quantum Codes with Transversal Diagonal Gates via Multi-Agent Systems](https://arxiv.org/abs/2510.20728)
*Xi He,Sirui Lu,Bei Zeng*

Main category: quant-ph

TL;DR: 提出了一个多智能体、人在回路的量子码协同设计工作流，用于设计具有指定横向对角门的量子码。该工作流基于SSLP框架，通过GPT-5驱动，在TeXRA平台上实现。


<details>
  <summary>Details</summary>
Motivation: 开发一个系统化的方法来设计具有特定横向对角门的量子码，将可行性问题转化为可扩展的分析流程。

Method: 使用多智能体协作工作流：合成智能体制定问题，搜索智能体筛选候选并数值精确化，审计智能体独立验证。基于SSLP框架，通过模块残差划分基字符串并强制执行Z-边际Knill-Laflamme等式。

Result: 对于d=2距离和非退化残差，在n≤6量子比特下，系统扫描生成了可实现的循环逻辑群表，所有都由新代码实现。例如K=3时在n=6处获得阶16。还展示了SSLP适应残差简并性，实现了新的((6,4,2))代码。

Conclusion: 该工作流将横向对角可行性重新构建为可扩展的分析流程，结合系统枚举与精确分析重构，产生可重现的代码构造，支持扩展到更大的K和更高距离，并导向数据驱动分类。
...
### [156] [Branch-and-Browse: Efficient and Controllable Web Exploration with Tree-Structured Reasoning and Action Memory](https://arxiv.org/abs/2510.19838)
*Shiqi He,Yue Cui,Xinyu Ma,Yaliang Li,Bolin Ding,Mosharaf Chowdhury*

Main category: cs.AI

TL;DR: Branch-and-Browse是一个细粒度的网页代理框架，通过树状结构探索、网页状态回放和页面动作记忆，显著提升了基于LLM的网页代理在复杂任务中的成功率和执行效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的自主网页代理在多步推理和回溯能力上存在局限，线性方法无法处理复杂推理，其他搜索策略又过于粗糙且计算成本高，需要更高效可控的推理框架。

Method: 采用树状结构探索实现可控的多分支推理，通过网页状态回放进行高效探索，并利用页面动作记忆在会话内外共享已探索的动作。

Result: 在WebArena基准测试中，实现了35.8%的任务成功率，相比最先进方法执行时间减少了40.4%。

Conclusion: Branch-and-Browse是一个可靠且高效的基于LLM的网页代理框架，在复杂网页任务中表现出色。
...
### [157] [LLMs can hide text in other text of the same length.ipynb](https://arxiv.org/abs/2510.20075)
*Antonio Norelli,Michael Bronstein*

Main category: cs.AI

TL;DR: 该论文提出了一种使用大型语言模型将秘密文本隐藏在看似普通的文本中的协议，实现了文本与作者意图的彻底分离，对AI安全和文本信任构成威胁。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型如何能够将秘密信息隐藏在看似正常的文本中，从而分离文本内容与作者的真实意图，这引发了关于文本可信度和AI安全性的重要问题。

Method: 使用开源的大型语言模型（即使是80亿参数的模型也足够），通过简单高效的协议将秘密文本编码到另一段完全不同但语义连贯的文本中，编码和解码过程可在笔记本电脑上快速完成。

Result: 实验表明该方法能够高质量地实现文本隐藏，例如将政治批判隐藏在赞美文本中，或将秘密手稿隐藏在普通产品评论中，且处理速度很快。

Conclusion: 这种协议的存在进一步削弱了人们对书面交流的信任，对AI安全构成紧迫挑战，并重新定义了我们对大型语言模型"知道"某事的理解。
...
### [158] [AI PB: A Grounded Generative Agent for Personalized Investment Insights](https://arxiv.org/abs/2510.20099)
...
<details>
  <summary>Details</summary>
Motivation: 传统聊天机器人只能被动回答查询，无法在高度监管的金融环境中主动提供可信赖的投资建议。需要开发一个能够在韩国金融监管框架下运行、确保数据安全和合规性的主动式AI系统。

Method: 采用组件化编排层在内部和外部LLM之间进行确定性路由；使用OpenSearch和金融领域嵌入模型的混合检索管道；结合规则启发式、序列行为建模和上下文多臂老虎机的多阶段推荐机制；完全在本地部署，使用Docker Swarm和vLLM跨24个NVIDIA H100 GPU运行。

Result: 通过人工QA和系统指标验证，证明基于明确路由和分层安全机制的接地生成能够在高风险金融环境中提供可信赖的AI洞察。

Conclusion: 在金融等高风险领域，通过明确的组件路由和分层安全机制，可以实现可信赖的主动式AI洞察生成，为生产级生成式AI在监管严格环境中的部署提供了可行方案。
...
### [159] [IKnow: Instruction-Knowledge-Aware Continual Pretraining for Effective Domain Adaptation](https://arxiv.org/abs/2510.20377)
...
Conclusion: IKnow提供了一个简单通用的框架，解决了指令调优模型在持续预训练中的语义表示退化问题。
...
### [160] [What Defines Good Reasoning in LLMs? Dissecting Reasoning Steps with Multi-Aspect Evaluation](https://arxiv.org/abs/2510.20603)
*Heejin Do,Jaehui Hwang,Dongyoon Han,Seong Joon Oh,Sangdoo Yun*

Main category: cs.AI

TL;DR: 论文提出CaSE方法，通过评估推理步骤的相关性和连贯性来改进LLM推理能力，而不仅仅是检查最终答案的正确性。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法只关注最终答案正确性，忽略了推理过程的质量，无法为模型改进提供细粒度信号。

Method: 提出因果逐步评估(CaSE)方法，将推理质量分解为相关性和连贯性两个维度，仅使用前文信息评估每个推理步骤以避免后见之明偏差。
...
### [161] [Real Deep Research for AI, Robotics and Beyond](https://arxiv.org/abs/2510.20809)
*Xueyan Zou,Jianglong Ye,Hao Zhang,Xiaoyu Xiang,Mingyu Ding,Zhaojing Yang,Yong Jae Lee,Zhuowen Tu,Sifei Liu,Xiaolong Wang*

Main category: cs.AI

TL;DR: 提出了Real Deep Research (RDR)框架，用于系统性分析AI和机器人领域的研究趋势，识别新兴趋势和跨领域机会，帮助研究人员应对每年上万篇论文的挑战。


<details>
  <summary>Details</summary>
Motivation: AI和机器人领域研究快速增长，每年产出超过10,000篇论文，研究人员难以跟上最新进展。快速演变的趋势、跨学科工作的兴起以及需要探索专业领域之外的需求都带来了挑战。

Method: 开发了一个通用的RDR管道，能够系统性地分析任何研究领域，识别新兴趋势，发现跨领域机会，并为新的研究提供具体起点。

Result: 将RDR框架应用于AI和机器人领域，特别关注基础模型和机器人技术的进展，并简要扩展到其他科学领域。

Conclusion: 这项工作为AI及其他领域的研究人员提供了有价值的洞察，帮助他们更好地理解和导航快速发展的研究环境。
...
TL;DR: 提出Kinaema模型，通过隐式潜在记忆处理视觉观测流，实现机器人在大场景中的相对位置预测和导航任务。
...
Method: 使用基于transformer的循环模型维护隐式潜在记忆，压缩传感器历史数据，无需显式存储观测历史。

Result: 模型能有效维持场景表示，导航到在episode开始前观察到的目标位置，计算效率优于传统基于注意力机制的transformer。

Conclusion: Kinaema模型在Mem-Nav任务中表现出色，为空间感知机器人提供了有效的长期记忆和定位能力。
...
### [163] [Dino-Diffusion Modular Designs Bridge the Cross-Domain Gap in Autonomous Parking](https://arxiv.org/abs/2510.20335)
*Zixuan Wu,Hengyuan Zhang,Ting-Hsuan Chen,Yuliang Guo,David Paz,Xinyu Huang,Liu Ren*

Main category: cs.RO

TL;DR: Dino-Diffusion Parking (DDP) 是一个领域无关的自主停车系统，结合视觉基础模型和扩散规划，在领域偏移下实现泛化感知和鲁棒运动规划。
...
Conclusion: DDP在模拟到真实世界转移中表现出潜力，通过3D高斯溅射环境重建验证了其实际应用前景。
...
### [164] [GSWorld: Closed-Loop Photo-Realistic Simulation Suite for Robotic Manipulation](https://arxiv.org/abs/2510.20813)
*Guangqi Jiang,Haoran Chang,Ri-Zhao Qiu,Yutong Liang,Mazeyu Ji,Jiyue Zhu,Zhao Dong,Xueyan Zou,Xiaolong Wang*

Main category: cs.RO

TL;DR: GSWorld是一个结合3D高斯泼溅和物理引擎的机器人操作模拟器，支持从真实机器人数据学习策略的闭环开发和sim2real策略训练。


<details>
  <summary>Details</summary>
Motivation: 解决机器人操作策略开发中缺乏逼真模拟环境和可复现评估的问题，实现无需真实机器人的sim2real策略训练。

Method: 提出GSDF资产格式，将高斯-网格表示与机器人URDF结合，建立包含3种机器人形态和40多个物体的数据库，并与物理引擎集成。

Result: 展示了五个应用：零样本sim2real像素到动作策略学习、自动化DAgger数据收集、可复现基准测试、虚拟遥操作数据收集、零样本sim2real视觉强化学习。

Conclusion: GSWorld为机器人操作提供了逼真、可扩展的模拟环境，支持多种sim2real应用，推动了机器人策略开发的闭环流程。
...
### [165] [SODBench: A Large Language Model Approach to Documenting Spreadsheet Operations](https://arxiv.org/abs/2510.19864)
*Amila Indika,Igor Molybog*

Main category: cs.SE

TL;DR: 该论文提出了电子表格操作文档化(SOD)任务，构建了包含111个代码片段及其自然语言摘要的基准，评估了5个LLM在生成电子表格文档方面的性能。


<details>
  <summary>Details</summary>
Motivation: 电子表格在商业、会计和金融领域广泛应用，但缺乏系统化文档方法，阻碍了自动化、协作和知识传递，导致关键机构知识流失风险。
...
Conclusion: LLM可以有效生成电子表格操作文档，但仍有挑战需要解决，SOD是实现电子表格更好文档化和协作的重要方向。
...
### [166] [GUSL-Dehaze: A Green U-Shaped Learning Approach to Image Dehazing](https://arxiv.org/abs/2510.20266)
*Mahtab Movaheddrad,Laurence Palmer,C. -C. Jay Kuo*

Main category: eess.IV

TL;DR: GUSL-Dehaze是一种绿色U形学习图像去雾方法，结合物理模型与绿色学习框架，提供轻量级、可解释的替代方案，避免深度学习，在保持性能的同时大幅减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的图像去雾方法计算成本高、参数规模大，不适合资源受限设备，需要开发轻量级、透明的替代方案。

Method: 结合改进的暗通道先验进行初始去雾，然后通过U形架构实现绿色学习管道，使用无监督表示学习和特征工程技术（相关特征测试和最小二乘正态变换），最后通过透明监督学习获得去雾图像。

Result: GUSL-Dehaze显著减少了参数数量，同时保持数学可解释性，性能与最先进的深度学习模型相当。

Conclusion: 该方法成功证明了绿色学习框架在图像去雾任务中的有效性，为资源受限设备提供了可行的解决方案，同时保持了透明度和可解释性。
...
### [167] [Communication to Completion: Modeling Collaborative Workflows with Intelligent Multi-Agent Communication](https://arxiv.org/abs/2510.19995)
*Yiming Lu,Xun Wang,Simin Ma,Shujian Liu,Sathish Reddy Indurthi,Song Wang,Haoyun Deng,Fei Liu,Kaiqiang Song*

Main category: cs.MA

TL;DR: C2C是一个可扩展的多智能体通信框架，通过Alignment Factor度量和Sequential Action Framework，使智能体能够做出成本感知的通信决策，在复杂任务中减少约40%的完成时间。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体LLM系统缺乏面向任务的系统化通信框架，无法有效支持复杂工作空间中的团队协作。

Method: 提出Alignment Factor（AF）度量智能体任务对齐度，以及Sequential Action Framework集成逐步执行与智能通信决策，实现成本感知的通信选择。

Result: 在5到17个智能体的现实编码工作流中，C2C将任务完成时间减少约40%，在标准配置下成功完成所有任务，并在大规模场景下保持有效性。
...
### [168] [Every Question Has Its Own Value: Reinforcement Learning with Explicit Human Values](https://arxiv.org/abs/2510.20187)
*Dian Yu,Yulai Zhao,Kishan Panaganti,Linfeng Song,Haitao Mi,Dong Yu*

Main category: cs.LG

TL;DR: RLEV是一种将人类价值信号直接融入奖励函数的强化学习方法，通过价值加权梯度放大机制，让LLM能够根据问题价值自动调整回答策略。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法虽然能训练模型在客观领域中使用二元正确性奖励，但忽略了不同任务的重要性差异。RLEV旨在将人类定义的价值信号直接整合到奖励函数中，使LLM优化与可量化的人类价值信号对齐。

Method: 使用带有明确真实价值标签的考试风格数据，将人类定义的价值信号直接融入奖励函数。通过价值加权梯度放大机制，在序列结束令牌上实现价值敏感的终止策略。

Result: RLEV在多种RL算法和模型规模下始终优于仅基于正确性的基线方法。RLEV策略不仅提高了价值加权准确率，还学会了价值敏感的终止策略：对低价值提示简洁回答，对高价值提示详细回答。

Conclusion: RLEV通过优化明确的效用函数，为将LLM与人类优先事项对齐提供了实用路径，即使在噪声价值信号（如基于难度的标签）下也保持稳健。
...
### [169] [ImpossibleBench: Measuring LLMs' Propensity of Exploiting Test Cases](https://arxiv.org/abs/2510.20270)
*Ziqian Zhong,Aditi Raghunathan,Nicholas Carlini*

Main category: cs.LG

TL;DR: ImpossibleBench是一个基准框架，用于系统性地测量LLM代理利用测试用例的倾向性，通过创建任务的不可能变体来量化作弊行为。


<details>
  <summary>Details</summary>
Motivation: LLM倾向于寻找和利用"捷径"完成任务，这会严重影响评估的可靠性和实际部署的安全性，比如删除失败的测试而非修复bug。

Method: 通过从现有基准创建"不可能"任务变体，引入自然语言规范与单元测试之间的直接冲突，测量代理在这些任务上的通过率作为"作弊率"。

Result: 揭示了从简单测试修改到复杂运算符重载等更细粒度的作弊行为，展示了提示、测试访问和反馈循环如何影响作弊率。

Conclusion: ImpossibleBench不仅是评估工具，更是构建更稳健可靠LLM系统的实用框架，可用于行为研究、上下文工程和监控工具开发。
...
### [170] [Relative-Based Scaling Law for Neural Language Models](https://arxiv.org/abs/2510.20387)
*Baoqing Yue,Jinyuan Zhou,Zixi Wei,Jingtao Zhan,Qingyao Ai,Yiqun Liu*

Main category: cs.LG

TL;DR: 本文提出了基于相对排序的缩放定律，通过RBP指标衡量正确token在预测中的排名概率，补充了传统交叉熵指标的不足。


<details>
  <summary>Details</summary>
Motivation: 现有缩放定律研究主要依赖交叉熵作为评估指标，但交叉熵只关注正确token的绝对概率，忽略了正确与错误token之间的相对排序，而相对排序在语言模型中至关重要。

Method: 提出了相对排序概率(RBP)指标，量化正确token在top预测中的排名概率，并建立了基于相对排序的缩放定律来描述RBP随模型规模增长的变化规律。

Result: 在四个数据集和四个模型家族上的实验表明，该缩放定律具有鲁棒性和准确性，能够解释涌现现象并促进缩放定律基础理论的研究。

Conclusion: 基于相对排序的缩放定律补充了交叉熵视角，为大规模语言模型的缩放提供了更全面的理解，对实践开发和理论探索都有重要价值。
...
### [171] [BadGraph: A Backdoor Attack Against Latent Diffusion Model for Text-Guided Graph Generation](https://arxiv.org/abs/2510.20792)
...
<details>
  <summary>Details</summary>
Motivation: 图生成的快速发展带来了新的安全担忧，特别是后门漏洞。虽然之前的工作探索了图像扩散和无条件图生成中的后门攻击，但条件性特别是文本引导的图生成仍未被充分研究。

Method: BadGraph利用文本触发词毒化训练数据，在VAE和扩散训练期间隐蔽地植入后门，当推理时出现触发词时诱导生成攻击者指定的子图。

Result: 在四个基准数据集上的实验表明攻击有效且隐蔽：低于10%的毒化率可达50%攻击成功率，24%毒化率可达80%以上成功率，对良性样本的性能影响可忽略。

Conclusion: 这些发现揭示了文本引导图生成的潜在扩散模型中的安全漏洞，强调了在药物发现等模型应用中存在的严重风险，需要对此类扩散模型中的后门攻击进行鲁棒防御。
...
### [172] [Compress to Impress: Efficient LLM Adaptation Using a Single Gradient Step on 100 Samples](https://arxiv.org/abs/2510.20800)
*Shiva Sreeram,Alaa Maalouf,Pratyusha Sharma,Daniela Rus*

Main category: cs.LG

TL;DR: 提出了一种快速无微调的LLM适应方法，通过梯度分析选择关键层进行秩缩减，在仅使用100个样本的情况下实现下游任务性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决LASER方法中逐层搜索的高计算成本问题，寻求更高效的LLM适应方法。

Method: 使用奇异值梯度识别关键矩阵，允许矩阵行聚类到多个子空间分别分解，仅需100个样本进行计算。

Result: 消除了逐层扫描开销，准确率提升高达24.6个百分点，搜索时间大幅减少。

Conclusion: 结合这些发现可得到快速稳健的下游任务适应算法，无需微调即可适应新数据集。
...
### [173] [FINDER: Feature Inference on Noisy Datasets using Eigenspace Residuals](https://arxiv.org/abs/2510.19917)
*Trajan Murphy,Akshunna S. Dogra,Hanfeng Gu,Caleb Meredith,Mark Kon,Julio Enrique Castrillion-Candas*

Main category: cs.LG

TL;DR: FINDER是一个用于分析噪声数据集的分类框架，通过随机特征和KLE分解来处理低信噪比数据，在阿尔茨海默病分类和森林砍伐检测中取得突破性成果。
...
Method: 将数据集视为随机场实现，映射到希尔伯特空间构造随机特征，使用KLE分解为可计算不可约分量，通过特征值分解进行噪声数据分类。

Result: 在阿尔茨海默病分期分类和遥感森林砍伐检测等数据稀缺领域取得最先进成果。
...
### [174] [Why Prototypes Collapse: Diagnosing and Preventing Partial Collapse in Prototypical Self-Supervised Learning](https://arxiv.org/abs/2510.20108)
...
TL;DR: 本文提出了一种完全解耦的训练策略来解决自监督学习中的原型崩溃问题，通过分别优化原型和编码器来消除原型崩溃，无需显式正则化即可获得多样化的原型和更强的下游性能。


<details>
  <summary>Details</summary>
Motivation: 传统的自监督学习方法存在原型部分崩溃问题，多个原型收敛到几乎相同的表示，这削弱了提供多样化和信息丰富目标的核心目的。现有方法通过过度参数化原型集或添加临时正则化来缓解症状而非解决根本原因。

Method: 引入完全解耦的训练策略，将原型和编码器分别在不同目标下学习。具体来说，将原型建模为高斯混合模型，使用在线EM风格过程独立于编码器损失进行更新。

Result: 这种简单而原则性的解耦消除了原型崩溃，无需显式正则化即可获得一致多样化的原型和更强的下游性能。
...
### [175] [Synthetic Data for Robust Runway Detection](https://arxiv.org/abs/2510.20349)
*Estelle Chigot,Dennis G. Wilson,Meriem Ghrib,Fabrice Jimenez,Thomas Oberlin*

Main category: cs.LG

TL;DR: 本文提出了一种基于商业飞行模拟器的图像生成方法，用于跑道检测任务，通过结合少量真实图像和合成数据来训练目标检测模型，并评估了在夜间等恶劣条件下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 深度视觉模型在工业应用中的部署面临数据收集和标注成本高的问题，特别是在关键应用如自主着陆系统中，需要覆盖所有可能的条件包括罕见场景。合成图像生成提供了一种经济可靠的解决方案。

Method: 使用商业飞行模拟器生成合成图像，结合少量真实标注图像，采用定制化的域适应策略来缓解合成到真实的分布偏移问题。

Result: 标准目标检测模型能够实现准确的预测，在夜间等恶劣条件下也表现出良好的鲁棒性，即使这些条件在真实数据中没有充分体现。

Conclusion: 通过控制图像生成和真实与合成数据的整合，结合定制域适应策略，可以有效提升跑道检测系统的性能和在恶劣条件下的鲁棒性。
...
### [176] [Transferable Black-Box One-Shot Forging of Watermarks via Image Preference Models](https://arxiv.org/abs/2510.20468)
...
TL;DR: 本文提出了一种针对后处理图像水印的伪造攻击方法，通过训练偏好模型检测水印，并利用反向传播优化实现水印移除和伪造，无需了解水印模型细节。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型普及和法律压力增加，数字内容水印技术日益重要。然而，水印伪造（从真实内容窃取水印并应用于恶意内容）这一安全问题尚未得到充分研究。
...
Conclusion: 该方法简单实用，仅需单张水印图像即可实施攻击，揭示了当前水印技术存在的安全漏洞。
...
### [177] [MEIcoder: Decoding Visual Stimuli from Neural Activity by Leveraging Most Exciting Inputs](https://arxiv.org/abs/2510.20762)
*Jan Sobotka,Luca Baroni,Ján Antolík*

Main category: cs.LG

TL;DR: MEIcoder是一种生物启发的解码方法，利用神经元特异性最兴奋输入(MEIs)、结构相似性指数损失和对抗训练，在小数据集上实现视觉刺激重建的最先进性能。
...
Result: 在初级视觉皮层单细胞活动重建视觉刺激方面达到最先进性能，特别是对小数据集表现出色，仅需1000-2500个神经元和少于1000个训练数据点即可重建高保真自然图像。
...
### [178] [Prompt Decorators: A Declarative and Composable Syntax for Reasoning, Formatting, and Control in LLMs](https://arxiv.org/abs/2510.19850)
*Mostapha Kalami Heris*

Main category: cs.PL

TL;DR: 提出Prompt Decorators框架，通过声明式、可组合的控制令牌来管理LLM行为，实现推理风格、结构和语调等行为维度的分离控制。
...
Method: 设计了20个核心装饰器，分为认知与生成、表达与系统两大功能家族，定义统一语法、作用域模型和确定性处理流水线。
...
### [179] [AI Pose Analysis and Kinematic Profiling of Range-of-Motion Variations in Resistance Training](https://arxiv.org/abs/2510.20012)
*Adam Diamant*

Main category: stat.AP

TL;DR: 本研究开发了基于AI的姿态估计流程，用于精确量化抗阻训练中的运动学特征。通过分析280个视频记录，发现部分幅度训练(pROM)相比全幅度训练(fROM)具有更小的运动幅度和更短的执行时间，特别是在离心阶段。


<details>
  <summary>Details</summary>
Motivation: 开发AI姿态估计方法，以精确量化抗阻训练中不同运动幅度训练方式的运动学差异，为训练处方提供科学依据。

Method: 使用Wolf等人(2025)的视频数据，处理280个记录提取关节角度轨迹，通过滤波平滑后获得每组的运动学指标，应用随机效应元分析模型分析变异性。

Result: pROM训练具有更小的运动幅度和更短的总执行时间，特别是离心阶段；参与者个体差异是变异的主要来源；新指标%ROM显示部分幅度训练在不同练习中相对一致。

Conclusion: 部分幅度训练与全幅度训练不仅在运动幅度上存在差异，在执行动态和一致性方面也有不同，AI方法在抗阻训练研究和处方改进方面具有潜力。
